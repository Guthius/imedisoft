...
public partial class ToothChartOpenGL:OpenGLWinFormsControl{//.SimpleOpenGlControl {
	...
	public ToothChartOpenGL(bool hardwareMode,int preferredPixelFormatNum) {
		InitializeComponent();
		TaoSetupContext += new EventHandler(ToothChart_TaoSetupContext);
		TaoRenderScene += new EventHandler(ToothChart_TaoRenderScene);
	...
	private void ToothChart_TaoSetupContext(object sender, EventArgs e){//event from base class when context needs to be setup.
		MakeRasterFont();
	}
	...
	public Bitmap GetBitmap(){
		...
		var dummy=new Bitmap(Width,Height);
		var g=Graphics.FromImage(dummy);
		var e=new PaintEventArgs(g,new Rectangle(0,0,Width,Height));
		Render(e);
		//Bitmap result=ReadFrontBuffer();
		var result=GetBitmapOfOpenGL();
		g.Dispose();
		return result;
	}
	...
	private Bitmap GetBitmapOfOpenGL(){
		...
		var bitmapData=bitmap.LockBits(new Rectangle(0,0,bitmap.Width,bitmap.Height),ImageLockMode.WriteOnly,PixelFormat.Format24bppRgb);
	...
	private void ToothChart_TaoRenderScene(object sender, EventArgs e){
		//if(suspendRendering){
		//	return;
		//}
		...
		//Lighting
		var ambI=.2f;//.1f;//Darker for testing
		var difI=.6f;//.3f;//Darker for testing
		var specI=1f;
		var light_ambient = new float[] { ambI,ambI,ambI,1f };//RGB,A=1 for no transparency. Default 0001
		var light_diffuse = new float[] { difI,difI,difI,1f };//RGBA. Default 1111. 'typical' 
		var light_specular = new float[] { specI,specI,specI,1f };//RGBA. Default 1111
		var light_position = new float[] { -0.5f,0.1f,1f,0f };//xyz(direction, not position), w=0 for infinite
		...
		specular_color_normal = [1.0f,1.0f,1.0f,1.0f];//1111 for white. RGBA
		specular_color_cementum = [0.1f,0.1f,0.1f,1.0f];//gray
		shininess = [90f];//0 to 128. Size of specular reflection. 128 smallest
		...
		var heightProj=(double)TcData.SizeOriginalProjection.Height;
		var widthProj=(double)TcData.SizeOriginalProjection.Width;
		if(TcData.IsWide) {
	...
	private void DrawScene() {
		var wrapper=(ToothChartWrapper)Parent;
		var wrappername=wrapper.Name;
		var isRCT8=wrapper.TcData.ListToothGraphics["8"].IsRct;
		for(var t=0;t<TcData.ListToothGraphics.Count;t++) {//loop through each tooth
	...
	private void DrawFacialView(ToothGraphic toothGraphic) {
		...
		Gl.glTranslatef(ToothChartData.GetTransX(toothGraphic.ToothId),//Move the tooth to the correct position for facial view
			ToothChartData.GetTransYfacial(toothGraphic.ToothId),
			0);
		...
		if(toothGraphic.Visible && toothGraphic.IsRct) {//draw RCT
			...
			Gl.glTranslatef(ToothChartData.GetTransX(toothGraphic.ToothId),ToothChartData.GetTransYfacial(toothGraphic.ToothId),0);
			Gl.glDisable(Gl.GL_LIGHTING);
			Gl.glEnable(Gl.GL_BLEND);
			Gl.glColor3f(
			...
			RotateAndTranslateUser(toothGraphic);
			var lines=toothGraphic.GetRctLines();
			for(var i=0;i<lines.Count;i++){
				Gl.glBegin(Gl.GL_LINE_STRIP);
				for(var j=0;j<lines[i].Vertices.Count;j++){
		...
		if(toothGraphic.Visible && groupBU!=null && groupBU.Visible) {//BU or Post
			...
			Gl.glDisable(Gl.GL_LIGHTING);
			Gl.glDisable(Gl.GL_BLEND);
			var colorBU=toothGraphic.GetGroup(ToothGroupType.Buildup).PaintColor;
			Gl.glColor3f(
		...
		if(toothGraphic.IsImplant){
			...
			var group=(ToothGroup)TcData.ListToothGraphics["implant"].Groups[0];
			var material_color=new float[] {
			...
			for(var i=0;i<group.Faces.Count;i++){//  .GetLength(0);i++) {//loop through each face
				Gl.glBegin(Gl.GL_POLYGON);
				for(var j=0;j<group.Faces[i].IndexList.Count;j++){//.Length;j++) {//loop through each vertex
	...
	private void DrawOcclusalView(ToothGraphic toothGraphic) {
		...
		Gl.glPushMatrix();//remember position of origin
		Gl.glTranslatef(ToothChartData.GetTransX(toothGraphic.ToothId),ToothChartData.GetTransYocclusal(toothGraphic.ToothId),0);
		if(ToothGraphic.IsMaxillary(toothGraphic.ToothId)) {
		...
		if(toothGraphic.Visible && toothGraphic.IsSealant){//draw sealant
			...
			RotateAndTranslateUser(toothGraphic);
			var line=toothGraphic.GetSealantLine();
			Gl.glBegin(Gl.GL_LINE_STRIP);
			for(var j=0;j<line.Vertices.Count;j++) {//loop through each vertex
	...
	private void DrawWatches(){
		...
		for(var t=0;t<TcData.ListToothGraphics.Count;t++) {//loop through each adult tooth
		...
		for(var t=0;t<TcData.ListToothGraphics.Count;t++) {//loop through each primary tooth
			var toothGraphic=TcData.ListToothGraphics[t];
			//If a tooth is marked to be watched then it is always visible, even if the tooth is missing/hidden.
	...
	private void RenderToothWatch(ToothGraphic toothGraphic){
		...
		if(ToothGraphic.IsRight(toothGraphic.ToothId)) {
			Gl.glTranslatef(ToothChartData.GetTransX(toothGraphic.ToothId)+toothGraphic.ShiftM,0,0);
		} 
		else {
			Gl.glTranslatef(ToothChartData.GetTransX(toothGraphic.ToothId)-toothGraphic.ShiftM,0,0);
		}
		var toMm=1f/TcData.ScaleMmToPix;
		var line=toothGraphic.GetWatchLine();
		Gl.glLineWidth(3f*TcData.PixelScaleRatio);
		Gl.glColor3f(1.0f,1.0f,1.0f);//White
		Gl.glBegin(Gl.GL_LINE_STRIP);
		for(var j=0;j<line.Vertices.Count;j++) {//loop through each vertex and render the white W lines.
			if(ToothGraphic.IsMaxillary(toothGraphic.ToothId)){
		...
		Gl.glBegin(Gl.GL_LINE_STRIP);
		for(var j=0;j<line.Vertices.Count;j++) {//loop through each vertex and render the colored W lines.
			if(ToothGraphic.IsMaxillary(toothGraphic.ToothId)){
		...
		Gl.glColor3f(1.0f,1.0f,1.0f);//White
		Gl.glBegin(Gl.GL_POINTS);
		for(var j=0;j<line.Vertices.Count;j++) {//loop through each vertex and render the colored W points.
			if(ToothGraphic.IsMaxillary(toothGraphic.ToothId)){
				Gl.glVertex3f(line.Vertices[j].X+TcData.PixelScaleRatio*(-6f*toMm),line.Vertices[j].Y+TcData.PixelScaleRatio*(150f*toMm),line.Vertices[j].Z+7f);
			}
			else{
	...
	private void DrawDrawingSegments(){
		...
		PointF pointMm;
		Color color;
		var scaleDrawing=(float)Width/(float)ToothChartData.SizeOriginalDrawing.Width;
	...
	private void DrawNumber(string tooth_id, bool isSelected, bool isFullRedraw) {
		...
		var labelSizeF=MeasureStringMm(displayNum);// g.MeasureString(displayNum,Font).Width/TcData.ScaleMmToPix;
		//SizeF labelSizeF
		var recMm=TcData.GetNumberRecMm(tooth_id,labelSizeF);
		//Rectangle recPix=TcData.ConvertRecToPix(recMm);
		if(isSelected){
			Gl.glColor3f(
...
//todo: get rid of this?
	...
	private void MakeRasterFont() {
		...
		fontsymbols['+']= ["00000", "00000", "00100", "00100", "11111", "00100", "00100", "00000", "00000"];
		fontsymbols['-']= ["00000", "00000", "00000", "00000", "11111", "00000", "00000", "00000", "00000"];
		fontsymbols['A']= ["0001000", "0001000", "0010100", "0010100", "0100010", "0100010", "0111110", "1000001", "1000001"];//A
		fontsymbols['B']= ["11110","10001","10001","10001","11110","10001","10001","10001","11110"];//B
		fontsymbols['C']= ["011110","100001","100000","100000","100000","100000","100000","100001","011110"];//C
		fontsymbols['D']= ["111100","100010","100001","100001","100001","100001","100001","100010","111100"];//D
		fontsymbols['E']= ["11111","10000","10000","10000","11110","10000","10000","10000","11111"];//E
		fontsymbols['F']= ["11111","10000","10000","10000","11110","10000","10000","10000","10000"];//F
		fontsymbols['G']= ["011110","100001","100000","100000","100111","100001","100001","100011","011101"];//G
		fontsymbols['H']= ["100001","100001","100001","100001","111111","100001","100001","100001","100001"];//H
		fontsymbols['I']= ["111","010","010","010","010","010","010","010","111"];//I
		fontsymbols['J']= ["00001","00001","00001","00001","00001","00001","10001","10001","01110"];//J
		fontsymbols['K']= ["100001","100010","100100","101000","110000","101000","100100","100010","100001"];//K
		fontsymbols['L']= ["10000","10000","10000","10000","10000","10000","10000","10000","11111"];//L
		fontsymbols['M']= ["1000001","1000001","1100011","1100011","1010101","1010101","1001001","1001001","1000001"];//M
		fontsymbols['N']= ["100001","110001","110001","101001","101001","100101","100011","100011","100001"];//N
		fontsymbols['O']= ["011110","100001","100001","100001","100001","100001","100001","100001","011110"];//O
		fontsymbols['P']= ["111110","100001","100001","100001","111110","100000","100000","100000","100000"];//P
		fontsymbols['Q']= ["011110","100001","100001","100001","100001","100001","100101","100010","011101"];//Q
		fontsymbols['R']= ["111110","100001","100001","100001","111110","100100","100010","100010","100001"];//R
		fontsymbols['S']= ["011110","100001","100000","100000","011110","000001","000001","100001","011110"];//S
		fontsymbols['T']= ["1111111","0001000","0001000","0001000","0001000","0001000","0001000","0001000","0001000"];//T
		fontsymbols['0']= ["01110", "10001", "10001", "10001", "10001", "10001", "10001", "10001", "01110"];//0
		fontsymbols['1']= ["0010", "1110", "0010", "0010", "0010", "0010", "0010", "0010", "0010"];//1
		fontsymbols['2']= ["01110", "10001", "00001", "00001", "00010", "00100", "01000", "10000", "11111"];//2
		fontsymbols['3']= ["01110", "10001", "00001", "00001", "00110", "00001", "00001", "10001", "01110"];//3
		fontsymbols['4']= ["00010", "00110", "00110", "01010", "01010", "10010", "11111", "00010", "00010"];//4
		fontsymbols['5']= ["11111", "10000", "10000", "11110", "10001", "00001", "00001", "10001", "01110"];//5
		fontsymbols['6']= ["01110", "10001", "10000", "10000", "11110", "10001", "10001", "10001", "01110"];//6
		fontsymbols['7']= ["11111", "00001", "00010", "00010", "00100", "00100", "01000", "01000", "01000"];//7
		fontsymbols['8']= ["01110", "10001", "10001", "10001", "01110", "10001", "10001", "10001", "01110"];//8
		fontsymbols['9']= ["01110", "10001", "10001", "10001", "01111", "00001", "00001", "10001", "01110"];//9
		...
		for(var i=0;i<fontsymbols.Length;i++) {
			var arrayBitsForSymbol=fontsymbols[i];
			...
			var letterW=arrayBitsForSymbol[0].Length;//All rows of pixels for this symbol will be the same number of pixels wide.
			var letterH=arrayBitsForSymbol.Length;//The number of pixels the symbol is tall is equal to the number of pixel rows.
			var arrayBytesForLetter=new byte[letterH];
			for(var h=0;h<letterH;h++) {//Draws the letter from the bottom up.
				arrayBytesForLetter[h]=0;
				var pixelRow=arrayBitsForSymbol[letterH-h-1];//from bottom and up
				for(var w=0;w<letterW;w++){
	...
	public void MakeDisplayLists(){
		...
		for(var t=1;t<=52;t++) {
			...
			for(var g=0;g<15;g++){//groups 0-14
				...
				if(group!=null){
					for(var f=0;f<group.Faces.Count;f++){//.GetLength(0);f++) {//loop through each face
						Gl.glBegin(Gl.GL_POLYGON);
						for(var j=0;j<group.Faces[f].IndexList.Count;j++) {//.Length;j++) {//loop through each vertex
							//The index for both will always be the same because we enforce a 1:1 relationship.
							//We show grabbing a float[3], but we could just as easily use the index itself.
	...
	protected override void OnMouseMove(MouseEventArgs e) {
		...
		if(TcData.CursorTool==CursorTool.Pointer) {
			...
			Gl.glBegin(Gl.GL_LINE_STRIP);
			//if we set 0,0 to center, then this is where we would convert it back.
			var pointMm=TcData.PointPixToMm(TcData.PointList[i-1]);
			Gl.glVertex3f(pointMm.X,pointMm.Y,0);
	...
	protected void OnSegmentDrawn(string drawingSegment){
		var tArgs=new ToothChartDrawEventArgs(drawingSegment);
