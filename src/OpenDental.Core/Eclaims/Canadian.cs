	...
	public class Canadian {
			...
			PrintCdaClaimFormDelegate printCdaClaimFormDelegate) 
		{
			...
			for(int p=0;p<claimProcsClaim.Count;p++) {
				...
				if(carrier.ElectID=="000125") {//GroupHealth Benefit Solutions
				...
				else {//Use identification system within the ODA Suggested Fee Guide for General Practitioners
			...
			List<InsPlan> listInsPlans,List<Benefit> listBenefits,List<InsSub> listInsSubs,Patient patient,bool isAdditive=false)
		{
				...
				foreach(CCDField field in dictProcDataClaim[claimProcCur.LineNumber]) {
					...
					switch(field.fieldId) {
						...
						case "G17"://Explanation Note Number 2
							int noteNumber=SIn.Int(field.valuestr);
			...
			if(SIn.Int(fieldInputter.GetFieldById("G10").valuestr,false)!=0) {//Number of Carrier Issued Procedure Codes
		...
		public static string PassToIca(string msgText,Clearinghouse clearinghouseClin,CanadianNetwork network,bool isAutomatic,out string errorMsg) {
			...
			int fileNum=SIn.Int(officeSequenceNumber)%1000;
		...
		private static string GetToothCodeForToothSextantQuadOrArch(Procedure proc,ProcedureCode procCode) {
			...
			switch(procCode.TreatArea) {
				...
				case TreatmentArea.Sextant:
					...
					if(proc.Surf=="1") {//Sextant 1: upper right premolar/molar region (18-14)
						return "18";//Upper right molar.
					}
					else if(proc.Surf=="2") {//Sextant 2: upper jaw incisor/canine region (13-23)
						return "13";//Upper right canine.
					}
					else if(proc.Surf=="3") {//Sextant 3: upper left premolar/molar region (24-28)
						return "24";//Upper left premolar.
					}
					else if(proc.Surf=="4") {//Sextant 4: lower left premolar/molar region (38-34)
						return "38";//Lower left molar.
					}
					else if(proc.Surf=="5") {//Sextant 5: lower jaw incisor/canine region (33-43)
						return "33";//Lower left canine.
					}
					else if(proc.Surf=="6") {//Sextant 6: lower right premolar/molar region (44-48)
				...
				foreach(CCDField field in listFields) {
					switch(field.fieldId) {
						...
						case "G44"://Eligible Amount for additional Lab procedure
							double eligibleAmount=SIn.Double(RawMoneyStrToDisplayMoney(field.valuestr));
