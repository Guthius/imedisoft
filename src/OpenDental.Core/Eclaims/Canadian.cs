...
namespace OpenDentBusiness.Eclaims
{
    public class Canadian
    {
        ...
        public static void ValidateExportPath(Clearinghouse clearinghouseClin)
        {
            //Warning: this path is not handled properly if trailing slash is missing:
            var saveFolder = clearinghouseClin.ExportPath;
        ...
        {
            ValidateExportPath(clearinghouseClin);
            Etrans etrans;
            Claim claim;
            ProviderDto billProv;
            ProviderDto treatProv;
            InsPlan insPlan;
            InsSub insSub;
            Carrier carrier;
            ...
            StringBuilder strb;
            var primaryEOBResponse = "";
            var primaryClaimRequestMessage = "";
            claimProcList = ClaimProcs.Refresh(queueItem.PatNum);
            var listClaims = Claims.GetClaimsFromClaimNums(claimProcList.Select(x => x.ClaimNum)
            ...
            var planNum = claim.PlanNum;
            var insSubNum = claim.InsSubNum;
            var patRelat = claim.PatRelat;
            var planNum2 = claim.PlanNum2;
            var insSubNum2 = claim.InsSubNum2;
            var patRelat2 = claim.PatRelat2;
            ...
            else if (claim.ClaimType == "S")
            {
                ...
                for (var i = 0; i < claimProcsClaim.Count; i++)
                {
                    var claimProcsForProc = ClaimProcs.GetForProc(claimProcList, claimProcsClaim[i].ProcNum);
                    var matchingPrimaryProc = false;
                    for (var j = 0; j < claimProcsForProc.Count; j++)
                    {
                        ...
                        {
                            var claimPri = listClaims.FirstOrDefault(x => x.ClaimNum == claimProcsForProc[j].ClaimNum);
                            if (claimPri != null && claimPri.ClaimType != "P")
                            {
                                //Make sure this claimproc isn't for a PreAuth or tertiary claim
                                continue;
                            }

                            claimNumPrimary = claimProcsForProc[j].ClaimNum;
                            matchingPrimaryProc = true;
                            claimProcsClaim[i].FeeBilled = claimProcsForProc[j].FeeBilled; // make sure secondary fee billed matches primary fee billed. 
                            break;
                        }
                    }
                ...
                var etransPrimary = Etranss.GetHistoryOneClaim(claimNumPrimary);
                for (var i = 0; i < etransPrimary.Count; i++)
                {
                    primaryClaimRequestMessage = EtransMessageTexts.GetMessageText(etransPrimary[i].EtransMessageTextNum);
                    var etransPrimaryAck = Etranss.GetEtrans(etransPrimary[i].AckEtransNum);
                ...
                {
                    //primaryEOBResponse.Length>=22
                    var messageVersion = primaryEOBResponse.Substring(18, 2); //Field A03 always exists on all messages and is always in the same location.
                    var messageType = primaryEOBResponse.Substring(20, 2); //Field A04 always exists on all messages and is always in the same location.
                ...
                var claimPrimary = Claims.GetClaim(claimNumPrimary);
            ...
            var carrierReceiver = carrier; //This is the value used for primary eclaims and preauths.
            ...
            var network = CanadianNetworks.GetNetwork(carrierReceiver.CanadianNetworkNum, clearinghouseClin, claim);
            ...
            if (carrierReceiver.CDAnetVersion != "02")
            {
                ...
                var patID = "";
                for (var p = 0; p < patPlansForPatient.Count; p++)
                {
            ...
            if (carrierReceiver.CDAnetVersion == "02")
            {
                ...
                var exceptionCode = TidyN(patient.CanadianEligibilityCode, 1); //Validated.
                if (exceptionCode == "4")
                {
            ...
            var C19PlanRecordPresent = (insPlan.CanadianPlanFlag == "A" || insPlan.CanadianPlanFlag == "N" || insPlan.CanadianPlanFlag == "V");
            if (carrierReceiver.CDAnetVersion != "02")
            {
                //version 04
                //C12 plan flag 1 A
            ...
            if (planNum2 != 0)
            {
                ...
                if (carrierReceiver.CDAnetVersion != "02")
                {
                    ...
                    strb.Append(TidyN(claim.ClaimType == "S" ? SIn.Int(primaryClaimData.GetFieldById("E07").Valuestr) : insPlan2.DentaideCardSequence, 2));
                }
            ...
            if (carrierReceiver.CDAnetVersion == "02")
            {
                ...
                var initialPlacement = "Y";
                var initialPlacementDate = DateTime.MinValue;
            ...
            if (carrierReceiver.CDAnetVersion != "02")
            {
                ...
                for (var t = 0; t < extracted.Count; t++)
                {
                    //F23 extracted tooth num 2 N
                    strb.Append(TidyN(Tooth.Display(extracted[t].ToothNum), 2)); //validated
            ...
            if (carrierReceiver.CDAnetVersion != "02")
            {
                ...
                if (claim.ClaimType == "PreAuth")
                {
                    if ( /* ODBuild.IsDebug() */ false)
                    {
                        ...
                        var currentPredeterminationPageNumber = 1;
                        var lastPredeterminationPageNumber = 1;
                        if (claim.PreAuthString != "")
                        {
                            var predetermNums = claim.PreAuthString.Split(new char[] {','});
                            currentPredeterminationPageNumber = SIn.Int(predetermNums[0]);
                            lastPredeterminationPageNumber = SIn.Int(predetermNums[1]);
                        }
                    ...
                    if (orthoRecordFlag)
                    {
                        ...
                        if ( /* ODBuild.IsDebug() */ false)
                        {
                            ...
                            if (claim.PreAuthString != "")
                            {
                                var preauthData = claim.PreAuthString.Split(new char[] {','});
                                if (preauthData.Length > 2)
                                {
                                    firstExamFee = SIn.Double(preauthData[2]);
                                }

                                if (preauthData.Length > 3)
                                {
                                    diagnosticPhaseFee = SIn.Double(preauthData[3]);
                                }
                            }
                        }
            ...
            for (var p = 0; p < claimProcsClaim.Count; p++)
            {
                ...
                //Some carriers reject claims if F10 is not a valid tooth code (regardless if submitting a non-tooth procedure).
                if (carrier.ElectID == "000125")
                {
                    //GroupHealth Benefit Solutions
                    //Always return a tooth code even if it is an arbitrary tooth code that falls within a corresponding sextant, quad, etc.
                    strb.Append(GetToothCodeForToothSextantQuadOrArch(proc, procCode));
                }
                else
                {
                    //Use identification system within the ODA Suggested Fee Guide for General Practitioners
            ...
            var errorMsg = "";
            var result = PassToIca(strb.ToString(), clearinghouseClin, network, isAutomatic, out errorMsg);
            //Attach an ack to the etrans
            var etransAck = new Etrans();
            var embeddedMsg = ""; //Will be set later on and checked to see if a second claim needs to be created
            ...
            else
            {
                ...
                var fieldG05 = fieldInputter.GetFieldById("G05");
                if (fieldG05 != null)
                {
                    etransAck.AckCode = fieldG05.Valuestr;
                    if (etransAck.AckCode == "M")
                    {
            ...
            var assignBenInsSub = Claims.GetAssignmentOfBenefits(claim, insSub);
            CCDFieldInputter fieldInputter2 = null;
            var canCreateSecClaim = (claim.ClaimType != "PreAuth" && claim.ClaimType != "S" && etransAck.Etype == EtransType.ClaimEOB_CA && planNum2 > 0);
            var hasImportedPrimaryEmbeddedEOB = false;
            if (fieldInputter != null && fieldInputter.GetValue("G39") != "" && fieldInputter.GetValue("G39") != "0000")
            {
                //There exists an embedded message within the response.
                embeddedMsg = fieldInputter.GetValue("G40");
                fieldInputter2 = new CCDFieldInputter(embeddedMsg);
                if (canCreateSecClaim && fieldInputter2.GetFieldById("A05").Valuestr == carrier2.ElectID)
                {
                    //Is the embedded message for the secondary carrier?
                    //Remove the embedded response from primary eTrans. Embedded message will be attached to secondary claim below.
                    var sbPrimaryOnly = new StringBuilder();
                    foreach (var field in fieldInputter.GetLoadedFields())
                    {
            ...
            else if (assignBenInsSub && clearinghouseClin.IsEraDownloadAllowed != EraBehaviors.None && (fieldInputter.MsgType.In("21", "23")))
            {
                ...
                if (fam == null)
                {
                    //Could have been loaded already above.  If fam is null, then so is subList, planList, and benefitList.
                    fam = Patients.GetFamily(claim.PatNum);
                    subList = InsSubs.RefreshForFam(fam);
                    planList = InsPlans.RefreshForSubList(subList);
                    benefitList = Benefits.Refresh(patPlansForPatient, subList);
            ...
            var fieldTransRefNum = fieldInputter.GetFieldById("G01");
            ...
            {
                //The below logic mimics EOBImportHelper().
                var dictProcDataClaim = fieldInputter.GetPerformedProcsDict();
                if (dictProcDataClaim.Count > 0)
                {
                    var isFullyEligible = true;
                    for (var i = 0; i < claimProcsClaim.Count; i++)
                    {
                        ...
                        foreach (var field in dictProcDataClaim[claimProcsClaim[i].LineNumber])
                        {
                            if (field.FieldId == "G14" && field.Valuestr != "100")
                            {
                                //G14 is elegible percent for procedure
            ...
            if (canCreateSecClaim)
            {
                ...
                var listSelectedProcs = new List<Procedure>();
                for (var i = 0; i < claimProcsClaim.Count; i++)
                {
                    //loop through the procs from claim1
                ...
                var patNote = PatientNotes.Refresh(patient.PatNum, patient.Guarantor);
                try
                {
                    claim2 = AccountModules.CreateClaim(claim2, "S", patPlansForPatient, planList, claimProcList, procListAll,
                ...
                if (claim2.ClaimNum > 0)
                {
                    //ClaimNum will be set if the secondary claim was created successfully.
                    ...
                    if (embeddedMsg.IsNullOrEmpty())
                    {
                        //No embedded secondary response
                        var responseMessageVersion = result.Substring(18, 2); //Field A03 always exists on all messages and is always in the same location.
                    ...
                    {
                        //Embedded secondary response
                        var etrans2 = CreateEtransForSendClaim(queueItem2.ClaimNum, queueItem2.PatNum, clearinghouseClin.HqClearinghouseNum, EtransType.ClaimCOB_CA); //Can throw exception
                        var etransAck2 = new Etrans();
                        ...
                        var embeddedFieldG05 = fieldInputter2.GetFieldById("G05");
                        ...
                        var assignBenInsSub2 = Claims.GetAssignmentOfBenefits(claim, insSub2);
                        if (assignBenInsSub2 && clearinghouseClin.IsEraDownloadAllowed != EraBehaviors.None && fieldInputter2.MsgType.In("21", "23"))
                        {
                            ...
                            var claimProcsClaim2 = ClaimProcs.GetForSendClaim(claimProcList, claim2.ClaimNum);
                            ...
                            SecurityLogs.MakeLogEntry(EnumPermType.InsPayCreate, claim2.PatNum
                                , "Claim for service date " + SOut.Date(claim2.DateService) + " amounts overwritten using received EOB amounts."
                                , LogSources.CanadaEobAutoImport);
                        }
        ...
        {
            var etrans = Etranss.CreateEtransForClaim(claimNum, patNum, clearinghouseNum, etype, batchNumber: 0, Security.CurUser.UserNum);
            try
            {
        ...
        {
            var fam = Patients.GetFamily(claim.PatNum);
            var listInsSubs = InsSubs.RefreshForFam(fam);
            var listInsPlans = InsPlans.RefreshForSubList(listInsSubs);
            var listBenefits = Benefits.Refresh(PatPlans.Refresh(claim.PatNum), listInsSubs);
        ...
        {
            var hasExistingPayments = (listClaimProcsForClaim.Exists(x => x.Status.In(ClaimProcStatus.Received, ClaimProcStatus.Supplemental, ClaimProcStatus.CapComplete) || x.ClaimPaymentNum != 0));
            ...
            if (!isAdditive)
            {
                for (var i = listClaimProcsForClaim.Count - 1; i >= 0; i--)
                {
                    var cp = listClaimProcsForClaim[i];
                    if (cp.ProcNum == 0)
                    {
            ...
            var isPreEob = (fieldInputter.MsgType == "23");
            ...
            var listPayPlans = PayPlans.GetValidInsPayPlans(listClaimProcsForClaim[0].PatNum, listClaimProcsForClaim[0].PlanNum, listClaimProcsForClaim[0].InsSubNum, listClaimProcsForClaim[0].ClaimNum);
            var dictProcDataClaim = fieldInputter.GetPerformedProcsDict();
            var listNoteFields = fieldInputter.GetExplantionNotesDict(); //Note number and note text fields
            ...
            var listExistingLineNums = listClaimProcsForClaim.Select(x => (int) x.LineNumber).ToList();
            //List of keys that are associated to EOB procs that can not be found in listClaimProcsForClaim
            var listMissingEOBProcs = dictProcDataClaim.Keys.ToList().FindAll(x => !listExistingLineNums.Contains(x));
            foreach (var lineNumber in listMissingEOBProcs)
            {
                //VERY RARE
                ...
                var cpByTotal = ByTotClaimProcHelper(claim, listPayPlans.Count == 1 ? listPayPlans[0].PayPlanNum : 0, isPreEob, eobBehavior);
            ...
            var listSavedLabClaimProcs = new List<ClaimProc>();
            foreach (var claimProcCur in listClaimProcsForClaim)
            {
                ...
                var listLabProcNums = listPatProcs.FindAll(x => claimProcCur.ProcNum != 0 && x.ProcNumLab == claimProcCur.ProcNum).Select(x => x.ProcNum).ToList();
                ...
                var listLabClaimProcs = listClaimProcs.FindAll(x => listLabProcNums.Contains(x.ProcNum) //Only consider claim procs associated to labs
                                                                    && x.InsSubNum == claimProcCur.InsSubNum
                                                                    && x.PlanNum == claimProcCur.PlanNum
                                                                    && x.ClaimNum == claimProcCur.ClaimNum);
                var claimProcLabOne = (listLabClaimProcs.Count >= 1) ? listLabClaimProcs[0] : null;
                var claimProcLabTwo = (listLabClaimProcs.Count == 2) ? listLabClaimProcs[1] : null;
                ...
                else if (eobBehavior == EraBehaviors.DownloadAndReceive)
                {
                    //Preauth claimproc retain the "Preauth" status.
                ...
                foreach (var field in dictProcDataClaim[claimProcCur.LineNumber])
                {
                    ...
                    switch (field.FieldId)
                    {
                        ...
                        case "G13": //Deductible Amount
                            if (isPreEob)
                            {
                                SetAmt(ref claimProcCur.DedEst, SIn.Double(RawMoneyStrToDisplayMoney(field.Valuestr)), isAdditive);
                            }
                            else
                            {
                                SetAmt(ref claimProcCur.DedApplied, SIn.Double(RawMoneyStrToDisplayMoney(field.Valuestr)), isAdditive);
                            }

                            break;
                        case "G14": //Eligible Percentage
                            SetAmt(ref claimProcCur.Percentage, SIn.Int(RawPercentToDisplayPercent(field.Valuestr)), isAdditive);
                            break;
                        case "G15": //Benefit Amount for the Procedure
                            if (isPreEob)
                            {
                                SetAmt(ref claimProcCur.InsEstTotal, SIn.Double(RawMoneyStrToDisplayMoney(field.Valuestr)), isAdditive);
                            }
                            else
                            {
                                SetAmt(ref claimProcCur.InsEstTotalOverride, SIn.Double(RawMoneyStrToDisplayMoney(field.Valuestr)), isAdditive); //Set this so if it's not marked received, pat port is still reflected correctly
                                if (eobBehavior == EraBehaviors.DownloadAndReceive)
                                {
                                    SetAmt(ref claimProcCur.InsPayAmt, SIn.Double(RawMoneyStrToDisplayMoney(field.Valuestr)), isAdditive);
                                }
                            }
                        ...
                        case "G56": //Deductible Amount for Lab Proc #1
                            if (claimProcLabOne != null)
                            {
                                if (isPreEob)
                                {
                                    SetAmt(ref claimProcLabOne.DedEst, SIn.Double(RawMoneyStrToDisplayMoney(field.Valuestr)), isAdditive);
                                }
                                else
                                {
                                    SetAmt(ref claimProcLabOne.DedApplied, SIn.Double(RawMoneyStrToDisplayMoney(field.Valuestr)), isAdditive);
                                }
                        ...
                        case "G57": //Eligible Percentage for Lab Proc #1
                            if (claimProcLabOne != null)
                            {
                                SetAmt(ref claimProcLabOne.Percentage, SIn.Int(RawPercentToDisplayPercent(field.Valuestr)), isAdditive);
                                continue;
                            }
                        ...
                        case "G58": //Benefit Amount for Lab Proc #1
                            if (claimProcLabOne != null)
                            {
                                if (isPreEob)
                                {
                                    SetAmt(ref claimProcLabOne.InsEstTotal, SIn.Double(RawMoneyStrToDisplayMoney(field.Valuestr)), isAdditive);
                                ...
                                {
                                    SetAmt(ref claimProcLabOne.InsEstTotalOverride, SIn.Double(RawMoneyStrToDisplayMoney(field.Valuestr)), isAdditive); //Set this so if it's not marked received, pat port is still reflected correctly
                                    if (eobBehavior == EraBehaviors.DownloadAndReceive)
                                    {
                                        SetAmt(ref claimProcLabOne.InsPayAmt, SIn.Double(RawMoneyStrToDisplayMoney(field.Valuestr)), isAdditive);
                        ...
                        case "G59": //Deductible Amount for Lab Proc #2
                            if (claimProcLabTwo != null)
                            {
                                if (isPreEob)
                                {
                                    SetAmt(ref claimProcLabTwo.DedEst, SIn.Double(RawMoneyStrToDisplayMoney(field.Valuestr)), isAdditive);
                                }
                                else
                                {
                                    SetAmt(ref claimProcLabTwo.DedApplied, SIn.Double(RawMoneyStrToDisplayMoney(field.Valuestr)), isAdditive);
                                }
                        ...
                        case "G60": //Eligible Percentage for Lab Proc #2
                            if (claimProcLabTwo != null)
                            {
                                SetAmt(ref claimProcLabTwo.Percentage, SIn.Int(RawPercentToDisplayPercent(field.Valuestr)), isAdditive);
                                continue;
                            }
                        ...
                        case "G61": //Benefit Amount for Lab Proc #2
                            if (claimProcLabTwo != null)
                            {
                                if (isPreEob)
                                {
                                    SetAmt(ref claimProcLabTwo.InsEstTotal, SIn.Double(RawMoneyStrToDisplayMoney(field.Valuestr)), isAdditive);
                                    //The following 2 commented out lines should not be necessary as they were taken care of before calling this function.
                                    //ClaimProcs.CanadianLabBaseEstHelper(...) ensures that lab procs and parent procs have the same status.
                                    //claimProcLabOne.Status=ClaimProcStatus.Preauth;
                                }
                                ...
                                {
                                    SetAmt(ref claimProcLabTwo.InsEstTotalOverride, SIn.Double(RawMoneyStrToDisplayMoney(field.Valuestr)), isAdditive); //Set this so if it's not marked received, pat port is still reflected correctly
                                    if (eobBehavior == EraBehaviors.DownloadAndReceive)
                                    {
                                        SetAmt(ref claimProcLabTwo.InsPayAmt, SIn.Double(RawMoneyStrToDisplayMoney(field.Valuestr)), isAdditive);
                        ...
                        case "G17": //Explanation Note Number 2
                            var noteNumber = SIn.Int(field.Valuestr);
                ...
                if (claimProcCur.IsNew)
                {
                    //When we identify a missing proc from the EOB we must insert a new claimProc.
            ...
            if (SIn.Int(fieldInputter.GetFieldById("G10").Valuestr, false) != 0)
            {
                //Number of Carrier Issued Procedure Codes
                var listCarrierIssuedProcs = new List<CarrierIssuedProcedure>();
                foreach (var listFieldsCur in fieldInputter.GetCarrierIssuedProcs())
                {
                ...
                for (var i = 0; i < listCarrierIssuedProcs.Count; i++)
                {
                    ...
                    {
                        var claimProcReferenced = listClaimProcsForClaim.FirstOrDefault(x => x.LineNumber == listCarrierIssuedProcs[i].ArrayLineNumbers[0]);
                        //Only manipulate the claim procedure that correlates to the line number specified.
                        if (claimProcReferenced != null)
                        {
                            var claimProcReferencedOld = claimProcReferenced.Copy();
                            if (isPreEob)
                            {
            ...
            if (isPreEob)
            {
                ...
                var procListAll = ClaimProcs.Refresh(claim.PatNum);
            ...
            var patPlansForPatient = PatPlans.Refresh(claim.PatNum);
        ...
        private static string ConvertEOBVersion(string primaryEOB, string versionTo)
        {
            try
            {
                var primaryEOBfields = new CCDFieldInputter(primaryEOB);
                var fieldA03 = primaryEOBfields.GetFieldById("A03");
                if (fieldA03 != null && fieldA03.Valuestr != versionTo)
                {
                    var strb = new StringBuilder(primaryEOB); //todo: perform format conversion here.
        ...
        public static string PassToIca(string msgText, Clearinghouse clearinghouseClin, CanadianNetwork network, bool isAutomatic, out string errorMsg)
        {
            ...
            {
                errorMsg = clearinghouseClin.ExportPath + " " + Lans.g("Canadian", "not found. Go To Setup > Family/Insurance > Clearinghouses > and double click desired clearinghouse to update path.");
                return ""; //Return empty response, since we never received one.
            }
            ...
            var isItrans = clearinghouseClin.CommBridge.In(EclaimsCommBridge.ITRANS, EclaimsCommBridge.ITRANS2);
            var isClaimstream = (clearinghouseClin.CommBridge == EclaimsCommBridge.Claimstream);
            var isCertificateNeeded = (network != null && isClaimstream);
            var saveFolder = clearinghouseClin.ExportPath;
            if (isCertificateNeeded)
            {
                //Download vendor identity certificate to appropriate subfolder if needed.
                ...
                var configFilePath = ODFileUtils.CombinePaths(saveFolder, "ccdws.ini");
                var subDir = "";
                ...
                var certFilePath = ODFileUtils.CombinePaths(saveFolder, certFileName);
                if (!string.IsNullOrEmpty(certFileName) && subDir != "" && !File.Exists(certFilePath))
                {
                    //Download a copy of the certificate from HQ if needed.
                    ...
                    try
                    {
                        ...
                        using (var writer = XmlWriter.Create(strbuild, settings))
                        {
                        ...
                        var node = doc.SelectSingleNode("//Error");
                        if (node != null)
                        {
                ...
                if (!string.IsNullOrEmpty(certFileName) && File.Exists(configFilePath))
                {
                    ...
                    var configTextModified = configText.Replace("OD_2018-02-26_2023-03-02_staging.pem", "OD_2023-02-05_2028-02-09_staging.pem");
            ...
            var officeSequenceNumber = msgText.Substring(12, 6); //Field A02. Office Sequence Number is always part of every message type and is always in the same place.
            var fileNum = SIn.Int(officeSequenceNumber) % 1000;
            //first, delete the result file from previous communication so that no such files can affect the loop logic below.
            var outputFile = ODFileUtils.CombinePaths(saveFolder, "output." + fileNum.ToString().PadLeft(3, '0'));
            ...
            try
            {
                ...
                var tempInputFile = ODFileUtils.CombinePaths(saveFolder, "tempinput." + fileNum.ToString().PadLeft(3, '0'));
                ...
                var inputFile = ODFileUtils.CombinePaths(saveFolder, "input." + fileNum.ToString().PadLeft(3, '0'));
            ...
            var nputFile = ODFileUtils.CombinePaths(saveFolder, "_nput." + fileNum.ToString().PadLeft(3, '0'));
            ...
            var result = Encoding.GetEncoding(850).GetString(resultBytes);
            //strip the prefix.  Example prefix: 123456,0,000,
            var resultPrefix = "";
            //Find position of third comma
            var match = Regex.Match(result, @"^\d*,\d+,\d+,");
            if (!match.Success)
            {
            ...
            if (result.Length < 42)
            {
                ...
                var responses = resultPrefix.Split(',');
                var errorCode = Lans.g("Canadian", "UNKNOWN");
                ...
                if (isItrans)
                {
                    ...
                    var errorFile = ODFileUtils.CombinePaths(dirClientProgram, "ica.log");
                    var errorlog = "";
                ...
                else if (isClaimstream)
                {
                    ...
                    var errorMessage = GetErrorMessageForCodeClaimstream(errorCode, ref errorDescription);
            ...
            else
            {
                ...
                try
                {
                    var messageData = new CCDFieldInputter(result);
                    ...
                    var mailboxIndicator = messageData.GetFieldById("A11");
        ...
        public static Clearinghouse GetCanadianClearinghouseHq(Carrier carrier)
        {
            ...
            var hqClearinghouseNum = Clearinghouses.AutomateClearinghouseHqSelection(payerid, EnumClaimMedType.Dental);
            if (hqClearinghouseNum != 0)
            {
                var clearinghouseHq = Clearinghouses.GetClearinghouse(hqClearinghouseNum);
            ...
            var clearinghouse = Clearinghouses.GetFirstOrDefault(x => PrefC.GetLong(PrefName.ClearinghouseDefaultDent) == x.ClearinghouseNum
        ...
        {
            var retVal = (number * 100).ToString("F0");
        ...
        public static string TidyN(string numText, int width)
        {
            ...
            try
            {
                var number = Convert.ToInt32(numText);
        ...
        {
            var E = materials.Contains("E");
            var C = materials.Contains("C");
            var M = materials.Contains("M");
            var X = materials.Contains("X");
            var I = materials.Contains("I");
            if (E && C && M && X && I)
            {
                return "Z";
            }
        ...
        ///<summary>Returns true if the LanguageAndRegion preference is set to 'fr-CA'.</summary>
        public static bool IsDentalOfficeFrench()
        {
            return (PrefC.GetString(PrefName.LanguageAndRegion) == "fr-CA"); //Canadian. en-CA or fr-CA
        }

        ///<summary>Returns the string representation of the identification code for the corresponding tooth, quad, arch, or sextant for the procedure and procedure code passed in.
        ///The code returned comes from the Identification System for Arches, Quadrants, Sextants, Joints section in the ODA Suggested Fee Guide for General Practitioners.</summary>
        private static string GetToothQuadOrArch(Procedure proc, ProcedureCode procCode)
        {
            ...
            switch (procCode.TreatArea)
            {
                ...
                case TreatmentArea.Tooth:
                    return Tooth.Display(proc.ToothNum);
                case TreatmentArea.ToothRange:
                    var range = proc.ToothRange.Split(',');
        ...
        ///<summary>Returns the string representation of a tooth code for the procedure and procedure code passed in.
        ///Always returns a tooth code between 11 and 85 which may be an arbitrary tooth number within the corresponding sextant, quad, or arch.
        ///Useful for carriers that require a valid tooth number in order to identify arches, quadrants, or sextants.
        ///Currently only returns tooth codes that relate to permanent dentition (18 - 48).</summary>
        private static string GetToothCodeForToothSextantQuadOrArch(Procedure proc, ProcedureCode procCode)
        {
            //Return the exact tooth number set on the procedure when the TreatArea is Tooth or an arbitrary tooth number that resides within the specified treatment area.
            switch (procCode.TreatArea)
            {
                case TreatmentArea.Surf:
                case TreatmentArea.Tooth:
                ...
                case TreatmentArea.Arch:
                    if (proc.Surf.ToUpper() == "U")
                    {
                        return "18"; //Upper right molar.
                    }
                    else if (proc.Surf.ToUpper() == "L")
                    {
                        return "48"; //Lower right molar.
                    }

                    return "00"; //Invalid or unspecified Arch.  This is also the number that represents "whole mouth" in Canada.
                case TreatmentArea.Quad:
                    if (proc.Surf == "UR")
                    {
                        return "18"; //Upper right molar.
                    ...
                    {
                        return "28"; //Upper left molar.
                    }
                    else if (proc.Surf == "LR")
                    {
                        return "48"; //Lower right molar.
                    }
                    else if (proc.Surf == "LL")
                    {
                        return "38"; //Lower left molar.
                    }

                    return "00"; //Invalid or unspecified Quad.  This is also the number that represents "whole mouth" in Canada.
                case TreatmentArea.Sextant:
                    //Oral cavity divided into six sextants.
                    if (proc.Surf == "1")
                    {
                        //Sextant 1: upper right premolar/molar region (18-14)
                        return "18"; //Upper right molar.
                    }
                    else if (proc.Surf == "2")
                    {
                        //Sextant 2: upper jaw incisor/canine region (13-23)
                        return "13"; //Upper right canine.
                    }
                    else if (proc.Surf == "3")
                    {
                        //Sextant 3: upper left premolar/molar region (24-28)
                        return "24"; //Upper left premolar.
                    }
                    else if (proc.Surf == "4")
                    {
                        //Sextant 4: lower left premolar/molar region (38-34)
                        return "38"; //Lower left molar.
                    }
                    else if (proc.Surf == "5")
                    {
                        //Sextant 5: lower jaw incisor/canine region (33-43)
                        return "33"; //Lower left canine.
                    }
                    else if (proc.Surf == "6")
                    {
                        //Sextant 6: lower right premolar/molar region (44-48)
                        return "44"; //Lower left premolar.
                    }
        ...
        public static string GetMissingData(ClaimSendQueueItem queueItem)
        {
            ...
            var providerFirst = Providers.GetFirst(); //Used in order to preserve old behavior...  If this fails, then old code would have failed.
            var billProv = Providers.GetFirstOrDefault(x => x.Id == claim.ProvBill) ?? providerFirst;
            var treatProv = Providers.GetFirstOrDefault(x => x.Id == claim.ProvTreat) ?? providerFirst;
            var insSub = InsSubs.GetSub(claim.InsSubNum, new List<InsSub>());
            var insPlan = InsPlans.GetPlan(claim.PlanNum, new List<InsPlan>());
            ...
            {
                //Is primary claim and the patient has secondary insurance.
            ...
            var claimProcList = ClaimProcs.Refresh(patient.PatNum); //all claimProcs for a patient.
            var claimProcsClaim = ClaimProcs.GetForSendClaim(claimProcList, claim.ClaimNum);
            var procListAll = Procedures.Refresh(claim.PatNum);
            Procedure proc;
            ProcedureCode procCode;
            var extracted = Procedures.GetCanadianExtractedTeeth(procListAll);
            ...
            if (patient.Age >= 18 && patient.CanadianEligibilityCode == 1)
            {
                //fulltimeStudent
...
//incomplete. Also duplicate for max
            ...
            if (carrier2 != null && carrier2.ElectID == "000064")
            {
            ...
            {
                //Is primary claim and the patient has secondary insurance.
                if (carrier2.IsCDA && !Regex.IsMatch(carrier2.ElectID, @"^[0-9]{6}$"))
                {
                    //If not CDA, then we send "999999" as per specification.
                    if (retVal != "")
                        retVal += ", ";
            ...
            if (claim.CanadianIsInitialUpper == "N" && claim.CanadianDateInitialUpper.Year < 1900)
            {
                //missing date
                if (retVal != "")
                    retVal += ", ";
            ...
            if (claim.CanadianIsInitialLower == "N" && claim.CanadianDateInitialLower.Year < 1900)
            {
                //missing date
                if (retVal != "")
                    retVal += ", ";
            ...
            */
            ...
            for (var i = 0; i < claimProcsClaim.Count; i++)
            {
                ...
                var labFeesForProc = Procedures.GetCanadianLabFees(proc.ProcNum, procListAll);
                if (claim.ClaimType != "PreAuth")
                {
                    for (var j = 0; j < labFeesForProc.Count; j++)
                    {
                        if (labFeesForProc[j].ProcStatus != ProcStat.C)
                        {
                            var procCodeLab = ProcedureCodes.GetProcCode(labFeesForProc[j].CodeNum);
                            if (retVal != "")
                            {
                                retVal += ", ";
                            }
                ...
                for (var j = 0; j < labFeesForProc.Count; j++)
                {
                    if (labFeesForProc[j].ProvNum != claim.ProvTreat)
                    {
                        ...
                        queueItem.Warnings += $"lab for proc {procCode.ProcCode} treat prov mismatch";
                    }
                }
            }
        ...
        private class CarrierIssuedProcedure
        {
            ...
            public CarrierIssuedProcedure(List<CCDField> listFields, Claim claimCur, long payPlanNum, bool isPreEob, EraBehaviors eraBehavior, Dictionary<int, string> listNoteFields)
            {
                ClaimProcByTotal = ByTotClaimProcHelper(claimCur, payPlanNum, isPreEob, eraBehavior);

                #region Process CCDFields

                var listClaimProcNotes = new List<string>();
                foreach (var field in listFields)
                {
                    switch (field.FieldId)
                    {
                        case "G18": //Reference to Line Number of the Submitted Procedure
                            ...
                            ArrayLineNumbers = Regex.Replace(field.Valuestr, "[^1-7]", "").Select(x => SIn.Byte(x.ToString())).ToArray();
                            break;
                        ...
                        case "G44": //Eligible Amount for additional Lab procedure
                            var eligibleAmount = SIn.Double(RawMoneyStrToDisplayMoney(field.Valuestr));
                            listClaimProcNotes.Add(Lans.g("Canadian", "Lab Procedure Eligible Amount") + ": " + eligibleAmount.ToString("N"));
                            break;
                        case "G21": //Deductible
                            Deductible = SIn.Double(RawMoneyStrToDisplayMoney(field.Valuestr));
                        ...
                        case "G22": //Eligible percentage
                            ClaimProcByTotal.Percentage = SIn.Int(RawPercentToDisplayPercent(field.Valuestr));
                            break;
                        case "G23": //Benefit Amount
                            BenefitAmount = SIn.Double(RawMoneyStrToDisplayMoney(field.Valuestr));
