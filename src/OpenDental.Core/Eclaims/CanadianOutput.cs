...
namespace OpenDentBusiness.Eclaims
{
    public class CanadianOutput
    {
        ...
        public static List<long> SendAttachments(Clearinghouse clearinghouseClin, long patNum, InsPlan plan, InsSub insSub, Claim claim)
        {
            ...
            var listFileInfoAttachments = GetInfoForAttachments(claim.Attachments); //Loaded here as part of validation.  Throws exceptions.
            var carrier = Carriers.GetCarrier(plan.CarrierNum);
            if (carrier == null)
            {
            ...
            var claimOfficeSequenceNumber = 0;
            try
            {
            ...
            var network = CanadianNetworks.GetNetwork(carrier.CanadianNetworkNum, clearinghouseClin, claim);
            var patient = Patients.GetPat(patNum);
            var provider = Providers.GetById(claim.ProvTreat);
            ...
            if (provider.NationalProviderId.Length != 9)
            {
                if (error != "") error += ", ";
                error += "Prov CDA num 9 digits";
            }

            if (provider.CanadianOfficeNumber.Length != 4)
            {
                if (error != "") error += ", ";
            ...
            while (listFileInfoAttachments.Count != 0)
            {
                ...
                strb.Append(TidyAN(provider.NationalProviderId, 9)); //already validated
                //B02 (treating) provider office number 4 AN
                strb.Append(TidyAN(provider.CanadianOfficeNumber, 4)); //already validated
                ...
                strb.Append(TidyAN("         ", 9));
                //B08 Receiving Office Number 4 AN
                strb.Append(TidyAN("    ", 4));
                //"For Future Use" 1 A
                strb.Append(TidyA(" ", 1));
                ...
                strb.Append(TidyN(claimOfficeSequenceNumber, 6));
                //F42 Original Transaction Reference Number 14 AN
                strb.Append(TidyAN(claim.CanadaTransRefNum, 14));
                //F43 Attachment Source 1 A
                strb.Append(TidyA("I", 1));
                //F44 Attachment Count 2 N. Max is 30 attachments under 9999999 bytes
                strb.Append(TidyN(claim.Attachments.Count, 2));
                //Need to loop through these codes for the number of times equal to F44
                long totalBytes = 0;
                for (var i = 0; i < listFileInfoAttachments.Count; i++)
                {
                    var lengthBase64 = ((listFileInfoAttachments[i].Length * 4) / 3) + 1; //Calculate total size when raw bytes are converted to base64 for outgoing format.
                    ...
                    strb.Append(TidyN(index, 2));
                    //F45 Attachment Type 3 AN. We can infer this from file extension instead of creating a UI
                    var attachmentType = listFileInfoAttachments[i].Extension.ToUpper().TrimStart('.');
                    ...
                    strb.Append(TidyAN(attachmentType, 3));
                    //F46 Attachment Length 7 N
                    var attachmentBytes = File.ReadAllBytes(listFileInfoAttachments[i].FullName);
                    var attachmentBase64 = Convert.ToBase64String(attachmentBytes);
                    totalBytes += attachmentBase64.Length;
                    strb.Append(TidyN(attachmentBase64.Length, 7));
                    //F48 Attachment File Date 8 N
                    strb.Append(TidyN(listFileInfoAttachments[i].CreationTime.ToString("yyyyMMdd"), 8));
                ...
                strb.Replace("0000000", TidyN(strb.Length, 7), 32, 7);

                #endregion Construct Outgoing Message

                string errorMsg;
                var result = PassToIca(strb.ToString(), clearinghouseClin, network, isAutomatic: false, out errorMsg);
                ...
                {
                    var fieldInputter = new CCDFieldInputter(result);
                    ...
                    var fieldG06 = fieldInputter.GetFieldById("G06");
            ...
            InsSub insSub, bool isAutomatic, PrintCCD printCCD)
        {
            ...
            var network = CanadianNetworks.GetNetwork(carrier.CanadianNetworkNum, clearinghouseClin);
            var patient = Patients.GetPat(patNum);
            var subscriber = Patients.GetPat(insSub.Subscriber);
            var provider = Providers.GetById(Patients.GetProvNum(patNum));
            if (clearinghouseClin == null)
            {
            ...
            int len;
            var C19PlanRecordPresent = false;
            if (carrier.CDAnetVersion == "02")
            {
                len = 178;
                strb.Append(TidyN(len, 4));
            }
            else
            {
                len = 214;
            ...
            if (carrier.CDAnetVersion == "04")
            {
                ...
                var provBilling = Providers.GetById(Providers.GetBillingProvNum(provider.Id, patient.ClinicNum));
                strb.Append(TidyAN(provBilling.NationalProviderId, 9)); //already validated
            }
            ...
            if (carrier.CDAnetVersion == "02")
            {
                ...
                strb.Append(TidyAN(plan.GroupNum, 8));
            }
            else
            {
                ...
                strb.Append(TidyAN(plan.GroupNum, 12));
            }

            //C11 primary division/section number 10 AN
            strb.Append(TidyAN(plan.DivisionNo, 10));
            if (carrier.CDAnetVersion == "02")
            {
                //C02 subscriber id number 11 AN
                strb.Append(TidyAN(insSub.SubscriberID.Replace("-", ""), 11)); //no extra validation for version 02
            }
            else
            {
            ...
            strb.Append(GetRelationshipCode(relat));
            ...
            strb.Append(GetEligibilityCode(patient.CanadianEligibilityCode, carrier.CDAnetVersion == "02")); //validated
            if (carrier.CDAnetVersion == "04")
            {
                //C12 plan flag 1 A
                strb.Append(GetPlanFlag(plan.CanadianPlanFlag));
            ...
            if (carrier.CDAnetVersion == "04")
            {
                ...
                strb.Append(TidyN(plan.DentaideCardSequence, 2));
                //C19 plan record 30 AN
                if (C19PlanRecordPresent)
                {
                    //todo: what text goes here?  Not documented
                    strb.Append(TidyAN("", 30));
                }
            }
            ...
            var strResponse = ""; //"Eligibility check on "+DateTime.Today.ToShortDateString()+"\r\n";
            //CCDField field=fieldInputter.GetFieldById("G05");//response status
            var valuestr = fieldInputter.GetValue("G05"); //response status
            switch (valuestr)
            {
                case "E":
        ...
        {
            var officeSequenceNumber = 0; //Clear the randomly generated office sequence number.
            originalEtransDateTime = DateTime.MinValue; //So we can get the latest matching transaction.
            var listEtrans = Etranss.GetAllForOneClaim(claim.ClaimNum).FindAll(x => x.Etype.In(EtransType.Claim_CA, EtransType.ClaimCOB_CA, EtransType.Predeterm_CA));
            for (var i = 0; i < listEtrans.Count; i++)
            {
                var ack = Etranss.GetEtrans(listEtrans[i].AckEtransNum);
                if (ack == null)
                {
                ...
                var messageText = EtransMessageTexts.GetMessageText(ack.EtransMessageTextNum);
                if (messageText == "")
                {
                    continue;
                }
                ...
                var transRefNum = "";
                var ccdFieldTransRefNum = messageData.GetFieldById("G01");
        ...
        public static long SendClaimReversal(Clearinghouse clearinghouseClin, Claim claim, InsPlan plan, InsSub insSub, bool isAutomatic, PrintCCD printCCD)
        {
            ...
            var network = CanadianNetworks.GetNetwork(carrier.CanadianNetworkNum, clearinghouseClin, claim);
            var etrans = Etranss.CreateCanadianOutput(claim.PatNum, carrier.CarrierNum, carrier.CanadianNetworkNum,
            ...
            etrans.OfficeSequenceNumber = GetOriginalOfficeSequenceNumber(claim, out var originalEtransDateTime);
            ...
            var serverDate = MiscData.GetNowDateTime().Date;
            ...
            if (carrier.CDAnetVersion == "02")
            {
                ...
                strb.Append(TidyN("133", 4));
            }
            else
            {
                //version 04
                //A07 message length N 5
                strb.Append(TidyN("164", 5));
            }
            ...
            //B01 CDA provider number 9 AN
            strb.Append(TidyAN(prov.NationalProviderId, 9)); //already validated
            //B02 provider office number 4 AN
            strb.Append(TidyAN(prov.CanadianOfficeNumber, 4)); //already validated
            if (carrier.CDAnetVersion != "02")
            {
                //version 04
                //B03 billing provider number 9 AN
        ...
        {
            var listFileInfos = new List<FileInfo>();
            for (var i = 0; i < listAttachments.Count; i++)
            {
                var attachmentPath = Path.Combine(EmailAttaches.GetAttachPath(), listAttachments[i].ActualFileName);
                ...
                var extension = Path.GetExtension(attachmentPath).ToLower();
                Bitmap bitmap = null;
                ...
                if (bitmap != null)
                {
                    //Convert images to to JPG.
                    var tempFilePath = ODFileUtils.CreateRandomFile(PrefC.GetTempFolderPath(), ".jpg");
                    var codec = ImageCodecInfo.GetImageEncoders().FirstOrDefault(x => x.MimeType == "image/jpeg");
                    var encoderParams = new EncoderParameters(2);
                    encoderParams.Param[0] = new EncoderParameter(Encoder.Quality, (long) 100);
                    encoderParams.Param[1] = new EncoderParameter(Encoder.ColorDepth, (long) 24); //RGB, no alpha.
                ...
                var attachmentSize = ((fileInfo.Length * 4) / 3) + 1; //Calculate total size when raw bytes are converted to base64 for outgoing format.
        ...
        public static List<Etrans> GetPaymentReconciliations(Clearinghouse clearinghouseClin, Carrier carrier, ProviderDto provTreat, ProviderDto provBilling,
            DateTime reconciliationDate, long clinicNum, bool isAutomatic, PrintCCD printCCD)
        {
            ...
            ValidateExportPath(clearinghouseClin);
            var etransAcks = new List<Etrans>();
            var pageNumber = 1;
            var totalPages = 1;
            do
            {
                ...
                var network = CanadianNetworks.GetNetwork(carrier.CanadianNetworkNum, clearinghouseClin);
                var etrans = Etranss.CreateCanadianOutput(0, carrier.CarrierNum, carrier.CanadianNetworkNum,
                ...
                //A07 message length N4
                strb.Append(TidyN("77", 5));
                //A09 carrier transaction counter 5 N
                strb.Append(TidyN(etrans.CarrierTransCounter, 5));
                //B01 CDA provider number 9 AN
                strb.Append(TidyAN(provTreat.NationalProviderId, 9)); //already validated
                //B02 (treating) provider office number 4 AN
                strb.Append(TidyAN(provTreat.CanadianOfficeNumber, 4)); //already validated
                //B03 billing provider number 9 AN
                //might need to account for possible 5 digit prov id assigned by carrier
                strb.Append(TidyAN(provBilling.NationalProviderId, 9)); //already validated
                //B04 billing provider office number 4 AN
                strb.Append(TidyAN(provBilling.CanadianOfficeNumber, 4)); //already validated
                //F33 Reconciliation Date 8 N
                strb.Append(reconciliationDate.ToString("yyyyMMdd"));
                //F38 Current Reconciliation Page Number N 1
                strb.Append(TidyN(pageNumber, 1));
                ...
                var fieldG62 = fieldInputter.GetFieldById("G62"); //Last reconciliation page number.
        ...
        public static Etrans GetSummaryReconciliation(Clearinghouse clearinghouseClin, Carrier carrier, CanadianNetwork network, ProviderDto provTreat,
        ...
        private static List<Etrans> GetOutstandingForClearinghouse(Clearinghouse clearinghouseClin, ProviderDto prov, string formatVersion, Carrier carrier,
            CanadianNetwork network, PrintCdaClaimForm printForm, PrintCCD printCCD)
        {
            ...
            ValidateExportPath(clearinghouseClin);
            var listCarriers = new List<Carrier>() {carrier};
            ...
            if (carrier == null && formatVersion != "02")
            {
                if (network != null && network.Abbrev.In("TELUS A", "TELUS B"))
                {
                    ...
                    {
                        var carrierABC = Carriers.GetCdaCarriersInUse().FirstOrDefault(x => x.ElectID == "000090");
                        if (carrierABC != null)
                        {
                            //Consider Alberta Blue Cross (ABC), but only if it is in use currently.
                ...
                {
                    //For ITRANS2, there is a bug where 999999 requests to network for all outstanding simply parrots the request back to us.
        ...
        private static List<Etrans> GetOutstandingForCarrier(Clearinghouse clearinghouseClin, ProviderDto prov, string formatVersion, Carrier carrier,
            CanadianNetwork network, PrintCdaClaimForm printForm, PrintCCD printCCD)
        {
            ...
            var listEtransAcks = new List<Etrans>();
            var isAutomatic = (printForm == null && printCCD == null);
            var exit = false;
            do
            {
                var etransROT = Etranss.CreateCanadianOutput(0, (carrier == null) ? 0 : carrier.CarrierNum, (network == null) ? 0 : network.CanadianNetworkNum,
                ...
                //A02 office sequence number 6 N
                strb.Append(TidyN(etransROT.OfficeSequenceNumber, 6));
                //A03 format version number 2 N
                ...
                if (formatVersion == "04")
                {
                    strb.Append(TidyN("64", 5));
                }
                else if (formatVersion == "02")
                {
                    strb.Append(TidyN("42", 4));
                }

                if (formatVersion == "04")
                {
                    //A09 carrier transaction counter 5 N
                    strb.Append(TidyN(etransROT.CarrierTransCounter, 5));
                }
                ...
                var result = PassToIca(strb.ToString(), clearinghouseClin, network, isAutomatic, out var errorMsg);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            var invalidTypeMsg = "Invalid outstanding transaction response type returned.";
                            etransAck.Etype = EtransType.AckError;
                ...
                var responseFormatVersion = fieldInputter.GetFieldById("A03").Valuestr;
                var fieldA04 = fieldInputter.GetFieldById("A04"); //message format
                ...
                if (responseFormatVersion == "02")
                {
                    ...
                    {
                        var fieldG08 = fieldInputter.GetFieldById("G08");
                ...
                else if (responseFormatVersion == "04")
                {
                    ...
                    if (fieldA04.Valuestr == "14")
                    {
                        ...
                        var fieldG05 = fieldInputter.GetFieldById("G05"); //response status
                        if (fieldG05.Valuestr == "R")
                        {
                            //We only expect the result to be 'R' or 'X' as specified in the documentation.
                            var fieldG07 = fieldInputter.GetFieldById("G07"); //disposition message
                            var fieldG08 = fieldInputter.GetFieldById("G08"); //error code
                            if (!isAutomatic)
                            {
                ...
                var fieldA02 = fieldInputter.GetFieldById("A02"); //office sequence number
                ...
                var etransOriginal = Etranss.GetForSequenceNumberCanada(fieldA02.Valuestr);
                if (etransOriginal != null)
                {
                    ...
                    var fieldA05 = fieldInputter.GetFieldById("A05"); //CarrierID, exists in all formats but 24-Email, and 16-Payment Reconciliation Response
                    ...
                    {
                        var carrierA05 = Carriers.GetAllByElectId(fieldA05.Valuestr).FirstOrDefault(x => x.IsCDA && !x.IsHidden); //Get Carrier from ElectID
                    ...
                    var ccdFieldsArrayErrorCodes = fieldInputter.GetFieldsById("G08"); //Error Codes. Up to 10.
                        ...
                        && ccdFieldsArrayErrorCodes.Length > 0)
                    {
                        ...
                        if (ccdFieldsArrayErrorCodes.Any(x => x.Valuestr == "124"))
                        {
                    ...
                    if (!exit)
                    {
                        ...
                        if (claim != null)
                        {
                            ...
                            {
                                var listAllProcs = Procedures.Refresh(claim.PatNum);
                                var listAllClaimProcs = ClaimProcs.Refresh(claim.PatNum);
                                var listClaimProcsForClaim = listAllClaimProcs.FindAll(x => x.ClaimNum == claim.ClaimNum);
                                //int patAge=0;
                                //if(clearinghouseClin.IsEraDownloadAllowed==EraBehaviors.DownloadDoNotReceive) {
                                var pat = Patients.GetPat(claim.PatNum);
                                //	patAge=pat.Age;
                                //}
                                EOBImportHelper(fieldInputter, listClaimProcsForClaim, listAllProcs, listAllClaimProcs, claim, true, null, clearinghouseClin.IsEraDownloadAllowed, pat);
                ...
                if (!exit && !isAutomatic)
                {
                    ...
                    {
                        using var msgbox = new MsgBoxCopyPaste(Lans.g("CanadianOutput", "Failed to display one of the ROT responses, here is the raw message") + ": " + Environment.NewLine + result);
                        msgbox.ShowDialog();
                    }
                }
        ...
        ///If the user initiated this call, then set the form and ccd delegate so the output can be printed.
        ///Otherwise, set form and ccd delegate to null to supress UI and printer output.</summary>
        public static List<Etrans> GetOutstandingForDefault(ProviderDto prov, string formatVersion = "", Carrier carrier = null,
            PrintCdaClaimForm printForm = null, PrintCCD printCCD = null)
        {
            ...
            var clearinghouseHq = GetCanadianClearinghouseHq(carrier);
            var clearinghouseClin = Clearinghouses.OverrideFields(clearinghouseHq, Clinics.ClinicNum);
            var netTelusA = CanadianNetworks.GetFirstOrDefault(x => x.Abbrev == "TELUS A");
            var netTelusB = CanadianNetworks.GetFirstOrDefault(x => x.Abbrev == "TELUS B");
            ...
            if (string.IsNullOrEmpty(formatVersion) || formatVersion == "04")
            {
                ...
                if (carrier == null)
                {
                    ...
                    else if (clearinghouseHq.CommBridge.In(EclaimsCommBridge.Claimstream, EclaimsCommBridge.None))
                    {
                        //Claimstream or CanadaFakeClearinghouse
                ...
                else
                {
                    ...
                    var net = CanadianNetworks.GetNetwork(carrier.CanadianNetworkNum, clearinghouseClin); //We always know which network if the carrier is specified.
