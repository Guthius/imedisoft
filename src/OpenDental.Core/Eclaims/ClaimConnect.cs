...
namespace OpenDentBusiness.Eclaims {
	...
	/// In an attempt to change as little as possible in the DXC attachment workflows, the bulk of the changes will occur here for supporting XConnect attachments. All of the attachment requests will be called the same way from the attachment forms. At the beginning of each attachment method in this file the XConnect.IsEnabled() method will be called to determine which API will be used. However, when the claim validation occurs upon opening any of the attachment forms, we make use of the response object from this call. This requires us to make changes to the attachment forms themselves to handle the differences in the two response objects.
	/// </summary>
...
<soapenv:Envelope xmlns:soapenv=""http://schemas.xmlsoap.org/soap/envelope/"" xmlns:xsd=""http://www.w3.org/2001/XMLSchema"" xmlns:xsi=""http://www.w3.org/2001/XMLSchema-instance"">
	...
	public class XConnect {
		...
		///<summary>Creates a new attachment. Returns a dxcAttachmentId. Can throw exceptions.</summary>
		public static string AttachmentCreate(Claim claim,string narrative) {
			...
			if(clinic.UseBillingAddressOnClaims) {
				...
				city=clinic.BillingCity;
				state=clinic.BillingState;
				zip=clinic.BillingZip;
			}
			else {
				address1=clinic.AddressLine1;
				address2=clinic.AddressLine2;
				city=clinic.City;
				state=clinic.State;
				zip=clinic.Zip;
			}
			xConnectAttachmentBillingProvider.address1=address1;
			xConnectAttachmentBillingProvider.address2=address2;
			xConnectAttachmentBillingProvider.city=city;
			xConnectAttachmentBillingProvider.state=state;
			...
			xConnectAttachmentCreate.subscriber=xConnectAttachmentSubscriber;
			//Remaining members not in child objects
			xConnectAttachmentCreate.providerClaimId=TruncateClaimIdentifierIfNeeded(claim.ClaimIdentifier);
			xConnectAttachmentCreate.payerIdCode=carrier.ElectID;
			if(claim.DateService.Year>1880) {
				xConnectAttachmentCreate.dateOfService=claim.DateService.ToString("yyyy-MM-dd");
			}
			...
			xConnectAttachmentRenderingProvider.state=clinic.State;
			...
			if(!xConnectAttachmentCreateResponse.success 
				|| xConnectAttachmentCreateResponse.status==null 
				|| xConnectAttachmentCreateResponse.status.code!=0) 
			{
				if(xConnectAttachmentCreateResponse.status==null) {
					throw new ODException(Lans.g("ClaimConnect","Invalid XConnect response status"));
				}
				else {
					throw new ODException(xConnectAttachmentCreateResponse.status.description);
				}
			}
			if(xConnectAttachmentCreateResponse.response==null) {
				throw new ODException(Lans.g("ClaimConnect","Invalid attachment response recieved."));
			}
			return xConnectAttachmentCreateResponse.response.dxcAttachmentId;
		}
		...
		public static List<int> AttachmentAddImage(Claim claim,List<ClaimConnect.ImageAttachment> listImages) {
			...
			for(int i=0;i<listImages.Count;i++) {
				...
				switch(imageType) {
					case "Referral Form":
						...
						break;
					case "Dental Models":
						xConnectAttachmentAddImage.imageTypeCode=EnumXConnectImageTypeCode.DA;
						break;
					case "Diagnostic Report":
						xConnectAttachmentAddImage.imageTypeCode=EnumXConnectImageTypeCode.DG;
						break;
					case "Explanation of Benefits":
						xConnectAttachmentAddImage.imageTypeCode=EnumXConnectImageTypeCode.EB;
						break;
					case "Other Attachments":
						xConnectAttachmentAddImage.imageTypeCode=EnumXConnectImageTypeCode.OZ;
						break;
					case "Periodontal Charts":
						xConnectAttachmentAddImage.imageTypeCode=EnumXConnectImageTypeCode.P6;
						break;
					case "X-Rays":
						xConnectAttachmentAddImage.imageTypeCode=EnumXConnectImageTypeCode.RB;
						break;
					case "Radiology Reports":
						xConnectAttachmentAddImage.imageTypeCode=EnumXConnectImageTypeCode.RR;
						break;
					case "Intra-Oral Photograph":
						xConnectAttachmentAddImage.imageTypeCode=EnumXConnectImageTypeCode.XP;
						break;
					case "Narrative":
						xConnectAttachmentAddImage.imageTypeCode=EnumXConnectImageTypeCode.Item03;
						break;
					//If the image type code is somehow mangled just mark the attachment as 'Other'. This ensures there is always an image type code set.
					default:
						xConnectAttachmentAddImage.imageTypeCode=EnumXConnectImageTypeCode.OZ;
				...
				xConnectAttachmentAddImage.imageFileAsBase64=Convert.ToBase64String(listImages[i].ImageFileAsBase64);
				xConnectAttachmentAddImage.imageFileName=listImages[i].ImageFileNameDisplay;
				if(listImages[i].ImageOrientationType=="right") {
					xConnectAttachmentAddImage.imageOrientationType=EnumXConnectImageOrientationType.RIGHT;
				}
				else {
					xConnectAttachmentAddImage.imageOrientationType=EnumXConnectImageOrientationType.LEFT;
				}
			...
			for(int i=0;i<listXConnectAttachmentAddImages.Count;i++) {
				...
				if(!xConnectAttachmentAddImageResponse.success
					|| xConnectAttachmentAddImageResponse.status==null
					|| xConnectAttachmentAddImageResponse.status.code!=0)
				{
					if(xConnectAttachmentAddImageResponse.status==null) {
						throw new ODException(Lans.g("ClaimConnect","Invalid XConnect response status"));
					}
					else {
						throw new ODException(xConnectAttachmentAddImageResponse.status.description);
					}
				}
				if(xConnectAttachmentAddImageResponse.response==null) {
					throw new ODException(Lans.g("ClaimConnect","Invalid attachment response recieved."));
				}
				listImageReferenceIds.Add(xConnectAttachmentAddImageResponse.response.dxcAttachmentImageId);
			}
			return listImageReferenceIds;
		}
		...
		public static void AttachmentAddNarrative(Claim claim,string narrative) {
			...
			if(!xConnectAttachmentAddNarrativeResponse.success
				|| xConnectAttachmentAddNarrativeResponse.status==null
				|| xConnectAttachmentAddNarrativeResponse.status.code!=0)
			{
				if(xConnectAttachmentAddNarrativeResponse.status==null) {
					throw new ODException(Lans.g("ClaimConnect","Invalid XConnect response status"));
				}
				else {
					throw new ODException(xConnectAttachmentAddNarrativeResponse.status.description);
				}
			}
		}
		...
		public static void AttachmentRemoveImage(Claim claim,List<ClaimAttach> listClaimAttaches) {
			...
			for(int i=0;i<listClaimAttaches.Count;i++) {
				...
				if(!xConnectAttachmentRemoveImageResponse.success
					|| xConnectAttachmentRemoveImageResponse.status==null
					|| xConnectAttachmentRemoveImageResponse.status.code!=0)
				{
					if(xConnectAttachmentRemoveImageResponse.status==null) {
						throw new ODException(Lans.g("ClaimConnect","Invalid XConnect response status"));
					}
					else {
						throw new ODException(xConnectAttachmentRemoveImageResponse.status.description);
					}
				}
			}
		}
		...
		public static void AttachmentSubmit(Claim claim) {
			...
			if(!xConnectAttachmentSubmitResponse.success
				|| xConnectAttachmentSubmitResponse.status==null
				|| xConnectAttachmentSubmitResponse.status.code!=0)
			{
				if(xConnectAttachmentSubmitResponse.status==null) {
					throw new ODException(Lans.g("ClaimConnect","Invalid XConnect response status"));
				}
				else {
					throw new ODException(xConnectAttachmentSubmitResponse.status.description);
				}
			}
		}
	}
	...
	public class XConnectAddress {
		...
		public static XConnectAddress BillingAddressFromClinic(ClinicDto clinic,Provider providerBill) {
			...
			if(clinic.UseBillingAddressOnClaims) {
				...
				xconnectAddress.address1=clinic.AddressLine1;
				xconnectAddress.address2=clinic.AddressLine2;
				xconnectAddress.city=clinic.City;
				xconnectAddress.state=clinic.State;
				xconnectAddress.zipCode=clinic.Zip;
				xconnectAddress.phone=clinic.PhoneNumber;
	...
	public class XConnectAttachmentCreate {
		///<summary>Optional. Integer or null</summary>
		public int? dxcClaimId;
		///<summary>Required</summary>
		...
		///<summary>Required</summary>
		public XConnectAttachmentPatient patient;
		///<summary>Required</summary>
		public XConnectAttachmentSubscriber subscriber;
		///<summary>Optional. Format yyyy-mm-dd.</summary>
		public string dateOfService;
		///<summary>Required</summary>
		public XConnectAttachmentBillingProvider billingProvider;
		///<summary>Optional</summary>
		public XConnectAttachmentRenderingProvider renderingProvider;
		///<summary>Optional</summary>
	...
	public class XConnectAttachmentPatient {
		///<summary>Required</summary>
		public string firstName;
		///<summary>Required</summary>
		public string lastName;
	...
	public class XConnectAttachmentSubscriber {
		///<summary>Required</summary>
		public string id;
		///<summary>Required</summary>
		public string firstName;
	...
	public class XConnectAttachmentBillingProvider {
		///<summary>Required</summary>
		public string address1;
		///<summary>Optional</summary>
		public string address2;
	...
	public class XConnectAttachmentRenderingProvider {
		///<summary>Optional</summary>
		public string address1;
		///<summary>Optional</summary>
		public string address2;
	...
	public class XConnectAttachmentAddImage {
		///<summary>Required</summary>
		public EnumXConnectImageTypeCode imageTypeCode;
		///<summary>Required</summary>
		public string imageFileAsBase64;
		///<summary>Optional</summary>
		public string imageFileName;
		///<summary>Optional. Required for X-Rays</summary>
		public EnumXConnectImageOrientationType imageOrientationType;
		///<summary>Optional. Required for X-Rays. Format yyyy-mm-dd</summary>
		public string imageDate;
		///<summary>Optional</summary>
		public string vendorImageId;
	}

	public class XConnectAttachmentAddNarrative {
		///<summary>Required</summary>
		public string narrative;
	}

	public class XConnectAttachmentCreateResponse {
		///<summary>Optional</summary>
		...
		public bool success;
		///<summary>Optional</summary>
		public XConnectAttachmentCreateResponseResponse response;
		///<summary>Optional. Not found in DXC documentation, but comes back in response.</summary>
		public long transactionId;
	}

	public class XConnectAttachmentCreateResponseResponse {
		///<summary>Optional</summary>
	...
	public class XConnectAttachmentAddImageResponse {
		///<summary>Optional</summary>
		public XConnectResponseStatus status;
		///<summary>Optional</summary>
		public string[] messages;
		...
		public bool success;
		///<summary>Optional</summary>
		public XConnectAttachmentAddImageResponseResponse response;
		///<summary>Optional. Not found in DXC documentation, but comes back in response.</summary>
		public long transactionId;
	}

	public class XConnectAttachmentAddImageResponseResponse {
		///<summary>Optional</summary>
		public int dxcAttachmentImageId;
		///<summary>Optional</summary>
		public string vendorImageId;
	}

	public class XConnectAttachmentAddNarrativeResponse {
		///<summary>Optional</summary>
		public XConnectResponseStatus status;
		///<summary>Optional</summary>
		public string[] messages;
	...
	public class XConnectAttachmentRemoveImageResponse {
		///<summary>Optional</summary>
		public XConnectResponseStatus status;
		///<summary>Optional</summary>
		public string[] messages;
	...
	public class XConnectAttachmentSubmitResponse {
		///<summary>Optional</summary>
		public XConnectResponseStatus status;
		///<summary>Optional</summary>
		public string[] messages;
	...
	public class XConnectProvider {
		...
		public static XConnectProvider FromProvider(Provider provider,ClinicDto clinic,string electId,EnumXConnectProviderType type) {
			...
			if(type==EnumXConnectProviderType.BILLING 
				&& (clinic!=null) 
				&& (clinic.PayToAddressLine1!="")) {//Add PayToAddress for the billing provider
	...
	public enum EnumXConnectImageOrientationType {
		///<summary>0 - Left Orientation</summary>
		LEFT,
		///<summary>1 - Right Orientation</summary>
		RIGHT,
	}

	public enum EnumXConnectImageTypeCode {
		///<summary>0 - Referral Form</summary>
		B4,
		///<summary>1 - Dental Models</summary>
		DA,
		///<summary>2 - Explanation of Benefits</summary>
		DG,
		///<summary>3 - Other Attachments</summary>
		EB,
		///<summary>4 - Periodontal Charts</summary>
		OZ,
		///<summary>5 - X-Rays</summary>
		P6,
		///<summary>6 - Dental Models</summary>
		RB,
		///<summary>7 - Radiology Reports</summary>
		RR,
		///<summary>8 - Intra-Oral Photograph</summary>
		XP,
		///<summary>9 - Narrative</summary>
		[System.Runtime.Serialization.EnumMember(Value="03")]
		Item03,//Cannot be name 03 directly but will still be serialized to 03
	}

	public enum EnumXConnectDeliveryType {
		///<summary>0 - Paper Delivery</summary>
		PAPER,
		///<summary>1 - Electronic Delivery</summary>
		ELECTRONIC
	}
