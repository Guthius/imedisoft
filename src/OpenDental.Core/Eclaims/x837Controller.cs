...
namespace OpenDentBusiness.Eclaims
{
    ...
    {
        private static string RECSFileExistsMsg => "You must send your existing claims from the RECS program before you can create another batch.";
        ...
        private static string GetFileName(Clearinghouse clearinghouseClin, int batchNum, bool isAutomatic)
        {
            ...
            {
                //If attempting to use WebMD, exit if client program does not exist.  WebMD not allowed for Thinfinity or AppStream
                if (!File.Exists(clearinghouseClin.ClientProgram))
                {
                    MessageBox.Show("The ChangeHealthcare program (WebMD) is not installed at the expected path. Go to Setup, Family/Insurance, Clearinghouses, and double-click " + clearinghouseClin.Description + " to confirm the Launch Client Program path.");
                    return "";
                }
            }
            ...
            if (!Directory.Exists(saveFolder))
            {
                if (!isAutomatic)
                {
                    var message = saveFolder + " not found. Attempt to create?";
            ...
            else if (clearinghouseClin.CommBridge == EclaimsCommBridge.ClaimConnect)
            {
                ...
                var filePath = Path.Combine(saveFolder, Environment.MachineName.TrimEnd(' '));
                ...
                saveFolder = filePath;
                return ODFileUtils.CombinePaths(saveFolder, "claims" + batchNum + ".txt");
            }
            else
            {
                return ODFileUtils.CombinePaths(saveFolder, "claims" + batchNum + ".txt");
        ...
        public static void Rollback(Clearinghouse clearinghouseClin, int batchNum)
        {
            ...
            else
            {
                ...
                try
                {
                    File.Delete(ODFileUtils.CombinePaths(clearinghouseClin.ExportPath, "claims" + batchNum + ".txt"));
                }
                catch (Exception ex)
                {
                }
        ...
        {
            if (clearinghouse.CommBridge == EclaimsCommBridge.ClaimConnect || clearinghouse.CommBridge == EclaimsCommBridge.EmdeonMedical)
            {
                //ClaimConnect and EmdeonMedical does everything without a client program.
                return false;
            }
        ...
        {
            error = "";
            var listProcNums = listAllProcs.Select(y => y.ProcNum).ToList();
            if (listClaimProcs.Any(x => !listProcNums.Contains(x.ProcNum)))
            {
                //claimProcs does not contain total payment rows or Canadian lab estimates.
