...
using Ionic.Zip;
using OpenDentBusiness.EmdeonITS;
...
namespace OpenDentBusiness.Eclaims
{
    public class EmdeonMedical
    {
        ...
        public static bool Launch(Clearinghouse clearinghouseClin, int batchNum, EnumClaimMedType medType)
        {
            ...
            try
            {
                ...
                var zipFileBytes = zipMemoryStream.GetBuffer();
                var zipFileBytesBase64 = Convert.ToBase64String(zipFileBytes);
                zipMemoryStream.Dispose();
                var isTest = (clearinghouseClin.ISA15 == "T");
                var messageType = (isTest ? "MCT" : "MCD"); //medical
                ...
                var response = itsws.PutFileExt(clearinghouseClin.LoginID, clearinghouseClin.Password, messageType, Path.GetFileName(batchFile), zipFileBytesBase64);
        ...
        {
            //called from FormClaimReports. clinic-level clearinghouse passed in.
            ...
            try
            {
                ...
                var listReportTypes = new List<EmdeonMedicalReportType>();
                //Also known and Institutional Claims.  Might not need this report type, but leaving here for backwards compatibility.
                listReportTypes.Add(new EmdeonMedicalReportType("HCTG", "HCDG", "HCTD", "HCDD"));
                //Might not need this report type, but leaving here for backwards compatibility.
                listReportTypes.Add(new EmdeonMedicalReportType("MCTG", "MCDG", "MCTD", "MCDD"));
                listReportTypes.Add(new EmdeonMedicalReportType("AETG", "AEVG", "AETD", "AEVD"));
                if (clearinghouseClin.IsEraDownloadAllowed != EraBehaviors.None)
                {
                    //ERA download is enabled.
                    listReportTypes.Add(new EmdeonMedicalReportType("EDT", "EDP", "EAT", "EAP"));
                }
                ...
                for (var i = 0; i < listReportTypes.Count; i++)
                {
                    ...
                    var itsws = new ITSWS();
                    itsws.Url = (isTest ? emdeonITSUrlTest : emdeonITSUrl);
                    var reportType = listReportTypes[i];
                    //Download the most up to date reports, but do not delete them from the server yet.
                    var response = itsws.GetFile(clearinghouseClin.LoginID, clearinghouseClin.Password, reportType.GetCodeForGetReport(isTest));
                    ...
                    if (response.ErrorCode == 0)
                    {
                        ...
                        var reportFileDataBase64 = response.Response;
                        var reportFileDataBytes = Convert.FromBase64String(reportFileDataBase64);
                        var reportFilePath = ODFileUtils.CreateRandomFile(clearinghouseClin.ResponsePath, ".zip");
                    ...
                    else if (response.ErrorCode != 209)
                    {
                        //Error 209 means mailbox is empty for requested report type.
    ...
    internal class EmdeonMedicalReportType(string testGetCode, string productionGetCode, string testDeleteCode, string productionDeleteCode)
    {
        ///<summary>Code to retrieve reports from the test web service.</summary>
        public string TestGetCode = testGetCode;

        ///<summary>Code to retrieve reports from the live web service.</summary>
        public string ProductionGetCode = productionGetCode;

        ///<summary>Code to remove reports from the test web service.</summary>
        public string TestDeleteCode = testDeleteCode;

        ///<summary>Code to remove reports from the live web service.</summary>
        public string ProductionDeleteCode = productionDeleteCode;
