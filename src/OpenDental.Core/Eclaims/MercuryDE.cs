        ...
        public static bool Launch(Clearinghouse clearinghouseClin, int batchNum, IODProgressExtended progress = null)
        {
            ...
            try
            {
                ...
                if (batchNum == 0)
                {
                    ...
                    var retrievePath = "/" + rootFolderName + "/Out/997/";
                    var fileList = ch.ls(retrievePath);
                    for (var i = 0; i < fileList.Count; i++)
                    {
                        ...
                        var exportFilePath = ODFileUtils.CombinePaths(clearinghouseClin.ResponsePath, getFileName);
                        ...
                        var archiveFilePath = retrievePath + "Archive/" + getFileName;
                        try
                        {
                ...
                else
                {
                    ...
                    {
                        var percent = (i / files.Length) * 100;
                        //We re-use the bar again for importing later, hence the tag.
                        ...
                        var accountNumber = _clearinghouseClin.ISA08;
                        var dateTimeStr = DateTime.Now.ToString("yyyyMMddhhmmss");
                        var remoteFileName = accountNumber + dateTimeStr + i.ToString().PadLeft(3, '0') + ".837D.txt";
                        var remoteTempFilePath = "/" + rootFolderName + "/In/837D/Temp/" + remoteFileName;
                        ...
                        var remoteFilePath = "/" + rootFolderName + "/In/837D/" + remoteFileName;
