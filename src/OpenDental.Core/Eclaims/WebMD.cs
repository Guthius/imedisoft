...
namespace OpenDentBusiness.Eclaims {
	public class WebMD {
		...
		private static void SubmitBatch(Clearinghouse clearinghouseClin,int batchNum) {//not called from anywhere currently.
			...
			for(var i=0;i<files.Length;i++) {
				...
				try {
					...
					zip.Save(ms);
					var fileTextZippedBase64=Convert.ToBase64String(ms.GetBuffer());
					var fi=new FileInfo(files[i]);
					var claimXML="<?xml version=\"1.0\" ?>"
					...
					var claimXMLbytes=Encoding.UTF8.GetBytes(claimXML);
					var myWebClient=new WebClient();
					myWebClient.Headers.Add("Content-Type","text/xml");
					var responseBytes=myWebClient.UploadData(emdeonServerUrl,claimXMLbytes);
		...
		public static string Benefits270(Clearinghouse clearhouse,string x12message) {
			...
			var retVal="";
			var url="https://eligibility.webmddental.com/WebMD_Eligibility_002/Request_002X12.aspx";
			//We used the WebClient class here, because that is what they used in their example Visual Basic code.
			var client=new WebClient();
			try {
				var postData=Encoding.ASCII.GetBytes(x12message);
				var response=client.UploadData(url,postData);
