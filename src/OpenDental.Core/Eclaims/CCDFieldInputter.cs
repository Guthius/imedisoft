...
{
    private List<CCDField> fieldList = [];
    ...
    {
        var version = message.Substring(18, 2); //Field A03
        MsgType = message.Substring(20, 2); //Field A04
        if (version == "04")
        {
            switch (MsgType)
            {
                ...
                case "07":
                    ...
                    break;
                case "08": //Eligibility Request
                    ParseEligibilityRequest_08(message);
                    break;
                case "09":
                    ParseAttachmentRequest_09(message);
                    break;
                case "11":
                ...
                case "18":
                    ...
                    break;
                case "19":
                    ParseAttachmentRequest_19(message);
                    break;
                case "21":
                    ParseEOB_21(message);
                    break;
    ...
    {
        var dictProcData = new Dictionary<int, List<CCDField>>();
        //The following list of fields is in order.
        var arrayFieldIds = new string[] {"F07", "G12", "G13", "G14", "G15", "G43", "G56", "G57", "G58", "G02", "G59", "G60", "G61", "G16", "G17"};
    ...
    {
        var listProcData = new List<List<CCDField>>();
        //The following list of fields is in order.
        //All fields are present in both version 2 and version 4 except G44.
        var arrayFieldIds = new string[] {"G18", "G19", "G20", "G44", "G21", "G22", "G23", "G24", "G25"};
        var listFields = GetFieldsByIds(arrayFieldIds); //Safe if G44 is not present.
    ...
    {
        var dictNoteData = new Dictionary<int, string>();
        //The following list of fields is in order.
        ...
        if (listFields.FirstOrDefault(x => x.FieldId == "G45") == null)
        {
            ...
            for (var i = 0; i < listFields.Count; i++)
            {
                var lineNumber = i + 1; //Line numbers are 1 based for Canadian claims.
    ...
    {
        var loadedFields = new CCDField[fieldList.Count];
    ...
    {
        var field = new CCDField(fieldId, MsgType, isVersion2);
        var len = field.GetRequiredLength(this);
        if (len >= 0 && message != null && message.Length < len)
        {
            //The remainder of the message (the very last line) is shorter than the field.
            if (fieldId == "G26")
            {
                //Note Text
        ...
        var substr = message.Substring(0, len);
    ...
    public string InputFields(string message, string fieldOrderStr)
    {
        ...
        for (var i = 0; i < fieldOrderStr.Length; i += 3)
        {
            var token = fieldOrderStr.Substring(i, 3);
            if (token == "###")
            {
                //Input field value by field id, then input the value number of fields with the next template.
                var valueFieldId = fieldOrderStr.Substring(i + 3, 3);
            ...
            {
                //Designates one character "For Future Use".
                message = message.Substring(1); //Truncate one leading character and continue.
            }
            else
            {
    ...
    {
        //lists are short, so just use a simple list search.
        var listFields = new List<CCDField>();
    ...
    ///<summary>Same as GetFieldsById, but gets only a single field. Returns field with empty value if there are multiple.
    ///Returns null if field was not found.</summary>
    public CCDField GetFieldById(string fieldId)
    {
        var fields = GetFieldsById(fieldId);
    ...
    private void ParseClaimAck_11(string message)
    {
        ...
        var fieldG06 = this.GetFieldById("G06");
        if (fieldG06 == null)
        {
            return; //error
        }
    ...
    private void ParseEOB_21(string message)
    {
        ...
        var fieldF06 = this.GetFieldById("F06");
        if (fieldF06 == null)
        {
            return; //error
        }
        ...
        var fieldG10 = this.GetFieldById("G10");
        if (fieldG10 == null)
        {
            return; //error	
        }
        ...
        var fieldG11 = this.GetFieldById("G11");
        if (fieldG11 == null)
        {
            return; //error
        }
    ...
    private void ParseEligibilityRequest_08(string message)
    {
        message = this.InputFields(message, "A01A02A03A04A05A06A10A07A09B01B02B03C01C11C02C17C03C05C06C07C08C09C12C18C16D01D02D03D04D10D11C19");
    }
    ...
    private void ParseAttachmentRequest_09(string message)
    {
        message = InputFields(message, "A01A02A03A04A05A06A10A07A09B01B02B07B08___C05C06C07C08F41F42F43F44");
        var fieldF44 = this.GetFieldById("F44");
        if (fieldF44 == null)
        {
            return; //error
        }
    ...
    private void ParseAttachmentRequest_19(string message)
    {
        InputFields(message, "A01A02A03A04A05A07A11B01B02G01G05G06G07G31" +
                             "###G06G08" +
                             "###G31G32");
    }
    ...
    private void ParsePaymentReconciliation_16(string message)
    {
        ...
        var fieldG37 = this.GetFieldById("G37");
        if (fieldG37 == null)
        {
            return; //error
        }
    ...
    public EtransType GetEtransType()
    {
        ...
        if (!isVersion2)
        {
            ...
            switch (msgType)
            {
                ...
                case "09":
                    return EtransType.Attachment_CA;
                case "11":
                ...
                case "19":
                    return EtransType.AttachmentAck_CA;
                case "21":
    ...
    {
        var arrayProcedureLineNumbers = this.GetFieldsById("F07");
        var arrayEligibleAmounts = this.GetFieldsById("G12");
        var arrayDeductibleAmounts = this.GetFieldsById("G13");
        var arrayEligiblePercentage = this.GetFieldsById("G14");
        var arrayBenefitAmountForTheProcedures = this.GetFieldsById("G15");
