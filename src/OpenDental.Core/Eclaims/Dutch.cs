...
public class Dutch
{
    ...
    public static bool CreateClaim(Clearinghouse clearinghouseClin, ClaimSendQueueItem queueItem, int batchNum)
    {
        ...
        var provBill = Providers.GetById(claim.ProvBill);
        ...
        var claimProcList = ClaimProcs.Refresh(pat.PatNum);
        var claimProcsForClaim = ClaimProcs.GetForSendClaim(claimProcList, claim.ClaimNum);
        var procList = Procedures.Refresh(claim.PatNum);
        Procedure proc;
        ProcedureCode procCode;
        //ProcedureCode procCode;
        for (var i = 0; i < claimProcsForClaim.Count; i++)
        {
            ...
            var strProcCode = procCode.ProcCode;
            ...
            strb.Append(Tooth.Display(proc.ToothNum) + t);
        ...
        var saveFile = ODFileUtils.CombinePaths(saveFolder, "claims" + claim.ClaimNum + ".txt");
    ...
    public static string DutchLNamePrefix(string fullLName)
    {
        ...
        var retVal = fullLName.Substring(fullLName.IndexOf(",") + 1); // van der
        retVal.TrimStart(' ');
        return retVal;
    }
    ...
    {
        var arrayFirstNames = pat.FName.Split(new char[] {'-'}, StringSplitOptions.RemoveEmptyEntries);
        var retVal = "";
        for (var i = 0; i < arrayFirstNames.Length; i++)
        {
    ...
    {
        var match = Regex.Match(address, "[0-9]+"); //find the first group of numbers
        return match.Value;
    }
