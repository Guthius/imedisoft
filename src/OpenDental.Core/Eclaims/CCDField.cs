...
namespace OpenDentBusiness.Eclaims {
	public class CCDField{
		...
		private class LengthFromAnotherField:LengthRequirement{
			...
			public override int CalcLength(CCDFieldInputter formData){
				//When the length of a field is defined by another field, the length field always comes before the corresponding field.
				//Therefore, we can always take the last instance of the length field, in case there are multiple in a loop.
				CCDField lengthField=formData.GetFieldsById(otherFieldId).LastOrDefault();
				if(lengthField==null){
					return -1;				
				}
				if(!Regex.IsMatch(lengthField.valuestr,"^[0-9]+$")){
					throw new ODException(this.ToString()+".CalcLength: Internal Error, cannot load field length from non-integer field value!");
				}
		...
		private class RangeValueRequirement:ValueRequirement {
			...
			public override bool MeetsRequirement(CCDFieldInputter formData,string value) {
				if(!Regex.IsMatch(value,"^[0-9]+$")) {
					throw new ODException(this.ToString()+".MeetsRequirements: Internal Error, cannot check range requirement against "+
		...
		private class DiscreteValuesBasedOnOtherField:ValueRequirement{
			...
			public override bool MeetsRequirement(CCDFieldInputter formData,string value) {
				...
				if(otherField==null){
					throw new ODException(this.ToString()+".MeetsRequirement: Internal error, field id "+otherFieldId+" does not exist!");
				}
		...
		///<summary>Code that identifies the CCD message type. Known as the "Transaction Code" from field "A04".</summary>
		...
		///<summary>Throws exceptions.  Create a CCD field using the field id. A field id uniquely identifies the field type, contents, etc...</summary>
			...
			if(valueRequirements.Count==0) {//Only add format requirement if does not already have specifically defined valid value(s).
				switch(format) {
					...
					case "D":
						...
						break;
					default:
						throw new ODException(this.ToString()+".CCDField: Internal error, unrecognized field format: "+format);
						break;
				}
			}
		}
		...
		private void SetValuesUsingFieldId_v4(string pFieldId){
			...
			switch(pFieldId)
			{
				...
				case "A07":
					...
					frenchFieldName="Longueur de message";
					format="N";
					if(MsgType=="09") {//Attachment Transaction Format
						lengthRequirement=new ConstLengthRequirement(7);
					}
					else {
						lengthRequirement=new ConstLengthRequirement(5);
