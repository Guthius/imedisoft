...
namespace DataConnectionBase {
	...
	public class SOut{
		...
		///<summary>Encapsulates the result with single quotes.</summary>
		...
		public static string DateT(DateTime myDateT,bool encapsulate){
			...
			var outDate="";
			try{
				outDate=myDateT.ToString("yyyy-MM-dd HH:mm:ss",CultureInfo.InvariantCulture);//new DateTimeFormatInfo());
			}
			catch(Exception ex) {
		...
		public static string Date(DateTime myDate,bool encapsulate){
			try{
				...
				var outDate=myDate.ToString("yyyy-MM-dd",new DateTimeFormatInfo());
				var frontCap="'";
				var backCap="'";
				if(encapsulate){
		...
		public static string TSpan(TimeSpan myTimeSpan) {
			...
			try {
				...
				var hours=(myTimeSpan.Days*24)+myTimeSpan.Hours;
		...
		public static string Time(TimeSpan myTimeSpan,bool encapsulate) {
			var retval=myTimeSpan.Hours.ToString().PadLeft(2,'0')+":"+myTimeSpan.Minutes.ToString().PadLeft(2,'0')+":"+myTimeSpan.Seconds.ToString().PadLeft(2,'0');
			if(encapsulate) {
				return "'"+retval+"'";
			}
			else {
		...
		///Set doUseEnUSFormat to true to use a period no matter what region, but this is only relevant when not rounding.</summary>
		...
		///<summary>Returns a string representing the passed in number rounded to the specified number of decimal places.</summary>
		public static string Double(double myDouble,int decimalPlaces) {
			//example format string "0.00##"
			if(decimalPlaces<0) {
