...
namespace OpenDentBusiness {
	public class EhrCCD {
		...
		public static string GenerateSummaryOfCare(Patient pat,out string warnings,bool canValidate=true) {
			string referralReason="Transfer of care to another provider.";
			return GenerateCCD(pat,referralReason,out warnings,canValidate:canValidate);
		}
		...
		public static string GenerateElectronicCopy(Patient pat,out string warnings) {
			string referralReason="Patient requested a copy of medical records.";
			return GenerateCCD(pat,referralReason,out warnings);
		}
		...
		public static string GeneratePatientExport(Patient pat,out string warnings) {
			string referralReason="Patient requested export.";
			return GenerateCCD(pat,referralReason,out warnings);
		}

		///<summary>Throws an exception if validation fails.</summary>
		private static string GenerateCCD(Patient pat,string referralReason,out string warnings,bool canValidate=true) {
			EhrCCD ccd=new EhrCCD();
			return ccd.GenerateCCD(pat,referralReason,true,true,true,true,true,true,true,true,true,true,true,true,null,DateTime.MinValue,out warnings,canValidate:canValidate);
		}
		...
		private string GenerateCCD(Patient pat,string referralReason,bool hasAllergy,bool hasEncounter,bool hasFunctionalStatus,bool hasImmunization,bool hasMedication,
			...
			DateTime date,out string warnings,bool canValidate=true) 
		{
			Medications.RefreshCache();
			string strErrors="";
			List<string> listWarnings=new List<string>();
			if(canValidate) {
				strErrors=ValidateAll(pat,date,listWarnings);
			}
			else {
				if(pat==null) {
					pat=new Patient();
			...
			using(_w=XmlWriter.Create(strBuilder,xmlSettings)) {
				...
				strPracticePhone=TelephoneNumbers.ReFormat(strPracticePhone);
				...
				if(true && _patOutCcd.ClinicNum!=0) {
					var clinicCustodian=Clinics.GetClinic(_patOutCcd.ClinicNum);
					...
					custodianPhone=clinicCustodian.PhoneNumber;//Validated
				}
				...
				if(!provLegal.IsNotPerson) {
					...
					if(true && _patOutCcd.ClinicNum!=0) {
						var clinicAuth=Clinics.GetClinic(_patOutCcd.ClinicNum);
						...
						legalAuthPhone=clinicAuth.PhoneNumber;//Validated
					}
...
=====================================================================================================");
				...
				if(!canValidate) {
					List<string> listIgnoredWarnings=new List<string>();//We only care about errors in this context.
					...
					if(!string.IsNullOrWhiteSpace(ValidateProblem(pat,listIgnoredWarnings))) {
						hasProblem=false;
					}
...
=====================================================================================================");
		...
		///<summary>Does validation on the filtered lists. NEEDS TO BE ENHANCED.
		...
		///<summary>Does validation on the filtered list. NEEDS TO BE ENHANCED.
		///The listWarnings cannot be null. Adds warnings to listWarnings. Warnings can be duplicated.</summary>
		private static string ValidateProblem(Patient pat,List<string> listWarnings) {
			FilterProblem(pat);
			for(int i=0;i<_listProblemsFiltered.Count;i++) {
				DiseaseDef diseaseDef=DiseaseDefs.GetItem(_listProblemsFiltered[i].DiseaseDefNum);//Uses cache.
				if(string.IsNullOrEmpty(diseaseDef?.SnomedCode)) {
					//It is OK to add duplicate warnings here, because duplicates are removed later.
					listWarnings.Add(Lans.g("EhrCCD","Problem missing SNOMED CT Code")+": "+diseaseDef.DiseaseName+(diseaseDef.IsHidden?" (hidden)":""));
				}
			}
