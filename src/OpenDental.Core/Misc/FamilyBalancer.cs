...
public class FamilyBalancer
{
    ...
    public readonly ConcurrentQueue<OutputMessage> ConcurrentQueueOutputMessages = new();
    ...
    private void FillGuarantorQueue(FamilyBalancerOptions familyBalanceOptions)
    {
        if (ConcurrentQueueGuarantors == null)
        {
            ...
            else if (familyBalanceOptions.UseDateChangedSince)
            {
                //Families with financial data changed since a specified date.
...
public class OutputMessage(LogLevel logLevel, string text)
{
