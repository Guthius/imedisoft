...
public class PayPlanCharge : TableBase
{
    ...
    public bool IsCreditAdjustment =>
        (ChargeType == PayPlanChargeType.Credit && LinkType == PayPlanLinkType.None && CompareDouble.IsLessThanZero(Principal)) ||
        (ChargeType == PayPlanChargeType.Credit && LinkType == PayPlanLinkType.Adjustment);

    public bool IsDebitAdjustment =>
        (ChargeType == PayPlanChargeType.Debit && LinkType == PayPlanLinkType.None && CompareDouble.IsLessThanZero(Principal)) ||
        (ChargeType == PayPlanChargeType.Debit && LinkType == PayPlanLinkType.Adjustment);

    public PayPlanCharge Copy()
    {
        return (PayPlanCharge) MemberwiseClone();
    }
}
...
public enum PayPlanChargeType
{
    ...
    /// <summary>
    /// The production can be either Procedure, Adjust, or Ortho. Only used for PPv2, not DPPs.
    /// </summary>
