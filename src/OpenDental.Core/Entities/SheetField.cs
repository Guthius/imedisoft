...
public class SheetField : TableBase
{
    ...
    public Rectangle Bounds
    {
        get
        {
            ...
            var arrayCoords = FieldValue.Split([";"], StringSplitOptions.RemoveEmptyEntries);
            foreach (var p in arrayCoords)
            {
                var listXYCoords = p.Split(',').Select(x => SIn.Int(x)).ToList();
                listPoints.Add(new Point(listXYCoords[0], listXYCoords[1]));
            }

            if (listPoints.Count > 0)
            {
                var xMin = listPoints.Min(point => point.X);
                var yMin = listPoints.Min(point => point.Y);
                var width = listPoints.Max(point => point.X) - xMin;
                var height = listPoints.Max(point => point.Y) - yMin;
    ...
    {
        get { return new RectangleF(XPos, YPos, Width, Height); }
    }
