...
public class ApptReminderRule : TableBase {
	...
	///<summary>Used only for serialization purposes</summary>
	[XmlElement("TimeSpanMultipleInvites",typeof(long))]
	public long TimeSpanMultipleInvitesXML {
		get {
			return TimeSpanMultipleInvites.Ticks;
		}
		set {
			TimeSpanMultipleInvites=TimeSpan.FromTicks(value);
		}
	}
	...
	public bool IsAfterApptScheduled {
		get {
			return IsValidDuration && TypeCur.In(ApptReminderType.ScheduleThankYou,ApptReminderType.NewPatientThankYou);
		}
	}
	...
	public ApptReminderRule Copy() {
		ApptReminderRule retVal = (ApptReminderRule)MemberwiseClone();
		return retVal;
	}
...
public enum ShortCodeTypeFlag {
	...
	[ShortCode(SmsMessageSource=new[] { SmsMessageSource.ConfirmationRequest,SmsMessageSource.ConfirmationAutoReply }
		,EServicePrefNames=new[] { PrefName.ApptConfirmAutoSignedUp,PrefName.ApptConfirmAutoEnabled })]
	Confirmations=0b1,
	[ShortCode(SmsMessageSource = new[] { SmsMessageSource.Recall,SmsMessageSource.RecallAuto }
		//WebSchedRecall is determined to be enabled or not via a web call.
		,EServicePrefNames=new[] { PrefName.WebSchedAutomaticSendTextSetting }
		,PrefIsEnabledValues=new[] { (int)WebSchedAutomaticSend.SendToText }
		,EnabledValueType=typeof(WebSchedAutomaticSend))]
	WebSchedRecall = 0b10,
	[ShortCode(SmsMessageSource = new[] { SmsMessageSource.Reminder }
		,EServicePrefNames=new[] { PrefName.ApptRemindAutoEnabled })]
	Reminders = 0b100,
	[ShortCode(SmsMessageSource = new[] { SmsMessageSource.ApptThankYou }
		,EServicePrefNames=new[] { PrefName.ApptThankYouAutoEnabled })]
	ThankYous = 0b1000,
	[ShortCode(SmsMessageSource = new[] { SmsMessageSource.WebSchedASAP }
		,EServicePrefNames=new[] { PrefName.WebSchedAsapEnabled })]
	WebSchedASAP = 0b10000,
	[ShortCode(SmsMessageSource = new[] { SmsMessageSource.Verify }
		,EServicePrefNames = new[] { PrefName.WebSchedVerifyASAPType,PrefName.WebSchedVerifyNewPatType,PrefName.WebSchedVerifyRecallType }
		,PrefIsEnabledValues=new[] { (int)WebSchedVerifyType.Text,(int)WebSchedVerifyType.TextAndEmail }
		,EnabledValueType=typeof(WebSchedVerifyType))]
	WebSchedVerify = 0b100000,
	[ShortCode(SmsMessageSource = new[] { SmsMessageSource.VerifyWSNP }
		,EServicePrefNames=new[] { PrefName.WebSchedNewPatDoAuthText })]
	WebSchedNewPat = 0b1000000,
	[ShortCode(SmsMessageSource = new[] { SmsMessageSource.Statements }
		//BillingDefaultsModesToText is a comma separated string, PIn.Bool() will return false if empty, true otherwise.
		,EServicePrefNames=new[] { PrefName.BillingDefaultsModesToText })]
	BillingStatements = 0b10000000,
	[ShortCode(SmsMessageSource=new[] { SmsMessageSource.Confirmation }
		//NotApplicable means only Texting is required for this eService 
		,EServicePrefNames=new[] { PrefName.NotApplicable })]
	ManualConfirmations=0b100000000,
	[ShortCode(SmsMessageSource=new[] { SmsMessageSource.AsapManual }
		//NotApplicable means only Texting is required for this eService 
		,EServicePrefNames=new[] { PrefName.NotApplicable })]
	ASAPManual=0b1000000000,
	[ShortCode(SmsMessageSource=new[] { SmsMessageSource.Arrival }
		,EServicePrefNames=new[] { PrefName.ApptConfirmAutoSignedUp,PrefName.ApptConfirmAutoEnabled })]
	Arrivals=0b10000000000,
	[ShortCode(SmsMessageSource=new[] { SmsMessageSource.GeneralMessage }
		,EServicePrefNames=new[] { PrefName.ApptGeneralMessageAutoEnabled })]
	GeneralMessage=0b100000000000,
}

public class ShortCodeAttribute:Attribute {
	private SmsMessageSource[] _smsMessageSource=new[] { Entities.SmsMessageSource.Undefined };
	private PrefName[] _eServicePrefNames=new[] { PrefName.NotApplicable };
