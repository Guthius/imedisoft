...
public class Benefit : TableBase, IComparable
{
    ...
    private string ToStringHelper(bool isLeadingIncluded = false)
    {
        ...
        {
            retVal += ", " + CoverageLevel; //EB02: Coverage level code. Situational
        }

        var ebenCat = CovCats.GetEbenCat(CovCatNum);
        if (ebenCat != EbenefitCategory.None)
        {
            retVal += ", " + ebenCat; //EB03: Service type code. Situational
        }
        ...
        {
            retVal += ", " + TimePeriod; //EB06: Time period qualifier. Situational
        }
        ...
        if (Percent != -1)
        {
            ...
            retVal += ", " + leadingStr + Percent + "%"; //EB08: Percent. Situational
        }

        if (QuantityQualifier != BenefitQuantity.None)
        {
            retVal += ", " + QuantityQualifier; //EB09: Quantity qualifier. Situational
        }

        if (Quantity != 0)
        {
            retVal += ", " + Quantity; //EB10: Quantity. Situational
        }
        ...
        if (CodeNum != 0)
        {
            var proccode = ProcedureCodes.GetProcCode(CodeNum);
    ...
    public string GetDisplayMonetaryAmt()
    {
        ...
        var amount = Math.Round(MonetaryAmt, 2); //Handles partial cents.
        //E.X. 4.25 => {"4","25"} OR 4.00 => {"4"}
        var listVals = amount.ToString().Split('.').ToList();
        var count = listVals.Count() > 1 ? listVals.ElementAt(1).Length : 0;
        if (count == 0)
        {
            //No decimals
    ...
    public int CompareTo(object obj)
    {
        ...
        {
            var codeGroup = CodeGroups.GetOne(CodeGroupNum);
            var codeGroupBen = CodeGroups.GetOne(ben.CodeGroupNum);
            var itemOrder = codeGroup?.ItemOrder ?? -1;
            var itemOrderBen = codeGroupBen?.ItemOrder ?? -1;
        ...
        var listTimePeriods = new List<BenefitTimePeriod> {BenefitTimePeriod.CalendarYear, BenefitTimePeriod.ServiceYear};
    ...
    public FrequencyOptions GetFrequencyOption()
    {
        ...
        if (TimePeriod == BenefitTimePeriod.ServiceYear || TimePeriod == BenefitTimePeriod.CalendarYear)
        {
            return FrequencyOptions._PerBenefitYear;
        }

        if (QuantityQualifier == BenefitQuantity.Months)
        {
            return FrequencyOptions.Every_Months;
        }

        if (TimePeriod == BenefitTimePeriod.NumberInLast12Months)
        {
