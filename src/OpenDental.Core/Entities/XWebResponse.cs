...
public class XWebResponse : TableBase {
	...
	public XWebTransactionType XTransactionType {
		get {
			...
			if(!Enum.TryParse(TransactionType,out xWebTransactionType)) {
	...
	public string GetFormattedNote(bool keepAmountPositive,bool doIncludeNote=true,bool isErrorMessage=false) {
		...
		string retVal="";
		if(!isErrorMessage) {
			retVal=Lans.g("Amount:")+" "+(keepAmountPositive ? Amount : -Amount).ToString("f")+"\r\n"
			       +Lans.g("Card Number:")+" "+MaskedAcctNum+"\r\n"
			       +Lans.g("Transaction ID:")+" "+TransactionID+"\r\n";
		}
		retVal+=Lans.g("Response:")+" "+ResponseDescription+"\r\n"
		        +Lans.g("Processed:")+" "+dateTimeProcessed.ToShortDateString()+" "+dateTimeProcessed.ToShortTimeString()+"\r\n"
		        +(doIncludeNote?Lans.g("Note:")+" "+PayNote:"");
		return retVal;
	}
}
