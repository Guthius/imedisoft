...
public class CreditCard : TableBase
{
    ...
    public bool IsXWeb()
    {
        return CCSource is CreditCardSource.XWeb or CreditCardSource.XWebPortalLogin or CreditCardSource.XWebPaymentPortal or CreditCardSource.XWebPaymentPortalGuest;
    }

    public bool IsPayConnectPortal()
    {
        return CCSource is CreditCardSource.PayConnectPortal or CreditCardSource.PayConnectPortalLogin;
    }

    public bool IsPaySimpleACH()
    {
        return CCSource is CreditCardSource.PaySimpleACH or CreditCardSource.PaySimplePaymentPortalACH;
    }

    public string GetTokenString()
    {
        if (!Programs.HasMultipleCreditCardProgramsEnabled() || (string.IsNullOrEmpty(XChargeToken) && string.IsNullOrEmpty(PayConnectToken) && string.IsNullOrEmpty(PaySimpleToken)))
        {
            if (IsPaySimpleACH())
            {
                return "(" + Lans.g("ACH") + ")";
        ...
        {
            tokens.Add(CCSource is CreditCardSource.EdgeExpressRCM or CreditCardSource.EdgeExpressCNP or CreditCardSource.EdgeExpressPaymentPortal 
                ? "EdgeExpress" : "XCharge");
        }

        if (!string.IsNullOrEmpty(PayConnectToken))
        {
            tokens.Add("PayConnect" + (IsPayConnectPortal() ? " Portal" : ""));
        }

        if (!string.IsNullOrEmpty(PaySimpleToken))
        {
            tokens.Add("PaySimple" + (IsPaySimpleACH() ? " " + Lans.g("ACH") : ""));
        }
