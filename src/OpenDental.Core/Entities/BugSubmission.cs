...
public class BugSubmission:TableBase {
	...
	public List<string> ListOdExceptionLines {
		get {
			if(_listOdExceptionLines==null) {
				...
				List<string> listExceptionLines=SimplifiedStackTrace.Split('\n').ToList();
	...
	[XmlIgnore,JsonIgnore]
	[CrudColumn(IsNotDbColumn=true)]
	private string _hashedStackTrace;
	...
	public string HashedStackTrace{
		get{
			if(_hashedStackTrace==null) {
				...
				_hashedStackTrace=Convert.ToBase64String(algorithm.ComputeHash(Encoding.Unicode.GetBytes(ExceptionStackTrace)));
			}
	...
	[XmlIgnore,JsonIgnore]
	[CrudColumn(IsNotDbColumn=true)]
	private string _hashedSimpleStackTrace;
	...
	public string HashedSimpleStackTrace{
		get{
			if(_hashedSimpleStackTrace==null) {
				HashAlgorithm algorithm=SHA256.Create();
				_hashedSimpleStackTrace=Convert.ToBase64String(algorithm.ComputeHash(Encoding.Unicode.GetBytes(SimplifiedStackTrace)));
			}
	...
	public List<MatchedBugInfo> ListPendingFixBugInfos {
		get { 
			return ListMatchedBugInfos.FindAll(x => x.BugId > 0 && x.VersionsFixed.IsNullOrEmpty());
		}
	}
	...
	public List<MatchedBugInfo> ListFixedBugInfos {
		get { 
			return ListMatchedBugInfos.FindAll(x => !x.VersionsFixed.IsNullOrEmpty());
		}
	}
	...
	public BugSubmission Copy() {
		return (BugSubmission)MemberwiseClone();
	}
	...
	private string GetDbInfoJSON(long patNum,string moduleName) {
		...
		ODException.SwallowAnyException(() => {
			...
			List<PrefName> listPrefs=new List<PrefName> {
				...
				PrefName.ReadOnlyServerCompName,
				PrefName.ReadOnlyServerDbName,
				PrefName.ReadOnlyServerMySqlUser,
				PrefName.ReadOnlyServerMySqlPassHash,
				PrefName.ReadOnlyServerURI
			};
		...
		ODException.SwallowAnyException(() => { _info.IsOfficeOnReplication=false; });
		ODException.SwallowAnyException(() => { _info.IsOfficeUsingMiddleTier=false; });
	...
	public MatchedBugInfo GetPertinentFixedVersion(Version programVersionOverride=null) {
		...
		if(programVersionOverride==null){
			programVersionOverride=new Version(ProgramVersion);
		}
		foreach(MatchedBugInfo bugInfo in ListFixedBugInfos) {
	...
	public class SubmissionInfo {
		...
		///<summary>Like E-Clipboard or ODMobile. (Enum ApplicationTarget: 0=None,1=CheckIin,2=MobileWeb)</summary>
