...
public class EmailMessage : TableBase
{
    ...
    {
        var e = (EmailMessage) MemberwiseClone();
    ...
    public bool IsAutomated => GetAutomatedTypes().Contains(MsgType);
    ...
    public static List<EmailMessageSource> GetMsgTypesForEmailClient(bool isAutomated = false)
    {
        if (isAutomated)
        {
            return GetAutomatedTypes();
        }

        //Manual message sources
        var listSourcesToExclude = GetAutomatedTypes(); //Exclude automated message sources
        ...
        return Enum.GetValues(typeof(EmailMessageSource))
            ...
            .Where(x => !listSourcesToExclude.Contains(x)).ToList();
    }
}
