...
public class TsiTransLog:TableBase {
	...
	public TsiTransLog Copy() {
		TsiTransLog retval=(TsiTransLog)MemberwiseClone();
		retval._dictTransByType=new Dictionary<Tuple<TsiFKeyType, long>, TsiTrans>();
		foreach(Tuple<TsiFKeyType,long> key in DictTransByType.Keys) {
			retval._dictTransByType[key]=DictTransByType[key].Copy();
		}
		return retval;
	}
}
