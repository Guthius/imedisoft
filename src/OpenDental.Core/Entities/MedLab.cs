...
public class MedLab:TableBase {
	...
	public bool IsPreliminaryResult {
		get {
			if(ResultStatus==ResultStatus.P) {
				return true;
			}
			List<MedLabResult> listResults=MedLabResults.GetForLab(MedLabNum);
			...
			string[] pidFields=OriginalPIDSegment.Split(new[] { "|" },StringSplitOptions.None);
			if(pidFields.Length<19) {
				return false;
			}
			string[] components=pidFields[18].Split(new[] { "^" },StringSplitOptions.None);
