...
public class PayPlanCrud
{
    ...
    public static DataTable ListToTable(List<PayPlan> listPayPlans, string tableName = "")
    {
        ...
        foreach (var payPlan in listPayPlans)
            table.Rows.Add(SOut.Long(payPlan.PayPlanNum), SOut.Long(payPlan.PatNum), SOut.Long(payPlan.Guarantor), SOut.DateTime(payPlan.PayPlanDate, false), SOut.Double(payPlan.APR), payPlan.Note, SOut.Long(payPlan.PlanNum), SOut.Double(payPlan.CompletedAmt), SOut.Long(payPlan.InsSubNum), SOut.Int((int) payPlan.PaySchedule), SOut.Int(payPlan.NumberOfPayments), SOut.Double(payPlan.PayAmt), SOut.Double(payPlan.DownPayment), SOut.Bool(payPlan.IsClosed), payPlan.Signature, SOut.Bool(payPlan.SigIsTopaz), SOut.Long(payPlan.PlanCategory), SOut.Bool(payPlan.IsDynamic), SOut.Int((int) payPlan.ChargeFrequency), SOut.DateTime(payPlan.DatePayPlanStart, false), SOut.Bool(payPlan.IsLocked), SOut.DateTime(payPlan.DateInterestStart, false), SOut.Int((int) payPlan.DynamicPayPlanTPOption), SOut.Long(payPlan.MobileAppDeviceNum), payPlan.SecurityHash, SOut.Long(payPlan.SheetDefNum));
        return table;
    }
