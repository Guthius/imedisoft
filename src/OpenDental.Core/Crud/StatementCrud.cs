...
public class StatementCrud
{
    ...
    public static DataTable ListToTable(List<Statement> listStatements, string tableName = "")
    {
        ...
        foreach (var statement in listStatements)
            table.Rows.Add(SOut.Long(statement.StatementNum), SOut.Long(statement.PatNum), SOut.Long(statement.SuperFamily), SOut.DateTime(statement.DateSent, false), SOut.DateTime(statement.DateRangeFrom, false), SOut.DateTime(statement.DateRangeTo, false), statement.Note, statement.NoteBold, SOut.Int((int) statement.Mode_), SOut.Bool(statement.HidePayment), SOut.Bool(statement.SinglePatient), SOut.Bool(statement.Intermingled), SOut.Bool(statement.IsSent), SOut.Long(statement.DocNum), SOut.DateTime(statement.DateTStamp, false), SOut.Bool(statement.IsReceipt), SOut.Bool(statement.IsInvoice), SOut.Bool(statement.IsInvoiceCopy), statement.EmailSubject, statement.EmailBody, SOut.Bool(statement.IsBalValid), SOut.Double(statement.InsEst), SOut.Double(statement.BalTotal), SOut.Int((int) statement.StatementType), statement.ShortGUID, statement.StatementURL, statement.StatementShortURL, SOut.Int((int) statement.SmsSendStatus), SOut.Int((int) statement.LimitedCustomFamily));
        return table;
    }
