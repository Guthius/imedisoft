...
/*Jordan is the only one allowed to edit this class.
...
using OpenDental.UI;
	...
	private void FillGrid(){
		...
		GridColumn gridColumn=new GridColumn(Lan.g("Table...","colname"),100);
		gridMain.Columns.Add(gridColumn);
		gridColumn=new GridColumn(Lan.g("Table...","colname"),100,HorizontalAlignment.Center);
		gridMain.Columns.Add(gridColumn);
		gridColumn=new GridColumn(Lan.g("Table...","colname"),100);
		gridMain.Columns.Add(gridColumn);
		...
		for(int i=0;i<_list.Count;i++){
			...
			gridRow.Cells.Add("");
			gridRow.Cells.Add(new GridCell(""){ColorBackG=.., etc});//this is not allowed
		  GridCell gridCell=new GridCell("");//instead of above line
			gridCell.ColorBackG=...;
...
public class GridOD:System.Windows.Forms.UserControl {
	...
	///<summary>A func that defines how to create a gridrow row from a datarow. This allows JIT conversion of each row.</summary>
	...
	private static SolidBrush _brushHeaderBackground=new SolidBrush(Color.FromArgb(223,234,245));//#DFEAF5
	...
	//Pens---------------------------------------------------------------------------------------------------
	///<summary>Review.  Also separates column headers and is line below headers.  Also used in a few other places where darker line needed.  Maybe change name _penGridlineDark?</summary>
	private static Pen _penColumnSeparator=new Pen(Color.FromArgb(120,120,120));//	#787878
	///<summary>Seems to only be used once as the line between title and headers.</summary>
	private static Pen _penGridInnerLine=new Pen(Color.FromArgb(102,102,122));//	#66667A
	private static Pen _penGridline=new Pen(Color.FromArgb(220,220,220));
	private static Pen _penOutline=new Pen(Color.FromArgb(77,100,147));//#4d6493
	...
	public int[] SelectedIndices {
		get {
			...
			var intArray=_listSelectedIndices.ToArray();
			return intArray;
		}
	}		
	...
	public void Export(string fileName) {
		var selectedFilePath=ODFileUtils.CombinePaths(Path.GetTempPath(),fileName);
		...
		try {
			using(var sw=new StreamWriter(selectedFilePath,false)) {
				var line="";
				...
				sw.WriteLine(line);
				for(var i = 0;i<ListGridRows.Count;i++) {
					line="";
	...
	///<summary>This is designed for a very specific scenario with no horizontal scrollbar and all column widths defined.  Use this method to get the ideal width of the grid, based on column widths.  This method does not take into account current grid width at all, preventing circular logic.</summary>
	...
	public int PrintPage(Graphics g,int pageNumber,Rectangle bounds,int marginTopFirstPage,bool HasHeaderSpaceOnEveryPage=false) {
		...
		//g.		
		var scaleTotal=_scaleMS*_zoomLocal;//example 1.5*1.2
		if(scaleTotal!=1) {
			_printWidth=(int)(Width/scaleTotal);//calc unscaled Width
		}
		...
		var scaleMS=_scaleMS;
		var zoomLocal=_zoomLocal;
		SetScaleAndZoom(1,1);
		...
		RectangleF textRect;
		var font=_fontCell;//do not dispose of this font ref.
		...
		var isFirstRowOnPage=true;//helps with handling a very tall first row
		...
		var gridPen=_penGridline;
		var lowerPen=new Pen(_penGridline.Color);
		...
		while(_printedRows<ListGridRows.Count) {
			...
			var noteW=0;
			_stringFormat.Alignment=StringAlignment.Near;
			for(var i=NoteSpanStart;i<=NoteSpanStop;i++) {
			...
			#region PrintPartOfNote
				...
				if(noteHeight<15) {
					...
					SetScaleAndZoom(scaleMS,zoomLocal);//Reset rows and columns back to current scale.
	...
	///<summary>This is an alternative to setting up all of the GridRows in advance.  Set the data here as a List of DataRows.  Then, set FuncConstructGridRow to do the work of filling one DataRow.  DataRows will only be filled as they become visible for the first time or in the background, resulting in massive scalability and good speed.</summary>
	public void SetData(List<DataRow> listDataRows)  {
		...
		for(var i=0;i<listDataRows.Count;i++){
			...
			//FuncConstructGridRow(_listDataRows[i]);//this part happens later
	...
	protected override void OnPaint(System.Windows.Forms.PaintEventArgs e) {
		...
		if(!string.IsNullOrEmpty(stringException)) {
			var exceptionFont=new Font(FontFamily.GenericSansSerif,16,FontStyle.Bold);
			var rectangle=new RectangleF(0,OriginY(),Width,Height-OriginY());
			var exceptionBrush=new SolidBrush(Color.Red);
			g.DrawString(stringException,exceptionFont,exceptionBrush,rectangle,_stringFormat);	
		}
	}

	///<summary>Draws a solid whitish background.</summary>
	...
	private void DrawRows(Graphics g) {
		...
		for(var i=0;i<ListGridRows.Count;i++) {
			...
			if(!ListGridRows[i].State.IsHeightCalculated){
				var heightBefore=ListGridRows[i].State.HeightTotal;
	...
	private void DrawRow(Graphics g, int rowI) {
		...
		var gridRow=ListGridRows[rowI];
		var top=-vScroll.Value+OriginY()+gridRow.State.YPos;//The gridline above this row. 
		var hMain=gridRow.State.HeightMain;
		var hNote=gridRow.State.HeightNote;
		var hTot=gridRow.State.HeightMain+gridRow.State.HeightNote;
		...
		if(_hoverRow==rowI){
			var r=colorRow.R-255+_colorHover.R;
			if(r<0) r=0;
			var gr=colorRow.G-255+_colorHover.G;
			if(gr<0) gr=0;
			var b=colorRow.B-255+_colorHover.B;
			if(b<0) b=0;
			colorRow=Color.FromArgb(r,gr,b);
		}
		using var brushRow=new SolidBrush(colorRow);
		...
		for(var i=0;i<gridRow.Cells.Count;i++) {
			...
			//colored row background
			//This is wrong.  We should not have been blending cell color with row color.  Cell is supposed to override row.
			...
			//hover row background gets blended to make it slightly darker, just like 247,251,253 is darker than white
			if(_hoverRow==rowI){
				var r=colorCell.R-255+_colorHover.R;
				if(r<0) r=0;
				var gr=colorCell.G-255+_colorHover.G;
				if(gr<0) gr=0;
				var b=colorCell.B-255+_colorHover.B;
				if(b<0) b=0;
				colorCell=Color.FromArgb(r,gr,b);
			}
			if(gridRow.Cells[i].IsButton) { 
				var rectangleButton=new Rectangle(-hScroll.Value+Columns[i].State.XPos+2,top+2,Columns[i].State.Width-4,hTot-4);
				var colorTop=Color.White;
				if(gridRow.Cells[i].ButtonIsPressed){
					colorTop=Color.LightGray;
				}
				using(var brushButton = new LinearGradientBrush(rectangleButton,colorTop,Color.LightGray,90)) {
					g.FillRectangle(brushButton,rectangleButton); 
				}
				var colorOutline=_colorTextDisabled;
				if(Enabled){
				...
				using(var brushCell=new SolidBrush(colorCell)) {
		...
		var penLower=new Pen(_penGridline.Color);//disposed a few pages down
		...
		for(var i=0;i<Columns.Count;i++) {
			...
			var rectangleText=new Rectangle(-hScroll.Value+Columns[i].State.XPos,top+1,Columns[i].State.Width,hMain);
			...
			if(_hasDropDowns && i==0) { //only draw the dropdown arrow in the first column of the row.
				if(gridRow.State.DropDownState== ODGridDropDownState.Up) {//arrow pointing right
					var topPoint = new PointF(-hScroll.Value+Columns[i].State.XPos+3,top+(hMain/2)-5);
					var botPoint = new PointF(topPoint.X, topPoint.Y+10);
					var rightPoint = new PointF(topPoint.X+5, topPoint.Y+5);
					...
					var leftPoint = new PointF(-hScroll.Value+Columns[i].State.XPos+1,top+(hMain/2)-2);
					var rightPoint = new PointF(leftPoint.X+10, leftPoint.Y);
					var botPoint = new PointF(leftPoint.X+5, leftPoint.Y+5);
				...
				//a small L indicator that shows that a row is a drop down child.
					...
					var topPoint = new PointF(-hScroll.Value+Columns[i].State.XPos+4,top+1);
					var midPoint = new PointF(-hScroll.Value+Columns[i].State.XPos+4,top+8);
					var rightPoint = new PointF(-hScroll.Value+Columns[i].State.XPos+9,top+8);
			...
			if(Columns[i].ImageList==null){//text
				...
				if(imageIndex != -1) {
					var img =Columns[i].ImageList.Images[imageIndex];
		...
		if(hNote>0 && NoteSpanStop>0 && NoteSpanStart<Columns.Count) {
			...
			var rectangleNote=new Rectangle(
				-hScroll.Value+Columns[NoteSpanStart].State.XPos,
				top+hMain+1,
				Columns[NoteSpanStop].State.Right-Columns[NoteSpanStart].State.XPos,
				hNote);
			_stringFormat.Alignment = StringAlignment.Near;
			using(var brush=new SolidBrush(gridRow.ColorText)){
	...
	private void DrawTitle(Graphics g) {
		...
		var addW=ScaleI(_heightTitle);
		var dividerX=Width-addW-3;
		const int dividerLineWidth=1;
		const int plusSignWidth=4;
		var brushPlusSign=Brushes.White;//new SolidBrush(ODColorTheme.GridTextBrush.Color);//cannot dispose a brush from ODColorTheme
		...
		using(var pDark=new Pen(Color.FromArgb(102,102,122))) {
	...
	private void DrawHeaders(Graphics g) {
		...
		for(var i=0;i<Columns.Count;i++) {
			...
			var rect=new RectangleF(Columns[i].State.XPos-hScroll.Value,ScaleI(_heightTitle)+1,
		...
		for(var i=0;i<Columns.Count;i++) {
			if(_sortedByColumnIdx == i) {
				var p=new PointF(-hScroll.Value+Columns[i].State.XPos+6,ScaleF(_heightTitle+_heightHeader/2f));
	...
	protected override void OnDoubleClick(EventArgs e) {
		...
		var row=PointToRow(((MouseEventArgs)e).Y);//test based on current ListGridRows
		if(row==-1) {
	...
	protected virtual void ContextMenu_Popup(object sender,EventArgs e) {
		...
		var menuItemCopy = this.ContextMenu.MenuItems.OfType<MenuItem>().FirstOrDefault(x => x.Text == "Copy Cell Text");
		...
		//Copy Rows----------------------------------------------------
		var menuItemCopyRows=this.ContextMenu.MenuItems.OfType<MenuItem>().FirstOrDefault(x => x.Text == "Copy Rows");
	...
	private void ContextMenuAddPopupLinks(object sender) {
		...
		if(ContextMenu!=null && _listMenuItemsLinks!=null) {
			...
			for(var i=_listMenuItemsLinks.Count-1;i>=0;i--) {//backward because removing
				ContextMenu.MenuItems.Remove(_listMenuItemsLinks[i]);
			}
		}
		var rowClick = PointToRow(_mouseClickLocation.Y);
		var colClick = PointToCol(_mouseClickLocation.X);
		...
		if(_mouseButtonLastPressed==MouseButtons.Right && rowClick<=this.ListGridRows.Count) {
			var gridRow = this.ListGridRows[rowClick];
			if(this.ContextMenu==null) {
	...
	private void CopyCellClick(object sender,EventArgs e) {
		try {
			var copyText = ListGridRows[_mouseDownRow].Cells[_mouseDownCol].Text;
	...
	private void CopyRowsClick(object sender,EventArgs e) {
		var listGridRowsSelected=new List<GridRow>();
		_listSelectedIndices.ForEach(x => listGridRowsSelected.Add(ListGridRows[x]));
		var stringBuilder=new StringBuilder();
		for(var i=0;i<listGridRowsSelected.Count;i++) {
			var parsedRow="";
			for(var j=0;j<listGridRowsSelected[i].Cells.Count;j++) {
	...
	protected override void OnMouseDown(MouseEventArgs e) {
		...
		switch(_selectionMode) {
			...
			case GridSelectionMode.OneRow:
				...
				var cell=ListGridRows[_mouseDownRow].Cells[_mouseDownCol];
				if(cell.IsButton) {
...
//todo: I think the comment above is wrong.  Nothing happens here because textEdit is always null before here.
//LostFocus happens before click.  That's where textEdit is cleared.
				comboBox.Visible=false;
			...
			case GridSelectionMode.MultiExtended:
				if(ControlIsDown()) {
					...
					if(_listSelectedIndices.Count==0) {
						...
						var fromRow=_listSelectedIndices[0];
						_listSelectedIndices.Clear();
						if(_mouseDownRow<fromRow) {//dragging down
							for(var i=_mouseDownRow;i<=fromRow;i++) {
								_listSelectedIndices.Add(i);
							}
						}
						else {
							for(var i=fromRow;i<=_mouseDownRow;i++) {
								_listSelectedIndices.Add(i);
							}
						}
					}
	...
	protected override void OnMouseMove(MouseEventArgs e) {
		...
		if(_mouseDownRow<curRow) {//dragging down
			for(var i=_mouseDownRow;i<=curRow;i++) {
				if(i==-1) {
					continue;
				}
			...
			for(var i=curRow;i<=_mouseDownRow;i++) {
				if(_listSelectedIndices.Contains(i)) {
					_listSelectedIndices.Remove(i);
				}
				else {
	...
	private void CreateEditBox() {
		...
		var editBoxLocationTop=-vScroll.Value+1+OriginY()+ListGridRows[_selectedCell.Y].State.YPos+ListGridRows[_selectedCell.Y].State.HeightMain+hScrollBarHeight;
		if(editBoxLocationTop > this.DisplayRectangle.Bottom) {
			var onScreenPixels=vScroll.Value+DisplayRectangle.Height-OriginY()-(ListGridRows[_selectedCell.Y].State.YPos)-hScrollBarHeight;
			var offScreenPixels=ListGridRows[_selectedCell.Y].State.HeightMain-onScreenPixels;
		...
		if(EditableUsesRTF) {
			var editRichBox=new RichTextBox();
	...
	private void editBox_NextCellRight() {
		...
		for(var i= _selectedCellOld.X+1; i < Columns.Count; i++) {
	...
	void textEdit_KeyUp(object sender,KeyEventArgs e) {
		...
		var heightText=(int)(1.03f*g.MeasureString(textEdit.Text+"\r\n",_fontCell,textEdit.Width).Height);
		g.Dispose();
	...
	protected override void OnKeyDown(KeyEventArgs e) {
		...
		if(e.KeyCode==Keys.Down) {
			if(_listSelectedIndices.Count>0 && _listSelectedIndices[0] < ListGridRows.Count-1) {
				var selectedNew=_listSelectedIndices[0]+1;
				_listSelectedIndices.Clear();
	...
	public void SheetDrawRow(int rowI,Graphics g,int x=0,int y=0,bool isBottom=false,bool isSheetGrid=false,bool isPrintingSheet=false,float scaleMS=1) {
		var fontLocal=_fontCell;//no need to dispose. Just a ref.
		...
		fontLocal=new Font(fontLocal.FontFamily,fontLocal.Size/scaleMS,fontLocal.Style);
		RectangleF textRect;
		SolidBrush textBrush;
		//selected row color
		if(_listSelectedIndices.Contains(rowI)) {
		...
		for(var i=0;i<Columns.Count;i++) {
			...
			var vertical= y - vScroll.Value+1+1;
			var horizontal= x - hScroll.Value+1+ Columns[i].State.XPos;
			var cellW= Columns[i].State.Width;
			var cellH= ListGridRows[rowI].State.HeightMain;
			...
			if(Columns[i].ImageList==null) {
				if(isPrintingSheet) {
					...
					var smallerFont = new Font(fontLocal.FontFamily,(float)(fontLocal.Size*0.96), fontLocal.Style);
		...
		//Outline the Title
		var pen=_penOutline;//does not need to be disposed.
		if(isSheetGrid) {
	...
	public void SheetDrawRowX(int rowI,XGraphics g,int x=0,int y=0,bool isBottom=false,bool isSheetGrid=false) {
		var fontNormal=new XFont(_fontCell.Name,_fontCell.Size);//There is no Dispose for xfonts, or we would.
		var fontBold=new XFont(_fontCellBold.Name,_fontCellBold.Size,XFontStyle.Bold);
		if(_fontForSheets!=null) {
		...
		XFont font;
		XRect textRect;
		var _xAlign=XStringAlignment.Near;
		...
		var gridPen=new XPen(_penGridline);
		//lines for note section
		if(ListGridRows[rowI].State.HeightNote>0) {
			//left vertical gridline
			if(NoteSpanStart!=0) {
		...
		var xPenLower=new XPen(_penGridline);
		if(rowI==ListGridRows.Count-1) {//last row
		...
		for(var i=0;i<Columns.Count;i++) {
			...
			var vertical=y-vScroll.Value-2;
			var horizontal=x-hScroll.Value+1+Columns[i].State.XPos;
			var cellW=Columns[i].State.Width;
			var cellH=ListGridRows[rowI].State.HeightMain;
			if(_hasEditableColumn) {//These cells are taller
		...
		//Left right and bottom lines of grid.  This creates the outline of the entire grid when not using outline control
		//Outline the Title
		var pen=new XPen(_penOutline.Color);
		if(isSheetGrid) {
	...
	public void SheetDrawTitle(Graphics g,int x,int y,float scaleMS) {
		var cTitleTop=_colorTitleTop;
		var cTitleBottom=_colorTitleBottom;
		var brushTitleBackground=new LinearGradientBrush(new Rectangle(x,y,Width,_heightTitle),cTitleTop,cTitleBottom,LinearGradientMode.Vertical);
		g.FillRectangle(brushTitleBackground,x,y,Width,_heightTitle);
		var font=new Font(_fontTitle.FontFamily,_fontTitle.Size/scaleMS,_fontTitle.Style);
		g.DrawString(_title,font,_brushTitleText,x+(Width/2-g.MeasureString(_title,font).Width/2),y+2);
		//Outline the Title
		//Draw line from LL to UL to UR to LR. top three sides of a rectangle.
	...
	public void SheetDrawTitleX(XGraphics g,int x,int y) {
		var cTitleTop=_colorTitleTop;
		var cTitleBottom=_colorTitleBottom;
		var cTitleText=_brushTitleText.Color;
		var brushTitleBackground=new LinearGradientBrush(new Rectangle(x,y,Width,_heightTitle),cTitleTop,cTitleBottom,LinearGradientMode.Vertical);
		var brushTitleText=new XSolidBrush(cTitleText);
		g.DrawRectangle(brushTitleBackground,ToPoints(x),ToPoints(y),ToPoints(Width),ToPoints(_heightTitle));
		var xTitleFont=new XFont(_fontTitle.FontFamily.ToString(),_fontTitle.Size,XFontStyle.Bold);
	...
	///<summary>If drawing to printer or pdf, pass in a scaleMS of 1. If drawing to screen, pass in the current ms scale, like 1.5.</summary>
	public void SheetDrawHeader(Graphics g,int x,int y,float scaleMS) {
		Color cOutline=cOutline=Color.Black;
		var cTitleTop=Color.White;
		var cTitleBottom=Color.FromArgb(213,213,223);
		var cTitleText=Color.Black;
		var cTitleBackG=Color.LightGray;
		...
		var font=new Font(_fontHeader.FontFamily,_fontHeader.Size/scaleMS,_fontHeader.Style);
		for(var i=0;i<Columns.Count;i++) {
			if(i!=0) {
			...
			var widthText=g.MeasureString(Columns[i].Heading, font).Width;
			var xPos=(float)x + (-hScroll.Value + Columns[i].State.XPos + Columns[i].State.Width /2f- widthText/2f);
			g.DrawString(Columns[i].Heading, font, Brushes.Black,xPos,(float)y + 1);
			if(_sortedByColumnIdx == i) {
				var p = new PointF(x + (-hScroll.Value+1+ Columns[i].State.XPos +6), y + (float)_heightHeader / 2f);
				if(_isSortedAscending) { //pointing up
					g.FillPolygon(Brushes.White,new PointF[] {
						new PointF(p.X-4.9f,p.Y+2f), //LLstub
						new PointF(p.X-4.9f,p.Y+2.5f), //LLbase
		...
		//Outline the Title
		using(var pen=new Pen(cOutline)) {
	...
	public void SheetDrawHeaderX(XGraphics g,int x,int y) {
		...
		var xHeaderFont=new XFont(_fontHeader.FontFamily.Name.ToString(),_fontHeader.Size,XFontStyle.Bold);
		for(var i=0; i < Columns.Count; i++) {
			if(i != 0) {
			...
			var xFloat=(float)x + (float)(-hScroll.Value + Columns[i].State.XPos + Columns[i].State.Width /2);//for some reason visual studio would not allow this statement within the DrawString Below.
	...
	public List<T> GetTags<T>() {
		var listTags=new List<T>();
		foreach(var row in ListGridRows) {
			if(row.Tag is T) {
	...
	private void LayoutScrolls() {
		...
		if(_hasMultilineHeaders) {//added in R12381, v15?
			...
			using var font=new Font(_fontHeader.FontFamily,UnscaleFontODZoom(_fontHeader.Size));
			for(var i=0;i<Columns.Count;i++){
				var sizeText=TextRenderer.MeasureText(Columns[i].Heading,font,new Size(Columns[i].State.Width,int.MaxValue),textFormatFlags);
				var heightThisHeader=sizeText.Height-3;
	...
	private void SetFonts(){
		...
		_fontCell=new Font(FontFamily.GenericSansSerif,ScaleFontODZoom(8.5f));//this.Font is ignored
		_fontCellBold=new Font(FontFamily.GenericSansSerif,ScaleFontODZoom(8.5f),FontStyle.Bold);
		_fontUnderline=new Font(FontFamily.GenericSansSerif,ScaleFontODZoom(8.5f),FontStyle.Underline);
		_fontUnderlineBold=new Font(FontFamily.GenericSansSerif,ScaleFontODZoom(8.5f),FontStyle.Underline | FontStyle.Bold);
		_fontHeader=new Font(FontFamily.GenericSansSerif,ScaleFontODZoom(8.5f),FontStyle.Bold);
		_fontTitle=new Font(FontFamily.GenericSansSerif,ScaleFontODZoom(10),FontStyle.Bold);
		Invalidate();
	...
	public void SortForced(int sortedByColumnIdx,bool sortedIsAscending) {
		...
		var rowsSorted=new List<GridRow>();
		for(var i=0;i<ListGridRows.Count;i++) {
		...
		BeginUpdate();
		ListGridRows.Clear();
		for(var i=0;i<rowsSorted.Count;i++) {
	...
	public bool SwapRows(int indxMoveFrom,int indxMoveTo) {
		...
		BeginUpdate();
		var dataRowFrom=ListGridRows[indxMoveFrom];
	...
	private int SortStringCompare(GridRow row1,GridRow row2) {
		var textRow1=row1?.Cells[_sortedByColumnIdx].Text??"";
	...
	private int SortDateParse(GridRow row1,GridRow row2) {
		var raw1=row1.Cells[_sortedByColumnIdx].Text;
		var raw2=row2.Cells[_sortedByColumnIdx].Text;
	...
	private int SortToothNumberParse(GridRow row1,GridRow row2) {
		...
		if(!Tooth.IsValidEntry(raw1)) {//only first invalid
			...
			var tooth1=Tooth.Parse(raw1);
			var tooth2=Tooth.Parse(raw2);
			var toothInt1=Tooth.ToInt(tooth1);
			var toothInt2=Tooth.ToInt(tooth2);
	...
	public void PerformCellDoubleClick(object sender,ODGridClickEventArgs e){
		CellDoubleClick?.Invoke(sender,e);
	}
	#endregion Methods - Perform Events
	...
	private static Color GetSelectedColor(Color colorBackground,Color colorSelected) {
		   ...
		   && colorBackground.B-colorBackground.G > 0)
		{
			...
			{
				var colorDarker=Color.FromArgb(colorSelected.R-20,colorSelected.G-20,colorSelected.B);
		...
		var newBackgroundColor=Color.FromArgb(
	...
	private static void DrawStringX(XGraphics xg,string str,XFont xfont,XBrush xbrush,XRect xbounds,XStringAlignment sa) {
		var g=Graphics.FromImage(new Bitmap(100,100));//only used for measurements.
		var topPad=0;// 2;
		var rightPad=5;//helps get measurements better.
		var scaleToPix=1d/ToPoints(1);
		...
		//pixels:
		var bounds=new Rectangle((int)(scaleToPix*xbounds.Left),
		...
		//pixels:
		var fit=new SizeF((float)(bounds.Width-rightPad),(float)(font.Height));
		var format=StringFormat.GenericTypographic;
		//pixels:
		var pixelsPerLine=(float)font.Height-0.5f;//LineSpacingForFont(font.Name) * (float)font.Height;
		float lineIdx=0;
		int chars;
		int lines;
		//points:
		RectangleF layoutRectangle;
		for(var ix=0;ix<str.Length;ix+=chars) {
			...
			//}
			//use points here:
			var adjustTextDown=10f;//this value was arrived at by trial and error.
	...
	private void ComputeColumns() {
		if(_hScrollVisible) {
			...
			if(sumDynamic>0){
				var widthExtra=Width-2-widthFixedSum;
				if(_printWidth>0){
				...
				var widthExtra=Width-2-widthFixedSum+ScaleI(Columns[Columns.Count-1].ColWidth);
				if(_printWidth>0){
	...
	private void ComputeRows(Graphics g,bool doActualCalc) {
		...
		var dt1=DateTime.Now;
		var heightEach=_fontCell.Height+1;//assume one row
		if(_hasEditableColumn) {
		...
		var dt2=DateTime.Now;
		var timeSpanHeights=dt2-dt1;//put a breakpoint below this line to look at the timespan
	...
	private void ComputeRowHeightOne(Graphics g,int i){
		...
		var fontNormal=_fontCell;//do not dispose any of these font refs.
		var fontBold=_fontCellBold;
		if(_fontForSheets!=null) {
			fontNormal=_fontForSheets;
			fontBold=_fontForSheetsBold;
		}
		for(var j=0;j<ListGridRows[i].Cells.Count;j++) {
		...
		if(WrapText) {//make row taller after measuring
			//for measurement, we need to use a scaled font (not sure why)
			var fontMeas=new Font(font.FontFamily,ScaleMS(font.Size),font.Style);
			...
			for(var j=0;j<ListGridRows[i].Cells.Count;j++) {
				if(_hasEditableColumn) {
					...
					heightCell=(int)Math.Ceiling(((1.03) 
					                              *(float)(g.MeasureString(ListGridRows[i].Cells[j].Text+"\r\n",fontMeas,Columns[j].State.Width,_stringFormat).Height))+4);//because textbox will be bigger
					//heightCell=(int)ScaleMS(heightCell);
					...
					hTemp=g.MeasureString(ListGridRows[i].Cells[j].Text,fontMeas,Columns[j].State.Width,_stringFormat).Height;
					//hTemp=ScaleMS(hTemp);
					if(ListGridRows[i].Cells[j].IsButton){
						hTemp=g.MeasureString(ListGridRows[i].Cells[j].Text,fontMeas,Columns[j].State.Width-1,_stringFormat).Height;
						//hTemp=ScaleMS(hTemp);
					}
					if(_hasDropDowns && j==0) {//only draw the dropdown arrow in the first column of the row.
						if(ListGridRows[i].State.DropDownState!=ODGridDropDownState.None || ListGridRows[i].DropDownParent!=null) {
							hTemp=g.MeasureString(ListGridRows[i].Cells[j].Text,fontMeas,Columns[j].State.Width-10,_stringFormat).Height;
							//hTemp=ScaleMS(hTemp);
							//might be easier to just not support wrap in a dropdown cell. Oh well.
							if(WrapText && Columns[j].TextAlign==HorizontalAlignment.Left){
								hTemp=g.MeasureString(ListGridRows[i].Cells[j].Text,fontMeas,Columns[j].State.Width-3,_stringFormat).Height;
								//hTemp=ScaleMS(hTemp);
							}
						}
					}
					...
					heightCell=(int)Math.Floor(hTemp);
				}
			...
			if(ListGridRows[i].State.HeightMain<4) {
				ListGridRows[i].State.HeightMain=(int)ScaleMS(font.Height);
			}
		}
			...
			if(_hasEditableColumn) {
				...
				ListGridRows[i].State.HeightMain=(int)ScaleMS(font.Height)+1;
			}
		}
		...
		if(_widthNote>0 && ListGridRows[i].Note!="") {
			var strToMeasure=ListGridRows[i].Note;
			if(strToMeasure.Length>TEXT_LENGTH_LIMIT){
				strToMeasure=strToMeasure.Substring(0,TEXT_LENGTH_LIMIT)+"\r\n"+Lans.g("...more...");
			}
			var heightNote=g.MeasureString(strToMeasure,fontNormal,_widthNote,_stringFormat).Height;//Notes cannot be bold.  Always normal font.
			heightNote=ScaleMS(heightNote);
			ListGridRows[i].State.HeightNote=(int)heightNote;
		}
		...
		if(_hasDropDowns) {
			if(ListGridRows[i].DropDownParent!=null){//if this is a child
				...
				var listParentRows=new List<GridRow>();
	...
	private void ComputeRowYposStartingAt(int startingAtIdx){
		...
		for(var i=startingAtIdx;i<ListGridRows.Count;i++) {
		...
		var dt2=DateTime.Now;
		var timeSpanYpos=dt2-dt1;//put a breakpoint below this line to look at the timespans
		_heightTotal=yPos;
	}
	...
	private void ComputeSheetSupplemental() {
		...
		var yPosCur=SheetYPos;
		var bottomCurPage=SheetPageHeight-SheetBottomMargin;
	...
	private void CreateComboBox() {
		var gridCell=ListGridRows[_selectedCell.Y].Cells[_selectedCell.X];
		var gridColumn=Columns[_selectedCell.X];
		...
		for(var i=0;i<gridColumn.ListDisplayStrings.Count;i++) {
	...
	private void DropDownRowClick(int mouseDownRow,int mouseDownY) {
		var curRow=PointToRow(mouseDownY);
	...
	private void FillLastPage(Graphics g){
		...
		var rowStart=0;//ListGridRows.Count-1;
		for(var i=ListGridRows.Count-1;i>=0;i--) {//start at the end and work backward
		...
		for(var i=rowStart;i<ListGridRows.Count;i++) {
			if(ListGridRows[i].Cells.Count==0){
				FillRow(i);	
			}
			ComputeRowHeightOne(g,i);
	...
	private void FillRow(int rowI){
		var gridRowState=ListGridRows[rowI].State.Copy();
	...
	///<summary>Scales by (most of) the OD zoom scale.</summary>
	private float ScaleFontODZoom(float val96){
		return val96*_zoomLocal;
	}

	///<summary>Scales by just the MS scale.</summary>
	private float ScaleMS(float val96){
		return val96*_scaleMS;//1.5
	}
	...
	private float UnscaleFontODZoom(float valScaled){
		return valScaled/_zoomLocal;
	}
	...
	public class GridColumnCollection{
		...
		public List<GridColumn> ToList(){
			var listGridColumns=new List<GridColumn>();
			for(var i=0;i<_listGridColumns.Count;i++){
...
///<summary>Specifies the selection behavior of a Grid.</summary>
...
//2022-05-03-Nathan and others find the grid slightly annoying when using our custom query feature.
//Suggested improvements:
//Allow changing width of column with mouse.
//If one row is selected in multi-select mode, arrow buttons should change the selection.
//If multiple rows are selected, arrow down should select only one row, which is below the top selected row.
