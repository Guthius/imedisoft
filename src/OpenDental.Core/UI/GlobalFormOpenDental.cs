...
namespace OpenDentBusiness {
	...
	public class GlobalFormOpenDental {
		...
		///<summary>This reference needs to be set to FormOpenDental on startup. This is so that our events can be invoked onto the UI thread when necessary. This reference is not a problem because the main form is always kept alive by the Application.Run loop anyway. Once the application exits, all resources including static references are cleaned up automatically.</summary>
		public static System.Windows.Forms.Control ControlMainForm;
		...
		public static void RefreshCurrentModule(bool isClinicRefresh) {
			...
			if(EventRefreshCurrentModule==null){//No subscribers
				return;
			}
