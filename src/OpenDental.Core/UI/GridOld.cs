...
///<summary>This is the GridOD as it existed before 21.3.  We can switch back to this in a single spot for debugging, if necessary.</summary>
[DefaultEvent("CellDoubleClick")]
public class GridOld:System.Windows.Forms.UserControl {
	...
	private Color _colorSelectedRow=Color.Silver;//gray 192
	...
	private void OnPageChanged() {
		...
		var firstLinkVal=(_pageCur>2 ? _pageCur-2 : -1);//When already viewing first or second page no link should ever show in first position.
		var secondLinkVal=(_pageCur>1 ? _pageCur-1 : -1);//When viewing anything other then first page there should be a be a link to previous page.
		var thirdLinkVal=(IsIndexValid(GetStartIndexForPage(_pageCur+1)) ? _pageCur+1 : -1);
		var fourthLinkVal=(IsIndexValid(GetStartIndexForPage(_pageCur+2)) ? _pageCur+2 : -1);
	...
	public ListGridColumns Columns { get; } = new ListGridColumns();
...
//todo: change name to SortingAllowByColumn, but it will affect many files
	...
	public int[] SelectedIndices {
		get {
			...
			var retVal=_selectedIndices.ToArray().Select(x => (int)x).Where(x => x>-1 && x<ListGridRows.Count).ToArray();
...
//todo: brushes aren't disposed
	...
	protected override void OnSizeChanged(EventArgs e) {
		...
		var scrollFraction=(float)vScroll.Value/vScroll.Maximum;
		LayoutScrollsAndFonts();
	...
	private void PopupHelper(object sender, EventArgs e) {
		...
		if(_mouseButtonLastPressed==MouseButtons.Right && rowClick<=this.ListGridRows.Count) {
			var row = this.ListGridRows[rowClick];
			if(this.ContextMenu==null) {
				this.ContextMenu=new ContextMenu();
				return;
			}
	...
	private void removeContextMenuLinks() {
		...
		foreach(var mi in _listMenuItemLinks) {
			...
			case GridSelectionMode.MultiExtended:
				if(ControlIsDown()) {
					...
					if(_selectedIndices.Count==0) {
						...
						var fromRow=(int)_selectedIndices[0];
						_selectedIndices.Clear();
		...
		if(_selectionMode==GridSelectionMode.OneRow) {
			if(e.KeyCode==Keys.Down) {
				if(_selectedIndices.Count>0 && (int)_selectedIndices[0] < ListGridRows.Count-1) {
					var prevRow=(int)_selectedIndices[0];
					_selectedIndices.Clear();
	...
	private void LayoutScrollsAndFonts() {
		...
		if(_hasMultilineHeaders) {
			...
			for(var i=0;i<Columns.Count;i++){
				var sizeText=TextRenderer.MeasureText(Columns[i].Heading,_fontHeader,new Size(Columns[i].State.Width,int.MaxValue),textFormatFlags);
	...
	private int SortStringCompare(GridRow row1,GridRow row2) {
		var textRow1=row1.Cells[SortedByColumnIdx].Text??"";
	...
	private int SortDateParse(GridRow row1,GridRow row2) {
		var raw1=row1.Cells[SortedByColumnIdx].Text;
		var raw2=row2.Cells[SortedByColumnIdx].Text;
	...
	private void TryFillDataToPage(int page) {
		...
		for(var pageTemp=1; pageTemp<=(page+2); pageTemp++) {
	...
	private PagingIndexState GetPagingIndexState(int index) {
		...
		var countItemsSeen=_dictPages.Values.Sum(x => x.Count);
	...
	public int PrintPage(Graphics g,int pageNumber,Rectangle bounds,int marginTopFirstPage,bool HasHeaderSpaceOnEveryPage=false) {
		...
		while(_printedRows<ListGridRows.Count) {
			...
			#region PrintPartOfNote
				...
				if(noteHeight<15) {
					SetScaleAndFont(scaleMS,zoomLocal);
	...
	private void ComputeRows(Graphics g) {
		...
		for(var i=0;i<Columns.Count;i++) {
			...
			if(Columns[i].ImageList!=null) {
				if(ScaleI(Columns[i].ImageList.ImageSize.Height)> heightImage) {
					heightImage=ScaleI(Columns[i].ImageList.ImageSize.Height)+1;
				}
			}
		}
		var yPos=0;
		for(var i=0;i<ListGridRows.Count;i++) {
			...
			if(widthNote>0 && ListGridRows[i].Note!="") {
				...
				var heightNote=g.MeasureString(strToMeasure,fontNormal,widthNote,_stringFormat).Height;//Notes cannot be bold.  Always normal font.
