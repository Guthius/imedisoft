...
public partial class SignatureBox:Control {
	...
	private float Unscale(float valScreen){
		var val96=(float)(valScreen/(_scaleMS*_zoomLocal));//example 180/(1.5*1.2)=100
		return val96;
	}
	...
	public string GetSigString(){
		...
		if(_strDigitalSig!=null){
			...
			rawString+=int.MinValue.ToString()+","+int.MinValue.ToString()+";";
			var byteArray = Encoding.UTF8.GetBytes(_strDigitalSig);
	...
	public void SetSigString(string sigString){
		...
		try{
			...
			var byteArrayEncrypted=Convert.FromBase64String(sigString);
			//create the streams
			...
			var rawString=Encoding.UTF8.GetString(byteArray);
			var strDigitalSigPrefix="";
			strDigitalSigPrefix+=int.MinValue.ToString()+","+int.MinValue.ToString()+";";
			strDigitalSigPrefix+=int.MinValue.ToString()+","+int.MinValue.ToString()+";";
			if(rawString.StartsWith(strDigitalSigPrefix)){
				var listBytes=new List<byte>();
				var listStrPoints=rawString.Split(';').ToList();
				for(var i=0;i<listStrPoints.Count;i++){
					var coords=listStrPoints[i].Split(new char[] {','});
					//We can't use floats here
					var point=new Point(Convert.ToInt32(coords[0]),Convert.ToInt32(coords[1]));
				...
				var listStrPoints=rawString.Split(new char[] {';'}).ToList();
				for(var i=0;i<listStrPoints.Count;i++){
					var coords=listStrPoints[i].Split(new char[] {','});
					var pointF=new PointF(Convert.ToSingle(coords[0]),Convert.ToSingle(coords[1]));
	...
	public Image GetSigImage(bool includeBorder){
		...
		if(IsDigitalSignature()) {
			...
			for(var i=1;i<listPointFs.Count;i++) {//skip the first point
	...
	protected override void OnMouseDown(MouseEventArgs e) {
		...
		mouseIsDown=true;
		var pointF=new PointF(Unscale(e.X),Unscale(e.Y));
		listPointFs.Add(pointF);
		//Invalidate();
	}
