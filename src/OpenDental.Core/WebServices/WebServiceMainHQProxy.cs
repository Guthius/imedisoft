...
namespace OpenDentBusiness
{
    ...
    public static class WebServiceMainHQProxy
    {
        ...
        public static EServiceSetup.SignupOut GetEServiceSetupFull(SignupPortalPermission permission, bool isSwitchClinicPref = false, EServiceSetup.SignupOut oldSignupOut = null)
        {
            ...
            clinics.AddRange(Clinics.GetDeepCopy());
            ...
            var shortCodePracticeTitle = string.IsNullOrWhiteSpace(PrefC.GetString(PrefName.ShortCodeOptInClinicTitle))
                ? PrefC.GetString(PrefName.PracticeTitle)
                : PrefC.GetString(PrefName.ShortCodeOptInClinicTitle);
            //Get all program property info we might want for HQ
            var serializedProperties = GetSerializedProgramProperties(clinics);
            var webServiceMainHq = GetWebServiceMainHQInstance();
            var signupOut = WebSerializer.ReadXml<EServiceSetup.SignupOut>
            (
                            ...
                            WebSerializer.WriteXml(new EServiceSetup.SignupIn()
                            {
                                ...
                                Clinics = clinics
                                    .Select(x => new EServiceSetup.SignupIn.ClinicLiteIn()
                                    {
                                        ...
                                        ShortCodeOptInYourDentist = ClinicPrefs.GetPref(PrefName.ShortCodeOptInClinicTitle, x.Id)?.ValueString
                                ...
                                DateTimeDentalOfficeDb = MiscData.GetNowDateTime(),
            ...
            var listPhonesHQ = EServiceSetup.SignupOut.SignupOutPhone.ToSmsPhones(signupOut.Phones);
            ...
            var smsSignups = GetSignups<EServiceSetup.SignupOut.SignupOutSms>(signupOut, eServiceCode.IntegratedTexting);
            var isSmsEnabled = false;
            //Clinics are ON so loop through all clinics and reconcile with HQ.
            var listClinicsAll = Clinics.GetDeepCopy();
            foreach (var clinicDb in listClinicsAll)
            {
                var clinicSignup =
                ...
                clinicDb.SmsContractSignedOn = clinicSignup.SmsContractDate;
                clinicDb.SmsMonthlyLimit = clinicSignup.MonthlySmsLimit;
                Clinics.Update(clinicDb);
            ...
            {
                var callfire = Programs.GetCur(ProgramName.CallFire);
                if (callfire != null)
                {
            ...
            var listEClipboardClinicNums = signupOut.EServices.Where(x => x.EService == eServiceCode.EClipboard && x.IsEnabled)
                .Select(x => x.ClinicNum).ToList();
            var clinicNumsEClipboard = string.Join(",", listEClipboardClinicNums);
            ...
            var listOCRScanClinicNums = signupOut.EServices.Where(x => x.EService == eServiceCode.OCR && x.IsEnabled).Select(x => x.ClinicNum).ToList();
            var clinicNumsOCR = string.Join(",", listOCRScanClinicNums);
            if (Prefs.UpdateString(PrefName.OCRClinicsSignedUp, clinicNumsOCR))
            {
                //HQ didn't match the local pref.
                isCacheInvalid = true;
                SecurityLogs.MakeLogEntry(EnumPermType.Setup, 0, $"OCR clinics signed up changed. Local pref set to {clinicNumsOCR}");
            }
            ...
            var listMobileWebClinicNumsBefore = MobileAppDevices.GetClinicSignedUpForMobileWeb();
            //Capture list of subscribers after sync.
            var listMobileWebClinicNumsAfter = signupOut.EServices.Where(x => x.EService == eServiceCode.MobileWeb && x.IsEnabled)
                .Select(x => x.ClinicNum).ToList();
            ...
            var clinicNumsMobileWeb = string.Join(",", listMobileWebClinicNumsAfter);
            ...
            var patientPortalURL = PrefC.GetString(PrefName.PatientPortalURL);
            var hostedURL = signupOut.EServices.FirstOrDefault(x => x.EService == eServiceCode.PatientPortal && x.ClinicNum == 0)?.HostedUrl ?? "";
            ...
            {
                Prefs.UpdateString(PrefName.PatientPortalURL, hostedURL);
                isCacheInvalid = true;
            }
        ...
        private static bool SyncHostedEmail(EServiceSetup.SignupOut signupOut, IWebServiceMainHQ webServiceMainHq)
        {
            ...
            var listClinicNumsNeedingSignups = new List<long>();
            var listClinicPrefsMassEmail = ClinicPrefs.GetPrefAllClinics(PrefName.MassEmailStatus, true).ToList();
            var listClinicPrefsSecureEmail = ClinicPrefs.GetPrefAllClinics(PrefName.EmailSecureStatus, true).ToList();
            var listSignupsMassEmail = signupOut.EServices.Where(x => x.EService == eServiceCode.EmailMassUsage).ToList();
            var listSignupsSecureEmail = signupOut.EServices.Where(x => x.EService == eServiceCode.EmailSecureAccess).ToList();
            ...
            foreach (var clinicNum in listClinicNumsNeedingSignups.Distinct())
            {
        ...
        {
            var isCacheInvalid = false;
            foreach (var signupHostedEmail in listSignupOuts)
            {
                var clinicNum = signupHostedEmail.ClinicNum;
                var prefHostedEmail = listClinicPrefs.FirstOrDefault(x => x.ClinicNum == clinicNum);
        ...
        public static bool CreateHostedEmailCredentials(long clinicNum, bool doSignupMassEmail, bool doSignupSecureEmail, IWebServiceMainHQ webServiceMainHQ = null)
        {
            ...
            var result = webServiceMainHQ.EmailHostingSignup(officeData);
            PayloadHelper.CheckForError(result);
            var guid = WebSerializer.DeserializeTag<string>(result, "AccountGUID");
            string secret = null;
            ODException.SwallowAnyException(() =>
            {
        ...
        {
            var isCacheInvalid = false;
            foreach (var pushablePref in listPrefs)
            {
                ...
                if (pushablePref.ClinicNum > 0 && true)
                {
                    if (Enum.TryParse(pushablePref.PrefName, out PrefName prefName))
                    {
                        var clinicPref = ClinicPrefs.GetPref(prefName, pushablePref.ClinicNum);
                        if (clinicPref != null && clinicPref.ValueString == pushablePref.ValueString)
                        {
                            //No change or doesn't exist
                            continue;
                        }

                        if (!ClinicPrefs.Upsert(prefName, pushablePref.ClinicNum, pushablePref.ValueString))
                        {
                            continue;
                        }
                    }
                ...
                else if (pushablePref.ClinicNum == 0)
                {
                    var preference = Prefs.GetPref(pushablePref.PrefName);
                    if (preference.ValueString == pushablePref.ValueString)
                    {
                        //No change
                        continue;
                    }
                ...
                {
                    //ClinicNum is greater than 0 but Clinics are not enabled
                    continue;
                }
        ...
        {
            var includeSecurityLogEntry = false;
            var logText = $"GetEServicesSetupFull called by user {Security.CurUser.UserName} ({Security.CurUser.UserNum})";
            var signupsLookupNew =
            ...
            if (signupOld != null)
            {
                ...
                var logChangesSummary = "";

                #region EService Signups

                var signupsLookupOld =
                ...
                foreach (var clinicNum in signupsLookupNew.Where(x => signupsLookupOld.Any(y => y.Key == x.Key)).Select(x => x.Key))
                {
                    var logChangesClinicSummary = "";
                    var listEServicesOld = signupsLookupOld[clinicNum].ToList();
                    var listEServicesNew = signupsLookupNew[clinicNum].ToList();
                    //Then handle changes where the signup already existed
                    foreach (var eServiceNew in listEServicesNew.Where(x => listEServicesOld.Any(y => y.EService == x.EService)))
                    {
                        var eServiceOld = listEServicesOld.First(x => x.EService == eServiceNew.EService);
                        ...
                        {
                            var smsOld = (EServiceSetup.SignupOut.SignupOutSms) eServiceOld;
                            var smsNew = (EServiceSetup.SignupOut.SignupOutSms) eServiceNew;
                            ...
                            {
                                var shortCodesOld = string.Join("|", EnumTools.GetFlags(smsOld.ShortCodeTypeFlags).Select(x => x.GetDescription()));
                                var shortCodesNew = string.Join("|", EnumTools.GetFlags(smsNew.ShortCodeTypeFlags).Select(x => x.GetDescription()));
                                logChangesClinicSummary += $"{Environment.NewLine}   {smsNew.EService} " +
                    ...
                    var listPhonesOld = signupOld.Phones.FindAll(x => x.ClinicNum == clinicNum);
                    var listPhonesNew = signupNew.Phones.FindAll(x => x.ClinicNum == clinicNum);
                    //The case where there are phones in the old list that aren't in the new one
                    foreach (var phoneOld in listPhonesOld.Where(x => !listPhonesNew.Any(y => y.PhoneNumber == x.PhoneNumber)))
                    {
                        logChangesClinicSummary += $"{Environment.NewLine}   " +
                    ...
                    foreach (var phoneNew in listPhonesNew.Where(x => !listPhonesOld.Any(y => y.PhoneNumber == x.PhoneNumber)))
                    {
                        logChangesClinicSummary += $"{Environment.NewLine}   " +
                    ...
                    foreach (var phoneNew in listPhonesNew.Where(x => listPhonesOld.Any(y => y.PhoneNumber == x.PhoneNumber)))
                    {
                        var phoneOld = listPhonesOld.First(x => x.PhoneNumber == phoneNew.PhoneNumber);
            ...
            foreach (var clinicNum in signupsLookupNew.Select(x => x.Key))
            {
                logText += $"{(string.IsNullOrWhiteSpace(logText) ? "" : Environment.NewLine)}Clinic: {Clinics.GetAbbr(clinicNum)} ({clinicNum})";
                foreach (var signupOutEService in signupsLookupNew[clinicNum])
                {
                    var statusText = "Disabled"; //if signupOutEService.IsEnabled is disabled that's the only state it can have so this is "default"
                    ...
                    {
                        var signupOutSms = (EServiceSetup.SignupOut.SignupOutSms) signupOutEService;
                        logText += $@"
      ...
      CountryCode: {signupOutSms.CountryCode}";
                ...
                if (signupNew.Phones.Any(x => x.ClinicNum == clinicNum))
                {
                    ...
                    foreach (var phone in signupNew.Phones.Where(x => x.ClinicNum == clinicNum))
                    {
                        logText += $@"
      PhoneNumber: {phone.PhoneNumber}
         ...
         InactiveCode: {phone.InactiveCode}";
            ...
            var eServiceSignalNum = OpenDentBusiness.EServiceSignals.Insert(new EServiceSignal()
            {
        ...
        public class EServiceSetup
        {
            ...
            public class SignupIn
            {
                ...
                ///<summary>Dental Office DB Time.</summary>
                ...
                public class ClinicLiteIn
                {
                    ...
                    public class AddressInfoLite
                    {
                        ...
                        public static string SerializeAddress(ClinicDto clinic)
                        {
                            var addressInfo = new AddressInfoLite();
                            addressInfo.Address = clinic.AddressLine1;
                            addressInfo.Address2 = clinic.AddressLine2;
                            addressInfo.City = clinic.City;
                            addressInfo.State = clinic.State;
                            addressInfo.Zip = clinic.Zip;
                            addressInfo.BillingAddress = clinic.BillingAddressLine1;
                            addressInfo.BillingAddress2 = clinic.BillingAddressLine2;
                            ...
                            addressInfo.PayToAddress = clinic.PayToAddressLine1;
                            addressInfo.PayToAddress2 = clinic.PayToAddressLine2;
            ...
            public class SignupOut
            {
                ...
                ///<summary>List of any features that are in 'limited beta' status, and the clinics signed up for them.</summary>
                public List<LimitedBetaFeature> ListLimitedBetaFeatures;
                ...
                ///<summary>HQ defined upload time, default to midnight if no other value is set.</summary>
                public DateTime DateTimeEServiceLogUploadNext;
                ...
                public class SignupOutSms : SignupOutEService
                {
                    ...
                    {
                        get { return (ShortCodeTypeFlag) EnumTools.GetFlags((ShortCodeTypeFlag) ShortCodeTypeFlagsInt).Sum(x => (int) x); }
                    }
                }
        ...
        #region Advertising - Postcards

        public static PostcardManiaMetaData GetAdvertisingPostcardsAccountData()
        {
            var payloadItems = new List<PayloadItem>();
            var officeData = PayloadHelper.CreatePayload(payloadItems, eServiceCode.Undefined);
            var result = GetWebServiceMainHQInstance().GetAdvertisingPostcardsAccounts(officeData);
            return WebSerializer.DeserializeTag<PostcardManiaMetaData>(result, AdvertisingPostcards.MassPostcardResponseNode);
        }

        public static PostcardManiaSSOResponse GetAdvertisingPostcardsSSO(PostcardManiaSSORequest req)
        {
            var payloadItems = new List<PayloadItem>();
            payloadItems.Add(new PayloadItem(req, AdvertisingPostcards.MassPostcardRequestNode));
            var officeData = PayloadHelper.CreatePayload(payloadItems, eServiceCode.Undefined);
            var result = GetWebServiceMainHQInstance().GetPostcardManiaSSO(officeData);
            return WebSerializer.DeserializeTag<PostcardManiaSSOResponse>(result, AdvertisingPostcards.MassPostcardResponseNode);
        }

        public static PostcardManiaAccountCreateResponse ManageAdvertisingPostcardsAccount(PostcardManiaAccountCreateRequest req)
        {
            var payloadItems = new List<PayloadItem>();
            payloadItems.Add(new PayloadItem(req, AdvertisingPostcards.MassPostcardRequestNode));
            var officeData = PayloadHelper.CreatePayload(payloadItems, eServiceCode.Undefined);
            var result = GetWebServiceMainHQInstance().ManageAdvertisingPostcardsAccount(officeData);
            return WebSerializer.DeserializeTag<PostcardManiaAccountCreateResponse>(result, AdvertisingPostcards.MassPostcardResponseNode);
        }

        public static PostcardManiaUploadPatientsResponse UploadAdvertisingPostcardsPatientList(PostcardManiaUploadPatientsRequest req)
        {
            var payloadItems = new List<PayloadItem>();
            payloadItems.Add(new PayloadItem(req, AdvertisingPostcards.MassPostcardRequestNode));
            var officeData = PayloadHelper.CreatePayload(payloadItems, eServiceCode.Undefined);
            var result = GetWebServiceMainHQInstance().UploadPostcardManiaPatientList(officeData);
            return WebSerializer.DeserializeTag<PostcardManiaUploadPatientsResponse>(result, AdvertisingPostcards.MassPostcardResponseNode);
        }
        ...
        public static List<ShortGuidResult> GetShortGUIDs(int numberToGet, int numberForSms, long clinicNum, eServiceCode eService)
        {
            ...
            var officeData = PayloadHelper.CreatePayload(PayloadHelper.CreatePayloadContent(listPayloadItems), eService);
            var result = WebServiceMainHQProxy.GetWebServiceMainHQInstance().GenerateShortGUIDs(officeData);
        ...
        {
            var updateServerName = PrefC.GetString(PrefName.WebServiceServerName).ToLower();
            if (updateServerName != "" && !ODEnvironment.IdIsThisComputer(updateServerName))
            {
                //When set, only send data from update server
                return;
            }

            var listUpdates = UpdateHistories.GetPreviousUpdateHistories(2); //Sorted by UpdateHistory.DateTimeUpdated DESC
            string versionFrom;
            string versionTo;
            if (listUpdates.Count > 0 && listUpdates[0].DateTimeUpdated.Date != DateTime.Today)
            {
                //Only send out version info to HQ on same day as update.)
                return;
            }
            ...
            {
                //Most likely a new customer with 1 or 0 updates.
        ...
        {
            ///<summary>Beginning window for when automatic messages can start being sent. Based off office's pref value for AutomaticCommunicationTimeStart</summary>
            public DateTime AutomaticCommunicationTimeStart;

            ///<summary>Ending window for when automatic messages should stop being sent. Based off office's pref value for AutomaticCommunicationTimeEnd</summary>
            public DateTime AutomaticCommunicationTimeEnd;
        }
        ...
        {
            var officeData = PayloadHelper.CreatePayload(PayloadHelper.CreatePayloadContent(clinicNum, "ClinicNum"), eServiceCode.Undefined);
            var result = GetWebServiceMainHQInstance().GetMobileSettings(officeData);
