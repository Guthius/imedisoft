	...
	public class WebServiceMainHQMockDemo : OpenDentBusiness.WebServiceMainHQ.WebServiceMainHQ, IWebServiceMainHQ {
		...
		private List<WebServiceMainHQProxy.EServiceSetup.SignupOut.SignupOutEService> GetEServicesForAll() {
			if(true) {
				...
				List<long> listClinicNums=new List<long> { 0 };
				listClinicNums.AddRange(Clinics.GetDeepCopy(true).Select(x => x.Id));
				foreach(long clinicNum in listClinicNums) {
		...
		private List<WebServiceMainHQProxy.EServiceSetup.SignupOut.SignupOutEService> GetEServicesForClinic(long clinicNum=0) {
			return new List<WebServiceMainHQProxy.EServiceSetup.SignupOut.SignupOutEService>() {
				...
				GetEServiceForCode(eServiceCode.OCR,clinicNum),
		...
		public new string SmsSend(string officeData) {
			List<SmsToMobile> listSmsToMobiles=null;
			XmlDocument doc=new XmlDocument();
			doc.LoadXml(officeData);
			XmlNode node=doc.SelectSingleNode("//ListSmsToMobile");
			if(node==null) {
				throw new ApplicationException("Unable to create list of messages to send.");
			}
			...
			if(listSmsToMobiles==null||listSmsToMobiles.Count==0) {
				throw new ApplicationException("Unable to create list of messages to send.");
			}
			for(int i=0; i<listSmsToMobiles.Count;i++) { //WSHQ.SmsSend typically sets all messages to Pending, emulate that here.
				listSmsToMobiles[i].SmsStatus=SmsDeliveryStatus.Pending;
			}
			return PayloadHelper.CreateSuccessResponse(new List<PayloadItem>() {
				new PayloadItem("Success","Success"),
		...
		public new string GetGoogleAccessToken(string officeData) {
			...
			try {
				...
				string redirectUri=WebSerializer.DeserializeNode(officeData,"RedirectUri",throwIfNotFound:false);
				string codeVerifier=WebSerializer.DeserializeNode(officeData,"CodeVerifier",throwIfNotFound:false);
				#endregion
