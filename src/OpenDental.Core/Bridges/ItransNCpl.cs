...
namespace OpenDentBusiness
{
    public class ItransNCpl
    {
        ...
        {
            string json;
            var dateTimeTrans = DateTime.Now;
            var clearinghouse = Clearinghouses.GetDefaultDental();
            ...
            if (clearinghouse.CommBridge == EclaimsCommBridge.ITRANS2 && !string.IsNullOrEmpty(clearinghouse.ResponsePath))
            {
                ...
                if (isAutomatic && PrefC.GetString(PrefName.WebServiceServerName).ToLower() != Dns.GetHostName().ToLower())
                {
                    //Only server can run when isOnlyServer is true.
                    return "Update can only run on the web service server " + PrefC.GetString(PrefName.WebServiceServerName) + ". Connect to the server and try again.";
                }
                ...
                process.Start();
                process.WaitForExit();
                var ncplFilePath = ODFileUtils.CombinePaths(clearinghouse.ResponsePath, "n-cpl.json");
                json = File.ReadAllText(ncplFilePath); //Read n-cpl.json
                dateTimeTrans = File.GetCreationTime(ncplFilePath);
            }
            else
            {
                //ITRANS2 not used or not setup correctly (ex using "ITRANS"), go to HQ for file content.
                try
                {
                    var result = WebServiceMainHQProxy.GetWebServiceMainHQInstance().CanadaCarrierUpdate(PayloadHelper.CreatePayload("", eServiceCode.Undefined));
            ...
            var msgTextPrev = EtransMessageTexts.GetMostRecentForType(EtransType.ItransNcpl);
            ...
            var etrans = Etranss.CreateEtrans(dateTimeTrans, clearinghouse.HqClearinghouseNum, json, 0);
        ...
        {
            var listCanadianNetworks = CanadianNetworks.GetDeepCopy();
            //Loop through all of the carriers present within the n-cpl.json file.
            foreach (var jsonCarrier in iTransNCpl.ListCarriers)
            {
                //Update carriers.
                var jsonCarrierPhone = GetPhoneNumberFromJsonCarrier(jsonCarrier); //Will be empty string if not found.
                //Find all of the carriers in the database that have a matching ElectID and are flagged as IsCDA.
                var listDbCarriers = Carriers.GetAllByElectId(jsonCarrier.Bin).FindAll(x => x.IsCDA);
                                                                                   ...
                                                                                   || x.CarrierName.ToLower() == jsonCarrier.Name.Fr.ToLower())))
                {
                    ...
                    var carrierNew = new Imedisoft.Core.Entities.Carrier();
                    ...
                    {
                        var add = jsonCarrier.Address.First();
        ...
        private static Carrier GetJsonCarrierForUpdate(ItransNCpl iTransNCpl, Imedisoft.Core.Entities.Carrier carrierOD)
        {
            var listJsonCarrierByElectID = iTransNCpl.ListCarriers.Where(x => x.Bin == carrierOD.ElectID).ToList();
            //Find claims processor as that is who is processing our claim, regardless of reseller information
            var jsonCarrier = listJsonCarrierByElectID.Where(x => x.Name.En == x.Claims_Processor?.Name.En).FirstOrDefault();
            if (jsonCarrier is null)
            {
                //If we can't find one then just use the first one in the list of carriers by elect id
        ...
        private static string GetPhoneNumberFromJsonCarrier(Carrier jsonCarrier)
        {
            ...
            var jsonCarrierPhone = jsonCarrier.Telephone?.FirstOrDefault()?.Name?.En ?? ""; //Will be empty string if not found
            if (!string.IsNullOrWhiteSpace(jsonCarrierPhone))
            {
                var jsonCarrierPhoneFormatted = TelephoneNumbers.FormatNumbersExactTen(jsonCarrierPhone); //strip 1 if present
        ...
        {
            var odCarrierOld = odCarrier.Copy();
            ...
            var listFields = Enum.GetValues(typeof(ItransImportFields)).Cast<ItransImportFields>().ToList();
        ...
        private static void TrySetCanadianNetworkNum(Carrier jsonCarrier, Imedisoft.Core.Entities.Carrier odCarrier, List<CanadianNetwork> listCanadianNetworks)
        {
            ...
            var jsonCarrierNetwork = jsonCarrier.Network.FirstOrDefault(x => x.Network_Folder == "TGB");
            if (jsonCarrierNetwork == null)
            {
                //We have a bypass that treats INSTREAM like Telus B.  Try looking for INSTREAM now.
                jsonCarrierNetwork = jsonCarrier.Network.FirstOrDefault(x => x.Network_Folder == "INS");
            }

            if (jsonCarrierNetwork == null)
            {
                //Otherswise default to first value in list.
            ...
            switch (jsonCarrierNetwork.Network_Folder)
            {
                ...
                default: //Unknown network_foler, CanadianNetworkNum will not be set.
                    ...
                    {
                        //Let HQ known if this ever happens so that it doesn't go unnoticed.
            ...
            var network = listCanadianNetworks.FirstOrDefault(x => x.Abbrev == odAbbr);
            if (network != null)
            {
        ...
        {
            var supportedTypes = CanSupTransTypes.None;
            ...
            UpdateSupportedTypes(ref supportedTypes, jsonCarrier.Attachment_09, CanSupTransTypes.Attachment_09);
