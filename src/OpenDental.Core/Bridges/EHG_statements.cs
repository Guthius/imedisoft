...
namespace OpenDentBusiness.Bridges {
	public class EHG_statements {
		...
		public static List <string> Validate(long clinicNum) {
			...
			{
				listErrors.Add(Lans.g("EHG_Statements","invalid")+" "+Lans.g("EHG_Statements",addressRemit.Source));
			}
			return listErrors;
		}
		...
		public static EHG_Address GetAddress(EbillAddress eBillAddress,ClinicDto clinic) {
			...
			if(eBillAddress==EbillAddress.ClinicPhysical) {
				...
				address.Phone=clinic.PhoneNumber;//enforced to be 10 digit fairly rigidly by the UI
		...
		public static void Send(string data,long clinicNum,out string alertMessage) {
			...
			string postData=
				...
				+"&Username="+HttpUtility.UrlEncode(billingUserName)
				+"&Password="+HttpUtility.UrlEncode(billingPassword);
			...
			for(int i=0;i<responseParams.Length;i++) {
				...
				switch(curParam) {
					...
					case "ErrorMessage":
						...
						break;
					case "AlertMessage":
						alertMessage=GetParamValue(responseParams[i]);
						break;
					default:
