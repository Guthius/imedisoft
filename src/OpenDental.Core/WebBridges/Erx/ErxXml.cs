...
namespace OpenDentBusiness {
	public class ErxXml {
		...
		public static string BuildNewCropClickThroughXml(Provider prov,Employee emp,Patient pat,long clinicNum,out NCScript ncScript) {
			...
			if(!true || clinicNum==0) { //No clinic.
				...
				var clinic=Clinics.GetClinic(clinicNum);//Set and validated before calling this method.
				...
				ncScript.Location.LocationAddress.country=country;//Validated above.
				ncScript.Location.primaryPhoneNumber=clinic.PhoneNumber;//Validated to be 10 digits within the chart.
				ncScript.Location.primaryFaxNumber=clinic.FaxNumber;//Validated to be 10 digits within the chart.
				ncScript.Location.pharmacyContactNumber=clinic.PhoneNumber;//Validated to be 10 digits within the chart.
			}
