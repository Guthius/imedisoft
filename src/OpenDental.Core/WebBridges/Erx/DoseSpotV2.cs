...
namespace OpenDentBusiness {
	...
	public class DoseSpotV2 {
		...
		public static string GetDoseSpotRoot() {
			...
			if(oIDExternal==null) {
				...
				oIDExternal.rootExternal=OIDInternals.OpenDentalOID+"."+_doseSpotPatNum+"."+OIDInternals.GetCustomerPatNum();
		...
		public static void MakeClinicErxAlert(ClinicErx clinicErx,bool clinicAutomaticallyAttached) {
			...
			var clinic=Clinics.GetClinic(clinicErx.ClinicNum);
		...
		public static bool SyncPrescriptionsFromDoseSpot(string clinicID,string clinicKey,string userID,long patNum,Action<List<RxPat>> onRxAdd = null) {
			...
			string token=DoseSpotRESTV2.GetToken(userID,clinicID,clinicKey);
			//Get rid of any deleted prescriptions.
			List<DoseSpotPrescription> listDoseSpotPerscriptions=DoseSpotRESTV2.GetPrescriptions(token,oIDExternal.IDExternal);
			...
			listDoseSpotMedicationWrappers.AddRange(DoseSpotRESTV2.GetSelfReported(token,oIDExternal.IDExternal).Select(x => new DoseSpotMedicationWrapper(null,x)).ToList());
				...
				if(doseSpotMedicationWrapper.PharmacyId.HasValue) {
					try {
						if(!dictPharmacyIdToPharmacyName.ContainsKey(doseSpotMedicationWrapper.PharmacyId.Value)) {
							dictPharmacyIdToPharmacyName.Add(doseSpotMedicationWrapper.PharmacyId.Value,DoseSpotRESTV2.GetPharmacyName(token,doseSpotMedicationWrapper.PharmacyId.Value));
						}
		...
		public static void SyncPrescriptionsToDoseSpot(string clinicID,string clinicKey,string userID,long patNum) {
			string token=DoseSpotRESTV2.GetToken(userID,clinicID,clinicKey);
			//No need to check MiddleTierRole; no call to db.
			OIDExternal oIDExternal=DoseSpotV2.GetDoseSpotPatID(patNum);
			if(oIDExternal==null) {
			...
			foreach(MedicationPat medicationPat in listMedicationPats) {
				...
				DoseSpotSelfReported doseSpotSelfReported=DoseSpotRESTV2.MedicationPatToDoseSpotSelfReport(medicationPat);
				...
				if(doseSpotSelfReported.SelfReportedMedicationId>0) {
					...
					try {
						DoseSpotRESTV2.PutSelfReportedMedications(token,oIDExternal.IDExternal,doseSpotSelfReported);
					}
					catch(ODException e) {
					...
					medicationPat.ErxGuid=Erx.OpenDentalErxPrefix+DoseSpotRESTV2.PostSelfReportedMedications(token,oIDExternal.IDExternal,doseSpotSelfReported);
					if(rxPat!=null) {
		...
		public static bool GetPrescriberNotificationCounts(string clinicID,string clinicKey,string userID
			...
			string token=DoseSpotRESTV2.GetToken(userID,clinicID,clinicKey);
			DoseSpotRESTV2.GetNotificationCounts(token,out countRefillReqs,out countTransactionErrors,out countPendingPrescriptionsCount);
		...
		public static void RegisterClinic(long clinicNum,string clinicID,string clinicKey,string userID
			...
			//}
			#endregion
			DoseSpotRESTV2.PostClinic(token,clinic,out clinicIdNew,out clinicKeyNew);
		...
		private static void ValidateClinic(ClinicDto clinic,bool isPractice = false) {
			...
			else if(!IsPhoneNumberValid(clinic.PhoneNumber)) {//If the phone number isn't valid, DoseSpot will break.
		...
		public static void SetMedicationHistConsent(Patient patient,long clinicNum,Program program = null,List<ProgramProperty> listProgramProperties = null) {
			...
			string token=DoseSpotRESTV2.GetToken(doseSpotUserID,doseSpotClinicID,doseSpotClinicKey);
			//Get DoseSpotPatID
			...
			DoseSpotRESTV2.PostMedicationHistoryConsent(token,oIDExternal.IDExternal);
		}
	...
	public class DoseSpotRESTV2 {
		...
		public static string GetToken(string userId,string clinicId,string clinicKey) {
			...
			string body=$"client_id={clinicId}" +
		...
		public static string AddPatient(string authToken,Patient patient) {
			...
			string primaryPhone=DoseSpotV2.GetPhoneAndType(patient,0,out string phoneType);
		...
		public static string EditPatient(string authToken,Patient patient,string doseSpotPatId) {
			...
			var resObj=Request(ApiRoute.EditPatient,HttpMethod.Put,"Bearer "+authToken,body,new {
				Id="",
				Result=new {ResultCode="",ResultDescription=""}
		...
		///<summary>Check the version of DoseSpot being used. Request migration if DoseSpotV1 is still being used</summary>
		public static void MigrateDoseSpotIfNeeded() {
			...
			//Returns 1 if migration request has been made.
			ProgramProperty programPropertyDoseSpotMigrationRequest=OpenDentBusiness.ProgramProperties.GetPropForProgByDesc(Programs.GetProgramNum(ProgramName.eRx),DoseSpotREST.PropertyDescs.DoseSpotApiMigrationRequested);
			string doseSpotClinicID="";
			string doseSpotClinicKey="";
			long clinicNum=Clinics.ClinicNum;
			string doseSpotAdminId="";
				if(!ODBuild.IsDebug()) {
					try {
						doseSpotAdminId=WebServiceMainHQProxy.GetWebServiceMainHQInstance()
						.BuildOAuthUrl(PrefC.GetString(PrefName.RegistrationKey),OAuthApplicationNames.DoseSpotAdminID.ToString());
					}
					catch(Exception ex) {
						return;
					}
			...
			if(programPropertyDoseSpotApiVersion.PropertyValue!="2") {
				...
				if(!ODBuild.IsDebug()) {
					...
					//Uncomment and add your clientID in order to test.
					//clientId="";
				}
				List<ClinicErx> listClinicErxs=ClinicErxs.GetWhere(x => x.EnabledStatus==ErxStatus.Enabled);
				if(listClinicErxs.Count>0) {
					string token="";
					try {
						//DoseSpot support specifically told us to get a V1 token using an AdminId.
						token=DoseSpotREST.GetToken(doseSpotAdminId,listClinicErxs[0].ClinicId,listClinicErxs[0].ClinicKey);
		...
		private static T Request<T>(ApiRoute apiRoute,HttpMethod httpMethod,string authHeader,string body,T responseType,string acceptType = "application/json",params string[] listRouteIDs) {
			using(WebClient webClient = new WebClient()) {
				...
				try {
					...
					string responseErr="";
					using(var streamReader = new StreamReader(((HttpWebResponse)webException.Response).GetResponseStream())) {
					...
					string errorMsg=webException.Message+(string.IsNullOrWhiteSpace(responseErr) ? "" : "\r\nRaw response:\r\n"+responseErr);
					throw new Exception(errorMsg,webException);//If it got this far and haven't rethrown, simply throw the entire exception.
				}
				catch(Exception ex) {
		...
		public static string GetApiUrl(ApiRoute apiRoute,params string[] arrayRouteIDs) {
			...
			switch(apiRoute) {
				case ApiRoute.Root:
					...
					break;
				case ApiRoute.Token:
					apiUrl+="/connect/token"; //changed from V1
					break;
				...
				case ApiRoute.LogMedicationHistoryConsent: //changed from V1
					//routeId[0]=PatientId
					apiUrl+=$"/api/patients/{arrayRouteIDs[0]}/medications/history/consent";
					break;
				case ApiRoute.PostClinic:
					apiUrl+=$"/api/clinics";
					break;
				case ApiRoute.PostClinicGroup: //changed from V1
					apiUrl+=$"/api/clinicGroups";
					break;
		...
		private static string MakeEncryptedClinicId(string clinicKey,bool isQueryStr = true) {
			return DoseSpotV2.CreateSsoCode(clinicKey,isQueryStr);
		}

		///<summary>This method uses the same equation as DoseSpot's other webservice encryption method.</summary>
		private static string MakeEncryptedUserId(string clinicKey,string userID,bool isQueryStr = true) {
			return DoseSpotV2.CreateSsoUserIdVerify(clinicKey,userID,isQueryStr);
		}
		...
		public static DoseSpotSelfReported MedicationPatToDoseSpotSelfReport(MedicationPat medicationPat) {
			...
			doseSpotSelfReported.MedicationStatus=DoseSpotREST.MedicationStatus.Discontinued;
			if(medicationPat.DateStop.Year<1880 || medicationPat.DateStop>=DateTime.Today) {
				doseSpotSelfReported.MedicationStatus=DoseSpotREST.MedicationStatus.Active;
			}
			else if(medicationPat.DateStop<DateTime.Today) {
				doseSpotSelfReported.MedicationStatus=DoseSpotREST.MedicationStatus.Completed;
