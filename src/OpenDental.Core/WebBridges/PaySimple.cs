...
public class PaySimple : WebBase
{
    public class PropertyDescs
    {
        ...
        public const string PaySimpleIsOnlinePaymentsEnabled = "PaySimpleIsOnlinePaymentsEnabled";
    }
    ...
    public static ApiResponse AddACHAccount(Patient pat, string routingNumber, string acctNumber, string bankName, bool isCheckings, long clinicNum)
    {
        ValidateProgram(clinicNum);
        ...
        var psCustomerId = GetCustomerIdForPat(pat.PatNum, pat.FName, pat.LName, clinicNum);
        try
        {
    ...
    private static ApiResponse AddACHAccount(long customerId, string routingNumber, string acctNumber, string bankName, bool isCheckings, long clinicNum)
    {
        ValidateProgram(clinicNum);
        if (customerId == 0)
        {
    ...
    public static void PostWebhook(long clinicNum, string[] arrayHookTypes, string url)
    {
        ValidateProgram(clinicNum);
        PaySimpleApi.PostWebhook(GetAuthHeader(clinicNum, true), PaySimpleApi.MakeNewWebhookData(url, arrayHookTypes));
    }

    public static ApiResponse MakePayment(long patNum, CreditCard cc, decimal payAmt, string ccNum, DateTime ccExpDate, bool isOneTimePayment, string billingZipCode = "", string cvv = "", long clinicNum = -1, string lname = "")
    {
        ValidateProgram(clinicNum);
        if (patNum == 0)
        {
        ...
        if (cc == null)
        {
            cc = new CreditCard()
            {
                PatNum = patNum,
                PaySimpleToken = "",
                CCSource = CreditCardSource.PaySimple, //Payment made from ODProper. PaymentPortal should never pass a null CreditCard object.
            };
        }

        if (string.IsNullOrWhiteSpace(cc.PaySimpleToken))
        {
            var patCur = Patients.GetPat(cc.PatNum);
            if (patCur == null)
            {
                patCur = new Patient()
                {
            ...
            var psCustomerId = GetCustomerIdForPat(patCur.PatNum, patCur.FName, patCur.LName, clinicNum);
    ...
    public static ApiResponse MakePaymentByToken(Patient pat, CreditCard cc, decimal payAmt, long clinicNum = -1, bool isWeb = false)
    {
        ValidateProgram(clinicNum);
        ...
        if (isWeb)
        {
            if (cc.CCSource.In(CreditCardSource.PaySimplePaymentPortal, CreditCardSource.PaySimplePaymentPortalGuest))
            {
                return MakePayment(cc.PatNum, cc, payAmt, "", DateTime.MinValue, false, "", "", clinicNum);
            }
            else if (cc.CCSource == CreditCardSource.PaySimplePaymentPortalACH)
            {
                return MakePaymentACH(pat, cc, payAmt, "", "", "", false, clinicNum);
            }
            else
            {
                throw new ODException(Lans.g("PaySimple", "Error making payment by token. Credit card source not supported."));
            }
        }
    ...
    private static ApiResponse MakePaymentNoPat(decimal payAmt, string ccNum, DateTime ccExpDate, string billingZipCode = "", string cvv = "", long clinicNum = -1, string lname = "")
    {
        ValidateProgram(clinicNum);
        ...
        var psCustomerId = AddCustomer("UNKNOWN", lname, "", clinicNum);
        var apiResponse = AddCreditCard(psCustomerId, ccNum, ccExpDate, billingZipCode);
        var accountId = apiResponse.PaySimpleToken;
    ...
    public static ApiResponse MakePaymentACH(Patient pat, CreditCard cc, decimal payAmt, string routingNumber, string acctNumber, string bankName, bool isCheckings, long clinicNum = -1)
    {
        ValidateProgram(clinicNum);
        if (pat == null || pat.PatNum == 0)
        {
        ...
        if (string.IsNullOrWhiteSpace(cc.PaySimpleToken))
        {
            ...
            try
            {
                var apiResponse = AddACHAccount(psCustomerId, routingNumber, acctNumber, bankName, isCheckings, clinicNum);
                cc.PaySimpleToken = apiResponse.PaySimpleToken;
            }
            catch (PaySimpleException ex)
            {
    ...
    public static void DeleteCreditCard(CreditCard creditCardCur)
    {
        ...
        PaySimpleApi.DeleteAccountCreditCard(GetAuthHeader(creditCardCur.ClinicNum), creditCardCur.PaySimpleToken);
    }

    public static void DeleteACHAccount(CreditCard creditCardCur)
    {
        ValidateProgram(creditCardCur.ClinicNum);
        if (string.IsNullOrWhiteSpace(creditCardCur.PaySimpleToken))
        {
        ...
        PaySimpleApi.DeleteAccountACH(GetAuthHeader(creditCardCur.ClinicNum), creditCardCur.PaySimpleToken);
    }
    ...
    private static string GetAuthHeader(long clinicNum = -1, bool isWebhook = false)
    {
        ...
        var apiUserName = ProgramProperties.GetPropValForClinicOrDefault(Programs.GetCur(ProgramName.PaySimple).ProgramNum
            , PropertyDescs.PaySimpleApiUserName
            , clinicNum);
        var apiKey = ProgramProperties.GetPropValForClinicOrDefault(Programs.GetCur(ProgramName.PaySimple).ProgramNum
    ...
    private static void ValidateProgram(long clinicNum = -1)
    {
        ...
        var apiUserName = ProgramProperties.GetPropValForClinicOrDefault(progPaySimple.ProgramNum, PropertyDescs.PaySimpleApiUserName, clinicNum);
        var apiKey = ProgramProperties.GetPropValForClinicOrDefault(progPaySimple.ProgramNum, PropertyDescs.PaySimpleApiKey, clinicNum);
        var payType = ProgramProperties.GetPropValForClinicOrDefault(progPaySimple.ProgramNum, PropertyDescs.PaySimplePayTypeCC, clinicNum);
    ...
    {
        var psCustomerId = PatientLinks.GetPatNumsLinkedFrom(patNum, PatientLinkType.PaySimple).FirstOrDefault();
        if (psCustomerId == 0)
        {
            //Patient doesn't have a PaySimpleCustomerId
    ...
    public class ApiResponse
    {
        ...
        public void BuildReceiptString(string ccNum, int ccExpMonth, int ccExpYear, string nameOnCard, long clinicNum, bool wasSwiped = false, bool isACH = false, bool isVoidingRefund = false)
        {
            var result = "";
            var xleft = 0;
            var xright = 15;
    ...
    private class PaySimpleApi
    {
        ...
        public static List<ApiWebhookResponse> GetWebhooks(string authHeader)
        {
            ...
            var listWebhooks = new List<ApiWebhookResponse>();
            ...
            for (var i = 0; i < response.FullResponse.total_item_count; i++)
            {
                ...
                var webhookData = new ApiWebhookResponse();
                ...
                for (var j = 0; j < response.FullResponse.data[i].event_types.Count(); j++)
                {
        ...
        public static void PostWebhook(string authHeader, string postData)
        {
        ...
        public static void DeleteAccountCreditCard(string authHeader, string accountId)
        {
            DeleteAccount(authHeader, accountId, ApiRoute.AccountCreditCard);
        }

        public static void DeleteAccountACH(string authHeader, string accountId)
        {
            DeleteAccount(authHeader, accountId, ApiRoute.AccountACH);
        }

        private static void DeleteAccount(string authHeader, string accountId, ApiRoute route)
        {
            ...
            using (var client = new WebClient())
            {
                ...
                try
                {
                    ...
                    else if (method == HttpMethod.Delete)
                    {
                        ...
                        var request = (HttpWebRequest) WebRequest.Create(GetApiUrl(route, routeId));
                ...
                catch (WebException wex)
                {
                    ...
                    {
                        //Operation timeouts do not give a response back. We speculate the timeouts happen when rapid-firing requests.
        ...
        public class RequestResponse<T>(T response, string rawResponse)
        {
            public readonly T FullResponse = response;
            public readonly string RawResponse = rawResponse;
        }
        ...
        {
            var nowAsString = DateTime.Now.ToString(@"yyyy-MM-ddTHH\:mm\:sszzz"); //This matches the PaySimple documentation for C#.
            var hash = "";
            var encoding = Encoding.UTF8;
            using (var hmac = new System.Security.Cryptography.HMACSHA256(encoding.GetBytes(apiKey)))
            {
                var hashBytes = hmac.ComputeHash(encoding.GetBytes(nowAsString));
        ...
        {
            var retVal = 0;
            var cardTypeName = CreditCardUtils.GetCardType(ccNum);
        ...
        {
            var apiUrl = "https://api.paysimple.com";
        ...
        private static void HandleWebException(string paySimpleErrorJson)
        {
            ...
            try
            {
                ...
                var strbError = new StringBuilder();
...
public class PaySimpleException(string message, string errorCode, List<string> errorMessages) : ApplicationException(message)
{
