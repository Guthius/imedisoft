...
namespace OpenDentBusiness {
	public class PayConnect {
		public static MockPayConnect Mock;
		public class MockPayConnect {
			public bool IsPostPaymentRequest;
		...
		{
			if(Mock!=null && Mock.IsProcessCreditCard) {
				return PayConnectREST.MockProcessCreditCardResponse();
			}
			try {
		...
		public static string GetHpfUrlForPayment(Patient pat,string accountToken,string email,string payNote,bool isMobile,double amount,bool saveToken,CreditCardSource ccSource,string logGuid="") {
			if(pat==null) {
		...
		public static PayConnectResponseWeb MakePaymentWithAlias(Patient pat,string email,string payNote,double amount,CreditCard cc,string logGuid="",
			bool doForceDuplicate=false,CreditCardSource ccSource=CreditCardSource.PayConnectPortal)
		{
			if(pat==null) {
				throw new ODException("No Patient Found",ODException.ErrorCodes.NoPatientFound);
			}
			...
			PayConnectService.creditCardRequest payConnectRequest=PayConnect.BuildSaleRequest(
				...
				PayConnectService.transType.SALE,"",true,isForced:doForceDuplicate);
			...
			PayConnectResponseWeb responseWeb=null;
			if(payConnectRequest!=null && payConnectResponse.Status.code==0) {//Success
				...
				PayConnectResponseWebs.Insert(responseWeb);
				if(EmailAddresses.GetValidMailAddress(responseWeb.EmailResponse)!=null) {
					EmailAddress emailAddressFrom=EmailAddresses.GetByClinic(pat.ClinicNum,true);
					Statements.EmailStatementPatientPortal(Statements.CreateReceiptStatement(pat,StatementMode.Email),responseWeb.EmailResponse,emailAddressFrom,pat);
				}
			}
			else {//Failed
						...
						switch(responseWebCur.ProcessingStatus) {
							...
							case PayConnectWebStatus.Completed:
								...
								if(device!=null && device.LastCheckInActivity>DateTime.Now.AddHours(-1)) {
									MobileNotifications.CI_RefreshPayment(device.MobileAppDeviceNum,responseWebCur.PatNum);
								}
								break;
	...
	public class PayConnectREST {
		...
		private static T MockPostPaymentRequestResponse<T>(T responseType) {
			string res=@"
			{
				""PayToken"": ""5v1JgWCU5hnA"",
		...
		public static PayConnectService.transResponse MockProcessCreditCardResponse() {
			PayConnectService.transResponse transResponse=new PayConnectService.transResponse();
			transResponse.AuthCode="0";
			transResponse.RefNumber="123456789";
			string[] messages={ "" };
			transResponse.Messages=messages;
			transResponse.PaymentToken=new PayConnectService.Token() { Status=new PayConnectService.Status() { code=0,description="Success"},
				 Expiration=new PayConnectService.expiration(),Messages=messages, TokenId="TokenId" };
			transResponse.Status=new PayConnectService.Status() { code=0, description="Success" };
		...
		private static T Request<T>(ApiRoute route,HttpMethod method,List<string> listHeaders,string body,T responseType,string queryStr="") {
			if(PayConnect.Mock!=null) {
				if(PayConnect.Mock.IsPostPaymentRequest) {
					return MockPostPaymentRequestResponse(responseType); 
				}
			}
		...
		public static PayConnectResponse ToPayConnectResponse(PayConnectService.transResponse response,PayConnectService.creditCardRequest request) {
