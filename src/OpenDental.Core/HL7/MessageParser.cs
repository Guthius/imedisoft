...
namespace OpenDentBusiness.HL7 {
	...
	public class MessageParser {
		...
		public static void Process(MessageHL7 msg,bool isVerboseLogging) {
			...
			for(int f=0;f<hl7defmsg.ListHL7DefSegments[pidDefOrder].hl7DefFields.Count;f++) {//Go through fields of PID segment and get patnum, chartnum, patient name, and/or birthdate to locate patient
				if(hl7defmsg.ListHL7DefSegments[pidDefOrder].hl7DefFields[f].FieldName=="pat.ChartNumber") {
					int chartNumOrdinal=hl7defmsg.ListHL7DefSegments[pidDefOrder].hl7DefFields[f].OrdinalPos;
				...
				else if(hl7defmsg.ListHL7DefSegments[pidDefOrder].hl7DefFields[f].FieldName=="pat.birthdateTime") {
					int patBdayOrdinal=hl7defmsg.ListHL7DefSegments[pidDefOrder].hl7DefFields[f].OrdinalPos;
					birthdate=FieldParser.DateTimeParse(msg.Segments[pidOrder].GetField(patBdayOrdinal).ToString());
				}
				else if(hl7defmsg.ListHL7DefSegments[pidDefOrder].hl7DefFields[f].FieldName=="pat.nameLFM") {
					int patNameOrdinal=hl7defmsg.ListHL7DefSegments[pidDefOrder].hl7DefFields[f].OrdinalPos;
				...
				#region patientIdList
				else if(hl7defmsg.ListHL7DefSegments[pidDefOrder].hl7DefFields[f].FieldName=="patientIds") {
					...
					int patIdListOrdinal=hl7defmsg.ListHL7DefSegments[pidDefOrder].hl7DefFields[f].OrdinalPos;
			...
			for(int i=0;i<hl7defmsg.ListHL7DefSegments.Count;i++) {
				if(hl7defmsg.ListHL7DefSegments[i].SegmentName!=SegmentNameHL7.SCH//SIU messages will have the SCH segment, used for eCW or other interfaces where OD is an auxiliary application
					&& hl7defmsg.ListHL7DefSegments[i].SegmentName!=SegmentNameHL7.ARQ)//SRM messages will have the ARQ segment, used for interfaces where OD is the filler application
				{
					continue;
				}
				//we found the SCH or ARQ segment
				int segOrder=hl7defmsg.ListHL7DefSegments[i].ItemOrder;
				for(int j=0;j<hl7defmsg.ListHL7DefSegments[i].hl7DefFields.Count;j++) {//Go through fields of SCH or ARQ segment and get AptNum
			...
			for(int i=0;i<hl7defmsg.ListHL7DefSegments.Count;i++) {
				try {
					...
					if(hl7defmsg.ListHL7DefSegments[i].CanRepeat) {
						listSegments=msg.GetSegments(hl7defmsg.ListHL7DefSegments[i].SegmentName,!hl7defmsg.ListHL7DefSegments[i].IsOptional);
					}
					else {
						SegmentHL7 seg=msg.GetSegment(hl7defmsg.ListHL7DefSegments[i].SegmentName,!hl7defmsg.ListHL7DefSegments[i].IsOptional);
					...
					for(int j=0;j<listSegments.Count;j++) {//normally only 1 or 0 in the list, but if it is a repeatable segment the list may contain multiple segments to process
						if(_isVerboseLogging) {
							EventLog.WriteEntry("OpenDentHL7","Process segment "+hl7defmsg.ListHL7DefSegments[i].SegmentName.ToString(),EventLogEntryType.Information);
						}
						ProcessSeg(pat,aptCur,hl7defmsg.ListHL7DefSegments[i],listSegments[j],msg);
						if(hl7defmsg.ListHL7DefSegments[i].SegmentName==SegmentNameHL7.SCH) {
		...
		public static void ProcessAck(MessageHL7 msg,bool isVerboseLogging) {
			...
			for(int i=0;i<hl7DefMsg.ListHL7DefSegments.Count;i++) {
				try {
					SegmentHL7 seg=msg.GetSegment(hl7DefMsg.ListHL7DefSegments[i].SegmentName,!hl7DefMsg.ListHL7DefSegments[i].IsOptional);
					if(seg!=null) {//null if segment was not found but is optional
						ProcessSeg(null,null,hl7DefMsg.ListHL7DefSegments[i],seg,msg);
					}
				}
				catch(ApplicationException ex) {//Required segment was missing, or other error.
		...
		public static void ProcessPR1(Patient pat,HL7DefSegment segDef,SegmentHL7 seg,MessageHL7 msg) {
			...
			switch(procCode.TreatArea) {
				...
				case TreatmentArea.Surf:
					...
					procCur.ToothNum=Tooth.Parse(strToothNum);
				...
				case TreatmentArea.ToothRange:
					...
					for(int i=0;i<listToothNums.Length;i++) {
						...
						if(Tooth.IsPrimary(Tooth.Parse(listToothNums[i]))) {
						...
						listToothNums[i]=Tooth.Parse(listToothNums[i]);
					}
			...
			procCur.ProcFee=Procedures.GetProcFee(pat,listPatPlan,listSubs,insPlanList,procCur);
		...
		public static long ProcessSCH(Patient pat,Appointment apt,HL7DefSegment segDef,SegmentHL7 seg,MessageHL7 msg) {
			...
			if(isNewApt) {
				...
				Appointments.Update(apt,aptOld);
				Appointments.TryAddPerVisitProcCodesToAppt(apt,aptOld.AptStatus);
			}
			_hl7MsgCur.AptNum=apt.AptNum;
			HL7Msgs.Update(_hl7MsgCur);
			if(_isVerboseLogging) {
