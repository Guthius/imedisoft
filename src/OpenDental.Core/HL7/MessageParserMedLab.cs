...
namespace OpenDentBusiness.HL7 {
	...
	public class MessageParserMedLab {
		...
		public static List<long> Process(MessageHL7 msg,string msgArchiveFilePath,bool isVerboseLogging,Patient selectedPat=null) {
			...
			if(hl7defmsg.ListHL7DefSegments.Count<2 || msg.Segments.Count<2) {
			...
			HL7DefSegment pidSegDef=hl7defmsg.ListHL7DefSegments[1];
			...
			if(hl7defmsg.ListHL7DefSegments.Count<1 || msg.Segments.Count<1) {
				throw new Exception("Could not process the MedLab HL7 message.  "
					+"The message or message definition for this type of message does not contain the correct number of segments.");
			}
			SegmentHL7 mshSegCur=msg.Segments[0];
			if(hl7defmsg.ListHL7DefSegments[0].SegmentName!=SegmentNameHL7.MSH || mshSegCur.Name!=SegmentNameHL7.MSH) {
				throw new Exception("Could not process the MedLab HL7 message.  "
		...
		public static HL7DefSegment GetSegDefByName(HL7DefMessage msgDef,SegmentNameHL7 segName,SegmentNameHL7 prevSegName) {
			...
			for(int i=0;i<msgDef.ListHL7DefSegments.Count;i++) {//start with the second segment in the def since the first segment won't have a predecessor
				if(msgDef.ListHL7DefSegments[i].SegmentName!=segName) {
					continue;
				}
				...
				if(prevSegName==SegmentNameHL7.Unknown) {
					segDef=msgDef.ListHL7DefSegments[i];
					break;
				}
				//move backward in the list of segments looking for the nearest preceding, required segment
				for(int j=1;j<i+1;j++) {
					if(msgDef.ListHL7DefSegments[i-j].SegmentName==prevSegName) {
						segDef=msgDef.ListHL7DefSegments[i];
						break;
					}
					...
					if(!msgDef.ListHL7DefSegments[i-j].IsOptional) {
						break;
					}
				}
				if(segDef!=null) {
