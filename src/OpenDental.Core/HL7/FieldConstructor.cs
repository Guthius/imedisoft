...
{
    public static string GenerateFieldADT(HL7Def def, string fieldName, Patient pat, ProviderDto prov, Patient guar, int sequenceNum, EventTypeHL7 eventType, SegmentNameHL7 segName)
    {
    ...
    public static string GenerateFieldSIU(HL7Def def, string fieldName, Patient pat, ProviderDto prov, Patient guar, Appointment apt, int sequenceNum, EventTypeHL7 eventType, SegmentNameHL7 segName)
    {
    ...
    public static string GenerateFieldSRR(HL7Def def, string fieldName, Patient pat, ProviderDto prov, Appointment apt, int sequenceNum, EventTypeHL7 eventType, SegmentNameHL7 segName)
    {
    ...
    public static string GenerateField(HL7Def def, string fieldName, MessageTypeHL7 msgType, Patient pat, ProviderDto prov, Procedure proc, Patient guar, Appointment apt, int sequenceNum, EventTypeHL7 eventType, string pdfDescription, string pdfDataString, MessageStructureHL7 msgStructure, SegmentNameHL7 segName)
    {
        var retval = "";
        ...
        switch (fieldName)
        {
            ...
            case "apt.AptNum":
                ...
                var aptOid = OIDInternals.GetForType(IdentifierType.Appointment);
                var aptOidRoot = "";
                if (aptOid != null)
                {
                ...
                {
                    var strCheckDigit = MessageParser.M11CheckDigit(apt.AptNum.ToString()).ToString();
                    var strCheckDigitScheme = "M11";
                    if (strCheckDigit == "-1")
                    {
                        strCheckDigit = "";
                        strCheckDigitScheme = "";
            ...
            case "apt.aptStatus":
                ...
                if (eventType == EventTypeHL7.S17)
                {
                    //S17 is event type for outbound SIU messages when we delete an appointment
                    return "Deleted";
                }
            ...
            case "apt.externalAptID":
                ...
                var listAptOidsExt = OIDExternals.GetByInternalIdAndType(apt.AptNum, IdentifierType.Appointment);
            ...
            case "apt.location":
                ...
                var aptClinicDescript = Clinics.GetDesc(apt.ClinicNum);
                var opCur = Operatories.GetOperatory(apt.Op);
                var opName = "";
                if (opCur != null)
                {
                    opName = opCur.OpName;
                ...
                var practiceName = PrefC.GetString(PrefName.PracticeTitle);
            ...
            case "guar.HmPhone":
                ...
                var hmPh = gXTN(guar.HmPhone, 10);
                var cPh = gXTN(guar.WirelessPhone, 10);
            ...
            case "guarIdList":
                ...
                var guarOid = OIDInternals.GetForType(IdentifierType.Patient);
                var guarOidRoot = "";
                if (guarOid != null)
                {
                ...
                var guarIdCheckDigitStr = MessageParser.M11CheckDigit(guar.PatNum.ToString()).ToString();
                ...
                var listGuarOidsExt = OIDExternals.GetByInternalIdAndType(guar.PatNum, IdentifierType.Patient);
                for (var i = 0; i < listGuarOidsExt.Count; i++)
                {
                    guarIdCheckDigitStr = MessageParser.M11CheckDigit(listGuarOidsExt[i].IDExternal).ToString();
                    if (guarIdCheckDigitStr == "-1")
                    {
                        //could not get a check digit from the external ID, could contain characters that are not numbers
            ...
            case "pat.location_V2_3":
                ...
                var patClinicDescript = Clinics.GetDesc(pat.ClinicNum);
            ...
            case "pat.site":
                ...
                var patSiteDescript = Sites.GetDescription(pat.SiteNum);
                if (patSiteDescript == "")
                {
                    return "";
                }
            ...
            case "patientIds":
                //Example: |1234^3^M11^&2.16.840.1.113883.3.4337.1486.6566.2&HL7^PI~7684^8^M11^&Other.Software.OID&^PI|
                var patOid = OIDInternals.GetForType(IdentifierType.Patient);
                var patOidRoot = "";
                if (patOid != null)
                {
                ...
                var patIdCheckDigitStr = MessageParser.M11CheckDigit(pat.PatNum.ToString()).ToString();
                ...
                var listPatOidsExt = OIDExternals.GetByInternalIdAndType(pat.PatNum, IdentifierType.Patient);
                for (var i = 0; i < listPatOidsExt.Count; i++)
                {
                    patIdCheckDigitStr = MessageParser.M11CheckDigit(listPatOidsExt[i].IDExternal).ToString();
                    if (patIdCheckDigitStr == "-1")
                    {
                        //could not get a check digit from the external ID, could contain characters that are not numbers
            ...
            case "proc.DiagnosticCode":
                ...
                var listDiagCodes = new List<string>();
                ...
                for (var i = 0; i < listDiagCodes.Count; i++)
                {
                    ...
                    var icd9Cur = Icd9s.GetByCode(listDiagCodes[i]);
                    var icd10Cur = Icd10s.GetByCode(listDiagCodes[i]);
            ...
            case "proc.location":
                //Point of Care^Room^^Facility^^Person Location Type
                //Example: ClinicDescript^OpName^^&PracticeTitle^^C  (C for clinic)
                if (proc == null || (proc.ClinicNum == 0 && pat.ClinicNum == 0))
                {
                    //if proc is null and both pat.ClinicNum and proc.ClinicNum are 0, return empty string
                    return "";
                }

                var procClinicDescript = Clinics.GetDesc(proc.ClinicNum); //could be blank if proc.ClinicNum is invalid
                ...
                if (apt != null)
                {
                    var procOp = Operatories.GetOperatory(apt.Op);
                    if (procOp != null)
                    {
            ...
            case "prov.provIdNameLFM_V2_3":
                ...
                var listProvOidExt = OIDExternals.GetByInternalIdAndType(prov.Id, IdentifierType.Provider);
                string provName;
                var provAbbr = fieldName.EndsWith("_V2_3") ? "" : prov.Abbr;
                if (fieldName.In("prov.provIdName", "prov.provIdName_V2_3"))
                {
                    provName = gConcat(def.ComponentSeparator, prov.LastName + ", " + prov.FirstName + " " + prov.MiddleName + ", " + prov.Suffix, "");
                }
                else
                {
                    provName = gConcat(def.ComponentSeparator, prov.LastName, prov.FirstName, prov.MiddleName, prov.Suffix);
                ...
                retval = gConcat(def.ComponentSeparator, OIDInternals.GetForType(IdentifierType.Provider).IDRoot + "." + prov.Id, provName, provAbbr);
                foreach (var oidExt in listProvOidExt)
                {
            ...
            case "sendingApp":
                ...
                var oidRoot = OIDInternals.GetForType(IdentifierType.Root);
                if (oidRoot == null)
                {
                    return "OD";
                }
    ...
    public static string GenerateFieldIN1(HL7Def def, string fieldName, int sequenceNum, PatPlan patplanCur, InsSub inssubCur, InsPlan insplanCur, Carrier carrierCur, int patplanCount, Patient patSub)
    {
        switch (fieldName)
        {
            ...
            case "carrier.Phone":
                ...
                var carrierPh = gXTN(carrierCur.Phone, 10);
                if (carrierPh == "")
                {
                    return "";
                }
            ...
            case "insplan.planNum":
                ...
                var insplanOid = OIDInternals.GetForType(IdentifierType.InsPlan); //returns root+".7" or whatever they have set in their oidinternal table for type InsPlan
                var insplanOidRoot = "ODInsPlan";
    ...
    private static string gRaceOld(Patient pat)
    {
        ...
        {
            //Uses the PatientRaceOld enum converted from the patient's current list of PatientRaces
    ...
    private static string gRace(Patient pat, HL7Def def, bool isV2_3)
    {
        ...
        var listPatRaces = PatientRaces.GetForPatient(pat.PatNum).FindAll(x => !string.IsNullOrEmpty(x.CdcrecCode));
        if (listPatRaces.Count == 0)
        {
            return "";
        }

        var listRepetitions = listPatRaces.Select(x => gConcat(def.ComponentSeparator, x.CdcrecCode, x.Description)).ToList();
        if (!isV2_3)
        {
    ...
    private static string gTreatArea(string componentSep, Procedure proc, bool isQuadAsToothNum)
    {
        ...
        else if (procCode.TreatArea == TreatmentArea.Surf)
        {
            ...
            retVal = gConcat(componentSep, Tooth.Display(proc.ToothNum), Tooth.SurfTidyForClaims(proc.Surf, proc.ToothNum));
        }
        ...
        {
            retVal = gConcat(componentSep, Tooth.Display(proc.ToothNum), proc.Surf);
        }
