...
namespace OpenDentBusiness.HL7 {
	...
	public static class MessageConstructor {
		...
		public static MessageHL7 GenerateDFT(List<Procedure> listProcs,EventTypeHL7 eventType,Patient pat,Patient guar,long aptNum,string pdfDescription,string pdfDataString) {
			...
			for(int i=0;i<hl7DefMessage.ListHL7DefSegments.Count;i++) {
				int repeatCount=1;
				...
				for(int j=0;j<repeatCount;j++) {//FT1 is optional and can repeat so add as many FT1's as procs in procList, IN1 is optional and can repeat as well, repeat for the number of patplans in patplanList
					...
					SegmentHL7 seg=new SegmentHL7(hl7DefMessage.ListHL7DefSegments[i].SegmentName);
					seg.SetField(0,hl7DefMessage.ListHL7DefSegments[i].SegmentName.ToString());
					for(int f=0;f<hl7DefMessage.ListHL7DefSegments[i].hl7DefFields.Count;f++) {
						string fieldName=hl7DefMessage.ListHL7DefSegments[i].hl7DefFields[f].FieldName;
		...
		public static MessageHL7 GenerateADT(Patient pat,Patient guar,EventTypeHL7 eventType) {
			...
			for(int i=0;i<hl7DefMessage.ListHL7DefSegments.Count;i++) {
				...
				for(int j=0;j<countRepeat;j++) {//IN1 is optional and can repeat so add as many as listPatplans
					...
					Carrier carrierCur=null;
					Patient patSub=null;
					if(hl7DefMessage.ListHL7DefSegments[i].SegmentName==SegmentNameHL7.IN1) {//index repeat is guaranteed to be less than listPatplans.Count
						patplanCur=listPatPlans[j];
						inssubCur=InsSubs.GetOne(patplanCur.InsSubNum);
						insplanCur=InsPlans.RefreshOne(inssubCur.PlanNum);
						carrierCur=Carriers.GetCarrier(insplanCur.CarrierNum);
					...
					SegmentHL7 seg=new SegmentHL7(hl7DefMessage.ListHL7DefSegments[i].SegmentName);
					seg.SetField(0,hl7DefMessage.ListHL7DefSegments[i].SegmentName.ToString());
					for(int k=0;k<hl7DefMessage.ListHL7DefSegments[i].hl7DefFields.Count;k++) {
						string fieldName=hl7DefMessage.ListHL7DefSegments[i].hl7DefFields[k].FieldName;
						if(fieldName=="") {//If fixed text instead of field name just add text to segment
		...
		public static MessageHL7 GenerateACK(string controlId,bool isAck,string ackEvent) {
			...
			for(int s=0;s<hl7DefMessage.ListHL7DefSegments.Count;s++) {
				SegmentHL7 seg=new SegmentHL7(hl7DefMessage.ListHL7DefSegments[s].SegmentName);
				seg.SetField(0,hl7DefMessage.ListHL7DefSegments[s].SegmentName.ToString());
				for(int f=0;f<hl7DefMessage.ListHL7DefSegments[s].hl7DefFields.Count;f++) {
					string fieldName=hl7DefMessage.ListHL7DefSegments[s].hl7DefFields[f].FieldName;
					if(fieldName=="") {//If fixed text instead of field name just add text to segment
						seg.SetField(hl7DefMessage.ListHL7DefSegments[s].hl7DefFields[f].OrdinalPos,hl7DefMessage.ListHL7DefSegments[s].hl7DefFields[f].FixedText);
					}
					else {
						seg.SetField(hl7DefMessage.ListHL7DefSegments[s].hl7DefFields[f].OrdinalPos,FieldConstructor.GenerateFieldACK(hl7Def,fieldName,controlId,isAck,ackEvent));
					}
				}
				msgHl7.Segments.Add(seg);
			}
				...
				for(int j=0;j<listProvs.Count;j++) {//AIP will be repeated if there is a dentist and a hygienist on the appt
					...
					for(int k=0;k<hl7DefMessage.ListHL7DefSegments[i].hl7DefFields.Count;k++) {
						...
						if(fieldName=="") {//If fixed text instead of field name just add text to segment
							...
							string fieldValue="";
							if(hl7DefMessage.ListHL7DefSegments[i].SegmentName==SegmentNameHL7.MSA) {
