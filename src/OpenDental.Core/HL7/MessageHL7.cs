...
{
    public readonly List<SegmentHL7> Segments;
    public readonly MessageTypeHL7 MsgType;
    public readonly EventTypeHL7 EventType;
    public string ControlId;
    public readonly string AckEvent;
    ...
    internal MessageHL7(MessageTypeHL7 msgType)
    {
        ...
        var enabledDef = HL7Defs.GetOneDeepEnabled();
        if (enabledDef != null)
        {
    ...
    public MessageHL7(string msgtext)
    {
        ...
        AckEvent = "";
        Segments = new List<SegmentHL7>();
        var rows = msgtext.Split(new string[] {"\r", "\n"}, StringSplitOptions.RemoveEmptyEntries);
        ...
        if (enabledDef != null)
        {
            for (var i = 0; i < rows.Length; i++)
            {
                ...
                var fields = rows[i].Split(new string[] {"|"}, StringSplitOptions.None);
...
//js 7/3/12 Make this more intelligent because we also now need the suffix
                var msgtype = segment.GetFieldComponent(8, 0); //We force the user to leave the 'messageType' field in this position, position 8 of the MSH segment
                var evnttype = segment.GetFieldComponent(8, 1);
                var msgStructure = segment.GetFieldComponent(8, 2);
    ...
    {
        var retVal = new List<SegmentHL7>();
        for (var i = 0; i < Segments.Count; i++)
        {
            if (Segments[i].Name != segmentName)
            {
    ...
    public bool TryGetSegmentOrder(SegmentNameHL7 segmentName, HL7DefMessage hl7defmsg, out int segmentOrder, out int segmentDefOrder)
    {
        ...
        segmentDefOrder = hl7defmsg.ListHL7DefSegments.FindIndex(x => x.SegmentName == segmentName);
