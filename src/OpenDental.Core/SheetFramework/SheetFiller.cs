...
{
    public static string FillFields(Sheet sheet, DataSet dataSet = null, Statement stmt = null, Patient patient = null, Family family = null, StaticTextData staticTextData = null, long refAttachProvNum = 0)
    {
        ...
        switch (sheet.SheetType)
        {
            ...
            case SheetTypeEnum.DepositSlip:
                var deposit = Deposits.GetOne((long) GetParamByName(sheet, "DepositNum").ParamValue);
        ...
        long aptNum = 0;
        if (sheetParameterAptNum is {ParamValue: not null})
        {
    ...
    public static List<long> GetListProcCodeNumsForStaticText(out List<long> listCodeNumsPano, out List<long> listCodeNumsExam, out List<long> listCodeNumsProphy, out List<long> listCodeNumsPerio, out List<long> listCodeNumsBW, out List<long> listCodeNumsSRP)
    {
    ...
    public static List<StaticTextReplacement> GetStaticTextReplacements(List<EnumStaticTextField> listEnumStaticTextFields, Patient patient, Family family, StaticTextData staticTextData, StaticTextFieldDependency staticTextFieldDependency, long aptNum, SheetTypeEnum sheetType = SheetTypeEnum.None)
    {
        ...
        if (patient != null)
        {
            ...
            {
                var procsList = staticTextData.ListProceduresPat; //there is another variable that does the same thing. Carefully combine them.
            ...
            if (ordinal == 0)
            {
                //No primary dental plan. See if they have a medical plan instead.
            ...
            {
                var treatPlan = treatPlanList[treatPlanList.Count - 1].Copy();
            ...
            for (var i = 0; i < proceduresList.Count; i++)
            {
                ...
                if (listCodeNumsBW.Contains(proc.CodeNum) && proc.ProcDate > dBW)
                {
                    //newest
                    dBW = proc.ProcDate;
            ...
            for (var i = 0; i < listRecalls.Count; i++)
            {
                ...
                {
                    //don't care about recalls that are disabled.
                    continue;
                }

                if (listRecalls[i].DisableUntilDate > DateTime.Today)
                {
                    //don't care about recalls that are disabled until the future
                    continue;
                }
            ...
            for (var i = 0; i < apptFutureList.Count; i++)
            {
            ...
            if (providerPri == null)
            {
                //Rare, temporary fix. We have previously seen this issue happen when a provider is removed from the DB somehow.
                providerPri = new ProviderDto()
                {
                    //If Provider is missing this will throw a UE where we attempt to use GetFullName(...)
                    FirstName = "",
                    LastName = "",
            ...
            for (var i = 0; i < listAllergies.Count; i++)
            {
                ...
                try
                {
                    //unknown methods can cause the allergy to be deleted sometimes
            ...
            {
                if (listFamPopups[i].DateTimeDisabled != DateTime.MinValue && listFamPopups[i].DateTimeDisabled < DateTime.Now)
                {
                    //If disabled
                    continue;
                }
        ...
        if ((sheet.SheetType == SheetTypeEnum.PatientLetter || sheet.SheetType == SheetTypeEnum.ReferralSlip || sheet.SheetType == SheetTypeEnum.ReferralLetter) && patient != null)
        {
            for (var f = 0; f < sheet.SheetFields.Count; f++)
            {
                ...
                while (match.Success)
                {
                    ...
                    if (examFields[0].RadioButtonGroup != "")
                    {
                    ...
                    else if (examFields[0].ReportableName != "")
                    {
                        //not a radio button, so either user defined single misc check boxes, combobox, or misc input field with reportable name
                    ...
                    else if (examFields[0].FieldName != "" && examFields[0].FieldName != "misc")
                    {
                        //internally defined
                        if (examFields[0].RadioButtonValue == "")
                        {
        ...
        foreach (var field in sheet.SheetFields)
        {
            switch (field.FieldName)
            {
                ...
                case "referredFrom":
                    ...
                    break;
                case "SSN":
                    if (CultureInfo.CurrentCulture.Name == "en-US" && pat.SSN.Length == 9)
                    {
        ...
        foreach (var field in sheet.SheetFields)
        {
            switch (field.FieldName)
            {
                ...
                case "Preferred":
                    ...
                    break;
                case "Race":
                    if (field.RadioButtonValue == PatientRaces.GetPatientRaceOldFromPatientRaces(pat.PatNum).ToString())
                    {
                        //==pat.Race.ToString()) {
                        field.FieldValue = "X";
                    }
                ...
                case "billingAddress":
                    ...
                    if (!string.IsNullOrWhiteSpace(patientGuar.Country))
                    {
                        if (CultureInfo.CurrentCulture.Name.EndsWith("CH") || CultureInfo.CurrentCulture.Name.EndsWith("SG"))
                        {
                            ...
                            if (!string.IsNullOrWhiteSpace(patientGuar.City + patientGuar.Zip))
                            {
                        ...
                        {
                            //all other cultures
                            if (!string.IsNullOrWhiteSpace(patientGuar.City + patientGuar.State + patientGuar.Zip))
                            {
    ...
    private static string[] totInsBalLabsHelper(Sheet sheet, Statement statement)
    {
        ...
        if (statement.IsInvoice)
        {
            //invoices can't be superstatements
    ...
    private static string[] CalcStatementVals(Sheet sheet, Statement statement, DataSet dataSet, Patient patient, Patient patientGuar)
    {
        ...
        if (statement.IsInvoice)
        {
            ...
            {
                var tableAcct = dataSet.Tables[i];
                var tableName = tableAcct.TableName;
        ...
        else
        {
            ...
            if (statement.SinglePatient)
            {
                for (var m = 0; m < tableMisc.Rows.Count; m++)
                {
                    ...
                    {
                        var amtPatInsEst = SIn.Double(tableMisc.Rows[m]["value"].ToString());
                    }
                }
        ...
        foreach (var field in sheet.SheetFields)
        {
            switch (field.FieldName)
            {
                ...
                case "ccNumberMaskedWithExp":
                    field.FieldValue = listCreditCard.IsNullOrEmpty() ? "" : string.Join(", ", listCreditCard.Select(x => $"{x.CCNumberMasked} Exp {x.CCExpiration.ToString("MM/yy")}"));
                    break;
    ...
    private static void FillFieldsForERA(Sheet sheet)
    {
        ...
        foreach (var field in sheet.SheetFields)
        {
            field.FieldValue = field.FieldName switch
            {
                "PayerName" => era.PayerName,
                "PayerID" => era.PayerId,
                "PayerAddress" => era.PayerAddress,
                "PayerCity" => era.PayerCity,
                "PayerState" => era.PayerState,
                "PayerZip" => era.PayerZip,
                "PayerContactInfo" => era.GetPayerContactInfo(),
                "PayeeName" => era.PayeeName,
                "PayeeId" => era.PayeeId,
                "TransHandlingDesc" => era.TransactionHandlingDescript,
                "PaymentMethod" => era.PayMethodDescript,
                "AcctNumEndingIn" => era.AccountNumReceiving,
                "Check#" => era.TransRefNum,
                "DateEffective" => era.DateEffective.ToShortDateString(),
                "InsPaid" => era.InsPaid.ToString("C2"),
                _ => field.FieldValue
            };
        }
    }
        ...
        foreach (var field in sheet.SheetFields)
        {
            switch (field.FieldName)
            {
                ...
                case "ClaimIndexNum":
                    ...
                    if (claimIndex != 0)
                    {
                        //Is 0 when IsSingleClaim parameter is true.
