...
namespace OpenDentBusiness {
	public class SheetFiller {
		...
		private static void FillFieldsForStatement(Sheet sheet,Statement statement,DataSet dataSet,Patient patient,Patient patientGuar=null) {
			...
			string[] stringArrayTotInsBalVals=CalcStatementVals(sheet,statement,dataSet,patient,patientGuar);
			string[] totInsBalLabs=totInsBalLabsHelper(sheet,statement);
			if(tableAppt==null){
				...
				switch(field.FieldName) {
					...
					case "statementShortURL":
						...
						try {
							...
							field.FieldValue=(field.FieldName=="statementURL" ? statement.StatementURL : statement.StatementShortURL);
							if(field.FieldValue!="") {
		...
		private static string AddressHelper(ClinicDto clinic) {
			string text;
			if(true && clinic!=null) {
		...
		private static string CityStateHelper(ClinicDto clinic) {
			string text;
			if(true && clinic!=null) {
		...
		private static string PhoneHelper(ClinicDto clinic) {
			string text;
			if(true && clinic!=null) {
