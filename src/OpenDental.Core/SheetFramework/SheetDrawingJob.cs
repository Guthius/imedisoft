...
namespace OpenDentBusiness {
	public class SheetDrawingJob {
		...
		///<summary>Used for API. Creates a file where the fullFileName is a local path. Can throw exceptions.</summary>
		public void CreatePdfForApi(Sheet sheet,string fullFileName) {
			PdfDocument pdfDocument=CreatePdf(sheet);
			SavePdfToFile(pdfDocument,fullFileName);
		}

		///<summary>Creates a PDF in memory and returns it, does not save to disk.<br>Use SheetDrawingJob.SavePdfToFile(PdfDocument pdf, string fileNameAndPath) to save to disk.</br></summary>
		public PdfDocument CreatePdf(Sheet sheet,Statement stmt=null,MedLab medLab=null,DataSet dataSet=null,Patient pat=null,Patient patGuar=null) {
		...
		public static bool SavePdfToFile(PdfDocument pdf, string fullPathAndFilename) {
		...
		public void DrawFieldGrid(SheetField field,Sheet sheet,Graphics g,XGraphics gx,DataSet dataSet,Statement stmt,MedLab medLab,
			bool isPrinting=false,Patient pat=null,Patient patGuar=null,float scaleMS=1) 
		{
			if(sheet.SheetType!=SheetTypeEnum.ERA) {
				DrawFieldGridHelper(field,sheet,g,gx,dataSet,stmt,medLab,isPrinting,pat: pat,patGuar: patGuar,scaleMS:scaleMS);
				return;
			}
			...
			for(int i=_idxPreClaimPaidPrinted; i<era.ListClaimsPaid.Count; i++) {
				...
				int codeGridHeight=DrawFieldGridHelper(fieldCodes,sheet,g,gx,dataSet,stmt,medLab,isPrinting,claim.ClpSegmentIndex,tableCodes);
				int subFieldHeight=sheetGridHeaderCopy.Height+procGridHeight+codeGridHeight;
				if(isOneClaimPerPage) {
		...
		public int DrawFieldGridHelper(SheetField sheetField,Sheet sheet,Graphics g,XGraphics gx,DataSet dataSet,Statement stmt,MedLab medLab,
			bool isPrinting=false,long eraClaimSegmentIdx=-1,DataTable table=null,Patient pat=null,Patient patGuar=null,float scaleMS=1) 
		{
			...
			List<DisplayField> listDisplayFields=SheetUtil.GetGridColumnsAvailable(sheetField.FieldName);
			if(table==null) {
				table=SheetDataTableUtil.GetDataTableForGridType(sheet,dataSet,sheetField.FieldName,stmt,medLab,patGuar: patGuar);
			}
			...
			foreach(DisplayField colCur in listDisplayFields) {
				...
				switch(sheetField.FieldName+"."+colCur.InternalName) {//Unusual switch statement to differentiate similar column names in different grids.
			...
			foreach(DataRow rowCur in table.Rows) {
				...
				foreach(DisplayField colCur in listDisplayFields) {
					...
					string value=SheetUtil.GetStringFromInternalName(sheetField,colCur,rowCur);
					row.Cells.Add(value);
				}
				...
				if(gridSheetRow.IsTitleRow) {
					...
					switch(sheetField.FieldName) {//Draw titles differently for different grids.
						...
						case "StatementInvoicePayment":
							...
							if(gx==null) {
								SizeF sizeFString=g.MeasureString("Payments & WriteOffs",font);
								g.FillRectangle(Brushes.White,sheetField.XPos,gridSheetRow.YPos-_yPosPrint,odGrid.Width,heightGridTitle);
						...
						case "TreatPlanBenefitsFamily":
							if(gx==null) {
								SizeF sizeFString=g.MeasureString("Family Insurance Benefits",font);
								g.FillRectangle(Brushes.White,sheetField.XPos,gridSheetRow.YPos-_yPosPrint,odGrid.Width,heightGridTitle);
						...
						case "TreatPlanBenefitsIndividual":
							if(gx==null) {
								SizeF sizeFString=g.MeasureString("Individual Insurance Benefits",font);
								g.FillRectangle(Brushes.White,sheetField.XPos,gridSheetRow.YPos-_yPosPrint,odGrid.Width,heightGridTitle);
