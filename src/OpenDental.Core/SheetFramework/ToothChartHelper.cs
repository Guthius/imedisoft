...
public class ToothChartHelper{
		...
		,List<PerioMeasure> listPerioMeasures=null,int width=500,int height=370,bool isSmaller=false,bool isForChartModule=false) 
	{
		...
		var toothInitialList=patNum==0?new List<ToothInitial>():ToothInitials.GetPatientData(patNum);
		...
		//Setup perio lists
		var dictPrefToColor=new Dictionary<PrefName,Color>();
		var listProcsForPerio=new List<Procedure>();
		var listProcedureCodesForPerio=new List<ProcedureCode>();
		var listDefsPerio=new List<Def>();
		if(isForPerio){
		...
		var args=GetToothChartData(listDefs,
	...
	public static Image GetToothChartImageProcess(List<Def> ListDefs,List<Procedure> ListProceduresFiltered,List<ToothInitial> ToothInitialList,ToothNumberingNomenclature ToothNumberingNomenclature,
		...
		if(IsForPerio) {
			...
			for(var t=1;t<=32;t++){
				var listProcsForTooth=ListProcsForPerio.FindAll(x=>x.ToothNum==t.ToString() && (x.ProcStatus==ProcStat.C || x.ProcStatus==ProcStat.EC || x.ProcStatus==ProcStat.EO));
				var isImplant=false;
				var isCrown=false;
				for(var p=0;p<listProcsForTooth.Count;p++) {
					var procedureCode=ProcedureCodes.GetProcCode(listProcsForTooth[p].CodeNum,ListProcedureCodesForPerio); //Passing in list skips db
			...
			switch(ProcedureCodes.GetProcCode(proc.CodeNum,listProcedureCodes).PaintType) {
				...
				case ToothPaintingType.SpaceMaintainer:
					        ...
					        || (ProcedureCodes.GetProcCode(proc.CodeNum,listProcedureCodes).AreaAlsoToothRange==true && proc.ToothRange!=""))
					{
						...
						for(var t=0;t<teeth.Length;t++) {//could still be length 0
							toothChart.SetSpaceMaintainer(teeth[t],cDark);
						}
					}
					break;
	...
	public static ToothChartHelperData GetToothChartData(List<Def> listDefs,List<Procedure> listProceduresFiltered,List<ToothInitial> toothInitialList,
		...
		var listProcedureCodesForProcs=ProcedureCodes.GetWhere(x=>x.CodeNum.In(listProceduresFiltered.Select(x=>x.CodeNum).ToArray()));
		var toothChartHelperData=new ToothChartHelperData();
