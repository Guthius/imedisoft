	...
	public class SheetUtil {
					...
					case SheetFieldType.Grid:
						if(sheet.SheetType==SheetTypeEnum.ERA) {
							...
							foreach(Hx835_Claim claim in era.ListClaimsPaid) {
								...
								SheetField fieldCodes=fieldCopy.Copy();
								fieldCodes.FieldName="EraClaimsPaidCodes";
								fieldCodes.YPos=fieldCopy.YPos+procGridHeight;
								int codeGridHeight=CalculateGridHeightHelper(fieldCodes,sheet,stmt,topMargin,bottomMargin,medLab,dataSet,claim.ClpSegmentIndex,tableCodes);
								int subFieldHeight=gridHeaderSheetDef.Height+procGridHeight+codeGridHeight;
		...
		public static Sheet CreateSheetFromWebSheet(long PatNum,WebForms_Sheet webFormsSheet) {
			...
			foreach(WebForms_SheetField field in webFormsSheet.SheetFields) {
				...
				sheetfieldCopy.SheetFieldDefNum=field.SheetFieldDefNum;
			...
			switch(sheetFieldDef.GrowthBehavior) {
				...
				case GrowthBehaviorEnum.FillDownFitColumns://Only grids have access to this option.
					...
					if(control is GridOD gridOD) {
						...
						Control controlGrid=control.Controls.OfType<Control>().FirstOrDefault(x => x is GridOD);
						if(controlGrid != null){
