...
public class SheetUtil {
	...
	public static void CalculateHeights(Sheet sheet,DataSet dataSet=null,Statement stmt=null,bool isPrinting=false,
		int topMargin=40,int bottomMargin=60,Patient pat=null,Patient patGuar=null)
	{
		int calcH;
		FontStyle fontstyle;
		...
		foreach(var field in sheet.SheetFields) {
			...
			if(field.FieldType==SheetFieldType.Image || field.FieldType==SheetFieldType.PatImage) {
				...
				switch(field.FieldType) {
					...
					case SheetFieldType.PatImage:
						...
						if(field.FieldValue.StartsWith("MountNum:")){
							...
							var document=Documents.GetByNum(SIn.Long(field.FieldValue));
							var paths=Documents.GetPaths(new List<long> { document.DocNum },ImageStore.GetDataFolder());
				...
				if(field.FieldType==SheetFieldType.PatImage){
					//don't adjust patImages to 0 height because we leave empty placeholders that user can edit.
				}
			...
			var printableHeight=sheet.Height-topMargin-bottomMargin;
			if((field.FieldType==SheetFieldType.SigBox || field.FieldType==SheetFieldType.SigBoxPractice) && field.Height<=printableHeight) {//if Sigbox and can fit a single page
				var currentPageNum=Convert.ToInt32(Math.Ceiling((double)(field.YPos-topMargin)/printableHeight));
				var bottomCurPage=SheetPrinting.BottomCurPage(field.YPos,sheet,out var pageCount);
				if(field.YPos<=bottomCurPage && field.YPos+field.Height>bottomCurPage) {//Field is split between 2 pages.
					var yPosNew=(currentPageNum*printableHeight)+topMargin+1;//Set to top of next page, plus 1 for padding.
					var amountChanged=yPosNew-field.YPos;
			...
			switch(field.FieldType) {
				case SheetFieldType.Grid:
					if(sheet.SheetType==SheetTypeEnum.ERA) {
						...
						var era=(X835)SheetParameter.GetParamByName(sheet.Parameters,"ERA").ParamValue;//Required field.
						var tableClaimsPaid=SheetDataTableUtil.GetDataTableForGridType(sheet,dataSet,field.FieldName,stmt);
						...
						calcH=0;
						foreach(var claim in era.ListClaimsPaid) {
							var pageMaxY=printableHeight*((calcH/printableHeight)+1);
							...
							var procGridHeight=CalculateGridHeightHelper(fieldCopy,sheet,stmt,topMargin,bottomMargin,dataSet,claim.ClpSegmentIndex,tableClaimsPaid);
							var fieldCodes=fieldCopy.Copy();
							fieldCodes.FieldName="EraClaimsPaidCodes";
							fieldCodes.YPos=fieldCopy.YPos+procGridHeight;
							var codeGridHeight=CalculateGridHeightHelper(fieldCodes,sheet,stmt,topMargin,bottomMargin,dataSet,claim.ClpSegmentIndex,tableCodes);
							var subFieldHeight=gridHeaderSheetDef.Height+procGridHeight+codeGridHeight;
						...
						calcH=CalculateGridHeightHelper(field,sheet,stmt,topMargin,bottomMargin,dataSet,pat: pat,patGuar: patGuar);
					}
					break;
				case SheetFieldType.Special:
					...
					break;
				default:
					using(var font=new Font(field.FontName,field.FontSize,fontstyle)) {
						var heightAndChars=GraphicsHelper.MeasureStringH(field.FieldValue,font,field.Width,int.MaxValue,field.TextAlign);
			...
			var heightOld=field.Height;
			var amountOfGrowth=calcH-field.Height;
			field.Height=calcH;
		...
		if(isPrinting && !Sheets.SheetTypeIsSinglePage(sheet.SheetType)) {
			...
			var originalSheetFieldCount=sheet.SheetFields.Count;
			for(var i=0;i<originalSheetFieldCount;i++) {
				var fieldCur=sheet.SheetFields[i];
	...
	public static List<Sheet> CreateBatch(SheetDef sheetDef,List<long> listPrimaryKeys) {
		...
		for(var i=0;i<listPrimaryKeys.Count;i++){
	...
	public static List<DisplayField> GetGridColumnsAvailable(string gridType) {
		...
		switch(gridType) {
			...
			case "EraClaimsPaid":
				...
				retVal.Add(new DisplayField { Category=DisplayFieldCategory.None,InternalName="FeeBilled",Description="Fee Billed",ColumnWidth=75,ItemOrder=++i });
				retVal.Add(new DisplayField { Category=DisplayFieldCategory.None,InternalName="PatResp",Description="Pat Resp.",ColumnWidth=75,ItemOrder=++i });
				retVal.Add(new DisplayField { Category=DisplayFieldCategory.None,InternalName="Contractual",Description="Contractual",ColumnWidth=75,ItemOrder=++i });
				retVal.Add(new DisplayField { Category=DisplayFieldCategory.None,InternalName="PayorReduct",Description="Payor Red.",ColumnWidth=75,ItemOrder=++i });
				retVal.Add(new DisplayField { Category=DisplayFieldCategory.None,InternalName="OtherAdjust",Description="Other Adj",ColumnWidth=75,ItemOrder=++i });
				retVal.Add(new DisplayField { Category=DisplayFieldCategory.None,InternalName="InsPaid",Description="Ins Paid",ColumnWidth=75,ItemOrder=++i });
				retVal.Add(new DisplayField { Category=DisplayFieldCategory.None,InternalName="AdjCodes",Description="Adj Codes",ColumnWidth=90,ItemOrder=++i });
			...
			case "EraClaimsPaidCodes":
				retVal.Add(new DisplayField { Category=DisplayFieldCategory.None,InternalName="Code",Description="Code",ColumnWidth=70,ItemOrder=++i });
				retVal.Add(new DisplayField { Category=DisplayFieldCategory.None,InternalName="Description",Description="Description",ColumnWidth=730,ItemOrder=++i });
				break;
			case "ApptsGrid":
			...
			case "ProgressNotes":
				if(ChartViews.GetDeepCopy().Count==0) {
					...
					foreach(var view in ChartViews.GetDeepCopy()) {
						var listDisplayFields=DisplayFields.GetForChartView(view.ChartViewNum);
						var widthGridTotal=listDisplayFields.Sum(x => x.ColumnWidth);
	...
	public static string GetClinicImagePath(string fileName) {
		...
		if(true) {
			...
			var imagePathClinic=ODFileUtils.CombinePaths(imagePath,Clinics.ClinicNum.ToString());
	...
	public static int GetLowestYPos(List<SheetFieldDef> listSheetFieldDefs) {
		...
		foreach(var sheetFieldDef in listSheetFieldDefs) {
			var bottom=sheetFieldDef.YPos + sheetFieldDef.Height;
	...
	public static SheetDef GetMedLabResultsSheetDef() {
		var listDefs=SheetDefs.GetCustomForType(SheetTypeEnum.MedLabResults);
		if(listDefs.Count>0) {
	...
	public static SheetDef GetStatementSheetDef(Statement statement=null) {
		var listDefs=SheetDefs.GetCustomForType(SheetTypeEnum.Statement);
	...
	public static string GetStringFromInternalName(SheetField field,DisplayField displayFieldColumn,DataRow row) {
		...
		if(displayFieldColumn.InternalName=="DPlan") {
			...
			if(displayFieldColumn.InternalName=="Prov") {
				...
				var dateTP=SIn.Date(row["DateTP"].ToString());
				if(dateTP.Year>=1880) {
	...
	public static void MoveAllDownBelowThis(Sheet sheet,SheetField sheetField,int amountOfGrowth) {
		for(var i=0;i<sheet.SheetFields.Count;i++) {
			if(sheet.SheetFields[i].YPos>sheetField.YPos) {//for all fields that are below this one
				sheet.SheetFields[i].YPos+=amountOfGrowth;//bump down by amount that this one grew
			}
		}
	}
	...
	public static void MoveAllDownWhichIntersect(Sheet sheet,SheetField sheetField,int amountOfGrowth) {
		var boundsCur=sheetField.Bounds;
		...
		var affectedFields=sheet.SheetFields.FindAll(field => field!=sheetField 
		...
		for(var i=0;i<affectedFields.Count;i++){
	...
	public static void MoveAllUpBelowThis(Sheet sheet,SheetField sheetFieldToRemove,int heightOld) {
		...
		var amountOfShrink=(heightOld - sheetFieldToRemove.Height);
		var listOtherSheetFields=sheet.SheetFields.FindAll(x => x!=sheetFieldToRemove && x.Height > 0);//ignore previously shrunken fields.
		...
		var top=sheetFieldToRemove.YPos;
		var bottom=sheetFieldToRemove.YPos+heightOld;
		...
		var listDanglingSheetFields=listOtherSheetFields.FindAll(x => top > x.YPos
		                                                              && ((x.YPos + x.Height) > top)
		...
		if(listDanglingSheetFields.Count > 0) {
			...
			var deepestDangler=listDanglingSheetFields.Max(x => x.YPos + x.Height);
			var dangleShrink=(bottom - deepestDangler);
		...
		var listPeekingSheetFields=listOtherSheetFields.FindAll(x => x.YPos > top && x.YPos < bottom);
		if(listPeekingSheetFields.Count > 0) {
			...
			var topmostPeeker=listPeekingSheetFields.Min(x => x.YPos);
			var peekerShrink=(topmostPeeker - top);
	...
	public static void SetControlSizeAndAnchors(SheetFieldDef sheetFieldDef,Control control,Control controlParent,float scaleMy=1,int fillHeightAdj=0,bool doAnchorToBottom=true) {
		var dpiScale=new UI.DpiScale(scaleMy);
		...
		switch(sheetFieldDef.GrowthBehavior) {
			...
			case GrowthBehaviorEnum.FillDownFitColumns://Only grids have access to this option.
				...
				else if(control is GridOld gridOld) {
					width=gridOld.GetIdealWidth();
				}
				else{//Must contain a Grid directly then
					var controlGrid=control.Controls.OfType<Control>().FirstOrDefault(x => x is GridOD);
					if(controlGrid != null){
						width=((GridOD)controlGrid).GetIdealWidth();
					}
					controlGrid=control.Controls.OfType<Control>().FirstOrDefault(x => x is GridOld);
					if(controlGrid != null){
						width=((GridOld)controlGrid).GetIdealWidth();
					}
				}
	...
	public static void SetDefaultValueForComboBoxes(Sheet sheet) {
		foreach(var field in sheet.SheetFields.Where(x => x.FieldType==SheetFieldType.ComboBox)) {
	...
	public static List<Sheet> TryCreateBatch(SheetDef sheetDef,List<long> listPrimaryKeys) {
		var retVal=new List<Sheet>();
		SheetParameter paramNew;
		foreach(var key in listPrimaryKeys) {
			var newSheet=CreateSheet(sheetDef);
			newSheet.Parameters=new List<SheetParameter>();
			paramNew=new SheetParameter(sheetDef.Parameters[0].IsRequired,sheetDef.Parameters[0].ParamName);
	...
	public static string FixFontsForPdf(Sheet sheetCur,bool isPrinting=false) {
		...
		var hasErrors=false;
		var listStrBadFonts=new List<string>();
		try {// check if font is compatible with PDFSharp by running it through XFont, if it suceeds, add to the list, otherwise throw error.
			var _=new XFont(sheetCur.FontName,sheetCur.FontSize,XFontStyle.Regular);
		}
		catch {
			hasErrors=true;
		...
		for(var i=0;i<sheetCur.SheetFields.Count;i++){
			if(sheetCur.SheetFields[i].FieldType==SheetFieldType.StaticText
			   ...
			   ||sheetCur.SheetFields[i].FieldType==SheetFieldType.OutputText) {
				try {// check if font is compatible with PDFSharp by running it through XFont, if it suceeds, add to the list, otherwise throw error.
					var _=new XFont(sheetCur.SheetFields[i].FontName,sheetCur.SheetFields[i].FontSize,XFontStyle.Regular);
				}
				catch {
					hasErrors=true;
		...
		if(hasErrors) {
			var str=Lans.g("FormSheetFillEdit","Form is trying to save or print with unsupported font(s):");
	...
	private static int CalculateGridHeightHelper(SheetField field,Sheet sheet,Statement stmt,int topMargin,int bottomMargin,DataSet dataSet,long eraClaimSegmentIdx=-1,DataTable table=null,Patient pat=null,Patient patGuar=null) 
	{
		using var odGrid=new GridOD();
		odGrid.BeginUpdate();
		...
		if(table==null) {
			table=SheetDataTableUtil.GetDataTableForGridType(sheet,dataSet,field.FieldName,stmt,patGuar: patGuar);
		}
		var columns=GetGridColumnsAvailable(field.FieldName);
		...
		for(var i=0;i<table.Rows.Count;i++) {
			...
			row=new GridRow();
			for(var c=0;c<columns.Count;c++) {//Selectively fill columns from the dataTable into the odGrid.
				//Some DisplayFields require additional formatting and/or logic to display properly in a grid.
				var value=GetStringFromInternalName(field,columns[c],table.Rows[i]);
				row.Cells.Add(value);
			}
	...
	private static void CalculateHeightsPageBreakForText(SheetField field,Sheet sheet) {
		...
		var bottomCurPage=SheetPrinting.BottomCurPage(field.YPos,sheet,out var pageCount);
		var heightMaxToFit=bottomCurPage-field.YPos;//the max height that the new text box can be in order to fit on the current page.
		var heightCalculated=GraphicsHelper.MeasureStringH(field.FieldValue,font,field.Width,heightMaxToFit,field.TextAlign).Height;
		...
		if(field.GrowthBehavior!=GrowthBehaviorEnum.None && field.Height < heightCalculated) {//there is more text than will fit in current field height
			...
			var amtGrowth=heightCalculated-field.Height;
		...
		var charsCalculated=GraphicsHelper.MeasureStringH(field.FieldValue,font,field.Width,heightMaxToFit,field.TextAlign).Chars;
		...
		if(charsCalculated==0 || (heightMaxToFit*0.96f) < (font.Height+2)) {
			var moveAmount=bottomCurPage+1-field.YPos;
		...
		var heightFieldBeforeSplit=field.Height;
		...
		var heightCombinedFieldsAfterSplit=fieldNew.Height+field.Height;
		var heightGrowthAfterSplit=heightCombinedFieldsAfterSplit-heightFieldBeforeSplit;
