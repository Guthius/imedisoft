	...
	public class SheetDataTableUtil {
		...
		public static DataTable GetDataTableForGridType(Sheet sheet,DataSet dataSet,string gridType,Statement stmt,MedLab medLab,Patient patGuar = null) {
			...
			switch(gridType) {
				...
				case "EraClaimsPaidCodes":
					retVal=GetTable_EraClaimsPaidCodes(sheet);
					break;
		...
		private static DataTable GetTable_StatementEnclosed(DataSet dataSet,Statement statement,Patient patientGuar=null) {
			...
			if(statement.StatementType!=StmtType.LimitedStatement) {
				...
				if(statement.SuperFamily!=0) {//Superfamily statement
					List<InstallmentPlan> listInstallmentPlans=statement.ListInstallmentPlans??InstallmentPlans.GetForSuperFam(patientGuar.SuperFamily);
					if(listInstallmentPlans.Count>0) {
						double installmentPlanTotal=0;
						foreach(InstallmentPlan plan in listInstallmentPlans) {
		...
		private static DataTable GetTable_EraClaimsPaidCodes(Sheet sheet) {
			...
			for(int i=0;i<x835.ListClaimsPaid.Count;i++) {
				List<string> listClaimProcAdjReasonCodesIncluded=new List<string>();
				Hx835_Claim claimPaidCur=x835.ListClaimsPaid[i];
				List<Hx835_Adj> listClaimProcAdjs=claimPaidCur.ListProcs.SelectMany(x => x.ListProcAdjustments).ToList();
				for(int j=0;j<listClaimProcAdjs.Count;j++) {
			...
			for(int i=0;i<x835.ListClaimsPaid.Count;i++) {
				List<string> listRemittanceAdviceCodesIncluded=new List<string>();
				Hx835_Claim claimPaidCur=x835.ListClaimsPaid[i];
				List<Hx835_Remark> listRemittanceAdviceRemarks=claimPaidCur.ListProcs.SelectMany(x => x.ListRemarks).ToList();
				for(int j=0;j<listRemittanceAdviceRemarks.Count;j++) {
