...
public class SheetsInternal
{
    public static SheetDef GetSheetDef(SheetInternalType sheetInternalType)
    {
        switch (sheetInternalType)
        {
            ...
            case SheetInternalType.ChartModule:
                var sheetDef = GetSheetFromResource(Properties.Resources.ChartModuleStandardLayout);
                return sheetDef;
    ...
    private static SheetDef GetSheetFromResource(string xmlDoc)
    {
        ...
        foreach (var sheetFieldDef in sheetDef.SheetFieldDefs)
        {
            ...
            sheetFieldDef.FieldValue = Regex.Replace(sheetFieldDef.FieldValue, @"(?<!\r)\n", "\r\n");
        }
    ...
    {
        var listSheetInternalTypes = Enum.GetValues(typeof(SheetInternalType)).OfType<SheetInternalType>().ToList(); //all
        ...
        for (var i = 0; i < listSheetInternalTypes.Count; i++)
        {
            var sheetDef = GetSheetDef(listSheetInternalTypes[i]);
    ...
    {
        var sheet = new SheetDef(SheetTypeEnum.MedLabResults)
        {
            Description = "MedLab Result Report",
            FontName = "Arial",
            FontSize = 8.5f,
            Width = 850,
            Height = 1100
        ...
        sheet.SheetFieldDefs.Add(SheetFieldDef.NewStaticText("Account Address", sheet.FontSize, "Arial", false, 455, 123, 341, 15, itemColor: KnownColor.GrayText, textAlign: HorizontalAlignment.Center));
    ...
    {
        return GetSheetDef(GetInternalType(sheetType));
    }
}