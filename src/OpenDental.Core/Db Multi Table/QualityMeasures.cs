...
namespace OpenDentBusiness {
	...
	public class QualityMeasures {
		...
		public static QualityMeasure GetEhrCqmData(QualityType2014 qtype,DateTime dateStart,DateTime dateEnd,long provNum) {
			...
			switch(qtype) {
				...
				case QualityType2014.CariesPrevent_3:
					...
					//Get Fluoride varnish application procedures that occurred during the measurement period
					...
					#endregion
					#endregion
					#region Get Fluoride Varnish Application Procedures
		...
		private static Dictionary<long,List<EhrCqmMeasEvent>> GetTobaccoAssessmentEvents(List<long> listPatNums,DateTime dateEnd) {
			...
			DataTable tableEvents=DataCore.GetTable(command);
		...
		private static Dictionary<long,List<EhrCqmMedicationPat>> GetMedPats(List<long> listPatNums,List<string> listValueSetOIDs,DateTime dateStart,DateTime dateEnd) {
			...
			DataTable tableAllMedPats=DataCore.GetTable(command);
		...
		private static Dictionary<long,List<EhrCqmNotPerf>> GetNotPerformeds(List<long> listPatNums,List<string> listValueSetOIDs,List<string> listReasonOIDs,DateTime dateStart,DateTime dateEnd) {
			...
			DataTable tableNotPerfs=DataCore.GetTable(command);
		...
		private static Dictionary<long,List<EhrCqmProblem>> GetProblems(List<long> listPatNums,List<string> listValueSetOIDs,DateTime dateStart,DateTime dateEnd) {
			...
			DataTable tableAllProbs=DataCore.GetTable(command);
		...
		private static Dictionary<long,List<EhrCqmProc>> GetProcs(List<long> listPatNums,List<string> listValueSetOIDs,DateTime dateStart,DateTime dateEnd) {
			...
			DataTable tableAllProcs=DataCore.GetTable(command);
		...
		private static Dictionary<long,List<EhrCqmMedicationPat>> GetVaccines(List<long> listPatNums,List<string> listValueSetOIDs,DateTime dateStart,DateTime dateEnd,bool isGiven) {
			...
			DataTable tableAllVaccinePats=DataCore.GetTable(command);
				...
				case QualityType2014.CariesPrevent_3:
					...
					for(int i=0;i<alldata.ListEhrPats.Count;i++) {
						...
						if(alldata.DictPatNumListProcs.ContainsKey(patNumCur)) {
							alldata.ListEhrPats[i].IsNumerator=true;
							alldata.ListEhrPats[i].Explanation="This patient had an eligible encounter in the measurement period and a fluoride varnish application procedure with code "
								+alldata.DictPatNumListProcs[patNumCur][0].ProcCode+" on "+alldata.DictPatNumListProcs[patNumCur][0].ProcDate.ToShortDateString()+".";
							continue;
						}
						alldata.ListEhrPats[i].Explanation="This patient had an eligible encounter in the measurement period, but did not have a fluoride varnish application procedure with a valid code in the measurement period.";
					}
					break;
				#endregion
				#region ChildCaries
		...
		private static string GetNumeratorExplain2014(QualityType2014 qtype) {
			switch(qtype) {
				...
				case QualityType2014.WeightAdult:
					return "Patients who, for every encounter in the measurement period, the most recent physical examination documenting BMI occurred during the encounter or during the previous six months.  If the BMI was outside of normal parameters at any exam, a follow-up plan was documented during the exam or during the previous six months.";
				case QualityType2014.CariesPrevent:
					return "Childred with an eligible fluoride varnish procedure performed during the measurement period.";
				case QualityType2014.CariesPrevent_1:
					return "Childred with an eligible fluoride varnish procedure performed during the measurement period.";
				case QualityType2014.CariesPrevent_2:
					return "Childred with an eligible fluoride varnish procedure performed during the measurement period.";
