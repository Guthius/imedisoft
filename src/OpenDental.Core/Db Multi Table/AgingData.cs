...
namespace OpenDentBusiness {
	...
	public class AgingData {
			...
			bool excludeIfUnsentProcs,bool isSuperBills,List<long> listClinicNums,int daysExcludeInsPending=0)
		{
			...
			string whereAndClaimDateSent="";
			if(!listClinicNums.IsNullOrEmpty()) {
				whereAndClinNum=$@"AND {guarOrPat}.ClinicNum IN ({string.Join(",",listClinicNums)})";
			}
			if(daysExcludeInsPending>0) {//Only add to query if days are above 0, since this query line with a value of 0 would select nothing when we want to select everything pending.
