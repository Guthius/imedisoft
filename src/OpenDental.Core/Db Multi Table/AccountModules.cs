...
namespace OpenDentBusiness {
	public class AccountModules {
		...
		public class LoadData {
			...
			public string DataSetMainXml {
				get {
					...
					return XmlConverter.DataSetToXml(DataSetMain);
				}
				set {
					if(value==null) {
						DataSetMain=null;
					...
					DataSetMain=XmlConverter.XmlToDataSet(value);
				}
			}
		}

		///<summary>If intermingled=true, the patnum of any family member will get entire family intermingled. Can throw exceptions.</summary>
		public static LoadData GetAll(long patNum,DateTime fromDate,DateTime toDate,bool intermingled,bool showProcBreakdown,
			...
			Action actGetSuperFamMembers=new Action(() => Logger.LogAction("Patients.GetBySuperFamily",LogPath.AccountModule,() => retVal.SuperFamilyMembers=Patients.GetBySuperFamily(pat.SuperFamily)));
			Action actGetSuperFamGuarantors=new Action(() => Logger.LogAction("Patients.GetSuperFamilyGuarantors",LogPath.AccountModule,() => retVal.SuperFamilyGuarantors=Patients.GetSuperFamilyGuarantors(pat.SuperFamily)));
			Action actGetListPatFieldsSuperFam=new Action(() => Logger.LogAction("PatFields.GetPatFieldsForSuperFam",LogPath.AccountModule,() => retVal.ListPatFieldsSuperFam=PatFields.GetPatFieldsForSuperFam(retVal.SuperFamilyMembers.ConvertAll(x => x.PatNum).Distinct().ToList())));
			#endregion Actions
		...
		public static DataSet GetStatementDataSet(Statement stmt,bool isComputeAging=true,bool doIncludePatLName=false){
			...
			//Gets 3 tables: account(or account###,account###,etc), patient, payplan.
			DataSet retVal;
			if(stmt.SuperFamily!=0 || stmt.LimitedCustomFamily!=EnumLimitedCustomFamily.None) {//Superfamily statement, Intermingled and SinglePatient should always be false
		...
		public static DataSet GetSuperFamAccount(Statement statementCur,bool isComputeAging=true,bool doIncludePatLName=false
			,bool doShowHiddenPaySplits=false,bool doExcludeTxfrs=false,List<Patient> listPatients=null)
		{
			...
			bool doShowProcBreakdown=false;
			if(!statementCur.IsInvoice) {
			...
			List<DataRow> listDataRows=dataSetRetVal.Tables["account"].Rows.OfType<DataRow>().ToList();
			...
			foreach(DataRow dataRow in listDataRows) {
				...
				if(dataRow["ClaimPaymentNum"].ToString()=="0" && dataRow["ClaimNum"].ToString()!="0"){//claims
					dataRow["balance"]="";
				}
			...
			foreach(DataRow dataRow in listDataRowsPayPlan) {
				...
				if(dataRow["PayNum"].ToString()=="0" && dataRow["PayPlanChargeNum"].ToString()=="0") {//this is the payplan description row
			...
			DataTable dataTableAccountSorted=dataSetRetVal.Tables["account"].Clone();//Easy way to copy the columns.
			...
			DataTable dataTablePayPlanSorted=dataSetRetVal.Tables["payplan"].Clone();//Easy way to copy the columns.
		...
		///<summary>Also gets the patient table, which has one row for each family member. Also currently runs aging.  Also gets payplan table.  
		///If a statement object is provided, then it's for a statement, and the resulting payplan table looks totally different.
		...
		private static DataSet GetAccount(long patNum,DateTime fromDate,DateTime toDate,bool intermingled,bool singlePatient,long statementNum
			...
			,Patient pat,Family family,out decimal patientPayPlanDue,out decimal dynamicPayPlanDue,out decimal balanceForward,Statement statement=null
			,bool doComputeAging=true,bool doIncludePatLName=false,List<long> listPayPlanNumsExclude=null,bool doShowHiddenPaySplits=false,bool doExcludeTxfrs=false) 
		{
			...
			DataTable dataTableAccount=new DataTable("account");
			if(doComputeAging) {
				//run aging.-------------------------------------------------------
			...
			for(int i=0;i<rawAdj.Rows.Count;i++){
				...
				if(sumAmt!=0 && procNum==0) {
					...
					else if(!isStatement && showProcBreakdown) {//Not a statement, use user pref.
						dataRow["description"]+="\r\n"+Lans.g("AccountModule","Pat Paid: ")+sumAmt.ToString("c");
					}
				}
			...
			string familyPayPlanNums=String.Join(",",PayPlans.GetForPats(family.ListPats.Select(x=>x.PatNum).ToList(),pat.PatNum)
			...
			//Payment query 2
			if(!string.IsNullOrWhiteSpace(familyPatNums) && statement.StatementType!=StmtType.LimitedStatement) {//Should not show in limited statements
			...
			#region Statements
			//Statement----------------------------------------------------------------------------------------
			List<long> listPatNums=family.ListPats.ToList().Select(x => x.PatNum).Distinct().ToList();
			...
			if(payPlanVersionCur==PayPlanVersions.AgeCreditsAndDebits) { //this information is only required for v2
				...
				List<Patient> listFamilyMembers=family.ListPats.ToList();
			...
			if(!isStatement) {
				...
				DataTable rawInstall=DataCore.GetTable(command);
			...
			foreach(DataRow labRow in labRows) {
				int labRowIndex=listDataRows.FindIndex(x => x["ProcNum"].ToString()==labRow["ProcNumLab"].ToString())+1;//+1 insert just after proc
			...
			if(singlePatient) {//This is usually used for Account module grid.  Always gets used for superstatements.
				Patient patGuarantor=family.ListPats[0];
			...
			if(rowsByPat==null){//just one table
				bal=0;
				foreach(DataRow rowCur in listDataRows) {
			...
			if(rowsByPat==null && statement.StatementType!=StmtType.LimitedStatement) {//LimitedStatements don't have a balance forward row
				foundBalForward=false;
				for(int i=listDataRows.Count-1;i>=0;i--) {//go backwards and remove from end
				...
				if(foundBalForward && (listDataRows.Count>0 || !CompareDecimal.IsZero(balanceForward))) {//don't add bal forward row if it will be the only row and there is no balance
					dataRow=dataTableAccount.NewRow();
					SetBalForwardRow(dataRow,balanceForward,pnum);
					listDataRows.Insert(0,dataRow);
				}
			}
			else if(statement.StatementType!=StmtType.LimitedStatement) {//LimitedStatements don't have a balance forward row
			...
			if(rowsByPat==null){
				...
				DataTable tablep;
				for(int p=0;p<rowsByPat.Length;p++){
					Patient patRowCur=family.ListPats[p];
		...
		///<summary>Empty familyPatNums is for all patients.</summary>
		...
		public static int SortRowsForStatmentPrinting(DataRow x,DataRow y) {
			...
			if(x["AdjNum"].ToString()!="0" && y["AdjNum"].ToString()!="0") {//if RandomKeys enabled, sorted by dateTimeSort below
			...
			if(x["PayNum"].ToString()!="0" && y["PayNum"].ToString()!="0") {//if RandomKeys enabled, sorted by dateTimeSort below
			...
			if(x["ClaimNum"].ToString()!="0" && y["ClaimNum"].ToString()!="0") {
				...
				if(x["ClaimPaymentNum"].ToString()=="0" && y["ClaimPaymentNum"].ToString()=="0") {//if RandomKeys enabled, sorted by dateTimeSort below
			...
			if(x["PayPlanNum"].ToString()!="0" && y["PayPlanNum"].ToString()!="0") {//if RandomKeys enabled, sorted by dateTimeSort below
			...
			if(x["StatementNum"].ToString()!="0" && y["StatementNum"].ToString()!="0") {//if RandomKeys enabled, sorted by dateTimeSort below
		...
		public static Claim CreateClaim(Claim claim,string claimType,List<PatPlan> PatPlanList,
			...
			InsSub SubCur,Relat relatOther,List<Fee> listFees=null) 
		{
			...
			List<Procedure> listProcs=Procedures.SortListByTreatPlanPriority(listSelectedProcs);
			...
			switch(claimType){
				case "P":
					...
					InsPlan insPlanSec=InsPlans.RefreshOne(sub.PlanNum);
