...
namespace OpenDentBusiness {
	public class ChartModules {
		...
		public static DataTable GetProgNotes(long patNum,bool isAuditMode,ChartModuleFilters componentsToLoad=null) {
				...
				|| componentsToLoad.ShowConditions)
			{
				...
				foreach(DataRow rowProc in rawProcs.Rows) {
					...
					row["ProcDate"]=dateT;
					double amt = SIn.Double(rowProc["ProcFee"].ToString());
					int qty = SIn.Int(rowProc["UnitQty"].ToString()) + SIn.Int(rowProc["BaseUnits"].ToString());
					if(qty>0) {
						amt *= qty;
					}
			...
			if(componentsToLoad.ShowCommLog) {
				...
				for(int i=0;i<rawComm.Rows.Count;i++) {
					...
					row["CodeNum"]="";
					row["colorBackG"]=Color.White.ToArgb();
					long commTypeDefNum=SIn.Long(rawComm.Rows[i]["CommType"].ToString());
					...
					if(SIn.DateTime(rawComm.Rows[i]["DateTimeEnd"].ToString()).Year>1880) {
						DateTime startTime=SIn.DateTime(rawComm.Rows[i]["CommDateTime"].ToString());
						DateTime endTime=SIn.DateTime(rawComm.Rows[i]["DateTimeEnd"].ToString());
			...
			if(componentsToLoad.ShowRX) {
				...
				for(int i=0;i<rawRx.Rows.Count;i++) {
					...
					row["EmailMessageHideIn"]="0";
					row["EmailMessageHtmlType"]="0";
					RxTypes rxType=SIn.Enum<RxTypes>(rawRx.Rows[i]["RxType"].ToString());
					row["RxType"]=rxType;
					//If RxPat entry is a log of pdmp bridge access
					if(rxType!=RxTypes.Rx) {
						DateTime timeAccessed=SIn.DateTime(rawRx.Rows[i]["DateTStamp"].ToString());
			...
			if(componentsToLoad.ShowTasks) {
				...
				List<long> taskNums=rawTask.Select().Select(x => SIn.Long(x["TaskNum"].ToString())).ToList();
				...
				for(int i=0;i<rawTask.Rows.Count;i++) {
					...
					long taskListNum=SIn.Long(rawTaskRow["TaskListNum"].ToString());
					...
					long taskNum=SIn.Long(rawTaskRow["TaskNum"].ToString());
			...
			for(int i=0;i<rawApt.Rows.Count;i++) {
				...
				row["colorBackG"]=Color.White.ToArgb();
				dateT=SIn.DateTime(rawApt.Rows[i]["AptDateTime"].ToString());
				apptStatus=SIn.Long(rawApt.Rows[i]["AptStatus"].ToString());
				row["colorBackG"]="";
			...
			if(componentsToLoad.ShowSheetsEForms) {
				...
				for(int i=0;i<rawSheet.Rows.Count;i++) {
					...
					if(PrefC.GetBool(PrefName.ShowFeaturePatientClone) && rawSheet.Rows[i]["PatNum"].ToString()!=patNum.ToString()) {
						Patient patientClone=listPatientClonesAll.FirstOrDefault(x => x.PatNum==SIn.Long(rawSheet.Rows[i]["PatNum"].ToString()));
				...
				for(int i=0;i<rawEForm.Rows.Count;i++) {
					...
					if(PrefC.GetBool(PrefName.ShowFeaturePatientClone) && rawEForm.Rows[i]["PatNum"].ToString()!=patNum.ToString()) {
						Patient patientClone=listPatientClonesAll.FirstOrDefault(x => x.PatNum==SIn.Long(rawEForm.Rows[i]["PatNum"].ToString()));
						if(patientClone!=null && !string.IsNullOrWhiteSpace(patientClone.FName)) {
							row["description"]="("+patientClone.FName+") ";
						}
					}
			...
			if(componentsToLoad.ShowCompleted){
				...
				DataTable rawDocs = dcon.GetTable(command);
				for(int i=0;i<rawDocs.Rows.Count;i++) {
					row=table.NewRow();
					row["AbbrDesc"]="";
					row["aptDateTime"]=DateTime.MinValue;
					row["AptNum"]=0;
					...
					EnumDocChartLetterStatus enumDocChartLetterStatus=SIn.Enum<EnumDocChartLetterStatus>(rawDocs.Rows[i]["ChartLetterStatus"].ToString());
		...
		public static DataTable GetPlannedApt(long patNum) {
			...
			string command="SELECT appointment.AptNum,ItemOrderPlanned,"
			...
			for(int i=0;i<rawPlannedAppts.Rows.Count;i++) {
				...
				row=table.NewRow();
				//PlannedAppt----------------------------------------------------------------------------
				long aptNum=SIn.Long(aptRow["AptNum"].ToString());
		...
		public static DataTable GetPlannedApptsForApi(long patNum,int offset,int limit,string dateTimeFormatString) {
			...
			string command="SELECT appointment.AptNum,appointment.ItemOrderPlanned,appointment.AptDateTime,appointment.Pattern,appointment.AptStatus,"
				+"COUNT(DISTINCT procedurelog.ProcNum) someAreComplete "
				+"FROM appointment "
				+"LEFT JOIN procedurelog ON procedurelog.PlannedAptNum=appointment.AptNum	AND procedurelog.ProcStatus="+SOut.Int((int)ProcStat.C)+" "
				+"WHERE appointment.AptStatus="+SOut.Int((int)ApptStatus.Planned)+" AND appointment.PatNum="+SOut.Long(patNum)+" ";
			...
			for(int i=0;i<rawPlannedAppts.Rows.Count;i++) {
				...
				row["ProcDescript"]=GetProcDescriptPlanned(appointmentLinkToPlanned,listProceduresPlannedAppt,dateSched,aptRow["ProcDescript"].ToString());
				row["ItemOrder"]=itemOrder.ToString();
				row["ProvNum"]=SIn.Long(aptRow["ProvNum"].ToString());
				aptStatus=(ApptStatus)SIn.Long(aptRow["AptStatus"].ToString());
		...
		public static void AttachProcToTPs(Procedure proc,List<TreatPlan> listTreatPlans,List<long> listTpNums,PatientData pd,long priorityNum) {
			if(proc.ProcStatus!=ProcStat.TP && proc.ProcStatus!=ProcStat.TPi) {
				return;
			}
			listTreatPlans=TreatPlans.GetAllCurrentForPat(pd.PatNum);
			//If there is no active TP, make sure to add an active treatment plan.
			if(listTreatPlans.All(x => x.TPStatus!=TreatPlanStatus.Active)) {
				...
				listTreatPlans=new List<TreatPlan>() { activeTreatPlan };
				...
				listTpNums.Add(listTreatPlans.FirstOrDefault(x => x.TPStatus==TreatPlanStatus.Active).TreatPlanNum);
			}
			...
			if(listTreatPlans.FindAll(x => listTpNums.Contains(x.TreatPlanNum)).All(x => x.TPStatus!=TreatPlanStatus.Active)) {
		...
		///<summary>Used to update/fill the data for the chart module's patient data. Moved here to also be used in the mobile app.</summary>
		...
		///<summary>Helps with finalizing a procedure.</summary>
		public static void AddProcSetCompleteHelper(Procedure procedure) {
			if(procedure.ProcStatus==ProcStat.C) {//User didn't cancel (delete) in FormProcEdit
			...
			ref List<ClaimProc> listClaimProcs,ref List<ClaimProcHist> listClaimProcHists,long diagnosisNum,long prognosisNum) 
		{
			...
			OrthoProcLink orthoProcLink=orthoCaseProcedureLinker.LinkProcedureToActiveOrthoCaseIfNeeded(procedure,doUpdateProcedure:true);
	...
	///<summary>Every row that shows in the Chart modules ProgNotes grid is associated to one of these types. This is not used in the database.</summary>
	public enum ProgNotesRowType {
		///<summary>Does not define a real row type. Used as default.</summary>
		None,
		///<summary>ProcNum is not 0.</summary>
		[ProgNotesRow("ProcNum")]
		Proc,
		///<summary>CommLogNum is not 0.</summary>
		[ProgNotesRow("CommLogNum")]
		CommLog,
		///<summary>WebChatSessionNum is not 0.</summary>
		[ProgNotesRow("WebChatSessionNum")]
		WebChatSession,
		///<summary>RxNum is not 0.</summary>
		[ProgNotesRow("RxNum")]
		Rx,
		///<summary>LabCaseNum is not 0.</summary>
		[ProgNotesRow("LabCaseNum")]
		LabCase,
		///<summary>TaskNum is not 0.</summary>
		[ProgNotesRow("TaskNum")]
		Task,
		///<summary>AptNum is not 0.</summary>
		[ProgNotesRow("AptNum")]
		Apt,
		///<summary>EmailMessageNum is not 0.</summary>
		[ProgNotesRow("EmailMessageNum")]
		EmailMessage,
		///<summary>SheetNum is not 0.</summary>
		[ProgNotesRow("SheetNum")]
		Sheet,
		///<summary>FormPatNum is not 0.</summary>
		[ProgNotesRow("FormPatNum")]
		FormPat,
		///<summary>EForm.</summary>
		[ProgNotesRow("EFormNum")]
		EForm,
		///<summary>Only used for Chart Letters, which are a specific kind of document. Other images won't show here.</summary>
		[ProgNotesRow("DocNum")]
		Document,
	}
