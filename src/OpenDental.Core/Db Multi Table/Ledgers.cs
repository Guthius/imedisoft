...
public class Ledgers
{
    ...
    {
        ComputeAging(guarantorNum > 0 ? [guarantorNum] : [], asOfDate);
    }
    ...
    public static void ComputeAging(List<long> listGuarantorNums, DateTime asOfDate)
    {
        ...
        {
            var famAgingCur = FamAgingCrud.SelectOne(GetAgingQueryString(asOfDate, listGuarantorNums));
        ...
        {
            var listFamAgings = FamAgingCrud.SelectMany(GetAgingQueryString(asOfDate, listGuarantorNums)
    ...
    {
        ;
        var listFamPatNums = Patients.GetFamily(patNum).ListPats.Select(x => x.PatNum).ToList();
        //Get all PatNums not in the patient's family
        var listPatNumsAssociatedToDiffFam = listPaySplits.Where(x => !listFamPatNums.Contains(x.PatNum)).Select(y => y.PatNum).Distinct().ToList();
        ...
        var strErrorMsg = "";
        var listGuarantorNums = Patients.GetGuarantorsForPatNums(listPatNumsAssociatedToDiffFam);
        ...
        if (listGuarantorNums.Count > 1)
        {
            ...
            else
            {
                ...
                try
                {
                    ComputeAging(listGuarantorNums, DateTime.Today.Date);
                }
                finally
                {
    ...
    {
        var command = GetAgingQueryString(DateTime.Today, listGuarantorNums);
    ...
    public static string GetAgingQueryString(DateTime asOfDate, List<long> listGuarantors = null, bool isHistoric = false, bool isInsPayWoCombined = true, bool hasDateLastPay = false, bool isGroupByGuar = true, bool isWoAged = false, bool doAgePatPayPlanPayments = false, bool doExcludeIncomeTransfers = false)
    {
        //Returns family amounts summed from 'tSums', with payments distributed into the oldest balances first.
        var command = "SELECT tSums.PatNum," //if grouped by guar, this is the guar's PatNum; if grouped by patient.PatNum rows are individual pats
    ...
    private static string GetAgingGuarTransQuery(DateTime asOfDate, List<long> listGuarantors, bool hasDateLastPay = false, bool isHistoric = false, bool isGroupByGuar = true, bool isWoAged = false, bool doAgePatPayPlanPayments = false, bool doExcludeIncomeTransfers = false)
    {
        ...
        var asOfDateStr = SOut.Date(asOfDate);
        var thirtyDaysAgo = SOut.Date(asOfDate.AddDays(-30));
        var sixtyDaysAgo = SOut.Date(asOfDate.AddDays(-60));
        var ninetyDaysAgo = SOut.Date(asOfDate.AddDays(-90));
        var familyPatNums = "";
        var command = "";
        ...
        List<string> listInstantTranTypes =
        [
            "'WriteoffOrig'"
        ];
        if (isAgedByProc)
    ...
    public static string GetTransQueryStringAgeByProc(DateTime asOfDate, string familyPatNums, bool isWoAged = false, bool isHistoric = false, bool doAgePatPayPlanPayments = false, bool isAgedByProc = false, bool doExcludeIncomeTransfers = false)
    {
        if (!isAgedByProc)
        {
        ...
        var tranType =
            ...
            + "END)";
        var command = "SELECT " + tranType + " TranType,0 PriKey,tranbyproc.PatNum,"
                      + "(CASE "
    ...
    public static string GetTransQueryString(DateTime asOfDate, string familyPatNums, bool isWoAged = false, bool isHistoric = false, bool doAgePatPayPlanPayments = false, bool doIncludeProcNum = false, bool isAgedByProc = false, bool doExcludeIncomeTransfers = false)
    {
        ...
        command += "SELECT 'Proc' TranType,pl.ProcNum PriKey,pl.PatNum,pl.ProcDate TranDate,pl.ProcFee*(pl.UnitQty+pl.BaseUnits) TranAmount,"
                   ...
                   + (isWoAged
                       ? "0"
                       : ("(CASE WHEN " + (isHistoric
                              ? ("cp.ProcDate <= " + asOfDateStr + " " //historic=NotRcvd OR Rcvd and DateCp>asOfDate
                                 + "AND (cp.Status = 0 OR (cp.Status = 1 AND cp.DateCP > " + asOfDateStr + "))")
                              : "cp.Status = 0") + " " //not historic=NotReceived
        ...
        if (isWoAged)
        {
            command += "UNION ALL "
                       ...
                       + "(CASE WHEN " + (isHistoric
                           ? ("cp.ProcDate <= " + asOfDateStr + " " //historic=ProcDate<=asOfDate and either NotRcvd OR Rcvd with DateCp>asOfDate
                              + "AND (cp.Status = 0 OR (cp.Status = 1 AND cp.DateCP > " + asOfDateStr + ")) ")
                           : "cp.Status = 0 ") //not historic=NotReceived
                       ...
                       + "AND (cp.ProcNum=0 OR p.ProcStatus=2) " //Either a total payment or associated to a completed procedure
                       + (isAllPats ? "" : ("AND cp.PatNum IN (" + familyPatNums + ") "))
                       + ("HAVING TranAmount != 0 OR InsWoEst != 0 ") //efficiency improvement for MySQL only.
                       + "UNION ALL "
                       ...
                       + "AND (cp.ProcNum=0 OR p.ProcStatus=2) " //Either a total payment or associated to a completed procedure
                       + (isAllPats ? "" : ("AND cp.PatNum IN (" + familyPatNums + ") "))
                       + ("HAVING TranAmount != 0 "); //efficiency improvement for MySQL only.
        }
        ...
        var listHiddenUnearnedDefNums = Defs.GetDefsForCategory(DefCat.PaySplitUnearnedType).FindAll(x => !string.IsNullOrEmpty(x.ItemValue))
        ...
        else if (payPlanVersionCur == PayPlanVersions.AgeCreditsAndDebits)
        {
            //v2, we should be looking for payplancharges and aging those as patient debits/credits accordingly
        ...
        else if (payPlanVersionCur == PayPlanVersions.AgeCreditsOnly)
        {
            //v3, we should only be aging payplancharge credits
    ...
    public static List<TsiTrans> GetTransForGuars(List<long> listGuarNums)
    {
        ...
        var dictTranTypes = new Dictionary<string, TsiFKeyType>()
        {
    ...
    public static DataTable GetDateBalanceBegan(List<PatAging> listGuarantors, bool isSuperBills, List<long> listClinicNums = null)
    {
        ...
        var dictSuperFamGNums = new Dictionary<long, List<long>>();
        foreach (var patAgeCur in listGuarantors)
        {
            ...
            {
                //not a superBill, just add all family members for this guarantor
                dictSuperFamGNums[patAgeCur.PatNum] = [patAgeCur.PatNum];
            }
        }
        ...
        if (!isAllPats)
        {
            ...
            var listPatNumsAll = Patients.GetAllFamilyPatNums(dictSuperFamGNums.SelectMany(x => x.Value).ToList());
        ...
        var dictGuarDateBals = GetGuarDateBals(DateTime.Today, patNumStr, listClinicNums);
        ...
        foreach (var patAgeCur in listGuarantors)
        {
            ...
            {
                listGuarNums = [patAgeCur.PatNum];
            }
            else
            {
                //must be superbill and this is the superhead
            ...
            listDateBals = [];
            listDateZeroBals = [];
            foreach (var guarNum in listGuarNums)
            {
                if (!dictGuarDateBals.ContainsKey(guarNum))
                {
                    //should never happen
                    continue;
                }
    ...
    {
        var isHistoric = (dateAsOf.Date != DateTime.Today);
        //Create dictionary that tells a story about the transactions and their dates for each family.
        var retval = new Dictionary<long, List<GuarDateBals>>();
        var clinicNumsJoin = "";
        ...
        var command = $@"SELECT patient.Guarantor PatNum,RawPatTrans.TranDate,
        ...
        var dictGuarCreditTotals = table.Select()
    ...
    public static void AgingExceptionHandler(Exception ex, Form currentForm, bool showDeadlockMessage = true)
    {
        ...
        if (mySqlEx == null || mySqlEx.Number != 1213)
        {
            //not a deadlock error, just throw
            throw ex;
        }
        ...
        currentForm.InvokeIfRequired(() =>
        {
            ...
            {
                var msg = Lans.g("Ledgers", "Deadlock error detected in aging transaction and rolled back. Try again later.");
