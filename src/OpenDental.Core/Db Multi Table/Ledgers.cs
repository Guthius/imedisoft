...
namespace OpenDentBusiness{
	...
	public class Ledgers {
		...
		public static double GetAgingComputationTime() {
			...
			return PIn.Double(DataCore.GetScalar(command));
		}
		...
		///<para>IMPORTANT: If guarantorNum is 0 (i.e. run for all patients), the calling method MUST check the AgingBeginDateTime
		...
		///<summary>Computes aging for the family specified. Specify guarantor=0 in order to calculate aging for all families. Gets all info from db.
		///<para>IMPORTANT: If listGuarantorNums has more than one guarantor in it, the calling method MUST check the
		...
		///Returns error message if someone is currently computing aging. Otherwise returns empty string.</summary>
		public static string ComputeAgingForPaysplitsAllocatedToDiffPats(long patNum,List<PaySplit> listPaySplits) {
			...
			List<long> listPatNumsAssociatedToDiffFam=listPaySplits.Where(x => !listFamPatNums.Contains(x.PatNum)).Select(y => y.PatNum).Distinct().ToList();
			...
			else {//only 1 guar so not using the famaging table, just run aging as usual
				Ledgers.ComputeAging(listGuarantorNums,DateTime.Today.Date);
			}
			return strErrorMsg;
		}
			...
			bool isGroupByGuar=true,bool isWoAged=false,bool doAgePatPayPlanPayments=false, bool doExcludeIncomeTransfers=false)
		{
			...
			if(isGroupByGuar) {
				...
				if(!isAllPats) {//only if for one fam
		...
		public static DataTable GetDateBalanceBegan(List<PatAging> listGuarantors,bool isSuperBills,List<long> listClinicNums=null) {
			...
			Dictionary<long,List<GuarDateBals>> dictGuarDateBals=GetGuarDateBals(DateTime.Today,patNumStr,listClinicNums);
