...
namespace OpenDentBusiness {
	public class PayPlanEdit {
		...
		public static void IssueChargesDueForDynamicPaymentPlans(List<PayPlan> listDynamicPayPlans,LogWriter log,bool isOpenDentalService=false) {
			...
			try {
				...
				foreach(PayPlan payplan in listDynamicPayPlans) {
					...
					List<PayPlanCharge> listPayPlanChargesExpected=listPayPlanChargesInDb.FindAll(x=>x.ChargeDate==nextExpectedDate && !x.Note.Contains("Down Payment"));
					...
					List<PayPlanCharge> listPayPlanChargesNew=new List<PayPlanCharge>();
					...
					foreach(PayPlanCharge payPlanCharge in listPayPlanChargesNew) {
		...
		///<summary>Applies prepayments to the charges passed in.</summary>
		private static void ApplyPrepaymentsToCharges(PayPlan payplan,List<PayPlanCharge> listPayPlanCharges,List<PaySplit> listPaySplits,LogWriter log) {
			double prepaymentAmount=PayPlanEdit.GetDynamicPayPlanPrepaymentAmount(listPaySplits);
			...
			PayPlanEdit.ApplyPrepaymentToDynamicPaymentPlan(payplan.Guarantor,prepaymentAmount,listPayPlanCharges,listPaySplits);//Guarantors on payment plans are the ones that make the payments.
		}
		...
		///<summary>Run by OpenDentalService.PaymentThread. Takes any hiddenUnearnedTotal from hidden unearned, and applies it to listPayPlanChargesToday.</summary>
		public static void ApplyPrepaymentToDynamicPaymentPlan(long patNum,double hiddenUnearnedTotal,List<PayPlanCharge> listPayPlanChargesNow,List<PaySplit> listPaySplits,long payPlanNum=0,bool isClosing=false) {
			...
			for(int i = 0;i<listPayPlanChargesNow.Count;i++) {
				if(CompareDouble.IsLessThanOrEqualToZero(hiddenUnearnedTotal)) {
					break;
				}
				PayPlanCharge charge=listPayPlanChargesNow[i];
				...
				double chargeInterestValue=charge.Interest;
				double chargePrincipalValue=charge.Principal;
				//Sum up all the payments on inserted charges broken up by debit type.
				double principalPaid=listPaySplits.FindAll(x=>x.PayPlanChargeNum!=0 && x.PayPlanChargeNum==charge.PayPlanChargeNum && x.PayPlanDebitType==PayPlanDebitTypes.Principal).Sum(x=>x.SplitAmt);
				double interestPaid=listPaySplits.FindAll(x=>x.PayPlanChargeNum!=0 && x.PayPlanChargeNum==charge.PayPlanChargeNum && x.PayPlanDebitType==PayPlanDebitTypes.Interest).Sum(x=>x.SplitAmt);
				double unknownPaid=listPaySplits.FindAll(x=>x.PayPlanChargeNum!=0 && x.PayPlanChargeNum==charge.PayPlanChargeNum && x.PayPlanDebitType==PayPlanDebitTypes.Unknown && x.UnearnedType==0).Sum(x=>x.SplitAmt);
				chargeInterestValue-=Math.Min(chargeInterestValue,interestPaid);
				if(!CompareDouble.IsZero(chargeInterestValue)){
					double unknownValueApplied=Math.Min(chargeInterestValue,unknownPaid);
				...
				if(!CompareDouble.IsZero(chargePrincipalValue)){
					double unknownValueApplied=Math.Min(chargePrincipalValue,unknownPaid);
					unknownPaid-=unknownValueApplied;
				...
				double splitInterestAmt=Math.Min(hiddenUnearnedTotal,chargeInterestValue);
				hiddenUnearnedTotal-=splitInterestAmt;
				double splitPrincipalAmt=Math.Min(hiddenUnearnedTotal,chargePrincipalValue);
		...
		private static List<PayPlanChargeSplits> CreatePrincipalChargeSplitsForAmount(double amountRequesting,DateTime chargeDate,long unearnedType,ref List<PayPlanProductionEntry> listProdEntries,PayPlan payPlan,Family fam) {
			...
			List<PayPlanCharge> listPayPlanChargesNew=CreateChargesForPrincipalPayment(amountRequesting,ref listProdEntries,chargeDate,payPlan,fam);
		...
		private static List<PayPlanCharge> CreateChargesForPrincipalPayment(double amountToCharge,ref List<PayPlanProductionEntry> listProdEntries,DateTime chargeDate,PayPlan payPlan,Family fam) {
			...
			for(int i=0;i<listEntries.Count;i++) {
				...
				PayPlanCharge payPlanCharge=GetPayPlanChargeForEntry(listEntries[i],payPlan,principal,chargeDate,fam);
			...
			long categoryDef) 
		{
			...
			for(int  i=0;i<dynamicPaymentPlanModuleData.ListPayPlanChargesDb.Count;i++){
	...
	public class DynamicPaymentPlanModuleData {
		...
		///<summary>List of charges for the payment plan. Both in the database, and expected.</summary>
