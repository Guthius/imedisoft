...
public partial class PatientData
{
    ...
    public List<OrthoCase> ListOrthoCases;
    ...
    public void Clear(params EnumPdTable[] pdTableArray)
    {
        ...
        if (pdTableArray.Contains(EnumPdTable.OrthoCase))
        {
            ListOrthoProcLinks = null;
            ListOrthoCases = null;
        }
    ...
    public static void GetFromDbSimple(long patNum, List<EnumPdTable> listPdTables, PatientData patientData)
    {
        ...
        if (listPdTables.Contains(EnumPdTable.OrthoCase))
        {
            patientData.ListOrthoCases = OrthoCases.GetPatientData(patNum);
            patientData.ListOrthoProcLinks = OrthoProcLinks.GetPatientData(patientData.ListOrthoCases);
        }
    ...
    private List<EnumPdTable> FilterPdTables(EnumPdTable[] pdTableArray)
    {
        ...
        if (pdTableArray.Contains(EnumPdTable.OrthoCase) && ListOrthoCases is null)
        {
            listPdTables.Add(EnumPdTable.OrthoCase);
        }
    ...
    private void CopyDataToOurPd(PatientData patientData, List<EnumPdTable> listPdTables)
    {
        ...
        if (listPdTables.Contains(EnumPdTable.OrthoCase))
        {
            ListOrthoProcLinks = new List<OrthoProcLink>(patientData.ListOrthoProcLinks);
            ListOrthoCases = new List<OrthoCase>(patientData.ListOrthoCases);
        }
