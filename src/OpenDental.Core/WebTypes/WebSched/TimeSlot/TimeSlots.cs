...
public class TimeSlots
{
    ...
    public static List<TimeSlot> GetAvailableWebSchedTimeSlots(long recallNum, DateTime dateStart, DateTime dateEnd, long provNum = 0, bool allowOtherProv = true, Logger.IWriteLine log = null)
    {
        ...
        var listProviders = Providers.GetProvidersForWebSched(recall.PatNum, clinic?.Id ?? 0);
        ...
        var recallType = RecallTypes.GetFirstOrDefault(x => x.RecallTypeNum == recall.RecallTypeNum);
        ...
        , DateTime dateStart, DateTime dateEnd, Recall recallCur = null, Logger.IWriteLine log = null, bool isFromWebSched = true)
    {
        ...
        var listOperatories = Operatories.GetOpsForWebSched();
        if (listOperatories.Count < 1)
        {
            //This is very possible for offices that aren't set up the way that we expect them to be.
            var errrorMessage = Lans.g("WebSched", "There are no operatories set up for Web Sched.");
            if (isFromWebSched)
            {
                errrorMessage += "\r\n" + Lans.g("WebSched", "Please call us to schedule your appointment.");
            }
        ...
        var listProvNums = listProviders.Where(x => !x.IsNotPerson).Select(x => x.ProvNum).Distinct().ToList();
        var timePatternRecall = recallType.TimePattern;
        ...
        if (recallCur != null)
        {
            var patCur = Patients.GetLim(recallCur.PatNum);
            var listRecalls = Recalls.GetList(recallCur.PatNum);
            timePatternRecall = Recalls.GetRecallTimePattern(recallCur, listRecalls, patCur, new List<string>());
        }

        var timePatternAppointment = RecallTypes.ConvertTimePattern(timePatternRecall);
        //Get restriction blockouts
        List<Schedule> listRestrictedToBlockouts = null;
        var listRestrictedToBlockoutDefLinks = DefLinks.GetListByFKey(recallType.RecallTypeNum, DefLinkType.RecallType);
        ...
        var listSchedules = Schedules.GetSchedulesAndBlockoutsForWebSched(listProvNums, dateStart, dateEnd, isRecall: true
            , (clinic == null) ? 0 : clinic.Id, log, listSchedulesBlockouts: listRestrictedToBlockouts);
    ...
    {
        var prefName = PrefName.WebSchedExistingPatDoubleBooking;
        ...
        var appointmentType = AppointmentTypes.GetApptTypeForDef(defNumApptType);
        ...
        var listOperatories = Operatories.GetOpsForDefAndCategory(defNumApptType, defCat);
        if (listOperatories.Count < 1)
        {
            //This is very possible for offices that aren't set up the way that we expect them to be.
        ...
        var timePattern = AppointmentTypes.GetTimePatternForAppointmentType(appointmentType);
        var listProviders = Providers.GetProvidersForWebSchedNewPatAppt();
        var clinic = Clinics.GetClinic(clinicNum);
        var listProvNums = listProviders.Select(x => x.ProvNum).Distinct().ToList();
        List<Schedule> listRestrictedToBlockouts = null;
        var listRestrictedToBlockoutDefLinks = DefLinks.GetDefLinksByType(DefLinkType.BlockoutType, defNumApptType);
        if (listRestrictedToBlockoutDefLinks.Count > 0)
        {
        ...
        var listSchedules = Schedules.GetSchedulesAndBlockoutsForWebSched(listProvNums, dateStart, dateEnd, isRecall: false, clinicNum,
        ...
        , List<Schedule> listRestrictToBlockouts = null)
    {
        ...
        var listProviderSchedules = listSchedules.FindAll(x => x.BlockoutType == 0);
        var listBlockoutSchedules = listSchedules.FindAll(x => x.BlockoutType > 0);
        ...
        var listApptsForOps = Appointments.GetAppointmentsForOpsByPeriod(Operatories.GetDeepCopy(true)
            .Where(x => (clinic == null) || (x.ClinicNum == clinic.Id)).Select(x => x.OperatoryNum).ToList(), dateStart, dateEnd, log);
        ...
        var dictProvSchedules = Appointments.GetApptSearchProviderScheduleForProvidersAndDate(
        ...
        var listProviderSchedulesWithOp = listProviderSchedules.FindAll(x => x.Ops.Intersect(listOpNums).ToList().Count > 0);
        var listScheduleOps = ScheduleOps.GetForSchedList(listProviderSchedules);
        //Now we need to get the dynamic schedules (not assigned to a specific operatory).
        var listProviderDynamicSchedules = listProviderSchedules.FindAll(x => !listScheduleOps.Exists(y => y.ScheduleNum == x.ScheduleNum));
        //Now that we have found all possible valid schedules, find all the unique time slots from them.
        var listProviderSchedulesAll = new List<Schedule>(listProviderSchedulesWithOp);
        ...
        var listAvailableTimeSlots = new List<TimeSlot>();
        var listUniqueDays = new List<DateTime>();
        ...
        foreach (var schedule in listProviderSchedulesAll)
        {
            var dateSched = schedule.SchedDate;
            ...
            var timeSchedStart = schedule.StartTime;
            var timeSchedStop = schedule.StopTime;
            ...
            var minsOver = (timeSchedStart.Minutes) % timeIncrement;
            if (minsOver > 0)
            {
                var minsToAdd = timeIncrement - minsOver;
            ...
            var listOpsForSchedule = new List<Operatory>();
            ...
            foreach (var op in listOpsForSchedule)
            {
        ...
        , long defNumApptType = 0, bool isDoubleBookingAllowed = true, List<Schedule> listRestrictToBlockouts = null)
    {
        ...
        var listBlockoutsForDateAndOp = listBlockouts.FindAll(x => x.SchedDate.Date == dateSched.Date && x.Ops.Contains(operatoryNum));
        //Filter out all appointments that are not pertinent to this dateSched and operatoryNum combo.
        var listApptsForDateAndOp = listApptsForOps.FindAll(x => x.AptDateTime.Date == dateSched.Date && x.Op == operatoryNum);
        ...
        if (listRestrictToBlockouts != null)
        {
            //Cannot use IsNullOrEmpty() because an empty list is valid, it means no blockouts were found
        ...
        var timeSlotStart = new TimeSpan(timeSchedStart.Ticks);
        ...
        var listPerfectSlotStarts = new List<TimeSpan>();
        //The first perfect time slot start will always be when our schedule starts.
        for (var timeSlotPerfect = new TimeSpan(timeSchedStart.Ticks); timeSlotPerfect <= new TimeSpan(timeSchedStop.Ticks); timeSlotPerfect = timeSlotPerfect.Add(new TimeSpan(0, apptLengthMins, 0)))
        {
        ...
        for (var timeSlotStop = timeSchedStart.Add(new TimeSpan(0, timeIncrement, 0)); timeSlotStop <= timeSchedStop; timeSlotStop = timeSlotStop.Add(new TimeSpan(0, timeIncrement, 0)))
        {
            //Check to see if we've found an opening.
            var timeSpanCur = timeSlotStop - timeSlotStart;
            //Check to see if there is an appointment or a blockout that collides with this blockout.
            var isOverlapping = false;

            #region Blockout Collisions

            var timeBlockoutStart = new TimeSpan();
            var timeBlockoutStop = new TimeSpan();
            //First we'll look at blockouts because it should be quicker than looking at the appointments
            foreach (var blockout in listBlockoutsForDateAndOp)
            {
            ...
            if (isOverlapping)
            {
                //This check is here so that we don't waste time looping through appointments if we don't need to.
            ...
            var timeApptStart = new TimeSpan();
            var timeApptStop = new TimeSpan();
            //Next we'll look for overlapping appointments
            foreach (var appointment in listApptsForDateAndOp)
            {
            ...
            if (timeSpanCur.TotalMinutes >= apptLengthMins)
            {
                ...
                var dateTimeSlotStart = new DateTime(dateSched.Year, dateSched.Month, dateSched.Day, timeSlotStart.Hours, timeSlotStart.Minutes, 0);
                var dateTimeSlotStop = new DateTime(dateSched.Year, dateSched.Month, dateSched.Day, timeSlotStop.Hours, timeSlotStop.Minutes, 0);
                var timeSlot = new TimeSlot(dateTimeSlotStart, dateTimeSlotStop, operatoryNum, schedule.ProvNum, defNumApptType);
                                                                                           ...
                                                                                           && x.ProvNum == schedule.ProvNum))) //We will return multiple time slots for the same time for different providers.
                {
                    ...
                    {
                        var recallProvNum = schedule.ProvNum;
                    ...
                    {
                        var hasAcceptableBlockout = false;
                        var apptLengthSpan = new TimeSpan(0, apptLengthMins, 0);
                        foreach (var blockout in listRestrictToBlockoutsForDateAndOp)
                        {
                            var blockStart = new DateTime(blockout.SchedDate.Year, blockout.SchedDate.Month, blockout.SchedDate.Day, blockout.StartTime.Hours, blockout.StartTime.Minutes, 0);
                            var blockEnd = new DateTime(blockout.SchedDate.Year, blockout.SchedDate.Month, blockout.SchedDate.Day, blockout.StopTime.Hours, blockout.StopTime.Minutes, 0);
                ...
                else
                {
                    ...
                    var timeSlotCur = listAvailableTimeSlots.First(x => (x.DateTimeStart == dateTimeSlotStart && x.DateTimeStop == dateTimeSlotStop));
                    var operatoryIn = Operatories.GetOperatory(operatoryNum);
                    var operatoryCur = Operatories.GetOperatory(timeSlotCur.OperatoryNum);
        ...
        , long provNum, string timePattern, DateTime dateTimeAppointmentStart, long defNumApptType = 0, bool isDoubleBookingAllowed = true)
    {
        ...
        if (apptType != null)
        {
            var listProcCodesApptTypes = new List<string>(apptType.CodeStr.Split(','));
            var apptTemp = new Appointment()
            {
                ProvNum = provNum,
                Pattern = timePattern,
            ...
            var dtAppts = Appointments.GetPeriodApptsTableMini(dateTimeAppointmentStart, provNum);
            ...
            var listProcCodesDB = Appointments.GetDoubleBookedCodes(apptTemp, dtAppts, procsMultApts, new Procedure[] { });
            //Check for appointment rules and specific blocked appointments/codes
            if (AppointmentRules.IsBlocked(listProcCodesApptTypes))
            {
                return listProcCodesDB.Count > 0;
            }
        }
        ...
        , DateTime dateTimeAppointmentStart)
    {
        ...
        var startingIncrement = (int) dateTimeAppointmentStart.TimeOfDay.TotalMinutes / 5;
        for (var i = 0; i < listProviderSchedulesForProv.Count; i++)
        {
            //There should only be one.
            //Check to make sure the ProvBar does not have any conflicts with the timePattern passed in.
            var listHasDouble = listProviderSchedulesForProv[i].IsProvAvailable.ToList().FindAll(x => !x);
            for (var j = 0; j < timePattern.Length; j++)
            {
                if (timePattern[j] == '/')
                {
                    //Don't worry about HYG conflicts.
                    continue;
                }

                if (!listProviderSchedulesForProv[i].IsProvAvailable[startingIncrement + j])
                {
                    //False means there is a collision in the providers schedule.
                    return true;
                }
            }
        }
