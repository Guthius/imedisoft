...
public class MountHelper
{
    public static void DrawMount(Graphics g, Mount mount, Patient patient, List<MountItem> listMountItems, List<Bitmap> listBitmaps, List<Document> listDocuments, List<ImageDraw> listImageDraws)
    {
        ...
        //outlines:
        using var penOutline = new Pen(Color.FromArgb(100, 100, 100));
        for (var i = 0; i < listMountItems.Count; i++)
        {
            if (listMountItems[i].ItemOrder == -1)
            {
    ...
    private static void DrawMountOne(Graphics g, Mount mount, Patient patient, MountItem mountItem, Bitmap bitmap, Document document)
    {
        ...
        {
            using var solidBrushBack = new SolidBrush(mount.ColorTextBack);
            var rectangleF = new RectangleF(0, 0, mountItem.Width, mountItem.Height);
            g.FillRectangle(solidBrushBack, rectangleF);
            using var font = new Font(FontFamily.GenericSansSerif, mountItem.FontSize);
            using var solidBrushFore = new SolidBrush(mount.ColorFore);
            var str = mountItem.TextShowing;
            str = Patients.ReplacePatient(str, patient);
            str = Mounts.ReplaceMount(str, mount);
            var patPlan = PatPlans.GetPatPlan(patient.PatNum, 1);
            InsSub insSub = null;
            if (patPlan != null)
            {
        ...
        else
        {
            ...
            var sizeUnscaled = ImageHelper.CalcSizeFit(bitmap.Width, bitmap.Height, document.DegreesRotated, new Size(mountItem.Width, mountItem.Height));
    ...
    private static void DrawMountOneBigNumber(Graphics g, MountItem mountItem)
    {
        ...
        var heightFontPoint = mountItem.Height / 15f * (96f / 72f);
        ...
        var sizeString = g.MeasureString(mountItem.ItemOrder.ToString(), font);
    ...
    public static Bitmap GetBitmapOfMountFromDb(long mountNum)
    {
        ...
        var patient = Patients.GetPat(mount.PatNum);
        var patFolder = ImageStore.GetPatientFolder(patient, ImageStore.GetDataFolder());
        var listImageDraws = ImageDraws.RefreshForMount(mount.MountNum);
        var listMountItems = MountItems.GetItemsForMount(mount.MountNum);
        var listDocuments = Documents.GetDocumentsForMountItems(listMountItems).ToList();
        var bitmapArray = new Bitmap[listDocuments.Count];
        ...
        var bitmap = GetBitmapOfMount(mount, patient, listMountItems, bitmapArray.ToList(), listDocuments, listImageDraws);
    ...
    {
        var bitmap = new Bitmap(mount.Width, mount.Height);
