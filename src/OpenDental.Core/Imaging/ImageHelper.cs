...
namespace OpenDentBusiness {
	public static class ImageHelper {
		...
		public static Bitmap GetBitmapPDF(string fullFilePath,MemoryStream memoryStream) {
			...
			Bitmap bitmapOriginal=new Bitmap(fullFilePath);
			...
			Bitmap bitmapQualityCopy=new Bitmap(bitmapOriginal);//Convert to a BMP to preserve the quality of the original image.
			if(memoryStream!=null) {
				memoryStream.Dispose();
			}
			memoryStream=new MemoryStream();
