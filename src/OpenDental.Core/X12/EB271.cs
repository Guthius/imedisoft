...
namespace OpenDentBusiness
{
	...
	public class EB271{
		...
		public DateTime GetInsHistDate(PrefName prefName) {
			...
			if(Segment.Get(1)=="F") {//a limitation.
				...
				X12Segment x12SegmentDtp=SupplementalSegments.FirstOrDefault(x => x.SegmentID=="DTP" && x.Elements[1]=="304");
				X12Segment x12SegmentMsg=SupplementalSegments.FirstOrDefault(x => x.SegmentID=="MSG");
				if(x12SegmentDtp==null || x12SegmentMsg==null) {
					return DateTime.MinValue;
				}
				if(prefName==PrefName.NotApplicable) {//DTP and MSG found but MSG not associated to internal PrefName
					return dateResult;
				}
				try {
					dateResult=DateTime.ParseExact(x12SegmentDtp.Get(3),"yyyymmdd",CultureInfo.InvariantCulture);
				}
				catch(Exception ex) {
					return DateTime.MinValue;
				}
				if(dateResult.Year<1880 || dateResult>DateTime.Today) {//Invalid date
		...
		public PrefName GetPrefNameInsHistDate() {
			PrefName prefName;
			X12Segment x12SegmentMsg=SupplementalSegments.FirstOrDefault(x => x.SegmentID=="MSG");
			if(x12SegmentMsg==null) {
				return prefName= PrefName.NotApplicable;
			}
			prefName=GetPrefNameMatchedMsg(x12SegmentMsg.Get(1));
			return prefName;
		}
		...
		///<returns>Returns matched PrefName based on given value, otherwise returns PrefName.Notapplicable.</returns>
		private static PrefName GetPrefNameMatchedMsg(string msgVal){
			msgVal=msgVal.ToUpper().Replace("BENEFITCLASS=","");
			PrefName prefName;
			switch(msgVal) {
				case "BITEWING X-RAYS":
					prefName=PrefName.InsHistBWCodes;
					break;
				case "EXAMS":
					prefName=PrefName.InsHistExamCodes;
					break;
				case "FULL MOUTH/PANOREX":
					prefName=PrefName.InsHistPanoCodes;
					break;
				case "PROPHYLAXIS":
					prefName=PrefName.InsHistProphyCodes;
					break;
		...
		public static int SetInsuranceHistoryDates(List<EB271> listEb271s,long patNum,InsSub insSub) {
			...
			List<Procedure> listProceduresEoAndC=Procedures.GetProcsByStatusForPat(patNum,new [] { ProcStat.EO,ProcStat.C });
			List<long> listProcNums=listProceduresEoAndC.Select(x=>x.ProcNum).ToList();
			List<ClaimProc> listClaimProcsForInsHistProcs=ClaimProcs.GetForProcs(listProcNums)
			...
			for(int i = 0;i<listEb271s.Count;i++) {
				PrefName prefName=listEb271s[i].GetPrefNameInsHistDate();
				DateTime dateInsHist=listEb271s[i].GetInsHistDate(prefName);
				if(dateInsHist==DateTime.MinValue) {//When dateInsHist is not MinValue, prefName is guarenteed to be valid.
					continue;//No insurance history date found for this eb271.
				}
				List<long> listCodeNums=ProcedureCodes.GetCodeNumsForInsHistPref(prefName);
				Procedure procedure=Procedures.GetMostRecentInsHistProc(listProceduresEoAndC,listCodeNums,prefName);
				List<ClaimProc> listClaimProcsForProc=new List<ClaimProc>();
				if(procedure!=null) {
					//Get all of the claimprocs for this procedure.
					listClaimProcsForProc=listClaimProcsForInsHistProcs.FindAll(x => x.ProcNum==procedure.ProcNum);
				}
				Procedures.InsertOrUpdateInsHistProcedure(patient,prefName,dateInsHist,insSub.PlanNum,insSub.InsSubNum,procedure,listClaimProcsForProc);
				countValidInsHist++;
			}
