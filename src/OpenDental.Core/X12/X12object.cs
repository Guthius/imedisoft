...
namespace OpenDentBusiness
{
    ...
    public class X12object
    {
        ...
        public X12object(string messageText)
        {
            ...
            var arrayRawSegments = messageText.Split(new string[] {Separators.Segment}, StringSplitOptions.None);
            FunctGroups = new List<X12FunctionalGroup>();
            Segments = new List<X12Segment>();
            X12Segment segment;
            for (var i = 0; i < arrayRawSegments.Length; i++)
            {
        ...
        public List<string> GetTranSetIds()
        {
            ...
            for (var i = 0; i < FunctGroups[0].Transactions.Count; i++)
            {
                var tranSetId = FunctGroups[0].Transactions[i].Header.Get(2);
        ...
        public X12Segment GetNextSegmentById(int startIndex, string segmentId, params string[] arrayElement01Values)
        {
            for (var i = startIndex; i < Segments.Count; i++)
            {
                ...
                {
                    var isMatchingElement01 = false;
                    for (var j = 0; j < arrayElement01Values.Length; j++)
                    {
        ...
        {
            var arrayIndiciesToKeep = new bool[Segments.Count];
            for (var i = 0; i < arrayIndiciesToKeep.Length; i++)
            {
                arrayIndiciesToKeep[i] = true;
            }

            for (var i = 0; i < listSegmentIndiciesToRemove.Count; i++)
            {
                var index = listSegmentIndiciesToRemove[i];
    ...
    public class X12Segment
    {
        ...
        public string Get(int elementPosition, int subelementPosition)
        {
            ...
            var subelements = Elements[elementPosition].Split(char.Parse(Separators.Subelement));
        ...
        public bool IsType(string segmentId, params string[] arrayElement01Values)
        {
            ...
            var isElement01Valid = false;
            var element01 = Get(1);
            for (var i = 0; i < arrayElement01Values.Length; i++)
            {
    ...
    /// </summary>
    /// <seealso href="https://www.cms.gov/medicare/coding-billing/place-of-service-codes/code-sets"/>
    public static class PlaceOfServiceCodes
    {
        ...
        public const string MilitaryTreatFacility = "26";
        public const string SkilledNursFacility = "31";
        public const string CustodialCareFacility = "33";
        public const string FederalQualifiedHealthCenter = "50";
        public const string PublicHealthClinic = "71";
        public const string OtherPlaceOfService = "99";

        public static PlaceOfService ToEnum(string code)
        {
            return code switch
            {
                School => PlaceOfService.School,
                Office => PlaceOfService.Office,
                Home => PlaceOfService.Office,
                MobileUnit => PlaceOfService.MobileUnit,
                InpatientHospital => PlaceOfService.InpatHospital,
                OutpatientHospital => PlaceOfService.OutpatHospital,
                MilitaryTreatFacility => PlaceOfService.OutpatHospital,
                SkilledNursFacility => PlaceOfService.SkilledNursFac,
                CustodialCareFacility => PlaceOfService.CustodialCareFacility,
                FederalQualifiedHealthCenter => PlaceOfService.FederalHealthCenter,
                PublicHealthClinic => PlaceOfService.PublicHealthClinic,
                _ => PlaceOfService.OtherLocation
            };
        }
    }
