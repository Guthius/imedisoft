...
namespace OpenDentBusiness {
    ...
    private List<X12Segment> _listSegments;
		...
		///<summary>Represents all the data from the entire 1000A PER*BL segment.</summary>
		public string PayerContactInfoBL { get; private set; }="";
		///<summary>Represents all the data from the entire 1000A PER*CX segment.</summary>
		public string PayerContactInfoCX { get; private set; }="";
		///<summary>Represents all the data from the entire 1000A PER*IC segment.</summary>
		public string PayerContactInfoIC { get; private set; } = "";
		///<summary>N1*PE N102 loop 1000B.</summary>
		...
		private void ProcessMessage() {
			...
			while(segNum<_listSegments.Count && _listSegments[segNum].SegmentID=="PER") {
				...
				//1000A PER*CX: Payer Business Contact Information.  Situational.  Repeat 1.  Guide page 94. We use this if present.
				if(_listSegments[segNum].Get(1)=="CX") {
					PayerContactInfoCX=ProcessPER(segNum);
				}
				...
				if(_listSegments[segNum].Get(1)=="BL") {
					PayerContactInfoBL=ProcessPER(segNum);
				}
				//1000A PER*IC: Payer WEB Site.  Situational.  Repeat 1.  Guide page 100.  We do not use.
				if(_listSegments[segNum].Get(1)=="IC") {
					PayerContactInfoIC=ProcessPER(segNum);
				}
				segNum++;
			}
		...
		public string GetPayerContactInfo() {
			if(PayerContactInfoBL!="") {
				return PayerContactInfoBL;
			}
			if(PayerContactInfoCX!="") {
				return PayerContactInfoCX;
			}
			return PayerContactInfoIC;
		}
		...
		private List<Hx835_Adj> ProcessCAS(int segNum) {
			...
			for(int k=2;k<=17;k+=3) {
				...
				if(strAdjReasonCode!="") {
					...
					adj.ReasonCode=strAdjReasonCode;
				}
		...
		///<summary>Gets the contact information from segment PER*CX or PER*BL appended into a single string.
		...
		private string endSegment="~"+Environment.NewLine;
		private int _seg=0;//segments for a particular ST-SE transaction
		///<summary>Is reset each time GenerateMessageText() is called.</summary>
		...
		public string GenerateMessageText() {
			...
			WriteSegment("ISA",
				"00",//ISA01 2/2 Authorization Information Qualifier: 00=No Authorization Information Present
				"          ",//ISA02 10/10 Authorization Information: Blank
				"00",//ISA03 2/2 Security Information Qualifier: 00=No Security Information Present
				"          ",//ISA04 10/10 Security Information: Blank
				"ZZ",//ISA05 2/2 Interchange ID Qualifier: ZZ=mutually defined
				"0123456789ABCDE",//ISA06 15/15 Interchange Sender ID:
				"ZZ",//ISA07 2/2 Interchange ID Qualifier: ZZ=mutually defined
				"0123456789ABCDE",//ISA08 15/15 Interchange Receiver ID:
				...
				LiteralElement("^"),//ISA11 1/1 Repetition Separator:
				...
				"T",//ISA15 1/1 Interchange Usage Indicator: T=Test
				LiteralElement(isa16)//ISA16 1/1 Component Element Separator:
			);
			//Just one functional group.
			WriteFunctionalGroup(batchNum);
			//Interchange Control Trailer
			WriteSegment("IEA",
				"1",//IEA01 1/5 Number of Included Functional Groups:
		...
		private void WriteFunctionalGroup(int batchNum) {
			...
			WriteSegment("GS",
				...
				"0123456789ABCDEF",//GS02 2/15 Application Receiver's Code:
			...
			WriteSegment("ST",
				"835",//ST01 3/3 Transaction Set Identifier Code: 
				transactionNum.ToString().PadLeft(4,'0')//ST02 4/9 Transaction Set Control Number:
				//ST03 1/35 Implementation Convention Reference: Not used.
			);
			//BPR: FINANCIAL INFORMATION. Required.
			WriteSegment("BPR",
				"H",//BPR01 1/2 Transaction Handling Code: H=Notification Only
				ListClaimsPaid.Sum(x => x.InsPaid),//BPR02 Monetary Amount: Total payment amount for this 835
				"C",//BPR03 1/1 Credit/Debit Flag Code: C=Credit
				"NON",//BPR04 3/3 Payment Method Code: NON=Non-Payment Data (required when BPR01 is H)
				"CCP",//BPR05 1/10 Payment Format Code: CCP=Cash Concentration/Disbursement plus Addenda (CCD+) (ACH)
				"01",//BPR06 2/2 (DFI) ID Number Qualifier: 01=ABA Transit Routing Number Including Check Digits
				"",//BPR07 3/12 (DFI) Identification Number: Only required when BPR04 is ACH, BOP, FWT.
				"DA",//BPR08 1/3 Account Number Qualifier: DA=Demand Deposit. Requires DA.
				"",//BPR09 1/35 Account Number: Only required when BPR04 is ACH, BOP, FWT.
				"",//BPR10 10/10 Originating Company Idenfitier: Only required when BPR04 is ACH, BOP, FWT.
				"",//BPR11 9/9 Originating Company Supplemental Code: Only required when BRP10 is present.
				"",//BPR12 2/2 (DFI) ID Number Qualifier: Only required when BPR04 is ACH, BOP, FWT.
				"",//BPR13 3/12 (DFI) Identification Number: Only required when BPR04 is ACH, BOP, FWT.
				"",//BPR14 1/3 Account Number Qualifier: Only required when BPR04 is ACH, BOP, FWT.
				"",//BPR15 1/35 Accont Number: Only required when BPR04 is ACH, BOP, FWT.
				DateTime.Today.ToString("yyyyMMdd")//BPR16 8/8 Date: When BPR04 is 'NON', enter the date of the 835.
			);
			//TRN: REASSOCIATION TRACE NUMBER. Required.
			WriteSegment("TRN",
				"1",//TRN01 1/2 Trace Type Code: 1=Current Transaction Trace Numbers
				ODRandom.Next(1,1000000).ToString(),//TRN02 1/50 Reference Identification: If this is a non-payment 835, this must be a unique remittance advince identification number.
				"1234567890"//TRN03 10/10 Originating Company Identifier: This must be a 1 followd by the payer's EIN (or TIN).
				//TRN04 1/50 Reference Identification: Situational.
			);
			//CUR: FOREIGN CURRENCY INFORMATION. Situational.
			//REF: RECEIVER IDENTIFICATION. Situational. Required when the receiver of the transaction is other than the payee.
			//REF: VERSION IDENTIFICATION. Situational. Requied when necessary to report version number of the adjudication system.
			//DTM: PRODUCTION DATE. Situational. Required when teh cut off date of the adjudication system remittance run is different from the date of the 835.
			//1000A N1: PR PAYER IDENTIFICATION. Required.
			WriteSegment("N1",
				"PR",//N101 2/3 Entity Identifier Code: PR=Payer
				"Fake ERA Generator"//N102 1/60 Name:
				//N103 1/2 Identification Code Qualifier: Situational
				//N104 2/80 Identification Code: Situational
			);
			//1000A N3: PAYER ADDRESS. Required.
			WriteSegment("N3",
				"123 Maple St"//N301 1/55 Address Information:
				//N302 - 1/55 Address Information: Address line 2
			);
			//1000A N4: PAYER CITY, STATE, ZIP CODE. Required.
			WriteSegment("N4",
				"Nowhere",//N401 2/30 City Name:
				"OR",//N402 2/2 State or Province Code:
				"12345"//N403 3/15 Postal Code:
				//N404 2/3 Country Code: Situational. Required when not United States.
				//N405 1/2 Location Qualifier: Not used.
				//N406 1/30 Location Identifier: Not used.
				//N407 1/3 Country Subdivision Code: Situational. Required when not United States.
			);
			//1000A REF 2U,EO,HI,HF ADDITIONAL PAYER IDENTIFICATION. Situational. We do not use.
			//1000A PER: CX PAYER BUSINESS CONTACT INFORMATION. Situational. We do not use.
			//1000A PER: BL PAYER TECHNICAL CONTACT INFORMATION. Required. However, real world ERAs leave this segment out and our import works without this. We do not use.
			//1000A PER: IC PAYER WEB SITE. Situational.
			//1000B N1: PE PAYEE IDENTIFICATION. Required.
			WriteSegment("N1",
				"PE",//N101 2/3 Entity Identifier Code: PE=Payee
				"Testing",//N102 1/60 Name:
				"FI",//N103 1/2 Identification Code Qualifier: FI=Federal Taxpayer's Identification Number
				"1234567890"//N104 2/80 Identification Code:
			);
			//1000B N3: PAYEE ADDRESS. Situational.
			...
			for(int i=0;i<ListClaimsPaid.Count;i++) {
				...
				//2100 CLP: CLAIM PAYMENT INFORMATION. Required.
				WriteSegment("CLP",
					ListClaimsPaid[i].ClaimTrackingNumber,//CLP01 1/38 Claim Submitter's Idenfitier: Required.
					ListClaimsPaid[i].CodeClp02,//CLP02 1/2 Claim Status Code: Required.
					ListClaimsPaid[i].ClaimFee,//CLP03 1/18 Total CLaim Charge Amount: Required.
					ListClaimsPaid[i].InsPaid,//CLP04 1/18 Claim Payment Amount: Required.
					ListClaimsPaid[i].PatientRespAmt,//CLP05 1/18 Patient Responsibility Amount: Situational.
					"12",//CLP06 1/2 Claim Filing Indicator Code: 12=Preferred Provider Organization (PPO)
					ListClaimsPaid[i].PayerControlNumber//CLP07 1/50 Reference Identification: Required.
					//The remaining elements are either situational or not used.
				);
				//2100 CAS: CO,OA,PI,PR CLAIM ADJUSTMENT. Repeat 99. Situational. Only needed for claim level adjustments.
				//2100 NM1: QC PATIENT NAME. Required.
				WriteSegment("NM1",
					"QC",//NM101 2/3 Entity Identifier Code: QC=Patient
					"1",//NM102 1/1 Entity Type Qualifier: 1=Person
					ListClaimsPaid[i].PatientName.Lname,//NM103 1/60 Last Name:
					ListClaimsPaid[i].PatientName.Fname,//NM104 1/35 Name First:
					ListClaimsPaid[i].PatientName.Mname,//NM105 1/25 Name Middle:
					//NM106 1/10 Name Prefix: Not Used.
					ListClaimsPaid[i].PatientName.Suffix,//NM107 1/10 Name Suffix:
					"MI",//NM108 1/2 Identification Code: MI=Member Identification Number
					ListClaimsPaid[i].PatientName.SubscriberId//NM109 2/80 Identification Code:
					//The remaining elements are not used.
				);
				//2100 NM1: IL INSURED NAME. Situational. Required when different than patient.
				if(ListClaimsPaid[i].PatientName.Lname!=ListClaimsPaid[i].SubscriberName.Lname
					|| ListClaimsPaid[i].PatientName.Fname!=ListClaimsPaid[i].SubscriberName.Fname
					|| ListClaimsPaid[i].PatientName.Mname!=ListClaimsPaid[i].SubscriberName.Mname
					|| ListClaimsPaid[i].PatientName.Suffix!=ListClaimsPaid[i].SubscriberName.Suffix
					|| ListClaimsPaid[i].PatientName.SubscriberId!=ListClaimsPaid[i].SubscriberName.SubscriberId)
				{
					WriteSegment("NM1",
						"IL",//NM101 2/3 Entity Identifier Code: IL=Insured or Subscriber
						"1",//NM102 1/1 Entity Type Qualifier: 1=Person
						ListClaimsPaid[i].SubscriberName.Lname,//NM103 1/60 Last Name:
						ListClaimsPaid[i].SubscriberName.Fname,//NM104 1/35 Name First:
						ListClaimsPaid[i].SubscriberName.Mname,//NM105 1/25 Name Middle:
						//NM106 1/10 Name Prefix: Not Used.
						ListClaimsPaid[i].SubscriberName.Suffix,//NM107 1/10 Name Suffix:
						"MI",//NM108 1/2 Identification Code: MI=Member Identification Number
						ListClaimsPaid[i].SubscriberName.SubscriberId//NM109 2/80 Identification Code:
						//The remaining elements are not used.
					);
				}
				//2100 NM1: 74 CORRECTED PATIENT/INSURED NAME. Situational.
				//2100 NM1: 82 SERVICE PROVIDER NAME. Situational. Required when rendering provider is different from the payee.
				//2100 NM1: TT CROSSOVER CARRIER NAME. Situational.
				//2100 NM1: PR CORRECTED PRIORITY PAYER NAME. Situational.
				//2100 NM1: GB OTHER SUBSCRIBER NAME. Situational.
				//2100 MIA: INPATIENT ADJUDICATION INFORMATION. Situational.
				//2100 MOA: OUTPATIENT ADJUDICATION INFORMATION. Situational.
				//2100 REF: 1L,1W,28,6P,9A,9C,BB,CE,EA,F8,G1,G3,IG,SY OTHER CLAIM RELATED IDENTIFICATION. Situational.
				//2100 REF: 0B,1A,1B,1C,1D,1G,1H,1J,D3,G2,LU RENDERING PROVIDER IDENTIFICATION. Situational. Additional identifiers when needed.
				//2100 DTM: 232,233 STATEMENT FROM OR TO DATE. Repeat 2. Situational.
				if(ListClaimsPaid[i].DateServiceStart!=DateTime.MinValue) {
					WriteSegment("DTM",
						"232",//DTM01 3/3 Date/Time Qualifier: 232=Claim Statement Period Start
						ListClaimsPaid[i].DateServiceStart.ToString("yyyyMMdd")//DTM02 8/8 Date
					);
				}
				if(ListClaimsPaid[i].DateServiceEnd!=DateTime.MinValue) {
					WriteSegment("DTM",
						"233",//DTM01 3/3 Date/Time Qualifier: 233=Claim Statement Period End
						ListClaimsPaid[i].DateServiceEnd.ToString("yyyyMMdd")//DTM02 8/8 Date
					);
				}
				//2100 DTM: 036 COVERAGE EXPIRATION DATE. Situational.
				//2100 DTM: 050 CLAIM RECEIVED DATE. Situational.
				WriteSegment("DTM",
					"050",//DTM01 3/3 Date/Time Qualifier: 050=Received
					DateTime.Today.ToString("yyyyMMdd")//DTM02 8/8 Date
				);
				//2100 PER: CX CLAIM CONTACT INFORMATION. Situational.
					...
					if(ListClaimsPaid[i].ListProcs[j].ProcCodeBilled!=ListClaimsPaid[i].ListProcs[j].ProcCodeAdjudicated) {
						svc06=CompositeElement(//Required when the procedure code in SVC01 is different than the submitted procedure code.
							"AD",//SVC01-1 2/2 Product/Service ID Qualifier: AD=American Dental Association Codes
								ListClaimsPaid[i].ListProcs[j].ProcCodeBilled//SVC01-2 1/48 Product/Service ID: Adjudicated Procedure Code
								//SVC01-3 2/2 Procedure Modifier. Situational.
								//SVC01-4 2/2 Procedure Modifier. Situational.
								//SVC01-5 2/2 Procedure Modifier. Situational.
								//SVC01-6 2/2 Procedure Modifier. Situational.
								//SVC01-7 1/48 Description. Not Used.
								//SVC01-8 1/18 Product/Service ID. Not Used.
						);
					}
					WriteSegment("SVC",
						...
						CompositeElement(
							"AD",//SVC01-1 2/2 Product/Service ID Qualifier: AD=American Dental Association Codes
							ListClaimsPaid[i].ListProcs[j].ProcCodeAdjudicated//SVC01-2 1/48 Product/Service ID: Adjudicated Procedure Code
							//SVC01-3 2/2 Procedure Modifier. Situational.
							//SVC01-4 2/2 Procedure Modifier. Situational.
							//SVC01-5 2/2 Procedure Modifier. Situational.
							//SVC01-6 2/2 Procedure Modifier. Situational.
						...
						ListClaimsPaid[i].ListProcs[j].ProcFee,//SVC02 1/18 Monetary Amount. Line Item Charge Amount. Required.
						ListClaimsPaid[i].ListProcs[j].InsPaid,//SVC03 1/18 Monetary Amount. Line Item Provider Payment Amount. Required.
						//SVC04 1/48 Product/Service ID. Situational. Only for SUBC revenue codes.
						//SVC05 1/15 Quantity: Situational. Required when the paid units of service are different than one.
						svc06//SVC06 COMPOSITE MEDICAL PROCEDURE IDENTIFIER. Situational. Required when the procedure code in SVC01 is different than the submitted procedure code.
						//SVC07 1/15 Quantity. Situational. Required when the paid units of service in SVC05 is different from the submitted units of service on the claim.
					);
					//2110 DTM: 150,151,472 SERVICE DATE. Repeat 2. Situational. Required when Statement From or Through Dates are not supplied of when different than the claim level.
					if(ListClaimsPaid[i].ListProcs[j].DateServiceStart!=DateTime.MinValue) {
						WriteSegment("DTM",
							"150",//DTM01 3/3 Date/Time Qualifier: 150=Service Period Start
							ListClaimsPaid[i].ListProcs[j].DateServiceStart.ToString("yyyyMMdd")//DTM02 8/8 Date
						);
					}
					if(ListClaimsPaid[i].ListProcs[j].DateServiceEnd!=DateTime.MinValue) {
						WriteSegment("DTM",
							"151",//DTM01 3/3 Date/Time Qualifier: 151=Service Period End
							ListClaimsPaid[i].ListProcs[j].DateServiceEnd.ToString("yyyyMMdd")//DTM02 8/8 Date
						);
					}
					//2110 CAS: SERVICE ADJUSTMENT. Repeat 99. Situational. Required when dollar amounts are being adjusted specific to the service or when SVC03 is different than SVC02.
					if(ListClaimsPaid[i].ListProcs[j].DeductibleAmt!=0) {
						WriteSegment("CAS",
							"PR",//CAS01 1/2 Claim Adjustment Group Code: PR=Patient Responsibility
							"1",//CAS02 1/5 Claim Adjustment Reason Code: 1=Deductible Amount
							ListClaimsPaid[i].ListProcs[j].DeductibleAmt//CAS03 1/18 Monetary Amount
						);
					}
					if(ListClaimsPaid[i].ListProcs[j].WriteoffAmt!=0) {
						WriteSegment("CAS",
							"CO",//CAS01 1/2 Claim Adjustment Group Code: CO=Contractual Obligations
							...
							ListClaimsPaid[i].ListProcs[j].WriteoffAmt//CAS03 1/18 Monetary Amount
						);
					}
					//2110 REF: 1S,APC,BB,E9,G1,G3,LU,RB SERVICE IDENTIFICATION. Situational.
					//2110 REF: 6R LINE ITEM CONTROL NUMBER. Situational.
						//TODO: We need a field to pull from which is unaltered. Leaving blank for now as matching should still work in one way or another.
					//2110 REF: 0B,1A,1B,1C,1D,1G,1H,1J,D3,G2,HPI,SY,TJ RENDERING PROVIDER INFORMATION. Situational. Required when the rendering provider on the procedure is different than the claim.
					//2110 REF: 0K HEALTHCARE POLICY IDENTIFICATION. Situational.
					//2110 AMT: B6,KH,T,T2,ZK,ZL,ZM,ZN,ZO SERVICE SUPPLEMENTAL AMOUNT. Situational.
					//2110 QTY: ZK,ZL,ZM,ZN,ZO SERVICE SUPPLEMENTAL QUANTITY. Situational.
					//2110 LQ: HE,RX HEALTH CARE REMARK CODES. Situational.
						//TODO: Explain adjustment reasons and which UI options caused them.
				}
			}
			//PLB: PROVIDER ADJUSTMENT. Situational.
			//Transaction Trailer
			WriteSegment("SE",
				(_seg+1).ToString(),//SE01 1/10 Number of Included Segments: Total segments, including ST & SE. We add 1 for this SE segment, since the seg variable is not incremented until after this segment is written.
				transactionNum.ToString().PadLeft(4,'0')//SE02 4/9 Transaction Set Control Number:
			);
			//Functional Group Trailer
			WriteSegment("GE",
				transactionNum.ToString(),//GE01 1/6 Number of Transaction Sets Included.  Always 1 for us.
				groupControlNumber//GE02 1/9 Group Control Number: Must be identical to GS06.
			);
		}

		private X835Literal LiteralElement(string literal) {
			return new X835Literal() { Literal=literal };
		}

		private X835Composite CompositeElement(params string[] arrayComponents) {
			arrayComponents=RemoveTrailingEmpties(arrayComponents);
			return new X835Composite() { ArrayComponents=arrayComponents };
		}
		...
		private void WriteSegment(params object[] arrayElements) {
			...
			for(int i=0;i<arrayElements.Length;i++) {
				...
				else if(arrayElements[i] is decimal dec) {
					arrayElementStrings[i]=dec.ToString("f2");
					if(arrayElementStrings[i].EndsWith(".00")) {//The ERA standard encourages implied zero cents when cents are not specified.
						arrayElementStrings[i]=arrayElementStrings[i].Substring(0,arrayElementStrings[i].Length-3);
					}
				}
				else if(arrayElements[i] is decimal dbl) {
					arrayElementStrings[i]=dbl.ToString("f2");
					if(arrayElementStrings[i].EndsWith(".00")) {//The ERA standard encourages implied zero cents when cents are not specified.
						arrayElementStrings[i]=arrayElementStrings[i].Substring(0,arrayElementStrings[i].Length-3);
					}
				}
				else if(arrayElements[i] is X835Composite comp) {
					arrayElementStrings[i]=string.Join(isa16,comp.ArrayComponents.Select(x => Sout(x,hasUnderscores:true,trimWhiteSpace:false)));
				}
				else if(arrayElements[i] is X835Literal literal) {
					arrayElementStrings[i]=literal.Literal;
				}
			}
			arrayElementStrings=RemoveTrailingEmpties(arrayElementStrings);
			//Calling Sout() here only enforces valid characters, not field lengths.
			_sb.Append(string.Join(s,arrayElementStrings));
			_sb.Append(endSegment);
			_seg++;
		}
		...
		private string Sout(string inputStr,int maxL=-1,int minL=-1,bool hasUnderscores=false,bool trimWhiteSpace=true) {
		...
		#endregion Generate 835
	}

	public class X835Composite {
		public string[] ArrayComponents;
	}

	public class X835Literal {
		public string Literal;
	}

	public enum X835Status {
		///<summary>1 - Just a place holder if there is an issue.  Should never show in UI.</summary>
		None,
		///<summary>2 - There are no received claims attached to the ERA.  There can be one or more detached claims on the ERA.</summary>
		Unprocessed,
		///<summary>3 - Some claims for this ERA have had financial information entered, no finalaized claim payment.</summary>
		Partial,
		///<summary>4 - Ignores manually detached.  All claims for this ERA have had financial information entered, no finalaized claim payment.</summary>
		NotFinalized,
		///<summary>5 - Some claims have been manually detached but all other claims have had financial information entered and finalaized claim payment created.</summary>
		[Description("Finalized*")]
		FinalizedSomeDetached,
		///<summary>6 - All claims have been manually detached.</summary>
		[Description("Finalized*")]
		FinalizedAllDetached,
		///<summary>7 - All claims for this ERA have had financial information entered and a finalaized claim payment was created.</summary>
		Finalized,
	}
	...
	public class Hx835_Claim {
		...
		public bool IsProcessed(List<Hx835_ShortClaimProc> listEraClaimProcs,List<Etrans835Attach> listAttaches) {
			...
			foreach(Hx835_Proc proc in ListProcs) {
				...
				listUnmatchedClaimProcsForEraProcs.Remove(matchedClaimProc);//So it cannot be matched twice.
			}
			if(listProcsNotMatched.Count>0) {//At least one procedure on this claim has not been matched. The claim could also have a total payment as a result.
				//Automatic entry does not proceed unless all procedures are matched, therefore this block of code is only for manual entry.
				//When the customer has the EraAllowTotalPayments pref disabled, they are blocked from finalizing payment for any ERA with total payments on it by design.
				//These offices manually move DedApplied, InsPayAmt, Writeoff, and AllowedOverride from the generated By Total line to the procedures lines.
		...
		private List<Hx835_ShortClaimProc> GetClaimProcsForEraProcs(List<Hx835_ShortClaimProc> listAllClaimProcs,List<long> listSplitClaimNums) {
			...
			foreach(Hx835_ShortClaimProc claimProc in listAllClaimProcs) {
				...
				else if(listSplitClaimNums.Contains(claimProc.ClaimNum) && claimProc.Status.In(ClaimProcStatus.Received,ClaimProcStatus.Supplemental,ClaimProcStatus.NotReceived)) {
	...
	public class Hx835_Proc {
		...
		private bool IsBasicMatch(Hx835_ShortClaimProc claimProc,bool isSupplemental) {
			...
			return ((decimal)claimProc.FeeBilled==ProcFee 
				&& claimProc.Status.In(ClaimProcStatus.Received,ClaimProcStatus.NotReceived,ClaimProcStatus.Preauth));
		}
	...
	///The purpose of using this object instead of a Claim object is to save memory by using a small fraction of the memory of a Claim object.</summary>
	public class Hx835_ShortClaim {
		...
		///<summary>A copy of the data from claim.ClaimStatus.  Single character.</summary>
		public string ClaimStatus;
		///<summary>A copy of the data from claim.PlanNum</summary>
		public long PlanNum;
		///<summary>A copy of the data from claim.DateSent</summary>
		public DateTime DateSent;
		///<summary>A copy of the data from claim.ClaimType.  This is a very short string.</summary>
	...
	///The purpose of using this object instead of a ClaimProc object is to save memory by using a small fraction of the memory of a ClaimProc object.</summary>
	public class Hx835_ShortClaimProc {
		...
		///<summary>A copy of the data from claimproc.WriteOffEst</summary>
		public double WriteOffEst;
		///<summary>A copy of the data from claimproc.WriteOffEstOverride</summary>
		public double WriteOffEstOverride;
		///<summary>A copy of the data from claimproc.Writeoff</summary>
		public double WriteOff;
		///<summary>A copy of the data from claimproc.CodeSent.  This is a very short string.</summary>
		...
		public Hx835_ShortClaimProc(ClaimProc claimProc) {
			...
			WriteOffEst=claimProc.WriteOffEst;
			WriteOffEstOverride=claimProc.WriteOffEstOverride;
		...
		/// <summary>Returns a list of all Hx835_ShortClaimProcs for the list of ProcNums
		/// that are on claims with a status of 'U'(unsent), 'H'(hold until primary), or 'W'(waiting to send).</summary>
		public static List<Hx835_ShortClaimProc> GetUnsentForProcNums(List<long> listProcNums) {
	...
	///<summary>Holds lists for Etrans and their associated Etrans835s and Etrans835Attaches.</summary>
	public class EraData {
		public List<Etrans835> ListEtrans835s=new List<Etrans835>();
		public List<Etrans> ListEtrans=new List<Etrans>();
		public List<Etrans835Attach> ListAttached=new List<Etrans835Attach>();
	}
