    ...
    public class X277 : X12object
    {
            ...
            : base(messageText)
        {
            ...
            for (var i = 0; i < segments.Count; i++)
            {
                ...
                {
                    var entityIdentifierCode = seg.Get(1);
                    ...
                    else if (entityIdentifierCode == "QC")
                    {
                        ...
                        do
                        {
                            ...
                            for (var j = 0; j < 3 && (seg.SegmentID == "REF"); j++)
                            {
                                i++;
                                if (i >= segments.Count)
                                {
        ...
        public DateTime GetInformationSourceReceiptDate()
        {
            if (segNumInfoSourceNM101 != -1)
            {
                try
                {
                    var dateStr = segments[segNumInfoSourceNM101 + 2].Get(3);
                    var dateYear = SIn.Int(dateStr.Substring(0, 4));
                    var dateMonth = SIn.Int(dateStr.Substring(4, 2));
                    var dateDay = SIn.Int(dateStr.Substring(6, 2));
        ...
        public DateTime GetInformationSourceProcessDate()
        {
            if (segNumInfoSourceNM101 != -1)
            {
                try
                {
                    var dateStr = segments[segNumInfoSourceNM101 + 3].Get(3);
                    var dateYear = SIn.Int(dateStr.Substring(0, 4));
                    var dateMonth = SIn.Int(dateStr.Substring(4, 2));
                    var dateDay = SIn.Int(dateStr.Substring(6, 2));
                    return new DateTime(dateYear, dateMonth, dateDay);
        ...
        private int GetSegNumLastSTC2200B()
        {
            ...
            {
                var segNum = segNumInfoReceiverNM101 + 2;
        ...
        {
            var segNum = GetSegNumLastSTC2200B();
            if (segNum != -1)
            {
                segNum++;
                if (segNum < segments.Count)
        ...
        public List<string> GetClaimTrackingNumbers()
        {
            ...
            for (var i = 0; i < segNumsClaimTrackingNumberTRN.Count; i++)
            {
                var seg = segments[segNumsClaimTrackingNumberTRN[i]]; //TRN segment.
        ...
        public string[] GetClaimInfo(string trackingNumber)
        {
            ...
            for (var i = 0; i < segNumsClaimTrackingNumberTRN.Count; i++)
            {
                var segNum = segNumsClaimTrackingNumberTRN[i];
                var seg = segments[segNum]; //TRN segment.
                if (seg.Get(2) == trackingNumber)
                {
                    //TRN02
                    ...
                    var segNumNM1 = segNumsPatientDetailNM1[segNumsPatientDetailNM1.Count - 1]; //very last NM1 segment
                    for (var j = 0; j < segNumsPatientDetailNM1.Count - 1; j++)
                    {
                    ...
                    var stc01 = seg.Get(1).Split(new string[] {Separators.Subelement}, StringSplitOptions.None);
                    ...
                    if (seg.SegmentID == "DTP")
                    {
                        var dateServiceStr = seg.Get(3);
                        var dateServiceStartYear = SIn.Int(dateServiceStr.Substring(0, 4));
                        var dateServiceStartMonth = SIn.Int(dateServiceStr.Substring(4, 2));
                        var dateServiceStartDay = SIn.Int(dateServiceStr.Substring(6, 2));
                        ...
                        {
                            //Date range.
                            var dateServiceEndYear = SIn.Int(dateServiceStr.Substring(9, 4));
                            var dateServiceEndMonth = SIn.Int(dateServiceStr.Substring(13, 2));
                            var dateServiceEndDay = SIn.Int(dateServiceStr.Substring(15, 2));
        ...
        public string GetHumanReadable()
        {
            ...
            var claimTrackingNumbers = GetClaimTrackingNumbers();
            for (var i = 0; i < claimTrackingNumbers.Count; i++)
            {
                var claimInfo = GetClaimInfo(claimTrackingNumbers[i]);
                for (var j = 0; j < claimInfo.Length; j++)
                {
                    result += claimInfo[j] + "\\t";
                }
