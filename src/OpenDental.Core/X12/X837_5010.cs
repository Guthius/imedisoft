...
namespace OpenDentBusiness
{
    public class X837_5010 : X12object
    {
        ...
        private static void WriteFunctionalGroup(StreamWriter sw, List<ClaimSendQueueItem> queueItems, int batchNum, Clearinghouse clearinghouseClin, EnumClaimMedType medType)
        {
            ...
            var HLcount = 1;
            var parentProv = 0; //the HL sequence # of the current provider.
            var parentSubsc = 0; //the HL sequence # of the current subscriber.
            var hasSubord = ""; //0 if no subordinate, 1 if at least one subordinate
            Claim claim;
            InsPlan insPlan;
            InsPlan otherPlan = null;
            InsSub sub;
            var otherSub = new InsSub();
            Patient patient;
            Patient subscriber;
            var otherSubsc = new Patient();
            Carrier carrier;
            var otherCarrier = new Carrier();
            ...
            ProviderDto provTreat; //claim level treating provider.
            ProviderDto billProv = null;
            ...
            for (var i = 0; i < queueItems.Count; i++)
            {
                ...
                if (!x837Controller.HasValidProcNums(claimProcs, procList, claim, out var error))
                {
                    throw new ODException(error);
                }

                initialList = ToothInitials.GetPatientData(claim.PatNum);
                patPlans = PatPlans.Refresh(patient.PatNum);
                //Get all procedures corresponding to the claimprocs which also have a site.
                var listSiteProcs = procList.FindAll(x => claimProcs.Exists(y => y.ProcNum == x.ProcNum) && x.SiteNum != 0);
                ...
                provClinicBill = ProviderClinics.GetFromList(claim.ProvBill, (clinic == null ? 0 : clinic.Id), listProvClinics, true);
                ...
                billProv = Providers.GetById(claim.ProvBill);
                ...
                var usePrvBilling = false;
                if (claim.ProvBill == claim.ProvTreat)
                {
                    usePrvBilling = true;
                }
                else if (medType.In(EnumClaimMedType.Medical, EnumClaimMedType.Institutional))
                {
                ...
                if (medType == EnumClaimMedType.Medical)
                {
                    sw.Write((billProv.IsTin ? "EI" : "SY") + s); //REF01 2/3 Reference Identification Qualifier: EI=Employer's Identification Number (EIN). SY=Social Security Number (SSN).
                }
                else if (medType == EnumClaimMedType.Institutional)
                {
                ...
                sw.Write(Sout(clinic.PhoneNumber, 256)); //PER04  1/256 Communication Number: Telephone number.
                EndSegment(sw); //PER05 through PER08 are situational and PER09 is not used. We do not use.
                if (clinic.PayToAddressLine1 != "")
                {
                    //A clinic Pay To address override is present.
                ...
                sw.Write("SBR" + s);
                var claimIsPrimary = true; //we currently support only primary and secondary.  Plan2, if present, must be the opposite.
                if (claim.ClaimType == "PreAuth")
                {
                    if (otherPlan != null && PatPlans.GetOrdinal(claim.InsSubNum2, patPlans) < PatPlans.GetOrdinal(claim.InsSubNum, patPlans))
                    {
                        //When there are two plans and the secondary plan is in the primary ins position.
                        claimIsPrimary = false;
                    }
                }
                else if (otherPlan != null && (otherPlan.IsMedical || insPlan.IsMedical))
                {
                    //When there are two plans and at least one plan is medical.
                    if (PatPlans.GetOrdinal(claim.InsSubNum2, patPlans) < PatPlans.GetOrdinal(claim.InsSubNum, patPlans))
                    {
                        //The insurance plan order is determined by patplan.ordinal
                        claimIsPrimary = false;
                    }
                }
                ...
                {
                    //Verification ensures that there are two plans associated with the claim in this case.
                    claimIsPrimary = false;
                }
                else if (claim.ClaimType == "Other")
                {
                    //Also used for a medical claim with only one insurance.
                ...
                var relationshipCode = ""; //empty if patient is not subscriber.
                if (claim.PatRelat == Relat.Self)
                {
                    //if patient is the subscriber
                    relationshipCode = "18";
                ...
                var subsOrPatID = GetSubscriberIDorPatID(patPlans, sub);
                sw.Write("NM1" + s
                ...
                var subscIncludeAddressAndGender = false;
                if (IsApex(clearinghouseClin) || IsClaimConnect(clearinghouseClin) || IsEmdeonDental(clearinghouseClin) || IsEmdeonMedical(clearinghouseClin) || IsLindsayTechnicalConsultants(clearinghouseClin) || IsOfficeAlly(clearinghouseClin) || IsEDS(clearinghouseClin) || IsVyneDental(clearinghouseClin))
                {
                    subscIncludeAddressAndGender = true;
                }
                else
                {
                    //X12 standard behavior for everyone else, including: BCBSIdaho, ColoradoMedicaid, Denti-Cal, PostNTrack, WashingtonMedicaid.
                    if (claim.PatRelat == Relat.Self)
                    {
                        //[js 3/14/13 only include when subscriber==patient]
                        subscIncludeAddressAndGender = true;
                    }
                ...
                if (!IsDentiCal(clearinghouseClin))
                {
                    //DentiCal complained that they don't usually want this (except for non-subparted NPIs, which we don't handle).  So far, nobody else has complained.
                ...
                var clm01 = claim.ClaimIdentifier; //Typically PatNum/ClaimNum. Check for uniqueness is performed in UI.
                if (IsDentiCal(clearinghouseClin))
                {
                ...
                for (var j = 0; j < claimProcs.Count; j++)
                {
                ...
                if (medType == EnumClaimMedType.Medical || medType == EnumClaimMedType.Dental)
                {
                    if (claim.AccidentDate.Year > 1880 || claim.SpecialProgramCode != EnumClaimSpecialProgram.none || claim.ClaimType == "PreAuth")
                    {
                        //if val for 11,12, or 19
                    ...
                    if (claim.SpecialProgramCode != EnumClaimSpecialProgram.none || claim.ClaimType == "PreAuth")
                    {
                        //if val for 12, or 19
...
//todo:how to handle preauths?
                ...
                if (medType == EnumClaimMedType.Dental)
                {
                    ...
                    var missingTeeth = ToothInitials.GetMissingOrHiddenTeeth(initialList);
                    bool doSkip;
                    var countMissing = 0;
                    for (var j = 0; j < missingTeeth.Count; j++)
                    {
                        ...
                        doSkip = false;
                        for (var p = 0; p < claimProcs.Count; p++)
                        {
                ...
                var pwk01 = "  "; //This double blank will never actually get sent because of the trim and if statement around the PWK creation.
                ...
                var pwk06 = claim.AttachmentID;
                if (pwk02 == "BM" && pwk06 == "")
                {
                    //If By Mail is checked, they will typically leave attachment ID blank, but X12 requires a value, so we make one up.
                    pwk06 = "00";
                }
                ...
                if (medType == EnumClaimMedType.Dental)
                {
                    ...
                    {
                        //validated to be empty for medical and inst
                        sw.Write("REF" + s
...
//todo: ServiceAuthException
                ...
                if (claim.CorrectionType != ClaimCorrectionType.Original
                    || claim.UniformBillType.Length > 2 && (claim.UniformBillType.Substring(2, 1) == "6" || claim.UniformBillType.Substring(2, 1) == "7" || claim.UniformBillType.Substring(2, 1) == "8"))
                {
                    //correction, replacement or void.
                    sw.Write("REF" + s
                ...
                if (IsClaimConnect(clearinghouseClin))
                {
                    //Since this information has only been requested by ClaimConnect and is optional in the specification, we should only add specific clearinghouses here when requested.
                    sw.Write("REF" + s
                ...
                for (var j = 0; j < Math.Min(maxNoteLength, note.Length); j += 80)
                {
                    sw.Write("NTE" + s
                ...
                var listDiagnoses = Procedures.GetUniqueDiagnosticCodes(Procedures.GetProcsFromClaimProcs(claimProcs), false, listDiagnosesVersions);
                ...
                if (medType == EnumClaimMedType.Institutional && listDiagnoses.Count > 0 && listDiagnoses[0] != "")
                {
                ...
                {
                    //Validation guarantees there is at least one (principal) diagnosis code as required
                    sw.Write("HI" + s
                                  + ((listDiagnosesVersions[0] == 9) ? "BK" : "ABK") + isa16 //HI01-1 1/3 Code List Qualifier Code: BK=ICD-9,ABK=ICD-10 Principal Diagnosis
                                  + Sout(listDiagnoses[0].Replace(".", ""), 30)); //HI01-2 1/30 Industry Code: Diagnosis code. No periods.
                    for (var j = 1; j < listDiagnoses.Count; j++)
                    {
                        //Validated below that there are 12 or fewer common diagnoses for the entire claim.
                        if (listDiagnoses[j] == "")
                        {
                            continue;
                        }
                ...
                else if (medType == EnumClaimMedType.Dental && listDiagnoses.Count > 0 && listDiagnoses[0] != "")
                {
                    //Ensure at least one diagnosis.
                    sw.Write("HI" + s
                                  + ((listDiagnosesVersions[0] == 9) ? "BK" : "ABK") + isa16 //HI01-1 1/3 Code List Qualifier Code: BK=ICD-9,ABK=ICD-10 Principal Diagnosis
                    ...
                    for (var j = 1; j < listDiagnoses.Count; j++)
                    {
                        //We allow up to 12 diagnosis codes per claim (for medical), but the dental format only allows up to 4 (see validation).
                        if (listDiagnoses[j] == "")
                        {
                            continue;
                        }
                ...
                if (medType == EnumClaimMedType.Institutional && listDiagnoses.Count > 0 && listDiagnoses[0] != "")
                {
                    //Ensure at least one (principal) diagnosis exists.
                    sw.Write("HI" + s
                ...
                var sendFacilityNameAndAddress = false;
                if (medType == EnumClaimMedType.Medical)
                {
                ...
                else if (medType == EnumClaimMedType.Dental)
                {
                    ...
                    {
                        //for other clearinghouses, the X12 specs say that we don't send it if it's the same as the billing prov.
                        //and since we always set it the same as the billing prov, we shouldn't send it.
                    }
                }

                var facilityProv = billProv; //If this provider changes in the future, then the validation section will also need to be updated.
                var facilityAddress1 = billingAddress1;
                var facilityAddress2 = billingAddress2;
                var facilityCity = billingCity;
                var facilityState = billingState;
                var facilityZip = billingZip;
                if (site != null && site.ProvNum > 0 && site.ProvNum != billProv.Id)
                {
                    facilityProv = Providers.GetById(site.ProvNum);
                ...
                if (!facilityProv.IsNotPerson || claim.PlaceService == PlaceOfService.Office)
                {
                    //Validated also
                ...
                provClinicTreat = ProviderClinics.GetFromList(claim.ProvTreat, (clinic == null ? 0 : clinic.Id), listProvClinics, true);
                ...
                if (medType == EnumClaimMedType.Medical)
                {
                    ...
                    if (sendFacilityNameAndAddress)
                    {
                        sw.Write("NM1" + s
                                       ...
                                       + "2" + s //NM102 1/1 Entity Type Qualifier: 2=Non-Person Entity.
                                       + Sout(facilityProv.LastName, 60) + s //NM103 1/60 Name Last or Organization Name: Laboratory or Facility Name.
                                       + s //NM104 1/35 Name First: Not used.
                                       + s //NM105 1/25 Name Middle: Not used.
                                       + s //NM106 1/10 Name Prefix: Not used.
                                       + s //NM107 1/10 Name Suffix: Not used.
                                       + "XX" + s //NM108 1/2 Identification Code Qualifier: XX=NPI.
                                       + Sout(facilityProv.NationalProviderId, 80)); //NM109 2/80 Identification Code: Laboratory or Facility Identifier. Validated.
                        EndSegment(sw); //NM110 through NM112 not used.
                ...
                if (medType == EnumClaimMedType.Institutional)
                {
                    ...
                    WriteNM1Provider("71", sw, provTreat.FirstName, provTreat.MiddleName, provTreat.LastName, provTreat.NationalProviderId, false);
                    ...
                    if (IsEmdeonMedical(clearinghouseClin) //For now, only for EmdeonMedical as no one else has reported an issue with this loop being missing.
                        && claimProcs.Any(x => int.TryParse(x.CodeSent, out var codeSent) && codeSent.Between(10000, 69990)))
                    {
                        WriteNM1Provider("72", sw, provTreat.FirstName, provTreat.MiddleName, provTreat.LastName, provTreat.NationalProviderId, false);
                    }
                    ...
                    if (sendFacilityNameAndAddress)
                    {
                        sw.Write("NM1" + s
                                       ...
                                       + s //NM107 1/10 Name Suffix: Not used.
                                       + "XX" + s //NM108 1/2 Identification Code Qualifier: XX=NPI.
                                       + Sout(facilityProv.NationalProviderId, 80)); //NM109 2/80 Identification Code: Laboratory or Facility Primary Identifier. Validated.
                        EndSegment(sw); //NM110 through NM112 not used.
                ...
                if (medType == EnumClaimMedType.Dental)
                {
                    ...
                    var sendClaimTreatProv = false;
                    if (claim.ProvTreat != claim.ProvBill)
                    {
                        //Emdeon will reject the claim if this segment is the same as the billing provider for all claims in the batch.
                ...
                var listOtherClaimProcs = new List<List<ClaimProc>>();
                var listProcWriteoffAmts = new List<double>();
                var listProcDeductibleAmts = new List<double>();
                var listProcPaidOtherInsAmts = new List<double>();
                var hasAdjForOtherPlans = false;
                var datePaidOtherIns = DateTime.MinValue;
                ...
                if (otherPlan != null)
                {
                    ...
                    var listProcNums = claimProcs.Select(x => x.ProcNum).Distinct().ToList();
                    var listOtherClaimNums = claimProcList.FindAll(x => x.ClaimNum != claim.ClaimNum
                    ...
                    var listTotalPaymentProcs = claimProcList.FindAll(x => listOtherClaimNums.Contains(x.ClaimNum) && x.ProcNum == 0);
                    foreach (var cpTotalPayment in listTotalPaymentProcs)
                    {
                    ...
                    var claimPatientPortionAmt = Math.Max(0, claim.ClaimFee - claimWriteoffAmt - claimDeductibleAmt - claimPaidOtherInsAmt);
                ...
                for (var j = 0; j < claimProcs.Count; j++)
                {
                    ...
                    if (medType == EnumClaimMedType.Medical)
                    {
                        ...
                        {
                            //If the all 4 procedure diagnoses are blank, we will use the primary diagnosis for entire claim.
                            if (listDiagnoses[0] != "")
                            {
                                //Ensure that a primary diagnosis exists.
                                sw.Write("1"); //use primary.
                            }
                        }
                        else
                        {
                            //There is at least one diagnostic code on the proc, and also at least one proc on the claim (at least the principal diagnosis).
                            var diagMatchCount = 0;
                            for (var d = 0; d < listDiagnoses.Count; d++)
                            {
                                //this list is filled with unique diagnosis codes so the following logic will create 4 correct entries.								
                                if (listDiagnoses[d] == "")
                                {
                                    continue;
                                }
                    ...
                    else if (medType == EnumClaimMedType.Dental)
                    {
                        ...
                        var unitQty = Math.Max(proc.UnitQty, 1); //Minimum of 1
                        var includeUnits = false;
                        if (unitQty >= 2)
                        {
                            //Standard behavior based on the X12 guide.
                            includeUnits = true;
                        }
                        ...
                        else if (procCode.TreatArea == TreatmentArea.Surf)
                        {
                            ...
                            var validSurfaces = Tooth.SurfTidyForClaims(proc.Surf, proc.ToothNum);
                            for (var k = 0; k < validSurfaces.Length; k++)
                            {
                                if (k > 0)
                                {
                                    sw.Write(isa16);
                        ...
                        {
                            var individTeeth = proc.ToothRange.Split(',');
                            for (var t = 0; t < individTeeth.Length; t++)
                            {
                                sw.Write("TOO" + s
                                               + "JP" + s //TOO01 1/3 Code List Qualifier Code: JP=Universal National Tooth Designation System.
                    ...
                    var ordinal = PatPlans.GetOrdinal(claim.InsSubNum, patPlans);
                    var ref02 = ("x" + proc.ProcNum.ToString() + "/" + ordinal + "/" + insPlan.PlanNum); //Version 3:
                    if (ref02.Length > 30)
                    {
                        ...
                        var overflowCount = (ref02.Length - 30);
                    ...
                    var provTreatProc = provTreat; //procedure level treating provider.
                    var provClinicProc = provClinicTreat;
                    if (medType == EnumClaimMedType.Medical && claim.ProvTreat != proc.ProvNum && PrefC.GetBool(PrefName.EclaimsSeparateTreatProv))
                    {
                        provTreatProc = Providers.GetById(proc.ProvNum);
                        provClinicProc = ProviderClinics.GetOneOrDefault(proc.ProvNum, (clinic == null ? 0 : clinic.Id));
                    }

                    ProviderDto provOrderProc = null;
                    if (medType == EnumClaimMedType.Medical)
                    {
                    ...
                    {
                        var provID = provClinicProc?.StateLicense;
                        if (string.IsNullOrEmpty(provID))
                        {
                            provID = (provOrderProc ?? provTreatProc)?.NationalProviderId ?? "";
                        }
                    ...
                    if (medType == EnumClaimMedType.Medical)
                    {
                        ...
                        if (provOrderProc != null && !provOrderProc.IsNotPerson)
                        {
                            //Ordering provider is a person.
                    ...
                    if (medType == EnumClaimMedType.Institutional)
                    {
                        ...
                        if (IsEmdeonMedical(clearinghouseClin) //Do this for Emdeon Medical because we also include Loop 2310B for them.
                            ...
                            && int.TryParse(claimProcs[j].CodeSent, out var codeSent) && codeSent.Between(10000, 69990)) //See task #2712788
                        {
                            provTreatProc = Providers.GetById(proc.ProvNum);
                            WriteNM1Provider("72", sw, provTreatProc.FirstName, provTreatProc.MiddleName, provTreatProc.LastName, provTreatProc.NationalProviderId, false);
                        }
                            ...
                            && PrefC.GetBool(PrefName.EclaimsSeparateTreatProv))
                        {
                            ...
                            WriteNM1Provider("82", sw, provTreatProc.FirstName, provTreatProc.MiddleName, provTreatProc.LastName, provTreatProc.NationalProviderId, false);
                    ...
                    {
                        foreach (var claimProcOther in listOtherClaimProcs[j])
                        {
                            sw.Write("SVD" + s
                    ...
                    if (hasAdjForOtherPlans && (IsApex(clearinghouseClin) || hasProcedureLevelCob))
                    {
                        var procPatientPortionAmt = Math.Max(0, claimProcs[j].FeeBilled - listProcWriteoffAmts[j] - listProcDeductibleAmts[j] - listProcPaidOtherInsAmts[j]);
                    ...
                    if (hasAdjForOtherPlans && !IsApex(clearinghouseClin) && hasProcedureLevelCob)
                    {
                        ...
                        var listOtherClaimNumsProcCur = claimProcList.FindAll(x => x.ClaimNum != claim.ClaimNum
                                                                                   && x.Status.In(ClaimProcStatus.CapClaim, ClaimProcStatus.Received, ClaimProcStatus.Supplemental)
                                                                                   && x.ProcNum == proc.ProcNum).Select(x => x.ClaimNum).Distinct().ToList();
                        var listTotalPaymentClaimprocsForProcCur = claimProcList.FindAll(x => listOtherClaimNumsProcCur
                            .Contains(x.ClaimNum) && x.ProcNum == 0);
                        var listOtherClaimProcsForProcCur = listOtherClaimProcs.SelectMany(x => x).Where(x => x.ProcNum == proc.ProcNum)
                            .Concat(listTotalPaymentClaimprocsForProcCur).ToList();
                        var datePaidOtherInsProcCur = DateTime.MinValue;
        ...
        ///<summary>Pass in either a clinic or HQ-level clearinghouse.</summary>
        private static bool IsVyneDental(Clearinghouse clearinghouse)
        {
            return (clearinghouse.ISA08 == "RSSLL");
        }
        ...
        {
            var name = "OPEN DENTAL SOFTWARE";
            var idCode = "810624427";
        ...
        private static void Write2010AASiteIDforEmdeon(StreamWriter sw, ProviderDto prov, string payorID)
        {
            var provIdents = ProviderIdents.GetForPayor(prov.Id, payorID);
            for (var i = 0; i < provIdents.Length; i++)
            {
        ...
        private static void WriteProv_REFG2orLU(StreamWriter sw, ProviderDto prov, string payorID)
        {
            var segmentType = "G2";
            var provID = "";
            var electID = ElectIDs.GetId(payorID);
            if (electID is {IsMedicaid: true})
            {
        ...
        private static void WriteNM1_DN(StreamWriter sw, long referringProv)
        {
            ...
            {
                //Could be null if referringProv=0 (common)
                return;
            }
        ...
        private static void WriteNM1Provider(string entityIdentifierCode, StreamWriter sw, ProviderDto prov)
        {
            WriteNM1Provider(entityIdentifierCode, sw, prov.FirstName, prov.MiddleName, prov.LastName, prov.NationalProviderId, prov.IsNotPerson);
        }
        ...
        private static void WritePRV_PE(StreamWriter sw, ProviderDto provTreat)
        {
            //2310B PRV: PE (dental) Rendering Provider Specialty Information.
            sw.Write("PRV" + s
                           + "PE" + s //PRV01 1/3 Provider Code: PE=Performing.
        ...
        private static string GetCarrierElectID(Carrier carrier, Clearinghouse clearinghouse)
        {
            ...
            if (electid == "" && IsTesia(clearinghouse))
            {
                //only for Tesia
        ...
        private static string GetRelatedCauses(Claim claim)
        {
            ...
            {
                // if(claim.AccidentRelated=="O"){ //other accident
                return "OA";
            }
        }
        ...
        {
            var filingcode = InsFilingCodes.GetEclaimCode(plan.FilingCode);
        ...
        {
            return drugUnit switch
            {
                EnumProcDrugUnit.Gram => "GR",
                EnumProcDrugUnit.InternationalUnit => "F2",
                EnumProcDrugUnit.Milligram => "ME",
                EnumProcDrugUnit.Milliliter => "ML",
                EnumProcDrugUnit.Unit => "UN",
                _ => "UN"
            };
        }
        ...
        private static ProviderDto GetOrderingProviderForProc(Claim claim, Carrier carrier, Procedure proc, ProviderDto provTreatProc)
        {
            ProviderDto provOrderProc = null;
            if (proc.ProvOrderOverride != 0)
            {
                provOrderProc = Providers.GetById(proc.ProvOrderOverride); //Override ordering provider at procedure level.
            }
            else if (proc.OrderingReferralNum != 0)
            {
                if (Referrals.TryGetReferral(proc.OrderingReferralNum, out var referralOrdering))
                {
                    provOrderProc = new ProviderDto
                    {
                        IsNotPerson = referralOrdering.NotPerson,
                        Abbr = "Referral: " + referralOrdering.GetNameFL(),
                        FirstName = referralOrdering.FName,
                        MiddleName = referralOrdering.MName,
                        LastName = referralOrdering.LName,
                        NationalProviderId = referralOrdering.NationalProvID
                    };
                }
            }
            else if (claim.ProvOrderOverride != 0)
            {
                provOrderProc = Providers.GetById(claim.ProvOrderOverride); //Override ordering provider at claim level.
            }
            else if (claim.OrderingReferralNum != 0)
            {
                if (Referrals.TryGetReferral(claim.OrderingReferralNum, out var referralOrdering))
                {
                    provOrderProc = new ProviderDto
                    {
                        IsNotPerson = referralOrdering.NotPerson,
            ...
            else if (Carriers.IsMedicaid(carrier))
            {
        ...
        public static string GetSubscriberIDorPatID(List<PatPlan> listPatPlans, InsSub insSub)
        {
            ...
            var patPlanMatching = listPatPlans.FirstOrDefault(x => x.InsSubNum == insSub.InsSubNum);
        ...
        public static void Validate(Clearinghouse clearinghouseClin, ClaimSendQueueItem queueItem, bool skipUB04 = false)
        {
            //,out string warning) {
            ...
            var claimItems = Claims.GetX12TransactionInfo(((ClaimSendQueueItem) queueItem).ClaimNum); //just to get prov. Needs work.
            ...
            var billProv = Providers.GetFirstOrDefault(x => x.Id == claimItems[0].ProvBill1) ?? providerFirst;
            ...
            var sub = InsSubs.GetSub(claim.InsSubNum, null);
            var patPlans = PatPlans.Refresh(claim.PatNum);
            ...
            if (IsEmdeonMedical(clearinghouseClin))
            {
                //The X12 standard has a basic character set, but Emdeon Medical only allows a subset of the basic character set, as seen in error messages within their interface.
                if (!billProv.IsNotPerson && !Regex.IsMatch(billProv.FirstName, "^[A-Za-z ']+$"))
                {
                    //If not a person, then X12 generation will leave this blank, regardless of what user entered.
                    Comma(strb);
                    strb.Append("Billing Prov FName may contain letters spaces and apostrophes only");
                }

                if (!Regex.IsMatch(billProv.LastName, "^[A-Za-z ']+$"))
                {
                    Comma(strb);
                    strb.Append("Billing Prov LName may contain letters spaces and apostrophes only");
                }

                if (!billProv.IsNotPerson && !Regex.IsMatch(billProv.MiddleName, "^[A-Za-z ']*$"))
                {
                    //If not a person, then X12 generation will leave this blank, regardless of what user entered.
                    Comma(strb);
            ...
            if (clinic.PayToAddressLine1 != "")
            {
                //Clinic Pay To address is present.
            ...
            if (treatProv.FirstName == "" && !treatProv.IsNotPerson)
            {
                Comma(strb);
            ...
            if (treatProv.TaxonomyCode is not null && treatProv.TaxonomyCode.Length != 10)
            {
                Comma(strb);
            ...
            if (claim.MedType == EnumClaimMedType.Dental)
            {
                if (claim.PlaceService != PlaceOfService.Office)
                {
                    ...
                    if (IsClaimConnect(clearinghouseClin))
                    {
                        if (!facilityProv.IsNotPerson)
                        {
                            //In medical, institutional and dental, the facility provider must be a non-person.
                            Comma(strb);
            ...
            var patPlan = PatPlans.GetFromList(patPlans, claim.InsSubNum); //can be null
            var hasWarnedPatID = false;
            ...
            if (electID is {IsMedicaid: true} && billProv.MedicaidId == "")
            {
                Comma(strb);
            ...
            if (claim.PlanNum2 > 0)
            {
                var insPlan2 = InsPlans.GetPlan(claim.PlanNum2, new List<InsPlan>());
                var sub2 = InsSubs.GetSub(claim.InsSubNum2, null);
                ...
                var subscriber2 = Patients.GetPat(sub2.Subscriber); //Always exists because validated in UI.
                ...
                var otherCarrier = Carriers.GetCarrier(insPlan2.CarrierNum);
                ...
                var patPlan2 = PatPlans.GetFromList(patPlans, claim.InsSubNum2); //can be null
            ...
            var subscriber = Patients.GetPat(sub.Subscriber);
            ...
            if (!Regex.IsMatch(patient.Zip.Trim(), "^[0-9]{5}\\-?([0-9]{4})?$"))
            {
            ...
            if (claim.AccidentRelated == "A" && claim.AccidentST.Length != 2)
            {
            ...
            var claimProcList = ClaimProcs.RefreshForClaim(claim.ClaimNum);
            var claimProcs = ClaimProcs.GetForSendClaim(claimProcList, claim.ClaimNum);
            ...
            var procList = Procedures.GetProcsFromClaimProcs(claimProcs);
            ...
            Procedure proc;
            ProcedureCode procCode;
            var princDiagExists = false;
            var hasZeroFeeBilled = false;
            for (var i = 0; i < claimProcs.Count; i++)
            {
                var p = "proc" + (i + 1).ToString() + "-";
                ...
                if (claim.MedType == EnumClaimMedType.Medical)
                {
                    ...
                    if (Regex.IsMatch(claimProcs[i].CodeSent, "^[0-9]{3}99$") && proc.ClaimNote.Trim() == "")
                    {
                        //CPT codes ending in 99.
                        Comma(strb);
                ...
                else if (claim.MedType == EnumClaimMedType.Dental)
                {
                    ...
                    if (procCode.IsProsth)
                    {
                        ...
                        if (proc.Prosthesis == "R" && proc.DateOriginalProsth.Year < 1880)
                        {
                ...
                if (claim.ProvTreat != proc.ProvNum && PrefC.GetBool(PrefName.EclaimsSeparateTreatProv))
                {
                    ...
                    if (claim.MedType != EnumClaimMedType.Institutional)
                    {
                        //Medical and Dental only. No where to send taxonomy code for instituational procedures.
                        if (provTreatProc.TaxonomyCode is not null && provTreatProc.TaxonomyCode.Length != 10)
                        {
                            Comma(strb);
                ...
                if (claim.MedType == EnumClaimMedType.Medical)
                {
                    var provOrderProc = GetOrderingProviderForProc(claim, carrier, proc, provTreatProc);
                    //Do not validate ordering provider override name or NPI if will not go out on eclaim or if already validated elsewhere.
                    if (provOrderProc != null && !provOrderProc.IsNotPerson && provOrderProc.Id != proc.ProvNum &&
                        provOrderProc.Id != claim.ProvTreat && provOrderProc.Id != claim.ProvBill)
                    {
                ...
                if (proc.SiteNum > 0)
                {
                    ...
                    if (site.ProvNum > 0)
                    {
                        var provFacility = Providers.GetById(site.ProvNum);
                        ...
                        if (!Regex.IsMatch(site.Zip.Trim(), "^[0-9]{5}\\-?([0-9]{4})?$"))
                        {
                            //#####, or #####-, or #####-####, or #########. Dashes are removed when X12 is generated.
                            Comma(strb);
            ...
            if (hasZeroFeeBilled)
            {
                ...
                warning += "Claim contains one or more procedures with a $0 fee billed";
            }
        ...
        {
            var regExp = @"\bbox(?:\b$|([\s|\-]+)?[0-9]+)|(p[\-\.\s]*o[\-\.\s]*|(post office|post)\s)b(\.|ox|in)?\b|(^p[\.]?(o|b)[\.]?$)";
