...
namespace OpenDentBusiness
{
    public class X270 : X12object
    {
        ...
        public static string GenerateMessageText(Clearinghouse clearinghouseClin, Carrier carrier, ProviderDto billProv, ClinicDto clinic, InsPlan insPlan, Patient subscriber, InsSub insSub, Patient patForRequest)
        {
            var isSubscriberRequest = (subscriber.PatNum == patForRequest.PatNum);
            var batchNum = Clearinghouses.GetNextBatchNumber(clearinghouseClin);
            ...
            var seg = 0; //count segments for the ST-SE transaction
            ...
            strb.AppendLine("NM1*1P*" //NM101: 1P=Provider
                            ...
                            + Sout(billProv.MiddleName, 25, 1) + "*" //NM105: Middle name
                            + "*" //NM106: not used
            ...
            {
                var ref4aSegment = "";
                var clearinghouseDentiCalHQ = Clearinghouses.GetFirstOrDefault(x => IsDentiCalClearinghouse(x), true);
                if (clearinghouseDentiCalHQ != null)
                {
                    var clearinghouseDentiCalClin = Clearinghouses.OverrideFields(clearinghouseDentiCalHQ, clearinghouseClin.ClinicNum);
            ...
            if (!string.IsNullOrEmpty(insPlan.GroupNum))
            {
                //Is allowed to be blank for Medicaid.
...
IEA*1*000012145~";
        ...
        public static string Validate(Clearinghouse clearinghouseClin, Carrier carrier, ProviderDto billProv, ClinicDto clinic, InsPlan insPlan, Patient subscriber, InsSub insSub, Patient patForRequest)
        {
            var strb = new StringBuilder();
            ...
            if (patForRequest.PatNum != subscriber.PatNum)
            {
                //Dependent validation.
                if (patForRequest.Birthdate.Year < 1880)
                {
            ...
            var eID = ElectIDs.GetId(carrier.ElectID);
            //Medicaid uses the patient First/Last/DOB to get benefits, not group number, so skip this validation if this is medicaid
            var isMedicaid = (eID != null && eID.IsMedicaid) || carrier.CarrierName.ToLower().Contains("medicaid");
            if (!isMedicaid)
            {
                if (insPlan.GroupNum == "")
                {
