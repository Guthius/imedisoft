...
{
	/// <summary>
	///     Saves EhrLab to DB and all child elements.  Note: this can be used to overwrite new data with old data when viewing
	///     old messages.
	...
	public static EhrLab SaveToDB(EhrLab ehrLab)
    {
        ...
        else if (GetByGUID(ehrLab.FillerOrderUniversalID, ehrLab.FillerOrderNum) != null) ehrLab.EhrLabNum = GetByGUID(ehrLab.FillerOrderUniversalID, ehrLab.FillerOrderNum).EhrLabNum; //identified by the filler order num... rarely
        ...
        for (var i = 0; i < ehrLab.ListEhrLabNotes.Count; i++)
        {
        ...
        for (var i = 0; i < ehrLab.ListEhrLabResults.Count; i++)
        {
        ...
        for (var i = 0; i < ehrLab.ListEhrLabResultsCopyTo.Count; i++)
        {
        ...
        for (var i = 0; i < ehrLab.ListEhrLabSpecimens.Count; i++)
        {
        ...
        for (var i = 0; i < ehrLab.ListRelevantClinicalInformations.Count; i++)
        {
    ...
    {
        return EhrLabCrud.SelectOne(ehrLabNum);
    }

    public static long GetNextOrderNum()
    {
        var command = "SELECT MAX(EhrLabNum) FROM ehrlab";
        long retVal = 0;
        try
        {
            retVal = SIn.Long(DataCore.GetScalar(command)); //can be null if table is empty.
        }
        catch
        {
        }
    ...
    {
        var command = "SELECT * FROM ehrlab WHERE (PlacerOrderNum='" + extension + "' AND PlacerOrderUniversalID='" + root + "'AND PlacerOrderNum!='' AND PlacerOrderUniversalID!='') "
                      + "OR (FillerOrderNum='" + extension + "' AND FillerOrderUniversalID='" + root + "' AND FillerOrderNum!='' AND FillerOrderUniversalID!='')";
        return EhrLabCrud.SelectOne(command);
    }

    
    public static List<EhrLab> GetAllForPat(long patNum)
    {
        var command = "SELECT * FROM ehrlab WHERE PatNum = " + SOut.Long(patNum);
        var retVal = EhrLabCrud.SelectMany(command);
    ...
    /// <summary>
    ///     Helper function to return a list of descriptions for the HL70203 enumeration.  First item in the list is
    ///     blank.
    /// </summary>
    ...
    /// <summary>
    ///     Helper function to return a list of descriptions for the HL70065 enumeration.  First item in the list is
    ///     blank.
    /// </summary>
    ...
    /// <summary>
    ///     Helper function to return a list of descriptions for the HL70123 enumeration.  First item in the list is
    ///     blank.
    /// </summary>
    ...
    {
        return EhrLabCrud.Insert(ehrLab);
    }

    
    public static void Update(EhrLab ehrLab)
    {
        EhrLabCrud.Update(ehrLab);
    }
    ...
    {
        var command = "DELETE FROM ehrlab WHERE EhrLabNum = " + SOut.Long(ehrLabNum);
        Db.NonQ(command);
    ...
    {
        //Patient patcur;
        var PIDSegment = ""; //used to store the PID segment and attached to each lab order generated by this message.
        var listRetVal = new List<EhrLab>();
        var ehrLabCur = new EhrLab();
        ...
        string[] fields;
        var noteSegment = ""; //used to attach notes to the right objects.
        for (var s = 0; s < segments.Length; s++)
        {
            ...
            switch (fields[0])
            {
                //Segment Identifier.
                case "MSH":
                    if (fields[8] != "ORU^R01^ORU_R01") throw new Exception("MSH.9 contained wrong value.  \"" + fields[8] + "\" was found, \"ORU^R01^ORU_R01\" was expected.");
                    if (fields[11] != "2.5.1") throw new Exception("MSH.12 shows message version \"" + fields[11] + "\", only version \"2.5.1\" is currently supported.");
                    containsRequiredSegmentsHelper(message); //validate required segments here, after we have verified this is an ORU_R01 message
                    if (fields[20].Split('~').Length == 0) throw new Exception("MSH.21 does not contain any values, the LRI_GU_RU_Profile value \"2.16.840.1.113883.9.17\" is expected.");
                    for (var i = 0; i < fields[20].Split('~').Length; i++)
                    {
                        if (i == fields[20].Split('~').Length) throw new Exception("MSH.21 (" + i + ") indicates sender's message does not conform to LRI_GU_RU_Profile \"2.16.840.1.113883.9.17\"");
                        if (fields[20].Split('~')[i] == "2.16.840.1.113883.9.17") break; //found expected value.
                    }
                ...
                case "PID":
                    PIDSegment = segment;
                    noteSegment = "PID";
                    for (var i = 0; i < fields[3].Split('~').Length; i++)
                    {
                        if (patCur == null)
                ...
                case "OBR":
                    noteSegment = "OBR";
                    ehrLabCur.SetIdOBR = SIn.Long(fields[1]);
                    if (isImport)
                    {
                    ...
                    if (ehrLabCur.FillerOrderNum != fields[3].Split('^')[0]) throw new Exception("Filler order numbers in OCR.3 and OBR.3 segments do not match.");
                    ...
                    try
                    {
                        ehrLabCur.UsiTextOriginal = fields[4].Split('^')[8];
                    }
                    catch
                    {
                    }
                    ...
                    if (fields[8].Length != 0) ehrLabCur.ObservationDateTimeEnd = fields[8];
                    try
                    {
                        ehrLabCur.SpecimenActionCode = (HL70065) Enum.Parse(typeof(HL70065), fields[11]);
                    }
                    catch
                    {
                    }

                    if (isImport) ehrLabCur.ListRelevantClinicalInformations = new List<EhrLabClinicalInfo>();
                    for (var i = 0; i < fields[13].Split('~').Length; i++)
                    {
                        if (fields[13].Length == 0) break; //nothing to process
                        var tempClinInfo = fields[13].Split('~')[i];
                        var ehrLabClinicalInfo = new EhrLabClinicalInfo();
                        ...
                        try
                        {
                            ehrLabClinicalInfo.ClinicalInfoTextOriginal = tempClinInfo.Split('^')[8];
                        }
                        catch
                        {
                        }
                    ...
                    try
                    {
                        ehrLabCur.ResultStatus = (HL70123) Enum.Parse(typeof(HL70123), fields[25]);
                    }
                    catch
                    {
                    }
                    ...
                    try
                    {
                        ehrLabCur.ParentObservationTextOriginal = fields[26].Split('^')[0].Split('&')[8];
                    }
                    catch
                    {
                    }
                    ...
                    //Results Copy To
                    if (isImport) ehrLabCur.ListEhrLabResultsCopyTo = new List<EhrLabResultsCopyTo>();
                    if (fields.Length < 28) break; //likely that fields beyond this are left out.
                    for (var i = 0; i < fields[28].Split('~').Length; i++)
                    {
                        var tempRCT = new EhrLabResultsCopyTo();
                        var stringRCT = fields[28].Split('~')[i];
                        if (stringRCT.Length == 0) continue; //usually only if nothing was sent, but also if an empty field was sent.
                    ...
                    if (fields.Length < 31) break; //next segment. all additional fields were omitted from this one.
                ...
                case "NTE":
                    ...
                    break;
                case "TQ1":
                    ehrLabCur.TQ1SetId = SIn.Long(fields[1]);
                ...
                case "OBX":
                    ...
                    //	ehrLabCur.ListEhrLabResults=new List<EhrLabResult>();
                    //}
                    var labResult = new EhrLabResult();
                    labResult.SetIdOBX = SIn.Long(fields[1]);
                    try
                    {
                        labResult.ValueType = (HL70125) Enum.Parse(typeof(HL70125), fields[2]);
                    }
                    catch
                    {
                    }
                    ...
                    if (fields[3].Split('^').Length > 8) labResult.ObservationIdentifierTextOriginal = fields[3].Split('^')[8];
                    ...
                    switch (labResult.ValueType)
                    {
                        ...
                        case HL70125.CWE:
                            ...
                            if (labResult.ValueType == HL70125.CWE) labResult.ObservationValueCodedElementTextOriginal = fields[5].Split('^')[8];
                            break;
                        case HL70125.DT:
                        case HL70125.TS:
                        ...
                        case HL70125.NM:
                            ...
                            labResult.ObservationValueNumeric = (fields[5].Contains("-") ? -1f : 1f) * SIn.Double(fields[5].Trim('+').Trim('-'));
                            break;
                        case HL70125.SN:
                            labResult.ObservationValueComparator = fields[5].Split('^')[0];
                            try
                            {
                                labResult.ObservationValueNumber1 = SIn.Double(fields[5].Split('^')[1]);
                            }
                            catch
                            {
                            ...
                            try
                            {
                                labResult.ObservationValueNumber2 = SIn.Double(fields[5].Split('^')[3]);
                            }
                            catch
                            {
                            } //optional, may be a null reference
                            ...
                            break;
                        case HL70125.TM:
                            labResult.ObservationValueTime = SIn.TimeSpan(fields[5]);
                            break;
                    }
                    ...
                    if (fields[6].Length != 0)
                    {
                        ...
                        try
                        {
                            labResult.UnitsTextOriginal = fields[6].Split('^')[8];
                        }
                        catch
                        {
                        }
                    ...
                    try
                    {
                        labResult.ObservationResultStatus = (HL70085) Enum.Parse(typeof(HL70085), fields[11]);
                    }
                    catch
                    {
                    }
                    ...
                    try
                    {
                        labResult.PerformingOrganizationIdentifierTypeCode = (HL70203) Enum.Parse(typeof(HL70203), fields[23].Split('^')[7]);
                    }
                    catch
                    {
                    }
                    ...
                    try
                    {
                        labResult.PerformingOrganizationAddressStateOrProvince = (USPSAlphaStateCode) Enum.Parse(typeof(USPSAlphaStateCode), fields[24].Split('^')[3]);
                    }
                    catch
                    {
                    }
                    ...
                    try
                    {
                        labResult.PerformingOrganizationAddressAddressType = (HL70190) Enum.Parse(typeof(HL70190), fields[24].Split('^')[6]);
                    }
                    catch
                    {
                    }
                    ...
                    if (fields.Length <= 25) break; //next segment. this one is finished.
                    ...
                    try
                    {
                        labResult.MedicalDirectorNameTypeCode = (HL70200) Enum.Parse(typeof(HL70200), fields[25].Split('^')[9]);
                    }
                    catch
                    {
                    }
                    ...
                    try
                    {
                        labResult.MedicalDirectorIdentifierTypeCode = (HL70203) Enum.Parse(typeof(HL70203), fields[25].Split('^')[12]);
                    }
                    catch
                    {
                    }
                ...
                case "SPM":
                    ...
                    //}
                    var ehrLabSpecimen = new EhrLabSpecimen();
                    ehrLabSpecimen.SetIdSPM = SIn.Long(fields[1]);
                    //Specimen Type
                    ...
                    try
                    {
                        ehrLabSpecimen.SpecimenTypeTextOriginal = fields[4].Split('^')[8];
                    }
                    catch
                    {
                    }
                    ...
                    //}
                    //Reject Reason
                    for (var i = 0; i < fields[21].Split('~').Length; i++)
                    {
                        if (fields[21].Length == 0) break; //nothing in this field
                        var ehrLabRR = new EhrLabSpecimenRejectReason();
                        ...
                        try
                        {
                            ehrLabRR.SpecimenRejectReasonTextOriginal = fields[21].Split('~')[i].Split('^')[8];
                        }
                        catch
                        {
                        }
                    ...
                    //Specimen Condition
                    for (var i = 0; i < fields[24].Split('~').Length; i++)
                    {
                        if (fields[24].Length == 0) break; //nothing in this field
                        var ehrLabSC = new EhrLabSpecimenCondition();
                        ...
                        try
                        {
                            ehrLabSC.SpecimenConditionTextOriginal = fields[24].Split('~')[i].Split('^')[8];
                        }
                        catch
                        {
                        }
    ...
    /// <summary>
    ///     Throws an exception if message does not contain all required segments, or contains too many segments of a
    ///     given type.  Does not validate contents of segments.
    /// </summary>
    private static void containsRequiredSegmentsHelper(string message)
    {
        var errors = "";
        var segments = message.Split(new[] {"\r\n"}, StringSplitOptions.RemoveEmptyEntries);
        for (var i = 0; i < segments.Length; i++) segments[i] = segments[i].Split('|')[0]; //now each segment only contains the segment identifier.
        ...
        if (mshCount != 1) errors += "Message should contain exactly 1 MSH segment, " + mshCount + " MSH segments found.\r\n";
        //PID
        var pidCount = 0;
        for (var i = 0; i < segments.Length; i++)
            if (segments[i] == "PID")
        ...
        if (pidCount != 1) errors += "Message should contain exactly 1 PID segment, " + pidCount + " PID segments found.\r\n";
        //ORC
        var orcCount = 0;
        for (var i = 0; i < segments.Length; i++)
            if (segments[i] == "ORC")
        ...
        if (pidCount == 0) errors += "Message should contain 1 or more ORC segments, " + pidCount + " PID segments found.\r\n";
        //ORC followed by OBR
        for (var i = 0; i < segments.Length; i++)
            if (segments[i] == "ORC")
