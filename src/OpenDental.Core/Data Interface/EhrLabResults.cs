...
public class EhrLabResults
{
    ...
    {
        ehrLabResult.EhrLabResultNum = EhrLabResultCrud.Insert(ehrLabResult);
        for (var i = 0; i < ehrLabResult.ListEhrLabResultNotes.Count; i++)
        {
            //save attached notes.
    ...
    {
        return EhrLabResultCrud.Insert(ehrLabResult);
    }

    ///<summary>Get all lab results for one patient.</summary>
    public static List<EhrLabResult> GetAllForPatient(long patNum)
    {
        var command = "SELECT * FROM ehrlabresult WHERE EhrLabNum IN (SELECT EhrLabNum FROM ehrlab WHERE PatNum=" + SOut.Long(patNum) + ")";
        return EhrLabResultCrud.SelectMany(command);
    }

    ///<summary>Returns all EhrLabResults for a given EhrLab.</summary>
    public static List<EhrLabResult> GetForLab(long ehrLabNum)
    {
        var command = "SELECT * FROM ehrlabresult WHERE EhrLabNum = " + SOut.Long(ehrLabNum);
        return EhrLabResultCrud.SelectMany(command);
    }

    /// <summary>
    ///     Only deletes the notes for the Lab, there may still be notes attached to LabResults, that are attached to the
    ///     lab.  Those notes are taken care of by DeleteForLabResults().
    /// </summary>
    public static void DeleteForLab(long ehrLabNum)
    {
        var command = "DELETE FROM ehrlabresult WHERE EhrLabNum = " + SOut.Long(ehrLabNum);
        Db.NonQ(command);
    }
    ...
    /// <summary>
    ///     Helper function to return a list of descriptions for the HL70085 enumeration.  First item in the list is
    ///     blank.
    /// </summary>
    ...
    /// <summary>
    ///     Helper function to return a list of descriptions for the HL70125 enumeration.  First item in the list is
    ///     blank.
    /// </summary>
    ...
    /// <summary>
    ///     Helper function to return a list of descriptions for the HL70190 enumeration.  First item in the list is
    ///     blank.
    /// </summary>
    ...
    /// <summary>
    ///     Helper function to return a list of descriptions for the HL70200 enumeration.  First item in the list is
    ///     blank.
    /// </summary>
    ...
    /// <summary>
    ///     Helper function to return a list of descriptions for the USPSAlphaStateCode enumeration.  First item in the
    ///     list is blank.
    /// </summary>
