...
public class Tasks
{
    ...
    private static List<Task> TableToList(DataTable table)
    {
        ...
        {
            if (table.Columns.Contains("IsUnread")) listTasks[i].IsUnread = SIn.Bool(table.Rows[i]["IsUnread"].ToString()); //1 or more will result in true.
            if (table.Columns.Contains("ParentDesc")) listTasks[i].ParentDesc = SIn.String(table.Rows[i]["ParentDesc"].ToString());
            if (table.Columns.Contains("LName")
                && table.Columns.Contains("FName")
                && table.Columns.Contains("Preferred"))
            {
                var lname = SIn.String(table.Rows[i]["LName"].ToString());
                var fname = SIn.String(table.Rows[i]["FName"].ToString());
                var preferred = SIn.String(table.Rows[i]["Preferred"].ToString());
        ...
        bool doRunOnReportServer)
    {
        ...
        for (var i = 0; i < listTasks.Count; i++)
        {
            ...
            txt = "";
            if (!listTasks[i].Descript.StartsWith("==") && listTasks[i].UserNum != 0) txt += Userods.GetName(SIn.Long(listTasks[i].UserNum.ToString())) + " - ";
            ...
            if (doIncludeTaskNote)
                listTaskNotes.FindAll(x => x.TaskNum == listTasks[i].TaskNum)
                    .ForEach(x => txt += "\r\n" //even on the first loop
                                         ...
                                         + " - " + x.Note);
            row["note"] = txt;
            if (listTasks[i].ObjectType == TaskObjectType.Patient) row["PatNum"] = listTasks[i].KeyNum;
            ...
            else if (listTasks[i].DateTimeOriginal.Year > 1880) row["dateCreate"] = listTasks[i].DateTimeOriginal.ToShortDateString();
            ...
            else if (listTasks[i].DateTimeEntry.TimeOfDay != TimeSpan.Zero) row["procTime"] = listTasks[i].DateTimeEntry.ToString("h:mm") + listTasks[i].DateTimeEntry.ToString("%t").ToLower();
            if (listTasks[i].DateTimeFinished.Year > 1880) row["dateComplete"] = listTasks[i].DateTimeFinished.ToString(Lans.GetShortDateTimeFormat());
        ...
        long patNum, bool doIncludeTaskNote, bool doIncludeCompleted, bool doIncludeAttachments, bool reachedLimit)
    {
        ...
        {
            listWhereClauses.Add("task.TaskStatus!=" + SOut.Long((int) TaskStatusEnum.Done));
            listWhereNoteClauses.Add("task.TaskStatus!=" + SOut.Long((int) TaskStatusEnum.Done));
            doJoinTaskOnTaskNote = true;
        }

        if (userNum != 0)
        {
            listWhereClauses.Add("task.UserNum=" + SOut.Long(userNum));
            listWhereNoteClauses.Add("tasknote.UserNum=" + SOut.Long(userNum));
        }
        ...
        {
            listWhereClauses.Add("task.TaskNum IN (" + string.Join(",", listTaskNums.Select(x => SOut.Long(x))) + ")");
            listWhereNoteClauses.Add("tasknote.TaskNum IN (" + string.Join(",", listTaskNums.Select(x => SOut.Long(x))) + ")");
        }
        ...
        {
            listWhereClauses.Add("DATE(task.DateTimeOriginal)>=" + SOut.Date(dateCreatedFrom));
            listWhereNoteClauses.Add("DATE(tasknote.DateTimeNote)>=" + SOut.Date(dateCreatedFrom));
        }

        if (dateCreatedTo > DateTime.MinValue)
        {
            listWhereClauses.Add("DATE(task.DateTimeOriginal)<=" + SOut.Date(dateCreatedTo));
            listWhereNoteClauses.Add("DATE(tasknote.DateTimeNote)<=" + SOut.Date(dateCreatedTo));
        }

        if (dateCompletedFrom > DateTime.MinValue)
        {
            listWhereClauses.Add("DATE(task.DateTimeFinished)>=" + SOut.Date(dateCompletedFrom));
            listWhereNoteClauses.Add("DATE(task.DateTimeFinished)>=" + SOut.Date(dateCompletedFrom));
            doJoinTaskOnTaskNote = true;
        }

        if (dateCompletedTo > DateTime.MinValue)
        {
            listWhereClauses.Add("DATE(task.DateTimeFinished)<=" + SOut.Date(dateCompletedTo));
            listWhereNoteClauses.Add("DATE(task.DateTimeFinished)<=" + SOut.Date(dateCompletedTo));
            doJoinTaskOnTaskNote = true;
        }

        var doIncludeKeywordSearch = false;
        if (taskIncluding != "")
        {
            doIncludeKeywordSearch = true;
            var listSearchesIncluding = TaskQuoteHelper(taskIncluding);
            for (var i = 0; i < listSearchesIncluding.Count; i++)
            {
                if (doIncludeAttachments)
                    listWhereClauses.Add("(task.Descript LIKE '%" + SOut.String(listSearchesIncluding[i]) + "%' OR taskattachment.Description LIKE '%" + SOut.String(listSearchesIncluding[i]) + "%' "
                                         + "OR taskattachment.TextValue LIKE '%" + SOut.String(listSearchesIncluding[i]) + "%')");
                else
                    listWhereClauses.Add("task.Descript LIKE '%" + SOut.String(listSearchesIncluding[i]) + "%'");
                listWhereNoteClauses.Add("tasknote.Note LIKE '%" + SOut.String(listSearchesIncluding[i]) + "%'");
            }
        }

        if (taskPriorityNum != 0)
        {
            listWhereClauses.Add("task.PriorityDefNum=" + SOut.Long(taskPriorityNum));
            listWhereNoteClauses.Add("task.PriorityDefNum=" + SOut.Long(taskPriorityNum));
            doJoinTaskOnTaskNote = true;
        }
        ...
        if (patNum != 0)
        {
            ...
            listWhereClauses.Add("task.KeyNum=" + SOut.Long(patNum));
            listWhereNoteClauses.Add("task.ObjectType=" + SOut.Int((int) TaskObjectType.Patient));
            listWhereNoteClauses.Add("task.KeyNum=" + SOut.Long(patNum));
            doJoinTaskOnTaskNote = true;
        }
        ...
        var noteJoinClause = "";
        if (doJoinTaskOnTaskNote) noteJoinClause = "INNER JOIN task ON tasknote.TaskNum=task.TaskNum ";
        var whereNoteClause = "";
        if (listWhereNoteClauses.Count > 0 && doIncludeTaskNote) whereNoteClause = "WHERE " + string.Join(" AND ", listWhereNoteClauses) + " ";
        //Here be query assembly:
        var doExcludeKeywordSearch = false;
        var command = "SELECT TaskNum FROM (";
        ...
        if (taskExcluding != "")
        {
            var listSearchesExcluding = TaskQuoteHelper(taskExcluding);
            doExcludeKeywordSearch = true;
            command += "SELECT task.TaskNum, task.DateTimeOriginal DateTimeSearch "
                       + "FROM task "
                       + "WHERE task.Descript NOT LIKE '%" + SOut.String(listSearchesExcluding[0]) + "%' ";
            ...
            if (!doIncludeCompleted) command += "AND task.TaskStatus!=" + SOut.Long((int) TaskStatusEnum.Done) + " ";
            if (doIncludeTaskNote)
            {
                command += "AND task.TaskNum "
                           + "NOT IN (SELECT tasknote.TaskNum FROM tasknote "
                           + "WHERE tasknote.Note LIKE '%" + SOut.String(listSearchesExcluding[0]) + "%' ";
                if (listSearchesExcluding.Count > 1)
                    for (var i = 1; i < listSearchesExcluding.Count; i++)
                        command += "OR tasknote.Note LIKE '%" + SOut.String(listSearchesExcluding[i]) + "%' ";
            ...
            if (doIncludeAttachments)
            {
                command += "AND task.TaskNum "
                           ...
                           + "WHERE taskattachment.Description LIKE '%" + SOut.String(listSearchesExcluding[0]) + "%' "
                           + "OR taskattachment.TextValue LIKE '%" + SOut.String(listSearchesExcluding[0]) + "%' ";
                if (listSearchesExcluding.Count > 1)
                    for (var i = 1; i < listSearchesExcluding.Count; i++)
                        command += "OR taskattachment.Description LIKE '%" + SOut.String(listSearchesExcluding[i]) + "%' "
                                   + "OR taskattachment.TextValue LIKE '%" + SOut.String(listSearchesExcluding[i]) + "%' ";

                command += ") ";
            }

            if (doIncludeKeywordSearch) command += "AND task.TaskNum IN ";
        }
        ...
        if ((doIncludeKeywordSearch && doExcludeKeywordSearch) //User supplied both include and exclude keywords.
            || (listWhereClauses.Count > 0 && doExcludeKeywordSearch))
        {
            //User filtered by patNum, userNum, date range, etc. and supplied exclude keyword(s).
            if (!doIncludeKeywordSearch) command += "AND task.TaskNum IN ";
            command += "(SELECT task.TaskNum "
                       + "FROM task ";
            if (doIncludeAttachments) command += "LEFT JOIN taskattachment ON task.TaskNum=taskattachment.TaskNum ";
            command += whereClause;
            if (doIncludeTaskNote)
                command += " UNION "
        ...
        else if (doExcludeKeywordSearch)
        {
            //User supplied only exclude keyword(s).
            //Do nothing. No need to build include query.
        }
        else
        {
            //Existing behavior. User supplied no keyword, or only include keyword(s), and/or filtered by patNum, userNum, date range, etc.
            command += "(SELECT task.TaskNum,task.DateTimeOriginal DateTimeSearch "
                       + "FROM task ";
            if (doIncludeAttachments) command += "LEFT JOIN taskattachment ON task.TaskNum=taskattachment.TaskNum ";
            command += whereClause
        ...
        if (reachedLimit) command += " LIMIT 50";
    ...
    private static List<string> TaskQuoteHelper(string taskSearch)
    {
        ...
        var regex = new Regex(@"(?<="")[^""]*(?="")|[^"" ]+");
        var listTaskSearches = regex.Matches(taskSearch)
            .Cast<Match>()
            .Select(x => x.Value)
        ...
        for (var i = 0; i < listTaskSearches.Count; i++)
        {
            if (char.IsWhiteSpace(listTaskSearches[i], 0))
            {
                //0th position is whitespace.
                var listSubstrings = listTaskSearches[i].Trim().Split(" ", StringSplitOptions.RemoveEmptyEntries).ToList();
    ...
    public static List<Task> GetMany(List<long> listTaskNums)
    {
        ...
        var command = "SELECT * FROM task WHERE TaskNum IN(" + string.Join(",", listTaskNums) + ") ORDER BY DateTimeEntry";
    ...
    public static List<Task> GetNewTasksThisUser(long userNum, long clinicNum, List<long> listTaskNums = null)
    {
        ...
        var command = "SELECT task.*,CASE WHEN(taskunread.TaskNum IS NOT NULL) THEN 1 ELSE 0 END IsUnread "
                      + "FROM taskancestor "
        ...
        command += "INNER JOIN tasklist ON tasklist.TaskListNum=taskancestor.TaskListNum "
                   + "INNER JOIN tasksubscription ON tasksubscription.TaskListNum=tasklist.TaskListNum AND tasksubscription.UserNum=" + SOut.Long(userNum) + " "
                   + "LEFT JOIN taskunread ON taskunread.TaskNum=task.TaskNum AND taskunread.UserNum=" + SOut.Long(userNum);
        if (Clinics.ClinicNum != 0 || !true)
        {
            command += TaskLists.BuildFilterJoins(clinicNum);
            command += " WHERE TRUE " + TaskLists.BuildFilterWhereClause(userNum, clinicNum, Clinics.GetClinic(clinicNum)?.RegionId ?? 0);
        }
    ...
    public static Dictionary<long, string> GetApptObjDescripts(List<long> listPatApts)
    {
        ...
        var command = @"SELECT patient.LName,patient.FName,patient.Preferred,patient.MiddleI,appointment.AptNum,appointment.AptDateTime,
        ...
        {
            var patName = Patients.GetNameLF(table.Rows[i]["LName"].ToString(), table.Rows[i]["FName"].ToString(),
                table.Rows[i]["Preferred"].ToString(), table.Rows[i]["MiddleI"].ToString()); //no call to db
            var appointment = new Appointment();
            appointment.AptNum = SIn.Long(table.Rows[i]["AptNum"].ToString());
            appointment.AptDateTime = SIn.DateTime(table.Rows[i]["AptDateTime"].ToString());
            var dateTimeStr = appointment.AptDateTime.ToString();
            if (SIn.Enum<ApptStatus>(table.Rows[i]["AptStatus"].ToString()) == ApptStatus.UnschedList) dateTimeStr = Lans.g("Tasks", "Unscheduled");
            appointment.ProcDescript = SIn.String(table.Rows[i]["ProcDescript"].ToString());
            appointment.Note = SIn.String(table.Rows[i]["Note"].ToString());
    ...
    public static void SetReminderGroupId(Task task)
    {
        ...
        while (true)
        {
            ...
            //query:
            if (GetCountReminderTasks(task.ReminderGroupId, DateTime.MinValue) == 0) break;
            task.ReminderGroupId = MiscUtils.CreateRandomAlphaNumericString(20);
        }
    }
        ...
        List<long> listClinicNumsFilter = null, List<long> listDefNumsRegionFilter = null)
    {
        ...
        var command = "SELECT task.*,"
                      + "(SELECT COUNT(*) FROM taskunread WHERE task.TaskNum=taskunread.TaskNum AND taskunread.UserNum=" + SOut.Long(userNum) + ") IsUnread, "
        ...
        command += "WHERE TaskListNum=0 "
                   + "AND DateTask < " + SOut.Date(new DateTime(1880, 01, 01)) + " "
                   + "AND IsRepeating=0 ";
        ...
        //No filter.
        if (showDone)
            command += " AND (TaskStatus !=" + SOut.Long((int) TaskStatusEnum.Done)
    ...
    public static List<Task> RefreshUserNew(long userNum, List<long> listClinicNumsFilter = null, List<long> listDefNumsRegionFilter = null)
    {
        ...
        command += "tasklist.Descript ParentDesc, " /*Renamed to keep same column name as old query*/
                   ...
                   + "AND taskunread.UserNum = " + SOut.Long(userNum) + " "
        ...
        for (var i = 0; i < table.Rows.Count; i++) listDataRows.Add(table.Rows[i]);

        #region Set Sort Variables. This greatly increases sort speed.

        var listDefsTaskPriorities = Defs.GetDefsForCategory(DefCat.TaskPriorities, true);
        for (var i = 0; i < listDefsTaskPriorities.Count; i++)
        ...
        var listDefs = Defs.GetDefsForCategory(DefCat.TaskPriorities);
        var listTaskCompareObjs = new List<TaskCompareObj>();
        for (var i = 0; i < listDataRows.Count; i++)
        {
            var taskCompareObj = new TaskCompareObj();
        ...
        var tableSorted = table.Clone(); //Easy way to copy the columns.
        tableSorted.Rows.Clear();
        for (var i = 0; i < listTaskCompareObjs.Count; i++) tableSorted.Rows.Add(listTaskCompareObjs[i].DataRowTask.ItemArray);
        var listTasks = TableToList(tableSorted);
        return listTasks;
    }
    ...
    public static List<Task> RefreshOpenTickets(long userNum, List<long> listClinicNumsFilter = null, List<long> listDefNumsRegionFilter = null)
    {
        var command = "SELECT task.*, "
                      ...
                      + "AND taskunread.UserNum=" + SOut.Long(userNum) + ") AS IsUnread, "
    ...
    public static List<Task> RefreshPatientTickets(long patNum, long userNum = 0, List<long> listClinicNumsFilter = null, List<long> listDefNumsRegionFilter = null)
    {
        var command = "SELECT task.*, "
                      + "(SELECT COUNT(*) FROM taskunread WHERE task.TaskNum=taskunread.TaskNum "
                      + "AND taskunread.UserNum=" + SOut.Long(Security.CurUser.UserNum) + ") AS IsUnread, "
                      + "tasklist.Descript AS ParentDesc "
                      + "FROM task "
                      + "LEFT JOIN tasklist ON task.TaskListNum=tasklist.TaskListNum ";
        ...
        command += "WHERE task.ObjectType=" + SOut.Int((int) TaskObjectType.Patient) + " "
                   + "AND task.KeyNum=" + SOut.Long(patNum) + " "
                   + "AND TaskStatus!=" + SOut.Int((int) TaskStatusEnum.Done) + " ";
        command += BuildFilterWhereClause(userNum, listClinicNumsFilter, listDefNumsRegionFilter);
        command += "ORDER BY DateTimeEntry";
        var table = DataCore.GetTable(command);
        ...
        DateTime dateStartFilter = new(), DateTime dateEndFilter = new(), Patient patientFilter = null)
    {
        ...
        if (userNumInbox > 0)
            ...
            command += "AND taskunread.UserNum=" + SOut.Long(userNumInbox) + ") IsUnread, ";
        else
            //otherwise, restrict by current user
            command += "AND taskunread.UserNum=" + SOut.Long(userNum) + ") IsUnread, ";
        ...
        if (isTaskSortApptDateTime)
        {
            var taskListParent = TaskLists.GetOne(listNum);
    ...
    public static List<Task> RefreshRepeating(TaskDateType taskDataType, long userNum, List<long> listClinicNumsFilter = null, List<long> listDefNumsRegionFilter = null)
    {
        var command =
            "SELECT task.*, "
            + "(SELECT COUNT(*) FROM taskunread WHERE task.TaskNum=taskunread.TaskNum "
            + "AND taskunread.UserNum=" + SOut.Long(userNum) + ") IsUnread, " //Not sure if this makes sense here
            + "patient.LName,patient.FName,patient.Preferred "
            + "FROM task "
            + "LEFT JOIN patient ON task.KeyNum=patient.PatNum AND task.ObjectType=" + SOut.Int((int) TaskObjectType.Patient) + " ";
        command += BuildFilterJoins(true, listClinicNumsFilter, listDefNumsRegionFilter);
        command += "WHERE IsRepeating=1 "
                   + "AND COALESCE(task.ReminderGroupId,'')='' " //no reminders
                   + "AND DateType=" + SOut.Long((int) taskDataType) + " ";
        command += BuildFilterWhereClause(userNum, listClinicNumsFilter, listDefNumsRegionFilter);
        command += "ORDER BY DateTimeEntry";
        var table = DataCore.GetTable(command);
        return TableToList(table);
        ...
        List<long> listDefNumsRegionFilter = null, DateTime dateStartFilter = new(), DateTime dateEndFilter = new(), Patient patientFilter = null)
    {
        ...
        if ((EnumTaskFilterType) PrefC.GetInt(PrefName.TasksGlobalFilterType) == EnumTaskFilterType.Disabled || !true)
        ...
        if (!hasPatientJoinAlready) command += " LEFT JOIN patient ON task.ObjectType=" + SOut.Int((int) TaskObjectType.Patient) + " AND task.KeyNum=patient.PatNum ";
        command += " LEFT JOIN appointment ON task.ObjectType=" + SOut.Int((int) TaskObjectType.Appointment) + " AND task.KeyNum=appointment.AptNum ";
        return command;
    }
        ...
        List<long> listDefNumsRegionFilter = null, DateTime dateStartFilter = new(), DateTime dateEndFilter = new(), Patient patientFilter = null)
    {
        if (currentUserNum == 0) //The currentUserNum will be zero when merging patients; cannot build the filter without a valid patnum
            return "";
        if (listClinicNumsFilter == null) listClinicNumsFilter = new List<long>();
        if (listDefNumsRegionFilter == null) listDefNumsRegionFilter = new List<long>();
        var clinicAnd = "";
        var dateRangeAnd = "";
        var patientAnd = "";
        ...
        if ((EnumTaskFilterType) PrefC.GetInt(PrefName.TasksGlobalFilterType) == EnumTaskFilterType.Disabled || !true) clinicAnd = "";
        var listClinicsUnrestricted = Clinics.GetAllForUserod(Userods.GetUser(currentUserNum));
        ...
        var listClinicNumsForQuery = new List<long> {0}; //if filtering by regions, this list will include all clinics within filtered regions
        var isHQClinic = true;
        var isHQRegion = true;
        if (listClinicNumsFilter.Count != 1 || !listClinicNumsFilter.Contains(0)) isHQClinic = false;
        if (listDefNumsRegionFilter.Count != 1 || !listDefNumsRegionFilter.Contains(0)) isHQRegion = false;
        ...
        if (listClinicNumsForQuery.Count > 1)
        {
            ...
            var strFkeys = string.Join(",", listClinicNumsForQuery.Select(x => SOut.Long(x)));
        ...
        if (dateStartFilter != DateTime.MinValue || dateEndFilter != DateTime.MinValue)
        {
            ...
            dateRangeAnd = " AND (appointment.AptDateTime BETWEEN" + SOut.Date(dateStartFilter) + " AND " + SOut.Date(dateEndFilter) + " "
                           + "OR (appointment.AptDateTime IS NULL AND task.DateTimeEntry BETWEEN" + SOut.Date(dateStartFilter) + " AND " + SOut.Date(dateEndFilter) + ")) ";
        }

        if (patientFilter != null) patientAnd = " AND (patient.PatNum=" + patientFilter.PatNum + " OR appointment.PatNum=" + patientFilter.PatNum + ") ";
    ...
    public static void Update(Task task, Task taskOld)
    {
        ...
        Update(task);
        if (task.TaskListNum == taskOld.TaskListNum) return;
    ...
    public static Task CopyReminderToNextDueDate(Task taskReminder)
    {
        ...
        var newTaskNum = Insert(taskNext);
    ...
    public static DateTime CalcTaskForwardDate(Task task)
    {
        ...
        if (task.DateTimeEntry.Date > DateTime.Today) dateMin = task.DateTimeEntry.Date;
        if (task.ReminderType.HasFlag(TaskReminderType.Daily))
            //Find the first day in the schedule which is also in the future.
            while (true)
            {
                if (task.DateTimeEntry.Date > dateMin) break;
            ...
            while (true)
            {
                if (task.DateTimeEntry.Date > dateMin && IsWeekDayFound(task.DateTimeEntry, task.ReminderType)) break;
            ...
            {
                if (task.DateTimeEntry.Date > dateMin) break;
                var dateTimeMonthStart = new DateTime(task.DateTimeEntry.Year, task.DateTimeEntry.Month, 1);
                var dateTimeMonthNext = dateTimeMonthStart.AddMonths(task.ReminderFrequency);
                var dayNext = Math.Min(task.DateTimeEntry.Day, DateTime.DaysInMonth(dateTimeMonthNext.Year, dateTimeMonthNext.Month));
            ...
            while (true)
            {
                ...
                var dateTimeYearMonthStart = new DateTime(task.DateTimeEntry.Year, task.DateTimeEntry.Month, 1);
                var dateTimeYearMonthNext = dateTimeYearMonthStart.AddYears(task.ReminderFrequency);
                var dayNext = Math.Min(task.DateTimeEntry.Day, DateTime.DaysInMonth(dateTimeYearMonthNext.Year, dateTimeYearMonthNext.Month));
    ...
    private static bool IsWeekDayFound(DateTime dateTimeEntry, TaskReminderType taskReminderType)
    {
        ...
        if (dateTimeEntry.Date.DayOfWeek == DayOfWeek.Tuesday && taskReminderType.HasFlag(TaskReminderType.Tuesday)) return true;
        if (dateTimeEntry.Date.DayOfWeek == DayOfWeek.Wednesday && taskReminderType.HasFlag(TaskReminderType.Wednesday)) return true;
        if (dateTimeEntry.Date.DayOfWeek == DayOfWeek.Thursday && taskReminderType.HasFlag(TaskReminderType.Thursday)) return true;
        if (dateTimeEntry.Date.DayOfWeek == DayOfWeek.Friday && taskReminderType.HasFlag(TaskReminderType.Friday)) return true;
        if (dateTimeEntry.Date.DayOfWeek == DayOfWeek.Saturday && taskReminderType.HasFlag(TaskReminderType.Saturday)) return true;
        if (dateTimeEntry.Date.DayOfWeek == DayOfWeek.Sunday && taskReminderType.HasFlag(TaskReminderType.Sunday)) return true;
        return false;
    }
    ...
    public static void Validate(Task task, Task taskOld)
    {
        ...
        if (WasTaskAltered(taskOld)) throw new Exception(Lans.g("Tasks", "Not allowed to save changes because the task has been altered by someone else."));
        if (task.IsNew)
        {
        ...
        {
            if (task.TaskStatus == TaskStatusEnum.Done) TaskEditCreateLog(Lans.g("Tasks", "Task marked done"), task);
            if (task.TaskStatus == TaskStatusEnum.New) TaskEditCreateLog(Lans.g("Tasks", "Task marked new"), task);
            //Nothing for case when Not New and Not Done. Put here in future is wanted
        }

        if (task.Descript != taskOld.Descript) TaskEditCreateLog(Lans.g("Tasks", "Task description edited"), task);
        if (task.UserNum != taskOld.UserNum) TaskEditCreateLog(Lans.g("Tasks", "Changed user from") + " " + Userods.GetName(taskOld.UserNum), task); //+" To "+Userods.GetName(task.UserNum)),task);
        if (task.KeyNum != taskOld.KeyNum)
        {
            ...
            if (taskOld.KeyNum > 0)
            {
                ...
                if (taskOld.ObjectType == TaskObjectType.Patient)
                {
                    //It was a patient
                ...
                else
                {
                    ...
                    var appointmentOld = Appointments.GetOneApt(taskOld.KeyNum);
        ...
        {
            var taskListOld = TaskLists.GetOne(taskOld.TaskListNum) ?? new TaskList {Descript = "<TaskListNum:" + taskOld.TaskListNum + ">"};
    ...
    public static void Delete(long taskNum)
    {
        ...
        var command = "DELETE FROM task WHERE TaskNum = " + SOut.Long(taskNum);
        Db.NonQ(command);
        command = "DELETE FROM taskancestor WHERE TaskNum = " + SOut.Long(taskNum);
        Db.NonQ(command);
        command = "DELETE FROM tasknote WHERE TaskNum = " + SOut.Long(taskNum);
        Db.NonQ(command);
        command = "DELETE FROM taskattachment WHERE TaskNum = " + SOut.Long(taskNum);
        Db.NonQ(command);
        command = "DELETE FROM taskunread WHERE TaskNum = " + SOut.Long(taskNum);
        Db.NonQ(command);
    }
    ...
    public static void TaskEditCreateLog(EnumPermType permissions, string logText, Task task)
    {
        ...
        if (task.KeyNum != 0)
        {
            ...
            else if (task.ObjectType == TaskObjectType.Appointment)
            {
                ...
                var appointment = Appointments.GetOneApt(task.KeyNum);
    ...
    public static int TaskComparer(TaskCompareObj taskCompareObjX, TaskCompareObj taskCompareObjY)
    {
        ...
        {
            var isObjectTypeAptx = SIn.Int(taskCompareObjX.DataRowTask["ObjectType"].ToString()) == (int) TaskObjectType.Appointment;
            var isObjectTypeAptY = SIn.Int(taskCompareObjY.DataRowTask["ObjectType"].ToString()) == (int) TaskObjectType.Appointment;
        ...
        if (PrefC.GetBool(PrefName.TasksNewTrackedByUser))
            if (taskCompareObjX.DataRowTask["IsUnread"].ToString() != taskCompareObjY.DataRowTask["IsUnread"].ToString())
                ...
                return -SIn.Long(taskCompareObjX.DataRowTask["IsUnread"].ToString()).CompareTo(SIn.Long(taskCompareObjY.DataRowTask["IsUnread"].ToString())); //sort unread to top.

        //2)Sort by Task Priority
        if (taskCompareObjX.DataRowTask["PriorityDefNum"].ToString() != taskCompareObjY.DataRowTask["PriorityDefNum"].ToString())
        {
            //we only care about task priority if they are different
            var xTaskPriorityDefNum = SIn.Long(taskCompareObjX.DataRowTask["PriorityDefNum"].ToString());
            var yTaskPriorityDefNum = SIn.Long(taskCompareObjY.DataRowTask["PriorityDefNum"].ToString());
            //0 will always be considered like the default task priority.
            if (xTaskPriorityDefNum == 0) xTaskPriorityDefNum = _defaultTaskPriorityDefNum;
            if (yTaskPriorityDefNum == 0) yTaskPriorityDefNum = _defaultTaskPriorityDefNum;
    ...
    public static int CompareTimes(DataRow dataRowX, DataRow dataRowY)
    {
        ...
        var dateTimeXMax = SIn.DateTime(dataRowX["DateTimeEntry"].ToString());
        var dateTimeYMax = SIn.DateTime(dataRowY["DateTimeEntry"].ToString());
    ...
    public static int CompareAptDateTimes(DataRow dataRowX, DataRow dataRowY)
    {
        ...
        var dateAptY = SIn.DateTime(dataRowY["AptDateTime"].ToString());
        if (dateAptX == null
            || (ApptStatus) SIn.Int(dataRowX["AptStatus"].ToString()) == ApptStatus.UnschedList)
            dateAptX = DateTime.MaxValue;
        if (dateAptY == null
            || (ApptStatus) SIn.Int(dataRowY["AptStatus"].ToString()) == ApptStatus.UnschedList)
    ...
    public static bool IsAuthorizedOrOwner(Task task)
    {
        ...
        if (task.UserNum != Security.CurUser.UserNum) return false;
        //If the task is not in the logged-in user's inbox.
        if (task.TaskListNum != Security.CurUser.TaskListInBox && task.TaskListNum != -1) return false;
        //Check to see if other users have added notes.
        var listTaskNotes = TaskNotes.GetForTask(task.TaskNum);
