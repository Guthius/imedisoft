...
/// <summary>
///     This class will likely never be used.  The famaging table is used to store intermediate calculations for aging and
///     once the patient
///     table is updated the data is never accessed again.  A new aging calculation begins with truncating this table.  All
///     edit commands, i.e. truncate,
///     insert, etc., take place in the queries in Ledgers.ComputeAging.
/// </summary>
public class FamAgings
{
    ...
    /// <summary>
    ///     Inserts many into the FamAging table.  Uses the existing pri key since these are basically copies of the patient
    ///     table data and should
    ///     always be inserted using the patient.PatNum as the pri key value in order to join on the patient table for update
    ///     later.
    /// </summary>
    public static void InsertMany(List<FamAging> listFamAgings)
    {
        FamAgingCrud.InsertMany(listFamAgings, true); //true to use existing pri key so these will match the patient.PatNums for joining later
    }
