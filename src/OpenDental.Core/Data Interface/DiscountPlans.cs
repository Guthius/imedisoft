...
public class DiscountPlans
{
    ...
    {
        DiscountPlanCrud.Insert(discountPlan);
    }

    public static List<DiscountPlan> GetAll(bool includeHidden)
    {
        var commandText = "SELECT * FROM discountplan";
        if (!includeHidden)
        {
            commandText += " WHERE IsHidden=0";
        }
        
        return DiscountPlanCrud.SelectMany(commandText);
    }

    public static List<DiscountPlan> GetDiscountPlansByPlanNum(List<long> discountPlanNums)
    {
        if (discountPlanNums.Count == 0) return [];

        var command = "SELECT * FROM discountplan WHERE discountplan.DiscountPlanNum IN (" + string.Join(",", discountPlanNums) + ")";
    ...
    public static int GetPatCountForPlan(long discountPlanNum)
    {
        ...
        return SIn.Int(Db.GetCount(
            "SELECT COUNT(discountplansub.PatNum) " + 
            "FROM discountplansub " + 
            "LEFT JOIN patient ON discountplansub.PatNum=patient.PatNum " + 
            "WHERE discountplansub.DiscountPlanNum=" + discountPlanNum + " " + 
            "AND patient.PatStatus NOT IN (" + (int) PatientStatus.Deleted + "," + (int) PatientStatus.Deceased + ")"));
    }
    ...
    public static string CheckDiscountFrequencyAndValidateDiscountPlanSub(List<Procedure> listProcedures, long patNum, DateTime aptDateTime, DiscountPlanSub discountPlanSub = null, List<Procedure> listProceduresAddHist = null)
    {
    ...
    public static string CheckDiscountFrequency(List<Procedure> listProcedures, long patNum, DateTime aptDateTime, DateTime dateEffective, DateTime dateTerm, DiscountPlan discountPlan, List<Procedure> listProceduresAddHist = null)
    {
    ...
    public static string HasMetDiscountFrequencyLimitation(Procedure procedure, DiscountPlan discountPlan, DateTime dateEffective, DateTime dateTerm, List<Procedure> listProceduresHist, List<ProcedureCode> listExamCodes, List<ProcedureCode> listXrayCodes, List<ProcedureCode> listProphyCodes, List<ProcedureCode> listFluorideCodes, List<ProcedureCode> listPerioCodes, List<ProcedureCode> listLimitedExamCodes, List<ProcedureCode> listPACodes)
    {
