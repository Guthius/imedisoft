...
public class Benefits
{
    ...
    public static double GetAnnualMaxDisplay(List<Benefit> listBenefits, long planNum, long patPlanNum, bool isFam)
    {
        ...
        for (var i = 0; i < listBenefits.Count; i++)
        {
            ...
            if (listBenefits[i].PatPlanNum == 0 && listBenefits[i].PlanNum != planNum) continue;
            if (listBenefits[i].BenefitType != InsBenefitType.Limitations) continue;
            if (listBenefits[i].QuantityQualifier != BenefitQuantity.None) continue;
            if (listBenefits[i].TimePeriod != BenefitTimePeriod.CalendarYear && listBenefits[i].TimePeriod != BenefitTimePeriod.ServiceYear) continue;
            if (isFam)
            {
            ...
            //coverage level?
            if (listBenefits[i].CodeNum != 0) continue;
            if (listBenefits[i].CovCatNum != 0)
            {
                var eBenefitCategory = CovCats.GetEbenCat(listBenefits[i].CovCatNum);
                if (eBenefitCategory != EbenefitCategory.General && eBenefitCategory != EbenefitCategory.None) continue;
            }

            listBenefitsMatching.Add(listBenefits[i]);
        }

        if (listBenefitsMatching.Count == 0) return -1;
    ...
    public static double GetDeductGeneralDisplay(List<Benefit> listBenefits, long planNum, long patPlanNum, BenefitCoverageLevel benefitCoverageLevel)
    {
        ...
        for (var i = 0; i < listBenefits.Count; i++)
        {
            if (listBenefits[i].PlanNum == 0 && listBenefits[i].PatPlanNum != patPlanNum) continue;
            if (listBenefits[i].PatPlanNum == 0 && listBenefits[i].PlanNum != planNum) continue;
            if (listBenefits[i].BenefitType != InsBenefitType.Deductible) continue;
            if (listBenefits[i].QuantityQualifier != BenefitQuantity.None) continue;
            if (listBenefits[i].TimePeriod != BenefitTimePeriod.CalendarYear && listBenefits[i].TimePeriod != BenefitTimePeriod.ServiceYear) continue;
            if (listBenefits[i].CoverageLevel != benefitCoverageLevel) continue;
            if (listBenefits[i].CodeNum != 0) continue;
            if (listBenefits[i].CodeGroupNum != 0) continue;
            if (listBenefits[i].CovCatNum != 0)
            {
                var eBenefitCategory = CovCats.GetEbenCat(listBenefits[i].CovCatNum);
                if (eBenefitCategory != EbenefitCategory.General && eBenefitCategory != EbenefitCategory.None) continue;
        ...
        List<ClaimProcHist> listClaimProcHists, List<ClaimProcHist> listClaimProcHistsLoop, InsPlan insPlan, long patNum)
    {
        ...
        var listBenefitsShort = new List<Benefit>();
        for (var i = 0; i < listBenefits.Count; i++)
        {
            if (listBenefits[i].PlanNum == 0 && listBenefits[i].PatPlanNum != patPlanNum) continue;
            if (listBenefits[i].PatPlanNum == 0 && listBenefits[i].PlanNum != planNum) continue;
        ...
        for (var i = 0; i < listBenefitsShort.Count; i++)
        {
            if (listBenefitsShort[i].CoverageLevel == BenefitCoverageLevel.Family) continue;
            if (listBenefitsShort[i].CodeNum > 0) continue;
            if (listBenefitsShort[i].CodeGroupNum > 0) continue;
        ...
        for (var i = 0; i < listBenefitsShort.Count; i++)
        {
            ...
            if (listBenefitsShort[i].CovCatNum != 0)
            {
                ...
                var isMatch = false;
                for (var j = 0; j < listCovSpansForCat.Count; j++)
                ...
                if (!isMatch) continue; //no match
                if (benefitInd != null && benefitInd.CovCatNum != 0)
                {
                    //must compare
                    //only use the new one if the item order is larger
                    if (CovCats.GetOrderShort(listBenefitsShort[i].CovCatNum) > CovCats.GetOrderShort(benefitInd.CovCatNum)) benefitInd = listBenefitsShort[i];
                }
                else
                {
        ...
        for (var i = 0; i < listBenefitsShort.Count; i++)
        {
            if (listBenefitsShort[i].CoverageLevel == BenefitCoverageLevel.Family) continue;
            if (listBenefitsShort[i].CodeNum > 0) continue;
            if (listBenefitsShort[i].CovCatNum > 0) continue;
            if (listBenefitsShort[i].CodeGroupNum == 0) continue;
            //See if the benefit's codegroup contains this proccode.
            var codeGroup = CodeGroups.GetOne(listBenefitsShort[i].CodeGroupNum);
            if (!ProcedureCodes.IsCodeInList(procCode, codeGroup.ProcCodes)) continue;
            //If benefits have codegroups w/ overlapping benefits
            if (benefitInd != null && benefitInd.CodeGroupNum != 0)
            {
                //This should never happen, but for the sake of answering "what if," we defer to the highest deductible
                if (listBenefitsShort[i].MonetaryAmt > benefitInd.MonetaryAmt) benefitInd = listBenefitsShort[i];
            }
            else
            {
                benefitInd = listBenefitsShort[i];
        ...
        for (var i = 0; i < listBenefitsShort.Count; i++)
        {
            ...
            if (listBenefitsShort[i].CodeNum == 0) continue;
            if (listBenefitsShort[i].CodeGroupNum > 0) continue;
            if (procCode == ProcedureCodes.GetStringProcCode(listBenefitsShort[i].CodeNum)) benefitInd = listBenefitsShort[i];
        }
        ...
        for (var i = 0; i < listBenefitsShort.Count; i++)
        {
            ...
            if (listBenefitsShort[i].CovCatNum != 0)
            {
                ...
                if (!isMatch) continue; //no match
                if (benefitFam != null && benefitFam.CovCatNum != 0)
                {
                    //must compare
                    //only use the new one if the item order is larger
                    if (CovCats.GetOrderShort(listBenefitsShort[i].CovCatNum) > CovCats.GetOrderShort(benefitFam.CovCatNum)) benefitFam = listBenefitsShort[i];
                }
                else
                {
                    //first one encountered for a category
        ...
        for (var i = 0; i < listBenefitsShort.Count; i++)
        {
            ...
            var codeGroup = CodeGroups.GetOne(listBenefitsShort[i].CodeGroupNum);
            if (!ProcedureCodes.IsCodeInList(procCode, codeGroup.ProcCodes)) continue;
            //if benefits have codegroups with overlapping benefits
            if (benefitFam != null && benefitFam.CodeGroupNum != 0)
            {
                //This should never happen, but for the sake of answering "what if," we defer to the highest deductible
                if (listBenefitsShort[i].MonetaryAmt > benefitFam.MonetaryAmt) benefitFam = listBenefitsShort[i];
            }
            else
            {
                benefitFam = listBenefitsShort[i];
        ...
        for (var i = 0; i < listBenefitsShort.Count; i++)
        {
            ...
            if (listBenefitsShort[i].CodeGroupNum > 0) continue;
            if (listBenefitsShort[i].CodeNum == 0) continue;
            if (procCode == ProcedureCodes.GetStringProcCode(listBenefitsShort[i].CodeNum)) benefitFam = listBenefitsShort[i];
        }
        ...
        if (benefitInd == null || benefitInd.MonetaryAmt == -1 || benefitInd.MonetaryAmt == 0) return 0;
        ...
        var listClaimProcHistsPat = listClaimProcHists.FindAll(x => x.PatNum == patNum);
        var retVal = GetDeductibleRemainingHelper(insPlan, dateProc, benefitInd, benefitIndGeneral, listClaimProcHistsPat);
        ...
        var listCovSpansOther = new List<CovSpan>();
        if (CovCats.GetEbenCat(benefitInd.CovCatNum) == EbenefitCategory.Diagnostic)
            for (var i = 0; i < listBenefitsShort.Count; i++)
            {
                //look through the benefits again
                if (listBenefitsShort[i].CoverageLevel != BenefitCoverageLevel.Individual) continue;
                if (CovCats.GetEbenCat(listBenefitsShort[i].CovCatNum) != EbenefitCategory.RoutinePreventive) continue;
        ...
        if (CovCats.GetEbenCat(benefitInd.CovCatNum) == EbenefitCategory.RoutinePreventive)
            for (var i = 0; i < listBenefitsShort.Count; i++)
            {
                //look through the benefits again
                if (listBenefitsShort[i].CoverageLevel != BenefitCoverageLevel.Individual) continue;
                if (CovCats.GetEbenCat(listBenefitsShort[i].CovCatNum) != EbenefitCategory.Diagnostic) continue;
                listCovSpansOther = CovSpans.GetForCat(listBenefitsShort[i].CovCatNum);
            }

        #endregion

        for (var i = 0; i < listClaimProcHistsLoop.Count; i++)
        {
            #region filter loopList

            if (listClaimProcHistsLoop[i].PlanNum != planNum) continue; //different plan.  Even the loop list can contain info for multiple plans.
            if (listClaimProcHistsLoop[i].PatNum != patNum) continue; //this is for someone else in the family
            ...
            if (benefitInd.CodeNum != 0)
            {
                //specific code
                if (ProcedureCodes.GetStringProcCode(benefitInd.CodeNum) != listClaimProcHistsLoop[i].StrProcCode) continue;
            }
            else if (benefitInd.CodeGroupNum != 0)
            {
                //specific codegroup
                if (!CodeGroups.IsProcInCodeGroup(listClaimProcHistsLoop[i].StrProcCode, benefitInd.CodeGroupNum)) continue;
            }
            else if (benefitInd.CovCatNum != 0)
            {
                //specific category
                ...
                if (listCovSpansOther != null)
                    for (var j = 0; j < listCovSpansOther.Count; j++)
            ...
            if (listClaimProcHistsLoop[i].Deduct == -1) continue;
        ...
        var listClaimProcHistsLoopInsPlan = listClaimProcHistsLoop.FindAll(x => x.PlanNum == planNum && x.PatNum == patNum);
        for (var i = 0; i < listClaimProcHistsLoopInsPlan.Count; i++) deductUsedInLoopList += Math.Max(listClaimProcHistsLoopInsPlan[i].Deduct, 0);
        ...
        if ((benefitFam == null && benefitFamGeneral == null) || (benefitFam?.MonetaryAmt == -1 && benefitFamGeneral?.MonetaryAmt == -1)) return retVal;

        #region calculate the amount family already paid this year

        var dedFam = GetDeductibleRemainingHelper(insPlan, dateProc, benefitFam, benefitFamGeneral, listClaimProcHists);
        ...
        for (var i = 0; i < listClaimProcHistsLoop.Count; i++)
        {
            if (listClaimProcHistsLoop[i].PlanNum != planNum) continue; //different plan
            ...
            if (benefitFam.CodeNum != 0)
            {
                //specific code
                if (ProcedureCodes.GetStringProcCode(benefitFam.CodeNum) != listClaimProcHistsLoop[i].StrProcCode) continue;
            }
            else if (benefitFam.CodeGroupNum != 0)
            {
                //specific codegroup
                if (!CodeGroups.IsProcInCodeGroup(listClaimProcHistsLoop[i].StrProcCode, benefitFam.CodeGroupNum)) continue;
            }
            else if (benefitFam.CovCatNum != 0)
            {
                //specific category
        ...
        if (dedFam <= 0) return 0; //then no deductible, regardless of what we computed for individual
        ...
        List<ClaimProcHist> listClaimProcHists)
    {
        ...
        var dateStart = BenefitLogic.ComputeRenewDate(procDate, insPlan.MonthRenew);
        var dateEnd = dateStart.AddYears(1).AddDays(-1); //Consider all claim procs with a ProcDate within one year of the renew date
        double generalUsed = 0;
        double categoryUsed = 0;
        for (var i = 0; i < listClaimProcHists.Count; i++)
        {
            if (listClaimProcHists[i].PlanNum != insPlan.PlanNum) continue; //different plan
            if (listClaimProcHists[i].ProcDate < dateStart || listClaimProcHists[i].ProcDate > dateEnd) continue;
            if (listClaimProcHists[i].Deduct == -1) continue;
            ...
            {
                var listCovSpansForCat = CovSpans.GetForCat(benefitInd.CovCatNum);
                for (var j = 0; j < listCovSpansForCat.Count; j++)
            ...
            //Payment towards a claimproc's deductible count towards all deductibles with codegroups containing the claimproc's proccode.
            if (benefitInd.CodeGroupNum > 0)
                if (CodeGroups.IsProcInCodeGroup(listClaimProcHists[i].StrProcCode, benefitInd.CodeGroupNum))
                    categoryUsed += listClaimProcHists[i].Deduct;
        ...
        if (benefitInd.CovCatNum > 0 || benefitInd.CodeNum > 0 || benefitInd.CodeGroupNum > 0)
        ...
        double insEstTotal, int patientAge, long insSubNum, double insEstTotalOverride, out LimitationTypeMet limitationTypeMet)
    {
        ...
        for (var i = 0; i < listBenefitsShort.Count; i++)
        {
            ...
            if (listBenefitsShort[i].CovCatNum == 0)
            {
                if (!IsCodeInGeneralSpan(procCodeStr)) continue;
        ...
        for (var i = 0; i < listBenefitsShort.Count; i++)
        {
            ...
            if (listBenefitsShort[i].CovCatNum != 0)
            {
                ...
                if (listBenefitsShort[i].QuantityQualifier == BenefitQuantity.AgeLimit && listBenefitsShort[i].Quantity > 0)
                {
                    ...
                    {
                        note = Lans.g("Benefits", "Age limitation:") + " " + listBenefitsShort[i].Quantity;
        ...
        for (var i = 0; i < listBenefitsShort.Count; i++)
        {
            ...
            if (listBenefitsShort[i].QuantityQualifier == BenefitQuantity.NumberOfServices
                ...
                continue; //exclude frequencies
            var codeGroup = CodeGroups.GetOne(listBenefitsShort[i].CodeGroupNum);
            if (!ProcedureCodes.IsCodeInList(procCodeStr, codeGroup.ProcCodes)) continue;
            //If it's an age based limitation, then make sure the patient age matches.
            //If we have an age match, then we exit the method right here.
            if (listBenefitsShort[i].QuantityQualifier == BenefitQuantity.AgeLimit && listBenefitsShort[i].Quantity > 0)
            {
            ...
            //if benefits have codegroups with overlapping procedures
            if (benefitInd != null && benefitInd.CodeGroupNum > 0)
            {
                //This should never happen, but for the sake of answering "what if," we defer to the lowest limit.
                if (listBenefitsShort[i].MonetaryAmt < benefitInd.MonetaryAmt) benefitInd = listBenefitsShort[i];
            }
            //First codegroup benefit found
            else
            {
                benefitInd = listBenefitsShort[i];
            }
        ...
        for (var i = 0; i < listBenefitsShort.Count; i++)
        {
            ...
            if (listBenefitsShort[i].CodeGroupNum > 0) continue;
            if (listBenefitsShort[i].CodeNum == 0) continue;
            if (procCodeStr != ProcedureCodes.GetStringProcCode(listBenefitsShort[i].CodeNum)) continue;
            if (listBenefitsShort[i].QuantityQualifier == BenefitQuantity.NumberOfServices
                || listBenefitsShort[i].QuantityQualifier == BenefitQuantity.Months
                || listBenefitsShort[i].QuantityQualifier == BenefitQuantity.Years)
                continue; //exclude frequencies
        ...
        var listCodeGroups = CodeGroups.GetDeepCopy(false);
        for (var i = 0; i < listCodeGroups.Count; i++)
        {
            var procCodes = listCodeGroups[i].ProcCodes;
            if (!procCodes.Contains(procCodeStr)) continue;
            var benefitAgeLimit = listBenefitsShort.Find(x => x.CodeGroupNum == listCodeGroups[i].CodeGroupNum && IsAgeLimit(x));
            if (benefitAgeLimit != null && benefitAgeLimit.Quantity > 0 && patientAge > benefitAgeLimit.Quantity)
            {
                note = Lans.g("Benefits", "Age limitation:") + " " + benefitAgeLimit.Quantity;
                limitationTypeMet = LimitationTypeMet.Aging;
                return 0; //not covered if too old.
            }
        }
        ...
        for (var i = 0; i < listBenefitsShort.Count; i++)
        {
            ...
            //benefits have codegroups with overlapping procedures
            if (benefitFam != null && benefitFam.CodeGroupNum > 0)
            {
                //This should never happen, but for the sake of answering "what if," we defer to the lowest limit.
                if (listBenefitsShort[i].MonetaryAmt < benefitFam.MonetaryAmt) benefitFam = listBenefitsShort[i];
        ...
        for (var i = 0; i < listBenefitsShort.Count; i++)
        {
            ...
            if (listBenefitsShort[i].CodeGroupNum > 0) continue;
            if (listBenefitsShort[i].CodeNum == 0) continue;
            if (procCodeStr == ProcedureCodes.GetStringProcCode(listBenefitsShort[i].CodeNum)) benefitFam = listBenefitsShort[i];
        }
        ...
        double maxInd = 0;
        if (benefitInd != null) maxInd = benefitInd.MonetaryAmt;
        ...
        if (benefitInd != null)
        {
            if (benefitInd.TimePeriod == BenefitTimePeriod.Lifetime) dateStart = DateTime.MinValue;
            for (var i = 0; i < listClaimProcHists.Count; i++)
            {
                if (listClaimProcHists[i].InsSubNum != insSubNum) continue; //different plan
                if (listClaimProcHists[i].ProcDate < dateStart || listClaimProcHists[i].ProcDate > dateEnd) continue;
                if (listClaimProcHists[i].PatNum != patNum) continue; //this is for someone else in the family //SHOULD PROBABLY NOT SKIP THIS IN THE CASE OF FAM BUT NO IND MAX. :(
                if (benefitInd.CodeNum != 0)
                {
                    //specific code
                    //Enhance this later when code spans are supported.
                    if (ProcedureCodes.GetStringProcCode(benefitInd.CodeNum) != listClaimProcHists[i].StrProcCode) continue;
                }
                else if (benefitInd.CodeGroupNum != 0)
                {
                    var codeGroup = CodeGroups.GetOne(benefitInd.CodeGroupNum);
                    if (!ProcedureCodes.IsCodeInList(listClaimProcHists[i].StrProcCode, codeGroup.ProcCodes)) continue;
                }
                else if (benefitInd.CovCatNum != 0)
                {
                    //specific category
                    listCovSpansForCat = CovSpans.GetForCat(benefitInd.CovCatNum);
                    var isMatch = false;
                    if (listClaimProcHists[i].StrProcCode == "")
                    {
                ...
                if (TighterLimitExists(listBenefitsShort, benefitInd, listClaimProcHists[i])) continue;
        ...
        if (benefitInd != null)
            for (var i = 0; i < listClaimProcHistsLoop.Count; i++)
            {
                ...
                //	continue;
                //}
                if (listClaimProcHistsLoop[i].InsSubNum != insSubNum) continue; //different plan.  Even the loop list can contain info for multiple plans.
                if (listClaimProcHistsLoop[i].PatNum != patNum) continue; //this is for someone else in the family
                if (benefitInd.CodeNum != 0)
                {
                    //specific code
                ...
                {
                    var codeGroup = CodeGroups.GetOne(benefitInd.CodeGroupNum);
                    if (!ProcedureCodes.IsCodeInList(listClaimProcHistsLoop[i].StrProcCode, codeGroup.ProcCodes)) continue;
                }
                else if (benefitInd.CovCatNum != 0)
                {
                    //specific category
                    listCovSpansForCat = CovSpans.GetForCat(benefitInd.CovCatNum);
                    var isMatch = false;
                    if (listClaimProcHistsLoop[i].StrProcCode == "")
                    {
                        //If this was a 'total' payment that was not attached to a procedure
                        if (CovCats.GetEbenCat(benefitInd.CovCatNum) == EbenefitCategory.General)
                ...
                else
                {
                    ...
                    var covCatGeneral = CovCats.GetForEbenCat(EbenefitCategory.General);
                    if (covCatGeneral != null)
                    {
                        //If there is a general category, then we only consider codes within it.  This is how we exclude ortho.
                        var listCovSpans = CovSpans.GetForCat(covCatGeneral.CovCatNum);
                        if (listClaimProcHistsLoop[i].StrProcCode != "" && !CovSpans.IsCodeInSpans(listClaimProcHistsLoop[i].StrProcCode, listCovSpans))
                            //for example, ortho
                            continue;
                    }
                }
                ...
                //At this point, we know that the proc in the loopList falls within this max benefit.
                //But it may also fall within a more restrictive benefit which would take precedence over this one.
                if (TighterLimitExists(listBenefitsShort, benefitInd, listClaimProcHistsLoop[i])) continue;
                maxInd -= listClaimProcHistsLoop[i].Amount;
            }

        if (benefitInd != null)
            if (benefitInd.MonetaryAmt != -1 && maxInd <= 0)
            {
        ...
        var insRemainingOverride = insEstTotalOverride; //insEstTotalOverride or the amount ind remaining.  Same concept as retVal but never returned.
        if (benefitInd != null && benefitInd.MonetaryAmt != -1)
            if (maxInd < insEstTotalOverride)
                //if there's not enough left in the individual annual max to cover this proc.
        ...
        var maxFam = benefitFam.MonetaryAmt;
        //reduce the family max by amounts already used----------------------------------------------------------
        for (var i = 0; i < listClaimProcHists.Count; i++)
        {
            if (listClaimProcHists[i].ProcDate < dateStart || listClaimProcHists[i].ProcDate > dateEnd) continue;
            if (listClaimProcHists[i].PlanNum != planNum) continue; //different plan
            ...
            {
                //specific code
                if (ProcedureCodes.GetStringProcCode(benefitFam.CodeNum) != listClaimProcHists[i].StrProcCode) continue;
            }
            else if (benefitFam.CodeGroupNum != 0)
            {
                var codeGroup = CodeGroups.GetOne(benefitFam.CodeGroupNum);
                if (!ProcedureCodes.IsCodeInList(listClaimProcHists[i].StrProcCode, codeGroup.ProcCodes)) continue;
            }
            else if (benefitFam.CovCatNum != 0)
            {
            ...
            if (benefitFam.CovCatNum == 0 && !IsCodeInGeneralSpan(listClaimProcHists[i].StrProcCode) && listClaimProcHists[i].StrProcCode != "") continue;
            //if no category, then benefits are not restricted by proc code.
        ...
        for (var i = 0; i < listClaimProcHistsLoop.Count; i++)
        {
            ...
            //don't subtract the estimates from procedures that are outside of the general coverage category 
            //StrProcCode can be blank for claimprocs with Status Adjustment
            if (benefitFam.CovCatNum == 0 && !IsCodeInGeneralSpan(listClaimProcHistsLoop[i].StrProcCode) && listClaimProcHistsLoop[i].StrProcCode != "") continue;
            //if no category, then benefits are not restricted by proc code.
        ...
        if (benefitInd == null || benefitInd.MonetaryAmt == -1 || maxFam < maxInd)
        {
            //restrict by maxFam
            if (insEstTotalOverride == -1)
            {
                //No insEstTotalOverride used, use normal insEstTotal
                if (maxFam < retVal)
                {
            ...
            {
                //If there is an override, calculate family max note from it instead
                //First calculate the note using override
                if (maxFam < insRemainingOverride)
                {
                    //if there's not enough left in the annual max to cover this proc.
                ...
                if (maxFam < retVal) return maxFam;
            }
        }
    ...
    {
        var retVal = true;
        var covCatGen = CovCats.GetForEbenCat(EbenefitCategory.General);
        if (covCatGen != null)
        {
            //Should always happen.  This would mean they are missing the General Coverage Category.
            var listCovSpansForGeneral = CovSpans.GetForCat(covCatGen.CovCatNum);
            var isMatch = false;
            for (var i = 0; i < listCovSpansForGeneral.Count; i++)
    ...
    private static bool TighterLimitExists(List<Benefit> listBenefitsShort, Benefit benefit, ClaimProcHist claimProcHist)
    {
        ...
        if (benefit.CodeNum != 0) //The benefit is already restrictive.  There isn't currently a way to have a more restrictive benefit, although in the future when code ranges are supported, a tighter code range would be more restrictive.
            return false;
        for (var b = 0; b < listBenefitsShort.Count; b++)
        {
            if (listBenefitsShort[b].BenefitNum == benefit.BenefitNum) continue; //skip self.
            if (listBenefitsShort[b].QuantityQualifier != BenefitQuantity.None) continue; //it must be some other kind of limitation other than an amount limit.  For example, BW frequency.
            if (listBenefitsShort[b].CodeNum != 0)
            {
                long codeNum = 0;
                if (ProcedureCodes.GetContainsKey(claimProcHist.StrProcCode)) codeNum = ProcedureCodes.GetOne(claimProcHist.StrProcCode).CodeNum;
            ...
            if (listBenefitsShort[b].CodeGroupNum != 0)
            {
                var codeGroup = CodeGroups.GetOne(listBenefitsShort[b].CodeGroupNum);
                if (!ProcedureCodes.IsCodeInList(claimProcHist.StrProcCode, codeGroup.ProcCodes))
                    //Irrelevant codegroup
                    continue;
                if (benefit.CovCatNum != 0)
                    //Code group is more restrictive than category
                    return true;
                //benefits have codegroups with overlapping procedures
                if (benefit.CodeGroupNum != 0)
                    //This should never happen, but for the sake of answering "what if," we defer to the lowest limit.
                    if (listBenefitsShort[b].MonetaryAmt < benefit.MonetaryAmt)
                        return true;

                continue;
            }

            if (listBenefitsShort[b].CovCatNum == 0) continue;
            //specific category
            if (benefit.CovCatNum != 0)
            {
                ...
                var orderBenefit = CovCats.GetOrderShort(benefit.CovCatNum);
                var orderTest = CovCats.GetOrderShort(listBenefitsShort[b].CovCatNum);
                if (orderBenefit == -1)
                {
                    //nothing to do here.  Treat it like a general limitation 
                }
                else if (orderTest < orderBenefit)
                {
            ...
            var listCovSpansForCat = CovSpans.GetForCat(listBenefitsShort[b].CovCatNum); //get the spans ;
    ...
    public static bool LimitationExistsNotGeneral(List<Benefit> listBenefits, long planNum, long patPlanNum, string strProcCode)
    {
        ...
        for (var i = 0; i < listBenefits.Count; i++)
        {
            ...
            if (listBenefits[i].CovCatNum != 0)
            {
                ...
                if (ebenefitCategory == EbenefitCategory.General || ebenefitCategory == EbenefitCategory.None) continue; //ignore this general benefit
                listCovSpans = CovSpans.GetForCat(listBenefits[i].CovCatNum);
                if (CovSpans.IsCodeInSpans(strProcCode, listCovSpans)) return true; //a limitation benefit exists for a category that contains this procedure code.
            }
        }
    ...
    public static int GetPercent(string procCodeStr, string planType, long planNum, long patPlanNum, List<Benefit> listBenefits)
    {
        ...
        for (var i = 0; i < listBenefits.Count; i++)
        {
            if (listBenefits[i].PlanNum == 0 && listBenefits[i].PatPlanNum != patPlanNum) continue;
            if (listBenefits[i].PatPlanNum == 0 && listBenefits[i].PlanNum != planNum) continue;
            if (listBenefits[i].BenefitType != InsBenefitType.CoInsurance) continue;
            if (listBenefits[i].Percent == -1) continue;
            listBenefitsShort.Add(listBenefits[i]);
        }
        ...
        for (var i = 0; i < listBenefitsShort.Count; i++)
        {
            ...
            if (listBenefitsShort[i].PatPlanNum == 0)
            {
                if (benefitPlan != null && benefitPlan.CovCatNum != 0)
                {
                    //must compare
                    //only use the new one if the item order is larger
                    if (CovCats.GetOrderShort(listBenefitsShort[i].CovCatNum) > CovCats.GetOrderShort(benefitPlan.CovCatNum)) benefitPlan = listBenefitsShort[i];
                }
                else
                {
                    //first one encountered for a category
            ...
            if (benefitPat != null && benefitPat.CovCatNum != 0)
            {
                //must compare
                //only use the new one if the item order is larger
                if (CovCats.GetOrderShort(listBenefitsShort[i].CovCatNum) > CovCats.GetOrderShort(benefitPat.CovCatNum)) benefitPat = listBenefitsShort[i];
            }
            else
            {
                //first one encountered for a category
        ...
        for (var i = 0; i < listBenefitsShort.Count; i++)
        {
            ...
            if (listBenefitsShort[i].CodeGroupNum == 0) continue;
            if (listBenefitsShort[i].CovCatNum > 0) continue;
            //See if the procedure's code is in the benefit's codegroup
            var codeGroup = CodeGroups.GetOne(listBenefitsShort[i].CodeGroupNum);
            if (!ProcedureCodes.IsCodeInList(procCodeStr, codeGroup.ProcCodes)) continue;
            //Benefit is pat-specific
            if (listBenefitsShort[i].PatPlanNum != 0)
            {
                //If benefits have overlapping codegroups
                if (benefitPat != null && benefitPat.CodeGroupNum != 0)
                {
                    //This should never happen, but for the sake of answering "what if," we defer to the higher-order codegroup
                    if (CodeGroups.GetOrder(listBenefitsShort[i].CodeGroupNum) > CodeGroups.GetOrder(benefitPat.CodeGroupNum)) benefitPat = listBenefitsShort[i];
                }
                else
                {
                    benefitPat = listBenefitsShort[i];
            ...
            //Benefit is plan-wide
            //If benefits have overlapping codegroups
            if (benefitPlan != null && benefitPlan.CodeGroupNum != 0)
            {
                //This should never happen, but for the sake of answering "what if," we defer to the higher-order codegroup
                if (CodeGroups.GetOrder(listBenefitsShort[i].CodeGroupNum) > CodeGroups.GetOrder(benefitPlan.CodeGroupNum)) benefitPlan = listBenefitsShort[i];
            }
            else
            {
                benefitPlan = listBenefitsShort[i];
        ...
        if (benefitPat != null) return benefitPat.Percent;
        if (benefitPlan != null) return benefitPlan.Percent;
        return 0;
    }
    ...
    public static bool IsExcluded(string strProcCode, List<Benefit> listBenefits, long planNum, long patPlanNum)
    {
        for (var i = 0; i < listBenefits.Count; i++)
        {
            if (listBenefits[i].PlanNum == 0 && listBenefits[i].PatPlanNum != patPlanNum) continue;
            if (listBenefits[i].PatPlanNum == 0 && listBenefits[i].PlanNum != planNum) continue;
            if (listBenefits[i].BenefitType != InsBenefitType.Exclusions) continue;
            if (listBenefits[i].CodeNum > 0)
            {
                if (strProcCode == ProcedureCodes.GetStringProcCode(listBenefits[i].CodeNum)) return true; //specific procedure code excluded
                continue;
            }

            if (listBenefits[i].CodeGroupNum > 0)
            {
                if (CodeGroups.IsProcInCodeGroup(strProcCode, listBenefits[i].CodeGroupNum)) return true; //Codegroup benefit excludes this procedure.
                continue;
            }

            if (listBenefits[i].CovCatNum == 0) continue;
            ...
            //see if the span matches
            var listCovSpansForCat = CovSpans.GetForCat(listBenefits[i].CovCatNum);
            //bool isMatch=false;
            for (var j = 0; j < listCovSpansForCat.Count; j++)
    ...
    {
        Benefit benefitNew;
        for (var i = 0; i < listBenefitsOld.Count; i++)
        {
            //loop through the old list
            benefitNew = null;
            for (var j = 0; j < listBenefitsNew.Count; j++)
            {
                if (listBenefitsNew[j] == null || listBenefitsNew[j].BenefitNum == 0) continue;
            ...
            if (benefitNew.PlanNum != listBenefitsOld[i].PlanNum
                ...
                Update(benefitNew, listBenefitsOld[i]); //logging taken care of in update
        }

        for (var i = 0; i < listBenefitsNew.Count; i++)
        {
            //loop through the new list
            if (listBenefitsNew[i] == null) continue;
            if (listBenefitsNew[i].BenefitNum != 0) continue;
            //benefit with benefitNum=0, so it's new
            Insert(listBenefitsNew[i]); //logging taken care of in Insert method.
        }
    }
    ...
    public static Benefit[,] GetDisplayMatrix(List<Benefit> listBenefitsForPat, List<PatPlan> listPatPlans, List<InsSub> listInsSubs)
    {
        ...
        var dictionaryBenefits = new Dictionary<Benefit, List<Benefit[]>>();
        InsSub insSub;
        for (var i = 0; i < listBenefitsForPat.Count; i++)
        for (var j = 0; j < listPatPlans.Count; j++)
        {
        ...
        var arrayList = new ArrayList(); //each object is a Benefit[]
        foreach (var arrBen in dictionaryBenefits.SelectMany(x => x.Value)) arrayList.Add(arrBen);
        IComparer iComparer = new BenefitArraySorter();
        arrayList.Sort(iComparer);
        var retVal = new Benefit[listPatPlans.Count, arrayList.Count];
        for (var y = 0; y < arrayList.Count; y++)
        for (var x = 0; x < listPatPlans.Count; x++)
    ...
    public static string GetFrequencyDisplay(FrequencyType frequencyType, List<Benefit> listBenefits, long planNum)
    {
        for (var i = 0; i < listBenefits.Count; i++)
        {
            if (listBenefits[i].PlanNum != planNum) continue;
            switch (frequencyType)
            {
                case FrequencyType.BW:
                    if (IsBitewingFrequency(listBenefits[i])) return GetFrequencyString(listBenefits[i]);
                    continue;
                case FrequencyType.PanoFMX:
                    if (IsPanoFrequency(listBenefits[i])) return GetFrequencyString(listBenefits[i]);
                    continue;
                case FrequencyType.Exam:
                    if (IsExamFrequency(listBenefits[i])) return GetFrequencyString(listBenefits[i]);
                    continue;
            } //end switch
        } //end i loop
    ...
    public static string GetCategoryString(Benefit benefit)
    {
        ...
        var isHidden = false;
        if (IsAgeLimit(benefit) && !CodeGroups.IsShownInAgeLimit(benefit.CodeGroupNum)) isHidden = true;
        if (IsFrequencyLimitation(benefit) && CodeGroups.IsHidden(benefit.CodeGroupNum)) isHidden = true;
        if (benefit.CodeGroupNum != 0)
        {
            retVal = CodeGroups.GetGroupName(benefit.CodeGroupNum, isHidden);
        }
        ...
        {
            var proccode = ProcedureCodes.GetProcCode(benefit.CodeNum);
    ...
    {
        var codeGroup = CodeGroups.GetOneForCodeGroupFixed(enumCodeGroupFixed);
        //If there is no valid code group then the benefit is not a frequency limitation.
        if (codeGroup == null) return false;
        var listCodeNums = ProcedureCodes.GetCodeNumsForProcCodes(codeGroup.ProcCodes);
        ...
        if (benefit.PatPlanNum > 0 && benefit.PatPlanNum != patPlanNum) return false;
        if (!IsFrequencyLimitation(benefit, isQuantityRequired)) return false;
        return true;
    }
    ...
    public static bool IsFrequencyLimitation(Benefit benefit, bool isQuantityRequired = false)
    {
        ...
        if (benefit.BenefitType != InsBenefitType.Limitations) return false;
        if (benefit.MonetaryAmt != -1) return false;
        if (benefit.Percent != -1) return false;
        if (benefit.QuantityQualifier.In(BenefitQuantity.Months, BenefitQuantity.Years))
        {
            if (benefit.TimePeriod != BenefitTimePeriod.None) return false;
        }
        else if (benefit.QuantityQualifier.In(BenefitQuantity.NumberOfServices))
        {
            if (!benefit.TimePeriod.In(BenefitTimePeriod.ServiceYear, BenefitTimePeriod.CalendarYear, BenefitTimePeriod.NumberInLast12Months)) return false;
        }
        else
        {
        ...
        if (benefit.CoverageLevel != BenefitCoverageLevel.None) return false;
        if (isQuantityRequired && benefit.Quantity == 0) return false;
        return true;
    }
    ...
    {
        if (benefit == null) return false;
        if (benefit.CodeGroupNum == 0) return false;
    ...
    {
        if (benefit == null) return false;
        return benefit.BenefitType == InsBenefitType.Limitations
               && benefit.CodeGroupNum == 0
