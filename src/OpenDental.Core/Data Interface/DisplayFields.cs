...
public class DisplayFields
{
    ...
    public static bool IsInUse(DisplayFieldCategory displayFieldCategory, string internalName)
    {
        ...
        var listDisplayFields = GetForCategory(displayFieldCategory);
        var isVisibleCategory = listDisplayFields.Find(x => x.InternalName == internalName) != null;
    ...
    public static List<DisplayField> GetForCategory(DisplayFieldCategory displayFieldCategory)
    {
        ...
        if (listDisplayFields.Count == 0)
            //default
            return GetDefaultList(displayFieldCategory);
    ...
    public static List<DisplayField> GetForChartView(long chartViewNum)
    {
        ...
        if (listDisplayFields.Count == 0)
            //default
            return GetDefaultList(DisplayFieldCategory.None);
    ...
    public static List<DisplayField> GetDefaultList(DisplayFieldCategory displayFieldCategory)
    {
        ...
        switch (displayFieldCategory)
        {
            ...
            case DisplayFieldCategory.ChartPatientInformation:
                ...
                {
                    //different default listDisplayFields for eCW:
            ...
            case DisplayFieldCategory.ArManagerExcludedGrid:
                listDisplayFields.Add(new DisplayField("Guarantor", true ? 140 : 240, displayFieldCategory));
                if (true) listDisplayFields.Add(new DisplayField("Clinic", 100, displayFieldCategory));
            ...
            case DisplayFieldCategory.ArManagerSentGrid:
                listDisplayFields.Add(new DisplayField("Guarantor", true ? 150 : 250, displayFieldCategory));
                if (true) listDisplayFields.Add(new DisplayField("Clinic", 100, displayFieldCategory));
    ...
    public static List<DisplayField> GetAllAvailableList(DisplayFieldCategory displayFieldCategory)
    {
        ...
        switch (displayFieldCategory)
        {
            ...
            case DisplayFieldCategory.PatientInformation:
                ...
                if (false) listDisplayFields.Add(new DisplayField("References", 0, displayFieldCategory));
            ...
            case DisplayFieldCategory.ChartPatientInformation:
                ...
                if (PrefC.GetBool(PrefName.ShowFeatureEhr)) listDisplayFields.Add(new DisplayField("Tobacco Use", 0, displayFieldCategory));
            ...
            case DisplayFieldCategory.OrthoChart:
                ...
                var listDistinctFieldNames = OrthoCharts.GetDistinctFieldNames();
                for (var j = 0; j < listDistinctFieldNames.Count; j++)
                {
                    if (listDisplayFields.Exists(x => x.Description == listDistinctFieldNames[j])) continue;

                    //If there aren't any display fields with the current field name, create one and add it to the list of available fields.
                    var displayField = new DisplayField("", 20, DisplayFieldCategory.OrthoChart);
            ...
            case DisplayFieldCategory.SuperFamilyGridCols:
                listDisplayFields.Add(new DisplayField("Name", 280, displayFieldCategory));
                listDisplayFields.Add(new DisplayField("Stmt", 50, displayFieldCategory));
                var listPatFieldDefs = PatFieldDefs.GetDeepCopy().FindAll(x => !x.IsHidden);
                for (var j = 0; j < listPatFieldDefs.Count; j++)
                {
                    var displayField = new DisplayField("", 100, DisplayFieldCategory.SuperFamilyGridCols);
    ...
    {
        var isDefault = true;
        var listDisplayFieldsDefault = GetDefaultList(displayFieldCategory);
        if (listDisplayFieldsShowing.Count != listDisplayFieldsDefault.Count)
            isDefault = false;
        else
            for (var i = 0; i < listDisplayFieldsShowing.Count; i++)
            {
        ...
        var command = "DELETE FROM displayfield WHERE Category=" + SOut.Long((int) displayFieldCategory);
        Db.NonQ(command);
        if (isDefault) return;
    ...
    private class DisplayFieldCache : CacheListAbs<DisplayField>
    {
        ...
        {
            var command = "SELECT * FROM displayfield ORDER BY ItemOrder";
            return DisplayFieldCrud.SelectMany(command);
        }
