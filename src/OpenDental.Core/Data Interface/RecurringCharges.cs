...
public class RecurringCharges
{
    ...
    {
        //Days of Month
        if (dateStop == DateTime.MinValue) dateStop = DateTime.MaxValue;
        if (dateStart == DateTime.MinValue) return DateTime.MinValue;
        if (string.IsNullOrWhiteSpace(chargeFrequency)) return DateTime.MinValue;
        if (dateStart > dateStop) return DateTime.MinValue;
        var dateTimeNextCharge = DateTime.MinValue;
        var dateCurrentlyChecking = dateStart;
        var isDayOfMonth = CreditCards.GetFrequencyType(chargeFrequency) == ChargeFrequencyType.FixedDayOfMonth;
        if (isDayOfMonth)
        {
            var listDays = CreditCards.GetDaysOfMonthForChargeFrequency(chargeFrequency).Split(",", StringSplitOptions.RemoveEmptyEntries).Select(x => SIn.Int(x)).ToList();
            //Iterate over 2 months, just in case we are at the end of the current month.
            for (var i = 0; i < 2; i++)
            {
                var daysInThisMonth = DateTime.DaysInMonth(dateCurrentlyChecking.Year, dateCurrentlyChecking.Month);
                //Loop over the possible charge days for the month
                for (var j = 0; j < listDays.Count; j++)
                {
                    ...
                    if (dateCurrentlyChecking < dateStart || dateCurrentlyChecking > dateStop) continue;
                    dateTimeNextCharge = dateCurrentlyChecking;
                    break;
                }

                //If we found a date where a charge should occur, break out.
                if (dateTimeNextCharge != DateTime.MinValue) break;
        ...
        {
            var frequency = CreditCards.GetDayOfWeekFrequency(chargeFrequency);
            var day = CreditCards.GetDayOfWeek(chargeFrequency);
            switch (frequency)
            {
                ...
                case DayOfWeekFrequency.First:
                    ...
                    if (dateTimeNextCharge < dateStart) dateTimeNextCharge = CreditCards.GetNthWeekdayofMonth(dateStart.AddMonths(1), 1, day);
                    break;
                case DayOfWeekFrequency.Second:
                    dateTimeNextCharge = CreditCards.GetNthWeekdayofMonth(dateStart, 2, day);
                    if (dateTimeNextCharge < dateStart) dateTimeNextCharge = CreditCards.GetNthWeekdayofMonth(dateStart.AddMonths(1), 2, day);
                    break;
                case DayOfWeekFrequency.Third:
                    dateTimeNextCharge = CreditCards.GetNthWeekdayofMonth(dateStart, 3, day);
                    if (dateTimeNextCharge < dateStart) dateTimeNextCharge = CreditCards.GetNthWeekdayofMonth(dateStart.AddMonths(1), 3, day);
                    break;
                case DayOfWeekFrequency.Fourth:
                    dateTimeNextCharge = CreditCards.GetNthWeekdayofMonth(dateStart, 4, day);
                    if (dateTimeNextCharge < dateStart) dateTimeNextCharge = CreditCards.GetNthWeekdayofMonth(dateStart.AddMonths(1), 4, day);
                    break;
                case DayOfWeekFrequency.Fifth:
                    dateTimeNextCharge = CreditCards.GetNthWeekdayofMonth(dateStart, 5, day);
                    if (dateTimeNextCharge < dateStart) dateTimeNextCharge = CreditCards.GetNthWeekdayofMonth(dateStart.AddMonths(1), 5, day);
                    break;
            }
        }
...
public class RecurringChargerator
{
    ...
    public List<RecurringChargeData> FillCharges(List<ClinicDto> listUserClinics)
    {
        ...
        if (true && Security.CurUser.ClinicIsRestricted) listClinicNums = listUserClinics.Select(x => x.Id).ToList();
        ...
        for (var i = ListRecurringChargeData.Count - 1; i > -1; i--)
        {
            ...
            var chargeCur = ListRecurringChargeData[i];
            var famBalTotal = (decimal) chargeCur.RecurringCharge.FamBal;
            decimal rptChargeAmt;
            //will be 0 if this is not a payplan row, if negative don't subtract from the FamBalTotal
            var payPlanDue = Math.Max((decimal) chargeCur.RecurringCharge.PayPlanDue, 0);
            var patNum = chargeCur.RecurringCharge.PatNum;
            ...
            //card in the list with a single query and return the DataTable.  We will leave it for now.
            decimal getProcTotal()
            {
                decimal total = 0;
                if (PrefC.GetBool(PrefName.BillingUseBillingCycleDay))
                    total = (decimal) CreditCards.TotalRecurringCharges(patNum, chargeCur.Procedures, chargeCur.BillingCycleDay);
                else
                    total = (decimal) CreditCards.TotalRecurringCharges(patNum, chargeCur.Procedures, chargeCur.DateStart.Day);
                total += payPlanDue; //payPlanDue will be 0 if this is not a payplan row.  If negative amount due on payplan, payPlanDue is set to 0 above
                return total;
            }

            ;
            decimal totalChargeAmt;
            if (!chargeCur.Procedures.IsNullOrEmpty()) //calculates repeating charges based on the presence of procedures on the patient's account that are linked to the CC
                totalChargeAmt = getProcTotal();
            else //calculates repeating charges by the ChargeAmt on the credit card which is the sum of repeat charge and payplan payment amount
                totalChargeAmt = (decimal) chargeCur.RecurringCharge.ChargeAmt;
            rptChargeAmt = Math.Min(totalChargeAmt, (decimal) chargeCur.RecurringCharge.ChargeAmt);
            ...
            var guarNum = chargeCur.Guarantor;
            ...
            if (!dictFamBals.ContainsKey(guarNum)) dictFamBals.Add(guarNum, famBalTotal);
            ...
            var chargeAmt = Math.Max(dictFamBals[guarNum], payPlanDue);
    ...
    public void SendCharges(List<RecurringChargeData> listRecurringChargeData, bool forceDuplicates)
    {
        ...
        if (listRecurringChargesNoBackPayments.Count == 0) return;
        ...
        try
        {
            ...
            ClearStats();
            var strBuilderResultFileXCharge = new StringBuilder();
            var strBuilderResultFilePayConnect = new StringBuilder();
            var strBuilderResultFilePaySimple = new StringBuilder();
            var listClinicNumsBadCredentialsXCharge = new List<long>();
            var listCreditCardSourcesOpenEdge = new List<CreditCardSource>
            {
                //X-Charge, XWeb, and EdgeExpress card sources are compatible with each other's processor.
            ...
            foreach (var chargeData in listRecurringChargesNoBackPayments)
            {
                if (!chargeData.CCIsRecurringActive) continue;
                var useEdgeExpress = !string.IsNullOrEmpty(chargeData.XChargeToken) && Programs.IsEnabled(ProgramName.EdgeExpress) &&
                                     listCreditCardSourcesOpenEdge.Contains(chargeData.CCSource);
                var useXCharge = !string.IsNullOrEmpty(chargeData.XChargeToken) && Programs.IsEnabled(ProgramName.Xcharge) &&
                                 chargeData.CCSource.In(listCreditCardSourcesOpenEdge.Select(x => x).ToArray());
                var usePayConnect = !string.IsNullOrEmpty(chargeData.PayConnectToken) && Programs.IsEnabled(ProgramName.PayConnect);
                var usePaySimple = !string.IsNullOrEmpty(chargeData.PaySimpleToken) && Programs.IsEnabled(ProgramName.PaySimple);
                if (useEdgeExpress)
                {
                ...
                if (PrefC.GetBool(PrefName.RecurringChargesInactivateDeclinedCards)
                    && chargeData.RecurringCharge.ChargeStatus == RecurringChargeStatus.ChargeDeclined)
                {
                    var cc = CreditCards.GetOne(chargeData.RecurringCharge.CreditCardNum);
        ...
        StringBuilder strBuilderResultFilePaySimple)
    {
        if (strBuilderResultFileXCharge.Length > 0)
            try
            {
                var xPath = Programs.GetProgramPath(Programs.GetCur(ProgramName.Xcharge));
        ...
        List<long> listClinicNumsBadCredentials, bool doAlwaysUseDTG)
    {
        ...
        var tokenCount = CreditCards.GetXChargeTokenCount(chargeData.XChargeToken, false);
        if (chargeData.XChargeToken != "" && tokenCount != 1)
        {
            var msg = tokenCount > 1 ? "A duplicate token was found" : "A token no longer exists";
            MarkFailed(chargeData, Lans.g(_lanThis, msg + ", the card cannot be charged."));
            return;
        }

        var patCur = Patients.GetPat(chargeData.RecurringCharge.PatNum);
        if (patCur == null)
        {
        ...
        if (wasChargeAttempted) CreatePayment(patCur, chargeData, strBuilderResultText.ToString(), amount, receipt.ToString(), ccSource, xWebResponseNum);
    }
    ...
    {
        var strBuilderResultText = new StringBuilder();
        strBuilderResultFile.AppendLine("Recurring charge results for " + DateTime.Now.ToShortDateString() + " ran at " + DateTime.Now.ToShortTimeString());
        strBuilderResultFile.AppendLine();
        var tokenCount = CreditCards.GetXChargeTokenCount(chargeData.XChargeToken, false);
        if (chargeData.XChargeToken != "" && tokenCount != 1)
        ...
        List<long> listClinicNumsBadCredentials, out StringBuilder strBuilderResultText, out double amount, out StringBuilder receipt)
    {
        ...
        var username = ProgramProperties.GetPropVal(_progCur.ProgramNum, "Username", clinicNumCur);
        ...
        var password = ProgramProperties.GetPropVal(_progCur.ProgramNum, "Password", clinicNumCur);
        if (_progCur.ProgName != ProgramName.Xcharge.ToString()) password = Class1.TryDecrypt(password);
        if (string.IsNullOrEmpty(username) || string.IsNullOrEmpty(password))
        {
            //clinicNumCur is not in listClinicNumsBadCredentials yet
            var clinicAbbr = "Headquarters";
            if (clinicNumCur > 0) clinicAbbr = Clinics.GetAbbr(clinicNumCur);
        ...
        if (_progCur.ProgName == ProgramName.Xcharge.ToString()) password = MiscUtils.Decrypt(password);
        ...
        var xPath = Programs.GetProgramPath(_progCur);
        var info = new ProcessStartInfo(xPath);
        info.Arguments = "";
        var amt = chargeData.RecurringCharge.ChargeAmt;
        var exp = chargeData.CCExpiration;
        var address = chargeData.Address;
        var addressPat = chargeData.AddressPat;
        var zip = chargeData.Zip;
        var zipPat = chargeData.ZipPat;
        var creditCardNum = chargeData.RecurringCharge.CreditCardNum;
        ...
        if (exp.Year > 1880) info.Arguments += "/EXP:" + exp.ToString("MMyy") + " ";
        if (address != "")
            info.Arguments += "\"/ADDRESS:" + CleanString(address) + "\" ";
        else if (addressPat != "") info.Arguments += "\"/ADDRESS:" + CleanString(addressPat) + "\" ";
        ...
        else if (zipPat != "" && !hasForeignKey) info.Arguments += "\"/ZIP:" + CleanString(zipPat) + "\" ";
        ...
        if (forceDuplicates) info.Arguments += "/ALLOWDUPLICATES ";
        ...
        var newAccount = "";
        var newExpiration = new DateTime();
        var status = 2; //0=Success,1=TransactionNotCompleted,2=Other
        var descript = "";
        ...
        try
        {
            using (TextReader reader = new StreamReader(resultfile))
            {
                ...
                while (line != null)
                {
                    ...
                    if (line.StartsWith("RESULT="))
                    {
                        if (line == "RESULT=SUCCESS")
                            status = 0; //Success
                        else if (line == "RESULT=Transaction not completed") status = 1; //TransactionNotCompleted
                    }
                    ...
                    else if (line.StartsWith("EXPIRATION="))
                    {
                        ...
                        newExpiration = new DateTime(SIn.Int("20" + expStr.Substring(2)), SIn.Int(expStr.Substring(0, 2)), 1); //First day of the month
                    }
                    else if (line.StartsWith("APPROVEDAMOUNT="))
                    {
                        amount = SIn.Double(line.Substring("APPROVEDAMOUNT=".Length));
                    }
                ...
                else if (status == 1 && descript.ToLower().Contains("xcharge.exe is already running"))
                {
                    //TransactionNotCompleted
                    MarkFailed(chargeData, Lans.g(_lanThis, "Result from XCharge:") + " " + strBuilderResultText, LogLevel.Information);
                }
                else
                {
                    //Other
                    MarkDeclined(chargeData, Lans.g(_lanThis, "Result from XCharge:") + " " + strBuilderResultText, LogLevel.Information);
                }
            }
        }
        catch (Exception ex)
        ...
        {
            var creditCardCur = CreditCards.GetOne(creditCardNum);
        ...
        out StringBuilder strBuilderResultText, out double amount, out StringBuilder receipt, out long xWebResponseNum)
    {
        ...
        try
        {
            var response = XWebs.MakePaymentWithAlias(chargeData.RecurringCharge.PatNum, Lans.g(_lanThis, "Made from automated recurring charge"),
        ...
        catch (Exception ex)
        {
            ...
            if (ex is ODException && ((ODException) ex).ErrorCodeAsEnum != ODException.ErrorCodes.XWebDTGFailed) return false;
    ...
    public void SendPayConnect(RecurringChargeData chargeData, bool forceDuplicates, StringBuilder strBuilderResultFile)
    {
        ...
        var payconnectVersion = ProgramProperties.GetPropVal(program.ProgramNum, "Program Version", Clinics.ClinicNum);
    ...
    private void SendPayConnectV1(RecurringChargeData chargeData, bool forceDuplicates, StringBuilder strBuilderResultFile)
    {
        ...
        if (true) dictClinicNumDesc = Clinics.GetClinicsNoCache().ToDictionary(x => x.Id, x => x.Description);
        dictClinicNumDesc[0] = PrefC.GetString(PrefName.PracticeTitle);
        strBuilderResultFile.AppendLine("Recurring charge results for " + DateTime.Now.ToShortDateString() + " ran at " + DateTime.Now.ToShortTimeString());
        strBuilderResultFile.AppendLine();
        var isPayConnectToken = true;
        var tokenOrCCMasked = chargeData.PayConnectToken;
        var tokenCount = CreditCards.GetPayConnectTokenCount(tokenOrCCMasked);
        ...
        var patNum = chargeData.RecurringCharge.PatNum;
        var patCur = Patients.GetPat(patNum);
        if (patCur == null)
        {
            MarkFailed(chargeData, Lans.g(_lanThis, "Unable to find patient") + " " + chargeData.RecurringCharge.PatNum);
            return;
        ...
        if (chargeData.PayConnectTokenExp == DateTime.MinValue) chargeData.PayConnectTokenExp = chargeData.CCExpiration;
        var exp = chargeData.PayConnectTokenExp;
        if (tokenOrCCMasked == "")
        {
        ...
        var payConnectRequest = PayConnect.BuildSaleRequest(
        ...
        var payConnectResponse = PayConnect.ProcessCreditCard(payConnectRequest, clinicNumCur,
            x => MarkDeclined(chargeData, x));
        var strBuilderResultText = new StringBuilder(); //this payment's result text, used in payment note and then appended to file string builder
        ...
        if (payConnectResponse == null || payConnectResponse.Status == null)
        {
            ...
            if (true && dictClinicNumDesc.ContainsKey(clinicNumCur)) strBuilderResultText.AppendLine("CLINIC=" + dictClinicNumDesc[clinicNumCur]);
        ...
        else if (payConnectResponse.Status.code != 0)
        {
            ...
            if (true && dictClinicNumDesc.ContainsKey(clinicNumCur)) strBuilderResultText.AppendLine("CLINIC=" + dictClinicNumDesc[clinicNumCur]);
            strBuilderResultText.AppendLine(Lans.g(_lanThis, "Transaction Type") + ": " + transType.SALE);
        ...
        else
        {
            ...
            chargeData.RecurringCharge.ChargeStatus = RecurringChargeStatus.ChargeSuccessful;
            Success++;
            var ccCur = CreditCards.GetOne(chargeData.RecurringCharge.CreditCardNum);
            ...
            strBuilderResultText.AppendLine("TRANS TYPE=" + transType.SALE);
            ...
            if (ccCur != null) strBuilderResultText.AppendLine("ACCOUNT=" + ccCur.CCNumberMasked); //XXXXXXXXXXXX1234, all but last four numbers replaced with X's
        ...
        var receipt = PayConnect.BuildReceiptString(payConnectRequest, payConnectResponse, null, clinicNumCur);
    ...
    private void SendPayConnectV2(RecurringChargeData chargeData, StringBuilder strBuilderResultFile)
    {
        ...
        var exp = chargeData.PayConnectTokenExp;
        if (exp.Year < 1880) exp = chargeData.CCExpiration;
        if (tokenOrCCMasked == "")
        {
            isPayConnectToken = false;
            tokenOrCCMasked = chargeData.CCNumberMasked;
        ...
        var clinicNumCur = chargeData.RecurringCharge.ClinicNum;
        double amount = 0;
        var response = PayConnect2.PostCreateTransactionByToken(patCur, exp.ToString("MMyy"), chargeData.PayConnectToken, zip, PayConnect2.FormatAmountForApi(chargeData.RecurringCharge.ChargeAmt), clinicNumCur, PayConnect2.TransactionType.Sale, PayConnect2.TransactionFrequency.Recurring);
        ...
        var payConnectResponse = PayConnect2.ApiResponseToPayConnectResponse(response);
        var strBuilderResultText = new StringBuilder(); //this payment's result text, used in payment note and then appended to file string builder
        strBuilderResultFile.AppendLine("PatNum: " + patNum + " Name: " + patCur.GetNameFLnoPref());
        ...
        else if (payConnectResponse.StatusCode != "0")
        {
            //error in transaction
        ...
        else
        {
            ...
            strBuilderResultText.AppendLine("TRANSACTION NUMBER=" + payConnectResponse.RefNumber);
            if (ccCur != null) strBuilderResultText.AppendLine("ACCOUNT=" + ccCur.CCNumberMasked); //XXXXXXXXXXXX1234, all but last four numbers replaced with X's
            if (payConnectResponse.PaymentToken != null && payConnectResponse.TokenExpiration > DateTime.MinValue) strBuilderResultText.AppendLine("EXPIRATION=" + payConnectResponse.TokenExpiration.ToString("MMyy"));
            if (isPayConnectToken)
                strBuilderResultText.AppendLine("CARD TYPE=PayConnect Token");
            else
                strBuilderResultText.AppendLine("CARD TYPE=" + CreditCardUtils.GetCardType(tokenOrCCMasked));
            if (payConnectResponse.AmountSurcharged > 0) strBuilderResultText.AppendLine("SURCHARGE FEE=" + payConnectResponse.AmountSurcharged.ToString("F2"));
            strBuilderResultText.AppendLine("AMOUNT=" + payConnectResponse.Amount.ToString("F2"));
            amount = (double) payConnectResponse.Amount;
        }

        var receipt = PayConnect.BuildReceiptString(transType.SALE, payConnectResponse.RefNumber, patCur.GetNameFLnoPref(),
    ...
    public void SendPaySimple(RecurringChargeData chargeData, StringBuilder strBuilderResultFile)
    {
        ...
        strBuilderResultFile.AppendLine("Recurring charge results for " + DateTime.Now.ToShortDateString() + " ran at " + DateTime.Now.ToShortTimeString());
        strBuilderResultFile.AppendLine();
        var paySimpleAccountId = chargeData.PaySimpleToken;
        var tokenCount = CreditCards.GetPaySimpleTokenCount(paySimpleAccountId, chargeData.CCSource == CreditCardSource.PaySimpleACH || chargeData.CCSource == CreditCardSource.PaySimplePaymentPortalACH);
        if (string.IsNullOrWhiteSpace(paySimpleAccountId) || tokenCount != 1)
        {
            var msg = tokenCount > 1 ? "A duplicate token was found" : "A token was not found";
            MarkFailed(chargeData, Lans.g(_lanThis, msg + ", the card cannot be charged."));
            return;
        }
        ...
        var exp = chargeData.CCExpiration; //We don't have a PaySimpleTokenExpiration, so use the CC's stored one.
        var amt = (decimal) chargeData.RecurringCharge.ChargeAmt;
        var zip = chargeData.Zip;
        var clinicNumCur = chargeData.RecurringCharge.ClinicNum;
        double resultAmt = 0;
        ...
        try
        {
            var response = PaySimple.MakePaymentByToken(patCur, new CreditCard
            {
            ...
            //add to strbuilder that will be written to txt file and to the payment note
            var clinicDesc = "";
            if (true && dictClinicNumDesc.ContainsKey(clinicNumCur)) clinicDesc = dictClinicNumDesc[clinicNumCur];
            var ccExpYear = -1;
            var ccExpMonth = -1;
            var expStr = "";
    ...
    private void MarkDeclined(RecurringChargeData chargeData, string errorMsg, LogLevel logLevel = LogLevel.Error)
    {
        if (chargeData.RecurringCharge.ChargeStatus == RecurringChargeStatus.NotYetCharged
            && PrefC.GetBool(PrefName.RecurringChargesInactivateDeclinedCards))
            chargeData.RecurringCharge.ChargeStatus = RecurringChargeStatus.ChargeDeclined;
    ...
    public List<RecurringChargeData> PaymentsWithinLockDate(List<RecurringChargeData> listChargeData)
    {
        ...
        var listRecurringCharges = new List<RecurringChargeData>();
        foreach (var chargeCur in listChargeData)
        {
            //Calculate what the new pay date will be.
            var newPayDate = GetPayDate(chargeCur);
            ...
            if (isBeforeLockDate)
            {
                if (warnings.Count == 0) warnings.Add(Lans.g(_lanThis, "Lock date limitation is preventing the recurring charges from running") + ":");
        ...
        long xWebResponseNum = 0, double merchantFee = 0)
    {
        ...
        if (true) paymentCur.ClinicNum = recCharge.RecurringCharge.ClinicNum;
        ...
        else if (ccSource == CreditCardSource.EdgeExpressCNP || ccSource == CreditCardSource.EdgeExpressRCM) ppPayTypeDesc = EdgeExpressProps.PaymentType;
        if (ccSource != CreditCardSource.PaySimpleACH && ccSource != CreditCardSource.PaySimplePaymentPortalACH) paymentCur.PayType = PrefC.GetLong(PrefName.RecurringChargesPayTypeCC);
        if (paymentCur.PayType == 0) //Pref default not set or this is ACH
            paymentCur.PayType = SIn.Int(ProgramProperties.GetPropVal(_progCur.ProgramNum, ppPayTypeDesc, paymentCur.ClinicNum));
        if (recCharge.PaymentType != 0) paymentCur.PayType = recCharge.PaymentType; //Set the payment type to the recurring charges payment type override.
        ...
        if (recCharge.RecurringCharge.ChargeStatus == RecurringChargeStatus.ChargeSuccessful) paymentCur.IsCcCompleted = true;
        ...
        {
            var xWebResponse = XWebResponses.GetOne(xWebResponseNum);
        ...
        var accountingMode = PrefC.GetEnum<RigorousAccounting>(PrefName.RigorousAccounting);
        if (CompareDouble.IsZero(amount))
        {
            ...
            var paySplit = CreateUnearnedSplitForPayment(paymentCur, amount, recCharge.PayPlanNum);
        ...
        var remainingAmt = amount - listPaySplits.Sum(x => x.SplitAmt);
    ...
    private List<PaySplit> GetLinkedSplitsForChargeData(Patient patCur, RecurringChargeData recCharge, Payment paymentCur)
    {
        ...
        var autoSplit = PaymentEdit.AutoSplitForPayment(patCur.PatNum, paymentCur, payPlanNum: recCharge.PayPlanNum,
        ...
        else
        {
            ...
            {
                var oldPayment = paymentCur.Clone();
    ...
    private List<PaySplit> GetNonLinkedSplitsForChargeData(Patient patCur, RecurringChargeData recCharge, Payment paymentCur)
    {
        ...
        if (recCharge.PayPlanNum > 0)
        {
            if (CompareDouble.IsLessThanOrEqualToZero(recCharge.RecurringCharge.PayPlanDue)) return listPaySplits;
            //Only make a split to the selected payment plan for what is due. No more, no less.
            var splitPatNum = paymentCur.PatNum;
        ...
        {
            //Non-payment plan split(s)
            //Regular payments need to apply to the provider that the family owes the most money to.
            long provNum = 0;
            var dt = Patients.GetPaymentStartingBalances(patCur.Guarantor, paymentCur.PayNum);
            double highestAmt = 0;
            for (var j = 0; j < dt.Rows.Count; j++)
            {
                var afterIns = SIn.Double(dt.Rows[j]["AfterIns"].ToString());
                if (highestAmt >= afterIns) continue;
                highestAmt = afterIns;
                if (PrefC.GetBool(PrefName.RecurringChargesUsePriProv))
                    provNum = patCur.PriProv;
                else
                    provNum = SIn.Long(dt.Rows[j]["ProvNum"].ToString());
            }
    ...
    {
        _doShutdown = true;
        if (!doWaitForCardToFinish) return;
