...
public class Etranss
{
    ...
    public static List<Etrans> GetErasOneClaim(string claimIdentifier, DateTime dateTClaimService)
    {
        ...
        if (string.IsNullOrEmpty(claimIdentifier)) return [];
        ...
        if (x12object != null)
        {
            ...
            else if (X277.Is277(x12object))
            {
                ...
                for (var i = 0; i < listClaimIdentifiers.Count; i++)
                {
                    ...
                    if (!dictionaryClaimMatchesByAck.ContainsKey(ack)) dictionaryClaimMatchesByAck.Add(ack, []);
    ...
    public static void ReceiveEraPayment(Claim claim, Hx835_Claim hx835_ClaimPaid, List<ClaimProc> listClaimProcsForClaim, bool isIncludeWOPercCoPay, bool isSupplementalPay, InsPlan insPlan = null, bool isAutomatic = false)
    {
        ...
        if (PrefC.GetBool(PrefName.ClaimPrimaryReceivedRecalcSecondary) && claim.ClaimStatus == "R" && claimOld.ClaimStatus != "R" && claim.ClaimType == "P") Claims.CalculateAndUpdateSecondariesFromPrimaries([claim]);
