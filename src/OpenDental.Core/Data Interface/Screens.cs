...
public class Screens
{
    ...
    public static Screen CreateScreenFromSheet(Sheet sheet, Screen screen = null)
    {
        ...
        if (sheet.SheetType != SheetTypeEnum.Screening || SheetParameter.GetParamByName(sheet.Parameters, "ScreenGroupNum") == null) return null;
        if (screen == null)
        {
            screen = new Screen();
        ...
        for (var i = 0; i < sheet.SheetFields.Count; i++)
            switch (sheet.SheetFields[i].FieldName)
            {
                case "Gender":
                    ...
                    break;
                case "Race/Ethnicity":
                    var patientRace = PatientRaceOld.Unknown;
                    Enum.TryParse(sheet.SheetFields[i].FieldValue.Split(';')[0], out patientRace);
                    screen.RaceOld = patientRace;
                    break;
                case "GradeLevel":
                    var patientGrade = PatientGrade.Unknown;
                    Enum.TryParse(sheet.SheetFields[i].FieldValue.Split(';')[0], out patientGrade);
                    screen.GradeLevel = patientGrade;
                    break;
                case "Age":
                    if (screen.Age != 0) break; //Already calculated via Birthdate.
                    byte byteAge = 0;
                    byte.TryParse(sheet.SheetFields[i].FieldValue, out byteAge);
                    screen.Age = byteAge;
                    break;
                case "Urgency":
                    var treatmentUrgency = TreatmentUrgency.Unknown;
                    Enum.TryParse(sheet.SheetFields[i].FieldValue.Split(';')[0], out treatmentUrgency);
                ...
                case "ChartSealantTreatment":
                    ...
                    if (sheet.SheetFields[i].FieldValue.Contains("C")) screen.HasCaries = YN.Yes; //Caries is present in TP'd chart.  Compl chart doesn't matter, it's only for sealant placement.

                    //Only mark "needs sealants" if TP chart has S marked for any tooth surface.
                    screen.NeedsSealants = YN.No;
                    if (sheet.SheetFields[i].FieldValue.Contains("S")) screen.NeedsSealants = YN.Yes;
                    break;
                ...
                case "MissingAllTeeth":
                    ...
                    break;
                case "Birthdate":
                    var dateBirth = new DateTime(1, 1, 1);
                    ...
                    var sheetFieldAge = sheet.SheetFields.FirstOrDefault(x => x.FieldName == "Age");
                    if (sheetFieldAge != null && string.IsNullOrEmpty(sheetFieldAge.FieldValue)) screen.Age = SIn.Byte(Patients.DateToAge(dateBirth).ToString());
                    break;
                case "Comments":
        ...
        , List<SheetField> listSheetFieldsProcOrig)
    {
        ...
        var listToothValsOld = new List<string>();
        //Process treatment planned sealants.
        for (var i = 0; i < sheet.SheetFields.Count; i++) //Go through the supplied sheet's fields and find the field.
                ...
                && sheet.SheetFields[i].FieldName == "ChartSealantTreatment")
            {
                ...
                if (listSheetFieldsChartOrig[0] != null)
                {
                    //Shouldn't be null if ChartSealantTreatment exists
        ...
        for (var i = 0; i < sheet.SheetFields.Count; i++) //Go through the supplied sheet's fields and find the field.
                ...
                && sheet.SheetFields[i].FieldName == "ChartSealantComplete")
            {
                ...
                if (listToothVals[0] == "1")
                    ...
                    continue; //Skip primary tooth charts because we do not need to create any TP procedures for them.
                listToothVals.RemoveAt(0); //Remove the toothchart type value
                if (listSheetFieldsChartOrig[1] != null)
                {
                    //Shouldn't be null if ChartSealantTreatment exists
        ...
        for (var i = 0; i < sheet.SheetFields.Count; i++)
        {
            if (sheet.SheetFields[i].FieldType != SheetFieldType.CheckBox) continue; //Only care about check box types.
            ...
            var sheetFieldOrig = listSheetFieldsProcOrig.FirstOrDefault(x => x.FieldName == sheet.SheetFields[i].FieldName && x.FieldType == SheetFieldType.CheckBox);
            ...
            var proc = Procedures.CreateProcForPatNum(sheet.PatNum, ProcedureCodes.GetCodeNum(strProcCode), "", "", ProcStat.C, provNum);
            if (proc != null) SecurityLogs.MakeLogEntry(EnumPermType.ProcEdit, sheet.PatNum, strProcCode + " " + Lans.g("Screens", "treatment planned during screening."));
        }
    }
        ...
        , List<string> listChartOrigVals)
    {
        ...
        {
            //toothValues is in the order from low to high tooth number in the chart
            ...
            var listSurfacesNew = listToothValues[i].Split(',').ToList();
            var listSurfacesOrig = listChartOrigVals[i].Split(',').ToList();
            var isDiff = false;
            for (var j = 0; j < listSurfacesOrig.Count; j++) //Both arrays have the same length unless the chart doesn't exist in the original.
            ...
            if (!isDiff) continue; //All the "S" surfaces are the same.
            var surf = "";
            var toothNum = 0;
            var isMolar = false;
            var isRight = false;
            ...
            if (i <= 1)
            {
                //Top left quadrant of toothchart
                toothNum = i + 2;
                isMolar = true;
                isRight = true;
                isLing = true;
            ...
            {
                //Top middle-left quadrant of toothchart
                toothNum = i + 2;
                isMolar = false;
                isRight = true;
                isLing = true;
            ...
            {
                //Top middle-right quadrant of toothchart
                toothNum = i + 8;
                isMolar = false;
                isRight = false;
                isLing = true;
            ...
            {
                //Top right quadrant of toothchart
                toothNum = i + 8;
                isMolar = true;
                isRight = false;
                isLing = true;
            ...
            {
                //Lower right quadrant of toothchart
                toothNum = i + 10;
                isMolar = true;
                isRight = false;
                isLing = false;
            ...
            {
                //Lower middle-right quadrant of toothchart
                toothNum = i + 10;
                isMolar = false;
                isRight = false;
                isLing = false;
            ...
            {
                //Lower middle-left quadrant of toothchart
                toothNum = i + 16;
                isMolar = false;
                isRight = true;
                isLing = false;
            ...
            {
                //Lower left quadrant of toothchart
                toothNum = i + 16;
                isMolar = true;
                isRight = true;
                isLing = false;
            ...
            if (isMolar)
            {
                if (isRight)
                {
                    if (listSurfacesNew[0] == "S") surf += "D";
                    if (listSurfacesNew[1] == "S") surf += "M";
                }
                else
                {
                    //Is Left side
                    if (listSurfacesNew[0] == "S") surf += "M";
                    if (listSurfacesNew[1] == "S") surf += "D";
                }

                if (isLing && listSurfacesNew[2] == "S") surf += "L";
                if (!isLing && listSurfacesNew[2] == "S") surf += "B";
            }
            else
            {
                //Front teeth, only look at 3rd surface position in control as that's the only one the user can see.
                if (listSurfacesNew[2] == "S") surf = "O"; //NOTE: Not sure what surface to enter here... This is just a placeholder for now until we figure it out...
            }

            if (toothNum != 0) tooth = toothNum.ToString();

            #endregion Parse Toothchart FieldValues

            surf = Tooth.SurfTidyForDisplay(surf, tooth);
            if (screenChartType == ScreenChartType.TP)
            {
                //Create TP'd sealant procs if they don't already exist for this patient.
                if (Procedures.GetProcForPatByToothSurfStat(patNum, toothNum, surf, ProcStat.TP) != null) continue;
                var procedure = Procedures.CreateProcForPatNum(patNum, ProcedureCodes.GetCodeNum("D1351"), surf, tooth, ProcStat.TP, provNum);
                if (procedure != null)
                    SecurityLogs.MakeLogEntry(EnumPermType.ProcEdit, patNum, "D1351 " + Lans.g("Screens", "treatment planned during screening with tooth")
                                                                                      + " " + procedure.ToothNum + " " + Lans.g("Screens", "and surface") + " " + procedure.Surf);
            }
            else if (screenChartType == ScreenChartType.C)
            {
                var procedure = Procedures.GetProcForPatByToothSurfStat(patNum, toothNum, surf, ProcStat.TP);
                if (procedure == null)
                {
                    //A TP procedure does not already exist.
                    procedure = Procedures.CreateProcForPatNum(patNum, ProcedureCodes.GetCodeNum("D1351"), surf, tooth, ProcStat.C, provNum);
                }
                else
                {
                    //TP proc already exists, set it complete.
                    var procOld = procedure.Copy();
    ...
    public static void DeleteScreens(List<long> listScreenNums)
    {
        ...
        var command = "DELETE FROM screen WHERE ScreenNum IN (" + string.Join(",", listScreenNums.Select(x => SOut.Long(x))) + ")";
        Db.NonQ(command);
    }
}