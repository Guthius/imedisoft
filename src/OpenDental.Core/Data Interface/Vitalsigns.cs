...
public class Vitalsigns
{
    ...
    {
        var command = "SELECT * FROM vitalsign WHERE PatNum = " + SOut.Long(patNum) + " ORDER BY DateTaken";
    ...
    {
        var command = "DELETE FROM vitalsign WHERE VitalsignNum = " + SOut.Long(vitalsignNum);
        Db.NonQ(command);
    }
    ...
    public static float CalcBMI(float weight, float height)
    {
        ...
        var bmi = weight * 703f / (height * height);
        return bmi;
    }
    ...
    public static InterventionCodeSet GetBMIInterventionCode(float bmi, int ageBeforeJanFirst)
    {
        ...
        {
            if (bmi < 18.5) return InterventionCodeSet.BelowNormalWeight;

            if (bmi < 25) return InterventionCodeSet.None;

            return InterventionCodeSet.AboveNormalWeight;
        }

        if (bmi < 23) return InterventionCodeSet.BelowNormalWeight;

        if (bmi < 30) return InterventionCodeSet.None;
    ...
    public static int GetBMIPercentileUsingZScore(float z)
    {
        ...
        for (var i = 0; i < listPercentiles.Count; i++)
    ...
    public static int GetBMIPercentile(float bmi, Patient patient, DateTime dateExam, List<GenderAge_LMS> listGenderAge_LMSs)
    {
        ...
        if (dateExam == DateTime.MinValue || dateExam < patient.Birthdate || patient.Birthdate == DateTime.MinValue) return -1;

        var years = dateExam.Year - patient.Birthdate.Year;
        if (dateExam.Month < patient.Birthdate.Month || (dateExam.Month == patient.Birthdate.Month && dateExam.Day < patient.Birthdate.Day))
            //have not had birthday this year
            years--;

        var months = dateExam.Month - patient.Birthdate.Month;
        if (patient.Birthdate.Day > dateExam.Day) months--;
        ...
        var listFloats = listGenderAge_LMSs.Find(x => x.GenderAge == genderAgeGroup).ListFloats;
        //use (((bmi/M)^L)-1)/(L*S) to get z-score
        var zScore = ((float) Math.Pow(bmi / listFloats[1], listFloats[0]) - 1) / (listFloats[0] * listFloats[2]);
    ...
    public static Vitalsign SetFields(Vitalsign vitalsign, DateTime date, int pulse, float height, float weight, int bpDiastolic, int bpSystolic, int bmiPercentile, Loinc loincHeightCode, Loinc loincWeightCode)
    {
        ...
        if (bmiPercentile > -1 && Loincs.GetByCode("59576-9") != null) vitalsign.BMIExamCode = "59576-9"; //Body mass index (BMI) [Percentile] Per age and gender, only code used for percentile, only visible if under 17 at time of exam

        if (loincHeightCode.LoincNum != 0) vitalsign.HeightExamCode = loincHeightCode.LoincCode;

        if (loincWeightCode.LoincNum != 0) vitalsign.WeightExamCode = loincWeightCode.LoincCode;
    ...
    public static string SetWeightCodes(InterventionCodeSet interventionCodeSet)
    {
        switch (interventionCodeSet)
        {
            case InterventionCodeSet.AboveNormalWeight:
                if (Snomeds.GetByCode("238131007") != null) return "238131007";

                break;
            case InterventionCodeSet.BelowNormalWeight:
                if (Snomeds.GetByCode("248342006") != null) return "248342006";
    ...
    public static long SetPregnancyDisease(Vitalsign vitalsign, long diseaseDefNumPreg)
    {
        ...
        var disease = Diseases.GetOne(vitalsign.PregDiseaseNum);
