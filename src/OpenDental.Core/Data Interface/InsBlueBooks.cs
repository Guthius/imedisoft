				...
				FROM (
					...
					INNER JOIN claimproc claimproc1
						ON claim.ClaimNum=claimproc1.ClaimNum
							AND claimproc1.Status IN ({SOut.Int((int) ClaimProcStatus.Received)},{SOut.Int((int) ClaimProcStatus.Supplemental)})
...
public class BlueBookEstimateData
{
    ...
    public BlueBookEstimateData(List<InsPlan> listInsPlans, List<InsSub> listInsSubs, List<PatPlan> listPatPlans, List<Procedure> listProcedures, List<SubstitutionLink> listSubstitutionLinks)
    {
        ...
        var insSubNumPrimary = PatPlans.GetInsSubNum(listPatPlans, ordinalPrimary);
        var insSub = InsSubs.GetSub(insSubNumPrimary, listInsSubs);
    ...
    private void Initialize(List<Procedure> listProcedures, List<SubstitutionLink> listSubstitutionLinks)
    {
        ...
        if (InsPlanPrimaryDental == null) return;
        if (InsPlanPrimaryDental.PlanType != "") return;
        if (!InsPlanPrimaryDental.IsBlueBookEnabled) return;
        if (PrefC.GetEnum<AllowedFeeSchedsAutomate>(PrefName.AllowedFeeSchedsAutomate) != AllowedFeeSchedsAutomate.BlueBook) return;
        if (listSubstitutionLinks == null) listSubstitutionLinks = SubstitutionLinks.GetAllForPlans(InsPlanPrimaryDental.PlanNum);
        var listProcCodeNums = new List<long>();
        for (var i = 0; i < listProcedures.Count; i++)
        {
            listProcCodeNums.Add(listProcedures[i].CodeNum);
            var procCode = ProcedureCodes.GetProcCode(listProcedures[i].CodeNum).ProcCode;
            var codeNumSubstitution = ProcedureCodes.GetSubstituteCodeNum(procCode, listProcedures[i].ToothNum, InsPlanPrimaryDental.PlanNum, listSubstitutionLinks);
            if (codeNumSubstitution != 0 && codeNumSubstitution != listProcedures[i].CodeNum) listProcCodeNums.Add(codeNumSubstitution);
        }
        ...
        var dateOldestLimit = FillDictRuleLimits();
    ...
    private DateTime FillDictRuleLimits()
    {
        ...
        for (var i = 0; i < ListInsBlueBookRules.Count; i++)
        {
            var insBlueBookRule = ListInsBlueBookRules[i];
            var dateLimit = DateTime.MinValue;
    ...
    public bool IsValidForEstimate(ClaimProc claimProc, bool canSetBlueBookUsed = true)
    {
        ...
        if (InsPlanPrimaryDental == null) return false;
        if (InsPlanPrimaryDental.PlanType != "") return false;
        if (!InsPlanPrimaryDental.IsBlueBookEnabled) return false;
        if (PrefC.GetEnum<AllowedFeeSchedsAutomate>(PrefName.AllowedFeeSchedsAutomate) != AllowedFeeSchedsAutomate.BlueBook) return false;
        if (claimProc.PlanNum != InsPlanPrimaryDental.PlanNum) return false;
        return true;
    }
    ...
    public double GetAllowed(Procedure procedure, Lookup<FeeKey2, Fee> lookupFees, bool isCodeSubstNone, List<SubstitutionLink> listSubstitutionLinks = null)
    {
        ...
        if (!isCodeSubstNone) codeNum = ProcedureCodes.GetSubstituteCodeNum(ProcedureCodes.GetProcCode(codeNum).ProcCode, procedure.ToothNum, InsPlanPrimaryDental.PlanNum, listSubstitutionLinks);
        LogText = "";
        double allowed = -1;
        InsBlueBookRule insBlueBookRule = null;
        for (var i = 0; i < ListInsBlueBookRules.Count; i++)
        {
            //Already ordered by InsBlueBookRule.ItemOrder.
            ...
            if (InsBlueBookRules.IsDateLimitedType(insBlueBookRule))
            {
                ...
                if (insBlueBookRule.RuleType == InsBlueBookRuleType.InsuranceCarrierGroup && CarrierPri.CarrierGroupName == 0) continue;
                //Skip if this rule is deactivated.
                if (insBlueBookRule.RuleType == InsBlueBookRuleType.InsurancePlan && !PrefC.GetBool(PrefName.InsBlueBookUsePlanNumOverride)) continue;
            ...
            {
                if (InsPlanPrimaryDental.ManualFeeSchedNum == 0) continue;
                if (lookupFees != null) listFees = lookupFees[new FeeKey2(codeNum, InsPlanPrimaryDental.ManualFeeSchedNum)].ToList();
        ...
        if (allowed == -1)
        {
            ...
            if (insBlueBookRule.RuleType.In(InsBlueBookRuleType.UcrFee, InsBlueBookRuleType.ManualBlueBookSchedule)) LogText += " No fee for code in fee schedule.";
        }
        ...
        else
        {
            ...
            if (insBlueBookRule.RuleType == InsBlueBookRuleType.UcrFee) LogText += $" ({PrefC.GetInt(PrefName.InsBlueBookUcrFeePercent).ToString()}% of UCR fee)";
            LogText += ".";
        }
    ...
    {
        DictRuleLimits.TryGetValue(insBlueBookRule.InsBlueBookRuleNum, out var dateLimit);
    ...
    /// <summary>
    ///     O(n) algorithm to find median. References: https://en.wikipedia.org/wiki/Selection_algorithm,
    ///     https://stackoverflow.com/questions/4140719/calculate-median-in-c-sharp, Introduction to Algorithms 3rd Edition,
    ///     Corman et al, pp 216
    /// </summary>
    private double GetMedian(List<InsBlueBook> listInsBlueBooks)
    {
        var start = 0;
        var end = listInsBlueBooks.Count - 1;
        var medianIndex = (listInsBlueBooks.Count - 1) / 2;
        while (true)
        {
            var pivotIndex = PartitionHelper(listInsBlueBooks, start, end);
            if (pivotIndex == medianIndex) return listInsBlueBooks[pivotIndex].AllowedOverride;
    ...
    {
        var pivot = listInsBlueBooks[end].AllowedOverride;
        var lastLow = start - 1;
    ...
    {
        if (i == j) return;
        var insBlueBookTemp = listInsBlueBooks[i].Copy();
    ...
    private double GetMostRecent(List<InsBlueBook> listInsBlueBooks)
    {
        ...
        for (var i = 0; i < listInsBlueBooks.Count; i++)
        {
            var insBlueBookCur = listInsBlueBooks[i];
            if (insBlueBookMostRecent == null || insBlueBookCur.ProcDate > insBlueBookMostRecent.ProcDate) insBlueBookMostRecent = insBlueBookCur;
        }
