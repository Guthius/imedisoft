...
public class InsVerifies
{
    ...
    public static InsVerify GetOneByFKey(long fkey, VerifyTypes verifyType)
    {
        ...
        var command = "SELECT * FROM insverify WHERE FKey=" + SOut.Long(fkey) + " AND VerifyType=" + SOut.Int((int) verifyType) + " ORDER BY DateLastVerified DESC LIMIT 1";
    ...
    {
        var command = "SELECT * FROM insverify WHERE InsVerifyNum=" + SOut.Long(insVerifyNum) + "";
        return InsVerifyCrud.SelectOne(command);
    }

    /// <summary>
    ///     Helper method to update or insert an insVerify utilizing the specified planNum or patPlanNum to be used as fKey and
    ///     verifyType.
    ///     If one does not already exist then a new insVerify will be inserted. Otherwise the insVerify is updated.
    /// </summary>
    public static long Upsert(long fKey, VerifyTypes verifyType)
    {
        ...
        {
            if (verifyType == VerifyTypes.InsuranceBenefit) return InsertForPlanNum(fKey);
    ...
    /// <summary>
    ///     Inserts a default InsVerify into the database based on the passed in patplan.  Used when inserting a new patplan.
    ///     Returns the primary key of the new InsVerify.
    /// </summary>
    public static long InsertForPatPlanNum(long patPlanNum)
    {
        ...
        return InsVerifyCrud.Insert(insVerify);
    }

    /// <summary>
    ///     Inserts a default InsVerify into the database based on the passed in insplan.  Used when inserting a new insplan.
    ///     Returns the primary key of the new InsVerify.
    /// </summary>
    ...
    {
        var command = "DELETE FROM insverify WHERE FKey=" + SOut.Long(fkey) + " AND VerifyType=" + SOut.Int((int) verifyType);
        Db.NonQ(command);
    }
    ...
    public static List<InsVerify> GetAll()
    {
        ...
        return InsVerifyCrud.SelectMany(command);
    }

    /// <summary>
    ///     Gets all InsVerifies for a given verifyType and secDateTEdit. Used by the ODAPI, please notify the
    ///     API team before making any modifications to this method.
    /// </summary>
        ...
        , long userNum, string carrierName, bool excludePatVerifyWhenNoIns, bool excludePatClones, List<long> listInsFilingCodeNums)
    {
        ...
        if (listClinicNums.Contains(-1))
        {
            ...
            if (!listRegionDefNums.Contains(0) && !listRegionDefNums.Contains(-1) && listRegionDefNums.Count > 0) //Specific region
                whereClinic += " AND clinic.Region IN(" + string.Join(",", listRegionDefNums.Select(x => SOut.Long(x))) + ") ";
        }
        else if (listClinicNums.Contains(0))
        {
            ...
            if (listClinicNums.Count(x => x != 0) > 0)
            {
                ...
                whereClinic += "(clinic.IsInsVerifyExcluded=0 AND clinic.ClinicNum IN(" + string.Join(",", listClinicNums.Select(x => SOut.Long(x))) + ") ";
                if (!listRegionDefNums.Contains(0) && !listRegionDefNums.Contains(-1) && listRegionDefNums.Count > 0) //Specific region
                    whereClinic += " AND clinic.Region IN(" + string.Join(",", listRegionDefNums.Select(x => SOut.Long(x))) + ") ";
                whereClinic += ")) ";
            }
        ...
        {
            //Specific Clinic
            whereClinic = "AND clinic.IsInsVerifyExcluded=0 AND clinic.ClinicNum IN(" + string.Join(",", listClinicNums.Select(x => SOut.Long(x))) + ") ";
            if (!listRegionDefNums.Contains(0) && !listRegionDefNums.Contains(-1) && listRegionDefNums.Count > 0) //Specific region
                whereClinic += " AND clinic.Region IN(" + string.Join(",", listRegionDefNums.Select(x => SOut.Long(x))) + ") ";
        }
        ...
        var insVerifyJoin2 = @"INNER JOIN insverify ON 
            ...
            : "") + @"))	";
        var whereClause = @"
				WHERE appointment.AptDateTime BETWEEN " + DbHelper.DtimeToDate(SOut.Date(dateStart)) + " AND " + DbHelper.DtimeToDate(SOut.Date(dateEnd.AddDays(1))) + @" 
				AND appointment.AptStatus IN (" + SOut.Int((int) ApptStatus.Scheduled) + "," + SOut.Int((int) ApptStatus.Complete) + @")
				" + (userNum == -1 ? "" : "AND insverify.UserNum=" + SOut.Long(userNum)) + @"
				" + (defNumStatus < 1 ? "" : "AND insverify.DefNum=" + SOut.Long(defNumStatus)) + @"
    ...
    private static string GetInsVerifyCleanupQuery(DateTime dateStart, DateTime dateEndStandard, DateTime dateEndMedicaid)
    {
        ...
        var command = @"SELECT InsVerifyNum
				...
				LEFT JOIN appointment ON appointment.PatNum=insverifies.PatNum
					AND appointment.AptStatus IN (" + SOut.Int((int) ApptStatus.Scheduled) + "," + SOut.Int((int) ApptStatus.Complete) + @")
					AND (((" + DbHelper.DtimeToDate("appointment.AptDateTime") + " BETWEEN " + SOut.Date(dateStart) + " AND " + SOut.Date(dateEndStandard) + @") AND insverifies.FilingCode IN(" + insFilingCodeNumsStandard + @"))"; //insFilingCodeNumsStandard will always contain zero.
        if (insFilingCodeNumsMedicaid != "") //Only add this bit if the list isn't empty, since calling empty IN() statements cause errors.
            command += @"
					OR
					((" + DbHelper.DtimeToDate("appointment.AptDateTime") + " BETWEEN " + SOut.Date(dateStart) + " AND " + SOut.Date(dateEndMedicaid) + @") AND insverifies.FilingCode IN(" + insFilingCodeNumsMedicaid + @"))";
        command += @")
    ...
    /// <summary>
    ///     Called in OpenDentalService.  Attempts to verify patient benefits for same list in FormInsVerificaitonList.cs.
    ///     Only runs for carriers that are flagged with TrustedEtransTypes.RealTimeEligibility.
    /// </summary>
    public static List<InsVerify> TryBatchPatInsVerify(LogWriter logWriter = null)
    {
        //Mimics FormInsVerificaitonList.GetRowsForGrid(...)
        var excludePatVerifyWhenNoIns = PrefC.GetBool(PrefName.InsVerifyExcludePatVerify);
        var excludePatClones = PrefC.GetBool(PrefName.ShowFeaturePatientClone) && PrefC.GetBool(PrefName.InsVerifyExcludePatientClones);
        var dateTimeStart = DateTime.Today.AddDays(-PrefC.GetInt(PrefName.InsVerifyDaysFromPastDueAppt)); //Mimics past due ins verifies logic
        var dateTimeEnd = DateTime.Today.AddDays(PrefC.GetInt(PrefName.InsVerifyAppointmentScheduledDays)); //Non past due logic
        var dateTimeLastPatEligibility = DateTime.Today.AddDays(-PrefC.GetInt(PrefName.InsVerifyPatientEnrollmentDays));
        var dateTimeLastPlanBenefits = DateTime.Today.AddDays(-PrefC.GetInt(PrefName.InsVerifyBenefitEligibilityDays));
        var dateTimeStartMedicaid = DateTime.Today.AddDays(-PrefC.GetInt(PrefName.InsVerifyDaysFromPastDueApptMedicaid));
        var dateTimeEndMedicaid = DateTime.Today.AddDays(PrefC.GetInt(PrefName.InsVerifyAppointmentScheduledDaysMedicaid));
        ...
        var listInsVerifyGridObjects = GetVerifyGridList(dateTimeStart, dateTimeEnd, dateTimeLastPatEligibility, dateTimeLastPlanBenefits
        ...
        if (listInsVerifyGridObjects.Count > 0)
        {
            //Avoid queries/logic if not necessary, but still update PrefName.InsVerifyServiceBatchLastRunDate below.
            ...
            var listInsSubNums = listInsVerifyGridObjects.Where(x => x.InsVerifyPat != null).Select(x => x.InsVerifyPat.InsSubNum).ToList();
        ...
        var listInsVerifies = new List<InsVerify>();
        for (var i = 0; i < listInsVerifyGridObjects.Count; i++)
            try
            {
                if (listInsVerifyGridObjects[i].IsOnlyInsRow() || !dictionaryTrustedCarriers.ContainsKey(listInsVerifyGridObjects[i].InsVerifyPat.CarrierNum))
                {
                    if (listInsVerifyGridObjects[i].InsVerifyPat != null)
                        //We've seen this be null before for a real customer. For now only effect is tied to unit test. Safe to skip if null.
                ...
                {
                    //No error yet.
                    if (etransRequest == null)
                    {
                        //Can happen when an AAA segment is returned.
                        errorStatus = Lans.g("InsVerifyService", "Unexpected carrier response.");
                    }
                    else
                    {
                        //Success, no errors so far and etrans returned
                        var isCoinsuranceInverted = dictionaryTrustedCarriers[listInsVerifyGridObjects[i].InsVerifyPat.CarrierNum].IsCoinsuranceInverted;
                        //AckEtrans and MessageText are not DB columns but are always set in x270.RequestBenefits(...). This is done to avoid queries.
                        var x271 = new X271(etransRequest.AckEtrans.MessageText);
                        //Per NADG we should be considering both in-network and out-of-network benefits
                        var listEb271s = x271.GetListEB(true, isCoinsuranceInverted);
                        listEb271s.AddRange(x271.GetListEB(false, isCoinsuranceInverted));
                        var listBenefitsForPat = Benefits.GetForPlanOrPatPlan(listInsVerifyGridObjects[i].InsVerifyPat.PlanNum, listInsVerifyGridObjects[i].InsVerifyPat.PatPlanNum);
                        ...
                        {
                            var strGroupNumInOd = dictionaryInsPlans[listInsVerifyGridObjects[i].InsVerifyPat.PlanNum].GroupNum;
                            var strGroupNumIn271 = x271.GetGroupNum();
                            errorStatus += ValidateGroupNumber(strGroupNumInOd, strGroupNumIn271);

                            #region Validate Plan dates

                            var datePlanStart = DateTime.MinValue;
                            var datePlanEnd = DateTime.MinValue;
                            var listDTP271sPlanDates = x271.GetListDtpSubscriber();
                            var listDTP271sPlanStartDates = listDTP271sPlanDates.FindAll(x => x.Segment.Get(1) == "539"); //539 => Policy Effective
                            var listDTP271sPlanEndDates = listDTP271sPlanDates.FindAll(x => x.Segment.Get(1) == "540"); //540 => Policy Expiration
                            ...
                            if (listDTP271sPlanStartDates.Count > 0) datePlanStart = X12Parse.ToDate(listDTP271sPlanStartDates.Last().Segment.Get(3)); //Mimics FormInsPlan.butGetElectronic_Click(...)
                            if (listDTP271sPlanEndDates.Count > 0) datePlanEnd = X12Parse.ToDate(listDTP271sPlanEndDates.Last().Segment.Get(3)); //Mimics FormInsPlan.butGetElectronic_Click(...)
                            ...
                            {
                                var doCreateAdjustments = PrefC.GetBool(PrefName.InsBatchVerifyCreateAdjustments);
                                ...
                                var doSetInsuranceHistoryDates = PrefC.GetBool(PrefName.InsBatchVerifyChangeInsHist);
                                if (doSetInsuranceHistoryDates) EB271.SetInsuranceHistoryDates(listEb271s, listInsVerifyGridObjects[i].GetPatNum(), InsSubs.GetOne(listInsVerifyGridObjects[i].InsVerifyPat.InsSubNum));
                            }
                        }
                    }
                }
                ...
                if (string.IsNullOrWhiteSpace(errorStatus))
                {
                    if (listInsVerifyGridObjects[i].IsForMedicaidPlan)
                    {
                        //If this object is for a Medicaid plan, use the Medicaid version of the InsVerify prefs.
                        var apptSchedDays = PrefC.GetInt(PrefName.InsVerifyAppointmentScheduledDaysMedicaid);
                        var insBenefitEligibilityDays = PrefC.GetInt(PrefName.InsVerifyBenefitEligibilityDaysMedicaid);
                        var patEnrollmentDays = PrefC.GetInt(PrefName.InsVerifyPatientEnrollmentDaysMedicaid);
                        InsVerifyOnVerify(listInsVerifyGridObjects[i], apptSchedDays, insBenefitEligibilityDays, patEnrollmentDays);
                    }
                    else
                    {
                        var apptSchedDays = PrefC.GetInt(PrefName.InsVerifyAppointmentScheduledDays);
                        var insBenefitEligibilityDays = PrefC.GetInt(PrefName.InsVerifyBenefitEligibilityDays);
                        var patEnrollmentDays = PrefC.GetInt(PrefName.InsVerifyPatientEnrollmentDays);
                        InsVerifyOnVerify(listInsVerifyGridObjects[i], apptSchedDays, insBenefitEligibilityDays, patEnrollmentDays);
                    }
    ...
    /// <summary>
    ///     Called when an error occurs and we want to update an insVerifyObj.Note.  Currently ony has pat ins verify
    ///     logic.
    /// </summary>
    ...
    /// <summary>
    ///     Called after recieving a valid 271 response when verifying patient insurance benefits.
    ///     Currently only has pat ins verify logic (not insurnace plan logic).
    /// </summary>
    ...
    /// <summary>
    ///     Checks to see if the group number in OD matches the group number in the 271 response. Returns "" for partial
    ///     matches with the insplan group num
    ///     or if no group number was found in the 271.
    /// </summary>
    public static string ValidateGroupNumber(string insPlanGroupNum, string x271GroupNum)
    {
        if (string.IsNullOrWhiteSpace(insPlanGroupNum) || insPlanGroupNum.Length < 3) return Lans.g("InsVerifyService", $"Group number on insurance plan is invalid, current:{insPlanGroupNum}, received:{x271GroupNum}. ");
        ...
        if (x271GroupNum.StartsWith(insPlanGroupNum) || x271GroupNum.EndsWith(insPlanGroupNum)) return "";
        return Lans.g("InsVerifyService", $"Group number mismatch, current:{insPlanGroupNum}, received:{x271GroupNum}. ");
    }

    /// <summary>
    ///     Checks to see if a policy start and policy end date was specified in the given x271 object.
    ///     Will always update InsSub.DateTerm if a valid policy start date was received, but only updates InsSub.DateEffective
    ///     if a valid date was receieved.
    ///     Returns an error string if the patient's appointment date does not fall in the range of the received date(s).
    /// </summary>
    public static string ValidatePlanDates(DateTime datePolicyStart, DateTime datePolicyEnd, InsSub insSub, long aptNum)
    {
        ...
        if (datePolicyStart.Year <= 1880 && datePolicyEnd.Year <= 1880) return "";
        var doChangeEffectiveDates = PrefC.GetBool(PrefName.InsBatchVerifyChangeEffectiveDates);
        //Only update the policy start date if we get a valid value.
        if (datePolicyStart.Year >= 1880 && doChangeEffectiveDates) insSub.DateEffective = datePolicyStart;
        ...
        var dateApt = Appointments.GetOneApt(aptNum).AptDateTime.Date;
        ...
        if (datePolicyStart.Year <= 1880 && datePolicyEnd < dateApt) //No start date, but we have an end date, and plan ended in the past
        ...
        if (datePolicyEnd.Year > 1880 && !DateTime.Today.Between(datePolicyStart, datePolicyEnd)) return Lans.g("InsVerifyService", $"Invalid policy dates: {datePolicyStart.ToShortDateString()} - {datePolicyEnd.ToShortDateString()} ");
        return "";
    }

    /// <summary>
    ///     Given a list of benefits for the patient's plan and a list of benefits received in a 271 response. Determines if
    ///     the general deductible and annual
    ///     max on the patient's insurance plan has a different amount than that received in the 271. Checks individual and
    ///     family coverage level. Returns all errors as a string.
    /// </summary>
    public static string ValidateAnnualMaxAndGeneralDeductible(List<Benefit> listBenefitsForPatPlan, List<Benefit> listBenefitsRecIn271)
    {
        ...
        for (var i = 0; i < listBenefitsForPatPlan.Count; i++)
            ...
            else if (Benefits.IsGeneralDeductible(listBenefitsForPatPlan[i], BenefitCoverageLevel.Family)) benefitGeneralDeductFam = listBenefitsForPatPlan[i];

        //Construct a list of annual max and general deductible benefits specified in the 271. 
        var listBenefitsAnnualMaxInd271 = new List<Benefit>();
        var listBenefitsAnnualMaxFam271 = new List<Benefit>();
        var listBenefitsGeneralDeductInd271 = new List<Benefit>();
        var listBenefitsGeneralDeductFam271 = new List<Benefit>();
        for (var i = 0; i < listBenefitsRecIn271.Count; i++)
        {
            ...
            else if (Benefits.IsGeneralDeductible(listBenefitsRecIn271[i], BenefitCoverageLevel.Family)) listBenefitsGeneralDeductFam271.Add(listBenefitsRecIn271[i]);
        }
        ...
        var stringBuilderErrorStatus = new StringBuilder();
        var doCheckAnnualMax = PrefC.GetBool(PrefName.InsBatchVerifyCheckAnnualMax);
        ...
        var doCheckDeductible = PrefC.GetBool(PrefName.InsBatchVerifyCheckDeductible);
    ...
    /// <summary>
    ///     This method determines whether an insurance adjustment needs to be made given the current benefits in Open Dental
    ///     and the EB271 segments from the 271 response.
    ///     An insurance adjustment will be made if there is an individual general deductible or annual max specified in Open
    ///     Dental and the
    ///     271 response specifies a 'remaining' benefit amount for the associated benefit (deductible or annual max).  Only
    ///     one insurance adjustment will be made.
    /// </summary>
    public static void CreateInsuranceAdjustmentIfNeeded(long patNum, long planNum, long insSubNum, List<Benefit> listBenefitsDb, List<EB271> listEB271s)
    {
        Benefit benefitGeneralDeductInd = null;
        Benefit benefitAnnualMaxInd = null;
        for (var i = 0; i < listBenefitsDb.Count; i++)
            if (Benefits.IsGeneralDeductible(listBenefitsDb[i], BenefitCoverageLevel.Individual))
                benefitGeneralDeductInd = listBenefitsDb[i];
            else if (Benefits.IsAnnualMax(listBenefitsDb[i], BenefitCoverageLevel.Individual)) benefitAnnualMaxInd = listBenefitsDb[i];

        var claimProcInsAdj = ClaimProcs.CreateInsPlanAdjustment(patNum, planNum, insSubNum);
        for (var i = 0; i < listEB271s.Count; i++)
        {
            ...
            {
                var rem271DeductAmt = SIn.Double(listEB271s[i].Segment.Get(7));
                ...
                if (rem271DeductAmt <= benefitGeneralDeductInd.MonetaryAmt) claimProcInsAdj.DedApplied = benefitGeneralDeductInd.MonetaryAmt - rem271DeductAmt;
            }
            ...
            {
                var rem271GenMaxAmt = SIn.Double(listEB271s[i].Segment.Get(7));
                ...
                if (rem271GenMaxAmt <= benefitAnnualMaxInd.MonetaryAmt) claimProcInsAdj.InsPayAmt = benefitAnnualMaxInd.MonetaryAmt - rem271GenMaxAmt;
            }
        }

        if (claimProcInsAdj.DedApplied != 0 || claimProcInsAdj.InsPayAmt != 0) ClaimProcs.Insert(claimProcInsAdj);
    }

    /// <summary>
    ///     Checks the if the received EB271 segment is an individual general deductible 'remaining' benefit.
    ///     Remaining benefit segments are flagged with a time period qualifier value of "29".
    /// </summary>
    ...
    /// <summary>
    ///     Checks the if the received EB271 segment is an individual annual max 'remaining' benefit.
    ///     Remaining benefit segments are flagged with a time period qualifier value of "29".
    /// </summary>
    ...
    /// <summary>
    ///     Calculates the time available to verify based on a variety of patient and appointment factors.
    ///     Looks at Appointment date, appointment creation date, last verification date, next scheduled verification,
    ///     insurance renewal
    /// </summary>
    ...
    /// <param name="dateTimeDaysUntilAppt">
    ///     The date and time to begin considering appointments for verification purposes.
    ///     Calculated DateTime from AptDateTime - PrefName.InsVerifyDaysFromPastDueAppt
    /// </param>
    /// <param name="dateTimeApptLastScheduled">The date and time of the most recent move of the appointment to the schedule.</param>
    /// <param name="dateTimeVerificationExpired">
    ///     The date and time that the verification has become invalid.
    ///     Calculated DateTime from DateTimeLastVerify + (PrefName.InsVerifyBenefitEligibilityDays or
    ///     PrefName.InsVerifyPatientEnrollmentDays)
    /// </param>
    /// <param name="dateBenefitRenewalNeeded">
    ///     The date that the insurance benefit year expires and needs to be renewed
    ///     regardless of the DateTimeLastVerify.
    /// </param>
