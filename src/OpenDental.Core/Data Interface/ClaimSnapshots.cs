...
public class ClaimSnapshots
{
    ...
    {
        if (listClaimProcNums == null || listClaimProcNums.Count == 0) return new List<ClaimSnapshot>();

        var command = "SELECT * FROM claimsnapshot WHERE ClaimProcNum IN(" + string.Join(",", listClaimProcNums.Select(x => SOut.Long(x))) + ")";
        return ClaimSnapshotCrud.SelectMany(command);
    }
    ...
    /// <summary>
    ///     Creates a snapshot for the claimprocs passed in.  Used for reporting purposes.
    ///     If called from Open Dental Service, ignore passed in claimprocs and make snapshots for the entire day of completed
    ///     procedures in a different method.
    ///     When passing in claimprocs, the implementor will need to ensure that only primary claimprocs are being saved.
    ///     Only creates snapshots if the feature is enabled and if the claimproc is of certain statuses.
    /// </summary>
    public static void CreateClaimSnapshot(List<ClaimProc> listClaimProcs, ClaimSnapshotTrigger triggerType, string claimType)
    {
        if (!PrefC.GetBool(PrefName.ClaimSnapshotEnabled)
            || SIn.Enum<ClaimSnapshotTrigger>(PrefC.GetString(PrefName.ClaimSnapshotTriggerType), true) != triggerType)
            return;
        ...
        var dictionaryCompletedProcFees = Procedures.GetProcsFromClaimProcs(listClaimProcs).ToDictionary(x => x.ProcNum, x => x.ProcFee);
        ...
        var listClaimSnapshotsOld = GetByClaimProcNums(listClaimProcs.Select(x => x.ClaimProcNum).ToList());
        ...
        for (var i = 0; i < listClaimProcs.Count; i++)
        {
            ...
            if (listClaimProcs[i].Status.In(ClaimProcStatus.Preauth, ClaimProcStatus.Adjustment, ClaimProcStatus.Estimate, ClaimProcStatus.CapEstimate)) continue;

            //get the procfee
            double procFee;
            if (!dictionaryCompletedProcFees.TryGetValue(listClaimProcs[i].ProcNum, out procFee)) procFee = 0;
            ...
            var claimSnapshotExisting = listClaimSnapshotsOld.Find(x => x.DateTEntry.Date == DateTime.Today.Date
    ...
    {
        var listProceduresCompleted = Procedures.GetCompletedByDateCompleteForDateRange(DateTime.Today, DateTime.Today);
        var listClaimProcs = ClaimProcs.GetForProcsWithOrdinal(listProceduresCompleted.Select(x => x.ProcNum).ToList(), 1)
            .FindAll(x => !x.Status.In(ClaimProcStatus.Preauth, ClaimProcStatus.Adjustment));
        var listPatPlans = PatPlans.GetListByInsSubNums(listClaimProcs.Select(x => x.InsSubNum).ToList());
        listClaimProcs = listClaimProcs
            ...
            .GroupBy(x => new {x.ProcNum, Ordinal = PatPlans.GetOrdinal(x.InsSubNum, listPatPlans.Where(y => y.PatNum == x.PatNum).ToList())})
        ...
        for (var i = 0; i < listClaimProcs.Count; i++)
        {
            ...
            //get the procfee
            double procFee = 0;
            var procedure = listProceduresCompleted.Find(x => x.ProcNum == claimProc.ProcNum);
            if (procedure != null) procFee = procedure.ProcFee;
    ...
    {
        var command = "SELECT COUNT(*) FROM claimsnapshot WHERE ProcNum=" + SOut.Long(claimSnapshot.ProcNum) + " AND ClaimProcNum='" + claimSnapshot.ClaimProcNum + "'";
        if (Db.GetCount(command) != "0") return 0; //Do nothing.

        return ClaimSnapshotCrud.Insert(claimSnapshot);
    }
    ...
    {
        if (listClaimProcNums == null || listClaimProcNums.Count < 1) return;

        var command = "DELETE FROM claimsnapshot WHERE ClaimProcNum IN (" + string.Join(",", listClaimProcNums.Select(x => SOut.Long(x))) + ")";
        Db.NonQ(command);
    }
