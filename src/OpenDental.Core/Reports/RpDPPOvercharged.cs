		...
		public static DataTable GetDPPOvercharged(DateTime dateStart,DateTime dateEnd,List<long> listClinicNums,List<long> listProvNums,long patNum) {
			...
			string query="SELECT payplan.DatePayPlanStart,"
				...
				query+="CASE payplanlink.LinkType "
					+"WHEN "+SOut.Int((int)PayPlanLinkType.Adjustment)+" THEN 'adjustment' "
					+"WHEN "+SOut.Int((int)PayPlanLinkType.Procedure)+" THEN procedurecode.ProcCode "
					+"ELSE '' END AS 'description',"
				+"IF(payplanlink.AmountOverride!=0,'X','') AS 'overridden',"
				+"CASE payplanlink.LinkType "
					+"WHEN "+SOut.Int((int)PayPlanLinkType.Adjustment)+" THEN adjustment.AdjAmt "
					+"WHEN "+SOut.Int((int)PayPlanLinkType.Procedure)+" THEN ROUND(procedurelog.ProcFee*GREATEST(1,procedurelog.BaseUnits+procedurelog.UnitQty)"
				...
				+"IF(payplanlink.AmountOverride!=0,payplanlink.AmountOverride,ROUND("
					+"CASE payplanlink.LinkType "
						+"WHEN "+SOut.Int((int)PayPlanLinkType.Adjustment)+" THEN adjustment.AdjAmt "
						+"WHEN "+SOut.Int((int)PayPlanLinkType.Procedure)+" THEN procedurelog.ProcFee*GREATEST(1,procedurelog.BaseUnits+procedurelog.UnitQty)"
				...
				+"LEFT JOIN (SELECT paysplit.ProcNum,paysplit.AdjNum,SUM(paysplit.SplitAmt) AS 'SumSplitOutside' "
					...
					+"ON (payplanlink.LinkType="+SOut.Int((int)PayPlanLinkType.Procedure)+" AND sumsplitoutside.ProcNum=payplanlink.FKey) "
					+"OR (payplanlink.LinkType="+SOut.Int((int)PayPlanLinkType.Adjustment)+" AND sumsplitoutside.AdjNum=payplanlink.FKey) "
				+"LEFT JOIN (SELECT paysplit.ProcNum,paysplit.AdjNum,paysplit.PayPlanNum,SUM(paysplit.SplitAmt) AS 'SumSplitOnPlan' "
					...
					+"ON ((sumsplitonplan.ProcNum=payplanlink.FKey AND payplanlink.LinkType="+SOut.Int((int)PayPlanLinkType.Procedure)+") "
					+"OR (sumsplitonplan.AdjNum=payplanlink.FKey AND payplanlink.LinkType="+SOut.Int((int)PayPlanLinkType.Adjustment)+")) "
				...
				+"LEFT JOIN (SELECT payplancharge.FKey,payplancharge.LinkType,SUM(payplancharge.Principal) AS 'SumPayPlanCharge' "
					+"FROM payplancharge WHERE payplancharge.ChargeType="+SOut.Int((int)PayPlanChargeType.Debit)+" "
				...
				+"WHERE payplan.DatePayPlanStart BETWEEN "+SOut.Date(dateStart)+" AND "+SOut.Date(dateEnd);
				if(patNum!=0) {//If no Patient is selected, show all patients
					query+=" AND payplan.PatNum="+SOut.Long(patNum);
				}
			DataTable result=ReportsComplex.RunFuncOnReportServer(() => DataCore.GetTable(query));
