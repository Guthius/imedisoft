	...
	public class RpTreatmentFinder {
			...
			string code1,string code2,List<long> listClinicNums,bool useTreatingProvider)
		{
			string command=$@"SELECT procedurelog.PatNum,SUM(procedurelog.ProcFee*(procedurelog.UnitQty+procedurelog.BaseUnits)) AmtPlanned
				...
				AND patient.PatStatus={SOut.Int((int)PatientStatus.Patient)}{(string.IsNullOrEmpty(code1)?"":$@"
				AND procedurecode.ProcCode>='{SOut.String(code1)}'
				AND procedurecode.ProcCode<='{SOut.String(code2)}'")}
				{(dateFrom.Year<=1880?"":$@"AND procedurelog.DateTP>={SOut.DateTime(dateFrom)}")}
				{((dateTo.Year<=1880 || dateFrom>dateTo)?"":$@"AND procedurelog.DateTP<={SOut.DateTime(dateTo)}")}
		...
		public static DataTable GetPatInfo(bool isProcsGeneral,DateTime renewDate,string patNumStr) {
			string commandIndInfo=$@"SELECT patplan.PatPlanNum,
				...
				ORDER BY NULL";//Removes filesort reference from query explain
			return DataCore.GetTable(commandIndInfo);
		}
		...
		public static DataTable GetFamInfo(bool isProcsGeneral,DateTime renewDate,string patNumStr) {
			string commandFamInfo=$@"SELECT claimproc.InsSubNum,
				...
				ORDER BY NULL";//Removes filesort reference from query explain
			return DataCore.GetTable(commandFamInfo);
		}

		private static string GetSelectCoverageStr() {
			return $@"SUM(CASE WHEN claimproc.Status={SOut.Int((int)ClaimProcStatus.NotReceived)} AND claimproc.InsPayAmt=0 "
			       +$@"THEN claimproc.InsPayEst ELSE 0 END) AmtPending,
				SUM(CASE WHEN claimproc.Status IN ({SOut.Int((int)ClaimProcStatus.Received)},{SOut.Int((int)ClaimProcStatus.Adjustment)},"
			       +$@"{SOut.Int((int)ClaimProcStatus.Supplemental)}) THEN claimproc.InsPayAmt ELSE 0 END) AmtUsed
				FROM claimproc";
		}
		...
		private static string GetWhereCoverageStr(bool isProcsGeneral,DateTime renewDate,string patNumStr) {
			return (isProcsGeneral?"":$@"LEFT JOIN procedurelog pl ON pl.ProcNum=claimproc.ProcNum
				...
				")
			       ...
			       +$@"{SOut.Int((int)ClaimProcStatus.Adjustment)},{SOut.Int((int)ClaimProcStatus.Supplemental)})
				AND claimproc.ProcDate BETWEEN {SOut.Date(renewDate)} AND {SOut.Date(renewDate.AddYears(1).AddDays(-1))}
		...
		public static DataTable GetAnnualMaxInfo(string patNumStr) {
			string commandAnnualMax=$@"SELECT benefit.PlanNum,
				...
				WHERE COALESCE(covcat.EbenefitCat,{SOut.Int((int)EbenefitCategory.General)})={SOut.Int((int)EbenefitCategory.General)}
				AND benefit.BenefitType={SOut.Int((int)InsBenefitType.Limitations)} 
				AND benefit.MonetaryAmt>0
				AND benefit.QuantityQualifier={SOut.Int((int)BenefitQuantity.None)}
				AND patplan.PatNum IN ({patNumStr})
				GROUP BY benefit.PlanNum
				ORDER BY NULL";//Removes filesort reference from query explain
			return DataCore.GetTable(commandAnnualMax);
		}
		...
		public static DataTable GetTableRaw(bool noIns,int monthStart,string patNumStr) {
			string command=$@"SELECT patient.PatNum,patient.LName,patient.FName,patient.Email,patient.HmPhone,patient.WirelessPhone,patient.WkPhone,
				...
				AND patplan.Ordinal=1 AND insplan.MonthRenew={SOut.Int(monthStart)}")}";
			return DataCore.GetTable(command);
		}
