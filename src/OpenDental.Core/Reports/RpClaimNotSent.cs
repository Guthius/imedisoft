			...
			,bool hasClaimTypeExpanded,ClaimNotSentStatuses claimStatusFilter) 
		{
			...
			switch(claimStatusFilter) {
				...
				case ClaimNotSentStatuses.Holding:
					claimFilter="AND claim.ClaimStatus IN('H','I')";
					break;
				default://All
			...
			command+="(CASE WHEN claim.ClaimStatus='U' THEN 'Unsent' WHEN "
				+"claim.ClaimStatus='H' THEN 'Hold' WHEN claim.ClaimStatus='I' THEN 'InProc' WHEN claim.ClaimStatus='W' THEN 'WaitQ' ELSE claim.ClaimStatus END) AS ClaimStatus, "
