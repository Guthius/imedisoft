...
namespace OpenDentBusiness.Email
{
    public static class SendEmail
    {
        ...
        ///be null.</summary>
        public static void WireEmailUnsecure(BasicEmailAddress address, BasicEmailMessage emailMessage, NameValueCollection nameValueCollectionHeaders, int emailSendTimeoutMs = EMAIL_SEND_TIMEOUT_MS, params AlternateView[] arrayAlternateViews)
        {
            ...
            else
            {
                ...
                if (isImplicitSsl)
                {
                    //Microsoft CDO supports implicit SSL, System.Net.Mail.SmtpClient only supports explicit SSL.
        ...
        ///<summary>Throws exceptions if failing to send emails or authenticate with Google/Microsoft.</summary>
        private static void SendEmailOAuth(BasicEmailAddress address, BasicEmailMessage message)
        {
            if (address.AuthenticationType == BasicOAuthType.Google)
            {
                ...
                try
                {
                    Message gmailMessage = CreateGmailMsg(address, message);
        ...
        private static Message CreateGmailMsg(BasicEmailAddress emailAddress, BasicEmailMessage emailMessage)
        {
            MimeMessage mimeMsg = CreateMIMEMsg(emailAddress, emailMessage);
            ...
            byte[] raw = Encoding.UTF8.GetBytes(rawString);
