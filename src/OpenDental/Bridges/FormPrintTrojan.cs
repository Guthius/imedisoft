...
public partial class FormPrintTrojan : FormODBase
{
    ...
    private void PrintCustom()
    {
        ...
        printPanel.Clear();
        Invoke(printGenerator, this); //Call the custom printing code.
        //Print page numbers.
        if (pageNumberFont != null)
        {
            ...
            {
                var text = "Page " + (i + 1);
                var size = Graph.MeasureString(text, pageNumberFont);
    ...
    {
        curPrintPage = 0;
        var orient = (landscape ? PrintoutOrientation.Landscape : PrintoutOrientation.Portrait);
