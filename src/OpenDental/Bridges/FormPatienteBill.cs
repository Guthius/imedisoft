...
public partial class FormPatienteBill:FormODBase {
	...
	protected bool isValidCredentials() {
		...
		// 
		var PatienteBillServer = PatienteBillServerAddress;
		var authRequest = @"Function=Auth&Source=STM&Username="+loginID+"&Password="+passWord+
		...
		var trace = new System.Diagnostics.ConsoleTraceListener();
	...
	private bool isValidFolder() {
		try {
			...
			Error("Error Creating Folder "+ex.Message);
		}
		return true;
	}
	#endregion
	...
	private bool PrepareEbill() {
		try {
			...
			var ProcessingInstrction = Doc.CreateProcessingInstruction("xml","version = \"1.0\" standalone=\"yes\"");
			Doc.AppendChild(ProcessingInstrction);
			// Prepare EISStatementFile Root Element
			var EISStatementFile = Doc.CreateNode(XmlNodeType.Element,"EISStatementFile","");
			Doc.AppendChild(EISStatementFile);

			// Prepare VendorID Attribute
			var VendorID = Doc.CreateAttribute("VendorID");
			VendorID.Value = "18";
			EISStatementFile.Attributes.Append(VendorID);

			// Prepare EISStatementFileOutputFormat Attribute
			var EISStatementFileOutputFormat = Doc.CreateAttribute("OutputFormat");
	...
	private void PreparePractice(XmlNode Practice) {
		...
		var PracticeName = Doc.CreateNode(XmlNodeType.Element,"Name","");
		...
		var PracticeAddress1 = Doc.CreateNode(XmlNodeType.Element,"Address1","");
		...
		var PracticeAddress2 = Doc.CreateNode(XmlNodeType.Element,"Address2","");
		...
		var PracticeCity = Doc.CreateNode(XmlNodeType.Element,"City","");
		...
		var PracticeState = Doc.CreateNode(XmlNodeType.Element,"State","");
		...
		var PracticeZip = Doc.CreateNode(XmlNodeType.Element,"Zip","");
		PracticeZip.InnerText = PrefC.GetString(PrefName.PracticeZip);
		Practice.AppendChild(PracticeZip);

		// Format Phone -- Start
		var formatPhone = PrefC.GetString(PrefName.PracticePhone);
		if(formatPhone.Length > 0) {
			formatPhone=TelephoneNumbers.ReFormat(formatPhone);
		}
		// Format Phone -- End

		// Prepare Phone Element for Practice Element
		var PracticePhone = Doc.CreateNode(XmlNodeType.Element,"Phone","");
		...
		var PracticeRemitAddress = Doc.CreateNode(XmlNodeType.Element,"RemitAddress","");
		...
		if(PrefC.GetString(PrefName.PracticeBillingAddress).Length == 0) {
			...
			var RemitAddressName = Doc.CreateNode(XmlNodeType.Element,"Name","");
			...
			var RemitAddressAddress1 = Doc.CreateNode(XmlNodeType.Element,"Address1","");
			...
			var RemitAddressAddress2 = Doc.CreateNode(XmlNodeType.Element,"Address2","");
			...
			var RemitAddressCity = Doc.CreateNode(XmlNodeType.Element,"City","");
			...
			var RemitAddressState = Doc.CreateNode(XmlNodeType.Element,"State","");
			...
			var RemitAddressZip = Doc.CreateNode(XmlNodeType.Element,"Zip","");
			...
			var RemitAddressPhone = Doc.CreateNode(XmlNodeType.Element,"Phone","");
			...
			// Prepare Address1 Element for RemitAddress Element
			var RemitAddressAddress1 = Doc.CreateNode(XmlNodeType.Element,"Address","");
			XmlCDataSection CDataRemitAddressAddress1;
	...
	private void PreapreRendringProvider(XmlNode Practice) {
		...
		if(PrefC.GetString(PrefName.PracticeDefaultProv).Length > 0) {				
			var RenderingTable = Providers.GetDefaultPracticeProvider();
			if(RenderingTable.Rows.Count > 0) {
				RPName = RenderingTable.Rows[0]["FName"] + " " + RenderingTable.Rows[0]["LName"] + " " + RenderingTable.Rows[0]["Suffix"];
				RPLicense = RenderingTable.Rows[0]["StateLicense"].ToString();
			}
			else {
				RPName = PrefC.GetString(PrefName.PracticeTitle);
				RPLicense = "";
			}
		}
		else {
		...
		var PracticeRenderingProvider = Doc.CreateNode(XmlNodeType.Element,"RenderingProvider","");
		Practice.AppendChild(PracticeRenderingProvider);

		// Prepare Name Element for RenderingProvider Element
		var RenderingProviderName = Doc.CreateNode(XmlNodeType.Element,"Name","");
		...
		var RenderingProviderLicenseNumber = Doc.CreateNode(XmlNodeType.Element,"LicenseNumber","");
		...
		var RenderingProviderState = Doc.CreateNode(XmlNodeType.Element,"State","");
	...
	private void PreparePatient(XmlNode Practice) {
		...
		for(var i = 0;i < PatientList.Count;i++) {
			...
			var PatientTable = Patients.GetGuarantorInfo(PatientID);
			...
			var PracticeEisStatement = Doc.CreateNode(XmlNodeType.Element,"EisStatement","");
			Practice.AppendChild(PracticeEisStatement);

			// Prepare EISStatementOutputFormat Attribute
			var EISStatementOutputFormat = Doc.CreateAttribute("OutputFormat");
			...
			var EISStatementCreditCardChoice = Doc.CreateAttribute("CreditCardChoice");
			...
			var EisStatementPatient = Doc.CreateNode(XmlNodeType.Element,"Patient","");
			PracticeEisStatement.AppendChild(EisStatementPatient);

			// Prepare Name Element For Patient Element
			var PatientName = Doc.CreateNode(XmlNodeType.Element,"Name","");
			...
			var PatientAccount = Doc.CreateNode(XmlNodeType.Element,"Account","");
			...
			var PatientAddress1 = Doc.CreateNode(XmlNodeType.Element,"Address1","");
			...
			var PatientAddress2 = Doc.CreateNode(XmlNodeType.Element,"Address2","");
			...
			var PatientCity = Doc.CreateNode(XmlNodeType.Element,"City","");
			...
			var PatientAccountSummary = Doc.CreateNode(XmlNodeType.Element,"AccountSummary","");
			EisStatementPatient.AppendChild(PatientAccountSummary);

			// Prepare PriorStatementDate Element For AccountSummary Element
			var PriorStatementDate = Doc.CreateNode(XmlNodeType.Element,"PriorStatementDate","");
			...
			var DueDate = Doc.CreateNode(XmlNodeType.Element,"DueDate","");
			...
			var StatementDate = Doc.CreateNode(XmlNodeType.Element,"StatementDate","");
			...
			var PriorBalance = Doc.CreateNode(XmlNodeType.Element,"PriorBalance","");
			...
			var RunningBalance = Doc.CreateNode(XmlNodeType.Element,"RunningBalance","");
			...
			var Adjustments = Doc.CreateNode(XmlNodeType.Element,"Adjustments","");
			...
			var NewCharges = Doc.CreateNode(XmlNodeType.Element,"NewCharges","");
			...
			var FinanceCharges = Doc.CreateNode(XmlNodeType.Element,"FinanceCharges","");
			...
			var Credits = Doc.CreateNode(XmlNodeType.Element,"Credits","");
			Credits.InnerText = "0.00";
			PatientAccountSummary.AppendChild(Credits);

			// Prepare EstInsPayments Element For AccountSummary Element
			var EstInsPayments = Doc.CreateNode(XmlNodeType.Element,"EstInsPayments","");
			...
			var PatientShare = Doc.CreateNode(XmlNodeType.Element,"PatientShare","");
			...
			var CurrentBalance = Doc.CreateNode(XmlNodeType.Element,"CurrentBalance","");
			...
			var PastDue30 = Doc.CreateNode(XmlNodeType.Element,"PastDue30","");
			...
			var PastDue60 = Doc.CreateNode(XmlNodeType.Element,"PastDue60","");
			...
			var PastDue90 = Doc.CreateNode(XmlNodeType.Element,"PastDue90","");
			...
			var PastDue120 = Doc.CreateNode(XmlNodeType.Element,"PastDue120","");
	...
	private void PrepareNotes(int PatientID,XmlNode EisStatementPatient) {
		...
		var NoteTable = Statements.GetStatementNotesPracticeWeb(PatientID);
		...
		var PatientNotes = Doc.CreateNode(XmlNodeType.Element,"Notes","");
		EisStatementPatient.AppendChild(PatientNotes);

		// Prepare Note1 Element For Notes Element
		var Note1 = Doc.CreateNode(XmlNodeType.Element,"Note1","");
	...
	private void PrepareDetailItems(int PatientID,XmlNode EisStatementPatient) {
		...
		var PatientDetailItems = Doc.CreateNode(XmlNodeType.Element,"DetailItems","");
		...
		var stmt = Statements.GetStatementInfoPracticeWeb(PatientID);
		...
		var tableAccount = dataSet.Tables["account"];
		string tablename;
		...
		for(var p = 0;p < tableAccount.Rows.Count;p++) {
			...
			var Item = Doc.CreateNode(XmlNodeType.Element,"Item","");
			PatientDetailItems.AppendChild(Item);
			// Prepare Date Element For Item Element
			var ItemDate = Doc.CreateNode(XmlNodeType.Element,"Date","");
			ItemDate.InnerText = date;
			Item.AppendChild(ItemDate);

			// Prepare PatientName Element For Item Element
			var ItemPatientName = Doc.CreateNode(XmlNodeType.Element,"PatientName","");
			...
			var ItemDescription = Doc.CreateNode(XmlNodeType.Element,"Description","");
	...
	private bool isPatientFileCreated() {
		...
		try {
			...
			Error("Patient eBill File Create Error "+ex.Message);
			return false;
		}
	}
	# endregion
	...
	private void Transmit() {
		if(isPatientFileCreated() == true) {
			try {
				...
				var gc_MimeSep = "---------------------------7d13e425b00d0";
				var gc_MimeFunction = "Content-Disposition: form-data; name=\"Function\"";
				var gc_MimeSource = "Content-Disposition: form-data; name=\"Source\"";
				var gc_MimeAuth = "Content-Disposition: form-data; name=\"AuthenticationID\"";
				var gc_MimeFile = "Content-Disposition: form-data; name=\"File\"; filename=\"";
				var tempFile = PrefC.GetRaw("PatienteBillPath") + "\\tmp.xml";
				var RequestStr = "--" + gc_MimeSep + "\r\n" + gc_MimeFunction +
				                 "\r\n\r\n" + "Upload\r\n" + "--" + gc_MimeSep + "\r\n" +
				                 gc_MimeSource + "\r\n\r\n" + "STM\r\n" + "--" + gc_MimeSep + "\r\n"
				                 + gc_MimeAuth + "\r\n\r\n" + AuthenticationID + "\r\n" +
				...
				var xmlTmp = new StreamWriter(tempFile);
				xmlTmp.Write(RequestStr);
				xmlTmp.Write(xmlStream.ReadToEnd());
				xmlTmp.Close();
				xmlStream.Close();

				var xmlUpload = new StreamReader(tempFile);
				var fileContents = Encoding.Default.GetBytes(xmlUpload.ReadToEnd());
				xmlUpload.Close();

				var request1 = (HttpWebRequest)WebRequest.Create(PatienteBillServerAddress + "?" + RequestStr);
				...
				var requestStream = request1.GetRequestStream();
				...
				// Delete temp file
				var trASH = new FileInfo(tempFile);
				trASH.Delete();
				Progress("End Transmit..");
				ProcessFinalResponse(finalResponse);
			}
			catch(Exception ex) {
				Error("Transmit failed. "+ex.Message);
			}
		}
	}
	# endregion
	...
	private void ProcessFinalResponse(string finalResponse) {
		try {
			...
			Error("Error Parsing Final response "+ex.Message);
		}
	}
