...
public partial class PinBoard:Control {
	...
	protected override void Dispose(bool disposing) {
		if(disposing){//managed resources
			...
			for(var i=0;i<ListPinBoardItems.Count;i++) {
				...
				var logText=Lan.g(this,"Deleted from pinboard while closing Open Dental")+": ";
				if(appt.AptDateTime.Year>1880) {
					logText+=appt.AptDateTime+", ";
				}
	...
	protected override void OnPaint(PaintEventArgs pe) {
		...
		if(ListPinBoardItems==null || ListPinBoardItems.Count==0) {
			...
			//This has me stumped.  I shouldn't have to scale by the MS amount to get it to look good.
			using Font font=new("Microsoft Sans Serif", 8.25f);
			var str="Drag\r\nAppointments to\r\nthis Pinboard";
			g.DrawString(str,font,Brushes.Gray,new RectangleF(0,0,Width,Height-20),format);
			//g.DrawString(str,font,Brushes.Gray,0,0);
			return;
		}
		for(var i=0;i<ListPinBoardItems.Count;i++) {
			var locY=i*13;
			g.DrawImage(ListPinBoardItems[i].BitmapAppt,1,locY,ListPinBoardItems[i].BitmapAppt.Width,ListPinBoardItems[i].BitmapAppt.Height);
			if(_selectedIndex==i){
				var dataRow=ListPinBoardItems[i].DataRowAppt;
				Pen penProvOutline;
				...
				var graphicsPathOutline=ControlApptPanel.GetRoundedPath(
	...
	protected override void OnMouseDown(MouseEventArgs e) {
		...
		var index=-1;
		for(var i=ListPinBoardItems.Count-1;i>=0;i--){
			var top=13*i;
		...
		if(e.Button==MouseButtons.Right) {
			...
			var menuItemProv=new MenuItem(Lan.g(this,"Change Provider"));
	...
	void menuItemProv_Click(object sender,EventArgs e) {
		var apt=Appointments.GetOneApt(ListPinBoardItems[SelectedIndex].AptNum);
		if(apt==null) {
		...
		if(apt.AptStatus!=ApptStatus.UnschedList) {
			var message=Providers.CheckApptProvidersTermDates(apt);
			if(message!="") {
		...
		var procsForSingleApt=Procedures.GetProcsForSingle(apt.AptNum,false);
		if(Security.IsAuthorized(EnumPermType.AppointmentResize,suppressMessage:true) && MsgBox.Show(this,MsgBoxButtons.YesNo,"Change length for new provider?")){
			...
			var calcPattern=Appointments.CalculatePattern(apt.ProvNum,apt.ProvHyg,codeNums,true);
		...
		var procFeeHelper=new ProcFeeHelper(apt.PatNum);
		var isUpdatingFees=false;
		var listProcsNew=procsForSingleApt.Select(x => Procedures.ChangeProcInAppointment(apt,x.Copy())).ToList();
	...
	public void AddAppointment(Bitmap bitmap,long aptNum,DataRow dataRow) {
		...
		var pinBoardItem=new PinBoardItem();
		...
		var listAptNums=new List<long> {aptNum};
		pinBoardItem.TableApptFields=Appointments.GetApptFieldsByApptNums(listAptNums);
		var listPatNums=new List<long> {SIn.Long(dataRow["PatNum"].ToString())};
