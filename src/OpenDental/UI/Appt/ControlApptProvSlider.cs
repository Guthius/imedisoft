...
public partial class ControlApptProvSlider : Control{
	...
	protected override void OnResize(EventArgs e){
		...
		//_fontCourier=new Font(FontFamily.GenericMonospace,F(8.5f));
		//_fontTime?.Dispose();
		//_fontTime=new Font(FontFamily.GenericSansSerif,F(8.5f));
		Invalidate();
	}
	...
	private void ToggleProv(int index){
		if(MinPerIncr==10) {
			...
			var strReplace=Pattern.Substring(pos,1)=="X"?"/":"X";//Opposite, either X or /
			var patternNew="";
			if(pos>0){
			...
			var strReplace=Pattern.Substring(pos,1)=="X"?"/":"X";
			var patternNew="";
			if(pos>0){
				patternNew=Pattern.Substring(0,pos);
			}
	...
	private void ToggleProv2(int index){
		...
		if(MinPerIncr==10) {
			...
			var strReplace=PatternSecondary.Substring(pos,1)=="X"?"/":"X";//Opposite, either X or /
			var patternNew="";
			if(pos>0){
			...
			var strReplace=PatternSecondary.Substring(pos,1)=="X"?"/":"X";
			var patternNew="";
			if(pos>0){
				patternNew=PatternSecondary.Substring(0,pos);
			}
	...
	protected override void OnMouseMove(MouseEventArgs e){
		...
		if(_isMouseDown){
			if(_isDraggingSlider){
				var yTravel=_pointMouse.Y-_pointMouseDown.Y;
				var idxTravel=(int)Math.Round(yTravel/_heightRow);
	...
	protected override void OnPaint(PaintEventArgs pe){
		...
		using var fontCourier=new Font(FontFamily.GenericMonospace,8.5f);
		for(var i=0;i<SelectedLength;i++){
			if(_provBarText.Length>i && _provBarText[i]!=' '){
				g.DrawString(_provBarText.Substring(i,1),fontCourier,Brushes.Black,_marginLR+_widthLeft*2+1,i*_heightRow+1);
			}
		}
		//outline-----------------------------------------------------------------------------------
		...
		var graphicsPath=ControlApptPanel.GetRoundedPath(_rectangleFSlider,2);
		if(_rectangleFSlider.Contains(_pointMouse) || _isDraggingSlider){
			g.FillPath(Brushes.White,graphicsPath);//hover effect
		}
		else{
			using(var brush237=new SolidBrush(Color.FromArgb(237,237,237))){
				g.FillPath(brush237,graphicsPath);
			}
		}
		using(var pen=new Pen(_colorSliderOutline)){
			g.DrawPath(pen,graphicsPath);
		}
		graphicsPath.Dispose();
		var timeSpan=TimeSpan.FromMinutes(Pattern.Length*5);
		var strTime=timeSpan.ToString("h':'mm");
		using var fontTime=new Font(FontFamily.GenericSansSerif,8.5f);
		var widthStr=g.MeasureString(strTime,fontTime).Width;//measures smaller unscaled string
		var x=Width/2f-widthStr/2;
		var y=SelectedLength*_heightRow+2;
		//g.DrawRectangle(Pens.Red,x,y,widthStr,heightStr);//for debugging
		g.DrawString(strTime,fontTime,Brushes.Black,x,y);
		//SelectedIndex------------------------------------------------------------------------------
		if(SelectedIndex>-1 && _isCursorFlashOn && this.Focused){
			using(var pen=new Pen(Color.Red,1)){
				g.DrawLine(pen,_marginLR+_widthLeft*2+3,	SelectedIndex*_heightRow+2,
					_marginLR+_widthLeft*2+3, (SelectedIndex+1)*_heightRow-2);
			}
		}
	}
