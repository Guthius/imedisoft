	...
	public partial class ControlApptPanel:UserControl {
		...
		public DateTime DateSelected{
			...
			set{
				...
				if(IsWeeklyView){
					...
					int dayOfWeekInt=(int)_dateSelected.DayOfWeek;
		...
		public float SizeFont{
			set{
				...
				_font=new Font(FontFamily.GenericSansSerif,LayoutManager.ScaleFontODZoom(_sizeFont96));
				_fontCourier=new Font(FontFamily.GenericMonospace,LayoutManager.ScaleFontODZoom(_sizeFont96));
				_fontMinutes=new Font(FontFamily.GenericSansSerif,LayoutManager.ScaleFontODZoom(_sizeFont96));
				_fontMinutesBold=new Font(FontFamily.GenericSansSerif,LayoutManager.ScaleFontODZoom(_sizeFont96),FontStyle.Bold);
				_fontOpsHeaders=new Font(FontFamily.GenericSansSerif,LayoutManager.ScaleFontODZoom(_sizeFont96));
				...
				//}
				using Font fontForHeight=new Font(FontFamily.GenericSansSerif,LayoutManager.ScaleF(_sizeFont96));
		...
		private void LayoutRecalcAfterResize(){
			_widthTime=LayoutManager.ScaleF(37f)*_sizeFont96/8f;
						...
						|| e.X<_widthTime+_widthProv*_listProvsVisible.Count+_widthOpCol*(i+1))//compare to the righthand side of op
					{
						...
						if(true && Clinics.ClinicNum!=0) {//HQ cannot have clinic specialties.
							...
							if(!string.IsNullOrEmpty(clinicSchedNote)) {
								...
								List<DefLink> listSpecialties=DefLinks.GetListByFKey(clinic.Id,DefLinkType.ClinicSpecialty);
		...
		protected override void OnMouseUp(MouseEventArgs e) {
			...
			try {
				...
				if(_isResizingAppt) {
					...
					Schedule overlappingBlockout=Appointments.GetBlockoutsOverlappingNoSchedule(curApt).OrderBy(x => x.StartTime).FirstOrDefault();
				...
				bool isMovingApptToDifferentDay=(appt.AptDateTime.Date!=apptOld.AptDateTime.Date);
		...
		public void PrintPage(object sender,System.Drawing.Printing.PrintPageEventArgs e) {
			...
			_font.Dispose();
			_font=new Font(FontFamily.GenericSansSerif,LayoutManager.ScaleFontODZoom(_sizeFont96));//Reset to default.
			//_heightLine=g.MeasureString("X",_font,100,StringFormat.GenericTypographic).Height;
			using Font fontForHeight=new Font(FontFamily.GenericSansSerif,LayoutManager.ScaleF(_sizeFont96));
			_heightLine=fontForHeight.Height;
		...
		public void RedrawAsNeeded(){
			...
			List<DayOfWeek> listDayOfWeeksOld=ListDayOfWeeks;
			...
			int widthMain, float widthNarrowedOnRight,DateTime dateStart,DateTime dateEnd,List<DayOfWeek> listDayOfWeeks)
		{
			...
			for(int i=0;i<tableAppointments.Rows.Count;i++){
				...
				List<ApptLayoutInfo> listApptLayoutInfosOverlap=listApptLayoutInfos.FindAll(
				...
				for(int k=0;k<listApptLayoutInfosOverlap.Count;k++){
		...
		public void RoundToNearestDateAndOp(int xPos,out DateTime date,out int opIdx,float widthAppt) {
			...
			int maxOps=_listOpsVisible.Count*ListDayOfWeeks.Count;//e.g. 2*7=14
		...
		private Color DesaturateColor(Color color,double saturationPercent=1) {
			byte byteOldR=color.R;
			byte byteOldG=color.G;
			byte byteOldB=color.B;
			double luma=(1/3f*byteOldR)+(1/3f*byteOldG)+(1/3f*byteOldB);//The "gray" portion of the image.
			int newR=(int)(byteOldR + saturationPercent * (luma - byteOldR));
			int newG=(int)(byteOldG + saturationPercent * (luma - byteOldG));
			int newB=(int)(byteOldB + saturationPercent * (luma - byteOldB));
		...
		private int DayOfWeekIdxToInt(int idxDay) {
			DayOfWeek dayOfWeek=ListDayOfWeeks[idxDay];
		...
		private List<DayOfWeek> GetAllDayOfWeeks(){
			List<DayOfWeek> listDayOfWeeks=new List<DayOfWeek>();
			if(PrefC.GetInt(PrefName.ApptWeekViewStartDay)==1) {
		...
		private void SetBitmapProvbars(Graphics gPrinting=null){
			...
			if(_listProvsVisible.Count==0 || IsWeeklyView){
				...
				if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.LessColor) {
					using HatchBrush hatchBrush=new HatchBrush(HatchStyle.BackwardDiagonal,_brushClosed.Color,Color.White);
					...
					using HatchBrush hatchBrush=new HatchBrush(HatchStyle.BackwardDiagonal,color,Color.White);
					gPrinting.FillRectangle(hatchBrush,0,0,_widthProv*_listProvsVisible.Count,_heightMain);
				}
				else {
			...
			for(int i=0;i<listSchedulesForType.Count;i++) {
				...
				for(int o=0;o<listSchedulesForType[i].Ops.Count;o++) {
					if(IsWeeklyView) {
						...
						int dayofweek=_listDayOfWeeks.IndexOf(listSchedulesForType[i].SchedDate.DayOfWeek);
						rect=new RectangleF(
					...
					if(PrefC.GetBool(PrefName.SolidBlockouts)) {
						if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.LessColor) {
							using HatchBrush hatchBrush=new HatchBrush(HatchStyle.ForwardDiagonal,brushBlockBackg.Color,Color.White);
							g.FillRectangle(hatchBrush,rect);
						}
						else if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.Grayscale) {
							Color color=DesaturateColor(brushBlockBackg.Color);
							using HatchBrush hatchBrush=new HatchBrush(HatchStyle.ForwardDiagonal,color,Color.White);
							g.FillRectangle(hatchBrush,rect);
						}
						else{
					...
					using SolidBrush solidBrush=new SolidBrush(_brushBlockText.Color);
					if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.Grayscale) {
						Color color=DesaturateColor(solidBrush.Color);
		...
		private void DrawGridLinesMain(Graphics g){
			...
			if(IsWeeklyView) {
				...
				for(int d=0;d<ListDayOfWeeks.Count;d++) {
		...
		private void DrawMainBackground(Graphics g) {
			...
			if(IsWeeklyView) {
				for(int d=0;d<ListDayOfWeeks.Count;d++) {//for each day of the week displayed
					...
					{
						if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.LessColor) {
							using HatchBrush hatchBrush=new HatchBrush(HatchStyle.DiagonalCross,_brushHoliday.Color,Color.White);
							g.FillRectangle(hatchBrush,d*_widthWeekDay,0,_widthWeekDay,_heightMain);
						}
						else if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.Grayscale) {
							Color color=DesaturateColor(_brushHoliday.Color);
							using HatchBrush hatchBrush=new HatchBrush(HatchStyle.DiagonalCross,color,Color.White);
							g.FillRectangle(hatchBrush,d*_widthWeekDay,0,_widthWeekDay,_heightMain);
						}
						else {
				...
				for(int i=0;i<ListOpsVisible.Count;i++) {//ops per page in day view
					...
					foreach(Schedule schedCur in provSchedsForOp) {
						...
						Color color=Providers.GetColor(schedCur.ProvNum);
						if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.Grayscale) {
							color=DesaturateColor(Providers.GetColor(schedCur.ProvNum));
						}
						using(SolidBrush solidBrushProvColor=new SolidBrush(color)) {
			...
			for(int i=0;i<listSchedulesWebSchedASAP.Count;i++) {
				...
				for(int o=0;o<listSchedulesWebSchedASAP[i].Ops.Count;o++) {
					...
					if(IsWeeklyView) {
						...
						int dayofweek=_listDayOfWeeks.IndexOf(listSchedulesWebSchedASAP[i].SchedDate.DayOfWeek);
					...
					if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.LessColor) {
						using HatchBrush hatchBrush=new HatchBrush(HatchStyle.ForwardDiagonal,Color.Yellow,Color.White);
						g.FillRectangle(hatchBrush,rectF);
					}
					else if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.Grayscale) {
						Color color=DesaturateColor(Color.Yellow);
						using HatchBrush hatchBrush=new HatchBrush(HatchStyle.ForwardDiagonal,color,Color.White);
						g.FillRectangle(hatchBrush,rectF);
					}
					else {
					...
					switch(_provBar[j,i]) {
						...
						case 1:
							if(DesignMode){
								...
								Color color=ListProvsVisible[j].ProvColor;
								if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.Grayscale) {
		...
		private void DrawOpsHeader(Graphics g){
			...
			if(IsWeeklyView) {
				for(int d=0;d<ListDayOfWeeks.Count;d++) {//for each day of the week displayed
					string strDay=DateStart.AddDays(DayOfWeekIdxToInt(d)).ToString("dddd-d");//Wednesday-24
		...
		private void DrawProvBarsHeader(Graphics g){
			...
			for(int i=0;i<ListProvsVisible.Count;i++){
				Color color=ListProvsVisible[i].ProvColor;
				if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.Grayscale) {
		...
		private void DrawMinutes(Graphics g,bool isLeft){
			...
			for(int i=0;i<24;i++) {
				...
				if(isLeft){
					xPos=_widthTime-LayoutManager.ScaleF(19)*_sizeFont96/8f;//align right
				}
				if(MinPerIncr==5) {
		...
		private void DrawOneAppt(Graphics g,DataRow dataRoww,string patternShowing,float widthAppt,float heightAppt,bool showBrokenX=true){
			...
			float radiusCorn=LayoutManager.ScaleF(_radiusCorn);
			...
			for(int i=0;i<patternShowing.Length;i++) {//Info.MyApt.Pattern.Length;i++){
				...
				if(patternShowing.Substring(i,1)=="X") {
					...
					if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.Grayscale) {
						Color color=DesaturateColor(brushProvBackground.Color);
			...
			for(int i=0;i<patternShowing.Length;i++) {
				...
				if(pattern2Showing.Substring(i,1)=="X" && patternShowing.Substring(i,1)=="X") {
					...
					if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.Grayscale) {
						Color color=DesaturateColor(brushProvBackground2.Color);
						using SolidBrush solidBrush=new SolidBrush(color);
		...
		private PointF DrawElement(Graphics g,int idxItem,PointF pointDraw,ApptViewStackBehavior stackBehavior,ApptViewAlignment align,DataRow dataRoww,List<ApptViewItem> listApptViewItems,float widthAppt,float heightAppt) 
		{
			...
			if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.Grayscale) {
				Color color=DesaturateColor(brush.Color);
		...
		private void DrawOutlineSelected(Graphics g){
			...
			for(int i=0;i<TableAppointments.Rows.Count;i++) {
				...
				GraphicsPath graphicsPathOutline=GetRoundedPath(rectangleF,LayoutManager.ScaleF(_radiusCorn));
		...
		private void DrawRedTimeLineMain(Graphics g){
			...
			float yVal=curTimeY;
			if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.Grayscale) {
				Color color=DesaturateColor(_penTimeLine.Color);
		...
		private void BubbleDraw(Point point,int idxAppt) {
			...
			using Font fontBubble=new Font(FontFamily.GenericSansSerif,LayoutManager.ScaleFontODZoom(9));
			using Font fontBubbleHeader=new Font(FontFamily.GenericSansSerif,LayoutManager.ScaleFontODZoom(10),FontStyle.Bold);
			...
			if(listDisplayFieldsBubble.Count > 1) {
				...
				int widthPatientName=(int)LayoutManager.ScaleMS(TextRenderer.MeasureText(dataRow["PatientName"].ToString(),fontBubbleHeader).Width);
				bool isWrapNeeded=(_rectangleBubble.Width < widthPatientName);
		...
		private void ComputeHeightProvOpHeader(){
			_heightProvOpHeaders=LayoutManager.Scale(16)*_sizeFont96/8f;
