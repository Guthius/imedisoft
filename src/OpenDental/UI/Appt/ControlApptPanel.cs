...
public partial class ControlApptPanel:UserControl {
	...
	private List<DayOfWeek> _listDayOfWeeks;
	...
	private SolidBrush _brushBackground=(SolidBrush)SystemBrushes.Control;
	private SolidBrush _brushBlack=(SolidBrush)Brushes.Black;
	private SolidBrush _brushBlockText=(SolidBrush)Brushes.Black;
	private SolidBrush _brushClosed=new SolidBrush(Color.FromArgb(219,219,219));
	private SolidBrush _brushHoliday=new SolidBrush(Color.FromArgb(255,128,128));
	private SolidBrush _brushLinesAndText=(SolidBrush)Brushes.Black;
	private SolidBrush _brushOpen=(SolidBrush)Brushes.White;
	private SolidBrush _brushTextBlack=(SolidBrush)Brushes.Black;
	private SolidBrush _brushTimeBar=(SolidBrush)Brushes.LightGray;
	private SolidBrush _brushWhite=(SolidBrush)Brushes.White;
	...
	public List<DayOfWeek> ListDayOfWeeks {
		get{ return _listDayOfWeeks; }
		set{
			_listDayOfWeeks=value;
		}
	}
	...
	public DateTime DateSelected{
		...
		set{
			...
			if(IsWeeklyView){
				...
				var dayOfWeekInt=(int)_dateSelected.DayOfWeek;
				_dateStart=_dateSelected.AddDays(-dayOfWeekInt).Date;//go back to current Sunday
				if(PrefC.GetInt(PrefName.ApptWeekViewStartDay)==1){//start on Monday
					if(_dateSelected.DayOfWeek==DayOfWeek.Sunday) {
						_dateStart=_dateStart.AddDays(-6).Date;//go back to previous monday
					}
					else{
						_dateStart=_dateStart.AddDays(1);
					}
				}
				_dateEnd=_dateStart.AddDays(6).Date;//Sets _dateEnd to the end of the week.
			}
			else {
	...
	public float MinPerIncr{
		...
		set{
			...
			var timeScrollOriginal=TimeSpan.FromHours(vScrollBar1.Value/_heightLine/_rowsPerHr);//also in RowsPerIncr
			_minPerIncr=value;
	...
	public float RowsPerIncr{
		...
		set{
			...
			var timeScrollOriginal=TimeSpan.FromHours(vScrollBar1.Value/_heightLine/_rowsPerHr);
	...
	public float SizeFont{
		set{
			...
			_font=new Font(FontFamily.GenericSansSerif,_sizeFont96);
			_fontCourier=new Font(FontFamily.GenericMonospace,_sizeFont96);
			_fontMinutes=new Font(FontFamily.GenericSansSerif,_sizeFont96);
			_fontMinutesBold=new Font(FontFamily.GenericSansSerif,_sizeFont96,FontStyle.Bold);
			_fontOpsHeaders=new Font(FontFamily.GenericSansSerif,_sizeFont96);
			...
			//}
			using var fontForHeight=new Font(FontFamily.GenericSansSerif,_sizeFont96);
			_heightLine=fontForHeight.Height;
	...
	public float WidthProvOnAppt{
		...
		set{
			if(value==_widthProvOnAppt){
				return;
			}
			_widthProvOnAppt=value;
			_isValidMain=false;
			RedrawAsNeeded();
		}
	}
	...
	private void ContrApptPanel_MouseWheel(object sender,MouseEventArgs e) {
		...
		var isNeg=e.Delta<0;
		var valueNew=vScrollBar1.Value;
		if(isNeg){
	...
	private void LayoutRecalcAfterResize(){
		_widthTime=37f*_sizeFont96/8f;
		_widthProv=8;
		//SizeFont
		vScrollBar1.Width=17;
	...
	protected override void OnDoubleClick(EventArgs e) {
		...
		if(SelectedAptNum>0) {//on appt
			...
			var appt=Appointments.GetOneApt(SelectedAptNum);
			if(appt==null) {
				OnApptNullFound();
				return;
			}
		...
		//not on apt, so trying to schedule an appointment---------------------------------------------------------------------
			...
			if(ListOpsVisible.Count==0 || ListDayOfWeeks.IsNullOrEmpty()) {//no ops visible.
				return;
			}
			...
			var timeSpanNew=RoundTimeDown(_timeClicked,MinPerIncr);
			var dateTimeStart=DateSelected+timeSpanNew;
	...
	protected override void OnMouseDown(MouseEventArgs e) {
		...
		{//clicked in operatory or weekday header
			...
			var txt="";
			for(var i=0;i<ListOpsVisible.Count;i++){
				   ...
				   || e.X<_widthTime+_widthProv*_listProvsVisible.Count+_widthOpCol*(i+1))//compare to the righthand side of op
				{
					...
					if(true && Clinics.ClinicNum!=0) {//HQ cannot have clinic specialties.
						...
						var clinicSchedNote=clinic.SchedulingNote;
						if(!string.IsNullOrEmpty(clinicSchedNote)) {
							if(txt!="") {
								txt+="\r\n------";
							}
							var listSpecialties=DefLinks.GetListByFKey(clinic.Id,DefLinkType.ClinicSpecialty);
		...
		var timeClicked=YPosToTime(e.Y);
		...
		var idxAppt=HitTestAppt(e.Location);
		...
		var dateTimeClicked=DateSelected+_timeClicked;
		var aptNumOld=SelectedAptNum;
		if(clickedAptNum>0) {//if clicked on an appt
			var patNumNew=SIn.Long(dataRow["PatNum"].ToString());
	...
	protected override void OnMouseUp(MouseEventArgs e) {
		...
		try {
			...
			if(_isResizingAppt) {
				...
				var timeSpanBottom=YPosToTime(contrTempAppt.Bottom-this.Location.Y);
				if(timeSpanBottom==null){
					return;
				}
				var timeSpanBottomRounded=RoundTimeToNearestIncrement(timeSpanBottom.Value,MinPerIncr);					
				//subtract to get the new length of appt
				var dateTimeTempAppt=SIn.DateTime(_dataRowTempAppt["AptDateTime"].ToString());
				var newspan=timeSpanBottomRounded-dateTimeTempAppt.TimeOfDay;
				...
				var newpatternL=Math.Max((int)newspan.TotalMinutes/5,1);
				...
				var pattern=SIn.String(_dataRowTempAppt["Pattern"].ToString());
				...
				var aptNumTemp=SIn.Long(_dataRowTempAppt["AptNum"].ToString());
				if(!PrefC.GetBool(PrefName.ApptsAllowOverlap)){
					var opNumTemp=SIn.Long(_dataRowTempAppt["Op"].ToString());
					for(var i=0;i<TableAppointments.Rows.Count;i++){
						...
						var dateTimeApptInSameOp=SIn.DateTime(TableAppointments.Rows[i]["AptDateTime"].ToString());
				...
				var curApt=Appointments.GetOneApt(aptNumTemp);
				if(curApt==null){
					OnApptNullFound();
					return;
				}
				curApt.Pattern=pattern;
				var datePrevious=curApt.DateTStamp;
				var overlappingBlockout=Appointments.GetBlockoutsOverlappingNoSchedule(curApt).OrderBy(x => x.StartTime).FirstOrDefault();
			...
			if(contrTempAppt.Location.X>this.Right) { //Dragging to pinboard, so place a copy there.
				...
				var patNum=SIn.Long(_dataRowTempAppt["PatNum"].ToString());
			...
			var timeSpanNewRounded=RoundTimeToNearestIncrement(timeSpanNew.Value,MinPerIncr);
			RoundToNearestDateAndOp(contrTempAppt.Location.X-this.Location.X,//passing in as coordinates of this control
				out var dateNew,
				out var opIdx,contrTempAppt.Width);
			if(opIdx<0){
			...
			var isMovingApptToDifferentDay=appt.AptDateTime.Date!=apptOld.AptDateTime.Date;
			if(appt.AptStatus==ApptStatus.Complete && isMovingApptToDifferentDay) {//Completed appointment moving to different day.
				MsgBox.Show(this,"Not allowed to move completed appointments to a different day.");
				return;
			}
	...
	private void DrawPrintingHeaderFooter(Graphics g,int totalPages,float pageWidth,float pageHeight) {
		...
		var yPos=25f;//starting pos
		//Print Title------------------------------------------------------------------------------
		string title;
		string date;
		if(IsWeeklyView) {
			title=Lan.g(this,"Weekly Appointments");
			date=DateStart.DayOfWeek+" "+DateStart.ToShortDateString()
			     +" - "+DateEnd.DayOfWeek+" "+DateEnd.ToShortDateString();
		}
		else {
			title=Lan.g(this,"Daily Appointments");
			date=DateSelected.DayOfWeek+"   "+DateSelected.ToShortDateString();
		}
		var titleFont=new Font("Arial",12,FontStyle.Bold);
		var xTitle = pageWidth/2-g.MeasureString(title,titleFont).Width/2;
		g.DrawString(title,titleFont,Brushes.Black,xTitle,yPos);//centered
		//Print Date--------------------------------------------------------------------------------
		var dateFont=new Font("Arial",8,FontStyle.Regular);
		var xDate = pageWidth/2-g.MeasureString(date,dateFont).Width/2;
		yPos+=20;
		g.DrawString(date,dateFont,Brushes.Black,xDate,yPos);//centered
		//Print Footer-----------------------------------------------------------------------------
		var page=PagesPrinted+1+" / "+totalPages;
		var xPage = pageWidth/2f-g.MeasureString(page,dateFont).Width/2f;
		yPos=pageHeight-35;
	...
	public void PrintPage(object sender,System.Drawing.Printing.PrintPageEventArgs e) {
		_isPrinting=true;
		var rectanglePageBounds=e.PageBounds;
		...
		var rectangleMarginBounds=new Rectangle(30,65,rectanglePageBounds.Width-30-30,rectanglePageBounds.Height-65-40);
		...
		var widthMainOnePage=rectangleMarginBounds.Width-_widthTime*2-widthProvs;
		var widthCol=widthMainOnePage/PrintingColsPerPage;//widthCol should be ignored for WeekView
		if(IsWeeklyView){
		...
		var startHour=DateTimePrintStart.Hour;
		var stopHour=DateTimePrintStop.Hour;
		if(stopHour==0) {
			stopHour=24;
		}
		...
		var hoursPerPage=(int)Math.Max(Math.Truncate(dHoursPerPage),1.0);//rounds down or defaults to 1 hour per page.
		var pagesTall=(int)Math.Ceiling(Math.Max((double)(stopHour-startHour)/hoursPerPage,1.0));//Number of hour blocks than can be displayed on a page.
		var totalPages=pagesAcross*pagesTall;
		...
		else if(dHoursPerPage<.5 && dHoursPerPage>=.25) {//Defaults to 15 min per page.
			dHoursPerPage=.25;
		} 
		else if(dHoursPerPage<.25 && dHoursPerPage>=0.2) {//Defaults to 12 min per page.
			dHoursPerPage=.2;
		}
		...
		var hourEnd=hourBegin+hoursPerPage;
		...
		var colStart=PrintingPageColumn*PrintingColsPerPage;
		var colStop=colStart+PrintingColsPerPage;
		...
		if(PagesPrinted==0){//only on first page
			...
			var yTop=(float)Math.Ceiling(_heightLine*_rowsPerHr*startHour);
			var height=(float)Math.Ceiling(_heightLine*_rowsPerHr*(stopHour-startHour));
			var bitmapClipboard=new Bitmap((int)_widthTime*2+_widthMain+(int)widthProvs,(int)height);
			var graphicsClipboard=Graphics.FromImage(bitmapClipboard);
			graphicsClipboard.InterpolationMode=InterpolationMode.High;
			var rectangleSource=new RectangleF(0,yTop,_widthTime,height);
			var rectangleDest=new RectangleF(0,0,_widthTime,height);	
		...
		var rectangleDestMain=new RectangleF(
		...
		//Prov Bars------------------------------------------------------------------------------------------------------
		if(_showProvBars && _bitmapProvBars!=null){
			var rectangleSourceProvBars=new RectangleF(
				0,
				hourBegin*_heightLine*_rowsPerHr,
				widthProvs,
				(hourEnd-hourBegin)*_heightLine*_rowsPerHr);
			var rectangleDestProvBars=new RectangleF(
		...
		//ProvBars Header---------------------------------------------------------------------------------------------------
		if(_showProvBars && _bitmapProvBarsHeader!=null){
			var rectangleDestProvBarHeader= new RectangleF(
				rectangleMarginBounds.X+_widthTime,
		...
		var retangleDestOpsHeader=new RectangleF(
			rectangleMarginBounds.X+_widthTime+widthProvs,
			rectangleMarginBounds.Y,
		...
		g.ResetTransform();
		//Time Bar Left--------------------------------------------------------------------------------------------------------
		var rectangleSourceTime=new RectangleF(
			0,
			hourBegin*_heightLine*_rowsPerHr,
			_widthTime,
			(hourEnd-hourBegin)*_heightLine*_rowsPerHr);
		var rectangleDestTime=new RectangleF(
		...
		_font.Dispose();
		_font=new Font(FontFamily.GenericSansSerif,_sizeFont96);//Reset to default.
		//_heightLine=g.MeasureString("X",_font,100,StringFormat.GenericTypographic).Height;
		using var fontForHeight=new Font(FontFamily.GenericSansSerif,_sizeFont96);
		_heightLine=fontForHeight.Height;
	...
	public string GetPatternShowing(string dbPattern) {
		...
		for(var i=0;i<dbPattern.Length;i++) {
			for(var j=0;j<RowsPerIncr;j++) {//Example: 10 MinPerIncr, 2 RowsPerIncr, so 12 per hour
	...
	public string GetProduction(List<DataRow> listDataRows,List<long> listOpNums,List<long> listProvNums,DateTime start,DateTime end) {
		...
		int indexProv;
		foreach(var dataRow in listDataRows){//TableAppointments.Rows){
			...
			var opNum=SIn.Long(dataRow["Op"].ToString());
			...
			var clinicNum=SIn.Long(dataRow["ClinicNum"].ToString());
			var productionVal=SIn.Decimal(dataRow["productionVal"].ToString());
			var netProductionVal=SIn.Decimal(dataRow["netProductionVal"].ToString());
			var writeoffPPO=SIn.Decimal(dataRow["writeoffPPO"].ToString());
		...
		var production=grossproduction.ToString("c0");
	...
	public void RedrawAsNeeded(){
		...
		var listDayOfWeeksOld=ListDayOfWeeks;
		ListDayOfWeeks=GetAllDayOfWeeks();//Default 7 days
		List<DayOfWeek> listDayOfWeeksScheduled;
		if(IsWeeklyView && ApptViewCur.OnlyScheduledProvDays) {
			//From the current week's schedule, gets the list of dayOfWeek that have providers scheduled.
			listDayOfWeeksScheduled=ListSchedules
				.Where(x => x.SchedType==ScheduleType.Provider)
				.Select(x => x.SchedDate.DayOfWeek)
				.Distinct().ToList();
			//From the current week's schedule, adds dayOfWeeks that have appointments scheduled.
			for(var i=0;i<TableAppointments.Rows.Count;i++){
				var dayOfWeek=SIn.Date(TableAppointments.Rows[i]["AptDateTime"].ToString()).DayOfWeek;
				var operatoryNum=SIn.Long(TableAppointments.Rows[i]["Op"].ToString());
			...
			ListDayOfWeeks.RemoveAll(x => !listDayOfWeeksScheduled.Contains(x));
		}
		if(!_isValidMain){
			ComputeColWidth();
		}
	...
	public void SetScrollByTime(TimeSpan timeOfDay){
		var valueNew=(int)(timeOfDay.TotalHours*_heightLine*(double)_rowsPerHr);
		if(valueNew<vScrollBar1.Minimum){
			valueNew=vScrollBar1.Minimum;
		}
	...
	public SizeF SetSize(string pattern) {
		...
		if(IsWeeklyView && ListDayOfWeeks.IsNullOrEmpty()) {
			widthSingleAppt=0;//No days are being shown, so width is 0
		}
		//height is based on original 5 minute pattern. Might result in half-rows
		var heightSingleAppt=pattern.Length*_heightLine*RowsPerIncr;
		if(MinPerIncr==10) {
	...
	public static List<ApptLayoutInfo> PlaceAppointments(DataTable tableAppointments,List<Operatory> listOperatories, 
		...
		int widthMain, float widthNarrowedOnRight,DateTime dateStart,DateTime dateEnd,List<DayOfWeek> listDayOfWeeks)
	{
		...
		var listApptLayoutInfos=new List<ApptLayoutInfo>();
		...
		for(var i=0;i<tableAppointments.Rows.Count;i++){
			var dataRow=tableAppointments.Rows[i];
			var strAptDateTime=dataRow["AptDateTime"].ToString();
			var aptDateTime=SIn.Date(strAptDateTime);
			...
			float narrowest=listApptLayoutInfosOverlap.Max(x=>x.OverlapSections);//example 5 to indicate there might be a 1/5 slot open
			...
			for(var k=0;k<listApptLayoutInfosOverlap.Count;k++){
				if(listApptLayoutInfosOverlap[k].OverlapSections==(int)narrowest){
					continue;
				}
				listApptLayoutInfosOverlap[k].OverlapSections=(int)narrowest;
				CreateRectangle(listApptLayoutInfosOverlap[k]);
				var recurseCount=0;
				ProcessOverlaps(listApptLayoutInfos,listApptLayoutInfosOverlap,widthCol,apptLayoutInfo.IdxOp,widthNarrowedOnRight,isWeeklyView,widthWeekDay,widthWeekAppt,recurseCount);
			}
			//Look for an open spot
			var foundSpot=false;
			ApptLayoutInfo apptLayoutInfoNarrow;
			for(var j=0;j<narrowest;j++){//example loop 0 to 4
				...
				if(listApptLayoutInfosOverlap.Exists(x=> Rectangle.Truncate(x.RectangleBounds).IntersectsWith(Rectangle.Truncate(apptLayoutInfoNarrow.RectangleBounds)))){
					continue;
				}
			...
			listApptLayoutInfos.Add(apptLayoutInfo);
			for(var k=0;k<listApptLayoutInfosOverlap.Count;k++){
				listApptLayoutInfosOverlap[k].OverlapSections=(int)narrowest+1;//example 6
				//OverlapPosition remains unchanged.  Haven't yet found a scenario where this fails.
				CreateRectangle(listApptLayoutInfosOverlap[k]);
			}
			var count=0;//will keep track have how deep the ProcessOverlaps recursion has gone
			...
			ProcessOverlaps(listApptLayoutInfos,listApptLayoutInfosOverlap,widthCol,apptLayoutInfo.IdxOp,widthNarrowedOnRight,isWeeklyView,widthWeekDay,widthWeekAppt,count);
		}
		...
		void CreateRectangle(ApptLayoutInfo apptLayoutInfoLocal){
			...
			var y=apptLayoutInfoLocal.RectangleBounds.Y;
			var height=apptLayoutInfoLocal.RectangleBounds.Height;
			...
			if(isWeeklyView){
				dayofweek=listDayOfWeeks.IndexOf((DayOfWeek)apptLayoutInfoLocal.DayOfWeek);
	...
	private static void ProcessOverlaps(List<ApptLayoutInfo> listApptLayoutInfos,List<ApptLayoutInfo> listApptLayoutInfosOverlap,float widthCol,float idxOp,
		float widthNarrowedOnRight,bool isWeeklyView,float widthWeekDay,float widthWeekAppt,int count)
	{
		...
		count++;
		for(var k=0;k<listApptLayoutInfosOverlap.Count;k++){
			for(var i=0;i<listApptLayoutInfos.Count;i++){
				if(listApptLayoutInfos[i].idxInTableAppointments==listApptLayoutInfosOverlap[k].idxInTableAppointments){
					continue;//self
				}
				...
				if(Rectangle.Truncate(listApptLayoutInfos[i].RectangleBounds).IntersectsWith(Rectangle.Truncate(listApptLayoutInfosOverlap[k].RectangleBounds))){
					//there is overlap, so fix it
					//we are leaving the previous overlapping appointment alone, and changing the other one
					if(listApptLayoutInfos[i].OverlapSections<listApptLayoutInfosOverlap[k].OverlapSections){
						...
						listApptLayoutInfos[i].OverlapSections=listApptLayoutInfosOverlap[k].OverlapSections;//size it to match the previous overlapping appointment
					}
					if(isWeeklyView){
					...
					ProcessOverlaps(listApptLayoutInfos, [listApptLayoutInfos[i]],//just pass in this single
	...
	public static TimeSpan RoundTimeDown(TimeSpan timeSpanIn,double minPerIncr){
		var base10Increment=minPerIncr/60d;//For example, 10 min becomes .167
		var totalIncrements=timeSpanIn.TotalHours/base10Increment;//not rounded, this is how many increments since midnight.
		var truncatedIncrements=Math.Truncate(totalIncrements);//rounded down
		var totalHours=truncatedIncrements*base10Increment;//for example, 8:10a would be 8.167
		var timeSpanTruncated=TimeSpan.FromHours(totalHours);//but there might be a stray second
	...
	public static TimeSpan RoundTimeToNearestIncrement(TimeSpan timeSpanIn,double minPerIncr){
		var base10Increment=minPerIncr/60d;//For example, 10 min becomes .167
		var totalIncrements=timeSpanIn.TotalHours/base10Increment;//not rounded, this is how many increments since midnight.
		var roundedIncrements=Math.Round(totalIncrements);
		var totalHours=roundedIncrements*base10Increment;//for example, 8:10a would be 8.167
		var timeSpanRounded=TimeSpan.FromHours(totalHours);//but there might be a stray second
	...
	public bool IsDoubleBooked(Appointment appt) {
		...
		var procsMultApts=Procedures.GetProcsMultApts(listAptNums);
		var listProceduresForOne=Procedures.GetProcsOneApt(appt.AptNum,procsMultApts);
		var listProcCodesDoubleBooked=Appointments.GetDoubleBookedCodes(appt,TableAppointments.Copy(),procsMultApts,listProceduresForOne.ToArray());
	...
	public void RoundToNearestDateAndOp(int xPos,out DateTime date,out int opIdx,float widthAppt) {
		...
		opIdx=-1;
		if(ListOpsVisible.Count==0 || ListDayOfWeeks.IsNullOrEmpty()){
			return;
		}
		var xPosMain=TranslateXContrToMain(xPos);
		if(xPosMain<0 && !IsWeeklyView){//Week view might not currently be displaying DateStart on appointment panel
		...
		if(!IsWeeklyView){
			...
			var midpoint=xPos+widthAppt/2;//coordinates of control, not main bitmap
		...
		var maxOps=_listOpsVisible.Count*ListDayOfWeeks.Count;//e.g. 2*7=14
		if(idxNearestOp==maxOps){//14 not allowed
			idxNearestOp=maxOps-1;//13, which is last op on 7th day
		}
		var idxDay=(int)((float)idxNearestOp/ListOpsVisible.Count); //rounds down.  e.g. 9/2=4.5= day 4
		//impossible for idxDay to be out of range
		date=DateStart.AddDays(DayOfWeekIdxToInt(idxDay));
	...
	public TimeSpan? YPosToTime(int yPos){
		...
		var yMain=yPos-_heightProvOpHeaders+vScrollBar1.Value;//now it's in bitmapMain coordinates
	...
	private Color DesaturateColor(Color color,double saturationPercent=1) {
		var byteOldR=color.R;
		var byteOldG=color.G;
		var byteOldB=color.B;
		double luma=1/3f*byteOldR+1/3f*byteOldG+1/3f*byteOldB;//The "gray" portion of the image.
		var newR=(int)(byteOldR + saturationPercent * (luma - byteOldR));
		var newG=(int)(byteOldG + saturationPercent * (luma - byteOldG));
		var newB=(int)(byteOldB + saturationPercent * (luma - byteOldB));
	...
	private int DayOfWeekIdxToInt(int idxDay) {
		var dayOfWeek=ListDayOfWeeks[idxDay];
		if(PrefC.GetInt(PrefName.ApptWeekViewStartDay)==1) {
			switch(dayOfWeek) {
	...
	private List<DayOfWeek> GetAllDayOfWeeks(){
		var listDayOfWeeks=new List<DayOfWeek>();
		if(PrefC.GetInt(PrefName.ApptWeekViewStartDay)==1) {
			listDayOfWeeks.Add(DayOfWeek.Monday);
			listDayOfWeeks.Add(DayOfWeek.Tuesday);
			listDayOfWeeks.Add(DayOfWeek.Wednesday);
			listDayOfWeeks.Add(DayOfWeek.Thursday);
			listDayOfWeeks.Add(DayOfWeek.Friday);
			listDayOfWeeks.Add(DayOfWeek.Saturday);
			listDayOfWeeks.Add(DayOfWeek.Sunday);
		}
		else {
			listDayOfWeeks.Add(DayOfWeek.Sunday);
			listDayOfWeeks.Add(DayOfWeek.Monday);
	...
	private void SetBitmapMain(Graphics gPrinting=null){
		...
		if(_widthMain<1){
			...
			using(var g=Graphics.FromImage(_bitmapMain)){
				g.SmoothingMode=SmoothingMode.HighQuality;
	...
	private void SetBitmapOpsHeaders(Graphics gPrinting=null){
		...
		if(_widthMain<1){
			...
			using(var g=Graphics.FromImage(_bitmapOpsHeader)){
				g.SmoothingMode=SmoothingMode.HighQuality;
				g.TextRenderingHint=TextRenderingHint.ClearTypeGridFit;
				DrawOpsHeader(g);
			}
	...
	private void SetBitmapProvbars(Graphics gPrinting=null){
		...
		if(_listProvsVisible.Count==0 || IsWeeklyView){
			...
			if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.LessColor) {
				using var hatchBrush=new HatchBrush(HatchStyle.BackwardDiagonal,_brushClosed.Color,Color.White);
				gPrinting.FillRectangle(hatchBrush,0,0,_widthProv*_listProvsVisible.Count,_heightMain);
			}
			else if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.Grayscale) {
				var color=DesaturateColor(_brushClosed.Color);
				using var hatchBrush=new HatchBrush(HatchStyle.BackwardDiagonal,color,Color.White);
				gPrinting.FillRectangle(hatchBrush,0,0,_widthProv*_listProvsVisible.Count,_heightMain);
			}
			else {
			...
			using(var g=Graphics.FromImage(_bitmapProvBars)){
				g.SmoothingMode=SmoothingMode.HighQuality;
	...
	private void SetBitmapProvBarsHeaders(Graphics gPrinting=null){
		...
		if(_listProvsVisible.Count==0){
			...
			using(var g=Graphics.FromImage(_bitmapProvBarsHeader)){
				g.SmoothingMode=SmoothingMode.HighQuality;
	...
	private void SetBitmapTempAppt(){
		...
		var patternShowing=GetPatternShowing(_dataRowTempAppt["Pattern"].ToString());
		...
		using(var g = Graphics.FromImage(_bitmapTempApptSingle)){
			g.SmoothingMode=SmoothingMode.HighQuality;
	...
	private void SetBitmapTimebarL(Graphics gPrinting=null){
		...
		//	return;
		//}
		var sizeTimebar=new Size((int)_widthTime,(int)(_heightLine*24*_rowsPerHr)+1);
		if(gPrinting!=null){
			gPrinting.SmoothingMode=SmoothingMode.HighQuality;
			gPrinting.TextRenderingHint=TextRenderingHint.AntiAlias;
			...
			using(var g=Graphics.FromImage(_bitmapTimebarLeft)){
				g.SmoothingMode=SmoothingMode.HighQuality;
				g.TextRenderingHint=TextRenderingHint.ClearTypeGridFit;
				DrawTimebar(g,true);
	...
	private void SetBitmapTimebarR(Graphics gPrinting=null){
		...
		if(gPrinting!=null){
			...
			using(var g=Graphics.FromImage(_bitmapTimebarRight)){
				g.SmoothingMode=SmoothingMode.HighQuality;
				g.TextRenderingHint=TextRenderingHint.ClearTypeGridFit;
				DrawTimebar(g,false);
				DrawRedTimeLineTimeBar(g);
	...
	private void DrawBackground(Graphics g) {
		if(_isPrinting && PrintColorBehavior.In(ApptPrintColorBehavior.LessColor,ApptPrintColorBehavior.Grayscale)) {
			g.FillRectangle(_brushWhite,0,0,_widthMain,_heightMain);
		}
		else {
	...
	private void DrawBlockouts(Graphics g){
		...
		var listSchedulesForType=Schedules.GetListForType(ListSchedules,ScheduleType.Blockout,0);
		string blockText;
		RectangleF rect;
		for(var i=0;i<listSchedulesForType.Count;i++) {
			using var brushBlockBackg=new SolidBrush(Defs.GetColor(DefCat.BlockoutTypes,listSchedulesForType[i].BlockoutType));
			using var penOutline=new Pen(Defs.GetColor(DefCat.BlockoutTypes,listSchedulesForType[i].BlockoutType),2);
			blockText=Defs.GetName(DefCat.BlockoutTypes,listSchedulesForType[i].BlockoutType)+"\r\n"+listSchedulesForType[i].Note;
			for(var o=0;o<listSchedulesForType[i].Ops.Count;o++) {
				if(IsWeeklyView) {
					if(GetIndexOp(listSchedulesForType[i].Ops[o])==-1) {
						continue;//don't display if op not visible
					}
					var dayofweek=_listDayOfWeeks.IndexOf(listSchedulesForType[i].SchedDate.DayOfWeek);
					rect=new RectangleF(
				...
				if(PrefC.GetBool(PrefName.SolidBlockouts)) {
					if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.LessColor) {
						using var hatchBrush=new HatchBrush(HatchStyle.ForwardDiagonal,brushBlockBackg.Color,Color.White);
						g.FillRectangle(hatchBrush,rect);
					}
					else if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.Grayscale) {
						var color=DesaturateColor(brushBlockBackg.Color);
						using var hatchBrush=new HatchBrush(HatchStyle.ForwardDiagonal,color,Color.White);
						g.FillRectangle(hatchBrush,rect);
					}
					else{
				...
				using var solidBrush=new SolidBrush(_brushBlockText.Color);
				if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.Grayscale) {
					var color=DesaturateColor(solidBrush.Color);
					solidBrush.Color=color;
				}
				DrawStringWithSpacing(g,blockText,_font,solidBrush,rect,_heightLine);
	...
	private void DrawGridLinesMain(Graphics g){
		//Vert
		if(IsWeeklyView) {
			var widthOp=_widthWeekDay/_listOpsVisible.Count;
			for(var d=0;d<ListDayOfWeeks.Count;d++) {
	...
	private void DrawMainBackground(Graphics g) {
		...
		if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.LessColor) {
			using var hatchBrush=new HatchBrush(HatchStyle.BackwardDiagonal,_brushClosed.Color,Color.White);
			g.FillRectangle(hatchBrush,0,0,_widthMain,_heightMain);
		}
		else if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.Grayscale) {
			var color=DesaturateColor(_brushClosed.Color);
			using var hatchBrush=new HatchBrush(HatchStyle.BackwardDiagonal,color,Color.White);
		...
		if(IsWeeklyView) {
			for(var d=0;d<ListDayOfWeeks.Count;d++) {//for each day of the week displayed
				...
				if(ListSchedules.FindAll(x => x.SchedType==ScheduleType.Practice && x.Status==SchedStatus.Holiday) //find all holidays
				   .Any(x => x.SchedDate.DayOfWeek==ListDayOfWeeks[d] 
				             && (x.ClinicNum==0 || true && x.ClinicNum==Clinics.ClinicNum))) //and either practice or for this clinic
				{
					if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.LessColor) {
						using var hatchBrush=new HatchBrush(HatchStyle.DiagonalCross,_brushHoliday.Color,Color.White);
						g.FillRectangle(hatchBrush,d*_widthWeekDay,0,_widthWeekDay,_heightMain);
					}
					else if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.Grayscale) {
						var color=DesaturateColor(_brushHoliday.Color);
						using var hatchBrush=new HatchBrush(HatchStyle.DiagonalCross,color,Color.White);
						g.FillRectangle(hatchBrush,d*_widthWeekDay,0,_widthWeekDay,_heightMain);
					}
					else {
				...
				for(var i=0;i<ListOpsVisible.Count;i++) {//for each operatory visible for this view and day
					provSchedsForOp=Schedules.GetProvSchedsForOp(ListSchedules,ListDayOfWeeks[d],ListOpsVisible[i]);
					//for each schedule assigned to this op
					foreach(var schedCur in provSchedsForOp.Where(x => x.SchedType==ScheduleType.Provider)) {
			...
			if(ListSchedules.FindAll(x => x.SchedType==ScheduleType.Practice && x.Status==SchedStatus.Holiday) //find all holidays
			   .Any(x => x.ClinicNum==0 || true && x.ClinicNum==Clinics.ClinicNum))//for the practice or clinic
			{
				if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.LessColor) {
					using var hatchBrush=new HatchBrush(HatchStyle.DiagonalCross,_brushHoliday.Color,Color.White);
					g.FillRectangle(hatchBrush,0,0,_widthOpCol*ListOpsVisible.Count,_heightMain);
				}
				else if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.Grayscale) {
					var color=DesaturateColor(_brushHoliday.Color);
					using var hatchBrush=new HatchBrush(HatchStyle.DiagonalCross,color,Color.White);
			...
			for(var i=0;i<ListOpsVisible.Count;i++) {//ops per page in day view
				...
				foreach(var schedCur in provSchedsForOp) {
				...
				foreach(var schedCur in provSchedsForOp) {
					...
					if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.Grayscale) {
						color=DesaturateColor(Providers.GetColor(schedCur.ProvNum));
					}
	...
	private void DrawStringWithSpacing(Graphics g,string str,Font font,Brush brush,RectangleF rectangleLayout,float heightLine){
		...
		var rectangleRemaining=rectangleLayout;//the top gets sequentially chopped off this rectangle
		var strRemaining=str;
		while(true){
			var sizeOneRow=new SizeF(rectangleLayout.Width,heightLine);
			...
			g.MeasureString(strRemaining,font,sizeOneRow,stringFormatLeft,out var charactersFitted,out var linesFilled);
			if(linesFilled==0 || charactersFitted==0){//can happen when half height rows can't draw			
				stringFormatLeft.Dispose();
				return;
			}
			//draw one row
			var strDraw=strRemaining.Substring(0,charactersFitted).Trim();//trailing CR would cause text to shift up by 1px, so trim.
	...
	private void DrawWebSchedASAPSlots(Graphics g){
		var listSchedulesWebSchedASAP=Schedules.GetListForType(ListSchedules,ScheduleType.WebSchedASAP,0);
		var penOutline=new Pen(Color.LightYellow,2);
		string txtBlock;
		RectangleF rectF;
		for(var i=0;i<listSchedulesWebSchedASAP.Count;i++) {
			txtBlock=Lan.g(this,"Web Sched ASAP Slot")+"\r\n"+listSchedulesWebSchedASAP[i].Note;
			for(var o=0;o<listSchedulesWebSchedASAP[i].Ops.Count;o++) {
				var timeSpan=listSchedulesWebSchedASAP[i].StopTime-listSchedulesWebSchedASAP[i].StartTime;
				if(IsWeeklyView) {
					if(GetIndexOp(listSchedulesWebSchedASAP[i].Ops[o])==-1) {
						continue;//don't display if op not visible
					}
					var dayofweek=_listDayOfWeeks.IndexOf(listSchedulesWebSchedASAP[i].SchedDate.DayOfWeek);
				...
				if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.LessColor) {
					using var hatchBrush=new HatchBrush(HatchStyle.ForwardDiagonal,Color.Yellow,Color.White);
					g.FillRectangle(hatchBrush,rectF);
				}
				else if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.Grayscale) {
					var color=DesaturateColor(Color.Yellow);
					using var hatchBrush=new HatchBrush(HatchStyle.ForwardDiagonal,color,Color.White);
					g.FillRectangle(hatchBrush,rectF);
				}
				else {
	...
	private void FillWithDiagonalLines(Graphics g,RectangleF rectToFill,Pen linePen,float pixelsBetweenLines) {
		...
		for(var x=rectToFill.X+pixelsBetweenLines;x<rectToFill.Right;x+=pixelsBetweenLines) {
			var y2=rectToFill.Bottom-(x-rectToFill.X);
			var x2=rectToFill.Left+Math.Max(rectToFill.Top-y2,0);
		...
		var offsetY=pixelsBetweenLines-rectToFill.Width%pixelsBetweenLines;
		for(var y=rectToFill.Bottom-offsetY;y>rectToFill.Top;y-=pixelsBetweenLines) {
			var x2=rectToFill.Right-(y-rectToFill.Top);
			var y2=rectToFill.Top+Math.Max(rectToFill.Left-x2,0);
	...
	private void DrawGridLinesProv(Graphics g){
		if(IsWeeklyView) {
			...
			for(var i=0;i<_listProvsVisible.Count;i++) {
	...
	private void DrawProvBarsAppts(Graphics g){
		...
		for(var j=0;j<ListProvsVisible.Count;j++){//_provBar.Length;j++) {
			//loop through increments.  If two rowsPerIncr, then the loop will handle both at the same time
			for(var i=0;i<24*(int)(60/_minPerIncr);i++){//_rowsPerHr
				var rectFill=new RectangleF(_widthProv*j+1,_heightLine*RowsPerIncr*i+1,_widthProv-1,_heightLine*RowsPerIncr-1);
				switch(_provBar[j,i]) {
					case 0:
						break;
					case 1:
	...
	private void DrawOpsHeader(Graphics g){
		if(_isPrinting && PrintColorBehavior.In(ApptPrintColorBehavior.LessColor,ApptPrintColorBehavior.Grayscale)) {
			//Leave it white  g.FillRectangle(_brushWhite,0,0,_widthMain,_heightProvOpHeaders);//redundant
		}
		else {
		...
		if(IsWeeklyView) {
			for(var d=0;d<ListDayOfWeeks.Count;d++) {//for each day of the week displayed
				var strDay=DateStart.AddDays(DayOfWeekIdxToInt(d)).ToString("dddd-d");//Wednesday-24
				var widthText=g.MeasureString(strDay,_fontOpsHeaders).Width;
				var xPos=d*_widthWeekDay+_widthWeekDay/2-widthText/2;//centered
			...
			for(var i=0;i<_listOpsVisible.Count;i++) {//ops per page in day view
				if(i==ListOpsVisible.Count) {//for printing
					break;
				}
				var colorOp=Color.Empty;
				...
				if(colorOp!=Color.Empty){
					if(_isPrinting && PrintColorBehavior.In(ApptPrintColorBehavior.LessColor,ApptPrintColorBehavior.Grayscale)) {
						g.FillRectangle(_brushWhite,i*_widthOpCol,0,_widthOpCol,_heightProvOpHeaders);
					}
					else {
						using var solidBrushProvColor=new SolidBrush(colorOp);
						g.FillRectangle(solidBrushProvColor,i*_widthOpCol,0,_widthOpCol,_heightProvOpHeaders);
					}
				}
				var sizeTextMeasured=g.MeasureString(ListOpsVisible[i].OpName,_fontOpsHeaders);
				var widthText=sizeTextMeasured.Width;
				float xPos;
				...
				var locationText=new PointF(xPos,2);
				var heightText=_heightProvOpHeaders-2;
	...
	private void DrawMinutes(Graphics g,bool isLeft){
		...
		var hFormat=Lans.GetShortTimeFormat(ci);
		string strTime;
		var yHour=0;//This will cause drawing times to always start at the top.
		float xPos;
		for(var i=0;i<24;i++) {
			...
			var sizef=g.MeasureString(strTime,_fontMinutesBold);
			xPos=1;//align left
			if(isLeft){
			...
			if(isLeft){
				xPos=_widthTime-19*_sizeFont96/8f;//align right
			}
			if(MinPerIncr==5) {
	...
	private void DrawTimebar(Graphics g,bool isLeft) {
		if(_isPrinting && PrintColorBehavior.In(ApptPrintColorBehavior.LessColor,ApptPrintColorBehavior.Grayscale)) {
			g.FillRectangle(_brushWhite,0,0,_widthTime,_heightMain);
		}
		else {
	...
	private void DrawAppts(Graphics g){
		_listApptLayoutInfos=PlaceAppointments(TableAppointments,ListOpsVisible,_heightLine,IsWeeklyView,
			MinPerIncr,ListDayOfWeeks.Count,RowsPerIncr,_widthMain,_widthNarrowedOnRight,
			DateStart,DateEnd,ListDayOfWeeks);
		for(var i=0;i<TableAppointments.Rows.Count;i++){
			var dataRow=TableAppointments.Rows[i];
			...
			var pattern=SIn.String(dataRow["Pattern"].ToString());
			var patternShowing=GetPatternShowing(pattern);
			var rect=_listApptLayoutInfos[i].RectangleBounds;
	...
	private void DrawOneAppt(Graphics g,DataRow dataRoww,string patternShowing,float widthAppt,float heightAppt,bool showBrokenX=true){
		...
		Color provColor;
		Color provColor2;
		var listDefs=Defs.GetDefsForCategory(DefCat.AppointmentColors);
		...
		using var brushProvBackground=new SolidBrush(provColor);
		using var brushProvBackground2=new SolidBrush(provColor2);
		...
		var aptDateTime=SIn.DateTime(dataRoww["AptDateTime"].ToString());
		var aptDateTimeArrived=SIn.DateTime(dataRoww["AptDateTimeArrived"].ToString());
		       ...
		       || aptDateTimeArrived.TimeOfDay>TimeSpan.FromHours(0) && aptDateTimeArrived>aptDateTime)) 
		{
			...
			for(var i=0;i<ListApptViewItemRowElements.Count;i++) {
		...
		if(widthAppt<14){//Narrow appointments will look very simple, with no rounded corners or text
			if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.LessColor) {
				if(SIn.Long(dataRoww["AptStatus"].ToString())==(int)ApptStatus.Complete) {
					using var hatchBrush=new HatchBrush(HatchStyle.WideUpwardDiagonal,brushBack.Color,Color.White);
					g.FillRectangle(hatchBrush,0,0,widthAppt,heightAppt);
				}
				else {
					g.FillRectangle(_brushWhite,0,0,widthAppt,heightAppt);
				}
			}
			else if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.Grayscale) {
				if(SIn.Long(dataRoww["AptStatus"].ToString())==(int)ApptStatus.Complete) {
					var color=DesaturateColor(brushBack.Color);
					using var hatchBrush=new HatchBrush(HatchStyle.WideUpwardDiagonal,color,Color.White);
					g.FillRectangle(hatchBrush,0,0,widthAppt,heightAppt);
				}
				else {
					g.FillRectangle(_brushWhite,0,0,widthAppt,heightAppt);
		...
		var radiusCorn=_radiusCorn;
		var graphicsPathRightSide=new GraphicsPath();
		graphicsPathRightSide.AddLine(WidthProvOnAppt,0,widthAppt-radiusCorn,0);//top
		graphicsPathRightSide.AddArc(widthAppt-radiusCorn*2f,0,radiusCorn*2,radiusCorn*2,270,90);//UR
		graphicsPathRightSide.AddLine(widthAppt,radiusCorn,widthAppt,heightAppt-radiusCorn);//right
		graphicsPathRightSide.AddArc(widthAppt-radiusCorn*2f,heightAppt-radiusCorn*2,radiusCorn*2,radiusCorn*2,0,90);//LR
		graphicsPathRightSide.AddLine(widthAppt-radiusCorn,heightAppt,WidthProvOnAppt,heightAppt);//bottom
		//graphicsPathRightSide.AddLine(8f,totalHeight,8f,0);//path doesn't include left
		var graphicsPathLeftSide=new GraphicsPath();
		graphicsPathLeftSide.AddLine(WidthProvOnAppt,heightAppt,radiusCorn,heightAppt);//bottom
		graphicsPathLeftSide.AddArc(0,heightAppt-radiusCorn*2,radiusCorn*2,radiusCorn*2,90,90);//LL
		graphicsPathLeftSide.AddLine(0,heightAppt-radiusCorn,0,radiusCorn);//left
		...
		if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.LessColor) {
			if(SIn.Long(dataRoww["AptStatus"].ToString())==(int)ApptStatus.Complete) {
				...
				g.FillPath(_brushWhite,graphicsPathRightSide);
			}
		}
		else if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.Grayscale) {
			if(SIn.Long(dataRoww["AptStatus"].ToString())==(int)ApptStatus.Complete) {
		...
		for(var i=0;i<patternShowing.Length;i++) {//Info.MyApt.Pattern.Length;i++){
			...
			if(patternShowing.Substring(i,1)=="X") {
				...
				if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.Grayscale) {
					var color=DesaturateColor(brushProvBackground.Color);
					using var solidBrush=new SolidBrush(color);
					g.FillRectangle(solidBrush,0.5f,i*_heightLine+0.5f,WidthProvOnAppt,_heightLine-heightOverflow);
				}
				else {
		...
		var pattern2=SIn.String(dataRoww["PatternSecondary"].ToString());
		var pattern2Showing=GetPatternShowing(pattern2);
		for(var i=0;i<patternShowing.Length;i++) {
			...
			if(pattern2Showing.Substring(i,1)=="X" && patternShowing.Substring(i,1)=="X") {
				...
				if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.Grayscale) {
					var color=DesaturateColor(brushProvBackground2.Color);
					using var solidBrush=new SolidBrush(color);
					g.FillRectangle(solidBrush,WidthProvOnAppt/2-0.5f,i*_heightLine+0.5f,WidthProvOnAppt/2,_heightLine-heightOverflow);
				}
				else {
		...
		var provBarText=dataRoww["ProvBarText"].ToString();
		for(var i=0;i<provBarText.Length;i++) {
		...
		#region Main rows
		var pointDraw=new PointF(WidthProvOnAppt+1,1);
		var elementI=0;
	...
	private PointF DrawElement(Graphics g,int idxItem,PointF pointDraw,ApptViewStackBehavior stackBehavior,ApptViewAlignment align,DataRow dataRoww,List<ApptViewItem> listApptViewItems,float widthAppt,float heightAppt) 
	{
		...
		var isConfirmedCircle=false;
		var isCareCreditStatus=false;
		...
		if(listApptViewItems[idxItem].ApptFieldDefNum>0) {
			var fieldName=ApptFieldDefs.GetFieldName(listApptViewItems[idxItem].ApptFieldDefNum);
			...
			var fieldName=PatFieldDefs.GetFieldName(listApptViewItems[idxItem].PatFieldDefNum);
			for(var i=0;i<TablePatFields.Rows.Count;i++) {
			...
			case EnumApptViewElement.HasDiscount_D:
				...
				break;
			case EnumApptViewElement.HasFamilyBalance:
				text=dataRoww["hasFamilyBalance"].ToString();
				break;
			...
			case EnumApptViewElement.PatientWard:
				text="Patient Ward: "+dataRoww["patientWard"];
				break;
			...
			case EnumApptViewElement.ProcsColored:
				var value=dataRoww["procsColored"].ToString();
				...
				var lines=value.Split(["</span>"],StringSplitOptions.RemoveEmptyEntries);
				var tempPt=new PointF(pointDraw.X,pointDraw.Y);
				var lastH=0;
				for(var i=0;i<lines.Length;i++) {
					var rgbInt="";
					var proc=lines[i];
					var m=Regex.Match(lines[i],"^<span color=\"(-?[0-9]*)\">(.*)$");
					if(m.Success) {
						rgbInt=m.Result("$1");
						proc=m.Result("$2");
					}
					...
					var c=Color.FromArgb(SIn.Int(rgbInt,false));
					...
					var procRect=new RectangleF(tempPt,procSize);
		...
		var brush=new SolidBrush(listApptViewItems[idxItem].ElementColor);
		if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.Grayscale) {
			var color=DesaturateColor(brush.Color);
		...
		if(align==ApptViewAlignment.Main) {//always stacks vertical
			if(isConfirmedCircle) {
				var sizeDesired=new Size((int)_heightLine,(int)_heightLine);
				using(var bitmapIcon=DrawElementGetIcon(sizeDesired, dataRoww)) {
				...
				var sizeDesired=new Size((int)_heightLine,(int)_heightLine);
				using(var bitmapIcon=DrawElementGetIcon(sizeDesired, dataRoww, text)) {
					g.DrawImage(bitmapIcon,pointDraw.X+1,pointDraw.Y);
				}
				DisposeObjects(brush,stringFormatLeft);
				return new PointF(pointDraw.X,pointDraw.Y+sizeDesired.Height);
				...
				if(isInsuranceColor) {
					...
					var size1=g.MeasureString(ins1,_font,sizeLayoutArea);
					if(dataRoww["insColor1"].ToString()!="") {
						var color=Color.FromArgb(SIn.Int(dataRoww["insColor1"].ToString()));
						if(color!=Color.Black && color!=Color.FromArgb(255,Color.Black)) {
							var pt=new PointF(pointDraw.X,pointDraw.Y-1);
							size1.Height-=1;
							var rectF=new RectangleF(pt,size1);	
							if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.Grayscale) {
								color=DesaturateColor(color);
								using var hatchBrush=new HatchBrush(HatchStyle.BackwardDiagonal,color,Color.White);
								g.FillRectangle(hatchBrush,rectF);
								...
								using var solidBrush=new SolidBrush(color);
								g.FillRectangle(solidBrush,rectF);
							}
						}
					}
					if(dataRoww["insColor2"].ToString()!="" && ins2!="") {
						var color=Color.FromArgb(SIn.Int(dataRoww["insColor2"].ToString()));
						if(color!=Color.Black && color!=Color.FromArgb(255,Color.Black)) {
							var pt=new PointF(pointDraw.X,pointDraw.Y-1+size1.Height);
							var size=g.MeasureString(ins2,_font,sizeLayoutArea);
							size.Height-=1;
							var rectF=new RectangleF(pt,size);
							if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.Grayscale) {
								color=DesaturateColor(color);
								using var hatchBrush=new HatchBrush(HatchStyle.BackwardDiagonal,color,Color.White);
								g.FillRectangle(hatchBrush,rectF);
		...
		#region UR
			...
			if(stackBehavior==ApptViewStackBehavior.Vertical) {
				var w=widthAppt-WidthProvOnAppt;
				if(isConfirmedCircle) {
					var sizeDesired=new Size((int)_heightLine,(int)_heightLine);
					var drawLocThis=new PointF(pointDraw.X-(int)sizeDesired.Width,pointDraw.Y+1);//upper left corner of this element
					using(var bitmapIcon=DrawElementGetIcon(sizeDesired, dataRoww)) {
					...
					var drawLocThis=new PointF(pointDraw.X-(int)sizeNote.Width,pointDraw.Y);//upper left corner of this element
				...
				if(isConfirmedCircle) {
					...
					var w=pointDraw.X-WidthProvOnAppt;
					sizeNote=g.MeasureString(text,_font,(int)w);
					sizeNote=new SizeF(sizeNote.Width,_heightLine+1);//only allowed to be one line high.
					if(sizeNote.Width<5) {
						sizeNote=new SizeF(5,sizeNote.Height);
		...
		#region LR
			...
			if(stackBehavior==ApptViewStackBehavior.Vertical) {
				...
				if(isConfirmedCircle) {
					var sizeDesired=new Size((int)_heightLine,(int)_heightLine);
					var drawLocThis=new PointF(pointDraw.X-(int)sizeDesired.Width,pointDraw.Y+1-_heightLine);//upper left corner of this element
					using(var bitmapIcon=DrawElementGetIcon(sizeDesired, dataRoww)) {
						g.DrawImage(bitmapIcon,drawLocThis);
					}
					DisposeObjects(brush,stringFormatLeft);
					...
					//g.MeasureString(text,baseFont,noteSize,format,out charactersFitted,out linesFilled);
					var drawLocThis=new PointF(pointDraw.X-(int)sizeNote.Width,pointDraw.Y-_heightLine);//upper left corner of this element
					rectangleLayout=new RectangleF(drawLocThis,sizeNote);
					rectangleLayout.Y+=2;
					rectBack=new RectangleF(drawLocThis.X,drawLocThis.Y+1,sizeNote.Width,_heightLine);
					if(listApptViewItems[idxItem].ElementDesc==EnumApptViewElement.MedOrPremed_plus.GetDescription()
				...
				var w=pointDraw.X-WidthProvOnAppt;//drawLoc is upper right of each element.  The first element draws at (totalWidth-1,0).
				if(isConfirmedCircle) {
					var sizeDesired=new Size((int)_heightLine,(int)_heightLine);
					if(pointDraw.X-sizeDesired.Width<=0) {
						return new PointF(pointDraw.X-sizeDesired.Width,pointDraw.Y);
					}
					var drawLocThis=new PointF(pointDraw.X-(int)sizeDesired.Width+1,pointDraw.Y+1-_heightLine);//upper left corner of this element
					using(var bitmapIcon=DrawElementGetIcon(sizeDesired, dataRoww)) {
						g.DrawImage(bitmapIcon,drawLocThis);
					}
					DisposeObjects(brush,stringFormatLeft);
					...
					var drawLocThis=new PointF(pointDraw.X-(int)sizeNote.Width,(int)(pointDraw.Y-_heightLine));//upper left corner of this element
					rectangleLayout=new RectangleF(drawLocThis,sizeNote);
					rectangleLayout.Y+=2;
					rectBack=new RectangleF(drawLocThis.X,drawLocThis.Y+1,sizeNote.Width,_heightLine);
					if(listApptViewItems[idxItem].ElementDesc==EnumApptViewElement.MedOrPremed_plus.GetDescription()
	...
	private Bitmap DrawElementGetIcon(Size sizeDesired, DataRow dataRow, string careCreditStatus="") {
		...
		var color=Defs.GetColor(DefCat.ApptConfirmed,SIn.Long(dataRow["Confirmed"].ToString()));
		var penOutline=new Pen(Color.FromArgb(00,00,00));//Black
		var brushIcon=new SolidBrush(color);
		if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.Grayscale) {
			color=DesaturateColor(color);
			brushIcon=new SolidBrush(color);
		}
		var bitmapIcon=new Bitmap(sizeDesired.Width+1,sizeDesired.Height+1);//Make the bitmap 1 pixel larger so that the ellipse isn't being cut off
		using(var g=Graphics.FromImage(bitmapIcon)) {
	...
	private void DrawOutlineSelected(Graphics g){
		...
		for(var i=0;i<TableAppointments.Rows.Count;i++) {
			...
			var graphicsPathOutline=GetRoundedPath(rectangleF,_radiusCorn);
	...
	private void DrawRedTimeLine(Graphics g){
		var curTimeY=(float)DateTime.Now.TimeOfDay.TotalHours*_heightLine*_rowsPerHr;
		var yVal=TranslateYMainToContr(curTimeY);
	...
	private void DrawRedTimeLineMain(Graphics g){
		...
		var yVal=curTimeY;
		if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.Grayscale) {
			var color=DesaturateColor(_penTimeLine.Color);
			using var pen=new Pen(color);
			g.DrawLine(pen,0,yVal,_widthMain,yVal);
		}
		else {
	...
	private void DrawRedTimeLineProv(Graphics g){
		...
		if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.Grayscale) {
			var color=DesaturateColor(_penTimeLine.Color);
			using var pen=new Pen(color);
			g.DrawLine(pen,0,yVal,_widthProv*_listProvsVisible.Count,yVal);
		}
		else {
	...
	private void DrawRedTimeLineTimeBar(Graphics g){
		...
		if(_isPrinting && PrintColorBehavior==ApptPrintColorBehavior.Grayscale) {
			var color=DesaturateColor(_penTimeLine.Color);
			using var pen=new Pen(color);
			g.DrawLine(pen,0,yVal,_widthTime,yVal);
		}
		else {
	...
	private void BubbleDraw(Point point,int idxAppt) {
		...
		using var fontBubble=new Font(FontFamily.GenericSansSerif,9);
		using var fontBubbleHeader=new Font(FontFamily.GenericSansSerif,10,FontStyle.Bold);
		var listDisplayFieldsBubble=DisplayFields.GetForCategory(DisplayFieldCategory.AppointmentBubble);
		//most data is already present in TableAppointment, but we do need to get the patient picture
		var showingPatientPicture=listDisplayFieldsBubble.Any(x => x.InternalName=="Patient Picture");
		var isNameWrapping=false;//This boolean is used to adjust the Y location of the patient picture.
		if(listDisplayFieldsBubble.Count > 1) {
			var displayFieldFirst=listDisplayFieldsBubble[0];
			var displayFieldSecond=listDisplayFieldsBubble[1];
			var widthPatientName=(int)TextRenderer.MeasureText(dataRow["PatientName"].ToString(),fontBubbleHeader).Width;
			var isWrapNeeded=_rectangleBubble.Width < widthPatientName;
		...
		var bitmap=new Bitmap(_rectangleBubble.Width,800);
		...
		g.SmoothingMode=SmoothingMode.HighQuality;
		using(var brushBackColor = new SolidBrush(Color.FromArgb(255,250,190))) {
			g.FillRectangle(brushBackColor,0,0,bitmap.Width,bitmap.Height);
		}
		Bitmap bitmapPatPict=null;
		if(showingPatientPicture) {
			var imageFolder=dataRow["ImageFolder"].ToString();
			if(imageFolder!=""
		...
		var rectanglePict=new Rectangle(6,(int)imageOffset,100,100);//Always draws in the same spot, unless the pat name wrapped
		//Draw the picture later, for the minor advantage of drawing on top of any horizontal line
		#region infoBubble Text
		var brush=Brushes.Black;//System brush, so we don't need to dispose.
		float x=0;
		float y=0;
		float h=0;
		string s;
		var font=fontBubbleHeader;//for first row
		for(var i=0;i<listDisplayFieldsBubble.Count;i++) {
			...
			if(CompareFloat.IsEqual(x,8)){//This is the second line
				x=110;
				y=14;
				font=fontBubble;
				if(!showingPatientPicture) { 
			...
			if(x==0){//This is first line
				x=8;//magic number. Ugh.
			}
			if(showingPatientPicture && y>=imageOffset+rectanglePict.Height) {
				x=2;
			}
			var widthBubble=_rectangleBubble.Width;//more concise
			switch(listDisplayFieldsBubble[i].InternalName) {
				...
				case "Provider":
					...
					continue;
				case "Production":
					s=Lan.g(this,"Production:")+" "+dataRow["production"];
					h=g.MeasureString(s,font,widthBubble-(int)x).Height;
					g.DrawString(s,font,brush,new RectangleF(x,y,widthBubble-(int)x,h));
					y+=h;
					continue;
				case "Confirmed":
					...
					continue;
				case "ASAP":
					var priority=(ApptPriority)SIn.Int(dataRow["Priority"].ToString());
				...
				case "Note":
					...
					var maxNoteLength=PrefC.GetInt(PrefName.AppointmentBubblesNoteLength);
				...
				case "Horizontal Line":
					if(showingPatientPicture && y<100+imageOffset) { //height of pat picture is 100 plus the image offset
						y=100+imageOffset;
					}
					y+=3;
					var penGray=new Pen(Brushes.Gray,1.5f);
				...
				case "Insurance Color":
					s=dataRow["insurance"].ToString();
					if(s!="") {
						var insuranceArray=s.Split(["\r\n"],StringSplitOptions.None);
						foreach(var str in insuranceArray) {
							h=g.MeasureString(str,font,widthBubble-(int)x).Height;
							var carrier=Carriers.GetCarrierByName(str.Replace("Ins1: ","").Replace("Ins2: ",""));
				...
				case "Estimated Patient Portion":
					var decimalPatPort=SIn.Decimal(dataRow["estPatientPortionRaw"].ToString());
					...
					continue;					
				case "Net Production":
					var decimalPort=SIn.Decimal(dataRow["netProductionVal"].ToString());
		...
		if(showingPatientPicture) { //only draw the pat picture if the display field is present
			using(var brushBackColor = new SolidBrush(Color.FromArgb(232,220,190))){
				g.FillRectangle(brushBackColor,rectanglePict);
			}
			if(bitmapPatPict==null){
				var stringFormat=new StringFormat();
				stringFormat.Alignment=StringAlignment.Center;
				var rectangleFUnavail=new RectangleF(rectanglePict.X,rectanglePict.Y+40,rectanglePict.Width,30);
				...
				g.DrawImage(bitmapPatPict,6,imageOffset,100,100);
		...
		using(var gfx=Graphics.FromImage(_bitmapBubble)){
	...
	private int XPosToOpIdx(int xPos) {
		...
		if(IsWeeklyView) {
			...
			if(PrefC.GetInt(PrefName.ApptWeekViewStartDay)==1) {
				day+=1;//Shifts day to match DayOfWeek Object 0-6 Sun-Mon.
				if(day>6) {
					day=0;
				}
			}
			retVal=(int)Math.Floor((xPos-_widthTime-ListDayOfWeeks.IndexOf((DayOfWeek)day)*_widthWeekDay)/_widthWeekAppt);
		}
		else {
	...
	private int XPosToDay(int xPos) {
		...
		var retVal=(int)Math.Floor((xPos-_widthTime)/_widthWeekDay);
		retVal=DayOfWeekIdxToInt(retVal);
		return retVal;
	}

	///<summary>Called once from RedrawAsNeeded.  Super fast.</summary>
	private void ComputeColWidth() {
		if(_listOpsVisible.Count==0 || ListDayOfWeeks.IsNullOrEmpty()) {
			_widthOpCol=0;
			if(IsWeeklyView) {
		...
		if(_isPrinting){
			...
			_widthWeekDay=(float)_widthMain/ListDayOfWeeks.Count;
			_widthWeekAppt=_widthWeekDay/_listOpsVisible.Count;
			return;
		}
		if(IsWeeklyView) {//no hscroll
			_widthWeekDay=(float)_widthMainVisible/ListDayOfWeeks.Count;
			_widthWeekAppt=_widthWeekDay/_listOpsVisible.Count;
			_widthMain=_widthMainVisible;
			hScrollBar1.Value=0;
		...
		if(hScrollBar1.Value>hScrollBar1.Maximum-hScrollBar1.LargeChange){
			var hval=hScrollBar1.Maximum-hScrollBar1.LargeChange;
			if(hval<0){
				hval=0;
			}
	...
	private void ComputeHeightProvOpHeader(){
		_heightProvOpHeaders=16*_sizeFont96/8f;
		for(var i=0;i<_listOpsVisible.Count;i++){
	...
	private void ProvBarShading(DataRow dataRow) {
		var patternShowing=GetPatternShowing(dataRow["Pattern"].ToString());//this includes multiple per incr. Example: 10 MinPerIncr, 2 RowsPerIncr, so 12 per hour
		var pattern2Showing=GetPatternShowing(dataRow["PatternSecondary"].ToString());
		var indexProv=-1;
		var indexProv2=-1;
		...
		var aptDateTime=SIn.DateTime(dataRow["AptDateTime"].ToString());
		//this is not really a yPos because it's ignoring RowsPerIncr
		var yPos=(aptDateTime.Hour*60/MinPerIncr+aptDateTime.Minute/MinPerIncr)*_heightLine;//*RowsPerIncr;
		var startIndex=(int)(yPos/_heightLine);//rounds down
		for(var k=0;k<patternShowing.Length;k+=(int)RowsPerIncr) {
		...
		for(var k = 0;k<pattern2Showing.Length;k+=(int)RowsPerIncr) {
	...
	private int HitTestAppt(Point point) {
		...
		var pointF=TranslateContrToMain(point);
		for(var i=0;i<_listApptLayoutInfos.Count;i++){
	...
	private bool HitTestApptBottom(Point point,int idxAppt) {
		...
		var yPos=point.Y-_heightProvOpHeaders+vScrollBar1.Value;
	...
	private string GetOperatoryProduction(long operatoryNum) {
		if(!ListApptViewItemRowElements.Exists(x => x.ElementDesc.In("Production","NetProduction"))){
			return"";
		}
		...
		return "Production: "+GetProduction(
			...
			[operatoryNum], [],DateStart,DateEnd);
	}
			...
			for(var i=0;i<ListOpsVisible.Count;i++){
				   ...
				   || point.X+hScrollBar1.Value<_widthTime+_widthProv*_listProvsVisible.Count+_widthOpCol*(i+1))//compare to the righthand side of op
				{
					...
					if(_opNumHeaderTipLast==ListOpsVisible[i].OperatoryNum){
						...
						txt=ListOpsVisible[i].OpName;
						var opProduction=GetOperatoryProduction(ListOpsVisible[i].OperatoryNum);
						if(opProduction!=""){
							txt+="\r\n"+opProduction;
						}
	...
	private void panelOpHoverTip_MouseMove(object sender,System.Windows.Forms.MouseEventArgs e) {
		...
		var point=new Point(e.X+panelHeaderTip.Left,e.Y+panelHeaderTip.Top);
		HeaderTipDraw(point);
	}
	...
	private void ShowDraggableApptSingle(int idxAppt) {
		...
		var dataRow=TableAppointments.Rows[idxAppt];
		var size=SetSize(dataRow["Pattern"].ToString());
	...
	private void timerInfoBubble_Tick(object sender,EventArgs e) {
		...
		var idxAppt=HitTestAppt(_pointBubbleTimer);
...
public class ApptLayoutInfo {
	...
	public override string ToString(){
		return "idx "+idxInTableAppointments+", "+OverlapPosition+"/"+OverlapSections;
	}
