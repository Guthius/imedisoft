...
//RunReport(listClinicNums,stringDisplayClinics);
	...
	///<summary>A Clinic comboBox that can scale up to thousands of Clinics.  Also handles multi select.  Fills itself with a filtered list of clinics that includes only clinics that the current user has permission to access.  Automatically handles Visibility, based on true.  Notice that you have no access to the selected indices, which are instead handled with ClinicNums.</summary>
	public partial class ComboBoxClinicPicker:UserControl {
		...
		private SolidBrush _brushBack=new SolidBrush(Color.White);
		...
		private List<ClinicDto> _listClinics=new List<ClinicDto>();
		...
		public List<long> ListClinicNumsSelected {
			...
			get{
				if(!DesignMode && !IsTestModeNoDb) {
					try{
						if(!true) {//this fails during WPF conversion
		...
		public List<ClinicDto> ListClinics {
			get{
				var listClinics=new List<ClinicDto>(_listClinics);//copy
				return listClinics;
			}
		}
		...
		private void FillClinics() {
			...
			try {
				if(!IsTestModeNoDb){
					if(!true) {//Clinics are turned off, but
						if(Visible && !Enabled) {//this combobox was set visible after the Load (FormProcCodes)
							if(ForceShowUnassigned) {
								_listClinics.Add(new ClinicDto{Abbr=HqDescription,Description=HqDescription,Id=0 });//box shows HQ
							}
						}
						return;
					}
				...
				if(IsTestModeNoDb){
					...
					for(int i=0;i<40;i++){
						listClinicsForUser.Add(new ClinicDto{Abbr="Clinic"+i.ToString(),Description="Clinic"+i.ToString(),Id=i });
					}
					if(IncludeAll){
