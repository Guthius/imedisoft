...
public partial class FormComboPicker : Form{
	...
	private void FormComboPicker_Load(object sender, EventArgs e){
		...
		var rectScreenBounds=Screen.GetWorkingArea(this);
		//listBoxMain ItemHeight=13, height=4+(13*items)
		_heightLineItem=Font.Height;
		var maxItems=rectScreenBounds.Height/_heightLineItem;//rounds down
	...
	private void FormComboPicker_Paint(object sender, PaintEventArgs e){
		...
		var solidBrushBack=new SolidBrush(Color.FromArgb(229,241,251));
		var penArrow=new Pen(Color.FromArgb(20,20,20),1.5f);
		var penBlueOutline=new Pen(Color.FromArgb(0,120,215));
		...
		var rectangleOutline=new Rectangle();
		...
		var totalToDraw=200+_indexTopShowing>ListStrings.Count ? ListStrings.Count : 200+_indexTopShowing;
	...
	protected override void WndProc(ref Message m) {
		if(m.Msg==WM_MOUSEWHEEL) { //Check for scrolling
			...
			var scrollEventArgs=delta > 0 ? new ScrollEventArgs(ScrollEventType.EndScroll,0,1) : new ScrollEventArgs(ScrollEventType.EndScroll,1,0);
	...
	private bool SetSearchedIndex(int startIdx, char charKey, bool addIdx=false) {
		for(var i=startIdx;i<ListStrings.Count;i++) {//loop through the items and only add the item if it is found
	...
	private void SetVScrollValue() {
		...
		if((_listSelectedIndices[0]-_indexTopShowing+1)*_heightLineItem>this.Height) {//the selected index is hidden below bounds
			for(var i=this.Height;i<=(_listSelectedIndices[0]-_indexTopShowing+1)*_heightLineItem;i+=_heightLineItem) {
				if(i==_listSelectedIndices[0]) {
					break;
				}
				if(vScroll.Value+vScroll.SmallChange>vScroll.Maximum-this.Height) {
