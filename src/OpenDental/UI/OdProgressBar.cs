...
public class OdProgressBar:ProgressBar {
	...
	protected override void OnPaint(PaintEventArgs e) {
		using(Image progressImage=new Bitmap(this.Width,this.Height)) {
			using(var g=Graphics.FromImage(progressImage)) {
				...
				var brushLine=new SolidBrush(_targetColor);
				if(_targetValue>Minimum && _targetValue<=Maximum) {
					//Using a rectangle for a line because using a line was causing issues
					var targetValueLocation=(int)(this.Width*(((double)_targetValue-(double)Minimum)/((double)Maximum-(double)Minimum)));
					var targetLine=new Rectangle(targetValueLocation,0,4,this.Height);
