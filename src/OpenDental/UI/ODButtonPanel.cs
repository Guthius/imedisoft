...
public partial class ODButtonPanel:UserControl {
	...
	protected override void OnSizeChanged(EventArgs e) {
		...
		_font=new Font(FontFamily.GenericSansSerif,8.5f);
		_heightRow=18;
	...
	private void ComputeWidthsAndLocations(Graphics g) {
		...
		for(var i=0;i<ListODPanelItems.Count;i++) {
			if(string.IsNullOrWhiteSpace(ListODPanelItems[i].Text)) {
				ListODPanelItems[i].ItemWidth=ListODPanelItems[i].Text.Length*3+8;//measure each whitespace character as a fixed 3px width. +8px padding.
			}
			else{
				ListODPanelItems[i].ItemWidth=(int)g.MeasureString(ListODPanelItems[i].Text,_font).Width+8;
			}
	...
	private void DrawItemBut(Graphics g,ODPanelItem odPanelItem) {
		var recOutline=new Rectangle(odPanelItem.Location.X,odPanelItem.Location.Y,odPanelItem.ItemWidth-1,_heightRow-1);
	...
	private void OnClickButton(ODPanelItem itemClick) {
		var pArgs=new ODButtonPanelEventArgs(SelectedItem,SelectedRow,MouseButtons.Left);
