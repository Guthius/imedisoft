...
public partial class LightSignalGrid:Control {
	...
	public LightSignalGrid() {
		InitializeComponent();
		DoubleBuffered=true;
		_buttonH=25;//this doesn't work here
		hotButton=-1;
	...
	private void RectifyScrollValue() {
		...
		ODException.SwallowAnyException(() => {
			...
			var maxScroll=_listSignalButtonStates.Count*_buttonH-this.Height;
	...
	protected override void OnPaint(PaintEventArgs e) {
		...
		int G;
		int B;
		for(var i=0;i<_listSignalButtonStates.Count;i++) {
			...
			switch(_listSignalButtonStates[i].State) {
				...
				case ToolBarButtonState.Pressed:
					using(var sb = new SolidBrush(baseC)) {
				...
				case ToolBarButtonState.Hover://this is darker
					...
					using(var solidBrush = new SolidBrush(mixedC)) {
	...
	public void SetButtons(SigButDef[] sigButDefArray) {
		...
		var countButtons=sigButDefArray[sigButDefArray.Length-1].ButtonIndex+1;
		_listSignalButtonStates= [];
		//_listSignalButtonStates=new SignalButtonState[butDefs[butDefs.Length-1].ButtonIndex+1];
		for(var i=0;i<countButtons;i++){
			var signalButtonState=new SignalButtonState();
	...
	protected override void OnMouseMove(System.Windows.Forms.MouseEventArgs e) {
		...
		//Mouse is not down
		var button=HitTest(e.X,e.Y);//this is the button the mouse is over at the moment.
	...
	private int HitTest(int x,int y) {
		...
		var retVal = (y+_scrollVal)/_buttonH;//integer division
	...
	protected void OnButtonClicked(int myButton,SigButDef myDef,SigMessage mySignal) {
		var bArgs=new ODLightSignalGridClickEventArgs(myButton,myDef,mySignal);
		if(ButtonClick!=null)
...
public class SignalButtonState{
	...
	public SignalButtonState Copy() {
		var sigButState=(SignalButtonState)MemberwiseClone();
