...
namespace OpenDental {
	...
	public class FormODBase : Form {
		...
		public bool IsImageFloatSelected {
			//We use this instead of the built-in focus/active paradigm because we want the form/image to be selected even when it doesn't have focus.
...
Refresh();
		...
		private void PanelBorders_MouseUp(object sender, MouseEventArgs e){
			...
			if(_regionButHelp.IsVisible(e.Location) && HasHelpButton){//IsVisible really means HitTest
				...
				if(formName=="") {//this is more common than the override
					formName=Name;
				}
		...
		private const int WM_WINDOWPOSCHANGING=0x0046;//70
