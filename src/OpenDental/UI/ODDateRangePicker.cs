...
public partial class ODDateRangePicker:UserControl {
	...
	public DateTime GetDateTimeTo(bool isDefaultMaxDateT=false) {
		try {
			var dateTime=datePickerTo.GetDateTime();
	...
	private void ChangeToVertical() {
		...
		datePickerTo.CalendarLocation=ODDatePicker.EnumCalendarLocation.ToTheRight;
		datePickerFrom.CalendarLocation=ODDatePicker.EnumCalendarLocation.ToTheRight;
	...
	private void butWeekPrevious_Click(object sender,EventArgs e) {
		...
		if(dateFrom.Year > 1880 && dateTo.Year>1880) {//if both dates are valid
			datePickerFrom.SetDateTime(dateFrom.AddDays(-7));
			datePickerTo.SetDateTime(dateTo.AddDays(-7));
		}
		else{
			//Otherwise, don't do anything.  I left all the old logic below, but it all seems wrong.
			//It was buggy, changing dates by 8 days instead of 7.
			//Possibly notify user?
			//Seems like dates should always be valid.
			return;
		}
		/*
		if(dateFrom.Year > 1880) {//dateFrom is valid, use it
	...
	private void SetAutoCloseControls(params Control[] arrayControls) {
		foreach(var control in arrayControls) {
