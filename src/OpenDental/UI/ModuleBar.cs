...
public class ModuleBar : System.Windows.Forms.Control{
	...
	public ModuleBar(){
		...
		_listButtons=
		[
			new ModuleBarButton(EnumModuleType.Appointments, "Appts", GetIcon(EnumModuleType.Appointments)), //0
			new ModuleBarButton(EnumModuleType.Family, "Family", GetIcon(EnumModuleType.Family)), //1
			new ModuleBarButton(EnumModuleType.Account, "Account", GetIcon(EnumModuleType.Account)), //2
			new ModuleBarButton(EnumModuleType.TreatPlan, "Tx Plan", GetIcon(EnumModuleType.TreatPlan)), //3
			new ModuleBarButton(EnumModuleType.Chart, "Chart", GetIcon(EnumModuleType.Chart)), //4
			new ModuleBarButton(EnumModuleType.Imaging, "Imaging", GetIcon(EnumModuleType.Imaging)), //5
			new ModuleBarButton(EnumModuleType.Manage, "Manage", GetIcon(EnumModuleType.Manage)) //6
		];
		_selectedIndex=0;
		ComputeButtonSizes();
		timer=new Timer();
	...
	protected void OnButtonClicked(ModuleBarButton myButton,bool myCancel){
		if(ButtonClicked != null){
			...
			var oArgs = new ButtonClicked_EventArgs(myButton,myCancel);
	...
	public EnumModuleType SelectedModule{
		...
		set{
			var moduleBarButton=_listButtons.FirstOrDefault(x=>x.ModuleType==value);
	...
	public int IndexOf(EnumModuleType moduleType){
		...
		var moduleBarButton=_listButtons.FirstOrDefault(x=>x.ModuleType==moduleType);
		if(moduleBarButton==null){
			return -1;
		}
	...
	public void RefreshButtons() {
		var isMedical=Clinics.IsMedicalPracticeOrClinic(Clinics.ClinicNum);
	...
	private void DrawButton(ModuleBarButton button,bool isHot,bool isPressed,bool isSelected,Graphics g){
		...
		if(isPressed || isSelected || isHot) {
			...
			g.FillPolygon(_brushBack, [ //it's one pixel to the right because of the way rect drawn.
			...
			g.FillPolygon(_brushBack, [ //it's one pixel down and right.
			...
			g.FillPolygon(_brushBack, [ //it's one pixel down
		...
		//Image
		var imgRect=new Rectangle((Width-32)/2,button.Bounds.Y+4,32,32);
		if(isPressed || isSelected) {
			if(button.Icon2==EnumIcons.None){
				IconLibrary.Draw(g,button.Icon,imgRect);
			}
			else{
				IconLibrary.Draw(g,button.Icon2,imgRect);//the chart module has two images
			}
		}
		else{
			IconLibrary.Draw(g,button.Icon,imgRect);
		...
		var textRect = new Rectangle(button.Bounds.X-1,imgRect.Bottom,button.Bounds.Width+2,button.Bounds.Bottom-imgRect.Bottom);
		g.DrawString(button.Caption,font,Brushes.Black,textRect,_stringFormat);
	}
	...
	protected override void OnMouseMove(System.Windows.Forms.MouseEventArgs e){
		base.OnMouseMove(e);
		var hotBut=GetButtonI(new Point(e.X,e.Y));
	...
	protected override void OnMouseUp(System.Windows.Forms.MouseEventArgs e){
		...
		var selectedBut=GetButtonI(new Point(e.X,e.Y));
		if(selectedBut==-1){
			return;
		}
	...
	private void ComputeButtonSizes(){
		...
		var top=0;
		var width=this.Width-2;//Designed for a Width of 51
		//int textHeight=0;
		for(var i=0;i<_listButtons.Count;i++){
			...
			_listButtons[i].Bounds=new Rectangle(0,top,width,_heightButton);//39+26);
			_listButtons[i].BrushHot?.Dispose();
			_listButtons[i].BrushHot=new LinearGradientBrush(new PointF(0,top+_heightButton-10),new PointF(0,top+_heightButton),
				_brushSelected.Color,_brushPressed.Color);
			top+=_heightButton+1;//39+26+1;
		}
		g.Dispose();
	}
