...
public class ODPrintPreviewControl: PrintPreviewControl {
	...
	protected override void OnPaint(PaintEventArgs pevent) {
		if(!false){//Always use base methods when not using web.
			base.OnPaint(pevent);
			return;
		}
		...
		try {
			if(_arrayPageInfo==null||_arrayPageInfo.Length==0) {
				...
				var controlPhysicalSize=new Size(PixelsToPhysical(new Point(Size), screendpi));
				//Point imagePixels=PhysicalToPixels(new Point(imageSize), screendpi);
				var virtualPixels=new Point(_virtualSize);
				// center pages on screen if small enough
				var offset=new Point(Math.Max(0, (Size.Width - virtualPixels.X) / 2),Math.Max(0, (Size.Height - virtualPixels.Y) / 2));
				offset.X-=position.X;
				offset.Y-=position.Y;
				var borderPixelsX=PhysicalToPixels(_border, screendpi.X);
				var borderPixelsY=PhysicalToPixels(_border, screendpi.Y);
				var originalClip=pevent.Graphics.Clip;
				var pageRenderArea=new Rectangle[_countDisplayedVerticalPages * _countDisplayHorizontalPages];
				var lastImageSize=Point.Empty;
				var maxImageHeight=0;
				try {
					for(var row=0;row<_countDisplayedVerticalPages;row++) {
						//Initialize our LastImageSize variable...
						lastImageSize.X=0;
						lastImageSize.Y=maxImageHeight*row;
						for(var column=0;column<_countDisplayHorizontalPages;column++) {
							var imageIndex=StartPage + column + row*_countDisplayHorizontalPages;
							if(imageIndex<_arrayPageInfo.Length) {
								var pageSize=_arrayPageInfo[imageIndex].PhysicalSize;
								if(autoZoom) {
									var zoomX=((double) controlPhysicalSize.Width - _border*(_countDisplayHorizontalPages + 1)) / (_countDisplayHorizontalPages*pageSize.Width);
									var zoomY=((double) controlPhysicalSize.Height - _border*(_countDisplayedVerticalPages + 1)) / (_countDisplayedVerticalPages*pageSize.Height);
									zoom=Math.Min(zoomX,zoomY);
								}
								imageSize=new Size((int)(zoom*pageSize.Width),(int)(zoom*pageSize.Height));
								var imagePixels=PhysicalToPixels(new Point(imageSize), screendpi);
								var x=offset.X + borderPixelsX * (column + 1) + lastImageSize.X;
								var y=offset.Y + borderPixelsY * (row + 1) + lastImageSize.Y;
				...
				for(var i=0;i<pageRenderArea.Length;i++) {
					if(i+StartPage<_arrayPageInfo.Length) {
						var box=pageRenderArea[i];
  ...
  tempGraphics.Dispose();
		*/
		...
		var pageSize=_arrayPageInfo[StartPage].PhysicalSize;
		var controlPhysicalSize=new Size(PixelsToPhysical(new Point(Size), screendpi));
		if(autoZoom) {
			var zoomX=((double) controlPhysicalSize.Width - _border*(_countDisplayHorizontalPages + 1)) / (_countDisplayHorizontalPages*pageSize.Width);
			var zoomY=((double) controlPhysicalSize.Height - _border*(_countDisplayedVerticalPages + 1)) / (_countDisplayedVerticalPages*pageSize.Height);
		...
		imageSize=new Size((int)(zoom*pageSize.Width),(int)(zoom*pageSize.Height));
		var virtualX=(imageSize.Width * _countDisplayHorizontalPages) + _border * (_countDisplayHorizontalPages +1);
		var virtualY=(imageSize.Height * _countDisplayedVerticalPages) + _border * (_countDisplayedVerticalPages +1);
	...
	private void SetPositionNoInvalidate(Point value) {
		...
		var scroll=new Rectangle(rect.X, rect.Y, rect.Width, rect.Height);
	...
	private int AdjustScroll(Message m, int pos, int maxPos, bool horizontal) {
		switch(LOWORD(m.WParam)) {
			...
			case SB_THUMBTRACK:
				...
				var direction = horizontal ? SB_HORZ : SB_VERT;
	...
	private void WmVScroll(ref Message m) {
		...
		var locPos = position;
		var pos = locPos.Y;
		var maxPos = Math.Max(Height, _virtualSize.Height/* - Height*/);
	...
	private void WmHScroll(ref Message m) {
		...
		var locPos = position;
		var pos = locPos.X;
		var maxPos = Math.Max(Width, _virtualSize.Width /*- Width*/);
