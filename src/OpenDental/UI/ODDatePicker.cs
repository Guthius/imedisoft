...
public partial class ODDatePicker:UserControl {
	...
	protected override void OnSizeChanged(EventArgs e) {
		...
		var bounds=new Rectangle(
			...
			1,
			102,
			20);//height gets ignored
		textDate.Bounds=bounds;
		bounds=new Rectangle(
			textDate.Right-1-16,
			textDate.Top+1,
			16,
			textDate.Height-2);//18
		butDrop.Bounds=bounds;
	}
	...
	public void HideCalendar() {
		...
		//this.Height=(22);//this doesn't make any sense, since the height was never increased in the first place.
	...
	public void ShowCalendar() {
		...
		switch(CalendarLocation) {
			case EnumCalendarLocation.Above:
				_pointLocOrigCalendar=new Point(1+_pointAdjustCalendarLoc.X,
					textDate.Location.Y-1-162+_pointAdjustCalendarLoc.Y);
				break;
			case EnumCalendarLocation.ToTheRight:
				_pointLocOrigCalendar=new Point(textDate.Right+1+_pointAdjustCalendarLoc.X,
					textDate.Location.Y-1+_pointAdjustCalendarLoc.Y);
				break;
			default://default to below
				_pointLocOrigCalendar=new Point(0+_pointAdjustCalendarLoc.X,
					21+_pointAdjustCalendarLoc.Y);
				break;
		}
		//calendar.Location=_locationOrigCalendar;//no need for LayoutManager
		var pointNew=_pointLocOrigCalendar;//calendar.Location;//Start from current context location.
