...
public class SheetLayoutController
{
    ...
    {
        var practiceDefaultSheetDefNum = PrefC.GetLong(PrefName.SheetsDefaultChartModule);
        ListSheetDefsLayout = SheetDefs.GetCustomForType(_sheetType);
        var sheetInternalType = SheetsInternal.GetInternalType(_sheetType);
        var sheetDefInternal = SheetsInternal.GetSheetDef(sheetInternalType);
        ...
        var sheetDefUser = GetLayoutForUser();
    ...
    public SheetDef GetLayoutForUser()
    {
        ...
        SheetDef def = null;
        var listPrefs = UserOdPrefs.GetByUserAndFkeyType(Security.CurUser.UserNum, UserOdFkeyType.DynamicChartLayout);
        var userPref = listPrefs.FirstOrDefault(); //There is only at most a single link per user.
        ...
        else
        {
            ...
            var sheetDefNum = ClinicPrefs.GetLong(PrefName.SheetsDefaultChartModule, Clinics.ClinicNum);
            if (!true //clinics are off
        ...
        bool isForcedRefresh = false, bool isUserSelection = false)
    {
        ...
        var listSheetFieldDefs = GetPertinentSheetFieldDefs(sheetFieldLayoutMode);
        var listControlsDynamic = new List<Control>();
        for (var i = 0; i < listSheetFieldDefs.Count; i++)
        {
            if (!TryGetControlForField(listSheetFieldDefs[i], dictionaryControls, out var control))
            {
            ...
            var heightAbove = GetStaticControlsHeightAbove(listSheetFieldDefs[i].YPos);
            var heightBelow = GetStaticControlsHeightBelow(listSheetFieldDefs[i].YPos);
            ...
            SheetUtil.SetControlSizeAndAnchors(listSheetFieldDefs[i], control, _controlHosting, 1, fillHeightAdj: heightAbove + heightBelow);
            control.Location = new Point(listSheetFieldDefs[i].XPos, listSheetFieldDefs[i].YPos + heightAbove);
    ...
    {
        var userPref = UserOdPrefs.GetFirstOrNewByUserAndFkeyType(Security.CurUser.UserNum, UserOdFkeyType.DynamicChartLayout);
        userPref.Fkey = _sheetDefDynamicLayoutCur.SheetDefNum;
        var sheetDefNumDefault = ClinicPrefs.GetLong(PrefName.SheetsDefaultChartModule, Clinics.ClinicNum);
        if (!true || Clinics.ClinicNum == 0 || sheetDefNumDefault == 0)
        {
    ...
    {
        var listFillDownDefs = _sheetDefDynamicLayoutCur.SheetFieldDefs
            .FindAll(x => x.LayoutMode == sheetFieldLayoutModeCur && x.GrowthBehavior == GrowthBehaviorEnum.FillDownFitColumns);
        var listControlsDynamic = new List<Control>();
        var listDefsForLayoutMode = _sheetDefDynamicLayoutCur.SheetFieldDefs.FindAll(x => x.LayoutMode == sheetFieldLayoutModeCur).ToList();
        for (var i = 0; i < listDefsForLayoutMode.Count; i++)
        {
            if (TryGetControlForField(listDefsForLayoutMode[i], dictionaryControls, out var control))
            {
                listControlsDynamic.Add(control);
            }
        }
        ...
        for (var i = 0; i < listFillDownDefs.Count; i++)
        {
            if (TryGetControlForField(listFillDownDefs[i], dictionaryControls, out var control))
            {
                var gridMaxX = control.Location.X + control.Width;
                var listControlsIntersecting = listControlsDynamic.FindAll(x => x != control && x.Bounds.IntersectsWith(control.Bounds));
                for (var j = 0; j < listControlsIntersecting.Count; j++)
                {
                    var diff = gridMaxX - listControlsIntersecting[j].Location.X + 1;
    ...
    {
        var listFillDownDefs = _sheetDefDynamicLayoutCur.SheetFieldDefs
            .FindAll(x => x.LayoutMode == sheetFieldLayoutModeCur && x.GrowthBehavior.In(
                GrowthBehaviorEnum.FillDownFitColumns, GrowthBehaviorEnum.FillDown))
            .OrderBy(x => x.YPos)
            .ToList();
        var listControlsDynamic = new List<Control>();
        var listSheetFieldDefsForLayout = _sheetDefDynamicLayoutCur.SheetFieldDefs.FindAll(x => x.LayoutMode == sheetFieldLayoutModeCur).ToList();
        for (var i = 0; i < listSheetFieldDefsForLayout.Count; i++)
        {
            if (TryGetControlForField(listSheetFieldDefsForLayout[i], dictControls, out var control))
            {
                listControlsDynamic.Add(control);
            }
        }
        ...
        for (var i = 0; i < listFillDownDefs.Count; i++)
        {
            if (TryGetControlForField(listFillDownDefs[i], dictControls, out var control))
            {
                ...
                var heightAbove = 0;
                var heightBelow = 0;
                var anchorToBottom = true;
                switch (listFillDownDefs[i].GrowthBehavior)
                {
                    case GrowthBehaviorEnum.FillDownFitColumns:
                        var minY = GetStaticControlsHeightAbove(listFillDownDefs[i].YPos); //Uses y position relative to the sheet def.
                        AdjustGridYPosition(listFillDownDefs[i], control, listControlsDynamic, minY);
                        heightBelow = GetStaticControlsHeightBelow(listFillDownDefs[i].YPos);
                        break;
                    case GrowthBehaviorEnum.FillDown:
                        ...
                        heightAbove = GetStaticControlsHeightAbove(listFillDownDefs[i].YPos);
                        heightBelow = GetStaticControlsHeightBelow(listFillDownDefs[i].YPos + heightAbove);
                        heightBelow += GetSheetFieldsHeightBelow(listFillDownDefs[i], heightAbove, listSheetFieldDefs, dictControls, out anchorToBottom);
                        break;
                }

                SheetUtil.SetControlSizeAndAnchors(listFillDownDefs[i], control, _controlHosting, 1, fillHeightAdj: heightAbove + heightBelow, anchorToBottom);
                control.Location = new Point(listFillDownDefs[i].XPos, listFillDownDefs[i].YPos + heightAbove);
            }
        }
    }
    ...
    {
        var listControlsAbove = listDynamicControls.FindAll(x => IsControlAbove(fieldControl, x));
    ...
    private int GetSheetFieldsHeightBelow(SheetFieldDef fieldDef, int heightOffset, List<SheetFieldDef> listSheetFieldDefs, Dictionary<string, Control> dictControls, out bool anchorToBottom)
    {
        ...
        var listSheetFieldDefsToCheck = listSheetFieldDefs.FindAll(x => !x.FieldName.Contains("Button"));
        var listBelowSheetFieldDefs = new List<Control>();
        if (TryGetControlForField(fieldDef, dictControls, out var controlOrigin))
        {
            for (var i = 0; i < listSheetFieldDefsToCheck.Count; i++)
            {
                if (TryGetControlForField(listSheetFieldDefsToCheck[i], dictControls, out var controlTarget) && IsControlBelow(controlOrigin, controlTarget))
                {
    ...
    {
        var heightAbove = 0;
        var listStaticControlsAbove = _listControlsStatic.FindAll(x => x.Visible && x.Location.Y <= yPos);
    ...
    {
        var heightBelow = 0;
        var listStaticControlsBelow = _listControlsStatic.FindAll(x => x.Visible && x.Location.Y >= yPos);
