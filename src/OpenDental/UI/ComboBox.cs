...
//Also see the similar controls: ComboBoxClinicPicker and UI.ListBox
//This comboBox is written from scratch, so we have more control. 
//It's optimized for very long lists that are even taller than the screen.
...
//To replace an existing ComboBox with a UI.ComboBox or a ComboBoxClinicPicker (which we're all done with):
...
public partial class ComboBox : Control{
	...
	protected override void OnPaint(PaintEventArgs e){
		...
		using var brushBack=new SolidBrush(_colorBack);
		...
		var widthMax=rectangle.Width-15;
		...
		var rectangleMask=new Rectangle(Width-1-15,rectangle.Y+4,15-1,rectangle.Height-5);
		g.FillRectangle(brush,rectangleMask);
		//the dropdown arrow, starting at the left
		//Default height is 21
		var yCent=Height/2f;//default is 10.5
		g.DrawLine(_penArrow,
			x1:Width-13,
			y1:yCent-1.5f,//was 9
			x2:Width-9.5f,
			y2:yCent+1.5f);//was 12
		g.DrawLine(_penArrow,
			x1:Width-9.5f,
			y1:yCent+1.5f,//was 12
			x2:Width-6,
			y2:yCent-1.5f);//was 9
	}
	...
	private void _formComboPicker_FormClosing(object sender, FormClosingEventArgs e){
		...
		if(SelectionModeMulti){
			if(!IsAllSelected){//if all is selected, we ignore other selections
				for(var i=0;i<_formComboPicker.SelectedIndices.Count;i++){
	...
	public object SelectedItem { 
		get {
			...
			var comboBoxItem=Items.GetComboBoxItemAt(_listSelectedIndices[0]);
	...
	private void ComboBoxJ_Scroll(object sender,ScrollEventArgs e) {
		...
		if(e.OldValue>e.NewValue) { //Scroll up
			var index=_listSelectedIndices[0]-1<0? _listSelectedIndices[0] : _listSelectedIndices[0]-1;
			SetSelected(index);
		}
		else { //Scroll down
	...
	public class ComboBoxItemCollection{
		private ComboBox _comboBoxOdParent;
		private List<ComboBoxItem> _listComboBoxItems;

		public ComboBoxItemCollection(ComboBox comboBoxJ){
		...
		public List<T> GetAll<T>(){
			...
			for(var i=0;i<_listComboBoxItems.Count;i++){
