...
using OpenDental.UI;
		...
		private void LayoutToolBar() {
			...
			toolBarMain.Buttons.Add(new ODToolBarButton(Lan.g(this,"Info"),bitmap,"","Info"));
		}
...
Notice that there are 3 ways to add images. Examples of each are above.
Boilerplate for the new WpfControls.UI.ToolBar is over in ToolBar.xaml.cs
*/
...
public class ToolBarOD : System.Windows.Forms.UserControl{
	...
	protected void OnButtonClicked(ODToolBarButton toolBarButton){
		if(toolBarButton.DateTimeLastClicked.AddMilliseconds(SystemInformation.DoubleClickTime)>DateTime.Now) {
			return;//ignore a second click that happens too quickly
		}
		toolBarButton.DateTimeLastClicked=DateTime.Now;
		if(toolBarButton.EventHandlerClick==null){
			var toolBarButtonClickEventArgs=new ODToolBarButtonClickEventArgs(toolBarButton);
			ButtonClick?.Invoke(this,toolBarButtonClickEventArgs);
			return;
		}
		toolBarButton.EventHandlerClick.Invoke(this,new EventArgs());
	}
	[Category("OD")]
	...
	private void TextPageNav_KeyDown(object sender,KeyEventArgs e) {
		...
		if(e.KeyCode==Keys.Enter && textPageNav.IsValid()) {
			var pageNum=textPageNav.MaxVal;
			try {
				pageNum=int.Parse(textPageNav.Text);
			}
			catch(Exception) {
				return;
			}
	...
	protected override void OnMouseMove(System.Windows.Forms.MouseEventArgs e){
		...
		//mouse is not down
		var toolBarButtonOver=HitTest(e.X,e.Y);
	...
	protected override void OnMouseUp(System.Windows.Forms.MouseEventArgs e){
		...
		_isMouseDown=false;
		var toolBarButton=HitTest(e.X,e.Y);
			...
			if(_toolBarButtonHot==toolBarButton){//if mouse was released over the same button as it was depressed
				        ...
				        && HitTestDrop(toolBarButton,e.X,e.Y))//and we are in the dropdown area on the right
				{
					if(toolBarButton.DropDownMenu!=null){//there is a dropdown menu to display
						...
						var contextMenu=toolBarButton.DropDownMenu.GetContextMenu();
					...
					if(toolBarButton.IsTogglePushed){
						toolBarButton.IsTogglePushed=false;
					}
					else{
						toolBarButton.IsTogglePushed=true;
					}
	...
	private void ComputeButtonsizes(Graphics g){
		_widthDropDown=(15);//The width of the dropdown rectangle where the triangle shows.
		var xPos=0;
		int width;
		for(var i=0;i<buttons.Count;i++){
			if(buttons[i].Style==ODToolBarButtonStyle.Separator) {
				...
				var widthImage=(buttons[i].Bitmap.Width);
				if(buttons[i].Text==""){
				...
				var widthImage=(imageList.ImageSize.Width);
				if(buttons[i].Text==""){
					width=widthImage+(7);//slightly wider than high for better 'feel'
				}
				else{
	...
	private void DrawButton(Graphics g,ODToolBarButton button){
		...
		var brushTogglePushed=_brushTogglePushed;
		var brushMain=_brushMain;
		var brushPushed=_brushPushed;
		...
		var colorGradientTop=Color.FromArgb(253,253,254);
		var colorGradientBottom=Color.FromArgb(181,190,215);
		...
		else if(button.Style==ODToolBarButtonStyle.ToggleButton && button.IsTogglePushed){
		...
		var isNotify=(button.Style==ODToolBarButtonStyle.DropDownButton && !string.IsNullOrWhiteSpace(button.NotificationText));//Notifies even when disabled.
		if(isNotify) {//Override dropdown background to show notification color.
			var rectDropDown=new Rectangle(button.Bounds.X+button.Bounds.Width-_widthDropDown,button.Bounds.Y,_widthDropDown,button.Bounds.Height);
		...
		var textWidth=button.Bounds.Width;
		if(button.Style==ODToolBarButtonStyle.DropDownButton){
			textWidth-=_widthDropDown;
		}
		Bitmap bitmap=null;
		var sizeImage=new Size();
		if(button.Icon!=EnumIcons.None){
			sizeImage=new Size((22),(22));
		}
		else if(button.Bitmap!=null){
			bitmap=button.Bitmap;
			sizeImage=new Size((bitmap.Width),(bitmap.Height));
		}
			...
			sizeImage=new Size((imageList.ImageSize.Width),(imageList.ImageSize.Height));
		}
		if(button.Icon!=EnumIcons.None || bitmap!=null){//draw image and text
			if(!button.Enabled){
				rectangleImage=new Rectangle(button.Bounds.X+(3),button.Bounds.Y+(1),sizeImage.Width,sizeImage.Height);
				if(button.Icon!=EnumIcons.None){
					IconLibrary.DrawDisabled(g,button.Icon,rectangleImage);
				}
				else{
					var bitmapDisabled=new Bitmap(bitmap.Width,bitmap.Height);//disabled bitmap is old small size
				...
				rectangleText=new Rectangle(button.Bounds.X+sizeImage.Width+(3),button.Bounds.Y,
				...
				rectangleImage=new Rectangle(button.Bounds.X+(4),button.Bounds.Y+(2),sizeImage.Width,sizeImage.Height);
				if(button.Icon!=EnumIcons.None){
					IconLibrary.Draw(g,button.Icon,rectangleImage);
				}
				else{
					g.DrawImage(bitmap,rectangleImage);
				}
				rectangleText=new Rectangle(button.Bounds.X+1+sizeImage.Width+(3),button.Bounds.Y+(1),
					textWidth-sizeImage.Width-(3),button.Bounds.Height);
			}
			else{
				rectangleImage=new Rectangle(button.Bounds.X+(3),button.Bounds.Y+(1),sizeImage.Width,sizeImage.Height);
			...
			if(button.Enabled) {
				...
				if(button.Style==ODToolBarButtonStyle.PageNav) {
					...
					if(textPageNav==null) {
						...
						textPageNav.Size=new Size(button.Bounds.Width/2-(10),button.Bounds.Height);
					...
					var wText=button.Bounds.Width/2-(10);
					textPageNav.Bounds=new Rectangle(
						xCenter-wText-(4),yCenter-textPageNav.Height/2,wText,textPageNav.Height);
		...
		if(!button.Enabled){
			...
			g.DrawRectangle(_penTogglePushed,//_penOutline,
		...
		if(button.Style==ODToolBarButtonStyle.DropDownButton){
			var adjDown=0;//The distance to push the triangle down to show the notification text.
			if(isNotify) {
				adjDown=(6);
				//Draw the notification text.
				var sizeText=TextRenderer.MeasureText(button.NotificationText,Font);
	...
	private bool HitTestDrop(ODToolBarButton button,int x,int y){
		var dropRect=new Rectangle(button.Bounds.X+button.Bounds.Width-_widthDropDown,button.Bounds.Y
	...
	private void InitializeBrushes(){
		...
		_brushPushedError?.Dispose();
		_brushNotify?.Dispose();
		_brushTogglePushed=new LinearGradientBrush(new Point(0,0),new Point(0,Height),Color.FromArgb(255,255,255),Color.FromArgb(191,220,255));
		//.FromArgb(191,201,229));//too bland
		...
		_brushMain=new LinearGradientBrush(new Point(0,0),new Point(0,Height),Color.FromArgb(255,255,255),Color.FromArgb(191, 201, 229));//#bfc9e5
		//171,181,209));//#ABB5D1 //too dark
