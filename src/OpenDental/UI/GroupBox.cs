...
public partial class GroupBox : System.Windows.Forms.ContainerControl{
	...
	public bool IsOnActualText(Point point){
		...
		if(point.X>5+sizeF.Width){
			return false;
		}
		if(point.Y>1+sizeF.Height){
			return false;
		}
		return true;
	}
	...
	protected override void OnPaint(PaintEventArgs pe){
		...
		var radius=4f;
		using var graphicsPath=GraphicsHelper.GetRoundedPath(new RectangleF(0,0,Width-1,Height-1),radius);
		Brush brush=new SolidBrush(BackColor);
		g.FillPath(brush,graphicsPath);
		if(ColorBackLabel!=Color.Empty){
			var sizeF=g.MeasureString(Text,Font);
			var rectangleF=new RectangleF(5,1,sizeF.Width,sizeF.Height);
		...
		g.DrawString(Text,Font,Brushes.Black,5,1);//yes, this is one pixel lower than original so that it doesn't touch the top line.	
	}
