...
//This listBox is written from scratch, so we have more control. 
//Some basic problems with MS stock listBox: color highlight was stupid blue, no hover effect, flicker on resize, and poor high dpi support.
//Also see the similar controls: ComboBoxOD and ComboBoxClinicPicker.
...
//
//Ignore these old notes:
//To replace an existing ListBox with a UI.ListBox (which we're all done with):
...
public partial class ListBox : Control {
	...
	private List<int> _listSelectedOrig= [];
	...
	private List<int> _listSelectedIndices= [];
	...
	public ListBox() {
		...
		vScrollBar.Bounds=new Rectangle(Width-vScrollBar.Width-1,1,vScrollBar.Width,Height-2);
	...
	public string[] ItemStrings{
		...
		set{
			...
			if(!DesignMode){
				Items.Clear();
				for(var i=0;i<_itemStrings.Length;i++){
	...
	public object SelectedItem {
		get{
			...
			var item=Items.GetObjectAt(_listSelectedIndices[0]);
			if(item==null) {
	...
	protected override void OnKeyDown(KeyEventArgs e) {
		...
		if(e.KeyCode==Keys.Up) {//arrow key up
			...
			var selected=_listSelectedIndices[_listSelectedIndices.Count-1];//get the last selected index
		...
		if(char.IsLetterOrDigit(charKey)) {//alpha or numeric character down
			...
			_listSelectedIndices.Clear();//clear the list so that it removes all of the multiple selected indices
			_listSelectedIndices.Add(selected);
			var foundMatch=SetSearchedIndex(_listSelectedIndices[0]+1,charKey);
	...
	protected override void OnPaint(PaintEventArgs e){
		...
		var maxItems=(heightAvail/_heightLine);//example 4.5
		...
		if(Items.Count>maxItems) {
			vScrollBar.Bounds=new Rectangle(Width-vScrollBar.Width-1,1,17,Height-2);
			//Give the maxmimum one extra line item so that the real last line item isn't cut off
			var oldScrollMaxValue=vScrollBar.Maximum;
			...
			if(vScrollBar.Maximum!=oldScrollMaxValue){//set the scrollbar value on update/load of the ListBox items
				SetVScrollValue();
			}
		}
		else {
			vScrollBar.Value=vScrollBar.Minimum;//shows the remaining items
			vScrollBar.Visible=false;
			vScrollBar.Enabled=false;
		}
		var brushText=Brushes.Black;//no dispose
		if(!Enabled){
		...
		var totalToDraw=(200+_indexTopShowing)>Items.Count ? Items.Count : 200+_indexTopShowing;
		for(var i=_indexTopShowing; i<totalToDraw; i++) {
			...
			var rectangle=new Rectangle(2,(int)((i-_indexTopShowing)*_heightLine)+2,Width-2,(int)_heightLine);
	...
	protected override void OnMouseDown(MouseEventArgs e){
		...
		if(SelectionMode==SelectionMode.MultiExtended) {
			...
			_listSelectedIndices= [_mouseDownIndex];
		}
	...
	protected override void OnMouseMove(MouseEventArgs e){
		...
		var listSelectedOrig=new List<int>(_listSelectedIndices);//for both multi and single, we need to remember this. Just local.
		if(SelectionMode==SelectionMode.MultiExtended) {
			CalcSelectedIndices();
		}
			...
			if(_hoverIndex>=0 && _hoverIndex<Items.Count) {
				_listSelectedIndices= [_hoverIndex];
			}
		}
	...
	protected override void OnResize(EventArgs e) {
		base.OnResize(e);
		if(vScrollBar.Enabled && vScrollBar.Value>0) {
			if(vScrollBar.Value>vScrollBar.Maximum-this.Height && vScrollBar.Maximum-this.Height>=0) {//checks when resizing if the items need to move down
				vScrollBar.Value=vScrollBar.Maximum-this.Height+2;
			}
		}
		if(!DesignMode){
			return;
	...
	public List<T> GetListSelected<T>() {
		var listSelected=new List<T>();
		for(var i=0;i<_listSelectedIndices.Count;i++){
	...
	public int IndexFromPoint(Point point) {
		var indexAtPoint=(int)((point.Y-3)/_heightLine)+_indexTopShowing;
	...
	protected override void WndProc(ref Message m) {
		if(m.Msg==WM_MOUSEWHEEL) {//scrolling
			int delta;
			if((long)m.WParam>=(long)int.MaxValue) { //Just in case the param is larger than the max
		...
		if(ShiftIsDown()) {
			...
			if(_listSelectedIndices.Count==0) {
				...
				if(_mouseDownIndex<fromRow) { //Dragging up
					for(var i=_mouseDownIndex;i<=fromRow;i++) {
						_listSelectedIndices.Add(i);
					}
				}
				else { //Dragging down
					for(var i=fromRow;i<=_mouseDownIndex;i++) {
						_listSelectedIndices.Add(i);
					}
				}
			}
	...
	private bool SetSearchedIndex(int startIdx, char charKey) {
		for(var i=startIdx;i<Items.Count;i++) {//loop through all of the items and only add the item if it is found
	...
	private void SetVScrollValue() {
		...
		if(vScrollBar.Value>vScrollBar.Maximum-this.Height) {//check if the scrollbar is below the last item
			var valueBottom=vScrollBar.Maximum-this.Height+2;
			valueBottom=Math.Min(vScrollBar.Value,valueBottom);
			valueBottom=Math.Max(vScrollBar.Minimum,valueBottom);
			vScrollBar.Value=valueBottom;
		}
		if(_listSelectedIndices.Count<1) {
			return;
		}
		if(_listSelectedIndices[_listSelectedIndices.Count-1]<_indexTopShowing) {//the latest selected index is above the bounds
			for(var i=_indexTopShowing;i>0;i--) {
			...
			for(var i=this.Height;i<=(_listSelectedIndices[_listSelectedIndices.Count-1]-_indexTopShowing+1)*_heightLine;i+=(int)_heightLine) {
				if(i==_listSelectedIndices[_listSelectedIndices.Count-1]) {
					break;
				}
				VScrollMoveDown();//adds to vScroll.Value
				if(vScrollBar.Value==vScrollBar.Maximum-this.Height+2) {
					break;
				}
			}
		}
	...
	private void VScrollMoveDown() {
		if(vScrollBar.Value+vScrollBar.SmallChange>vScrollBar.Maximum-this.Height) {//if scrollbar is near the bottom
			vScrollBar.Value=vScrollBar.Maximum-this.Height+2;//set scrollbar to the bottom
		}
		else {
			vScrollBar.Value+=vScrollBar.SmallChange;//move scrollbar one item down
		}
	}
	...
	private void VScrollMoveUp() {
		if(vScrollBar.Value-vScrollBar.SmallChange<0) {//if scrollbar is near top
			vScrollBar.Value=0;//set scrollbar to the top
		}
		else {
			vScrollBar.Value-=vScrollBar.SmallChange;//move scrollbar one item up
		}
	}
	...
	public class ListBoxItemCollection{
		...
		public bool Contains<T>(T item) {
			for(var i=0; i<_listListBoxItems.Count;i++) {
				if(item is string) {
		...
		/*
		///<summary>Adds a dummy object called "None", with a key of 0.</summary>
		public void AddNone<T>(Func<T,string> funcItemToString=null) where T:new(){
			string item=null;
			if(funcItemToString==null) {
				item=funcItemToString(item);
			}
			Add(item,new T());
		}*/
