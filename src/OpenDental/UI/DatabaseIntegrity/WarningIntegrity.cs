...
public partial class WarningIntegrity : UserControl{
	...
	public void SetTypeAndVisibility(EnumWarningIntegrityType warningIntegrityType,bool isHashValid){
		...
		var databaseIntegrity=DatabaseIntegrities.GetOneClass(warningIntegrityType);
		if(databaseIntegrity is null){
			//Connection to HQ failed or HQ intentionally didn't provide a DefaultClass type. Don't show warnings.
			//See WebServiceCustomerUpdates.DatabaseIntegrityGetList2 for why a DefaultClass would be missing.
			Visible=false;
			return;
		}
		...
		if(_databaseIntegrity.Behavior==EnumIntegrityBehavior.None){
			Visible=false;
			return;
		}
	...
	protected override void OnPaint(PaintEventArgs pe){
		...
		var str="";//this is to get the points so I can use them over in WPF. AddCurve is not supported.
		for(var i=0;i<_listTips.Count;i++){
			//draw curve at this
		...
		using var solidBrushOrange=new SolidBrush(_color);
		g.FillPath(solidBrushOrange,graphicsPath);
		using var penOrange=new Pen(_color);
		...
		var rectangleFCircle=new RectangleF(Width/2f-1.15f*_scale,13*_scale,2.3f*_scale,2.3f*_scale);
	...
	private void FillListTips(PointF[] pointFArray, float rounding){
		_listTips= [];
		for(var p=0;p<pointFArray.Length;p++){
			Line linePrevious;
			if(p==0){
			...
			var pointFBetween = GetPointFBetween(tip.PointFTipPrevious1, tip.PointFTipNext3, 0.5f);
	...
	private Line ShortenLine(Line line, float byPixels){
		var lengthOriginal = (float)Math.Sqrt(Math.Pow(line.PointFPrevious.X - line.PointFNext.X, 2)+ Math.Pow(line.PointFPrevious.Y - line.PointFNext.Y, 2));
		var proportion = (lengthOriginal - byPixels) / lengthOriginal;
		var pointFPrevious = GetPointFBetween(line.PointFPrevious, line.PointFNext, proportion);
		var pointFNext = GetPointFBetween(line.PointFPrevious, line.PointFNext, 1 - proportion);
	...
	private PointF GetPointFBetween(PointF pointF1, PointF pointF2, float proportion){
		var x=pointF1.X + (pointF2.X - pointF1.X) * proportion;
		var y=pointF1.Y + (pointF2.Y - pointF1.Y) * proportion;
