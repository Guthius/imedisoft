...
public partial class Button : System.Windows.Forms.Button {
	...
	private static Color _colorMainBottom=Color.FromArgb(225,232,235);//205,212,215);
		...
		try {
			...
			if(this.Image==null) {
				if(Width<textWidth+buffer) {
					this.Width=textWidth+buffer;
				}
			}
			else {
				if(Width<textWidth+Image.Size.Width+buffer) {
					this.Width=textWidth+Image.Size.Width+buffer;
				}
			}
//todo: this is broken for LayoutManager, but only noticed for foreign language and is rare
	...
	protected override void OnPaint(PaintEventArgs p) {
		...
		var recOutline=new RectangleF(0,0,Width-1.2f,Height-1.2f);//lower edge looks better than at -1px
		float radius=3;
		using var graphicsPath=OpenDentBusiness.GraphicsHelper.GetRoundedPath(recOutline,radius);
	...
	private void DrawTextAndImage(Graphics g) {
		var stringFormat=GetStringFormat(this.TextAlign);
		if(ShowKeyboardCues) {
		...
		//Image:
		var rectangleText=new Rectangle();
		Rectangle rectangleImage;
		var pointImage= new Point(6, 4);
		Size sizeScaled;
		if(Icon!=EnumIcons.None){
			sizeScaled=new Size(22,22);
		}
		else{
			sizeScaled=new Size(Image.Width,Image.Height);
		}
		...
		var colorGradientTop=Color.FromArgb(252,253,254);
		var colorGradientBottom=Color.FromArgb(228,234,237);
		...
		if(this.Enabled) {
			...
			if(Icon!=EnumIcons.None){
				IconLibrary.Draw(g,Icon,rectangleImage);
			}
			else{
			...
			if(Icon!=EnumIcons.None){
				...
				var bitmapDisabled=new Bitmap(Image.Width,Image.Height);//disabled bitmap is old small size
	...
	public static void DrawSimpleButton(Graphics g,Rectangle rectangle,string text,Font font,Brush brush){
		var recOutline=new RectangleF(rectangle.X,rectangle.Y,rectangle.Width-1.2f,rectangle.Height-1.2f);
		float radius=3;
		var graphicsPath=OpenDentBusiness.GraphicsHelper.GetRoundedPath(recOutline,radius);
		g.FillPath(brush,graphicsPath);
		g.DrawPath(_penBorder,graphicsPath);
		graphicsPath.Dispose();
		var stringFormat=GetStringFormat(ContentAlignment.MiddleCenter);
		RectangleF rectangleTextGlow;
