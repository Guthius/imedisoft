...
public partial class ZoomSlider : Control{
	...
	private void TextBoxEdit_KeyUp(object sender, KeyEventArgs e){
		var widthText=TextRenderer.MeasureText(textBoxEdit.Text,textBoxEdit.Font).Width;
	...
	protected override void OnPaint(PaintEventArgs pe){
		...
		using var fontForMeasure=new Font(_font.FontFamily,_font.Size);
		var widthOne=(int)g.MeasureString("1",fontForMeasure).Width+2;//because "1" is the minimum value
		//_maximum won't change while mouse is down because drag can't take _value to larger than _maximum
		var widthMax=(int)g.MeasureString(_maximum.ToString(),fontForMeasure).Width+2;
		var widthValue=(int)g.MeasureString(Round(_value).ToString(),fontForMeasure).Width+2;
		var bufferLeft=widthOne/2;
		var bufferRight=widthMax/2;
		...
		var xValuePixels=_rectangleTickLine.Left
		                 +(int)(((float)_rectangleTickLine.Right-_rectangleTickLine.Left)//how many pixels in tick line
			                 /(_maximum-1) *_value);
		_rectangleSlider=new Rectangle(xValuePixels-widthValue/2,9,widthValue,Height-11);
		...
		//Minus
		GraphicsHelper.DrawLine(g,_penBlack,_rectangleMinus.Left+4f,12f,
			_rectangleMinus.Left+13f,12f);
		//Plus:
		GraphicsHelper.DrawLine(g,_penBlack,_rectanglePlus.Left+4f,12f,
			_rectanglePlus.Left+14f,12f);
		GraphicsHelper.DrawLine(g,_penBlack,_rectanglePlus.Left+9f,7f,
			_rectanglePlus.Left+9f,17f);
		...
		var xFitPixels=(int)(((float)_rectangleTickLine.Right-_rectangleTickLine.Left)//how many pixels to the Fit tick line
			/(_maximum-1) *_fit);
		...
		var sVal=Round(_value).ToString();
		g.DrawString(sVal,_font,brushText,_rectangleSlider.X+1,_rectangleSlider.Y-1);
		var points=new PointF[3];
		points[0]=new PointF(xValuePixels,yTickLine);//top point
		points[1]=new PointF(xValuePixels-2.5f,yTickLine+5f);//left point
		points[2]=new PointF(xValuePixels+2.5f,yTickLine+5f);//right point
	...
	protected override void OnMouseDown(MouseEventArgs e){
		...
		if(_rectangleTickLineMargin.Contains(_pointMouseDown)){
			if(_rectangleSlider.Contains(_pointMouseDown)){
				...
				var xPixels=e.X-_rectangleTickLine.Left;
		...
		if(_isMouseDown){
			if(_isMouseDownOnSlider){
				...
				if(e.X+_xOffsetMouseDownSlider<_rectangleTickLine.Left){
					...
					var xPixels=e.X+_xOffsetMouseDownSlider-_rectangleTickLine.Left;
					_value=(float)xPixels/_rectangleTickLine.Width*_maximum;
				}
				Zoomed?.Invoke(this, new EventArgs());
			}
	...
	private void CalculateScale(){
		...
		_penBlack?.Dispose();
		_penDivider=new Pen(Color.FromArgb(180,180,180),1);
		_penSliderOutline=new Pen(Color.FromArgb(50,50,130),1);
		_penTickLine=new Pen(Color.SlateGray,1);
		_penBlack=new Pen(Color.Black,1.5f);
		//recalculate most rectangles
		_rectangleMinus=new Rectangle(0,0,17,Height-2);
		_rectangle100=new Rectangle(Width-30,0,29,Height-2);
		_rectangleFit=new Rectangle(_rectangle100.Left-27,0,27,Height-2);
		_rectanglePlus=new Rectangle(_rectangleFit.Left-17,0,17,Height-2);
