...
public partial class UnmountedBar:Control {
	...
	protected override void OnResize(EventArgs e){
		...
		_linearGradientBrush=new LinearGradientBrush(new Point(0,0),new Point(0,_heightHeader96),_colorTitleTop,_colorTitleBottom);
		_fontTitle?.Dispose();
		_fontTitle=new Font(FontFamily.GenericSansSerif,10f,FontStyle.Bold);
		CalculatePositions();
		Invalidate();
	}
	...
	protected override void OnPaint(PaintEventArgs pe) {
		...
		g.Clear(_colorBack);
		g.FillRectangle(_linearGradientBrush,0,0,Width,_heightHeader96);
		var title="Unmounted";
	...
	private void butDelete_Click(object sender, EventArgs e){
		...
		if(_listUnmountedObjects[SelectedIndex].Document!=null){
			var patnum=_listUnmountedObjects[SelectedIndex].Document.PatNum;
			var patFolder=ImageStore.GetPatientFolder(Patients.GetPat(patnum),ImageStore.GetDataFolder());
	...
	private void CalculatePositions(){
		...
		var padding=3;//just 3 pixels, unscaled, around all 4 sides of each item. 3 pixels between items, not 6
		var xPos=padding;
		var yPos=_heightHeader96+padding;
		var heightTotal=Height;
		if(heightTotal==0){
			heightTotal=200;
		}
		//they all have the same height
		var height=heightTotal-_heightHeader96-1-2*padding;
		for(var i=0;i<_listUnmountedObjects.Count;i++){
	...
	private void DrawMountOne(Graphics g,int i){
		...
		if(_listUnmountedObjects[i].Document.CropW > 0 && _listUnmountedObjects[i].Document.CropH > 0){
			...
			var scale=ImageTools.CalcScaleFit(
