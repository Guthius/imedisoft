...
public partial class CheckBox : Control {
	...
	public ContentAlignment CheckAlign {
		...
		set{
			if(!value.In(ContentAlignment.MiddleLeft,ContentAlignment.MiddleRight,ContentAlignment.TopLeft)){
				...
				throw new Exception(value+" is not a valid alignment. This checkbox only supports MiddleLeft, MiddleRight, and TopLeft.");
			}
			else{
				_checkAlign=value;
			}
	...
	public CheckState CheckState {
		...
		set { 
			...
			if(_checkState==CheckState.Checked){
				_checked=true;//using private field to prevent infinite loop
			}
	...
	protected override void OnPaint(PaintEventArgs e){
		...
		var sqrSize=_sqrSize96;
		//int rectTop=(int)Math.Round(Height/2f-sqrSize/2f,0);
		var rectangleBox=new Rectangle(0,0,100,100);//This defines the checkbox itself.
		var rectangleText=new Rectangle(0,0,100,100);
		...
		if(CheckAlign==ContentAlignment.TopLeft){
			...
			var rectTop=(int)Math.Round(Height/2f-sqrSize/2f,0);
		...
		var penBox=new Pen(ColorOD.Gray(50));//disposed
		var solidBrushText=new SolidBrush(Color.Black);
		if(!Enabled){
			penBox=new Pen(ColorOD.Gray(180));
			solidBrushText=new SolidBrush(ColorOD.DisabledText);
		}
		...
		if(CheckState==CheckState.Indeterminate){
			using var solidBrushIndet=new SolidBrush(ColorOD.Gray(50));
			var intInflate=-2;
			var rectangleIndet=rectangleBox;
			rectangleIndet.Inflate(intInflate,intInflate);
			g.FillRectangle(solidBrushIndet,rectangleIndet);
		}
		else if(Checked){
			using var pen=new Pen(ColorOD.Gray(90),1.6f);
			//short start of check
			g.DrawLine(pen,
				x1:rectangleBox.Left+1.25f,
				y1:rectangleBox.Top+5.75f,
				x2:rectangleBox.Left+4.75f,
				y2:rectangleBox.Top+9.25f);
			//long end of check
			//the end points don't match up because we have to hide the overlap
			g.DrawLine(pen,
				x1:rectangleBox.Left+4f,
				y1:rectangleBox.Top+9.25f,
				x2:rectangleBox.Left+10.5f,
				y2:rectangleBox.Top+2.75f);
		}
		var textTop=Height/2-Font.Height/2;
	...
	public bool IsOnActualText(Point point){
		var sqrSize=_sqrSize96;
		using var g=CreateGraphics();
		//rectangleText=new Rectangle(0,0,Width-sqrSize-4,Height);
		var widthTxt=g.MeasureString(Text,Font,Width-sqrSize-4).Width;
		if(CheckAlign==ContentAlignment.MiddleLeft){
			if(point.X<sqrSize+3){
				return false;//over box
			}
			if(point.X>sqrSize+4+widthTxt){
				return false;//over white space
			}
			return true;
		}
		if(CheckAlign==ContentAlignment.MiddleRight){
			if(point.X>Width-sqrSize-3){
				return false;//over box
			}
			if(point.X<Width-sqrSize-4-widthTxt){
				return false;//over white space
			}
			return true;
		}
		//ContentAlignment.TopLeft)){//I don't think we need to worry about these on FormPreferences, so skip
		return true;
	}
	...
	private bool HitTestBox(int x){
		if(CheckAlign==ContentAlignment.MiddleLeft){
			if(x<_rectangle.Right+(4)){
				return true;
			}
		}
		else{//middle right
			if(x>_rectangle.Left-(4)){
				return true;
			}
		}
		return false;
