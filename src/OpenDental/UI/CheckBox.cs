...
namespace OpenDental.UI {
	...
	public partial class CheckBox : Control {
		...
		public bool IsOnActualText(Point point){
			...
			//rectangleText=new Rectangle(0,0,Width-sqrSize-4,Height);
			float widthTxt=g.MeasureString(Text,Font,Width-sqrSize-4).Width;
			widthTxt=LayoutManager.ScaleMS(widthTxt);//Maybe the graphics object behaves differently?  I shouldn't have to do this.
			if(CheckAlign==ContentAlignment.MiddleLeft){
				if(point.X<sqrSize+LayoutManager.Scale(3)){
			...
			if(CheckAlign==ContentAlignment.MiddleRight){
				if(point.X>Width-sqrSize-LayoutManager.Scale(3)){
					return false;//over box
				}
