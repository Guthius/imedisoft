...
*/
		...
		private List<EnumCategoryBetterDiag> _listEnumCategoryBetterDiagsShown=new List<EnumCategoryBetterDiag>();
		...
		private List<OpenDentBusiness.Pearl.EnumCategoryOD> _listPearlEnumCategoryODsShown=new List<OpenDentBusiness.Pearl.EnumCategoryOD>();
		...
		public void DrawHeaderAndImageToPDF(Document document,Rectangle rectanglePrintArea,XGraphics xg,XFont xFontTitle,XFont xFontSubTitle) {
			...
			Rectangle rectangleAvail=new Rectangle(rectanglePrintArea.X,yTitle,rectanglePrintArea.Width,rectanglePrintArea.Height-yTitle);
		...
		public async void PrintPDF(string filePath) {
			...
			IsPrinting=true;
			ODWebView2 odWebView2Print=new ODWebView2();
			try {
		...
		private async Task<bool> odWebView2_Print(ODWebView2 odWebView2) {
			...
			CoreWebView2PrintStatus printStatus=await odWebView2.CoreWebView2.PrintAsync(printerSettings);
		...
		public string CreatePDF(List<Document> listDocumentsCategory) {
			...
			Rectangle rectangle=new Rectangle(25,40,800,1020);
			using PdfDocument pdfDocument=new PdfDocument();
			PdfDocument pdfDocumentPage=new PdfDocument();
			#region PDFs
			List<Document> listDocumentsPDF=listDocumentsCategory.FindAll(x => x.FileName.ToLower().EndsWith(".pdf"));
			for(int i=0;i<listDocumentsPDF.Count;i++) {
				pdfDocumentPage=PdfReader.Open(ODFileUtils.CombinePaths(filePath,listDocumentsPDF[i].FileName),PdfDocumentOpenMode.Import);
				for(int j=0;j<pdfDocumentPage.PageCount;j++) {
			...
			XGraphics xg;
			using Font fontTitle=new Font(FontFamily.GenericSansSerif,12,FontStyle.Bold);
			using Font fontSubTitle=new Font(FontFamily.GenericSansSerif,10);
			XFont xFontTitle=new XFont(fontTitle.Name,fontTitle.Size,XFontStyle.Bold);
			XFont xFontSubTitle=new XFont(fontSubTitle.Name,fontSubTitle.Size);
			//Only image printing is supported at this time
			List<Document> listDocuments=listDocumentsCategory.FindAll(x => x.MountItemNum == 0 && ImageStore.HasImageExtension(x.FileName));
			...
			for(int i=0;i<listDocuments.Count;i++) {
				...
				string fileName=FileAtoZ.CombinePaths(filePath,listDocuments[i].FileName);
			...
			#endregion
			string path=ODFileUtils.CreateRandomFile(filePath,".pdf");
			try {
		...
		public void SetShowDrawings(bool showOD,bool showAI,List<OpenDentBusiness.Pearl.EnumCategoryOD> listEnumCategoryODs,List<EnumCategoryBetterDiag> listEnumCategoryBetterDiags){
			...
			ProgramName programName=Programs.GetActiveImagingAIProgram();
		...
		public void ToolBarTasks_Click(object sender,EventArgs e) {
			...
			List<long> listTaskNums=TaskAttachments.GeyManyByDocNum(GetDocumentShowing(0).DocNum).Select(x => x.TaskNum).ToList();
			...
			if(_drawMode==EnumDrawMode.LineMeasure){
				...
				for(int i=0;i<_listImageDraws.Count;i++){
					...
					for(int p=1;p<listPointFs2.Count;p++) {
						...
						float length=ImageDraws.CalcLengthLine(listPointFs2);
						//Find the scale
			...
			if(_cropPanAdj==EnumCropPanAdj.Pan && !_isMouseDownPanel){
				...
				if(idxHover==-1){
					...
					if(!panelHover.Visible || labelHover.Text!=_listImageDraws[idxHover].Details) {
						...
						Color colorBack=Color.FromArgb(255,_listImageDraws[idxHover].ColorDraw);
		...
		private void DrawImageToPDF(Document document, XGraphics xg) {
			XGraphicsState xGraphicsState=xg.Save();
			//we're at the center of the crop area, and working in image coordinates
			if(document.CropW>0 && document.CropH>0) {
			...
			XImage xImage=XImage.FromGdiPlusImage(GetBitmapShowing(0));
			xg.DrawImage(xImage,
				0,
				0,
		...
		private void DrawDrawingPDF(float rotateText,ImageDraw imageDraw,XGraphics xg) {
			if(imageDraw.DrawType==ImageDrawType.Text){
				Graphics g=Graphics.FromImage(new Bitmap(100,100));//Only used for measuring
				XGraphicsState graphicsState=xg.Save();
				XPoint xpoint=imageDraw.GetTextPointPDF();
				xg.TranslateTransform(xpoint.X,xpoint.Y);
				using Font font=new Font(FontFamily.GenericSansSerif,imageDraw.GetFontSize());
				string str=imageDraw.GetTextString();
				XFont xfont=ImageHelper.GetXFont(str, font);
				SizeF sizeF=g.MeasureString(str,font);
				g.Dispose();
				Rectangle rectangle=new Rectangle(new Point(0,0),new Size((int)GraphicsHelper.PixelsToPoints((float)sizeF.Width),(int)GraphicsHelper.PixelsToPoints((float)sizeF.Height)));
				...
				|| imageDraw.DrawType==ImageDrawType.Line)
			{
				...
				List<XPoint> listXPoints=imageDraw.GetPointsForPDF();
				XPen pen=new XPen(imageDraw.ColorDraw,2);//2 pixels wide
				XBrush xbrush=new SolidBrush(imageDraw.ColorDraw);
				for(int p=1;p<listXPoints.Count;p++){//start at the second point.
					xg.DrawLine(pen,listXPoints[p-1],listXPoints[p]);
					RectangleF rectangleF=new RectangleF((float)listXPoints[p].X-1,(float)listXPoints[p].Y-1,2,2);
			...
			if(!_stringMeasure.IsNullOrEmpty()){
				...
				XFont xfont=ImageHelper.GetXFont(_stringMeasure,font);
				SizeF size=xg.MeasureString(_stringMeasure,font).ToSizeF();
				size=new Size((int)LayoutManager.ScaleMS(size.Width),(int)LayoutManager.ScaleMS(size.Height));
				Rectangle rectangle=new Rectangle(_pointMeasureText,size.ToSize());
		...
		private void DrawDrawing(float rotateText, ImageDraw imageDraw, Graphics g) {
			if(imageDraw.DrawType==ImageDrawType.Text){
				...
				Point point=imageDraw.GetTextPoint();
				...
				if(imageDraw.ColorBack!=Color.Empty){
					using SolidBrush brushBack=new SolidBrush(imageDraw.ColorBack);
					g.FillRectangle(brushBack,rectangle);
				}
				using SolidBrush solidBrush=new SolidBrush(imageDraw.ColorDraw);
			...
			{
				g.SmoothingMode=SmoothingMode.HighQuality;
				List<PointF> listPointFs=imageDraw.GetPoints();
				using Pen pen=new Pen(imageDraw.ColorDraw,2);//2 pixels wide
				using SolidBrush solidBrush=new SolidBrush(imageDraw.ColorDraw);
		...
		private int HitTestHover(PointF pointfBitmap){
			bool isPearl=Programs.IsEnabled(ProgramName.Pearl);
			...
			if(true) {
				...
				if(localPath!="") {
					...
					if(!ODFileUtils.IsFileInUse(pdfFilePath)) {
						FileAtoZ.Download(FileAtoZ.CombinePaths(atoZFolder,atoZFileName),pdfFilePath);
					}
				}
			}
			else {
