...
*/
	...
	///<summary>List of Better Diagnostics types that are toggled to display. Set as local copy of ControlImages._listEnumCategoryBetterDiagsShown.</summary>
	private List<EnumCategoryBetterDiag> _listEnumCategoryBetterDiagsShown= [];
	...
	///<summary>List of Pearl types that are toggled to display. Set as local copy of ControlImages._listPearlEnumCategoryODsShown.</summary>
	private List<OpenDentBusiness.Pearl.EnumCategoryOD> _listPearlEnumCategoryODsShown= [];
	...
	private List<PointF> _listPointFsDrawing= [];
	...
	///<summary>Indicates whether Pearl drawings should be shown.</summary>
	private bool _showDrawingsAI;
	...
	public ControlImageDisplay() {
		...
		if(!false) {//WebView2 does not work on Cloud
	...
	public void DeleteDocument(bool isVerbose,bool doSecurityCheck,Document document) {
		if(doSecurityCheck) {
			if(!SecurityL.IsAuthorizedToDeleteImage(document)) {
				return;
			}
		}
		var taskAttachment=TaskAttachments.GetOneByDocNum(document.DocNum);
		...
		ClearPDFBrowser();
		var documentArray=new Document[1] { document };	
		try {
	...
	public void DocumentAcquiredForMount(Document document,Bitmap bitmap,MountItem mountItem,bool flipOnAcquire){
		...
		if(mountItem.ItemOrder==-1){//unmounted
			...
			var listDocuments=_documentArrayShowing.ToList();
			listDocuments.Add(document);
			_documentArrayShowing=listDocuments.ToArray();
			var listBitmaps=_bitmapArrayShowing.ToList();
	...
	public void EnableToolBarButtons(){
		...
		if(IsDocumentShowing()){
			...
			var doShowPrint=panelMain.Visible;
			var doShowExport=!GetDocumentShowing(0).FileName.IsNullOrEmpty();
	...
	public void formVideo_BitmapCapturedMount(object sender, Bitmap bitmap, Document document){
		...
		var listMountItemsAvail=GetAvailSlots(1);
		if(listMountItemsAvail is null){//no more available slots, so we are done
			_idxSelectedInMount=-1;
			panelMain.Invalidate();
			return;
		}
		var mountItem=_listMountItems.FirstOrDefault(x=>x.MountItemNum==listMountItemsAvail[0].MountItemNum);
	...
	public List<MountItem> GetAvailSlots(int countNeed=-1){
		...
		var listMountItemsAvail=new List<MountItem>();
		var idxItem=_idxSelectedInMount;
		if(idxItem==-1){
			idxItem=0;
		}
	...
	public List<ImageDraw> GetImageDraws() {
		return _listImageDraws;
	}
	...
	public List<WpfControls.UI.UnmountedObj> GetUmountedObjs(){
		var listUnmountedObjs=new List<WpfControls.UI.UnmountedObj>();
		for(var i=0;i<_listMountItems.Count;i++){
			if(_listMountItems[i].ItemOrder!=-1){
				continue;
			}
			var unmountedObj=new WpfControls.UI.UnmountedObj();
	...
	public void InvalidateSettingsColor() {
		...
		if(IsMountItemSelected()){
			...
			if(_bitmapDicomRaw!=null){
				var bitmapTemp=DicomHelper.ApplyWindowing(_bitmapDicomRaw,_documentArrayShowing[_idxSelectedInMount].WindowingMin,_documentArrayShowing[_idxSelectedInMount].WindowingMax);
	...
	public void Parent_KeyDown(Keys keys){
		...
		if(keys==Keys.ShiftKey){
			...
			var pointInPanel=panelMain.PointToClient(Control.MousePosition);
			var mouseEventArgs=new MouseEventArgs(MouseButtons.Left,1,pointInPanel.X,pointInPanel.Y,0);
	...
	public void PreselectFirstItem(){
		...
		for(var i=0;i<_documentArrayShowing.Length;i++){
	...
	public void printDocument_PrintPage(object sender,System.Drawing.Printing.PrintPageEventArgs e) {
		...
		using var fontSubTitles=new Font(FontFamily.GenericSansSerif,10);
		int xTitle;
		int yTitle;
		string str;
		int widthStr;
		if(IsDocumentShowing()){
			var document=GetDocumentShowing(0);
			...
			yTitle+=20;
			var rectangleAvail=new Rectangle(e.MarginBounds.X,yTitle,e.MarginBounds.Width,e.MarginBounds.Height-yTitle);
		...
		if(_idxSelectedInMount>-1 && _documentArrayShowing[_idxSelectedInMount]!=null) {
			...
			g.TranslateTransform(rectangleAvail.X+rectangleAvail.Width/2,rectangleAvail.Y+rectangleAvail.Height/2);
			var scaleMount=ImageTools.CalcScaleFit(
	...
	/// <summary>Document must be an image. Draws the header followed by the image on xgraphics within the printable area specified. Used for PDFs. Logic is similar to the print document section of printDocument_PrintPage.</summary>
	public void DrawHeaderAndImageToPDF(Document document,Rectangle rectanglePrintArea,XGraphics xg,XFont xFontTitle,XFont xFontSubTitle) {
		int xTitle;
		int yTitle;
		string str;
		int widthStr;
		xTitle=rectanglePrintArea.X+rectanglePrintArea.Width/2;
		yTitle=rectanglePrintArea.Top;
		//Job 20502 added this info for all patient images
		//Job 35304 removed this info except for xrays because some people found it annoying
		//Job 50474 added this info back for toothchart.
		//Job 51614 removed this for Perio, Graphical Perio, and toothcharts.
		...
		if(document.PrintHeading) {
			str=PatientCur.GetNameLF();
			widthStr=(int)xg.MeasureString(str,xFontTitle).Width;
			XBrush xbrush=Brushes.Black;
			xg.DrawString(str,xFontTitle,xbrush,GraphicsHelper.PixelsToPoints(xTitle-widthStr/2),GraphicsHelper.PixelsToPoints(yTitle));
			yTitle+=xFontTitle.Height+4;
			str="DOB: "+PatientCur.Birthdate.ToShortDateString();
			widthStr=(int)xg.MeasureString(str,xFontTitle).Width;
			xg.DrawString(str,xFontTitle,xbrush,GraphicsHelper.PixelsToPoints(xTitle - widthStr / 2),GraphicsHelper.PixelsToPoints(yTitle));
			yTitle+=xFontTitle.Height;
			str=document.DateCreated.ToShortDateString();
			widthStr=(int)xg.MeasureString(str,xFontTitle).Width;
			xg.DrawString(str,xFontTitle,xbrush,GraphicsHelper.PixelsToPoints(xTitle - widthStr / 2),GraphicsHelper.PixelsToPoints(yTitle));
			yTitle+=xFontTitle.Height;
		...
		yTitle+=(int)GraphicsHelper.PixelsToPoints(20);
		var rectangleAvail=new Rectangle(rectanglePrintArea.X,yTitle,rectanglePrintArea.Width,rectanglePrintArea.Height-yTitle);
		xg.TranslateTransform(GraphicsHelper.PixelsToPoints(rectangleAvail.X+rectangleAvail.Width/2),
			GraphicsHelper.PixelsToPoints(rectangleAvail.Y+rectangleAvail.Height/2));
		float scaleFactor;
		if(document.CropW==0) {//no crop
			scaleFactor=ImageTools.CalcScaleFit(rectangleAvail.Size,
				new Size(GetBitmapShowing(0).Width,GetBitmapShowing(0).Height),document.DegreesRotated);
		}
		else {
			scaleFactor=ImageTools.CalcScaleFit(rectangleAvail.Size,new Size(document.CropW,document.CropH),document.DegreesRotated);
		}
		xg.ScaleTransform(scaleFactor,scaleFactor);
		//We are now at center of page and crop area, and working in image coords
		DrawImageToPDF(document,xg);
	}

	/// <summary>Prints the file at the given filepath using WebView2. Async because we need to await for webview2 to initialize itself.</summary>
	public async void PrintPDF(string filePath) {
		if(filePath.IsNullOrEmpty() || !filePath.ToLower().EndsWith(".pdf")) {
			return;
		}
		IsPrinting=true;
		var odWebView2Print=new ODWebView2();
		try {
			await odWebView2Print.Init();
		}
		catch(Exception ex) {
			//Init failed, nothing to do
			FriendlyException.Show("Error printing PDFs.",ex);
			odWebView2Print.Dispose();
			IsPrinting=false;
			return;
		}
		odWebView2Print.NavigationCompleted+=odWebView2_NavigationCompleted;
		//Open the PDF in WebView2 so that we can print.
		odWebView2Print.OdWebView2Navigate(filePath);
	}

	private async void odWebView2_NavigationCompleted(object sender,CoreWebView2NavigationCompletedEventArgs e) {
		//If we get here, the document has loaded in webview2 so print it.
		ODWebView2 odWebView2=null;
		try {
			odWebView2=(ODWebView2)sender;
			await odWebView2_Print(odWebView2);
		}
		catch {
			ShowError=true;
			IsPrinting=false;
	...
	/// <summary>Prints the top-level PDF in _odWebView2 using the default printer. https://learn.microsoft.com/en-us/microsoft-edge/webview2/how-to/print?tabs=dotnetcsharp</summary>
	private async Task<bool> odWebView2_Print(ODWebView2 odWebView2) {
		//Create default print settings
		var webView2Environment=await CoreWebView2Environment.CreateAsync();
		var printerSettings=webView2Environment.CreatePrintSettings();
		//Print the current document
		var printStatus=await odWebView2.CoreWebView2.PrintAsync(printerSettings);
	...
	/// <summary>Creates one PDF with pages for all the documents passed in. Saves the PDF as a temp file and returns that full path to that temp file. Accepted file types are PDF and images. Mounts are not supported at this time.</summary>
	public string CreatePDF(List<Document> listDocumentsCategory) {
		var filePath=ImageStore.GetPatientFolder(PatientCur,ImageStore.GetDataFolder());
		//Create the pdf
		//Assume 8.5 x 11 sheet with Margins(25,40,800,1020). This mirrors default printing logic for single images.
		var rectangle=new Rectangle(25,40,800,1020);
		using var pdfDocument=new PdfDocument();
		var pdfDocumentPage=new PdfDocument();
		#region PDFs
		var listDocumentsPDF=listDocumentsCategory.FindAll(x => x.FileName.ToLower().EndsWith(".pdf"));
		for(var i=0;i<listDocumentsPDF.Count;i++) {
			pdfDocumentPage=PdfReader.Open(ODFileUtils.CombinePaths(filePath,listDocumentsPDF[i].FileName),PdfDocumentOpenMode.Import);
			for(var j=0;j<pdfDocumentPage.PageCount;j++) {
				pdfDocument.AddPage(pdfDocumentPage.Pages[j]);
			}
		}
		pdfDocumentPage.Dispose();
		#endregion
		#region Images
		XGraphics xg;
		using var fontTitle=new Font(FontFamily.GenericSansSerif,12,FontStyle.Bold);
		using var fontSubTitle=new Font(FontFamily.GenericSansSerif,10);
		var xFontTitle=new XFont(fontTitle.Name,fontTitle.Size,XFontStyle.Bold);
		var xFontSubTitle=new XFont(fontSubTitle.Name,fontSubTitle.Size);
		//Only image printing is supported at this time
		var listDocuments=listDocumentsCategory.FindAll(x => x.MountItemNum == 0 && ImageStore.HasImageExtension(x.FileName));
		...
		for(var i=0;i<listDocuments.Count;i++) {
			...
			try {
				_bitmapArrayShowing[0]=ImageHelper.GetBitmapPDF(fileName,memoryStream);
			}
			catch {
				//File IO exception most likely, can't load bitmap
				ShowError=true;
				continue;
			}
			_documentArrayShowing=new Document[1];
			_documentArrayShowing[0]=listDocuments[i];
			_listImageDraws=ImageDraws.RefreshForDoc(listDocuments[i].DocNum);
			_nodeTypeKeyCatSelected=new NodeTypeKeyCat();
			_nodeTypeKeyCatSelected.ImageNodeType=EnumImageNodeType.Document;
			ImageHelper.ApplyColorSettings(_bitmapArrayShowing[0],listDocuments[i].WindowingMin,listDocuments[i].WindowingMax);
			_documentArrayShowing[0]=listDocuments[i];
			DrawHeaderAndImageToPDF(listDocuments[i],rectangle,xg,xFontTitle,xFontSubTitle);
		...
		#endregion
		var path=ODFileUtils.CreateRandomFile(filePath,".pdf");
		try {
	...
	public void SelectTreeNode2(NodeTypeAndKey nodeTypeAndKey,string localPathImportedCloud="") {
		...
		if(nodeTypeAndKey.NodeType==EnumImageNodeType.Document){
			...
			if(_bitmapRaw==null && _bitmapDicomRaw==null) {
				...
				if(ImageHelper.HasImageExtension(document.FileName)) {
					var srcFileName = ODFileUtils.CombinePaths(PatFolder,document.FileName);
		...
		if(nodeTypeAndKey.NodeType==EnumImageNodeType.Mount){
			...
			var listMissingMountNums=new List<int>();//List to count missing images not found in the A-Z folder
			...
			if(listMissingMountNums.Count>0) {//Notify user of any files that were unable to load
				var errorMessage=Lan.g(this,"Files not found for mount:")+"\r\n";
				for(var m=0;m<listMissingMountNums.Count;m++) {
	...
	public void SetDrawMode(EnumDrawMode drawMode){
		...
		switch(_drawMode){
			...
			case EnumDrawMode.Polygon://for now, just use the pen
				using(var memoryStream=new MemoryStream(Properties.Resources.Pen)){
					panelMain.Cursor=new Cursor(memoryStream);
				}
				break;
			case EnumDrawMode.Eraser:
				using(var memoryStream=new MemoryStream(Properties.Resources.EraseCircle)){
					panelMain.Cursor=new Cursor(memoryStream);
				}
				break;
			case EnumDrawMode.ChangeColor:
				using(var memoryStream=new MemoryStream(Properties.Resources.ColorChanger)){
					panelMain.Cursor=new Cursor(memoryStream);
				}
				break;
		}
	...
	public void SetShowDrawings(bool showOD,bool showAI,List<OpenDentBusiness.Pearl.EnumCategoryOD> listEnumCategoryODs,List<EnumCategoryBetterDiag> listEnumCategoryBetterDiags){
		_showDrawingsOD=showOD;
		_showDrawingsAI=showAI;
		var programName=Programs.GetActiveImagingAIProgram();
		if(programName==ProgramName.None) {
			_showDrawingsAI=false;
		}
		_listPearlEnumCategoryODsShown=listEnumCategoryODs;
		_listEnumCategoryBetterDiagsShown=listEnumCategoryBetterDiags;
		if(IsBitmapShowing()) {
	...
	public void SetWindowingSlider() {
		var windowingEventArgs=new WindowingEventArgs();
		if(IsDocumentShowing()){
	...
	{
		panelMain.Size = ClientSize;//LayoutManager didn't seem to be doing this in some cases.
		if(_odWebView2!=null) {
			_odWebView2.Size=ClientSize;
		}
		if(IsDocumentShowing()){
			if(_bitmapRaw==null && _bitmapDicomRaw==null){
			...
			if(_bitmapRaw!=null){
				var sizeImage=new System.Windows.Size(_bitmapRaw.Width,_bitmapRaw.Height);
			...
			if(_bitmapDicomRaw!=null){
				var sizeImage=new System.Windows.Size(_bitmapDicomRaw.Width,_bitmapDicomRaw.Height);
				if(GetDocumentShowing(0).CropW>0){
					sizeImage=new System.Windows.Size(GetDocumentShowing(0).CropW,GetDocumentShowing(0).CropH);
				}
	...
	public void ToolBarCopy_Click(object sender, EventArgs e){
		...
		var fileName="";
		var dbNameOrUri="";//Used to prevent issues with copying images across different databases.
		...
		if(IsMountItemSelected()){
			...
			if(ImageHelper.HasImageExtension(GetDocumentShowing(0).FileName) || GetDocumentShowing(0).FileName.EndsWith(".dcm")){
				...
				if(!ImageHelper.HasImageBeenEdited(GetDocumentShowing(0))) {//Do a file copy since no changes were made
					var filePathSource=Documents.GetPath(GetDocumentShowing(0).DocNum);
	...
	public void ToolBarDelete_Click(object sender,EventArgs e){
		...
		var nodeTypeAndKeyAbove=ImageSelector_.GetItemAbove();//always successful
		if(IsDocumentShowing()){
		...
		var isEmpty=true;
		var listMountItems=MountItems.GetItemsForMount(_mountShowing.MountNum);
		var listDocumentsShowing=_documentArrayShowing.Where(x => x!=null).ToList();
		var documentArrayInMountDb=Documents.GetDocumentsForMountItems(listMountItems);
		for(var i=0;i<documentArrayInMountDb.Length;i++) {
			if(documentArrayInMountDb[i]==null){
				continue;
			}
			isEmpty=false;
			var document=listDocumentsShowing.Find(x=> x.DocNum==documentArrayInMountDb[i].DocNum);
		...
		var defDocCategory = Defs.GetDef(DefCat.ImageCats,_mountShowing.DocCategory);
		var logText = "Mount Deleted: "+_mountShowing.Description+" with category "
	...
	public void ToolBarExport_Click(bool doExportAsTiff=false){
		if(IsDocumentShowing()){
			...
			var isTiff = Path.GetExtension(GetDocumentShowing(0).FileName)==".tiff" || Path.GetExtension(GetDocumentShowing(0).FileName)==".tif";
			...
			if(doExportAsTiff) {
				...
				if(isTiff) {
					var newFileName=GetDocumentShowing(0).FileName.Replace(Path.GetExtension(GetDocumentShowing(0).FileName), ".jpg");
			...
			if(ImageHelper.HasImageExtension(GetDocumentShowing(0).FileName)){
				using var bitmapCopy=ImageHelper.CopyWithCropRotate(GetDocumentShowing(0),GetBitmapShowing(0));
				if(bitmapCopy==null){
			...
			var defDocCategory=Defs.GetDef(DefCat.ImageCats,GetDocumentShowing(0).DocCategory);
			var logText="Document Exported: "+GetDocumentShowing(0).FileName+" with category "
		...
		if(IsMountItemSelected()){
			...
			var isTiff = Path.GetExtension(_documentArrayShowing[_idxSelectedInMount].FileName)==".tiff" || Path.GetExtension(_documentArrayShowing[_idxSelectedInMount].FileName)==".tif";
			var saveFileDialog=new SaveFileDialog();
			...
			if(doExportAsTiff) {
				...
				if(isTiff) {
					var newFileName=GetDocumentShowing(_idxSelectedInMount).FileName.Replace(Path.GetExtension(GetDocumentShowing(_idxSelectedInMount).FileName), ".jpg");
					saveFileDialog.FileName=newFileName;
				}
				else {
			...
			if(ImageHelper.HasImageExtension(GetDocumentShowing(_idxSelectedInMount).FileName))
			{
				...
				using var bitmapCopy=new Bitmap(_listMountItems[_idxSelectedInMount].Width,_listMountItems[_idxSelectedInMount].Height);
				using var g=Graphics.FromImage(bitmapCopy);
				//translate to center of drawing area (and center of mount item)
				g.TranslateTransform(bitmapCopy.Width/2,bitmapCopy.Height/2);
				var scaleMount=ImageTools.CalcScaleFit(
			...
			MsgBox.Show(this,"Done.");
			var defDocCategory=Defs.GetDef(DefCat.ImageCats,_documentArrayShowing[_idxSelectedInMount].DocCategory);
			var logText="Document Exported: "+_documentArrayShowing[_idxSelectedInMount].FileName+" within mount "
		...
		if(IsMountShowing()){
			...
			var bitmapExport=new Bitmap(_mountShowing.Width,_mountShowing.Height);
			var g=Graphics.FromImage(bitmapExport);
			...
			MsgBox.Show(this,"Done.");
			var defDocCategory=Defs.GetDef(DefCat.ImageCats,_mountShowing.DocCategory);
			var logText="Mount Exported: "+_mountShowing.Description+" with category "
			            +defDocCategory.ItemName+" to "+Path.GetDirectoryName(saveFileDialog.FileName);
	...
	public void ToolBarFlip_Click(){
		if(IsDocumentShowing()) {
			if(!SecurityL.IsAuthorizedToEditImage(GetDocumentShowing(0))) {
				return;
			}	
			var documentOld=GetDocumentShowing(0).Copy();
			...
			var defDocCategory=Defs.GetDef(DefCat.ImageCats,GetDocumentShowing(0).DocCategory);
			var logText="Document Edited: "+GetDocumentShowing(0).FileName+" with category "
			            +defDocCategory.ItemName+" flipped horizontally";
			SecurityLogs.MakeLogEntry(EnumPermType.ImageEdit,PatientCur.PatNum,logText);
		}
		if(IsMountItemSelected()){ 
			if(!SecurityL.IsAuthorizedToEditImage(_documentArrayShowing[_idxSelectedInMount])) {
				return;
			}	
			var documentOld=_documentArrayShowing[_idxSelectedInMount].Copy();
			...
			var defDocCategory=Defs.GetDef(DefCat.ImageCats,_documentArrayShowing[_idxSelectedInMount].DocCategory);
			var logText="Document Edited: "+_documentArrayShowing[_idxSelectedInMount].FileName+" within mount "
	...
	public void ToolBarInfo_Click(object sender,EventArgs e){
		if(IsMountItemSelected()){
			...
			var frmDocInfo=new FrmDocInfo(PatientCur,_documentArrayShowing[_idxSelectedInMount]);
		...
		if(IsMountShowing()) {
			var frmMountEdit=new FrmMountEdit();
		...
		if(IsDocumentShowing()) {
			var frmDocInfo=new FrmDocInfo(PatientCur,GetDocumentShowing(0));
			frmDocInfo.ShowDialog();
	...
	public void ToolBarTasks_Click(object sender,EventArgs e) {
		if(!IsDocumentShowing()) {
			return;
		}
		var listTaskNums=TaskAttachments.GeyManyByDocNum(GetDocumentShowing(0).DocNum).Select(x => x.TaskNum).ToList();
		if(listTaskNums.IsNullOrEmpty()) {
			//No TaskNums found for the selected document. Pass null to FormTaskSearch to display no tasks.
			listTaskNums=null;
		}
		using var formTaskSearch=new FormTaskSearch(listTaskNums);
	...
	public void ToolBarPaste_Click(object sender,EventArgs e){
		...
		if(IsMountShowing()){
			if(bitmapPaste!=null){
				...
				Document doc=null;
				try {
					doc=ImageStore.Import(bitmapPaste,GetCurrentCategory(),ImageType.Photo,PatientCur);//Makes log
					var docOld=doc.Copy();
				...
				if(OpenDentBusiness.Bridges.Pearl.DoAutoUploadForImageCategory(doc.DocCategory)) {
					var pearl=OpenDentBusiness.Bridges.Pearl.SetupPearlForSendingSingle(PatientCur,doc,bitmapPaste,mountItem:_listMountItems[_idxSelectedInMount]);
					pearl?.SendOnThread();//Mount showing, importing single image
				}
				else if(OpenDentBusiness.Bridges.BetterDiag.DoAutoUploadForImageCategory(doc.DocCategory)) {
					OpenDentBusiness.Bridges.BetterDiag.SendSingleOnThread(PatientCur,doc,bitmapPaste,mountItem:_listMountItems[_idxSelectedInMount]);
				}
				Cursor=Cursors.Default;
				EventSelectTreeNode?.Invoke(this,null);
			}
				...
				var listAvail=GetAvailSlots(stringArrayfileNames.Length);
				if(listAvail==null){
				...
				for(var i=0;i<stringArrayfileNames.Length;i++) {
					...
					if(OpenDentBusiness.Bridges.Pearl.DoAutoUploadForImageCategory(doc.DocCategory)) {
						var pearl = OpenDentBusiness.Bridges.Pearl.SetupPearlForSendingSingle(PatientCur,doc,filePath:stringArrayfileNames[i],mountItem:listAvail[i]);
						pearl?.SendOnThread();//Mount showing, importing multiple images
					}
					else if(OpenDentBusiness.Bridges.BetterDiag.DoAutoUploadForImageCategory(doc.DocCategory)) {
						OpenDentBusiness.Bridges.BetterDiag.SendSingleOnThread(PatientCur,doc,filePath:stringArrayfileNames[i],mountItem:listAvail[i]);
					}
				}//for					
				Cursor=Cursors.Default;
				EventSelectTreeNode?.Invoke(this,null);
		...
		if(bitmapPaste!=null){
			...
			var frmDocInfo = new FrmDocInfo(PatientCur,document,isDocCreate: true);
			...
			if(OpenDentBusiness.Bridges.Pearl.DoAutoUploadForImageCategory(document.DocCategory)) {
				var pearl=OpenDentBusiness.Bridges.Pearl.SetupPearlForSendingSingle(PatientCur,document,bitmapPaste);
				pearl?.SendOnThread();//Not mount, importing single image
			}
			else if(OpenDentBusiness.Bridges.BetterDiag.DoAutoUploadForImageCategory(document.DocCategory)) {
				OpenDentBusiness.Bridges.BetterDiag.SendSingleOnThread(PatientCur,document,bitmapPaste);
			}
		}
		else{//files
			for(var i=0;i<stringArrayfileNames.Length;i++) {
				...
				if(stringArrayfileNames.Length>1){
					...
					if(OpenDentBusiness.Bridges.Pearl.DoAutoUploadForImageCategory(document.DocCategory)) {
						var pearl=OpenDentBusiness.Bridges.Pearl.SetupPearlForSendingSingle(PatientCur,document,filePath:stringArrayfileNames[i]);
						pearl?.SendOnThread();//Not mount, importing single image as part of multiple (will repeat on loop)
					}
					else if(OpenDentBusiness.Bridges.BetterDiag.DoAutoUploadForImageCategory(document.DocCategory)) {
						OpenDentBusiness.Bridges.BetterDiag.SendSingleOnThread(PatientCur,document,filePath:stringArrayfileNames[i]);
					}
					nodeTypeAndKey=new NodeTypeAndKey(EnumImageNodeType.Document,document.DocNum);
					continue;
				}
	...
	public void ToolBarPasteTypeAndKey(NodeTypeAndKey nodeTypeAndKey,bool showDocInfo){
		...
		if(nodeTypeAndKey.NodeType==EnumImageNodeType.Document){//pasting from document
			...
			var documentOriginal=Documents.GetByNum(nodeTypeAndKey.PriKey);
			var patientOriginal=Patients.GetPat(documentOriginal.PatNum);
			...
			var mountOriginal=Mounts.GetByNum(nodeTypeAndKey.PriKey);
			var patientOriginal=Patients.GetPat(mountOriginal.PatNum);
			var mount=mountOriginal.Copy();
			mount.PatNum=PatientCur.PatNum;
			mount.DocCategory=GetCurrentCategory();
			Mounts.Insert(mount);
			var listMountItemsOrig=MountItems.GetItemsForMount(mountOriginal.MountNum);
			for(var i=0;i<listMountItemsOrig.Count;i++){
				var mountItem=listMountItemsOrig[i].Copy();
				mountItem.MountNum=mount.MountNum;
				MountItems.Insert(mountItem);
				var documentOriginal=Documents.GetDocumentForMountItem(listMountItemsOrig[i].MountItemNum);
	...
	public void ToolBarRotateL_Click(){
		...
		if(IsMountItemSelected()) {
			...
			if(_documentArrayShowing[_idxSelectedInMount].CropW != 0){
				...
				var w=_documentArrayShowing[_idxSelectedInMount].CropH;
	...
	public void ToolBarSize_Click(){
		if(IsDocumentShowing()){
			...
			var frmDocumentSize=new FrmDocumentSize();
			...
			if(frmDocumentSize.IsDialogOK) {
				EventFillTree?.Invoke(this,true);
				EventSelectTreeNode?.Invoke(this,null);
				var defDocCategory=Defs.GetDef(DefCat.ImageCats,_documentArrayShowing[0].DocCategory);
				var logText="Document Edited: "+_documentArrayShowing[0].FileName+" with category "
		...
		if(IsMountItemSelected()){
			...
			var frmDocumentSizeMount=new FrmDocumentSizeMount();
	...
	public void ToolBarZoomOne_Click(){
		...
		var oldZoom=ZoomSliderValue;
		var newZoom=ImageTools.CalcScaleFit(new Size(panelMain.Width,panelMain.Height),
			new Size(_listMountItems[_idxSelectedInMount].Width,_listMountItems[_idxSelectedInMount].Height),0);
		var intZoom=(int)(newZoom*95);//convert from fraction to percent and fit in bounds with some padding
	...
	private void menuPanelMain_Click(object sender,System.EventArgs e) {
		...
		switch(((MenuItem)sender).Index) {
			...
			case 7://info
				...
				break;
			case 8://Tasks
				ToolBarTasks_Click(this,new EventArgs());
				break;
		}
	}
	...
	private void panelMain_DragDrop(object sender,DragEventArgs e) {
		...
		var stringArrayFiles=(string[])e.Data.GetData(DataFormats.FileDrop);
		if(stringArrayFiles.Length==0){
			return;
		}
		var pointOver=panelMain.PointToClient(new Point(e.X,e.Y));
		_idxSelectedInMount=HitTestInMount(pointOver.X,pointOver.Y);
		var listMountItemsAvail=GetAvailSlots(stringArrayFiles.Length);
		...
		for(var i=0;i<stringArrayFiles.Length;i++) {
			...
			Documents.Update(document,documentOld);
			if(OpenDentBusiness.Bridges.Pearl.DoAutoUploadForImageCategory(document.DocCategory)) {
				var pearl=OpenDentBusiness.Bridges.Pearl.SetupPearlForSendingSingle(PatientCur,document,filePath:stringArrayFiles[i],mountItem:listMountItemsAvail[i]);
				pearl?.SendOnThread();
			}
			else if(OpenDentBusiness.Bridges.BetterDiag.DoAutoUploadForImageCategory(document.DocCategory)) {
				OpenDentBusiness.Bridges.BetterDiag.SendSingleOnThread(PatientCur,document,filePath:stringArrayFiles[i],mountItem:listMountItemsAvail[i]);
			}
		}//for
		EventSelectTreeNode?.Invoke(this,null);
	}
	...
	private void panelMain_MouseDoubleClick(object sender,MouseEventArgs e) {
		if(_drawMode==EnumDrawMode.Text){
			...
			var frmImageDrawEdit=new FrmImageDrawEdit();
		...
		if(IsMountShowing()){
			if(_idxSelectedInMount==-1){
				var idx=HitTestInMount(e.X,e.Y,includeText:true);
				if(idx>-1){//double clicked on text
					var frmMountItemEdit=new FrmMountItemEdit();
	...
	private void panelMain_MouseDown(object sender, MouseEventArgs e){
		...
		for(var i=0;i<menuPanelMain.MenuItems.Count;i++) {
		...
		if(_drawMode==EnumDrawMode.Text){
			var pointCursorInBitmap=ControlPointToBitmapPoint(e.Location);
			var idxText=HitTestText(e.Location);
			if(idxText!=-1){
			...
			frmImageDrawEdit.ImageDrawCur.FontSize=fontSizeApparent/ZoomSliderValue*100f*1;
			frmImageDrawEdit.ZoomVal=ZoomSliderValue;
			if(IsMountShowing()){
		...
		if(_drawMode==EnumDrawMode.Pen){
			var pointFPenInBitmap=ControlPointToBitmapPointF(e.Location);
		...
		if(_drawMode==EnumDrawMode.Line){
			...
			var testRadius=3f/ZoomSliderValue*100f;//so the test radius appears the same to the user, regardless of zoom
			for(var i=0;i<_listImageDraws.Count;i++){
				if(_listImageDraws[i].DrawType!=ImageDrawType.Line){
					continue;
				}
				var listPointFs=_listImageDraws[i].GetPoints();
				//Only test the last point. Not allowed to extend from any other point.
				var dist=(float)Math.Sqrt(Math.Pow(pointFPenInBitmap.X-listPointFs[listPointFs.Count-1].X,2)+Math.Pow(pointFPenInBitmap.Y-listPointFs[listPointFs.Count-1].Y,2));
				if(dist>testRadius){
					continue;
				}
		...
		if(_drawMode==EnumDrawMode.LineEditPoints){
			...
			for(var i=0;i<_listImageDraws.Count;i++){
				...
				for(var p=0;p<listPointFs.Count;p++) {
					var dist=(float)Math.Sqrt(Math.Pow(pointFPenInBitmap.X-listPointFs[p].X,2)+Math.Pow(pointFPenInBitmap.Y-listPointFs[p].Y,2));
					if(dist>testRadius){
						continue;
					}
		...
		if(_drawMode==EnumDrawMode.LineSetScale){
			...
			for(var i=0;i<_listImageDraws.Count;i++){
				...
				for(var p=1;p<listPointFs.Count;p++) {
					...
					var frmImageScale=new FrmImageScale();
					frmImageScale.Pixels=ImageDraws.CalcLengthLine(listPointFs);
					...
					for(var j=0;j<_listImageDraws.Count;j++){
		...
		if(e.Button==MouseButtons.Right){
			...
			var idxSelectedInMountOld=_idxSelectedInMount;
		...
		if(_cropPanAdj==EnumCropPanAdj.Adj){
			...
			var pointRaw=ControlPointToBitmapPoint(e.Location);
			var idxSelectedInMountOld=_idxSelectedInMount;
			_idxSelectedInMount=-1;
			for(var i=0;i<_listMountItems.Count;i++){
				var rect=new Rectangle(_listMountItems[i].Xpos,_listMountItems[i].Ypos,_listMountItems[i].Width,_listMountItems[i].Height);
	...
	private void panelMain_MouseMove(object sender, MouseEventArgs e){
		...
		var imageNodeType=GetSelectedType();//for debugging
		var pointCursorInBitmap=ControlPointToBitmapPoint(e.Location);
		if(_drawMode==EnumDrawMode.Text){
			if(_isMouseDownPanel) {//dragging
				...
				var pointDownInBitmap=ControlPointToBitmapPoint(_pointMouseDown);
				var pointNew=new Point(
		   ...
		   || _drawMode==EnumDrawMode.Polygon)
		{
			...
			var pointFPenInBitmap=ControlPointToBitmapPointF(e.Location);
			var pointFLast=new PointF();
			if(_listPointFsDrawing.Count>0){
				pointFLast=_listPointFsDrawing[_listPointFsDrawing.Count-1];
			}
			var dist=Math.Abs(pointFLast.X-pointFPenInBitmap.X)+Math.Abs(pointFLast.Y-pointFPenInBitmap.Y);
			if(dist<3){
		...
		if(_drawMode==EnumDrawMode.LineEditPoints){
			...
			if(_isMouseDownPanel) {
				...
				var listPointFs=_listImageDraws[_idxImageDraw].GetPoints();
			...
			for(var i=0;i<_listImageDraws.Count;i++){
				...
				var listPointFs2=_listImageDraws[i].GetPoints();
				//First, test end points
				for(var p=0;p<listPointFs2.Count;p++) {
					var dist=(float)Math.Sqrt(Math.Pow(pointFPenInBitmap.X-listPointFs2[p].X,2)+Math.Pow(pointFPenInBitmap.Y-listPointFs2[p].Y,2));
					if(dist<testRadius){
						if(IsShiftDown()){
				...
				for(var p=1;p<listPointFs2.Count;p++) {
		...
		if(_drawMode==EnumDrawMode.LineMeasure){
			var pointFPenInBitmap=ControlPointToBitmapPointF(e.Location);
			var testRadius=3f/ZoomSliderValue*100f;//so the test radius appears the same to the user, regardless of zoom
			var pointShift=new Point((int)(17f/ZoomSliderValue*100f),(int)(4f/ZoomSliderValue*100f));
			var stringMeasure="";
			for(var i=0;i<_listImageDraws.Count;i++){
				if(_listImageDraws[i].DrawType!=ImageDrawType.Line){
					continue;
				...
				for(var p=1;p<listPointFs2.Count;p++) {
					...
					var length=ImageDraws.CalcLengthLine(listPointFs2);
					//Find the scale
					...
					var scaleVal=imageDrawScale.GetScale();
					var stringUnits=imageDrawScale.GetScaleUnits();
					var decimalPlaces=imageDrawScale.GetDecimals();
					var lengthScaled=length/scaleVal;
					stringMeasure=lengthScaled.ToString("f"+decimalPlaces);
		...
		if(_cropPanAdj==EnumCropPanAdj.Pan && !_isMouseDownPanel){
			//hover effect
			var pointFPenInBitmap=ControlPointToBitmapPointF(e.Location);//either bitmap coords or mount coords
			var idxHover=HitTestHover(pointFPenInBitmap);
			if(idxHover==-1){
				panelHover.Visible=false;
			}
			else{
				//panelHover was initialized in the ctor at set visible false
				if(!panelHover.Visible || labelHover.Text!=_listImageDraws[idxHover].Details) {
					//just the first time that it's made visible or if hovering over different ImageDraw.
					panelHover.Visible=true;
					var colorBack=Color.FromArgb(255,_listImageDraws[idxHover].ColorDraw);
					panelHover.BackColor=colorBack;//Set to fully opaque
					using var g=panelHover.CreateGraphics();
					using var font=new Font(FontFamily.GenericSansSerif,8.25f);
					var sizeF=g.MeasureString(_listImageDraws[idxHover].Details,font,300);//,stringFormat
					var sizePanel=new Size((int)sizeF.Width+11,(int)sizeF.Height+11);
					panelHover.Size=sizePanel;
					...
					labelHover.ForeColor=Color.Black;
					if(colorBack.R*2 + colorBack.G*7 + colorBack.B<500) {
						labelHover.ForeColor=Color.White;
					}
				}
				//this needs to be done whether or not it was already visible
				panelHover.Location=new Point(e.X+15,e.Y+5);
			}
		}
		if(!_isMouseDownPanel) {
			return;
		}
		var scaleTrans=(float)100/ZoomSliderValue;//example, 200%, 100/200=.5, indicating .5 bitmap pixels for each screen pixel
		if(_cropPanAdj==EnumCropPanAdj.Crop) {
			var rectangle1=new Rectangle(Math.Min(e.Location.X,_pointMouseDown.X), Math.Min(e.Location.Y,_pointMouseDown.Y),
				Math.Abs(e.Location.X-_pointMouseDown.X),	Math.Abs(e.Location.Y-_pointMouseDown.Y));
			var rectangle2=new Rectangle(0,0,panelMain.Width-1,panelMain.Height-1);
			_rectangleCrop=Rectangle.Intersect(rectangle1,rectangle2);
			panelMain.Invalidate();
		}
		if(_cropPanAdj==EnumCropPanAdj.Pan){
			//scaleTrans=1;
			var xTrans=(int)(_pointTranslationOld.X+(e.Location.X-_pointMouseDown.X)*scaleTrans);
			var yTrans=(int)(_pointTranslationOld.Y+(e.Location.Y-_pointMouseDown.Y)*scaleTrans);
		...
		if(_cropPanAdj==EnumCropPanAdj.Adj){
			...
			var xTrans=_pointDragStart.X+(ControlPointToBitmapPoint(e.Location).X-ControlPointToBitmapPoint(_pointMouseDown).X);
			var yTrans=_pointDragStart.Y+(ControlPointToBitmapPoint(e.Location).Y-ControlPointToBitmapPoint(_pointMouseDown).Y);
	...
	private void panelMain_MouseUp(object sender, MouseEventArgs e){
		...
		if(GetSelectedType()==EnumImageNodeType.Mount && !isClick && _cropPanAdj==EnumCropPanAdj.Adj && _isDraggingMount){
			...
			var idxNewPos=HitTestInMount(_pointDragNow.X,_pointDragNow.Y,true);//pointCenter.X,pointCenter.Y);
			if(idxNewPos==-1){
				_isDraggingMount=false;
				panelMain.Invalidate();
				return;
			...
			if(idxNewPos!=_idxSelectedInMount && _documentArrayShowing[idxNewPos]!=null){
				...
				var mountItemNumOldPos=_documentArrayShowing[_idxSelectedInMount].MountItemNum;
				...
				var bitmapOldPos=_bitmapArrayShowing[_idxSelectedInMount];//no dispose, just ref
				_bitmapArrayShowing[_idxSelectedInMount]=_bitmapArrayShowing[idxNewPos];
				_bitmapArrayShowing[idxNewPos]=bitmapOldPos;
				var documentOldPos=_documentArrayShowing[_idxSelectedInMount];
			...
			var xDrag=ControlPointToBitmapPoint(e.Location).X-ControlPointToBitmapPoint(_pointMouseDown).X;
			var yDrag=ControlPointToBitmapPoint(e.Location).Y-ControlPointToBitmapPoint(_pointMouseDown).Y;
			if(idxNewPos!=_idxSelectedInMount){
				var movedPosNoCrop=false;//moving to a new mount position, but no crop was specified, then don't add one
				_documentArrayShowing[_idxSelectedInMount].MountItemNum=_listMountItems[idxNewPos].MountItemNum;
				if(_documentArrayShowing[_idxSelectedInMount].ToothNumbers==_listMountItems[_idxSelectedInMount].ToothNumbers){//only if unchanged by user
					_documentArrayShowing[_idxSelectedInMount].ToothNumbers=_listMountItems[idxNewPos].ToothNumbers;
				}
			...
			if(_documentArrayShowing[_idxSelectedInMount].CropW==0 || _documentArrayShowing[_idxSelectedInMount].CropH==0){
				var sizeCrop=ImageHelper.CalcSizeFit(
			...
			var rectangleBitmap=new Rectangle(-_bitmapArrayShowing[_idxSelectedInMount].Width/2,//left
			...
			var doRectanglesIntersect=regionBitmap.IsVisible(rectangleCrop);
		...
		if(_cropPanAdj==EnumCropPanAdj.Crop){
			...
			var pointRaw1=new Point(_rectangleCrop.X,_rectangleCrop.Y);
			var pointRaw2=new Point(_rectangleCrop.X+_rectangleCrop.Width,_rectangleCrop.Y+_rectangleCrop.Height);
			//untangle so that pointCrop1 is always at the upper left
			var rectangle=new Rectangle(Math.Min(pointRaw1.X,pointRaw2.X), Math.Min(pointRaw1.Y,pointRaw2.Y),
				Math.Abs(pointRaw1.X-pointRaw2.X),	Math.Abs(pointRaw1.Y-pointRaw2.Y));
			var pointFCrop1=new PointF(rectangle.X,rectangle.Y);
			var pointFCrop2=new PointF(rectangle.Right,rectangle.Bottom);
			...
			var documentOld=GetDocumentShowing(0).Copy();
			var width=pointFCrop2.X-pointFCrop1.X;
			var height=pointFCrop2.Y-pointFCrop1.Y;
			...
			_rectangleCrop=new Rectangle();
			var defDocCategory=Defs.GetDef(DefCat.ImageCats,_documentArrayShowing[0].DocCategory);
			var logText="Document Edited: "+_documentArrayShowing[0].FileName
	...
	private void panelMain_Paint(object sender, PaintEventArgs e){
		...
		var scaleFactor=ZoomSliderValue/100f;
		g.ScaleTransform(scaleFactor,scaleFactor);
	...
	private void panelMain_MouseWheel(object sender, MouseEventArgs e){
		...
		var deltaZoom=ZoomSliderValue*(float)e.Delta/SystemInformation.MouseWheelScrollDelta/8f;//For example, -15
	...
	private void ChangeColorAt(PointF pointF){
		var testRadius=5f/ZoomSliderValue*100f;//example: zoomed out, 5/60*100=8.3, so test radius is larger on the image
		var wandXoffset=2f/ZoomSliderValue*100f;
		var wandYoffset=1f/ZoomSliderValue*100f;
		//The wand tip doesn't quite match cursor target, we add 1 or 2
		var pointFWandTip=new PointF(pointF.X+wandXoffset,pointF.Y+wandYoffset);
		...
		for(var i=0;i<_listImageDraws.Count;i++) {
			...
			if(_listImageDraws[i].DrawType==ImageDrawType.Text){
				var pointText=_listImageDraws[i].GetTextPoint();
				var str=_listImageDraws[i].GetTextString();
				using var font=new Font(FontFamily.GenericSansSerif,_listImageDraws[i].GetFontSize());
				var widthText=TextRenderer.MeasureText(str,font).Width;
				var rectangleText=new Rectangle(pointText.X,pointText.Y,widthText,font.Height);
	...
	public void ClearObjects(){
		...
		if(_bitmapArrayShowing!=null && _bitmapArrayShowing.Length>0){
			for(var i=0;i<_bitmapArrayShowing.Length;i++){
	...
	private Point ControlPointToBitmapPoint(Point pointPanel) {
		var pointF=ControlPointToBitmapPointF(pointPanel);
	...
	private PointF ControlPointToBitmapPointF(Point pointPanel) {
		...
		if(GetDocumentShowing(0).IsFlipped){
			var matrixFlip=new Matrix(-1,0,0,1,0,0);
	...
	private void CreateDocumentThumbnail() {
		...
		var document=GetDocumentShowing(0);
		var bitmapOriginal=GetBitmapShowing(0);
		...
		var scale=ImageTools.CalcScaleFit(new Size(99,99),new Size(bitmapOriginal.Width,bitmapOriginal.Height),0);
		g.ScaleTransform(scale,scale);
		DrawDocument(g);
		...
		var fileNameFull=Path.Combine(PatFolder,"Thumbnails",document.FileName);
		if(true) {
	...
	private void CreateMountThumbnail(){
		...
		var scale=ImageTools.CalcScaleFit(new Size(99,99),new Size(_mountShowing.Width,_mountShowing.Height),0);
		g.ScaleTransform(scale,scale);
		DrawMount(g);
		var pathThumbnails=Path.Combine(PatFolder,"Thumbnails");
		if(true && !Directory.Exists(pathThumbnails)) {
		...
		var fileName="Mount"+_mountShowing.MountNum+".jpg";
	...
	private void DownloadDocumentNoteFile(NodeTypeAndKey nodeTypeAndKey) {
		...
		var downloadPrefix="_download_:";
		var base64Prefix="_rawBase64_:";
		...
		var progressOD=new ProgressWin();
		var documentImported=new Document();
		if(document.Note.StartsWith(downloadPrefix)) {
			var url=document.Note.Substring(downloadPrefix.Length);
			var tempFileName=ODFileUtils.CombinePaths(PrefC.GetTempFolderPath(),PatientCur.PatNum+Path.GetExtension(url));
			...
			var base64=document.Note.Substring(base64Prefix.Length);
			var extension=StringTools.SubstringBefore(base64,"_");
			...
			var tempFileName=ODFileUtils.CombinePaths(PrefC.GetTempFolderPath(),PatientCur.PatNum+extension);
		...
		var nodeTypeAndKey2=new NodeTypeAndKey(EnumImageNodeType.Document,documentImported.DocNum);
	...
	private void DrawDocument(Graphics g){
		...
		DrawDrawings(-document.DegreesRotated,g);
		g.ResetClip();
	...
	/// <summary>Draws an image on the given XGraphics object. Used for PDFs. Logic is similar to DrawDocument with the necessary changes needed to make XGraphics happy.</summary>
	private void DrawImageToPDF(Document document, XGraphics xg) {
		var xGraphicsState=xg.Save();
		//we're at the center of the crop area, and working in image coordinates
		if(document.CropW>0 && document.CropH>0) {
			xg.IntersectClip(new Rectangle((int)GraphicsHelper.PixelsToPoints(-document.CropW/2),(int)GraphicsHelper.PixelsToPoints(-document.CropH/2),
				(int)GraphicsHelper.PixelsToPoints(document.CropW),
				(int)GraphicsHelper.PixelsToPoints(document.CropH)));
		}
		//Translate from center of crop area to center of image
		//This step is not intuitive because our crop coordinates are positive to UR instead of the LR that we are using here.
		xg.TranslateTransform(GraphicsHelper.PixelsToPoints(-document.CropX),GraphicsHelper.PixelsToPoints(document.CropY));
		//rotate around center of image
		xg.RotateTransform(document.DegreesRotated);
		if(document.IsFlipped) {
			var matrix = new Matrix(-1,0,0,1,0,0);
			xg.MultiplyTransform(matrix);
		}
		//Translate from center of image to UL corner of image
		xg.TranslateTransform(GraphicsHelper.PixelsToPoints(-GetBitmapShowing(0).Width/2),
			GraphicsHelper.PixelsToPoints(-GetBitmapShowing(0).Height/2));
		var xImage=XImage.FromGdiPlusImage(GetBitmapShowing(0));
		xg.DrawImage(xImage,
			0,
			0,
			GraphicsHelper.PixelsToPoints(xImage.PixelWidth),
			GraphicsHelper.PixelsToPoints(xImage.PixelHeight));
		xImage.Dispose();
		xImage=null;
		DrawDrawings(-document.DegreesRotated,xg:xg);
		xg.Restore(xGraphicsState);
		if(_rectangleCrop.Width>0 && _rectangleCrop.Height>0) {//Drawn last so it's on top.
			xg.Restore(xGraphicsState);//panel coords
			xg.DrawRectangle(Pens.Blue,_rectangleCrop);
		}
	}
	...
	private void DrawMount(Graphics g){
		...
		using var solidBrushBack=new SolidBrush(_mountShowing.ColorBack);
		...
		//outlines:
		using var penOutline=new Pen(ColorOD.Gray(100));
		for(var i=0;i<_listMountItems.Count;i++){
			if(_listMountItems[i].ItemOrder==-1){//unmounted
				continue;
			}
	...
	private void DrawDrawings(float rotateText,Graphics g=null,XGraphics xg=null){
		var listDocNums=new List<long>();
		if(IsMountShowing()) {
			var listUnmountedObjs=GetUmountedObjs();
		...
		for(var i=0;i<_listImageDraws.Count;i++){
			...
			var programName=Programs.GetActiveImagingAIProgram();
			if(_listImageDraws[i].ImageAnnotVendor!=EnumImageAnnotVendor.OpenDental //This is an AI drawing
			   && (programName==ProgramName.None || !_showDrawingsAI)) //And AI programs are disabled or AI drawings are hidden
			{
				continue;//AI drawings hidden or not enabled.
			}
			...
			if(programName==ProgramName.Pearl){
				if(!_listPearlEnumCategoryODsShown.Contains(_listImageDraws[i].PearlLayer)
				   || !_listImageDraws[i].ImageAnnotVendor.In(EnumImageAnnotVendor.OpenDental,EnumImageAnnotVendor.Pearl)) 
				{
					continue;//Pearl category is hidden or this ImageDraw is from a different vendor.
				}
			}
			else if(programName==ProgramName.BetterDiagnostics) {
				if(!_listEnumCategoryBetterDiagsShown.Contains(_listImageDraws[i].BetterDiagLayer)
				   || !_listImageDraws[i].ImageAnnotVendor.In(EnumImageAnnotVendor.OpenDental,EnumImageAnnotVendor.BetterDiagnostics)) 
				{
					continue;//Better Diagnostics category is hidden or this ImageDraw is from a different vendor.
				}
			}
			if(g!=null && xg==null) {
			...
			if(g==null && xg!=null) {
				DrawDrawingPDF(rotateText,_listImageDraws[i],xg);
			}
		}
	}

	/// <summary>Draws one drawing on an XGraphics object. Used for PDFs. This logic is similar to DrawDrawing with the necessary changes needed to draw using XGraphics.</summary>
	private void DrawDrawingPDF(float rotateText,ImageDraw imageDraw,XGraphics xg) {
		if(imageDraw.DrawType==ImageDrawType.Text){
			var g=Graphics.FromImage(new Bitmap(100,100));//Only used for measuring
			var graphicsState=xg.Save();
			var xpoint=imageDraw.GetTextPointPDF();
			xg.TranslateTransform(xpoint.X,xpoint.Y);
			using var font=new Font(FontFamily.GenericSansSerif,imageDraw.GetFontSize());
			var str=imageDraw.GetTextString();
			var xfont=ImageHelper.GetXFont(str, font);
			var sizeF=g.MeasureString(str,font);
			g.Dispose();
			var rectangle=new Rectangle(new Point(0,0),new Size((int)GraphicsHelper.PixelsToPoints(sizeF.Width),(int)GraphicsHelper.PixelsToPoints(sizeF.Height)));
			xg.RotateTransform(rotateText);
			if(imageDraw.ColorBack!=Color.Empty){
				XBrush xbrushBack=new SolidBrush(imageDraw.ColorBack);
				xg.DrawRectangle(xbrushBack,rectangle);
			}
			XBrush xbrush=new SolidBrush(imageDraw.ColorDraw);
			xfont=ImageHelper.GetXFont(str,font);
			xg.DrawString(str,xfont,xbrush,rectangle,XStringFormats.Center);
			xg.Restore(graphicsState);
		}
		if(imageDraw.DrawType==ImageDrawType.Pen
		   || imageDraw.DrawType==ImageDrawType.Line)
		{
			xg.SmoothingMode=XSmoothingMode.HighQuality;
			var listXPoints=imageDraw.GetPointsForPDF();
			var pen=new XPen(imageDraw.ColorDraw,2);//2 pixels wide
			XBrush xbrush=new SolidBrush(imageDraw.ColorDraw);
			for(var p=1;p<listXPoints.Count;p++){//start at the second point.
				xg.DrawLine(pen,listXPoints[p-1],listXPoints[p]);
				var rectangleF=new RectangleF((float)listXPoints[p].X-1,(float)listXPoints[p].Y-1,2,2);
				xg.DrawEllipse(xbrush,rectangleF);
			}
		}
		if(imageDraw.DrawType==ImageDrawType.Polygon){
			xg.SmoothingMode=XSmoothingMode.HighQuality;
			var listXPoints=imageDraw.GetPointsForPDF();
			XBrush xbrush=new SolidBrush(imageDraw.ColorDraw);
			xg.DrawPolygon(xbrush,listXPoints.ToArray(),XFillMode.Alternate);
		}
		//for the measurement hover effect:
		if(!_stringMeasure.IsNullOrEmpty()){
			XBrush xbrush=new SolidBrush(ColorTextBack);
			var fontSize=8f/ZoomSliderValue*100f;
			using var font=new Font(FontFamily.GenericSansSerif,fontSize);
			var xfont=ImageHelper.GetXFont(_stringMeasure,font);
			var size=xg.MeasureString(_stringMeasure,font).ToSizeF();
			var rectangle=new Rectangle(_pointMeasureText,size.ToSize());
			xg.DrawRectangle(xbrush,rectangle);
			XBrush xbrushText=new SolidBrush(ColorFore);
			GraphicsHelper.DrawStringX(xg,_stringMeasure,xfont,xbrush,rectangle,HorizontalAlignment.Left);
			xg.DrawString(_stringMeasure,font,xbrushText,_pointMeasureText);
		}
	}

	/// <summary>Draws one drawing on a Graphics object. Used for images.</summary>
	private void DrawDrawing(float rotateText, ImageDraw imageDraw, Graphics g) {
		if(imageDraw.DrawType==ImageDrawType.Text){
			var graphicsState=g.Save();
			var point=imageDraw.GetTextPoint();
			...
			if(imageDraw.ColorBack!=Color.Empty){
				using var brushBack=new SolidBrush(imageDraw.ColorBack);
				g.FillRectangle(brushBack,rectangle);
			}
			using var solidBrush=new SolidBrush(imageDraw.ColorDraw);
		...
		{
			g.SmoothingMode=SmoothingMode.HighQuality;
			var listPointFs=imageDraw.GetPoints();
			using var pen=new Pen(imageDraw.ColorDraw,2);//2 pixels wide
			using var solidBrush=new SolidBrush(imageDraw.ColorDraw);
			for(var p=1;p<listPointFs.Count;p++){//start at the second point.
		...
		//for the measurement hover effect:
		if(!_stringMeasure.IsNullOrEmpty()){
			var solidBrushBack=new SolidBrush(ColorTextBack);
			var fontSize=8f/ZoomSliderValue*100f;
			using var font=new Font(FontFamily.GenericSansSerif,fontSize);
			var size= g.MeasureString(_stringMeasure,font).ToSize();
			var rectangle=new Rectangle(_pointMeasureText,size);
			g.FillRectangle(solidBrushBack,rectangle);				
			var solidBrushText=new SolidBrush(ColorFore);
	...
	private void DrawMountOne(Graphics g,int i,bool translateToItemPos=true){
		...
		if(_listMountItems[i].TextShowing!=""){
			using var solidBrushBack=new SolidBrush(_mountShowing.ColorTextBack);
			var rectangleF=new RectangleF(0,0,_listMountItems[i].Width,_listMountItems[i].Height);
			g.FillRectangle(solidBrushBack,rectangleF);
			using var font=new Font(FontFamily.GenericSansSerif,_listMountItems[i].FontSize);
			using var solidBrushFore=new SolidBrush(_mountShowing.ColorFore);
			var str=_listMountItems[i].TextShowing;
			str=Patients.ReplacePatient(str,PatientCur);
			str=Mounts.ReplaceMount(str,_mountShowing);
			var patPlan=PatPlans.GetPatPlan(PatientCur.PatNum,1);
			InsSub insSub=null;
			if(patPlan!=null){
	...
	private void DrawMountOneBigNumber(Graphics g,int i){
		...
		var heightFontPoint=_listMountItems[i].Height/15f*(96f/72f);
		if(_listMountItems[i].Width<_listMountItems[i].Height){
			heightFontPoint=_listMountItems[i].Width/15f*(96f/72f)/1;
		}
		using var font=new Font(FontFamily.GenericSansSerif,heightFontPoint);
		var sizeString=g.MeasureString(_listMountItems[i].ItemOrder.ToString(),font);
		sizeString=new Size((int)sizeString.Width,(int)sizeString.Height);
	...
	private void EraseAt(PointF pointF){
		var circleRadius=5f/ZoomSliderValue*100f;//example: zoomed out, 5/60*100=8.3, so eraser circle covers a larger area of image
		var testRadius=6f/ZoomSliderValue*100f;
		var circleDiameter=(int)(12f/ZoomSliderValue*100f);
		var pointFCircleCenter=new PointF(pointF.X+circleRadius,pointF.Y+circleRadius);//adjusted because tip is actually at UL of a cursor
		...
		for(var i=0;i<_listImageDraws.Count;i++) {
			...
			if(_listImageDraws[i].DrawType==ImageDrawType.Text){
				var pointText=_listImageDraws[i].GetTextPoint();
				var str=_listImageDraws[i].GetTextString();
				var fontSize=_listImageDraws[i].GetFontSize();
				using var font=new Font(FontFamily.GenericSansSerif,fontSize);
				var widthText=TextRenderer.MeasureText(str,font).Width;
				//this isn't really the rectangle where the text is.  It's adjusted for the eraser size.
				var heightFont=font.Height;
				var rectangleText=new Rectangle(pointText.X-circleDiameter,pointText.Y-circleDiameter,
	...
	private long GetCurrentCategory() {
		if(_nodeTypeKeyCatSelected==null || _nodeTypeKeyCatSelected.ImageNodeType==EnumImageNodeType.None || _nodeTypeKeyCatSelected.DefNumCategory==0) {//No Image Category selected.
			var imageCategoryDefault=PrefC.GetLong(PrefName.ImageCategoryDefault);
	...
	///<summary>Only for Pearl and BetterDiagnostics. Pass in bitmap or mount coords. Looks for polygon or square (Line) that meets criteria for hover. Returns index within _listImageDraws, or -1 if no hit.</summary>
	private int HitTestHover(PointF pointfBitmap){
		var isPearl=Programs.IsEnabled(ProgramName.Pearl);
		//The only Pearl types that have details to hover on are polyAnnotations and boxAnnotations
		if(!isPearl && !Programs.IsEnabled(ProgramName.BetterDiagnostics)){
			return -1;
		}
		for(var i=0;i<_listImageDraws.Count;i++){
			if(!_listImageDraws[i].ImageAnnotVendor.In(EnumImageAnnotVendor.Pearl,EnumImageAnnotVendor.BetterDiagnostics)){
				continue;
			}
			...
			if(isPearl) {
				if(_listImageDraws[i].PearlLayer==OpenDentBusiness.Pearl.EnumCategoryOD.Measurements
				   || !_listPearlEnumCategoryODsShown.Contains(_listImageDraws[i].PearlLayer)
				  ) {
					continue;
				}
			}
	...
	private int HitTestInMount(int x,int y,bool isMountCoords=false,bool includeText=false){
		...
		for(var i=0;i<_listMountItems.Count;i++){
			...
			var rectangle=new Rectangle(_listMountItems[i].Xpos,_listMountItems[i].Ypos,_listMountItems[i].Width,_listMountItems[i].Height);
	...
	private bool HitTestLine(PointF pointFEnd1,PointF pointFEnd2,PointF pointFToTest,float testRadius){
		...
		var x2=pointFEnd2.X;
		var y2=pointFEnd2.Y;
		var x0=pointFToTest.X;
		var y0=pointFToTest.Y;
		var lengthLine=(float)Math.Sqrt(Math.Pow(x2-x1,2)+Math.Pow(y2-y1,2));
		var distPointToLine=Math.Abs((x2-x1)*(y1-y0)-(x1-x0)*(y2-y1)) / lengthLine;//this distance is always perpendicular
		...
		var distToEnd1=(float)Math.Sqrt(Math.Pow(x1-x0,2)+Math.Pow(y1-y0,2));
		var distToEnd2=(float)Math.Sqrt(Math.Pow(x2-x0,2)+Math.Pow(y2-y0,2));
		var distMax=(float)Math.Sqrt(Math.Pow(lengthLine,2)+Math.Pow(distPointToLine,2));
	...
	private int HitTestText(Point pointPanel){
		for(var i=0;i<_listImageDraws.Count;i++){
			...
			var pointTextInImage=_listImageDraws[i].GetTextPoint();
			PointF[] pointFArray= [pointTextInImage];
			matrix.TransformPoints(pointFArray);
			var pointTextInPanel=Point.Round(pointFArray[0]);
			//Font must be in panel scale rather than image scale
			var sizeFont=_listImageDraws[i].GetFontSize()*ZoomSliderValue/100f;
			using var font=new Font(FontFamily.GenericSansSerif,sizeFont);
			var str=_listImageDraws[i].GetTextString();
			var widthText=TextRenderer.MeasureText(str,font).Width;
			//Using panel coords, define a rectangle where this text is located.
			var heightFont=font.Height;
			var rectangleText=new Rectangle(pointTextInPanel.X,pointTextInPanel.Y,widthText,heightFont);
	...
	private void ThumbnailRefresh(){
		if(IsMountShowing()){
			var idxSelected=_idxSelectedInMount;
	...
	private void ToolBarImportMount(){
		...
		var listMountItemsAvail=GetAvailSlots(stringArrayFileNames.Length);
		if(listMountItemsAvail==null){
			MsgBox.Show("Not enough slots in the mount for the number of files selected.");
			return;
		}
		...
		for(var i=0;i<stringArrayFileNames.Length;i++) {
			...
			Documents.Update(document,documentOld);
			if(OpenDentBusiness.Bridges.Pearl.DoAutoUploadForImageCategory(document.DocCategory)) {
				var pearl=OpenDentBusiness.Bridges.Pearl.SetupPearlForSendingSingle(PatientCur,document,filePath:stringArrayFileNames[i],mountItem:listMountItemsAvail[i]);
				pearl?.SendOnThread();
			}
			else if(OpenDentBusiness.Bridges.BetterDiag.DoAutoUploadForImageCategory(document.DocCategory)) {
				OpenDentBusiness.Bridges.BetterDiag.SendSingleOnThread(PatientCur,document,filePath:stringArrayFileNames[i],mountItem:listMountItemsAvail[i]);
			}
	...
	private void ToolBarImportSingle() {
		...
		NodeTypeAndKey nodeTypeAndKey=null;
		Document document=null;
		var isBlockingNavigation=false;
		if(!false){
...
But this was just a side effect of the need to reset zoom if the window had moved to a new screen with different zoom. 
That was done in FormImageFloat.OnResizeEnd
