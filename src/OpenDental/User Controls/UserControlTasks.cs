...
public partial class UserControlTasks : UserControl
{
    ...
    private List<long> _listTaskNumsExpanded = [];
    ...
    private List<long> _listSignalNumsSentTask = [];

    ///<summary>All instances of this control that are currently open.</summary>
    private static List<UserControlTasks> _listUserControlTaskss = [];
    ...
    private List<long> _listClinicNumsFilter = [];

    ///<summary>The current region selected for filtering</summary>
    private List<long> _listDefNumsRegionFilter = [];
    ...
    private List<Task> _listTasksCopy = [];
    ...
    {
        tabControl.Bounds = new Rectangle(0, 49, Width, 20);
        monthCalendar.Bounds = new Rectangle(0, 70, 227, 162);
        tree.Width = Width;
    ...
    {
        foreach (var control in _listUserControlTaskss)
        {
    ...
    public void InitializeOnStartup()
    {
        ...
        tabUser.Text = "For " + Security.CurUser.UserName;
        tabNew.Text = "New for " + Security.CurUser.UserName;
        
        if (PrefC.GetBool(PrefName.TasksShowOpenTickets))
        {
            if (!tabControl.TabPages.Contains(tabOpenTickets))
            {
                tabControl.TabPages.Add(tabOpenTickets);
            }
        }
        else
        {
        ...
        if (_listTaskListsHistory == null)
        {
            //first time opening
            _listTaskListTreeHistory = [];
        ...
        var listUserOdPrefsForCollapsing = UserOdPrefs.GetByUserAndFkeyType(Security.CurUser.UserNum, UserOdFkeyType.TaskCollapse);
    ...
    public UserControlTasksTab TaskTab
    {
        get
        {
            ...
            if (tabControl.SelectedTab == tabMain)
            {
        ...
        set
        {
            var tabPageOld = tabControl.SelectedTab;
            ...
            if (tabControl.SelectedTab != tabPageOld)
            {
                //Tab changed, refresh the tree.
                _listTaskListTreeHistory = []; //clear the tree no matter which tab selected.
                FillTree();
                FillGrid();
            }
        }
    ...
    private void SetOpenTicketTab(int countSet)
    {
        ...
        tabOpenTickets.Text = Lan.g(this, "Open Tasks") + " (" + countSet + ")";
    }
    ...
    {
        var taskList = TaskLists.GetOne(_TriageListNum);
        tabControl.SelectedTab = tabMain;
    ...
    public void LayoutToolBar()
    {
        ...
        ToolBarMain.Buttons.Add(new ODToolBarButton("Add Task List", 0, "", "AddList"));
        ToolBarMain.Buttons.Add(new ODToolBarButton("Add Task", 1, "", "AddTask")
        {
        ...
        ToolBarMain.Buttons.Add(new ODToolBarButton
        {
            ...
            Tag = "BlockSubsc",
            IsTogglePushed = Security.CurUser.DefaultHidePopups
        });
    ...
    private void FillTree()
    {
        ...
        string descNode;
        for (var i = 0; i < _listTaskListTreeHistory.Count; i++)
        {
    ...
    private void LayoutTreeAndGrid()
    {
        ...
        {
            tree.Location = new Point(tree.Left, monthCalendar.Bottom + 1); //Show the calendar.
        }
        else
        {
            tree.Location = new Point(tree.Left, tabControl.Bottom); //Hide the calendar.
        }

        tree.Height = _listTaskListTreeHistory.Count * tree.ItemHeight + 8;
        tree.Refresh();
        gridMain.Location = new Point(0, tree.Bottom);
    }
    ...
    private void SetFiltersToDefault(TaskList taskList = null)
    {
        ...
        if (taskList == null && tree.SelectedNode != null)
        {
            //Check if there is a current tasklist selected.
            taskList = _listTaskListTreeHistory.FirstOrDefault(x => x.TaskListNum == (long) tree.SelectedNode.Tag);
        }

        if (taskList != null)
        {
            //And if so, use its GlobalTaskFilterType override.
            _enumTaskFilterTypeForList = taskList.GlobalTaskFilterType;
        }

        if (Clinics.ClinicNum == 0)
        {
            //HQ clinic, do not apply filtering.
            _enumTaskFilterTypeForList = EnumTaskFilterType.None;
        }

        if (_enumTaskFilterTypeForList == EnumTaskFilterType.Default)
        {
            //Get the global default filter setting.
        ...
        switch (_enumTaskFilterTypeForList)
        {
            case EnumTaskFilterType.None:
                _listClinicNumsFilter = [0];
                _listDefNumsRegionFilter = [0];
                break;
            case EnumTaskFilterType.Clinic:
                _listClinicNumsFilter = [Clinics.ClinicNum]; //Default to currently selected clinic.
                _listDefNumsRegionFilter = [0];
                break;
            case EnumTaskFilterType.Region:
                //Default to currently selected clinic's region.  Use 0 if no region defined.
                _listDefNumsRegionFilter = [Clinics.GetClinic(Clinics.ClinicNum)?.RegionId ?? 0];
                _listClinicNumsFilter = [0];
                break;
        }
    ...
    public static void RefillLocalTaskGrids(Task task, List<TaskNote> listTaskNotes, List<long> listSentSignalNums, bool canKeepTask = true)
    {
        ...
        foreach (var control in _listUserControlTaskss)
        {
            if (!control.Visible && !control.IsDisposed)
            {
                //Verify control is visible and active
                continue;
            }

            long taskListNum = 0; //Default to one of the main trunks.
            if (control._listTaskListTreeHistory != null && control._listTaskListTreeHistory.Count > 0)
            {
                //not on main trunk
                                     ...
                                     || (control.tabControl.SelectedTab == control.tabPatientTickets && task.KeyNum == FormOpenDental.PatNumCur))) //Patient tab
            {
                ...
                {
                    for (var i = 0; i < control._listTasks.Count; i++)
                    {
                        ...
                        if (i == control._listTasks.Count - 1)
                        {
                            //Looped through all current Tasks and didn't find this Task, so it must be new to the TaskList.
                            control._listTasks.Add(task); //Task newly moved to this TaskList.
                            break;
                        }
                    }
    ...
    public static void RefillLocalTaskGrids(TaskList taskList, List<long> listSentSignalNums, bool canKeepTaskList = true)
    {
        ...
        var listTaskListNumsSubscribed = ListTaskListNumsSubscribed; //Get list copy above loop to avoid making unnecessary copies.
        foreach (var control in _listUserControlTaskss)
        {
            long taskListNum = 0; //Default to one of the main trunks.
            if (control._listTaskListTreeHistory != null && control._listTaskListTreeHistory.Count > 0)
            ...
            if (taskList != null)
            {
                foreach (var list in control._listTaskLists)
                {
            ...
            else if (!control._isShowArchivedTaskLists && taskList.TaskListStatus == TaskListStatusEnum.Archived)
            {
                ...
                {
                    var dictTaskLists = control._dictTaskLists;
                         ...
                         || (control.tabControl.SelectedTab == control.tabUser && listTaskListNumsSubscribed.Contains(taskList.TaskListNum))))
            {
                ...
                {
                    for (var i = 0; i < control._listTaskLists.Count; i++)
                    {
                        ...
                        if (taskList.Descript.CompareTo(control._listTaskLists[i].Descript) >= 0)
                        {
                            //Does taskList come after this list?
                            insertIndex = i + 1; //Insert here to maintain order.
                        }

                        if (i == control._listTaskLists.Count - 1)
                        {
                            //Looped through all current TaskLists and didn't find this TaskList, so it must be new to the TaskList.
                            control._listTaskLists.Insert(insertIndex, taskList); //TaskList newly moved to this TaskList.
                            break;
                        }
                    }
            ...
            {
                //TaskList is not in current TaskList,or was deleted(canKeepTaskList==false)
                control._listTaskLists.RemoveAll(x => x.TaskListNum == taskList.TaskListNum);
            }
    ...
    {
        if (parent != 0 || tabControl.SelectedTab == tabNew)
        {
            //Not a trunk, or on the New for User tab. 
        ...
        {
            FillGrid([]); //Invalidate view without calling db.
        }
    }
    ...
    private List<Signalod> RemoveSentSignalNums(List<Signalod> listSignalodsReceived)
    {
        ...
        for (var i = listSignalodsReceived.Count - 1; i >= 0; i--)
        {
            var signalNumReceived = listSignalodsReceived[i].SignalNum;
    ...
    {
        if (PrefC.GetBool(PrefName.TasksNewTrackedByUser) && task.IsUnread)
        {
            //The new way
        ...
        if (!PrefC.GetBool(PrefName.TasksNewTrackedByUser) && task.TaskStatus == TaskStatusEnum.New)
        {
            //Tasks are shared by everyone.
            return true;
        }
    ...
    private void FillGrid(List<Signalod> listSignalods = null, bool isManualRefresh = false, bool isFilterRefresh = false)
    {
        ...
        gridMain.Height = ClientSize.Height - gridMain.Top;
        if (listSignalods == null)
        {
            //Full refresh.
            if (!isFilterRefresh)
            {
        ...
        else
        {
            ...
            else
            {
                ...
                foreach (var signalod in listSignalods)
                {
                    ...
                    {
                        if (_listTasks.Exists(x => x.TaskNum == signalod.FKey))
                        {
            ...
            && (tabControl.SelectedTab == tabDate || tabControl.SelectedTab == tabWeek || tabControl.SelectedTab == tabMonth))
        {
            ...
            for (var i = 0; i < _listTaskLists.Count; i++)
            {
                if (_listTaskLists[i].FromNum == 0)
                {
            ...
            var listTaskListsRepeating = new List<TaskList>();
            var tasksRepeating = new List<Task>();
            if (tabControl.SelectedTab == tabDate)
            {
                listTaskListsRepeating = TaskLists.RefreshRepeating(TaskDateType.Day, Security.CurUser.UserNum, Clinics.ClinicNum
                    , Clinics.GetClinic(Clinics.ClinicNum)?.RegionId ?? 0);
            ...
            changeMade = false;
            bool alreadyExists;
            for (var i = 0; i < listTaskListsRepeating.Count; i++)
            {
                //if already exists, skip
                alreadyExists = false;
                for (var j = 0; j < _listTaskLists.Count; j++)
                {
            ...
            for (var i = 0; i < tasksRepeating.Count; i++)
            {
                //if already exists, skip
                alreadyExists = false;
                for (var j = 0; j < _listTasks.Count; j++)
                {
                    //loop through Main list
        ...
        var listTaskLists = GetFilteredTaskLists();
        //bool isTaskSelectedVisible=gridMain.IsTagVisible(_clickedTask);
        var isHqAndTriageList = false && taskListNum == _TriageListNum; //True if HQ and looking at 'Triage' task list
        gridMain.BeginUpdate();
        gridMain.Columns.Clear();
        var gridColumn = new GridColumn("", 17);
        gridColumn.ImageList = imageListTree;
        gridMain.Columns.Add(gridColumn); //Checkbox column
        if (tabControl.SelectedTab == tabNew && !PrefC.GetBool(PrefName.TasksNewTrackedByUser))
        {
            //The old way
        ...
        gridMain.Columns.Add(gridColumn);
        if (!isHqAndTriageList)
        {
            //Everything that is not HQ's triage task list will have the attachments column
        ...
        var categoryStr = "";
        int imageindex;
        var listDefsTaskCategory = Defs.GetDefsForCategory(DefCat.TaskCategories);
        var listDefsTaskPriority = Defs.GetDefsForCategory(DefCat.TaskPriorities);
        ...
        for (var i = 0; i < listTaskLists.Count; i++)
        {
            ...
            {
                imageindex = 3; //orange
                tasklistdescript = tasklistdescript + "(" + listTaskLists[i].NewTaskCount + ")";
            }
        ...
        var listAptNums = _listTasks.Where(x => x.ObjectType == TaskObjectType.Appointment).Select(y => y.KeyNum).ToList();
        ...
        for (var i = 0; i < _listTasks.Count; i++)
        {
            ...
            var colorTaskPriority = Defs.GetColor(DefCat.TaskPriorities, _listTasks[i].PriorityDefNum, listDefsTaskPriority);
            ...
            notes = "";
            var listTaskNotes = _listTaskNotes.FindAll(x => x.TaskNum == _listTasks[i].TaskNum);
            if (!_listTaskNumsExpanded.Contains(_listTasks[i].TaskNum) && listTaskNotes.Count > 1)
            {
                var taskNoteLast = listTaskNotes[listTaskNotes.Count - 1];
            ...
            {
                //Expanded
                foreach (var note in listTaskNotes)
                {
            ...
            else if (_listTaskNumsExpanded.Contains(_listTasks[i].TaskNum))
            {
                //Expanded
            ...
            else
            {
                ...
                {
                    //Descript length <= 250 and notes <=1 and note length is <= 250.  No collapse option.
                    gridRow.Cells.Add("");
                    if (!isHqAndTriageList)
                    {
                        //Everything that is not HQ's triage task list will have the attachments column
            ...
            if (!isHqAndTriageList)
            {
                //HQ's triage list should not have color coded row as that task list is treated differently
            ...
            if (taskSelected != null && _listTasks[i].TaskNum == taskSelected.TaskNum)
            {
                //_clickedTask can be a TaskList
                selectedIndex = gridMain.ListGridRows.Count - 1;
            }
        }
    ...
    {
        var patNum = task.ObjectType == TaskObjectType.Patient ? task.KeyNum : 0;
    ...
    {
        if (FillGridEvent == null)
        {
            //Delegate has not been assigned, so we do not care.
            return;
        }

        var descriptTaskList = "";
        if (tabControl.SelectedTab == tabNew)
        {
            //Special case tab. All grid rows are guaranteed to be task so we manually set values.
            descriptTaskList = Lan.g(this, "New for") + " " + Security.CurUser.UserName;
        }
        else if (_listTaskListTreeHistory.Count > 0)
        {
            //Not in main trunk
        ...
        {
            var tasksNewCount = _listTaskLists.Sum(x => x.NewTaskCount);
            if (PrefC.GetBool(PrefName.TasksNewTrackedByUser))
            {
            ...
            TitleControlParent = Lan.g(this, "Tasks") + " - " + descriptTaskList + " (" + tasksNewCount + ")";
        }
    ...
    {
        //get all children:
        var listTaskListsChildren = TaskLists.RefreshChildren(taskList.TaskListNum, Security.CurUser.UserNum, 0, TaskType.Normal);
        var listTasksChild = Tasks.RefreshChildren(taskList.TaskListNum, true, DateTime.MinValue, Security.CurUser.UserNum, 0, TaskType.Normal);
        for (var i = 0; i < listTaskListsChildren.Count; i++)
        {
        ...
        for (var i = 0; i < listTasksChild.Count; i++)
        {
    ...
    {
        var filterToUpper = textListFilter.Text.ToUpper().Trim();
        ...
        var listTaskLists = _listTaskLists.FindAll(x => x.Descript.ToUpper().Trim().Contains(filterToUpper));
        //Ordering will be:
    ...
    private void RefreshMainLists(long taskListNum, DateTime date, bool isManualRefresh = false)
    {
        ...
        var tabPageSelected = tabControl.SelectedTab;
        ...
        if (taskListNum != 0)
        {
            ...
            var userNumInbox = TaskLists.GetMailboxUserNum(_listTaskListTreeHistory[0].TaskListNum);
        ...
        else if (tabControl.SelectedTab == tabUser)
        {
            ...
            _listTaskLists = TaskLists.RefreshUserTrunk(Security.CurUser.UserNum, Clinics.ClinicNum, Clinics.GetClinic(Clinics.ClinicNum)?.RegionId ?? 0);
            lock (_listTaskListNumsSubscribed)
            {
                _listTaskListNumsSubscribed = _listTaskLists.Select(x => x.TaskListNum).ToList();
            }

            _listTasks = []; //no tasks in the user trunk
        }
        else if (tabControl.SelectedTab == tabNew)
        {
            _listTaskLists = []; //no task lists in new tab
        ...
        else if (tabControl.SelectedTab == tabMain)
        {
            ...
            {
                //Navigating from Main tab child task list, use the referenced lists instead of running a query.
        ...
        else if (tabControl.SelectedTab == tabReminders)
        {
            ...
            {
                //Navigating from Reminder tab child task list, use the referenced lists instead of running a query.
                _listTaskLists = _listTaskListsCopy;
                _listTasks = _listTasksCopy;
            }
        }
        ...
        {
            _listTaskLists = TaskLists.RefreshRepeatingTrunk(Security.CurUser.UserNum, Clinics.ClinicNum, Clinics.GetClinic(Clinics.ClinicNum)?.RegionId ?? 0);
        ...
        if (_dictTaskLists == null)
        {
            ...
            foreach (var taskList in _listTaskLists)
            {
                try
                {
        ...
        else
        {
            ...
            {
                for (var i = _listTaskNumsExpanded.Count - 1; i >= 0; i--)
                {
    ...
    {
        var listTaskListsAll = TaskLists.GetAll();
        var listSubscribedTaskListNums = TaskSubscriptions.GetTaskSubscriptionsForUser(userNum).Select(x => x.TaskListNum).ToList();
        var listTaskListsQueue = listTaskListsAll.FindAll(x => listSubscribedTaskListNums.Contains(x.TaskListNum)); //Task lists to consider.
        var listTaskListsSubscribed = new List<TaskList>();
        while (listTaskListsQueue.Count > 0)
        {
            var taskList = listTaskListsQueue[0];
            listTaskListsQueue.RemoveAt(0); //pop
            if (!listTaskListsSubscribed.Contains(taskList))
            {
                //Avoid duplicate return values
                listTaskListsSubscribed.Add(taskList); //Each item added to the queue will be part of the return list.
            }

            var taskListChildren = listTaskListsAll.FindAll(x => x.Parent == taskList.TaskListNum); //Children of taskList.
            foreach (var child in taskListChildren)
            {
                if (!listTaskListsSubscribed.Contains(child) && !listTaskListsQueue.Contains(child))
                {
                    //Avoid duplicate return values.
    ...
    private void tabControl_Selecting(object sender, int e)
    {
        ...
        _listTaskListTreeHistory = []; //clear the tree no matter which tab clicked.
        _hasListSwitched = true;
        SetFiltersToDefault(); //Fills Tree and Grid
    ...
    {
        _listTaskListTreeHistory = []; //clear the tree
        FillTree();
        FillGrid();
    }
    ...
    {
        using var formTaskOptions = new FormTaskOptions(_isShowFinishedTasks, _dateTimeStartShowFinished, _isTaskSortApptDateTime
        ...
        var pointFormLocation = this.PointToScreen(ToolBarMain.Location); //Since we cant get ToolBarMain.Buttons["Options"] location directly.
        ...
        var rectangleScreenDim = SystemInformation.VirtualScreen; //Dimensions of users screen. Includes if user has more then 1 screen.
        if (pointFormLocation.X + formTaskOptions.Width > rectangleScreenDim.Width)
        {
        ...
        if (pointFormLocation.Y + formTaskOptions.Height > rectangleScreenDim.Height)
        {
            //Not all of form will be on screen, so adjust.
    ...
    {
        long keyNum;
        var frmTaskFilter = new FrmTaskFilter();
        var task = gridMain.SelectedTag<Task>();
        if (task == null)
        {
            keyNum = 0;
        }
        else
        {
            ...
            else if (task.ObjectType == TaskObjectType.Appointment)
            {
                ...
                if (appointment != null)
                {
                    //Appointment could get deleted after clicking on task, so check for null
        ...
        if (frmTaskFilter.ClearAllClicked)
        {
            ...
            if (_listTaskListTreeHistory.Count > 0)
            {
                var taskList = _listTaskListTreeHistory.Last();
    ...
    private void AddList_Clicked()
    {
        ...
        if (tabControl.SelectedTab == tabUser && _listTaskListTreeHistory.Count == 0)
        {
            //trunk of user tab
        ...
        {
            var signalNum = Signalods.SetInvalid(InvalidType.TaskList, KeyType.Undefined, taskList.Parent); //Signal for source parent tasklist.
            RefillLocalTaskGrids(taskList, listSentSignalNums: [signalNum]);
        }
    }
    ...
    private void AddTask(bool isReminder)
    {
        ...
        else if (tabControl.SelectedTab == tabUser && _listTaskListTreeHistory.Count == 0)
        {
            //trunk of user tab
            task.TaskListNum = -1; //Force FormTaskEdit to ask user to pick a task list.
        }
    ...
    public void Search_Clicked()
    {
        ...
        if (Regex.IsMatch(textClip, @"^tasknum:\d+$"))
        {
            //very restrictive specific match for "TaskNum:##"
            var taskNum = SIn.Long(textClip.Substring(8));
            // if the tasknum was the same as last time then we have already tried this search once
            if (taskNum != _taskNumOld)
            {
                // if #'s differ then we are doing a fresh search and should just look for the tasknum
                _taskNumOld = taskNum;
                var task = Tasks.GetOne(taskNum);
                if (task != null)
                {
                    //don't show the task search form and just open up the task that has been found
        ...
        var formTaskbarSearch = new FormTaskSearch();
    ...
    private void Done_Clicked()
    {
        ...
        catch (Exception ex)
        {
            ...
            var idx = _listTasks.FindIndex(x => x.TaskNum == taskOld.TaskNum);
            if (idx > -1)
            {
        ...
        var signalNum = Signalods.SetInvalid(InvalidType.Task, KeyType.Task, task.TaskNum); //Only needs to send signal for the one task.
        RefillLocalTaskGrids(task, _listTaskNotes.FindAll(x => x.TaskNum == task.TaskNum), [signalNum]); //No db call.
    }

    private void Edit_Clicked()
    {
        var taskList = gridMain.SelectedTag<TaskList>();
        var task = gridMain.SelectedTag<Task>();
        if (taskList != null)
        {
            //is list
            ...
            var signalNum = Signalods.SetInvalid(InvalidType.TaskList, KeyType.Undefined, taskList.Parent); //Signal for source parent tasklist.
            RefillLocalTaskGrids(taskList, [signalNum]); //No db call.
        }
        else if (task != null)
        {
            //task
            var formTaskEdit = new FormTaskEdit(task); //Handles signals for this task edit.
            formTaskEdit.Show(); //non-modal
        }
        else
        {
            //both are null; object was removed and right-click never left.
    ...
    private void Copy_Clicked()
    {
        ...
        else if (task != null)
        {
            //task
            _taskListClip = null;
            var taskCur = gridMain.SelectedTag<Task>().Copy();
            if (!PrefC.GetBool(PrefName.TasksForApptAllowMultiple)
                && taskCur.ObjectType == TaskObjectType.Appointment
                && taskCur.KeyNum > 0)
            {
                MsgBox.Show("Not allowed to copy Tasks with Appointments attached.");
                return;
            }
    ...
    private void Paste_Clicked()
    {
        if (_taskListClip != null)
        {
            ...
            var taskListNew = _taskListClip.Copy();
            var parentNumClipTL = _taskListClip.Parent;
            if (_listTaskListTreeHistory.Count > 0)
            {
                //not on main trunk
            ...
            var listSignalNums = new List<long>();
        ...
        else if (_taskClip != null)
        {
            //a task is on the clipboard
            var taskNew = _taskClip.Copy();
            var clipTaskTaskListNum = _taskClip.TaskListNum;
            if (_listTaskListTreeHistory.Count > 0)
            {
                //not on main trunk
            ...
            {
                //cut
                if (clipTaskTaskListNum == taskNew.TaskListNum)
                {
            ...
            else
            {
                ...
                for (var t = 0; t < listTaskNotes.Count; t++)
                {
            ...
            var taskHist = new TaskHist(taskNew);
        ...
        //If attaching multiple tasks to an appointment is not allowed, clear the clipboard after pasting a task with an appointment attached.
        if (_taskClip != null
            && _taskClip.ObjectType == TaskObjectType.Appointment
            && _taskClip.KeyNum != 0
            && _wasCut
            && !PrefC.GetBool(PrefName.TasksForApptAllowMultiple))
        {
            _taskClip = null;
            _taskListClip = null;
        }
    ...
    private FormTaskEdit SendToMe_Clicked(bool openTask = true)
    {
        ...
        try
        {
            ...
            Cursor = Cursors.Default;
            var formTaskEdit = new FormTaskEdit(task, task.Copy());
    ...
    {
        var task = gridMain.SelectedTag<Task>();
        var formTaskEditOpened = SendToMe_Clicked(openTask: false);
    ...
    {
        var taskGoTo = task ?? gridMain.SelectedTag<Task>();
        ...
        FormOpenDental.S_TaskGoTo(taskGoTo.ObjectType, taskGoTo.KeyNum);
    }
    ...
    private void MarkRead(Task taskMarked)
    {
        ...
        if (PrefC.GetBool(PrefName.TasksNewTrackedByUser))
        {
            ...
            var signalNum = Signalods.SetInvalid(InvalidType.Task, KeyType.Task, taskMarked.TaskNum); //Signal for markedTask.
            RefillLocalTaskGrids(taskMarked, _listTaskNotes.FindAll(x => x.TaskNum == taskMarked.TaskNum), [signalNum]);
        ...
        else
        {
            ...
            {
                var task = taskMarked.Copy();
                var taskOld = task.Copy();
                task.TaskStatus = TaskStatusEnum.Viewed;
                try
                {
                    Tasks.Update(task, taskOld);
                    var signalNum = Signalods.SetInvalid(InvalidType.Task, KeyType.Task, task.TaskNum); //Send signal for this task.
                    RefillLocalTaskGrids(task, _listTaskNotes.FindAll(x => x.TaskNum == task.TaskNum), [signalNum]);
                }
                catch (Exception ex)
                {
                    ODMessageBox.Show(ex.Message);
    ...
    private void NavAtt_Clicked(TaskAttachment taskAttachment)
    {
        ...
        var task = Tasks.GetOne(taskAttachment.TaskNum);
        if (task == null)
        {
            MsgBox.Show(this, "Please select a valid task.");
            return;
        ...
        var formTaskAttachmentEdit = new FormTaskAttachmentEdit(task);
        ...
        UserControlTasks.RefillLocalTaskGrids(task, _listTaskNotes, [signalNum]);
    }
    ...
    private void MoveListIntoAncestor(TaskList taskListNew, long taskListNumParentOld)
    {
        if (_wasCut)
        {
            ...
            var listTaskListsChildren = TaskLists.RefreshChildren(taskListNew.TaskListNum, Security.CurUser.UserNum, 0, TaskType.All);
            for (var i = 0; i < listTaskListsChildren.Count; i++)
            {
    ...
    {
        var listTaskListsChildren = TaskLists.RefreshChildren(taskListNew.TaskListNum, Security.CurUser.UserNum, 0, TaskType.All);
        var listTasksChild = Tasks.RefreshChildren(taskListNew.TaskListNum, true, DateTime.MinValue, Security.CurUser.UserNum, 0, TaskType.All); //No filtering, because all child tasks should move regardless of filtration.
        TaskLists.Update(taskListNew); //Not making a new TaskList, just moving an old one
        for (var i = 0; i < listTaskListsChildren.Count; i++)
        {
            //updates all the child tasklists and recursively calls this method for each of their children lists.
    ...
    {
        //get all children:
        var listTaskListChildren = TaskLists.RefreshChildren(taskListNew.TaskListNum, Security.CurUser.UserNum, 0, TaskType.All);
        var listTasksChild = Tasks.RefreshChildren(taskListNew.TaskListNum, true, DateTime.MinValue, Security.CurUser.UserNum, 0, TaskType.All); //No filtering, because all child tasks should duplicate regardless of filtration.
        if (_wasCut)
        {
            //Not making a new TaskList, just moving an old one
            TaskLists.Update(taskListNew);
        }
        else
        {
            //copied -- We are making a new TaskList, we're keeping the old one as well
            TaskLists.Insert(taskListNew);
        }

        //now we have a new taskListNum to work with
        for (var i = 0; i < listTaskListChildren.Count; i++)
        {
            //updates all the child tasklists and recursively calls this method for each of their children lists.
        ...
        for (var i = 0; i < listTasksChild.Count; i++)
        {
            ...
            else
            {
                ...
                var noteList = TaskNotes.GetForTask(listTasksChild[i].TaskNum);
                var taskNumNew = Tasks.Insert(listTasksChild[i]);
                for (var t = 0; t < noteList.Count; t++)
                {
    ...
    private void Delete_Clicked()
    {
        ...
        if (taskList != null)
        {
            ...
            var listTasks = Tasks.RefreshChildren(taskList.TaskListNum, true, DateTime.MinValue, Security.CurUser.UserNum, 0, TaskType.All);
            var listTaskLists = TaskLists.RefreshChildren(taskList.TaskListNum, Security.CurUser.UserNum, 0, TaskType.All);
            var countHiddenTasks = listTaskLists.Sum(x => x.NewTaskCount) + listTasks.Count - taskList.NewTaskCount;
            ...
            var signalNum = Signalods.SetInvalid(InvalidType.TaskList, KeyType.Undefined, taskList.Parent); //Signal for source tasklist.
            RefillLocalTaskGrids(taskList, [signalNum], false); //No db calls.
        }
        else if (task != null)
        {
            //Is task
            //This security logic should match FormTaskEdit for when we enable the delete button.
            var isTaskForCurUser = true;
            ...
            if (task.TaskListNum != Security.CurUser.TaskListInBox)
            {
            ...
            if (isTaskForCurUser)
            {
                var listTaskNotes = TaskNotes.GetForTask(task.TaskNum); //so we can check so see if other users have added notes
                for (var i = 0; i < listTaskNotes.Count; i++)
                {
            ...
            {
                var logText = Lan.g(this, "Deleted task from tasklist");
                if (taskList != null)
                {
            ...
            var countDocuments = TaskAttachments.GetCountDocumentForTaskNum(task.TaskNum);
            ...
            var taskHist = new TaskHist(task);
    ...
    {
        //get all children:
        var listTaskListsChildren = TaskLists.RefreshChildren(taskList.TaskListNum, Security.CurUser.UserNum, 0, TaskType.All);
        var listTasksChild = Tasks.RefreshChildren(taskList.TaskListNum, true, DateTime.MinValue, Security.CurUser.UserNum, 0, TaskType.All);
        for (var i = 0; i < listTaskListsChildren.Count; i++)
        {
    ...
    private void gridMain_CellDoubleClick(object sender, ODGridClickEventArgs e)
    {
        ...
        if (tabControl.SelectedTab == tabNew && e.Col == 2 && PrefC.GetBool(PrefName.TasksNewTrackedByUser))
        {
            //+/- column (an index varaible would help)
            return; //Don't double click on expand column, because it already has a single click functionality.
        }

        if (tabControl.SelectedTab == tabNew && e.Col == 3 && !PrefC.GetBool(PrefName.TasksNewTrackedByUser))
        {
            //ST column (an index varaible would help)
            return; //Don't double click on ST column.
        }

        if (tabControl.SelectedTab == tabNew && e.Col == 4 && !PrefC.GetBool(PrefName.TasksNewTrackedByUser))
        {
            //Job column (an index varaible would help)
            return; //Don't double click on Job column.
        }

        if (e.Col == 1)
        {
            //Task List column (an index varaible would help)
    ...
    private void gridMain_CellClick(object sender, ODGridClickEventArgs e)
    {
        ...
        var taskSelected = gridMain.SelectedTag<Task>();
        _taskCollapsedState = -1;
        if (tabControl.SelectedTab == tabNew && !PrefC.GetBool(PrefName.TasksNewTrackedByUser))
        {
    ...
    private void SetMenusEnabled()
    {
        ...
        var task = gridMain.SelectedTag<Task>();
        //Done----------------------------------
        if (gridMain.SelectedIndices.Length == 0 || taskList != null)
        {
        ...
        else if (tabControl.SelectedTab == tabUser && taskList != null)
        {
            //user tab and is a list
            menuItemSubscribe.Enabled = false;
            menuItemUnsubscribe.Enabled = true;
        }
        else if (tabControl.SelectedTab == tabMain && taskList != null)
        {
            //main and tasklist
            menuItemSubscribe.Enabled = true;
            menuItemUnsubscribe.Enabled = false;
        }
        else if (tabControl.SelectedTab == tabReminders && taskList != null)
        {
            //reminders and tasklist
            menuItemSubscribe.Enabled = true;
            menuItemUnsubscribe.Enabled = false;
        }
        else
        ...
        if (gridMain.SelectedIndices.Length > 0 && task != null)
        {
            //is task
            ...
            else
            {
                ...
                var defArray = Defs.GetDefsForCategory(DefCat.TaskPriorities, true).ToArray();
                foreach (var def in defArray)
                {
                    var menuItem = menuItemPriority.MenuItems.Add(def.ItemName);
                    menuItem.Click += (_, _) => menuTaskPriority_Click(task, def);
                }
            }
        ...
        if (gridMain.SelectedIndices.Length > 0 && task != null)
        {
            ...
            var listTaskAttachments = TaskAttachments.GetManyByTaskNum(task.TaskNum);
            if (!listTaskAttachments.IsNullOrEmpty())
            {
                menuNavAttachment.MenuItems.Clear(); //clear whatever items were in the menu before.
                MenuItem menuItemNew;
                string description;
                for (var i = 0; i < listTaskAttachments.Count; i++)
                {
                    var taskAttachment = listTaskAttachments[i];
    ...
    private void IgnoreTaskClick()
    {
        ...
        foreach (MenuItem menuItem in gridMain.ContextMenu.MenuItems)
        {
            //disable ContextMenu options
    ...
    private void menuTaskPriority_Click(Task task, Def defPriority)
    {
        ...
        try
        {
            ...
            taskHist.UserNumHist = Security.CurUser.UserNum;
            TaskHists.Insert(taskHist);
            var signalNum = Signalods.SetInvalid(InvalidType.Task, KeyType.Task, taskNew.TaskNum);
            RefillLocalTaskGrids(taskNew, _listTaskNotes.FindAll(x => x.TaskNum == taskNew.TaskNum), [signalNum]);
        }
        catch (Exception ex)
        {
            //Happens when two users edit the same task at the same time.
            ODMessageBox.Show(ex.Message);
        }
    }
    ...
    {
        var treeNodeSelected = tree.GetNodeAt(e.X, e.Y);
        ...
        for (var i = _listTaskListTreeHistory.Count - 1; i > 0; i--)
        {
        ...
        var taskListNewSelection = _listTaskListTreeHistory.FirstOrDefault(x => x.TaskListNum == (long) treeNodeSelected.Tag);
