...
public partial class UserControlTasks:UserControl {
	...
	private void UserControlTasks_Resize(object sender,EventArgs e) {
		...
		FillGrid([]);//Sets height.  Does not run query.
		LayoutTreeAndGrid();
	}
	...
	public void LayoutToolBar() {
		...
		var odToolBarButtonOptions=new ODToolBarButton();
		...
		var odToolBarButtonTask=new ODToolBarButton(Lan.g(this,"Add Task"),1,"","AddTask");
		...
		var odToolBarButton=new ODToolBarButton();
	...
	private void FillGrid(List<Signalod> listSignalods=null,bool isManualRefresh=false,bool isFilterRefresh=false) {
		   ...
		   && (tabControl.SelectedTab==tabDate || tabControl.SelectedTab==tabWeek || tabControl.SelectedTab==tabMonth))
		{
			...
			for(var i=0;i<listTaskListsRepeating.Count;i++) {
				...
				alreadyExists=false;
				for(var j=0;j<_listTaskLists.Count;j++) {//loop through Main list
			...
			for(var i=0;i<tasksRepeating.Count;i++) {
				//if already exists, skip
				alreadyExists=false;
				for(var j=0;j<_listTasks.Count;j++) {//loop through Main list
	...
	private void MoveTaskList(TaskList taskListNew,bool isInMainOrUser) {
		...
		for(var i=0;i<listTaskListsChildren.Count;i++) { //updates all the child tasklists and recursively calls this method for each of their children lists.
	...
	private void DuplicateExistingList(TaskList taskListNew,bool isInMainOrUser) {
		...
		for(var i=0;i<listTaskListChildren.Count;i++) { //updates all the child tasklists and recursively calls this method for each of their children lists.
		...
		for(var i = 0;i<listTasksChild.Count;i++) { //updates all the child tasks. If the task list was cut, then just update the child tasks' ancestors.
			if(_wasCut) {
