...
public partial class UserControlDashboardWidget:UserControl {
	...
	private List<SheetField> _listSheetFields= [];
	...
	public UserControlDashboardWidget(SheetDef sheetDefWidget,LayoutManagerForms layoutManager) {
		...
		Size=new Size(_sheetDef.Width,_sheetDef.Height);
	...
	private void SetOrRefreshData(Action<IDashWidgetField,SheetField> actionSetOrRefresh) {
		foreach(var control in UIHelper.GetAllControls(this)) {
			var sheetField=_sheet.SheetFields.FirstOrDefault(x => GetSheetFieldID(x)==control.Name);
	...
	private void RefreshPatient() {
		var patient=Patients.GetPat(FormOpenDental.PatNumCur);
	...
	private void SetStaticFields(Patient pat,Family fam=null,StaticTextData staticTextData=null) {
		...
		SheetFiller.FillFields(_sheet,patient:pat,family:fam,staticTextData:staticTextData);
	}
	...
	public void RefreshView() {
		this.InvokeIfRequired(() => {
			...
			foreach(var sheetField in _sheet.SheetFields) {
	...
	private void RefreshSheetField(SheetField field) {
		...
		var type=GetControlTypeForDisplay(field);
		var control=UIHelper.GetAllControls(this).FirstOrDefault(x => x.Name==GetSheetFieldID(field) && x.GetType()==type);
		if(control==null) {
			control=CreateControl(field,type);
		}
		control.Location=new Point(field.XPos,field.YPos);
		control.Text=field.FieldValue;
		control.ForeColor=field.ItemColor;
		control.Size=new Size(field.Width,field.Height);
	...
	protected override void OnPaint(PaintEventArgs e) {
		...
		foreach(var field in _listSheetFields) {
			using(var pen=new Pen(field.ItemColor)) {
				switch(field.FieldType) {
					case SheetFieldType.Line:
						var p1=new Point(field.XPos,field.YPos);
						var p2=new Point(field.XPos+field.Width,field.YPos+field.Height);
						e.Graphics.DrawLine(pen,p1,p2);
						break;
					case SheetFieldType.Rectangle:
						e.Graphics.DrawRectangle(pen,
							field.XPos,field.YPos,
	...
	private void CleanupDisplay() {
		...
		for(var i=Controls.Count-1;i>=0;i--) {
			var ctr=Controls[i];
			if(!_sheet.SheetFields.Select(x => GetSheetFieldID(x)).Contains(ctr.Name)) {
	...
	public void CloseWidget() {
		foreach(var ctr in UIHelper.GetAllControls(this)) {
			CloseControl(ctr);
		}
