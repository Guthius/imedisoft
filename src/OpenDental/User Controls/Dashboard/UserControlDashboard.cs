...
public partial class UserControlDashboard:UserControl {
	...
	private void StartRefreshThread(Func<UserControlDashboardWidget,bool> funcData,string name,Action postProcess=null) {
		...
		_threadRefresh=new ODThread((o) => {
			...
			foreach(var widget in ListOpenWidgets) {
				if(funcData(widget)) {
		...
		_threadRefresh.AddExceptionHandler(_ => { });//Don't crash program on Dashboard data fetching failure.
		_threadRefresh.AddExitHandler(new ODThread.WorkerDelegate((_) => postProcess?.Invoke()));
	...
	private void SetData(ODEventArgs e) {
		...
		var data=new PatientDashboardDataEventArgs(e.Tag);
	...
	public bool AddWidget(SheetDef sheetDef) {
		...
		this.InvokeIfRequired(() => { 
			...
			if(!widget.IsHandleCreated) {
				var handle=widget.Handle;//Ensure the handle for this object is created on the Main thread.
			}
		});
	...
	public void CloseDashboard(bool isLoggingOff) {
		...
		for(var i=ListOpenWidgets.Count-1;i>=0;i--) {
