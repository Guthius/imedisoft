...
public partial class DashToothChart:PictureBox,IDashWidgetField {
	...
	public void RefreshData(Patient pat,SheetField sheetField) {
		var patNum=pat?.PatNum??0;
		_sheetField=sheetField;
		var treatPlan=TreatPlans.GetActiveForPat(patNum);
		var listAppts=Appointments.GetTodaysApptsForPat(patNum);
		_imgToothChart?.Dispose();
	...
	private Image GetToothChart(Patient pat,TreatPlan treatPlan,List<Procedure> listProcs=null,List<Appointment> listAppts=null) {
		...
		var listProcsFiltered=OpenDentBusiness.SheetPrinting.FilterProceduresForToothChart(listProcs,treatPlan,true);
	...
	private List<ProcTP> GetTreatProcTPs(List<Procedure> listProcsAll) {
		var listProcs=Procedures.SortListByTreatPlanPriority(listProcsAll.FindAll(x => x.ProcStatus==ProcStat.TP),PrefC.IsTreatPlanSortByTooth);
