...
public partial class ControlImageDock:UserControl {
	...
	private void imageDockHeader_Win(object sender,EventArgs e) {
		...
		var listFormImageFloats=FuncListFloaters();
		var listStrings = new List<string>();
		listStrings.Add(Text);
		for(var i = 0;i<listFormImageFloats.Count;i++) {
		...
		var pointLScaled=new Point(Width-159,23-4);
		var pointL=PointToScreen(pointLScaled);
		var pointR=PointToScreen(new Point(Width-99,23-4));
		var win_PointL=new System.Windows.Point(pointL.X,pointL.Y);
		var win_PointR=new System.Windows.Point(pointR.X,pointR.Y);
