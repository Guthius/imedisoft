...
public partial class UserControlHostedURL:UserControl {
	...
	private bool IsTextingEnabled {
		get {
			return (!true && SmsPhones.IsIntegratedTextingEnabled()) ||
			       (true && Clinics.IsTextingEnabled(Signup.ClinicNum));
		}
	}
	...
	public UserControlHostedURL(WebServiceMainHQProxy.EServiceSetup.SignupOut.SignupOutEService signup) {
		...
		Signup=signup;
		_clinicCur=Clinics.GetFirstOrDefault(x => x.Id==Signup.ClinicNum)??Clinics.GetPracticeAsClinicZero();
		_isEmailValidForClinic=(EmailAddresses.GetFirstOrDefault(x => x.EmailAddressNum==_clinicCur.EmailAddressId)!=null);
		FillControl();
		if(true) {
	...
	private void FillControl() {
		...
		var strWebSchedNewPatWebFormsURL="";
		var strWebSchedExistingPatWebFormsURL="";
		if(Signup.ClinicNum==0) { //HQ always uses pref.
			...
			var prefNewPat=ClinicPrefs.GetPref(PrefName.WebSchedNewPatWebFormsURL,Signup.ClinicNum);
			if(prefNewPat!=null) {
				strWebSchedNewPatWebFormsURL=prefNewPat.ValueString;
			}
			var prefExistingPat=ClinicPrefs.GetPref(PrefName.WebSchedExistingPatWebFormsURL,Signup.ClinicNum);
	...
	private static void AddContextMenu(System.Windows.Forms.TextBox text) {
		if(text.ContextMenuStrip==null) {
			...
			var browse = new ToolStripMenuItem("Browse");
			browse.Click+=(_, _) => {
	...
	private void checkExistingPatEmail_Click(object sender,EventArgs e) {
		var hasShownEmailWarningMsg=false;
		if(!checkExistingPatEmail.Checked && !checkExistingPatText.Checked) {
			checkExistingPatEmail.Checked=true;
			var errorMsg="At least one authentication method must be selected for Existing Patient. Defaulting to email.";
