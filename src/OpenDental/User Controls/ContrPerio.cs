...
namespace OpenDental
{
	...
	public class ContrPerio : System.Windows.Forms.Control{
		...
		private void CalculateAttGing(ColRow colRowProbingCell,bool alsoInvalidate){
			ColRow colRowAttGingLoc=new ColRow(colRowProbingCell.Col,GetTableRow
				(_idxExamSelected,GetSection(colRowProbingCell.Row),PerioSequenceType.AttGing));
			ColRow colRowMGJLoc=new ColRow(colRowProbingCell.Col,GetTableRow
		...
		private void DrawRow(int rowIndex,PaintEventArgs e){
			...
			for(int i=0;i<_perioCellArray.GetLength(0);i++){//loop through all columns in the row
				...
				PerioSequenceType perioSequenceType=_perioSequenceTypeArray[GetSection(rowIndex)][GetSectionRow(rowIndex)];
		...
		private void DrawText(System.Windows.Forms.PaintEventArgs e){
			...
			for(int r = 0;r<listIndexesGMRows.Count;r++) {
				//Brush backgroundBrush=new SolidBrush(_colorBackShort);
				RectangleF rectangleF=GetBounds(0,listIndexesGMRows[r]);
				float x=rectangleF.Right-g.MeasureString("Recession",font).Width-1;
				float y=rectangleF.Top-1;
