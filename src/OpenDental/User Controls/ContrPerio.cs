...
public class ContrPerio : System.Windows.Forms.Control{
	...
	public ContrPerio(){
		...
		//Upper facial:
		_perioSequenceTypeArray[0]=new PerioSequenceType[6+_rowsProbing6];
		...
		_perioSequenceTypeArray[0][4]=PerioSequenceType.AttGing;
		_perioSequenceTypeArray[0][5]=PerioSequenceType.MGJ;
		for(var i=0;i<_rowsProbing6;i++){
			_perioSequenceTypeArray[0][6+i]=PerioSequenceType.Probing;
		}
		//Upper lingual:
		...
		//Lower lingual:
		_perioSequenceTypeArray[2]=new PerioSequenceType[5+_rowsProbing6];
		...
		_perioSequenceTypeArray[2][3]=PerioSequenceType.AttGing;
		_perioSequenceTypeArray[2][4]=PerioSequenceType.MGJ;
		for(var i=0;i<_rowsProbing6;i++){
			_perioSequenceTypeArray[2][5+i]=PerioSequenceType.Probing;
		}
		//Lower facial:
		_perioSequenceTypeArray[3]=new PerioSequenceType[6+_rowsProbing6];
		...
		_perioSequenceTypeArray[3][4]=PerioSequenceType.AttGing;
		_perioSequenceTypeArray[3][5]=PerioSequenceType.MGJ;
		for(var i=0;i<_rowsProbing6;i++){
			_perioSequenceTypeArray[3][6+i]=PerioSequenceType.Probing;
		}
		ClearDataArray();
		ListColRowsSelected= [];
	}
	...
	public string ComputeIndex(BleedingFlags bleedingFlags){
		...
		var unskippedTeeth=32-_listSkippedTeeth.Count;
		if(unskippedTeeth==0) {
			return "";
		}
	...
	public List<string> CountTeeth(PerioSequenceType perioSequenceType){
		...
		switch(perioSequenceType){
			...
			case PerioSequenceType.CAL:
				...
				break;
			case PerioSequenceType.AttGing:
				prefVal=PrefC.GetInt(PrefName.PerioRedAttGing);
				break;
	...
	public Rectangle GetRectangleBoundsShowing() {
		var rectangle=new Rectangle(new Point(0,0),new Size(_widthShowing+2,_heightShowing+2));//Add 2 pixels to the top and bottom for the border.
		return rectangle;
	}
	...
	public static bool IsImplant(int intTooth) {
		...
		var listProceduresForTooth=_listProcedures.FindAll(x => x.ToothNum==intTooth.ToString() && x.ProcStatus.In(
			ProcStat.C,ProcStat.EC,ProcStat.EO));
		for(var i = 0;i<listProceduresForTooth.Count;i++) {
			var procedureCode=ProcedureCodes.GetProcCode(listProceduresForTooth[i].CodeNum);
			if(procedureCode.PaintType==ToothPaintingType.Implant && !listProceduresForTooth[i].HideGraphics) {
				return true;
			}
		}
		return false;
	...
	public void LoadData(bool selectCell=true){
		...
		var cellText="";
		var cellBleed=0;
		for(var examI=0;examI<=_idxExamSelected;examI++){//exams, earliest through current
			for(var seqI=0;seqI<Enum.GetNames(typeof(PerioSequenceType)).Length;seqI++){//sequences
				for(var toothI=1;toothI<33;toothI++){//measurements. Always 33 per sequence(0 is not used).
					var examNumSelected=ListPerioExams[examI].PerioExamNum;
					var perioSequenceType=(PerioSequenceType)seqI;
					var perioMeasure=GetPerioMeasure(examNumSelected,perioSequenceType,toothI);
					if(perioMeasure==null){//.PerioMeasureNum==0)
						continue;//no measurement for this seq and tooth
					}
					for(var surfI=0;surfI<Enum.GetNames(typeof(PerioSurf)).Length;surfI++){//surfaces(6or7)
						...
						//calculate CAL and Att Ging. All ging will have been done before any probing.
						if(seqI==(int)PerioSequenceType.Probing){
							CalculateCAL(colRowSelected,alsoInvalidate:false);
							CalculateAttGing(colRowSelected,alsoInvalidate: false);
						}
					}//for surfI
				}//for toothI
			}//for seqI
		...
		ListColRowsSelected= [new ColRow(1, GetTableRow(_idxExamSelected, 0, PerioSequenceType.Probing))];
		...
		var intTooth=GetToothNumCur(GetSection(ListColRowsSelected[0].Row));
	...
	public void SaveCurExam(PerioExam perioExam) {
		...
		var perioExamFromDb=PerioExams.GetOnePerioExam(perioExam.PerioExamNum);
		...
		var listPerioMeasureItemsBleeding=ListPerioMeasureItemsChanged.FindAll(x => x.SeqType==PerioSequenceType.Probing)
		...
		for(var i=0;i<ListPerioMeasureItemsChanged.Count();i++){
			var perioSequenceType=ListPerioMeasureItemsChanged[i].SeqType;
			var toothI=ListPerioMeasureItemsChanged[i].ToothNum;
			//new measurement
			var perioExamNumSelected=ListPerioExams[_idxExamSelected].PerioExamNum;
			var perioMeasure=GetPerioMeasure(perioExamNumSelected,perioSequenceType,toothI);
	...
	public void SetBleedingFlagForColRow(ColRow colRow,BleedingFlags bleedingFlags) {
		...
		var perioCell=GetPerioCell(colRow,false);
	...
	public void ToggleSkip(long perioExamNum) {
		...
		for(var i=0;i<selectedTeeth.Count;i++){
	...
	private void AdvanceCell(bool isReverse=false){
		...
		var colRowToSet=TryFindNextCell(ListColRowsSelected[0],isReverse);
		if(colRowToSet==null) {
			return;
		}
	...
	private void CalculateCAL(ColRow colRowProbingCell,bool alsoInvalidate){
		var colRowCalLoc=new ColRow(colRowProbingCell.Col,GetTableRow
			(_idxExamSelected,GetSection(colRowProbingCell.Row),PerioSequenceType.CAL));
		var colRowGingLoc=new ColRow(colRowProbingCell.Col,GetTableRow
		...
		var probValue=SIn.Int(_perioCellArray[colRowProbingCell.Col,colRowProbingCell.Row].Text);
		var gingValue=SIn.Int(_perioCellArray[colRowGingLoc.Col,colRowGingLoc.Row].Text);
		if(gingValue>100) {
	...
	private void CalculateAttGing(ColRow colRowProbingCell,bool alsoInvalidate){
		var colRowAttGingLoc=new ColRow(colRowProbingCell.Col,GetTableRow
			(_idxExamSelected,GetSection(colRowProbingCell.Row),PerioSequenceType.AttGing));
		var colRowMGJLoc=new ColRow(colRowProbingCell.Col,GetTableRow
			(_idxExamSelected,GetSection(colRowProbingCell.Row),PerioSequenceType.MGJ));
		//Stop if MGJ doesn't exist.  Upper lingual doesn't have MGJ.
		if(colRowMGJLoc.Col==-1 || colRowMGJLoc.Row==-1) {
			return;
		}
		if(_perioCellArray[colRowProbingCell.Col,colRowProbingCell.Row].Text==null 
		   || _perioCellArray[colRowProbingCell.Col,colRowProbingCell.Row].Text==""
		   || _perioCellArray[colRowMGJLoc.Col,colRowMGJLoc.Row].Text==null 
		   || _perioCellArray[colRowMGJLoc.Col,colRowMGJLoc.Row].Text=="")
		{
			_perioCellArray[colRowAttGingLoc.Col,colRowAttGingLoc.Row].Text="";
			if(alsoInvalidate){
		...
		var probValue=SIn.Int(_perioCellArray[colRowProbingCell.Col,colRowProbingCell.Row].Text);
		var MGJValue=SIn.Int(_perioCellArray[colRowMGJLoc.Col,colRowMGJLoc.Row].Text);
	...
	private void ClearDataArray(){
		...
		var y=0;
		for(var section=0;section<4;section++){
			for(var i=0;i<_perioSequenceTypeArray[section].Length;i++){
				...
				switch(_perioSequenceTypeArray[section][i]){
					...
					case PerioSequenceType.MGJ:
						...
						break;
					case PerioSequenceType.AttGing:
						_perioCellArray[0,y].Text=Lan.g(this,"auto Att Ging");
						break;
					case PerioSequenceType.Probing:
						break;
					default:
	...
	private void ClearValue(){
		...
		for(var i=0;i<ListColRowsSelected.Count;i++) {
			var perioCell=_perioCellArray[ListColRowsSelected[i].Col,ListColRowsSelected[i].Row];
			perioCell.Text="";
			...
			if(_perioSequenceTypeArray[GetSection(ListColRowsSelected[i].Row)][GetSectionRow(ListColRowsSelected[i].Row)]==PerioSequenceType.Probing){ 
				...
				//Any time we change GM, we invalidate because recession sometimes flips to GM.
				if(PrefC.GetBool(PrefName.PerioRecessionInsteadOfGM)) {
					Invalidate();//Causes a redraw which will update "Recession" labels to "Ging Marg" if any gingival margin values are negative
				}
			...
			//Clears out auto Att Ging when either MGJ or probing depth are cleared out.
			if(_perioSequenceTypeArray[GetSection(ListColRowsSelected[i].Row)][GetSectionRow(ListColRowsSelected[i].Row)]==PerioSequenceType.Probing){ 
				CalculateAttGing(ListColRowsSelected[i],alsoInvalidate:true);
			}
			else if(_perioSequenceTypeArray[GetSection(ListColRowsSelected[i].Row)][GetSectionRow(ListColRowsSelected[i].Row)]==PerioSequenceType.MGJ){
				CalculateAttGing(new ColRow(ListColRowsSelected[i].Col,GetTableRow
					(_idxExamSelected,GetSection(ListColRowsSelected[i].Row),PerioSequenceType.Probing)),alsoInvalidate:true);
			}
		}
	}
	...
	private void EnterValueLetter(string letter){
		...
		if(letter=="b" || letter=="s" || letter=="p" || letter=="c") {
			var hasText=false;
			for(var k=0;k<ListColRowsSelected.Count;k++) {
				var perioCell=GetPerioCell(ListColRowsSelected[k],false);
				var isSkippedTooth=_listSkippedTeeth.Contains(perioCell.ToothNum);
				if(ThreeAtATime){
					//don't backup
				}
		...
		var section=GetSection(ListColRowsSelected[0].Row);
		for(var i=0;i<_rowPosArray.Length;i++) {
			var idxSectRow=GetSectionRow(i);
			...
			if(letter=="m") {
				var colSelected=ListColRowsSelected[0].Col;
				var colLeft=colSelected-1;
				var colRight=colSelected+1;
	...
	private void EnterValueNumber(int number){
		...
		for(var i=0;i<ListColRowsSelected.Count;i++) {
			...
			var perioCell=GetPerioCell(ListColRowsSelected[i],setText:false);
			//might be able to eliminate these two lines
			perioCell.Text=number.ToString();
			_perioCellArray[ListColRowsSelected[i].Col,ListColRowsSelected[i].Row]=perioCell;
			Invalidate(Rectangle.Ceiling(GetBounds(ListColRowsSelected[i].Col,ListColRowsSelected[i].Row)));
			SetHasChanged(ListColRowsSelected[i].Col,ListColRowsSelected[i].Row);
			var perioSequenceType=_perioSequenceTypeArray[GetSection(ListColRowsSelected[i].Row)][GetSectionRow(ListColRowsSelected[i].Row)];
			if(perioSequenceType==PerioSequenceType.Probing){ 
				CalculateCAL(ListColRowsSelected[i],alsoInvalidate:true);
			}
			else if(_perioSequenceTypeArray[GetSection(ListColRowsSelected[i].Row)][GetSectionRow(ListColRowsSelected[i].Row)]==PerioSequenceType.GingMargin){
				var colRow=new ColRow(ListColRowsSelected[i].Col,GetTableRow
			...
			//Calculate auto Att Ging when an MGJ or probing depth value is entered.
			if(perioSequenceType==PerioSequenceType.Probing){ 
				CalculateAttGing(ListColRowsSelected[i],alsoInvalidate:true);
			}
			else if(_perioSequenceTypeArray[GetSection(ListColRowsSelected[i].Row)][GetSectionRow(ListColRowsSelected[i].Row)]==PerioSequenceType.MGJ){
				var colRow=new ColRow(ListColRowsSelected[i].Col,GetTableRow
					(_idxExamSelected,GetSection(ListColRowsSelected[i].Row),PerioSequenceType.Probing));
				CalculateAttGing(colRow,alsoInvalidate:true);
			}
		}
		AdvanceCell();
	}
	...
	private void FillDates(){
		int intTableRow;
		for(var examI=0;examI<_idxExamSelected+1;examI++){//-ProbingOffset;examI++){
			for(var sectionI=0;sectionI<4;sectionI++){
	...
	private void FillPosArrays(){
		...
		var yPos=0;
		//U facial
		for(var i=_perioSequenceTypeArray[0].Length-1;i>=0;i--){
		...
		yPos+=_heightTooth+1;
		//upper lingual
		for(var i=0;i<_perioSequenceTypeArray[1].Length;i++){
		...
		for(var i=_perioSequenceTypeArray[2].Length-1;i>=0;i--){
		...
		yPos+=_heightTooth+1;
		//lower facial
		for(var i=0;i<_perioSequenceTypeArray[3].Length;i++){
		...
		_colPosArray[0]=0;
		var colPos=1+_widthLeft+1;//We loop through the 16 teeth, since they are ints
		for(var i=0;i<16;i++){//looping 0 to 15
	...
	private int GetCellBleedValue(int examIndex,int intTooth,PerioSurf perioSurf){
		var colRow=GetCell(examIndex,PerioSequenceType.Probing,intTooth,perioSurf);
	...
	private ColRow GetCellFromPixel(int x,int y){
		...
		var col=0;
		for(var i=0;i<_colPosArray.Length;i++){
			if(x<_colPosArray[i]){
				col=i-1;
				break;
			}
	...
	private int GetCellValue(int examIndex,PerioSequenceType perioSequenceType,int intTooth,PerioSurf perioSurf){
		var colRow=GetCell(examIndex,perioSequenceType,intTooth,perioSurf);
		if(colRow.Col==-1 || colRow.Row==-1){
			return -1;
		}
		//if(intTooth==4)
	...
	private PerioCell GetPerioCell(ColRow colRow,bool setText) {
		var perioCell=_perioCellArray[colRow.Col,colRow.Row];
	...
	private int GetTableRow(int examIndex,int section,PerioSequenceType perioSequenceType){
		if(perioSequenceType==PerioSequenceType.Probing || perioSequenceType==PerioSequenceType.BleedSupPlaqCalc){
			...
			var sectionRow=examIndex-_offsetExam//correct for offset
		...
		for(var i=0;i<_perioSequenceTypeArray[section].Length-_rowsProbing6;i++){
	...
	private int GetToothNumFromColRow(int section,ColRow colRow) {
		var intTooth=(int)Math.Ceiling((double)colRow.Col/3);
	...
	private void KeyArrowDown_Clicked(ColRow colRowSelected) {
		...
		var colSelected=colRowSelected.Col;
		var rowDownOne=colRowSelected.Row+1;
		var idxMax=_rowPosArray.Length-1;
		for(var i=0;i<idxMax-colRowSelected.Row;i++) {//all of the rows below current row
			var rowNext=rowDownOne+i;
			var sectRowIdx=GetSectionRow(rowNext);
			var section=GetSection(rowNext);
			...
			if(_perioSequenceTypeArray[section][sectRowIdx]==PerioSequenceType.AttGing){//Skip auto att ging rows.
				continue;
			}
	...
	private void KeyArrowUp_Clicked(ColRow colRowSelected) {
		...
		var colCol=colRowSelected.Col;
		var rowUpOne=colRowSelected.Row-1;
		for(var i=0;i<colRowSelected.Row;i++) {//all the rows above current row
			var rowPrevious=rowUpOne-i;
			var sectRowIdx=GetSectionRow(rowPrevious);
			var section=GetSection(rowPrevious);
			...
			if(_perioSequenceTypeArray[section][sectRowIdx]==PerioSequenceType.Mobility) {
				var colLeft=colCol-1;
				var colRight=colCol+1;
	...
	private void SetHasChanged(int col,int row){
		...
		var sectionRow=GetSectionRow(row);
		var intTooth=(int)Math.Ceiling((double)col/3);//1-16
		...
		var perioSequenceType=_perioSequenceTypeArray[section][sectionRow];
	...
	protected override void OnKeyDown(KeyEventArgs e)
	{
		...
		var perioCell=GetPerioCell(ListColRowsSelected[0],setText:false);
	...
	protected override void OnMouseDown(MouseEventArgs e){
		...
		base.OnMouseDown(e);
		var listColRowsPrev=new List<ColRow>(ListColRowsSelected);
		ListColRowsSelected.Clear();//Each new drag unselects previous drag.
		for(var i=0;i<listColRowsPrev.Count;i++) {
	...
	protected override void OnMouseMove(MouseEventArgs e) {
		...
		var colRowCellMovedOver=GetCellFromPixel(e.X,e.Y);
		if(colRowCellMovedOver.Col==0){
			return;//Left column only for dates
		}
		var section=GetSection(colRowCellMovedOver.Row);
		var sectRowIdx=GetSectionRow(colRowCellMovedOver.Row);
	...
	protected override void OnMouseUp(MouseEventArgs e) {
		...
		var colRowNewCell=GetCellFromPixel(e.X,e.Y);
		if(colRowNewCell.Col==0){
			return;//Left column only for dates
		}
		var section=GetSection(colRowNewCell.Row);
		if(section==-1){//clicked on a toothNum
			var intTooth=(int)Math.Ceiling((double)colRowNewCell.Col/3);
		...
		var sectRowIdx=GetSectionRow(colRowNewCell.Row);
		if(sectRowIdx<0 || sectRowIdx>=_perioSequenceTypeArray[section].Length) {
			//User clicked on a tooth row, not a section row.
			return;
		}
		...
		else if(_perioSequenceTypeArray[section][sectRowIdx]==PerioSequenceType.AttGing){
			return;//never allowed to edit Att Ging
		}
	...
	protected override void OnResize(EventArgs e){
		base.OnResize(e);
		//_widthMeas=(10);
		_widthTooth=33;//3*(10+1)
		_widthLeft=72;	//increased from 65 to hold non English-US short dates (dd-mmm-yyyy)
		_heightProb=15;
		_heightShort=12;
		_heightTooth=16;
		FillPosArrays();
	...
	private void DrawBackground(System.Windows.Forms.PaintEventArgs e){
		...
		var yPos1=1+_rowsProbing6*(_heightProb+1);
		var yPos2=yPos1+(_perioSequenceTypeArray[0].Length-_rowsProbing6)*(_heightShort+1)-1;
		top=yPos1;
		bottom=yPos2;
	...
	private void DrawCurCells(System.Windows.Forms.PaintEventArgs e){
		...
		var g=e.Graphics;
		for(var i=0;i<ListColRowsSelected.Count;i++) {
			var rectangleFBounds=GetBounds(ListColRowsSelected[i].Col,ListColRowsSelected[i].Row);
			if(_perioSequenceTypeArray[GetSection(ListColRowsSelected[i].Row)][GetSectionRow(ListColRowsSelected[i].Row)]==PerioSequenceType.Probing){
			...
			var top=(int)rectangleFBounds.Top;
			var bottom=(int)rectangleFBounds.Bottom;
			var left=(int)rectangleFBounds.Left;
			var right=(int)rectangleFBounds.Right;
			...
			{//if the clipping rect includes any part of the CurCell
				...
				g.FillRectangle(new SolidBrush(_colorHighlight),left+1,top
	...
	private void DrawGridlines(System.Windows.Forms.PaintEventArgs e){
		var g=e.Graphics;
		var penMain=new Pen(_colorLinesMain);
		var penMinor=new Pen(_colorLinesMinor);
		...
		for(var i=1;i<_colPosArray.Length;i+=3){
			if(i==1 || i==25){
		...
		var y=(int)_rowPosArray[GetTableRow(0,0,PerioSequenceType.MGJ)];
		...
		using(var penHeavy=new Pen(Color.Black,2f)){
	...
	private void DrawRow(int rowIndex,PaintEventArgs e){
		...
		var redThresh=0;
		var cellValue=0;
		for(var i=0;i<_perioCellArray.GetLength(0);i++){//loop through all columns in the row
			...
			if(i==0){//first column, dates and row labels
				var x=rectangleF.Right-g.MeasureString(_perioCellArray[i,rowIndex].Text,font).Width-1;
				var y=rectangleF.Top-1;
				...
				if(GetSection(rowIndex)!=-1 && _perioSequenceTypeArray[GetSection(rowIndex)][GetSectionRow(rowIndex)]==PerioSequenceType.GingMargin){//GingMargin row Label
					continue;//Don't draw label, this is handled in DrawText() in accordance with the PerioRecessionInsteadOfGM pref
				}
			...
			if(_perioSequenceTypeArray[GetSection(rowIndex)][GetSectionRow(rowIndex)]==PerioSequenceType.Probing){
				if((_perioCellArray[i,rowIndex].Bleeding & BleedingFlags.Plaque) > 0){
					e.Graphics.FillRectangle(new SolidBrush(_colorPlaque),rectangleF.X+0,rectangleF.Y,2.5f,3.5f);
				}
				if((_perioCellArray[i,rowIndex].Bleeding & BleedingFlags.Calculus) > 0){
					e.Graphics.FillRectangle(new SolidBrush(_colorCalc),rectangleF.X+2.5f,rectangleF.Y,2.5f,3.5f);
				}
				if((_perioCellArray[i,rowIndex].Bleeding & BleedingFlags.Blood) > 0){
					e.Graphics.FillRectangle(new SolidBrush(_colorBlood),rectangleF.X+5f,rectangleF.Y,2.5f,3.5f);
				}
				if((_perioCellArray[i,rowIndex].Bleeding & BleedingFlags.Suppuration) > 0){
					e.Graphics.FillRectangle(new SolidBrush(_colorSuppuration),rectangleF.X+7.5f,rectangleF.Y,2.5f,3.5f);
				}
			...
			switch(_perioSequenceTypeArray[GetSection(rowIndex)][GetSectionRow(rowIndex)]){
				...
				case PerioSequenceType.CAL:
					...
					break;
				case PerioSequenceType.AttGing:
					redThresh=PrefC.GetInt(PrefName.PerioRedAttGing);
					break;
				case PerioSequenceType.Furcation:
			...
			var perioSequenceType=_perioSequenceTypeArray[GetSection(rowIndex)][GetSectionRow(rowIndex)];
			if(((perioSequenceType==PerioSequenceType.MGJ || perioSequenceType==PerioSequenceType.AttGing) && cellValue<=redThresh)
			   ||perioSequenceType!=PerioSequenceType.MGJ && perioSequenceType!=PerioSequenceType.AttGing && cellValue>=redThresh)
			{
				if(drawOld) {
	...
	private void DrawSkippedTeeth(System.Windows.Forms.PaintEventArgs e){
		...
		for(var i=0;i<_listSkippedTeeth.Count;i++){
			...
			var top=(int)retangleFBounds.Top;
			var bottom=(int)retangleFBounds.Bottom;
			var left=(int)retangleFBounds.Left;
			var right=(int)retangleFBounds.Right;
			//test clipping rect later
	...
	private void DrawText(System.Windows.Forms.PaintEventArgs e){
		...
		for(var i=0;i<_perioCellArray.GetLength(1);i++){//loop through all rows in the table
			DrawRow(i,e);
		}
		//Update any "Recession" labels to "Ging Marg" if there are any negative gingival margin values or vice versa
		var setLabelToRecession=true;
		if(!DesignMode){
			setLabelToRecession=PrefC.GetBool(PrefName.PerioRecessionInsteadOfGM);
		}
		var listIndexesGMRows=new List<int>();
		//Loop through each row looking for gingival margin rows
		for(var r = 0;r<_perioCellArray.GetLength(1);r++) {
			if(_perioCellArray[0,r].Text!="Ging Marg"  && _perioCellArray[0,r].Text!="Recession") {
				continue;
			}
			listIndexesGMRows.Add(r);//Keep track of each gingival margin row's index
			//Search all columns in this row for negative values
			for(var c = 1;c<_perioCellArray.GetLength(0);c++) {//Start at 1 to avoid labels
				var CellValue=SIn.Int(_perioCellArray[c, r].Text);
		...
		//Draw label for each gingival margin row
		for(var r = 0;r<listIndexesGMRows.Count;r++) {
			//Brush backgroundBrush=new SolidBrush(_colorBackShort);
			var rectangleF=GetBounds(0,listIndexesGMRows[r]);
			var x=rectangleF.Right-g.MeasureString("Recession",font).Width-1;
			var y=rectangleF.Top-1;
			if(setLabelToRecession) {
				e.Graphics.DrawString("Recession",font,new SolidBrush(Color.Black),x,y);
				continue;
			}
			x=rectangleF.Right-g.MeasureString("Ging Marg",font).Width-1;//"Recession" and "Ging marg" are the same # of characters, but just to be safe
			e.Graphics.DrawString("Ging Marg",font,new SolidBrush(Color.Black),x,y);
		}
	}
	...
	{
		var perioAdvancePosMove=GetPrevious(perioAdvancePos,listPerioAdvancePoss);
		if(getNext){
		...
		var isMaxillaryMGJ = perioSequenceType==PerioSequenceType.MGJ && perioAdvancePosMove.ToothNum.Between(1,16);
		var isMaxillaryMGJOrMobility = isMaxillaryMGJ || perioSequenceType==PerioSequenceType.Mobility;
		if(isMaxillaryMGJOrMobility && !perioAdvancePosMove.IsFacial) {
			...
			var perioAdvancePosMGJ=perioAdvancePosMove;
	...
	public ColRow TryFindNextCell(ColRow colRow,bool isReverse,bool setDirection=true) {
		var colRowNext=colRow;
		var perioSequenceType=_perioSequenceTypeArray[GetSection(colRowNext.Row)][GetSectionRow(colRowNext.Row)];
		var startedOnSkipped=false;//special situation:
		var section=GetSection(colRowNext.Row);//used to test skipped teeth
		var newSection=section;//in case it doesn't change
		var intTooth=GetToothNumCur(section);//used to test skipped teeth
		if(_listSkippedTeeth.Contains(intTooth)){
			startedOnSkipped=true;
		}
		var limit=0;
		var count=0;//count the number of positions skipped.
		while(true){
			...
			var isValidLoc=true;//used when testing for skipped tooth and mobility location
	...
	private ColRow TryAutoAdvanceFacialsFirst(ColRow colRowSelected,bool startedOnSkipped,bool isReverse,bool setDirection) {
		var perioCellSelected=GetPerioCellFromColRow(colRowSelected);
		var colRowNext=colRowSelected;
		var perioSequenceType=GetSequenceForColRow(colRowNext);
		var section=GetSection(colRowNext.Row);//used to test skipped teeth
		var listPerioAdvancePoss=PerioAdvancePos.GetPath(EnumAdvanceSequence.FacialsFirst);
		var isFacial=perioCellSelected.IsFacial;
		var perioAdvancePos=listPerioAdvancePoss.FirstOrDefault(x => x.ToothNum==perioCellSelected.ToothNum && x.IsFacial==isFacial);
		...
		var perioAdvancePosNext=GetPerioAdvancePosFromList(perioAdvancePos,getNext:true,perioSequenceType,listPerioAdvancePoss);
		var shouldAdvanceLeft=false;
		...
		var perioCellNext=GetPerioCellFromColRow(colRowNext);
		...
		var perioAdvancePosPrev=GetPerioAdvancePosFromList(perioAdvancePos,getNext:false,perioSequenceType,listPerioAdvancePoss);
		var didStartOnOneOrAutoAdv = startedOnCellOne && colRowNext.Col==1 && !perioAdvancePos.IsFacial && perioAdvancePosPrev!=null;
		var skipSameToothCheck=didStartOnOneOrAutoAdv || perioSequenceType==PerioSequenceType.Mobility;
	...
	private ColRow TryAutoAdvanceRight(ColRow colRowSelected,int section,bool setDirection,PerioSequenceType perioSequenceType,bool startedOnSkipped,int count) 
	{
		...
		var newSection=0;
		var newRow=0;
		var intTooth=GetToothNumCur(section);
...
public class PerioAdvancePos {
	...
	public static List<PerioAdvancePos> GetPath(EnumAdvanceSequence enumAdvanceSequence){
		var listPerioAdvancePoss=new List<PerioAdvancePos>();
	...
	{
		var listToothNumbers=GenerateToothRange(startTooth,stopTooth);
		var listPerioAdvancePoss=new List<PerioAdvancePos>();
		for(var i=0;i<listToothNumbers.Count;i++){
			var perioAdvancePos=new PerioAdvancePos(listToothNumbers[i],isFacial,enumCurrentDirection);
	...
	private static List<int> GenerateToothRange(int intStart,int intStop){
		var listToothNumbers=new List<int>();
		...
		for(var i=intStart;i<=intStop;i++){//example 1,16 will include both 1 and 16.
			listToothNumbers.Add(i);
		}
		return listToothNumbers;
	}
