...
public partial class UserControlSetupWizFeeSched:SetupWizControl {
	...
	private void FillGrid(){
		...
		var col=new GridColumn(Lan.g("TableFeeScheds","Description"),145);
		...
		foreach(var feeSched in _listFeeScheds) {
			row=new GridRow();
			row.Tag=feeSched;
	...
	private void butAdd_Click(object sender, System.EventArgs e) {
		...
		for(var i=0;i<_listFeeScheds.Count;i++){
			if(frmFeeSchedEdit.FeeSchedCur.FeeSchedNum==_listFeeScheds[i].FeeSchedNum){
	...
	private void butImport_Click(object sender,EventArgs e) {
		...
		var feeSched=((FeeSched)gridMain.SelectedGridRows[0].Tag);//get selected fee sched from grid.
		try { 
	...
	private void butEditFee_Click(object sender,EventArgs e) {
		...
		using var FormPC = new FormProcCodes();
		FormPC.ShowDialog();
	}

	private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		var formFeeSchedEdit=new FrmFeeSchedEdit();
		formFeeSchedEdit.FeeSchedCur=_listFeeScheds[e.Row];
		formFeeSchedEdit.ShowDialog();
		if(!formFeeSchedEdit.IsDialogOK) {
			return;
		}
		FeeScheds.RefreshCache();
		_listFeeScheds=FeeScheds.GetDeepCopy();
		...
		FillGrid();
		for(var i=0;i<_listFeeScheds.Count;i++){
			if(formFeeSchedEdit.FeeSchedCur.FeeSchedNum==_listFeeScheds[i].FeeSchedNum){
				gridMain.SetSelected(i,true);
			}
		}	
	}
