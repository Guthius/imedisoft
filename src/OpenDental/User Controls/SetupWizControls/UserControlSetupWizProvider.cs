...
namespace OpenDental.User_Controls.SetupWizard {
	public partial class UserControlSetupWizProvider:SetupWizControl {
		...
		private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			...
			FormPE.ProviderCur = selectedProv;
			FormPE.ShowDialog();
		...
		private void butAdd_Click(object sender,EventArgs e) {
			...
			using FormProvEdit FormPE = new FormProvEdit();
			FormPE.ProviderCur=new Provider();
			FormPE.ProviderCur.IsNew=true;
			...
			if(gridMain.SelectedIndices.Length>0) {//place new provider after the first selected index. No changes are made to DB until after provider is actually inserted.
				FormPE.ProviderCur.ItemOrder=((Provider)gridMain.ListGridRows[gridMain.SelectedIndices[0]].Tag).ItemOrder;//now two with this itemorder
			}
			else if(gridMain.ListGridRows.Count>0) {
				FormPE.ProviderCur.ItemOrder=((Provider)gridMain.ListGridRows[gridMain.ListGridRows.Count-1].Tag).ItemOrder+1;
			}
			else {
				FormPE.ProviderCur.ItemOrder=0;
			}
			FormPE.IsNew=true;
			FormPE.ShowDialog();
			if(FormPE.DialogResult!=DialogResult.OK) {
