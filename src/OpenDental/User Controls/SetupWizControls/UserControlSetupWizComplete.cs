using System;

namespace OpenDental.User_Controls.SetupWizard;

public partial class UserControlSetupWizComplete : SetupWizControl
{
    public UserControlSetupWizComplete(string name)
    {
        InitializeComponent();
        labelDone.Text = "Congratulations! You have finished setting up your " + name + "!";
        labelEnd.Text = "\r\n" + "You can always go back through this setup wizard if you need to make any modifications to your " + name + ".";
    }

    private void UserControlSetupWizComplete_Load(object sender, EventArgs e)
    {
        IsDone = true;
    }
}