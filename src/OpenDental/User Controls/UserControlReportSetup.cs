...
public partial class UserControlReportSetup:UserControl {
	...
	public void InitializeOnStartup(bool refreshData,long userGroupNum,bool isPermissionMode) {
		...
		if(refreshData) {
			...
			ListGroupPermissionsOld= [];
			foreach(var perm in ListGroupPermissionsForReports) {
	...
	private void FillGrids() {
		...
		//Add Columns
		var widthDisplayNameCol=140;
		gridProdInc.Columns.Clear();
		var displayColumnTitle=Lans.g("Display Name");
		var allowedColumnTitle=Lans.g("Allowed");
		var subMenuColumnTitle=Lans.g("Favorite");
		var hiddenColumnTitle=Lans.g("Hidden");
		...
		foreach(var reportCur in ListDisplayReportAll) {
			var row= new GridRow();
			if(_isPermissionMode) {
				...
				if(GroupPermissions.HasPermission(_listUserGroups[comboUserGroup.SelectedIndex].UserGroupNum,
					   ...
					   groupPermissions:ListGroupPermissionsForReports))
				{
					row.Cells.Add("X");
				}
				else {
	...
	private void ReportPermissionsSetAllForGroup() {
		...
		if(!ListGroupPermissionsForReports.Exists(x => x.UserGroupNum==_listUserGroups[comboUserGroup.SelectedIndex].UserGroupNum && x.FKey==0)) {
			var groupPerm=new GroupPermission();
	...
	private void grid_CellClick(object sender,ODGridClickEventArgs e) {
		...
		var selectedCat=(DisplayReportCategory)_selectedGrid.Tag;
		...
		var clicked=ListDisplayReportAll.Find(x => x.Category == selectedCat && x.ItemOrder == _selectedCell.Y);
		if(_isPermissionMode) {
			if(_selectedCell.X==1) {
					   ...
					   groupPermissions:ListGroupPermissionsForReports))
				{
					...
					if(HasReportPermissionsAllForGroup()) {
						...
						ListGroupPermissionsForReports.RemoveAll(x => x.UserGroupNum==_listUserGroups[comboUserGroup.SelectedIndex].UserGroupNum);
						for(var i=0;i < ListDisplayReportAll.Count;i++) {
					...
					var listDisplayReports=ListDisplayReportAll;
					...
					var listGroupPerms=ListGroupPermissionsForReports.FindAll(x => x.UserGroupNum==_listUserGroups[comboUserGroup.SelectedIndex].UserGroupNum && x.FKey!=0);
	...
	private void butUp_Click(object sender,EventArgs e) {
		...
		var selectedCat = (DisplayReportCategory)_selectedGrid.Tag;
		var selectedReport=ListDisplayReportAll.Find(x => x.Category == selectedCat && x.ItemOrder == _selectedCell.Y);
		if(selectedReport.ItemOrder==0) {
			return; //the item is already the first in the list and cannot go up anymore.
		}
		var switchReport=ListDisplayReportAll.Find(x => x.Category == selectedCat && x.ItemOrder == _selectedCell.Y-1);
