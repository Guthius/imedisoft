...
public partial class UserControlReminderMessage:UserControl {
	...
	public UserControlReminderMessage(ApptReminderRule apptReminder,bool doCheckDisclaimer=true) {
		InitializeComponent();
	...
	private void RefreshEmail() {
		...
		try {
			var text=MarkupEdit.TranslateToXhtml(_templateEmail,isEmail:true,scale:1);
			//browserEmailBody.Document.Body.Style = "font-size:"+LayoutManager.ScaleF(12).ToString("f1")+";";
	...
	public List<string> ValidateTemplates() {
		...
		if(Rule.TypeCur==ApptReminderType.GeneralMessage) {
			...
			if (sms!="" || email!="") {
				var language=MiscUtils.GetCultureFromThreeLetter(Rule.Language)?.EnglishName??Rule.Language;
				if (language.IsNullOrEmpty()) {
					language="Default";
				}
				var and="";
				var tab=Lan.g(this,"tab");
				if(sms!="" && email!="") {
					and=" "+Lan.g(this,"and")+" ";
					tab=Lan.g(this,"tabs");
				}
				var errorMessage=Lan.g(this,"The [Appts] tag template has not been configured for ")+sms+and+email+Lan.g(this," messages for ")+language+". "+Lan.g(this,"Go to the ")+language+Lan.g(this," tab")+", "+Lan.g(this,"click on the Advanced button")+", "+Lan.g(this,"and set up Aggregated ")+sms+and+email+Lan.g(this," Template Per Appointment in the ")+sms+and+email+Lan.g(this," Templates ")+tab+".";
		...
		if(Rule.Language!="" && Rule.ApptReminderRuleNum==0 && !Rule.TypeCur.In(ApptReminderType.Birthday,ApptReminderType.GeneralMessage)) {
			var err=Lan.g(this,"Aggregated templates have not been set up for all additional languages. Click on the 'Advanced' button to set up aggregated templates.");
			if(Rule.TypeCur==ApptReminderType.Arrival) {
	...
	private List<string> ErrorText(string mode,ApptReminderType reminderType) {
		...
		if(reminderType==ApptReminderType.ConfirmationFutureDay) {
			if(mode=="texts") {
				listErrors.Add(Lan.g(this,"Automated eConfirmation texts cannot contain ")+ApptThankYouSents.AddToCalendar+Lan.g(this," when not signed up for eConfirmations."));
			}
			else {
				listErrors.Add(Lan.g(this,"Automated eConfirmation emails cannot contain ")+ApptThankYouSents.AddToCalendar+Lan.g(this," when not signed up for eConfirmations."));
			}
		}
		return listErrors;
	}
