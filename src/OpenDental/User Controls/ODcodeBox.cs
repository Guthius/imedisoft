...
namespace OpenDental {
	public class ODcodeBox:RichTextBox {
		...
		public void HighlightLine(int lineNum,int highlightStart,int hightlightEnd,Color color) {
			int idx=0;
			if(lineNum!=0) {
				idx=GetListIndicesOfNewLines()[lineNum-1]+1;//GetPosFromCharIndex(index+1);
			}
			Select(idx+highlightStart,hightlightEnd);
		...
		public void AddEmptyLine(int lineNum) {
			...
			for(int i=0;i<Lines.Length;i++) {
				...
				contentRight+=Lines[i];
				if(i<Lines.Length-1) {
					contentRight+='\n';
				}
			}
			string newLines=contentLeft+'\n'+contentRight;
