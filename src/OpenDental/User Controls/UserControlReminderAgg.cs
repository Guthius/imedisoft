...
public partial class UserControlReminderAgg:UserControl {
	...
	private List<string> ListTagsExludedFromAutoReply {
		get {
			return ["[Appts]", "[ConfirmCode]", "[ConfirmURL]"];
		}
	}
	...
	private void LoadControls() {
		...
		if(Rule.TypeCur==ApptReminderType.PatientPortalInvite) {
			...
			labelEmailAggPerAppt.Text+="  "+Lans.g("Replaces the [Credentials] tag.");
		}
		...
		if(Rule.TypeCur==ApptReminderType.Arrival) {
			for(var i = tabTemplates.TabPages.Count-1;i>=0;i--) {
	...
	private string GetTagsAvailable() {
		var listTagsAvailable=ApptReminderRules.GetAvailableAggTags(Rule.TypeCur);
		if(tabTemplates.SelectedTab==tabAutoReplyTemplate) {
			...
			listTagsAvailable.Add(ApptThankYouSents.AddToCalendar);
	...
	private void butEditEmail_Click(object sender,EventArgs e) {
		using var formEE=new FormEmailEdit();
	...
	public List<string> ValidateTemplates() {
		...
		var allTextTemplates=new List<string> { textEmailSubjAggShared.Text, textEmailAggPerAppt.Text, textSMSAggPerAppt.Text, textSMSAggShared.Text, textAggregateAutoReply.Text, textSingleAutoReply.Text, textArrivalResponse.Text, textComeIn.Text };
		...
		if(Rule.TypeCur==ApptReminderType.ConfirmationFutureDay) {
			...
			var listInvalidTags=ListTagsExludedFromAutoReply.FindAll(x => textSingleAutoReply.Text.ToLower().Contains(x.ToLower()));
	...
	public List<string> AddCalendarTagErrors() {
		...
		var addToCalTag=ApptThankYouSents.AddToCalendar.ToLower();
	...
	private List<string> ErrorText(string mode,ApptReminderType reminderType,bool isAgg) {
		...
		if(isAgg) {
			if(reminderType==ApptReminderType.ConfirmationFutureDay) {
				listErrors.Add(Lan.g(this,"Automated Confirmation Aggregate auto-reply texts cannot contain ")+ApptThankYouSents.AddToCalendar+Lan.g(this,". Use Per Appointment instead."));
			}
			if(reminderType==ApptReminderType.Reminder) {
				if(mode=="texts") {
					listErrors.Add(Lan.g(this,"Automated Reminder Aggregate texts cannot contain ")+ApptThankYouSents.AddToCalendar+Lan.g(this,". Use Per Appointment instead."));
				}
				else {
					listErrors.Add(Lan.g(this,"Automated Reminder Aggregate emails cannot contain ")+ApptThankYouSents.AddToCalendar+Lan.g(this,". Use Per Appointment instead."));
				}
			}
			if(reminderType==ApptReminderType.ScheduleThankYou) {
				if(mode=="texts") {
					listErrors.Add(Lan.g(this,"Automated Thank-You Aggregate texts cannot contain ")+ApptThankYouSents.AddToCalendar+Lan.g(this,". Use Per Appointment instead."));
				}
				else {
					listErrors.Add(Lan.g(this,"Automated Thank-You Aggregate emails cannot contain ")+ApptThankYouSents.AddToCalendar+Lan.g(this,". Use Per Appointment instead."));
				}
			}
		}
		else {
			if(reminderType==ApptReminderType.ConfirmationFutureDay) {
				listErrors.Add(Lan.g(this,"Automated Confirmation auto-reply texts cannot contain ")+ApptThankYouSents.AddToCalendar+Lan.g(this," when not signed up for eConfirmations."));
			}
			if(reminderType==ApptReminderType.Reminder) {
				if(mode=="texts") {
					listErrors.Add(Lan.g(this,"Automated Reminder texts cannot contain ")+ApptThankYouSents.AddToCalendar+Lan.g(this," when not signed up for eConfirmations."));
				}
				else {
					listErrors.Add(Lan.g(this,"Automated Reminder emails cannot contain ")+ApptThankYouSents.AddToCalendar+Lan.g(this," when not signed up for eConfirmations."));
				}
			}
			if(reminderType==ApptReminderType.ScheduleThankYou) {
				if(mode=="texts") {
					listErrors.Add(Lan.g(this,"Automated Thank-You texts cannot contain ")+ApptThankYouSents.AddToCalendar+Lan.g(this," when not signed up for eConfirmations."));
				}
				else {
					listErrors.Add(Lan.g(this,"Automated Thank-You emails cannot contain ")+ApptThankYouSents.AddToCalendar+Lan.g(this," when not signed up for eConfirmations."));
				}
			}
		}
		return listErrors;
