...
public partial class GraphScheduleDay:UserControl {
	...
	private int barHeightPixels96=14;
	...
	private List<List<Schedule>> _schedulesList= [];
	...
	public void SetSchedules(List<Schedule> schedules) {
		...
		for(var schedIndex=0;schedIndex<schedules.Count;schedIndex++) {
			var schedule=schedules[schedIndex];
			...
			switch(schedule.SchedType) {
				...
				case ScheduleType.Practice:
					...
					_schedulesList.Add([..new[] {schedule}]);
					break;
				...
				case ScheduleType.Employee:
					...
					var isFound=false;
					for(var masterIndex=0;masterIndex<_schedulesList.Count;masterIndex++) {
						var existingSchedules=_schedulesList[masterIndex];
						var existingSchedule=existingSchedules[0];
		...
		var sortStartTime=new ScheduleStartComparer();
		for(var i=0;i<_schedulesList.Count;i++) {
	...
	private int GetPanel1GraphHeight() {
		return Math.Max(panel1.Height,_schedulesList.Count*(BarSpacingPixels+barHeightPixels96));
	}
	...
	private float GetFontHeight() {
		...
		while(true) {
			...
			if(size.Height+.2f<barHeightPixels96) { //still too small so increment the font size and try again
				emSize+=.1F;
				continue;
			}
			return emSize;		
	...
	private void panel1_Paint(object sender,PaintEventArgs e) {
		var penLine=new Pen(Color.Black,LineWidthPixels);
		var font=new Font(this.Font.FontFamily,GetFontHeight(),this.Font.Style);
		try {
			//offset the drawing region to current scroll location
			e.Graphics.TranslateTransform(panel1.AutoScrollPosition.X,panel1.AutoScrollPosition.Y);
			//size the drawable region
			var rcBounds=new RectangleF(penLine.Width/2,penLine.Width/2,panel1.Width-penLine.Width,GetPanel1GraphHeight()+penLine.Width);
			//fill the background
			...
			var pixelsPerMinute=rcBounds.Width/(float)TotalTime.TotalMinutes;
			//get the pixel width of each hour
			var hourWidth=rcBounds.Width/(float)TotalTime.TotalHours;
			for(var hour=0;hour<=(int)TotalTime.TotalHours;hour++) {
				var xPos=rcBounds.Left+hour*hourWidth;
				//draw 15 minute dashed lines
				for(var quarter=0;quarter<=3;quarter++) {
					using(var penDash=new Pen(quarter==0?Color.Black:Color.LightGray,LineWidthPixels)) {							
			...
			var yPosStart=rcBounds.Top;
			for(var i=0;i<_schedulesList.Count;i++) {
				yPosStart+=BarSpacingPixels;
				var schedules=_schedulesList[i];
				for(var j=0;j<schedules.Count;j++) {
					var schedule=schedules[j];
					var xPosStart=Math.Max(rcBounds.Left,rcBounds.Left+(float)schedule.StartTime.Subtract(this.StartTime).TotalMinutes*pixelsPerMinute);
					var xWidth=Math.Min(rcBounds.Right,(float)(schedule.StopTime.Subtract(schedule.StartTime).TotalMinutes*pixelsPerMinute));
					var desc="";
					var barColor=EmployeeBarColor;
					var textColor=EmployeeTextColor;
			...
			if(DateTime.Now>=DateTime.Today.AddHours(StartHour) && DateTime.Now<=DateTime.Today.AddHours(EndHour)) {
				using(var penNow=new Pen(Color.FromArgb(128,Color.Red),LineWidthPixels*2)) {
					var minutes = (int)DateTime.Now.TimeOfDay.Subtract(TimeSpan.FromHours(StartHour)).TotalMinutes;
					var xPos=rcBounds.Left+minutes*pixelsPerMinute;
					e.Graphics.DrawLine(penNow,xPos,rcBounds.Bottom,xPos,rcBounds.Top);
				}
			}
			if(panel1.AutoScrollMinSize.Height!=(int)yPosStart) {
				//create vertical scrollbar only
				panel1.AutoScrollMinSize=new Size(0,(int)yPosStart);
	...
	private void panel2_Paint(object sender,PaintEventArgs e) {
		...
		var font=new Font(this.Font,this.Font.Style);
		try {
			//draw the back color as the background
			using(Brush brushBack=new SolidBrush(this.BackColor)) {
				e.Graphics.FillRectangle(brushBack,panel2.ClientRectangle);
			}
			//shrink the drawable region to accomodate half the pen width
			var rcBounds=new RectangleF(penLine.Width/2,penLine.Width/2,panel2.Width-penLine.Width,panel2.Height-penLine.Width);
			if(panel1.VerticalScroll.Visible) {
				...
				e.Graphics.FillRectangle(SystemBrushes.Control,rcBounds.Right,0,SystemInformation.VerticalScrollBarWidth,panel2.Height);				
			}
			//fill the background
			...
			var hourWidth=rcBounds.Width/(float)TotalTime.TotalHours;
			for(var hour=0;hour<=(int)TotalTime.TotalHours;hour++) {
				var dt=DateTime.Today.AddHours(StartHour+hour);
				var stringHour=dt.ToString("htt").ToLower();
				var szHour=e.Graphics.MeasureString(stringHour,font);
				var xPos=rcBounds.Left+hour*hourWidth;
				var yPos=rcBounds.Top;
				//draw the tick
