...
public class ODtextBox:RichTextBox {//System.ComponentModel.Component
	...
	public ODtextBox() {
		...
		var onClick=new EventHandler(menuItem_Click);
			...
			switch(m.Msg) {
				...
				case (int)WM_IME.WM_IME_COMPOSITION:
					if(m.LParam.ToInt32()==(int)WM_IME.GCS_RESULTSTR) {
						var hImm=ImmGetContext(this.Handle);
						var dwSize=ImmGetCompositionString(hImm,(int)WM_IME.GCS_RESULTSTR,null,0);
						var outstr=new byte[dwSize];
						ImmGetCompositionString(hImm,(int)WM_IME.GCS_RESULTSTR,outstr,dwSize);
						_msgText+=Encoding.Unicode.GetString(outstr);
	...
	private void contextMenu_Popup(object sender,System.EventArgs e) {
		...
		var textCur=SelectedText!=""?SelectedText:Text; //Edit Auto Note
	...
	private bool IsOnMisspelled(Point PositionOfClick) {
		var charIndex=this.GetCharIndexFromPosition(PositionOfClick);
		var charLocation=this.GetPositionFromCharIndex(charIndex);
		...
		var c=this.GetCharFromPosition(PositionOfClick);
	...
	private void menuItem_Click(object sender,System.EventArgs e) {
		...
		switch(contextMenu.MenuItems.IndexOf((MenuItem)sender)) {
			...
			case 4:
				...
				var originalCaret=this.SelectionStart;
				if(MatchReplWord==null) {
	...
	private void ODtextBox_ContentsResized(object sender,ContentsResizedEventArgs e) {
		...
		var pointTextEnd=this.GetPositionFromCharIndex(Text.Length-1);
	...
	protected override void OnKeyUp(KeyEventArgs e) {
		...
		var originalCaret=base.SelectionStart;
		if(HasInlineAutoNotesEnabled) {
			var newText=QuickPasteNotes.Substitute(Text,_quickPasteType);
			if(Text!=newText) {
				Text=newText;
	...
	public void ClearWavyLines(int widthOverride=-1,SpellCheckResult spellCheckResult=null) {
		...
		var bitmapOverlay=new Bitmap(this.Width,this.Height);
		...
		foreach(var lineHeight in listVisibleLineHeights) {
			start.Y+=lineHeight;
	...
	public SpellCheckResult SpellCheck() {
		...
		var listMisspelledWords=new List<MatchOD>();
		foreach(var wordCur in listVisibleWords) {
		...
		foreach(var word in listMisspelledWords) {
		...
		using var graphicsTextBox=Graphics.FromHwnd(this.Handle);
		graphicsTextBox.DrawImage(bitmapOverlay,0,0,Width,Height);
		return spellCheckResult;
	}
	...
	private List<MatchOD> GetVisibleWords() {
		var charBounds=GetVisibleCharIndices();
		var startCharIndex=charBounds.StartCharIndex;
		...
		var endCharIndex=charBounds.EndCharIndex+1;
		...
		var visibleText=Text.Substring(startCharIndex,endCharIndex-startCharIndex);
	...
	private List<MatchOD> GetWords(string input,int offset=0) {
		var listMatches=new List<MatchOD>();
		var matchCollection=Regex.Matches(input,@"(\S+)");//use Regex.Matches because our matches include the index within our text for underlining
		foreach(Match m in matchCollection) {
			var g=m.Groups[0];//Group 0 is the entire match
			if(g.Value.Length<2) {//only allow 'words' that are at least two chars long, 1 char 'words' are assumed spelled correctly
				continue;
			}
			var word=new MatchOD();
	...
	public CharBounds GetVisibleCharIndices() {
		var startCharIndex=this.GetCharIndexFromPosition(new Point(0,0));
		...
		var endCharIndex=this.GetCharIndexFromPosition(new Point(this.ClientSize));
	...
	private List<int> GetVisibleLineHeights(int widthOverride=-1) {
		...
		var startLineIndex=richTextBox.GetLineFromCharIndex(charBounds.StartCharIndex);
		...
		var endCharLineIndex=richTextBox.GetLineFromCharIndex(charBounds.EndCharIndex) + 1;
		var listLines=new List<int>();
		//Go through each line and grab y-position of top of line.
		for(var i=startLineIndex;i<=endCharLineIndex;i++) {
			var charIdx=richTextBox.GetFirstCharIndexFromLine(i);
			var posThisLine=richTextBox.GetPositionFromCharIndex(charIdx);
			listLines.Add(posThisLine.Y);
		}
		var listLineHeights=new List<int>();
		for(var i=0;i<listLines.Count-1;i++) {
	...
	private void DrawWave(MatchOD match,List<int> listVisibleLineHeights,List<WavyLine> listWavyLines=null) {
		...
		var lineFirstVisibleChar=this.GetLineFromCharIndex(this.GetCharIndexFromPosition(new Point(0,0)));
		var lineMatchCur=this.GetLineFromCharIndex(match.StartCharIndex);
		...
		var startLineHeight=listVisibleLineHeights[startLineIndex];
		var pointStart=this.GetPositionFromCharIndex(match.StartCharIndex);//accounts for scroll position
		var pointEnd=this.GetPositionFromCharIndex(match.StartCharIndex+match.Value.Length);//accounts for scroll position
		...
		if(pointEnd.Y>pointStart.Y) {//I don't know how it's possible for a misspelled word to span multiple lines because we keepwords together, based on spaces.  But whatever.
			...
			while(pointEndTemp.Y<=pointEnd.Y && lineIndex<listVisibleLineHeights.Count) {//One line at a time.
				...
				if(countWaves>1) {//otherwise draw straight line further down
					...
					for(var i=0;i<=countWaves;i++) {//each iteration through loop is one V shaped wave
			...
			float lengthWave=4;//four pixels wide at 96dpi
			var countWaves=(int)((pointEnd.X-pointStart.X)/lengthWave);//round down the count
			if(countWaves>1) {//otherwise draw straight line further down
				var listPointFs=new List<PointF>();
				for(var i=0;i<=countWaves;i++) {//each iteration through loop is one V shaped wave
					listPointFs.Add(new PointF(pointStart.X+i*lengthWave,pointStart.Y));
