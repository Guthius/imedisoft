...
public partial class UserControlSecurityTree:UserControl {
	...
	private void FillNodes(TreeNode node,List<long> listUserGroupNums) {
		...
		for(var i = 0;i<listGroupPerms.Count;i++) {
			   ...
			   && listGroupPerms[i].PermType==(EnumPermType)node.Tag) 
			{
				...
				if(listGroupPerms[i].NewerDate.Year>1880) {
					...
					node.Text+=" ("+Lan.g(this,"if days newer than")+" "+listGroupPerms[i].NewerDays+")";
				}
				if((EnumPermType)node.Tag==EnumPermType.AdjustmentTypeDeny) {
					var listGroupPermAdjustmentTypesDenied=GroupPermissions.GetAdjustmentTypeDenyPermsForUserGroup(listGroupPerms[i].UserGroupNum);
	...
	private void treePermissions_MouseDown(object sender,System.Windows.Forms.MouseEventArgs e) {
		...
		var permEcEo=EnumPermType.ProcExistingEdit;
			...
			if(perm.PermType==permEcEo) {
				...
				foreach(var permission in listLimitedPermissions) {
					...
					try {
						...
						SecurityLogs.MakeLogEntry(EnumPermType.SecurityAdmin,0,Lans.g("Permission ")+"'"+permLimited.PermType+"' "
						                                                       +Lans.g("granted to")+" '"+UserGroups.GetGroup(perm.UserGroupNum).Description+"'");
					}
					catch(Exception ex) {
						ODMessageBox.Show(ex.Message);
						return;
				...
				try {
					...
					SecurityLogs.MakeLogEntry(EnumPermType.SecurityAdmin,0,Lans.g("Permission ")+"'"+permFeeSchedLimited.PermType+"' "
					                                                       +Lans.g("granted to")+" '"+UserGroups.GetGroup(permFeeSchedLimited.UserGroupNum).Description+"'");
				}
				catch(Exception ex) {
					ODMessageBox.Show(ex.Message);
					return;
			...
			if(listLimitedPermissions.Contains(((EnumPermType)_clickedPermNode.Tag))) {
				...
				if(GroupPermissions.HasPermission(_listUserGroupNums.First(),permEcEo,0)) {
					try {
						...
						SecurityLogs.MakeLogEntry(EnumPermType.SecurityAdmin,0,Lans.g("Permission")+" '"+permEcEo+"' "
						                                                       +Lans.g("revoked from")+" '"+UserGroups.GetGroup(_listUserGroupNums.First()).Description+"'");
					}
					catch(Exception ex) {
						ODMessageBox.Show(ex.Message);
						return;
