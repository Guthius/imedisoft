...
public partial class UserControlSecurityUserGroup:UserControl {
	...
	private List<Userod> GetFilteredUsersHelper() {
		...
		if(comboClinic.SelectedIndex>0) {
			listUserOds.RemoveAll(x => x.ClinicNum!=comboClinic.GetSelected<ClinicDto>().Id);
		}
	...
	public void FillGridUsers() {
		_isFillingList=true;
		var selectedUser=SelectedUser;//preserve user selection.
		gridUsers.BeginUpdate();
		gridUsers.Columns.Clear();
		var tableName="TableSecurity";
		...
		if(true) {
			gridUsers.Columns.Add(new GridColumn(Lan.g(tableName,"Clinic"),80));
			gridUsers.Columns.Add(new GridColumn(Lan.g(tableName,"Clinic\r\nRestr"),38,HorizontalAlignment.Center));
		}
		gridUsers.Columns.Add(new GridColumn(Lan.g(tableName,"Strong\r\nPwd"),45,HorizontalAlignment.Center));
		gridUsers.ListGridRows.Clear();
		var listFilteredUsers=GetFilteredUsersHelper();
		foreach(var user in listFilteredUsers) {
			var row=new GridRow();
			row.Cells.Add(user.UserName);
			row.Cells.Add(Employees.GetName(user.EmployeeNum));
	...
	public void FillListUserGroupTabUserGroups() {
		_isFillingList=true;
		var selectedGroup = SelectedUserGroup; //Preserve Usergroup selection.
	...
	private void FillAssociatedUsers() {
		...
		var listUsers = Userods.GetForGroup(SelectedUserGroup.UserGroupNum);
		foreach(var userCur in listUsers) {
