...
public partial class SmsThreadView:UserControl {
	...
	private void FillMessageThread() {
		var listControls=panelScroll.Controls.Cast<Control>().ToList();
		listControls.ForEach(x => x.Parent=null);
		var location=panelScroll.Location;
		...
		var horizontalMargin=(int)(bodyWidth*0.02);
		var y=0;
		...
		for(var i=listControls.Count-1;i>=0;i--) {
			var control=listControls[i];//Freeze the variable so we can remove it before disposing it below.
			if(!control.Name.Contains("_")) {
				continue;
			}
			var id=control.Name.Substring(control.Name.IndexOf("_")+1);
		...
		for(var i=0;i<_listSmsThreadToDisplay.Count;i++) {
			var smsThreadMessage=_listSmsThreadToDisplay[i];
			y+=verticalPadding;
			var labelMessageHeader=new Label();
			...
			labelMessageHeader.Text=((smsThreadMessage.UserName==null)?"":(smsThreadMessage.UserName+"  "))+smsThreadMessage.MsgDateTime;
			var size=TextRenderer.MeasureText(labelMessageHeader.Text,panelScroll.Font,
				new Size(bodyWidth,int.MaxValue),TextFormatFlags.WordBreak);
			...
			y+=size.Height+1;
			var labelMessageStatus=new Label();
			...
			var richTextBoxMessage=new RichTextBox();
			...
			var message=smsThreadMessage.Message.Trim();//Trim leading and trailing whitespace for 2 reasons: 1) Prevents scrolling within the textbox when trying to highlight and copy the text 2) Matches the web view behavior.
		...
		var labelBottomSpacer=new Label();
	...
	private Size GetSizeForText(string text) {
		...
		using var g=panelScroll.CreateGraphics();
	...
	public bool UpdateRichTextBoxText(int index,string msg) {
		var richTextBoxes=panelScroll.Controls.Cast<Control>().OfType<RichTextBox>().ToArray();
		if(!index.Between(0,richTextBoxes.Length-1)) {
			return false;
		}
		var richTextBox=richTextBoxes[index];
		var size=GetSizeForText(richTextBox.Text);
	...
	public static string ProcessAiMessage(string text) {
		...
		foreach(var line in text.Split("\n", StringSplitOptions.None)) {
			...
			var processedLine=ReplaceOpenAiLinks(line);//Convert OpenAI links to hyperlinks
	...
	private static string ReplaceOpenAiLinks(string line) {
		...
		if(match.Success && match.Groups.Count>=2) {
			var linkText=match.Groups[1].Value;//Extract the display text (between [ and ])
			var url=match.Groups[2].Value;//Extract the URL (between ( and ))
	...
	private static void ProcessOpenAiBoldWords(StringBuilder sb,string line) {
		...
		while((boldStart=processedLine.IndexOf("**")) != -1) {
			var boldEnd=processedLine.IndexOf("**",boldStart+2);//+2 to move past ** chars
			if(boldEnd==-1) {
	...
	private Control AddOrUpdatePanelScrollChildControl(Control control) {
		...
		if(existingControl is RichTextBox existingRtb && control is RichTextBox rtb && !rtb.Rtf.IsNullOrEmpty()) {
			existingRtb.Rtf=rtb.Rtf;
		}
		else {
		...
		control.Dispose();//This control does not have any child controls.
		existingControl.Bounds=existingControl.Bounds;
		return existingControl;
	}

	private void TextBoxMessage_LinkClicked(object sender,LinkClickedEventArgs e) {	
		try {
			var links=e.LinkText.Split([" ","\t"],StringSplitOptions.RemoveEmptyEntries);
			var link=e.LinkText;
			if(links.Length>0) {
				link=links[0];
			}
			...
			ODMessageBox.Show(Lans.g("Failed to open web browser.  Please make sure you have a default browser set and are connected to the internet then try again."),Lans.g("Attention"));
		}
	...
	private void BuildListMessages() {
		_listSmsThreadToDisplay= [];	//We'll hold what messages are to be shown in this list.
		...
		var maxPage=(int)Math.Ceiling((double)_listSmsThreadMessages.Count/CountMessagesToDisplay);	//# messages per page, #/count-1=page index
		if(_smsThreadPage > maxPage) {	
			_smsThreadPage=maxPage;
		}
		labelCurrentPage.Text=(_smsThreadPage) +" "+Lan.g(this,"of")+" "+ (maxPage);
		//Here we fill the reference list that is displayed depending on which page we're on.
		var firstMessageIdx=CountMessagesToDisplay*(_smsThreadPage-1);
		var lastMessageIdx=Math.Min(CountMessagesToDisplay*_smsThreadPage,_listSmsThreadMessages.Count)-1;
		for(var i=firstMessageIdx;i<=lastMessageIdx;i++) {	
		...
		if(_listSmsThreadMessages.Count<=CountMessagesToDisplay) {
			...
			panelScroll.Location=new Point(0,panelNavigation.Location.Y+panelNavigation.Height);//Just below panelNavigation
			panelScroll.Height=Height-panelScroll.Location.Y;
		}			
		if(_smsThreadPage==maxPage) {
	...
	private void TextBoxMessage_MouseDown(object sender,MouseEventArgs e) {
		...
		if(e.Button==System.Windows.Forms.MouseButtons.Right) {
			...
			menuItem.Click+=(_,_) => ODClipboard.SetClipboard(text);
	...
	private void butEnd_Click(object sender,EventArgs e) {
		_smsThreadPage=int.MaxValue;	//This is reset back to the maximum page, so we arrive directly at the end.
		FillMessageThread();
	}
	...
	private void MouseWheel_Scroll(object sender,MouseEventArgs e) {
		...
		var val=panelScroll.VerticalScroll.Value-e.Delta;
