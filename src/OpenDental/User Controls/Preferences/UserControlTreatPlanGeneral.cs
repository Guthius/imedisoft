...
public partial class UserControlTreatPlanGeneral:UserControl {
	...
	private bool UpdateProcCodesPref(PrefName prefName,string textProcCodes,string labelText,ref string errorMessage) {
		var listProcCodesInvalid=new List<string>();
		var listProcCodes=textProcCodes
	...
	public void FillTreatPlanGeneral() {
		...
		groupTreatPlanSort.Enabled = true;
	...
	public bool SaveTreatPlanGeneral() {
		...
		if(PrefC.GetString(PrefName.TreatmentPlanNote)!=textTreatNote.Text) {
			var listTreatPlanNums=TreatPlans.GetNumsByNote(PrefC.GetString(PrefName.TreatmentPlanNote));//Find active/inactive TP's that match exactly.
			if(listTreatPlanNums.Count>0) {
				var dr=ODMessageBox.Show(Lan.g(this,"Unsaved treatment plans found with default notes")+": "+listTreatPlanNums.Count+"\r\n"
		...
		Changed|=Prefs.UpdateBool(PrefName.TreatPlanSortByTooth,radioTreatPlanSortTooth.Checked);
