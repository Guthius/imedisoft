...
public partial class UserControlImagingGeneral:UserControl {
	...
	private void butBrowseImageCategoryDefault_Click(object sender,EventArgs e) {
		var listDefs=Defs.GetDefs(DefCat.ImageCats, [_imageCategoryDefault]);
		using var formDefinitionPicker=new FormDefinitionPicker(DefCat.ImageCats,listDefs);
	...
	private void butVideoImageCategoryDefault_Click(object sender,EventArgs e) {
		var listDefs=Defs.GetDefs(DefCat.ImageCats, [_videoImageCategoryDefault]);
		using var formDefinitionPicker=new FormDefinitionPicker(DefCat.ImageCats,listDefs);
		formDefinitionPicker.IsMultiSelectionMode=false;
		if(formDefinitionPicker.ShowDialog()==DialogResult.Cancel){
			return;
	...
	private void checkImagesModuleUsesOld2020_Click(object sender,EventArgs e) {
		var isImageModuleSettingChanged=(PrefC.GetBool(PrefName.ImagesModuleUsesOld2020)!=checkImagesModuleUsesOld2020.Checked);
	...
	public void FillImagingGeneral() {
		...
		checkImagingOrderDescending.Checked=PrefC.GetBool(PrefName.ImagingOrderDescending);
	}
	...
	public bool SaveImagingGeneral() {
		...
		var scaleValue=MountDefs.SetScale((float)textScale.Value,textDecimals.Value,textUnits.Text);
		Changed|=Prefs.UpdateString(PrefName.ImagingDefaultScaleValue,scaleValue);
		Changed|=Prefs.UpdateBool(PrefName.ImagingOrderDescending,checkImagingOrderDescending.Checked);
