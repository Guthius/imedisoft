...
public partial class UserControlServerConnections:UserControl {
	...
	private void SetReadOnlyServerUIEnabled() {
		if(!checkUseReadOnlyServer.Checked) {
			...
			//groupMiddleTier.Enabled=false;//Can't do this because we want hover info to still work
			labelURI.ForeColor=ColorOD.DisabledText;//just looks disabled, but it's really not
			textMiddleTierURI.Enabled=false;
		}
		else {
			...
			if(radioReadOnlyServerDirect.Checked) {
				...
				labelURI.ForeColor=ColorOD.DisabledText;
				textMiddleTierURI.Enabled=false;
			}
			else {
				groupConnectionSettings.Enabled=false;
				groupMiddleTier.Enabled=true;
				labelURI.ForeColor=Color.Black;
				textMiddleTierURI.Enabled=true;
			}
		}
	}
