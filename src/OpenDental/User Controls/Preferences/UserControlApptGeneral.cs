...
public partial class UserControlApptGeneral:UserControl {
	...
	private List<BrokenApptProcedure> _listBrokenApptProcedures= [];
	...
	private void checkApptsRequireProc_Click(object sender,EventArgs e) {
		var prefValSync=ListPrefValSyncs.Find(x=>x.PrefName_==PrefName.ApptsRequireProc);	
	...
	public void FillApptGeneral() {
		...
		foreach(BrokenApptProcedure option in Enum.GetValues(typeof(BrokenApptProcedure))) {
			...
			var index=comboBrokenApptProc.Items.Add(Lans.g(option.ToString()));
		...
		var EnumApptProvPromptArray=(EnumApptProvPrompt[])Enum.GetValues(typeof(EnumApptProvPrompt));
		for(var i=0;i<EnumApptProvPromptArray.Length;i++) {
			comboApptProvChangePrompt.Items.Add(Lan.g(this,EnumApptProvPromptArray[i].GetDescription()));
		}
		comboApptProvChangePrompt.SelectedIndex=PrefC.GetInt(PrefName.ApptModuleProviderPrompt);
	...
	public bool SaveApptGeneral() {
		...
		Changed|=Prefs.UpdateInt(PrefName.ApptModuleProviderPrompt,comboApptProvChangePrompt.SelectedIndex);
