...
public partial class UserControlEnterpriseGeneral:UserControl {
	...
	private void butChange_Click(object sender,EventArgs e) {
		...
		using var formSecurityLock=new FormSecurityLock();
	...
	private void checkPatientPhoneUsePhoneNumberTable_Click(object sender,EventArgs e) {
		if(!_doUsePhonenumTable && checkPatientPhoneUsePhonenumberTable.Checked) {
			var msgText="When enabling the use of the phonenumber table a one-time sync of patient phone numbers needs to take place.  This could "+
		...
		var prefValSync=ListPrefValSyncs.Find(x=>x.PrefName_==PrefName.PatientPhoneUsePhonenumberTable);
	...
	private void checkPatientSelectFilterRestrictedClinics_Click(object sender,EventArgs e) {
		var prefValSync=ListPrefValSyncs.Find(x=>x.PrefName_==PrefName.PatientSelectFilterRestrictedClinics);
	...
	private void checkEnterpriseAllowRefreshWhileTyping_Click(object sender,EventArgs e) {
		var prefValSync=ListPrefValSyncs.Find(x=>x.PrefName_==PrefName.EnterpriseAllowRefreshWhileTyping);
	...
	private bool ValidateEntries() {
		...
		if(textSecurityLogOffAfterMinutes.Text!="") {
			try {
				var logOffMinutes = int.Parse(textSecurityLogOffAfterMinutes.Text);
	...
	private void FillHiddenPrefs() {
		...
		checkHasClinicsEnabled.Checked=true;
		var updateStreamline=GetHiddenPrefString(PrefName.UpdateStreamLinePassword);
		...
		var updateAlterLargeTablesDirectly=GetHiddenPrefString(PrefName.UpdateAlterLargeTablesDirectly);
	...
	public void FillEnterpriseGeneral() {
		...
		var patientSelectSearchMinChars=PrefC.GetString(PrefName.PatientSelectSearchMinChars);
		...
		var patientSelectSearchPauseMs=PrefC.GetString(PrefName.PatientSelectSearchPauseMs);
		...
		var ynPatientSelectSearchWithEmptyParams=SIn.Enum<YN>(PrefC.GetInt(PrefName.PatientSelectSearchWithEmptyParams));
