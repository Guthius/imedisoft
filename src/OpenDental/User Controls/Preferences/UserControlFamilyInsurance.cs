...
public partial class UserControlFamilyInsurance:UserControl {
	...
	private void checkInsDefaultAssignmentOfBenefits_Click(object sender,EventArgs e) {
		...
		var promptMsg=Lan.g(this,"Would you like to immediately change all plans to use assignment of benefits?\r\n"
		...
		var subsAffected=InsSubs.SetAllSubsAssignBen(checkInsDefaultAssignmentOfBenefits.Checked);
	...
	private void checkInsDefaultShowUCRonClaims_Click(object sender,EventArgs e) {
		...
		var plansAffected=InsPlans.SetAllPlansToShowUcr();
		ODMessageBox.Show(Lan.g(this,"Plans affected: ")+plansAffected);
	}
	...
	public void FillFamilyInsurance() {
		...
		checkOutOfNetworkBlankLikeZero.Checked=PrefC.GetBool(PrefName.InsOutOfNetworkBlankLikeZero);
		...
		var listCobs=Enum.GetValues(typeof(EclaimCobInsPaidBehavior)).Cast<EclaimCobInsPaidBehavior>().ToList();
	...
	public bool SaveFamilyInsurance() {
		...
		Changed|=Prefs.UpdateBool(PrefName.InsOutOfNetworkBlankLikeZero,checkOutOfNetworkBlankLikeZero.Checked);
