...
public partial class UserControlEnterpriseReports:UserControl {
	...
	private void textReportingServerCompName_Validating(object sender,CancelEventArgs e) {
		var prefValSync=ListPrefValSyncs.Find(x=>x.PrefName_==PrefName.ReportingServerCompName);
	...
	private void textReportingServerURI_Validating(object sender,CancelEventArgs e) {
		var prefValSync=ListPrefValSyncs.Find(x=>x.PrefName_==PrefName.ReportingServerURI);
	...
	private string[] GetDatabases() {
		...
		if(textReportingServerMySqlUser.Text=="") {
			dataConnection=new DataConnection(textReportingServerCompName.Text,"mysql","root",textMysqlPass.Text);
		}
		else {
			dataConnection=new DataConnection(textReportingServerCompName.Text,"mysql",textReportingServerMySqlUser.Text,textMysqlPass.Text);
		}
		...
		var stringArrayNames=new string[table.Rows.Count];
