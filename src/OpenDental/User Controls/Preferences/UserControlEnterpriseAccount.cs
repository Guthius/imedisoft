...
public partial class UserControlEnterpriseAccount:UserControl {
	...
	private void butReplacements_Click(object sender,EventArgs e) {
		...
		var frmMessageReplcements=new FrmMessageReplacements(listMessageReplaceTypes);
		...
		var cursorIndex=textClaimIdPrefix.SelectionStart;
	...
	private void comboPaymentClinicSetting_ChangeCommitted(object sender,EventArgs e) {
		var prefValSync=ListPrefValSyncs.Find(x=>x.PrefName_==PrefName.PaymentClinicSetting);
	...
	private void checkPaymentsPromptForPayType_Click(object sender,EventArgs e) {
		var prefValSync=ListPrefValSyncs.Find(x=>x.PrefName_==PrefName.PaymentsPromptForPayType);
	...
	private void textClaimIdPrefix_Validating(object sender,CancelEventArgs e) {
		var prefValSync=ListPrefValSyncs.Find(x=>x.PrefName_==PrefName.ClaimIdPrefix);
		prefValSync.PrefVal=textClaimIdPrefix.Text;
		SyncChanged?.Invoke(this,new EventArgs());
	}

	private void comboPayPlansVersion_ChangeCommitted(object sender,EventArgs e) {
		var prefValSync=ListPrefValSyncs.Find(x=>x.PrefName_==PrefName.PayPlansVersion);
	...
	private void textBillingElectBatchMax_Validating(object sender,CancelEventArgs e) {
		var prefValSync=ListPrefValSyncs.Find(x=>x.PrefName_==PrefName.BillingElectBatchMax);
	...
	public void FillEnterpriseAccount() {
		...
		if(_claimReportReceiveInterval==0) {
			...
			var dateClaimReportReceiveTime=PrefC.GetDateT(PrefName.ClaimReportReceiveTime);
		...
		var listRigorousAccountings=Enum.GetValues(typeof(RigorousAccounting)).OfType<RigorousAccounting>().ToList();
		for(var i=0;i<listRigorousAccountings.Count;i++) {
		...
		var listRigorousAdjustments=Enum.GetValues(typeof(RigorousAdjustments)).OfType<RigorousAdjustments>().ToList();
		for(var i=0;i<listRigorousAdjustments.Count;i++) {
	...
	public void FillSynced(){
		var prefValSync=ListPrefValSyncs.Find(x=>x.PrefName_==PrefName.AgingServiceTimeDue);
