...
public partial class UserControlOrtho:UserControl {
	...
	private List<long> _listOrthoPlacementCodeNums= [];
	...
	private void butOrthoHardwareSpecs_Click(object sender,EventArgs e) {
		using var formOrthoHardwareSpecs=new FormOrthoHardwareSpecs();
		formOrthoHardwareSpecs.ShowDialog();
	}

	private void butOrthoPrescriptions_Click(object sender, EventArgs e){
		using var formOrthoRxSetup=new FormOrthoRxSetup();
	...
	private void butDelete_Click(object sender,EventArgs e) {
		...
		var listProcedureCodes=listboxOrthoPlacementProcs.GetListSelected<ProcedureCode>();
	...
	private void RefreshListBoxProcs() {
		...
		for(var i=0;i<_listOrthoPlacementCodeNums.Count;i++) {
			var procedureCode=ProcedureCodes.GetProcCode(_listOrthoPlacementCodeNums[i]);
	...
	public void FillOrtho() {
		...
		if(strListOrthoNums!="") {
			_listOrthoPlacementCodeNums.AddRange(strListOrthoNums.Split(',').ToList().Select(x => SIn.Long(x)));
		}
		RefreshListBoxProcs();
