...
public partial class FormClockEventEdit : FormODBase {
	...
	private List<TimeClockStatus> _listTimeClockStatusesShown= [];
	...
	private void FillInitialControlsHelper() {
		...
		for(var i=0;i<Enum.GetValues(typeof(TimeClockStatus)).Length;i++) {
			var statusDescript=((TimeClockStatus)i).GetDescription();
		...
		//Time Spans -----------------------------------------------------------------------------
		//Clocked time------------------------------
		var timeSpanClocked=TimeSpan.Zero;
		if(_clockEvent.TimeDisplayed2.Year>1880) {
		...
		if(timeSpanClocked>TimeSpan.Zero) {
			var timeSpanRegular=timeSpanClocked
		...
		//Rate 2 spans -----------------------------------------------------------------------------
		if(timeSpanClocked>TimeSpan.Zero) {
			var timeSpanTotal=timeSpanClocked+(_clockEvent.AdjustIsOverridden?_clockEvent.Adjust:_clockEvent.AdjustAuto);//clockedTime+(Adj or AdjAuto)
			var timeSpanRate1Hours=timeSpanTotal-(_clockEvent.Rate2Hours==TimeSpan.FromHours(-1)?_clockEvent.Rate2Auto:_clockEvent.Rate2Hours);//totalTime-(Rate2 or Rate2Auto)
	...
	private void FillAutoControlsHelper() {
		...
		if(timeSpanClocked>TimeSpan.Zero) {
			var timeSpanTotal=timeSpanClocked+(_clockEvent.AdjustIsOverridden?_clockEvent.Adjust:_clockEvent.AdjustAuto);//clockedTime+(Adj or AdjAuto)
			var timeSpanRate2=_clockEvent.Rate2Hours==TimeSpan.FromHours(-1)?_clockEvent.Rate2Auto:_clockEvent.Rate2Hours;//Rate2 or Rate2Auto
			var timeSpanRate3=_clockEvent.Rate3Hours==TimeSpan.FromHours(-1)?_clockEvent.Rate3Auto:_clockEvent.Rate3Hours;//Rate3 or Rate3Auto
			var timeSpanRate1Hours=timeSpanTotal-timeSpanRate2-timeSpanRate3;//totalTime-(Rate2 or Rate2Auto)-(Rate3 or Rate3Auto)
			textTotalHours.Text=ClockEvents.Format(timeSpanTotal);
			textRate1Auto.Text=ClockEvents.Format(timeSpanRate1Hours);
		}
		//Rate 2 Time -----------------------------
	...
	private void butDelete_Click(object sender, System.EventArgs e) {
		...
		SecurityLogs.MakeLogEntry(EnumPermType.TimecardDeleteEntry,0,
			"Original entry: "+_clockEvent.TimeEntered1);
	...
	private void butSave_Click(object sender, System.EventArgs e) {
		...
		//  return;
		//}
		var timeDisplayed1=DateTime.MinValue;
		try{
		...
		var timeDisplayed2=DateTime.MinValue;
		...
		if(PayPeriods.CannotEditPayPeriodOfDate(timeDisplayed1,_clockEvent.EmployeeNum)) {
			var message=Lan.g(this,"You only have permission to edit your time card for the current pay period. The ");
			var messageTwo=Lan.g(this," you have entered does not fall within the current pay period.");
			ODMessageBox.Show(message+groupBox1.Text+messageTwo);
			return;
		}
		var timeSpanOvertime=TimeSpan.Zero;
		var timeSpanAdjust=TimeSpan.Zero;
		if(_clockEvent.ClockStatus!=TimeClockStatus.Break) {
			if(textOTimeHours.Text!="") {
				try {
