...
public partial class FormCarrierEdit : FormODBase {
	...
	private void FormCarrierEdit_Load(object sender,System.EventArgs e) {
		...
		var listDefsCarrierGroupNames=Defs.GetDefsForCategory(DefCat.CarrierGroupNames,true);//Only Add non hidden definitions
		...
		var strOrthoConsolidatePref=$" ({(PrefC.GetBool(PrefName.OrthoInsPayConsolidated) ? "On" : "Off")})";
		...
		var listDependentPlans=Carriers.DependentPlans(CarrierCur);
		textPlans.Text=listDependentPlans.Count.ToString();
		comboPlans.Items.Clear();
		for(var i=0;i<listDependentPlans.Count;i++){
		...
		if(CultureInfo.CurrentCulture.Name.EndsWith("CA")) {//Canadian. en-CA or fr-CA
			...
			var newHeight=(this.Height-groupCDAnet.Height-checkIsCDAnet.Height);//Dynamically hide the CDAnet groupbox and Is CDAnet Carrier checkbox.
			this.Size=new Size(560,newHeight);
		}
		...
		for(var i=0;i<listCanadianNetworks.Count;i++) {
	...
	private void textState_TextChanged(object sender, System.EventArgs e) {
		var cursor=textState.SelectionStart;
	...
	private void butSave_Click(object sender, System.EventArgs e) {
		...
		var carrierOld=CarrierCur.Copy();
		...
		var logMessage=GetSecurityLogMessage(carrierOld);
