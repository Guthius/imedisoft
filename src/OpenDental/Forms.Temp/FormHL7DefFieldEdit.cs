...
public partial class FormHL7DefFieldEdit:FormODBase {
	...
	private List<FieldNameAndType> _listFieldNameAndTypes;
	...
	private void FormHL7DefFieldEdit_Load(object sender,EventArgs e) {
		_listFieldNameAndTypes= [];
		FillFieldNameList();
		for(var i=0;i<_listFieldNameAndTypes.Count;i++) {
			listFieldNames.Items.Add(_listFieldNameAndTypes[i].Name);
			if(HL7DefFieldCur!=null && HL7DefFieldCur.FieldName==_listFieldNameAndTypes[i].Name) {
		...
		for(var i=0;i<Enum.GetNames(typeof(DataTypeHL7)).Length;i++) {
			comboDataType.Items.Add(Lan.g("enumDataTypeHL7",Enum.GetName(typeof(DataTypeHL7),i)));
		}
	...
	private void FillFieldNameList() {
		_listFieldNameAndTypes=FieldNameAndType.GetFullList();
	}
	...
	private void listFieldNames_Click(object sender,EventArgs e) {
		textFixedText.Clear();
		comboDataType.SelectedIndex=(int)_listFieldNameAndTypes[listFieldNames.SelectedIndex].DataType;
		comboDataType.Enabled=false;
	}
