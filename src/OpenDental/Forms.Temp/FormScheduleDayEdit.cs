...
public partial class FormScheduleDayEdit:FormODBase {
	...
	private void FormScheduleDay_Load(object sender,System.EventArgs e) {
		...
		butViewGraph.Visible=false;
		butOkSchedule.Visible=ShowOkSchedule;
		_listClinics=
		[
			new ClinicDto {Abbr = ""} //so HQ always comes before other clinics in the sort order; only for notes and holidays
		];
		var listClinicsUser=Clinics.GetForUserod(Security.CurUser);
		for(var i=0;i<listClinicsUser.Count;i++){
	...
	private void comboClinicChanged(){
		var clinicNumOrig=comboClinic.ClinicNumSelected;
	...
	private void FillProvsAndEmps() {
		...
		//Emp Listbox
		var listEmployeeNumsPreviouslySelected=listEmp.GetListSelected<Employee>().Select(x => x.EmployeeNum).ToList();
		listEmp.Items.Clear();
	...
	private void FillGrid() {
		...
		var col=new GridColumn(Lan.g("TableSchedDay","Provider"),100);
		...
		string note;
		string opDesc;
		for (var i=0;i<_listSchedules.Count;i++) {
			row=new GridRow();
			//Prov
			...
			//ops
			opDesc="";
			for (var j=0;j<_listSchedules[i].Ops.Count;j++) {
				var operatory=Operatories.GetOperatory(_listSchedules[i].Ops[j]);
			...
			var searchTextLower=textSearch.Text.ToLower().Trim();
	...
	private int CompareSchedule(Schedule scheduleX,Schedule scheduleY){
		...
		if(scheduleX.ClinicNum!=scheduleY.ClinicNum
		   && _listClinics.Exists(x=>x.Id == scheduleX.ClinicNum)
		   && _listClinics.Exists(x=>x.Id == scheduleY.ClinicNum))
		{
			return _listClinics.Find(x=>x.Id == scheduleX.ClinicNum).Abbr.CompareTo(_listClinics.Find(x=>x.Id == scheduleY.ClinicNum).Abbr);
		}
	...
	private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		var selectedSchedule=(Schedule)gridMain.ListGridRows[e.Row].Tag;
		if(selectedSchedule==null) {
			return;//nothing to do
		}
		using var formScheduleEdit=new FormScheduleEdit();
	...
	private void butAddTime_Click(object sender, System.EventArgs e) {
		...
		_listProvNumsSelected= [];//list of provNums selected
		...
		var listProviderNums=new List<long>();
		...
		var listEmployeeNums=new List<long>();
		...
		var listEmployeeNames=new List<string>();
		for(var i=0;i<listEmployeeNums.Count;i++) {
			var employee=Employees.GetFirstOrDefault(x => x.EmployeeNum==listEmployeeNums[i]);
			if(employee==null) {
				continue;
			}
	...
	private void butProvNote_Click(object sender,EventArgs e) {
		...
		for (var i=0;i<listEmpIndices.Count;i++) {
			scheduleTemp=new Schedule();
			scheduleTemp=schedule.Copy();
			scheduleTemp.SchedType=ScheduleType.Employee;
	...
	private void butDelete_Click(object sender,EventArgs e) {
		...
		var listSchedulesToRemove=gridMain.SelectedTags<Schedule>();
		...
		if(countProviders>0) {
			var message=Lan.g(this,"Delete schedules on this day for")+" "+countProviders+" "+Lan.g(this,"provider(s)?");
		...
		var listEmployeeNames=new List<string>();
		for(var i=0;i<listSchedulesToRemove.Count;i++) {
			var employee=Employees.GetFirstOrDefault(x => x.EmployeeNum==listSchedulesToRemove[i].EmployeeNum);
			if(employee==null) {
				continue;
			}
			listEmployeeNames.Add(employee.FName);//Internal database includes the initial of the last name.
