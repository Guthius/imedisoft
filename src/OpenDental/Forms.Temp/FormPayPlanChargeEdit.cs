...
public partial class FormPayPlanChargeEdit : FormODBase {
	...
	private void FormPayPlanCharge_Load(object sender, System.EventArgs e) {
		...
		if(!CompareDouble.IsZero(_payPlan.APR)) {
			...
			if(Security.IsAuthorized(EnumPermType.PayPlanChargeDateEdit,true)) {
				var strPlanType=Lan.g(this,"Patient ");
				...
				var logText=strPlanType+Lan.g(this,"Payment Plan Charge with amount due of")
	...
	private void butSave_Click(object sender, System.EventArgs e) {
		...
		var chargeDate=SIn.Date(textDate.Text);
		var principal=SIn.Double(textPrincipal.Text);
		var interest=SIn.Double(textInterest.Text);
		if(_payPlan.IsDynamic) {
			...
			var isEstimatedChargeDate=_dynamicPaymentPlanModuleData.ListPayPlanChargesExpected.FindAll(x=>x.PayPlanChargeNum==0).Select(x=>x.ChargeDate).Contains(chargeDate);
			...
			var payPlanProductionEntry=_dynamicPaymentPlanModuleData.ListPayPlanProductionEntries
				.First(x=>x.PriKey==PayPlanChargeCur.FKey && x.LinkType==PayPlanChargeCur.LinkType);
			var remainingProd=((double)payPlanProductionEntry.AmountRemaining);
			var amountCharged=_dynamicPaymentPlanModuleData.ListPayPlanChargesExpected
			...
			if(principal > remainingProd) {
				var msgString=Lans.g("Principal cannot be greater than the remaining production value of")+ " " + remainingProd.ToString("c") + ".";
	...
	private void butDelete_Click(object sender, System.EventArgs e) {
		...
		var listPayPlanChargesNotDeleted=PayPlanCharges.DeleteDebitsWithoutPayments([PayPlanChargeCur],doDelete:false);
