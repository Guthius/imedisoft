...
public partial class FormEtrans834Preview:FormODBase {
	...
	void FillGridInsPlans() {
		...
		if(gridInsPlans.Columns.Count==0) {
			...
			var col=new UI.GridColumn("Payer",100,HorizontalAlignment.Left,UI.GridSortingStrategy.StringCompare);
	...
	private void Load834_Unsafe() {
		...
		var rowCount=0;
		for(var i=0;i<_x834.ListTransactions.Count;i++) {
			var hx834_Tran=_x834.ListTransactions[i];
			for(var k=0;k<hx834_Tran.ListMembers.Count;k++) {
				rowCount++;
			}
		}
		for(var i=0;i<_x834.ListTransactions.Count;i++) {
			...
			for(var j=0;j<hx834_Tran.ListMembers.Count;j++) {
				...
				if(hx834_Member.ListHealthCoverage.Count==0) {
					...
					for(var a=0;a<hx834_Member.ListHealthCoverage.Count;a++) {
						var hx834_healthCoverage=hx834_Member.ListHealthCoverage[a];
						var row=new UI.GridRow();
						gridInsPlans.ListGridRows.Add(row);
	...
	private void FillGridRow(UI.GridRow row,Hx834_Member hx834_Member,Hx834_HealthCoverage hx834_HealthCoverage) {
		...
		var listPatientMatches=PatientFor834Import.GetPatientLimitedsByNameAndBirthday(hx834_Member.Pat,_listPatientLimiteds);
	...
	private void gridInsPlans_CellDoubleClick(object sender,UI.ODGridClickEventArgs e) {
		...
		frmPatientSelect.PatientInitial=hx834_Member.Pat;
		frmPatientSelect.ShowDialog();
		if(frmPatientSelect.IsDialogOK) {
			hx834_Member.Pat.PatNum=frmPatientSelect.PatNumSelected;
			gridInsPlans.BeginUpdate();
			...
			for(var i=0;i<gridInsPlans.ListGridRows.Count;i++) {
	...
	private bool TryImport834() {
		...
		Action<int, Patient> actionStatus=(idxRow,patient) => {
			ShowStatus("Progress "+(idxRow).ToString().PadLeft(6)+"/"+gridInsPlans.ListGridRows.Count.ToString().PadLeft(6)
			           +"  Importing plans for patient "+patient.GetNameLF());
		};
		var importInsurancePlansReturnData=EtransL.ImportInsurancePlansUsingPatientLimited(_x834,_listPatientLimiteds,checkIsPatientCreate.Checked,checkIsEmployerCreate.Checked,checkDropExistingIns.Checked, actionStatus);
		Cursor=Cursors.Default;
		#region summary output
		var msg=Lan.g(this,"Done.");
		if(importInsurancePlansReturnData.createdPatsCount > 0) {
			msg+="\r\n"+Lan.g(this,"Number of patients created:")+" "+importInsurancePlansReturnData.createdPatsCount;
		}
		if(importInsurancePlansReturnData.updatedPatsCount > 0) {
			msg+="\r\n"+Lan.g(this,"Number of patients updated:")+" "+importInsurancePlansReturnData.updatedPatsCount;
		}
		if(importInsurancePlansReturnData.skippedPatsCount > 0) {
			msg+="\r\n"+Lan.g(this,"Number of patients skipped:")+" "+importInsurancePlansReturnData.skippedPatsCount;
			msg+=importInsurancePlansReturnData.stringBuilderErrorMessages.ToString();
		}
		if(importInsurancePlansReturnData.createdCarrierCount > 0) {
			msg+="\r\n"+Lan.g(this,"Number of carriers created:")+" "+importInsurancePlansReturnData.createdCarrierCount;
			msg+=importInsurancePlansReturnData.stringBuilderErrorMessages.ToString();
		}
		if(importInsurancePlansReturnData.createdInsPlanCount > 0) {
			msg+="\r\n"+Lan.g(this,"Number of insurance plans created:")+" "+importInsurancePlansReturnData.createdInsPlanCount;
			msg+=importInsurancePlansReturnData.stringBuilderErrorMessages.ToString();
		}
		if(importInsurancePlansReturnData.updatedInsPlanCount > 0) {
			msg+="\r\n"+Lan.g(this,"Number of insurance plan updates:")+" "+importInsurancePlansReturnData.updatedInsPlanCount;
			msg+=importInsurancePlansReturnData.stringBuilderErrorMessages.ToString();
		}
		if(importInsurancePlansReturnData.createdInsSubCount > 0) {
			msg+="\r\n"+Lan.g(this,"Number of insurance subscriptions created:")+" "+importInsurancePlansReturnData.createdInsSubCount;
			msg+=importInsurancePlansReturnData.stringBuilderErrorMessages.ToString();
		}
		if(importInsurancePlansReturnData.updatedInsSubCount > 0) {
			msg+="\r\n"+Lan.g(this,"Number of insurance subscriptions updated:")+" "+importInsurancePlansReturnData.updatedInsSubCount;
			msg+=importInsurancePlansReturnData.stringBuilderErrorMessages.ToString();
		}
		if(importInsurancePlansReturnData.createdPatPlanCount > 0) {
			msg+="\r\n"+Lan.g(this,"Number of patients added to insurance:")+" "+importInsurancePlansReturnData.createdPatPlanCount;
			msg+=importInsurancePlansReturnData.stringBuilderErrorMessages.ToString();
		}
		if(importInsurancePlansReturnData.droppedPatPlanCount > 0) {
			msg+="\r\n"+Lan.g(this,"Number of patients dropped from insurance:")+" "+importInsurancePlansReturnData.droppedPatPlanCount;
			msg+=importInsurancePlansReturnData.stringBuilderErrorMessages.ToString();
		}
		if(importInsurancePlansReturnData.updatedPatPlanCount > 0) {
			msg+="\r\n"+Lan.g(this,"Number of patients insurance information updated:")+" "+importInsurancePlansReturnData.updatedPatPlanCount;
			msg+=importInsurancePlansReturnData.stringBuilderErrorMessages.ToString();
		}
		if(importInsurancePlansReturnData.createdEmployerCount > 0) {
			msg+="\r\n"+Lan.g(this,"Number of employers created:")+" "+importInsurancePlansReturnData.createdEmployerCount;
			msg+=importInsurancePlansReturnData.stringBuilderErrorMessages.ToString();
		}
