...
public partial class FormMedLabPatSelect:FormODBase {
	...
	private void FillGridPatInfo() {
		...
		var listPats=GetPatInfoFromPidSegments();
		GridRow row;
		for(var i=0;i<listPats.Count;i++) {
	...
	private List<string[]> GetPatInfoFromPidSegments() {
		...
		var hL7DefSegmentPid=hL7DefMessage.ListHL7DefSegments[1];
		...
		for(var i=0;i<ListMedLabs.Count;i++) {
			var fields=ListMedLabs[i].OriginalPIDSegment.Split(["|"],StringSplitOptions.None);
			var listFieldHL7s=new List<FieldHL7>();
			...
			var gender="";
			var ssn="";
			for(var j=0;j<hL7DefSegmentPid.hl7DefFields.Count;j++) {
				var itemOrder=hL7DefSegmentPid.hl7DefFields[j].OrdinalPos;
			...
			for(var j=0;j<listPats.Count;j++) {
			...
			if(!isDuplicate) {
				listPats.Add([patName,birthdate,gender,ssn]);
			}
		}
		return listPats;
	}
	...
	private void butPatSelect_Click(object sender,EventArgs e) {
		...
		if(PatientCur!=null && PatientCur.PatNum==frmPatientSelect.PatNumSelected) {
			return;
		}
		PatientCur=Patients.GetPat(frmPatientSelect.PatNumSelected);
		textName.Text=PatientCur.GetNameFL();
