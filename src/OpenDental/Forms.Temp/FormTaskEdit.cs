...
public partial class FormTaskEdit:FormODBase {
	...
	private void FormTaskListEdit_Load(object sender,System.EventArgs e) {
		...
		splitContainer.SplitterDistance=splitContainer.SplitterDistance;
	}
	...
	private void LoadTask() {
		...
		if(IsNew) {
			...
			if(isTaskForCurUser) {//this just allows getting the NoteList less often
				...
				for(var i=0;i<_listTaskNotes.Count;i++) {
		...
		if(_defNumPrioritySelected==0 && IsNew && TaskCur.ReminderType!=TaskReminderType.NoReminder) {
			for(var i=0;i<_listDefsTaskPriorities.Count;i++) {
		...
		for(var i=0;i<_listDefsTaskPriorities.Count;i++) {//Add non-hidden defs first
		...
		if(comboTaskPriorities.SelectedIndex==-1) {//Priority for task wasn't found in the non-hidden priorities list (and isn't triageBlue), so it must be a hidden priority.
			var listDefsCategories=Defs.GetDefsForCategory(DefCat.TaskPriorities);//Get all priorities
			for(var i=0;i<listDefsCategories.Count;i++) {
		...
		for(var i=0;i<_listTaskReminderTypes.Count;i++) {
		...
		for(var i=0;i<Enum.GetNames(typeof(TaskDateType)).Length;i++) {
		...
		if(IsNew) {
			...
			if(_listTaskNotes.Count==0) {//no notes, so reply to owner
				...
				for(var i=_listTaskNotes.Count-1;i>=0;i--) {
	...
	private void SetTaskStartingLocation() { 
		var listFormTaskEdits=Application.OpenForms.OfType<FormTaskEdit>().ToList();
		Point pointStart;
		...
		if(listFormTaskEdits.Count<=1) {
			var formOpenDental=Application.OpenForms.OfType<FormOpenDental>().ToList().FirstOrDefault();//Should be exactly 1.
		...
		var formTaskEditPrevious=listFormTaskEdits[listFormTaskEdits.Count-2];
		...
		pointStart.X+=OFFSET;
		pointStart.Y+=OFFSET;
		var rectangle=new Rectangle(pointStart,this.Size);
	...
	private void FillGrid() {
		...
		var col=new GridColumn(Lan.g(this,"Date Time"),120);
	...
	private void FillObject() {
		...
		var appointment=Appointments.GetOneApt(TaskCur.KeyNum);
		if(appointment==null) {
	...
	private void FillListCategory() {
		...
		var listDefsCategories=Defs.GetDefsForCategory(DefCat.TaskCategories);
	...
	private void RefreshReminderGroup() {
		var taskReminderType=_listTaskReminderTypes[comboReminderRepeat.SelectedIndex];
		...
		var reminderFrequency=SIn.Int(textReminderRepeatFrequency.Text,false);
	...
	private void listCategory_SelectionChangeCommitted(object sender,EventArgs e) {
		var defSelectedCategory=listCategory.GetSelected<Def>();
	...
	private void butChange_Click(object sender,System.EventArgs e) {
		...
		if(TaskCur.ObjectType==TaskObjectType.Patient) {
			TaskCur.KeyNum=frmPatientSelect.PatNumSelected;
		}
		if(TaskCur.ObjectType!=TaskObjectType.Appointment) {
			FillObject();
			return;
		}
		using var formApptsOther=new FormApptsOther(frmPatientSelect.PatNumSelected,null);//Select only, can't create new appt so don't need pinboard appointments.
		formApptsOther.IsForTask=true;
	...
	private void butGoto_Click(object sender,System.EventArgs e) {
		//GoTo no longer saves/closes the task.
		FormOpenDental.S_TaskGoTo(TaskCur.ObjectType,TaskCur.KeyNum);
	}
	...
	private void butAttachmnents_Click(object sender,EventArgs e) {
		TaskCur.IsNew=IsNew;
		using var formTaskAttachments=new FormTaskAttachments(TaskCur);
		...
		FillTextAttachments();
		var signalNum=Signalods.SetInvalid(InvalidType.Task,KeyType.Task,TaskCur.TaskNum);
		UserControlTasks.RefillLocalTaskGrids(TaskCur,_listTaskNotes, [signalNum]);
	}
	...
	private bool SaveCur() {
		...
		var dateTimeEntry=DateTime.MinValue;
		var taskReminderType=_listTaskReminderTypes[comboReminderRepeat.SelectedIndex];
		...
		if(!TaskCur.IsReadOnly && _taskOld!=null && _taskOld.IsReadOnly) {
			var isReadOnly=MsgBox.Show(MsgBoxButtons.YesNo,"Original task was read only. Set updated task to read only?");
		...
		var taskHist=new TaskHist(_taskOld);
	...
	private void butDelete_Click(object sender,EventArgs e) {
		...
		if(!IsNew) {
			...
			if(TaskCur.TaskListNum==0) {
				...
				var taskList=TaskLists.GetOne(TaskCur.TaskListNum);
				if(taskList!=null) {
		...
		var countDocuments=TaskAttachments.GetCountDocumentForTaskNum(TaskCur.TaskNum);
	...
	private void butReply_Click(object sender,EventArgs e) {
		...
		var taskListNum=TaskCur.TaskListNum;
		var inbox=Userods.GetInbox(_userNumReplyTo);
		if(inbox==0) {
	...
	private void butSend_Click(object sender,EventArgs e) {
		...
		var taskListNum=TaskCur.TaskListNum;
		using var formTaskListSelect=new FormTaskListSelect(listObjectType.GetSelected<TaskObjectType>(),IsNew);
	...
	public bool SaveCopy(List<long> listTaskListNums) {
		for(var i=0;i< listTaskListNums.Count;i++) {
			var taskCopy=TaskCur.Copy();
			...
			try {
				...
				for(var j=0;j<_listTaskNotes.Count;j++) {
					var taskNote=_listTaskNotes[j].Copy();
			...
			for(var t=0;t<_listTaskAttachments.Count;t++) {
				var taskAttachment=_listTaskAttachments[t].Copy();
			...
			if(taskCopy.TaskListNum==Userods.GetInbox(Security.CurUser.UserNum)) {//My inbox.
				var formTaskEdit=new OpenDental.FormTaskEdit(taskCopy);//Maintain previous behavior. If I send to myself, should popup.
	...
	private void SetFormReadOnly(bool doSetReadOnly) {
		...
		var taskListNum= Security.CurUser.TaskListInBox;
	...
	private void FormTaskEdit_FormClosing(object sender,FormClosingEventArgs e) {
		...
		if(DialogResult!=DialogResult.OK && _listDefsTaskPriorities.Count>=1) {//X click. There's two DialogResult.Cancels in LoadTask() caused by _listDefsTaskPriorities.Count<1 already addressed.
			...
			if(IsNew) { 
				var listTaskAttachments=TaskAttachments.GetManyByTaskNum(TaskCur.TaskNum);
