...
public partial class FormHL7DefEdit:FormODBase {
	...
	private void FormHL7DefEdit_Load(object sender,EventArgs e) {
		...
		for(var i=0;i<Enum.GetNames(typeof(ModeTxHL7)).Length;i++) {
			comboModeTx.Items.Add(Lan.g("enumModeTxHL7",Enum.GetName(typeof(ModeTxHL7),i)));
		...
		for(var i=0;i<_listDefsImageCat.Count;i++) {
			comboLabImageCat.Items.Add(_listDefsImageCat[i].ItemName);
			if(_listDefsImageCat[i].DefNum==HL7DefCur.LabResultImageCat) {
	...
	private void FillGrid() {
		...
		var col=new GridColumn(Lan.g(this,"Message"),150);
		...
		for(var i=0;i<HL7DefCur.hl7DefMessages.Count;i++) {
			var row=new GridRow();
			row.Cells.Add(HL7DefCur.hl7DefMessages[i].MessageType+", "+HL7DefCur.hl7DefMessages[i].MessageStructure+", "+HL7DefCur.hl7DefMessages[i].InOrOut);
			row.Cells.Add("");
			row.Cells.Add(HL7DefCur.hl7DefMessages[i].Note);
			row.Tag=HL7DefCur.hl7DefMessages[i];
			gridMain.ListGridRows.Add(row);
			if(HL7DefCur.hl7DefMessages[i].ListHL7DefSegments==null) {
				continue;
			}
			for(var j=0;j<HL7DefCur.hl7DefMessages[i].ListHL7DefSegments.Count;j++) {
				row=new GridRow();
				row.Cells.Add("");
				row.Cells.Add(HL7DefCur.hl7DefMessages[i].ListHL7DefSegments[j].SegmentName.ToString());
				row.Cells.Add(HL7DefCur.hl7DefMessages[i].ListHL7DefSegments[j].Note);
				row.Tag=HL7DefCur.hl7DefMessages[i];
				gridMain.ListGridRows.Add(row);
			}
		}
	...
	private void butBrowseIn_Click(object sender,EventArgs e) {
		...
		folderBrowserDialog.SelectedPath=textInPathOrSocket.Text;
		if(folderBrowserDialog.ShowDialog()==DialogResult.OK) {
			textInPathOrSocket.Text=folderBrowserDialog.SelectedPath;
		}
	}

	private void butBrowseOut_Click(object sender,EventArgs e) {
		var folderBrowserDialog=new FolderBrowserDialog();
		folderBrowserDialog.SelectedPath=textOutPathSocketOrDir.Text;
		if(folderBrowserDialog.ShowDialog()==DialogResult.OK) {
			textOutPathSocketOrDir.Text=folderBrowserDialog.SelectedPath;
		}
	}

	private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		using var formHL7DefMessageEdit=new FormHL7DefMessageEdit();
		formHL7DefMessageEdit.HL7DefMessageCur=(HL7DefMessage)gridMain.ListGridRows[e.Row].Tag;
		formHL7DefMessageEdit.IsHL7DefInternal=HL7DefCur.IsInternal;
		formHL7DefMessageEdit.HL7InternalType_=HL7DefCur.InternalType;
		formHL7DefMessageEdit.ShowDialog();
		FillGrid();
	}
	...
	private void checkEnabled_Click(object sender,EventArgs e) {
		...
		var isHL7Enabled=HL7Defs.IsExistingHL7Enabled(HL7DefCur.HL7DefNum,HL7DefCur.InternalType==HL7InternalType.MedLabv2_3);
		if(isHL7Enabled) {
	...
	private void butDelete_Click(object sender,EventArgs e) {
		...
		for(var m=0;m<HL7DefCur.hl7DefMessages.Count;m++) {
			for(var s=0;s<HL7DefCur.hl7DefMessages[m].ListHL7DefSegments.Count;s++) {
				for(var f=0;f<HL7DefCur.hl7DefMessages[m].ListHL7DefSegments[s].hl7DefFields.Count;f++) {
					HL7DefFields.Delete(HL7DefCur.hl7DefMessages[m].ListHL7DefSegments[s].hl7DefFields[f].HL7DefFieldNum);
				}
				HL7DefSegments.Delete(HL7DefCur.hl7DefMessages[m].ListHL7DefSegments[s].HL7DefSegmentNum);
			}
	...
	private void butAdd_Click(object sender,EventArgs e) {
		//This button is only enabled if this is a custom def.
		using var formHL7DefMessageEdit=new FormHL7DefMessageEdit();
		formHL7DefMessageEdit.HL7DefMessageCur=new HL7DefMessage();
		formHL7DefMessageEdit.HL7DefMessageCur.HL7DefNum=HL7DefCur.HL7DefNum;
		formHL7DefMessageEdit.HL7DefMessageCur.IsNew=true;
		formHL7DefMessageEdit.IsHL7DefInternal=false;
		formHL7DefMessageEdit.HL7InternalType_=HL7DefCur.InternalType;
		formHL7DefMessageEdit.ShowDialog();
		FillGrid();
	}
	...
	private void RadioAddPts_Click(object sender,EventArgs e) {
		var password=new InputBox("In our online manual, on the HL7 page, look for the password and enter it below.");
		password.ShowDialog();
	...
	private bool ValidateData() {
		...
		switch(comboModeTx.SelectedIndex) {
			...
			case (int)ModeTxHL7.TcpIp:
				...
				var strIpPort=textOutPathSocketOrDir.Text.Split(':');
				...
				try {
					int.Parse(textInPathOrSocket.Text);
				}
				catch {
			...
			case (int)ModeTxHL7.Sftp:
				...
				var strAddressPort=textInPathOrSocket.Text.Split(':');
	...
	private void butSave_Click(object sender,EventArgs e) {
		...
		if(comboLabImageCat.SelectedIndex>=0) {
			HL7DefCur.LabResultImageCat=_listDefsImageCat[comboLabImageCat.SelectedIndex].DefNum;
		}
