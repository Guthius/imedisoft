...
public partial class FormMessageToPayEdit:FormODBase {
	...
	private void butSend_Click(object sender,EventArgs e) {
		...
		var commOptOut=CommOptOuts.GetForPat(_patient.PatNum);
		var isOptedOutEmail=false;
		var isOptedOutText=false;
		if(commOptOut!=null) {
		...
		var statement=GetRegularStatement();
		var dataSetAccount=AccountModules.GetAccount(_patient.PatNum,statement);
		Statements.CalcBalTotalInsEst(statement,dataSetAccount);
		statement.StatementNum=Statements.Insert(statement);//Moved from GetRegularStatement() to allow setting of BalTotal/InsEst before inserting here
		var sheetDef=SheetUtil.GetStatementSheetDef(statement);
		var listDefsForImageCats=Defs.GetDefsForCategory(DefCat.ImageCats,true);
		...
		var emailSent=false;
		var textSent=false;
		var filePath=CreateStatementPDF(statement);
	...
	private void butPreview_Click(object sender,EventArgs e) {
		var statement=GetRegularStatement();
		var dataSetStatement=AccountModules.GetAccount(_patient.PatNum,statement,isComputeAging:false,doIncludePatLName:true);
		Statements.CalcBalTotalInsEst(statement,dataSetStatement);
		statement.StatementNum=Statements.Insert(statement);//Moved from GetRegularStatement() to allow setting of BalTotal/InsEst before inserting here
		if(statement.StatementNum==0) {//Something went wrong with insertion
			return;
		}
		var sheetDef=SheetUtil.GetStatementSheetDef(statement);
	...
	private void SetEnabledMessageTemplateControls() {
		//Email controls
		var isEmailChecked=checkEmail.Checked;
	...
	private void LoadMessageTemplates() {
		var msgToPayLite=new MsgToPayLite(_patient);//Used only for tag replacements
	...
	private Statement GetRegularStatement() {
		var statement=Statements.GenerateStatement(_patient,DateTime.MinValue,DateTime.Today,StatementMode.Electronic,isSinglePatient:radioPatient.Checked);
	...
	private bool SendText(Statement statement) {
		...
		smsToMobile.MsgText=_msgToPayTagReplacer.ReplaceTagsForStatement(textMessage.Text,_patient,statement,_clinic);
		statement.TagOD=smsToMobile.GuidMessage;
		var sendSuccessful=false;
		try {
			var listSmsToMobiles=SmsToMobiles.SendSmsMany(ListTools.FromSingle(smsToMobile),userod:Security.CurUser);
	...
	private bool SendEmail(Statement statement,string savedPdfPath) {
		...
		emailMessage.BodyText=_msgToPayTagReplacer.ReplaceTagsForStatement(_msgToPayEmailTemplate.Template,_patient,statement,_clinic,true);
		emailMessage.HtmlText=_msgToPayTagReplacer.ReplaceTagsForStatement(_msgToPayEmailHtmlText,_patient,statement,_clinic,true);
		var emailAddress=EmailAddresses.GetNewEmailDefault(Security.CurUser.UserNum,_patient.ClinicNum);
		...
		emailMessage.Subject=_msgToPayTagReplacer.ReplaceTagsForStatement(textSubject.Text,_patient,statement,_clinic,true);
	...
	private void RefreshEmailBrowser() {
		...
		var msgToPayLite=new MsgToPayLite(_patient);
		...
		var templateMessage=_msgToPayTagReplacer.ReplaceTags(_msgToPayEmailTemplate.Template,msgToPayLite,_clinic,true); 
