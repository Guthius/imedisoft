...
public partial class FormRecallTypeEdit:FormODBase {
	...
	private void comboSpecial_SelectionChangeCommitted(object sender,EventArgs e) {
		if(_countForType>0){
			ODMessageBox.Show(Lan.g(this,"Cannot change Special Type. Patients using this recall type: ") + _countForType);
		...
		if(countForTypePrev>0) {
			ODMessageBox.Show(Lan.g(this,"Cannot change Special Type to one that is set for another recall type and in use by patients.  "
			                             +"Patients using the other recall type: ")+countForTypePrev);
			SetSpecialIdx();//sets back to what it was when form opened
			return;
		}
		SetSpecialText();
	...
	private void FillTriggers(){
		...
		string str;
		for(var i=0;i<_listRecallTriggers.Count;i++){
	...
	private void FillProcs(){
		...
		var stringArrayProcs=RecallTypeCur.Procedures.Split(',');
		string str;
		for(var i=0;i<stringArrayProcs.Length;i++){
	...
	private void butRemoveProc_Click(object sender,EventArgs e) {
		...
		var stringArrayProcs=RecallTypeCur.Procedures.Split(',');
		var listRecallProcs=new List<string>(stringArrayProcs);
		listRecallProcs.RemoveAt(listProcs.SelectedIndex);
		RecallTypeCur.Procedures="";
		for(var i=0;i<listRecallProcs.Count;i++){
			if(i>0){
				RecallTypeCur.Procedures+=",";
			}
	...
	private void butSelectBlockouts_Click(object sender,EventArgs e) {
		using var formDefinitionPicker=new FormDefinitionPicker(DefCat.BlockoutTypes,_listDefsCur);
	...
	private void butSave_Click(object sender, System.EventArgs e) {
		...
		var listRestrictionBlockoutDefNums=_listDefsCur.Select(x => x.DefNum).ToList();
