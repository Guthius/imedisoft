...
public partial class FormTsiHistory:FormODBase {
	...
	private void FormTsiHistory_Load(object sender,EventArgs e) {
		...
		var listClientIDs=new List<string>();
		var progNum=Programs.GetProgramNum(ProgramName.Transworld);
		if(progNum>0) {
		...
		var listTsiTransTypes=Enum.GetValues(typeof(TsiTransType)).OfType<TsiTransType>().ToList();
		...
		var listAcctStatuses=new[] { "Active","Suspended","Inactive" }.ToList();
		...
		if(FormOpenDental.PatNumCur>0) {
			var family=Patients.GetFamily(FormOpenDental.PatNumCur);
		...
		var listLogClientIDs=_listTsiTransLogs.Select(x => x.ClientId).Distinct().Where(x => !listClientIDs.Contains(x)).ToList();
		for(var i = 0;i<listLogClientIDs.Count;i++) {
	...
	private void FillGrid() {
		...
		var listLogIndexesFiltered=GetListIndexesFiltered();
		#region Set Grid Columns
		gridMain.BeginUpdate();
		gridMain.Columns.Clear();
		gridMain.Columns.Add(new GridColumn("Guarantor",true?200:300){ IsWidthDynamic=true,DynamicWeight=true?20:30 });
		if(true) {
		...
		var rowToReselect=-1;
		GridRow row;
		for(var i=0;i<listLogIndexesFiltered.Count;i++) {
			var tsiTransLog=_listTsiTransLogs[listLogIndexesFiltered[i]];
			row=new GridRow();
			var patNum="";
			if (checkShowPatNums.Checked) {
				patNum=tsiTransLog.PatNum+" - ";
			}
			var patName = _listPatientsLim.Find(x => x.PatNum==tsiTransLog.PatNum)?.GetNameLF();
			patName??=Patients.GetNameLF(tsiTransLog.PatNum);
			row.Cells.Add(patNum+patName);
			if(true) {
				var clinicAbbr=_listClinics.Find(x => x.Id==tsiTransLog.ClinicNum)?.Abbr;
	...
	private List<int> GetListIndexesFiltered() {
		...
		var dateStart=datePicker.GetDateTimeFrom();
		var dateEnd=datePicker.GetDateTimeTo();
		dateEnd=(dateEnd==DateTime.MinValue?DateTime.Today:dateEnd);
		var listSelectedTransTypes=new List<TsiTransType>();
		if(!comboTransTypes.IsAllSelected) {
			listSelectedTransTypes=comboTransTypes.GetListSelected<TsiTransType>();
		}
		var listSelectedAcctStatuses=new List<string>();
		if(!comboAcctStatuses.IsAllSelected) {
			listSelectedAcctStatuses=comboAcctStatuses.GetListSelected<string>();
		}
		var listSuspendedGuarNums=TsiTransLogs.GetSuspendedGuarNums();
		var listActiveCollGuarNums=Patients.GetListCollectionGuarNums(/*false*/);
		var listClientIDs=new List<string>();
		...
		for(var i=0;i<_listTsiTransLogs.Count;i++) {
			var clinicNum=_listTsiTransLogs[i].ClinicNum;
			if(true && clinicNum<=0) {
	...
	private void gridMain_CellClick(object sender,ODGridClickEventArgs e) {
		...
		for(var i=0;i<gridMain.ListGridRows.Count;i++) {
			var tsiTransLog=_listTsiTransLogs[(int)gridMain.ListGridRows[i].Tag];
	...
	private void textRawMsg_SelectionChanged(object sender,EventArgs e) {
		...
		var selectedLogIndex=gridMain.SelectedTag<int>();
		if(selectedLogIndex<0 || selectedLogIndex>_listTsiTransLogs.Count) {
			return;
		}
		var tsiTransLogSelected=_listTsiTransLogs[selectedLogIndex];
		var selectionStart=textRawMsg.SelectionStart;
		var selectionLength=textRawMsg.SelectionLength;
		...
		var fieldDescripts=msgHeader.Split('|');
		var countPipesToLeftOfCursor=textRawMsg.Text.Substring(0,selectionStart).Count(x => x=='|');
		if(countPipesToLeftOfCursor<fieldDescripts.Length) {
			textSelectedFieldName.Text=fieldDescripts[countPipesToLeftOfCursor];
		}
		var selectedFieldStartIndex=textRawMsg.Text.Substring(0,selectionStart).LastIndexOf('|')+1;
		var selectedFieldEndIndex=textRawMsg.Text.IndexOf('|',selectionStart);
		selectedFieldEndIndex=(selectedFieldEndIndex==-1?textRawMsg.Text.Length:selectedFieldEndIndex);
		var richTextBox_=new RichTextBox();
	...
	private void FillTextSelectedFieldDetails(string fieldDescription,string selectedText) {
		switch(fieldDescription) {
			...
			case "TRANSACTION TYPE":
				...
				var tsiTransType=SIn.Enum<TsiTransType>(selectedText,true);
	...
	private void butFind_Click(object sender,EventArgs e) {
		...
		var family=Patients.GetFamily(frmPatientSelect.PatNumSelected);
		textPatient.Text=family.GetNameInFamLF(frmPatientSelect.PatNumSelected);
		_listPatNumsFam=family.ListPats.Select(x => x.PatNum).ToList();
		FillGrid();
	}
	...
	private void menuItemGoTo_Click(object sender,EventArgs e) {
		...
		var logIndex=gridMain.SelectedGridRows[0].Tag;//Index is original location in _listTsiTransLogsAll
		...
		GlobalFormOpenDental.GoToModule(EnumModuleType.Account,patNum:_listTsiTransLogs[(int)logIndex].PatNum);
		SendToBack();
	}
