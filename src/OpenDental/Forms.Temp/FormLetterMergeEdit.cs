...
public partial class FormLetterMergeEdit : FormODBase {
	...
	private void FillPatSelect(){
		...
		for(var i=0;i<_letterMerge.Fields.Count;i++){
			for(var j=0;j<listPatSelect.Items.Count;j++){
	...
	private void FillListReferral(){
		...
		for(var i=0;i<_letterMerge.Fields.Count;i++){
			for(var j=0;j<listReferral.Items.Count;j++){
	...
	private void FillListOther(){
		...
		for(var i=0;i<_letterMerge.Fields.Count;i++){
			for(var j=0;j<listOther.Items.Count;j++){
	...
	private void butNew_Click(object sender, System.EventArgs e) {
		...
		var templateFile=ODFileUtils.CombinePaths(PrefC.GetString(PrefName.LetterMergePath),textTemplateName.Text);
		...
		try {
			...
			word_Document=word_Application.Documents.Add();
			word_Document.SaveAs(templateFile);
	...
	private void butSave_Click(object sender, System.EventArgs e) {
		...
		for(var i=0;i<listPatSelect.SelectedIndices.Count;i++){
