...
public partial class FormSheetFillEdit:FormODBase {
	...
	private List<RectangleF> _listRectangleFsChars= [];
	private List<SignatureBoxWrapper> _listSignatureBoxWrappers= [];
	private List<RichTextBox> _listRichTextBoxesSignatures= [];
	...
	public static void ShowForm(Sheet sheet,FormClosingEventHandler formClosingEventHandler=null,bool isReadOnly=false) {
		var formSheetFillEdit=new FormSheetFillEdit();
	...
	private int GetBottomOfPage(int pageCount) {
		var marginPerPage=_marginsPrint.Top+_marginsPrint.Bottom;
	...
	private void butAddField_Click(object sender,EventArgs e) {
		using var formSheetFieldAdd=new FormSheetFieldAdd();
	...
	private void butChangePat_Click(object sender,EventArgs e) {
		...
		if(frmPatientSelect.IsDialogOK) {
			...
			SheetCur.PatNum=frmPatientSelect.PatNumSelected;
	...
	private void butEmail_Click(object sender,EventArgs e) {
		...
		var hasSheetFromDb=!IsStatement && !IsNewReferralLetterWithProcsOrChart();
		...
		var strEmailAddress="";
		var subject=SheetCur.Description;
		if(SheetCur.PatNum!=0 && SheetCur.SheetType!=SheetTypeEnum.DepositSlip) {
			...
			if(SheetCur.SheetType==SheetTypeEnum.LabSlip) {
				var sheetParamLabCaseNum=SheetParameter.GetParamByName(SheetCur.Parameters,"LabCaseNum");//auto populate lab email.
				var labCase=LabCases.GetOne(SIn.Long(sheetParamLabCaseNum.ParamValue.ToString()));
		...
		if(SheetCur.SheetType==SheetTypeEnum.ReferralSlip || SheetCur.SheetType==SheetTypeEnum.ReferralLetter) {
			var sheetParameter=SheetParameter.GetParamByName(SheetCur.Parameters,"ReferralNum");
			if(sheetParameter==null) {//it can be null sometimes because of old bug in db.
				...
				var referralNum=SIn.Long(sheetParameter.ParamValue.ToString());
		...
		var pdfFile=EmailSheet(strEmailAddress,subject);
	...
	private void butPrintOrEmail_Click(object sender,EventArgs e) {
		...
		//it will have the extra parameter we are looking for.
		//A new sheet will also have a PatNum parameter which we will ignore.
		using var formSheetOutputFormat=new FormSheetOutputFormat();
		   ...
		   || SheetCur.SheetType==SheetTypeEnum.ReferralLetter)
		{
			...
			var parameter=SheetParameter.GetParamByName(SheetCur.Parameters,"ReferralNum");
			if(parameter==null){//it can be null sometimes because of old bug in db.
				...
				var referralNum=SIn.Long(parameter.ParamValue.ToString());
				try{
					referral=Referrals.GetReferral(referralNum);
				}
				catch (Exception ex) {
		...
		if(formSheetOutputFormat.IsEmailPatOrLab){
			pdfFile=EmailSheet(formSheetOutputFormat.EmailPatOrLabAddress,SheetCur.Description);//subject could be improved
		}
	...
	private void butSave_Click(object sender,EventArgs e) {
		...
		if(SheetCur.SheetType==SheetTypeEnum.ExamSheet) {//Exam sheet saves and keeps the window open
			butSave.Text=Lans.g("Save");//If they decide to click it again, change the button text back
	...
	private void butToKiosk_Click(object sender,EventArgs e) {
		...
		var terminalNum=Sheets.GetMaxTerminalNum(SheetCur.PatNum)+1;
	...
	private void butUnlock_Click(object sender,EventArgs e) {
		...
		if(SheetCur.SheetType==SheetTypeEnum.PaymentPlan) {
			var payPlan=(PayPlan)SheetParameter.GetParamByName(SheetCur.Parameters,"payplan").ParamValue;
			for(var i=0;i<panelMain.Controls.Count;i++){
				...
				var sigBox=(OpenDental.UI.SignatureBoxWrapper)panelMain.Controls[i];
				sigBox.Enabled=true;
			}
	...
	private void FormSheetFillEdit_FormClosing(object sender,FormClosingEventArgs e) {
		if(_listSignatureBoxWrappers!=null) {
			for(var i=0;i<_listSignatureBoxWrappers.Count;i++) {
	...
	private void FormSheetFillEdit_Load(object sender,EventArgs e) {
		if(SheetCur.IsLandscape){
			Width=(int)Math.Max(SheetCur.Height+190,butOK.Right+27);
			Height=(int)(SheetCur.Width+65);
		}
		else{
			Width=(int)Math.Max(SheetCur.Width+190,butOK.Right+27);
			Height=(int)(SheetCur.Height+65);
		}
		...
		if(_isAutoSave && !IsInTerminal) {//only visible if the Autosave Form usage has been set and is not in kiosk mode
			...
			//Jordan the schema is wrong here. We should never refer back to the template SheetDef
			//but we're going to leave it because too hard to fix right now.
			var sheetDefForSheetCur=SheetDefs.GetFirstOrDefault(x=>x.SheetDefNum==SheetCur.SheetDefNum);
		...
		for(var i=0;i<SheetCur.SheetFields.Count;i++) {
			if(SheetCur.SheetFields[i].FieldValue.Contains("AutoNoteNum:")) {
				var autoNoteNum=Convert.ToInt64(SheetCur.SheetFields[i].FieldValue.Substring(SheetCur.SheetFields[i].FieldValue.IndexOf(":")+1));
				var autoNote=AutoNotes.GetWhere(x => x.AutoNoteNum==autoNoteNum).FirstOrDefault();
		...
		panelMain.Height=Math.Max(SheetCur.HeightPage,bottomLastField);//+20 for Hscrollbar?
		...
		var listSheetFieldsPositiveTabOrder=SheetCur.SheetFields.FindAll(x=>x.TabOrder>0).ToList();
		...
		if(listSheetFieldsPositiveTabOrder.Count>0) {
			...
			var sheetFieldFirstTab=listSheetFieldsPositiveTabOrder.Find(x=>x.TabOrder==_tabCurrent);
		...
		if(SheetCur.SheetType==SheetTypeEnum.PaymentPlan) {
			...
			if(payPlan.Signature!="" && (payPlan.Signature!=null || !payPlan.IsNew)) {
				for(var i=0;i<panelMain.Controls.Count;i++){
					...
					var signatureBoxWrapper=(OpenDental.UI.SignatureBoxWrapper)panelMain.Controls[i];
	...
	private void menuItemCombo_Click(object sender,EventArgs e) {
		...
		var sheetField=(SheetField)menuItem.Tag;
	...
	private void panelMain_MouseDoubleClick(object sender,MouseEventArgs e) {
		...
		var sheetField=HitTest(pointSheet);
		if(sheetField==null) {
			return;
		}
		...
		var pageCount=Sheets.CalculatePageCount(SheetCur,_marginsPrint);
	...
	private void panelMain_MouseMove(object sender,MouseEventArgs e) {
		...
		using var g=Graphics.FromHwnd(panelMain.Handle);
		g.SmoothingMode=SmoothingMode.HighQuality;
		float scale=1;
		g.ScaleTransform(scale,scale);
		var pen=new Pen(Brushes.Black,2f);			
		var idx=_listPoints.Count-1;
	...
	private void panelMain_MouseUp(object sender,MouseEventArgs e) {
		...
		for(var i=0;i<_listPoints.Count;i++){
			if(i>0){
				sheetField.FieldValue+=";";
			}
			sheetField.FieldValue+=_listPoints[i].X+","+_listPoints[i].Y;
		}
		sheetField.FontName="";
	...
	private void panelMain_Paint(object sender,PaintEventArgs e) {
		...
		for(var i=0;i<SheetCur.SheetFields.Count;i++){
			...
			var sizeField=new Size(SheetCur.SheetFields[i].Width,SheetCur.SheetFields[i].Height);
			var scaleImage=ImageTools.CalcScaleFit(sizeField,SheetCur.SheetFields[i].BitmapLoaded.Size,0);
		...
		for(var i=0;i<SheetCur.SheetFields.Count;i++){
			...
			var rectangle=new Rectangle(SheetCur.SheetFields[i].XPos,SheetCur.SheetFields[i].YPos,SheetCur.SheetFields[i].Width,SheetCur.SheetFields[i].Height);
			if(SheetCur.SheetFields[i].FieldType==SheetFieldType.InputField){
				var colorBack=Color.FromArgb(245,245,200);
			...
			using var font=new Font(SheetCur.SheetFields[i].FontName,SheetCur.SheetFields[i].FontSize,fontStyle);
			var colorText=SheetCur.SheetFields[i].ItemColor;
			if(SheetCur.SheetFields[i].FieldType==SheetFieldType.InputField){
		...
		for(var i=0;i<SheetCur.SheetFields.Count;i++){
			...
			//FieldValue is X
			using var pen=new Pen(Color.Black);//checkboxes have no color choice, and they've always been drawn with 1 px thickness
		...
		for(var i=0;i<SheetCur.SheetFields.Count;i++){
			...
			var yPosArrowStart=SheetCur.SheetFields[i].YPos+SheetCur.SheetFields[i].Height/2f;
			using var _penArrow=new Pen(Color.FromArgb(20,20,20),1.5f);
			if(_tabCurrent==SheetCur.SheetFields[i].TabOrder && _tabCurrent>0) {
				var colorHighlight=Color.FromArgb(240,210,100);//orangish
			...
			g.DrawLine(_penArrow,
				x1:xPosArrowStart-13,
				y1:yPosArrowStart-1.5f,
				x2:xPosArrowStart-9.5f,
				y2:yPosArrowStart+1.5f);
			g.DrawLine(_penArrow,
				x1:xPosArrowStart-9.5f,
				y1:yPosArrowStart+1.5f,
				x2:xPosArrowStart-6,
				y2:yPosArrowStart-1.5f);
			...
			using var font=new Font(FontFamily.GenericSansSerif,SheetCur.SheetFields[i].Height*.58f);
			var str=SheetFields.GetComboSelectedOption(SheetCur.SheetFields[i]);
		...
		for(var i=0;i<SheetCur.SheetFields.Count;i++){
			...
			using var pen=new Pen(colorText,1f);//This 1 gets scaled up slightly if there is any scaling
		...
		//Pen drawings-----------------------------------------------------------------------------------------------------------------
		using var penDraw=new Pen(Brushes.Black,2f);//This 2 gets scaled up slightly if there is any scaling
		for(var i=0;i<SheetCur.SheetFields.Count;i++){
			if(!SheetCur.SheetFields[i].FieldType.In(SheetFieldType.Drawing)){
				continue;
			}
			var stringArrayPoints=SheetCur.SheetFields[i].FieldValue.Split(';');
			var listPoints=new List<Point>();
			for(var p=0;p<stringArrayPoints.Length;p++){
				var stringArrayXy=stringArrayPoints[p].Split(',');
				if(stringArrayXy.Length==2){
					var point=new Point(SIn.Int(stringArrayXy[0]),SIn.Int(stringArrayXy[1]));
					listPoints.Add(point);
				}
			}
			for(var pt=1;pt<listPoints.Count;pt++){
		...
		for(var i=1;i<_listPoints.Count;i++){
		...
		for(var i=1;i<pageCount;i++) {
			var bottomYOfPage=GetBottomOfPage(i);
	...
	private void TextBox_PreviewKeyDown(object sender,PreviewKeyDownEventArgs e) {
		...
		var textBox=(ODtextBox)sender;
		var sheetField=(SheetField)textBox.Tag;
		...
		{
			var tabOrder=sheetField.TabOrder;
			...
			var listSheetFieldsTabs=SheetCur.SheetFields
			...
			var sheetFieldNext=listSheetFieldsTabs[0];
	...
	private void PanelMain_PreviewKeyDown(object sender,PreviewKeyDownEventArgs e)	{
		...
		if(e.KeyCode==Keys.Space) {
			var sheetField=SheetCur.SheetFields.Find(x=>x.TabOrder==_tabCurrent);
			if(sheetField==null) {
				return;
			}
			if(sheetField.FieldType==SheetFieldType.InputField) {
		...
		if(e.KeyCode==Keys.Tab) {
			var listSheetFields=SheetCur.SheetFields.FindAll(x=>x.TabOrder>_tabCurrent && (x.FieldType==SheetFieldType.InputField || x.FieldType==SheetFieldType.CheckBox || x.FieldType==SheetFieldType.ComboBox)).OrderBy(x=>x.TabOrder).ToList();
			panelMain.Invalidate();
			if(listSheetFields.Count==0) {
				_tabCurrent=0;
				return;
			}
			var sheetFieldNext=listSheetFields[0];
			_tabCurrent=sheetFieldNext.TabOrder;
			if(sheetFieldNext.FieldType==SheetFieldType.CheckBox || sheetFieldNext.FieldType==SheetFieldType.ComboBox) {
				return;
			}
	...
	private void timerTextChanged_Tick(object sender,EventArgs e) {
		timerTextChanged.Stop();
		var textBox=(ODtextBox)timerTextChanged.Tag;
		if(textBox.IsDisposed || textBox.Disposing){
			return;
		}
		timerTextChanged.Tag=null;
		var sheetField=(SheetField)textBox.Tag;
		var fieldValue=textBox.Text;
		//sheetField.FieldValue=textBox.Text;
	...
	private void CreateFloatingComboOptions(SheetField sheetField){
		...
		var listItems=SheetFields.GetComboMenuItems(sheetField);
		for(var i=0;i<listItems.Count;i++){
			var menuItem=new MenuItem(listItems[i],menuItemCombo_Click);
			menuItem.Tag=sheetField;
			contextMenu.MenuItems.Add(menuItem);
		}
		var yPos=sheetField.YPos;
		var height=sheetField.Height;
		//int scrollVal=panelScroll.VerticalScroll.Value;
		var point=new Point(sheetField.XPos,yPos+height);//+scrollVal);
	...
	private void CreateFloatingTextBox(SheetField sheetField,Point point,bool isFromCheckBox=false,bool isRightClick=false){
		var text=sheetField.FieldValue;
		...
		textBox.Location=new Point((int)(sheetField.XPos+2.5f),sheetField.YPos);//small adjustments made to help it line up with orig.
		textBox.Width=sheetField.Width-1;//tested this with left and right align
		...
		var fontSize=sheetField.FontSize;
		...
		if(sheetField.Height<=textBox.Font.Height+5 && sheetField.GrowthBehavior==GrowthBehaviorEnum.None) {
	...
	private string EmailSheet(string addr_to,string subject) {
		...
		var emailMessageSource=EmailMessageSource.Sheet;
	...
	private void ClearSigs(){
		...
		for(var i=0;i<_listRichTextBoxesSignatures.Count;i++) {
	...
	private int HitTestChars(SheetField sheetField,Point point,string text){
		var pointUnscaled=new Point(point.X,point.Y);
		_listRectangleFsChars= [];
		...
		var font=new Font(sheetField.FontName,sheetField.FontSize,fontStyle);
		//We're only allowed to measure 32 chars at a time
		var cStart=0;//the starting idx of the group of 32 or less chars that we're about to measure
		var stringFormat = new StringFormat();
		...
		while(true){
			...
			var characterRangeArray=new CharacterRange[length];//each CharacterRange is just one char
			for(var i=0;i<characterRangeArray.Length;i++){
				characterRangeArray[i]=new CharacterRange(cStart+i,1);
			}
			stringFormat.SetMeasurableCharacterRanges(characterRangeArray);
			var rectangleF=new RectangleF(0,0,sheetField.Width,sheetField.Height);
			var regionArray=g.MeasureCharacterRanges(text,font,rectangleF,stringFormat);
			for(var r=0;r<regionArray.Length;r++){
				var rectangleFBoundsChar=regionArray[r].GetBounds(g);
				//I use global so I can draw them for testing.
				var rectangleFGlobal=new RectangleF();
		...
		var idxSelectedChar=-1;
		for(var i=0;i<_listRectangleFsChars.Count;i++){
	...
	private string LayoutFields() {
		...
		var width=SheetCur.Width;
		if(SheetCur.IsLandscape){
			width=SheetCur.Height;
		}
		var bottomLastField=0;
		if(SheetCur.SheetFields.Count>0) {
		...
		var height=Math.Max(SheetCur.HeightPage,bottomLastField);//+20 for Hscrollbar?
		panelMain.Size=new Size(width,height);
		var strErr="";
		_listSignatureBoxWrappers= [];
		var listSheetFieldsSorted=new List<SheetField>();
		...
		for(var i=panelMain.Controls.Count-1;i>=0;i--) {
		...
		for(var i=0;i<SheetCur.SheetFields.Count;i++){
			...
			var signatureBoxWrapper=new OpenDental.UI.SignatureBoxWrapper();
			...
			signatureBoxWrapper.Location=new Point(SheetCur.SheetFields[i].XPos,SheetCur.SheetFields[i].YPos);
			signatureBoxWrapper.Width=SheetCur.SheetFields[i].Width;
			signatureBoxWrapper.Height=SheetCur.SheetFields[i].Height;
			signatureBoxWrapper.SetScaleAndZoom(1,1);//This is not redundant
			if(SheetCur.SheetFields[i].FieldValue.Length>0) {//a signature is present
				var sigIsTopaz=SheetCur.SheetFields[i].FieldValue[0]=='1';
				var signature="";
				if(SheetCur.SheetFields[i].FieldValue.Length>1) {
					signature=SheetCur.SheetFields[i].FieldValue.Substring(1);
				}
				var keyData=Sheets.GetSignatureKey(SheetCur);
				signatureBoxWrapper.FillSignature(sigIsTopaz,keyData,signature);
			}
			if(SheetCur.SheetType==SheetTypeEnum.PaymentPlan) {
				var payPlan=(PayPlan)SheetParameter.GetParamByName(SheetCur.Parameters,"payplan").ParamValue;
				if(payPlan.Signature!="") {//a PP sig is present
					var keyData=(string)SheetParameter.GetParamByName(SheetCur.Parameters,"keyData").ParamValue;
			...
			if(signatureBoxWrapper.IsValid 
			   && SheetCur.SheetFields[i].FieldValue.Length>0
			   && isOldSigXWebForms) 
			{
				...
				richTextBox.Text=Lan.g(this,"Typed signature in WebForms")+": "+SheetCur.SheetFields[i].DateTimeSig.ToShortDateString()+" "+SheetCur.SheetFields[i].DateTimeSig.ToShortTimeString();
	...
	private void LoadImageOne(SheetField sheetField){
		...
		var filePathAndName=ODFileUtils.CombinePaths(SheetUtil.GetImagePath(),sheetField.FieldName);
	...
	private void LoadImageOnePat(SheetField sheetField){
		...
		if(sheetField.FieldValue.StartsWith("MountNum:")){
			var mountNum=SIn.Long(sheetField.FieldValue.Substring(9));
			sheetField.BitmapLoaded=MountHelper.GetBitmapOfMountFromDb(mountNum);
			return;
		}
		var docNum=SIn.Long(sheetField.FieldValue);
	...
	private void RepositionControls(){
		for(var i=0;i<panelMain.Controls.Count;i++){
			...
			var sheetField=(SheetField)panelMain.Controls[i].Tag;
			var point=new Point(sheetField.XPos,sheetField.YPos);
			panelMain.Controls[i].Location=point;
		}
	}
	...
	private void ResizeTextBox(SheetField sheetField,string fieldValue,ODtextBox textBox=null) { 
		...
		using var font=new Font(sheetField.FontName,sheetField.FontSize,fontstyle);
		using var g=this.CreateGraphics();
		//no need worry about scaling here. Not sure why.
		var sizeF=g.MeasureString(fieldValue,font,sheetField.Width);
		var calcH=(int)sizeF.Height;
		if(calcH<=sheetField.Height){//no growth needed. 
			return;
		}
		//the field height needs to change, so:
		//calculate growth in 96dpi
		var amountOfGrowth=calcH-sheetField.Height;
		sheetField.Height=calcH;
		if(textBox!=null){
			textBox.Height=calcH;
		}
	...
	private void SelectFirstOptionComboBoxes() {
		for(var i=0;i<panelMain.Controls.Count;i++){
			...
			var sheetComboBox=(SheetComboBox)panelMain.Controls[i];
	...
	private string GetTextFromWpfRichTextBox(System.Windows.Controls.RichTextBox richTextBox) {
		...
		var contentEnd=richTextBox.Document.ContentEnd;
		//Read in all of the text that is currently present within the RTB.
		var rangeStartToEnd=new System.Windows.Documents.TextRange(contentStart,contentEnd);
	...
	private bool TryToSaveData() {
		...
		var dateTimeSheet=DateTime.MinValue;
		try {
		...
		var isNewReferralLetterWithProcsOrChart=IsNewReferralLetterWithProcsOrChart();
		if(SheetCur.IsNew) {
		...
		DidChangeSheet=true;
		for(var i=0;i<SheetCur.SheetFields.Count;i++){//Set all sheetfield.SheetNums
		...
		var listSheetFieldsSigBoxes=new List<SheetField>();
		...
		for(var i=0;i<panelMain.Controls.Count;i++){
			...
			var keyData=Sheets.GetSignatureKey(SheetCur);
			var signature=sigBox.GetSignature(keyData);
		...
		if(isNewReferralLetterWithProcsOrChart) {
			...
			if(defNum==0) {
				...
				var def=Defs.GetCatList((int)DefCat.ImageCats).FirstOrDefault(x => !x.IsHidden);
				defNum=def.DefNum;
				ODMessageBox.Show(this,Lan.g(this,"The Image Category Definition \"Letters\" could not be found.  Referral letter saved to:\r\n")+def.ItemName);
			}
			var doc=ImageStore.Import(_tempPdfFile,defNum,patient);
	...
	private void FillFieldsFromScreenings(){
		for(var i=0;i<panelMain.Controls.Count;i++){
			...
			var screenToothChart=(ScreenToothChart)panelMain.Controls[i];
			...
			for(var j=0;j<listUserControlScreenTeeth.Count;j++) {
				if(j > 0) {
	...
	private bool VerifyRequiredFields(){
		...
		for(var i=0;i<_listSignatureBoxWrappers.Count;i++) {
			var sheetField=(SheetField)_listSignatureBoxWrappers[i].Tag;
			if(!sheetField.FieldType.In(SheetFieldType.SigBox,SheetFieldType.SigBoxPractice)){
				continue;
			}
			var sigBox= _listSignatureBoxWrappers[i];
		...
		for(var i=0;i<SheetCur.SheetFields.Count;i++){
			...
			if(SheetCur.SheetFields[i].FieldType==SheetFieldType.InputField){
				var sheetField=SheetCur.SheetFields[i];
				...
				if(sheetField.FieldValue!="X"){//required but this one not checked
					   ...
					   && sheetField.RadioButtonGroup=="") //doesn't belong to a custom group
					{
						...
						var isValueSet=false;//we will be checking to see if at least one in the group has a value
						var numGroupButtons=0;//a count of the buttons in the group
						for(var j=0;j<SheetCur.SheetFields.Count;j++){
							if(SheetCur.SheetFields[j].FieldType!=SheetFieldType.CheckBox){
								continue;//skip everything that's not a checkbox
							}
							var sheetField2=SheetCur.SheetFields[j];
	...
	private void SaveSignaturePayPlan() {
		...
		if((payPlan.IsNew || hasSigChanged) && isValidSig) {
			...
			if(category==0) {
				var listImageCatDefsShort=Defs.GetDefsForCategory(DefCat.ImageCats,true);
				var listImageCatDefsLong=Defs.GetDefsForCategory(DefCat.ImageCats);
			...
			var filePathAndName=PrefC.GetRandomTempFile(".pdf");
	...
	private bool SaveAsDocument(char charUsage,string genericFileName="") {
		...
		var listDefsImageCat=Defs.GetDefsForCategory(DefCat.ImageCats,true).Where(x => x.ItemValue.Contains(charUsage.ToString())).ToList();
		...
		//Jordan 2024-10-07-We have a loop below which can save to multiple image categories.
		//This is wrong. It clearly says in FormDefEditImages that U should be "only one".
		//But maybe someone is already using multiple for some reason, so we won't fix it.
		for(var i=0;i<listDefsImageCat.Count;i++) {
			var documentSave=new Document();
			documentSave.DocNum=Documents.Insert(documentSave);
			var fileName=genericFileName+documentSave.DocNum;
			...
			if(true) {
				var filePath=ImageStore.GetPatientFolder(patient,ImageStore.GetDataFolder());
	...
	private void DeleteSheet(bool doPromptForDelete) {
		...
		if(SheetCur.ShowInTerminal > 0) {
			var message="This form has been sent to be filled on a kiosk.  If you continue, the patient will lose any information that is "
			            +"on their screen.\r\nContinue anyway?";
	...
	private bool OkToSaveBecauseNoOtherEdits() {
		...
		var sheet=Sheets.GetOne(SheetCur.SheetNum);
		if(sheet==null){
			return true;
		}
	...
	private void FixFontsForPdf(Sheet sheetCur,bool isPrinting=false) {
		var errorString=SheetUtil.FixFontsForPdf(sheetCur,isPrinting);
