...
public partial class FormSheetFieldCheckBox:FormODBase {
	...
	private void FormSheetFieldCheckBox_Load(object sender,EventArgs e) {
		...
		var listSheetFieldDefs=SheetFieldsAvailable.GetList(SheetDefCur.SheetType,OutInCheck.Check);
	...
	private void FillListMedical(MedicalListType medListType) {
		var medSelection=SheetFieldDefCur.FieldName.Remove(0,Math.Min(SheetFieldDefCur.FieldName.Length,8));
		...
		switch(medListType) {
			case MedicalListType.allergy:
				...
				for(var i=0; i<listMedical.Items.Count; i++){
	...
	private void listFields_SelectedIndexChanged(object sender,EventArgs e) {
		...
		if(fieldNameSelected=="misc") {
			...
			for(var i=0;i<_listRadioButtonVals.Count;i++) {
	...
	private void AddAllergy(SheetFieldDef SheetFieldDefCur) {
		...
		_listAllergyDefs.Add(allergyDef);
		FillListMedical(MedicalListType.allergy);
	}
	...
	private void AddProblem(SheetFieldDef SheetFieldDefCur) {
		...
		using var formDiseaseDefEdit=new FormDiseaseDefEdit(diseaseDef,isSheetField:true);
	...
	private void butAddProc_Click(object sender,EventArgs e) {
		var listGridCols=new List<GridColumn>();
		...
		listGridCols.Add(col);
		var listProcedureCodesMouth=ProcedureCodes.GetProcCodesByTreatmentArea(false,TreatmentArea.Mouth,TreatmentArea.None)
			.OrderBy(x => x.ProcCode).ToList();
		var listGridRows=new List<GridRow>();
		for(var i=0;i<listProcedureCodesMouth.Count;i++){
			var row=new GridRow (listProcedureCodesMouth[i].ProcCode,listProcedureCodesMouth[i].AbbrDesc,listProcedureCodesMouth[i].Descript);
			row.Tag=listProcedureCodesMouth[i];
			listGridRows.Add(row);
		}
		using var formGridSelection=new FormGridSelection(listGridCols,listGridRows,"Add Procedure","Procedures");
		if(formGridSelection.ShowDialog()!=DialogResult.OK) {
			return;
		}
		for(var i=0;i<formGridSelection.ListSelectedTags.Count;i++){
			var fieldName="Proc:"+((ProcedureCode)formGridSelection.ListSelectedTags[i]).ProcCode;
	...
	private void SaveAndClose(){
		...
		var updateGroupCaptionForFieldName=new Action<string>((caption) => {
			SheetFieldDefCur.UiLabelMobile=caption;
			var listSheetFieldDefs=SheetDefCur.SheetFieldDefs.FindAll(x =>
				x.FieldType==SheetFieldType.CheckBox &&
				!string.IsNullOrEmpty(x.FieldName) &&
		...
		if(groupRadio.Visible) {
			...
			//All items with this group name get this UiLabelMobile.
			var listSheetFieldDefs2=SheetDefCur.SheetFieldDefs.FindAll(x =>
				x.FieldType==SheetFieldType.CheckBox &&
				!string.IsNullOrEmpty(x.RadioButtonGroup) &&
				x.RadioButtonGroup==SheetFieldDefCur.RadioButtonGroup &&
				x.Language==SheetFieldDefCur.Language);
			for(var i=0;i<listSheetFieldDefs2.Count;i++){
