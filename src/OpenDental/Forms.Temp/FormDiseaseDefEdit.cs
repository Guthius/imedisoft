...
public partial class FormDiseaseDefEdit : FormODBase {
	...
	private bool _isSheetField;

		
	public FormDiseaseDefEdit(DiseaseDef diseaseDef,bool isSheetField=false)
	{
		//
	...
	private void butSnomed_Click(object sender,EventArgs e) {
		...
		if(DiseaseDefs.ContainsSnomed(formSnomeds.SnomedSelected.SnomedCode,DiseaseDefCur.DiseaseDefNum)) {//DiseaseDefNum could be zero
			MsgBox.Show(this,"Snomed code already exists in the problems list.");
			return;
		}
		DiseaseDefCur.SnomedCode=formSnomeds.SnomedSelected.SnomedCode;
		var snomedDescription=Snomeds.GetCodeAndDescription(formSnomeds.SnomedSelected.SnomedCode);
		textSnomed.Text=snomedDescription;
		if(snomedDescription=="") {
	...
	private void butDelete_Click(object sender,EventArgs e) {
		...
		if(DiseaseDefs.IsDiseaseDefInUse(DiseaseDefCur.DiseaseDefNum)) {
