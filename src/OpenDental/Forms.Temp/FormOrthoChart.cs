...
public partial class FormOrthoChart:FormODBase {
	...
	private List<DateTime_CanEdit> _listDateTime_CanEdits= [];
	...
	private void FormOrthoChart_Load(object sender,EventArgs e) {
		...
		var dateTime=DateTime.Today.AddDays(GroupPermissions.NewerDaysMax);
		...
		if(_isLockedByMe){//we will usually have control
			var orthoChartRowToday=_listOrthoChartRows.FirstOrDefault(x=>x.DateTimeService.Date==DateTime.Today.Date);
	...
	private void FillLockedInitial(){
		...
		if(_userNumLocked==0){
			...
			OrthoChartLogs.Log("FillLockedInitial(), _userNumLocked="+_userNumLocked+" so I am locked out.",Environment.MachineName,_patient.PatNum,_userodCurUser.UserNum);
	...
	private void timerLocked_Tick(object sender, EventArgs e){
		...
		var userNum=PatientNotes.GetUserNumOrthoLocked(_patient.PatNum);
		...
		OrthoChartLogs.Log("timerLocked_Tick(), _userNum="+userNum+", indicating that that user just took control.",Environment.MachineName,_patient.PatNum,_userodCurUser.UserNum);
		_userNumLogging=userNum;
	...
	private void buttonTakeControl_Click(object sender, EventArgs e){
		...
		if(userNum==-5) {//We are taking control from a workstation that changed users to sign an orthochartrow.
			OrthoChartLogs.Log("buttonTakeControl_Click(), userNum=-5, so we are taking control from a workstation that changed users to sign an orthochartrow. Changing userNum to "+_userNumLocked,
		...
		if(userNum!=_userNumLocked){
			OrthoChartLogs.Log("buttonTakeControl_Click(), userNum!=_userNumLocked, so someone else (usernum "+userNum+") grabbed control in the last few seconds, so we don't want to immediately grab it.",
				Environment.MachineName,_patient.PatNum,_userodCurUser.UserNum);
			...
			var name=Userods.GetName(userNum);
		...
		//take control
		PatientNotes.SetUserNumOrthoLocked(_patient.PatNum,_userodCurUser.UserNum);
		OrthoChartLogs.Log("buttonTakeControl_Click(), Took control from userNum "+_userNumLocked,Environment.MachineName,_patient.PatNum,_userodCurUser.UserNum);
		_userNumLocked=_userodCurUser.UserNum;			
	...
	private void FillTabs() {
		...
		for(var i=0;i<_listOrthoChartTabs.Count;i++) {
			var tabPage=new OpenDental.UI.TabPage(_listOrthoChartTabs[i].TabName);
			tabPage.Tag=_listOrthoChartTabs[i];
			LayoutManagerForms.Add(tabPage,tabControl);
	...
	private void FillDisplayFields() {
		...
		_listDisplayFieldNames= [];
		for(var i=0;i<_listDisplayFieldsOrth.Count;i++) {
...
//todo: there was a warning on this method that I do not understand: Do not call unless you have saved changes to database first.
	private void FillGrid() {
		...
		var orthoChartTab=_listOrthoChartTabs[tabControl.SelectedIndex];
		...
		var gridMainScrollValue=gridMain.ScrollValue;
		...
		for(var i=0;i<listDisplayFields.Count;i++) {
			...
			if(!string.IsNullOrEmpty(listDisplayFields[i].PickList)) {
				var listComboOption=listDisplayFields[i].PickList.Split(["\r\n"],StringSplitOptions.None).ToList();
		...
		GridRow row;
		var dateTimeFrom=_dateTimeFrom;
		var dateTimeTo=_dateTimeTo;
		...
		for(var r=0;r<_listOrthoChartRows.Count;r++){
			...
			var dateTimeService=_listOrthoChartRows[r].DateTimeService.ToShortDateString();
			...
			for(var i=0;i<listDisplayFields.Count;i++) {
				...
				if(listDisplayFields[i].InternalName=="Signature") {
					...
					var orthoChart=_listOrthoChartRows[r].ListOrthoCharts.FirstOrDefault(x=>x.FieldName==listDisplayFields[i].Description);
					if(orthoChart!=null) {
				...
				if(!string.IsNullOrEmpty(listDisplayFields[i].PickList)) {
					var listComboOption=listDisplayFields[i].PickList.Split(["\r\n"],StringSplitOptions.None).ToList();
					var cell=new GridCell(cellValue);
	...
	private void FillOrtho() {
		...
		var row = new GridRow();
		var dateFirstOrthoProc = Procedures.GetFirstOrthoProcDate(_patientNote);
	...
	private void ClearSignature(int idxRow) {
		var orthoChartRow=(OrthoChartRow)gridMain.ListGridRows[idxRow].Tag;
	...
	private void gridMain_CellLeave(object sender,ODGridClickEventArgs e) {
		var orthoChartRow=(OrthoChartRow)gridMain.ListGridRows[e.Row].Tag;
		//Get the date for the ortho chart that was just edited.
		var dateTime=orthoChartRow.DateTimeService;
		var provNum=orthoChartRow.ProvNum;
		var oldText=GetValueFromList(orthoChartRow,(string)gridMain.Columns[e.Col].Tag);
		var newText=gridMain.ListGridRows[e.Row].Cells[e.Col].Text;
		Logger.LogAction(() => {
			if(CanEditRow(dateTime)) {
				if(newText != oldText) {
					...
					if(_showSigBox) {
						Logger.LogAction(() => { signatureBoxWrapper.ClearSignature(false); });
					}
	...
	private void LogOrthoChartsUsedForSigHash(List<OrthoChart> listOrthoCharts,bool isValidating,bool isForPatNum) {
		var logMsg=$"OrthoCharts used for sig has.  OrthoCharts Count: {listOrthoCharts.Count} values: {string.Join(",",listOrthoCharts.Select(y => $"FieldName={y.FieldName} FieldValue={y.FieldValue}"))}{Environment.NewLine}IsValidating: {isValidating} IsForPatNum: {isForPatNum}";
		OrthoChartLogs.Log(logMsg,Environment.MachineName,_patient.PatNum,_userodCurUser.UserNum);
	}
	...
	private void DisplaySignature(int idxRow,bool hasRefresh=true) {
		Logger.LogAction(() => {
			...
			var orthoChartRow=(OrthoChartRow)gridMain.ListGridRows[idxRow].Tag;
			var dateTime=orthoChartRow.DateTimeService;
			var listOrthoCharts=orthoChartRow.ListOrthoCharts;
			var orthoSignature=new OrthoSignature(orthoChartRow.Signature);
			...
			var sigColumnName="";
			var displayFieldSig=_listDisplayFieldsOrth.Find(x => x.InternalName=="Signature");
			if(displayFieldSig!=null){
				sigColumnName=displayFieldSig.Description;
			}
			var listOrthoChartsForSigHash=listOrthoCharts.FindAll(x => x.FieldValue!="" && x.FieldName!=sigColumnName);
			LogOrthoChartsUsedForSigHash(listOrthoChartsForSigHash,isValidating:true, isForPatNum:false);
			var keyData=OrthoCharts.GetKeyDataForSignatureHash(_patient,listOrthoChartsForSigHash,dateTime);
			Logger.LogAction(() => {
			...
			if(signatureBoxWrapper.IsValid) {
				...
				if(_sigColIdx > 0) {//User might be viewing a tab that does not have the signature column.  Greater than 0 because index 0 is a Date column.
					...
					if(orthoSignature.SigString=="") {
						...
						var userNum=orthoChartRow.UserNum;
	...
	private void SetSignatureButtonVisibility(int idxRow) {
		var dateTime=((OrthoChartRow)gridMain.ListGridRows[idxRow].Tag).DateTimeService;
	...
	private void SaveSignatureToDict(int idxRow) {
		...
		string keyData;
		//Get the "translated" name for the signature column.
		var sigColumnName=_listDisplayFieldsOrth.FirstOrDefault(x => x.InternalName=="Signature")?.Description??"";
		var listOrthoCharts=orthoChartRow.ListOrthoCharts.FindAll(x => x.FieldName!=sigColumnName && x.FieldValue!="");
		...
		var orthoSignature=new OrthoSignature();
	...
	private bool CanEditRow(DateTime dateTime) {
		...
		var dateTime_CanEdit=new DateTime_CanEdit();
		...
		var orthoChartRow=gridMain.GetTags<OrthoChartRow>().FirstOrDefault(x => x.DateTimeService==dateTime);
	...
	private string GetValueFromList(OrthoChartRow orthoChartRow,string columnHeading) {
		...
		var orthoChart=orthoChartRow.ListOrthoCharts.FirstOrDefault(x=>x.FieldName==columnHeading);
	...
	private void gridMain_CellSelectionCommitted(object sender,ODGridClickEventArgs e) {
		var dateTime=((OrthoChartRow)gridMain.ListGridRows[gridMain.SelectedCell.Y].Tag).DateTimeService;
		if(!CanEditRow(dateTime)) {
			return;
		}
		_hasChanged=true;
	...
	private void butAdd_Click(object sender,EventArgs e) {
		var frmOrthoChartAddDate=new FrmOrthoChartAddDate();
	...
	private void butUseAutoNote_Click(object sender,EventArgs e) {
		...
		if(frmAutoNoteCompose.IsDialogOK) {
			var pointSelectedCell=new Point(gridMain.SelectedCell.X,gridMain.SelectedCell.Y);
	...
	private void butChangeUser_Click(object sender,EventArgs e) {
		var idxRow=gridMain.SelectedCell.Y;
		if(idxRow==-1) {
			MsgBox.Show(this,"Please select a ortho chart first.");//This shouldn't happen.
			return;
		}
		var date=((OrthoChartRow)gridMain.ListGridRows[idxRow].Tag).DateTimeService;
		if(!CanEditRow(date)) {
			MsgBox.Show(this,"You need either Ortho Chart Edit (full) or Ortho Chart Edit (same user, signed) to edit this ortho chart.");
			return;
		}
		using var formLogOnChangeUser=new FormLogOn(isSimpleSwitch:true);
		formLogOnChangeUser.ShowDialog();
		if(formLogOnChangeUser.DialogResult!=DialogResult.OK) {
			return;
		}
		var userNumOld=_userodCurUser.UserNum;
		_userodCurUser=formLogOnChangeUser.UserodSimpleSwitch;
		var userNumNew=_userodCurUser.UserNum;//_curUser.UserNum is now the changed user.
		EnableSignatureBox(_userodCurUser);
	...
	private void EnableSignatureBox(Userod userodCurUser) {
		var canUserSignNote=Userods.CanUserSignNote(userodCurUser);//only show if user can sign
	...
	private void butAudit_Click(object sender,EventArgs e) {
		...
		try {
			securityLogArrayOrthoChartLogs=SecurityLogs.Refresh(_patient.PatNum, [EnumPermType.OrthoChartEditFull],null);
		...
		var dictDatesOfServiceLogEntries=new SortedDictionary<DateTime,List<SecurityLog>>();
		//Add all dates from grid first, some may not have audit trail entries, but should be selectable from FormAO
		for(var i=0;i<gridMain.ListGridRows.Count;i++) {
			var date=((OrthoChartRow)gridMain.ListGridRows[i].Tag).DateTimeService;
			if(dictDatesOfServiceLogEntries.ContainsKey(date)) {
				continue;
			}
			dictDatesOfServiceLogEntries.Add(date, []);
		}
		//Add Ortho Audit Trail Entries
		for(var i=0;i<securityLogArrayOrthoChartLogs.Length;i++) {
			var dateCur=OrthoCharts.GetOrthoDateFromLog(securityLogArrayOrthoChartLogs[i]);
			if(!dictDatesOfServiceLogEntries.ContainsKey(dateCur)) {
				dictDatesOfServiceLogEntries.Add(dateCur, []);
			}
			dictDatesOfServiceLogEntries[dateCur].Add(securityLogArrayOrthoChartLogs[i]);//add entry to existing list.
		}
		using var formAuditOrtho=new FormAuditOrtho();
	...
	private void butDelete_Click(object sender,EventArgs e) {
		var orthoChartRowSelected=gridMain.SelectedTag<OrthoChartRow>();
		...
		var listDisplayFields=GetDisplayFieldsForCurrentTab();
		var listFieldNamesToDelete=listDisplayFields.Select(x => x.Description).ToList();
	...
	private void butPrint_Click(object sender,EventArgs e) {
		...
		try {
			...
			if(/* ODBuild.IsDebug() */ false) {
				using var formRpPrintPreview = new FormRpPrintPreview(printDocument);
	...
	private void pd_PrintPage(object sender,System.Drawing.Printing.PrintPageEventArgs e) {
		...
		#region printHeading
		//========== TITLE ==========
		var title=_listOrthoChartTabs[0].TabName;
	...
	private void SaveToDb(){
		...
		var listOrthoChartsNew=new List<OrthoChart>();
		var listOrthChartRowNumsToDelete=new List<long>();
		for(var i=0;i<_listOrthoChartRows.Count;i++){
		...
		var listOrthoChartRowsFiltered=_listOrthoChartRows.FindAll(x => !listOrthChartRowNumsToDelete.Contains(x.OrthoChartRowNum));
