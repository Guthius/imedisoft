...
using Deposit = Imedisoft.Core.Entities.Deposit;
...
public partial class FormDepositEdit:FormODBase {
	...
	private List<long> _listPayNumsAttached= [];
	...
	private void FormDepositEdit_Load(object sender,System.EventArgs e) {
		...
		if(IsNew) {
			...
			_listPayTypeDefNums= [];
			for(var i=0;i<listDefsPaymentTypes.Count;i++) {
			...
			_listInsPayTypeDefNums= [];
			for(var i=0;i<listDefsInsurancePaymentTypes.Count;i++) {
			...
			if(Accounts.DepositsLinked() && !IsQuickBooks()) {
				...
				for(var i=0;i<_listDepositAccounts.Count;i++) {
			...
			var transaction=Transactions.GetAttachedToDeposit(_depositCur.DepositNum);
			if(transaction==null) {
				labelDepositAccount.Visible=false;
				comboDepositAccount.Visible=false;
	...
	private void FillClassRefs() {
		var isQBAndClassRefsEnabled=IsQuickBooks() && PrefC.GetBool(PrefName.QuickBooksClassRefsEnabled);
		if(!isQBAndClassRefsEnabled){//Neither QB nor QBO and their respective class refs feature is on, nothing to fill.
			return;
		}
		if(!IsNew) {
		...
		for(var i=0;i<listClassNames.Count;i++) {
			if(listClassNames[i]=="") {
				continue;
			}
	...
	private void FillGrids(){
		if(IsNew){
			var dateTimeStart=SIn.Date(textDateStart.Text);
			long clinicNum=0;
			if(!comboClinic.IsAllSelected){
				clinicNum=comboClinic.ClinicNumSelected;
			}
			var listPayTypes=new List<long>();//[listPayType.SelectedIndices.Count];
			for(var i=0;i<listPayType.SelectedIndices.Count;i++) {
				listPayTypes.Add(_listPayTypeDefNums[listPayType.SelectedIndices[i]]);
			}
			var listInsPayTypes=new List<long>();
			for(var i=0;i<listInsPayType.SelectedIndices.Count;i++) {
		...
		for(var i=0;i<_listPaymentsPat.Count;i++){
			listPatNums.Add(_listPaymentsPat[i].PatNum);
		}
		var patientArray=Patients.GetMultPats(listPatNums);
		gridPat.BeginUpdate();
		gridPat.Columns.Clear();
		var col=new GridColumn(Lan.g("TableDepositSlipPat","Date"),80);
		...
		for(var i=0;i<_listClaimPayments.Count;i++){
			row=new OpenDental.UI.GridRow();
	...
	private void ComputeAmt(){
		...
		decimal amount=0;
		for(var i=0;i<gridPat.SelectedIndices.Length;i++){
			amount+=(decimal)_listPaymentsPat[gridPat.SelectedIndices[i]].PayAmt;
		}
		for(var i=0;i<gridIns.SelectedIndices.Length;i++){
	...
	private void Search() {
		...
		for(var i=0;i<gridIns.ListGridRows.Count;i++) {
			var isBold=false;
		...
		gridIns.Invalidate();
		var isScrollSetPat=false;
		for(var i=0;i<gridPat.ListGridRows.Count;i++) {
			var isBold=false;
	...
	private bool CreateDepositQB(bool allowContinue) {
		using var formQBAccountSelect=new FormQBAccountSelect(false);
	...
	private bool SaveToDB(){
		...
		//Prevent backdating----------------------------------------------------------------------------------------
		var dateTime=SIn.Date(textDate.Text);
		//We enforce security here based on date displayed, not date entered
		...
		var listSelectedPayNums=gridPat.SelectedIndices.OfType<int>().Select(x => _listPaymentsPat[x].PayNum).ToList();
		if(listSelectedPayNums.Count>0) {
			var countAlreadyAttached=Payments.GetCountAttachedToDeposit(listSelectedPayNums,_depositCur.DepositNum);//Depositnum might be 0
		...
		var listSelectedClaimPaymentNums=gridIns.SelectedIndices.OfType<int>().Select(x => _listClaimPayments[x].ClaimPaymentNum).ToList();
		if(listSelectedClaimPaymentNums.Count>0) {
			var countAlreadyAttached=ClaimPayments.GetCountAttachedToDeposit(listSelectedClaimPaymentNums,_depositCur.DepositNum);//Depositnum might be 0
			if(countAlreadyAttached>0) {
		...
		if(IsNew && !_hasBeenSavedToDB){
			...
			var hasDepositInfo=Accounts.DepositsLinked();
			var isQbOrQbo=IsQuickBooks() && !false;
		...
		for(var i=0;i<gridPat.SelectedIndices.Length;i++){
			var paymentSelected=_listPaymentsPat[gridPat.SelectedIndices[i]];
		...
		for(var i=0;i<gridIns.SelectedIndices.Length;i++){
			var claimPaymentSelected=_listClaimPayments[gridIns.SelectedIndices[i]];
		...
		if(!_isOnOKClick) {
			...
			var listPayNumsDeselected=_listPayNumsAttached.Except(listSelectedPayNums).ToList();
			var listClaimPaymentNumsDelected=_listClaimPaymentNumsAttached.Except(listSelectedClaimPaymentNums).ToList();
	...
	private void butSendQB_Click(object sender,EventArgs e) {
		if(false && IsQuickBooks()) {
			MsgBox.Show(this,"QuickBooks is not available while viewing through the web.");
			return;
		}
		var dateTime=SIn.Date(textDate.Text);//We use security on the date showing.
		if(!Security.IsAuthorized(EnumPermType.DepositSlips,dateTime)) {
			return;
		}
		_depositCur.DateDeposit=dateTime;
	...
	private Sheet CreateAndFillSheet() {
		...
		var listSheetDefsDeposit=SheetDefs.GetCustomForType(SheetTypeEnum.DepositSlip);
		...
		var sheet=SheetUtil.CreateSheet(sheetDef,0);//Does not insert.
	...
	private void butEmailPDF_Click(object sender,EventArgs e) {
		...
		var emailAddress=EmailAddresses.GetByClinic(Clinics.ClinicNum);
	...
	private void butRefresh_Click(object sender, System.EventArgs e) {
		...
		if(listInsPayType.SelectedIndices.Count==0 && listPayType.SelectedIndices.Count==0) {
			for(var i=0;i<listInsPayType.Items.Count;i++) {
				listInsPayType.SetSelected(i,true);
			}
			for(var j=0;j<listPayType.Items.Count;j++) {
	...
	private void butSave_Click(object sender, System.EventArgs e) {
		...
		var meetsDepositCriteria=Accounts.DepositsLinked() && _depositCur.Amount>0;
		var isQbOrQbo=IsQuickBooks();
