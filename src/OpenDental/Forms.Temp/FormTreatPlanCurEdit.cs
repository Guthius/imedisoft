...
public partial class FormTreatPlanCurEdit:FormODBase {
	...
	private void FormTreatPlanCurEdit_Load(object sender,EventArgs e) {
		...
		this.Text=TreatPlanCur.Heading+" - {"+Lans.g(TreatPlanCur.TPStatus.ToString())+"}";
		...
		_listProceduresAll=Procedures.GetProcsByStatusForPat(TreatPlanCur.PatNum, ProcStat.TP, ProcStat.TPi);
	...
	private void FillGrid(ref GridOD grid) {
		...
		for(var i = 0;i<listProcedures.Count;i++) {
			...
			var description=ProcedureCodes.GetLaymanTerm(listProcedures[i].CodeNum);
			...
			for(var j = 0;j<listAptNums.Count;j++) {
				var appointment =_listAppointments.Find(x => x.AptNum==listAptNums[j]);
				if(appointment!=null) {
	...
	private void SelectLabFees(ref GridOD grid) {
		var listProcNumsSelected=grid.SelectedTags<Procedure>().Select(x => x.ProcNum).ToList();
		...
		var listProceduresAll=grid.GetTags<Procedure>();
		for(var i=0;i < listProceduresAll.Count;i++) {
	...
	private void butLeft_Click(object sender,EventArgs e) {
		...
		for(var i = 0;i<gridAll.SelectedIndices.Length;i++) {
			var procedure =(Procedure)gridAll.ListGridRows[gridAll.SelectedIndices[i]].Tag;
			if(procedure==null) {
				continue;
			}
	...
	private void butRight_Click(object sender,EventArgs e) {
		...
		var hasMsgShow=false;
		for(var i = 0;i<gridTP.SelectedIndices.Length;i++) {
			var procedure =(Procedure)gridTP.ListGridRows[gridTP.SelectedIndices[i]].Tag;
			if(procedure==null || (TreatPlanCur.TPStatus==TreatPlanStatus.Active && (procedure.AptNum>0 || procedure.PlannedAptNum>0))) {
				...
				if(!hasMsgShow) {//Only show msgbox once.
					var msg="Not allowed to move procedures to Available Procedures that are attached to an Appointment.";
	...
	private void grids_MouseDown(object sender,MouseEventArgs e) {
		...
		var grid=(GridOD)sender;
		var row=grid.PointToRow(e.Y);
		if(row<0 || row>=grid.ListGridRows.Count) {
			return;
		}
		var procedure=(Procedure)grid.ListGridRows[row].Tag;
		if(procedure==null) {
