...
public partial class FormPerioSetup:FormODBase {
	...
	private void FormPerioSetup_Load(object sender,EventArgs e) {
		FillTextBoxes();
		if(PrefC.GetBool(PrefName.PerioRecessionInsteadOfGM)){
			radioRecession.Checked=true;
		}
		else{
			radioGingMarg.Checked=true;
		}
	}

	private void FillTextBoxes() {
		var rawPerioMeasures=PrefC.GetString(PrefName.PerioDefaultProbeDepths);
	...
	private string GetProbingDepthsWithCommas(string probingDepthsAll) {
		var retVal="";
		for(var i=0;i<probingDepthsAll.Length;i++) {
	...
	private void butSave_Click(object sender,EventArgs e) {
		...
		var changed=false;
		var formattedPerioMesureRow=textF1.Text.Replace(",","");
		...
		changed |=Prefs.UpdateString(PrefName.PerioDefaultProbeDepths,formattedPerioMesureRow);
		changed |=Prefs.UpdateBool(PrefName.PerioRecessionInsteadOfGM, radioRecession.Checked);
		if(changed){
