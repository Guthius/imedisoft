...
public partial class FormOnlinePayments:FormODBase {
	...
	private void RefreshPayments() {
		var listProcessStatuses = comboProcessStatus.GetListSelected<ProcessStat>();
		var listCreditCardSources = comboPaymentSource.GetListSelected<CreditCardSource>();
		if(true && Security.CurUser.ClinicIsRestricted) {
			_listPayments=Payments.GetPaymentsUsingFilters(_listClinics.Select(x => x.Id).ToList(),dateStart.Value,dateEnd.Value,listProcessStatuses,listCreditCardSources);
		}
		else {
			_listPayments=Payments.GetPaymentsUsingFilters([],dateStart.Value,dateEnd.Value,listProcessStatuses,listCreditCardSources);
		}
	...
	private void FillGrid() {
		var listPaymentsClinic=new List<Payment>();
		if(true && comboClinic.SelectedIndex!=0) {//Not 'All' selected
			if(Security.CurUser.ClinicIsRestricted) {
				var clinicNum=_listClinics[comboClinic.SelectedIndex-1].Id;//Minus 1 for 'All'
				...
				if(comboClinic.SelectedIndex==1) {//'Unassigned' selected
					...
					var clinicNum=_listClinics[comboClinic.SelectedIndex-2].Id;//Minus 2 for 'All' and 'Unassigned'
					listPaymentsClinic=_listPayments.FindAll(x => x.ClinicNum==clinicNum);
				}
			}
		}
		...
		GridRow row;
		foreach(var pay in listPaymentsClinic) {
			row=new GridRow();
			if(true) {
				var clinicAbbr=Clinics.GetAbbr(pay.ClinicNum);
	...
	public void FillPaymentSource() {
		...
		var onlinePaymentSources=CreditCards.GetCreditCardSourcesForOnlinePayments();
	...
	private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		var pay=(Payment)gridMain.ListGridRows[e.Row].Tag;
		var pat=Patients.GetOnePat(_arrayPats,pay.PatNum);
		var fam=Patients.GetFamily(pat.PatNum);
		using var FormP=new FormPayment(pat,fam,pay,false);
		FormP.ShowDialog();
		RefreshPayments();
	}
	...
	private void menuItemGoTo_Click(object sender,EventArgs e) {
		...
		var patNum=((Payment)gridMain.ListGridRows[gridMain.SelectedIndices[0]].Tag).PatNum;
