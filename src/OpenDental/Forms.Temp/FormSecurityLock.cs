...
public partial class FormSecurityLock:FormODBase {
	...
	private string CreateSecurityLog(string textDateOld,string textDaysOld,int days,bool includesAdminOld) {
		...
		var textDateOldCopy=textDateOld;
	...
	private void butSave_Click(object sender, System.EventArgs e) {
		...
		var textDateOld=PrefC.GetDate(PrefName.SecurityLockDate).ToShortDateString();
		var textDaysOld=PrefC.GetInt(PrefName.SecurityLockDays).ToString();
		var includesAdminOld=PrefC.GetBool(PrefName.SecurityLockIncludesAdmin);
		   ...
		   | Prefs.UpdateBool(PrefName.SecurityLockIncludesAdmin,checkAdmin.Checked)  )
		{
			...
			var log=CreateSecurityLog(textDateOld,textDaysOld,days,includesAdminOld);
			SecurityLogs.MakeLogEntry(EnumPermType.SecurityGlobal,0,log);
		}
