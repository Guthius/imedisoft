...
public partial class FormFeeSchedTools :FormODBase {
	...
	private void FormFeeSchedTools_Load(object sender, System.EventArgs e) {
		...
		comboGroup.Location=new Point(129,46);
		comboGroupTo.Location=new Point(129,45);
		FillComboBoxes();
		if(!CultureInfo.CurrentCulture.Name.EndsWith("CA")) {//Canadian. en-CA or fr-CA
			butImportCanada.Visible=false;
		}
		if(!Fees.IsUsingEffectiveDate()) {
			odDatePickerEffectiveDateSelected.SetDateTime(DateTime.MinValue);
			odDatePickerEffectiveDateCopy.SetDateTime(DateTime.MinValue);
			odDatePickerEffectiveDateImportExport.SetDateTime(DateTime.MinValue);
			odDatePickerEffectiveDateIncrease.SetDateTime(DateTime.MinValue);
		}
	}
	...
	private void FillComboBoxes() {
		...
		var comboProvIdx=comboProvider.SelectedIndex;
		var comboProvToIdx=comboProviderTo.SelectedIndex;
	...
	private void FillFeeSchedGroupComboBox(UI.ComboBox comboFeeSchedGroup,long feeSchedNumSelected,long feeSchedGroupNum) {
		var listComboGroupNums=new List<long>();
		var listFeeSchedGroups=FeeSchedGroups.GetAllForFeeSched(feeSchedNumSelected);
		for(var i=0;i<listFeeSchedGroups.Count;i++)  {
			if(!listComboGroupNums.Contains(listFeeSchedGroups[i].FeeSchedGroupNum) && listFeeSchedGroups[i].ListClinicNumsAll.Count>0) {//Skip duplicate/empty groups.
				comboFeeSchedGroup.Items.Add(listFeeSchedGroups[i].Description,listFeeSchedGroups[i]);
	...
	private void butClear_Click(object sender, System.EventArgs e) {
		if(true) {
			if(!MsgBox.Show(this,MsgBoxButtons.OKCancel,"This will clear all values from the selected fee schedule for the currently selected clinic, provider, and effective date.  Are you sure you want to continue?")) {
				return;
			}
		}
		...
		if(PrefC.GetBool(PrefName.ShowFeeSchedGroups) && !checkShowGroups.Checked) {
			var feeSchedGroup=FeeSchedGroups.GetOneForFeeSchedAndClinic(feeSchedNum,comboClinic.ClinicNumSelected);//get the selected clinic num
			if(feeSchedGroup!=null) {
				MsgBox.Show(Lans.g("Selected clinic is a member of Fee Schedule Group: ")+feeSchedGroup.Description
				                                                                         +Lans.g(" and must be cleared at the group level."));
				return;
			}
		}
		...
		for(var i=0; i<listClinicNums.Count;i++)  {
			Fees.DeleteFees(feeSchedNum,listClinicNums[i],provNum,odDatePickerEffectiveDateSelected.GetDateTime());
			...
			if(odDatePickerEffectiveDateSelected.GetDateTime()!=DateTime.MinValue) {
				logText+=Lan.g(this," with the most recent effective date");
			}
	...
	private void butCopy_Click(object sender, System.EventArgs e) {
		var listClinicNumsTo=comboClinicTo.ListClinicNumsSelected;
		if(checkShowGroups.Checked) {
		...
		if(true && listClinicNumsTo.Count==0) {
		...
		var feeSched=_listFeeScheds[comboFeeSched.SelectedIndex];
		if(feeSched.FeeSchedNum==feeSchedTo.FeeSchedNum && fromProvNum==toProvNum) {
			if(!true || listClinicNumsTo.Contains(fromClinicNum)) {//If clinics disabled, can cause false negative so shortcircuit
		...
		if(PrefC.GetBool(PrefName.ShowFeeSchedGroups) && !checkShowGroups.Checked) {
			...
			for(var i=0;i<listClinicNumsTo.Count;i++) {
				var feeSchedGroup=FeeSchedGroups.GetOneForFeeSchedAndClinic(feeSchedTo.FeeSchedNum,listClinicNumsTo[i]);
				if(feeSchedGroup!=null) {
					var clinic=Clinics.GetClinic(listClinicNumsTo[i]);
					MsgBox.Show(this,Lans.g("Clinic: ")+clinic.Abbr+Lans.g(" is a member of Fee Schedule Group: ")+feeSchedGroup.Description
					                 +Lans.g(" for the selected Fee Schedule and must be copied at the Fee Schedule Group level."));
					return;
				}
			}
		}
		if(!MsgBox.Show(this,MsgBoxButtons.OKCancel,"All fees that exactly match the \"Copy To\" fee schedule/clinic/provider/effective date combination will be deleted.  Then new fees will be copied in.  Are you sure you want to continue?")){
			return;
		}
		var progressOD=new ProgressWin();
		progressOD.ActionMain=() => FeeScheds.CopyFeeSchedule(
			feeSched,fromClinicNum,fromProvNum,feeSchedTo,listClinicNumsTo,toProvNum,
			odDatePickerEffectiveDateSelected.GetDateTime(),odDatePickerEffectiveDateCopy.GetDateTime());
	...
	private void butIncrease_Click(object sender, System.EventArgs e) {
		...
		if(!MsgBox.Show(this,MsgBoxButtons.OKCancel,"This will overwrite all values of the selected fee schedule/clinic/provider/effective date combo.  Previously entered fee "
		...
		var feeSchedNum=_listFeeScheds[comboFeeSched.SelectedIndex].FeeSchedNum;
		if(PrefC.GetBool(PrefName.ShowFeeSchedGroups) && !checkShowGroups.Checked) {
			var feeSchedGroup=FeeSchedGroups.GetOneForFeeSchedAndClinic(feeSchedNum,clinicNum);
			if(feeSchedGroup!=null) {
				MsgBox.Show(Lans.g("Selected clinic is a member of Fee Schedule Group: ")+feeSchedGroup.Description
				                                                                         +Lans.g(" and must be increased at the group level."));
				return;
			}
		}
		if(checkShowGroups.Checked) {
		...
		progressOD.ActionMain=() => {
			listFees=Fees.IncreaseNew(percent,round,listFees,odDatePickerEffectiveDateIncrease.GetDateTime());
			...
			for(var i=0;i<listFees.Count;i++) {
				...
				var logText="Procedure: "+procCode+", "
	...
	private bool EvaluateOverrides(long clinicNum,long provNum,long feeSchedNum,List<Fee> listFees) {
		...
		var countTotalFeesForSched=Fees.GetCountByFeeSchedNum(feeSchedNum);
		var msgText="";
		var clinicName=Clinics.GetAbbr(clinicNum);
		var provName=Providers.GetAbbr(provNum);
		var feeSchedDesc=_listFeeScheds.FirstOrDefault(x => x.FeeSchedNum==feeSchedNum).Description;
	...
	private void butExport_Click(object sender,EventArgs e) {
		var feeSchedNum=_listFeeScheds[comboFeeSched.SelectedIndex].FeeSchedNum;
		var feeSchedDesc=FeeScheds.GetDescription(feeSchedNum);
		...
		var progressOD=new ProgressWin();
		progressOD.ActionMain=() => FeeScheds.ExportFeeSchedule(feeSched.FeeSchedNum,clinicNum,provNum,filePath,odDatePickerEffectiveDateImportExport.GetDateTime());
	...
	private void butImport_Click(object sender,EventArgs e) {
		if(!MsgBox.Show(this,MsgBoxButtons.OKCancel,"If you want a clean slate, you should clear the current fee schedule first.  When imported, any fees that are found in the text file will overwrite values of the selected fee schedule/clinic/provider/effective date combo.  Are you sure you want to continue?"))
		{
			return;
		}
		...
		var feeSched=_listFeeScheds[comboFeeSched.SelectedIndex];
		if(PrefC.GetBool(PrefName.ShowFeeSchedGroups) && !checkShowGroups.Checked) {
			var feeSchedGroup=FeeSchedGroups.GetOneForFeeSchedAndClinic(feeSched.FeeSchedNum,clinicNum);
			if(feeSchedGroup!=null) {
				MsgBox.Show(Lans.g("Selected clinic is a member of Fee Schedule Group: ")+feeSchedGroup.Description
				                                                                         +Lans.g(" and must be imported at the group level."));
				return;
			}
		}
		if(checkShowGroups.Checked) {
		...
		var frmFeeSchedNoteEdit=new FrmFeeSchedNoteEdit();
		var feeSchedNote=new FeeSchedNote();
		feeSchedNote.IsNew=true;
		feeSchedNote.DateEntry=DateTime.Today;
		if(checkShowGroups.Checked){
			feeSchedNote.ClinicNums=comboGroup.GetStringSelectedItems();
			feeSchedNote.ListClinicNums=comboGroup.GetSelected<FeeSchedGroup>().ListClinicNumsAll;
		}
		else{
			feeSchedNote.ClinicNums=clinicNum.ToString();
			feeSchedNote.ListClinicNums.Add(clinicNum);
		}
		feeSchedNote.FeeSchedNum=feeSched.FeeSchedNum;
	...
	private void butImportCanada_Click(object sender,EventArgs e) {
		...
		Cursor=Cursors.WaitCursor;
		using var formFeeSchedPickRemote=new FormFeeSchedPickRemote();
		...
		var feeData="";
		var actionCloseFeeSchedImportCanadaProgress=ODProgress.Show();
		if(formFeeSchedPickRemote.IsFileChosenProtected()) {
			...
			using var formFeeSchedPickAuthOntario=new FormFeeSchedPickAuthOntario(formFeeSchedPickRemote.GetSelectedFeeDA());
			...
			var feeData64=xmlNode.InnerXml;
			var byteArrayFeeData=Convert.FromBase64String(feeData64);
		...
		var outputMessage="Done. Number imported: "+numImported;
		if(numSkipped>0) {
	...
	private void butUpdateFees_Click(object sender,EventArgs e) {
		...
		var progressBarHelper=new ProgressBarHelper("Fee Schedule Update Progress",progressBarEventType:ProgBarEventType.Header);
	...
	private void butUpdateWriteoffs_Click(object sender,EventArgs e) {
		...
		var listWriteoffClinics=comboGlobalUpdateClinics.ListClinicNumsSelected;
		var doUpdatePrevClinicPref=comboGlobalUpdateClinics.IsAllSelected;//keeps track of current ClinicNum in db to resume if interrupted
		...
		{
			var msgText=Lan.g(this,"This tool was paused or interrupted during a previous run.  Would you like to resume the previous run?")+"\r\n\r\n"
			...
			var dialogResult=ODMessageBox.Show(this,msgText,"",MessageBoxButtons.YesNoCancel);
		...
		var progressBarHelper=new ProgressBarHelper(Lan.g(this,"Write-off Update Progress"),progressBarEventType:ProgBarEventType.Header);
		var progressExtended=new ODProgressExtended(this,tag:progressBarHelper
	...
	private void butPickFeeSched_Click(object sender,EventArgs e) {
		...
		using var formFeeScheds=new FormFeeScheds(true);
		formFeeScheds.ShowDialog();
		var selectedIndex= _listFeeScheds.FindIndex(x => x.FeeSchedNum==formFeeScheds.SelectedFeeSchedNum);//Returns index of the found element or -1.
		...
		var buttonPick=(UI.Button)sender;
	...
	private void butPickGroup_Click(object sender,EventArgs e){
		if(checkShowGroups.Checked) {
			var listFeeSchedGroupsToShow=comboGroup.Items.GetAll<FeeSchedGroup>();//s.AsEnumerable<ODBoxItem<FeeSchedGroup>>()).Select(x => x.Tag).ToList();
			var listGridColumnsHeaders=new List<GridColumn>();
			var col=new GridColumn(Lan.g(this,"Description"),50);
			col.IsWidthDynamic=true;
			listGridColumnsHeaders.Add(col);
			var listGridRowValues=new List<GridRow>();
			for(var i=0;i<listFeeSchedGroupsToShow.Count;i++) {
				var row=new GridRow(listFeeSchedGroupsToShow[i].Description);
				row.Tag=listFeeSchedGroupsToShow[i];
				listGridRowValues.Add(row);
			}
			var formTitle=Lan.g(this,"Fee Schedule Group Picker");
			var gridTitle=Lan.g(this,"Fee Schedule Groups");
			using var formGridSelection=new FormGridSelection(listGridColumnsHeaders,listGridRowValues,formTitle,gridTitle);
	...
	private void butPickGroupTo_Click(object sender,EventArgs e) {
		if(checkShowGroups.Checked) {
			var listFeeSchedGroupsToShow=comboGroupTo.Items.GetAll<FeeSchedGroup>();//Items.AsEnumerable<ODBoxItem<FeeSchedGroup>>()).Select(x => x.Tag).ToList();
			var listGridColumnsHeaders=new List<GridColumn>();
			var col=new GridColumn(Lan.g(this,"Description"),50);
			col.IsWidthDynamic=true;
			listGridColumnsHeaders.Add(col);
	...
	private void butPickProvider_Click(object sender,EventArgs e){
		...
		var selectedIndex= Providers.GetIndex(frmProviderPick.ProvNumSelected)+1;//All provider combo boxes have a none option, so always add 1.
