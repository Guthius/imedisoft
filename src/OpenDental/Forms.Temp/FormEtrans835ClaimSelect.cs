...
public partial class FormEtrans835ClaimSelect:FormODBase {
	...
	public FormEtrans835ClaimSelect(long patNum,Hx835_Claim hx835_Claim) {
		...
		_patNum=patNum;
		if(_patNum==0) {
			_errorProvider.SetError(butPatFind,Lans.g("Patient not found"));
		}
		if(_hx835_Claim.IsReversal) {
			this.Text+=" - "+Lans.g("Pick Original Claim for this Claim Reversal");
		}
		else if(_hx835_Claim.IsSplitClaim) {
			this.Text+=" - "+Lans.g("Pick Original Claim for this Split Claim");
		...
		var claimFee=hx835_Claim.ClaimFee;
		if(_hx835_Claim.IsReversal) {
	...
	private void FillGridClaims() {
		var sortByColIdx=gridClaims.GetSortedByColumnIdx();  //Keep previous sorting
		var isSortAsc=gridClaims.IsSortedAscending();
		if(sortByColIdx==-1) {
			sortByColIdx=0;
			isSortAsc=false;
		}
		...
		gridClaims.Columns.Add(col);
		var listClaims=Claims.Refresh(_patNum);
		for(var i=0;i<listClaims.Count;i++) {
			...
			var claimStatus=Claims.GetClaimStatusEnumFromCurClaim(listClaims[i]);
			row.Cells.Add(claimStatus.GetDescription());//Status
			if(true) {//Using clinics
				var clinic=Clinics.GetClinic(listClaims[i].ClinicNum);
				if(clinic==null) {
	...
	private void HighlightRows() {
		...
		var fee=SIn.Double(textClaimFee.Text);
		var countRowsHighlighted=0;
		var idxLastRowSelected=0;
		gridClaims.BeginUpdate();
		for(var i=0;i<gridClaims.ListGridRows.Count;i++) {
			...
			var claim=(Claim)gridClaims.ListGridRows[i].Tag;
			var ynIsFeeMatch=YN.No;  //If fee matches then yes, if fee doesnt match then no, if no fee entered then unknown
			var ynIsDateMatch=YN.No; //If both dates match then yes, if both dates dont match then no, if no dates entered then unknown
			//Check fee
			...
			var hx835_Proc=_hx835_Claim.ListProcs.FirstOrDefault();
			var isPlanMatch=true;//Assume true for backwards compatiablity, because older 837s did not send out the Ordinal and PlanNum.
			var isBasicMatch=(ynIsFeeMatch==YN.Yes || ynIsDateMatch==YN.Yes) && (ynIsFeeMatch!=YN.No && ynIsDateMatch!=YN.No);//If either match and neither don't match
	...
	private void FillClaimDetails(int rowIndex) {
		var claimSelected=(Claim)gridClaims.ListGridRows[rowIndex].Tag;
		var isSupplemental=(claimSelected.ClaimStatus=="R");
		...
		if(gridClaimDetails.Columns.Count==0) {
			...
			var col=new UI.GridColumn("ProcStatus",50);
		...
		var listHx835_ProcsUnmatched=new List<Hx835_Proc>();
		var listHx835_ProcsClaimProcsMatched=new List<Tuple<Hx835_Proc,ClaimProc>>();
		for(var i=0;i<_hx835_Claim.ListProcs.Count;i++) {
			var claimProc=_listClaimProcs.FirstOrDefault(x =>
		...
		for(var i=0;i<listHx835_ProcsUnmatched.Count;i++) {
			var row=new UI.GridRow();
			row.Cells.Add("");
			row.Cells.Add("");
			row.Cells.Add("");
		...
		for(var i=0;i<listHx835_ProcsClaimProcsMatched.Count;i++) {
			var hx835_Proc=listHx835_ProcsClaimProcsMatched[i].Item1;
			var claimProc=listHx835_ProcsClaimProcsMatched[i].Item2;
			var row=new UI.GridRow();
	...
	private void gridClaims_CellDoubleClick(object sender,UI.ODGridClickEventArgs e) {
		...
		var claim=Claims.GetClaim(((Claim)gridClaims.ListGridRows[e.Row].Tag).ClaimNum);//This is the easiest way to determine if the claim was deleted.
		...
		using var formClaimEdit=new FormClaimEdit(claim,Patients.GetPat(_patNum),Patients.GetFamily(_patNum));
		...
		if(formClaimEdit.DialogResult==DialogResult.OK) {
			...
			var claimStatus=Claims.GetClaimStatusEnumFromCurClaim(claim);
			row.Cells.Add(claimStatus.GetDescription());//Status
			if(true) {//Using clinics
	...
	private bool HasValidSplitClaimTotals() {
		...
		for(var i=0;i<_hx835_Claim.ListProcs.Count;i++) {
			var claimProcMatched=_listClaimProcs.FirstOrDefault(x =>
					//Mimics proc matching in claimPaid.GetPaymentsForClaimProcs(...)
					x.ProcNum!=0 && ((x.ProcNum==_hx835_Claim.ListProcs[i].ProcNum)//Consider using Hx835_Proc.TryGetMatchedClaimProc(...)
					                 || (x.CodeSent==_hx835_Claim.ListProcs[i].ProcCodeBilled 
					                     && (decimal)x.FeeBilled==_hx835_Claim.ListProcs[i].ProcFee
	...
	private void butOK_Click(object sender,EventArgs e) {
		...
		var claimSelected=(Claim)gridClaims.ListGridRows[gridClaims.GetSelectedIndex()].Tag;
		var fee=(double)_hx835_Claim.ClaimFee;
		if(_hx835_Claim.IsReversal) {
			fee=Math.Abs(fee);
		}
		var isValidClaimFee=true;
