...
public partial class FormFieldDefLink:FormODBase {
	...
	private void FormFieldDefLink_Load(object sender,EventArgs e) {
		var stringArrayFieldLocations=Enum.GetNames(typeof(FieldLocations));
		for(var i=0;i<stringArrayFieldLocations.Length;i++) {
	...
	private void FillGridDisplayed() {
		...
		var selectedIdx=gridDisplayed.GetSelectedIndex();
		...
		switch((FieldLocations)comboFieldLocation.SelectedIndex) 
		{
			...
			case FieldLocations.OrthoChart:
				for(var i=0;i<_listPatFieldDefs.Count;i++) {
			...
			case FieldLocations.AppointmentEdit://AppointmentEdit is the only place where ApptFields are used.
				for(var i=0;i<_listApptFieldDefs.Count;i++) {
	...
	private void FillGridHidden() {
		var selectedIdx=gridHidden.GetSelectedIndex();
		...
		GridRow row;
		var listFieldDefLinksForLoc=_listFieldDefLinks.FindAll(x => x.FieldLocation==(FieldLocations)comboFieldLocation.SelectedIndex);
		var listFieldDefLinksToDelete=new List<FieldDefLink>();//Some links could exists to deleted FieldDefs prior to 18.1
		for(var i=0;i<listFieldDefLinksForLoc.Count;i++) {
			switch(listFieldDefLinksForLoc[i].FieldDefType) {
				case FieldDefTypes.Patient:
					var patFieldDef=_listPatFieldDefs.Find(x => x.PatFieldDefNum==listFieldDefLinksForLoc[i].FieldDefNum);
				...
				case FieldDefTypes.Appointment:
					var apptFieldDef=_listApptFieldDefs.Find(x => x.ApptFieldDefNum==listFieldDefLinksForLoc[i].FieldDefNum);
		...
		for(var i=0;i<listFieldDefLinksToDelete.Count;i++) {
	...
	private void butRight_Click(object sender,EventArgs e) {
		...
		switch((FieldLocations)comboFieldLocation.SelectedIndex) 
		{
			...
			case FieldLocations.GroupNote:
			case FieldLocations.OrthoChart:
				var patFieldDef=(PatFieldDef)gridDisplayed.ListGridRows[gridDisplayed.GetSelectedIndex()].Tag;
				...
				break;
			case FieldLocations.AppointmentEdit://AppointmentEdit is the only place where ApptFields are used.
				var apptFieldDef=(ApptFieldDef)gridDisplayed.ListGridRows[gridDisplayed.GetSelectedIndex()].Tag;
				fieldDefLink=new FieldDefLink();
	...
	private bool ApptFieldDefInUseOnApptView(FieldDefLink fieldDefLink) {
		if(ApptViewItems.GetWhere(x => x.ApptFieldDefNum == fieldDefLink.FieldDefNum,true).Count > 0) {
			var fieldName = ApptFieldDefs.GetFieldName(fieldDefLink.FieldDefNum);
	...
	private void ShowInUseWarning(FieldDefLink fieldDefLink,string fieldName) {
		...
		var listApptViewNums = ApptViewItems.GetWhere(x => x.ApptFieldDefNum == fieldDefLink.FieldDefNum && fieldDefLink.FieldDefType == FieldDefTypes.Appointment)
			.Select(x => x.ApptViewNum)
			.Distinct()
			.ToList();
		var listApptViews = ApptViews.GetWhere(x => listApptViewNums.Contains(x.ApptViewNum));
