...
public partial class FormRadOrderList:FormODBase {
	...
	private List<Procedure> _listProceduresNonCpoe= [];
	...
	private void FillGrid() {
		...
		var col=new GridColumn(Lan.g("TableRadiologyOrders","Date"),90,HorizontalAlignment.Center,GridSortingStrategy.DateParse);
		...
		GridRow row;
		for(var i=0;i<_listProceduresNonCpoe.Count;i++) {
			var patient=_listPatients.FirstOrDefault(x => x.PatNum==_listProceduresNonCpoe[i].PatNum);
			var procedureCode=_listProcedureCodes.FirstOrDefault(x => x.CodeNum==_listProceduresNonCpoe[i].CodeNum);
			var appointment=_listAppointments.FirstOrDefault(x => x.AptNum==_listProceduresNonCpoe[i].AptNum);
	...
	private void butGotoFamily_Click(object sender,EventArgs e) {
		...
		var procedure=(Procedure)gridMain.ListGridRows[gridMain.GetSelectedIndex()].Tag;
		GlobalFormOpenDental.GoToModule(EnumModuleType.Family, patNum:procedure.PatNum);
	}
	...
	private void butGotoChart_Click(object sender,EventArgs e) {
		...
		var procedure=(Procedure)gridMain.ListGridRows[gridMain.GetSelectedIndex()].Tag;
		GlobalFormOpenDental.GoToModule(EnumModuleType.Chart,patNum:procedure.PatNum);
	}
	...
	private void butSelected_Click(object sender,EventArgs e) {
		...
		for(var i=0;i<gridMain.SelectedIndices.Length;i++) {
			var procedure=(Procedure)gridMain.ListGridRows[gridMain.SelectedIndices[i]].Tag;
