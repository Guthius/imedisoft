...
public partial class FormReference:FormODBase {
	...
	private void FillMain(bool limit) {
		...
		var listBillingTypes=listBillingType.GetListSelected<Def>().Select(x=>x.DefNum).ToList();
		...
		var col=new GridColumn("PatNum",50);
		...
		GridRow row;
		for(var i=0;i<_tableRef.Rows.Count;i++) {
			row=new GridRow();
	...
	private void gridMain_CellDoubleClick(object sender,UI.ODGridClickEventArgs e) {
		var custReference=CustReferences.GetOne(SIn.Long(_tableRef.Rows[e.Row]["CustReferenceNum"].ToString()));
		using var formReferenceEdit=new FormReferenceEdit(custReference);
	...
	private void butOK_Click(object sender,EventArgs e) {
		...
		ListCustReferencesSelected= [];
		for(var i=0;i<gridMain.SelectedIndices.Length;i++) {
			var custReference=CustReferences.GetOne(SIn.Long(_tableRef.Rows[gridMain.SelectedIndices[i]]["CustReferenceNum"].ToString()));
