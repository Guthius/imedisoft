...
public partial class FormTimeAdjustEdit : FormODBase {
	...
	private void FormTimeAdjustEdit_Load(object sender, System.EventArgs e) {
		...
		var listDefs=Defs.GetDefsForCategory(DefCat.TimeCardAdjTypes).OrderBy(x => x.ItemName).ToList();
	...
	private string CreateUnpaidProtectedLeaveSecurityLogText(UnpaidProtectedLeaveLogType unpaidProtectedLeaveLogType,TimeAdjust timeAdjust,TimeAdjust timeAdjustEdited=null) {
		var stringTimeAdjustNum=$"TimeAdjustNum: {timeAdjust.TimeAdjustNum}";
		var stringEmployeeName=$"{Lan.g(this,"Employee")}: {Employees.GetName(timeAdjust.EmployeeNum)}";
		var stringTimeEntry=$"{Lan.g(this,"Date/Time Entry")}: {timeAdjust.TimeEntry}";
		var stringHours=$"{Lan.g(this,"Hours")}: {HoursStringHelper(timeAdjust)}";
		var stringNote=$"{Lan.g(this,"Note")}: \"{timeAdjust.Note}\"";
		var stringChangedTo=Lan.g(this,"changed to");
		string logText;
	...
	private void butDelete_Click(object sender,EventArgs e) {
		...
		if(_timeAdjust.IsUnpaidProtectedLeave) {
			var logText=CreateUnpaidProtectedLeaveSecurityLogText(UnpaidProtectedLeaveLogType.Deleted,_timeAdjustOld);
			...
			SecurityLogs.MakeLogEntry(EnumPermType.TimeAdjustEdit,0,
				$"Time Card Adjustment deleted for Employee: {Employees.GetName(_timeAdjust.EmployeeNum)}.");
		}
	...
	private void butSave_Click(object sender,System.EventArgs e) {
		...
		if(checkOvertime.Checked){
			...
			var def=comboPTO.GetSelected<Def>();
		...
		if(IsNew){
			...
			if(_timeAdjust.IsUnpaidProtectedLeave) {
				var logText=CreateUnpaidProtectedLeaveSecurityLogText(UnpaidProtectedLeaveLogType.Created,_timeAdjust);
				SecurityLogs.MakeLogEntry(EnumPermType.ProtectedLeaveAdjustmentEdit,0,logText);
			}
			else {
				SecurityLogs.MakeLogEntry(EnumPermType.TimeAdjustEdit,0,
					$"Time Card Adjustment created for Employee: {Employees.GetName(_timeAdjust.EmployeeNum)}.");
			}
		}
		else{
			TimeAdjusts.Update(_timeAdjust,_timeAdjustOld);
			if(_timeAdjust.IsUnpaidProtectedLeave || _timeAdjustOld.IsUnpaidProtectedLeave) {
				var logText=CreateUnpaidProtectedLeaveSecurityLogText(UnpaidProtectedLeaveLogType.Edited,_timeAdjustOld,_timeAdjust);
				SecurityLogs.MakeLogEntry(EnumPermType.ProtectedLeaveAdjustmentEdit,0,logText);
			}
			else {
				SecurityLogs.MakeLogEntry(EnumPermType.TimeAdjustEdit,0,
					$"Time Card Adjustment edited for Employee: {Employees.GetName(_timeAdjust.EmployeeNum)}.");
			}
		}
		DialogResult=DialogResult.OK;
	}
