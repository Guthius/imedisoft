...
public partial class FormSheetFieldImage:FormODBase {
	...
	private void butImport_Click(object sender,EventArgs e) {
		...
		FillCombo();
		for(var i=0;i<comboFieldName.Items.Count;i++){
	...
	private void ShrinkToFit(){
		...
		if(pictureBox.Image.Width>SheetDefCur.Width || pictureBox.Image.Height>SheetDefCur.Height){//image would be too big
			var ratioImgWtoH=((float)pictureBox.Image.Width)/((float)pictureBox.Image.Height);
			var ratioSheetWtoH=((float)SheetDefCur.Width)/((float)SheetDefCur.Height);
			float ratioNew;
			int newW;
			int newH;
