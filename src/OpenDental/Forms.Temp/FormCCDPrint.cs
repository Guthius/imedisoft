...
public partial class FormCCDPrint:FormODBase {
	...
	private CCDFieldInputter _ccdReceived;
	...
	private CCDFieldInputter _ccdSent=null;
	...
	private long _claimResponseDefNum=0;
	///<summary>For saving the printed document to PDF.</summary>
	private string _docDescript="";
	///<summary>For saving the printed document to PDF.</summary>
	private MigraDoc.DocumentObjectModel.Document _pdfDocument=null;
	...
	private void Init(){
		...
		if(_claim==null) {//ex: for eligibility, when there is no claim to overwrite _carrier below.
			...
			if(_etrans.PlanNum!=0) {
				_insplan=InsPlans.GetPlan(_etrans.PlanNum, []);
			}
			if(_etrans.InsSubNum!=0) {
				_insSub=InsSubs.GetSub(_etrans.InsSubNum, []);
		...
		if(_etrans.PatNum!=0) { //Some transactions are not patient specific.
			...
			_claim=Claims.GetClaim(_etrans.ClaimNum);//Claim can be null for eligibility.
			if(_claim!=null) {
				//Get primary info
				_insSub=InsSubs.GetSub(_claim.InsSubNum, []);
				_patientSubscriber=Patients.GetPat(_insSub.Subscriber);
				_insplan=InsPlans.GetPlan(_claim.PlanNum, []);
				...
				if(_claim.InsSubNum2!=0) {
					...
					_insSub2=InsSubs.GetSub(_claim.InsSubNum2, []);
					_patientSubscriber2=Patients.GetPat(_insSub2.Subscriber);
					_insplan2=InsPlans.GetPlan(_claim.PlanNum2, []);
	...
	private void butOverride_Click(object sender,EventArgs e) {
		...
		if(!_ccdReceived.HasValidPaymentLines()) {
		...
		var listProcedures=Procedures.Refresh(_etrans.PatNum);
		var listClaimProcs=ClaimProcs.Refresh(_etrans.PatNum);
	...
	private void butBack_Click(object sender,EventArgs e) {
		...
		labelPage.Text=(printPreviewControl1.StartPage+1)+" / "+_totalPages;
	}
	...
	public void ShowDisplayMessages(){
		...
		var ccdFieldArrayDisplayMessage=_ccdReceived.GetFieldsById("G32");
		for(var i=0;i<ccdFieldArrayDisplayMessage.Length;i++){
		...
		var ccdFieldArrayNoteTexts=_ccdReceived.GetFieldsById("G26");
		var listStringDisplayMessages=new List<string>();
		...
		while(listStringDisplayMessages.Count>0) {
			var indexOfMinVal=0;
			for(var j=1;j<listDisplayMessageNumbers.Count;j++) {
		...
		var msg=stringBuilderMessage.ToString();
		if(msg.Length>0){
	...
	private void pd_PrintPage(object sender,System.Drawing.Printing.PrintPageEventArgs e){
		for(byte i=0;i<255;i++){
			var sizeF=e.Graphics.MeasureString(""+Canadian.GetCanadianChar(i),_documentGenerator.standardFont);
		...
		if(_isDirty){//Only render the document containers the first time through.
			...
			//We cannot import a PDF of this printed document unless we know which patient to import images to.
			//Additionally, we only import a PDF if this is the first time the response is being viewed (_isAutoPrint).
			if(_patient!=null && _isAutoPrint) {
				_claimResponseDefNum=Defs.GetImageCat(ImageCategorySpecial.N);//Returns 0 if not found.
			}
			try {
				if(_responseStatus=="R") {
					PrintRejection(e.Graphics);
					_docDescript=Lan.g(this,"Rejection");
				}
				else if(_transactionCode=="12") {
					PrintReversalResponse_12(e.Graphics);
					_docDescript=Lan.g(this,"Reversal Response");
				}
				else if(_transactionCode=="15") {
					PrintSummaryReconciliation_15(e.Graphics);
					_docDescript=Lan.g(this,"Summary Reconciliation");
				}
				else if(_transactionCode=="16") {
					PrintPaymentReconciliation_16(e.Graphics);
					_docDescript=Lan.g(this,"Payment Reconciliation");
				}
				else if(_transactionCode=="24") {
					PrintEmail_24(e.Graphics);
					_docDescript=Lan.g(this,"Email Response");
				}
				else {
					switch(_formId) {
						default:
							DefaultPrint(e.Graphics);
							_claimResponseDefNum=0;//Do not save PDF to images.
							break;
						case null:
						...
						case "01"://CDA EOB Form
							PrintEOB(e.Graphics);
							_docDescript=Lan.g(this,"EOB");
							break;
						case "02"://Dentaide Form
							PrintDentaide(e.Graphics);
							_docDescript=Lan.g(this,"Dentaide");
							break;
						case "03"://Claim Acknowledgement Form
							PrintClaimAck(e.Graphics);
							_docDescript=Lan.g(this,"Claim Ack");
							break;
						case "04"://Employer Certified Form
							PrintEmployerCertified(e.Graphics);
							_docDescript=Lan.g(this,"Employer Certified");
							break;
						...
						case "06"://Predetermination Acknowledgement Form
							...
							_docDescript=Lan.g(this,"Predetermination Ack");
							break;
						case "07"://Predetermination EOB Form
							PrintEOB(e.Graphics);
							_docDescript=Lan.g(this,"Predetermination EOB");
							break;
						case "08"://Eligibility Form
							PrintEligibility(e.Graphics);
							_docDescript=Lan.g(this,"Eligibility");
							break;
					}
				}
			...
			if(_claimResponseDefNum!=0) {//Only save PDF to images when the Claim Responses ImageCat is set and the response print document was generated correclty.
				_pdfDocument=new MigraDoc.DocumentObjectModel.Document();
				_pdfDocument.DefaultPageSetup.PageWidth=MigraDoc.DocumentObjectModel.Unit.FromInch(e.PageBounds.Size.Width/100.0);
				_pdfDocument.DefaultPageSetup.PageHeight=MigraDoc.DocumentObjectModel.Unit.FromInch(e.PageBounds.Size.Height/100.0);
				_pdfDocument.DefaultPageSetup.TopMargin=MigraDoc.DocumentObjectModel.Unit.FromInch(0);//Margin is 0 because the page image contains built-in margins.
				_pdfDocument.DefaultPageSetup.LeftMargin=MigraDoc.DocumentObjectModel.Unit.FromInch(0);//Margin is 0 because the page image contains built-in margins.
				_pdfDocument.DefaultPageSetup.BottomMargin=MigraDoc.DocumentObjectModel.Unit.FromInch(0);//Margin is 0 because the page image contains built-in margins.
				_pdfDocument.DefaultPageSetup.RightMargin=MigraDoc.DocumentObjectModel.Unit.FromInch(0);//Margin is 0 because the page image contains built-in margins.
			}
		}
		...
		if(_claimResponseDefNum!=0) {//Add current page as an image to the PDF.
			using(var pageImage=new Bitmap(e.PageBounds.Size.Width,e.PageBounds.Size.Height))
			using(var g=Graphics.FromImage(pageImage)) {
				//Tested all 6 TextRenderingHints. AntiAliasGridFit and SingleBitPerPixelGridFit created identical output.
				//However, the summary of AntiAliasGridFit makes it sound more expensive than SingleBitPerPixelGridFit.
				g.TextRenderingHint=System.Drawing.Text.TextRenderingHint.SingleBitPerPixelGridFit;
				_documentGenerator.PrintPage(g,_pagesPrinted);
				var section=_pdfDocument.AddSection();
				var textFrame=MigraDocHelper.CreateContainer(section);
				//Draw light border for context.
				MigraDocHelper.DrawRectangle(textFrame,Color.LightGray,e.MarginBounds.X,e.MarginBounds.Y,e.MarginBounds.Width,e.MarginBounds.Height);
				MigraDocHelper.DrawBitmap(textFrame,pageImage,0,0);//No offset because the image is as big as the page and the image contains built-in margins.
			}
		}
		if(_doPrintPageNumbers){
			_text="Page "+_pagesPrinted+(_isFrench?" de ":" of ")+_totalPages;
		...
		if(_pagesPrinted<_totalPages){					
			...
			labelPage.Text=(printPreviewControl1.StartPage+1)+" / "+_totalPages;
			if(_claimResponseDefNum!=0) {//Import the PDF to the patient images in the Claim Response folder.
				var pdfDocumentRenderer=new PdfDocumentRenderer(true,PdfFontEmbedding.Always);
				pdfDocumentRenderer.Document=_pdfDocument;
				pdfDocumentRenderer.RenderDocument();
				using(var ms=new MemoryStream()) {
					pdfDocumentRenderer.PdfDocument.Save(ms,closeStream:false);
					ms.Position=0;
					var arrayImageBytes=new byte[ms.Length];//Cannot use ms.GetBuffer() because the buffer can be longer than ms.Length.
					ms.Read(arrayImageBytes,0,(int)ms.Length);
					var doc=ImageStore.Import(arrayImageBytes,_claimResponseDefNum,ImageType.File,_patient,fileExtension:".pdf");
					var docOld=doc.Copy();
					doc.Description=_docDescript;
	...
	private void PrintClaimForm(bool isPreview) {
		...
		if(isPreview) {
			FormClaimPrint.ShowCdaClaimForm(_claim);//Show the form on screen to save paper.
		}
		else {
	...
	private void PrintEmail_24(Graphics g){
		...
		_x=_documentGenerator.StartElement(_verticalLine);
		var ccdFieldArrayNoteLines=_ccdReceived.GetFieldsById("G53");//BODY OF MESSAGE
	...
	private void PrintSummaryReconciliation_15(Graphics g){
		...
		_documentGenerator.HorizontalLine(g,_penBreakLine,_documentGenerator.bounds.Left,_documentGenerator.bounds.Right,0);
		_documentGenerator.StartElement();
		var ccdFieldArrayCdaProviderNumbers=_ccdReceived.GetFieldsById("B01");
		var ccdFieldArrayCarrierIdentificationNumbers=_ccdReceived.GetFieldsById("A05");
		var ccdFieldArrayOfficeSequenceNumbers=_ccdReceived.GetFieldsById("A02");
		var ccdFieldArrayTransactionReferenceNumbers=_ccdReceived.GetFieldsById("G01");
		var ccdFieldArrayTransactionPayments=_ccdReceived.GetFieldsById("G38");
		var cdaProviderNumCol=_x;
		float cdaProviderNumColWidth=75;
		var carrierIdentificationNumCol=cdaProviderNumCol+cdaProviderNumColWidth;
		float carrierIdentificationNumColWidth=125;
		var officeSequenceNumCol=carrierIdentificationNumCol+carrierIdentificationNumColWidth;
		float officeSequenceNumColWidth=140;
		var transactionReferenceNumCol=officeSequenceNumCol+officeSequenceNumColWidth;
		float transactionReferenceNumColWidth=125;
		var transactionPaymentCol=transactionReferenceNumCol+transactionReferenceNumColWidth;
		var font=_documentGenerator.standardFont;
	...
	private void PrintPaymentReconciliation_16(Graphics g){
		...
		_documentGenerator.StartElement();
		var ccdFieldArraycdaProviderNumbers=_ccdReceived.GetFieldsById("B01");
		var ccdFieldArrayProviderOfficeNumbers=_ccdReceived.GetFieldsById("B02");
		var ccdFieldArrayBillingProviderNumbers=_ccdReceived.GetFieldsById("B03");
		var ccdFieldArrayCarrierIdentificationNumbers=_ccdReceived.GetFieldsById("A05");
		var ccdFieldArrayOfficeSequenceNumbers=_ccdReceived.GetFieldsById("A02");
		var ccdFieldArrayTransactionReferenceNumbers=_ccdReceived.GetFieldsById("G01");
		var ccdFieldArrayTransactionPayments=_ccdReceived.GetFieldsById("G38");
		var cdaProviderNumCol=_x;
		float cdaProviderNumColWidth=75;
		var providerOfficeNumCol=cdaProviderNumCol+cdaProviderNumColWidth;
		float providerOfficeNumColWidth=75;
		var billingProviderNumCol=providerOfficeNumCol+providerOfficeNumColWidth;
		float billingProviderNumColWidth=120;
		var carrierIdentificationNumCol=billingProviderNumCol+billingProviderNumColWidth;
		float carrierIdentificationNumColWidth=125;
		var officeSequenceNumCol=carrierIdentificationNumCol+carrierIdentificationNumColWidth;
		float officeSequenceNumColWidth=140;
		var transactionReferenceNumCol=officeSequenceNumCol+officeSequenceNumColWidth;
	...
	private void DefaultPrint(Graphics g) {
		...
		_documentGenerator.DrawString(g,_text,_center-g.MeasureString(_text,_fontHeading).Width/2,0,_fontHeading);
		_x=_documentGenerator.StartElement(_verticalLine);
		var ccdFieldArrayLoadedFields=_ccdReceived.GetLoadedFields();
	...
	private void PrintDentaide(Graphics g){
		...
		var headerHeight=(int)_verticalLine;
		...
		if(_transactionCode=="13") {//predetermination ACK
			...
			var listEtrans=Etranss.GetAllForOneClaim(_claim.ClaimNum);//Claim not null here.
			Etrans eTransPredetermEob=null;
			for(var i=0;i<listEtrans.Count;i++) {//Etrans entries are chronological, so we find the most recent one starting at the end of the list.
			...
			if(eTransPredetermEob!=null) {
				var predetermResponseMessage=EtransMessageTexts.GetMessageText(eTransPredetermEob.EtransMessageTextNum);
		...
		var sizeF2=PrintPrimaryDependantNo(g,rightMidCol,sizeF1.Height,"PATIENT'S OFFICE ACCOUNT NO","NO DE DOSSIER DU PATIENT");
		var sizeF3=PrintOfficeSequenceNumber(g,rightMidCol,sizeF1.Height+sizeF2.Height);
		...
		_text=GetPatientRelationshipString("C03",_claim?.PatRelat);//Field C03
		_documentGenerator.DrawString(g,_text,leftMidCol,0);
		_text=GetPatientRelationshipString("E06",_claim?.PatRelat2);//Field E06
		_documentGenerator.DrawString(g,_text,rightCol,0);
		//Spaces don't show up with underline, so we will have to underline manually.
		var underlineWidth=g.MeasureString("***",_documentGenerator.standardFont).Width;
		var isStudent="   ";
		var isHandicapped="   ";
		var relatCode=GetPatientRelationshipCode("C03",_claim?.PatRelat);
		if(relatCode=="3"){//Child
			_text="";
		...
		_x=_documentGenerator.StartElement();
		var initialPlacementUpperStr=GetDbDataStr("F15",_claim?.CanadianIsInitialUpper);
		var initialPlacementLowerStr=GetDbDataStr("F18",_claim?.CanadianIsInitialLower);			
		...
		if(initialPlacementUpperStr=="N") {//Field F15 - Avoid invalid upper initial placement data.
			_text=GetMaterialDescription(GetDbDataInt("F20",_claim?.CanadianMaxProsthMaterial));//Field F20
			_documentGenerator.DrawString(g,_text,_x+180,0);
			_text=DateToString(GetDbDataDateTime("F04",_claim?.CanadianDateInitialUpper));//Field F04
		...
		if(initialPlacementLowerStr=="N") {//Field F18 - Avoid invalid lower initial placement data.				
			_text=GetMaterialDescription(GetDbDataInt("F21",_claim?.CanadianMandProsthMaterial));//Field F21
			_documentGenerator.DrawString(g,_text,_x+180,0);
			_text=DateToString(GetDbDataDateTime("F19",_claim?.CanadianDateInitialLower));//Field F19
			_documentGenerator.DrawString(g,_text,_x+420,0);
		}
		_x=_documentGenerator.StartElement();
		if(_isFrench) {
		...
		_x=_documentGenerator.StartElement();
		if(_isFrench) {
			sizeF1=_documentGenerator.DrawString(g,_bullet+". "+("S'agit-il d'un traitement en vue de soins d'orthodontie?"),_x,0);
		}
		else {
			sizeF1=_documentGenerator.DrawString(g,_bullet+". "+("Is treatment for orthodontic purposes? "),_x,0);
		}
		_bullet++;
		if(GetDbDataBool("F05",_claim?.IsOrtho)){//Field F05
		...
		_documentGenerator.DrawString(g,_text,_x+sizeF1.Width,0);
		_x=_documentGenerator.StartElement(_verticalLine);
		var isEOB=_transactionCode=="21" || _transactionCode=="23";
		var ccdFieldArrayNoteNumbers=_ccdReceived.GetFieldsById("G45");//Used to looking up note reference numbers.
		if(_listClaimProcs!=null) {
			...
			//of each procedure.
			var ccdFieldsArrayProcedureLineNumbers=_ccdReceived.GetFieldsById("F07");
			var ccdFieldsArrayEligibleAmounts=_ccdReceived.GetFieldsById("G12");
			var ccdFieldsArrayDeductibleAmounts=_ccdReceived.GetFieldsById("G13");
			var ccdFieldsArrayEligiblePercentages=_ccdReceived.GetFieldsById("G14");
			var ccdFieldsArrayDentaidePayAmounts=_ccdReceived.GetFieldsById("G15");
			var ccdFieldsArrayExplainationNoteNumbers1=_ccdReceived.GetFieldsById("G16");
			var ccdFieldsArrayExplainationNoteNumbers2=_ccdReceived.GetFieldsById("G17");
			var ccdFieldsArrayEligibleLabAmounts1=_ccdReceived.GetFieldsById("G43");
			var ccdFieldsArrayDeductibleLabAmounts1=_ccdReceived.GetFieldsById("G56");
			var ccdFieldsArrayEligibleLabPercentage1=_ccdReceived.GetFieldsById("G57");
			var ccdFieldsArrayBenefitLabAmount1=_ccdReceived.GetFieldsById("G58");
			var ccdFieldsArrayEligibleLabAmounts2=_ccdReceived.GetFieldsById("G02");
			var ccdFieldsArrayDeductibleLabAmounts2=_ccdReceived.GetFieldsById("G59");
			var ccdFieldsArrayEligibleLabPercentage2=_ccdReceived.GetFieldsById("G60");
			var ccdFieldsArrayBenefitLabAmount2=_ccdReceived.GetFieldsById("G61");
			var noteColumn=_x;
			float noteColumnWidth=(isEOB?55:0);
			var procedureColumn=noteColumn+noteColumnWidth;
			float procedureColumnWidth=50;
			var toothColumn=procedureColumn+procedureColumnWidth;
			float toothColumnWidth=45;
			var surfaceColumn=toothColumn+toothColumnWidth;
			float surfaceColumnWidth=55;
			var feeColumn=surfaceColumn+surfaceColumnWidth;
			float feeColumnWidth=75;
			var eligibleFeeColumn=feeColumn+feeColumnWidth;
			float eligibleFeeColumnWidth=(isEOB?75:0);
			var labColumn=eligibleFeeColumn+eligibleFeeColumnWidth;
			float labColumnWidth=80;
			var eligibleLabColumn=labColumn+labColumnWidth;
			float eligibleLabColumnWidth=(isEOB?75:0);
			var deductibleColumn=eligibleLabColumn+eligibleLabColumnWidth;
			float deductibleColumnWidth=(isEOB?75:0);
			var percentCoveredColumn=deductibleColumn+deductibleColumnWidth;
			float percentCoveredColumnWidth=(isEOB?90:0);
			var dentaidePaysColumn=percentCoveredColumn+percentCoveredColumnWidth;
			float dentaidePaysColumnWidth=(isEOB?65:0);
			var endNoteColumn=dentaidePaysColumn+dentaidePaysColumnWidth;
			var font=_documentGenerator.standardFont;
			_documentGenerator.standardFont=_fontStandardSmall;
			double totalFee=0;
			double totalLab=0;
			double totalPaid=0;
			var numProcsPrinted=0;
			if(isEOB) {
			...
			for(var p=1;p<=_listClaimProcs.Count;p++) {
				_x=_documentGenerator.StartElement();
				var procLineNum=numProcsPrinted+p;
				...
				var procedure=Procedures.GetOneProc(claimProc.ProcNum,true);
				_text=claimProc.CodeSent.PadLeft(6,' ');//Field F08
				_documentGenerator.DrawString(g,_text,procedureColumn,0);
				_text=Tooth.Display(procedure.ToothNum);//Field F10
			...
			if(isEOB) {
				...
				var ccdFieldsArrayProcs=_ccdReceived.GetFieldsById("G19");
				var ccdFieldsArrayCarrierEligibleAmts=_ccdReceived.GetFieldsById("G20");
				var ccdFieldsArrayCarrierEligibleLabAmts=_ccdReceived.GetFieldsById("G44");
				var ccdFieldsArrayCarrierDeductAmts=_ccdReceived.GetFieldsById("G21");
				var ccdFieldsArrayCarrierAts=_ccdReceived.GetFieldsById("G22");
				var ccdFieldsArrayCarrierBenefitAmts=_ccdReceived.GetFieldsById("G23");
				var ccdFieldsArrayCarrierNotes1=_ccdReceived.GetFieldsById("G24");
				var ccdFieldsArrayCarrierNotes2=_ccdReceived.GetFieldsById("G25");
	...
	private void PrintEmployerCertified(Graphics g){
		...
		_x=_documentGenerator.StartElement(_verticalLine);
		var size=_documentGenerator.DrawString(g,_isFrench?"EMPLOYEUR: ":"EMPLOYER: ",_x,0);
	...
	private void PrintProcedureListAck(Graphics g,string payableToStr) {
		var amountWidth=g.MeasureString("****.**",_documentGenerator.standardFont).Width;
		var procedureCodeColumn=_x;
		var procedureToothColumn=procedureCodeColumn+470;
		var procedureSurfaceColumn=procedureToothColumn+25;
		var procedureDateColumn=procedureSurfaceColumn+52;
		float procedureDateColumnWidth=_hasPredetermination?0:80;
		var procedureChargeColumn=procedureDateColumn+procedureDateColumnWidth;
		var procedureTotalColumn=procedureChargeColumn+amountWidth+10;//charge col width is amount width.
		_x=_documentGenerator.StartElement();
		...
		var listClaimProcsOrdered=new List<ClaimProc>();
		for(var i=0;i<_listClaimProcs.Count;i++) {
			var claimProc=_listClaimProcs[i];
		...
		for(var i=0;i<listClaimProcsOrdered.Count;i++) {
			var claimProc=listClaimProcsOrdered[i];
			...
			var feeBilledTotal=(decimal)claimProc.FeeBilled;
			var listProceduresLab=Procedures.GetCanadianLabFees(claimProc.ProcNum,listProcedures);
			for(var j=0;j<listProceduresLab.Count;j++) {
	...
	private void PrintProcedureListEOB(Graphics g) {
		...
		_documentGenerator.standardFont=_fontStandardSmall;
		var procedureCodeColumn=_x;
		var procedureToothColumn=procedureCodeColumn+270;
		var procedureDateColumn=procedureToothColumn+25;
		float procedureDateColumnWidth=_hasPredetermination?0:75;
		var procedureChargeColumn=procedureDateColumn+procedureDateColumnWidth;
		var procedureEligibleColumn=procedureChargeColumn+60;
		var procedureDeductColumn=procedureEligibleColumn+60;
		var procedureAtColumn=procedureDeductColumn+60;
		var procedureBenefitColumn=procedureAtColumn+30;
		var procedureNotesColumn=procedureBenefitColumn+60;
		_x=_documentGenerator.StartElement();
		_documentGenerator.DrawString(g,_isFrench?"ACTE":"PROCEDURE",procedureCodeColumn,0);
		_documentGenerator.DrawString(g,_isFrench?"D#":"TH#",procedureToothColumn,0);
		if(!_hasPredetermination) {
		...
		var ccdFieldsArrayEligibleAmounts=_ccdReceived.GetFieldsById("G12");
		var ccdFieldsArrayDeductibleAmounts=_ccdReceived.GetFieldsById("G13");
		var ccdFieldsArrayEligiblePercentage=_ccdReceived.GetFieldsById("G14");
		var ccdFieldsArrayBenefitAmountForTheProcedures=_ccdReceived.GetFieldsById("G15");
		var ccdFieldsArrayExplainationNotes1=_ccdReceived.GetFieldsById("G16");
		var ccdFieldsArrayExplainationNotes2=_ccdReceived.GetFieldsById("G17");
		var ccdFieldsArrayEligibleLabAmounts1=_ccdReceived.GetFieldsById("G43");
		var ccdFieldsArrayDeductibleLabAmounts1=_ccdReceived.GetFieldsById("G56");
		var ccdFieldsArrayEligibleLabPercentage1=_ccdReceived.GetFieldsById("G57");
		var ccdFieldsArrayBenefitLabAmount1=_ccdReceived.GetFieldsById("G58");
		...
		//List the carrier inserted procedures into the procedure list.
		var ccdFieldsArrayCarrierProcs=_ccdReceived.GetFieldsById("G19");
		var ccdFieldsArrayCarrierEligibleAmts=_ccdReceived.GetFieldsById("G20");
		var ccdFieldsArrayCarrierEligibleLabAmts=_ccdReceived.GetFieldsById("G44");
		var ccdFieldsArrayCarrierDeductAmts=_ccdReceived.GetFieldsById("G21");
		var ccdFieldsArrayCarrierAts=_ccdReceived.GetFieldsById("G22");
	...
	private SizeF PrintTransactionReferenceNumber(Graphics g,float X,float Y){
		var ccdFieldCarrierClaimNos=_ccdReceived.GetFieldsById("G01");
	...
	private SizeF PrintDentistAddress(Graphics g,float X,float Y,float maxWidthInPixels){
		var sizeF1=_documentGenerator.DrawString(g,_isFrench?"ADRESSE: ":"ADDRESS: ",X,Y);
		var sizeF2=PrintAddress(g,X+sizeF1.Width,Y,PrefC.GetString(PrefName.PracticeAddress),PrefC.GetString(PrefName.PracticeAddress2),
	...
	private SizeF PrintOfficeSequenceNumber(Graphics g,float X,float Y) {
		var ccdFieldOfficeSequenceNumbers=_ccdReceived.GetFieldsById("A02");
	...
	private SizeF PrintCertificateNo(Graphics g,float X,float Y,bool isPrimary) {
		...
		var ccdFieldC12=_ccdReceived.GetFieldById("C12");//Plan flag. 'N' for NIHB, 'A' for Newfoundland MCP Plan - Provincial Medical Plan, 'V' for Veteran's Affairs Plan. There may be other values.
		var ccdFieldC13=_ccdReceived.GetFieldById("C13");//Band number for NIHB plans.
		var ccdFieldC14=_ccdReceived.GetFieldById("C14");//Family number for NIHB plans.
	...
	private SizeF PrintSubscriberAddress(Graphics g,float X,float Y,bool isPrimary,float maxWidthInPixels) {
		...
		var line2="";
		var line3="";
		var patientSubscriber=isPrimary?_patientSubscriber:_patientSubscriber2;
	...
	private SizeF PrintInsuredAddress(Graphics g,float X,float Y,bool isPrimary,int maxCharsPerLine) {
		var sizeF1=_documentGenerator.DrawString(g,_isFrench?"ADRESSE DU TITULAIRE: ":"INSURED/MEMBER ADDRESS: ",X,Y);
		var sizeF2=PrintSubscriberAddress(g,X+sizeF1.Width,Y,isPrimary,maxCharsPerLine);
		return new SizeF(sizeF1.Width+sizeF2.Width,Math.Max(sizeF1.Height,sizeF2.Height));
	}
	...
	private SizeF PrintRelationshipToSubscriber(Graphics g,float X,float Y,bool doUseCaps) {
		_text=GetPatientRelationshipString("C03",_claim?.PatRelat);
		var engStr="RELATIONSHIP TO INSURED/MEMBER";
		var frStr="PARENTÉ AVEC TITULAIRE";
		var label=_isFrench?frStr:engStr;
	...
	private SizeF PrintSubscriberBirthday(Graphics g,float X,float Y,bool doUseCaps) {
		...
		var engStr="Birthdate";
		var frStr="Date de naissance";
		var label=_isFrench?frStr:engStr;
		return _documentGenerator.DrawField(g,doUseCaps?label.ToUpper():label,_text,true,X,Y);
	}
	...
	private SizeF PrintAddress(Graphics g,float X,float Y,string line1,string line2,string line3,float minWidthInPixels,float maxWidthInPixels) {
		...
		var address=line1+"\n"+line2+"\n"+line3;
		var lineWidth=Math.Max(minWidthInPixels,g.MeasureString(address,_documentGenerator.standardFont).Width);
		float yoff=0;
	...
	private string GetTruncatedString(Graphics g,Font font,string str,float maxWidthInPixels){
		...
		var result=str;
		var sizeFStr=g.MeasureString(result,font);
		...
		while(result.Length>0 && sizeFStr.Width>maxWidthInPixels){
			...
			var chrsToReplace=Math.Min(3,result.Length);
	...
	private bool PrintDependencies(Graphics g,bool doFillOut){
		...
		var isStudentHeight=_documentGenerator.DrawString(g,strIsStudent,_x,0).Height;
		//Spaces don't show up with underline, so we will have to underline manually.
		var underlineWidth=g.MeasureString("***",_documentGenerator.standardFont).Width;
		...
		var isHandicappedHeight=_documentGenerator.DrawString(g,isHandicapped,_x,0).Height;
	...
	private void PrintAccidentBullet(Graphics g,string questionStr){
		...
		var dateAccident=GetDbDataDateTime("F02",_claim?.AccidentDate);
		var isAccident=IsValidDate(dateAccident);
		...
		_x=_documentGenerator.StartElement();
		_x+=_documentGenerator.DrawField(g,_isFrench?"Si Oui, donner date":"If yes, give date",(isAccident?DateToString(dateAccident):"________")+" ",true,_x,0).Width;
	...
	private void PrintInitialPlacementBullet(Graphics g){
		var initialPlacementUpper=GetDbDataStr("F15",_claim?.CanadianIsInitialUpper);
		var initialPlacementLower=GetDbDataStr("F18",_claim?.CanadianIsInitialLower);
		_x+=_documentGenerator.DrawString(g,_bullet+". ",_x,0).Width;
		_bullet++;
		_documentGenerator.PushX(_x);//Begin indentation.
	...
	private void PrintPaymentSummary(Graphics g) {
		var amountWidth=(float)Math.Ceiling((double)g.MeasureString("******.**",_documentGenerator.standardFont).Width);
		...
		var ccdFieldTotalPayable=_ccdReceived.GetFieldById("G55");
		if(ccdFieldTotalPayable==null) {
			ccdFieldTotalPayable=_ccdReceived.GetFieldById("G28");
		}
		...
		if(payableTo=="1") {//Pay the subscriber.
			...
			var sizeF1=_documentGenerator.DrawString(g,_text,_x,0);
	...
	private void PrintPracticeAddress(Graphics g,float xPos){
		if(_clinic==null){
			...
			_text=Patients.GetAddressFull(_clinic.AddressLine1,_clinic.AddressLine2,_clinic.City,_clinic.State,_clinic.Zip);
		}
	...
	private void PrintMissingToothList(Graphics g){
		var initialPlacementUpper=GetDbDataStr("F15",_claim?.CanadianIsInitialUpper);
		var initialPlacementLower=GetDbDataStr("F18",_claim?.CanadianIsInitialLower);
		if((initialPlacementUpper=="Y" || initialPlacementUpper=="O") &&
		   (initialPlacementLower=="Y" || initialPlacementLower=="O")) {//Only print extractions when F15 or F18 are "Yes"
			var title=(_isFrench?"D#":"TH")+"  DATE(YYYYMMDD)\t";
			var titleWidth=g.MeasureString(title,_documentGenerator.standardFont).Width;
			var cycleOrthoDateCount=(int)((_documentGenerator.bounds.Right-_x)/titleWidth);
			for(var i=0;i<Math.Min(cycleOrthoDateCount,_listProcedureExtracted.Count);i++) {
				_x+=_documentGenerator.DrawString(g,title,_x,0).Width;
			}
			var j=0;
			for(var i=0;i<_listProcedureExtracted.Count;i++) {//Count specified by field F22
				...
				if(IsValidDate(_listProcedureExtracted[i].ProcDate)) {//Tooth is considered unextracted if it doesn't have a date.
					var thWidth=_documentGenerator.DrawString(g,Tooth.Display(_listProcedureExtracted[i].ToothNum).PadLeft(2,' ')+" ",_x,0).Width;//Field F23
	...
	private void PrintNoteList(Graphics g) {
		var ccdFieldsArrayNoteOutputFlags=_ccdReceived.GetFieldsById("G41");
		var ccdFieldsArrayNoteNumbers=_ccdReceived.GetFieldsById("G45");
		var ccdFieldsArrayNoteTexts=_ccdReceived.GetFieldsById("G26");
		var listStringsDisplayMessages=new List<string>();
		var listDisplayMessageNumbers=new List<int>();
	...
	private int PrintErrorList(Graphics g) {
		var ccdFieldArrayErrors=_ccdReceived.GetFieldsById("G08");
	...
	private bool AssignmentOfBenefits() {
		if(_claim?.ClaimType=="S") {
			if(_insSub2!=null) {
				return Claims.GetAssignmentOfBenefits(_claim,_insSub2);//Claim not null here.
			}
		}
		if(_insSub!=null) {
			return Claims.GetAssignmentOfBenefits(_claim,_insSub);//Claim not null here.
		}
		return false;
	}
	...
	private DateTime DateNumToDateTime(string number) {
		if(number=="00000000") {
			return DateTime.MinValue;
		}
		return new DateTime(Convert.ToInt32(number.Substring(0,4)),Convert.ToInt32(number.Substring(4,2)),Convert.ToInt32(number.Substring(6,2)));
	}

	///<summary>Input string is expected to have the form 'YYYYMMDD'.</summary>
	private string DateNumToPrintDate(string number){
		return DateToString(DateNumToDateTime(number));
	}
	...
	private string GetPatientRelationshipString(string fieldId,Relat? relat){
		var relatCode=GetPatientRelationshipCode(fieldId,relat);
		return GetRelationshipDescriptFromCode(relatCode);
	}

	private string GetPatientRelationshipCode(string fieldId,Relat? relat) {
		var relatCode=GetDbDataStr(fieldId,"");
		if(relatCode=="") {
			relatCode=Canadian.GetRelationshipCode(relat.Value);
		}
		return relatCode;
	}

	///<summary>Given a CDA relationship code (from fields C03 and E06), returns a description string.</summary>
	private string GetRelationshipDescriptFromCode(string relationshipCode) {
	...
	private DateTime GetDbDataDateTime(string fieldId,DateTime? dateDefault) {
		var dateAsStr=GetDbDataStr(fieldId,"");
		if(dateAsStr=="") {
			return dateDefault.Value;//Can throw here if dateDefault is null.
		}
		return DateNumToDateTime(dateAsStr);
	}

	///<summary>Can throw exception if the data could not be found anywhere in the database.</summary>
	private int GetDbDataInt(string fieldId,int? intDefault) {
		var intAsStr=GetDbDataStr(fieldId,"");
		if(intAsStr=="") {
			return intDefault.Value;//Can throw here if intDefault is null.
		}
		return Convert.ToInt32(intAsStr);
	}
	...
	private bool GetDbDataBool(string fieldId,bool? boolDefault) {
		var boolAsStr=GetDbDataStr(fieldId,"");
		if(boolAsStr=="") {
			return boolDefault.Value;//Can throw here if boolDefault is null.
		}
		return(boolAsStr!="N");
	}
