...
public partial class FormClaimPayEdit:FormODBase {
	...
	private void FormClaimPayEdit_Load(object sender, System.EventArgs e) {
		...
		_listDefsClaimPaymentGroups=Defs.GetDefsForCategory(DefCat.ClaimPaymentGroups,isShort:true);
	...
	private void FillAutoDepositDetails() {
		...
		comboDepositAccountNum.Items.AddDefs(Defs.GetDefsForCategory(DefCat.AutoDeposit,isShort:true));
		...
		if(_depositOld!=null) {//check for an existing deposit
			...
			for(var i=0;i<this.groupBoxDeposit.Controls.Count;i++) {
				if(this.groupBoxDeposit.Controls[i]==butDepositEdit) {
					continue;
				}
				ODException.SwallowAnyException(() => { this.groupBoxDeposit.Controls[i].Enabled=(_depositOld==null); });
			}
		}
	...
	private void CheckUIState() {
		...
		var clinicNum=GetClinicNumSelected();
		var listDefs=Defs.GetDefsForCategory(DefCat.PaymentTypes,isShort:true);
	...
	private void butCarrierSelect_Click(object sender,EventArgs e) {
		...
		if(formCarriers.DialogResult==DialogResult.OK) {
			textCarrierName.Text=formCarriers.CarrierSelected.CarrierName;
		}
	}
	...
	private void FillComboPaymentGroup(long selectedDefNum=0) {
		...
		if(_listDefsClaimPaymentGroups.Count==0) {
			comboPayGroup.Visible=false;
			butPickPaymentGroup.Visible=false;
			labelClaimPaymentGroup.Visible=false;
			return;
		}
		for(var i = 0;i<_listDefsClaimPaymentGroups.Count;i++) {
			var def=_listDefsClaimPaymentGroups[i];
			comboPayGroup.Items.Add(def.ItemName);
			if(selectedDefNum==def.DefNum) {
	...
	private void butDepositEdit_Click(object sender,EventArgs e) {
		...
		using var formDepositEdit=new FormDepositEdit(GetDepositCur());
		formDepositEdit.ShowDialog();
		if(formDepositEdit.DialogResult==DialogResult.OK) {//Made changes, update deposit values
	...
	private void panelXcharge_MouseClick(object sender,MouseEventArgs e) {
		...
		payment.ClinicNum=GetClinicNumSelected();
		var formPayment=new FormPayment(new Patient(),new Family(),payment,false);
		...
		try {
			...
			FriendlyException.Show(Lan.g(this,"Error processing transaction.\r\n\r\nPlease contact support with the details of this error:")
			                       ...
			                       +"\r\nLast valid milestone reached: "+formPayment.XchargeMilestone,ex);
		}
		if(tranDetail!=null) {
			if(textNote.Text!="") {
				textNote.Text+="\r\n";
	...
	private void butPayConnect_Click(object sender,EventArgs e) {
		...
		using var formPayment=new FormPayment(patient,family,payment,doPreferCurrentPat:false);
		var tranDetail=formPayment.MakePayConnectTransaction(SIn.Double(textAmount.Text));
		if(tranDetail!=null) {
			if(textNote.Text!="") {
				textNote.Text+="\r\n";
			}
	...
	private void butPaySimple_MouseClick(object sender,MouseEventArgs e) {
		...
		var family=new Family();
		using var formPayment=new FormPayment(patient,family,payment,doPreferCurrentPat:false);
		var tranDetail=formPayment.MakePaySimpleTransaction(SIn.Double(textAmount.Text),textCarrierName.Text);
		if(tranDetail!=null) {
			if(textNote.Text!="") {
				textNote.Text+="\r\n";
			}
	...
	///background.</summary>
	private void SetRequiredTextBox(Label labelCur,System.Windows.Forms.TextBox textBoxCur,bool areConditionsMet) {
		if(areConditionsMet) {
			labelCur.Text=labelCur.Text.Replace("*","")+"*";
			if(textBoxCur.Text=="") {
				_isMissingRequiredFields=true;
				if(_isValidating) {
					_errorProvider.SetError(textBoxCur,"Text box cannot be blank.");
				}
			}
			else {
				_errorProvider.SetError(textBoxCur,"");
			}
		}
		else {
			labelCur.Text=labelCur.Text.Replace("*","");
	...
	private void SetRequiredComboBox(Label labelCur,OpenDental.UI.ComboBox comboBoxCur,bool areConditionsMet,int disallowedIdx,string errorMsg) {
		if(areConditionsMet) {
			...
			if(comboBoxCur.SelectedIndex==disallowedIdx) {
				_isMissingRequiredFields=true;
				if(_isValidating) {
					_errorProvider.SetError(comboBoxCur,Lan.g(this,errorMsg));
				}
			}
			else {
				_errorProvider.SetError(comboBoxCur,"");
			}
		}
		else {
			labelCur.Text=labelCur.Text.Replace("*","");
	...
	private void butSave_Click(object sender, System.EventArgs e) {
		...
		var defNumPaymentType=comboPayType.GetSelectedDefNum();
		...
		var defPaymentType=Defs.GetDef(DefCat.InsurancePaymentType,defNumPaymentType);
		...
		if(IsNew){
			...
			var listClaimPaySplit=Claims.GetAttachedToPayment(ClaimPaymentCur.ClaimPaymentNum);
			double insPayTotal=0;
			for(var i=0; i<listClaimPaySplit.Count; i++) {
