...
public partial class FormEServicesEConnector:FormODBase {
	...
	private eServiceSignalSeverity FillTextListenerServiceStatus() {
		var eserviceSignalSeverity=EServiceSignals.GetListenerServiceStatus();
	...
	private void butInstallEConnector_Click(object sender,EventArgs e) {
		...
		var updateServerName=PrefC.GetString(PrefName.WebServiceServerName);
		//Only ask the user if they want to set the Update Server Name preference if it is not already set.
		var doOverrideBlankUpdateServerName=false;
		var isInvalidUpdateServerNameAllowed=false;
	...
	private void FillGridListenerService() {
		...
		var listEServiceSignals=EServiceSignals.GetServiceHistory(eServiceCode.ListenerService,DateTime.Today.AddDays(-30),DateTime.Today);
		gridListenerServiceStatusHistory.BeginUpdate();
		gridListenerServiceStatusHistory.Columns.Clear();
		var col=new GridColumn(Lan.g(this,"DateTime"),120);
	...
	private void butStartListenerService_Click(object sender,EventArgs e) {
		...
		try {
			...
			var error=Lan.g(this,"There was an problem starting eConnector Services.  Try running eConnector in administrator "
		...
		var serviceErrors=ServicesHelper.StartServices(listServiceControllers);
		Cursor=Cursors.Default;
		if(!string.IsNullOrEmpty(serviceErrors)) {
			var error=Lan.g(this,"There was a problem starting eConnector Services.  Please go manually start the following eConnector Services")
			          +":\r\n"+serviceErrors;
	...
	private void butListenerAlertsOff_Click(object sender,EventArgs e) {
		...
		var eServiceSignal=new EServiceSignal();
	...
	private void butSave_Click(object sender,EventArgs e) {
		...
		var interval=SIn.Int(textLogCleanupInterval.Text);
