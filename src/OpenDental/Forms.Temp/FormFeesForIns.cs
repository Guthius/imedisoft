...
public partial class FormFeesForIns : FormODBase {
	...
	private void FormFeesForIns_Load(object sender,EventArgs e) {
		var stringArrayEnumNames=Enum.GetNames(typeof(FeeScheduleType));
		for(var i=0;i<stringArrayEnumNames.Length;i++) {
			var feeSchedType=(FeeScheduleType)i;
	...
	private void ResetSelections(){
		...
		for(var i=0;i<_listFeeSchedsForType.Count;i++){
	...
	private void FillGrid() {
		...
		var col=new GridColumn("Employer",170);
		...
		for(var i = 0;i<_table.Rows.Count;i++) {
			...
			var isFixedBenefitFeeSched=_listFixedBenefitFeeSchedNums.Contains(SIn.Long(_table.Rows[i]["FeeSched"].ToString()));
			var doAddName=true;
	...
	private bool ChangePlansWarningsOk(long newFeeSchedNum,List<InsPlanRow> listInsPlanRowsToChange) {
		...
		var feeScheduleTypeSelected=(FeeScheduleType)listType.SelectedIndex;
		...
		var countPlansToDisable=GetCountPlansToDisableBlueBook(newFeeSchedNum,listInsPlanRowsToChange);
		...
		var countPlansToEnable=GetCountPlansToEnableBlueBook(newFeeSchedNum,listInsPlanRowsToChange);
	...
	private bool GetPasswordFromUser() {
		var inputBoxPassword=new InputBox("To prevent accidental changes, please enter password.  It can be found in our manual.");
	...
	private void pd_PrintPage(object sender,System.Drawing.Printing.PrintPageEventArgs e) {
		...
		if(!_hasHeadingPrinted) {
			...
			text="Fee Schedule Type: "+listType.SelectedItem;
			...
			if(comboFeeSchedWith.SelectedIndex!=0) {
				text="With Fee Schedule: "+comboFeeSchedWith.SelectedItem;
				yPos+=(int)g.MeasureString(text,fontHeading).Height;
				g.DrawString(text,fontSubHeading,Brushes.Black,center-g.MeasureString(text,fontSubHeading).Width/2,yPos);
			}
			if(comboFeeSchedWithout.SelectedIndex!=0) {
				text="Without Fee Schedule: "+comboFeeSchedWithout.SelectedItem;
				yPos+=(int)g.MeasureString(text,fontHeading).Height;
				g.DrawString(text,fontSubHeading,Brushes.Black,center-g.MeasureString(text,fontSubHeading).Width/2,yPos);
			}
			yPos+=20;
	...
	private void butChangeInsPlanType_Click(object sender,EventArgs e) {
		...
		var newInsPlanType = comboInsPlanType.GetSelected<string>();
		var enableBlueBook=newInsPlanType=="" //new plan type is Cat%
		                   && PrefC.GetEnum<AllowedFeeSchedsAutomate>(PrefName.AllowedFeeSchedsAutomate)==AllowedFeeSchedsAutomate.BlueBook;
		var listInsPlanRowsToChange=gridMain.SelectedTags<InsPlanRow>().FindAll(x=>x.PlanType!=newInsPlanType);
		...
		if(listInsPlanRowsToChange.Any(x=>x.PlanType.In("","p")) && newInsPlanType.In("f","c")) {
			...
			for(var i=0;i<listInsPlanRowsToChange.Count;i++) {
				if(listInsPlanRowsToChange[i].PlanType.In("f","c")) {
					continue;
				}
				var listBenefits=Benefits.GetForPlanOrPatPlan(listInsPlanRowsToChange[i].PlanNum,0);
				listBenefits.Where(x=>x.Percent>0).ForEach(x=>Benefits.Delete(x));
			}
		}
		var listInsPlanNumsToChange=listInsPlanRowsToChange.Select(x=>x.PlanNum).ToList();
		InsPlans.ChangeInsPlanTypes(listInsPlanNumsToChange,newInsPlanType,enableBlueBook);
		FillGrid();
		comboInsPlanType.SelectedIndex=0;
		Cursor=Cursors.Default;
		ODMessageBox.Show(Lan.g(this,"Plans changed: ")+listInsPlanNumsToChange.Count);
	}
	...
	private void butChangeFeeSchedule_Click(object sender,System.EventArgs e) {
		...
		var listInsPlanRowsToChange=gridMain.SelectedTags<InsPlanRow>().FindAll(x => x.FeeSched!=newFeeSchedNum);
		if(listType.GetSelected<string>().Equals("FixedBenefit")) {
			var doNotChange=listInsPlanRowsToChange.Any(x => x.PlanType!="p" || !_listFixedBenefitFeeSchedNums.Contains(x.FeeSched));
			if(doNotChange) {
		...
		Cursor=Cursors.WaitCursor;
		var listInsPlanNumsToChange=listInsPlanRowsToChange.Select(x => x.PlanNum).ToList();
		var rowsChanged=InsPlans.ChangeFeeScheds(listInsPlanNumsToChange,newFeeSchedNum,(FeeScheduleType)listType.SelectedIndex,_disableBlueBook,_enableBlueBook);
		FillGrid();
		Cursor=Cursors.Default;
		ODMessageBox.Show(Lan.g(this,"Plans changed: ")+rowsChanged);
	}
