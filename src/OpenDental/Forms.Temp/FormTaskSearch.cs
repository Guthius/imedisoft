...
public partial class FormTaskSearch:FormODBase {
	...
	private void FormTaskSearch_Load(object sender,EventArgs e) {
		...
		if(_listPreLoadedTaskNums!=null || !string.IsNullOrEmpty(TaskNum)) {
			var listTaskNums=new List<long> {};
			...
			_tableTasks=Tasks.GetDataSet(userNum, [],listTaskNums," "," "," "," ",textIncluding.Text,textExcluding.Text,0,0,checkBoxIncludesTaskNotes.Checked,
				checkBoxIncludeCompleted.Checked,checkIncludeAttachments.Checked,true);
			FillGrid();
		}
		textTaskNum.Select();
		FillContextMenu();
	...
	private void FillGrid() {
		...
		var col=new GridColumn("Created",70,HorizontalAlignment.Left);
		...
		GridRow row;
		for(var i=0;i<_tableTasks.Rows.Count;i++) {
			row=new GridRow();
	...
	private void FillContextMenu() {
		...
		var menuItemGoTo=gridTasks.ContextMenu.MenuItems.OfType<MenuItem>().FirstOrDefault(x => x.Text == "Go To");
	...
	private void MenuPopup(object sender,EventArgs e) {
		var mouseLocationGridRow=gridTasks.PointToRow(_pointLastClicked.Y);
		var menuItemGoTo=gridTasks.ContextMenu.MenuItems.OfType<MenuItem>().FirstOrDefault(x => x.Text == "Go To");
		var menuItemNavJob=gridTasks.ContextMenu.MenuItems.OfType<MenuItem>().FirstOrDefault(x => x.Text == "Navigate to Job");
		if(mouseLocationGridRow==-1) {
			menuItemSetEnabled(menuItemGoTo,false);
			menuItemSetEnabled(menuItemNavJob,false);
			return;
		}
		var taskNum=SIn.Long(gridTasks.ListGridRows[mouseLocationGridRow].Tag.ToString());
	...
	private void menuItemGoTo_Click(object sender,EventArgs e) {
		var index=gridTasks.PointToRow(_pointLastClicked.Y);
		var strTaskNum=(string)gridTasks.ListGridRows[index].Tag;
		var taskNum=SIn.Long(strTaskNum);
	...
	private void RefreshTable() {
		...
		var listTaskListNums=new List<long>();
		if(textTaskList.Text!="") {
			listTaskListNums=TaskLists.GetNumsByDescription(textTaskList.Text);
		...
		if(textTaskNum.Text!="") {
			try {
				listTaskNums=textTaskNum.Text.Split([','],StringSplitOptions.RemoveEmptyEntries).Select(x =>SIn.Long(x)).ToList();
			}
			catch {
		...
		_tableTasks=Tasks.GetDataSet(userNum,listTaskListNums,listTaskNums,dateCreatedFrom.Text,dateCreatedTo.Text,dateCompletedFrom.Text,
			...
			checkIncludeAttachments.Checked,checkLimit.Checked);
	}

	private void gridTasks_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		var taskNum=SIn.Long(gridTasks.ListGridRows[e.Row].Tag.ToString());
		if(IsSelectionMode) {
	...
	private void butPatPicker_Click(object sender,EventArgs e) {
		...
		if(frmPatientSelect.IsDialogOK) {
			var patNum=frmPatientSelect.PatNumSelected;
	...
	private void butUserPicker_Click(object sender,EventArgs e) {
		...
		frmUserPick.ListUserodsFiltered=_listUserodsAll;
		frmUserPick.IsSelectionMode=true;
		if(!checkShowHiddenUsers.Checked) {
			frmUserPick.ListUserodsFiltered=_listUserodsNotHidden;
		}
		frmUserPick.ShowDialog();
		if(frmUserPick.IsDialogOK) {
			comboUsers.SetSelectedKey<Userod>(frmUserPick.UserNumSelected,x => x.UserNum);
		}
	}
