...
public partial class FormInsRemain : FormODBase
{
    ...
    private void FillGrid()
    {
        ...
        var listPatientsSubscribers = Patients.GetLimForPats(listInsSubs.Select(x => x.Subscriber).ToList());
        GridRow gridRow;
        ...
        var listProceduresCompleted = Procedures.GetCompletedForDateRange(
            DateTime.Today.AddYears(-1),
            DateTime.Today,
            listPatNums: [_patient.PatNum]);
        //Get all of the claimprocs associated to the completed procedures in order to link procedures to insurance plans.
        var listClaimProcs = ClaimProcs.GetForProcs(listProceduresCompleted.Select(x => x.ProcNum).ToList());
        for (var i = 0; i < listBenefits.Count; i++)
        {
            ...
            {
                //if not calendar year, then it must be service year
                int monthRenew = InsPlans.RefreshOne(listBenefits[i].PlanNum).MonthRenew; //monthrenew only stores the month as an int.
                if (DateTime.Today.Month >= monthRenew)
                {
                    //if the the current date is past the renewal month, use the current year
            ...
            {
                //benefit.PatPlanNum was not set so benefit.PlanNum must be set.
                dictionaryListClaimProcsPerSub = listClaimProcs.FindAll(x => x.PlanNum == listBenefits[i].PlanNum)
                    .GroupBy(x => x.InsSubNum)
                    .ToDictionary(x => x.Key, x => x.ToList());
            }

            foreach (var insSubNum in dictionaryListClaimProcsPerSub.Keys)
            {
                //The insSubNum should have a corresponding entry within listInsSubs.
                var insSub = listInsSubs.FirstOrDefault(x => x.InsSubNum == insSubNum);
                if (insSub == null)
                {
                    continue; //If not found then there are claimprocs associated to an inssub that is associated to a dropped or missing plan.
                }

                var insPlan = listInsPlans.FirstOrDefault(x => x.PlanNum == insSub.PlanNum);
                var carrier = listCarriers.FirstOrDefault(x => x.CarrierNum == insPlan.CarrierNum);
                var patientSubscriber = listPatientsSubscribers.FirstOrDefault(x => x.PatNum == insSub.Subscriber);
                var covCat = CovCats.GetCovCat(listBenefits[i].CovCatNum);
                //Filter out any procedures that are not associated to the insurance plan of the current benefit.
                var listProceduresFilter = listProcedures.FindAll(x => dictionaryListClaimProcsPerSub[insSubNum].Select(y => y.ProcNum).Contains(x.ProcNum));
                //Calculate the amount used for one benefit.
                var amtUsed = CovCats.GetAmtUsedForCat(listProceduresFilter, covCat);
                var amtRemain = listBenefits[i].Quantity - amtUsed;
    ...
    private void FillSummary()
    {
        ...
        var listClaimProcHists = ClaimProcs.GetHistList(_patient.PatNum, listBenefits, listPatPlans, listInsPlans, DateTime.Today, listInsSubs);
