...
public partial class FormInsPlans:FormODBase {
	...
	private void FillGrid(bool isGetAll=false){
		...
		var col=new GridColumn(Lans.g("TableInsurancePlans","Employer"),140);
	...
	private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e){
		var insPlan=InsPlans.GetPlan(SIn.Long(_table.Rows[e.Row]["PlanNum"].ToString()),null);
		if(!InsPlanExists(insPlan)) {
			return;
		}
		using var formInsPlan=new FormInsPlan(insPlan,null,null);
	...
	private void butMerge_Click(object sender,EventArgs e) {
		...
		var insPlanSelectedArray=new InsPlan[gridMain.SelectedIndices.Length];
		for(var i=0;i<insPlanSelectedArray.Length;i++){
		...
		using var formInsPlansMerge=new FormInsPlansMerge();
		...
		var planCount=GetInsBlueBookCount(formInsPlansMerge.InsPlanToMergeTo,insPlanSelectedArray);
		...
		//Do the merge.
		var insPlanToMergeTo=formInsPlansMerge.InsPlanToMergeTo.Copy();
		...
		if(stringBuilder.Length>0) {
			var msgBoxCopyPaste=new MsgBoxCopyPaste("Encountered the following error(s) when merging insurance plans:\n"+stringBuilder);
			msgBoxCopyPaste.Show();
		}
		//merge complete
		if(didMerge) {
			var logText=Lan.g(this,"Merged the following PlanNum(s): ")+string.Join(", ",listMergedPlanNums)+" "+Lan.g(this,"into")+" "+insPlanToMergeTo.PlanNum;
			SecurityLogs.MakeLogEntry(EnumPermType.InsPlanMerge,0,logText);
		}
		FillGrid();
		//highlight the merged plan
		for(var i=0;i<_table.Rows.Count;i++){
			for(var j=0;j<insPlanSelectedArray.Length;j++){
	...
	private int GetInsBlueBookCount(InsPlan insPlanToMergeTo,InsPlan[] insPlanSeletedArray) {
		...
		for(var i=0;i<insPlanSeletedArray.Length;i++) {
	...
	private void butHide_Click(object sender,EventArgs e) {
		var unusedCount=InsPlans.UnusedGetCount();
		if(unusedCount==0) {
			MsgBox.Show(this,"All plans are in use.");
			return;
		}
		var msgText=unusedCount+" "+Lan.g(this,"plans found that are not in use by any subscribers.  Hide all of them?");
