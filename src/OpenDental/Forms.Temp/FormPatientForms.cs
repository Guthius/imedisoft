...
public partial class FormPatientForms:FormODBase {
	...
	private void FillGrid(bool refreshFromDb){
		...
		var col=new GridColumn(Lan.g(this,"Type"),60);
	...
	private void butAddEForm_Click(object sender,EventArgs e) {
		...
		var eForm=EForms.CreateEFormFromEFormDef(eFormDef,PatNum,EnumEFormStatus.None);//sets IsNew=true
	...
	private void butTerminal_Click(object sender,EventArgs e) {
		...
		var listDataRows=gridMain.ListGridRows.Select(x=>(DataRow)x.Tag).ToList();//Get the list of DataRows from the grid.
	...
	private void butPreFill_Click(object sender,EventArgs e) {
		...
		var sheetDefOriginal=SheetDefs.GetSheetDef(sheet.SheetDefNum,hasExceptions:false);
		...
		var sheetNew=Sheets.PreFillSheetFromPreviousAndDatabase(sheetDefOriginal,sheet);
		sheetNew.IsNew=true;
