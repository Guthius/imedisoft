...
public partial class FormTaskPreferences:FormODBase {
	...
	private void FillComboGlobalFilter() {
		var globalPref=(EnumTaskFilterType)PrefC.GetInt(PrefName.TasksGlobalFilterType);
	...
	private void FillComboImageCategoryFolders() {
		var defNumImageCategory=PrefC.GetLong(PrefName.TaskAttachmentCategory);
		var listDefsImageCategories=Defs.GetDefsForCategory(DefCat.ImageCats).FindAll(x => x.ItemValue.Contains("Y"));//Image categories that have task attachment usage
