...
public partial class FormPayPlanTemplates:FormODBase {
	...
	private void FillTemplates() {
		...
		GridColumn col;
		col=new GridColumn(Lan.g(gridPayPlanTemplates.TranslationName,"Name"),160,HorizontalAlignment.Left);//0
		gridPayPlanTemplates.Columns.Add(col);
		if(true) {
			col=new GridColumn(Lan.g(gridPayPlanTemplates.TranslationName,"Clinic"),80,HorizontalAlignment.Center);//1
			gridPayPlanTemplates.Columns.Add(col);
		}
		col=new GridColumn(Lan.g(gridPayPlanTemplates.TranslationName,"APR"),64,HorizontalAlignment.Center);//2
		gridPayPlanTemplates.Columns.Add(col);
		col=new GridColumn(Lan.g(gridPayPlanTemplates.TranslationName,"Interest\r\nDelay"),105,HorizontalAlignment.Center);//3
		gridPayPlanTemplates.Columns.Add(col);
		col=new GridColumn(Lan.g(gridPayPlanTemplates.TranslationName,"Payment Amount"),110,HorizontalAlignment.Right);//4
		gridPayPlanTemplates.Columns.Add(col);
		col=new GridColumn(Lan.g(gridPayPlanTemplates.TranslationName,"Number of\r\nPayments"),70,HorizontalAlignment.Center);//5
		gridPayPlanTemplates.Columns.Add(col);
		...
		gridPayPlanTemplates.Columns.Add(col);
		col=new GridColumn(Lan.g(gridPayPlanTemplates.TranslationName,"Treatment Plan\r\nOption"),80,HorizontalAlignment.Center);//8
		gridPayPlanTemplates.Columns.Add(col);
		...
		for(var i=0;i<listTemplates.Count;i++) {
			...
			if(true) {
				var clinicAbbr=Clinics.GetAbbr(listTemplates[i].ClinicNum);
