...
public partial class FormEServicesEClipboard:FormODBase {
	...
	private void FormEServicesEClipboard_Load(object sender,EventArgs e) {
		...
		textByodSmsTemplate.MouseHover+=new EventHandler((_,_) => {
			var availableTags=Lans.g("Available Tags:\n")+OpenDentBusiness.AutoComm.ByodTagReplacer.BYOD_TAG+Lans.g("(required)\n")
	...
	private void butSave_Click(object sender,EventArgs e) {
		...
		var listClinicsChanged=_clinicPrefHelper.GetClinicsWithChanges(PrefName.EClipboardUseDefaults).FindAll(x=>x!=0);
		EClipboardPushPrefs();
	...
	private void butConfStatuses_Click(object sender,EventArgs e) {
		var formEServicesAutoMsgingAdvanced=new FormEServicesAutoMsgingAdvanced();
	...
	private void gridImages_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		var eClipboardImageCaptureDefSelected=gridImages.SelectedTag<EClipboardImageCaptureDef>();
		if(eClipboardImageCaptureDefSelected==null) {
			return;
		}
		using var formEClipboardImageCaptureDefEdit=new FormEClipboardImageCaptureDefEdit();
	...
	private void gridForms_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		using var formEClipboardSheetRule=new FormEClipboardSheetRule();
		...
		if(formEClipboardSheetRule.IsDeleted){
			var eClipboardSheetDef=(EClipboardSheetDef)gridForms.ListGridRows[e.Row].Tag;
	...
	private void butImageAdd_Click(object sender,EventArgs e){
		var clinicNum=GetClinicNumEClipboardTab();
		if(checkEClipboardUseDefaults.Checked) {
			clinicNum=0;
		}
		var eClipboardImageCaptureDef=new EClipboardImageCaptureDef();
		eClipboardImageCaptureDef.ClinicNum=clinicNum;
		eClipboardImageCaptureDef.Frequency=EnumEClipFreq.TimeSpan;
		var interval=new Interval(days:0,weeks:0,months:0,years:1);
	...
	private void butSheetAdd_Click(object sender,EventArgs e) {
		...
		var listSheetDefs=new List<SheetDef>();
		...
		for(var i=0;i<gridForms.ListGridRows.Count;i++){
			//Remove any current sheets in use from the list of sheet defs available to add.
			var eClipboardSheetDef2=(EClipboardSheetDef)gridForms.ListGridRows[i].Tag;
		...
		var listSheetDefsSelected=frmSheetPicker.ListSheetDefsSelected;
		var sheetDef=listSheetDefsSelected[0];//listSheetDefsSelected can only contain 1 SheetDef since frmSheetPicker.MultiSelect is not set to true.
		var eClipboardSheetDef=new EClipboardSheetDef();
	...
	private void butBrandingProfile_Click(object sender,EventArgs e) {
		using var formMobileBrandingProfileEdit= new FormMobileBrandingProfileEdit();
	...
	private void CheckEClipboardAllowCheckIn_Click(object sender, EventArgs e){
		var strAllowCheckIn=SOut.Bool(checkEClipboardAllowCheckIn.Checked);
	...
	private void CheckAllowPaymentCheckIn_Click(object sender,EventArgs e) {
		var strAllowPayments=SOut.Bool(checkEClipboardAllowPaymentCheckIn.Checked);
	...
	private void CheckEClipboardAllowSheets_Click(object sender, EventArgs e){
		var strAllowSheets=SOut.Bool(checkEClipboardAllowSheets.Checked);
	...
	private void CheckEClipboardCreateMissingForms_Click(object sender, EventArgs e){
		SetUIEClipboardEnabled();
		var strCanCreateMissingForms=SOut.Bool(checkEClipboardCreateMissingForms.Checked);
	...
	private void CheckEClipboardPopupKiosk_Click(object sender, EventArgs e){
		var strHasPopupKiosk=SOut.Bool(checkEClipboardPopupKiosk.Checked);
	...
	private void checkEnableByodSms_Click(object sender,EventArgs e) {
		SetUIEClipboardEnabled();
		var strEnableByodSms=SOut.Bool(checkEnableByodSms.Checked);
	...
	private void checkAppendByodToArrivalResponseSms_Click(object sender,EventArgs e) {
		var strByodForResponseSms=SOut.Bool(checkAppendByodToArrivalResponseSms.Checked);
	...
	private void checkRequire2FA_Click(object sender, EventArgs e) {
		var strRequire2FA=SOut.Bool(checkRequire2FA.Checked);
	...
	private void checkDisplayIndividually_Click(object sender, EventArgs e) {
		var strChecked=SOut.Bool(checkDisplayIndividually.Checked);
	...
	private void EClipboardPushPrefs() {
		...
		var listClinicNumsChanged=_clinicPrefHelper.GetClinicsWithChanges();
		if(listClinicNumsChanged.Contains(0)) {
			...
			listClinicNumsChanged.AddRange(
				...
				Clinics.GetDeepCopy(shortList:true)
					//Filter to only those clinics that currently use defaults (they have no rules of their own).
					.Where(x => ClinicPrefs.GetBool(PrefName.EClipboardUseDefaults,x.Id))
		...
		var listClinicNumsDistinct=listClinicNumsChanged.Distinct().ToList();
		for(var i=0;i<listClinicNumsDistinct.Count;i++) {
	...
	private void EClipboardSetControlsForClinicPrefs(){
		var clinicNum=clinicPickerEClipboard.ClinicNumSelected;
	...
	private void FillGridImages() {
		...
		var listEClipboardImageCaptureDefs=_listEClipboardImageCaptureDefs.FindAll(x => !x.IsSelfPortrait && x.ClinicNum==clinicNum);
		var eClipboardImageCaptureDefSelfPortrait=_listEClipboardImageCaptureDefs.Find(x => x.IsSelfPortrait && x.ClinicNum==clinicNum);
		gridImages.ListGridRows.Clear();
		gridImages.Columns.Clear();
		gridImages.BeginUpdate();
		var col=new GridColumn("Definition",100);
		...
		if(eClipboardImageCaptureDefSelfPortrait!=null){//First add self portait if not null
			...
			var enumOcrCaptureType=eClipboardImageCaptureDefSelfPortrait.OcrCaptureType;
			row.Cells.Add(enumOcrCaptureType.GetDescription());
			if(eClipboardImageCaptureDefSelfPortrait.Frequency==EnumEClipFreq.TimeSpan){
				var interval=new Interval(eClipboardImageCaptureDefSelfPortrait.ResubmitInterval);
		...
		for(var i=0;i<listEClipboardImageCaptureDefs.Count;i++){
			var row=new GridRow();
			...
			var enumOcrCaptureType=listEClipboardImageCaptureDefs[i].OcrCaptureType;
			row.Cells.Add(enumOcrCaptureType.GetDescription());
			if(listEClipboardImageCaptureDefs[i].Frequency==EnumEClipFreq.TimeSpan){
				var interval=new Interval(listEClipboardImageCaptureDefs[i].ResubmitInterval);
				row.Cells.Add(interval.ToString());//Col: Frequency
			}
	...
	private void FillGridForms() {
		...
		var listEClipboardSheetDefsOrdered=_listEClipboardSheetDefs.FindAll(x => x.ClinicNum==clinicNum).OrderBy(x => x.ItemOrder).ToList();
		for(var i=0;i<listEClipboardSheetDefsOrdered.Count;i++){//Replaces the old "SetEClipboardSheetOrder" method.
		...
		var col=new GridColumn("",60);//Can either be "Sheet" or "eForm". Does not correspond to 'SheetType' column in SheetDef or 'FormType' column in eFormDef. May need to rename this column to reduce ambiguity.
		...
		for(var i=0;i<listEClipboardSheetDefsOrdered.Count;i++){
			var row=new GridRow();
			...
			var gridCell=new GridCell(Lan.g("enumPrefillCondition",listEClipboardSheetDefsOrdered[i].PrefillStatus.ToString()));
			...
			if(listEClipboardSheetDefsOrdered[i].Frequency==EnumEClipFreq.TimeSpan){
				var interval=new Interval(listEClipboardSheetDefsOrdered[i].ResubmitInterval);
	...
	private void SetUIEClipboardEnabled() {
		...
		var isClinicSignedUp=MobileAppDevices.IsClinicSignedUpForEClipboard(GetClinicNumEClipboardTab());
		...
		var doUseDefaults=GetClinicNumEClipboardTab()!=0 && checkEClipboardUseDefaults.Checked;
		var enableSheets=checkEClipboardCreateMissingForms.Checked;
	...
	private void SwapSheets(bool doMoveUp) {
		...
		var selectedIdx=gridForms.GetSelectedIndex();
		...
		var eClipboardSheetDefSelected=(EClipboardSheetDef)gridForms.ListGridRows[selectedIdx].Tag;
		var eClipboardSheetDefSwap=(EClipboardSheetDef)gridForms.ListGridRows[swapIdx].Tag;
		var selectedItemOrder=eClipboardSheetDefSelected.ItemOrder;
	...
	private void UpdateEClipboardDefaultsIfNeeded(PrefName prefName,string newVal) {
		if(GetClinicNumEClipboardTab()==0) { //we are making changes to the default
			var listClinicPrefs=_clinicPrefHelper.GetWhere(PrefName.EClipboardUseDefaults,"1").ToList();
	...
	private void InsertInternalEFormsIfNeeded(){
		var didInsertInternal=EForms.InsertInternalToDb();
