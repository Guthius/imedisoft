...
public partial class FormSheetDefs:FormODBase {
	...
	private List<SheetTypeEnum> _listSheetTypeEnumsFilter= [];
	...
	private void FormSheetDefs_Load(object sender, System.EventArgs e) {
		...
		var listSheetTypeEnums=_listSheetDefsInternal.Select(x => x.SheetType).Distinct().ToList();
		listFilter.Items.AddList(listSheetTypeEnums, x => x.ToString());
		for(var i=0; i<listFilter.Items.Count; i++) {
	...
	private void FillGrid1(){
		...
		for(var i=0;i<_listSheetDefsInternal.Count;i++){
	...
	private void butCopy2_Click(object sender, EventArgs e) {
		...
		var sheetDef=grid2.SelectedTag<SheetDef>();
	...
	private void butCopy_Click(object sender,EventArgs e) {
		...
		var sheetDef=grid1.SelectedTag<SheetDef>();
		sheetDef.IsNew=true;
		sheetDef.RevID=1;
	...
	private void butDefault_Click(object sender,EventArgs e) {
		using var formSheetDefDefaults=new FormSheetDefDefaults();
		formSheetDefDefaults.ShowDialog();
	}

	private void grid1_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		using var formSheetDefEdit=new FormSheetDefEdit(grid1.SelectedTag<SheetDef>());
	...
	private void butTools_Click(object sender,EventArgs e) {
		using var formSheetTools=new FormSheetTools();
