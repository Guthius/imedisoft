...
public partial class FormTimeCardBenefitRp:FormODBase {
	...
	private void FillGridMain() {
		...
		var listTimeAdjustAll = TimeAdjusts.GetAllForPeriod(_dateMonthT2,_dateMonthT2.AddMonths(3));
		for (var i=0;i<listEmployees.Count;i++) {
			...
			var timeSpan2 = TimeSpan.FromTicks(listClockEvents
			...
			var gridCell = new GridCell(string.Format("{0:0.00}",Math.Round(timeSpan2.TotalHours,2,MidpointRounding.AwayFromZero)));
			...
			//Month T-1
			var timeSpan1 = TimeSpan.FromTicks(listClockEvents
				.FindAll(x => x.EmployeeNum==listEmployees[i].EmployeeNum
			...
			row.Cells.Add(gridCell);
			//Month T-0
			var timeSpan0 = TimeSpan.FromTicks(listClockEvents
				.FindAll(x => x.EmployeeNum==listEmployees[i].EmployeeNum
	...
	private void butExportGrid_Click(object sender,EventArgs e) {
		...
		var filename = ODFileUtils.CombinePaths(folderBrowserDialog.SelectedPath,"BenefitsRpt_"+_dateNow.ToString("yyyyMMdd")+"_"+DateTime.Now.ToString("hhmm")+".csv");
