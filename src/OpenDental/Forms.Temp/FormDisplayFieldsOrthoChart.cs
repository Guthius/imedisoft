...
public partial class FormDisplayFieldsOrthoChart : FormODBase {
	...
	private List<OrthoChartTabFields> _listOrthoChartTabDisplayFields= [];
	...
	private void LoadDisplayFields() {
		...
		_listOrthoChartTabDisplayFields= [];
		...
		for(var i=0;i<_listOrthoChartTabs.Count;i++) {
			var orthoChartTabFields=new OrthoChartTabFields();
			orthoChartTabFields.OrthoChartTab=_listOrthoChartTabs[i];
			orthoChartTabFields.ListDisplayFields= [];
			var listOrthoChartTabLinks=_listOrthoChartTabLinks.FindAll(x => x.OrthoChartTabNum==_listOrthoChartTabs[i].OrthoChartTabNum);
			listOrthoChartTabLinks.OrderBy(x => x.ItemOrder);
			for(var j=0;j<listOrthoChartTabLinks.Count;j++) {
		...
		var listDisplayFieldsOrphaned=_listDisplayFieldsAll.FindAll(x => x.DisplayFieldNum==0
	...
	private void FillGrids() {
		...
		var orthoChartTabFields=GetSelectedFields();
		var listOrthoChartTabLinks=_listOrthoChartTabLinks.FindAll(x => x.OrthoChartTabNum==orthoChartTabFields.OrthoChartTab.OrthoChartTabNum);
		_listDisplayFieldsAvail=GetAllFields();
		for(var i=0;i<orthoChartTabFields.ListDisplayFields.Count;i++){
			_listDisplayFieldsAvail.Remove(orthoChartTabFields.ListDisplayFields[i]);
			var row=new GridRow();
			row.Tag=orthoChartTabFields.ListDisplayFields[i];
			var description=orthoChartTabFields.ListDisplayFields[i].Description;
			...
			var columnWidth=orthoChartTabFields.ListDisplayFields[i].ColumnWidth;
			var orthoChartTabLink=listOrthoChartTabLinks.FirstOrDefault(x => x.DisplayFieldNum==orthoChartTabFields.ListDisplayFields[i].DisplayFieldNum);
		...
		for(var i=0;i<_listDisplayFieldsAvail.Count;i++) {
	...
	private List<DisplayField> GetAllFields(bool hasOrphanedFields=true) {
		var listDisplayFields=new List<DisplayField>();
		for(var i=0;i<_listOrthoChartTabDisplayFields.Count;i++) {
			if(!hasOrphanedFields && _listOrthoChartTabDisplayFields[i].OrthoChartTab==null) {
				continue;//Do not include orphaned fields.
			}
			for(var j=0;j<_listOrthoChartTabDisplayFields[i].ListDisplayFields.Count;j++) {
	...
	private OrthoChartTabFields GetSelectedFields() {
		var orthoChartTabNum=_listOrthoChartTabs[comboOrthoChartTabs.SelectedIndex].OrthoChartTabNum;
	...
	private void butSetupTabs_Click(object sender,EventArgs e) {
		using var form=new FormOrthoChartSetup();
		if(form.ShowDialog()!=DialogResult.OK) {
			return;
		}
		var listOldTabDisplayFields=_listOrthoChartTabDisplayFields;
		_listOrthoChartTabDisplayFields= [];
		...
		_listOrthoChartTabs=OrthoChartTabs.GetDeepCopy(true);
		for(var i=0;i<_listOrthoChartTabs.Count;i++) {
			var orthoChartTabFieldsOld=listOldTabDisplayFields.FirstOrDefault(x => x.OrthoChartTab!=null 
	...
	private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		var displayField=(DisplayField)gridMain.ListGridRows[e.Row].Tag;
		//OrthoChartTabFields orthoChartTabFields=GetSelectedFields();
		var orthoChartTabNum=_listOrthoChartTabs[comboOrthoChartTabs.SelectedIndex].OrthoChartTabNum;
		var orthoChartTabLink=_listOrthoChartTabLinks.FirstOrDefault(x => x.OrthoChartTabNum==orthoChartTabNum
		...
		using var formDisplayFieldOrthoEdit=new FormDisplayFieldOrthoEdit(displayField,GetAllFields(),orthoChartTabLink);
	...
	private void butLeft_Click(object sender,EventArgs e) {
		...
		if(textCustomField.Text!="") {//Add new ortho chart field
			...
			var listDisplayFieldsAll=GetAllFields();
			...
			var displayFieldNew=new DisplayField("",100,DisplayFieldCategory.OrthoChart);
			...
			var orthoChartTabFieldsOrphaned=_listOrthoChartTabDisplayFields.Find(x => x.OrthoChartTab==null);
			for(var i=0;i<listAvailable.SelectedIndices.Count;i++) {
				var displayField=_listDisplayFieldsAvail[listAvailable.SelectedIndices[i]];
	...
	private void butRight_Click(object sender,EventArgs e) {
		...
		if(gridMain.SelectedIndices.Length==1){
			var displayField=(DisplayField)gridMain.ListGridRows[gridMain.SelectedIndices[0]].Tag;
			var isFieldInOtherTab=false;
			for(var j=0;j<_listOrthoChartTabDisplayFields.Count;j++) {
			...
			var listStringsInUse=new List<string>();
			for(var i=0;i<gridMain.SelectedIndices.Length;i++) {
				var index=gridMain.SelectedIndices[i];
				var displayField=(DisplayField)gridMain.ListGridRows[index].Tag;
				var isFieldInOtherTab=false;
				for(var j=0;j<_listOrthoChartTabDisplayFields.Count;j++) {
					if(_listOrthoChartTabDisplayFields[j].OrthoChartTab==null) {
						continue;//Do not consider the orphaned tab.
			...
			if(listStringsInUse.Count>0){
				var msg="You should not remove the following fields because they are already in use by patients:\r\n";
				for(var i=0;i<listStringsInUse.Count;i++){
		...
		var listDisplayFieldsRemoved=new List<DisplayField>();
		var orthoChartTabFields=GetSelectedFields();
		for(var i=gridMain.SelectedIndices.Length-1;i>=0;i--) {//go backwards
			var index=gridMain.SelectedIndices[i];
			var displayField=(DisplayField)gridMain.ListGridRows[index].Tag;
		...
		for(var i=0;i<listDisplayFieldsRemoved.Count();i++) {
			var isFieldOrphaned=true;
			for(var j=0;j<_listOrthoChartTabDisplayFields.Count;j++) {
				if(_listOrthoChartTabDisplayFields[j].OrthoChartTab==null) {
					continue;//Do not consider the orphaned tab.
				}
			...
			if(orthoChartTabFields==null) {//An orphaned list doesn't exist yet so create one.
				var orphanedFields=new OrthoChartTabFields();
				orphanedFields.OrthoChartTab=null;//These are fields not associated to any tab.  Purposefully use null.
				orphanedFields.ListDisplayFields=
				[
					listDisplayFieldsRemoved[i]
				];
	...
	private void butUp_Click(object sender,EventArgs e) {
		...
		var intArrayIndices=new int[gridMain.SelectedIndices.Length];
		for(var i=0;i<gridMain.SelectedIndices.Length;i++){
			intArrayIndices[i]=gridMain.SelectedIndices[i];
		}
		var orthoChartTabFields=GetSelectedFields();
		for(var i=0;i<intArrayIndices.Length;i++){
	...
	private void butDown_Click(object sender,EventArgs e) {
		...
		for(var i=intArrayIndices.Length-1;i>=0;i--) {//go backwards
	...
	private void butSave_Click(object sender,EventArgs e) {
		...
		var listDisplayFieldsAll=GetAllFields(false);
		...
		for(var i=_listOrthoChartTabLinks.Count-1;i>=0;i--) {
			var orthoChartTabLink=_listOrthoChartTabLinks[i];
			var orthoChartTabFields=_listOrthoChartTabDisplayFields.FirstOrDefault(x => x.OrthoChartTab!=null 
			...
			var displayField=orthoChartTabFields.ListDisplayFields.FirstOrDefault(x => x.DisplayFieldNum==orthoChartTabLink.DisplayFieldNum);
		...
		for(var i=0;i<_listOrthoChartTabDisplayFields.Count;i++) {
			...
			for(var j=0;j<_listOrthoChartTabDisplayFields[i].ListDisplayFields.Count;j++) {
				var orthoChartTabLink=_listOrthoChartTabLinks.FirstOrDefault(x => x.OrthoChartTabNum==_listOrthoChartTabDisplayFields[i].OrthoChartTab.OrthoChartTabNum 
		...
		if(orthoChartTabFieldsOrphaned!=null) {//An orphaned list actually exists.
			...
			var listDisplayFieldsToDelete=orthoChartTabFieldsOrphaned.ListDisplayFields.FindAll(x => x.DisplayFieldNum!=0);
