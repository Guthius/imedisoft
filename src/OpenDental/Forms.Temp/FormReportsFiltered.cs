...
public partial class FormReportsFiltered:FormODBase {
	...
	private void listBoxFavorites_MouseDown(object sender,MouseEventArgs e) {
		var selected=listBoxFavorites.IndexFromPoint(e.Location);
		if(selected==-1) {
			return;
		}
	...
	private void FillList() {
		...
		if(_listDisplayReports.Count>0) {
			var listGroupPermissions=GroupPermissions.GetPermsForReports(Security.CurUser);
			...
			var hasAllPerm=listGroupPermissions.Any(x => x.FKey==0);
			if(!hasAllPerm) {
				_listDisplayReports.RemoveAll(x => !listGroupPermissions.Exists(y => y.FKey==x.DisplayReportNum));
			}
		}
		for(var i=0;i<_listDisplayReports.Count;i++) {
