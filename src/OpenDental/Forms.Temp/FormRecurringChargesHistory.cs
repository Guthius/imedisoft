...
public partial class FormRecurringChargesHistory:FormODBase {
	...
	private void FormRecurringChargesHistory_Load(object sender,EventArgs e) {
		...
		comboAutomated.Items.Add(Lans.g("Automated and Manual"));
		comboAutomated.Items.Add(Lans.g("Automated Only"));
		comboAutomated.Items.Add(Lans.g("Manual Only"));
	...
	private void FillGrid() {
		...
		for(var i=0;i<listRecurringCharges.Count;i++) {
			var isAutomated=(listRecurringCharges[i].UserNum==0);
			if(!datePicker.IsInDateRange(listRecurringCharges[i].DateTimeCharge)) {
				continue;
			}
			if(true && !comboClinics.ListClinicNumsSelected.Contains(listRecurringCharges[i].ClinicNum)){
				continue;
			}
			...
			var patient = _listPatients.Find(x => x.PatNum == listRecurringCharges[i].PatNum);
			if(patient is null){
				row.Cells.Add(Lans.g("UNKNOWN"));
			}
			else{
				row.Cells.Add(patient.GetNameLF());
			}
			if(true) {
				row.Cells.Add(Clinics.GetFirstOrDefault(x => x.Id==listRecurringCharges[i].ClinicNum)?.Description??"");
			}
			row.Cells.Add(listRecurringCharges[i].DateTimeCharge.ToString());
			row.Cells.Add(Lans.g(listRecurringCharges[i].ChargeStatus.GetDescription()));
	...
	private void contextMenu_Popup(object sender,EventArgs e) {
		var recurringCharge=gridMain.SelectedTag<RecurringCharge>();
	...
	private void menuItemGoTo_Click(object sender,EventArgs e) {
		...
		GlobalFormOpenDental.GoToModule(EnumModuleType.Account,patNum:recurringCharge.PatNum);
	}
	...
	private void menuItemOpenPayment_Click(object sender,EventArgs e) {
		...
		var payment=Payments.GetPayment(recurringCharge.PayNum);
	...
	private void menuItemDeletePending_Click(object sender,EventArgs e) {
		...
		var reccuringChargeDb=RecurringCharges.GetOne(recurringCharge.RecurringChargeNum);
