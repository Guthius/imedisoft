...
public partial class FormLabCases : FormODBase {
	...
	private void FillGrid() {
		...
		GridRow row;
		var operatoryNums = new List<long>();
		if(comboClinic.IsAllSelected && !Security.CurUser.ClinicIsRestricted) {//"All"
			...
			for(var i=0;i<comboClinic.ListClinicNumsSelected.Count;i++) {
		...
		for(var i=0;i<_table.Rows.Count;i++){
			...
			var clinicNum=SIn.Long(_table.Rows[i]["ClinicNum"].ToString());
	...
	private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		var row=(DataRow)gridMain.ListGridRows[e.Row].Tag;
		var selectedLabCase=SIn.Long(row["LabCaseNum"].ToString());
	...
	private void menuItemGoTo_Click(object sender,EventArgs e) {
		...
		var row=gridMain.SelectedTag<DataRow>();
		if(row["AptNum"].ToString()=="0") {
			MsgBox.Show(this,"There are no appointments for unattached lab cases.");
			return;
		}
		var appointment=Appointments.GetOneApt(SIn.Long(row["AptNum"].ToString()));
