...
public partial class FormClaimProc : FormODBase {
	...
	public void Initialize() {
		...
		if(_claimProc.IsOverpay) {
			...
			var isOverpaid=(_claimProc.InsEstTotalOverride<0);
			var listClaimProcs=ClaimProcs.RefreshForClaims([_claimProc.ClaimNum]).ToList();
			var formClaimOverpay=new FormClaimOverpay(_claimProc.ClaimNum,listClaimProcs,_patient.PatNum,isOverpaid,_claimProc.ClaimProcNum);
			formClaimOverpay.ShowDialog();
			DialogResult=DialogResult.OK;
			return;
		}
		if(_claimProc.ClaimNum>0) {
			var claim=Claims.GetClaim(_claimProc.ClaimNum);
			if(claim==null) {
				MsgBox.Show(this,"Claim has been deleted by another user.");
				DialogResult=DialogResult.Abort;
				return;
		...
		var insSub=InsSubs.GetSub(_claimProc.InsSubNum,_listInsSub);
		...
		var dateWriteoffSec=_claimProc.SecDateEntry;//if this is a total payment, there is no proc so use ClaimProcCur.SecDateEntry
		...
		if(_claimProc.ClaimNum>0) {//attached to claim
			...
			for(var i=0;i<panelEstimateInfo.Controls.Count;i++) {
				panelEstimateInfo.Controls[i].Visible=false;
			}
			for(var i=0;i<groupClaimInfo.Controls.Count;i++) {
		...
		for(var i=0;i<_listDefsPayTracks.Count;i++) {
	...
	private void FillInitialAmounts(){
		if(IsProc){
			...
			var insPlan=InsPlans.GetPlan(_claimProc.PlanNum,_listInsPlans);
			var insFeeSchedNum=FeeScheds.GetFeeSched(_patient,_listInsPlans,_listPatPlans,_listInsSub,_procedure.ProvNum);
			...
			var stringProcCode=ProcedureCodes.GetStringProcCode(_procedure.CodeNum);
			//int codeNum=proc.CodeNum;
			var substCodeNum=_procedure.CodeNum;
	...
	private void FillAllowed(BlueBookEstimateData blueBookEstimateData=null){
		if(IsProc){
			...
			var allowed=InsPlans.GetAllowedForProc(_procedure,_claimProc,_listInsPlans,GetListSubLinks(),null,blueBookEstimateData,appointment);
			if(allowed==-1){
	...
	private void butUpdateAllowed_Click(object sender, System.EventArgs e) {
		...
		var fee=Fees.GetFee(_procedure.CodeNum,feeSchedNum,_procedure.ClinicNum,_procedure.ProvNum);
		using var formFeeEdit=new FormFeeEdit();
		if(fee==null) {
			var feeSched=FeeScheds.GetFirst(x => x.FeeSchedNum==feeSchedNum);
			fee=new Fee();
		...
		var datePrevious=fee.SecDateTEdit;
	...
	private void ComputeAmounts(){
		...
		if(IsProc) {
			...
			textPatPortion1.Text=ClaimProcs.GetPatPortion(_procedure, [_claimProc]).ToString("f");
		}
		...
		if(groupClaimInfo.Visible){
			...
			if(IsProc) {
				//Compute the patient portion ignorant of other claimprocs and adjustments to preserve old behavior.
				textPatPortion2.Text=ClaimProcs.GetPatPortion(_procedure, [_claimProc]).ToString("f");
	...
	private void comboStatus_SelectionChangeCommitted(object sender,EventArgs e) {
		...
		if(_claimProcOld.Status==ClaimProcStatus.Supplemental) {
			...
			var listClaimProcsForClaims=ClaimProcs.RefreshForClaim(_claimProcOld.ClaimNum)
	...
	private void textWriteOff_Enter(object sender,EventArgs e) {
		...
		var dateWriteoffSec=_claimProc.SecDateEntry;
		if(IsProc && _procedure.ProcDate!=DateTime.MinValue) {
			dateWriteoffSec=_procedure.DateEntryC;
		}
	...
	private void checkPayPlan_Click(object sender,EventArgs e) {
		if(checkPayPlan.Checked) {
			var listPayPlans=PayPlans.GetValidInsPayPlans(_claimProc.PatNum,_claimProc.PlanNum,_claimProc.InsSubNum,_claimProc.ClaimNum);
	...
	private List<ClaimProc> GetListClaimProcHypothetical() {
		var listClaimProcHypothetical=new List<ClaimProc>();
		var claimProcHypothetical=_claimProc.Copy();
	...
	private void ButBlueBookLog_Click(object sender,EventArgs e) {
		var listInsBlueBookLogs=InsBlueBookLogs.GetAllByClaimProcNum(_claimProc.ClaimProcNum);
		var insBlueBookLog=BlueBookEstimateData_.CreateInsBlueBookLog(_claimProc,false);
		...
		using var formClaimProcBlueBookLog=new FormClaimProcBlueBookLog(listInsBlueBookLogs);
	...
	private void butSave_Click(object sender, System.EventArgs e) {
		...
		var claimWriteoffTotal=ClaimProcs.GetClaimWriteOffTotal(_claimProc.ClaimNum,_claimProc.ProcNum, [_claimProc]);
		if(claimWriteoffTotal+_claimProc.WriteOff<0) {
			MsgBox.Show(this,"The current write-off value will cause the procedure's total write-off to be negative.  Please change it to at least "+(_claimProc.WriteOff-(claimWriteoffTotal+_claimProc.WriteOff))+" to continue.");
			return;
		}
		...
		if(_doSaveToDb) {
			...
			var insBlueBookLog=BlueBookEstimateData_.CreateInsBlueBookLog(_claimProc);
			if(insBlueBookLog!=null) {
	...
	private void MakeAuditTrailEntries() {
		...
		var writeoffEstOld=ClaimProcs.GetWriteOffEstimate(_claimProcOld);//WriteOffEst is never user editable, we have to check overrides
		var writeoffEstCur=ClaimProcs.GetWriteOffEstimate(_claimProc);//WriteOffEst is never user editable, we have to check overrides
