...
public partial class FormProcBandingSelect:FormODBase {
	...
	private List<Procedure> _listProceduresTpBanding= [];
	...
	private void FillGrid() {
		...
		GridRow row;
		for(var i=0;i<_listProceduresTpBanding.Count;i++) {
			row=new GridRow();
			var procedureCode=ProcedureCodes.GetProcCode(_listProceduresTpBanding[i].CodeNum);
