...
public partial class FormDatabaseMaintTemp:FormODBase {
	...
	private void FillDatabaseNames(){
		...
		var listDbNames=DatabaseMaintenances.GetDatabaseNames();
		for(var i=0;i<listDbNames.Count;i++){
	...
	private void butPrint_Click(object sender,EventArgs e) {
		var fileName=CodeBase.ODFileUtils.CombinePaths(PrefC.GetTempFolderPath(),"results.txt");
	...
	private void butFix3_Click(object sender,EventArgs e) {
		Cursor=Cursors.WaitCursor;
		textResults.Text=DatabaseMaintenances.FixMissingClaimProcs();
		Cursor=Cursors.Default;
	}
