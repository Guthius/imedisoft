...
public partial class FormWebSchedCarrierRuleCopyToClinics:FormODBase {
	private ClinicDto _clinicCopyFrom;
	...
	private void FormWebSchedCarrierRuleCopyToClinics_Load(object sender, EventArgs e) {
		...
		_clinicCopyFrom=comboClinicsFrom.GetSelected<ClinicDto>();
	...
	private void FillGridWebSchedCarrierRules(bool getRulesFromDb=false) {		
		if(getRulesFromDb) {
			_listWebSchedCarrierRules=WebSchedCarrierRules.GetWebSchedCarrierRulesForClinic(_clinicCopyFrom.Id);
		}
	...
	private void CopyRulesHelper(List<ClinicDto> listClinicsTo) {
		var clinicNumFrom=comboClinicsFrom.GetSelected<ClinicDto>().Id;
		var logMessage="";
		var listWebSchedCarrierRulesFromClinic=gridWebSchedCarrierRules.SelectedTags<WebSchedCarrierRule>();
		var listWebSchedCarrierRulesToClinics=WebSchedCarrierRules.GetWebSchedCarrierRulesForClinics(listClinicsTo.Select(x => x.Id).ToList());
		if(checkOverride.Checked) {
			...
			var listWebSchedCarrierRulesToDelete=listWebSchedCarrierRulesToClinics
		...
		var listWebSchedCarrierRulesToInsert=new List<WebSchedCarrierRule>();//Create a master insert list for one InsertMany statement below
		//Copy all rules of the from clinic to all selected clinics.
		for(var i=0;i<listClinicsTo.Count;i++) {
			...
			var listWebSchedCarrierRulesTempCopy=listWebSchedCarrierRulesFromClinic.Select(x => x.Copy()).ToList();
			//NOTE - update the clinic num of the deep copy.
			for(var j=0;j<listWebSchedCarrierRulesTempCopy.Count;j++) {
				listWebSchedCarrierRulesTempCopy[j].ClinicNum=listClinicsTo[i].Id;
	...
	private void comboClinicsFrom_SelectedIndexChanged(object sender,EventArgs e) {
		if(comboClinicsFrom.GetSelected<ClinicDto>()==null) {
			return;
		}
	...
	private void butCopyRules_Click(object sender,EventArgs e) {
		...
		var listClinics=listClinicsCopyTo.GetListSelected<ClinicDto>();
	...
	private void butCopyToAll_Click(object sender,EventArgs e) {
		...
		var listClinics=listClinicsCopyTo.Items.GetAll<ClinicDto>();
		CopyRulesHelper(listClinics);
	}
