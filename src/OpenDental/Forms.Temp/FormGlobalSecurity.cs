...
public partial class FormGlobalSecurity : FormODBase
{
    ...
    private void FormGlobalSecurity_Load(object sender, EventArgs e)
    {
        ...
        var listGroupsNotAdmin = UserGroups.GetList().FindAll(x => !GroupPermissions.HasPermission(x.UserGroupNum, EnumPermType.SecurityAdmin, 0));
        for (var i = 0; i < listGroupsNotAdmin.Count; i++)
        {
    ...
    {
        int logOffMinutes;
        var isLogOffMinutesParseValid = int.TryParse(textLogOffAfterMinutes.Text, out logOffMinutes);
