...
public partial class FormEmailInbox:FormODBase {
	...
	private void FormEmailInbox_Resize(object sender,EventArgs e) {
		...
		if(!_hasBeenScaledForZoom) {
			if(WindowState==FormWindowState.Normal) {
				...
				Height=735;//Maximum allowed height
				Width=1246;//Maximum allowed width
	...
	private void FillComboEmail() {
		...
		var defaultEmailAddressNum=PrefC.GetLong(PrefName.EmailDefaultAddressNum);
		//Get all addresses that are tied to either the current user or no user at all
		var listEmailAddressNumsForUserClinics=new List<long>();
		var listEmailAddressNumsAllClinics=new List<long>();
		if(true) {
			listEmailAddressNumsForUserClinics=Clinics.GetForUserod(Security.CurUser).Select(x=>x.EmailAddressId??0).ToList();
			listEmailAddressNumsAllClinics=Clinics.GetDeepCopy().Select(x => x.EmailAddressId??0).ToList();
		}
		var listEmailAddresses=EmailAddresses.GetDeepCopy().Where(x=>
		...
		var isPreviousAddressAvailable=listEmailAddresses.Any(x => x.EmailAddressNum==emailAddressNumPreviouslySelected);//Checks if previously selected email address still available in comboBox
	...
	private void FillGridInbox() {
		...
		var listEmailMessageNumsSelected=new List<long>();
		for(var i=0;i<gridInbox.SelectedIndices.Length;i++) {
			var emailMessage=(EmailMessage)gridInbox.ListGridRows[gridInbox.SelectedIndices[i]].Tag;
			listEmailMessageNumsSelected.Add(emailMessage.EmailMessageNum);
		}
		var sortByColIdx=gridInbox.GetSortedByColumnIdx();
		var isSortAsc=gridInbox.IsSortedAscending();
		if(sortByColIdx==-1) {
		...
		var widthColDateReceived=140;
		var widthColMessageType=120;
		var widthColFrom=200;
		var widthColSig=40;
		var widthColPatient=140;
		...
		if(!listEmailMessagesFiltered.IsNullOrEmpty()) {
			var listPatNums=listEmailMessagesFiltered.Select(x => x.PatNum).ToList();
		...
		for(var i=0;i<listEmailMessagesFiltered.Count;i++) {
			var emailMessage=listEmailMessagesFiltered[i];
			...
			var sigTrust="";//Blank for no signature, N for untrusted signature, Y for trusted signature.
			for(var j=0;j<emailMessage.Attachments.Count;j++) {
			...
			var patNumRegardingPatient=emailMessage.PatNum;
		...
		for(var i=0;i<gridInbox.ListGridRows.Count;i++) {
			var emailMessage=(EmailMessage)gridInbox.ListGridRows[i].Tag;
	...
	private void FillGridSent() {
		...
		var listEmailMessageNumsSelected=new List<long>();
		for(var i=0;i<gridSent.SelectedIndices.Length;i++) {
			var emailMessage=(EmailMessage)gridSent.ListGridRows[gridSent.SelectedIndices[i]].Tag;
			listEmailMessageNumsSelected.Add(emailMessage.EmailMessageNum);
		}
		//sorting
		var sortByColIdx=gridSent.GetSortedByColumnIdx();
		var isSortAsc=gridSent.IsSortedAscending();
		if(sortByColIdx==-1) {
			//Default to sorting by Date Received descending.
			sortByColIdx=3;
			isSortAsc=false;
		...
		var widthColSent=180;
		var widthColDateSent=120;
		var widthColMsg=95;
		var widthColAutomatedSig=40;
		var widthColPatient=140;
		gridSent.BeginUpdate();
		gridSent.Columns.Clear();
		//add columns
		var col=new GridColumn("Sent From",widthColSent,HorizontalAlignment.Left);
		...
		var emailAddressSelected=GetSelectedAddress();
		...
		if(!listEmailsFiltered.IsNullOrEmpty()) {
			var listPatNums=listEmailsFiltered.Select(x => x.PatNum).ToList();
			listPatNums.AddRange(listEmailsFiltered.Select(x => x.PatNumSubj).ToList());
			RefreshPatientsList(listPatNums.Distinct().ToList());
		}
		//add rows
		for(var i=0;i<listEmailsFiltered.Count;i++) {
			...
			var sigTrust="";
			for(var j=0;j<listEmailsFiltered[i].Attachments.Count;j++) {
			...
			var patNum=listEmailsFiltered[i].PatNum;
			//Webmail messages should list the patient as the PatNumSubj, which means "the patient whom this message is regarding".
		...
		for(var i=0;i<gridSent.ListGridRows.Count;i++) {
			var emailMessage=(EmailMessage)gridSent.ListGridRows[i].Tag;
			if(listEmailMessageNumsSelected.Contains(emailMessage.EmailMessageNum)) {
	...
	private void InitEmailHideInFlags() {
		...
		for(var i=0;i<_listHideInFlags.Count;i++) {
	...
	private void RefreshShowIn() {
		...
		for(var i=0;i<_listHideInFlags.Count;i++) {
			...
			if(emailMessageCur==null) {
				...
				var isShowing=!emailMessageCur.HideIn.HasFlag(_listHideInFlags[i]);//HideInFlags logic inverted for "showing"
		...
		for(var r=0;r<GetActiveGrid().SelectedGridRows.Count;r++) {
			var emailMessage=(EmailMessage)GetActiveGrid().SelectedGridRows[r].Tag;
	...
	private void FillSearch() {
		var searchBody=textSearchBody.Text;
		var searchEmail=textSearchEmail.Text;
		var dateSearchFrom=SIn.Date(textDateFrom.Text); //returns MinVal if empty or invalid.
		var dateSearchTo=SIn.Date(textDateTo.Text); //returns MinVal if empty or invalid.
		var listEmailMessagesInboxSearched=new List<EmailMessage>();
		var listEmailMessagesSentSearched=new List<EmailMessage>();
		if(searchBody!="") {
			...
			var listEmailMessagesSearched=EmailMessages.GetBySearch(_patNum,searchEmail,dateSearchFrom,dateSearchTo,searchBody,checkSearchAttach.Checked);
			//inbox emails
			...
			//Filter Inbox Emails
			for(var i=0;i<_listEmailMessagesInbox.Count;i++) {
				if(_patNum!=0) {
			...
			//Filter Sent Emails
			for(var i=0;i<_listEmailMessagesSent.Count;i++) {
				if(_patNum!=0) {
	...
	private void gridInboxSent_CellClick(object sender,UI.ODGridClickEventArgs e) {
		...
		var emailPreviewControlActive=(GetActiveMailbox()==MailboxType.Inbox)?emailPreviewControlInbox:emailPreviewControlSent;
		SetButtonsEnabled();
		RefreshShowIn();
		...
		var emailMessage=(EmailMessage)GetActiveGrid().ListGridRows[e.Row].Tag;
		if(EmailMessages.IsSecureWebMail(emailMessage.SentOrReceived)) {
		...
		for(var i=0;i<emailMessage.Attachments.Count;i++) {
			...
			using var formEmailDigitalSignature=new FormEmailDigitalSignature(x509Certificate2);
	...
	private void gridInboxSent_CellDoubleClick(object sender,UI.ODGridClickEventArgs e) {
		...
		if(EmailMessages.IsSecureWebMail(emailMessage.SentOrReceived)) {
			...
			using var formWebMailMessageEdit=new FormWebMailMessageEdit(emailMessage.PatNum,emailMessage);
			...
			var formEmailMessageEdit=new FormEmailMessageEdit(emailMessage,GetSelectedAddress(),false);
	...
	private void butChangePat_Click(object sender,EventArgs e) {
		...
		Cursor=Cursors.WaitCursor;
		for(var i=0;i<GetActiveGrid().SelectedIndices.Length;i++) {
			var emailMessage=(EmailMessage)GetActiveGrid().ListGridRows[GetActiveGrid().SelectedIndices[i]].Tag;
			emailMessage.PatNum=frmPatientSelect.PatNumSelected;
			EmailMessages.UpdatePatNum(emailMessage);
		}
		var countMsgsMoved=GetActiveGrid().SelectedIndices.Length;
		FillInboxOrSent();
	...
	private void RefreshInboxEmailList() {
		var dateFrom=SIn.Date(textDateFrom.Text); //returns MinVal if empty or invalid.
		var dateTo=SIn.Date(textDateTo.Text); //returns MinVal if empty or invalid.
	...
	private void RefreshLists() {
		var dateFrom=SIn.Date(textDateFrom.Text); //returns MinVal if empty or invalid.
		var dateTo=SIn.Date(textDateTo.Text); //returns MinVal if empty or invalid.
		var listEmailsForSelection=EmailMessages.GetMailboxForAddress(GetSelectedAddress(),dateFrom,dateTo,MailboxType.Inbox,MailboxType.Sent);
	...
	private void RefreshPatientsList(List<long> listPatNums) {
		var listPatNumsNew=listPatNums.Except(_listPatients.Select(x=>x.PatNum)).ToList();//not already in list
		var listPatientsRefresh=Patients.GetLimForPats(listPatNumsNew);
		for(var i=0;i<listPatientsRefresh.Count;i++) {
	...
	private string GetPatientName(long patNum) {
		...
		var patient=_listPatients.Find(x=>x.PatNum==patNum);
		if(patient!=null) {
			return patient.GetNameLF();
		}
		var listPatNumsNew=new List<long>();
	...
	private void butPickPat_Click(object sender,EventArgs e) {
		...
		if(frmPatientSelect.IsDialogOK) {
			...
			textSearchPat.Text=GetPatientName(frmPatientSelect.PatNumSelected);
		}
	}
	...
	private void butCompose_Click(object sender,EventArgs e) {
		...
		var formEmailMessageEdit=new FormEmailMessageEdit(emailMessage,GetSelectedAddress(),true,null,_listEmailMessagesInbox,_listEmailMessagesSent);
	...
	private void ReplyClickHelper(bool isReplyAll=false) {
		...
		var emailMessageSelected=EmailMessages.GetOne(((EmailMessage)gridInbox
			.ListGridRows[gridInbox.GetSelectedIndex()].Tag).EmailMessageNum);//Refresh from the database to get the full body text.
		var isSecureEmail=EmailMessages.IsSecureEmail(emailMessageSelected.SentOrReceived);
		isReplyAll|=isSecureEmail;//Secure Email is ALWAYS 'Reply All'
		var formEmailMessageEdit=new FormEmailMessageEdit(EmailMessages.CreateReply(emailMessageSelected,GetSelectedAddress(),isReplyAll)
	...
	private void butForward_Click(object sender,EventArgs e) {
		...
		var emailMessageSelected=EmailMessages.GetOne(((EmailMessage)gridInbox
			.ListGridRows[gridInbox.GetSelectedIndex()].Tag).EmailMessageNum);//Refresh from the database to get the full body text.
		var formEmailMessageEdit=new FormEmailMessageEdit(EmailMessages.CreateForward(emailMessageSelected,GetSelectedAddress()),GetSelectedAddress(),true
	...
	protected override void ProcessSignalODs(List<Signalod> signals) {
		if(signals.Exists(x => x.IType==InvalidType.Email)) {
			Cursor=Cursors.WaitCursor;
			//an address may have changed. refill the combobox
			FillComboEmail();
		}
		if(signals.Exists(x => x.IType==InvalidType.EmailMessages && x.FKeyType==KeyType.EmailAddress && x.FKey==GetSelectedAddress().EmailAddressNum)) {
			Text="Email Client for "+GetSelectedAddress().EmailUsername;
			_isRefreshInbox=true;
			FillGridInbox();
		}
		if(signals.Exists(x => x.IType==InvalidType.EmailMessages && x.FKeyType==KeyType.Undefined)) {
			Cursor=Cursors.WaitCursor;
			_isRefreshInbox=true;
			_isRefreshSent=true;
			FillGridInbox();
	...
	private HideInFlags GetHideInFlagsFromListBox() {
		var hideInFlags=HideInFlags.None;
		for(var i=0;i<listShowIn.SelectedIndices.Count;i++) {
	...
	private void listHideInFlags_MouseClick(object sender,MouseEventArgs e) {
		...
		var hideInFlags=GetHideInFlagsFromListBox();
		for(var i=0;i<GetActiveGrid().SelectedGridRows.Count;i++) {
			...
			var emailMessageOld=emailMessage.Copy();
	...
	private void butDelete_Click(object sender,EventArgs e) {
		...
		for(var i=0;i<GetActiveGrid().SelectedIndices.Length;i++) {
			...
			if(emailMessage.PatNum!=0) {
				...
				EmailMessages.Update(emailMessage,emailMessageOld:emailMessageOld);
				continue;
			}
			if(EmailMessages.IsSecureWebMail(emailMessage.SentOrReceived)) {
