...
public partial class FormRecallEdit : FormODBase {
	...
	private void FormRecallEdit_Load(object sender, System.EventArgs e) {
		...
		_listDefsRecallUnschedStatus=Defs.GetDefsForCategory(DefCat.RecallUnschedStatus,true);
		for(var i=0;i<_listDefsRecallUnschedStatus.Count;i++){
			comboStatus.Items.Add(_listDefsRecallUnschedStatus[i].ItemName);
			if(_listDefsRecallUnschedStatus[i].DefNum==RecallCur.RecallStatus){
	...
	private void comboType_SelectionChangeCommitted(object sender,EventArgs e) {
		...
		var interval=_listRecallTypes[comboType.SelectedIndex].DefaultInterval;
		textYears.Text=interval.Years.ToString();
		textMonths.Text=interval.Months.ToString();
		textWeeks.Text=interval.Weeks.ToString();
		textDays.Text=interval.Days.ToString();
		var listRecallTriggers=RecallTriggers.GetForType(_listRecallTypes[comboType.SelectedIndex].RecallTypeNum);
		if(listRecallTriggers.Count>0){
			return;
		}
		//if no triggers, then it's a manual type
		RecallCur.DatePrevious=DateTime.Today;
		//textDatePrevious.Text=DateTime.Today.ToShortDateString();
		var dateTimeDue=DateTime.Today+interval;
		textDateDue.Text=dateTimeDue.ToShortDateString();
	}
	...
	private void butSave_Click(object sender, System.EventArgs e) {
		...
		for(var i=0;i<textPattern.Text.Length;i++) {
		...
		var disableUntilBalance=SIn.Double(textBalance.Text);
		...
		if(comboStatus.SelectedIndex==0){
			...
			RecallCur.RecallStatus=_listDefsRecallUnschedStatus[comboStatus.SelectedIndex-1].DefNum;
		}
		RecallCur.Note=textNote.Text;
		RecallCur.Priority=RecallPriority.Normal;
		if(checkASAP.Checked) {
			RecallCur.Priority=RecallPriority.ASAP;
		}
		if(IsNew){
