...
public partial class FormQueryFavorites:FormODBase {
	...
	private void FormQueryFavorites_SizeChanged(object sender,EventArgs e) {
		splitContainer.SplitterDistance=454;
	}
	...
	private void FillGrid(bool doRefreshList=true,bool isScrollToSelection=true) {
		...
		var isAuthorized=Security.IsAuthorized(EnumPermType.UserQueryAdmin,true);
		var stringArraySearchTerms = Regex.Split(textSearch.Text,@"\W");//matches any non-word character
		//get all queries that contain ALL of the search terms entered, either in the query text or the query description.
		var listUserQueryDisplay = _listUserQueries
		...
		for(var i=0;i<listUserQueryDisplay.Count;i++) {
		...
		gridMain.EndUpdate();
		var indexSelected=gridMain.ListGridRows.Select(x => (UserQuery)x.Tag).ToList().FindIndex(y => y.QueryNum==selectedQueryNum);
		if(indexSelected>-1) {
	...
	private void gridMain_CellClick(object sender,ODGridClickEventArgs e) {
		if(e.Col==1) {//Released Column
			var userQuery=gridMain.SelectedTag<UserQuery>();
	...
	private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		...
		if(UserQueryCur.IsPromptSetup && UserQueries.ParseSetStatements(UserQueryCur.QueryText).Count > 0) {
			...
			using var formQueryParser = new FormQueryParser(UserQueryCur);
			formQueryParser.ShowDialog();
			if(formQueryParser.DialogResult==DialogResult.OK) {
				var formUserQuery = new FormUserQuery(UserQueryCur.QueryText,submitQueryOnLoad:true);
