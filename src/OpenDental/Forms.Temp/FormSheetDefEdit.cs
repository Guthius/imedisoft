...
public partial class FormSheetDefEdit:FormODBase {
	...
	private void butAlignCenterH_Click(object sender,EventArgs e) {
		...
		var listSheetFieldDefsShowing=listBoxFields.Items.GetAll<SheetFieldDef>();
		var listSheetFieldDefsSelected=new List<SheetFieldDef>();
		for(var i=0;i<listBoxFields.SelectedIndices.Count;i++) {
		...
		var minX=listSheetFieldDefsSelected.Min(d => d.BoundsF.Left);
		var maxX=listSheetFieldDefsSelected.Max(d => d.BoundsF.Right);
		var avgX=(int)((minX+maxX)/2);
	...
	private void butAlignLeft_Click(object sender,EventArgs e) {
		...
		var minX=(int)listSheetFieldDefsSelected.Min(d => d.BoundsF.Left);
	...
	private void butAlignRight_Click(object sender,EventArgs e) {
		...
		var maxX=(int)listSheetFieldDefsSelected.Max(d => d.BoundsF.Right);
	...
	private void butAlignTop_Click(object sender,EventArgs e) {
		...
		var minY=(int)listSheetFieldDefsSelected.Min(d => d.BoundsF.Top);
	...
	private void butAutoSnap_Click(object sender,EventArgs e) {
		var prompt="Please enter auto snap column width between 10 and 100, or 0 to disable.";
		...
		var funcOkClick=new Func<string, bool>((inputVal)=> { 
			var val=SIn.Int(inputVal,false);
	...
	private void butDelete_Click(object sender,EventArgs e) {
		...
		if(!GetSelectedLanguageThreeLetters().IsNullOrEmpty()){//User clicked 'Delete' while viewing a translated view.
			var msg=$"You are about to delete the entire {GetSelectedLanguageDisplay()} language translation for this sheet.\n"
	...
	private void butEdit_Click(object sender,EventArgs e) {
		using var formSheetDef=new FormSheetDef();
	...
	private void ShowMobile(){
		var rectangleWorkingArea=System.Windows.Forms.Screen.FromControl(this).WorkingArea;
		var widthBoth=Width+_sheetEditMobileCtrl.Width;
		var rectangleBoth=new Rectangle(
		...
		var rectangleFloat=new Rectangle(Right,rectangleWorkingArea.Top,_sheetEditMobileCtrl.Width,rectangleWorkingArea.Height);
	...
	private void butPageRemove_Click(object sender,EventArgs e) {
		...
		var lowestYPos=SheetUtil.GetLowestYPos(SheetDef_.SheetFieldDefs);
		var minimumPageCount=0;
		var arbitraryHeight=lowestYPos-_margins.Bottom;//-60;
		var onePageHeight=SheetDef_.Height;
	...
	private void butRedo_Click(object sender,EventArgs e) {
		...
		var idxTarget=_listUndoLevels.Count-_undoLevel;
	...
	private void butTabOrder_Click(object sender,EventArgs e) {
		...
		if(_isTabMode) {
			...
			_listSheetFieldDefsTabOrder= []; //clear or create the list of tab orders.
		}
		else {
	...
	private void butUndo_Click(object sender,EventArgs e) {
		...
		var listSheetFieldDefNumsSelected=listBoxFields.GetListSelected<SheetFieldDef>().Select(x=>x.SheetFieldDefNum).ToList();
		var idxTarget=_listUndoLevels.Count-2-_undoLevel;
		...
		if(SheetDef_.SheetFieldDefs.Count!=_listUndoLevels[idxTarget].ListSheetFieldDefs.Count){
			...
			for(var i=0;i<SheetDef_.SheetFieldDefs.Count;i++){
		...
		if(isOnlyMovements){
			for(var i=0;i<listBoxFields.Items.Count;i++){
				var sheetFieldDefOld=(SheetFieldDef)listBoxFields.Items.GetObjectAt(i);
				var sheetFieldDefNew=SheetDef_.SheetFieldDefs.Find(x=>x.SheetFieldDefNum==sheetFieldDefOld.SheetFieldDefNum);
			...
			for(var i=0;i<listBoxFields.Items.Count;i++){
				...
				var sheetFieldDef=(SheetFieldDef)listBoxFields.Items.GetObjectAt(i);
	...
	private void comboLanguages_SelectionChangeCommitted(object sender,EventArgs e) {
		var selectedThreeLetterLanguage=GetSelectedLanguageThreeLetters();
		var wasTranslationAdded=false;
		...
		var listSheetFieldDefsTranslatedFields=SheetDef_.SheetFieldDefs.FindAll(x => x.Language==selectedThreeLetterLanguage);
	...
	private void FormSheetDefEdit_FormClosing(object sender,FormClosingEventArgs e) {
		var listSheetFieldDefs=SheetDef_.SheetFieldDefs.FindAll(x=>x.FieldType==SheetFieldType.Image);
	...
	private void FormSheetDefEdit_Load(object sender,EventArgs e) {
		if(SheetDef_.IsLandscape){
			Width=SheetDef_.Height+315;
			Height=SheetDef_.Width+65;
		}
		else{
			Width=SheetDef_.Width+315;
			Height=SheetDef_.Height+65;
		}
		if(Width<600){
			Width=600;
		}
		...
		var screen=System.Windows.Forms.Screen.FromHandle(this.Handle);
		...
		var listSheetFieldTypes=SheetDefs.GetVisibleButtons(SheetDef_.SheetType);
	...
	private void listFields_MouseDoubleClick(object sender,MouseEventArgs e) {
		var idx=listBoxFields.IndexFromPoint(e.Location);
		if(idx==-1) {
			return;
		}
		...
		var sheetFieldDef=listSheetFieldDefs[0];
		var SheetFieldDefOld=sheetFieldDef.Copy();
	...
	private void panelMain_MouseDown(object sender,MouseEventArgs e) {
		var pointDoc=new Point(e.X,e.Y);
		if(_isAltDown) {
		...
		var listSheetFieldDefsShowing=listBoxFields.Items.GetAll<SheetFieldDef>();
		var idx=listSheetFieldDefsShowing.IndexOf(sheetFieldDef);
		if(_isCtrlDown) {
	...
	private void panelMain_MouseDoubleClick(object sender,MouseEventArgs e) {
		...
		Invalidate();
		var pointSheet=new Point(e.X,e.Y);
		var sheetFieldDef=HitTest(pointSheet.X,pointSheet.Y);
		if(sheetFieldDef==null) {
			return;
		}
		LaunchEditWindow(sheetFieldDef,isEditMobile:false);
	...
	private void panelMain_MouseMove(object sender,MouseEventArgs e) {
		var listSheetFieldDefsShowing=listBoxFields.Items.GetAll<SheetFieldDef>();
		if(_isResizing){
			var width=_sizeResizeOriginal.Width+(e.X-_pointMouseOriginalPos.X);
			var height=_sizeResizeOriginal.Height+(e.Y-_pointMouseOriginalPos.Y);
			if(width<5){
				width=5;
			}
			if(height<5){
		...
		for(var i=0;i<listBoxFields.SelectedIndices.Count;i++) {
			var sheetFieldDef=listSheetFieldDefsShowing[listBoxFields.SelectedIndices[i]];
			var pointOriginalLocation=_listPointsOfOriginalControls[i];
			var newX=pointOriginalLocation.X+(e.X-_pointMouseOriginalPos.X);
			var newY=pointOriginalLocation.Y+(e.Y-_pointMouseOriginalPos.Y);
	...
	private void panelMain_MouseUp(object sender,MouseEventArgs e) {
		...
		if(_isDraggingSelectionRect) { 
			...
			var rectangleSelectionBounds=new Rectangle(
			...
			for(var i=0;i<listSheetFieldDefsShowing.Count;i++) {
				var sheetFieldDef=listSheetFieldDefsShowing[i]; //to speed this process up instead of referencing the array every time.
				if(sheetFieldDef.FieldType==SheetFieldType.Line){
					if(Math.Abs(listSheetFieldDefsShowing[i].Width)<2){
						//vertical line
						var rectangleTempDefBounds=new Rectangle(sheetFieldDef.Bounds.X-1,sheetFieldDef.Bounds.Y,2,sheetFieldDef.Bounds.Height);
						...
						//horiz line
						var rectangleTempDefBounds=new Rectangle(sheetFieldDef.Bounds.X,sheetFieldDef.Bounds.Y-1,sheetFieldDef.Bounds.Width,2);
				...
				if(sheetFieldDef.FieldType==SheetFieldType.Rectangle) {
					var rectangleTempDefBounds=new Rectangle(sheetFieldDef.Bounds.X+4,sheetFieldDef.Bounds.Y+4,sheetFieldDef.Bounds.Width-8,sheetFieldDef.Bounds.Height-8);
	...
	private void panelMain_Paint(object sender,PaintEventArgs e) {
		...
		//Draw pagebreak
		using var penDashPage=new Pen(Color.Green);
		penDashPage.DashPattern= [4.0F,3.0F,2.0F,3.0F];
		using var penDashMargin=new Pen(Color.Green);
		penDashMargin.DashPattern= [1.0F,5.0F];
		var margins=_margins.Top+_margins.Bottom;
		for(var i=1;i<SheetDef_.PageCount+1;i++) {
			if(SheetDef_.PageCount==1) {
	...
	private void sheetEditMobile_SheetDefSelected(object sender,long sheetFieldDefNum) {
		...
		var listSheetFieldDefNums=new List<long>();
		for(var i=0;i<listSheetFieldDefsShowing.Count;i++){
	...
	private void butOutputText_Click(object sender,EventArgs e) {
		...
		var font=new Font(SheetDef_.FontName,SheetDef_.FontSize);
		using var formSheetFieldOutput=new FormSheetFieldOutput();
	...
	private void butInputField_Click(object sender,EventArgs e) {
		...
		var font=new Font(SheetDef_.FontName,SheetDef_.FontSize);
		using var formSheetFieldInput=new FormSheetFieldInput();
	...
	private void butStaticText_Click(object sender,EventArgs e) {
		var font=new Font(SheetDef_.FontName,SheetDef_.FontSize);
		using var formSheetFieldStatic=new FormSheetFieldStatic();
	...
	private void butCheckBox_Click(object sender,EventArgs e) {
		...
		using var formSheetFieldCheckBox=new FormSheetFieldCheckBox();
	...
	private void butComboBox_Click(object sender,EventArgs e) {
		using var formSheetFieldComboBox=new FormSheetFieldComboBox();
	...
	private void butLine_Click(object sender,EventArgs e) {
		using var formSheetFieldLine=new FormSheetFieldLine();
	...
	private void butRectangle_Click(object sender,EventArgs e) {
		using var formSheetFieldRect=new FormSheetFieldRect();
	...
	private void AddSignatureBox(SheetFieldType sheetFieldType) {
		using var formSheetFieldSigBox=new FormSheetFieldSigBox();
	...
	private void butSpecial_Click(object sender,EventArgs e) {
		using var formSheetFieldSpecial=new FormSheetFieldSpecial();
		...
		var isChartModuleSheetType=EnumTools.GetAttributeOrDefault<SheetLayoutAttribute>(SheetDef_.SheetType).IsChartModule;
	...
	private void butGrid_Click(object sender,EventArgs e) {
		var isChartModuleSheetType=EnumTools.GetAttributeOrDefault<SheetLayoutAttribute>(SheetDef_.SheetType).IsChartModule;
		using var formSheetFieldGrid=new FormSheetFieldGrid();
		...
		if(SheetDefs.IsDashboardType(SheetDef_)) {
			...
			frmSheetFieldGridType.SheetDefCur=SheetDef_;
			frmSheetFieldGridType.LayoutMode=_sheetFieldLayoutMode;
			frmSheetFieldGridType.ShowDialog();
			if(!frmSheetFieldGridType.IsDialogOK) {
				return;
			}
			formSheetFieldGrid.SheetFieldDefCur=SheetFieldDef.NewGrid(frmSheetFieldGridType.SheetGridTypeSelected,0,0,100,100); //is resized in dialog window.
	...
	private void CheckShowChecked(){
		var listSheetFieldDefsSelected=listBoxFields.GetListSelected<SheetFieldDef>();
		FillFieldList();
		for(var i=0;i<listBoxFields.Items.Count;i++){
	...
	private void AddNewSheetFieldDef(SheetFieldDef sheetFieldDef) {
		...
		var doAddToTranslatedViews=false;
	...
	private List<SheetFieldDef> DeepCopy(List<SheetFieldDef> listSheetFieldDefs){
		var listSheetFieldDefsRet=new List<SheetFieldDef>();
		for(var i=0;i<listSheetFieldDefs.Count;i++){
			var sheetFieldDef=listSheetFieldDefs[i].Copy();//does not copy ImageField
	...
	private void MoveSheetFieldDefs(SheetFieldDef sheetFieldDef,int newX,int newY) {
		if(checkSynchMatchedFields.Checked){
			if(HasTranslatedSheetDefS(sheetFieldDef,out var listSheetFieldDefs)){
				for(var i=0;i<listSheetFieldDefs.Count;i++){
	...
	private void CopyControlsToMemory() {
		...
		var listSheetFieldDefsShowing=listBoxFields.Items.GetAll<SheetFieldDef>();
		var strPrompt=Lan.g(this,"The following selected fields can cause conflicts if they are copied:\r\n");
		var isConflictingfield=false;
		for(var i=0;i<listBoxFields.SelectedIndices.Count;i++) {
			var sheetFieldDef=listSheetFieldDefsShowing[listBoxFields.SelectedIndices[i]];
		...
		_listSheetFieldDefsCopyPaste= []; //empty the remembered field list
		for(var i=0;i<listBoxFields.SelectedIndices.Count;i++) {
	...
	private void DrawAutoSnapLines(Graphics g) {
		...
		using var pen=new Pen(ColorOD.Gray(235));//Color.LightGray);//211
		for(var i=0;i<_listSnapXVals.Count();i++) {//Vertical lines
			g.DrawLine(pen,new Point(_listSnapXVals[i],0),new Point(_listSnapXVals[i],this.Height));
		}
		for(var i=0;i<_listSnapYVals.Count();i++) {//Horizontal lines
	...
	public void FillFieldList() {
		...
		var selectedLanguage=GetSelectedLanguageThreeLetters();
		for(var i=0;i<SheetDef_.SheetFieldDefs.Count();i++) {
			...
			switch(SheetDef_.SheetFieldDefs[i].FieldType) {
				...
				case SheetFieldType.PatImage:
					...
					break;
				case SheetFieldType.Line:
					txt=Lan.g(this,"Line:")+SheetDef_.SheetFieldDefs[i].XPos+","+SheetDef_.SheetFieldDefs[i].YPos+","+"W:"+SheetDef_.SheetFieldDefs[i].Width+","+"H:"+SheetDef_.SheetFieldDefs[i].Height;
					break;
				case SheetFieldType.Rectangle:
					txt=Lan.g(this,"Rect:")+SheetDef_.SheetFieldDefs[i].XPos+","+SheetDef_.SheetFieldDefs[i].YPos+","+"W:"+SheetDef_.SheetFieldDefs[i].Width+","+"H:"+SheetDef_.SheetFieldDefs[i].Height;
					break;
				case SheetFieldType.SigBox:
				...
				case SheetFieldType.SigBoxPractice:
					...
					break;
				case SheetFieldType.CheckBox:
					txt=SheetDef_.SheetFieldDefs[i].TabOrder+": ";
					...
					break;
				case SheetFieldType.ComboBox:
					txt=SheetDef_.SheetFieldDefs[i].TabOrder > 0 ? SheetDef_.SheetFieldDefs[i].TabOrder+": " : "";
					txt+=Lan.g(this,"ComboBox:")+SheetDef_.SheetFieldDefs[i].XPos+","+SheetDef_.SheetFieldDefs[i].YPos
					     +","+"W:"+SheetDef_.SheetFieldDefs[i].Width;
					break;
				case SheetFieldType.InputField:
					txt=SheetDef_.SheetFieldDefs[i].TabOrder+": "+SheetDef_.SheetFieldDefs[i].FieldName;
					break;
	...
	private List<SheetFieldDef> GetPertinentSheetFieldDefs(SheetDef sheetDef=null) {
		...
		var selectedLanguage=GetSelectedLanguageThreeLetters();
		var listSheetFieldDefs=new List<SheetFieldDef>();
	...
	private bool HasSelectedDiagonalLine(SheetFieldDef sheetFieldDef,Rectangle rectangleCheck) {
		...
		var lineYStart=(double)sheetFieldDef.Bounds.Y;//y value starting point
		var hasLongerHeight=false;
		//Determine the lines max length using either height or width, whichever is longer
		var lineMaxLength=Math.Abs(sheetFieldDef.Width);
		...
		var slope=((double)sheetFieldDef.Height+(double)sheetFieldDef.Bounds.Y-(double)sheetFieldDef.Bounds.Y)
		          /((double)sheetFieldDef.Width+(double)sheetFieldDef.Bounds.X-(double)sheetFieldDef.Bounds.X);
		var pointOnLine=new Point(0,0);
		for(var i=0;i<lineMaxLength;i++) {//Step pixel by pixel
	...
	private SheetFieldDef HitTest(int x,int y) {
		...
		for(var i=listSheetFieldDefsShowing.Count-1;i>=0;i--) {
			...
			if(listSheetFieldDefsShowing[i].FieldType==SheetFieldType.Line) {
				if(Math.Abs(listSheetFieldDefsShowing[i].Width)<2){
					...
					//diagonal line
					var rectClickedLocation=new Rectangle(x-1,y-1,3,3);//Create a rectangle 3x3 pixel area to make it easier to click on the line
			...
			var rectangleFieldDefBounds=listSheetFieldDefsShowing[i].Bounds;
	...
	private SheetFieldDef HitTestResize(int x,int y) {
		...
		var listSheetFieldDefsShowing=listBoxFields.Items.GetAll<SheetFieldDef>();
		var sheetFieldDef=listSheetFieldDefsShowing[listBoxFields.SelectedIndices[0]];
		if(sheetFieldDef.FieldType==SheetFieldType.Image) {
			return null;
		}
		...
		var rectangleHandleLR=new Rectangle(sheetFieldDef.Bounds.Right-2,sheetFieldDef.Bounds.Bottom-2,_sizeResizeGrabHandle,_sizeResizeGrabHandle);
	...
	private void InitializeComponentSheetEditMobile() {
		...
		_sheetEditMobileCtrl.HasMobileLayoutChanged+=new EventHandler<bool>((_,hasMobileLayout) => {
		...
		_sheetEditMobileCtrl.NewMobileHeader+=new EventHandler<SheetEditMobileCtrl.NewMobileFieldValueArgs>((_,e) => {
			var sheetFieldDef=SheetDef_.SheetFieldDefs.FirstOrDefault(x => x.SheetFieldDefNum==e.SheetFieldDefNum);
			if(sheetFieldDef!=null) {
				sheetFieldDef.UiLabelMobile=e.NewFieldValue;
				FillFieldList();
			}
		});
		_sheetEditMobileCtrl.NewStaticText+=new EventHandler<SheetEditMobileCtrl.NewMobileFieldValueArgs>((_,e) => {
			var sheetFieldDef=SheetDef_.SheetFieldDefs.FirstOrDefault(x => x.SheetFieldDefNum==e.SheetFieldDefNum);
			if(sheetFieldDef!=null) {
		...
		_sheetEditMobileCtrl.SheetFieldDefEdit+=new EventHandler<SheetEditMobileCtrl.SheetFieldDefEditArgs>((_,e) => {
			if(!SheetDef_.SheetFieldDefs.Any(x => e.SheetFieldDefNums.Contains(x.SheetFieldDefNum))) {
				return;
			}
			var listSheetFieldDefs=SheetDef_.SheetFieldDefs.FindAll(x => e.SheetFieldDefNums.Contains(x.SheetFieldDefNum));
			var sheetFieldDef=listSheetFieldDefs[0];
			...
			if(listSheetFieldDefs.Any(x => x.FieldType==SheetFieldType.CheckBox)) {
				var listHighlighted=_sheetEditMobileCtrl.GetHighlightedFieldDefs(listSheetFieldDefs.Select(x => x.SheetFieldDefNum).ToList());//should only come back with 1
	...
	private void LaunchEditWindow(SheetFieldDef sheetFieldDef,bool isEditMobile) {
		...
		var sheetFieldDefCopy=sheetFieldDef.Copy();//Keep a copy of the unaltered sheetFieldDef for comparison
		var hasClickedDelete=false;
		//not every field will have been saved to the database, so we can't depend on SheetFieldDefNum.
		var idx=SheetDef_.SheetFieldDefs.IndexOf(sheetFieldDef);
		switch(sheetFieldDef.FieldType) {
			case SheetFieldType.InputField:
				using(var formSheetFieldInput=new FormSheetFieldInput()) {
					formSheetFieldInput.SheetDefCur=SheetDef_;
					...
					if(sheetFieldDef.FieldValue.Contains("AutoNoteNum:")) {
						var autoNoteNum=sheetFieldDef.FieldValue.Substring(sheetFieldDef.FieldValue.IndexOf(":")+1);
		...
		if(sheetFieldDefCopy.FieldType!=SheetFieldType.MobileHeader) {
			...
			else if(HasTranslatedSheetDefS(sheetFieldDefCopy,out var listSheetFieldDefsMatched)) {//def was not deleted.
				//Update unchanged, translated SheetFieldDefs to reflect new values.
				for(var i=0;i<listSheetFieldDefsMatched.Count;i++){
					var sheetFieldDef2=sheetFieldDef.Copy();
	...
	private void LoadImageOne(SheetFieldDef sheetFieldDef){
		...
		var filePathAndName=ODFileUtils.CombinePaths(SheetUtil.GetImagePath(),sheetFieldDef.FieldName);
		if(true && File.Exists(filePathAndName)) {
			var bitmap=(Bitmap)Image.FromFile(filePathAndName);
	...
	private void PasteControlsFromMemory(Point pointOrigin) {
		...
		if(IsChartModuleSheetType() && _listSheetFieldDefsCopyPaste!=null) {
			...
			var listGridNames=SheetUtil.GetGridsAvailable(SheetDef_.SheetType,_sheetFieldLayoutMode);
			var listSheetFieldDefsSpecial=SheetFieldsAvailable.GetSpecial(SheetDef_.SheetType,_sheetFieldLayoutMode);
		...
		if(pointOrigin.X==0 && pointOrigin.Y==0) { //allows for cascading pastes in the upper right hand corner.
			var rectangle=panelMain.Bounds; //Gives relative position of panel (scroll position)
			//Current resized height/width of parent panel but must be unscaled since we are dealing with unscaled figures below
			var h=panelLeft.Height; 
			var w=panelLeft.Width;
			var maxH=0;
			var maxW=0;
			for(var i=0;i<_listSheetFieldDefsCopyPaste.Count;i++) { //calculate height/width of control to be pasted
			...
			var verticalScrollWidth=panelLeft.VerticalScroll.Visible ? 10 : 0;
			var horizontalScrollHeight=panelLeft.HorizontalScroll.Visible ? 10 : 0;
		...
		for(var i=0;i<_listSheetFieldDefsCopyPaste.Count;i++) { //calculate offset
			minX=Math.Min(minX,_listSheetFieldDefsCopyPaste[i].XPos);
			minY=Math.Min(minY,_listSheetFieldDefsCopyPaste[i].YPos);
		}
		var listSheetFieldDefsNew=new List<SheetFieldDef>();
		for(var i=0;i<_listSheetFieldDefsCopyPaste.Count;i++) { //create new controls
			var sheetFieldDef=_listSheetFieldDefsCopyPaste[i].Copy();
	...
	private void RefreshLanguages(string selectedThreeLetterLanguage=null) {
		var listAllLanguages=PrefC.GetString(PrefName.LanguagesUsedByPatients)//Must be before initial InitLayoutModes().
			...
			.Where(x => x!=Patients.LanguageDeclinedToSpecify).ToList();
		var listUsedLanguageThreeLetters=SheetDef_.SheetFieldDefs.FindAll(x => !x.Language.IsNullOrEmpty())
			.Select(x => x.Language)
			.Distinct().ToList();
	...
	private void RenumberTabOrderHelper() {
		for(var i=0;i<_listSheetFieldDefsTabOrder.Count;i++) {
	...
	private bool TryAddTranslation(out string selectedLangauge) {
		...
		var formInputBox=new InputBox(inputBoxParam);
	...
	private void SetPanelMainSize(){
		var sizeNew=new Size(SheetDef_.Width,SheetDef_.Height);
		if(SheetDef_.IsLandscape) {
			sizeNew=new Size(SheetDef_.Height,SheetDef_.Width);
		}
		sizeNew.Height=SheetDef_.HeightTotal;
		if(SheetDef_.PageCount>1){
		...
		panelMain.Bounds=new Rectangle(4,4,sizeNew.Width,sizeNew.Height);
	}
	...
	private bool UpdateRevisionID() {
		...
		var staticTextWasChanged=false;
		for(var i=0;i<SheetDef_.SheetFieldDefs.Count;i++) {//Iterate through field defs to find all static text fields.
			if(SheetDef_.SheetFieldDefs[i].FieldType!=SheetFieldType.StaticText) {
				continue;
			}
			var sheetFieldDefOld=sheetDefStored.SheetFieldDefs.Find(x => x.SheetFieldDefNum==SheetDef_.SheetFieldDefs[i].SheetFieldDefNum);
	...
	private bool ValidateInOK() {
		...
		var listSheetFieldDefsMedCheckBox=new List<SheetFieldDef>();
		var listSheetFieldDefsInputMedList=new List<SheetFieldDef>();
		var listSheetFieldDefsCheckMedList=new List<SheetFieldDef>();
		var listSheetFieldDefsErroneousDuplicates=new List<SheetFieldDef>();
		var listSheetFieldDefsToDelete=new List<SheetFieldDef>();
		//Verify radio button groups.
		for(var i=0;i<SheetDef_.SheetFieldDefs.Count;i++) {
			var sheetFieldDef=SheetDef_.SheetFieldDefs[i];
			       ...
			       || sheetFieldDef.RadioButtonValue!="")) //for built-in radio groups
			{
				...
				for(var j=0;j<SheetDef_.SheetFieldDefs.Count;j++) {
					var sheetFieldDef2=SheetDef_.SheetFieldDefs[j];
			...
			if(sheetFieldDef.FieldType==SheetFieldType.CheckBox && sheetFieldDef.FieldName.StartsWith("allergy:") || sheetFieldDef.FieldName.StartsWith("problem:")) {
				for(var j=0;j<listSheetFieldDefsMedCheckBox.Count();j++) { //check for duplicates
				...
				for(var j=0;j<listSheetFieldDefsInputMedList.Count();j++) { //check for duplicates
				...
				for(var j=0;j<listSheetFieldDefsCheckMedList.Count();j++) { //check for duplicates
		...
		if(listSheetFieldDefsErroneousDuplicates.Count>0) {
			var errorMessage=GetDuplicateSheetFieldDefErrorMessage(listSheetFieldDefsErroneousDuplicates);
			ODMessageBox.Show(errorMessage);
			return false;
		}
		if(listSheetFieldDefsToDelete.Count>0) {
			var fieldsDeletedMessage=new StringBuilder();
		...
		if(listSheetFieldDefsCheckMedList.Count!=0 && listSheetFieldDefsInputMedList.Any(x => !listSheetFieldDefsCheckMedList.Exists(y => y.FieldName==x.FieldName.Replace("inputMed","checkMed")))) {
			var inputMedMissingCheckBox=listSheetFieldDefsInputMedList.Select(x=>x.FieldName).Where(x=>!listSheetFieldDefsCheckMedList.Exists(y => y.FieldName==x.Replace("inputMed","checkMed"))).FirstOrDefault();
		...
		try {
			...
			var sheetDef=(SheetDef)xmlSerializer.Deserialize(textReader);
		}
		catch(Exception ex) {
	...
	private string GetDuplicateSheetFieldDefErrorMessage(List<SheetFieldDef> listSheetFieldDefs) {
		var listSheetFieldDefsCopy=new List<SheetFieldDef>(listSheetFieldDefs);
		...
		var message=new StringBuilder(Lan.g(this,"Duplicate fields found. Only one of the following fields is allowed per language")+":\r\n");
		//If this error occured in a language other than the default.
		for(var i=0;i<listSheetFieldDefsCopy.Count;i++) {
			var sheetFieldDef=listSheetFieldDefsCopy[i];
			var fieldName=sheetFieldDef.FieldName;
			var fieldType=sheetFieldDef.FieldType.GetDescription();
			var language="";
	...
	private GridOD CreateDynamicGrid(SheetFieldDef fieldDef,List<DisplayField> listColumns) {
		...
		var row=new GridRow();//Add empty row
		for(var c=0;c<listColumns.Count; c++) {
	...
	private GridOD CreateGrid(List<DisplayField> columns) {
		...
		var row=new GridRow(); //Add dummy row
		for(var c=0;c<columns.Count;c++) {
	...
	private void DrawCheckBox(SheetFieldDef sheetFieldDef,Graphics g,bool isSelected) {
		...
		var rectTab=new Rectangle(sheetFieldDef.XPos-1, //X
	...
	private void DrawFields(SheetDef sheetDef,Graphics g,bool isHighlightEligible=true) {
		...
		var listSheetFieldDefs=GetPertinentSheetFieldDefs(sheetDef);
		var selectedLanguage=GetSelectedLanguageThreeLetters();
		var listSheetFieldDefsSelected=listBoxFields.GetListSelected<SheetFieldDef>();
	...
	private void DrawFieldType(SheetFieldType sheetFieldType,SheetFieldDef sheetFieldDef,bool isHighlightEligible,string selectedLanguage,List<SheetFieldDef> listSheetFieldDefsSelected,SheetDef sheetDef,Graphics g){
		...
		var isSelected=listSheetFieldDefsSelected.Contains(sheetFieldDef);
		var isFieldSelected=isHighlightEligible && isSelected;
		switch(sheetFieldDef.FieldType) {
	...
	private void DrawGrid(SheetFieldDef sheetFieldDef,Graphics g,bool isSelected) {
		...
		var heightGridHeader=15;
		var heightGridRow=13+2;
		var listDisplayFieldColumns=SheetUtil.GetGridColumnsAvailable(sheetFieldDef.FieldName);
		GridOD grid;
		if(IsChartModuleSheetType()) {
		...
		var listSheetFieldDefsShowing=listBoxFields.Items.GetAll<SheetFieldDef>();
		var yPosGrid=sheetFieldDef.YPos;
		SizeF sizeStr;
		var drawHeaders=true;
		switch(sheetFieldDef.FieldName) {
			...
			case "EraClaimsPaid":
				var xPosGrid=sheetFieldDef.XPos;
				...
				yPosGrid+=heightGridTitle;
				/*---------------------------------------------------------------------------------------------*/
				grid.SheetDrawHeader(g,sheetFieldDef.XPos,yPosGrid,1);
				...
				/*---------------------------------------------------------------------------------------------*/
				var listDisplayField=SheetUtil.GetGridColumnsAvailable("EraClaimsPaidCodes");
				var gridCode=CreateGrid(listDisplayField);
				gridCode.SheetDrawHeader(g,sheetFieldDef.XPos,yPosGrid+1,1);
				yPosGrid+=heightGridHeader;
			...
			case "Procedures":
				...
				if(sheetFieldDef.GrowthBehavior.In(GrowthBehaviorEnum.FillDown,GrowthBehaviorEnum.FillRightDown,GrowthBehaviorEnum.FillDownFitColumns)) {
					...
					switch(sheetFieldDef.GrowthBehavior) {
						case GrowthBehaviorEnum.FillDownFitColumns:
							var minY=0;//Mimics height logic in SheetUserControl.IsControlAbove(...)
							var listSheetFieldDefAboveControls=listSheetFieldDefsShowing.FindAll(x => IsSheetFieldDefAbove(sheetFieldDef,x));
						...
						case GrowthBehaviorEnum.FillDown:
							var listSheetFieldDefsBelowControls=listSheetFieldDefsShowing.FindAll(x => IsSheetFieldDefBelow(sheetFieldDef, x));
				...
				grid.SheetDrawTitle(g,sheetFieldDef.XPos,yPosGrid,1);
				yPosGrid+=heightGridTitle;
				gridRemainingHeight-=heightGridTitle;
				grid.SheetDrawHeader(g,sheetFieldDef.XPos,yPosGrid,1);
				yPosGrid+=heightGridHeader;
				gridRemainingHeight-=heightGridHeader;
				var gridRowHeight=heightGridRow;//Zero until we calculate the height of the first row.
		...
		if(sheetFieldDef.FieldName=="StatementPayPlan" || sheetFieldDef.FieldName=="StatementDynamicPayPlan" || sheetFieldDef.FieldName=="StatementPayPlanGrid" || sheetFieldDef.FieldName=="StatementPayPlanOld") {
			var text="Payment Plan Amount Due: "+"0.00";
			var rectangleF=new RectangleF(sheetFieldDef.Width-sheetFieldDef.Width-60,yPosGrid,sheetFieldDef.Width,heightGridTitle);
			g.FillRectangle(Brushes.White,rectangleF);
			var stringFormat=new StringFormat();
			stringFormat.Alignment=StringAlignment.Far;
			g.DrawString(text,new Font(FontFamily.GenericSansSerif,10,FontStyle.Bold),new SolidBrush(Color.Black),rectangleF,stringFormat);
		}
		if(sheetFieldDef.FieldName=="StatementInvoicePayment") {
			...
			if(PrefC.GetBool(PrefName.InvoicePaymentsGridShowNetProd)) {
				g.DrawString("Total Payments & WriteOffs:  0.00",new Font(FontFamily.GenericSansSerif,10,FontStyle.Bold),new SolidBrush(Color.Black),rectangleF,stringFormat);
			}
			else {
				g.DrawString("Total Payments:  0.00",new Font(FontFamily.GenericSansSerif,10,FontStyle.Bold),new SolidBrush(Color.Black),rectangleF,stringFormat);
			}
		}
		#endregion
		if(isSelected) {
			   ...
			   || sheetFieldDef.GrowthBehavior==GrowthBehaviorEnum.FillDownFitColumns) 
			{
				...
				for(var c=0;c<listDisplayFieldColumns.Count;c++) {
					sheetFieldDef.Width+=listDisplayFieldColumns[c].ColumnWidth;
				}
			}
			using var pen=new Pen(_colorRed,1.6f);
	...
	private void DrawInsurance(SheetFieldDef sheetFieldDef,Graphics g,Pen pen,SolidBrush solidBrush) {
		var bitmapIns=new Bitmap(sheetFieldDef.Width,sheetFieldDef.Height);
		Control controlIns;
		switch(sheetFieldDef.FieldName) {
		...
		var rectangleBound=OpenDentBusiness.SheetPrinting.DrawScaledImage(sheetFieldDef.XPos,sheetFieldDef.YPos,sheetFieldDef.Width,sheetFieldDef.Height,g,null,bitmapIns);
		var color=GetOutlineColorForSheetFieldDef(sheetFieldDef,Color.LightGray);
		using var penOutline=new Pen(color);
	...
	private void DrawLine(SheetFieldDef sheetFieldDef,Graphics g,bool isSelected) {
		var colorLine=sheetFieldDef.ItemColor;
		var color=GetOutlineColorForSheetFieldDef(sheetFieldDef,colorLine);
		if(isSelected) {
	...
	private void DrawPatImage(SheetFieldDef sheetFieldDef,Graphics g,bool isSelected) {
		var colorOutline=GetOutlineColorForSheetFieldDef(sheetFieldDef,Color.Black);
		var colorText=_colorGray;
		if(checkBlue.Checked){
		...
		using var penOutline=new Pen(colorOutline);
		...
		if(isSelected && listBoxFields.SelectedIndices.Count==1){
			var rectangleHandleLR=new Rectangle(sheetFieldDef.Bounds.Right-2,sheetFieldDef.Bounds.Bottom-2,
				_sizeResizeGrabHandle,_sizeResizeGrabHandle);
			using var pen2=new Pen(ColorOD.Gray(10));
	...
	private void DrawAlignmentLines(Graphics g){
		...
		using var pen=new Pen(Color.Blue,1);//because we scaled prior to all drawing and we want this line to be one screen pixel.
		var listSheetFieldDefsShowing=listBoxFields.Items.GetAll<SheetFieldDef>();
		var listSheetFieldDefsSelected=listBoxFields.GetListSelected<SheetFieldDef>();
		for(var i=0;i<listBoxFields.SelectedIndices.Count;i++) {
			for(var j=0;j<listSheetFieldDefsShowing.Count;j++) {
	...
	private void DrawSigBox(SheetFieldDef sheetFieldDef,Graphics g,bool isSelected) {
		var colorOutline=GetOutlineColorForSheetFieldDef(sheetFieldDef,_colorGray);
		var colorText=_colorGray;
		if(checkBlue.Checked){
			colorOutline=GetOutlineColorForSheetFieldDef(sheetFieldDef,_colorBlue);
			colorText=_colorBlue;
		...
		using var pen=new Pen(colorOutline);
		using Brush brushText=new SolidBrush(colorText);
		var rectangle=new Rectangle(sheetFieldDef.XPos,sheetFieldDef.YPos,sheetFieldDef.Width,sheetFieldDef.Height);
		g.FillRectangle(Brushes.White,rectangle);
		g.DrawRectangle(pen,rectangle);
		var str="(signature box)";
	...
	private void DrawSpecial(SheetFieldDef sheetFieldDef,Graphics g,int sheetDefWidth,bool isSelected) {
		var colorPen=GetOutlineColorForSheetFieldDef(sheetFieldDef,_colorGray);
		var colorBrush=_colorGray;
		if(isSelected) {
			colorPen=GetOutlineColorForSheetFieldDef(sheetFieldDef,_colorRed,true);
			colorBrush=_colorRed;
		}
		using var pen=new Pen(colorPen);
		using var solidBrush=new SolidBrush(colorBrush);
		Panel panel;
		switch(sheetFieldDef.FieldName) {
			#region toothChart
			case "toothChart":
				if(_imageToothChart!=null){
					var rectangleBoundingBox=OpenDentBusiness.SheetPrinting.DrawScaledImage(sheetFieldDef.XPos,sheetFieldDef.YPos,sheetFieldDef.Width,sheetFieldDef.Height,g,null,_imageToothChart);
			...
			case "toothChartLegend":
				...
				OpenDentBusiness.SheetPrinting.DrawToothChartLegend(sheetFieldDef.XPos,sheetFieldDef.YPos,width,0,listDefs,g,null,SheetDefs.IsDashboardType(SheetDef_),1);
				break;
			case "familyInsurance":
			case "individualInsurance":
			...
			case "ChartModuleTabs":
				var tabControlPrimary=new UI.TabControl();
			...
			case "TreatmentNotes":
				...
				var textBoxControlTreatNotes=new System.Windows.Forms.TextBox();
			...
			case "TrackToothProcDates":
				...
				var trackBarControl=new TrackBar();
			...
			case "SetPriorityListBox":
				...
				var rowCount=(panel.Height-label.Height)/Font.Height;//Truncate to the nearest row, just as the listBox does internally.
	...
	private void DrawStringField(SheetFieldDef sheetFieldDef,Graphics g,bool isSelected) {
		...
		using var font=new Font(sheetFieldDef.FontName,sheetFieldDef.FontSize,fontStyle);
		Color colorText;
		if(isSelected) {
			var colorOutline=GetOutlineColorForSheetFieldDef(sheetFieldDef,_colorRedOutline,isSelected:true);
			colorText=_colorRed;
			using var pen=new Pen(colorOutline);
			g.DrawRectangle(pen,sheetFieldDef.Bounds);
		}
		else {
			var colorOutline = GetOutlineColorForSheetFieldDef(sheetFieldDef,_colorGrayOutline);
		...
		if(isSelected && listBoxFields.SelectedIndices.Count==1){
			var rectangleHandleLR=new Rectangle(sheetFieldDef.Bounds.Right-2,sheetFieldDef.Bounds.Bottom-2,
				_sizeResizeGrabHandle,_sizeResizeGrabHandle);
			using var pen=new Pen(ColorOD.Gray(10));
		...
		using var solidBrush=new SolidBrush(colorText);
		var stringFormat=new StringFormat();
		...
		if(sheetFieldDef.FieldType==SheetFieldType.InputField || sheetFieldDef.FieldType==SheetFieldType.OutputText) {
			...
			var rectangleText=new Rectangle(sheetFieldDef.Bounds.X,sheetFieldDef.Bounds.Y,sheetFieldDef.Bounds.Width,sheetFieldDef.Bounds.Height*2);
	...
	private void DrawTabMode(SheetFieldDef sheetFieldDef,Graphics g,bool isHighlightEligible) {
		...
		var rectangleTab=new Rectangle(sheetFieldDef.XPos-1, //X
			sheetFieldDef.YPos-1, //Y
			(int)g.MeasureString(sheetFieldDef.TabOrder.ToString(),_fontTabOrder).Width+1, //Width
			12); //height
	...
	private static bool IsWesternChars(string inputstring) {
		...
		var matchCollection=regex.Matches(inputstring);
