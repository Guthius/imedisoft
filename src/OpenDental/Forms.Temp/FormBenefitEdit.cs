...
public partial class FormBenefitEdit : FormODBase {
	...
	private void FormBenefitEdit_Load(object sender, System.EventArgs e) {
		...
		var selectedIndex=listCovCats.FindIndex(x => x.CovCatNum==BenefitCur.CovCatNum);
	...
	private void butSave_Click(object sender, System.EventArgs e) {
		...
		var covCatNum=listCategory.GetSelectedKey<CovCat>(x => x.CovCatNum);
		var codeNum=ProcedureCodes.GetCodeNum(textProcCode.Text);
		var codeGroupNum=comboCodeGroup.GetSelectedKey<CodeGroup>(x => x.CodeGroupNum);
		var benefitType=listBenefitType.GetSelected<InsBenefitType>();
		var quantityQualifier=listQuantityQualifier.GetSelected<BenefitQuantity>();
		int percent;
		if(textPercent.Text=="") {
			percent=-1;
		}
		...
		var quantity=SIn.Byte(textQuantity.Text);
		var timePeriod=listTimePeriod.GetSelected<BenefitTimePeriod>();
		var coverageLevel=listCoverageLevel.GetSelected<BenefitCoverageLevel>();
		var treatArea=(TreatmentArea)listTreatArea.SelectedIndex;
		var countCatProcCodeGroup=0;
		if(covCatNum!=0) {
