...
public partial class FormInsEditLogs:FormODBase {
	...
	private void FillGrid() {
		...
		var listGridRows = ConstructGridRows();
	...
	private List<GridRow> ConstructGridRows() {
		...
		for(var i=0;i<_listInsEditLogs.Count;i++) {
			...
			var userod = listUserods.Find(x=>x.UserNum==_listInsEditLogs[i].UserNum);
			if(userod==null) {
		...
		for(var i=0;i<listGridRows.Count;i++){
			var insEditLog=(InsEditLog)listGridRows[i].Tag;
			...
			for(var j=0;j<listGridRows.Count;j++){
				var insEditLogParent=(InsEditLog)listGridRows[j].Tag;
	...
	private void TranslateBeforeAndAfter() {
		for(var i=0;i<_listInsEditLogs.Count;i++) {
			var beforeKey = SIn.Long(_listInsEditLogs[i].OldValue,false);
			var afterKey = SIn.Long(_listInsEditLogs[i].NewValue,false);
			switch(_listInsEditLogs[i].FieldName) {
				case "CarrierNum":
					if(_listInsEditLogs[i].LogType == InsEditLogType.Carrier) {
						break;
					}
					var carrierNameBefore=Carriers.GetCarrier(beforeKey).CarrierName;
					var carrierNameAfter=Carriers.GetCarrier(afterKey).CarrierName;
