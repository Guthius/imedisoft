...
public partial class FormClaimPayTotal : FormODBase {
	...
	private void FormClaimPayTotal_Load(object sender,EventArgs e) {
		...
		var dateTClaimProcMin=_listProcedures.FindAll(x => ClaimProcArray.Any(y => y.ProcNum==x.ProcNum)).Select(x => x.DateEntryC).Min();
	...
	private void FormClaimPayTotal_Shown(object sender,EventArgs e) {
		var insPlan=InsPlans.GetPlan(ClaimProcArray[0].PlanNum,_listInsPlans);
	...
	private void FillGrid(){
		...
		var listStringsDefDescriptions=new List<string>();
		listStringsDefDescriptions.Add("None");
		for(var i=0;i<_listDefs.Count;i++){
		...
		for(var i=0;i<ClaimProcArray.Length;i++) {
			...
			if(ClaimProcArray[i].ProcNum==0) {
				...
				var procedure=Procedures.GetProcFromList(_listProcedures,ClaimProcArray[i].ProcNum);//will return a new procedure if none found.
				...
				var descript=procedureCode.Descript;
			...
			if(CultureInfo.CurrentCulture.Name.EndsWith("CA")) {//Canadian. en-CA or fr-CA
				var labFeesForProc=Procedures.GetCanadianLabFees(ClaimProcArray[i].ProcNum,_listProcedures).Sum(x => x.ProcFee);
			...
			if(PrefC.GetBool(PrefName.ClaimEditShowPayTracking)) {
				var isDefPresent=false;
				for(var j=0;j<_listDefs.Count;j++) {
	...
	private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		...
		using var formClaimProc=new FormClaimProc(ClaimProcArray[e.Row],null,_family,_patient,_listInsPlans,listClaimProcHists,ref listClaimprocHistsLoop,_listPatPlans,false,_listInsSubs);
	...
	private void gridMain_CellTextChanged(object sender,EventArgs e) {
		...
		if(_doShowPatResp && gridMain.GetSelectedIndex() > -1) {
			...
			var gridRowSelected=gridMain.SelectedGridRows.First();
			var claimProc=(ClaimProc)gridRowSelected.Tag;
			...
			var insPayAmt=SIn.Double(gridRowSelected.Cells[gridMain.Columns.GetIndex(Lan.g("TableClaimProc","Ins Pay"))].Text);
			var writeOff=SIn.Double(gridRowSelected.Cells[gridMain.Columns.GetIndex(Lan.g("TableClaimProc","Write-off"))].Text);
			//Calculate the new Pat Resp value and update the text in the corresponding cell.
			var patResp=procFee-insPayAmt-writeOff;
	...
	private void FillTotals(){
		...
		for(var i=0;i<gridMain.ListGridRows.Count;i++){
			...
			var insPayAmtCur=SIn.Double(gridMain.ListGridRows[i].Cells[gridMain.Columns.GetIndex(Lan.g("TableClaimProc","Ins Pay"))].Text);
			insPayAmt+=insPayAmtCur;
			var writeOffCur=SIn.Double(gridMain.ListGridRows[i].Cells[gridMain.Columns.GetIndex(Lan.g("TableClaimProc","Write-off"))].Text);
			writeOff+=writeOffCur;
			if(_doShowPatResp) {
				var procFeeCur=Procedures.GetProcFromList(_listProcedures,ClaimProcArray[i].ProcNum).ProcFeeTotal;
				if(ClaimProcArray[i].Status==ClaimProcStatus.Supplemental) {
					procFeeCur=0;
				}
	...
	private bool SaveGridChanges(bool skipChecks=false){
		...
		double dbl=0;
		var idxDeduct=gridMain.Columns.GetIndex(Lan.g("TableClaimProc","Deduct"));
		var idxAllowed=gridMain.Columns.GetIndex(Lan.g("TableClaimProc","Allowed"));
		var idxInsPay=gridMain.Columns.GetIndex(Lan.g("TableClaimProc","Ins Pay"));
		var idxWriteOff=gridMain.Columns.GetIndex(Lan.g("TableClaimProc","Write-off"));
		var idxStatus=gridMain.Columns.GetIndex(Lan.g("TableClaimProc","Status"));
		for(var i=0;i<gridMain.ListGridRows.Count;i++){
			var isValid=true;
			if(gridMain.ListGridRows[i].Cells[idxDeduct].Text != "") {
				isValid=double.TryParse(gridMain.ListGridRows[i].Cells[idxDeduct].Text,out var result);//we don't care about the result
			}
			if(gridMain.ListGridRows[i].Cells[idxAllowed].Text != "") {
				isValid &= double.TryParse(gridMain.ListGridRows[i].Cells[idxAllowed].Text,out var result);	
			}
			if(gridMain.ListGridRows[i].Cells[idxInsPay].Text != "") {
				isValid &= double.TryParse(gridMain.ListGridRows[i].Cells[idxInsPay].Text,out var result);	
			}
			if(gridMain.ListGridRows[i].Cells[idxWriteOff].Text != "") {
				isValid &= double.TryParse(gridMain.ListGridRows[i].Cells[idxWriteOff].Text,out var result);	
			}
			if(!isValid) {
			...
			if(gridMain.ListGridRows[i].Cells[idxWriteOff].Text != "") {
				...
				var claimWriteOffTotal=ClaimProcs.GetClaimWriteOffTotal(ClaimProcArray[0].ClaimNum,ClaimProcArray[i].ProcNum,ClaimProcArray.ToList());
				if(claimWriteOffTotal+dbl<0) {
					MsgBox.Show(this,"The current write-off value for supplemental payment "+(i+1)+" will cause the procedure's total write-off to be negative.  Please change it to be at least "+(dbl-(claimWriteOffTotal+dbl))+" to continue.");
					return false;
				}	
			}
		}
		...
		for(var i=0;i<ClaimProcArray.Length;i++) {
			...
			if(PrefC.GetBool(PrefName.ClaimEditShowPayTracking)) {
				var idxCol=gridMain.Columns.GetIndex(Lan.g("TableClaimProc","Pay Tracking"));
				if(idxCol>-1 && idxCol<gridMain.ListGridRows[i].Cells.Count) {
					var idx=gridMain.ListGridRows[i].Cells[idxCol].ComboSelectedIndex;
	...
	private List<ClaimProc> GetListClaimProcHypothetical() {
		...
		for(var i=0;i<ClaimProcArray.Length;i++) {
			var claimProc=ClaimProcArray[i].Copy();
			var idxInsPay=gridMain.Columns.GetIndex(Lan.g("TableClaimProc","Ins Pay"));
			var idxWriteOff=gridMain.Columns.GetIndex(Lan.g("TableClaimProc","Write-off"));
	...
	public void butWriteOff_Click(object sender,EventArgs e) {
		...
		var isCanadian=CultureInfo.CurrentCulture.Name.EndsWith("CA");//Canadian. en-CA or fr-CA 
		var listProceduresLabs=new List<Procedure>();
		if(isCanadian) {
		...
		if(isCanadian && dialogResult==DialogResult.Yes) {//Canadian lab fees should be written off.
			var claim=Claims.GetClaim(ClaimProcArray[0].ClaimNum);//There should be at least one, since a claim can only be created with one or more procedures.
			var claimProcTotalLabs=new ClaimProc();
			...
			for(var i=0;i<ClaimProcArray.Length;i++) {
				var claimProc=ClaimProcArray[i];
				...
				var listProceduresLabsCur=listProceduresLabs.FindAll(x => x.ProcNumLab==claimProc.ProcNum);
				...
				for(var j=0;j<listProceduresLabsCur.Count;j++) {
					//Go get the claimproc that is associated to this lab procedure.
					var claimProcLab=ClaimProcArray.First(x => x.ProcNum==listProceduresLabsCur[j].ProcNum);
	...
	private void butSave_Click(object sender,EventArgs e) {
		...
		for(var i=0;i<ClaimProcArray.Length;i++) {
			var claimProcOld=_listClaimProcsOld.FirstOrDefault(x => x.ProcNum==ClaimProcArray[i].ProcNum);
