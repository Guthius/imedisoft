...
public partial class FormOrthoRxSetup:FormODBase {
	...
	private void FillGrid(int selectedIndex=-1){
		...
		var col=new GridColumn(Lan.g("TableRxs","Hardware Spec"),110);
		...
		for(var i=0;i<_listOrthoRxs.Count;i++){
			if(_listOrthoRxs[i].ItemOrder!=i){
				_listOrthoRxs[i].ItemOrder=i;
				OrthoRxs.Update(_listOrthoRxs[i]);//fixes any item order bugs
			}
			var orthoHardwareSpec=listOrthoHardwareSpecs.Find(x=>x.OrthoHardwareSpecNum==_listOrthoRxs[i].OrthoHardwareSpecNum);
			var row=new GridRow();
	...
	private void butUp_Click(object sender, System.EventArgs e) {
		...
		var orthoRxSelected=_listOrthoRxs[idx];
		orthoRxSelected.ItemOrder--;
		OrthoRxs.Update(orthoRxSelected);
		var orthoRxHardwareSpecAbove=_listOrthoRxs[idx-1];
	...
	private void butDown_Click(object sender, System.EventArgs e) {
		...
		orthoRxSelected.ItemOrder++;
		OrthoRxs.Update(orthoRxSelected);
		var orthoRxBelow=_listOrthoRxs[idx+1];
