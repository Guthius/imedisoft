...
public partial class FormSmsTextMessaging:FormODBase {
	...
	private List<SmsFromMobile> _listSmsFromMobiles= [];
	private List<SmsToMobile> _listSmsToMobiles= [];
	...
	private TextMessageGrouped GetSelectedSmsGroup() {
		...
		var selectedTag=gridMessages.ListGridRows[gridMessages.GetSelectedIndex()].Tag;
	...
	private void AddPatientNames(List<long> listPatNums) {
		var listPatNumsNew=listPatNums.FindAll(x=>x!=0 && !_listPatients.Exists(y=>y.PatNum==x));
		if(listPatNumsNew.Count()>0) {
			var listPatientsNew=Patients.GetLimForPats(listPatNumsNew);
	...
	private void FormSmsTextMessaging_Load(object sender,EventArgs e) {
		...
		this.FormClosing+=new FormClosingEventHandler((_,_) => {
	...
	private void FillGridTextMessages(bool isRedrawOnly=false,bool retainSort=true) {
		...
		if(true && comboClinics.ListClinicNumsSelected.Count==0) {
		...
		var idxColSortBy=gridMessages.GetSortedByColumnIdx();
		var isSortAsc=gridMessages. IsSortedAscending();
		if(idxColSortBy==-1 || !retainSort) {
			idxColSortBy=0;
			isSortAsc=false;
		}
		if(!isRedrawOnly) {
			var listSmsFromStatuses=new List<SmsFromStatus>();
	...
	private void FillGridTextMessagesGroupedNo(int idxColSortBy,bool isSortAsc,long patNumSelected,SmsToMobile smsToMobileSelected,SmsFromMobile smsFromMobileSelected) {
		...
		for(var i=0;i<_listSmsFromMobiles.Count;i++) {
			...
			var clinic=Clinics.GetClinic(_listSmsFromMobiles[i].ClinicNum);
		...
		if(checkSent.Checked) {
			for(var i=0;i<_listSmsToMobiles.Count;i++) {
				...
				var smsStatus=_listSmsToMobiles[i].SmsStatus.ToString(); //Default to the actual status.
				...
				var clinic=Clinics.GetClinic(_listSmsToMobiles[i].ClinicNum);
				row.Cells.Add(clinic.Abbr);//Clinic
				if(checkHidden.Checked) {
	...
	private void FillGridTextMessagesGroupedYes(int idxColSortBy,bool isSortAsc,long patNumSelected,TextMessageGrouped textMessageGroupedSelected) {
		...
		var listTextMessageGrouped=GetMessageGroups();
		for(var i=0;i<listTextMessageGrouped.Count;i++) {
			var row=new UI.GridRow();
	...
	private List<TextMessageGrouped> GetMessageGroups() {
		var listTextMessageGroupedsToMobile=new List<TextMessageGrouped>();
		var listTextMessageGroupedsFromMobile=_listSmsFromMobiles
			...
			.Select(x => new TextMessageGrouped {
				...
				ClinicAbbr=Clinics.GetClinic(x.First().ClinicNum).Abbr,
		...
		var listTextMessageGroupedAll=listTextMessageGroupedsFromMobile.Union(listTextMessageGroupedsToMobile)
			...
			.Select(x => {
				...
				var textMessageGrouped=x.First();
	...
	private void FillGridMessageThread() {
		...
		if(GetSelectedPatNum()==0 && GetSelectedSmsGroup()==null) { //A message with no patNum was selected and is not a group, or nothing is selected at all.
			if(GetSelectedSmsToMobile()!=null) { //SmsToMobile is selected.
				smsThreadView.ListSmsThreadMessages= [new SmsThreadMessage("T" + GetSelectedSmsToMobile().SmsToMobileNum, GetSelectedSmsToMobile().DateTimeSent, GetSelectedSmsToMobile().MsgText, true, true, true, GetSelectedSmsToMobile().SmsStatus)];
			}
			else if(GetSelectedSmsFromMobile()!=null) { //SmsFromMobile is selected.
				smsThreadView.ListSmsThreadMessages= [new SmsThreadMessage("F" + GetSelectedSmsFromMobile().SmsFromMobileNum, GetSelectedSmsFromMobile().DateTimeReceived, GetSelectedSmsFromMobile().MsgText, true, true, true, SmsDeliveryStatus.None)];
			}
		...
		var listSmsThreadMessages=new List<SmsThreadMessage>();
		long patNumForDb;
		...
		var listSmsFromMobile=SmsFromMobiles.GetMessages(DateTime.MinValue,DateTime.MinValue,GetListSelectedClinicNums(),
			patNumForDb,true,strNumberForDb, []);
		for(var i=0;i<listSmsFromMobile.Count;i++) {
		...
		var listSmsToMobile=SmsToMobiles.GetMessages(DateTime.MinValue,DateTime.MinValue,GetListSelectedClinicNums(),patNumForDb,
			strNumberForDb);
		for(var i=0;i<listSmsToMobile.Count;i++) {
			var isHighlighted=false;
		...
		var listPatNumsInMessages=listSmsFromMobile.Select(x => x.PatNum).Union(listSmsToMobile.Select(x => x.PatNum)).Distinct().ToList();
		var patNames="";
		labelPatientsForPhone.Visible=false;
	...
	private void gridMessages_SelectionCommitted(object sender,EventArgs e) {
		var listSmsFromMobilesMarkReceivedRead=new List<SmsFromMobile>();
		...
		if(listSmsFromMobilesMarkReceivedRead.Count>0) {
			...
			for(var i=0;i<listSmsFromMobilesMarkReceivedRead.Count;i++) {
				var smsFromMobileOld=listSmsFromMobilesMarkReceivedRead[i].Copy();
			...
			if(gridMessages.ListGridRows[gridMessages.SelectedIndices[0]].Tag is TextMessageGrouped textMessageGrouped) {
				...
				var smsFromMobileLatest=textMessageGrouped.ListSmsFromMobiles.OrderByDescending(x => x.DateTimeReceived).FirstOrDefault()??new SmsFromMobile { DateTimeReceived=DateTime.MinValue };
				var smsToMobileLatest=textMessageGrouped.ListSmsToMobiles.OrderByDescending(x => x.DateTimeSent).FirstOrDefault()??new SmsToMobile {DateTimeSent=DateTime.MinValue };
		...
		var selectedPatNum=GetSelectedPatNum();
		for(var i=0;i<gridMessages.ListGridRows.Count;i++){
			long patNum=0;
	...
	private void SetReceivedSelectedStatus(SmsFromStatus smsFromStatus) {
		...
		Cursor=Cursors.WaitCursor;
		var row=gridMessages.ListGridRows[gridMessages.GetSelectedIndex()];
		var smsFromMobile=GetSelectedSmsFromMobile();
		var smsFromMobileOld=smsFromMobile.Copy();
	...
	private void menuItemChangePat_Click(object sender,EventArgs e) {
		...
		var frmPatientSelect=new FrmPatientSelect();
		var listPatients=Patients.GetPatientsByPhone(GetSelectedSmsFromMobile().MobilePhoneNumber,				
		...
		if(Clinics.ClinicNum!=0) {
			var listClinicNums=GetListSelectedClinicNums().Union(new List<long> { GetSelectedSmsFromMobile().ClinicNum }).ToList();
			listPatients=listPatients.Where(x => listClinicNums.Contains(x.ClinicNum)).ToList();
		}
		frmPatientSelect.ListPatNumsExplicit=listPatients.Select(x=> x.PatNum).ToList();
		frmPatientSelect.ShowDialog();
		if(frmPatientSelect.IsDialogCancel) {
			return;
		}
		...
		var smsFromMobile=GetSelectedSmsFromMobile();
		var smsFromMobileOld=smsFromMobile.Copy();
		smsFromMobile.PatNum=frmPatientSelect.PatNumSelected;
		SmsFromMobiles.Update(smsFromMobile,smsFromMobileOld);
		//Upsert the Commlog.
		if(smsFromMobile.CommlogNum==0) {
			...
			var commlog=Commlogs.GetOne(smsFromMobile.CommlogNum);
			var commlogOld=commlog.Copy();
			commlog.PatNum=frmPatientSelect.PatNumSelected;
	...
	private void HideOrUnhideMessages(bool isHide) {
		...
		Cursor=Cursors.WaitCursor;
		if(GetSelectedSmsFromMobile()!=null) {
			var smsFromMobileOld=GetSelectedSmsFromMobile().Copy();
			...
			var smsToMobileOld=GetSelectedSmsToMobile().Copy();
	...
	private void menuItemBlockNumber_Click(object sender,EventArgs e) {
		...
		var strNumberToBlock=GetSelectedMobileNumber();
		var question=Lan.g(this,"Block incoming texts from")+" "+strNumberToBlock+"? "+Lan.g(this,"This cannot be undone.");
		if(GetSelectedPatNum()!=0) {
			var pat=Patients.GetPat(GetSelectedPatNum());
			if(pat!=null) {
	...
	private void butSend_Click(object sender,EventArgs e) {
		...
		for(var i=0;i<gridMessages.ListGridRows.Count;i++) {
			var row=gridMessages.ListGridRows[i];
			...
			if(row.Tag is TextMessageGrouped) {
				var smsGroup=(TextMessageGrouped)row.Tag;
				...
				var smsFrom=(SmsFromMobile)row.Tag;
	...
	private class TextMessageGrouped {
		...
		public List<SmsFromMobile> ListSmsFromMobiles= [];
		public List<SmsToMobile> ListSmsToMobiles= [];
	}
	#endregion
}