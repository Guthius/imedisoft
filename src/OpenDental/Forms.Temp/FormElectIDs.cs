...
public partial class FormElectIDs : FormODBase {
	...
	private void FormElectIDs_Load(object sender, System.EventArgs e) {
		...
		var clearinghouseDefaultDental=Clearinghouses.GetDefaultDental();
		var clearinghouseDefaultMedical=Clearinghouses.GetClearinghouse(PrefC.GetLong(PrefName.ClearinghouseDefaultMed));
	...
	private void FillElectIDs(long electIDNum) {
		...
		var col=new GridColumn(Lan.g("TableApptProcs","Carrier"),320);
		...
		GridRow row;
		var selectedIndex=-1;
		for(var i=0;i<_listElectIDsToShow.Count;i++) {
			row=new GridRow();
	...
	private void gridElectIDs_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		if(IsSelectMode) {
			...
			using var formElectIDEdit=new FormElectIDEdit();
