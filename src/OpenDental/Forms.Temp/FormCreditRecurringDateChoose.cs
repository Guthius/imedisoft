...
public partial class FormCreditRecurringDateChoose:FormODBase {
	...
	private bool FillComboBoxMonthSelect() {
		if(CreditCards.GetFrequencyType(_creditCard.ChargeFrequency)==ChargeFrequencyType.FixedDayOfMonth) {
			...
			var listDaysOfMonth=CreditCards.GetDaysOfMonthForChargeFrequency(_creditCard.ChargeFrequency).Split(',').Select(x => SIn.Int(x))
			...
			if(listDaysOfMonth.Count>1) {
				...
				for(var i=0;i<listDaysOfMonth.Count;i++) {
					var monthOffset=0;
					if(listDaysOfMonth[i]>DateTime.Today.Day) {
						monthOffset=-1;
					}
					var thisMonth=GetDateForDayOfMonth(DateTime.Today.AddMonths(monthOffset),listDaysOfMonth[i]); //This Month/Closest Month
					var lastMonth=GetDateForDayOfMonth(DateTime.Today.AddMonths(monthOffset-1),listDaysOfMonth[i]); //Last Month/Previous Month
	...
	private DateTime GetValidPayDate(DateTime date) {
		int dayOfMonth;
		var datePay=date; 
		if(false && PrefC.GetBool(PrefName.BillingUseBillingCycleDay)) {
