...
public partial class FormLateCharges:FormODBase {
	...
	private void FormLateCharges_Shown(object sender,EventArgs e) {
		...
		var listDefaultBillTypes=PrefC.GetString(PrefName.LateChargeDefaultBillingTypes)
			.Split([','],StringSplitOptions.RemoveEmptyEntries)
			.Select(x => SIn.Long(x,false))
			.ToList();
		for(var i=0;i<listBillType.Items.Count;i++) {
		...
		var dateLastRun=PrefC.GetDate(PrefName.LateChargeLastRunDate);
	...
	private decimal CalculateLateChargeAmount(decimal statementBalanceRemaining) {
		var minCharge=SIn.Decimal(textMinCharge.Text);
		var maxCharge=SIn.Decimal(textMaxCharge.Text);
		var percent=SIn.Int(textChargePercent.Text);
		if(percent==0) {
			return minCharge;
		}
		var chargeAmount=(((decimal)percent)/100)*statementBalanceRemaining;
	...
	private List<long> CreateLateCharges(List<StatementData> listStatementDatas,DateTime dateForCharges,DateTime dateRangeEnd)	{
		...
		var listGuarantorNums=listStatementDatas.SelectMany(x => x.ListPatNumsGuarantor).Distinct().ToList();
		var listFamilyProdBalances=new List<FamilyProdBalances>();
		for(var i=0;i<listGuarantorNums.Count;i++){
			listFamilyProdBalances.Add(new FamilyProdBalances(listGuarantorNums[i]));
		}
		//Create
		var listLateChargeAdjNums=new List<long>();
		//Looping through each statement data (family) set to create late charges.
		for(var i=0;i<listStatementDatas.Count;i++) {
			var statementBalanceRemaining=CalcStatementBalanceRemaining(listStatementDatas[i],listFamilyProdBalances,
				out var listProcNumsOnLateCharge,out var listAdjNumsOnLateCharge,out var listPayPlanChargeNumsOnLateCharge);
			...
			var lateChargeAmount=CalculateLateChargeAmount(statementBalanceRemaining);
			...
			var provNum=GetProvNumForLateCharge(listStatementDatas[i].ProvNumPriGuarantor);
			var adjNum=Adjustments.CreateLateChargeAdjustment((double)lateChargeAmount,dateForCharges
	...
	{
		listProcNumsOnLateCharge= [];
		listAdjNumsOnLateCharge= [];
		listPayPlanChargeNumsOnLateCharge= [];
		decimal statementBalanceRemaining=0;
		for(var i=0;i<statementData.ListStatementProds.Count;i++) {
			var statementProd=statementData.ListStatementProds[i];
			var prodItemBalance=FindAndRemoveStatementProdBalance(statementProd,statementData.ListPatNumsGuarantor,listFamilyProdBalances);
	...
	{
		for(var i=0;i<listGuarantorPatNums.Count;i++) {
			var familyProdBalances=listFamilyProdBalances.Find(x=>x.GuarantorPatNum==listGuarantorPatNums[i]);
			if(familyProdBalances is null){
				continue;
			}
			var amount=familyProdBalances.GetAmountForStatementProdIfExists(statementProd);
			return amount;
		}
		return 0;
	}
	...
	private void butUndo_Click(object sender,EventArgs e) {
		...
		var stringMessageConfirm=Lan.g(this,"Delete all Late Charge adjustments from")+" "+SIn.Date(textDateUndo.Text).ToShortDateString()+"?";
		if(ODMessageBox.Show(this,stringMessageConfirm,"",MessageBoxButtons.OKCancel)!=DialogResult.OK) {
			return;
		}
		var chargeUndoDataLate=new Adjustments.ChargeUndoData();
		...
		{
			var message="";
			var listPatients=Patients.GetLimForPats(chargeUndoDataLate.ListSkippedPatNums);
			for(var i=0;i<listPatients.Count;i++) {
				var patient=listPatients[i];
				message+=$"PatNum: {patient.PatNum}, {patient.GetNameFL()}\r\n";
			}
			using var msgBoxCopyPasteSkipped=new MsgBoxCopyPaste(message);
	...
	private void butRun_Click(object sender,EventArgs e) {
		...
		var listSelectedBillingTypes=listBillType.GetListSelected<Def>().Select(x => x.DefNum).ToList();
		var dateRangeEnd=DateTime.Now.AddDays(-SIn.Int(textDateRangeEnd.Text)).Date;
		var dateRangeStart=DateTime.Now.AddDays(-SIn.Int(textDateRangeStart.Text)).Date;
		DateTime dateTemp;
		var dateNewCharges=SIn.Date(textDateNewCharges.Text);
		...
		progressOD.StartingMessage=Lans.g("Getting statements...");
		progressOD.ShowDialog();
		if(progressOD.IsCancelled) {
		...
		progressOD.StartingMessage=Lans.g("Creating Late Charges...");
		progressOD.ShowDialog();
		if(progressOD.IsCancelled) {
		...
		ODMessageBox.Show(this,Lan.g(this,"Late Charges ran successfully. Adjustments created:")+" "+listLateChargeAdjNums.Count);
