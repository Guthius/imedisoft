...
public partial class FormSheetFieldComboBox:FormODBase {
	...
	private void FormSheetFieldComboBox_Load(object sender,EventArgs e) {
		...
		var arrayOptions=SheetFieldDefCur.FieldValue.Split(';')[1].Split('|');
	...
	private void listComboType_SelectionChangeCommitted(object sender,EventArgs e) {
		...
		if(listComboType.SelectedItem.ToString().In("Gender Identity","Sexual Orientation")) {
			if(PrefC.GetBool(PrefName.EasyHidePublicHealth)) {
				MsgBox.Show(Lan.g(this,$"Public Health is required to use {listComboType.SelectedItem}. Turn on Public Health in the Show Features window"));
				listComboType.ClearSelected();
				return;
			}
			else if(Snomeds.GetCodeCount()==0) {
				MsgBox.Show(Lan.g(this,$"SNOMEDCT codes must be downloaded to use {listComboType.SelectedItem}. SNOMEDCT codes can be imported from the Code Systems Importer window"));
				listComboType.ClearSelected();
				return;
			}
		}
		if(listComboType.SelectedItem.ToString()=="Clear") {
			...
			var enumVals=Enum.GetNames(typeof(PatientRaceOld));
			...
			var enumVals=Enum.GetNames(typeof(PatientGrade));
			listboxComboOptions.Items.AddList(enumVals,x => x.ToString());
		}
		else if(listComboType.SelectedItem.ToString()=="Urgency") {//Urgency
			var enumVals=Enum.GetNames(typeof(TreatmentUrgency));
			listboxComboOptions.Items.AddList(enumVals,x => x.ToString());
		}
		listComboType.ClearSelected();
	}
	...
	private void butUp_Click(object sender,EventArgs e) {
		...
		var idx=listboxComboOptions.SelectedIndex;
		var item=listboxComboOptions.SelectedItem.ToString();
	...
	private void SaveAndClose(){
		...
		for(var i=0;i<listboxComboOptions.Items.Count;i++) {
			if(i>0) {
