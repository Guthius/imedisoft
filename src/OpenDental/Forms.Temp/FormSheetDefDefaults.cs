...
public partial class FormSheetDefDefaults:FormODBase {
	...
	private long _clinicNumPrevSelected;
	...
	private void FillComboLabel() {
		comboLabel.Items.Clear();
		var listSheetDefs=SheetDefs.GetDeepCopy().FindAll(x => x.SheetType==SheetTypeEnum.LabelPatient);
		comboLabel.Items.Add(Lan.g(this,"Default"),new SheetDef());
		comboLabel.Items.AddList(listSheetDefs,x => x.Description);
		comboLabel.SetSelectedKey<SheetDef>(PrefC.GetLong(PrefName.LabelPatientDefaultSheetDefNum),x=>x.SheetDefNum);
		if(comboLabel.SelectedIndex==-1) {
	...
	private void UpdateSheetDefDefault(UI.ComboBox comboBox,PrefName prefName,bool isIndependentOfClinic=false) {
		...
		if(comboClinicDefault.ClinicNumSelected==0 || isIndependentOfClinic) {
			var defaultPref=Prefs.GetPref(prefName.GetDescription());
			defaultPref.ValueString=comboBox.GetSelected<SheetDef>().SheetDefNum.ToString();
			Prefs.Update(defaultPref);
			Prefs.RefreshCache();
			return;
		}
		var clinicPref=ClinicPrefs.GetPref(prefName,comboClinicDefault.ClinicNumSelected);
	...
	private void SelectComboBoxesDefault(UI.ComboBox comboBox,PrefName prefName) {
		...
		if(clinicPref==null || comboClinicDefault.ClinicNumSelected==0) {
			var pref=Prefs.GetPref(prefName.GetDescription());
	...
	private void comboClinicDefault_SelectionChangeCommitted(object sender,EventArgs e) {
		...
		var tempClinicNum=comboClinicDefault.ClinicNumSelected;//Store the selected clinic while we make updates to the previous one
	...
	private void butSave_Click(object sender,EventArgs e) {
		UpdateDefaultSheets();
		var sheetDefNum=comboLabel.GetSelectedKey<SheetDef>(x => x.SheetDefNum);
