...
public partial class FormDiseaseDefs:FormODBase {
	...
	public List<long> ListDiseaseDefNumsColored= [];
	...
	private void FormDiseaseDefs_Load(object sender,EventArgs e) {
		...
		if(ListDiseaseDefsSelected==null) {
			ListDiseaseDefsSelected= [];
		}
		_listDiseaseDefsOld=_listDiseaseDefs.Select(x => x.Copy()).ToList();
		_listDiseaseDefsShowing= [];//fillGrid takes care of filling this.
		FillGrid();
	}
	...
	private void FillGrid() {
		...
		GridRow row;
		for(var i=0;i<_listDiseaseDefsShowing.Count;i++) {
			row=new GridRow();
			...
			if(ListDiseaseDefNumsColored.Contains(_listDiseaseDefsShowing[i].DiseaseDefNum)) {
	...
	private List<DiseaseDef> FilterListByTerms(List<DiseaseDef> listDiseaseDefs,string filterText,SearchTypes searchType) {
		var listDiseaseDefsFiltered=new List<DiseaseDef>();
		if(string.IsNullOrEmpty(filterText)) {
			listDiseaseDefsFiltered=listDiseaseDefs;//Nothing to filter on, return the current list.
		}
		else {
			var stringArrayFilterTerms=filterText.Split(' ');//Each space is a new filter item. we could add other punctuation here if we wanted to. 
			for(var i=0;i<listDiseaseDefs.Count;i++) {
				//Get the correct value to filter against.
				var diseaseDefValue=listDiseaseDefs[i].DiseaseName;
				...
				var doFiltersMatch=stringArrayFilterTerms.All(x => diseaseDefValue.ToLower().Contains(x.ToLower()));
	...
	private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		...
		#endregion
		var diseaseDef = (DiseaseDef)gridMain.ListGridRows[gridMain.GetSelectedIndex()].Tag;
		#region Selection Mode. Close the Form
		if(IsSelectionMode) {//selection mode.
			var isInvalidSnomedCode=!string.IsNullOrWhiteSpace(diseaseDef.SnomedCode) && Snomeds.GetByCode(diseaseDef.SnomedCode)==null;
			if(!IsMultiSelect && isInvalidSnomedCode && !MsgBox.Show(this,MsgBoxButtons.YesNo,"You have selected a Problem with an invalid SNOMED CT code. Go to Lists, Problems to assign a valid "
			                                                                                  +"SNOMED CT code to the Problem definition. Do you want to proceed with the invalid SNOMED CT code?")) {
				return;
			}
		...
		using var formDiseaseDefEdit=new FormDiseaseDefEdit(diseaseDef);
	...
	private void butAdd_Click(object sender,System.EventArgs e) {
		...
		using var formDiseaseDefEdit=new FormDiseaseDefEdit(diseaseDef);//also sets ItemOrder correctly if using alphabetical during the insert diseaseDef call.
	...
	private void butAlphabetize_Click(object sender,EventArgs e) {
		...
		for(var i=0;i<_listDiseaseDefs.Count;i++) {
	...
	private void butOK_Click(object sender,EventArgs e) {
		...
		var isInvalidSnomedCode=!string.IsNullOrWhiteSpace(_listDiseaseDefs[gridMain.GetSelectedIndex()].SnomedCode) && Snomeds.GetByCode(_listDiseaseDefs[gridMain.GetSelectedIndex()].SnomedCode)==null;
		if(IsSelectionMode && !IsMultiSelect && isInvalidSnomedCode) {
			if(!MsgBox.Show(this,MsgBoxButtons.YesNo,"You have selected a Problem with an invalid SNOMED CT code. Go to Lists, Problems to assign a valid SNOMED CT code to the Problem definition. "
			                                         +"Do you want to proceed with the invalid SNOMED CT code?")) {
				return;
			}
		}
