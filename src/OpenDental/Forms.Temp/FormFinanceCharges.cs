...
public partial class FormFinanceCharges : FormODBase {
	...
	private void FormFinanceCharges_Shown(object sender,EventArgs e) {
		...
		var defaultChargeMethod = PrefC.GetString(PrefName.BillingChargeOrFinanceIsDefault);
	...
	private bool RunAgingEnterprise(bool isOnLoad=false) {
		...
		if(!PrefC.IsAgingAllowedToStart()) {
			if(isOnLoad) {
				ODMessageBox.Show(this,Lan.g(this,"In order to add finance charges, aging must be calculated, but you cannot run aging until it has finished "
				                                  +"the current calculations which began on")+" "+PrefC.GetDateT(PrefName.AgingBeginDateTime)+".\r\n"+Lans.g("If you believe the current aging "
		...
		Prefs.UpdateString(PrefName.AgingBeginDateTime,SOut.DateTime(dateTNow,false));//get lock on pref to block others
		Signalods.SetInvalid(InvalidType.Prefs);//signal a cache refresh so other computers will have the updated pref as quickly as possible
		var progressOD=new ProgressWin();
		progressOD.ActionMain=()=>{
			Ledgers.ComputeAging(0,dateToday);
			Prefs.UpdateString(PrefName.DateLastAging,SOut.Date(dateToday,false));
		};
	...
	private List<PatAging> GetFinanceBillingAgingList() {
		var listSelectedBillTypes=listBillType.SelectedIndices.Select(x => _listDefsBillingType[x].DefNum).ToList();
		var serializableDictionaryPatAgingData=AgingData.GetAgingData(singlePatient:false,includeChanged:true,
			checkExcludeInsPending.Checked,excludeIfUnsentProcs:false,isSuperBills:false, [],daysExcludeInsPending:textDaysInsPendingExclude.Value);
		var listPendingInsPatNums=new List<long>();
		var listKeys=serializableDictionaryPatAgingData.Keys.ToList();//keys are almost certainly not the same as the value of i below
		if(checkExcludeInsPending.Checked) { //Only fill list if excluding pending ins
			for(var i=0;i<serializableDictionaryPatAgingData.Count;i++) { 
		...
		var dateLastStatement=SIn.Date(textExcludeNotBilledSince.Text);
		var hasFilterSinceLastStatement=true;
		...
		var excludeNegativeCredits=false;
		var isSuperStatements=false;
		var isSinglePatient=false;
		var listPatAgings=Patients.GetAgingList(age,dateLastStatement,listSelectedBillTypes,checkBadAddress.Checked,excludeNegativeCredits,SIn.Double(textExcludeLessThan.Text)
			,checkExcludeInactive.Checked,checkIgnoreInPerson.Checked, [],isSuperStatements,isSinglePatient,listPendingInsPatNums,
			[],new Dictionary<long,List<PatAgingTransaction>>(),checkExcludeAccountNoTil.Checked,hasFilterSinceLastStatement,isFinanceBilling:true);
	...
	private void butUndo_Click(object sender,EventArgs e) {
		var dateUndo=SIn.Date(textDateUndo.Text);
		if(dateUndo.Year<1880) {
			MsgBox.Show(this,"There are no previous billing/finance charges to undo.");
			return;
		}
		var chargeType=(radioFinanceCharge.Checked?"Finance":"Billing");
		if(ODMessageBox.Show(Lan.g(this,"Undo all "+chargeType.ToLower()+" charges for")+" "+textDateUndo.Text+"?","",MessageBoxButtons.OKCancel)
		   !=DialogResult.OK)
		{
			return;
		}
		var isBillingCharge=radioBillingCharge.Checked;
		var chargeUndoData=new Adjustments.ChargeUndoData();
		var progressOD=new ProgressWin();
		...
		ODMessageBox.Show(Lan.g(this,chargeType+" charge adjustments deleted")+": "+chargeUndoData.CountDeletedAdjustments);
		if(!chargeUndoData.ListSkippedPatNums.IsNullOrEmpty()
		   && ODMessageBox.Show($"Some "+chargeType.ToLower()+" charges could not be deleted because they have pay splits or a payment plans attached. "
		...
		{
			var message="";
			var listPatients=Patients.GetLimForPats(chargeUndoData.ListSkippedPatNums);
			for(var i=0;i<listPatients.Count;i++) {
				var patient=listPatients[i];
				message+=$"PatNum: {patient.PatNum}, {patient.GetNameFL()}\r\n";
			}
			using var msgBoxCopyPastePatsSkipped=new MsgBoxCopyPaste(message);
	...
	private void butRun_Click(object sender,System.EventArgs e) {
		...
		if(SIn.Double(textAPR.Text) < 2) {
			if(!MsgBox.Show(this,MsgBoxButtons.OKCancel,"The APR is much lower than normal. Do you wish to proceed?")) {
				return;
			}
		}
		var chargeType=(radioFinanceCharge.Checked?"Finance":"Billing");//For display only
		var listPatAgings=GetFinanceBillingAgingList();//Get the Aging List for Finance and Billing
		var adjType=PrefC.GetLong(PrefName.FinanceChargeAdjustmentType);
		...
		if(!checkCompound.Checked) {
			...
			var dateTMaxAdj=MiscData.GetNowDateTime().Date.AddDays(-daysOver);
			listAdjustments=Adjustments.GetAdjustForPatsByType(listPatAgings.Select(x => x.PatNum).ToList(),adjType,dateTMaxAdj);
		}
		var isRadioBillingChargeChecked=radioBillingCharge.Checked;
		var listPatNums=listPatAgings.Select(x => x.PatNum).ToList();
		var listActions=new List<Action>();
		var listInstallmentPlans=new List<InstallmentPlan>();
		...
		progressOD.ShowDialog();
		if(progressOD.IsCancelled) {
			ODMessageBox.Show(Lan.g(this,$"{_chargesAdded} {chargeType} charges added out of {listPatAgings.Count}"));
			return;
		}
		if(radioFinanceCharge.Checked) {
			if(Prefs.UpdateString(PrefName.FinanceChargeAPR,textAPR.Text) 
			   | Prefs.UpdateString(PrefName.FinanceChargeLastRun,SOut.Date(date,false))
			...
			if(Prefs.UpdateString(PrefName.BillingChargeAmount,textBillingCharge.Text)
			   | Prefs.UpdateString(PrefName.BillingChargeLastRun,SOut.Date(date,false))
		...
		ODMessageBox.Show(Lan.g(this,chargeType+" charges added")+": "+_chargesAdded);
		if(!RunAgingEnterprise()) {
	...
	private void AddBillingOrFinanceCharge(PatAging patAging,int listPatAgingsCount,List<Adjustment> listAdjustments,List<InstallmentPlan> listInstallmentPlans) {
		var isRadio60Checked=radio60.Checked;
		var isRadio30Checked=radio30.Checked;
		var isRadioBillingChargeChecked=radioBillingCharge.Checked;
		var billingChargeText=textBillingCharge.Text;
		var aprText=textAPR.Text;
		var atLeastText=textAtLeast.Text;
		var overText=textOver.Text;
		var chargeType=(radioFinanceCharge.Checked?"Finance":"Billing");
		var adjType=PrefC.GetLong(PrefName.FinanceChargeAdjustmentType);
		var date=SIn.Date(textDate.Text);
		_chargesProcessed++;
		...
		var overallBalance=patAging.BalOver90;
		if(isRadio60Checked) {
		...
		if(radioSpecificProv.Checked) {
			var provNum=comboSpecificProv.GetSelectedProvNum();
		...
		List<InstallmentPlan> listInstallmentPlans=null)
	{
		if(listInstallmentPlans!=null) {
			var installmentPlan=listInstallmentPlans.Find(x => x.PatNum==patNum);
		...
		adjustment.AdjAmt = Math.Round(((SIn.Double(APR) * .01d / 12d) * overallBalance),2);
		if(CompareDouble.IsZero(adjustment.AdjAmt) || adjustment.AdjAmt<SIn.Double(ifOver)) {
		...
		adjustment.AdjAmt=Math.Max(adjustment.AdjAmt,SIn.Double(atLeast));
	...
	private void AddBillingCharge(long patNum,DateTime date,string billingChargeAmount,long priProv) {
		var AdjustmentCur = new Adjustment();
		...
		AdjustmentCur.AdjAmt = SIn.Double(billingChargeAmount);
