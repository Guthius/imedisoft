...
public partial class FormEmailAddresses:FormODBase {
	...
	private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		var selectedAddress=gridMain.SelectedTag<EmailAddress>();
		...
		if(IsSelectionMode) {
			...
			using var formEmailAddressEdit=new FormEmailAddressEdit(selectedAddress,isOpenedFromEmailSetup:true);
	...
	private void FillGrid() {
		...
		var listEmailAddresses=EmailAddresses.GetDeepCopy()
		...
		var listUserodNums=new List<long>();
		...
		GridRow row;
		for(var i=0;i<listEmailAddresses.Count;i++) {
			row=new GridRow();
	...
	private void butAdd_Click(object sender,EventArgs e) {
		using var formEmailAddressEdit=new FormEmailAddressEdit(userNum:0,isOpenedFromEmailSetup:true);
		formEmailAddressEdit.ShowDialog();
		if(formEmailAddressEdit.DialogResult==DialogResult.OK) {
			FillGrid();
			IsChanged=true;
	...
	private void butSave_Click(object sender,EventArgs e) {
		if(IsSelectionMode) {
			...
			var inboxCheckIntervalMinuteCount=0;
			try {
