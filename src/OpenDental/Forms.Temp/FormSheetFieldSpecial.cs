...
public partial class FormSheetFieldSpecial:FormODBase {
	...
	private List<SheetFieldDef> _listSheetFieldDefsAvailable;
	...
	private void FormSheetFieldDefEdit_Load(object sender,EventArgs e) {
		...
		listBoxAvailable.Items.AddList(_listSheetFieldDefsAvailable,x => x.FieldName);
		if(SheetFieldDefCur.IsNew) {
			listBoxAvailable.SetSelected(0);
			SheetFieldDefCur=_listSheetFieldDefsAvailable[0];
		}
		else {
			listBoxAvailable.SetSelected(_listSheetFieldDefsAvailable.FindIndex(x => x.FieldName==SheetFieldDefCur.FieldName),true);
	...
	private void SheetFilterChanged() {
		if(EnumTools.GetAttributeOrDefault<SheetLayoutAttribute>(SheetDefCur.SheetType).IsChartModule && SheetFieldDefCur.FieldName=="toothChart") {
			var height=(int)Math.Round(SIn.Double(textWidth.Text)*ToothChartData.SizeOriginalDrawing.Height)/ToothChartData.SizeOriginalDrawing.Width;
	...
	private void FillFields() {
		...
		var isChartModuleSheetType=EnumTools.GetAttributeOrDefault<SheetLayoutAttribute>(SheetDefCur.SheetType).IsChartModule;
		SheetUtilL.FillComboGrowthBehavior(comboGrowthBehavior,SheetFieldDefCur.GrowthBehavior,isChartModuleSheetType);
		...
		switch(((SheetFieldDef)listBoxAvailable.SelectedItem).FieldName){
			case "toothChart":
				...
				if(EnumTools.GetAttributeOrDefault<SheetLayoutAttribute>(SheetDefCur.SheetType).IsChartModule) {
	...
	private void listBoxAvailable_SelectedIndexChanged(object sender,EventArgs e) {
		...
		SheetFieldDefCur=_listSheetFieldDefsAvailable[listBoxAvailable.SelectedIndex];
		FillFields();
	}
