...
public partial class FormTerminalConnection : FormODBase,ITerminalConnector {
	...
	public void Send(char charSingle){
		...
		if(_terminalConnector.IsSpecialCode(charSingle)){
			...
			textTx.Text+=charSingle+"\r\n";
			textProgress.Text+="Sent char "+charSingle+"\r\n";
		}
		ScrollProgress();
	}
	...
	public string WaitFor(string expectedText,double timeoutMS){
		...
		ScrollProgress();
		var textReceived=_terminalConnector.WaitFor(expectedText,timeoutMS);
	...
	public string WaitFor(string expectedText1,string expectedText2,double timeoutMS){
		...
		ScrollProgress();
		var textReceived=_terminalConnector.WaitFor(expectedText1,expectedText2,timeoutMS);
		textProgress.Text+="Receieved: "+textReceived+"'\r\n";
		ScrollProgress();
		return textReceived;
	}
	...
	public void WaitFor(byte byteExpected,double timeoutMS){
		textProgress.Text+="Waiting for byte "+byteExpected+"\r\n";
		ScrollProgress();
		var byteReceived=_terminalConnector.WaitFor(byteExpected,timeoutMS);
		if(_terminalConnector.IsSpecialCode((char)byteReceived)) {
			...
			textProgress.Text+="Receieved expected byte: "+byteReceived+"'\r\n";
		}
	}
	...
	public byte GetOneByte(double timeoutMS){
		var byteReceived=_terminalConnector.GetOneByte(timeoutMS);
		if(_terminalConnector.IsSpecialCode((char)byteReceived)) {
	...
	public byte[] GetBytes(int numberOfBytes, double timeoutMS){
		var byteArrayReceived=_terminalConnector.GetBytes(numberOfBytes,timeoutMS);
	...
	public string Receive(double timeoutMS){
		...
		ScrollProgress();
		var textReceived=_terminalConnector.Receive(timeoutMS);
		textProgress.Text+="Receieved: "+textReceived+"'\r\n";
		ScrollProgress();
		return textReceived;
	}
	...
	public void Pause(double ms){
		textProgress.Text+="Pausing for "+ms+" ms\r\n";
		ScrollProgress();
