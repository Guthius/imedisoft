...
public partial class FormInsHistSetup:FormODBase {
	...
	private void FormInsHistSetup_Load(object sender,EventArgs e) {
		...
		var listProcNums=_listProceduresEoAndC.Select(x => x.ProcNum).ToList();
		var listClaimProcs=ClaimProcs.GetForProcs(listProcNums);
		//Find all of the claimprocs associated with this inssub.
		_listClaimProcsForEoAndCProcs= [];
		listProcNums.Clear();
		...
		var listPrefs=Prefs.GetInsHistPrefs();
		...
		for(var i=0;i<listPrefNames.Count;i++) {
			...
			var isPrefSet=listPrefs.Exists(x => x.PrefName==listPrefNames[i].ToString() && !string.IsNullOrWhiteSpace(x.ValueString));
			var textBox=GetControlForPrefName(listPrefNames[i]);
			if(!isPrefSet) {
			...
			var listCodeNums=ProcedureCodes.GetCodeNumsForInsHistPref(listPrefNames[i]);
			var procedure=Procedures.GetMostRecentInsHistProc(_listProceduresEoAndC,listCodeNums,listPrefNames[i]);
	...
	private void butSave_Click(object sender,EventArgs e) {
		...
		var error="";
		var listProceduresToDelete=new List<Procedure>();
		var listPrefNames = Prefs.GetInsHistPrefNames();
		for(var i=0;i<listPrefNames.Count;i++){
			var textBox=GetControlForPrefName(listPrefNames[i]);
			...
			var listCodeNumsForPref=ProcedureCodes.GetCodeNumsForInsHistPref(listPrefNames[i]);
			var procedure=Procedures.GetMostRecentInsHistProc(_listProceduresEoAndC,listCodeNumsForPref,listPrefNames[i]);
			...
			var dateEntered=SIn.Date(textBox.Text);
		...
		for(var i=0;i<listProceduresToDelete.Count;i++){
			try {
