...
public partial class FormCarriers : FormODBase {
	...
	private List<ItransImportFields> _listItransImportFields= [];
	...
	private void FormCarriers_Load(object sender, System.EventArgs e) {
		...
		if(CultureInfo.CurrentCulture.Name.EndsWith("CA") && clearingHouse.Eformat==ElectronicClaimFormat.Canadian) {
			...
			var itransImportFields=(ItransImportFields)PrefC.GetInt(PrefName.ItransImportFields);
	...
	private void FillGrid(){
		var selectedCarrierNums=new List<string>();
	...
	private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		var carrier=Carriers.GetCarrier(SIn.Long(_table.Rows[e.Row]["CarrierNum"].ToString()));
		if(IsSelectMode) {
	...
	private void gridMain_CellClick(object sender,ODGridClickEventArgs e) {
		var cell=gridMain.ListGridRows[e.Row].Cells[e.Col];
	...
	private void butItransUpdateCarriers_Click(object sender,EventArgs e) {
		...
		var itransImportFields=ItransImportFields.None;
		if(checkITransPhone.Checked) {
			itransImportFields=(itransImportFields|ItransImportFields.Phone);
			listFields.Add(Lans.g("Phone"));
		}
		if(checkItransAddress.Checked) {
			itransImportFields=(itransImportFields|ItransImportFields.Address);
			listFields.Add(Lans.g("Address"));
		}
		var stringBuilder=new StringBuilder();
		if(listFields.Count>0) {
			stringBuilder.Insert(0,Lans.g("The following carrier fields will be imported when missing for carriers matched by their Electronic IDs:")
			                       +" "+string.Join(", ",listFields)
			                       +"\r\n");
		}
		if(checkItransMissing.Checked) {
			itransImportFields=(itransImportFields|ItransImportFields.AddMissing);
			stringBuilder.AppendLine(Lans.g("New carriers will be added if missing from the database based on Electronic ID."));
		}
		if(stringBuilder.Length>0) {
			stringBuilder.AppendLine(Lans.g("Continue?"));
		...
		var errorMsg=ItransNCpl.TryCarrierUpdate(false,itransImportFields);
	...
	private void butAdd_Click(object sender, System.EventArgs e) {
		...
		var phoneFormatted=TelephoneNumbers.ReFormat(textPhone.Text);
	...
	private void butCombine_Click(object sender, System.EventArgs e) {
		...
		var pickedCarrierNums=new List<long>();
		for(var i=0;i<gridMain.SelectedIndices.Length;i++) {
		...
		DataValid.SetInvalid(InvalidType.Carriers);
		var logText=Lan.g(this,"The following carriers were combined into")+$" '{carrierName}': {carrierNames}";
	...
	private bool VerifyCarrierCombineData(long pickedCarrierNum,List<long> listCarrierNums) {
		var listCarriers = Carriers.GetCarriers(listCarrierNums);
		var listWarnings = new List<string>();
		var carrier = listCarriers.FirstOrDefault(x => x.CarrierNum==pickedCarrierNum);
		...
		if(listWarnings.Count>0) {
			var warningMessage=Lan.g(this,"WARNING!")+" "+Lan.g(this,"Mismatched data has been detected between selected carriers")+":\r\n\r\n"
			                   +string.Join("\r\n",listWarnings)+"\r\n\r\n"
