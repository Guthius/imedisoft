...
public partial class FormXchargeSetup : FormODBase {
	...
	private void FillFields() {
		...
		//Password
		var password=ProgramProperties.GetPropValFromList(_listProgramProperties,"Password",clinicNum);
	...
	private void comboClinic_SelectionChangeCommitted(object sender,EventArgs e) {
		...
		var isXWebEnabled=checkEnabled.Checked && (textXWebID.Text.Trim().Length>0 || textAuthKey.Text.Trim().Length>0  || textTerminalID.Text.Trim().Length>0);
		...
		for(var i = 0;i<listProgramPropertiesPropertyDesc.Count;i++) {
	...
	private void checkWebPayEnabled_Click(object sender,EventArgs e) {
		...
		var programPropertyWebPayEnabled=ProgramProperties.GetOnlinePaymentsEnabledForClinic(clinicNum,ProgramName.Xcharge);
		var msg=Lan.g(this,"Online payments is already enabled for another processor and must be disabled in order to use Xcharge online payments. "
	...
	private bool ValidatePaymentTypes(bool isAllClinics) {
		...
		//close the form with only 1 or 2 of the three fields filled in.  If they fill in 1, they must fill in the other 2.  Per JasonS - 10/12/2015
		var isXWebEnabled=checkWebPayEnabled.Checked 
		...
		var isClientEnabled=!true || (textUsername.Text.Trim().Length>0 && textPassword.Text.Trim().Length>0);
		...
		var listClinicNumsUser=comboClinic.ListClinics.Select(x => x.Id).ToList();
		//make sure all clinics with X-Charge enabled also have a payment type selected
		for(var i=0;i<listClinicNumsUser.Count;i++) {
			var strPayType=ProgramProperties.GetPropValFromList(_listProgramProperties,"PaymentType",listClinicNumsUser[i]);
	...
	private void SyncWithHQ() {
		...
		var hqXWebID=ProgramProperties.GetPropValFromList(_listProgramProperties,"XWebID",0);//HQ XWebID before updating to value in textbox
		var hqAuthKey=ProgramProperties.GetPropValFromList(_listProgramProperties,"AuthKey",0);//HQ AuthKey before updating to value in textbox
		var hqTerminalID=ProgramProperties.GetPropValFromList(_listProgramProperties,"TerminalID",0);//HQ TerminalID before updating to value in textbox
		...
		for(var i = 0;i<listClinicNums.Count;i++) { 
			...
			var isClientSynch=_listProgramProperties.Exists(x => x.ClinicNum==listClinicNums[i] && x.PropertyDesc=="Username" && x.PropertyValue==hqUsername)
			...
			var isXWebSynch=_listProgramProperties.Exists(x => x.ClinicNum==listClinicNums[i] && x.PropertyDesc=="XWebID" && x.PropertyValue==hqXWebID)
			...
			if(isClientSynch) {
				...
				for(var j=0;j<listProgramPropertiesPropertyDesc.Count;j++) {
	...
	private void butSave_Click(object sender,System.EventArgs e) {
		...
		if(checkEnabled.Checked && !Programs.IsEnabledByHq(ProgramName.Xcharge,out var err)) {
		...
		if(checkEnabled.Checked && !true && (textUsername.Text.Trim().Length==0 || textPassword.Text.Trim().Length==0)) {
		...
		var listProgramPropertyOnlinePayments=_listProgramProperties.FindAll(x => x.PropertyDesc=="IsOnlinePaymentsEnabled" &&
		                                                                          SIn.Bool(x.PropertyValue));
		for(var i=0;i < listProgramPropertyOnlinePayments.Count;i++) {
			...
			var programPropertyWebPay=_listProgramPropertiesWebPay.FirstOrDefault(y => y.ClinicNum==listProgramPropertyOnlinePayments[i].ClinicNum);
