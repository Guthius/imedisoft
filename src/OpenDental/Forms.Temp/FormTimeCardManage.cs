...
public partial class FormTimeCardManage:FormODBase {
	...
	private GridOD FillMain(bool isForGridPrint=false) {
		...
		grid.BeginUpdate();
		grid.Columns.Clear();
		var col=new GridColumn(Lan.g(this,"Employee"),140);
		...
		GridRow row;
		for(var i=0;i<listEmployeeTimeCards.Count;i++) {
			row=new GridRow();
	...
	private void gridMain_CellDoubleClick(object sender,UI.ODGridClickEventArgs e) {
		...
		var listEmployeesCopy=_listEmployees.Select(x => x.Copy()).ToList();
		using var formTimeCard=new FormTimeCard(listEmployeesCopy);
	...
	private GridOD GetGridForPrinting(Employee employee) {
		var gridTimeCard=new GridOD();
		gridTimeCard.TranslationName="";
		var listClockEvents=ClockEvents.Refresh(employee.EmployeeNum,SIn.Date(textDateStart.Text),SIn.Date(textDateStop.Text),false);
		var listTimeAdjusts=TimeAdjusts.Refresh(employee.EmployeeNum,SIn.Date(textDateStart.Text),SIn.Date(textDateStop.Text));
		#region Hide time card note (we will show it at the top of the printout above the grid)
		var datePayPeriodStart=_listPayPeriods[_idxPayPeriodSelected].DateStart.Date;
		var dateTimeMidnightFirstDayOfPeriod=new DateTime(datePayPeriodStart.Year,datePayPeriodStart.Month,datePayPeriodStart.Day,0,0,0);
		...
		#endregion
		var arrayListMerged=new ArrayList();
		...
		var objectDateComparer=new ObjectDateComparer();
		...
		GridRow row;
		var timeSpanArrayWeeklyTotals=new TimeSpan[arrayListMerged.Count];
		var timeSpanAltered=new TimeSpan(0);//used to display altered times
		var timeSpanOne=new TimeSpan(0);//used to sum one pair of clock-in/clock-out
		TimeSpan timeSpanOneAdj;
		TimeSpan timeSpanOneOT;
		var timeSpanDay=new TimeSpan(0);//used for daily totals.
		var timeSpanWeek=new TimeSpan(0);//used for weekly totals.
		var timeSpanPTO=new TimeSpan(0);//used for PTO totals.
		var timeSpanUnpaidProtectedLeave=new TimeSpan(0);//used for Unpaid Protected Leave totals.
		if(arrayListMerged.Count>0){
			timeSpanWeek=ClockEvents.GetWeekTotal(employee.EmployeeNum,GetDateForRow(0,arrayListMerged));
		}
		var timeSpanPeriod=new TimeSpan(0);//used to add up totals for entire page.
		var timeSpanOT=new TimeSpan(0);//overtime for the entire period
		var timeSpanRate2=new TimeSpan(0);//rate2 hours total
		var timeSpanRate3=new TimeSpan(0);//rate3 hours total
		var calendar=CultureInfo.CurrentCulture.Calendar;
		var calendarWeekRule=CultureInfo.CurrentCulture.DateTimeFormat.CalendarWeekRule;
		var dateTimeCurrent=DateTime.MinValue;
		var dateTimePrevious=DateTime.MinValue;
		for(var i=0;i<arrayListMerged.Count;i++){
			row=new GridRow();
			var type=arrayListMerged[i].GetType();
			row.Tag=arrayListMerged[i];
			dateTimePrevious=dateTimeCurrent;
			#region ClockEvent Row
			if(type==typeof(ClockEvent)){
				var clockEvent=(ClockEvent)arrayListMerged[i];
			...
			#region Adjustment Row
				...
				var timeAdjust=(TimeAdjust)arrayListMerged[i];
				...
				{
					var cell=new GridCell(ClockEvents.Format(timeSpanWeek));
	...
	private void PrintEveryTimeCard(object sender,System.Drawing.Printing.PrintPageEventArgs e) {
		...
		var employee=gridMain.GetTags<EmployeeTimeCard>()[_pagesPrinted].Employee;
	...
	private void PrintEmployeeTimeCard(object sender, System.Drawing.Printing.PrintPageEventArgs e) {
		//A preview of every single emp on their own page will show up. User will print from there.
		var employee=gridMain.SelectedTags<EmployeeTimeCard>()[_pagesPrinted].Employee;
	...
	private void PrintTimeCard(object sender, System.Drawing.Printing.PrintPageEventArgs e, Employee employee,int pagesToPrint) {
		...
		var gridTimeCard=GetGridForPrinting(employee);
		var linesPrinted=0;
		...
		using var fontTitle=new Font(FontFamily.GenericSansSerif,11,FontStyle.Bold);
		using var fontHeader=new Font(FontFamily.GenericSansSerif,8,FontStyle.Bold);
		using var solidBrush=new SolidBrush(Color.Black);
		using var pen=new Pen(Color.Black);
		//Title
		str=employee.FName+" "+employee.LName;
		str+="\r\n"+Lan.g(this,"Note")+": "+_timeAdjustNote?.Note??"";
		var threeLineHeight=(int)e.Graphics.MeasureString("1\r\n2\r\n3",fontTitle).Height;
		var marginBothSides=(int)xPos*2;//110
		var heightNoteString=(int)e.Graphics.MeasureString(str,fontTitle,e.PageBounds.Width-marginBothSides).Height;
		var heightRect=Math.Min(heightNoteString,threeLineHeight);
		using var stringFormatNote=new StringFormat{ Trimming=StringTrimming.Word };
		...
		for(var i=0;i<ColCaption.Length;i++) {
		...
		while(yPos < e.PageBounds.Height-75-50-32-16 && linesPrinted < gridTimeCard.ListGridRows.Count) {
			for(var i=0;i<colPos.Length-1;i++) {
	...
	private void butDaily_Click(object sender,EventArgs e) {
		...
		var errorAllEmployees=TimeCardRules.ValidateOvertimeRules([0]);//Validates the "all employees" timecard rules first.
		...
		for(var i=0;i<gridMain.SelectedIndices.Length;i++) {
			try {
				var employee=gridMain.SelectedTag<EmployeeTimeCard>().Employee;
		...
		var listIndicesSelected=new List<int>();
		for(var i=0;i<gridMain.SelectedIndices.Length;i++) {
	...
	private void butWeekly_Click(object sender,EventArgs e) {
		...
		Cursor=Cursors.WaitCursor;
		var aggregateErrors="";
		var listEmployeeTimeCards=gridMain.SelectedTags<EmployeeTimeCard>();
		for(var i=0;i<listEmployeeTimeCards.Count;i++) {
			try {
	...
	private void butExportGrid_Click(object sender,EventArgs e) {
		...
		var headers=string.Join("\t",listHeaders);
		stringBuilder.AppendLine(headers);
		var listEmployeeTimeCards=gridMain.GetTags<EmployeeTimeCard>();
		for(var i=0;i<listEmployeeTimeCards.Count;i++) {
	...
	private void butExportADP_Click(object sender,EventArgs e) {
		var hasVisiblePtoDef=false;
		//Check to confirm if any PTO definitions are hidden, if not show these defs on the timecard
		var listDefsPtoTypes=Defs.GetDefsForCategory(DefCat.TimeCardAdjTypes);
		for(var i=0;i<listDefsPtoTypes.Count();i++) {
		...
		var coCode=PrefC.GetString(PrefName.ADPCompanyCode);
		var batchID=_listPayPeriods[_idxPayPeriodSelected].DateStop.ToString("yyyyMMdd");//max 8 characters
		...
		var listEmployeeTimeCards=gridMain.GetTags<EmployeeTimeCard>();
		for(var i=0;i<listEmployeeTimeCards.Count;i++) {
			var errorsForEmployee="";
			var warningsForEmployee="";
			var fileNum=listEmployeeTimeCards[i].Employee.PayrollID;
			...
			var r1hours=listEmployeeTimeCards[i].Rate1Hours.TotalHours.ToString("F2");//adp allows 2 digit precision
			if(r1hours=="0.00"){//Was changing Exactly 80.00 hours with 8 hours.
				r1hours="";
			}
			var r1OThours=listEmployeeTimeCards[i].Rate1OTHours.TotalHours.ToString("F2");//adp allows 2 digit precision
			if(r1OThours=="0.00") {
				r1OThours="";
			}
			var r2hours=listEmployeeTimeCards[i].Rate2Hours.TotalHours.ToString("F2");//adp allows 2 digit precision
			if(r2hours=="0.00") {
				r2hours="";
			}
			var r2OThours=listEmployeeTimeCards[i].Rate2OTHours.TotalHours.ToString("F2");//adp allows 2 digit precision
			if(r2OThours=="0.00") {
				r2OThours="";
			}
			var r3hours  =listEmployeeTimeCards[i].Rate3Hours.TotalHours.ToString("F2");//adp allows 2 digit precision
			if(r3hours=="0.00") {
				r3hours="";
			}
			var r3OThours=listEmployeeTimeCards[i].Rate3OTHours.TotalHours.ToString("F2");//adp allows 2 digit precision
			if(r3OThours=="0.00") {
				r3OThours="";
			}
			var ptoHours=listEmployeeTimeCards[i].PTOHours.TotalHours.ToString("F2");//adp allows 2 digit precision
			if(ptoHours=="0.00") {
				ptoHours="";
			}
			var textToAdd="";
			var employeeName=Employees.GetName(listEmployeeTimeCards[i].Employee);
			...
			for(var j=0;j<textToAdd.Length;j++) {
				var charAsInt=(int)textToAdd[j];
			...
			if(errorsForEmployee!="") {
				errors+=Employees.GetName(listEmployeeTimeCards[i].Employee)+":\r\n"+errorsForEmployee+"\r\n";
			}
			if(warningsForEmployee!="") {
				warnings+=Employees.GetName(listEmployeeTimeCards[i].Employee)+":\r\n"+warningsForEmployee+"\r\n";
			}
		}
		using var folderBrowserDialog=new FolderBrowserDialog();
		if(!false) {
	...
	private void butExportADPRun_Click(object sender,EventArgs e) {
		...
		var ADPRunIID=PrefC.GetString(PrefName.ADPRunIID);					// 1 - IID == ADPRunIID
		var payPeriodInterval=PrefC.GetEnum<PayPeriodInterval>(PrefName.PayPeriodIntervalSetting);
		var payFrequency=payPeriodInterval.ToString();							// 2 - Pay Frequency: OD only supports Weekly, Biweekly, or Monthly pay period intervals
		var startDate=textDateStart.Text;													// 3 - Pay Period Start
		var endDate=textDateStop.Text;															// 4 - Pay Period End
		var dollars="";																						// 8 - Dollars, not required
		var separateCheck=0;																					// 9 - Separate Check, ADP defaults 0
		var workedDepartment="";																		//10 - Worked Department, not required
		//Loop through employees to fill out remaining columns
		var listEmployeeTimeCards=gridMain.GetTags<EmployeeTimeCard>();
		for(var i=0;i<listEmployeeTimeCards.Count;i++) {
			var textToAdd=ADPRunIID+","+payFrequency.First()+","+startDate+","+endDate+",";
		...
		var fileName=payFrequency+"-"+startDate.Replace("/","")+"-"+endDate.Replace("/","");
	...
	private bool PayFrequencyMatchesDateRange(PayPeriodInterval payPeriodInterval) {
		...
		var dateEnd=SIn.Date(textDateStop.Text);
	...
	private void butTimeCardBenefits_Click(object sender,EventArgs e) {
		var formTimeCardBenefitRp = new FormTimeCardBenefitRp();
