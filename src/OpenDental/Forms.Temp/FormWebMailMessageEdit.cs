...
public partial class FormWebMailMessageEdit:FormODBase {
	...
	private List<EmailAttach> _listEmailAttaches= [];
	...
	private string GetBodyTextInsecure(){
		var bodyTextInsecure=PrefC.GetString(PrefName.PatientPortalNotifyBody);
	...
	private void FormWebMailMessageEdit_Load(object sender,EventArgs e) {
		...
		var listUserods=Userods.GetUsersWithProviders();
	...
	private void FillFields() {
		...
		if(_emailMessage==null) {
			for(var i=0;i<_listPatientsForPHI.Count;i++) {
				var patient=_listPatientsForPHI[i];
			...
			if(_webMailMode==WebMailMode.Reply) {
				...
				var toAddressOld=_emailMessage.ToAddress;
				var fromAddressOld=_emailMessage.FromAddress;
	...
	private void SetEnabledHelper() {
		if(_webMailMode==WebMailMode.View) {
			...
			listAttachments.ContextMenu=new ContextMenu([menuItemAttachmentPreview]);
	...
	private void listAttachments_MouseDown(object sender,MouseEventArgs e) {
		...
		if(e.Button==MouseButtons.Right) {
			var idxClicked=listAttachments.IndexFromPoint(e.X,e.Y);
			if(idxClicked!=-1) {
	...
	private void menuItemAttachmentRemove_Click(object sender,EventArgs e) {
		try {
			...
			File.Delete(Path.Combine(EmailAttaches.GetAttachPath(),_listEmailAttaches[listAttachments.SelectedIndex].ActualFileName));				
	...
	private void butSend_Click(object sender,EventArgs e) {
		...
		butSend.Enabled=false;
		var emailType=PrefC.GetEnum<EmailType>(PrefName.PortalWebEmailTemplateType);
		...
		if(_allowSendNotificationMessage) {
			...
			var clinic=Clinics.GetClinic(_patient.ClinicNum)??Clinics.GetPracticeAsClinicZero();
