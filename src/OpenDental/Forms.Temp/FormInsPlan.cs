...
public partial class FormInsPlan : FormODBase {
	...
	private UI.ListBox _listBoxEmps;
	/// <summary>displayed from within code, not designer</summary>
	private UI.ListBox _listBoxCarriers;
	...
	public FormInsPlan(InsPlan insPlan,PatPlan patPlan,InsSub insSub) {
		...
		_insSub=insSub;
		_listBoxEmps=new UI.ListBox();//Instead of UI.ListBox for consistency with listBoxCarriers.
		...
		_listBoxEmps.Font=new Font(Font.FontFamily,Font.Size);
		...
		_listBoxCarriers.Location=new Point(
			tabControlInsPlan.Left+tabPageInsPlanInfo.Left+panelPlan.Left+groupPlan.Left+groupCarrier.Left+textCarrier.Left,
		...
		_listBoxCarriers.Font=new Font(Font.FontFamily,Font.Size);
	...
	private void FormInsPlan_Load(object sender,System.EventArgs e) {
		...
		if(!string.IsNullOrWhiteSpace(perVisitPatAmountProcCode) && !string.IsNullOrWhiteSpace(perVisitInsAmountProcCode)) {
			labelPerVisit.Visible=false;
		}
		if(!Security.IsAuthorized(EnumPermType.InsPlanEdit,true)) {
			var labelNoPermission=new Label();
			...
			textPerVisitPatAmount.Enabled=false;
			textPerVisitInsAmount.Enabled=false;
		}
		if(!Security.IsAuthorized(EnumPermType.InsuranceVerification,true)) {
		...
		var insVerifyBenefits=InsVerifies.GetOneByFKey(_insPlan.PlanNum,VerifyTypes.InsuranceBenefit);
		...
		_listBenefits=Benefits.GetForPlanOrPatPlan(_insPlan.PlanNum,patPlanNum);
		_listBenefitsOld=_listBenefits.Select(x => x.Copy()).ToList();
		...
		var program=Programs.GetCur(ProgramName.Trojan);
		...
		if(_patPlan==null) {
			...
			for(var i=0;i<Enum.GetNames(typeof(Relat)).Length;i++) {
			...
			if(_patPlan.PatPlanNum!=0) {
				...
				if(IsNewPatPlan) {
					...
					var insVerifyPatPlan=InsVerifies.GetOneByFKey(_patPlan.PatPlanNum,VerifyTypes.PatientEnrollment);
		...
		FillBenefits();
		var dateTimeLast270=Etranss.GetLastDate270(_insPlan.PlanNum);
		...
		_dateTimePatPlanLastVerified=SIn.Date(textDateLastVerifiedPatPlan.Text);
		...
		FillOrtho();
		if(!Security.IsAuthorized(EnumPermType.InsPlanEdit,true) || !Security.IsAuthorized(EnumPermType.CarrierEdit,true)) {
	...
	private void FillOrtho() {
		...
		for(var i = 0;i<Enum.GetValues(typeof(OrthoClaimType)).Length;i++) {
		...
		for(var i=0;i<Enum.GetValues(typeof(OrthoAutoProcFrequency)).Length;i++) {
	...
	private void SetEnabledOrtho() {
		if(!Security.IsAuthorized(EnumPermType.InsPlanOrthoEdit,true)) {
			//Disable every control within the Ortho tab.
			for(var i=0;i<panelOrtho.Controls.Count;i++) {
	...
	private void FillFormWithPlanCur(bool isPicked) {
		...
		if(_insPlan.PlanType=="p") {
			...
			var feeSchedCopay=FeeScheds.GetFirstOrDefault(x => x.FeeSchedNum==_insPlan.CopayFeeSched 
		...
		_insPlanTypeComboItemSelected=SIn.Enum<InsPlanTypeComboItem>(comboPlanType.SelectedIndex);
		...
		for(var i=0;i<_listClaimForms.Count;i++) {
			...
			var descript=_listClaimForms[i].Description;
		...
		for(var i=0;i<Enum.GetNames(typeof(EnumCobRule)).Length;i++) {
		...
		var selectedFilingCodeNum=_insPlan.FilingCode;
		...
		for(var i=0;i<_listInsFilingCodes.Count;i++) {
		...
		comboBoxZeroWriteOffsOnAnnualMaxLimitation.Items.Clear();
		comboBoxZeroWriteOffsOnAnnualMaxLimitation.Items.AddList(Enum.GetValues(typeof(YN)).Cast<YN>(),(input)=>{ return input==YN.Unknown?"Default":input.GetDescription();});
		comboBoxZeroWriteOffsOnAnnualMaxLimitation.SetSelected((int)_insPlan.InsPlansZeroWriteOffsOnAnnualMaxOverride);
		comboZeroWriteOffsOnLimitationOrAging.Items.Clear();
		comboZeroWriteOffsOnLimitationOrAging.Items.AddList(Enum.GetValues(typeof(YN)).Cast<YN>(),(input)=>{ return input==YN.Unknown?"Default":input.GetDescription();});
		comboZeroWriteOffsOnLimitationOrAging.SetSelectedEnum((int)_insPlan.InsPlansZeroWriteOffsOnFreqOrAgingOverride);
		textPerVisitPatAmount.Text=_insPlan.PerVisitPatAmount.ToString("f");
		textPerVisitInsAmount.Text=_insPlan.PerVisitInsAmount.ToString("f");
		...
		if(insVerifyBenefits!=null && insVerifyBenefits.DateLastVerified.Year>1880) {//Only show a date if this insurance has ever been verified
			textDateLastVerifiedBenefits.Text=insVerifyBenefits.DateLastVerified.ToShortDateString();
			_dateTimeInsPlanLastVerified=SIn.Date(textDateLastVerifiedBenefits.Text);
		}
	...
	private List<FeeSched> GetFilteredCopayFeeSched(List<FeeSched> listFeeSchedCopays) {
		...
		var listFeeSchedFiltered=new List<FeeSched>();
		for(var i=0;i<listFeeSchedCopays.Count;i++) {
	...
	private void FillOtherSubscribers() {
		...
		var countSubs=InsSubs.GetSubscriberCountForPlan(_insPlan.PlanNum,excludeSub!=-1);
		...
		var listStringSubs=InsSubs.GetSubscribersForPlan(_insPlan.PlanNum,excludeSub);
	...
	private void butOtherSubscribers_Click(object sender,EventArgs e) {
		using var formSubscribersList=new FormSubscribersList();
	...
	private void FillPatientAdjustments() {
		var listClaimProcs=ClaimProcs.Refresh(_patPlan.PatNum);
		...
		listAdj.Items.Clear();
		string s;
		for(var i=0;i<_arrayListAdj.Count;i++) {
	...
	private void FillPayor() {
		...
		var listPayorNames=ElectIDs.GetDescripts(textElectID.Text);
		if(listPayorNames.Count>1) {
			comboElectIDdescript.Items.Add("multiple payors use this ID");
		}
		for(var i=0;i<listPayorNames.Count;i++) {
			comboElectIDdescript.Items.Add(listPayorNames[i]);
		}
		if(listPayorNames.Count>0) {
	...
	private void comboPlanType_SelectionChangeCommitted(object sender,System.EventArgs e) {
		   ...
		   && comboPlanType.SelectedIndex.In((int)InsPlanTypeComboItem.MedicaidOrFlatCopay,(int)InsPlanTypeComboItem.Capitation)) 
		{
			...
			for(var i=_listBenefits.Count-1;i>=0;i--) {
			...
			for(var i=0;i<_listBenefits.Count;i++) {
		...
		var InsPlanTypeComboItemPrevSelection=_insPlanTypeComboItemSelected;
		_insPlanTypeComboItemSelected=SIn.Enum<InsPlanTypeComboItem>(comboPlanType.SelectedIndex);
	...
	private bool IsFixedBenefitMismatch(FeeSched feeSched) {
		...
		var isFixedBenefitSched=(feeSched.FeeSchedType==FeeScheduleType.FixedBenefit);
		var isFixedBenefitPlanType=(_insPlanTypeComboItemSelected==InsPlanTypeComboItem.PPOFixedBenefit);
	...
	//private bool IsHorizontalScrollBarVisibleForListBox(System.Windows.Forms.ListBox listBox) {
	...
	private int CalculateListBoxHeight<T>(UI.ListBox listBox, List<T> listItems) {
		...
		var h=(13*listItems.Count+5);//+horizontalScrollBarHeight;
	...
	private void FillComboCoPay() {
		var listFilteredCopayFeeSched=GetFilteredCopayFeeSched(_listFeeSchedsCopay);
	...
	private void butAdjAdd_Click(object sender,System.EventArgs e) {
		var claimProc=ClaimProcs.CreateInsPlanAdjustment(_patPlan.PatNum,_insPlan.PlanNum,_insSub.InsSubNum);
		using var formInsAdj=new FormInsAdj(claimProc);
	...
	private void butHistory_Click(object sender,EventArgs e) {
		using var formInsHistSetup=new FormInsHistSetup(_patPlan.PatNum,_insSub);
	...
	private void listAdj_DoubleClick(object sender,System.EventArgs e) {
		...
		using var formInsAdj=new FormInsAdj((ClaimProc)_arrayListAdj[listAdj.SelectedIndex]);
		formInsAdj.ShowDialog();
		FillPatientAdjustments();
	}
	...
	private void butPick_Click(object sender,EventArgs e) {
		if(!IsNewPlan && !Security.IsAuthorized(EnumPermType.InsPlanPickListExisting,true)) {
		...
		frmInsPlanSelect.empText=textEmployer.Text;
		frmInsPlanSelect.carrierText=textCarrier.Text;
		...
		if(frmInsPlanSelect.InsPlanSelected.PlanNum==0) {//user clicked Blank
			_insPlan=new InsPlan();
			_insPlan.PlanNum=_insPlanOld.PlanNum;
		}
		else {//user selected an existing plan
			_insPlan=frmInsPlanSelect.InsPlanSelected;
			textInsPlanNum.Text=frmInsPlanSelect.InsPlanSelected.PlanNum.ToString();
		}
		...
		FillBenefits();
		if(IsNewPlan || frmInsPlanSelect.InsPlanSelected.PlanNum==0) {//New plan or user clicked blank.
	...
	private void textEmployer_KeyUp(object sender,System.Windows.Forms.KeyEventArgs e) {
		...
		var listEmployersSimilar=Employers.GetSimilarNames(textEmployer.Text);
		for(var i=0;i<listEmployersSimilar.Count;i++) {
			_listBoxEmps.Items.Add(listEmployersSimilar[i].EmpName);
		}
		var height=CalculateListBoxHeight<Employer>(_listBoxEmps,listEmployersSimilar);
		_listBoxEmps.Size=new Size(_listBoxEmps.Width,height);
	...
	private void textCarrier_KeyUp(object sender,System.Windows.Forms.KeyEventArgs e) {
		...
		for(var i=0;i<_listCarriersSimilar.Count;i++) {
		...
		var height=CalculateListBoxHeight<Carrier>(_listBoxCarriers,_listCarriersSimilar);
		_listBoxCarriers.Size=new Size(_listBoxCarriers.Width,height);
	...
	private void textElectID_Validating(object sender,System.ComponentModel.CancelEventArgs e) {
		...
		var listElectIDs=ElectIDs.GetDescripts(textElectID.Text);
		if(listElectIDs.Count==0) {//if none found in the predefined list
	...
	private void butChange_Click(object sender,EventArgs e) {
		...
		try {
			...
			if(PrefC.GetBool(PrefName.SubscriberAllowChangeAlways)) {
				var dialogResult=ODMessageBox.Show(Lan.g(this,"Warning!  Do not change unless fixing database corruption.  ")+"\r\n"+ex.Message);
				if(dialogResult!=DialogResult.OK) {
					return;
				}
			}
			else {
				ODMessageBox.Show(Lan.g(this,"Not allowed to change.")+"\r\n"+ex.Message);
				return;
			}
		}
		...
		var family=Patients.GetFamily(_insSub.Subscriber);
		var frmFamilyMemberSelect=new FrmFamilyMemberSelect(family,true);
		frmFamilyMemberSelect.ShowDialog();
		if(frmFamilyMemberSelect.IsDialogCancel) {
			return;
		}
		_insSub.Subscriber=frmFamilyMemberSelect.SelectedPatNum;
		var patientSubscriber=Patients.GetLim(frmFamilyMemberSelect.SelectedPatNum);
	...
	private void CheckAssign_Click(object sender,EventArgs e) {
		if(!Security.IsAuthorized(EnumPermType.InsPlanChangeAssign)) {
	...
	private void FillComboFilingSubtype(long selectedFilingCode) {
		...
		var listInsFilingCodeSubtype=InsFilingCodeSubtypes.GetForInsFilingCode(selectedFilingCode);
		for(var i=0;i<listInsFilingCodeSubtype.Count;i++) {
	...
	private void comboFilingCode_SelectionChangeCommitted(object sender,EventArgs e) {
		var insFilingCode=comboFilingCode.GetSelected<InsFilingCode>();
	...
	private void butImportTrojan_Click(object sender,System.EventArgs e) {
		...
		var trojanObject=Trojan.ProcessTextToObject(File.ReadAllText(file));
		...
		if(trojanObject.PlanNote!="") {
			if(textPlanNote.Text=="") {
				...
				var stringArrayNote=new string[2];
		...
		//Keep all limitation or age limit benefits.
		var listBenefitsToKeep=_listBenefits.FindAll(x => Benefits.IsFrequencyLimitation(x) || Benefits.IsAgeLimit(x));
		_listBenefits= [];
		_listBenefits.AddRange(listBenefitsToKeep);
		for(var i=0;i<trojanObject.BenefitList.Count;i++) {
			...
			for(var i=1;i<122;i++) {
				...
				switch(i) {
					...
					case Iap.EClaims:
						...
						var payorIDloc=field.LastIndexOf("Payor ID#:");
					...
					case Iap.Deductible:
						_insSub.BenefitNotes+="\r\n"+"Deductible: "+field;
						if(field.StartsWith("$")) {
							stringArraySplitField=field.Split(' ');
							benefit=new Benefit();
							...
							benefit.MonetaryAmt=SIn.Double(stringArraySplitField[0].Remove(0,1));//removes the $
					...
					case Iap.Preventive:
						...
						percent=SIn.Int(stringArraySplitField[0]);
	...
	private void EligibilityCheckCanada(Clearinghouse clearinghouseHq) {
		...
		var carrier=Carriers.GetCarrier(_insPlan.CarrierNum);
		if(!carrier.IsCDA) {
		...
		var relat=(Relat)comboRelationship.SelectedIndex;
	...
	private void butBenefitNotes_Click(object sender,System.EventArgs e) {
		...
		using var formInsBenefitNotes=new FormInsBenefitNotes();
		if(_insSub.BenefitNotes!="") {
	...
	private void butDelete_Click(object sender,System.EventArgs e) {
		...
		var warningMsg="This plan doesn't have a carrier attached and probably is being created by another user right now.  Click OK to delete plan anyway.";
		...
		if(textLinkedNum.Text!="0") {//Other subscribers are present.  
			if(_insSub==null) {//viewing from big list
				...
				var dateTimeSubChange=_insSub.SecDateTEdit;
				if(_patPlan!=null) {
				...
				SecurityLogs.MakeLogEntry(EnumPermType.InsPlanEdit,_patPlan?.PatNum??0,logText,_insPlan?.PlanNum??0,
		...
		var dateTimePrevious=_insPlan.SecDateTEdit;
		try {
	...
	private bool DropClickHelper() {
		...
		var warningMsg="This plan doesn't have a carrier attached and probably is being created by another user right now.  Click OK to drop plan anyway.";
		if(_carrier.CarrierNum==0 && !MsgBox.Show(this,MsgBoxButtons.YesNo,warningMsg)) {
			return false;
		}
		...
		var listClaims=Claims.Refresh(_patPlan.PatNum);
		...
		_hasDropped=true;
		var logText=Lan.g(this,"The insurance plan for the carrier")+" "+Carriers.GetCarrier(_insPlan.CarrierNum).CarrierName+" "+Lan.g(this,"was dropped.");
		SecurityLogs.MakeLogEntry(EnumPermType.InsPlanEdit,_patPlan?.PatNum??0,logText,_insPlan?.PlanNum??0,
			_insPlan.SecDateTEdit);
	...
	private void FillBenefits() {
		...
		var col=new GridColumn("Pat",28, HorizontalAlignment.Center);
		...
		for(var i=0;i<_listBenefits.Count;i++) {
			...
			else if(Benefits.IsFrequencyLimitation(_listBenefits[i])) {
			...
			if(_listBenefits[i].Quantity>0) {
				...
				{
					row.Cells.Add(_listBenefits[i].Quantity+" "+Lan.g(this,"times per year")+" ");
				}
				...
				{
					row.Cells.Add(_listBenefits[i].Quantity+" "+Lan.g(this,"times in the last 12 months")+" ");
				}
				else {
					row.Cells.Add(_listBenefits[i].Quantity+" "
	...
	private void gridBenefits_DoubleClick(object sender,EventArgs e) {
		if(IsNewPlan && _insPlan.PlanNum != _insPlanOld.PlanNum) {  //If adding a new plan and picked existing plan from list
			...
			ODMessageBox.Show(Lan.g(this,"You have picked an existing insurance plan and changes cannot be made to benefits until you have saved the plan for this new subscriber.")
		...
		using var formInsBenefits=new FormInsBenefits(_insPlan.PlanNum,patPlanNum);
		formInsBenefits.ListBenefits=_listBenefits.Select(x => x.Copy()).ToList();
		...
		_listBenefits=formInsBenefits.ListBenefits;
		FillBenefits();
		...
		if(_insPlan.EmployerNum==0) {//no employer was previously entered.
			if(textEmployer.Text=="") {
				...
				if(_patPlan!=null && _employerNameOrig=="" && _insPlan.PlanNum==_insPlanOriginal.PlanNum) {
					...
					var insSubDB=InsSubs.GetOne(patPlanDB.InsSubNum);
					var insPlanDB=InsPlans.GetPlan(insSubDB.PlanNum,null);
	...
	private void butGetElectronic_Click(object sender,EventArgs e) {
		...
		{
			var error="So far, eligibility checks only work with ClaimConnect, EDS, WebMD (Emdeon Dental), and CDAnet.";
		...
		try {
			var clearinghouseClin=Clearinghouses.OverrideFields(clearinghouseHq,Clinics.ClinicNum);
			string error;
			var etrans=x270Controller.RequestBenefits(clearinghouseClin,_insPlan,_patPlan.PatNum,_carrier,_insSub,out error);
			if(etrans != null) {
				//show the user a list of benefits to pick from for import--------------------------
				var isDependentRequest=(_patPlan.PatNum!=_insSub.Subscriber);
				var carrier=Carriers.GetCarrier(_insPlan.CarrierNum);
				using var formEtrans270Edit=new FormEtrans270Edit(_patPlan.PatPlanNum,_insPlan.PlanNum,_insSub.InsSubNum,isDependentRequest,_insSub.Subscriber,carrier.IsCoinsuranceInverted);
				...
				if(formEtrans270Edit.ShowDialog()==DialogResult.OK) {
					...
					#region Plan Notes
					var patName=Patients.GetNameLF(_patPlan.PatNum);
					   ...
					   && MsgBox.Show(this,MsgBoxButtons.YesNo,"The plan has ended.  Would you like to drop this plan?"))
					{
						   ...
						   && MsgBox.Show(this,MsgBoxButtons.YesNo,"Would you like to add a popup to collect new insurance information from patient?"))
						{
							...
							var frmPopupEdit=new FrmPopupEdit();
	...
	private void butHistoryElect_Click(object sender,EventArgs e) {
		//button not visible if SubCur is null
		using var formBenefitElectHistory=new FormBenefitElectHistory(_insPlan.PlanNum,_patPlan.PatPlanNum,_insSub.InsSubNum,_insSub.Subscriber,_insPlan.CarrierNum);
		formBenefitElectHistory.ListBenefits=_listBenefits;
		formBenefitElectHistory.ShowDialog();
		var dateLast270=Etranss.GetLastDate270(_insPlan.PlanNum);
	...
	private bool IsEmployerValid() {
		...
		var insSubDB=InsSubs.GetOne(patPlanDB.InsSubNum);
		var insPlanDB=InsPlans.GetPlan(insSubDB.PlanNum,null);
		var hasExistingEmployerChanged=(insPlanDB.CarrierNum!=0 && insPlanDB.EmployerNum!=_insPlan.EmployerNum && insPlanDB.PlanNum==_insPlan.PlanNum);//not new insplan and employer db not same as selection and insplan still used.
		if(_employerNameOrig=="") {//no employer was previously entered.
			if(textEmployer.Text=="") {
				//no change
			}
			else {
				if(!Security.IsAuthorized(EnumPermType.InsPlanEdit,true)) {//Employer was changed and they don't have perms to make new insplan (they picked plan from list).
					//Validate plan's employer in DB
					var employerDB=Employers.GetByName(textEmployer.Text);
					if(employerDB==null) {
			...
			if(textEmployer.Text=="") {
				if(!Security.IsAuthorized(EnumPermType.InsPlanEdit,true)) {//Employer is now empty.  Need to see if the insplan in DB also has empty employer, or if someone else put one on it.
			...
			//if text has changed
				...
				if(!Security.IsAuthorized(EnumPermType.InsPlanEdit,true)) {//Without permission, they must have picked from list.  Verify employer still exists.  If it does, verify the insplan still has same employer.
					var employerDB=Employers.GetByName(textEmployer.Text);
					if(employerDB==null) {
						MsgBox.Show(this,"The Employer for this insurance plan has been combined or deleted since the plan was loaded.  Please choose another insurance plan.");
						return false;
	...
	private bool IsCarrierValid() {
		var carrierForm=GetCarrierFromForm();
		var patPlanDB=PatPlans.GetByPatPlanNum(_patPlan.PatPlanNum);
		var insSubDB=InsSubs.GetOne(patPlanDB.InsSubNum);
		var insPlanDB=InsPlans.GetPlan(insSubDB.PlanNum,null);//Can have CarrierNum==0 if this is a new plan.
		var hasExistingCarrierChanged=(insPlanDB.CarrierNum!=_carrier.CarrierNum && insPlanDB.CarrierNum!=0 && insPlanDB.CarrierNum!=_carrierNumOrig);
	...
	private bool FillPlanCurFromForm() {
		...
		if(!textPerVisitPatAmount.IsValid()) {
			MsgBox.Show(this,"Please enter a valid amount for 'Patient Copay' in the 'Other Ins Info' tab.");
			return false;
		}
		if(!textPerVisitInsAmount.IsValid()) {
			MsgBox.Show(this,"Please enter a valid amount for 'Insurance' in the 'Other Ins Info' tab.");
			return false;
		}
		if(_insSub!=null) {
			//Subscriber: Only changed when user clicks change button.
			_insSub.SubscriberID=textSubscriberID.Text;
			_insSub.DateEffective=SIn.Date(textDateEffect.Text);
			_insSub.DateTerm=SIn.Date(textDateTerm.Text);
		...
		//carrier-----------------------------------------------------------------------------------------------------
		if(Security.IsAuthorized(EnumPermType.InsPlanEdit,true)) {//User has the ability to edit carrier information.  Check for matches, create new Carrier if applicable.
			var carrierForm=GetCarrierFromForm();
			var carrierOld=_carrier.Copy();
			if(_carrier.CarrierNum==_carrierNumOrig && Carriers.Compare(carrierForm,_carrier) && _insPlan.PlanNum==_insPlanOld.PlanNum) {
				...
				if(_patPlan!=null) {
					...
					var insPlanDB=InsPlans.GetPlan(_insPlan.PlanNum,null);
				...
				if(CultureInfo.CurrentCulture.Name.EndsWith("CA")) {//Canadian. en-CA or fr-CA
					...
					if(!carrierFound) {
						...
						using var formCarrierEdit=new FormCarrierEdit();
			...
			if(Carriers.GetCarrier(_carrier.CarrierNum).CarrierName=="" && _insPlan.PlanNum!=_insPlanOld.PlanNum) {//Carrier not found, it must have been deleted or combined
				...
				var planNumDb=_insPlanOld.PlanNum;
				if(_patPlan!=null) {
				...
				var insPlanDB=InsPlans.GetPlan(planNumDb,null);
				_carrier=Carriers.GetCarrier(insPlanDB.CarrierNum);
				_insPlanOriginal.CarrierNum=_carrier.CarrierNum;
				_insPlan.CarrierNum=_carrier.CarrierNum;
			}
		...
		if(comboCopay.SelectedIndex==0) {
			if(comboPlanType.SelectedIndex==(int)InsPlanTypeComboItem.PPOFixedBenefit) {
				MsgBox.Show(this,"Fixed Benefit insurance plans require a Fixed Benefit Amounts fee schedule to be selected.");
				return false;
			}
			...
			if(IsFixedBenefitMismatch(FeeScheds.GetFirstOrDefault(x => x.FeeSchedNum==comboCopay.GetSelectedKey<FeeSched>(x=>x.FeeSchedNum)))) {
				ODMessageBox.Show(Lans.g("PPO Fixed Benefits Fee Schedules can only be assigned to PPO Fixed Benefits plan types. "
				                         +"Please make a valid selection from ")+labelCopayFeeSched.Text);
				return false;	
			}
			if(ODMessageBox.Show(this,Lans.g("The selected ")+labelCopayFeeSched.Text
		...
		if(comboOutOfNetwork.SelectedIndex==0) {
			if(IsNewPlan
			   ...
			   && PrefC.GetEnum<AllowedFeeSchedsAutomate>(PrefName.AllowedFeeSchedsAutomate)==AllowedFeeSchedsAutomate.LegacyBlueBook) {
				...
				var feeSched=FeeScheds.GetByExactName(_carrier.CarrierName,FeeScheduleType.OutNetwork);
				if(feeSched==null) {
		...
		//Ortho----------------------------------------------------------------------------------------------
		var OrthoClaimTypeArray=(OrthoClaimType[])Enum.GetValues(typeof(OrthoAutoProcFrequency));
		...
		var orthoAutoProcFrequencyArray=(OrthoAutoProcFrequency[])Enum.GetValues(typeof(OrthoAutoProcFrequency));
		...
		_insPlan.OrthoAutoFeeBilled=SIn.Double(textOrthoAutoFee.Text);
		_insPlan.PerVisitPatAmount=SIn.Double(textPerVisitPatAmount.Text);
		_insPlan.PerVisitInsAmount=SIn.Double(textPerVisitInsAmount.Text);
		return true;
	}
	...
	private bool CheckForReceivedClaims() {
		...
		if(patNum==0) {//Editing insurance plans from Lists > Insurance Plans.
			...
			if(claimCount!=0) {
				if(ODMessageBox.Show(Lan.g(this,"There are")+" "+claimCount+" "+Lan.g(this,"received claims for this insurance plan that will have the carrier changed")+".  "+Lan.g(this,"You should NOT do this if the patient is changing insurance")+".  "+Lan.g(this,"Use the Drop button instead")+".  "+Lan.g(this,"Continue")+"?","",MessageBoxButtons.OKCancel)==DialogResult.Cancel) {
					return false; //abort
				}
			}
		}
	...
	private void butSubstCodes_Click(object sender,EventArgs e) {
		var insPlanCurCopy=_insPlan.Copy();
		insPlanCurCopy.CodeSubstNone=checkCodeSubst.Checked;
		using var formInsPlanSubstitution=new FormInsPlanSubstitution(insPlanCurCopy);
	...
	private void butAuditPat_Click(object sender,EventArgs e) {
		...
		using var formInsEditPatLog=new FormInsEditPatLog(_patPlan);
	...
	private void butAudit_Click(object sender,EventArgs e) {
		...
		GetEmployerNum();
		using var formInsEditLogs = new FormInsEditLogs(_insPlan,_listBenefits);
	...
	private void butPatOrtho_Click(object sender,EventArgs e) {
		...
		var defaultFee=SIn.Double(textOrthoAutoFee.Text);
		var carrierName=SIn.String(textCarrier.Text);
		var subID=SIn.String(textSubscriberID.Text);
		if(defaultFee==0) {
			defaultFee=_insPlan.OrthoAutoFeeBilled;
		}
		using var formOrthoPat = new FormOrthoPat(_patPlan,_insPlan,carrierName,subID,defaultFee);
	...
	private void butPickOrthoProc_Click(object sender,EventArgs e) {
		...
		if(formProcCodes.DialogResult == DialogResult.OK) {
			_procedureCodeOrthoAuto=ProcedureCodes.GetProcCode(formProcCodes.CodeNumSelected);
			textOrthoAutoProc.Text=_procedureCodeOrthoAuto.ProcCode;
		}
	}
	...
	private void butSave_Click(object sender,System.EventArgs e) {
		...
		if((radioChangeAll.Checked || (radioCreateNew.Checked && textLinkedNum.Text=="0")) //These are the two scenarios in which InsPlans.Update will be called instead of Insert.
		   && (_insPlan==null || InsPlans.GetPlan(_insPlan.PlanNum, [])==null)) 
		{
		...
		if(_insSub!=null && InsPlans.GetPlan(_insSub.PlanNum, [])==null) {
		...
		_insPlan.InsPlansZeroWriteOffsOnFreqOrAgingOverride=comboZeroWriteOffsOnLimitationOrAging.GetSelected<YN>();
		_insPlan.InsPlansZeroWriteOffsOnAnnualMaxOverride=comboBoxZeroWriteOffsOnAnnualMaxLimitation.GetSelected<YN>();
		...
		try {
			...
			if(_insPlan.CarrierNum!=_insPlanOriginal.CarrierNum) {
				var carrierNameOrig=Carriers.GetCarrier(_insPlanOriginal.CarrierNum).CarrierName;
				var carrierNameNew=Carriers.GetCarrier(_insPlan.CarrierNum).CarrierName;
			...
			ODMessageBox.Show(Lan.g(this,"Error Code 1")+".  "+Lan.g(this,"Please contact support")+"\r\n"+"\r\n"+ex.Message+"\r\n"+ex.StackTrace);
			return;
		}
		...
		try {
			...
			if(_insSubOld!=null) {//Editing an old plan for a subscriber.
				if(_insSubOld.AssignBen!=checkAssign.Checked) {
					...
					SecurityLogs.MakeLogEntry(EnumPermType.InsPlanChangeAssign,_insSub.Subscriber,Lan.g(this,"Assignment of Benefits (pay dentist) changed from")
			...
			ODMessageBox.Show(Lan.g(this,"Error Code 2")+".  "+Lan.g(this,"Please contact support")+"\r\n"+"\r\n"+ex.Message+"\r\n"+ex.StackTrace);
			return;
		}
		...
		try {
			...
			//PatPlan-------------------------------------------------------------------------------------------
			if(_patPlan!=null) {
				if(SIn.Long(textOrdinal.Text)!=_patPlan.Ordinal) {//Ordinal changed by user
					_patPlan.Ordinal=(byte)(PatPlans.SetOrdinal(_patPlan.PatPlanNum,SIn.Int(textOrdinal.Text)));
					_hasOrdinalChanged=true;
				}
				else if(SIn.Long(textOrdinal.Text)!=PatPlans.GetByPatPlanNum(_patPlan.PatPlanNum).Ordinal) {
				...
				if(!SIn.Date(textDateLastVerifiedPatPlan.Text).Date.Equals(_dateTimePatPlanLastVerified.Date)) {
					var insVerify=InsVerifies.GetOneByFKey(_patPlan.PatPlanNum,VerifyTypes.PatientEnrollment);
					if(insVerify!=null) {
						insVerify.DateLastVerified=SIn.Date(textDateLastVerifiedPatPlan.Text);
			...
			ODMessageBox.Show(Lan.g(this,"Error Code 3")+".  "+Lan.g(this,"Please contact support")+"\r\n"+"\r\n"+ex.Message+"\r\n"+ex.StackTrace);
			return;
		}
		...
		if(Security.IsAuthorized(EnumPermType.InsPlanEdit,true)) {
			if(_insSub==null) {//editing from big list.  No subscriber.  'pick from list' button not visible, making logic easier.
				...
				try {
					...
					ODMessageBox.Show(Lan.g(this,"Error Code 4")+".  "+Lan.g(this,"Please contact support")+"\r\n"+"\r\n"+ex.Message+"\r\n"+ex.StackTrace);
					return;
				}
				...
				if(IsNewPlan) {
					if(InsPlans.AreEqualValue(_insPlan,_insPlanOriginal)) {//New plan, no changes
						...
						try {
							if(_insPlan.PlanNum != _insPlanOld.PlanNum) {//clicked 'pick from list' button
								...
								try {
									...
									InsPlans.Delete(_insPlanOld,canDeleteInsSub:false,insertInsEditLogs:false);
									_listBenefitsOld= [];
									removeLogs=true;
								}
								catch(ApplicationException ex) {
									...
									SecurityLogs.MakeLogEntry(EnumPermType.InsPlanEdit,_patPlan?.PatNum??0
							...
							ODMessageBox.Show(Lan.g(this,"Error Code 5")+".  "+Lan.g(this,"Please contact support")+"\r\n"+"\r\n"+ex.Message+"\r\n"+ex.StackTrace);
							return;
						}
						...
						if(_insPlan.PlanNum != _insPlanOld.PlanNum) {//clicked 'pick from list' button, and then made changes
							...
							try {
								...
								ODMessageBox.Show(Lan.g(this,"Error Code 6")+".  "+Lan.g(this,"Please contact support")+"\r\n"+"\r\n"+ex.Message+"\r\n"+ex.StackTrace);
								return;
							}
						}
					...
					if(InsPlans.AreEqualValue(_insPlan,_insPlanOriginal)) {//If no changes
						...
						try {
							...
							ODMessageBox.Show(Lan.g(this,"Error Code 7")+".  "+Lan.g(this,"Please contact support")+"\r\n"+"\r\n"+ex.Message+"\r\n"+ex.StackTrace);
							return;
						}
						...
						if(_insPlan.PlanNum != _insPlanOld.PlanNum) {//clicked 'pick from list' button, and then made changes
							if(radioChangeAll.Checked) {
								...
								try {
									...
									ODMessageBox.Show(Lan.g(this,"Error Code 8")+".  "+Lan.g(this,"Please contact support")+"\r\n"+"\r\n"+ex.Message+"\r\n"+ex.StackTrace);
									return;
								}
								...
								try {
									...
									ODMessageBox.Show(Lan.g(this,"Error Code 9")+".  "+Lan.g(this,"Please contact support")+"\r\n"+"\r\n"+ex.Message+"\r\n"+ex.StackTrace);
									return;
								}
							...
							try {
								...
								ODMessageBox.Show(Lan.g(this,"Error Code 10")+".  "+Lan.g(this,"Please contact support")+"\r\n"+"\r\n"+ex.Message+"\r\n"+ex.StackTrace);
								return;
							}
			...
			if(_insSub!=null) {
				if(IsNewPlan) {
					...
					try {
						...
						ODMessageBox.Show(Lan.g(this,"Error Code 5a")+".  "+Lan.g(this,"Please contact support")+"\r\n"+"\r\n"+ex.Message+"\r\n"+ex.StackTrace);
						return;
					}
		...
		try {
			if(!SIn.Date(textDateLastVerifiedBenefits.Text).Date.Equals(_dateTimeInsPlanLastVerified.Date)) {
				var insVerify=InsVerifies.GetOneByFKey(_insPlan.PlanNum,VerifyTypes.InsuranceBenefit);
				insVerify.DateLastVerified=SIn.Date(textDateLastVerifiedBenefits.Text);
				InsVerifyHists.InsertFromInsVerify(insVerify);
			}
			...
			if(_insSub!=null) {//Update SubCur if needed
				...
				if(_insSubOld.Subscriber!=_insSub.Subscriber) {//If the subscriber was changed, include an audit trail entry
					var dictPatNames=Patients.GetPatientNames([_insSubOld.Subscriber, _insSub.Subscriber]);
					//PatPlanCur will be null if editing insurance plans from Lists > Insurance Plans.
					//However, the Change button is invisible from List > Insurance Plans, so we can count on PatPlanCur not null.
					SecurityLogs.MakeLogEntry(EnumPermType.InsPlanChangeSubsc,_patPlan.PatNum,
			...
			ODMessageBox.Show(Lan.g(this,"Error Code 11")+".  "+Lan.g(this,"Please contact support")+"\r\n"+"\r\n"+ex.Message+"\r\n"+ex.StackTrace);
			return;
		}
		...
		try {
			...
			var carrierOrig=Carriers.GetCarrier(_insPlanOriginal.CarrierNum);
			var carrierNew=Carriers.GetCarrier(_insPlan.CarrierNum);
			if(_insPlan.CarrierNum!=_insPlanOriginal.CarrierNum) {
				_hasCarrierChanged=true;
				//_patPlan will be null if editing insurance plans from Lists > Insurance Plans.				
				var carrierNameOrig=carrierOrig.CarrierName;
				var carrierNameNew=carrierNew.CarrierName;
				if(carrierNameOrig!=carrierNameNew) {//The CarrierNum could have changed but the CarrierName might not have changed.  Only make an audit entry if the name changed.
					SecurityLogs.MakeLogEntry(EnumPermType.InsPlanChangeCarrierName,_patPlan?.PatNum??0,Lan.g(this,"Carrier name changed in Edit Insurance Plan window from")+" "
				...
				var message=GetSecurityLogMessage(carrierNew,carrierOrig);
				SecurityLogs.MakeLogEntry(EnumPermType.CarrierEdit,0,message);
			}
		}
		catch(Exception ex) {
			ODMessageBox.Show(Lan.g(this,"Error Code 12")+".  "+Lan.g(this,"Please contact support")+"\r\n"+"\r\n"+ex.Message+"\r\n"+ex.StackTrace);
			return;
		}
		#endregion Carrier
		#region Carrier FeeSched
		try {
			var carrierCur=Carriers.GetCarrier(_insPlan.CarrierNum);
			if(_insPlanOriginal.FeeSched!=0 && _insPlanOriginal.FeeSched!=_insPlan.FeeSched) {
				var feeSchedOld=FeeScheds.GetDescription(_insPlanOriginal.FeeSched);
				var feeSchedNew=FeeScheds.GetDescription(_insPlan.FeeSched);
				var logText=Lan.g(this,"The fee schedule associated with insurance plan number")+" "+_insPlan.PlanNum+" "+Lan.g(this,"for the carrier")+" "+carrierCur.CarrierName+" "+Lan.g(this,"was changed from")+" "+feeSchedOld+" "+Lan.g(this,"to")+" "+feeSchedNew;
				SecurityLogs.MakeLogEntry(EnumPermType.InsPlanEdit,_patPlan?.PatNum??0,logText,_insPlan?.PlanNum??0,
					_insPlanOriginal.SecDateTEdit);
			}
			if(InsPlanCrud.UpdateComparison(_insPlanOriginal,_insPlan)) {
				var logText=Lan.g(this,"Insurance plan")+" "+_insPlan.PlanNum+" "+Lan.g(this,"for the carrier")+" "+carrierCur.CarrierName+" "+Lan.g(this,"has changed.");
				SecurityLogs.MakeLogEntry(EnumPermType.InsPlanEdit,_patPlan?.PatNum??0,logText,_insPlan?.PlanNum??0,
					_insPlanOriginal.SecDateTEdit);
			}
		}
		catch(Exception ex) {
			ODMessageBox.Show(Lan.g(this,"Error Code 13")+".  "+Lan.g(this,"Please contact support")+"\r\n"+"\r\n"+ex.Message+"\r\n"+ex.StackTrace);
			return;
		}
	...
	private bool DoShowBluebookDeletionMsgBox() {
		var isBluebookOn=PrefC.GetEnum<AllowedFeeSchedsAutomate>(PrefName.AllowedFeeSchedsAutomate)==AllowedFeeSchedsAutomate.BlueBook;
	...
	private void FormInsPlan_Closing(object sender, System.ComponentModel.CancelEventArgs e) {
		if(DialogResult==DialogResult.OK) {
			...
			if(IsNewPatPlan//Only when assigning new insurance
			   ...
			   && Security.IsAuthorized(EnumPermType.PatientBillingEdit,true)
			   && PrefC.GetBool(PrefName.PatInitBillingTypeFromPriInsPlan))
			{
				var patient=Patients.GetPat(_patPlan.PatNum);
				if(patient.BillingType!=_insPlan.BillingType) {
					var patNew=patient.Copy();
					...
					AutomationL.Trigger(EnumAutomationTrigger.BillingTypeSet,null,patNew.PatNum);
					Patients.InsertBillTypeChangeSecurityLogEntry(patient,patNew);
				}
			}
			if(_insSub!=null && _hasDeleted) {
				var listPatPlansForSub=PatPlans.GetListByInsSubNums([_insSub.InsSubNum]);
				for(var i=0;i<listPatPlansForSub.Count;i++) {
		...
		if(IsNewPlan) {//this would also be new coverage
			...
			try {
				...
				InsPlans.Delete(_insPlanOld,canDeleteInsSub:false,insertInsEditLogs:false);//does dependency checking.
				InsEditLogs.DeletePreInsertedLogsForPlanNum(_insPlanOld.PlanNum);
	...
	private bool IsPatPlanRemoved() {
		if(_patPlan!=null) {
			var patPlanExists=PatPlans.GetByPatPlanNum(_patPlan.PatPlanNum);
