...
public partial class FormClaimAttachment:FormODBase {
	...
	public FormClaimAttachment(Claim claim,EclaimsCommBridge eClaimsCommBridge) {
		InitializeComponent();
		_claim=claim;
		_listImageAttachmentsDXC= [];
		_listImageAttachmentsEDS= [];
	...
	private void FormClaimAttachment_Load(object sender,EventArgs e) {
		...
		var listDefsClaimAttachments=CheckImageCatDefs();
	...
	private void FillGrid() {
		...
		var row=new GridRow();
		for(var i=0;i<countAttachedImages;i++) {
			if(_eclaimsCommBridge==EclaimsCommBridge.ClaimConnect) {//Will never use ClaimConnect
	...
	private bool ValidateClaimDXC() {
		...
		for(var i=0;i<validateClaimResponse.ValidationErrors.Length;i++) {
	...
	private bool ValidateAttachmentsNeededEDS() {
		...
		var stringBuilder=new StringBuilder();
		for(var i=0;i<listPayerResponses.Payers.Count;i++) {
			var listProcedureResponses=listPayerResponses.Payers[i].ProcedureResponses.FindAll(x => x.Response);
			for(var j=0;j<listProcedureResponses.Count;j++) {
	...
	private void timerMonitorClipboard_Tick(object sender,EventArgs e) {
		...
		var listProcesses=GetProcessesSnipTool();
		...
		using var bitmapClipboard=GetImageFromClipboard(isSilent:true,doShowProgressBar:false);
	...
	private bool KillProcesses(List<Process> listProcessesToKills) {
		var countProcesseskilled=0;
		for(var i=listProcessesToKills.Count-1;i>=0;i--) {//backward
			try {
	...
	private List<Process> GetProcessesSnipTool() {
		var snippingToolProcess="SnippingTool";
		var snipAndSketchProcess="ScreenSketch";
		var processesArrayRunnings=Process.GetProcesses();
		var listProcesses=new List<Process>();
		for(var i=0;i<processesArrayRunnings.Length;i++) {
	...
	public static bool DoesSnipAndSketchExist() {
		try {
			using var registryKeyScreenSketchProtocol=Microsoft.Win32.Registry.ClassesRoot.OpenSubKey(_snipSketchURI);
	...
	public static bool DoesSnippingToolExist() {
		...
		var snippingToolPath32=$"{windowsFolder}\\sysnative\\SnippingTool.exe";
		var snippingToolPath64=$"{windowsFolder}\\system32\\SnippingTool.exe";
	...
	public static bool StartSnipAndSketchOrSnippingTool() {
		...
		if(DoesSnipAndSketchExist()) {
			var processSnipAndSketch=new Process();
		...
		if(DoesSnippingToolExist()) {
			var processSnippingTool=new Process();
	...
	private void CreateImageAttachment(Bitmap bitmap,bool isSnip=false) {
		...
		WindowState=FormWindowState.Normal;
		BringToFront();
		using var formClaimAttachmentItemEdit=new FormClaimAttachmentItemEdit(bitmap,_eclaimsCommBridge);
	...
	private void ButPasteImage_Click(object sender,EventArgs e) {
		using var bitmapPasted=GetImageFromClipboard();
	...
	private Bitmap GetImageFromClipboard(bool isSilent=false, bool doShowProgressBar=true) {
		var bitmapClipboard=ODClipboard.GetImage();
		...
		var fileName=stringArrayFiles.First();
	...
	private void gridAttachedImages_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		if(_eclaimsCommBridge==EclaimsCommBridge.ClaimConnect) {//Will never use ClaimConnect
			var imageAttachmentSelected=gridAttachedImages.SelectedTag<ClaimConnect.ImageAttachment>();
			using var image=GetSelectedImage();
			using var formClaimAttachmentItemEdit=new FormClaimAttachmentItemEdit(
				image,
			...
			var imageAttachmentSelected=gridAttachedImages.SelectedTag<EDS.ImageAttachment>();
			using var image=GetSelectedImage();
			using var formClaimAttachmentItemEdit=new FormClaimAttachmentItemEdit(
				image,
	...
	private void ContextMenu_ItemClicked(object sender,ToolStripItemClickedEventArgs e) {
		var toolStripItem=e.ClickedItem;
		if(toolStripItem.Text=="Delete") {
			//Delete every selected row
			for(var i=gridAttachedImages.SelectedIndices.Length-1;i>=0;i--) {//because we are removing items
				if(_eclaimsCommBridge==EclaimsCommBridge.ClaimConnect) {//Will never use ClaimConnect
	...
	private void textNarrative_TextChanged(object sender,EventArgs e) {
		...
		if(_eclaimsCommBridge==EclaimsCommBridge.EDS) {
			var imageAttachment=gridAttachedImages.SelectedTag<EDS.ImageAttachment>();
	...
	private void CreateAndSendAttachments() {
		if(_eclaimsCommBridge==EclaimsCommBridge.ClaimConnect) {//Will never use ClaimConnect
			...
			var listImageAttachmentsToSends=new List<ClaimConnect.ImageAttachment>();
			for(var i=0;i<gridAttachedImages.ListGridRows.Count;i++) {
			...
			var listImageAttachments=gridAttachedImages.GetTags<EDS.ImageAttachment>();
	...
	private void BatchSendAttachments() {
		...
		for(var i=0;i<gridAttachedImages.ListGridRows.Count;i++) {
			var listImageAttachments=new List<ClaimConnect.ImageAttachment>();
	...
	private void AddAttachments(List<ClaimConnect.ImageAttachment> listImageAttachments) {
		if(string.IsNullOrWhiteSpace(_claim.AttachmentID)) {
			...
			var attachmentId=ClaimConnect.OpenAttachment(_claim,textNarrative.Text);
	...
	private void ClearAttachmentID() {
		...
		var oldAttachmentID=_claim.AttachmentID;
		_claim.AttachmentID="";
		_claim.Narrative="";//Clear out narrative when changing attachments
		textNarrative.Text="";
		var claimSecDateTEdit=_claim.SecDateTEdit;//Preserve the date prior to any claim updates effecting it.
		Claims.Update(_claim);
	...
	private void buttonOK_Click(object sender,EventArgs e) {
		...
		if(_eclaimsCommBridge==EclaimsCommBridge.EDS) {
			...
			var listImageAttachments=gridAttachedImages.GetTags<EDS.ImageAttachment>().FindAll(x => x.DocumentTypeCode==EDS.EnumDocumentTypeCode.Narrative && x.Narrative.IsNullOrEmpty());
			if(!listImageAttachments.IsNullOrEmpty()) {
				var message=Lans.g("FormClaimAttachment","The following attachments require Narratives")+"\r\n"+
		...
		var defClaimAttachCat=CheckImageCatDefs().FirstOrDefault();
		...
		var listClaimAttachments=new List<ClaimAttach>();
		for(var i=0;i<gridAttachedImages.ListGridRows.Count;i++) {
			if(_eclaimsCommBridge==EclaimsCommBridge.ClaimConnect) {//Will never use ClaimConnect
				var imageAttachmentRow=((ClaimConnect.ImageAttachment)gridAttachedImages.ListGridRows[i].Tag);
				if(PrefC.GetBool(PrefName.SaveDXCAttachments)) {
					var documentCur=ImageStore.Import(imageAttachmentRow.ImageFileAsBase64,imageTypeDefNum,ImageType.Attachment,_patient);
					imageAttachmentRow.ImageFileNameActual=documentCur.FileName;
				}
				//Create attachment objects
				var claimAttach=CreateClaimAttachment(imageAttachmentRow.ImageFileNameDisplay,imageAttachmentRow.ImageFileNameActual);
				claimAttach.ImageReferenceId=_listImageReferenceIds[i];
				listClaimAttachments.Add(claimAttach);
			}
			else if(_eclaimsCommBridge==EclaimsCommBridge.EDS) {
				var imageAttachmentRow=((EDS.ImageAttachment)gridAttachedImages.ListGridRows[i].Tag);
				if(PrefC.GetBool(PrefName.SaveEDSAttachments)) {
					var documentCur=ImageStore.Import(imageAttachmentRow.FileData,imageTypeDefNum,ImageType.Attachment,_patient);
					imageAttachmentRow.FileNameActual=documentCur.FileName;
				}
				//Create attachment objects
				var claimAttach=CreateClaimAttachment(imageAttachmentRow.FileDisplayName,imageAttachmentRow.FileNameActual);
				listClaimAttachments.Add(claimAttach);
			}
		}
