...
public partial class FormLabCaseSelect : FormODBase {
	...
	private void FillGrid(){
		...
		var col=new GridColumn(Lan.g("TableLabCaseSelect","Date Created"),80);
	...
	private void butAdd_Click(object sender,EventArgs e) {
		if(IsSelectingUnattached) {
			...
			ListLabCaseNumsSelected= [formLabCaseEdit.LabCaseCur.LabCaseNum];
	...
	private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		var labCaseSelected=gridMain.SelectedTag<LabCase>();
		if(IsSelectingUnattached) {
			ListLabCaseNumsSelected= [labCaseSelected.LabCaseNum];
			DialogResult=DialogResult.OK;
		}
		else {
			using var formLabCaseEdit=new FormLabCaseEdit();
