...
public partial class FormTimeCardSetup:FormODBase {
	...
	private void FillGrid() {
		...
		for (var i=0;i<_listPayPeriods.Count;i++) {
			var payPeriod = _listPayPeriods[i];
	...
	private void FillRules() {
		...
		var listTimeCardRules=TimeCardRules.GetDeepCopy().OrderBy(x => x.IsOvertimeExempt)
		...
		var col=new GridColumn("Employee",150,GridSortingStrategy.StringCompare);
		...
		UI.GridRow row;
		for(var i=0;i<listTimeCardRules.Count;i++) {
			row=new GridRow();
			if(listTimeCardRules[i].EmployeeNum==0) {
				row.Cells.Add(Lan.g(this,"All Employees"));
			}
			else {
				var emp=Employees.GetEmp(listTimeCardRules[i].EmployeeNum);
	...
	private bool IsSafeToDelete(List<PayPeriod> listPayPeriodsSelected) {
		...
		var listClockEventsAll = ClockEvents.GetAllForPeriod(listPayPeriodsSelected.Min(x => x.DateStart), listPayPeriodsSelected.Max(x => x.DateStop));
		for(var i=0;i<listPayPeriodsSelected.Count;i++) {
			var listClockEventsForPeriod = listClockEventsAll.FindAll(x => x.TimeDisplayed1 >= listPayPeriodsSelected[i].DateStart 
			...
			for(var j=0;j<listClockEventsForPeriod.Count;j++) {
	...
	private void gridMain_CellDoubleClick(object sender,OpenDental.UI.ODGridClickEventArgs e) {
		using var formPayPeriodEdit=new FormPayPeriodEdit((PayPeriod)gridMain.ListGridRows[e.Row].Tag);
	...
	private void butAddRule_Click(object sender,EventArgs e) {
		using var formTimeCardRuleEdit=new FormTimeCardRuleEdit();
	...
	private void butDeleteRules_Click(object sender,EventArgs e) {
		...
		var listTimeCardRuleNums=gridRules.SelectedTags<TimeCardRule>().Select(x => x.TimeCardRuleNum).ToList();
	...
	private void gridRules_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		using var formTimeCardRuleEdit=new FormTimeCardRuleEdit((TimeCardRule)gridRules.ListGridRows[e.Row].Tag);
		formTimeCardRuleEdit.ShowDialog();
		FillRules();
		_hasChanged=true;
	}
	...
	private void butDelete_Click(object sender,EventArgs e) {
		...
		var listPayPeriodsSelected = new List<PayPeriod>();
