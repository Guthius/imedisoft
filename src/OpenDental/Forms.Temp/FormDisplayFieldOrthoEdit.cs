...
public partial class FormDisplayFieldOrthoEdit : FormODBase {
	...
	private void FormDisplayFieldOrthoEdit_Load(object sender,EventArgs e) {
		...
		var widthCol=_displayField.ColumnWidth;
	...
	private void butUp_Click(object sender,EventArgs e) {
		...
		var selectionStart=textPickList.SelectionStart;
		//calculate which row to highlight, based on selection start.
		var selectedRow=0;
		var sumPreviousLines=0;
		var stringArrayLinesOriginal=new string[textPickList.Lines.Length];
		textPickList.Lines.CopyTo(stringArrayLinesOriginal,0);
		for(var i=0;i<textPickList.Lines.Length;i++) {
			if(i>0) {
	...
	private void butSave_Click(object sender, System.EventArgs e) {
		...
		var displayFieldOther=_listDisplayFields.FirstOrDefault(x => x!=_displayField && x.Description==textDescription.Text);
		...
		var widthCol=SIn.Int(textWidth.Text);
