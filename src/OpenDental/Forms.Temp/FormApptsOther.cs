...
public partial class FormApptsOther:FormODBase {
	...
	public List<long> ListAptViewJumpTos= [];
	...
	private List<Def> _listDefsProgNoteColors= [];
	private List<Appointment> _listPlannedAppts= [];
	private List<Appointment> _listPlannedApptsIncomplete= [];
	...
	public void MakeRecallAppointment() {
		...
		var dateTimeApt=DateTime.MinValue;
		...
		if(IsInitialDoubleClick) {
			...
			if(HL7Defs.IsExistingHL7Enabled()) {
				...
				var messageHL7=MessageConstructor.GenerateSIU(_patient,_family.GetPatient(_patient.Guarantor),EventTypeHL7.S12,appointment);
		...
		var recall=Recalls.GetRecallProphyOrPerio(_patient.PatNum);//shouldn't return null.
	...
	public void MakeRecallFamily() {
		...
		Appointment appointment=null;
		Recall recall;
		var countAlreadySched=0;
		var countNoRecalls=0;
		var countPatsRestricted=0;
		var countPatsArchivedOrDeceased=0;
		for(var i=0;i<_family.ListPats.Length;i++) {
			var patientCur=_family.ListPats[i];
			...
			var listRecalls=listRecallsPat.FindAll(x => x.RecallTypeNum==RecallTypes.PerioType || x.RecallTypeNum==RecallTypes.ProphyType);
	...
	public void MakeAppointment() {
		...
		var listAppointmentsUnsched=Appointments.GetUnschedApptsForPat(_patient.PatNum);
		...
		long aptNum=0;
		var isSchedulingUnscheduled=false;
		   ...
		   MsgBox.Show(this,MsgBoxButtons.YesNo,"This patient has an unscheduled appointment, would you like to use an existing unscheduled appointment?"))
		{
			if(listAppointmentsUnsched.Count==1) {
				...
				using var formUnschedListPatient=new FormUnschedListPatient(_patient);
		...
		using var formApptEdit=new FormApptEdit(aptNum,patNum:_patient.PatNum,useApptDrawingSettings:IsInitialDoubleClick,patient:_patient,dateTNew:DateTNew,opNumNew:OpNumNew);
		...
		var appointment=formApptEdit.GetAppointmentCur();
		if(IsInitialDoubleClick) {
			...
			if(operatory!=null) {
				if(operatory.SetProspective && _patient.PatStatus!=PatientStatus.Prospective) { //Don't need to prompt if patient is already prospective.
					if(MsgBox.Show(this,MsgBoxButtons.OKCancel,"Patient's status will be set to Prospective.")) {
						...
						var logEntry=Lan.g(this,"Patient's status changed from ")+patOld.PatStatus.GetDescription()+Lan.g(this," to ")
	...
	private void butPin_Click(object sender, System.EventArgs e) {
		...
		var apptOther=ListApptOthers.FirstOrDefault(x => x.AptNum==gridMain.SelectedTag<long>());
	...
	private void listFamily_Click(object sender,EventArgs e) {
		...
		var patNumOld=_patient.PatNum;
		var patNumNew=((Patient)listViewFamily.SelectedItems[0].Tag).PatNum;
	...
	private void butGoTo_Click(object sender, System.EventArgs e) {
		...
		var apptOtherSelected=ListApptOthers.FirstOrDefault(x => x.AptNum==gridMain.SelectedTag<long>());
	...
	private void FillGrid() {
		gridMain.SetScaleAndZoom(1,1);
		long selectedApptOtherNum=gridMain.GetSelectedIndex();
		var selectedIndex=-1;
		gridMain.BeginUpdate();
		gridMain.Columns.Clear();
		var col=new GridColumn(Lan.g("FormApptsOther","Appt Status"),100);
		...
		for(var i=0;i<ListApptOthers.Count;i++) {
			...
			if(ListApptOthers[i].AptDateTime.Year > 1880) {
				                                                   ...
				                                                   && ListApptOthers[i].AptStatus!=ApptStatus.Broken) 
				{
					...
					var strText=Lan.g("enumApptStatus","Planned")+" ";
					var idxPlannedApt=_listPlannedApptsIncomplete.FindIndex(x => x.AptNum==ListApptOthers[i].AptNum);
					var listProceduresPlannedAppt=listProceduresForPlannedAppts.FindAll(x => x.PlannedAptNum==ListApptOthers[i].AptNum);
					var hasCompletedProcedures=listProceduresPlannedAppt.Exists(x=>x.ProcStatus==ProcStat.C);
					var hasNonCompletedProcedures=listProceduresPlannedAppt.Exists(x=>x.ProcStatus!=ProcStat.C);
					if(IsShowCompletePlanned) {
						for(var p=0;p<_listPlannedAppts.Count;p++) {
							if(_listPlannedAppts[p].AptNum==ListApptOthers[i].AptNum) {
								strText+="#"+_listPlannedAppts[p].ItemOrderPlanned;
							}
						}
					}
					else {
	...
	public void RefreshData() {
		...
		//If is for task and pref is set, clear out appointments that are already attached to a task.
		if(IsForTask && !PrefC.GetBool(PrefName.TasksForApptAllowMultiple)) {
			var listAttached=Appointments.GetApptNumsAttachedToTask(ListApptOthers.Select(x=>x.AptNum).ToList());
			ListApptOthers.RemoveAll(x=>x.AptNum.In(listAttached.ToArray()));
		}
		_listPlannedAppts=Appointments.GetRefreshedPlannedAppts(_patient.PatNum);
	...
	private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		...
		var currentScroll=gridMain.ScrollValue;
		var aptNum=gridMain.SelectedTag<long>();//Tag is AptNum
		if(IsSelectedApptOtherNull()) {
			return;
		}
		...
		if(formApptEdit.PinClicked) {
			...
			var apptOther=ListApptOthers.FirstOrDefault(x => x.AptNum==(long)gridMain.ListGridRows[e.Row].Tag);
