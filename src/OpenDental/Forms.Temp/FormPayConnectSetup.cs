...
public partial class FormPayConnectSetup : FormODBase {
	...
	private List<ProgramProperty> _listProgramPropertiesXWebWebPay= [];
	...
	private void FillFields() {
		...
		var payTypeDefNum=ProgramProperties.GetPropValFromList(_listProgramProperties,"PaymentType",clinicNum);
		var processingMethod=ProgramProperties.GetPropValFromList(_listProgramProperties,PayConnect.ProgramProperties.DefaultProcessingMethod,clinicNum);
		...
		var programVersion=SIn.Int(ProgramProperties.GetPropValFromList(_listProgramProperties,"Program Version",clinicNum));
		...
		var accountType=SIn.Int(ProgramProperties.GetPropValFromList(_listProgramProperties,"PayConnect2.0 Integration Type: 0 for normal, 1 for surcharge",clinicNum));
		if(accountType==1) {
	...
	private void FillGridTerminals() {
		...
		gridODTerminals.Columns.Add(col);
		GridRow row;
		for(var i=0;i<_listPayTerminals.Count;i++) {
	...
	private void SynchWithHQ() {
		if(!true || _listUserClinicNums[_indexClinicRevert]>0) {//using clinics, and modifying the HQ clinic. otherwise return.
			return;
		}
		var hqUsername=ProgramProperties.GetPropValFromList(_listProgramProperties,"Username",0);//HQ Username before updating to value in textbox
		var hqPassword=ProgramProperties.GetPropValFromList(_listProgramProperties,"Password",0);//HQ Password before updating to value in textbox
		var hqPayType=ProgramProperties.GetPropValFromList(_listProgramProperties,"PaymentType",0);//HQ PaymentType before updating to combo box selection
		...
		var listClinicNums=_listProgramProperties.Select(x => x.ClinicNum).Where(x => x>0).Distinct().ToList();
		...
		for(var i=0;i<listClinicNums.Count;i++) {
			...
			var password=CDT.Class1.TryEncrypt(textPassword.Text);//Encrypt password before setting so it's not plaintext.
	...
	private void UpdateListProgramPropertiesForClinic(long clinicNum) {
		...
		var processingMethodSelected=comboDefaultProcessing.SelectedIndex.ToString();
	...
	private void checkPatientPortalPayEnabled_Click(object sender,EventArgs e) {
		...
		var programPropertyWebPayEnabled=ProgramProperties.GetOnlinePaymentsEnabledForClinic(clinicNum,ProgramName.PayConnect);
		var msg=Lan.g(this,"Online payments are already enabled for another processor and must be disabled in order to use PayConnect online payments. "
	...
	private void linkLabel1_LinkClicked(object sender,LinkLabelLinkClickedEventArgs e) {
		var url="https://www.payconnect.com/";
		Process.Start(url);
	}
	...
	private void gridODTerminals_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		var payTerminal=(PayTerminal)gridODTerminals.ListGridRows[e.Row].Tag;
		using var formPayTerminalEdit=new FormPayTerminalEdit(payTerminal);
		formPayTerminalEdit.ShowDialog();
		FillGridTerminals();
	}
		
	private void butAddTerminal_Click(object sender,EventArgs e) {
		using var formPayTerminalEdit=new FormPayTerminalEdit();
		formPayTerminalEdit.ShowDialog();
		FillGridTerminals();
	}
	...
	private void butDownloadDriver_Click(object sender,EventArgs e) {
		...
		var zipFileName=ODFileUtils.CombinePaths(PrefC.GetTempFolderPath(),"VeriFoneUSBUARTDriver_Vx_1.0.0.52_B5.zip");
		try {
		...
		ODMessageBox.Show(Lans.g("Download complete. Run the Setup.exe file in")+" "+PrefC.GetTempFolderPath()+" "
		                  +Lans.g("if it does not start automatically."));
	}
	...
	private void butMerchantInfo_Click(object sender,EventArgs e) {
		...
		var data=Lan.g(this,"Merchant ID")+$": {payConnect2Response.GetMerchantInfoResponse.MerchantId}\n"+
		         ...
		         Lan.g(this,"Reporting Emails")+$": {string.Join(", ",payConnect2Response.GetMerchantInfoResponse.ReportingEmails??[])}\n"+
	...
	private void butSave_Click(object sender, System.EventArgs e) {
		...
		if(checkEnabled.Checked && !Programs.IsEnabledByHq(ProgramName.PayConnect,out var err)) {
			MsgBox.Show(err);
			return;
		}
		...
		if(checkEnabled.Checked //if PayConnect is enabled
		   ...
		   && (!true  //and either clinics are not enabled (meaning this is the only set of username, password, payment type values)
		...
		if(radioVersion1.Checked) {//version 1 specific validation
			if(checkEnabled.Checked && !checkTerminal.Checked && !true && 
		...
		for(var i=0;i<_listUserClinicNums.Count;i++) {
			...
			var password=CDT.Class1.TryDecrypt(ProgramProperties.GetPropValFromList(_listProgramProperties,"Password",_listUserClinicNums[i]));
		...
		var strPayConnectOnlinePaymentsEnabled=PayConnect.ProgramProperties.PatientPortalPaymentsEnabled;
		var listProgramPropertiesPayConnectOnlinePayments=_listProgramProperties.FindAll(x => x.PropertyDesc==strPayConnectOnlinePaymentsEnabled &&
		                                                                                      SIn.Bool(x.PropertyValue));
		for(var i=0;i < listProgramPropertiesPayConnectOnlinePayments.Count;i++) {
			...
			var programPropertyWebOnlinePayments=_listProgramPropertiesXWebWebPay.FirstOrDefault(y => y.ClinicNum==listProgramPropertiesPayConnectOnlinePayments[i].ClinicNum);
		...
		for(var i=0;i<_listProgramProperties.Count;i++) {
			if(_listProgramProperties[i].IsHighSecurity && _listProgramProperties[i].PropertyValue!=_listProgramProperties[i].TagOD.ToString()) {
				var stringBuilderLogText=new StringBuilder();
				stringBuilderLogText.Append(_program.ProgDesc+"'s "+_listProgramProperties[i].PropertyDesc);
				if(true) {
					var clinicNumCur=_listProgramProperties[i].ClinicNum;
					if(clinicNumCur==0) {
