...
public partial class FormDiscountPlans:FormODBase {
	...
	private void FillGrid() {
		var listDiscountPlans=DiscountPlans.GetAll(checkShowHidden.Checked);
		var listDiscountPlanNums=listDiscountPlans.Select(x => x.DiscountPlanNum).ToList();
		var listCountPerPlan= DiscountPlans.GetPatCountsForPlans(listDiscountPlanNums);
		...
		GridRow row;
		var selectedIdx=-1;
		for(var i=0;i<listDiscountPlans.Count;i++) {
			var def=Defs.GetDef(DefCat.AdjTypes,listDiscountPlans[i].DefNum);
			row=new GridRow();
			...
			var countPerPlan=listCountPerPlan.FirstOrDefault(x => x.DiscountPlanNum==listDiscountPlans[i].DiscountPlanNum);
