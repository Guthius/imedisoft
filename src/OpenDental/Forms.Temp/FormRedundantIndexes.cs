...
public partial class FormRedundantIndexes:FormODBase {
	...
	private void FillGrid() {
		var table=DatabaseMaintenances.GetRedundantIndexesTable();
		if(table.Rows.Count==0) {
			return;
		}
		gridMain.BeginUpdate();
		gridMain.Columns.Clear();
		gridMain.Columns.Add(new GridColumn(Lan.g(this,"Table"),165));
		gridMain.Columns.Add(new GridColumn(Lan.g(this,"Index"),165));
		gridMain.Columns.Add(new GridColumn(Lan.g(this,"Index Columns"),200));
	...
	private void checkLogAddStatements_CheckedChanged(object sender,EventArgs e) {
		...
		var msgText="It is recommended that you log the statements to add the indexes back in case you wish to undo the actions performed by this "
	...
	private void butDrop_Click(object sender,EventArgs e) {
		...
		if(checkLogAddStatements.Checked) {
			...
			using var msgBoxCopyPaste=new MsgBoxCopyPaste(Lan.g(this,"SQL statements to add the selected index(es) back will be in the following location")+":\r\n"+path);
		...
		if(checkLogAddStatements.Checked) {
			try {
				...
				using var msgBoxCopyPaste=new MsgBoxCopyPaste(Lan.g(this,"Could not create or modify the log file. Copy and paste the following queries into a text file and "
	...
	private void SaveLogToFile(string logText) {
		...
		try {
			...
			File.AppendAllText(GetLogFilePath(),DateTime.Now+" - Computer Name: "+machineName+new string('-',45)+Environment.NewLine+logText);
		}
	...
	private string GetLogFilePath() {
		var path=ODFileUtils.CombinePaths(ImageStore.GetDataFolder(),"DropIndexLogs");
