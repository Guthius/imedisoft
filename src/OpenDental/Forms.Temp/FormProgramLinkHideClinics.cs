...
public partial class FormProgramLinkHideClinics:FormODBase {
	...
	public FormProgramLinkHideClinics(Program program,List<ProgramProperty> listProgramProperties,List<ClinicDto> listClinicsUser) {
		...
		var listClinicsVisible=listClinicsUser.Select(x => x).ToList();//Copy so we don't affect the source list.
		var listClinicsHidden=new List<ClinicDto>();
		if(!listProgramProperties.IsNullOrEmpty()) {
			...
			for(var i=listClinicsVisible.Count-1;i>=0;i--) {//Count backwards to allow for clean removal from lists.
	...
	private void SyncHiddenProgramProperties() {
		...
		var listProgramPropertiesHidden=ProgramProperties.GetForProgram(_programNum)
		...
		var listProgramPropertiesHiddenClinics= new List<ProgramProperty>();
		var listClinics=listboxHiddenClinics.Items.GetAll<ClinicDto>();
		...
		for(var i=0;i<listProgramPropertiesHidden.Count;i++) {
		...
		for(var i=0;i<listProgramPropertiesHiddenClinics.Count;i++) {
	...
	private void butRight_Click(object sender,EventArgs e) {
		var listClinicNumsMoved=MoveClinics(listBoxFrom:listboxHiddenClinics,listBoxTo:listboxVisibleClinics);
	...
	private void butLeft_Click(object sender,EventArgs e) {
		var listClinicNumsMoved=MoveClinics(listBoxFrom:listboxVisibleClinics,listBoxTo:listboxHiddenClinics);
	...
	private List<long> MoveClinics(UI.ListBox listBoxFrom,UI.ListBox listBoxTo) {
		if(listBoxFrom.SelectedIndices.Count==0) {
			return [];//If nothing is selected, change nothing so the UI doesn't refresh.
		}
		var listClinicNumsToMove=listBoxFrom.GetListSelected<ClinicDto>().Select(x => x.Id).ToList();
		var listClinicNumsFrom=listBoxFrom.Items.GetAll<ClinicDto>().Select(x => x.Id).ToList();
	...
	private void CheckOrderAlphabetical_CheckedChanged(object sender,EventArgs e) {
		...
		for(var i=0; i<listClinicsVisible.Count;i++) {
		...
		for(var i=0; i<listClinicsHidden.Count;i++) {
