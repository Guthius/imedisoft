...
public partial class FormSheetFieldInput:FormODBase {
	...
	private void FormSheetFieldInput_Load(object sender,EventArgs e) {
		...
		for(var i=0;i<_listSheetFieldDefsAvail.Count;i++){
	...
	private void butAutoNote_Click(object sender,EventArgs e) {
		...
		AutoNoteNum=formAutoNotes.SelectedAutoNote.AutoNoteNum;
		textAutoNote.Text=formAutoNotes.SelectedAutoNote.AutoNoteName;
	...
	private void listFields_SelectionChangeCommitted(object sender,EventArgs e) {
		...
		var fieldName=_listSheetFieldDefsAvail[listFields.SelectedIndex].FieldName;
		if(fieldName=="misc") {
			labelReportableName.Visible=true;
			textReportableName.Visible=true;
		...
		if(fieldName.StartsWith("inputMed")) {
			...
			if(int.TryParse(fieldName.Replace("inputMed",""),out inputMedNum)) {
				textUiLabelMobile.Text="Input Medication "+inputMedNum;
			}
		}
	...
	private void SaveAndClose(){
		...
		if(listFields.SelectedIndex==-1) {
			...
			SheetFieldDefCur.FieldValue="AutoNoteNum:"+AutoNoteNum;
		}
		else{
