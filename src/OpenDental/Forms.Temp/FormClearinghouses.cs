...
public partial class FormClearinghouses:FormODBase {
	...
	private void FormClearinghouses_Load(object sender, System.EventArgs e) {
		...
		var claimReportReceiveInterval=PrefC.GetInt(PrefName.ClaimReportReceiveInterval);
	...
	private void FillGrid(){
		...
		for(var i=0;i<_listClearinghousesClinicsAll.Count;i++) {
		...
		var col=new GridColumn(Lan.g(this,"Description"),150);
		...
		GridRow row;
		for(var i=0;i<_listClearinghousesHq.Count;i++) {
			var clearinghouseArrayTags=new Clearinghouse[3];//[0]=clearinghouseHq, [1]=clearinghouseClinic, [2]=clearinghouseCur per ODGridRow
			...
			for(var j=0;j<_listClearinghousesClinics.Count;j++) {
			...
			var clearinghouse=clearinghouseArrayTags[2];
			row=new GridRow();
	...
	private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		var clearinghouseHq=((Clearinghouse[])(gridMain.ListGridRows[e.Row].Tag))[0].Copy();//cannot be null
		using var formClearinghouseEdit=new FormClearinghouseEdit();
		...
		formClearinghouseEdit.ListClearinghousesClin= [];
		formClearinghouseEdit.ListClearinghousesClinOld= [];
		for(var i=0;i<_listClearinghousesClinicsAll.Count;i++) {
	...
	private void butDefaultDental_Click(object sender,EventArgs e) {
		...
		var ch=_listClearinghousesHq[gridMain.GetSelectedIndex()];
	...
	private void butDefaultMedical_Click(object sender,EventArgs e) {
		...
		var clearinghouse=_listClearinghousesHq[gridMain.GetSelectedIndex()];
	...
	private void butSave_Click(object sender, System.EventArgs e) {
		...
		var reportCheckIntervalMinuteCount=0;
		try {
		...
		var doRestartToShowChanges=false;
		var doInvalidateCache=false;
	...
	private void FormClearinghouses_Closing(object sender, System.ComponentModel.CancelEventArgs e) {
		...
		var clearinghouseDent=Clearinghouses.GetClearinghouse(PrefC.GetLong(PrefName.ClearinghouseDefaultDent));
		...
		var clearinghouseMed=Clearinghouses.GetClearinghouse(PrefC.GetLong(PrefName.ClearinghouseDefaultMed));
