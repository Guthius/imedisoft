...
public partial class FormTrackNext : FormODBase {
	...
	private void InitDateRange() {
		var dayCount=PrefC.GetInt(PrefName.PlannedApptDaysPast);
		if(dayCount==-1) {
	...
	private void setupToolStripMenuItem_Click(object sender,EventArgs e) {
		using var formTrackNextSetup=new FormTrackNextSetup();
	...
	private void FillGrid(){
		...
		var col=new GridColumn(Lan.g(this,"Patient"),140);
		...
		for(var i = 0;i<_listAppointmentsPlanned.Count;i++) {
			row=new GridRow();
			var patName=Lan.g(this,"UNKNOWN");
			var patient=listPatients.Find(x => x.PatNum==_listAppointmentsPlanned[i].PatNum);
			if(patient!=null) {
	...
	private void grid_MouseUp(object sender,MouseEventArgs e) {
		if(e.Button==MouseButtons.Right && gridMain.SelectedIndices.Length>0) {
			var patient=Patients.GetLim(_listAppointmentsPlanned[gridMain.SelectedIndices[gridMain.SelectedIndices.Length-1]].PatNum);
	...
	private void SelectPatient_Click() {
		...
		var patNum=_listAppointmentsPlanned[gridMain.SelectedIndices[gridMain.SelectedIndices.Length-1]].PatNum;
	...
	private void SeeChart_Click() {
		...
		var patient=Patients.GetPat(_listAppointmentsPlanned[gridMain.SelectedIndices[gridMain.SelectedIndices.Length-1]].PatNum);
	...
	private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		var currentSelection=gridMain.GetSelectedIndex();
		var currentScroll=gridMain.ScrollValue;
		var patient=Patients.GetPat(_listAppointmentsPlanned[e.Row].PatNum);//Only one can be selected at a time in this grid.
		GlobalFormOpenDental.PatientSelected(patient,true);
		using var formApptEdit=new FormApptEdit(_listAppointmentsPlanned[e.Row].AptNum);
