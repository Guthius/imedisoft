...
public partial class FormPayPlan : FormODBase {
	...
	private void FormPayPlan_Load(object sender,System.EventArgs e) {
		...
		if(IsInsPayPlan) {
			...
			LayoutManagerForms.MoveLocation(labelTxAmtInfo,new Point(labelTxAmtInfo.Location.X,labelTxAmtInfo.Location.Y-20));
		}
		else {
			...
			LayoutManagerForms.MoveLocation(textDate,new Point(textDate.Location.X+22,textDate.Location.Y));//line up with text boxes below
			LayoutManagerForms.MoveLocation(labelDateAgreement,new Point(labelDateAgreement.Location.X+22,labelDateAgreement.Location.Y));
		}
	...
	private void FillCharges() {
		...
		var listGridRowsPayPlan=new List<GridRow>();
		var numCharges=1;
		for(var i=0;i<_listPayPlanCharges.Count;i++) {//Payplan Charges
		...
		if(_payPlan.PlanNum==0) {//Normal payplan
			_listPaySplits= [];
			var tablePaymentsBundled=PaySplits.GetForPayPlan(_payPlan.PayPlanNum);
			...
			for(var i=0;i<_tableClaimProcsBundled.Rows.Count;i++) {
		...
		_totInt=0;
		double totPay=0;
		for(var i=0;i<listGridRowsPayPlan.Count;i++) {
		...
		_totInt=0;
		var countDebits=0;
		for(var i=0;i<_listPayPlanCharges.Count;i++) {
			if(_listPayPlanCharges[i].ChargeType==PayPlanChargeType.Credit) {
		...
		var listPayPlanChargesDebits = _listPayPlanCharges.FindAll(x => x.ChargeType == PayPlanChargeType.Debit).OrderBy(x => x.ChargeDate).ToList();
	...
	private void butChangeGuar_Click(object sender,System.EventArgs e) {
		...
		_payPlan.Guarantor=frmPatientSelect.PatNumSelected;
		textGuarantor.Text=Patients.GetLim(_payPlan.Guarantor).GetNameLF();
	}
	...
	private void butChangePlan_Click(object sender,System.EventArgs e) {
		using var formInsPlanSelect=new FormInsPlanSelectFam(_payPlan.PatNum);
	...
	private void gridCharges_CellDoubleClick(object sender,OpenDental.UI.ODGridClickEventArgs e) { 
		...
		if(gridCharges.ListGridRows[e.Row].Tag.GetType()==typeof(PayPlanCharge)) {
			var payPlanCharge=(PayPlanCharge)gridCharges.ListGridRows[e.Row].Tag;
			var payPlanChargeOldAmt=payPlanCharge.Principal;
			using var formPayPlanChargeEdit=new FormPayPlanChargeEdit(payPlanCharge,_payPlan);//This automatically takes care of our in-memory list because the Tag is referencing our list of objects.
			...
			var paySplit=(PaySplit)gridCharges.ListGridRows[e.Row].Tag;
			...
			using var formPayment2=new FormPayment(_patient,_family,payment,false);//FormPayment may inserts and/or update the paysplits. 
			...
			var rowClaimProcBundled=(DataRow)gridCharges.ListGridRows[e.Row].Tag;
			var claim=Claims.GetClaim(SIn.Long(rowClaimProcBundled["ClaimNum"].ToString()));
			if(claim==null) {
				...
				using var formClaimEdit=new FormClaimEdit(claim,_patient,_family);//FormClaimEdit inserts and/or updates the claim and/or claimprocs, which could potentially change the bundle.
	...
	private void butAdd_Click(object sender,System.EventArgs e) {
		var payPlanCharge=PayPlanEdit.CreateDebitCharge(_payPlan,_family,comboProv.GetSelectedProvNum(),comboClinic.ClinicNumSelected,0,0,DateTime.Today,"");
		using var formPayPlanChargeEdit=new FormPayPlanChargeEdit(payPlanCharge,_payPlan);
	...
	private void butSignPrint_Click(object sender,EventArgs e) {
		...
		var keyData=PayPlans.GetKeyDataForSignature(_payPlan);
	...
	private void butPrint_Click(object sender,System.EventArgs e) {
		...
		var areaSectionType=AreaSectionType.ReportHeader;
		...
		var contenAlignmentL=ContentAlignment.MiddleLeft;
		var contentAlignmentR=ContentAlignment.MiddleRight;
		var yPos=140;
		var space=30;
		var x1=175;
		var x2=275;
	...
	private void butPayPlanTx_Click(object sender,EventArgs e) {
		using var formPayPlanCredits=new FormPayPlanCredits(_patient,_payPlan,comboProv.GetSelectedProvNum());
	...
	private bool AdjustPatPayPlanChargesForClose() {
		var listCreditedProcNums=_listPayPlanCharges.Where(x => x.ProcNum!=0).Select(x => x.ProcNum).ToList();
		var listCreditedProcedures=Procedures.GetManyProc(listCreditedProcNums,includeNote:false);
		var prompt=Lan.g(this,"Interest will be removed from future charges and they will be made due immediately. Would you like to continue?");
		...
		var payPlanChargeCloseout=PayPlanEdit.CalculatePatPayPlanCloseoutCharge(_listPayPlanCharges,listCreditedProcedures,_payPlan,DateTime.Today);
	...
	private void CreateOrRecalculateScheduleCharges(bool isRecalculate) {
		var payPlanTerms=GetTermsFromUI();
		if(isRecalculate) {
			var payPlanRecalculationData=PayPlanEdit.PayPlanRecalculationData.CreateRecalculationData(payPlanTerms,_payPlan,_family
	...
	private Sheet PayPlanToSheet(PayPlan payPlan) {
		var sheet=SheetUtil.CreateSheet(SheetDefs.GetInternalOrCustom(SheetInternalType.PaymentPlan),_patient.PatNum);
	...
	private void butAdj_Click(object sender,EventArgs e) {
		...
		var def=Defs.GetDef(DefCat.AdjTypes,PrefC.GetLong(PrefName.PayPlanAdjType));
		...
		var negAdjAmt=-(SIn.Double(inputBox.StringResult));
		var totalRemainingBal=PayPlans.GetBalance(_payPlan.PayPlanNum,_listPayPlanCharges,_listPaySplits);
		...
		var totalNegFutureAdjs=_listPayPlanCharges.FindAll(x => x.ChargeType==PayPlanChargeType.Debit && x.Principal<0 && x.ChargeDate > DateTime.Today).Sum(x => x.Principal);
		...
		if(SIn.Double(textCompletedAmt.Text)>0) {
			var payPlanChargeTxOffset=new PayPlanCharge {
			...
			var listPayPlanChargesCredit=_listPayPlanCharges.FindAll(x => x.ChargeType==PayPlanChargeType.Credit);
	...
	private void UpdateTxPaymentPlanChargeSummary(List<PayPlanCharge> listPayPlanChargesCredit) {
		...
		for(var i=0;i<listPayPlanChargesCredit.Count;i++) {
		...
		var txTotalAmt=PayPlans.GetTxTotalAmt(_listPayPlanCharges);
	...
	private void OpenUnlockCodeForPayPlan(){
		MobileDataByte funcInsertDataForUnlockCode(string unlockCode) {
			using var pdfDocument=GetPayPlanPDF(_payPlan);
			...
			try {
				var mobileDataByteNum=MobileDataBytes.InsertPDF(pdfDocument,_patient.PatNum,unlockCode,eActionType.PaymentPlan,listTagValues);
	...
	private void PushSelectedPayPlanToEclipboard(MobileAppDevice mobileAppDevice){
		...
		using var pdfDocument=GetPayPlanPDF(_payPlan);//Cant be null due to above check.
		try {
			MobileNotifications.CI_SendPaymentPlan(pdfDocument,_payPlan,mobileAppDevice.MobileAppDeviceNum);
		}
		catch (Exception ex) {
			//Error occurred
	...
	private void SaveData(bool isPrinting=false) {
		...
		if(IsInsPayPlan) { //if insurance payment plan, remove all other credits and create one credit for the completed amt.
			...
			var payPlanChargeAdd=new PayPlanCharge();
