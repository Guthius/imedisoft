...
public partial class FormRepeatChargesUpdate :FormODBase{
	...
	private bool CheckBeginDateTime() {
		...
		var repeatingChargesBeginDateTime=PrefC.GetDateT(PrefName.RepeatingChargesBeginDateTime);
		...
		if(Security.IsAuthorized(EnumPermType.SecurityAdmin,true)) {
			var message=Lans.g("Repeating Charges last started on")+" "+repeatingChargesBeginDateTime
			            +Lans.g(".  Restart repeating charges?");
		...
		MsgBox.Show(Lans.g("Repeating Charges last started on")+" "+repeatingChargesBeginDateTime
		            +Lans.g(".  Contact a user with SecurityAdmin permission to restart repeating charges."));
		return false;
	}
	...
	private void butOK_Click(object sender,EventArgs e) {
		...
		var repeatChargeResult=RepeatCharges.RunRepeatingCharges(MiscData.GetNowDateTime(),checkRunAging.Checked);
		var metrics=repeatChargeResult.ProceduresAddedCount+" "+Lan.g(this,"procedures added.")+"\r\n"+repeatChargeResult.ClaimsAddedCount+" "
		...
		if(!string.IsNullOrEmpty(repeatChargeResult.ErrorMsg.ToString())) {
			SecurityLogs.MakeLogEntry(EnumPermType.RepeatChargeTool,0,"Repeat Charge Tool Error: "+repeatChargeResult.ErrorMsg);
