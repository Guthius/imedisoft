...
public partial class FormJournal:FormODBase {
	...
	private void FormJournal_Load(object sender,EventArgs e) {
		var screen=System.Windows.Forms.Screen.FromControl(this);//automatically returns screen that contains largest portion of this form
		Height=screen.WorkingArea.Height;
		Top=screen.WorkingArea.Top;
		textDateTo.Text=DateInitialAsOf.ToShortDateString();
		var dateFirstofYear=new DateTime(DateInitialAsOf.Year,1,1);
		var dateOneYearPlus=dateFirstofYear.AddYears(-1);
	...
	private void FillGrid(bool isPrinting=false) {
		...
		var gridToFill=isPrinting?_gridMainPrint:gridMain;
		...
		var col=new GridColumn(Lan.g("TableJournal","Chk #"),60,HorizontalAlignment.Center);
		...
		var dateTo=string.IsNullOrEmpty(textDateTo.Text)?DateTime.MaxValue:SIn.Date(textDateTo.Text);
		var filterAmtFrom=textAmountFrom.IsValid()?SIn.Double(textAmountFrom.Text):0;
		var filterAmtTo=textAmountTo.IsValid()?SIn.Double(textAmountTo.Text):0;
		...
		GridRow row;
		decimal bal=0;
		for(var i=0;i<_listJournalEntries.Count;i++) {
	...
	private void Add_Click(){
		...
		using var formTransactionEdit=new FormTransactionEdit(transaction,_account.AccountNum);
	...
	private void Reconcile_Click() {
		var selectedRow=gridMain.GetSelectedIndex();
		var scrollValue=gridMain.ScrollValue;
		using var formReconciles=new FormReconciles(_account.AccountNum);
	...
	private void Print_Click(){
		...
		var orient=PrintoutOrientation.Portrait;
	...
	private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		...
		var scrollValue=gridMain.ScrollValue;
		var transactionNum=(long)gridMain.ListGridRows[e.Row].Tag;
		if(transactionNum==0){
			MsgBox.Show(this,"Cannot edit auto entries.");
			return;
		}
		var transaction=Transactions.GetTrans(transactionNum);
		if(transaction==null){
