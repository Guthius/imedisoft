...
public partial class FormProcSelect : FormODBase {
	...
	private void FillGrid(){
		...
		var constructResults=PaymentEdit.ConstructAndLinkChargeCredits(
			_patNum,doIncludeTreatmentPlanned:doIncludeTreatmentPlanned,
			doIncludeExplicitCreditsOnly:doIncludeExplicitCreditsOnly);
		var listAccountEntries=constructResults.ListAccountEntries.FindAll(x => x.PatNum==_patNum);
		...
		var listProcNums=listAccountEntries.FindAll(x => x.GetType()==typeof(Procedure)).Select(x => x.ProcNum).ToList();
		_listClaimProcs=constructResults.ListClaimProcs.FindAll(x => listProcNums.Contains(x.ProcNum));
		var listDefsPosAdj=Defs.GetPositiveAdjTypes();
		gridMain.BeginUpdate();
		gridMain.Columns.Clear();
		var col=new GridColumn(Lan.g("TableProcSelect","Date"),70,HorizontalAlignment.Center);
		...
		var listAccountEntriesProcedures=listAccountEntries.FindAll(x => x.GetType()==typeof(Procedure));
		GridRow row;
		for(var i=0;i<listAccountEntries.Count;i++) {
			...
			if(listAccountEntries[i].GetType()==typeof(Adjustment)) {
				...
				var accountEntryProcedure=listAccountEntriesProcedures.Find(x => x.ProcNum==listAccountEntries[i].ProcNum
				...
				var defAdjType=listDefsPosAdj.FirstOrDefault(x => x.DefNum==((Adjustment)listAccountEntries[i].Tag).AdjType);
	...
	private void RefreshBreakdown() {
		...
		var listAccountEntries=gridMain.SelectedTags<AccountEntry>();
		var procedureBreakdown=new ProcedureBreakdown(listAccountEntries,_listClaimProcs,ListPaySplits);
	...
	private class ProcedureBreakdown {
		...
		public ProcedureBreakdown(List<AccountEntry> listAccountEntries,List<ClaimProc> listClaimProcsAll,List<PaySplit> listPaySplits) {
			...
			var listProcNums=listAccountEntries.Select(x => x.ProcNum).ToList();
			//Insurance estimates/payments and writeoffs/writeoff estimates
			//ClaimProcs are always treated as explicitly linked.
			var listClaimProcs=listClaimProcsAll.FindAll(x => listProcNums.Contains(x.ProcNum));
			double insEst=0;
			double insPay=0;
			double writeOff=0;
