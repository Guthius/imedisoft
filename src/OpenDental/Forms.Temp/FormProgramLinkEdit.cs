...
public partial class FormProgramLinkEdit : FormODBase {
	...
	private void FormProgramLinkEdit_Load(object sender, System.EventArgs e) {
		...
		HideClinicControls(true);//Hide the "Hide Button for Clinics" button based upon the user's clinics being on or off.
	...
	private void SetAdvertising() {
		...
		var programProperty = ProgramProperties.GetForProgram(ProgramCur.ProgramNum).FirstOrDefault(x => x.PropertyDesc=="Disable Advertising");
	...
	private void FillForm(){
		...
		var listToolButItems=ToolButItems.GetForProgram(ProgramCur.ProgramNum);
	...
	private void FillGrid(){
		...
		GridRow row;
		for(var i=0;i<listProgramProperties.Count;i++) {
			...
			if(listProgramProperties[i].IsMasked) {
				string password;
				var isXVWebPassword=ProgramCur.ProgName==ProgramName.XVWeb.ToString() && listProgramProperties[i].PropertyDesc==XVWeb.ProgramProps.Password;
				if(isXVWebPassword) {
	...
	private void ShowPLButHiddenLabel() {
		...
		if(true && !listProgramProperties.IsNullOrEmpty()) {//If anything is in list, they have a hidden clinic.
	...
	private void DisableUIElementsBasedOnClinicRestriction() {
		if(true && Security.CurUser.ClinicIsRestricted) {//Clinics are Enabled and the user is restricted.
			...
			var listControlsAll=UIHelper.GetAllControls(this).Where(x => listControls.Contains(x)).ToList();
			for(var i=0;i<listControlsAll.Count;i++) {
	...
	private void butOutputFile_Click(object sender,EventArgs e) {
		using var formProgramLinkOutputFile=new FormProgramLinkOutputFile(ProgramCur);
	...
	private void butImport_Click(object sender,EventArgs e) {
		...
		try {
			var imageImported=Image.FromFile(importFilePath);
	...
	private void ShowFormProgramProperty(ProgramProperty programProperty) {
		if(!ProgramProperties.CanEditProperties([programProperty],false)) {
			return;
		}
		var propIsPassword=programProperty.PropertyDesc.Contains("Password");//We encrypt all passwords now, so just check for "Password" keyword.
		using var formProgramProperty=new FormProgramProperty(propIsPassword);
	...
	private void ShowComboBoxForProgramProperty(ProgramProperty programProperty,List<string> listValuesForDb,List<string> listForDisplays
		,string prompt) 
	{
		var programPropertyOld=programProperty.Copy();
		var inputBox=new InputBox(prompt,listForDisplays,listValuesForDb.FindIndex(x => x==programProperty.PropertyValue));
		inputBox.ShowDialog();
	...
	private void butClinicLink_Click(object sender,EventArgs e) {
		...
		var listClinics=Clinics.GetForUserod(Security.CurUser,doIncludeHQ:true,hqClinicName:Lan.g(this,"HQ"));//Include HQ if user not restricted.
		...
		var listProgramPropertiesForUser=ProgramProperties.GetForProgram(ProgramCur.ProgramNum)
			.FindAll(x =>  x.PropertyDesc==ProgramProperties.PropertyDescs.ClinicHideButton 
			               && listClinics.Select(y => y.Id).Contains(x.ClinicNum));
		using var formProgramLinkHideClinics=new FormProgramLinkHideClinics(ProgramCur,listProgramPropertiesForUser,listClinics);
	...
	private void butSave_Click(object sender, System.EventArgs e) {
		...
		if(checkEnabled.Checked && !Programs.IsEnabledByHq(ProgramCur,out var err)) {
			MsgBox.Show(err);
			return;
		}
		...
		var listToolBarsAvails=listToolBars.GetListSelected<EnumToolBar>();
		for(var i=0;i<listToolBarsAvails.Count;i++){
