...
public partial class FormDashboardWidgetSetup:FormODBase {
	...
	private void FormDashboardSetup_Load(object sender,EventArgs e) {
		...
		FillGridInternal();
		FillGridCustom();
		var actionFillGridCustom=() => FillGridCustom(doRefreshCache:false);
	...
	private void InitializeOnStartup() {
		...
		var userGroup=UserGroups.GetForUser(Security.CurUser.UserNum).FirstOrDefault();
		if(userGroup!=null) {
		...
		var listUserGroupNums=_listUserGroups.Select(x => x.UserGroupNum).ToList();
	...
	private void FillGridInternal() {
		...
		var listSheetDefsWidgets=Enum.GetValues(typeof(SheetInternalType)).OfType<SheetInternalType>()
	...
	private void FillGrid(GridOD grid,bool isSelectionMaintained,List<SheetDef> listSheetDefsWidgets) {
		var listSheetDefsSelected=grid.SelectedTags<SheetDef>();
		grid.BeginUpdate();
		grid.Columns.Clear();
		GridColumn col;
		...
		for(var i=0;i<listSheetDefsWidgets.Count;i++) {
			...
			if(grid==gridCustom) {
				var userGroupSelected=comboUserGroup.GetSelected<UserGroup>();
		...
		for(var i=0;i<listSheetDefsSelected.Count;i++) {
	...
	private bool HasDashboardWidgetPermissionAll(long userGroupNum=0) {
		...
		var listGroupPermissions=_listGroupPermissions.FindAll(x => x.UserGroupNum==userGroupNum);
		var groupPermissionAll=listGroupPermissions.FirstOrDefault(x => x.FKey==0);
		...
		var listSheetDefsWidgets=SheetDefs.GetCustomForType(SheetTypeEnum.PatientDashboardWidget);
		var countExplicitGroupPermissions=_listGroupPermissions.Count(x => x.PermType==EnumPermType.DashboardWidget && x.UserGroupNum==userGroupNum && x.FKey!=0);
	...
	private bool ToggleDashboardPermission(UserGroup userGroup,SheetDef sheetDef) {
		...
		var listSheetDefsWidgets=gridCustom.GetTags<SheetDef>();
		//If user has 'all' permission remove it and create explicit list of all permissions.
		var hasAll=_listGroupPermissions.Any(x => x.UserGroupNum==userGroup.UserGroupNum && x.FKey==0);
		if(hasAll) {
	...
	private long EditWidget(SheetDef sheetDef=null) {
		if(sheetDef==null) {
			...
			sheetDef.SheetFieldDefs= [];
			sheetDef.IsNew=true;
		}
		SheetDefs.GetFieldsAndParameters(sheetDef);
		using var formSheetDefEdit=new FormSheetDefEdit(sheetDef);
	...
	private void gridInternal_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		...
		var sheetDefSelected=gridInternal.SelectedTag<SheetDef>();
		using var formSheetDefEdit=new FormSheetDefEdit(sheetDefSelected);
	...
	private void gridCustom_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		...
		var sheetDefSelected=gridCustom.SelectedTag<SheetDef>();
	...
	private void gridCustom_CellClick(object sender,ODGridClickEventArgs e) {
		...
		var userGroupSelected=comboUserGroup.GetSelected<UserGroup>();
		var sheetDefSelected=gridCustom.SelectedTag<SheetDef>();
		var strIsAllowed=ToggleDashboardPermission(userGroupSelected,sheetDefSelected)?"X":"";
	...
	private void ButAdd_Click(object sender,EventArgs e){
		var sheetDefWidgetNum=EditWidget();//Adding a new Dashboard Widget.
		if(sheetDefWidgetNum==-1) {
			return;
		}
		var sheetDef=SheetDefs.GetWhere(x => x.SheetDefNum==sheetDefWidgetNum).FirstOrDefault();
		_hasChanged=true;
		if(sheetDef!=null) {
	...
	private void butCopy_Click(object sender,EventArgs e) {
		...
		var sheetDef=gridInternal.SelectedTag<SheetDef>().Copy(); 
		sheetDef.IsNew=true;
	...
	private void butDuplicate_Click(object sender,EventArgs e) {
		...
		var sheetDef=gridCustom.SelectedTag<SheetDef>().Copy();
	...
	private void SelectDashboardDef(SheetDef sheetDef) {
		...
		var sheetDefSelectedIndex=gridCustom.GetTags<SheetDef>().FindIndex(x => x.SheetDefNum==sheetDef.SheetDefNum);
		gridCustom.SetSelected(new Point(0,sheetDefSelectedIndex));
	}

	private void butTools_Click(object sender,EventArgs e) {
		using var formSheetTools=new FormSheetTools(true);
		...
		SheetDefs.RefreshCache();
		SheetFieldDefs.RefreshCache();
		var sheetDefImported=SheetDefs.GetFirstOrDefault(x => x.SheetDefNum==formSheetTools.SheetDefNumImported);
