...
public partial class FormClinicEdit : FormODBase {
	...
	private void FormClinicEdit_Load(object sender, System.EventArgs e) {
		...
		for(var i=0;i<_listDefsRegions.Count;i++) {
		...
		var clinicPrefInsAutoReceiveNoAssign=ClinicPrefs.GetPref(PrefName.InsAutoReceiveNoAssign,ClinicCur.Id);
		...
		var clinicPrefAlwaysAssignBenToPatient=ClinicPrefs.GetPref(PrefName.InsDefaultAssignBen,ClinicCur.Id);
		if(clinicPrefAlwaysAssignBenToPatient!=null) {
			checkAlwaysAssignBenToPatient.Checked=true;
		}
		var emailAddress=EmailAddresses.GetOne(ClinicCur.EmailAddressId??0);
	...
	private void FillSpecialty() {
		...
		for(var i = 0;i<ListDefLinksSpecialties.Count;i++) {
			row=new GridRow();
			specialtyDescript="";
			var def = listDefs.FirstOrDefault(x => x.DefNum==ListDefLinksSpecialties[i].DefNum);
			if(def!=null) {
	...
	private void butSave_Click(object sender, System.EventArgs e) {
		...
		if(checkHidden.Checked) {
			...
			var listUserodsRestricted = Userods.GetUsersOnlyThisClinic(ClinicCur.Id);
		...
		if(comboInsAutoReceiveNoAssign.SelectedIndex==2) {//2=Use Global Preference
			...
			var strInsAutoReceiveNoAssignClinicPref=SOut.Int(comboInsAutoReceiveNoAssign.SelectedIndex);
