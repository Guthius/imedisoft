...
public partial class FormMobileCode:FormODBase {
	...
	private Func<string,MobileDataByte> _funcUnlockGeneratedMobileDataByte;
	...
	public bool HasMobileCodeBeenReceived;
	...
	public FormMobileCode(Func<string,MobileDataByte> funcUnlockGeneratedMobileDataByte) {
		InitializeComponent();

		_funcUnlockGeneratedMobileDataByte=funcUnlockGeneratedMobileDataByte;
	}
	...
	private bool TryInvoke(string unlockCode,out MobileDataByte mobileDataByte){
		mobileDataByte=null;
		try {
			mobileDataByte=_funcUnlockGeneratedMobileDataByte.Invoke(unlockCode);
		}
		catch(Exception ex){
			picBoxMain.TextNullImage=ex.Message;
			picBoxMain.Image=null;
		}
		return (mobileDataByte!=null);
	}
	...
	private void SetFormLocked(string unlockCode){
		...
		barcodeWriterQr.Options=encodingOptions;
		barcodeWriterQr.Format=ZXing.BarcodeFormat.QR_CODE;
		using var bitmapQr=new Bitmap(barcodeWriterQr.Write(unlockCode)); 
		picBoxMain.Image?.Dispose();
		picBoxMain.Image=(bitmapQr.Clone() as Bitmap);//Picture box will handle disposing
		#region Fill and disable textbox using rawCode.
		var charArrayRawCode=unlockCode.ToCharArray();
		var i=1;
		for(var idx=0;idx<charArrayRawCode.Length;idx++) {
			TextBox textBox;
			switch(i) {
				case 1:
					textBox=textBox1;
			...
			textBox.Text=charArrayRawCode[idx].ToString();
	...
	private void SetFormUnlocked(){
		...
		picBoxMain.Image=null;
		var listTextBoxes=Controls.OfType<TextBox>().ToList();
		for(var i=0;i<listTextBoxes.Count;i++) {
			listTextBoxes[i].Enabled=true;
