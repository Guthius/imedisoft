...
public partial class FormInsEditPatLog:FormODBase {
	...
	private List<GridRow> ConstructGridRows() {
		...
		var dictionaryLongUserods=Userods.GetDeepCopy().ToDictionary(x => x.UserNum,x => x);
		for(var i=0;i<_listInsEditPatLogs.Count;i++){
			if(!_listInsEditPatLogs[i].DateTStamp.Between(dateFrom,dateTo)) {
				continue;
			}
			row=new GridRow();
			row.Cells.Add(_listInsEditPatLogs[i].DateTStamp.ToString());
			var userod = listUserods.Find(x=>x.UserNum==_listInsEditPatLogs[i].UserNum);
			if(userod==null) {
