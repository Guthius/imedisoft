...
public partial class FormClaimEdit : FormODBase {
	...
	private void FormClaimEdit_Shown(object sender,EventArgs e) {
		var listClaimProcsForClaim=_listClaimProcsForClaim.ToList();
		if(PrefC.GetBool(PrefName.ClaimEditRequireNoMissingData) && IsNew){//We only really care about new claims
			var claimSendQueueItemArray=Claims.GetQueueList(_claim.ClaimNum,_claim.ClinicNum,_claim.CustomTracking);
			...
			var claimSendQueueItem = Eclaims.GetMissingData(_clearinghouse,claimSendQueueItemArray.ElementAtOrDefault(0),true);
		...
		var automationTrigger=EnumAutomationTrigger.ClaimOpen;
		if(IsNew) {
		...
		var listProcedures=Procedures.GetManyProc(_listClaimProcsForClaim.Select(x => x.ProcNum).ToList(),false);
		var listProceduresIncomplete=listProcedures.FindAll(x => x.ProcStatus!=ProcStat.C 
		...
		var listCodeNums = ProcedureCodes.GetOrthoBandingCodeNums();
		for(var i = 0;i < _listClaimProcsForClaim.Count;i++) {
			var claimProc = _listClaimProcsForClaim[i];
			var procedure = listProcedures.Find(x => x.ProcNum==claimProc.ProcNum);
			if(procedure==null) {
				continue;
			}
	...
	private void FormClaimEdit_Load(object sender, System.EventArgs e) {
		...
		for(var i=0;i<_listClaimStatuses.Count;i++) {
			comboClaimStatus.Items.Add(_listClaimStatuses[i].GetDescription(false));
		}
		comboSpecialProgram.Items.Clear();
		var stringArrayEnumClaimSpecialPrograms=Enum.GetNames(typeof(EnumClaimSpecialProgram));
		for(var i=0;i<stringArrayEnumClaimSpecialPrograms.Length;i++) {
			comboSpecialProgram.Items.Add(stringArrayEnumClaimSpecialPrograms[i]);
		}
		var stringArrayEnumRelat=Enum.GetNames(typeof(Relat));
		for(var i=0;i<stringArrayEnumRelat.Length;i++){;
		...
		var insPlan=InsPlans.GetPlan(_claim.PlanNum,_listInsPlans);
		//If there is no insplan then we need to close
		if(insPlan==null) {
			MsgBox.Show(Lans.g("Invalid insurance plan associated with claim. Please run database maintenance method")+" "+nameof(DatabaseMaintenances.InsSubNumMismatchPlanNum)
			            +" "+Lans.g("found in the old tab"));
		...
		var stringArrayEnumsPlaceOfService=Enum.GetNames(typeof(PlaceOfService));
		for(var i=0;i<stringArrayEnumsPlaceOfService.Length;i++) {
		...
		var stringArrayEnumYN=Enum.GetNames(typeof(YN));
		for(var i=0;i<stringArrayEnumYN.Length;i++) {
		...
		var claimSendQueueItemsArray=Claims.GetQueueList(_claim.ClaimNum,_claim.ClinicNum,0);
		...
		if(IsNew) {
			...
			var listProceduresSites=_listProcedures.FindAll(x => _listClaimProcsForClaim.Any(y => y.ProcNum==x.ProcNum) && x.SiteNum!=0);
			//Null is an acceptable site.
			if(listProceduresSites.Count > 0) {
				var site=Sites.GetFirstOrDefault(x => x.SiteNum==listProceduresSites[0].SiteNum);
				if(site!=null) {
	...
	private void SetBounds(){
		var rectangleWorkingArea=System.Windows.Forms.Screen.FromPoint(MousePosition).WorkingArea;
		...
		var heightAvail=rectangleWorkingArea.Height;
		if(heightAvail>871){
			heightAvail=871;
		}
		...
		var heightChangeZoom=(Height-Height)/2;
	...
	private void SetFormReadOnly(System.Windows.Forms.Control controlsInput) {
		for(var i=0;i<controlsInput.Controls.Count;i++) {
			for(var j=0;j<controlsInput.Controls[i].Controls.Count;j++) {//Make sure all sub controls are read only.
	...
	private void comboClaimStatus_SelectionChangeCommitted(object sender,EventArgs e) {
		var claimStatus=_listClaimStatuses.Where(x=>x.GetDescription(useShortVersionIfAvailable:true)==_claimOld.ClaimStatus).FirstOrDefault();
	...
	private void FillCombosProv() {
		var listProvidersForClinics=Providers.GetProvsForClinic(comboClinic.ClinicNumSelected);
		var providerNum=comboProvBill.GetSelectedProvNum();
	...
	private List<ClaimStatus> GetListComboClaimStatus() {
		var listClaimStatusRetVals=Enum.GetValues(typeof(ClaimStatus)).OfType<ClaimStatus>().ToList();
	...
	public void FillForm(){
		...
		if(TryGetClaimStatusEnumFromCurClaim(out var claimStatus)){
			comboClaimStatus.SelectedIndex=_listClaimStatuses.IndexOf(claimStatus);
		}
		else {//This should never happen.
			comboClaimStatus.SetSelectedKey<ClaimStatus>(-1,_ => _listClaimStatuses.IndexOf(claimStatus),_ => Lan.g(this,"Not Set"));
		}
		...
		for(var i=0;i<Enum.GetNames(typeof(EnumClaimMedType)).Length;i++){
		...
		for(var i=0;i<_listClaimForms.Count;i++){
		...
		var stringArrayClaimCorrectionTypeNames=Enum.GetNames(typeof(ClaimCorrectionType));
		var arrayClaimCorrectionTypeValues=Enum.GetValues(typeof(ClaimCorrectionType));
		comboCorrectionType.Items.Clear();
		for(var i=0;i<stringArrayClaimCorrectionTypeNames.Length;i++) {
		...
		var stringArrayFlags=_claim.AttachedFlags.Split(',');
		for(var i=0;i<stringArrayFlags.Length;i++){
		...
		if(_claimCondCodeLog!=null && _claimCondCodeLog.ClaimNum!=0) {
			textCode0.Text=_claimCondCodeLog.Code0;
			textCode1.Text=_claimCondCodeLog.Code1;
			textCode2.Text=_claimCondCodeLog.Code2;
			textCode3.Text=_claimCondCodeLog.Code3;
			textCode4.Text=_claimCondCodeLog.Code4;
			textCode5.Text=_claimCondCodeLog.Code5;
			textCode6.Text=_claimCondCodeLog.Code6;
			textCode7.Text=_claimCondCodeLog.Code7;
			textCode8.Text=_claimCondCodeLog.Code8;
			textCode9.Text=_claimCondCodeLog.Code9;
			textCode10.Text=_claimCondCodeLog.Code10;
		}
	...
	private static void FillValCode(List<ClaimValCodeLog> listClaimValCodeLogs,int valCodeIdx,System.Windows.Forms.TextBox textBoxVCCode,System.Windows.Forms.TextBox textBoxVCAmount) {
	...
	private void FillCanadian() {
		...
		var listStringMissingTeeth=ToothInitials.GetMissingOrHiddenTeeth(_loadData.ListToothInitials);
		var stringMissingTeeth="";
		for(var i=0;i<listStringMissingTeeth.Count;i++){
			if(i>0){
				stringMissingTeeth+=", ";
			}
	...
	private void SetCanadianExtractedTeeth() {
		...
		if(comboMaxProsth.SelectedIndex==1 || comboMandProsth.SelectedIndex==1){
			var listProcedureExtracted=Procedures.GetCanadianExtractedTeeth(_listProcedures);				
			for(var i=0;i<listProcedureExtracted.Count;i++) {
	...
	private void butRecalc_Click(object sender, System.EventArgs e) {
		...
		var listBenefits=Benefits.Refresh(_listPatPlans,_listInsSubs);
	...
	private void FillGrids(bool doRefreshData=true){
		...
		var listClaimProcsTxfr=_listClaimProcsForClaim.FindAll(x => x.IsTransfer);
		if(listClaimProcsTxfr.Count > 0) {
			_listClaimProcsForClaim.RemoveAll(x => x.IsTransfer);
			for(var i=0;i<listClaimProcsTxfr.Count;i++) {
				...
				var index=_listClaimProcsForClaim.FindIndex(x => x.ProcNum==listClaimProcsTxfr[i].ProcNum);
		...
		var listClaimProcsIntermingled=new List<ClaimProc>();//This list will contain procs and their labs, such that attached labs follow their parent proc.
		for(var i=0;i<_listClaimProcsForClaim.Count;i++){
			row=new GridRow();
			...
			var procedure=Procedures.GetProcFromList(_listProcedures,_listClaimProcsForClaim[i].ProcNum);
			var listClaimProcsForLab=GetListClaimProcsForProcNumLabsParent(procedure.ProcNum);
			if(CultureInfo.CurrentCulture.Name.EndsWith("CA") && listClaimProcsForLab.Count!=0) {//This proc has one or more labs attached.
				for(var j=0;j<listClaimProcsForLab.Count;j++) {
		...
		if(_loadData.DoShowPatResp) {
			var procFeeTotal=gridProc.GetTags<ClaimProc>()
		...
		for(var i=0;i<_dataTablePayments.Rows.Count;i++){
			row=new GridRow();
	...
	private void FillGridSentAttachments(GridOD gridODAttachments) {
		...
		GridRow row;
		for(var i=0;i<_claim.Attachments.Count;i++) {
			row=new GridRow();
	...
	private void gridSent_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		...
		var doSaveToImagingModule=false;
		if(tabControlAttach.SelectedTab==tabEDS) {
		...
		{
			//Local storage
			var pathAndFileName=Path.Combine(patFolder,claimAttach.ActualFileName);
			try {
	...
	private void ClaimProcRowHelper(GridRow row,ClaimProc claimProc) {
		...
		var date=claimProc.ProcDate.ToShortDateString();
		...
		if(claimProc.ProcNum==0) {
			...
			var procedure=Procedures.GetProcFromList(_listProcedures,claimProc.ProcNum);
			...
			var procedureCodeSent=ProcedureCodes.GetProcCode(claimProc.CodeSent);
			var insPlan=_listInsPlans.Where(x => x.PlanNum == claimProc.PlanNum).FirstOrDefault();
			var descript=Procedures.GetClaimDescript(claimProc,procedureCodeSent,procedureCode,insPlan);
	...
	private void FillStatusHistory(bool doRefreshStatusEntries=true) {
		...
		for(var i=0;i<listClaimTrackingsCustomStatusEntries.Count;i++) {
			row=new GridRow();
			...
			var defValue=Defs.GetName(DefCat.ClaimCustomTracking,listClaimTrackingsCustomStatusEntries[i].TrackingDefNum);//get definition Name
	...
	private void SelectLabProcs() {
		...
		for(var i=0;i<gridProc.SelectedIndices.Count();i++) {
			var index=gridProc.SelectedIndices[i];
			if(_listClaimProcsForClaim[index].ProcNum==0) {
				continue;//current index is associated to a 'Total Payment' row.
			}
			var procedure=_listProcedures.FirstOrDefault(x => x.ProcNum==_listClaimProcsForClaim[index].ProcNum);
			var listClaimProcsForLab=GetListClaimProcsForProcNumLabsParent(procedure.ProcNum);//This Claimproc's Procedure is a Lab
			...
			if(procedure.ProcNumLab==0 && listClaimProcsForLab.Count!=0) {
				for(var j=1;j<=GetListLabsForProc(procedure.ProcNum,_listClaimProcsForClaim[index]).Count;j++) {
	...
	private List<ClaimProc> GetListLabsForProc(long procNumParent,ClaimProc claimProc) {
		var listClaimProcsForLab=GetListClaimProcsForProcNumLabsParent(procNumParent);
		if(claimProc.Status==ClaimProcStatus.Supplemental) { 
	...
	private List<ClaimProc> GetListClaimProcsForProcNumLabsParent(long procNumParent) {
		var listClaimProcsForLabOnClaim=new List<ClaimProc>();
		if(procNumParent==0) {
		...
		var listProceduresLab=_listProcedures.FindAll(x => x.ProcNumLab==procNumParent);
		for(var i=0;i<listProceduresLab.Count;i++){//For each lab proc associated to the passed in procNumParent, add it to the list
	...
	private void gridProc_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		...
		var claimProc=_listClaimProcsForClaim[e.Row];
		var isPendingSupplemental=claimProc.IsOverpay && claimProc.Status==ClaimProcStatus.NotReceived;
	...
	private void tabControlMain_SelectedIndexChanged(object sender,EventArgs e) {
		if(tabControlMain.SelectedTab==tabHistory){
			gridStatusHistory.Bounds=new Rectangle(
				x:6, y:33,
				width:tabControlMain.ClientSize.Width-12,
				height:tabControlMain.ClientSize.Height-33-2);
		}
	}
	...
	private bool CheckRecalcEstimates(ClaimProc claimProc) {
		...
		using var formClaimProc=new FormClaimProc(claimProc,null,_family,_patient,_listInsPlans,listClaimProcHists,ref listClaimProcHistsLoop,_listPatPlans,true,_listInsSubs);
		...
		var claimProcInitial=formClaimProc.ClaimProcInitial;//Unedited ClaimProc with base estimates calculated in FormClaimProc.
		var isClaimProcRecalcNeeded=false;
	...
	private void gridPay_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		var tempClaimNum=_claim.ClaimNum;
		var claimPayment=ClaimPayments.GetOne(SIn.Long(_dataTablePayments.Rows[e.Row]["ClaimPaymentNum"].ToString()));
		using var formClaimPayBatch=new FormClaimPayBatch(claimPayment);
	...
	private void butOtherCovChange_Click(object sender, System.EventArgs e) {
		using var formInsPlanSelect=new FormInsPlanSelectFam(_patient.PatNum);
	...
	private void butPayTotal_Click(object sender, System.EventArgs e) {
		...
		if(_claim.ClaimType=="PreAuth"){
			ODMessageBox.Show(Lan.g(this,"PreAuthorizations can only be entered by procedure."));
			return;
		}
		...
		if(DoConsolidateOrthoPayments()) {
			var insPlan = InsPlans.GetPlan(_claim.PlanNum,_listInsPlans);
			var orthoAutoCodeNum = InsPlans.GetOrthoAutoProc(insPlan);
	...
	private bool PayAsTotalToClaimProcsExplicitly() { 
		...
		var result=SIn.Double(inputBox.StringResult);
		var listClaimProcs=new List<ClaimProc>();
		for(var i=0;i<_listClaimProcsForClaim.Count;i++) {
			listClaimProcs.Add(_listClaimProcsForClaim[i].Copy());
		}
		var listClaimProcsForPayTotal=_listClaimProcsForClaim.FindAll(x => x.Status==ClaimProcStatus.NotReceived || x.Status==ClaimProcStatus.Supplemental).ToList(); //Filter list down to only procs that expect payments
		for(var i=0;i<listClaimProcsForPayTotal.Count;i++) {
			if(i==0) {
				...
				var listPayPlans=PayPlans.GetValidInsPayPlans(_claim.PatNum,_claim.PlanNum,_claim.InsSubNum,_claim.ClaimNum);
		...
		using var formClaimPayTotal=new FormClaimPayTotal(_patient,_family,_listInsPlans,_listPatPlans,_listInsSubs,GetBlueBookEstimateData(),totalPayAmt:result);
		...
		if(formClaimPayTotal.DialogResult!=DialogResult.OK){
			...
			for(var i=0;i<formClaimPayTotal.ClaimProcArray.Length;i++){
	...
	private bool PayAsTotal() { 
		...
		//Automatically set PayPlanNum if there is a payplan with matching PatNum, PlanNum, and InsSubNum that has not been paid in full.
		//By sending in ClaimNum, we ensure that we only get the payplan a claimproc from this claim was already attached to or payplans with no claimprocs attached.
		var listPayPlans=PayPlans.GetValidInsPayPlans(claimProc.PatNum,claimProc.PlanNum,claimProc.InsSubNum,claimProc.ClaimNum);
		...
		List<ClaimProcHist> listClaimProcHistsLoop=null;
		using var formClaimProc=new FormClaimProc(claimProc,null,_family,_patient,_listInsPlans,null,ref listClaimProcHistsLoop,_listPatPlans,true,_listInsSubs);
		formClaimProc.IsInClaim=true;
	...
	private void butPayProc_Click(object sender, System.EventArgs e) {
		...
		if(gridProc.SelectedIndices.Length==0){
			...
			ODMessageBox.Show(Lan.g(this,"All procedures in the list have already been paid."));
			return;
		}
		var areAllProcs=true;
		for(var i=0;i<gridProc.SelectedIndices.Length;i++){
			if(_listClaimProcsForClaim[gridProc.SelectedIndices[i]].ProcNum==0)
				areAllProcs=false;
		}
		if(!areAllProcs){
			ODMessageBox.Show(Lan.g(this,"You can only select procedures."));
			return;
		}
		for(var i=0;i<gridProc.SelectedIndices.Length;i++) {
			if(_listClaimProcsForClaim[gridProc.SelectedIndices[i]].ClaimPaymentNum!=0) {//if attached to a check
				ODMessageBox.Show(Lan.g(this,"Procedures that are attached to checks cannot be included."));
				return;
			}
		}
		for(var i=0;i<gridProc.SelectedIndices.Length;i++) {
			...
			{
				ODMessageBox.Show(Lan.g(this,"Procedures that are already received cannot be included."));
		...
		if(DoConsolidateOrthoPayments()) {
			var listIntOrthoAutoGridRows = new List<int>();
			var insPlan = InsPlans.GetPlan(_claim.PlanNum,_listInsPlans);
			var orthoAutoCodeNum = InsPlans.GetOrthoAutoProc(insPlan);
			for(var i = 0;i<gridProc.SelectedIndices.Length;i++) {
				//if all the procedures on this claim are ortho auto procedures...
			...
			if(listIntOrthoAutoGridRows.Count>0) {
				...
				var claimBanding =Claims.GetOrthoBandingClaim(_patient.PatNum,insPlan.PlanNum);
				if(claimBanding==null) {
					...
					using var formClaimEdit=new FormClaimEdit(claimBanding,_patient,_family,true);
				...
				for(var i=0;i<listIntOrthoAutoGridRows.Count;i++) {
		...
		for(var i=0;i<gridProc.SelectedIndices.Length;i++) {
			...
			if(_claim.ClaimType=="PreAuth"){
				...
				if(i==0) {
					//Automatically set PayPlanNum if there is a payplan with matching PatNum, PlanNum, and InsSubNum that has not been paid in full.
					//By sending in ClaimNum, we ensure that we only get the payplan a claimproc from this claim was already attached to or payplans with no claimprocs attached.
					var listPayPlans=PayPlans.GetValidInsPayPlans(listClaimProcs[i].PatNum,listClaimProcs[i].PlanNum,listClaimProcs[i].InsSubNum,listClaimProcs[i].ClaimNum);
					if(listPayPlans.Count==1) {
		...
		if(listClaimProcs.Count > 0) {
			if(_claim.ClaimType=="PreAuth") {
				using var formClaimPayPreAuth=new FormClaimPayPreAuth(_patient,_family,_listInsPlans,_listPatPlans,_listInsSubs);
				...
				//save changes now
				for(var i=0;i<formClaimPayPreAuth.ListClaimProcs.Count;i++) {
				...
				using var formClaimPayTotal=new FormClaimPayTotal(_patient,_family,_listInsPlans,_listPatPlans,_listInsSubs,GetBlueBookEstimateData());
	...
	private void butPaySupp_Click(object sender, System.EventArgs e) {
		if(DoConsolidateOrthoPayments()) {
			var insPlanCur=InsPlans.GetPlan(_claim.PlanNum,_listInsPlans);
			var orthoAutoCodeNum=InsPlans.GetOrthoAutoProc(insPlanCur);
			//if all the procedures on this claim are ortho auto procedures...
			if(_listClaimProcsForClaim.All(x => Procedures.GetProcFromList(_listProcedures,x.ProcNum).CodeNum == orthoAutoCodeNum)) {
	...
	private void menuItemAddAdj_Click(object sender,EventArgs e) {
		...
		var procNum=((ClaimProc)gridProc.SelectedGridRows[0].Tag).ProcNum;
		var procedureForAdj=new Procedure();
	...
	private void MakeSuppPayment() {
		...
		if(gridProc.SelectedIndices.Length==0){
			ODMessageBox.Show(Lan.g(this,"This is only for additional payments on procedures already marked received.  Please highlight procedures first."));
			return;
		}
		var areAllRecd=true;
		var hasClickedTotalPayment=false;
		for(var i=0;i<gridProc.SelectedIndices.Length;i++){
			if(_listClaimProcsForClaim[gridProc.SelectedIndices[i]].Status!=ClaimProcStatus.Received) {
				areAllRecd=false;
			}
		...
		if(!areAllRecd){
			ODMessageBox.Show(Lan.g(this,"All selected procedures must be status received."));
			return;
		}
		if(hasClickedTotalPayment) {
			ODMessageBox.Show(Lan.g(this,"Select the procedures that you want to enter a supplemental payment for.  If you want to make a supplemental "
			                             +"payment on a payment previously entered as total, click 'As Total' again."));
			return;
		}
		var listClaimProcsSelected=gridProc.SelectedIndices.Select(x => _listClaimProcsForClaim[x]).ToList();
		var listClaimProcsSupplemental=ClaimProcs.CreateSuppClaimProcs(listClaimProcsSelected);
		using var formClaimPayTotal=new FormClaimPayTotal(_patient,_family,_listInsPlans,_listPatPlans,_listInsSubs,GetBlueBookEstimateData());
...
//fix: need to debug the recalculation feature to take this status into account.
	...
	private void butSplit_Click(object sender, System.EventArgs e) {
		...
		UpdateClaim();
		if(gridProc.SelectedIndices.Length==0){
			ODMessageBox.Show(Lan.g(this,"Please highlight procedures first."));
			return;
		}
		var listSelectedProcNums=new List<long>();
		for(var i=0;i<gridProc.SelectedIndices.Length;i++){
			if(_listClaimProcsForClaim[gridProc.SelectedIndices[i]].ProcNum==0){
				ODMessageBox.Show(Lan.g(this,"Only procedures can be selected."));
				return;
			}
			if(_listClaimProcsForClaim[gridProc.SelectedIndices[i]].InsPayAmt!=0){
				ODMessageBox.Show(Lan.g(this,"All selected procedures must have zero insurance payment amounts."));
				return;
			}
		...
		var listClaimProcsToSplit=gridProc.SelectedTags<ClaimProc>().ToList();
		var claimSplit=Claims.InsertSplitClaim(_claim,listClaimProcsToSplit);
		_listClaimProcs=ClaimProcs.Refresh(_patient.PatNum);
		var listClaimProcsOriginal=_listClaimProcs.Where(x => x.ClaimNum==_claim.ClaimNum).ToList();
		//Update the multi-visit groups
		for(var i=0;i<_listClaimProcs.Count;i++) {
			...
			var procedure=Procedures.GetOneProc(_listClaimProcs[i].ProcNum,false);
		...
		var claimStatus=_listClaimStatuses.Where(x=>x.GetDescription(useShortVersionIfAvailable:true)==_claim.ClaimStatus).FirstOrDefault();
		comboClaimStatus.SelectedIndex=_listClaimStatuses.IndexOf(claimStatus);
		FillGrids();
	}
	...
	private void butViewEob_Click(object sender,EventArgs e) {
		var listLongClaimPaymentNums=_listClaimProcsForClaim.FindAll(x => x.ClaimPaymentNum!=0).Select(x => x.ClaimPaymentNum).ToList();
		var listClaimPayments=ClaimPayments.GetClaimPaymentsWithEobAttach(listLongClaimPaymentNums);
	...
	private bool TrySelectClaimPayment(List<ClaimPayment> listClaimPayments,out ClaimPayment claimPayment) {
		claimPayment=null;
		var listGridColumnHeaders=new List<GridColumn> {
		...
		var listGridRowValues=new List<GridRow>();
		listClaimPayments.ForEach(x => {
			var row=new GridRow(x.CarrierName,x.CheckDate.ToShortDateString(),SOut.Double(x.CheckAmt));
			row.Tag=x;
			listGridRowValues.Add(row);
		});
		var formTitle=Lan.g(this,"EOB Picker");
		var gridTitle=Lan.g(this,"EOBs");
		using var formGridSelection=new FormGridSelection(listGridColumnHeaders,listGridRowValues,formTitle,gridTitle);
	...
	private void FinalizePayment(bool isThisClaimOnly) {
		...
		var listClaimProcs=ClaimProcs.AttachAllOutstandingToPayment(claimPayment.ClaimPaymentNum,PrefC.DateClaimReceivedAfter,onlyOneClaimNum);
		if(listClaimProcs.Count==0) {
		...
		var amt=listClaimProcs.Sum(x => x.InsPayAmt);
	...
	public static void FormFinalizePaymentHelper(ClaimPayment claimPayment,Claim claim,Patient patient,Family family,long onlyOneClaimNum=0) {
		...
		using var formClaimPayBatch=new FormClaimPayBatch(claimPayment,true);
		...
		if(formClaimPayBatch.DialogResult!=DialogResult.OK) {
			...
			var listEobAttaches=EobAttaches.Refresh(claimPayment.ClaimPaymentNum);
			for(var i=0;i<listEobAttaches.Count;i++) {
	...
	private void butReferralEdit_Click(object sender,EventArgs e) {
		...
		var referral=WpfControls.ReferralL.GetReferral(_claim.ReferringProv);
		if(referral==null) {
			textRefProv.Text="";
			_claim.ReferringProv=0;
			butReferralEdit.Enabled=false;
		...
		var frmReferralEdit=new FrmReferralEdit(referral);
	...
	private void FillAttachments(){
		if(CultureInfo.CurrentCulture.Name.EndsWith("CA")) {
			listCanadianAttachments.Items.Clear();
			for(var i=0;i<_claim.Attachments.Count;i++){
				listCanadianAttachments.Items.Add(_claim.Attachments[i].DisplayedFileName);
			}
		}
		else {
	...
	private void butAttachPerioHelper() {
		...
		using var bitmap=new Bitmap(602,665);
		var g=Graphics.FromImage(bitmap);
		contrPerioGrid.DrawChart(g);
		g.Dispose();
		using var bitmapBig=new Bitmap(602,715);
		g=Graphics.FromImage(bitmapBig);
		g.Clear(Color.White);
		var text=_patient.GetNameFL();
		...
		g.Dispose();
		var rnd=new Random();
		var newName=DateTime.Now.ToString("yyyyMMdd")+"_"+DateTime.Now.TimeOfDay.Ticks+rnd.Next(1000)+".jpg";
		var attachPath=EmailAttaches.GetAttachPath();
		var newPath=Path.Combine(attachPath,newName);
		try{
	...
	private void butExportHelper() {
		var claimAttachExportPath=PrefC.GetString(PrefName.ClaimAttachExportPath);
		if(!Directory.Exists(claimAttachExportPath)){
			if(ODMessageBox.Show(Lan.g(this,"The claim export path no longer exists at:")+" "+claimAttachExportPath+"\r\n"
			                     +Lan.g(this,"Would you like to create it?"),"", MessageBoxButtons.YesNo)==DialogResult.Yes) 
			{
				try {
					...
					ODMessageBox.Show(Lan.g(this,"The directory was unable to be created.  Try running as Administrator."));
					return;
				}
			}
			else {
		...
		for(var i=0;i<_claim.Attachments.Count;i++){
			var curAttachPath=Path.Combine(EmailAttaches.GetAttachPath(),_claim.Attachments[i].ActualFileName);
			var newFilePath=Path.Combine(claimAttachExportPath,
			...
			if(!File.Exists(curAttachPath)) {
				ODMessageBox.Show(Lan.g(this,"The attachment file")+" "+curAttachPath+" "+Lan.g(this,"has been moved, deleted or is inaccessible."));
				return;
			}
			try {
				...
				ODMessageBox.Show(Lan.g(this,"The attachment")+" "+curAttachPath+" "
	...
	private void butAttachAdd_Click(object sender,EventArgs e) {
		using var formImageSelectClaimAttach=new FormImageSelectClaimAttach();
		...
		if(textRadiographs.IsValid()) {
			var radiographs=SIn.Int(textRadiographs.Text);
			radiographs++;
	...
	private void butAttachPerio_Click(object sender,EventArgs e) {
		butAttachPerioHelper();
	}

	private void butCanadianAttachAdd_Click(object sender,EventArgs e) {
		using var formImageSelectClaimAttach=new FormImageSelectClaimAttach();
		formImageSelectClaimAttach.PatNum=_claim.PatNum;
	...
	private void butCanadianAttachPerio_Click(object sender,EventArgs e) {
		butAttachPerioHelper();
	}

	private void butCanadianExport_Click(object sender,EventArgs e) {
		butExportHelper();
	}
	...
	private void contextMenuAttachments_Popup(object sender,EventArgs e) {
		if(listAttachments.SelectedIndex==-1 && listCanadianAttachments.SelectedIndex==-1) {
	...
	private void menuItemOpen_Click(object sender,EventArgs e) {
		var selectedIndex=listAttachments.SelectedIndex;
		if(CultureInfo.CurrentCulture.Name.EndsWith("CA")) {
			selectedIndex=listCanadianAttachments.SelectedIndex;
		}
		FileAtoZ.OpenFile(Path.Combine(EmailAttaches.GetAttachPath(),_claim.Attachments[selectedIndex].ActualFileName),
			_claim.Attachments[selectedIndex].DisplayedFileName);
	}
	...
	private void menuItemRemove_Click(object sender,EventArgs e) {
		if(CultureInfo.CurrentCulture.Name.EndsWith("CA")) {
			_claim.Attachments.RemoveAt(listCanadianAttachments.SelectedIndex);
		}
		else {
	...
	private void listAttachments_DoubleClick(object sender,EventArgs e) {
		...
		FileAtoZ.OpenFile(Path.Combine(EmailAttaches.GetAttachPath(),_claim.Attachments[listAttachments.SelectedIndex].ActualFileName),
	...
	private void listAttachments_MouseDown(object sender,MouseEventArgs e) {
		...
		if(e.Button==MouseButtons.Right) {
			var clickedIndex=listAttachments.IndexFromPoint(e.Location);
			if(clickedIndex!=-1) {
	...
	private void listCanadianAttachments_DoubleClick(object sender,EventArgs e) {
		if(listCanadianAttachments.SelectedIndex==-1) {
			return;
		}
		FileAtoZ.OpenFile(Path.Combine(EmailAttaches.GetAttachPath(),_claim.Attachments[listCanadianAttachments.SelectedIndex].ActualFileName),
			_claim.Attachments[listCanadianAttachments.SelectedIndex].DisplayedFileName);
	}		

	private void listCanadianAttachments_MouseDown(object sender,MouseEventArgs e) {
		//A right click also needs to select an items so that the context menu will work properly.
		if(e.Button==MouseButtons.Right) {
			var clickedIndex=listCanadianAttachments.IndexFromPoint(e.Location);
			if(clickedIndex!=-1) {
				listCanadianAttachments.SelectedIndex=clickedIndex;
			}
		}
	}

	private void butMissingTeethHelp_Click(object sender,EventArgs e) {
		ODMessageBox.Show("As explained in the manual, extracted teeth are pulled from the procedure history.  Any extraction with a status of Complete, Existing Current, or Existing Other will be included.  But the extraction must also have a valid date.  So to add an extracted tooth to this list, go to the Chart module, and add an extraction with a status of EO and a date that is as accurate as possible.  Furthermore, extracted teeth will only show here if at least one of the fields for initial placement upper or lower is marked Yes.\r\n\r\nMissing teeth are not pulled from procedure history, but from the missing teeth tab of the Chart module.  Teeth can be marked missing without having an extraction date.");
	}
	...
	private void butLabel_Click(object sender, System.EventArgs e) {
		...
		var insPlanCur=InsPlans.GetPlan(_claim.PlanNum,_listInsPlans);
		var carrierCur=Carriers.GetCarrier(insPlanCur.CarrierNum);
	...
	private void butPreview_Click(object sender, System.EventArgs e) {
		...
		UpdateClaim();
		using var formClaimPrint=new FormClaimPrint();
	...
	private void ButPrint_Click(object sender,System.EventArgs e) {
		...
		var dateTimeClaimCurSectDateTEdit=_claim.SecDateTEdit;//Preserve the date prior to any claim updates effecting it.
		UpdateClaim();
		var formClaimPrint=new FormClaimPrint();
		formClaimPrint.PatNum=_claim.PatNum;
		formClaimPrint.ClaimNum=_claim.ClaimNum;
	...
	private void butSendCanadaAttachments_Click(object sender,EventArgs e) {
		...
		var dateTimeClaimCurSectEdit=_claim.SecDateTEdit;//Preserve the date prior to any claim updates effecting it.
		var claimSendQueueItemsArrayCA=Claims.GetQueueList(_claim.ClaimNum,_claim.ClinicNum,0);
		long clinicNum=0;
		if(true) {
			clinicNum=_claim.ClinicNum;
		}
		var clearinghouseHq=ClearinghouseL.GetClearinghouseHq(claimSendQueueItemsArrayCA[0].ClearinghouseNum);
		var clearinghouseClin=Clearinghouses.OverrideFields(clearinghouseHq,clinicNum);
		var insPlan=InsPlans.GetPlan(_claim.PlanNum,null);
		var insSub=InsSubs.GetOne(_claim.InsSubNum);
		Cursor=Cursors.WaitCursor;
		try {
			CanadianOutput.SendAttachments(clearinghouseClin,_patient.PatNum,insPlan,insSub,_claim);
			MsgBox.Show(this,"Done.");
		}
		catch(Exception ex) {
			ODMessageBox.Show(ex.Message);//No translation here, because the message should already be in the language the dental office has registered with the carrier.
			return;
		}
		finally {
	...
	private bool CheckProviderTerm() {
		var listLongInvalidProvNums=Providers.GetInvalidProvsByTermDate([comboProvBill.GetSelectedProvNum(), _provNumOrdering, comboProvTreat.GetSelectedProvNum()],SIn.DateTime(textDateService.Text));
	...
	private void SendClaim() {
		...
		if(CultureInfo.CurrentCulture.Name.EndsWith("CA") && _claim.ClaimType=="PreAuth") {
			for(var i=0;i<_listPatPlans.Count;i++) {
		...
		if(/* ODEnvironment.IsCloudServer */ false && Clearinghouses.IsDisabledForWeb(clearinghouseClin)) {
			ODMessageBox.Show(Lans.g("Eclaims","This clearinghouse is not available while using Open Dental Cloud."));
			return;
		}
		//string warnings;
		//string missingData=
		...
		if(claimSendQueueItemsArrayCA[0].MissingData!=""){
			ODMessageBox.Show(Lan.g(this,"Cannot send claim until missing/invalid data is fixed:")+"\r\n"+claimSendQueueItemsArrayCA[0].MissingData);
			return;
		}
		if(XConnect.IsEnabled(clearinghouseClin)) {
			...
			try {
				...
				if((xConnectWebResponse?.response?.claimStatus?.message?.Length??0)>0) {//Errors will go in the messages array of the object
					ODMessageBox.Show("XConnect Validation Failed: "+xConnectWebResponse.status.code+"\r\n"
					                  +Lan.g(this,"Cannot send claim until missing/invalid data is fixed:")+"\r\n"
					                  +string.Join("\r\n",xConnectWebResponse.response.claimStatus.message));
					return;
				}
				for(var i=0;i<xConnectWebResponse.response.claimItems.Count();i++) {
					var xConnectClaimItemResponse=xConnectWebResponse.response.claimItems[i];
				...
				MsgBox.Show("XConnect Validation Call Failed: "+ex.Message);
				return;//Allowing user to continue would defeat the purpose of validation before sending. The point is to save DXC phone calls regarding validation issues.
			}
		}
		else if(clearinghouseClin.IsAttachmentSendAllowed && clearinghouseClin.CommBridge==EclaimsCommBridge.ClaimConnect) {
			//Jordan 2024 This validation is just checking for required attachment.
			//I suspect that the 277 is completely inadequate to convey the complexity of the missing information.
			//So we are moving to a more modern custom validation instead of the X12 standards validation that is useless.
		...
		if(clearinghouseHq.Eformat==ElectronicClaimFormat.Canadian) {
			...
			var listClaimSendQueueItems=new List<ClaimSendQueueItem>();
			listClaimSendQueueItems.Add(claimSendQueueItemsArrayCA[0]);
			var enumClaimMedType=claimSendQueueItemsArrayCA[0].MedType;
			using var formClaimFormItemEdit=new FormClaimFormItemEdit();
	...
	private bool SendClaimCanadian(Clearinghouse clearinghouseClin,ClaimSendQueueItem claimSendQueueItem) {
		...
		if(!string.IsNullOrEmpty(claimSendQueueItem.Warnings)) {
			...
			if(ODMessageBox.Show(this,sb.ToString(),"",MessageBoxButtons.OKCancel)==DialogResult.Cancel) {
				return false;
			}
		}
		try {
			...
			using var msgBoxCopyPaste=new MsgBoxCopyPaste(ex.Message);
			...
			//The message is translated before thrown, so we do not need to translate here.
			//We show the message in a copy/paste window so that our techs and users can quickly copy the message and search for a solution.
			using var msgBoxCopyPaste=new MsgBoxCopyPaste(ex.ToString());
			msgBoxCopyPaste.ShowDialog();
			return false;
		}
		finally {
	...
	private bool HasIcd9Codes() {
		var listProceduresOnClaim=_listProcedures.FindAll(x => _listClaimProcsForClaim.Any(y => y.ProcNum==x.ProcNum));			
		if(Icd9s.HasIcd9Codes(listProceduresOnClaim)) {
			var msgText="There are ICD-9 codes attached to a procedure.  Would you like to send the claim without the ICD-9 codes? ";
	...
	private void butResend_Click(object sender,EventArgs e) {
		...
		using var formClaimResend=new FormClaimResend();
	...
	private void butHistory_Click(object sender,EventArgs e) {
		var listETrans=Etranss.GetHistoryOneClaim(_claim.ClaimNum);
	...
	private void butReverse_Click(object sender,EventArgs e) {
		...
		var etransAck=Etranss.GetEtrans(etransNumAck);
	...
	///<summary>This button has beeen removed from the UI to force users to use the right click attachment workflows. See the right click options in ControlAccount. Example: FormClaimAttachPasteDXC and FormClaimAttachHistory.</summary>
	...
	private void butEDSClaimAttachment_Click(object sender,EventArgs e) {
		var formClaimAttachment=new FormClaimAttachment(_claim,EclaimsCommBridge.EDS);
		OpenAttachmentForm(formClaimAttachment);
	}
	...
	private void OpenAttachmentForm(FormClaimAttachment formClaimAttachment) {
		...
		if(comboClaimStatus.SelectedIndex==_listClaimStatuses.IndexOf(ClaimStatus.WaitingToSend)) {//Waiting to send claim
			...
			var updateData=UpdateClaim();
			var claimSendQueueItemsArray=updateData.ListSendQueueItems;
	...
	private void EditOverpay(bool isOverpaid) {
		...
		using var formClaimOverpay=new FormClaimOverpay(_claim.ClaimNum,_listClaimProcsForClaim,_patient.PatNum,isOverpaid,selectedClaimProcNum);
	...
	public static bool AddClaimCustomTracking(Claim claim,string noteText="") {
		using var formClaimCustomTrackingUpdate=new FormClaimCustomTrackingUpdate(claim,noteText);
		formClaimCustomTrackingUpdate.ShowDialog();
		if(formClaimCustomTrackingUpdate.DialogResult!=DialogResult.OK) {
			return false;
		}
		var claimTracking=formClaimCustomTrackingUpdate.ListClaimTrackingsNew.FirstOrDefault();
		if(claimTracking==null){
			//No valid TrackingErrorDefNum was found, no selection/insert can be made
			return false; 
		}
		var trackingErrorDefNum=claimTracking.TrackingErrorDefNum;//A valid TrackingErrorDefNum was found and can be selected/inserted
	...
	private void gridStatusHistory_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		var claimTracking=(ClaimTracking)gridStatusHistory.ListGridRows[e.Row].Tag;
		if(Security.IsAuthorized(EnumPermType.ClaimHistoryEdit,claimTracking.DateTimeEntry)){
			using var formClaimCustomTrackingUpdate=new FormClaimCustomTrackingUpdate(_claim,claimTracking);
			formClaimCustomTrackingUpdate.ShowDialog();
			if(formClaimCustomTrackingUpdate.DialogResult==DialogResult.OK) {
	...
	private bool ClaimIsValid() {
		...
		var isSentOrReceived= comboClaimStatus.SelectedIndex.In(_listClaimStatuses.IndexOf(ClaimStatus.Sent),_listClaimStatuses.IndexOf(ClaimStatus.Received));
		var claimIsValidState=ClaimL.ClaimIsValid(textDateService.Text,_claim.ClaimType,isSentOrReceived,textDateSent.Text,_listClaimProcsForClaim
	...
	private bool CanadianWarnings(){
		...
		if(textDateInitialUpper.Text!="") {
			if(SIn.Date(textDateInitialUpper.Text)>DateTime.Today) {
				if(warning!="") {
					warning+="\r\n";
				}
				warning+="Initial max date must be in the past.";
			}
			if(SIn.Date(textDateInitialUpper.Text).Year<1900) {
				if(warning!="") {
					warning+="\r\n";
				}
		...
		if(textDateInitialLower.Text!=""){
			if(SIn.Date(textDateInitialLower.Text)>DateTime.Today){
				if(warning!=""){
					warning+="\r\n";
				}
				warning+="Initial mand date must be in the past.";
			}
			if(SIn.Date(textDateInitialLower.Text).Year<1900){
				if(warning!=""){
					warning+="\r\n";
				}
		...
		if(warning!=""){
			var dialogResult=ODMessageBox.Show("Warnings:\r\n"+warning+"\r\nDo you wish to continue anyway?","",
	...
	private ClaimEdit.UpdateData UpdateClaim(){
		...
		//patnum
		_claim.DateService=SIn.Date(textDateService.Text);
		if(textDateSent.Text==""){
			_claim.DateSent=DateTime.MinValue;
		}
		else{
			_claim.DateSent=SIn.Date(textDateSent.Text);
		}
		_claim.ClaimStatus=_listClaimStatuses[comboClaimStatus.SelectedIndex].GetDescription(true);
		var wasSentOrReceived= _claim.ClaimStatus.In(ClaimStatus.Sent.GetDescription(true),ClaimStatus.Received.GetDescription(true));
		...
		if(textDateRec.Text==""){
			...
			_claim.DateReceived=SIn.Date(textDateRec.Text);
		}
		//planNum
		...
		_claim.PriorDate=SIn.Date(textPriorDate.Text);
		...
		_claim.OrthoTotalM=SIn.Byte(textOrthoTotalM.Text);
		_claim.OrthoRemainM=SIn.Byte(textOrthoRemainM.Text);
		_claim.OrthoDate=SIn.Date(textOrthoDate.Text);
		...
		_claim.ShareOfCost=SIn.Double(textShareOfCost.Text);
		//attachments
		_claim.Radiographs=SIn.Byte(textRadiographs.Text);
		_claim.AttachedImages=SIn.Int(textAttachImages.Text);
		_claim.AttachedModels=SIn.Int(textAttachModels.Text);
		var listStringFlags=new List<string>();
		...
		if(!_clearinghouse.IsAttachmentSendAllowed) {
			...
			for(var i=0;i<listStringFlags.Count;i++){
				if(i>0){
					_claim.AttachedFlags+=",";
				}
		...
		if(CultureInfo.CurrentCulture.Name.EndsWith("CA")) {//Canadian. en-CA or fr-CA
			...
			_claim.AccidentDate=SIn.Date(textCanadianAccidentDate.Text);
			...
			_claim.CanadianDateInitialUpper=SIn.Date(textDateInitialUpper.Text);
			...
			_claim.CanadianDateInitialLower=SIn.Date(textDateInitialLower.Text);
			...
			_claim.AccidentDate=SIn.Date(textAccidentDate.Text);
			_claim.IsOrtho=checkIsOrtho.Checked;
		}
		...
		var listProceduresToUpdatePlaceOfService=new List<Procedure>();
		for(var i=0;i<_listClaimProcsForClaim.Count;i++) {
			var procedure=Procedures.GetProcFromList(_listProcedures,_listClaimProcsForClaim[i].ProcNum);
		...
		var updateData=ClaimEdit.UpdateClaim(_claim,_listClaimValCodeLogs,_claimCondCodeLog,listProceduresToUpdatePlaceOfService,_patient,
			wasSentOrReceived,PermissionClaimEdit);
		for(var i=0;i<listProceduresToUpdatePlaceOfService.Count;i++) {
	...
	private void GetValCodes(List<ClaimValCodeLog> listClaimValCodeLogs,int valCodeIdx,long claimNum,System.Windows.Forms.TextBox textBoxVCCode,System.Windows.Forms.TextBox textBoxVCAmount) {
		if(valCodeIdx<listClaimValCodeLogs.Count) {//update existing ClaimValCodeLog
			...
			listClaimValCodeLogs[valCodeIdx].ValAmount=SIn.Double(textBoxVCAmount.Text);
		}
		else if(SIn.Double(textBoxVCAmount.Text)>0 || textBoxVCCode.Text!="") {//add a new ClaimValCodeLog
			var claimValCodeLog=new ClaimValCodeLog();
			claimValCodeLog.ValCode=textBoxVCCode.Text;
			claimValCodeLog.ValAmount=SIn.Double(textBoxVCAmount.Text);
	...
	private void butDelete_Click(object sender, System.EventArgs e) {
		...
		var dateTimeClaimCurSectEdit=_claim.SecDateTEdit;//Preserve the date prior to any claim updates effecting it.
		UpdateClaim();
		var isPaymentAttached=false;
		var isClaimInsPayAttached=false;
		...
		if(_claim.ClaimStatus==ClaimStatus.Received.GetDescription(true)){//received
			ODMessageBox.Show(Lan.g(this,"You cannot delete this claim while status is Received.  You will have to change the status first."));
			return;
		}
		var listETrans835Attaches=Etrans835Attaches.GetForClaimNums(_claim.ClaimNum).Select(x => x.Etrans835AttachNum).ToList();
		if(_claim.ClaimType=="PreAuth"){
			var msgAttaches="";
			...
			if(ODMessageBox.Show(msgAttaches+Lan.g(this,"Delete PreAuthorization?"),"",MessageBoxButtons.OKCancel)!=DialogResult.OK){
				return;
			}
		}
		else{
			...
			if(CultureInfo.CurrentCulture.Name.EndsWith("CA")) {//Canadian. en-CA or fr-CA
				if(ODMessageBox.Show(msgAttaches
				...
				if(ODMessageBox.Show(msgAttaches+Lan.g(this,"Delete Claim?"),"",MessageBoxButtons.OKCancel)!=DialogResult.OK){
					return;
				}
			}
		}
	...
	private void DeleteClaimHelper(List<long> list835Attaches=null) {
		   ...
		   || _claim.ClaimType=="Cap") //all cap claimprocs are just duplicates
		{
			...
			for(var i=0;i<_listClaimProcsForClaim.Count;i++) {
				...
				var procedure=Procedures.GetProcFromList(_listProcedures,_listClaimProcsForClaim[i].ProcNum);
				var listClaimProcsForLab=GetListClaimProcsForProcNumLabsParent(procedure.ProcNum);//Get any potential Claimprocs for the Proc "i"
				if(listClaimProcsForLab.Count!=0) {//And proc "i" has labs. 
					for(var j=0;j<listClaimProcsForLab.Count;j++) {//Foreach lab
				...
				var patPlan=_listPatPlans.FirstOrDefault(x => x.InsSubNum==_listClaimProcsForClaim[i].InsSubNum);
				if(patPlan==null) {
					continue;
				}
				if(patPlan.Ordinal==1) {
					...
					var paidOtherInsTotal=ClaimProcs.GetPaidOtherInsTotal(_listClaimProcsForClaim[i],_listPatPlans);
					var writeOffOtherIns=ClaimProcs.GetWriteOffOtherIns(_listClaimProcsForClaim[i],_listPatPlans);
				...
				var insBlueBookLog=GetBlueBookEstimateData().CreateInsBlueBookLog(_listClaimProcsForClaim[i]);
	...
	private void MarkAllReceived() {
		for(var i = 0;i<_listClaimProcsForClaim.Count;i++) {
			...
			var listClaimProcsForLab=GetListClaimProcsForProcNumLabsParent(_listClaimProcsForClaim[i].ProcNum);
			for(var j=0;j < listClaimProcsForLab.Count;j++) {
	...
	private BlueBookEstimateData GetBlueBookEstimateData() {
		if(_blueBookEstimateData==null) {
			var listProcedures=_listProcedures.FindAll(x => _listClaimProcsForClaim.Any(y => y.ProcNum==x.ProcNum));
	...
	private bool ReceiveAsNoPaymentIfNeeded() {
		...
		var claim=Claims.GetClaim(_claim.ClaimNum);
	...
	private void butSave_Click(object sender, System.EventArgs e) {
		...
		if(comboClaimStatus.SelectedIndex==_listClaimStatuses.IndexOf(ClaimStatus.WaitingToSend)){//waiting to send
			...
			if(!string.IsNullOrEmpty(claimSendQueueItemsArray[0].ErrorsPreventingSave)) {
				ODMessageBox.Show(claimSendQueueItemsArray[0].ErrorsPreventingSave);
				return;
			}
			else if(claimSendQueueItemsArray[0].MissingData!="") {
				if(ODMessageBox.Show(Lan.g(this,"Cannot send claim until missing/invalid data is fixed:")+"\r\n"+claimSendQueueItemsArray[0].MissingData+"\r\n\r\nContinue anyway?",
			...
			if(XConnect.IsEnabled(clearinghouseClin)) {
				...
				try {
					xConnectWebResponse=XConnect.ValidateClaim(_claim);
					if((xConnectWebResponse?.response?.claimStatus?.message?.Length??0)>0) {//Errors will go in the messages array of the object
						if(ODMessageBox.Show("XConnect Validation Failed: "+xConnectWebResponse.status.code+"\r\n"
						                     +Lan.g(this,"Cannot send claim until missing/invalid data is fixed:")+"\r\n"
						                     +string.Join("\r\n",xConnectWebResponse.response.claimStatus.message+"\r\n"
						                                                                                         +"Continue anyway?"),
							   "",MessageBoxButtons.OKCancel)==DialogResult.OK)
	...
	private void SaveCleanup(){
		var dateTimeClaimSecEdit=_claim.SecDateTEdit;//Preserve the date prior to any claim updates effecting it.
		if(_isDeleting) {
		...
		var receivedAsNoPay=ReceiveAsNoPaymentIfNeeded();
		...
		if(comboClaimStatus.SelectedIndex==_listClaimStatuses.IndexOf(ClaimStatus.Received) && _isPaymentEntered) {
			...
			if(_claim.ClaimType!="PreAuth" && Security.IsAuthorized(EnumPermType.ClaimSend,true)) {
				...
				if(PrefC.GetBool(PrefName.ClaimMedReceivedPromptForPrimaryClaim) && _claim.MedType.In(EnumClaimMedType.Medical,EnumClaimMedType.Institutional)) {
					...
					ClaimL.PromptForPrimaryOrSecondaryClaim(_listClaimProcsForClaim);//Check for unsent secondary claimprocs.
				}
			}
			...
			if(PrefC.GetBool(PrefName.ClaimPrimaryReceivedRecalcSecondary) && !receivedAsNoPay && _claim.ClaimType=="P") {
				var listClaims=Claims.GetPrimaryOrSecondaryClaimsNotReceived(_listClaimProcsForClaim);
				var listBenefits=Benefits.GetAllForPatPlans(_listPatPlans,_listInsSubs);
				for(var i=0;i<listClaims.Count;i++) {
					var listProcedures=Procedures.GetProcsForClaimNum(listClaims[i].ClaimNum);
	...
	private void FormClaimEdit_FormClosing(object sender, CancelEventArgs e) {
		...
		var claimFromDb=Claims.GetClaim(_claim.ClaimNum);
		if(claimFromDb==null) {
		...
		if(PrefC.GetBool(PrefName.ClaimSnapshotEnabled)
		   && SIn.Enum<ClaimSnapshotTrigger>(PrefC.GetString(PrefName.ClaimSnapshotTriggerType),true)!=ClaimSnapshotTrigger.Service)
		{
