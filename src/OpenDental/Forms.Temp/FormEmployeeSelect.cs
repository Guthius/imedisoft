...
public partial class FormEmployeeSelect : FormODBase {
	...
	private void FillGrid(){
		...
		_listEmployeesShowing= [];
		for(var i=0;i<_listEmployeesFull.Count;i++){
			if(textSearch.Text!=""){
		...
		var col=new GridColumn(Lan.g("FormEmployeeSelect","FName"),70);
		...
		for(var i=0;i<_listEmployeesShowing.Count;i++){
			...
			row.Cells.Add(_listEmployeesShowing[i].IsHidden?"X":"");
			var employee=Employees.GetEmp(_listEmployeesShowing[i].ReportsTo);
			if(employee is null){
				row.Cells.Add("");
			}
			else{
	...
	private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		var employeeNum=_listEmployeesShowing[e.Row].EmployeeNum;
		using var formEmployeeEdit=new FormEmployeeEdit();
		...
		_isChanged=true;
		for(var i=0;i<_listEmployeesShowing.Count;i++){
			if(_listEmployeesShowing[i].EmployeeNum==employeeNum){
