...
public partial class FormProcCodes:FormODBase {
	...
	public List<ProcedureCode> ListProcedureCodesSelected= [];
	...
	///<summary>Used for the fee schedule note context menu to tell which col the user clicked on.</summary>
	...
	private void FormProcCodes_Load(object sender,System.EventArgs e) {
		...
		var isShowingGroups=PrefC.GetBool(PrefName.ShowFeeSchedGroups);
		...
		_needsSynch=false;
		for(var i=0;i<Enum.GetNames(typeof(ProcCodeListSort)).Length;i++) {
		...
		var menuItemFeeSchedNotes=new MenuItem();
		menuItemFeeSchedNotes.Click+=MenuItem_Click;
		menuItemFeeSchedNotes.Text="Fee Schedule Notes";
		contextMenu.MenuItems.Add(menuItemFeeSchedNotes);
	...
	private void FillComboBoxes() {
		...
		var comboProv1Idx=comboProvider1.SelectedIndex;
		var comboProv2Idx=comboProvider2.SelectedIndex;
		var comboProv3Idx=comboProvider3.SelectedIndex;
		var feeSchedGroupNum1Selected=comboFeeSchedGroup1.GetSelected<FeeSchedGroup>()?.FeeSchedGroupNum??0;
		var feeSchedGroupNum2Selected=comboFeeSchedGroup2.GetSelected<FeeSchedGroup>()?.FeeSchedGroupNum??0;
		var feeSchedGroupNum3Selected=comboFeeSchedGroup3.GetSelected<FeeSchedGroup>()?.FeeSchedGroupNum??0;
		...
		comboFeeSched1.Items.AddList(_listFeeScheds,x => x.Description+(x.FeeSchedType==FeeScheduleType.Normal?"":" ("+x.FeeSchedType+")"));
		comboFeeSched1.SetSelectedKey<FeeSched>(feeSchedNum1Selected,x => x.FeeSchedNum);
		comboFeeSched2.Items.AddList(_listFeeScheds,x => x.Description+(x.FeeSchedType==FeeScheduleType.Normal?"":" ("+x.FeeSchedType+")"));
		comboFeeSched2.SetSelectedKey<FeeSched>(feeSchedNum2Selected,x => x.FeeSchedNum);
		comboFeeSched3.Items.AddList(_listFeeScheds,x => x.Description+(x.FeeSchedType==FeeScheduleType.Normal?"":" ("+x.FeeSchedType+")"));
		...
		var defaultClinicText=(true ? "Default" : "None");
		...
		var stringArrayProvAbbrs=new[] { Lan.g(this,"None") }.Concat(_listProviders.Select(x => x.Abbr)).ToArray();
	...
	private void AddFeeSchedGroupToComboBox(FeeSchedGroup feeSchedGroup,UI.ComboBox comboBoxFeeSchedGroup,long feeSchedGroupNumSelected) {
		...
		if(!feeSchedGroup.ListClinicNumsAll.Exists(x => _listClinics.Exists(y => y.Id==x))){ 
	...
	private void SynchAndFillListFees(bool doIncludeSynch){
		...
		var listClinicNums3=new List<long> { 0 };
		if(true) { //Clinics is on
			listClinicNums1= [comboClinic1.ClinicNumSelected];
			listClinicNums2= [comboClinic2.ClinicNumSelected];
			listClinicNums3= [comboClinic3.ClinicNumSelected];
			if(PrefC.GetBool(PrefName.ShowFeeSchedGroups)) {
				//First groupbox
	...
	private void SaveLogs() {
		for(var i=0;i<_listSecurityLogs.Count;i++) {
	...
	private void FillCats() {
		var listDefNums=new List<long>();
		for(var i=0;i<listCategories.SelectedIndices.Count;i++) {
		...
		for(var i=0;i<_defArrayCat.Length;i++) {
	...
	private void FillGrid() {
		...
		var listDefsCat=new List<Def>();
		for(var i=0;i<listCategories.SelectedIndices.Count;i++) {
			listDefsCat.Add(_defArrayCat[listCategories.SelectedIndices[i]]);
		}
		var feeSched1=_listFeeScheds[comboFeeSched1.SelectedIndex]; //First feesched will always have something selected.
		...
		var col=new GridColumn(Lan.g("TableProcedures","Category"),90);
		...
		gridMain.ListGridRows.Clear();
		GridRow row;
		var searchAbbr=textAbbreviation.Text;
		var searchDesc=textDescription.Text;
		var searchCode=textCode.Text;
		...
		if(_procCodeListSorts==ProcCodeListSort.Category) {
			for(var i=0;i<listDefsCat.Count;i++) {
		...
		for(var i=0;i<listProcedureCodes.Count;i++) {
			...
			var categoryName=Defs.GetName(DefCat.ProcCodeCats,listProcedureCodes[i].ProcCat);
			...
			var fee1=Fees.GetFee(listProcedureCodes[i].CodeNum,feeSched1.FeeSchedNum,clinic1Num,provider1Num,_listFees);
			Fee fee2=null;
			if(feeSched2!=null) {
	...
	private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		...
		var procedureCode=(ProcedureCode)gridMain.ListGridRows[e.Row].Tag;
		var defProcCat=Defs.GetDefsForCategory(DefCat.ProcCodeCats).FirstOrDefault(x => x.DefNum==procedureCode.ProcCat);
		using var formProcCodeEdit=new FormProcCodeEdit(procedureCode);
	...
	private void GridCellUpdateAndSecurityCheck(ODGridClickEventArgs e) {
		...
		var codeNum=((ProcedureCode)gridMain.ListGridRows[e.Row].Tag).CodeNum;
		...
		var feeAmtNewStr=gridMain.ListGridRows[e.Row].Cells[e.Col].Text;
		double feeAmtNew=0;
		...
		if(PrefC.GetBool(PrefName.ShowFeeSchedGroups) && provNum==0 && !isEditingGroup) {//Ignore provider fees and don't block from editing a group.
			var feeSchedGroupForClinic=FeeSchedGroups.GetOneForFeeSchedAndClinic(feeSched.FeeSchedNum,clinicNum);
			if(feeSchedGroupForClinic!=null) {
				...
				MsgBox.Show(Lans.g("Fee Schedule")+": "+feeSched.Description+" "+Lans.g("for Clinic")+": "+(_listClinics.FirstOrDefault(x => x.Id==clinicNum)?.Abbr??"")+" "
				            +Lans.g("is part of Fee Schedule Group")+": "+feeSchedGroupForClinic.Description+". "+Lans.g("The fees must be edited at the group level."));
				return;
			}
		}
		if(feeAmtNewStr!="") {
		...
		if(feeSched.IsGlobal) { //Global fee schedules have only one fee so blindly insert/update the fee. There will be no more localized copy.
			...
			if(feeAmtNewStr=="") { //They want to delete the fee
				...
				{ 
					...
					{
						for(var i=0;i<feeSchedGroupSelected.ListClinicNumsAll.Count;i++){
							if(feeSchedGroupSelected.ListClinicNumsAll[i]==clinicNum) {
								continue;
							}
							var listFees=_listFees.FindAll(x => x.FeeSched==feeSched.FeeSchedNum && x.CodeNum==codeNum && x.ClinicNum==feeSchedGroupSelected.ListClinicNumsAll[i] && x.ProvNum==provNum);
							...
							for(var j=0;j<listFees.Count;j++){
			...
			//The fee did not previously exist, or the fee found isn't for the currently set settings.
				   ...
				   && feeSchedGroupSelected.ListClinicNumsAll.Any(x => x!=clinicNum))
				{
					for(var i=0;i<feeSchedGroupSelected.ListClinicNumsAll.Count;i++){
						...
						var listFees=_listFees.FindAll(x => x.FeeSched==feeSched.FeeSchedNum && x.CodeNum==codeNum && x.ClinicNum==feeSchedGroupSelected.ListClinicNumsAll[i] && fee.ProvNum==provNum);
						if(listFees.IsNullOrEmpty()) {
							var feeAdd=new Fee();
							feeAdd.FeeSched=feeSched.FeeSchedNum;
							feeAdd.CodeNum=codeNum;
		...
		var securityLog=SecurityLogs.MakeLogEntryNoInsert(EnumPermType.ProcFeeEdit,0,"Procedure: "+ProcedureCodes.GetStringProcCode(fee.CodeNum)
	...
	private void MenuItem_Click(object sender,EventArgs e) {
		if(gridMain.GetSelectedIndex()<0){
			MsgBox.Show("Please select a procedure");
			return;
		}
		var col=gridMain.PointToCol(_pointLastClicked.X);
		if(col!=4 && col!=5 && col!=6){
			MsgBox.Show("Please select a fee column");
			return;
		}
		//Now we are guaranteed to be within the fee schedule columns
		var frmFeeSchedNotes =new FrmFeeSchedNotes();
		if(col==4){//Fee col 1
			frmFeeSchedNotes.FeeSchedCur=_listFeeScheds[comboFeeSched1.SelectedIndex];
			if(checkGroups1.Checked){//Using fee schedule groups
				frmFeeSchedNotes.ClinicNum=0;//All clinics
			}
			else{
				frmFeeSchedNotes.ClinicNum=comboClinic1.GetSelectedClinic().Id;
			}
		}
		if(col==5){//Fee col 2
			//ComboFeeSched2 and comboFeeSched3 have a dummy "None" option that we need to worry about
			var selectedIndex=comboFeeSched2.SelectedIndex-1;
			if(selectedIndex==-1){
				MsgBox.Show("Please select a fee schedule for this column first");
				return;
			}
			frmFeeSchedNotes.FeeSchedCur=_listFeeScheds[selectedIndex];
			if(checkGroups2.Checked){//Using fee schedule groups
				frmFeeSchedNotes.ClinicNum=0;//All clinics
			}
			else{
				frmFeeSchedNotes.ClinicNum=comboClinic2.GetSelectedClinic().Id;
			}
		}
		if(col==6){//Fee col 3
			//ComboFeeSched2 and comboFeeSched3 have a dummy "None" option that we need to worry about
			var selectedIndex=comboFeeSched3.SelectedIndex-1;
			if(selectedIndex==-1){
				MsgBox.Show("Please select a fee schedule for this column first");
				return;
			}
			frmFeeSchedNotes.FeeSchedCur=_listFeeScheds[selectedIndex];
			if(checkGroups3.Checked){//Using fee schedule groups
				frmFeeSchedNotes.ClinicNum=0;//All clinics
			}
			else{
				frmFeeSchedNotes.ClinicNum=comboClinic3.GetSelectedClinic().Id;
			}
		}
		frmFeeSchedNotes.ShowDialog();
	}
	...
	private void butEditCategories_Click(object sender,EventArgs e) {
		...
		using var formDefinitions=new FormDefinitions(DefCat.ProcCodeCats);
	...
	private void butEditFeeSched_Click(object sender,System.EventArgs e) {
		...
		using var formFeeScheds=new FormFeeScheds(false); //The Fee Scheds window can add or hide schedules.  It cannot delete schedules.
	...
	private void butTools_Click(object sender,System.EventArgs e) {
		...
		var schedNum=_listFeeScheds[comboFeeSched1.SelectedIndex].FeeSchedNum;
		using var formFeeSchedTools=new FormFeeSchedTools(schedNum,_listFeeScheds,_listProviders,_listClinics);
	...
	private void butExport_Click(object sender,EventArgs e) {
		...
		var listProcedureCodes=new List<ProcedureCode>();
		for(var i=0;i<gridMain.ListGridRows.Count;i++) {
			var procedureCode=(ProcedureCode)gridMain.ListGridRows[i].Tag;
		...
		var xmlSerializer=new XmlSerializer(typeof(List<ProcedureCode>));
	...
	public static int ImportProcCodes(string path,List<ProcedureCode> listProcedureCodes,string xmlData) {
		...
		if(path!="") {
			...
			var xmlDocumentNcodes=new XmlDocument();
			...
			var listXmlNodes=xmlDocumentNcodes.ChildNodes[1].ChildNodes.Cast<XmlNode>().ToList();
			for(var i=0;i<listXmlNodes.Count;i++){//loop through procedureCodes
				//foreach(XmlNode procNode in xmlDocumentNcodes.ChildNodes[1]){//1=ArrayOfProcedureCode
				var procCode="";
				var procCatDescript="";
				for(var j=0;j<listXmlNodes[i].ChildNodes.Count;j++){
	...
	private void butNew_Click(object sender,System.EventArgs e) {
		...
		using var formProcCodeNew=new FormProcCodeNew();
	...
	private void butPickFeeSched_Click(object sender,EventArgs e) {
		var selectedIndex=GetFeeSchedIndexFromPicker();
		...
		var buttonPicker=(UI.Button)sender;
	...
	private void butPickClinic_Click(object sender,EventArgs e){
		var listClinicsToShow=new List<ClinicDto>();
		var listFeeSchedGroupsToShow=new List<FeeSchedGroup>();
		var buttonPicker=(UI.Button)sender;
		var isPickingFeeSchedGroup=false;
	...
	private void butPickProvider_Click(object sender,EventArgs e){
		var selectedIndex=GetProviderIndexFromPicker()+1;//All provider combo boxes have a none option, so always add 1.
	...
	private long GetClinicNumFromPicker(List<ClinicDto> listClinics) {
		using var formClinics=new FormClinics();
		formClinics.IsSelectionMode=true;
		formClinics.Clinics=listClinics.ToList();
		formClinics.ShowDialog();
		return formClinics.SelectedClinicId;
	}
	...
	private int GetFeeSchedGroupIndexFromPicker(List<FeeSchedGroup> listFeeSchedGroups) {
		...
		for(var i = 0;i<listFeeSchedGroups.Count;i++) {
			var row=new GridRow(listFeeSchedGroups[i].Description);
