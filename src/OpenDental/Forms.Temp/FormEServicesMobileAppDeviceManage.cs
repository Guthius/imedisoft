...
public partial class FormEServicesMobileAppDeviceManage: FormODBase {
	...
	private void FillGridMobileAppDevices() {
		...
		var col=new GridColumn("Device Name",100){IsWidthDynamic=true };
		...
		var listMobileAppDevices=_listMobileAppDevicesAll;
		...
		for(var i=0;i<listMobileAppDevices.Count;i++) {
			var row=new GridRow();
			var mobileAppDevice=listMobileAppDevices[i];
			...
			if(_canEditEClipboard || _canEditODTouch) {
				...
				var gridCell=new GridCell("Delete");
	...
	private void SetUIEClipboardEnabled() {
		...
		var hasSetupPermission=Security.IsAuthorized(EnumPermType.EServicesSetup,true);
		var selectedClinicNum=GetClinicNumEClipboardTab();
		//Determine if the selected clinic is signed up for eClipboard.
		var isClinicSignedUpEClipboard=MobileAppDevices.IsClinicSignedUpForEClipboard(selectedClinicNum);
		//Determine if the selected clinic is signed up for ODTouch.
		var isClinicSignedUpODTouch=ClinicPrefs.IsOdTouchAllowed(selectedClinicNum);
		if(true && selectedClinicNum==0) {
			isClinicSignedUpEClipboard=Clinics.GetForUserod(Security.CurUser).Any(x => MobileAppDevices.IsClinicSignedUpForEClipboard(x.Id));
			isClinicSignedUpODTouch=Clinics.GetForUserod(Security.CurUser).Any(x => ClinicPrefs.IsOdTouchAllowed(x.Id));
		}
	...
	private void gridMobileAppDevices_CellClick(object sender,ODGridClickEventArgs e) {
		...
		var idxeClipboardColumn=gridMobileAppDevices.Columns.GetIndex("eClipboard");
		var idxODTouchColumn=gridMobileAppDevices.Columns.GetIndex("ODTouch");
		if(!e.Col.In(idxeClipboardColumn,idxODTouchColumn)) {//They did not select the right column.
			return;
		}
		var mobileAppDevice=gridMobileAppDevices.SelectedTag<MobileAppDevice>();
		...
		if(e.Col==idxODTouchColumn) {
			...
			var clinicNum=mobileAppDevice.ClinicNum;
			var deviceLimitForClinic=_clinicPrefHelper.GetIntVal(PrefName.ODTouchDeviceLimit,clinicNum);
			var deviceCountForClinic=_listMobileAppDevicesAll.Where(x=>x.ClinicNum==clinicNum && x.IsODTouchEnabled).Count();
			var previousDeviceCountForClinic=_listMobileAppDevicesOld.Where(x=>x.ClinicNum==clinicNum && x.IsODTouchEnabled).Count();
	...
	private void butSave_Click(object sender,EventArgs e) {
		...
		for(var i=0;i<_listMobileAppDevicesAll.Count;i++) {
			var mobileAppDeviceOld=_listMobileAppDevicesOld.Find(x => x.MobileAppDeviceNum==_listMobileAppDevicesAll[i].MobileAppDeviceNum);
