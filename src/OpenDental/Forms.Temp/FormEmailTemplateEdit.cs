...
public partial class FormEmailTemplateEdit : FormODBase {
	...
	private List<EmailAttach> _listEmailAttachesOld= [];
	...
	private void FillAttachments() {
		...
		var col=new GridColumn("",90); //No name column, since there is only one column.
		...
		if(_hasTextChanged) {
			...
			try {
				...
				if(_isRaw) {
					...
					htmlText=MarkupEdit.TranslateToXhtml(textBodyText.Text,isEmail:true,canAggregate:false);
