...
public partial class FormLogOn : FormODBase
{
    ...
    {
        var textSelectOnLoad = textPassword;
    ...
    private void FillListBox()
    {
        ...
        var listUserNames = Userods.GetUserNamesNoCache();
        for (var i = 0; i < listUserNames.Count; i++)
        {
        ...
        if (listUser.SelectedIndex == -1 && listUser.Items.Count > 0)
        {
    ...
    private void _dispatcherTimer_Tick(object sender, EventArgs e)
    {
        ...
        if (_keyboardInput.Length < 8)
        {
            //IDs will always come in as 8 digits from the reader no matter the badge ID length
            _keyboardInput = "";
            return;
        }

        var userod = Userods.GetUserByBadgeId(_keyboardInput); //Find the user
        if (userod == null)
        {
            //Kickout if no user was found
            _keyboardInput = "";
            return;
        }
    ...
    private void FormLogOn_KeyDown(object sender, KeyEventArgs e)
    {
        ...
        if (!isNumber)
        {
            //If obvious keyboard input clear the string
            _keyboardInput = "";
            return;
        }
    ...
    private void butOK_Click(object sender, EventArgs e)
    {
        ...
        var passwordTyped = textPassword.Text;
        Userod userod = null;
        if (isEcw)
        {
