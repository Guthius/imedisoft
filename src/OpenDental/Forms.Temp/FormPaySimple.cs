...
public partial class FormPaySimple:FormODBase {
	...
	private PaySimple.ApiResponse ProcessPayment(int expYear,int expMonth) {
		...
		if(_transType.In(PaySimple.TransType.SALE,PaySimple.TransType.RETURN,PaySimple.TransType.VOID)) {//Only print a receipt if transaction is an approved SALE, RETURN, or VOID			
			...
			var isVoidingRefund=SIn.Double(textAmount.Text)>0 && _transType==PaySimple.TransType.VOID;
	...
	private PaySimple.ApiResponse ProcessPaymentACH() {
		PaySimple.ApiResponse apiResponseRetVal=null;
		var accountNumber=textCheckSaveNumber.Text;
		//if the user has chosen to store CC tokens and the stored CC has a token and the token is not expired,
		//then use it instead of the CC number and CC expiration.
		...
		try {
			var result=WebServiceMainHQProxy.GetWebServiceMainHQInstance()
				.InsertPaySimpleACHId(PayloadHelper.CreatePayload(
					PayloadHelper.CreatePayloadContent(apiResponseRetVal.RefNumber,"PaymentId"),eServiceCode.PaySimple));
		...
		var source=_creditCard?.CCSource??CreditCardSource.PaySimpleACH;
	...
	private void PrintReceipt(string receiptStr) {
		...
		var fontXBody=MigraDocHelper.CreateFont(8,false);
	...
	private bool VerifyData(out int expYear,out int expMonth) {
		...
		var paytype=ProgramProperties.GetPropValForClinicOrDefault(_program.ProgramNum,PaySimple.PropertyDescs.PaySimplePayTypeCC,_clinicNum);
		...
		try {
			switch(_transType) {
				...
				case PaySimple.TransType.AUTH:
					...
					var paySimpleCustomerId=PaySimple.GetCustomerIdForPat(_patient.PatNum,_patient.FName,_patient.LName,_clinicNum);
				...
				case PaySimple.TransType.VOID:
					...
					break;
				default:
					throw new Exception("Invalid transmission type: "+_transType);
			}
		}
		catch(PaySimpleException ex) {
	...
	private void butSave_Click(object sender,EventArgs e) {
		...
		var isSuccess=(ApiResponseOut!=null);
		Cursor=Cursors.Default;
		if(isSuccess) {
