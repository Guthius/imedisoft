...
public partial class FormScheduleEdit : FormODBase	{
	...
	private void FormScheduleEdit_Load(object sender, System.EventArgs e) {
		...
		if(true) {
			if(ClinicNum==0) {
				...
				var abbr=Clinics.GetAbbr(ClinicNum);
	...
	private void butSave_Click(object sender, System.EventArgs e) {
		#region Validation
		var dateTStart=DateTime.MinValue;
		var dateTStop=DateTime.MinValue;
		if(!_isHolidayOrNote) {
			...
			var listSchedulesProvOnly=ListSchedules.FindAll(x=>x.SchedType==ScheduleType.Provider);
			var listOperatoryNumsSelected=new List<long>();
			if(listBoxOps.SelectedIndices.Contains(0)) {
				//not specified, so empty list
			}
			else {
				for(var i=0;i<listBoxOps.SelectedIndices.Count;i++) {
		...
		long clinicNum=0;
		if(_isHolidayOrNote && ScheduleCur.SchedType==ScheduleType.Practice && true) {//prov notes do not have a clinic
			clinicNum=comboClinic.ClinicNumSelected;
			if(ScheduleCur.Status==SchedStatus.Holiday) {//duplicate holiday check
				var listSchedules=ListSchedules.FindAll(x => x.SchedType==ScheduleType.Practice && x.Status==SchedStatus.Holiday);//scheds in local list
