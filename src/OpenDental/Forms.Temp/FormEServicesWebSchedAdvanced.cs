...
public partial class FormEServicesWebSchedAdvanced:FormODBase {
	...
	private void FormEServicesWebSchedAdvanced_Load(object sender,EventArgs e) {
		...
		_listUserControlHostedURLs= [];
		_listClinicPrefs= [];
		_listEserviceClinicNames=GetEserviceClinicNames();
		FillGridClinics();
		if(!true || _listEserviceClinicNames.Count()==1) {
	...
	private void AddClinicPrefToList(PrefName prefName,long clinicNum) {
		...
		var clinicPref=ClinicPrefs.GetPref(prefName,clinicNum);
		if(clinicPref!=null) {
	...
	private void LoadAndDisplayClinic(EServiceClinicName eserviceClinicName) {
		...
		var userControlHostedURL = (UserControlHostedURL)panelHostedURLs.Controls.Find("UserControlHostedURL",searchAllChildren: true).FirstOrDefault();
		if(userControlHostedURL==null) {//No controls displaying, so just display the current one.
			...
			var userControlHostedURLSaved = _listUserControlHostedURLs.FirstOrDefault(x => x.GetClinicNum()==eserviceClinicName.SignupOutEService.ClinicNum);
	...
	private void FillGridClinics() {
		...
		var gridColumn=new GridColumn("Clinic",200);
		...
		for(var i=0;i<_listEserviceClinicNames.Count;i++) {
			gridRow=new GridRow();
	...
	private ClinicPref GetClinicPrefToSave(long clinicNum,PrefName prefName,string value) {
		var clinicPref=_listClinicPrefs.FirstOrDefault(x => x.ClinicNum==clinicNum && x.PrefName==prefName)?.Clone();
		if(clinicPref==null) {
	...
	private void LayoutUserControlHostedURL(EServiceClinicName eserviceClinicName,bool doSave=false) {
		panelHostedURLs.Controls.Clear();
		var userControlHostedURL=new UserControlHostedURL(eserviceClinicName.SignupOutEService);
		if(doSave) {
			_listUserControlHostedURLs.Add(userControlHostedURL);
		}
		...
		userControlHostedURL.Controls.Add(panelHostedURLs);
	}
	...
	private void SaveWebSchedAdvanced() {
		...
		for(var i=0;i<_listUserControlHostedURLs.Count;i++) {
			var clinicNum=_listUserControlHostedURLs[i].GetClinicNum();
			var strAllowChildren=_listUserControlHostedURLs[i].GetPrefValue(PrefName.WebSchedNewPatAllowChildren);
			var strNewPatDoAuthEmail=_listUserControlHostedURLs[i].GetPrefValue(PrefName.WebSchedNewPatDoAuthEmail);
			var strNewPatDoAuthText=_listUserControlHostedURLs[i].GetPrefValue(PrefName.WebSchedNewPatDoAuthText);
			var strExistingPatDoAuthEmail=_listUserControlHostedURLs[i].GetPrefValue(PrefName.WebSchedExistingPatDoAuthEmail);
			var strExistingPatDoAuthText=_listUserControlHostedURLs[i].GetPrefValue(PrefName.WebSchedExistingPatDoAuthText);
			var webFormsURL=_listUserControlHostedURLs[i].GetPrefValue(PrefName.WebSchedNewPatWebFormsURL);
			var webFormsExistingPatURL=_listUserControlHostedURLs[i].GetPrefValue(PrefName.WebSchedExistingPatWebFormsURL);
			if(clinicNum==0) {
	...
	private void butSave_Click(object sender,EventArgs e) {
		...
		var userControlHostedURL = (UserControlHostedURL)panelHostedURLs.Controls.Find("UserControlHostedURL",true).FirstOrDefault();
