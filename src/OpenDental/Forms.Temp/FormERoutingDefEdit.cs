...
public partial class FormERoutingDefEdit:FormODBase {
	...
	public FormERoutingDefEdit(long clinicNum) {
		...
		_listERoutingActionDefs= [];
		_listERoutingActionDefsOld= [];
		_listERoutingDefLinks= [];
	}
	...
	private void FillActionsGrid() {
		var indexCur=gridERoutingActions.GetSelectedIndex();
		gridERoutingActions.BeginUpdate();
		gridERoutingActions.ListGridRows.Clear();
		gridERoutingActions.Columns.Clear();
		gridERoutingActions.Columns.Add(new GridColumn { Heading="Action Type", IsWidthDynamic=true });
		gridERoutingActions.Columns.Add(new GridColumn { Heading="Label Override", IsWidthDynamic=true});
		gridERoutingActions.Columns.Add(new GridColumn { Heading="Default", ColWidth=60, TextAlign=HorizontalAlignment.Center});
		_listERoutingActionDefs.ForEach(x => {
			gridERoutingActions.ListGridRows.Add(
				new UI.GridRow(x.ERoutingActionType.GetDescription(), x.LabelOverride, x.ForeignKey!=0 ? "X": "") { Tag=x }
			);
		});
	...
	private void FillLinkTypesGrid() {
		...
		_listERoutingDefLinks.ForEach(x => {
			var columnVal=x.ERoutingType.GetDescription();
			if(x.Fkey > 0) {
	...
	private void Save() {
		...
		if(_eRoutingDef.IsNew) {
			...
			var defNum=ERoutingDefs.Insert(_eRoutingDef);
			...
			for(var i=0;i<_listERoutingActionDefsOld.Count;i++) {
				var eRoutingActionDef=_listERoutingActionDefs.Find(x => x.ERoutingActionDefNum==_listERoutingActionDefsOld[i].ERoutingActionDefNum);
	...
	private void ShowERoutingActionDefEditHelper(ERoutingActionDef eRoutingActionDef){
		...
		var frmERoutingActionDefEdit=new FrmERoutingActionDefEdit(eRoutingActionDef);
	...
	private void butUp_Click(object sender,EventArgs e) {
		var eRoutingActionDef = gridERoutingActions.SelectedTag<ERoutingActionDef>();
	...
	private void butAddLinkType_Click(object sender,EventArgs e) {
		...
		var eRoutingDefLink = new ERoutingDefLink();
	...
	private void OpenFormBillingTypes() {
		var selectedDefs = Defs.GetDefs(DefCat.BillingTypes,_listERoutingDefLinks.Where(x => x.ERoutingType==EnumERoutingType.BillingType && x.Fkey>0).Select(x => x.Fkey).ToList());
		using var formDefPicker = new FormDefinitionPicker(DefCat.BillingTypes,selectedDefs);
	...
	private void OpenFormApptTypes() {
		...
		var listApptTypesSelected = _listERoutingDefLinks.Where(x => x.ERoutingType==EnumERoutingType.Appointment && x.Fkey>0).Select(y => y.Fkey).ToList();
		formApptTypes.SelectedAppointmentTypes = AppointmentTypes.GetWhere(x => listApptTypesSelected.Contains(x.AppointmentTypeNum));
		if(formApptTypes.ShowDialog() == DialogResult.OK) {
			formApptTypes.SelectedAppointmentTypes.ForEach(apptType => {
