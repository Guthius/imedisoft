...
public partial class FormPodiumSetup:FormODBase {
	...
	private void FormPodiumSetup_Load(object sender,EventArgs e) {
		...
		if(Security.CurUser.ClinicIsRestricted) {//user is clinic restricted and shouldn't have access to HQ
			listUserClinicNums=Clinics.GetForUserod(Security.CurUser).Select(x => x.Id).ToList();
		}
		else {
			listUserClinicNums=Clinics.GetForUserod(Security.CurUser,true).Select(x => x.Id).ToList();
		}
		...
		var listProgramPropertiesLocationID=ProgramProperties.GetForProgram(_program.ProgramNum).FindAll(x => x.PropertyDesc==Podium.PropertyDescs.LocationID);
		_listProgramPropertiesChanged= [];
		for(var i=0;i<listProgramPropertiesLocationID.Count;i++) {//If clinics is off, this will only grab the program property with a 0 clinic num (_listUserClinicNums will only have 0).
	...
	private void SetAdvertising() {
		...
		var programProperty=ProgramProperties.GetForProgram(_program.ProgramNum).FirstOrDefault(x => x.PropertyDesc=="Disable Advertising");
	...
	private void comboClinic_SelectionChangeCommitted(object sender,EventArgs e) {
		...
		var programProperty=_listProgramPropertiesChanged.Find(x => x.ClinicNum==_clinicNum);
		if(programProperty==null){
	...
	private void SaveClinicCurProgramPropertiesToList() {
		...
		if(_clinicNum==0) {
			...
			var locationIdOld=programProperty.PropertyValue;
			for(var i=0;i<_listProgramPropertiesChanged.Count;i++) {
		...
		var programPropertyLocationID=new ProgramProperty();
		programProperty=_listProgramPropertiesChanged.Find(x => x.ClinicNum==_clinicNum);
		...
		if(programPropertyLocationID.ClinicNum==0) {//No program property for current clinic, since _clinicNumCur!=0
			var programPropertyLocationIDNew=programPropertyLocationID.Copy();
	...
	private void UpsertProgramPropertiesForClinics() {
		var listProgramPropertyLocationIDsFromDb=ProgramProperties.GetForProgram(_program.ProgramNum).FindAll(x => x.PropertyDesc==Podium.PropertyDescs.LocationID);
		for(var i=0;i<_listProgramPropertiesChanged.Count;i++) {
