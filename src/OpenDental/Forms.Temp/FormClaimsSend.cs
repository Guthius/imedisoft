...
public partial class FormClaimsSend:FormODBase {
	...
	private void FormClaimsSend_Load(object sender, System.EventArgs e) {
		...
		var menuItemDeleteClaim=new MenuItem();
		...
		for(var i=0;i<_listClearinghouses.Count;i++){
		...
		if(_listDefsClaimCustomTracking.Count==0){
			...
			for(var i=0;i<_listDefsClaimCustomTracking.Count;i++) {
				comboCustomTracking.Items.Add(_listDefsClaimCustomTracking[i].ItemName);
			}
		}
		using var formClaimReports=new FormClaimReports(); //the currently selected clinic is what the combobox defaults to.
		...
		var listSelectedItems=new List<int>();
		for(var i=0;i<_listEtransTypes.Count;i++) {
	...
	public void FillClinicsList(long claimCustomTrackingNum) {//Not using ComboBoxClinicPicker due to adding number of unsent clinics to label of each item
		...
		var listClinicsPrevious=new List<ClinicDto>();
		if(comboClinic.GetListSelected<ClinicDto>().Count!=0) {//Only 0 the first time this method is run.
			listClinicsPrevious=comboClinic.GetListSelected<ClinicDto>();
		}
		comboClinic.Items.Clear();
		_listNumberOfClaims.Clear();
		if(!Security.CurUser.ClinicIsRestricted) {
			var clinicUnassigned=new ClinicDto();
			clinicUnassigned.Abbr=Lan.g(this,"Unassigned/Default");
			comboClinic.Items.Add(clinicUnassigned.Abbr,clinicUnassigned);
			comboClinic.SetSelectedKey<ClinicDto>(clinicUnassigned.Id,x=>x.Id,_=>clinicUnassigned.Abbr);
		}
		for(var i=0;i<_listClinics.Count;i++) {
			var countClaims=0;
			for(var j=0;j<_claimSendQueueItemArrayQueueAll.Length;j++) {
				if(_claimSendQueueItemArrayQueueAll[j].ClinicNum!=_listClinics[i].Id) {
					continue;
				}
			...
			var strClinicWithCount=_listClinics[i].Abbr+"  ("+countClaims+")";
			comboClinic.Items.Add(strClinicWithCount,_listClinics[i]);
			if(_listClinics[i].Id==Clinics.ClinicNum) {
				comboClinic.SetSelectedKey<ClinicDto>(Clinics.ClinicNum,x=>x.Id,_=>strClinicWithCount);
			}
		}
		var listSelectedIndices=new List<int>();
		var listClinicNumOptions=comboClinic.Items.GetAll<ClinicDto>().Select(x=>x.Id).ToList();
		for(var i=0;i<listClinicsPrevious.Count;i++) {
			var selectedClinicIndex=listClinicNumOptions.IndexOf(listClinicsPrevious[i].Id);
	...
	public void LayoutToolBars(){
		...
		ToolBarMain.Buttons.Add(new ODToolBarButton(ODToolBarButtonStyle.Separator));
		var button=new ODToolBarButton(Lan.g(this,"Send E-Claims"),4,Lan.g(this,"Send claims Electronically"),"Eclaims");
		button.Style=ODToolBarButtonStyle.DropDownButton;
	...
	private void MenuItemPopup(object sender,EventArgs e) {
		var menuItemDeleteClaim=gridMain.ContextMenu.MenuItems.OfType<MenuItem>().FirstOrDefault(x => x.Name=="menuItemDeleteClaim");
	...
	private void menuItemDeleteClaim_Click(object sender,EventArgs e) {
		var claimSendQueueItem=gridMain.SelectedTag<ClaimSendQueueItem>();
		if(claimSendQueueItem==null) {
			return;
		}
		if(claimSendQueueItem.ProcedureCodeString=="") {
			var claim=Claims.GetClaim(claimSendQueueItem.ClaimNum);
			var listClaimProcs=ClaimProcs.RefreshForClaim(claim.ClaimNum);
			...
			var listEtrans835AttachNums=Etrans835Attaches.GetForClaimNums(claim.ClaimNum).Select(x => x.Etrans835AttachNum).ToList();
	...
	private void FillGrid(bool rememberSelection=false,bool isRefreshRequired=true) {
		...
		var listOldSelectedClaimNums=new List<long>();
		if(rememberSelection) {
			oldScrollValue=gridMain.ScrollValue;
			for(var i=0;i<gridMain.SelectedTags<ClaimSendQueueItem>().Count;i++) {
				listOldSelectedClaimNums.Add(gridMain.SelectedTags<ClaimSendQueueItem>()[i].ClaimNum);
			}
		}
		if(isRefreshRequired) {
			var claimSendQueueItemArray=Claims.GetQueueList(0,0,0);//Get fresh new "all" list from db.
			for(var i=0;i<claimSendQueueItemArray.Length;i++) {
				...
				for(var j=0;j<_claimSendQueueItemArrayQueueAll.Length;j++) {//Go through the old list of all claims.
		...
		var claimSendQueueItemArrayFiltered=GetListQueueFiltered();//We update the class wide variable because it is used in double clicking and other events.
		gridMain.BeginUpdate();
		gridMain.Columns.Clear();
		var col=new GridColumn(Lan.g("TableQueue","DateService"),75,HorizontalAlignment.Center);//new column
		...
		GridRow row;
		for(var i=0;i<claimSendQueueItemArrayFiltered.Length;i++) {
			row=new GridRow();
			...
			if(!claimSendQueueItemArrayFiltered[i].CanSendElect) {
				...
				if(claimSendQueueItemArrayFiltered[i].MissingData.Contains(Eclaims.GetNoProceduresOnClaimMessage()) && !claimSendQueueItemArrayFiltered[i].MissingData.Contains("and delete this one")) {
					var deleteString=" and delete this one";
					var insertPosition=claimSendQueueItemArrayFiltered[i].MissingData.IndexOf(Eclaims.GetNoProceduresOnClaimMessage())+Eclaims.GetNoProceduresOnClaimMessage().Length;
	...
	private ClaimSendQueueItem[] GetListQueueFiltered() {
		...
		if(true) {
			listClinicNums=comboClinic.GetListSelected<ClinicDto>().Select(x => x.Id).ToList();
		}
		if(comboCustomTracking.SelectedIndex!=0) {
		...
		if(true) {//Filter by clinic only when clinics are enabled.
	...
	private void formProcNotBilled_GoToChanged(ODEventArgs e) {
		...
		GlobalFormOpenDental.GoToModule(EnumModuleType.Account,patNum:(long)e.Tag);
	}
	...
	private void toolBarButPrint_Click(){
		...
		if(gridMain.SelectedTags<ClaimSendQueueItem>().Count==0){
			for(var i=0;i<gridMain.ListGridRows.Count;i++) {
				var claimSendQueueItem=(ClaimSendQueueItem)gridMain.ListGridRows[i].Tag;
		...
		for(var i=0;i<gridMain.SelectedGridRows.Count;i++) {
			var claimSendQueueItem=(ClaimSendQueueItem)gridMain.SelectedGridRows[i].Tag;
	...
	private void toolBarButLabels_Click(){
		...
		for(var i=0;i<gridMain.SelectedGridRows.Count;i++) {
			...
			insPlan=InsPlans.GetPlan(claim.PlanNum, []);
	...
	private void RefreshAndValidateSelections() {
		var listClaimSendQueueItems=new List<ClaimSendQueueItem>();//List of claims needing to be validated.
		//List of claimNums to fetch new ClaimSendQuiteItems
		var listQueueClaimNums=gridMain.SelectedTags<ClaimSendQueueItem>().Select(x => x.ClaimNum).ToList();
		var claimSendQueueItemArrayRefreshItems=Claims.GetQueueList(listQueueClaimNums,0,0);
		var claimAlreadySentCount=0;
		for(var j=0;j<claimSendQueueItemArrayRefreshItems.Length;j++) {//Loop through all the refreshed ClaimSendQueueItems
			for(var k=0;k<_claimSendQueueItemArrayQueueAll.Length;k++) {//Loop through all the ClaimSendQueueItems in the grid's main list
	...
	private void SendEclaimsToClearinghouse(long hqClearinghouseNum) {
		...
		if(gridMain.SelectedTags<ClaimSendQueueItem>().Count==0){//if none are selected
			...
			if(hqClearinghouseNum!=0 && gridMain.SelectedTags<ClaimSendQueueItem>().Count==0) {
				...
				//	return;
				//}
				for(var i=0;i<gridMain.ListGridRows.Count;i++) {//loop through all filtered rows
					var queueItem=(ClaimSendQueueItem)gridMain.ListGridRows[i].Tag;
			...
			if(hqClearinghouseNum!=0) {//if they used the dropdown list to specify clearinghouse
				for(var i=0;i<gridMain.SelectedGridRows.Count;i++) {
					var claimSendQueueItem=(ClaimSendQueueItem)gridMain.SelectedGridRows[i].Tag;
					var cleainghouserRow=Clearinghouses.GetClearinghouse(claimSendQueueItem.ClearinghouseNum);
			...
			if(hqClearinghouseNum!=0) {//if they used the dropdown list to specify clearinghouse
				for(var i=0;i<gridMain.SelectedGridRows.Count;i++) {
					var claimSendQueueItem=(ClaimSendQueueItem)gridMain.SelectedGridRows[i].Tag;
					var clearinghouseRow=Clearinghouses.GetClearinghouse(claimSendQueueItem.ClearinghouseNum);
		...
		var listClaimSendQueueItemsSelected=gridMain.SelectedTags<ClaimSendQueueItem>();
		Cursor=Cursors.WaitCursor;
		var listClaimSendQueueItems=ClaimL.SendClaimSendQueueItems(listClaimSendQueueItemsSelected,hqClearinghouseNum);
		var listClaimNums=new List<long>();
		...
		var listClaimSendQueueItemsTemps=new List<ClaimSendQueueItem>(_claimSendQueueItemArrayQueueAll);
		for(var i=0;i<listClaimSendQueueItems.Count;i++) {
			if(listClaimSendQueueItems[i].ClaimStatus=="S") {
		...
		for(var i=0;i<listClaimSendQueueItems.Count;i++){
			for(var j=0;j<_tableHistory.Rows.Count;j++){
				var claimNum=SIn.Long(_tableHistory.Rows[j]["ClaimNum"].ToString());
	...
	private ClaimSendQueueItem ValidateProvidersTerms(ClaimSendQueueItem claimSendQueueItems,List<Claim> listClaims) {
		var claim=listClaims.Find(x => x.ClaimNum==claimSendQueueItems.ClaimNum);
		var listInvalidProvs=Providers.GetInvalidProvsByTermDate([claim.ProvBill, claim.ProvTreat, claim.ProvOrderOverride],claimSendQueueItems.DateService);
	...
	private void ValidateClaims(List<ClaimSendQueueItem> listClaimSendQueueItems) {
		...
		var listClaimSendQueueItemsToValidate=listClaimSendQueueItems.FindAll(x => !x.IsValid && x.CanSendElect);
		if(listClaimSendQueueItemsToValidate.Count==0) {
			return;
		}
		Cursor.Current=Cursors.WaitCursor;
		//Loop through and validate all claims.
		var clearinghouseHq=ClearinghouseL.GetClearinghouseHq(listClaimSendQueueItemsToValidate[0].ClearinghouseNum);
		...
		var listClaims=Claims.GetClaimsFromClaimNums(listClaimSendQueueItemsToValidate.Select(x => x.ClaimNum).ToList());
		for(var i=0;i<listClaimSendQueueItemsToValidate.Count;i++) {
			...
			if(listClaimSendQueueItemsToValidate[i].MissingData=="" && XConnect.IsEnabled(clearinghouseClin)) {
				var claim=listClaims.FirstOrDefault(x => x.ClaimNum==listClaimSendQueueItems[i].ClaimNum);
				...
				try {
					...
					for(var j=0;j<xConnectWebResponse.response.claimItems.Count();j++) {
						var xConnectClaimItemResponse=xConnectWebResponse.response.claimItems[j];
		...
		for(var i=0;i<_claimSendQueueItemArrayQueueAll.Length;i++) {
			var claimSendQueueItemValidated=listClaimSendQueueItemsToValidate.Find(x => x.ClaimNum==_claimSendQueueItemArrayQueueAll[i].ClaimNum);
	...
	private void toolBarButReports_Click() {
		using var formClaimReports=new FormClaimReports();
		formClaimReports.ShowDialog();
		FillHistory();//To show 277s after imported.
	}

	private void toolBarButOutstanding_Click() {
		using var formCanadaOutstandingTransactions=new FormCanadaOutstandingTransactions();
		formCanadaOutstandingTransactions.ShowDialog();
	}

	private void toolBarButPayRec_Click() {
		using var formCanadaPaymentReconciliation=new FormCanadaPaymentReconciliation();
		formCanadaPaymentReconciliation.ShowDialog();
	}

	private void toolBarButSummaryRec_Click() {
		using var formCanadaSummaryReconciliation=new FormCanadaSummaryReconciliation();
	...
	private void butNextUnsent_Click(object sender,EventArgs e) {
		var indexClinicOffset=0;
		//Check to see if Unassigned/Default is an option in the clinic combo box.
		if(comboClinic.Items.GetAll<ClinicDto>().Any(x => x.Id==0)) {
			//We want to skip Unassigned/Default so treat the selected indices as 1-based.
			indexClinicOffset=1;
		}
		var indexClinicNext=-1;
		var indexSelectedMax=comboClinic.SelectedIndices.Max();
		for(var i=0;i<_listNumberOfClaims.Count;i++) {
			var indexClinic=i+indexClinicOffset;
	...
	private void FillHistory(){
		...
		var dateTo=dateRangePicker.GetDateTimeTo();
		...
		var listSelectedEtransTypes=new List<EtransType>();
		for(var i=0;i<comboHistoryType.SelectedIndices.Count;i++) {//Some selected, add only those selected
			var selectedIdx=(int)comboHistoryType.SelectedIndices[i];
		...
		if(CultureInfo.CurrentCulture.Name.EndsWith("CA")) {//Canadian. en-CA or fr-CA
			...
			GridRow row;
			for(var i=0;i<_tableHistory.Rows.Count;i++) {
				row=new GridRow();
				...
				var user=Userods.GetUser(SIn.Long(_tableHistory.Rows[i]["UserNum"].ToString()));
	...
	private void panelSplitter_MouseMove(object sender,System.Windows.Forms.MouseEventArgs e) {
		if(!_isMouseDownOnSplitter)
			return;
		var splitterNewY=_ySplitterOriginal+(panelSplitter.Top+e.Y)-_yMouseOriginal;
		...
		panelHistory.Location=new Point(panelHistory.Location.X,panelSplitter.Bottom);
	...
	private void ToolBarHistory_ButtonClick(object sender,ODToolBarButtonClickEventArgs e) {
		switch(e.Button.Tag.ToString()){
			...
			case "OutstandingClaims":
				using(var formRpOutstandingIns=new FormRpOutstandingIns()) {
	...
	private void Undo_Click(){
		...
		if(gridHistory.SelectedIndices.Length>1){//if there are multiple items selected.
			...
			for(var i=0;i<gridHistory.SelectedIndices.Length;i++) {
	...
	private void menuItemHistoryGoToAccount_Click(object sender,EventArgs e) {
		...
		var row=_tableHistory.Rows[gridHistory.GetSelectedIndex()];			
	...
	private void gridHistory_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		Cursor=Cursors.WaitCursor;
		var etrans=Etranss.GetEtrans(SIn.Long(_tableHistory.Rows[e.Row]["EtransNum"].ToString()));
		if(etrans.Etype==EtransType.StatusNotify_277) {
			using var formEtrans277Edit=new FormEtrans277Edit();
		...
		var scroll=gridHistory.ScrollValue;
		FillHistory();
		for(var i=0;i<_tableHistory.Rows.Count;i++){
	...
	private void pd2_PrintPage(object sender,System.Drawing.Printing.PrintPageEventArgs e) {
		var rectangleBounds=new Rectangle(50,40,1035,800);//Some printers can handle up to 1042
		var g=e.Graphics;
		string text;
		...
		var centerPos=rectangleBounds.X+rectangleBounds.Width/2;
		#region printHeading
		...
		#endregion
		var rectangleOriginalBounds=gridHistory.Bounds; //Store the gridHistory bounds to revert after printing.
		gridHistory.Size=new Size(1035,gridHistory.Height);
		...
		gridHistory.Size=new Size(rectangleOriginalBounds.Width,rectangleOriginalBounds.Height);
		g.Dispose();
	}
