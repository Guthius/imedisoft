...
public partial class FormWebView:FormODBase {
		...
		if(UrlBrowseTo=="") {
			...
			try {
				if(PostData==null) {
					...
					var byteArrayPostData=Encoding.ASCII.GetBytes(PostData);
					using var memoryStream = new MemoryStream(byteArrayPostData);
					var coreWebView2WebResourceRequest=webViewMain.CoreWebView2.Environment.CreateWebResourceRequest(UrlBrowseTo,
	...
	public void LayoutToolBars() {
		...
		var contextMenuSettings = new ContextMenu();
		var menuItemClearCache = new MenuItem();
		...
		var toolBarButtonSettings = new ODToolBarButton(Lan.g(this,"Settings"),-1,"","Settings");
