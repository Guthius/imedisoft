...
public partial class FormProcButtonEdit:FormODBase {
	...
	private void FormChartProcedureEntryEdit_Load(object sender, System.EventArgs e) {
		...
		for(var i=0;i<_listDefsProcButtonCat.Count;i++){
		...
		var longArrayCodeNum=ProcButtonItems.GetCodeNumListForButton(_procButton.ProcButtonNum).ToArray();
		var longArrayAuto=ProcButtonItems.GetAutoListForButton(_procButton.ProcButtonNum).ToArray();
		listADA.Items.Clear();
		for(var i=0;i<longArrayCodeNum.Length;i++) {
		...
		for(var i=0;i<_listAutoCodesShortDeep.Count;i++) {
			listAutoCodes.Items.Add(_listAutoCodesShortDeep[i].Description);
			for(var j=0;j<longArrayAuto.Length;j++){
	...
	private void butUp_Click(object sender,EventArgs e) {
		...
		var selectedProcCode=listADA.GetStringSelectedItems();
		var movingProcCode=listADA.Items.GetTextShowingAt(listADA.SelectedIndex-1);
	...
	private void butSave_Click(object sender, System.EventArgs e) {
		...
		for(var i=0;i<listAutoCodes.SelectedIndices.Count;i++){
			var autoCode=_listAutoCodesShortDeep[listAutoCodes.SelectedIndices[i]];
		...
		for(var i=0;i<listADA.Items.Count;i++){
