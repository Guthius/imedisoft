...
public partial class FormPatientEdit : FormODBase {
	...
	public FormPatientEdit(){
		...
		listBoxEmps.Location=new Point(textEmployer.Left,textEmployer.Bottom);
		listBoxEmps.Font=new Font(this.Font.FontFamily,this.Font.Size);
		...
		listBoxCounties.Location=new Point(tabControlPatInfo.Left+tabPublicHealth.Left+textCounty.Left,
			tabControlPatInfo.Top+tabPublicHealth.Top+textCounty.Bottom);
		listBoxCounties.Font=new Font(this.Font.FontFamily,this.Font.Size);
		...
		listBoxSites.Location=new Point(tabControlPatInfo.Left+tabPublicHealth.Left+textSite.Left,
			tabControlPatInfo.Top+tabPublicHealth.Top+textSite.Bottom);
		listBoxSites.Font=new Font(this.Font.FontFamily,this.Font.Size);
		...
		listBoxMedicaidStates.Font=new Font(this.Font.FontFamily,this.Font.Size);
		...
		listBoxStates.Font=new Font(this.Font.FontFamily,this.Font.Size);
	...
	private void FormPatientEdit_Load(object sender, System.EventArgs e) {
		...
		for(var i=0;i<_listLanguages.Count;i++) {
			if(_listLanguages[i]=="") {
				continue;
			}
			var cultureInfo=CodeBase.MiscUtils.GetCultureFromThreeLetter(_listLanguages[i]);
		...
		var listFeeScheds=FeeScheds.GetDeepCopy(false);
		for(var i=0;i<listFeeScheds.Count;i++) {
			...
			comboFeeSched.Items.Add(listFeeScheds[i].Description+(listFeeScheds[i].IsHidden ? " "+Lans.g("(Hidden)") : ""),listFeeScheds[i]);
		...
		if(_isUsingNewRaceFeature) {
			...
			var listMultiRaces=Patients.GetMultiRaces();
			comboBoxMultiRace.Items.AddList(listMultiRaces,x => Lan.g("enumPatRace", x.ToString()));
			var listEthnicities=Patients.GetEthinicities();
			comboEthnicity.Items.AddList(listEthnicities,x => Lan.g(this, x.ToString()));
			var listPatientRaces=PatientRaces.GetForPatient(Patient.PatNum);
			comboEthnicity.SelectedIndex=0;//none
			for(var i=0;i<listPatientRaces.Count;i++) {
		...
		var stringArrayEnumPatientGrade=Enum.GetNames(typeof(PatientGrade));
		for(var i=0;i<stringArrayEnumPatientGrade.Length;i++){
		...
		var stringArrayEnumTreatmentUrgency=Enum.GetNames(typeof(TreatmentUrgency));
		for(var i=0;i<stringArrayEnumTreatmentUrgency.Length;i++){
		...
		var idx=1; //Starts at 1 to account for "None"
		comboExcludeECR.Items.Add("None");
		for(var i=0;i<Enum.GetValues(typeof(CommOptOutMode)).Length;i++) {
	...
	private void hideButViewBirthDate() {
		...
		label20.Location=new Point(label20.Location.X-(77),label20.Location.Y);
		textAge.Location=new Point(textAge.Location.X-(77),textAge.Location.Y);
	}

	private void butShortCodeOptIn_Click(object sender,EventArgs e) {
		var frmShortCodeOptIn=new FrmShortCodeOptIn();
	...
	private void FillShortCodes(YN txtMsgOk) {
		var patient=Patient.Copy();
		patient.TxtMsgOk=txtMsgOk;
		var patComm=Patients.GetPatComms(ListTools.FromSingle(patient)).FirstOrDefault();
		var isShortCodeInfoNeeded=patComm?.IsPatientShortCodeEligible(patient.ClinicNum)??false;
	...
	private void FillCombosProv() {
		var provNum=comboPriProv.GetSelectedProvNum();
		var listProviders=Providers.GetProvsForClinic(comboClinic.ClinicNumSelected);
	...
	private void SetRequiredFields() {
		...
		var areAnyConditionsMet=false;
		bool areConditionsMet;
		if(_listRequiredFields==null) {
			_listRequiredFields=RequiredFields.GetRequiredFields();
		}
		for(var i=0;i<_listRequiredFields.Count;i++) {
			...
			switch(_listRequiredFields[i].FieldName) {
				...
				case RequiredFieldName.AdmitDate:
					SetRequiredControl(labelAdmitDate,odDatePickerAdmitDate,areConditionsMet,-1, [],"Hospital admit date must be selected.");
					;						break;
				...
				case RequiredFieldName.Birthdate:
					SetRequiredControl(labelBirthdate,odDatePickerBirthDate,areConditionsMet,-1, [],"birthdate must be selected.");
					break;
				...
				case RequiredFieldName.DateFirstVisit:
					SetRequiredControl(labelDateFirstVisit,odDatePickerDateFirstVisit,areConditionsMet,-1, [],"Date First Visit must be selected.");
					break;
	...
	private bool ConditionsAreMet(RequiredField requiredField) {
		var listRequiredFieldConditions=requiredField.ListRequiredFieldConditions;
		if(listRequiredFieldConditions.Count==0) {//This RequiredField is always required
			return true;
		}
		var areConditionsMet=false;
		var previousFieldName=-1;
		for(var i=0;i<listRequiredFieldConditions.Count;i++) {
			...
			switch(listRequiredFieldConditions[i].ConditionType) {
				...
				case RequiredFieldName.Birthdate://But actually using Age for calculations						
					...
					var dateBirth=DateTime.MinValue;
					...
					var ageEntered=DateTime.Today.Year-dateBirth.Year;
					if(dateBirth>DateTime.Today.AddYears(-ageEntered)) {
						ageEntered--;
					}
					var listRequiredFieldConditionsAge=listRequiredFieldConditions.FindAll(x => x.ConditionType==RequiredFieldName.Birthdate);
					//There should be no more than 2 conditions of type Birthdate
					var listAreCondsMet=new List<bool>();
					for(var j=0;j<listRequiredFieldConditionsAge.Count;j++) {
	...
	private void SetRequiredTextBox(Label label,System.Windows.Forms.TextBox textBox,bool areConditionsMet) {
		SetRequiredControl(label,textBox,areConditionsMet,-1, [],"Text box cannot be blank.");
	}
	...
	private void SetRequiredComboBoxOD(Label label,UI.ComboBox comboBox,bool areConditionsMet,int disallowedIdx,string errorMsg) {
		SetRequiredControl(label,comboBox,areConditionsMet,disallowedIdx, [],errorMsg);
	}
	...
	private void ComboBox_SelectionChangeCommited(object sender,System.EventArgs e) {
		...
		var defBillingTypeSelected=comboBillType.GetSelected<Def>();
		var defNumClinicDefaultBillingType=SIn.Long(ClinicPrefs.GetPrefValue(PrefName.PracticeDefaultBillType,comboClinic.ClinicNumSelected));
		...
		if(IsNew && defBillingTypeSelected.DefNum!=defNumClinicDefaultBillingType) {
			var clinicDefaultBillingTypeName=_listDefsBillingType.Find(x => x.DefNum==defNumClinicDefaultBillingType)?.ItemName;
	...
	private void textSSN_Validating(object sender, System.ComponentModel.CancelEventArgs e) {
		...
		if(textSSN.Text.Length==9){//if just numbers, try to reformat.
			var isValidSSN=true;
			for(var i=0;i<textSSN.Text.Length;i++){
	...
	private void CalcAge() {
		...
		var dateBirth=odDatePickerBirthDate.GetDateTime();
	...
	private void comboZip_SelectionChangeCommitted(object sender, System.EventArgs e) {
		...
		var zipCodeSelected=comboZip.GetSelected<ZipCode>();
	...
	private void textZip_Validating(object sender, System.ComponentModel.CancelEventArgs e) {
		...
		if(listZipCodes.Count==0){
			...
			var frmZipSelect=new FrmZipSelect();
	...
	private void butShowMap_Click(object sender,EventArgs e) {
		...
		try
		{
			var mapUrl="http://maps.google.com/maps?t=m&q="+textAddress.Text+" "+textAddress2.Text+" "+textCity.Text+" "+textState.Text;
	...
	private void PromptForSmsIfNecessary(Patient patientNew,Patient patientOld) {
		...
		{
			var msg=PrefC.GetString(PrefName.ShortCodeOptInScript);
		...
		var message=PrefC.GetString(PrefName.ShortCodeOptedOutScript)
	...
	private void UpdateLocalNameHelper() {
		...
		for(var i=0;i<Family.ListPats.Length;i++) {//update the Family object as well
		...
		for(var i=0;i<_listGuardians.Count;i++) {
	...
	private void butPickResponsParty_Click(object sender,EventArgs e) {
		...
		var frmFamilyMemberSelect=new FrmFamilyMemberSelect(Family);
	...
	private void textEmployer_KeyUp(object sender, System.Windows.Forms.KeyEventArgs e) {
		...
		var h=13*listEmployersSimilar.Count+5;
	...
	private void textMedicaidState_KeyUp(object sender,System.Windows.Forms.KeyEventArgs e) {
		...
		var listStateAbbrsSimilar=StateAbbrs.GetSimilarAbbrs(textMedicaidState.Text);
		for(var i=0;i<listStateAbbrsSimilar.Count;i++) {
			listBoxMedicaidStates.Items.Add(listStateAbbrsSimilar[i].Abbr);
		}
		var h=13*listStateAbbrsSimilar.Count+5;
	...
	private void textState_KeyUp(object sender,System.Windows.Forms.KeyEventArgs e) {
		...
		var listStateAbbrsSimilar=StateAbbrs.GetSimilarAbbrs(textState.Text);
		for(var i=0;i<listStateAbbrsSimilar.Count;i++) {
	...
	private void listRelationships_DoubleClick(object sender,EventArgs e) {
		...
		UpdateLocalNameHelper();
		var frmGuardianEdit=new FrmGuardianEdit(_listGuardians[listRelationships.SelectedIndex],Family);
	...
	private void butAddGuardian_Click(object sender,EventArgs e) {
		...
		var frmGuardianEdit=new FrmGuardianEdit(guardian,Family);
		frmGuardianEdit.ShowDialog();
		if(frmGuardianEdit.IsDialogOK) {
	...
	private void butGuardianDefaults_Click(object sender,EventArgs e) {
		...
		var result=Patients.SetDefaultRelationships(Patient,Family,(PatientPosition)listPosition.SelectedIndex);
		if(result.IsFailure()) {
	...
	private void butRaceEthnicity_Click(object sender,EventArgs e) {
		var frmPatientRaceEthnicity=new FrmPatientRaceEthnicity();
	...
	private void butReferredFrom_Click(object sender,EventArgs e) {
		...
		if(MsgBox.Show(this,MsgBoxButtons.YesNo,"Is the referral source an existing patient?")) {
			...
			var isReferralNew=true;
			var referralMatch=Referrals.GetFirstOrDefault(x => x.PatNum==frmPatientSelect.PatNumSelected);
			if(referralMatch!=null) {
				referral=referralMatch;
				isReferralNew=false;
			}
			var frmReferralEdit=new FrmReferralEdit(referral);//the ReferralNum must be added here
	...
	private void FillReferrals() {
		...
		var result=Referrals.GetReferralText(Patient.PatNum,textReferredFrom);
		if(result.IsFailure()) {
	...
	private void ShowPatientEditEmail() {
		var frmPatientEditEmail=new FrmPatientEditEmail(textEmail.Text);
	...
	private void butViewBirthdate_Click(object sender,EventArgs e) {
		...
		var logtext="Date of birth unmasked in Patient Edit";
	...
	private void butSave_Click(object sender ,System.EventArgs e) {
		var isValid=true;
		SetRequiredFields();
		var objectArrayParameters=new object[] { isValid,Patient };
		...
		if(Patient.PatStatus==PatientStatus.Deceased && _patientOld.PatStatus!=PatientStatus.Deceased) {
			var listAppointmentsFuture=Appointments.GetFutureSchedApts(Patient.PatNum);
			if(listAppointmentsFuture.Count>0) {
				...
				{
					for(var i=0;i<listAppointmentsFuture.Count;i++) {
		...
		if(IsNew) {//Check if patient already exists.
			var listPatients=Patients.GetListByName(Patient.LName,Patient.FName,Patient.PatNum);
		...
		var allAddressFieldsAreEmpty=string.IsNullOrWhiteSpace(textAddress.Text) 
		...
		var isTextCountryVisibleAndFilled=((string.IsNullOrWhiteSpace(textCountry.Text) && textCountry.Visible==true)
		...
		if(!_isUsingNewRaceFeature) {
			_listPatientRaces= [];
			for(var i=0;i<comboBoxMultiRace.SelectedIndices.Count;i++) {
				var selectedIdx=(int)comboBoxMultiRace.SelectedIndices[i];
		...
		if(comboExcludeECR.SelectedIndices.Contains(0)) { //If "None" is selected
			...
			var listCommOptOutMode=comboExcludeECR.GetListSelected<CommOptOutMode>();
		...
		#region ValidateData
		var result=Patients.ValidatePatientEdit(Patient,_patientOld,IsNew,textSite.Text);
		if(result.IsFailure()) {
		...
		#endregion
		#region Save
		result=Patients.SavePatientEdit(Security.CurUser,Patient,_patientOld,_patientNote,Family,IsNew,checkRestrictSched.Checked,checkArriveEarlySame.Checked,
	...
	private void FormPatientEdit_Closing(object sender, System.ComponentModel.CancelEventArgs e) {
		...
		if(IsNew){
			var listRefAttaches=RefAttaches.GetRefAttaches([Patient.PatNum]);
