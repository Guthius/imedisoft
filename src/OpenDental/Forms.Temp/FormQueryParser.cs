...
public partial class FormQueryParser:FormODBase {
	...
	private void FormQueryParser_Load(object sender,EventArgs e) {
		...
		var action = () => {
			...
			var isTypingInQuery=(textQuery.Focused || gridMain.SelectedCell.Y==-1);
	...
	private void FillGrid(bool isTypingText = false) {
		var pointSelectedCell = gridMain.SelectedCell;
		var listSetStmts = UserQueries.ParseSetStatements(_userQuery.QueryText);
		...
		for(var i=0;i< listSetStmts.Count;i++) { //for each SET statement
			var listQuerySetStmtObjects = UserQueries.GetListQuerySetStmtObjs(listSetStmts[i]); //find the variable name
			for(var j=0; j<listQuerySetStmtObjects.Count; j++) {
				var row = new GridRow();
	...
	private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		var querySetStmtObject = (QuerySetStmtObject)gridMain.SelectedGridRows[0].Tag;
		...
		var stmtOld = querySetStmtObject.Stmt;
		//Regular expression for the expression @Variable = Value.
		var regex=new Regex(Regex.Escape(querySetStmtObject.Variable)+@"\s*=\S*?[;|,]*"+Regex.Escape(querySetStmtObject.Value));
		var stmtNew=regex.Replace(stmtOld,querySetStmtObject.Variable+"="+inputBox.StringResult,1);
	...
	private void SelectTextForSelectedRow() {
		var querySetStmtObject = (QuerySetStmtObject)gridMain.SelectedGridRows[0].Tag;
		var startidx = textQuery.Text.IndexOf(querySetStmtObject.Stmt);
		if(startidx == -1) {
	...
	private void gridMain_CellLeave(object sender,ODGridClickEventArgs e) {
		...
		var stmtOld = querySetStmtObject.Stmt;
		var varOld = querySetStmtObject.Variable;
		var valOld = querySetStmtObject.Value;
		var valNew = rowLeaving.Cells[1].Text;
		...
		//Regular expression for the expression @Variable = Value.
		var regex=new Regex(Regex.Escape(varOld)+@"\s*=\S*?[;|,]*"+Regex.Escape(valOld));
		var stmtNew=regex.Replace(stmtOld,varOld+"="+valNew);
