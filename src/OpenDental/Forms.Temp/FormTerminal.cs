...
public partial class FormTerminal:FormODBase {
	...
	private void FormTerminal_Load(object sender,EventArgs e) {
		...
		labelWelcome.Location=new Point(Size.Width/2-labelWelcome.Width/2,Convert.ToInt32(Size.Height*0.22f));
		labelForms.Location=new Point(Size.Width/2-labelForms.Width/2,labelWelcome.Location.Y+labelWelcome.Height+23);
		listForms.Location=new Point(Size.Width/2-listForms.Width/2,labelForms.Location.Y+labelForms.Height+3);
		labelThankYou.Location=new Point(Size.Width/2-labelThankYou.Width/2,listForms.Location.Y+listForms.Height);
		butDone.Location=new Point(Size.Width/2-butDone.Width/2,labelThankYou.Location.Y+labelThankYou.Height);
		labelConnection.Location=new Point(Size.Width/2-labelConnection.Width/2,labelConnection.Location.Y);
	...
	protected override void ProcessSignalODs(List<Signalod> signals) {
		...
		if(signals.Any(x => x.IType==InvalidType.Kiosk && (x.FKeyType!=KeyType.ProcessId || x.FKey!=processId))) {
	...
	private void timer1_Tick(object sender,EventArgs e) {
		...
		var listSheets=Sheets.GetForTerminal(terminalActive.PatNum);
	...
	private void AutoShowSheets() {
		...
		var listSheetNumsFilled=new List<long>();
		var patNum=PatNum;
		var sheetNum=_listSheets?.FirstOrDefault()?.SheetNum??0;//defaults to 0, if 0 no sheet will display
		while(patNum==PatNum && sheetNum>0) {//if patient is changed or no more sheets to display, break out of loop
			var sheet=Sheets.GetSheet(sheetNum);//we want the very freshest copy of the sheet, so we go straight to the database for it
	...
	private void listForms_DoubleClick(object sender,EventArgs e) {
		...
		var sheet=Sheets.GetSheet(_listSheets[listForms.SelectedIndex].SheetNum);
		_formSheetFillEdit=new FormSheetFillEdit();
		_formSheetFillEdit.SheetCur=sheet;
		_formSheetFillEdit.IsInTerminal=true;
		_formSheetFillEdit.ShowDialog();
