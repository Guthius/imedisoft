...
public partial class FormMedLabs:FormODBase {
	...
	private List<ClinicDto> _listClinicsUser;
	...
	private List<ClinicDto> _listClinicsLabAcct;
	...
	private void FormMedLabs_Load(object sender,EventArgs e) {
		...
		if(!PrefC.GetBool(PrefName.MedLabReconcileDone) && true) {
			var countMedLabs=MedLabs.GetCountForPatient(0);
			...
			var reconcileFailedCount=MedLabs.Reconcile();
		...
		_listClinicsUser= [];
		if(true) {
			_listClinicsUser.Add(new ClinicDto { Id=-1,Description=Lan.g(this,"All") });//ClinicNum will be -1 at index 0, "All" means all the user has access to
			if(!Security.CurUser.ClinicIsRestricted) {
				...
				_listClinicsUser.Add(new ClinicDto { Id=0,Description=Lan.g(this,"Unassigned") });
			}
			_listClinicsUser.AddRange(Clinics.GetForUserod(Security.CurUser));
			for(var i = 0;i<_listClinicsUser.Count;i++) {
				comboClinic.Items.Add(_listClinicsUser[i].Description);
			}
			_listClinicsLabAcct=_listClinicsUser.FindAll(x => !string.IsNullOrEmpty(x.MedLabAccountNumber));
			if(!Security.CurUser.ClinicIsRestricted && Clinics.ClinicNum==0) {//if unrestricted and the currently selected clinic is HQ
				...
				comboClinic.SelectedIndex=_listClinicsUser.FindIndex(x => x.Id==Clinics.ClinicNum);
			}
	...
	private void FillGrid() {
		...
		GridRow row;
		var dateEnd=SIn.Date(textDateEnd.Text);
		...
		var listClinicsSelected=new List<ClinicDto>();
		if(clinic.Id==-1) {//"All" clinic
			listClinicsSelected=_listClinicsUser.FindAll(x => x.Id>-1);//will include ClinicNum 0 ("Unassigned" clinic) if user is unrestricted
		}
		else {//a single clinic was selected, either the "Unassigned" clinic or a regular clinic
			listClinicsSelected.Add(clinic);
		}
		var listMedLabs=MedLabs.GetOrdersForPatient(_patientSelected,checkIncludeNoPat.Checked,checkOnlyNoPat.Checked,SIn.Date(textDateStart.Text),
			dateEnd,listClinicsSelected);
		var listPatients=Patients.GetLimForPats(listMedLabs.Select(x => x.PatNum).Where(x => x>0).Distinct().ToList());
		for(var i = 0;i<listMedLabs.Count;i++) {
			...
			var nameFL="";
			var patient=listPatients.Find(x => x.PatNum==listMedLabs[i].PatNum); 
			if(patient!=null) {
			...
			if(true) {
				var clinicDesc="";
				var clinicLabAcct=_listClinicsLabAcct.Find(x => x.MedLabAccountNumber==listMedLabs[i].PatAccountNum);
				if(clinicLabAcct!=null) {
					clinicDesc=clinicLabAcct.Description;
				}
				row.Cells.Add(clinicDesc);
			}
			row.Tag=listMedLabs[i].PatNum+","+listMedLabs[i].SpecimenID+","+listMedLabs[i].SpecimenIDFiller;
	...
	private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		using var formMedLabEdit=new FormMedLabEdit();
		long patNum=0;
		var stringArrayPatSpecimenIds=gridMain.ListGridRows[e.Row].Tag.ToString().Split(',');
