...
public partial class FormTaskAttachmentEdit:FormODBase {
	...
	private void butAttach_Click(object sender,EventArgs e) {
		using var formImageSelect=new FormImageSelect();
	...
	private bool TryImportDoc(string filePath) {
		var imageCategory=PrefC.GetLong(PrefName.TaskAttachmentCategory);
		if(imageCategory==0) {
		...
		try {
			...
			FriendlyException.Show(Lans.g("FormTaskAttachmentEdit","An error occurred while importing the document."),ex);
			return false;
		}
		TaskAttachmentCur.DocNum=_document.DocNum;
		return true;
	...
	private void butViewDoc_Click(object sender,EventArgs e) {
		...
		var errMsg=Documents.OpenDoc(TaskAttachmentCur.DocNum);
		if(errMsg!="") {
			MsgBox.Show(errMsg);
		}
	}
	...
	private void butDelete_Click(object sender,EventArgs e) {
		...
		if(!MsgBox.Show(this,MsgBoxButtons.YesNo,stringBuilder.ToString())) {
			return;//User decided they did not want to delete the attachment
		}
