...
public partial class FormProcEdit : FormODBase {
	...
	private void FillFees(){
		var listProcedureCodes=new List<ProcedureCode> { ProcedureCodes.GetProcCode(_procedure.CodeNum) };
	...
	private void FormProcInfo_Load(object sender,System.EventArgs e) {
		...
		_isStartingUp=false;
		if(IsNew) {
			//This is so that when procedures are pre-inserted as deleted status, we force an update on proc status to the value we stuffed into the form when the user clicks ok.
		...
		var hasAutoNotePrompt=Regex.IsMatch(textNotes.Text,Procedures.AutoNotePromptRegex);
	...
	private void FillComboClinic() {
		var clinicNum=comboClinic.ClinicNumSelected;
	...
	private void FillControlsOnStartup(){
		if(_procedure.ProcStatus==ProcStat.D && _procedure.IsLocked) {//only set this when coming in with this status. 
			...
			var isInProcess=ProcMultiVisits.IsProcInProcess(_procedure.ProcNum,true);
		...
		comboDx.Items.Clear();
		for(var i=0;i<_listDefsDiagnosis.Count;i++){
		...
		for(var i=0;i<_listDefsPrognosis.Count;i++) {
		...
		for(var i=0;i<_listDefsTxPriority.Count;i++){
		...
		for(var i=0;i<_listDefsBillingType.Count;i++) {
		...
		for(var i=0;i<Enum.GetNames(typeof(ProcUnitQtyType)).Length;i++) {
		...
		for(var i=0;i<Enum.GetNames(typeof(EnumProcDrugUnit)).Length;i++){
		...
		var keyData=Procedures.GetSignatureKeyData(_procedure);
	...
	private void PromptForAutoNotes() {
		var listMatches=Regex.Matches(textNotes.Text,@"\[\[.+?\]\]").OfType<Match>().ToList();
		...
		for(var i=0;i<listMatches.Count;i++) {
			var autoNoteTitle = listMatches[i].Value.TrimStart('[').TrimEnd(']');
			var note=AutoNotes.GetByTitle(autoNoteTitle);
			var matchloc=textNotes.Text.IndexOf(listMatches[i].Value,location);
			...
			if(frmAutoNoteCompose.IsDialogOK) {
				...
				var resultstr=textNotes.Text.Substring(0,matchloc)+frmAutoNoteCompose.StrCompletedNote;
	...
	private void SetControlsUpperLeft(){
		   ...
		   || _procedureCode.AreaAlsoToothRange)
		{
			...
			listBoxTeeth.ColumnWidth=16;
			listBoxTeeth2.Visible=true;
			listBoxTeeth2.ColumnWidth=16;
			...
			var stringArrayToothNums=new string[0];
			...
			for(var i=0;i<stringArrayToothNums.Length;i++) {
			...
			for(var toothNum=1;toothNum<=16;toothNum++) {
				var toothId=toothNum.ToString();
			...
			for(var toothNum=32;toothNum>=17;toothNum--) {
				var toothId=toothNum.ToString();
				if(_listPriTeeths.Contains(toothNum.ToString())) {//Is Primary
					toothId=Tooth.PermToPri(toothId);
				}
			...
			for(var i=0;i<stringArrayToothNums.Length;i++) {
				if(Tooth.IsMaxillary(stringArrayToothNums[i])) {//Works for primary or permanent tooth numbers.
					var toothIndex=Tooth.ToInt(stringArrayToothNums[i])-1;//Works for primary or permanent tooth numbers.
					listBoxTeeth.SetSelected(toothIndex,true);
				}
				else {//Mandibular
					var toothIndex=32-Tooth.ToInt(stringArrayToothNums[i]);//Works for primary or permanent tooth numbers.
					if(toothIndex<0) {
						...
						var toothIndex2=32-Tooth.ToInt(stringArrayToothNums[i].Substring(stringArrayToothNums[i].Length-2,2));
						listBoxTeeth.SetSelected(toothIndex,true);
	...
	private void SetControlsDisabled(List<Control> listControls) {
		...
		for(var i=0;i<listControls.Count;i++) {
	...
	private void SetControlsEnabled(bool isSilent) {
		...
		var dateForPerm=Procedures.GetDateForPermCheck(_procedure);//Use ProcDate to compare to the date/days newer restriction.
		var isProcStatComplete=_procedure.ProcStatus==ProcStat.C;
		var permissions=GroupPermissions.SwitchExistingPermissionIfNeeded(EnumPermType.ProcCompleteEdit,_procedure);
		#region Setting up dictionary for removing controls
		var dictPermControls=new Dictionary<EnumPermType, List<Control>>();
		dictPermControls[EnumPermType.ProcCompleteEdit]= [butChange, textReferral, butReferral, comboPriority, butAddEstimate, butAddExistAdj, comboProv, butPickProv, comboClinic];
		...
		dictPermControls[EnumPermType.ProcCompleteStatusEdit]= [comboProcStatus, butSetComplete, butDelete];
		dictPermControls[EnumPermType.ProcCompleteNote]= [textNotes, signatureBoxWrapper, buttonUseAutoNote, butEditAutoNote, textClaimNote, butAppend, textProc];
		dictPermControls[EnumPermType.ProcCompleteEditMisc]= [checkHideGraphics, checkNoBillIns, comboClinic, textSurfaces, comboDx];
		...
		#endregion
		var listDisabled=new List<Control>();
		var isGlobalDateLocked=Security.IsGlobalDateLock(permissions,dateForPerm,isSilent);//only used to silence other security messages.
		if(!isProcStatComplete) {//either Eo or Ec
			...
			var isSuppressed=(!isProcStatComplete||isSilent||isGlobalDateLocked);//don't want a bunch of popups in a row so suppressing the message in checking permissions
			foreach(var permission in dictPermControls.Keys) {
				var isAuthorized=Security.IsAuthorized(permission,dateForPerm,isSuppressed,isGlobalDateLocked);
				if(!isAuthorized) {
	...
	private void FillReferral(bool doRefreshData=true) {
		...
		var listRefAttaches=_loadData.ListRefAttaches;
	...
	private void FillIns(bool refreshClaimProcsFirst){
		...
		var col=new GridColumn(Lan.g("TableProcIns","Ins Plan"),190);
		...
		for(var i=0;i<_listClaimProcs.Count;i++) {
			...
			var deductibleDisplay=ClaimProcs.GetDeductibleDisplay(_listClaimProcs[i]);
	...
	private void gridIns_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		using var formClaimProc=new FormClaimProc(_listClaimProcs[e.Row],_procedure,_family,_patient,_listInsPlans,ListClaimProcHists,ref ListClaimProcHistsLoop,_listPatPlans,true,_listInsSubs);
	...
	private void butAddEstimate_Click(object sender, System.EventArgs e) {
		...
		var insPlan=formInsPlanSelect.InsPlanSelected;
		var insSub=formInsPlanSelect.InsSubSelected;
		_listClaimProcs=ClaimProcs.RefreshForProc(_procedure.ProcNum);
		var claimProcForProcInsPlan=_listClaimProcs
			.Where(x => x.PlanNum == insPlan.PlanNum && x.Status != ClaimProcStatus.Preauth && !x.IsOverpay)
			.FirstOrDefault();
		var claimProc = new ClaimProc();
		var blueBookEstimateData=new BlueBookEstimateData(_listInsPlans,_listInsSubs,_listPatPlans, [_procedure],_listSubstitutionLinks);
		...
		if(claimProcForProcInsPlan!=null) {
			...
			if(insPlan.PlanType=="c") {//capitation
				var procFee = SIn.Double(textProcFee.Text);
			...
			var patPlanNum = PatPlans.GetPatPlanNum(insSub.InsSubNum,_listPatPlans);
			if(patPlanNum > 0) {
				var paidOtherInsTotal = ClaimProcs.GetPaidOtherInsTotal(claimProc,_listPatPlans);
				var writeOffOtherIns = ClaimProcs.GetWriteOffOtherIns(claimProc,_listPatPlans);
		...
		using var formClaimProc=new FormClaimProc(claimProc,_procedure,_family,_patient,_listInsPlans,ListClaimProcHists,ref ListClaimProcHistsLoop,_listPatPlans,true,_listInsSubs);
	...
	private void FillPayments(bool doRefreshData=true){
		...
		var col=new GridColumn(Lan.g("TableProcPay","Entry Date"),70,HorizontalAlignment.Center);
	...
	private void gridPay_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		var payment=Payments.GetFromList((_listPaySplits[e.Row]).PayNum,_listPayments);
	...
	private void FillAdj(){
		var adjustmentArray=_loadData.ArrAdjustments;
		_listAdjustments=Adjustments.GetForProc(_procedure.ProcNum,adjustmentArray);
		gridAdj.BeginUpdate();
		gridAdj.Columns.Clear();
		var col=new GridColumn(Lan.g("TableProcAdj","Entry Date"),70,HorizontalAlignment.Center);
		...
		for(var i=0;i<_listAdjustments.Count;i++){
			row=new GridRow();
			var adjustment=_listAdjustments[i];
	...
	private void gridAdj_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		using var formAdjust=new FormAdjust(_patient,_listAdjustments[e.Row]);
		if(formAdjust.ShowDialog()!=DialogResult.OK) {
			return;
		}
		_loadData.ArrAdjustments=Adjustments.GetForProcs([_procedure.ProcNum]).ToArray();
		FillAdj();
	...
	private void butAddAdjust_Click(object sender, System.EventArgs e) {
		...
		using var formAdjust=new FormAdjust(_patient,adjustment,isTsiAdj);
	...
	private void butAddExistAdj_Click(object sender,EventArgs e) {
		...
		using var formAdjustmentPicker=new FormAdjustmentPicker(_patient.PatNum,true,clinicNum:_procedure.ClinicNum, provNum: _procedure.ProvNum);
		if(formAdjustmentPicker.ShowDialog()!=DialogResult.OK) {
			return;
		}
		if(!Security.IsAuthorized(EnumPermType.AdjustmentEdit,formAdjustmentPicker.SelectedAdjustment.AdjDate)) {
			return;
		}
		var estPatPort=ClaimProcs.GetPatPortion(_procedure,_loadData.ListClaimProcsForProc,_loadData.ArrAdjustments.ToList());
		var procPatPaid=(decimal)PaySplits.GetTotForProc(_procedure);
		var adjRemAmt=estPatPort-procPatPaid+(decimal)formAdjustmentPicker.SelectedAdjustment.AdjAmt;
		if(adjRemAmt<0 && (decimal)formAdjustmentPicker.SelectedAdjustment.AdjAmt<0) {
		...
		formAdjustmentPicker.SelectedAdjustment.ProcNum=_procedure.ProcNum;
		formAdjustmentPicker.SelectedAdjustment.ProcDate=_procedure.ProcDate;
		Adjustments.Update(formAdjustmentPicker.SelectedAdjustment);
		_loadData.ArrAdjustments=Adjustments.GetForProcs([_procedure.ProcNum]).ToArray();
		FillAdj();
		Signalods.SetInvalid(InvalidType.BillingList);
	}
	...
	private void textSurfaces_TextChanged(object sender, System.EventArgs e) {
		var cursorPos = textSurfaces.SelectionStart;
	...
	private void butChange_Click(object sender, System.EventArgs e) {
		...
		var procedureOld=_procedure.Copy();
		var procedureCodeOld=ProcedureCodes.GetProcCode(_procedure.CodeNum);
		...
		_procedure.ProcFee=Procedures.GetProcFee(_patient,_listPatPlans,_listInsSubs,_listInsPlans,_procedure);
		...
		if(_listPaySplits.Count>0 || _listAdjustments.Count>0) {
			...
			_listClaimProcs=ClaimProcs.RefreshForProc(_procedure.ProcNum);
			var procFeeOld=procedureOld.ProcFeeTotal;
			var procFee=_procedure.ProcFeeTotal;
			var sumWriteoff=ClaimProcs.ProcWriteoff(_listClaimProcs,_procedure.ProcNum);
			var sumInsPaids=ClaimProcs.ProcInsPay(_listClaimProcs,_procedure.ProcNum);
			var sumInsEstsNotReceived=ClaimProcs.ProcEstNotReceived(_listClaimProcs,_procedure.ProcNum);
			//Adjustments are already negative if a discount, etc.
			var sumAdjs=_listAdjustments.Cast<Adjustment>().ToList().FindAll(x => x.ProcNum==_procedure.ProcNum).Sum(x => x.AdjAmt);
			var sumPaySplits=_listPaySplits.Cast<PaySplit>().ToList().FindAll(x => x.ProcNum==_procedure.ProcNum).Sum(x => x.SplitAmt);
			var credits=sumWriteoff+sumInsPaids+sumInsEstsNotReceived-sumAdjs+sumPaySplits;
			...
			if(credits>procFee) {//Procedure is overallocated.
				var strMsg=Lan.g(this,"The fee will be changed from")+" "+procFeeOld.ToString("c")+" "
	...
	private void comboProcStatus_SelectionChangeCommitted(object sender,EventArgs e) {
		...
		if(comboProcStatus.GetSelected<ProcStat>()==ProcStat.TP) {//fee starts out 0 if EO, EC, etc.  This updates fee if changing to TP so it won't stay 0.
			...
			if(_procedure.ProcFee==0) {
				_procedure.ProcFee=Procedures.GetProcFee(_patient,_listPatPlans,_listInsSubs,_listInsPlans,_procedure,listFees:_listFees);
				textProcFee.Text=_procedure.ProcFee.ToString("f");
			}
		}
		if(comboProcStatus.GetSelected<ProcStat>()==ProcStat.C) {
			var isAllowedToCompl=true;
			...
			if(_procedure.AptNum!=0) {//if attached to an appointment
				var appointment=Appointments.GetOneApt(_procedure.AptNum);
		...
		var selecteStat=comboProcStatus.GetSelected<ProcStat>();
	...
	private void butSetComplete_Click(object sender, System.EventArgs e) {
		...
		var dateTimeProc=DateTime.MinValue;
		if(_procedure.AptNum!=0){//if attached to an appointment
			var appointment=Appointments.GetOneApt(_procedure.AptNum);
	...
	private void butSearch_Click(object sender,EventArgs e) {
		var inputBox=new InputBox(Lan.g(this,"Search for"));
		inputBox.ShowDialog();
		if(inputBox.IsDialogCancel) {
			return;
		}
		var searchText=inputBox.StringResult;
		var index=textNotes.Find(inputBox.StringResult);//Gets the location of the first character in the control.
	...
	private void UpdateFinalMin() {
		...
		try {
			...
			try {//Try DateTime format.
				var sTime=DateTime.Parse(textTimeStart.Text);
		...
		try {
			...
			try {//Try DateTime format.
				var eTime=DateTime.Parse(textTimeEnd.Text);
		...
		var total=(((stopTime/100)*60)+(stopTime%100))-(((startTime/100)*60)+(startTime%100));
	...
	private void butInvalidate_Click(object sender,EventArgs e) {
		var permissions=GroupPermissions.SwitchExistingPermissionIfNeeded(EnumPermType.ProcCompleteStatusEdit,_procedure);
		var dateForPerm=Procedures.GetDateForPermCheck(_procedure);
		...
		SecurityLogs.MakeLogEntry(permissions,_patient.PatNum,Lan.g(this,"Invalidated: ")+
		                                                      ProcedureCodes.GetStringProcCode(_procedure.CodeNum)+" ("+_procedure.ProcStatus+"), "
	...
	private void PickDiagnosisCode(System.Windows.Forms.TextBox textBoxDiagnosisCode) {
	...
	private void butDelete_Click(object sender, System.EventArgs e) {
		...
		var result=Procedures.DeleteProcedure(_procedure,_procedureOld);
		if(result.IsFailure()) {
	...
	private bool EntriesAreValid() {
		...
		//
		var translationSource=nameof(FormProcEdit);//This maintains existing translations, Procedures.EntriesAreValid(...) use to exist in this file.
		Action<string> actionOnFailure=(msg) => {
		...
		var actionOnProcedureCodeFailure=() => {
			var error="The procedure code you were trying to chart appears not to exist any more. Please use the change button to select a new procedure code.";
			var msgBoxCopyPaste=new MsgBoxCopyPaste(error);
		...
		var isCheckTypeCodeNonXChecked=checkTypeCodeA.Checked
		                               || checkTypeCodeB.Checked
		                               || checkTypeCodeC.Checked
		                               || checkTypeCodeE.Checked
		                               || checkTypeCodeL.Checked
		                               || checkTypeCodeS.Checked;
		var isQuadrantSelected=radioUL.Checked || radioUR.Checked || radioLL.Checked || radioLR.Checked;
		var hasOrthoProcLink=_orthoProcLink!=null;
		var isTextDrugNdcNotBlank=textDrugNDC.Text!="";
		var isSigChangedAndNotBlank=_signatureChanged && !signatureBoxWrapper.SigIsBlank;
		var unityQty=SIn.Int(textUnitQty.Text, false);
		var isTextDateOriginalProsthBlank=textDateOriginalProsth.Text=="";
		var dateTimeProc=SIn.Date(textDate.Text);
		var result=Procedures.EntriesAreValid(textNotes.Text,isSigChangedAndNotBlank,textTimeStart.Text,textTimeEnd.Text,unityQty,comboProv.GetSelectedProvNum(),textMedicalCode.Text,isTextDrugNdcNotBlank,textDrugQty.Text,
	...
	private void SaveAndClose() {
		...
		Func<long,Procedure> funcPromptFormACLI=(verifyCode) => { 
			var frmAutoCodeLessIntrusive=new FrmAutoCodeLessIntrusive(_patient,_procedure,_procedureCode,verifyCode,_listPatPlans,_listInsSubs,_listInsPlans,
				_listBenefits,_listClaimProcs,listBoxTeeth.Text
			);
			if(!frmAutoCodeLessIntrusive.ShowDialog()
		...
		var translationSource=nameof(FormProcEdit);
		...
		if(!Procedures.VerifyProviderChange(_procedure,_procedureOld,_listAdjustments.Cast<Adjustment>().ToList(),out var hasSplitProvChanged,out var hasAdjProvChanged,
		...
		var procedureDate=DateTime.Parse(textDate.Text);
		var listPmvs=ProcMultiVisits.GetGroupsForProcsFromDb(_listClaimProcs.Select(x => x.ProcNum).ToArray());
		for(var i=0;i<_listClaimProcs.Count;i++) {//if the proc date has changed update the ClaimProcs
			var pmv=listPmvs.FirstOrDefault(x => x.ProcNum==_listClaimProcs[i].ProcNum);
			if(_listClaimProcs[i].ClaimNum!=0 && pmv!=null && !ProcMultiVisits.IsGroupInProcess([pmv])) {
			...
			if(_listClaimProcs[i].DateCP!=procedureDate && procedureDate.Year>1880) {
				var claimProcOld=_listClaimProcs[i].Copy();
		...
		if(hasAdjProvChanged) {
			for(var i=0;i<_listAdjustments.Count;i++) {//update the attached adjustments
		...
		var doClose=Procedures.TryAutoCodesPrompt(ref _procedure,_procedureOld,_procedureCode,(listBoxTeeth.SelectedIndices.Count < 1),_patient,ref _listClaimProcs,funcPromptFormACLI);
		if(!doClose) {
		...
		var isProcLinkedToOrthoCase=(_orthoProcLink!=null);
		//The actual update----------------------------------------------------------------------------------------------------------------------------------
		...
		if(_procedureOld.ProcStatus!=ProcStat.C && _procedure.ProcStatus==ProcStat.C && !_isQuickAdd) {
			...
			Procedures.AfterProcsSetComplete([_procedure]);
		}
		Signalods.SetInvalid(InvalidType.BillingList);
		DialogResult=DialogResult.OK;
	...
	private void ClearAdultToothSelections() {
		...
		if(_procedureCode.TreatArea==TreatmentArea.ToothRange || _procedureCode.AreaAlsoToothRange) {
			...
			for(var j=0; j<listBoxTeeth.Items.Count; j++) {
			...
			for(var j=0; j<listBoxTeeth2.Items.Count; j++) {
	...
	private void SaveSignature(){
		if(_signatureChanged){
			var keyData=Procedures.GetSignatureKeyData(_procedure, textNotes.Text);
	...
	private void butSave_Click(object sender,System.EventArgs e) {
		...
		if(_hasUserChanged && !_procedureOld.Signature.IsNullOrEmpty()) {
			...
			if(AlertSubs.GetAllAlertTypesForUser(_procedureOld.UserNum).Contains(AlertType.SignatureCleared)) {
				var procCode=ProcedureCodes.GetStringProcCode(_procedureOld.CodeNum);
				var alertDescription="Procedure Note Changed for PatNum: "+_procedureOld.PatNum
		...
		if(!_isQuickAdd){
			if(_procedure.ProcStatus==ProcStat.C 
			   || (_procedure.ProcStatus==ProcStat.TP && _procedure.Note=="")){
				var procNotes=ProcNotes.SetProcCompleteNoteHelper(_isQuickAdd,_procedure,_procedureOld,comboProv.GetSelectedProvNum(),textNotes.Text);
	...
	private void FormProcEdit_FormClosing(object sender,FormClosingEventArgs e) {
		...
		if(DialogResult==DialogResult.OK) {
			...
			var histList=ClaimProcs.GetHistList(_procedure.PatNum,_listBenefits,_listPatPlans,_listInsPlans,-1,DateTime.Today,_listInsSubs);
			...
			{
				var procedureParent=Procedures.GetOneProc(_procedure.ProcNumLab,false);
				if(procedureParent!=null) {//A null parent proc could happen in rare cases for older databases.
					var listClaimProcsParent=ClaimProcs.RefreshForProc(procedureParent.ProcNum);
