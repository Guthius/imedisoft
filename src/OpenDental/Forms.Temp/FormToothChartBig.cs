...
public partial class FormToothChartingBig:FormODBase {
	...
	private void FillToothChart(){
		...
		var listSelectedTeeth=new List<string>(toothChartWrapper.SelectedTeeth);
		...
		if(_showBySelectedTeeth) {
			for(var i=0;i<listSelectedTeeth.Count;i++) {
	...
	private void DrawProcGraphics() {
		...
		var listProvNums=_toothChartRelay.GetPertinentProvNumsForToothColorPref(Security.CurUser,PatientCur,ListAppointments);
		for(var i=0;i<ListDataRowsProcs.Count;i++) {
			...
			var procedureCode = ProcedureCodes.GetProcCode(ListDataRowsProcs[i]["ProcCode"].ToString());
			var procStat = (ProcStat)SIn.Long(ListDataRowsProcs[i]["ProcStatus"].ToString());
			var provNum=SIn.Long(ListDataRowsProcs[i]["ProvNum"].ToString());
			var doApplyColorPref=_toothChartRelay.DoesToothColorPrefApply(listProvNums,provNum);
			_toothChartRelay.GetToothColors(procedureCode,procStat,doApplyColorPref,out var cDark,out var cLight);
