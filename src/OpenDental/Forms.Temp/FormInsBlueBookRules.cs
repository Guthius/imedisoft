...
public partial class FormInsBlueBookRules:FormODBase {
	...
	private void FillRulesGridRows() {
		...
		for(var i=0;i<_listInsBlueBookRules.Count;i++) {
			var insBlueBookRule=_listInsBlueBookRules[i].Copy();
			var row=new GridRow();
	...
	private void FillRuleGridRow(GridRow row,InsBlueBookRule insBlueBookRule) {
		...
		if(InsBlueBookRules.IsDateLimitedType(insBlueBookRule)) {
			var ruleTypeCell="*Allowed fees from received claims";
	...
	private void GridInsBlueBookRules_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		var insBlueBookRule=(InsBlueBookRule)gridInsBlueBookRules.ListGridRows[e.Row].Tag;
		...
		using var formInsBlueBookRuleEdit=new FormInsBlueBookRuleEdit(insBlueBookRule);
	...
	private void checkUsePlanNumInHierarchy_Click(object sender,EventArgs e) {
		for(var i=0;i<gridInsBlueBookRules.ListGridRows.Count;i++) {
			var row=gridInsBlueBookRules.ListGridRows[i];
			var insBlueBookRule=(InsBlueBookRule)row.Tag;
	...
	private void MoveSelectedRuleInGrid(bool isMovingUp) {
		var indexSelected=gridInsBlueBookRules.GetSelectedIndex();
		if(indexSelected==-1) {
			MsgBox.Show(this,"Please select a rule first.");
			return;
		}
		var indexToSwap=indexSelected+1;
		if(isMovingUp) {
	...
	private void UpdateRules() {
		for(var i=0;i<gridInsBlueBookRules.ListGridRows.Count;i++) {
			var insBlueBookRule=(InsBlueBookRule)gridInsBlueBookRules.ListGridRows[i].Tag;
			insBlueBookRule.ItemOrder=i;
			var insBlueBookRuleOld=_listInsBlueBookRules.FirstOrDefault(x => x.InsBlueBookRuleNum==insBlueBookRule.InsBlueBookRuleNum);
	...
	private void GenerateAllowedFeeSchedules() {
		...
		var schedsAdded=InsPlans.GenerateAllowedFeeSchedules();
		Cursor=Cursors.Default;
		ODMessageBox.Show(Lan.g(this,"Done.  Allowed fee schedules added: ")+schedsAdded);
