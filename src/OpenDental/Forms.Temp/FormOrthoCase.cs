...
public partial class FormOrthoCase:FormODBase {
	...
	private List<OrthoProcLink> _listOrthoProcLinksVisit= [];
	///<summary>List of all Visit procedure completed for the OrthoCase.</summary>
	private List<Procedure> _listProceduresVisit= [];
	...
	private List<OrthoProcLink> _listOrthoProcLinksDetached= [];
	...
	private List<Procedure> ListAllCompletedProcs() {
		var listProceduresLinked=new List<Procedure> {_procedureBanding,_procedureDebond};
	...
	private void SetExistingOrthoCaseControls() {
		...
		if(checkIsTransfer.Checked) {
			labelBandingDate.Text=Lans.g("Transfer Date");
		}
		else {
			labelBandingDate.Text=Lans.g("Banding Date");
			FillTextBandingProc();
		...
		var totalCompleted=_listProceduresVisit.Sum(x => x.ProcFeeTotal);
		...
		var insTotalCompleted=SIn.Double(textPrimaryInsCompleted.Text)+SIn.Double(textSecondaryInsCompleted.Text);
		...
		labelExpectedDebondDate.Text=Lans.g("Expected Debond Date");
		labelTreatmentLength.Text=Lans.g("Expected days of treatment");
		if(_procedureDebond!=null && _procedureDebond.ProcStatus==ProcStat.C) {
			textExpectedDebondDate.Text=_orthoCase.DebondDate.ToShortDateString();
			labelExpectedDebondDate.Text=Lans.g("Debond Date");
			labelTreatmentLength.Text=Lans.g("Days of treatment");
		}
	...
	private void RefreshGridOrthoScheduleRows() {
		...
		//Add Banding
		GridRow row;
		var notCompleteText=Lans.g("TableOrthoSchedule","Not Complete");
		var dateCompleted=notCompleteText;
		if(!checkIsTransfer.Checked) {
			row=new GridRow();
		...
		//Add Visits
		var visitRowCount=Math.Max(SIn.Int(textVisitCountPlanned.Text,false),_listProceduresVisit.Count);
		for(var i=0;i<visitRowCount;i++) {
			row=new GridRow();
			...
			if(i+1<SIn.Int(textVisitCountPlanned.Text,false)) {
				...
				var lastVisitAmount=SIn.Double(textAllVisitsAmount.Text)-i*SIn.Double(textVisitAmount.Text);
				var lastVisitPercent=lastVisitAmount/SIn.Double(textTotalFee.Text)*100;
	...
	private void ButAddBandingProcedure_Click(object sender,EventArgs e) {
		using var formProcBandingSelect=new FormProcBandingSelect(_patient.PatNum);
	...
	private void FillTextBandingProc() {
		...
		var procedureCode=ProcedureCodes.GetProcCode(_procedureBanding.CodeNum);
	...
	private void TextVisitCountPlanned_TextChanged(object sender,EventArgs e) {
		...
		_doPreventTextChangedEvent=true;
		var visitCount=SIn.Int(textVisitCountPlanned.Text,false);
		if(visitCount<0) {
			...
			var allVisitsAmount=SIn.Double(textAllVisitsAmount.Text);
	...
	private void TextVisitPercent_TextChanged(object sender,EventArgs e) {
		...
		_doPreventTextChangedEvent=true;
		var visitPercent=SIn.Double(textVisitPercent.Text);
		var allVisitsPercent=SIn.Double(textAllVisitsPercent.Text);
		if(CompareDouble.IsLessThan(visitPercent,0) || CompareDouble.IsGreaterThan(visitPercent,allVisitsPercent)) {
			...
			var totalFee=SIn.Double(textTotalFee.Text);
	...
	private void TextVisitAmount_TextChanged(object sender,EventArgs e) {
		...
		_doPreventTextChangedEvent=true;
		var visitAmount=SIn.Double(textVisitAmount.Text);
		var allVisitsAmount=SIn.Double(textAllVisitsAmount.Text);
		...
		,ValidDouble validDoubleSecondLinkedField=null)
	{
		...
		_doPreventTextChangedEvent=true;
		var totalFee=SIn.Double(textTotalFee.Text);
		var changedFieldNumber=SIn.Double(validDoubleChangedField.Text);
	...
	private void SetAmountAndFeeFieldColors() {
		var sumAmountFields=SIn.Double(textBandingAmount.Text)+SIn.Double(textDebondAmount.Text)+SIn.Double(textAllVisitsAmount.Text);
		...
		var sumFeeFields=SIn.Double(textPrimaryInsuranceFee.Text)+SIn.Double(textSecondaryInsuranceFee.Text)+SIn.Double(textPatientFee.Text);
	...
	private void SelectOrthoGridRowsHelper() {
		...
		var indexSelectedRow=gridOrthoSchedule.ListGridRows.IndexOf(gridOrthoSchedule.SelectedGridRows[0]);
		gridOrthoSchedule.SelectionMode=GridSelectionMode.MultiExtended;
		for(var i = indexSelectedRow+1;i<gridOrthoSchedule.ListGridRows.Count;i++) {
	...
	private void ButDetachProc_Click(object sender,EventArgs e) {
		...
		for(var i=0;i<gridOrthoSchedule.SelectedGridRows.Count;i++) {
	...
	private bool HasErrors() {
		...
		var amountFieldsTotal=SIn.Double(textBandingAmount.Text)+SIn.Double(textDebondAmount.Text)+SIn.Double(textAllVisitsAmount.Text);
	...
	private void ButPatPayPlan_Click(object sender,EventArgs e) {
		...
		var listProcedures=ListAllCompletedProcs();
		using var formPayPlanDynamic=new FormPayPlanDynamic(_payPlanPatient,false);
		...
		if(checkIsTransfer.Checked && _orthoProcLinkBanding!=null) {//OrthoCase has been changed to a transfer. Delete banding proc link if it exists.
			...
			if(_orthoProcLinkBanding==null) {//OrthoCase has been changed to a non-transfer. Link banding proc.
				var orthoProcLink=new OrthoProcLink {
					OrthoCaseNum=_orthoCase.OrthoCaseNum,
					ProcNum=_procedureBanding.ProcNum,
				...
				var orthoProcLinkNewBanding=_orthoProcLinkBanding.Copy();
	...
	private void ButSave_Click(object sender,EventArgs e) {
		...
		if(_isNew) {
			//Ortho Case
			var orthoCaseNew=new OrthoCase();
			...
			var orthoCaseNum=OrthoCases.Insert(orthoCaseNew);
			_orthoCase=orthoCaseNew;
			//Ortho Schedule
			var orthoScheduleNew=new OrthoSchedule();
			...
			var orthoScheduleNum=OrthoSchedules.Insert(orthoScheduleNew);
			//Ortho Plan Link
			var orthoPlanLinkScheduleNew=new OrthoPlanLink();
