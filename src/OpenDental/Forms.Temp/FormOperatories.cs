...
public partial class FormOperatories : FormODBase {
	...
	private void FillGrid(){
		var listOpNumsSelected=gridMain.SelectedTags<Operatory>().Select(x => x.OperatoryNum).ToList();
		var scrollValueCur=gridMain.ScrollValue;
	...
	private void butPickClinic_Click(object sender,EventArgs e) {
		...
		comboClinic.ClinicNumSelected=formClinics.SelectedClinicId;
		FillGrid();
	}
	...
	private void butAdd_Click(object sender, System.EventArgs e) {
		...
		if(true && !comboClinic.IsAllSelected && !comboClinic.IsNothingSelected) {
	...
	private void butCombine_Click(object sender,EventArgs e) {
		...
		var listOpNumsSelected=new List<long>();
		...
		using var formOperatoryPick=new FormOperatoryPick(_listOperatories.FindAll(x => listOpNumsSelected.Contains(x.OperatoryNum)));
		formOperatoryPick.ShowDialog();
		if(formOperatoryPick.DialogResult!=DialogResult.OK) {
			return;
		}
		var opNumMaster=formOperatoryPick.OperatoryNumSelected;
		#endregion
		...
		var listAppointmentsToMerge=Operatories.MergeApptCheck(opNumMaster,listOpNumsSelected.FindAll(x => x!=opNumMaster));
		...
		if(apptCount>0) {
			var selectedOpName=_listOperatories.First(x => x.OperatoryNum==opNumMaster).Abbrev;//Safe
	...
	private void butUp_Click(object sender, System.EventArgs e) {
		...
		var indexSelected = gridMain.GetSelectedIndex();
		if(indexSelected==0){
			return;//already at the top
		}
		var operatorySelected = (Operatory)gridMain.ListGridRows[indexSelected].Tag;
		var operatoryAboveSelected = (Operatory)gridMain.ListGridRows[indexSelected - 1].Tag;
		if(!CanReorderOps(operatorySelected,operatoryAboveSelected)) {
			return;
		}
		var selectedItemOrder=operatorySelected.ItemOrder;
	...
	private void butDown_Click(object sender, System.EventArgs e) {
		...
		var operatorySelected = (Operatory)gridMain.ListGridRows[indexSelected].Tag;
		var operatoryBelowSelected = (Operatory)gridMain.ListGridRows[indexSelected + 1].Tag;
	...
	private bool CanReorderOps(Operatory operatory1,Operatory operatory2) {
		var strErr="";
		if(!true || comboClinic.IsAllSelected) {
			return true;
		}
