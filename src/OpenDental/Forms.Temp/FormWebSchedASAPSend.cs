...
public partial class FormWebSchedASAPSend:FormODBase {
	...
	private void FormWebSchedASAPSend_Load(object sender,EventArgs e) {
		var clinic=Clinics.GetClinic(_clinicNum)??Clinics.GetDefaultForTexting()??Clinics.GetPracticeAsClinicZero();
		var listPatNums=(_listAppointments.Select(x => x.PatNum).Union(_listRecalls.Select(x => x.PatNum))).Distinct().ToList();
		_listPatComms=Patients.GetPatComms(listPatNums,clinic,isGetFamily: false);
		var textTemplate=ClinicPrefs.GetPrefValue(PrefName.WebSchedAsapTextTemplate,_clinicNum);
		var emailTemplate=ClinicPrefs.GetPrefValue(PrefName.WebSchedAsapEmailTemplate,_clinicNum);
		var emailSubject=ClinicPrefs.GetPrefValue(PrefName.WebSchedAsapEmailSubj,_clinicNum);
	...
	private void RefreshEmail() {
		...
		ODException.SwallowAnyException(() => {
			var text=MarkupEdit.TranslateToXhtml(_emailText,isEmail:true);
	...
	private void FillSendDetails() {
		...
		var countTexts=_asapListSender.CountTextsToSend;
		var countEmails=_asapListSender.CountEmailsToSend;
		...
		for(var i=0;i<_asapListSender.ListAsapComms.Count;i++) { 
			var row=new GridRow();
			var patDetail=_asapListSender.GetListPatientDetails().First(x => x.PatNum==_asapListSender.ListAsapComms[i].PatNum);
		...
		if(countTexts==1) {
			...
			var minutesBetweenTexts=_asapListSender.MinutesBetweenTexts;
		...
		if(GetSendMode()!=AsapComms.SendMode.Email && _asapListSender.IsOutsideSendWindow) {
			labelAnticipated.Text+=Lan.g(this,"Because it is currently outside the automatic send window, texts will not start sending until")+" "
			                                                                                                                                  +_asapListSender.DateTimeStartSendText+".\r\n";
		}
		var countTextToSendAtEndTime=_asapListSender.ListAsapComms.Count(x => x.DateTimeSmsScheduled==_asapListSender.DateTimeTextSendEnd);
		if(PrefC.DoRestrictAutoSendWindow && countTextToSendAtEndTime > 1) {
			labelAnticipated.Text+=Lan.g(this,"In order to not send texts outside the automatic send window,")+" "+countTextToSendAtEndTime
			                       +" "+Lan.g(this,"texts will be sent at")+" "+_asapListSender.DateTimeTextSendEnd+".\r\n";
		}
		if(countEmails > 0) {
	...
	private void butSend_Click(object sender,EventArgs e) {
		...
		var message=_asapListSender.CountTextsToSend+" "+Lan.g(this,"text"+(_asapListSender.CountTextsToSend==1?"":"s")+" and")+" "
