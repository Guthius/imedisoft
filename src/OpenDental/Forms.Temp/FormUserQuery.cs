...
namespace OpenDental
{
    public partial class FormUserQuery : FormODBase
    {
        ...
        private readonly bool _doSubmitOnLoad;
        ...
        public FormUserQuery(string query = "", bool submitQueryOnLoad = false, UserQuery userQuery = null)
        {
            ...
            toolTipExport.SetToolTip(butExportTxt, Lans.g("Export as tab delimited .txt file"));
            toolTipExport.SetToolTip(butExportCsv, Lans.g("Export as comma delimited .csv file"));
        }
        ...
        private void FormQuery_Load(object sender, EventArgs e)
        {
            ...
            _menuItemGoToPatient.Click += MenuItemGoToPatient_Click;
            
            _gridResults.ContextMenu ??= new ContextMenu();

            if (_userQuery == null)
            {
                //default to raw format when UserQueryDefaultRaw pref is enabled
                radioRaw.Checked = PrefC.GetBool(PrefName.UserQueryDefaultRaw);
                radioHuman.Checked = !PrefC.GetBool(PrefName.UserQueryDefaultRaw);
            }
            else
            {
                radioRaw.Checked = _userQuery.DefaultFormatRaw;
                radioHuman.Checked = !_userQuery.DefaultFormatRaw;
            }
        ...
        {
            using var formQueryEdit = new FormQueryEdit();
            
            formQueryEdit.UserQueryCur = new UserQuery
            {
                QueryText = textQuery.Text,
                IsNew = true,
                IsPromptSetup = true,
                DefaultFormatRaw = PrefC.GetBool(PrefName.UserQueryDefaultRaw)
            };

            if (formQueryEdit.ShowDialog() != DialogResult.OK)
            {
                return;
            }
        ...
        private void butFavorites_Click(object sender, EventArgs e)
        {
            ...
            radioRaw.Checked = formQueryFavorites.UserQueryCur.DefaultFormatRaw;
            radioHuman.Checked = !formQueryFavorites.UserQueryCur.DefaultFormatRaw;
        ...
        private void butPaste_Click(object sender, EventArgs e)
        {
            ...
            {
                textQuery.Text = (string) dataObject.GetData(DataFormats.Text);
            }
            else
            {
        ...
        private void FormQuery_Layout(object sender, LayoutEventArgs e)
        {
        ...
        private void ExportQuery(string delimiter)
        {
            ...
            var line = "";
            for (var i = 0; i < _gridResults.Columns.Count; i++)
            {
                var columnCaption = _gridResults.Columns[i].Heading;
            ...
            string cell;
            for (var i = 0; i < _gridResults.ListGridRows.Count; i++)
            {
                line = "";
                for (var j = 0; j < _gridResults.Columns.Count; j++)
                {
        ...
        private void radioHuman_Click(object sender, EventArgs e)
        {
            FillForm();
        }

        ///<summary>Formats the current report to be human-readable. Does NOT run the whole query again.</summary>
        private void radioRaw_Click(object sender, EventArgs e)
        {
            FillForm();
        }
        ...
        private void FillGrid(bool showRawData = false)
        {
            ...
            for (var i = 0; i < _listQueryColumns.Count; i++)
            {
                _listQueryColumns[i].Width = TextRenderer.MeasureText(_listQueryColumns[i].Name, _gridResults.Font).Width + 10;
            }

            var listColumnAligments = new List<HorizontalAlignment>(_listQueryColumns.Count);
            for (var i = 0; i < _listQueryColumns.Count; i++)
            {
            ...
            var rowCount = 1; //row count always starts at 1
            float localZoom = 1; //get the current zoom factor for the local computer
            for (var r = 0; r < _listListsRawData.Count; r++)
            {
                var gridRow = new GridRow();
                if (checkNumberedRows.Checked)
                {
                    //add numbered rows if selected
                    gridRow.Cells.Add(new GridCell(rowCount++.ToString())); //increment rowCount after filling cell
                }

                for (var c = 0; c < _listListsRawData[r].Count; c++)
                {
                    var value = _listListsRawData[r][c];
                    ...
                    gridRow.Cells.Add(new GridCell(valueChanged));
                    ...
                    var textWidth = (int) (TextRenderer.MeasureText(valueChanged, _gridResults.Font).Width / localZoom) + 10;
            ...
            {
                var totalsRow = new GridRow();
                if (checkNumberedRows.Checked)
                {
                    totalsRow.Cells.Add(new GridCell());
                }

                for (var i = 0; i < _listQueryColumns.Count; i++)
                {
                    totalsRow.Cells.Add(new GridCell(_listQueryColumns[i].ShowTotal ? _listQueryColumns[i].Total.ToString("F") : ""));
            ...
            if (checkNumberedRows.Checked)
            {
                _gridResults.Columns.Add(new GridColumn("#", 60, comboAlignment.GetSelected<HorizontalAlignment>(), sortingStrategy: GridSortingStrategy.AmountParse));
            }
        ...
        {
            var columnName = _listQueryColumns[index].Name;
            var retVal = "";
            columnName = columnName.ToLower();
            if (columnName[0] == '$')
            {
                retVal = SIn.Double(dataString).ToString("F");
            ...
            switch (columnName)
            {
                ...
                case "notperson":
                    ...
                    retVal = SIn.Bool(dataString).ToString();
                    break;
                ...
                case "mydate": //this is a workaround for the daily payment report
                    _listQueryColumns[index].SortingStrategy = GridSortingStrategy.DateParse;
                    retVal = SIn.Date(dataString).ToString("d");
                    break;
                case "birthdateforage": //age
                    _listQueryColumns[index].SortingStrategy = GridSortingStrategy.AmountParse;
                    retVal = PatientLogic.DateToAgeString(SIn.Date(dataString));
                    break;
                //time 
                case "aptdatetime":
                case "nextschedappt":
                ...
                case "tempovertime":
                    ...
                    {
                        _listQueryColumns[index].SortingStrategy = GridSortingStrategy.AmountParse;
                        retVal = SIn.TimeSpan(dataString).TotalHours.ToString("n");
                        break;
                    }

                    if (_timeCardShowSeconds)
                    {
                        //Colon format with seconds
                        _listQueryColumns[index].SortingStrategy = GridSortingStrategy.TimeParse;
                        retVal = SIn.TimeSpan(dataString).ToStringHmmss();
                        break;
                    }

                    _listQueryColumns[index].SortingStrategy = GridSortingStrategy.TimeParse;
                    retVal = SIn.TimeSpan(dataString).ToStringHmm();
                    break;
                //double
                case "adjamt":
                case "monthbalance":
                ...
                case "adjtype":
                    //fill list if empty
                    retVal = Defs.GetName(DefCat.AdjTypes, SIn.Long(dataString), _listDefsAdjTypes);
                    break;
                case "confirmed":
                    retVal = Defs.GetValue(DefCat.ApptConfirmed, SIn.Long(dataString));
                    break;
                case "dx":
                    retVal = Defs.GetName(DefCat.Diagnosis, SIn.Long(dataString), _listDefsDiagnosis);
                    break;
                case "discounttype":
                    retVal = Defs.GetName(DefCat.DiscountTypes, SIn.Long(dataString), _listDefsDiscountTypes);
                    break;
                case "doccategory":
                    retVal = Defs.GetName(DefCat.ImageCats, SIn.Long(dataString), _listDefsImageCats);
                    break;
                case "op":
                    retVal = Operatories.GetAbbrev(SIn.Long(dataString));
                    break;
                case "paytype":
                    retVal = Defs.GetName(DefCat.PaymentTypes, SIn.Long(dataString), _listDefsPaymentTypes);
                    break;
                case "proccat":
                    retVal = Defs.GetName(DefCat.ProcCodeCats, SIn.Long(dataString), _listDefsProcCodeCats);
                    break;
                case "unschedstatus":
                case "recallstatus":
                    retVal = Defs.GetName(DefCat.RecallUnschedStatus, SIn.Long(dataString), _listDefsRecallUnschedStatus);
                    break;
                case "billingtype":
                    retVal = Defs.GetName(DefCat.BillingTypes, SIn.Long(dataString), _listDefsBillingTypes);
                    break;
                ...
                case "withpat":
                    ...
                    var patNum = SIn.Long(dataString);
                ...
                case "priplannum":
                case "secplannum":
                    if (_dictionaryPlanCarrier.ContainsKey(SIn.Long(dataString)))
                        retVal = _dictionaryPlanCarrier[SIn.Long(dataString)];
                    else
                        retVal = "";
                    break;
                //referralnum
                ...
                //enumerations:
                case "aptstatus":
                    retVal = ((ApptStatus) SIn.Long(dataString)).ToString();
                    break;
                case "category":
                    ...
                    {
                        retVal = ((DisplayFieldCategory) SIn.Long(dataString)).ToString();
                    }
                    else
                    {
                        retVal = ((DefCat) SIn.Long(dataString)).ToString();
                    }

                    break;
                case "renewmonth":
                    _listQueryColumns[index].SortingStrategy = GridSortingStrategy.DateParse;
                    retVal = ((Month) SIn.Long(dataString)).ToString();
                    break;
                case "patstatus":
                    retVal = ((PatientStatus) SIn.Long(dataString)).ToString();
                    break;
                case "gender":
                    retVal = ((PatientGender) SIn.Long(dataString)).ToString();
                    break;
                //case "lab":
                ...
                //  break;
                case "position":
                    retVal = ((PatientPosition) SIn.Long(dataString)).ToString();
                    break;
                case "deductwaivprev":
                case "flocovered":
                case "misstoothexcl":
                case "procstatus":
                    retVal = ((ProcStat) SIn.Long(dataString)).ToString();
                    break;
                case "majorwait":
                case "hascaries":
                case "needssealants":
                ...
                case "missingallteeth":
                    retVal = ((YN) SIn.Long(dataString)).ToString();
                    break;
                case "prirelationship":
                case "secrelationship":
                    retVal = ((Relat) SIn.Long(dataString)).ToString();
                    break;
                case "treatarea":
                    retVal = ((TreatmentArea) SIn.Long(dataString)).ToString();
                    break;
                case "specialty":
                    retVal = Defs.GetName(DefCat.ProviderSpecialties, SIn.Long(dataString));
                    break;
                case "placeservice":
                    retVal = ((PlaceOfService) SIn.Long(dataString)).ToString();
                    break;
                case "employrelated":
                    retVal = ((YN) SIn.Long(dataString)).ToString();
                    break;
                case "schedtype":
                    retVal = ((ScheduleType) SIn.Long(dataString)).ToString();
                    break;
                case "dayofweek":
                    retVal = ((DayOfWeek) SIn.Long(dataString)).ToString();
                    break;
                case "raceOld":
                    retVal = ((PatientRaceOld) SIn.Long(dataString)).ToString();
                    break;
                case "gradelevel":
                    retVal = ((PatientGrade) SIn.Long(dataString)).ToString();
                    break;
                case "urgency":
                    retVal = ((TreatmentUrgency) SIn.Long(dataString)).ToString();
                    break;
                case "reftype":
                    retVal = ((ReferralType) SIn.Int(dataString)).ToString();
                    break;
                //miscellaneous:
                case "provnum":
                case "provhyg":
                ...
                case "provtreat":
                case "provbill":
                    retVal = Providers.GetAbbr(SIn.Long(dataString));
                    break;
                case "covcatnum":
                    retVal = CovCats.GetDesc(SIn.Long(dataString));
                    break;
                case "referringprov":
                ...
                case "feesched":
                case "feeschednum":
                    retVal = FeeScheds.GetDescription(SIn.Long(dataString));
                    break;
        ...
        public void SubmitQueryThreaded(bool wasSqlValidated = false)
        {
            ...
            _odThreadQuery = new ODThread((_) => ExecuteQuery(checkReportServer.Checked, wasSqlValidated));
        ...
        private void FillDataFromQuery(DataTable table)
        {
            ...
            {
                _listListsRawData.Add(row.ItemArray.AsEnumerable().Select(x => SIn.ByteArray(x)).ToList());
            }
        }
        ...
        private void MenuItemGoToPatient_Click(object sender, EventArgs e)
        {
            ...
            if (long.TryParse(_gridResults.SelectedTag<string>(), out var patNum))
            {
                ...
                switch (_queryExceptionState)
                {
                    ...
                    default:
                        if (e.Message.ToLower().Contains("critical error"))
                        {
                            ...
                            using var msgBoxCopyPaste = new MsgBoxCopyPaste(e.Message + "\r\n" + Lans.g("Please call support.") + "\r\n\r\n" + e.StackTrace);
        ...
        private void LayoutHelperForState(QueryExecuteState queryExecuteState)
        {
            switch (queryExecuteState)
            {
                ...
                case QueryExecuteState.Executing:
                    ...
                    Text = Lans.g("User Query - Executing Query...");
                ...
                case QueryExecuteState.Formatting:
                    ...
                    Text = Lans.g("User Query - Formatting Query...");
        ...
        private void ExecuteQuery(bool useReportServer, bool wasSqlValidated)
        {
            var hasStackTrace = UserHasStackTracePref();
            _serverThreadId = DataConnectionCancelable.GetServerThread();
            var table = DataConnectionCancelable.GetTableConAlreadyOpen(_serverThreadId, _query, wasSqlValidated, useReportServer, hasStackTrace, suppressMessage: true);
            /*
        ...
        {
            var userodPref = UserOdPrefs.GetFirstOrNewByUserAndFkeyType(Security.CurUser.UserNum, UserOdFkeyType.QueryMonitorHasStackTraces);
            if (userodPref.IsNew)
            {
