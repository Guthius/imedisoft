...
public partial class FormEtrans835ProcEdit:FormODBase {
	...
	private void FillProcedureAdjustments() {
		...
		var widthCol=gridProcedureAdjustments.Width-10-colWidthDescription-colWidthAdjAmt;
		...
		for(var i=0;i<_hx835_Proc.ListProcAdjustments.Count;i++) {
			var hx835_Adj=_hx835_Proc.ListProcAdjustments[i];
			var row=new GridRow();
			row.Tag=hx835_Adj;
	...
	private void FillRemarks() {
		...
		for(var i=0;i<_hx835_Proc.ListRemarks.Count;i++) {
			var row=new GridRow();
	...
	private void FillSupplementalInfo() {
		...
		for(var i=0;i<_hx835_Proc.ListSupplementalInfo.Count;i++) {
			var hx835_Info=_hx835_Proc.ListSupplementalInfo[i];
			var row=new GridRow();
			row.Tag=hx835_Info;
	...
	private void gridProcedureAdjustments_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		var hx835_Adj=(Hx835_Adj)gridProcedureAdjustments.ListGridRows[e.Row].Tag;
	...
	private void gridRemarks_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		var remark=(string)gridRemarks.ListGridRows[e.Row].Tag;
		var msgBoxCopyPaste=new MsgBoxCopyPaste(remark);
		msgBoxCopyPaste.Show(this);//This window is just used to display information.
	}
	...
	private void pd_PrintPage(object sender,PrintPageEventArgs e) {
		...
		if(_eraProcPrintingProgress==EraProcPrintingProgress.DocumentHeader) {//Only print the document heading on the first page.
			...
			g.DrawString(text,fontLarge,Brushes.Black,midX-g.MeasureString(text,fontLarge).Width/2,yPos);
			yPos+=25;
			using var fontSmall=new Font("Arial",9,FontStyle.Bold);
	...
	private void PrintGridHelper(GridOD grid,Rectangle rectangle,Graphics g,PrintPageEventArgs e) {
		var y=grid.PrintPage(g,_pageNumber,rectangle,_yPos);
