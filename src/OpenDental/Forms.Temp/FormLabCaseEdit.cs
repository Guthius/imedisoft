...
public partial class FormLabCaseEdit : FormODBase {
	...
	private void FormLabCaseEdit_Load(object sender, System.EventArgs e) {
		...
		var patient=Patients.GetPat(LabCaseCur.PatNum);
		...
		for(var i=0;i<_listLaboratories.Count;i++){
	...
	private void textDateDue_TextChanged(object sender,EventArgs e) {
		try{
			var date=DateTime.Parse(textDateDue.Text);
	...
	private void listLab_SelectedIndexChanged(object sender,EventArgs e) {
		...
		for(var i=0;i<_listLabTurnarounds.Count;i++){
			listTurnaround.Items.Add(_listLabTurnarounds[i].Description+", "+_listLabTurnarounds[i].DaysActual);
		}
	}
	...
	private void listTurnaround_Click(object sender,EventArgs e) {
		...
		var dateDue=LabTurnarounds.ComputeDueDate
			(MiscData.GetNowDateTime().Date,_listLabTurnarounds[listTurnaround.SelectedIndex].DaysActual);
		textDateDue.Text=dateDue.ToShortDateString()+" "+dateDue.ToShortTimeString();
	...
	private void butSlip_Click(object sender,EventArgs e) {
		if(_sheet==null) {//create new
			...
			var laboratory=_listLaboratories[listLab.SelectedIndex];
			SheetDef sheetDef;
			...
			if(ListProcCodeNums!=null) {
				...
				var listEnumStaticTextFields=SheetFiller.GetAllStaticTextFieldsForSheet(_sheet);
				...
				var family=Patients.GetFamily(LabCaseCur.PatNum);
				var patient=Patients.GetPat(LabCaseCur.PatNum);
