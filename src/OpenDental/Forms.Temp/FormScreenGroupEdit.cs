...
public partial class FormScreenGroupEdit:FormODBase {
	...
	private List<Screen> _listScreens;
	...
	private void FormScreenGroup_Load(object sender, System.EventArgs e) {
		...
		var listStringCountyNames=Counties.GetListNames();
		...
		for(var i=0;i<comboCounty.Items.Count;i++) {
			var countyName=comboCounty.Items.GetObjectAt(i).ToString();
			if(countyName==_screenGroup.County) {
				comboCounty.SelectedIndex=i;
				break;
			}
		}
		var listGradeSchoolNames=Sites.GetDeepCopy().Select(x => x.Description).ToList();
		...
		for(var i=0;i<comboGradeSchool.Items.Count;i++) {
			var gradeSchoolName=comboGradeSchool.Items.GetObjectAt(i).ToString();
		...
		if(comboGradeSchool.SelectedIndex<0 && _screenGroup.GradeSchool!="") {//if GradeSchool is not in list, just display the GradeSchool upon load
			comboGradeSchool.SetSelectedKey<long>(-1,x=>x,_=>_screenGroup.GradeSchool);
		}
		...
		if(PrefC.GetBool(PrefName.ScreeningsUseSheets)) {
			...
			for(var i=0;i<_listSheetDefsScreening.Count();i++) {
			...
			if(_screenGroup.SheetDefNum==0) {
				...
				var idx=_listSheetDefsScreening.FindIndex(x => x.SheetDefNum==_screenGroup.SheetDefNum);
	...
	private void FillGrid() {
		...
		GridRow row;
		var listScreensToDelete=new List<Screen>();
		var listSheets=Sheets.GetSheets(_listScreens.Select(x=>x.SheetNum).ToList());
		for(var i=0;i<_listScreens.Count();i++) {
			row=new GridRow();
			var screenPat=_listScreenPats.FirstOrDefault(x => x.ScreenPatNum==_listScreens[i].ScreenPatNum);
			var sheet=listSheets.FirstOrDefault(x=>x.SheetNum==_listScreens[i].SheetNum);
		...
		if(listScreensToDelete.Count>0) {
			var listScreenPatNumsToShow=listScreensToDelete.Select(y=>y.ScreenPatNum).ToList();
			//Get the PatNum from the list of ScreenPat objects.
			var listPatNumsToShow=_listScreenPats.FindAll(x=>listScreenPatNumsToShow.Contains(x.ScreenPatNum)).Select(x=>x.PatNum).ToList();
			var listPatients=Patients.GetLimForPats(listPatNumsToShow);
			var removedSheetPatientNames="";
			...
			var listScreenNumsToRemove=listScreensToDelete.Select(x=>x.ScreenNum).ToList();
	...
	private void FillScreenPats() {
		...
		GridRow row;
		for(var i=0;i<_listScreenPats.Count();i++) {
			row=new GridRow();
			var pat=Patients.GetLim(_listScreenPats[i].PatNum);
			row.Cells.Add(pat.GetNameLF());
			row.Cells.Add(_listScreenPats[i].PatScreenPerm.ToString());
			var screen=_listScreens.FirstOrDefault(x => x.ScreenPatNum==_listScreenPats[i].ScreenPatNum);
	...
	private void AddAnonymousScreensForSheets() {
		...
		var sheetDef=SheetDefs.GetInternalOrCustom(SheetInternalType.Screening);
		...
		while(true) {
			...
			screen.ScreenNum=0;
			//For Anonymous patients always default to unknowns.
			var listSheetFields=sheet.SheetFields.FindAll(x => x.FieldType==SheetFieldType.ComboBox);
			for(var i=0;i<listSheetFields.Count();i++) {
				var startIndex=listSheetFields[i].FieldValue.IndexOf(';');
				if(startIndex < 0) {
	...
	private void StartScreensForPats() {
		...
		var selectedIdx=gridScreenPats.GetSelectedIndex();
		var i=selectedIdx;
		if(i==-1) {
			i=0;
		}
		while(true) {
			var screenPat=_listScreenPats[i];
			...
			var patient=Patients.GetPat(screenPat.PatNum);//Get a patient so we can pre-fill some of the information (age/sex/birthdate/grade)
	...
	private void StartScreensForPatsWithSheets() {
		...
		while(true) {
			...
			var listSheetFieldsChartOrigVals=new List<SheetField>();
			var sheetFieldTreatment=sheet.SheetFields.Find(x=>x.FieldName=="ChartSealantTreatment" && x.FieldType==SheetFieldType.ScreenChart);
			var sheetFieldComplete=sheet.SheetFields.Find(x=>x.FieldName=="ChartSealantComplete" && x.FieldType==SheetFieldType.ScreenChart);
			...
			var listSheetFieldProcOrigVals=new List<SheetField>();
			var sheetFieldAssess=sheet.SheetFields.Find(x=>x.FieldName=="AssessmentProc" && x.FieldType==SheetFieldType.CheckBox);
			var sheetFieldFluoride=sheet.SheetFields.Find(x=>x.FieldName=="FluorideProc" && x.FieldType==SheetFieldType.CheckBox);
			...
			var listSheetFieldsProcs=sheet.SheetFields.FindAll(x => x.FieldName.StartsWith("Proc") && x.FieldType==SheetFieldType.CheckBox);
			...
			if(formSheetFillEdit.SheetCur!=null) {//It wasn't deleted, create a screen.
				...
				Screens.ProcessScreenChart(sheet,ScreenChartType.TP|ScreenChartType.C,provNum,listSheetFieldsChartOrigVals,listSheetFieldProcOrigVals);
			}
			screen.ScreenGroupOrder++;
			FillGrid();
			i=(i+1)%_listScreenPats.Count;//Causes the index to loop around when it gets to the end of the list so we can get to the beginning again.
	...
	private void ViewScreenForPat(Screen screenCur) {
		using var formScreenEdit=new FormScreenEdit();
		formScreenEdit.ScreenGroupCur=_screenGroup;
		formScreenEdit.IsNew=false;
		formScreenEdit.ScreenCur=screenCur;
		var screenPat=_listScreenPats.FirstOrDefault(x => x.ScreenPatNum==screenCur.ScreenPatNum);
		formScreenEdit.ScreenPatCur=screenPat;//Null represents anonymous.
		formScreenEdit.ShowDialog();
	}

	private void ViewScreenForPatWithSheets(Screen screenCur) {
		var sheet=Sheets.GetSheet(screenCur.SheetNum);
		if(sheet==null) {
			MsgBox.Show(this,"Sheet no longer exists.  It may have been deleted from the Chart Module.");
			return;
		}
		var listSheetFieldsChartOrigVals=new List<SheetField>();
		var sheetFieldTreatment=sheet.SheetFields.Find(x=>x.FieldName=="ChartSealantTreatment");
		var sheetFieldComplete=sheet.SheetFields.Find(x=>x.FieldName=="ChartSealantComplete");
		if (sheetFieldTreatment==null) {
			listSheetFieldsChartOrigVals.Add(null); //Adds null entry if it's not found, which is fine.
		}
		else {
		...
		var listSheetFieldsProcOrigVals=new List<SheetField>();
		var sheetFieldAssess=sheet.SheetFields.Find(x=>x.FieldName=="AssessmentProc");
		var sheetFieldFluoride=sheet.SheetFields.Find(x=>x.FieldName=="FluorideProc");
		if(sheetFieldAssess!=null) {
		...
		var listSheetFieldProcs=sheet.SheetFields.FindAll(x => x.FieldName.StartsWith("Proc") && x.FieldType==SheetFieldType.CheckBox);
		...
		if(formSheetFillEdit.ShowDialog()==DialogResult.OK) {
			...
			Screens.ProcessScreenChart(sheet,ScreenChartType.TP|ScreenChartType.C,provNum,listSheetFieldsChartOrigVals,listSheetFieldsProcOrigVals);						
		}
	...
	private void patContextMenuItem_Click(object sender,EventArgs e) {
		...
		var idx=patContextMenu.MenuItems.IndexOf((MenuItem)sender);
	...
	private void gridScreenPats_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		...
		var family=Patients.GetFamily(_listScreenPats[gridScreenPats.GetSelectedIndex()].PatNum);
		var patient=family.GetPatient(_listScreenPats[gridScreenPats.GetSelectedIndex()].PatNum);
		...
		var screenPat=_listScreenPats.FirstOrDefault(x => x.PatNum==patient.PatNum);
		for(var i=0;i<_listScreens.Count();i++) {
	...
	private void butAddPat_Click(object sender,EventArgs e) {
		...
		var screenPat=_listScreenPats.FirstOrDefault(x => x.PatNum==frmPatientSelect.PatNumSelected);
		if(screenPat!=null) {
		...
		screenPat.PatNum=frmPatientSelect.PatNumSelected;
