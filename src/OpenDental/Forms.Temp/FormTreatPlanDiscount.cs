...
public partial class FormTreatmentPlanDiscount:FormODBase {
	...
	private void butSave_Click(object sender,EventArgs e) {
		var countProcsLinkedToOrthoCase=0;
		...
		var listProcNumsLinkedToOrthoCases=OrthoProcLinks.GetManyForProcs(_listProcedures.Select(x => x.ProcNum).ToList()).Select(y => y.ProcNum).ToList();
		for(var j=0;j<_listProcedures.Count;j++) {
			...
			if(_listProcedures[j].Discount!=_listProceduresOld[j].Discount) {//Discount was changed
				var message=Lan.g(this,"Discount created or changed from Treat Plan module for procedure")
		...
		if(countProcsLinkedToOrthoCase>0) {
			var countProcsSkipped=countProcsLinkedToOrthoCase.ToString();
			ODMessageBox.Show(this,Lans.g("Procedures attached to ortho cases cannot have discounts. Procedures skipped:")+" "+countProcsSkipped);
		}
