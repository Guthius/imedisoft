...
public partial class FormSchedule:FormODBase {
	...
	private void FillEmployeesAndProviders() {
		...
		if(_listEmpNumsPreSelected!=null || _listProvNumsPreSelected!=null) {
			if(_listEmpNumsPreSelected!=null && _listEmpNumsPreSelected.Count>0) {
				//Employee Listbox
				for(var i=1;i<listBoxEmps.Items.Count;i++) {
			...
			if(listEmpNumsPreviouslySelected.Count > 0) {
				for(var i=0; i<listBoxEmps.Items.Count; i++) {
	...
	private void FillGrid(bool doRefreshData=true){
		...
		gridMain.ListGridRows.Clear();
		GridRow row;
		for(var i=0;i<_tableScheds.Rows.Count;i++){
			row=new GridRow();
			if(_scheduleWeekendFilter==ScheduleWeekendFilter.Weekend || _scheduleWeekendFilter==ScheduleWeekendFilter.FullWeek){
		...
		var colI=(int)DateTime.Today.DayOfWeek;
		if(_scheduleWeekendFilter==ScheduleWeekendFilter.WorkWeek){
			colI--;
		}
	...
	private void SelectAllProvsAndEmps() {
		...
		for(var i=1;i<listBoxProvs.Items.Count;i++) {//i=1 to skip the none
			listBoxProvs.SetSelected(i,true);
		}
		listBoxEmps.ClearSelected();
		for(var i=1;i<listBoxEmps.Items.Count;i++) {//i=1 to skip the none
			listBoxEmps.SetSelected(i,true);
		}
	}
	...
	private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		...
		var dateSelected=Schedules.GetDateCal(_dateFromDate,e.Row,clickedCol);
		...
		using var formScheduleDayEdit=new FormScheduleDayEdit(dateSelected,comboClinic.ClinicNumSelected,provAbbr,empFName,true);
	...
	private void butClearWeek_Click(object sender,EventArgs e) {
		...
		var dateSelectedStart=Schedules.GetDateCal(_dateFromDate,gridMain.SelectedCell.Y,startI);
	...
	private void butCopyDay_Click(object sender,EventArgs e) {
		...
		var selectedCol=gridMain.SelectedCell.X;
		if(_scheduleWeekendFilter==ScheduleWeekendFilter.WorkWeek) {
			selectedCol++;
		}
	...
	private void PasteSelected() {
		var isWeekCopied = (_dateCopyStart!=_dateCopyEnd);
		...
		if(checkReplace.Checked) {
			if(listProvNums.Count > 0) {
				var countDistinctProvNums=listSchedulesToCopy.Where(x => x.ProvNum!=0).Select(y => y.ProvNum).Distinct().Count();
		...
		if(checkWarnProvOverlap.Checked) {
			...
			var listSchedulesProv=listSchedulesToCopy.FindAll(x => x.SchedType==ScheduleType.Provider);
			var listProvNumsOverlapping=Schedules.GetOverlappingSchedProvNumsForRange(listSchedulesProv,dateSelectedStart,dateSelectedEnd
		...
		var listSchedulesHolidays=GetHolidaySchedules(dateSelectedStart,dateSelectedEnd);
		if(checkReplace.Checked) {
			var listSchedulesToDelete=Schedules.GetSchedulesToDelete(dateSelectedStart,dateSelectedEnd,listProvNums,listEmployeeNums,
		...
		var weekDelta=0;
		if(isWeekCopied){
			var span=dateSelectedStart-_dateCopyStart;
	...
	private void butRepeat_Click(object sender,EventArgs e) {
		...
		//Only check overlapping provider schedules.
		var listSchedulesProv=listSchedulesToCopy.FindAll(x => x.SchedType==ScheduleType.Provider);
		var listProvNumsOverlapping=new List<long>();
		var dateEnd=dateSelectedEnd;
		...
		for(var r=0;r<repeatCount;r++){//for example, user wants to repeat 3 times.
			if(checkReplace.Checked) {
	...
	private List<Schedule> GetHolidaySchedules(DateTime dateTimeStart,DateTime dateTimeEnd) {
		...
		listClinicNums.AddRange(comboClinic.ListClinics.Select(x=>x.Id).ToList());//Retrieve all clinicNums from the combobox
		...
		var listSchedulesHoliday=Schedules.GetAllHolidaysForDateRange(dateTimeStart,dateTimeEnd,listClinicNums);
	...
	private List<Schedule> FilterScheduleList(List<Schedule> listSchedules,bool removeHolidays){
		if(removeHolidays) {
			var listSchedulesHoliday=listSchedules.FindAll(x => x.Status==SchedStatus.Holiday);
			for(var i=0;i<listSchedulesHoliday.Count;i++) {
		...
		for(var i=listSchedules.Count-1;i>=0;i--) {
			if(_scheduleWeekendFilter==ScheduleWeekendFilter.Weekend 
