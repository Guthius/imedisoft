...
public partial class FormPerio : FormODBase {
	...
	private string ConvertListToString(List<string> listStringsTeeth){
		...
		var retVal="";
		for(var i=0;i<listStringsTeeth.Count;i++){
			if(i>0)
				retVal+=",";
	...
	private void FillCounts(){
		...
		textCountAttGing.Text=contrPerio.CountTeeth(PerioSequenceType.AttGing).Count.ToString();
	...
	private int FirstEmptyPositionX(ColRow colRow) {
		...
		var colRowNextAvailable=contrPerio.TryFindNextCell(colRow,isReverse:false,setDirection:false);
	...
	private int GetIndexCount(string value,string target,int count) {
		...
		for(var i=0;i<value.Length-target.Length+1;i++) {
	...
	private ColRow GetNextToothColRow(int toothOld,bool isFacial) {
		var colRow=GetColRowForTooth(toothOld,isFacial);
		var colRowNext=contrPerio.TryFindNextCell(colRow,isReverse:false);
		if(colRowNext==null){
			return colRow;
		}
		return colRowNext;
	...
	private ColRow GetPreviousToothColRow(int toothOld,bool isFacial,bool doSetDirection=true) {
		var colRow=GetColRowForTooth(toothOld,isFacial);
		var colRowPrev=contrPerio.TryFindNextCell(colRow,isReverse:true,doSetDirection);
		if(colRowPrev==null){
			return colRow;
		}
		return colRowPrev;
	...
	private List<int> GetSkippedTeeth() {
		...
		for(var i=0;i<_listMissingTeeth.Count;i++) {
			...
			var toothNum=SIn.Int(_listMissingTeeth[i]);
	...
	private ColRow GetSurfaceColRowForTooth(int toothNum,bool isFacial,EnumPerioMMidD perioSurface) {
		var colRowForTooth=GetColRowForTooth(toothNum,isFacial);
		//Get set to either the right(Lingual) or left(Facial) position.
		var xPos=colRowForTooth.Col;
	...
	private int GetToothNumFromColRow(ColRow colRow) {
		var perioCell=contrPerio.GetPerioCellFromColRow(colRow);
	...
	private ColRow NextEmptyColRowHelper(ColRow colRowForTooth,bool isReverse) {
		var colRowNextAvailable=contrPerio.TryFindNextCell(colRowForTooth,isReverse);
		if(colRowNextAvailable==null){
		...
		var toothNum=GetToothNumFromColRow(colRowNextAvailable);
	...
	private void RefreshListExams(bool skipRefreshMeasures=false) {
		...
		gridODExam.Columns.Add(gridColumn);
		for(var i=0;i<contrPerio.ListPerioExams.Count;i++) {
			var gridRow=new OpenDental.UI.GridRow();
			...
			var index5thNewLine=GetIndexCount(contrPerio.ListPerioExams[i].Note,"\n",5);//Our goal is to get no more than 5 lines into the text box.
			var noteEnd=90;//90 characters is about what it takes to get most strings to fit the 5 lines.
	...
	private void SetDefault(string rawPerioMeasures) {
		...
		var listSkippedTeeth=GetSkippedTeeth();
		var rowF1=rawPerioMeasures.Substring(0,48);
		var rowL1=rawPerioMeasures.Substring(48,48);
		var rowL32=rawPerioMeasures.Substring(96,48);
		var rowF32=rawPerioMeasures.Substring(144);
		var listPerioMeasures=new List<PerioMeasure>();
		for(var i=0;i<16;i++) {
			if(listSkippedTeeth.Contains(i+1)) {
				continue;
			}
			var perioMeasureUpper=new PerioMeasure();//1-16.
		...
		for(var i=0;i<16;i++) {
			...
			var perioMeasureLower=new PerioMeasure();//32-17.
	...
	private void SetLabelFwdReverse(){
		var perioCell=contrPerio.GetCurrentCell() ;
		var listPerioAdvancePoss=PerioAdvancePos.GetPath(contrPerio.EnumAdvanceSequence_);
		var perioAdvancePos=listPerioAdvancePoss.Find(x => x.ToothNum==perioCell.ToothNum && x.IsFacial==perioCell.IsFacial);
	...
	private ColRow TryGetNextAvailableColRowForTooth(int toothNum,bool isFacial) {
		...
		var colRowNextAvailable=new ColRow(FirstEmptyPositionX(colRowForTooth),colRowForTooth.Row);
	...
	private void FormPerio_Closing(object sender, System.ComponentModel.CancelEventArgs e) {
		...
		if(_perioExam!=null) { //When no Exam is selected or ExamList is empty we don't want to save ExamNotes.
			var perioExamOld=_perioExam.Copy();
		...
		if(_userOdPrefCurrentOnly==null) {
			...
			var userOdPrefOld=_userOdPrefCurrentOnly.Clone();
		...
		var userOdPrefAdvance=UserOdPrefs.GetByUserAndFkeyType(Security.CurUser.UserNum,UserOdFkeyType.PerioAutoAdvanceFacialsFirst).FirstOrDefault();
		if(userOdPrefAdvance==null) {
			...
			var userOdPrefOld=userOdPrefAdvance.Clone();
	...
	private void FormPerio_Load(object sender, System.EventArgs e) {
		...
		textRedAttGing.Text =PrefC.GetString(PrefName.PerioRedAttGing);
		...
		if(!PrefC.GetBool(PrefName.PerioRecessionInsteadOfGM)) { 
			labelRecession.Text="Ging Marg";
		}
		var program=Programs.GetCur(ProgramName.BolaAI);
		...
		if(!Security.IsAuthorized(EnumPermType.PerioEditCopy,true)) {
			butCopyPrevious.Visible=false;
		}
		//Procedure[] procList=Procedures.Refresh(PatCur.PatNum);
		var listToothInitials=ToothInitials.GetPatientData(_patient.PatNum);
	...
	private void FormPerio_ChangeTitle() {
		...
		if(contrPerio.ListColRowsSelected.IsNullOrEmpty()){
			return;
		}
		var colRow=contrPerio.ListColRowsSelected[0];
		var perioCell=contrPerio.GetPerioCellFromColRow(colRow);
		var perioSequenceType=contrPerio.GetSequenceForColRow(colRow);
		var perioSequenceTypeAbbr="";
		...
		var facialOrLingual="L";
		if(perioCell.IsFacial) {
			facialOrLingual="F";
		}
		var abbrMCD="D";//Distal
	...
	private void menuItemSetup_Click(object sender,EventArgs e) {
		...
		using var formPerioSetup=new FormPerioSetup();
		if(formPerioSetup.ShowDialog()!=DialogResult.OK) {
			return;
		}
		if(PrefC.GetBool(PrefName.PerioRecessionInsteadOfGM)) {
			labelRecession.Text="Recession";
		}
		else {
			labelRecession.Text="Ging Marg";
		}
	...
	private void pd2_PrintPage(object sender, PrintPageEventArgs printPageEventArgs){//raised for each page to be printed.
		var g=printPageEventArgs.Graphics;
		...
		if(_patient.ClinicNum!=0) {
			var clinic=Clinics.GetClinic(_patient.ClinicNum);
		...
		g.DrawString(_patient.GetNameFLFormal()+" (DOB: "+_patient.Birthdate.ToShortDateString()+")",new Font("Arial",11),Brushes.Black
			,new RectangleF(xPos,yPos,650,25),stringFormat);
		yPos+=20;
		var time=MiscData.GetNowDateTime().ToShortDateString();
		...
		g.DrawString(Lan.g(this,"Teeth with MGJ less than or equal to")+" "+textRedMGJ.Text+" mm: "
			...
			,font,Brushes.Black,xPos,yPos);
		yPos+=20;
		var GMTerm="Gingival Margin";//Use the term Gingival Margin by default
		if(PrefC.GetBool(PrefName.PerioRecessionInsteadOfGM)) {
			GMTerm="Recession";//Use the term Recession instead of Gingival Margin
		}
		g.DrawString(Lan.g(this,"Teeth with "+GMTerm+" greater than or equal to")+" "+textRedGing.Text+" mm: "
		...
		g.DrawString(Lan.g(this,"Teeth with CAL greater than or equal to")+" "+textRedCAL.Text+" mm: "
			...
			,font,Brushes.Black,xPos,yPos);
		yPos+=20;
		g.DrawString(Lan.g(this,"Teeth with Attached Gingiva less than or equal to")+" "+textRedAttGing.Text+" mm: "
		             +ConvertListToString(contrPerio.CountTeeth(PerioSequenceType.AttGing))
			,font,Brushes.Black,xPos,yPos);
		yPos+=20;
	...
	private void updownRed_MouseDown(object sender, System.Windows.Forms.MouseEventArgs e) {
		...
		var prefName=PrefName.PerioRedProb;
		...
		else if(sender==updownAttGing) {
			prefName=PrefName.PerioRedAttGing;
		}
		...
		if(sender==updownProb){
			...
			textRedAttGing.Text=val.ToString();
			textCountAttGing.Text=contrPerio.CountTeeth(PerioSequenceType.AttGing).Count.ToString();
		}
		else if(sender==updownFurc){
	...
	private void butColorBleed_Click(object sender, System.EventArgs e) {
		...
		var defMiscColors=_listDefsMiscColors[(int)DefCatMiscColors.PerioBleeding].Copy();
	...
	private void butColorCalculus_Click(object sender, System.EventArgs e) {
		...
		var defMiscColors=_listDefsMiscColors[(int)DefCatMiscColors.PerioCalculus].Copy();
		defMiscColors.ItemColor=colorDialog1.Color;
		DefL.Update(defMiscColors);
		Cache.Refresh(InvalidType.Defs);
		_isLocalDefsChanged=true;
	...
	private void butColorPlaque_Click(object sender, System.EventArgs e) {
		...
		var defMiscColors=_listDefsMiscColors[(int)DefCatMiscColors.PerioPlaque].Copy();
		defMiscColors.ItemColor=colorDialog1.Color;
		DefL.Update(defMiscColors);
		Cache.Refresh(InvalidType.Defs);
		_isLocalDefsChanged=true;
	...
	private void butColorPus_Click(object sender, System.EventArgs e) {
		...
		var defMiscColors=_listDefsMiscColors[(int)DefCatMiscColors.PerioSuppuration].Copy();
		defMiscColors.ItemColor=colorDialog1.Color;
		DefL.Update(defMiscColors);
		Cache.Refresh(InvalidType.Defs);
		_isLocalDefsChanged=true;
	...
	private void butCopyPrevious_Click(object sender,EventArgs e) {
		...
		var listPerioMeasures=PerioMeasures.GetAllForExam(contrPerio.ListPerioExams[contrPerio.ListPerioExams.Count-1].PerioExamNum);
		for(var i=0;i<listPerioMeasures.Count;i++) { //add all of the previous exam's measures to this perio exam.
	...
	private void butDefault_Click(object sender,EventArgs e) {
		...
		var prefProbeDepths=PrefC.GetString(PrefName.PerioDefaultProbeDepths);
	...
	private void butDelete_Click(object sender, System.EventArgs e) {
		...
		var selectedIndex=gridODExam.GetSelectedIndex();
...
//		}
	...
	private void butGraphical_Click(object sender,EventArgs e) {
		...
		contrPerio.ListPerioMeasures=PerioMeasures.GetForPatient(_patient.PatNum);
		FillGrid();
		using var formPerioGraphical=new FormPerioGraphical(contrPerio.ListPerioExams[gridODExam.GetSelectedIndex()],_patient);
	...
	private void butPrint_Click(object sender, System.EventArgs e) {
		...
		var fontOriginal=contrPerio.Font;
		var sizeContrOriginal=contrPerio.Size;
		contrPerio.Font=new Font("Microsoft Sans Serif", 8.25f);
		contrPerio.Size=new Size(602, 642);//Revert to default drawn grid size for printing, be sure to adjust this if the default size is changed on ContrPerio.cs, this will then trigger the ContrPerio.OnResize Event adjusting all of the formatting necessary for printing.
		...
		contrPerio.Size=sizeContrOriginal;
		contrPerio.Focus();
	}
	...
	private void butSaveImage_Click(object sender,EventArgs e) {
		...
		contrPerio.ListPerioExams=PerioExams.Refresh(_patient.PatNum);
		contrPerio.ListPerioMeasures=PerioMeasures.GetForPatient(_patient.PatNum);
		var listColRowsOriginal=new List<ColRow>(contrPerio.ListColRowsSelected);//Preserve the currently selected ColRow so it can be set after Save is completed.
		FillGrid(!_isExamInUse);
		contrPerio.ListColRowsSelected= [];//Clear list so no cell is highlighted on saved image. JobNum:56018
		//Document doc=new Document();
		//try {
		using var bitmapPerioPrintImage=new Bitmap(750,1000);
		bitmapPerioPrintImage.SetResolution(96,96);
		using var g=Graphics.FromImage(bitmapPerioPrintImage);
		g.Clear(Color.White);
		...
		SizeF sizeF;
		//Title
		using var fontTitle=new Font("Arial",15);
		var title=Lan.g(this,"Periodontal Charting");
		...
		y+=sizeF.Height;
		//Patient name
		var patNameAndBirthdate=_patient.GetNameFLFormal()+" (DOB: "+_patient.Birthdate.ToShortDateString()+")";
		...
		y+=sizeF.Height;
		//Now draw the grid
		var rectangle=contrPerio.GetRectangleBoundsShowing();
		using var bitmapGrid=new Bitmap(rectangle.Width,rectangle.Height);
		...
		var defNumCategory=Defs.GetImageCat(ImageCategorySpecial.T);
		if(defNumCategory==0) {
	...
	private void butBolaLaunch_Click(object sender, EventArgs e) {
		var path=Programs.GetProgramPath(ProgramName.BolaAI);
		var pathExpanded=Environment.ExpandEnvironmentVariables(path);//This will handle %USERPROFILE% in the path
	...
	private void textExamNotes_Leave(object sender,EventArgs e) {
		if(_perioExam!=null) { 
			...
			if(PerioExams.Update(_perioExam,perioExamOld)) {
				var selectionOld=gridODExam.GetSelectedIndex();
	...
	private void textInputBox_TextChanged(object sender,EventArgs e) {
		var charArrayInputs=textInputBox.Text.ToLower().ToCharArray();
		for(var i=0;i<charArrayInputs.Length;i++) {
	...
	private ColRow GetNextAvailableColRowForTooth(PerioAdvancePos perioAdvancePos,PerioAdvancePos perioAdvancePosNext,PerioAdvancePos perioAdvancePosPrev) {
		...
		var colRowForTooth=GetColRowForTooth(perioAdvancePos.ToothNum,perioAdvancePos.IsFacial);
		var perioSequenceType=contrPerio.GetSequenceForColRow(colRowForTooth);
	...
	private void GoToMobility() {
		var perioCell=contrPerio.GetCurrentCell();
		var toothNum=perioCell.ToothNum;
		var isFacial=_perioCellCurLocation.IsFacial;
		...
		var colRowForMobility=GetColRowForMobility(toothNum);
	...
	private void GoToSequenceType(PerioSequenceType perioSequenceType) {
		var toothNum=_perioCellCurLocation.ToothNum;
		var enumCurrentDirectionOld=contrPerio.EnumCurrentDirection_;
		var isFacial=_perioCellCurLocation.IsFacial;
		var colRowForTooth=GetColRowForTooth(toothNum,isFacial);
		...
		var colRowForSequenceType=new ColRow(colRowForTooth.Col,yPos);
		//Teeth 1-32 F isReverse=false, Teeth 1-32 L isReverse=true;
		//Find the next available colrow for the sequence type.
		var colRowNextAvailable=TryGetNextEmptyColRow(colRowForSequenceType,!isFacial);
	...
	private void GoToToothSurface(int toothNum,bool isFacial,EnumPerioMMidD perioSurface) {
		var colRowForTooth=GetSurfaceColRowForTooth(toothNum,isFacial,perioSurface);
		contrPerio.SetNewCell(colRowForTooth.Col,colRowForTooth.Row);
		FormPerio_ChangeTitle();
	}
	...
	private void ResumePath() {
		var listSkippedTeeth=new List<int>();//int 1-32
		if(contrPerio.ListPerioExams.Count > 0) {
			//set skipped teeth based on the last exam in the list: 
			listSkippedTeeth=PerioMeasures.GetSkipped(contrPerio.ListPerioExams[contrPerio.ListPerioExams.Count-1].PerioExamNum);
		}
		var listPerioAdvancePoss=PerioAdvancePos.GetPath(contrPerio.EnumAdvanceSequence_);
		ColRow colRowNextAvailable;
		for(var i=0;i<listPerioAdvancePoss.Count;i++) {
	...
	private bool SkipTooth(int toothNum,bool doVerifyByVoice=true) {
		...
		var isRadioRightChecked=radioRight.Checked;
		contrPerio.SaveCurExam(_perioExam);
		var selectedExam=contrPerio.IdxExamSelected;
		var listSkippedTeeth=new List<int>();//int 1-32
		if(contrPerio.ListPerioExams.Count > 0) {
			//set skipped teeth based on the last exam in the list: 
			listSkippedTeeth=PerioMeasures.GetSkipped(contrPerio.ListPerioExams[contrPerio.ListPerioExams.Count-1].PerioExamNum);
		...
		if(_perioCellCurLocation.ToothNum==toothNum) {//Go to the next tooth if the _perioCellCurLocation is on the tooth being skipped. 
			var colRowNextTooth=GetNextToothColRow(toothNum,_perioCellCurLocation.IsFacial);
			contrPerio.SetNewCell(colRowNextTooth.Col,colRowNextTooth.Row);
		}
		else {
			var colRowTooth=new ColRow(-1,-1);
			if(!contrPerio.ListColRowsSelected.IsNullOrEmpty()){
				colRowTooth=contrPerio.ListColRowsSelected[0];
			}
