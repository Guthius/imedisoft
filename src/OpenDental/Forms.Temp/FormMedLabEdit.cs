...
public partial class FormMedLabEdit:FormODBase {
	...
	private void SetFields() {
		...
		var labNote="";
		var clinicalInfo="";
		var listTestIds=new List<string>();
		var testsOrdered="";
		for(var i=0;i<ListMedLabs.Count;i++) {
	...
	private void FillGridResults() {
		...
		GridRow row;
		var obsDescriptPrev="";
		for(var i=0;i<_listMedlabResults.Count;i++) {
			...
			var obsDescript="";
			var medLab=MedLabs.GetOne(_listMedlabResults[i].MedLabNum);
	...
	private void FillGridFacilities() {
		...
		GridRow row;
		for(var i=0;i<_listMedlabFacilities.Count;i++) {
			var medLabFacility=_listMedlabFacilities[i];
			row=new GridRow();
			...
			var directorName=medLabFacility.DirectorFName;
	...
	private void gridResults_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		using var formMedLabResultHist=new FormMedLabResultHist();
	...
	private void butPatSelect_Click(object sender,EventArgs e) {
		using var formMedLabPatSelect=new FormMedLabPatSelect();
	...
	private void butProvSelect_Click(object sender,EventArgs e) {
		...
		var provider=Providers.GetProv(frmProviderPick.ProvNumSelected);
		for(var i=0;i<ListMedLabs.Count;i++) {
	...
	private void butPDF_Click(object sender,EventArgs e) {
		...
		var sheetDef=SheetUtil.GetMedLabResultsSheetDef();
		var sheet=SheetUtil.CreateSheet(sheetDef,_medLab.PatNum);
		...
		var tempPath=ODFileUtils.CombinePaths(PrefC.GetTempFolderPath(),_medLab.PatNum+".pdf");
		SheetPrinting.CreatePdf(sheet,tempPath,null,_medLab);
		var hl7Def=HL7Defs.GetOneDeepEnabled(true);
		var category=hl7Def.LabResultImageCat;
		if(category==0) {
		...
		var filePathAndName="";
		if(true) {
			var patFolder=ImageStore.GetPatientFolder(Patients.GetPat(_medLab.PatNum),ImageStore.GetDataFolder());
	...
	private void butShowHL7_Click(object sender,EventArgs e) {
		Cursor=Cursors.WaitCursor;
		var listFileNamesDateMod=new List<string[]>();
		for(var i=0;i<ListMedLabs.Count;i++) {
			var filePath=ODFileUtils.CombinePaths(ImageStore.GetDataFolder(),ListMedLabs[i].FileName);
			var isFileAdded=false;
			for(var j=0;j<listFileNamesDateMod.Count;j++) {
			...
			var dateModified=DateTime.MinValue.ToString();
			try {
			...
			listFileNamesDateMod.Add([filePath,dateModified]);
		}
		using var formMedLabHL7MsgText=new FormMedLabHL7MsgText();
	...
	private void MoveLabsAndImagesHelper() {
		...
		var patOld=Patients.GetPat(_medLab.PatNum);
		MedLabs.UpdateAllPatNums(ListMedLabs.Select(x => x.MedLabNum).ToList(),PatCur.PatNum);
		var atozFrom="";
		var atozTo="";
		if(true) {
			var atozPath=ImageStore.GetDataFolder();
			...
			atozFrom=ODFileUtils.CombinePaths(ImageStore.GetDataFolder(),"MedLabEmbeddedFiles",'/');
		...
		var listDocNums=ListMedLabs
			.SelectMany(x => x.ListMedLabResults
				.Select(y => y.DocNum)
				.Where(y => y>0))
			.Distinct().ToList();
		var listDocuments=Documents.GetByNums(listDocNums);
		var fileMoveFailures=0;
		for(var i=0;i<listDocuments.Count;i++) {
			var document=listDocuments[i];
			var destFileName=Documents.GetUniqueFileNameForPatient(PatCur,document.DocNum,Path.GetExtension(document.FileName));
			if(true) {
				var fromFilePath=ODFileUtils.CombinePaths(atozFrom,document.FileName);
				...
				var destFilePath=ODFileUtils.CombinePaths(atozTo,destFileName);
				if(File.Exists(destFilePath)) {//should never happen, since we already got a unique file name, but just in case
					...
					destFileName=patOld.PatNum+"_"+document.FileName;//try to prepend patient's PatNum to the original file name
					...
					while(File.Exists(destFilePath)) {
						...
						destFileName=patOld.PatNum+"_"+Path.GetFileNameWithoutExtension(document.FileName)
	...
	private void butDelete_Click(object sender,EventArgs e) {
		...
		var failedCount=MedLabs.DeleteLabsAndResults(_medLab);
		if(failedCount>0) {
			ODMessageBox.Show(this,Lans.g("Some images referenced by the MedLabResults could not be deleted and will have to be removed manually.")
			                       +"\r\n"+Lans.g("Number failed")+": "+failedCount);
		}
		DialogResult=DialogResult.OK;
	}
