...
public partial class FormClaimFormEdit : FormODBase {
	...
	private void FormClaimFormEdit_Load(object sender, System.EventArgs e) {
		...
		var size=new Size(_claimForm.Width,_claimForm.Height);
		pictureBoxClaimForm.Size=size;
		FillListItems();
	...
	private void FillSelectedXYWH(){
		if(listItems.SelectedIndices.Count==0) {
			...
			var isWSame=true;
			var isHSame=true;
			for(var i=1;i<listItems.SelectedIndices.Count;i++) {//loop starts with second items to compare
	...
	private void FillListItems() {
		...
		for(var i=0;i<_claimForm.Items.Count;i++) {
	...
	private void panelClaimForm_Paint(object sender, System.Windows.Forms.PaintEventArgs e) {
		...
		for(var i=0;i<_claimForm.Items.Count;i++){
			...
			if(extension==".jpg"){
				g.DrawImage(image
					,_claimForm.Items[i].XPos
					,_claimForm.Items[i].YPos
					,image.Width/image.HorizontalResolution*100
					,image.Height/image.VerticalResolution*100);
			}
		...
		//Then text
		using var font=new Font(_claimForm.FontName,_claimForm.FontSize);
		for(var i=0;i<_claimForm.Items.Count;i++){
			if(_claimForm.Items[i].ImageFileName!=""){
				continue;
			}
			var colorPanel=Color.Blue;
			if(listItems.SelectedIndices.Contains(i)){
				colorPanel=Color.Red;
			}
			using var solidBrush=new SolidBrush(colorPanel);
			using var pen=new Pen(solidBrush.Color);
			var xPosRect=_claimForm.Items[i].XPos;
			var xPosText=xPosRect;
	...
	private Image GetImage(ClaimFormItem claimFormItem,out string extension) {
		...
		if(claimFormItem.ImageFileName=="ADA2006.gif") {
			...
			var fileName=ODFileUtils.CombinePaths(ImageStore.GetDataFolder(),claimFormItem.ImageFileName);
			if(true) {
	...
	private void listItems_DoubleClick(object sender, System.EventArgs e) {
		...
		var index=listItems.SelectedIndices[0];
		var claimFormItemSelected= _claimForm.Items[index];
	...
	private void textXPos_Validated(object sender, System.EventArgs e) {
		...
		for(var i=0;i<listItems.SelectedIndices.Count;i++){
	...
	private void butFont_Click(object sender, System.EventArgs e) {
		var font=new Font(_claimForm.FontName,_claimForm.FontSize);
	...
	private void pictureBoxClaimForm_MouseDown(object sender, System.Windows.Forms.MouseEventArgs e) {
		...
		var g=pictureBoxClaimForm.CreateGraphics();
		//start at the end of the list and work backwards until match
		for(var i=_claimForm.Items.Count-1;i>=0;i--) {
			var width=_claimForm.Items[i].Width;
			var height=_claimForm.Items[i].Height;
			using var font=new Font(_claimForm.FontName,_claimForm.FontSize);
			...
			var isRightAligned = _stringArrayDisplays[i]=="1234.00" || _stringArrayDisplays[i]=="AB";
			if(e.Y<_claimForm.Items[i].YPos){
				continue;
			}
			if(e.Y>_claimForm.Items[i].YPos+height){
				continue;
			}
			if(isRightAligned){
				if(e.X>_claimForm.Items[i].XPos){
					continue;
				}
				if(e.X<_claimForm.Items[i].XPos-width){
					continue;
				}
			}
			else{
		...
		for(var i=0;i<listItems.SelectedIndices.Count;i++) {//then a normal loop to set _pointFArrayOldItemLocs for dragging
	...
	private void panelClaimForm_MouseMove(object sender, System.Windows.Forms.MouseEventArgs e) {
		...
		for(var i=0;i<listItems.SelectedIndices.Count;i++){
			var claimFormItem=_claimForm.Items[listItems.SelectedIndices[i]];
			claimFormItem.XPos=_pointFArrayOldItemLocs[i].X+(e.X-_pointFMouseDownLoc.X);
			claimFormItem.YPos=_pointFArrayOldItemLocs[i].Y+(e.Y-_pointFMouseDownLoc.Y);
			claimFormItem.XPos=Math.Max(claimFormItem.XPos,0);
	...
	private void FormClaimFormEdit_Closing(object sender, System.ComponentModel.CancelEventArgs e) {
		foreach(var img in _dictImages.Values) {
			img?.Dispose();
		}
