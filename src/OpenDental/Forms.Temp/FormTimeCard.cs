...
public partial class FormTimeCard:FormODBase {
	...
	private string GetTitle() {
		var textString=Lan.g(this,"Time Card for")+" "+EmployeeCur.FName+" "+EmployeeCur.LName;
		if(_cannotEditOwnTimecard) {
			textString+=Lan.g(this," - You cannot modify your time card");
		}
		else if(_cannotEditSelectedPayPeriod) {
			var currentPayPeriod=PayPeriods.GetForDate(DateTime.Today);
			var startDate=currentPayPeriod>-1 ? _listPayPeriods[currentPayPeriod].DateStart : DateTime.Today;
	...
	private TimeAdjust GetOrCreatePayPeriodNote() {
		var date=_listPayPeriods[IdxPayPeriodSelected].DateStart.Date;
		var dateMidnightFirstDay=new DateTime(date.Year,date.Month,date.Day,0,0,0);
		var timeAdjustNoteRow=TimeAdjusts.GetPayPeriodNote(EmployeeCur.EmployeeNum,dateMidnightFirstDay);
	...
	private void FillMain(bool fromDB){
		...
		for(var i=0;i<_listClockEvents.Count;i++) {
			_arrayListMerged.Add(_listClockEvents[i]);
		}
		for(var i=0;i<_listTimeAdjusts.Count;i++) {
		...
		var calendarWeekRule=CalendarWeekRule.FirstFullWeek;//CultureInfo.CurrentCulture.DateTimeFormat.CalendarWeekRule;
		var dateTime=DateTime.MinValue;
		var dateTimePrev=DateTime.MinValue;
		Type type;
		ClockEvent clockEvent;
		TimeAdjust timeAdjust;
		for(var i=0;i<_arrayListMerged.Count;i++){
			row=new GridRow();
	...
	private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		...
		if(gridMain.ListGridRows[e.Row].Tag.GetType()==typeof(TimeAdjust)) {
			var adjust=(TimeAdjust)gridMain.ListGridRows[e.Row].Tag;
			...
			using var formTimeAdjustmentEdit=new FormTimeAdjustEdit(adjust);
			formTimeAdjustmentEdit.ShowDialog();
		}
		else {
			var ce=(ClockEvent)gridMain.ListGridRows[e.Row].Tag;
			using var formClockEventEdit=new FormClockEventEdit(ce);
	...
	private void butAdj_Click(object sender,EventArgs e) {
		...
		using var FormT=new FormTimeAdjustEdit(adjust);
		FormT.IsNew=true;
		FormT.ShowDialog();
	...
	private void butCalcWeekOT_Click(object sender,EventArgs e) {
		...
		var timeCardRule=TimeCardRules.GetTimeCardRule(EmployeeCur);
	...
	private void butCalcDaily_Click(object sender,EventArgs e) {
		...
		var errors=TimeCardRules.ValidateOvertimeRules([EmployeeCur.EmployeeNum]);
		if(errors != "") {
	...
	private void butBenefits_Click(object sender,EventArgs e) {
		using var formTimeCardBenefitRp=new FormTimeCardBenefitRp();
	...
	private void pd_PrintPage(object sender,PrintPageEventArgs e) {
		...
		var fontTitle=new Font(FontFamily.GenericSansSerif,11,FontStyle.Bold);
		var fontHeader=new Font(FontFamily.GenericSansSerif,8,FontStyle.Bold);
		var solidBrush=new SolidBrush(Color.Black);
		var pen=new Pen(Color.Black);
		//Title
		str=EmployeeCur.FName+" "+EmployeeCur.LName;
		str+="\r\n"+Lan.g(this,"Note")+": "+_timeAdjustNote.Note;
		var heightThreeLine=(int)e.Graphics.MeasureString("1\r\n2\r\n3",fontTitle).Height;
		...
		var heightRect=Math.Min(heightNoteString,heightThreeLine);
		...
		for(var i=0;i<stringArrayColCaption.Length;i++) {
		...
		while(yPos < e.PageBounds.Height-75-50-32-16 && _linesPrinted < gridMain.ListGridRows.Count) {
			for(var i=0;i<intArrayColPos.Length-1;i++) {
			...
			//Column lines		
			for(var i=0;i<intArrayColPos.Length;i++) {
	...
	private void timerUpdateBreak_Tick(object sender, System.EventArgs e) {
		if(IsBreaks) {
			var idx = gridMain.GetSelectedIndex();
