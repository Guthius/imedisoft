...
public partial class FormEServicesWebSchedRecall:FormODBase {
	...
	private void FormEServicesWebSchedRecall_Load(object sender,EventArgs e) {
		...
		var recallApptDays=PrefC.GetInt(PrefName.WebSchedRecallApptSearchAfterDays);
		var strNumRecallApptDays="";
		if(recallApptDays>0) {
		...
		for(var i=0;i<_listProvidersAll.Count;i++) {
		...
		var confirmStatus=PrefC.GetLong(PrefName.WebSchedRecallConfirmStatus);
	...
	private void FillGridWebSchedOperatories() {
		...
		var widthColOpName=170;
		var widthColClinic=80;
		if(!true) {
		...
		GridRow row;
		for(var i=0;i<_listOperatoriesWebSched.Count;i++) {
			row=new GridRow();
	...
	private void FillGridWebSchedRecallTypes() {
		...
		for(var i=0;i<_listRecallTypes.Count;i++) {
		...
		var col=new GridColumn(Lan.g("TableRecallTypes","Description"),130);
		...
		for(var i=0;i<_listRecallTypes.Count;i++) {
			...
			var timeLength=RecallTypes.ConvertTimePattern(_listRecallTypes[i].TimePattern).Length * 5;
			if(timeLength==0) {
				row.Cells.Add("");
			}
			else {
				row.Cells.Add(timeLength+" "+Lan.g("TableRecallTypes","mins"));
			}
	...
	private void FillGridWebSchedTimeSlots(List<TimeSlot> listTimeSlots) {
		...
		var col=new GridColumn("",20);
		col.IsWidthDynamic=true;
		col.TextAlign=HorizontalAlignment.Center;
		gridWebSchedTimeSlots.Columns.Add(col);
		gridWebSchedTimeSlots.ListGridRows.Clear();
		GridRow row;
		var dateTimeSlotLast=DateTime.MinValue;
		for(var i=0;i<listTimeSlots.Count;i++) {
	...
	private void FillGridWebSchedTimeSlots() {
		...
		var dateStart=SIn.Date(textWebSchedDateStart.Text);
		var recallType=_listRecallTypes[comboWebSchedRecallTypes.SelectedIndex];
		var clinic=_listClinicsAll.Find(x => x.Id==_clinicNum);//null clinic is treated as unassigned.
		var listProviders=new List<Provider>(_listProvidersAll);//Use all providers by default.
		var provider=_listProvidersAll.Find(x => x.ProvNum==_provNum);
		if(provider!=null) {
			//Only use the provider that the user picked from the provider picker.
			listProviders= [provider];
		}
		var listTimeSlots=new List<TimeSlot>();
	...
	private void FillBlockoutTypeListboxes() {
		...
		var listDefLinksRecallType=DefLinks.GetDefLinksByType(DefLinkType.RecallType);
		//Restricted-To Blockouts
		var listRestrictedToBlockoutRecallTypeNums=listDefLinksRecallType.Select(x => x.DefNum).Distinct().ToList();
		var listRestrictedToBlockoutNums=listDefLinksRecallType.Select(x => x.DefNum).Distinct().ToList();
		var listDefsRestrictedToBlockouts=Defs.GetDefs(DefCat.BlockoutTypes,listRestrictedToBlockoutNums);
		//Allowed Blockouts
		var listAllowedBlockoutTypes=Defs.GetDefs(DefCat.BlockoutTypes,PrefC.GetWebSchedRecallAllowedBlockouts);
		var listAllowedBlockoutTypeNums=listAllowedBlockoutTypes.Select(x => x.DefNum).Distinct().ToList();
		var listDefAllowedBlockouts=Defs.GetDefs(DefCat.BlockoutTypes,listAllowedBlockoutTypeNums.FindAll(x => !listRestrictedToBlockoutRecallTypeNums.Contains(x)));
		//Fill the list box for Restricted to Blockouts. "BlockoutName (RecalType1.Name,RecallType2.Name)"
		var listRecallTypeNums=new List<long>();
		var listRecallTypes=new List<RecallType>();
		var listRecallTypeNames=new List<string>();
		var listBlockoutReasonAssociations=new List<string>();
		for(var i=0;i<listDefsRestrictedToBlockouts.Count;i++) {
	...
	private void FillListboxWebSchedProviderRule() {
		var clinicPrefProviderRule=ClinicPrefs.GetPref(PrefName.WebSchedProviderRule,comboClinicProvRule.ClinicNumSelected,isDefaultIncluded:true);
	...
	private void butProvRulePickClinic_Click(object sender,EventArgs e) {
		...
		comboClinicProvRule.ClinicNumSelected=formClinics.SelectedClinicId;
		FillListboxWebSchedProviderRule();
	}
	...
	private void butWebSchedPickClinic_Click(object sender,EventArgs e) {
		...
		comboWebSchedClinic.SelectedIndex=_listClinicsAll.FindIndex(x => x.Id==formClinics.SelectedClinicId)+1;//+1 for 'Unassigned'
		_clinicNum=formClinics.SelectedClinicId;
	}
	...
	private void butWebSchedRecallBlockouts_Click(object sender,EventArgs e) {
		var stringArrayDefNums=PrefC.GetString(PrefName.WebSchedRecallIgnoreBlockoutTypes).Split(','); //comma-delimited list.
		var listBlockoutTypes=new List<long>();
		for(var i=0;i<stringArrayDefNums.Length;i++) {
			listBlockoutTypes.Add(SIn.Long(stringArrayDefNums[i]));
		}
		var listDefsBlockoutTypes=Defs.GetDefs(DefCat.BlockoutTypes,listBlockoutTypes);
		using var formDefinitionPicker=new FormDefinitionPicker(DefCat.BlockoutTypes,listDefsBlockoutTypes);
		...
		if(formDefinitionPicker.DialogResult==DialogResult.OK) {
			...
			var strListWebSchedRecallIgnoreBlockoutTypes=string.Join(",",formDefinitionPicker.ListDefsSelected.Select(x => x.DefNum));
	...
	private void butWebSchedRecallNotify_Click(object sender,EventArgs e) {
		using var formEServicesWebSchedNotify=new FormEServicesWebSchedNotify(WebSchedNotifyType.Recall);
	...
	private void checkUseDefaultProvRule_Click(object sender,EventArgs e) {
		...
		if(checkUseDefaultProvRule.Checked) {
			if(ClinicPrefs.DeletePrefs(comboClinicProvRule.ClinicNumSelected, [PrefName.WebSchedProviderRule])>0) {
			...
			if(clinicPrefProviderRule!=null) {
				ClinicPrefs.DeletePrefs(comboClinicProvRule.ClinicNumSelected, [PrefName.WebSchedProviderRule]);
			}
	...
	private void comboWebSchedClinic_SelectionChangeCommitted(object sender,EventArgs e) {
		...
		if(comboWebSchedClinic.SelectedIndex>0) {//Greater than 0 due to "Unassigned"
			_clinicNum=comboWebSchedClinic.GetSelected<ClinicDto>().Id;
		}
	}
	...
	private void WebSchedRecallAutoSendRadioButtons_CheckedChanged(object sender,EventArgs e) {
		...
		var listSetupErrors=new List<string>();
		var isEmailSendInvalid=false;
		var isTextSendInvalid=false;
	...
	private void SaveWebSchedRecall() {
		var webSchedAutomaticSendNew=WebSchedAutomaticSend.SendToEmailOnlyPreferred;
		...
		var webSchedAutomaticSendOld=(WebSchedAutomaticSend)PrefC.GetInt(PrefName.WebSchedAutomaticSendSetting);
		if(Prefs.UpdateInt(PrefName.WebSchedAutomaticSendSetting,(int)webSchedAutomaticSendNew)) {
			SecurityLogs.MakeLogEntry(EnumPermType.Setup,0,"WebSched automated email preference changed from "+webSchedAutomaticSendOld+" to "+webSchedAutomaticSendNew+".");
		}
		var webSchedAutomaticSendTextNew=WebSchedAutomaticSend.SendToText;
		if(radioDoNotSendText.Checked) {
			webSchedAutomaticSendTextNew=WebSchedAutomaticSend.DoNotSend;
		}
		var webSchedAutomaticSendTextOld=(WebSchedAutomaticSend)PrefC.GetInt(PrefName.WebSchedAutomaticSendTextSetting);
		if(Prefs.UpdateInt(PrefName.WebSchedAutomaticSendTextSetting,(int)webSchedAutomaticSendTextNew)) {
			SecurityLogs.MakeLogEntry(EnumPermType.Setup,0,"WebSched automated text preference changed from "+webSchedAutomaticSendTextOld+" to "+webSchedAutomaticSendTextNew+".");
		}
		var textsPerBatchOld=PrefC.GetInt(PrefName.WebSchedTextsPerBatch);
		var textsPerBatchNew=SIn.Int(textWebSchedPerBatch.Text,false);
		if(Prefs.UpdateInt(PrefName.WebSchedTextsPerBatch,textsPerBatchNew)) {
			SecurityLogs.MakeLogEntry(EnumPermType.Setup,0,"WebSched batch size preference changed from "+textsPerBatchOld+" to "+textsPerBatchNew+".");
		}
		var recallApptDays=SIn.Int(textWebSchedRecallApptSearchDays.Text);
		...
		var valueWebSchedRecallDoubleBooking=checkWSRDoubleBooking.Checked ? 1 : 0;
