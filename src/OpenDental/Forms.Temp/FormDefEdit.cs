...
public partial class FormDefEdit : FormODBase {
	...
	private void FormDefEdit_Load(object sender, System.EventArgs e) {
		...
		if(_def.DefNum.In(PrefC.GetLong(PrefName.AppointmentTimeArrivedTrigger),PrefC.GetLong(PrefName.AppointmentTimeDismissedTrigger),
		...
		if(_def.DefNum.In(PrefC.GetLong(PrefName.AppointmentTimeArrivedTrigger))) {
		...
		if(_defCatOptions.IsValueDefNum) {
			...
			var defNumCur=SIn.Long(_def.ItemValue??"");
			if(defNumCur>0) {
	...
	private void butSelect_Click(object sender,EventArgs e) {
		var defNumParent=SIn.Long(_def.ItemValue);//ItemValue could be blank, in which case defNumCur will be 0
		var listDefsMatchingParent=_listDefs.FindAll(x => x.DefNum==defNumParent);
		using var formDefinitionPicker=new FormDefinitionPicker(_def.Category,listDefsMatchingParent,_def.DefNum);
	...
	private void butSave_Click(object sender, System.EventArgs e) {
		...
		switch(_def.Category){
			case DefCat.AccountQuickCharge:
				var stringArrayProcCodesAccountQuickCharge=textValue.Text.Split(',');
				try {
			...
			case DefCat.ApptProcsQuickAdd:
				var stringArrayProcCodesApptProcsQuickAdd=textValue.Text.Split(',');
				try {
			...
			case DefCat.ClaimCustomTracking:
				var value=0;
				if(!int.TryParse(textValue.Text,out value) || value<0) {
					MsgBox.Show(this,"Days Suppressed must be a valid non-negative number.");
					return;
				}
				break;
			case DefCat.CommLogTypes:
				var listCommItemTypes=Commlogs.GetCommItemTypes().Select(x => x.GetDescription(useShortVersionIfAvailable:true)).ToList();
			...
			case DefCat.PaySplitUnearnedType:
				...
				var pref=Prefs.GetPref(PrefName.TpUnearnedType.GetDescription());
				if(_def.DefNum.ToString()==pref.ValueString && checkHidden.Checked) {//If the current selected Def is the default, and we're setting to hidden, set the new default to the first 'shown' Def
					var listDefsForUnearnedType=_listDefs.FindAll(x => x.Category==DefCat.PaySplitUnearnedType);
					var listDefsNonHiddenForUnearnedType=listDefsForUnearnedType.FindAll(x => x.IsHidden==false && x.DefNum!=_def.DefNum && x.ItemValue.ToLower().Contains("x"));
	...
	private static void UpdateAutoCommExcludes(UI.CheckBox checkBox,List<long> listExcludeNums,Def def,PrefName prefName) {
		if(checkBox.Checked) {
		...
		var toString=string.Join(",",listExcludeNums.Distinct().OrderBy(x => x));
		Prefs.UpdateString(prefName,toString);
	}

	private static void UpdateByod(UI.CheckBox check,List<long> listExcludeNums,Def def,PrefName prefName) {
		if(check.Checked) {
			listExcludeNums.Add(def.DefNum);
		}
		else {
		...
		//Do not want to insert 0 into db. If included we won't send autocomm for status 0.
		listExcludeNums.Remove(0);
		var strExcludeNums=string.Join(",",listExcludeNums.Distinct().OrderBy(x => x));
