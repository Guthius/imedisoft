...
public partial class FormPrinterSetup : FormODBase {
	...
	public void FillGrid(){
		...
		var col=new GridColumn("Situation",150);
		...
		var listPrintSituations=new List<PrintSituation>{
		...
		for(var i=0;i<listPrintSituations.Count;i++){
			var printerForSit=Printers.GetForSit(listPrintSituations[i]);
			row=new GridRow();
	...
	private void gridPrinters_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		var printer=gridPrinters.SelectedTag<Printer>();
		using var formPrinterEdit=new FormPrinterEdit(printer);
	...
	private void butSave_Click(object sender, System.EventArgs e){
		...
		if(checkSimple.Checked && !PrefC.GetBool(PrefName.EasyHidePrinters)){
			...
			var printerDefault=gridPrinters.ListGridRows.Select(x=>((Printer)x.Tag)).FirstOrDefault(x=>x.PrintSit==PrintSituation.Default);
