...
public partial class FormPopupsForFam:FormODBase {
	...
	private void FormPopupsForFam_Load(object sender,EventArgs e) {
		...
		_listPatNumsFamily=Patients.GetAllFamilyPatNums([PatientCur.PatNum]);
		FillGrid();
	}
	...
	private void FillGrid() {
		...
		var listAutomationNums=FormOpenDental.DicBlockedAutomations.Keys.ToList();
		...
		var listAutomations=Automations.GetDeepCopy();
		for(var i=0;i<listAutomationNums.Count();i++){
			...
			foreach(var keyValuePair in dictionaryPatDate) {
				if(!Patients.GetAllFamilyPatNums([keyValuePair.Key]).Contains(PatientCur.PatNum)) {
				...
				var popup=new Popup();
				popup.PopupNum=listAutomationNums[i];
				popup.PatNum=keyValuePair.Key;
				popup.PopupLevel=EnumPopupLevel.Automation;
				popup.DateTimeEntry=keyValuePair.Value.AddMinutes(-10);
				popup.Description=automation.MessageContent;
		...
		var listPopupsPat=new List<Popup>();
		var listPopupsHiddenPat=new List<Popup>();
		var listPopupsAutomated=new List<Popup>();
		var listPopupsDisabledPat=new List<Popup>();
		var listPopupsDisabledHiddenPat=new List<Popup>();
		var listPopupsDisabledAutomated=new List<Popup>();
		...
		for(var i = 0;i<_listPopups.Count();i++) {
		...
		var col=new GridColumn(Lan.g("TablePopupsForFamily","Patient"),120);
		...
		for(var i=0;i<_listPopups.Count;i++) {
			...
			if(_listPopups[i].PopupLevel==EnumPopupLevel.Automation) {
				...
				var popupEvent=_listPopupEvents.FirstOrDefault(x => x.PopupNum==_listPopups[i].PopupNum);
