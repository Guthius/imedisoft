...
public partial class FormPayConnect:FormODBase {
	...
	private bool VerifyData(){
		...
		var paytype=ProgramProperties.GetPropVal(_program.ProgramNum,"PaymentType",_clinicNum);
		...
		var password=CDT.Class1.TryDecrypt(ProgramProperties.GetPropVal(_program.ProgramNum,"Password",_clinicNum));
	...
	private bool ProcessPaymentWebService(int expYear,int expMonth) {
		...
		if(!_creditCard.IsNew && _transResponse.PaymentToken!=null) {
			var transactionTokenExpField=new DateTime(_transResponse.PaymentToken.Expiration.year,_transResponse.PaymentToken.Expiration.month,
