...
public partial class FormEServicesAutoMsging:FormODBase {
	...
	private ApptReminderType[] _apptReminderTypeArray=
	[
		ApptReminderType.Reminder
	...
	private void FormEServicesAutoMsging_Load(object sender,EventArgs e) {
		...
		//Most controls were disabled in a hardcoded list before, but the Activate New Pat Thank You button was not, which was likely a bug.
		//Using DisableAllExcept ensures all controls are disabled.
	...
	private void SaveToDb() {
		var listApptReminderRules=_listApptReminderRules.FindAll(x => x.ClinicNum==_clinic.Id);
		ApptReminderRules.SyncByClinicAndTypes(listApptReminderRules,_clinic.Id,_apptReminderTypeArray);
		if(_clinic.Id!=0) {
	...
	private void FillRemindConfirmData() {
		...
		var listApptReminderRules=new List<ApptReminderRule>();
		long clinicNum=0;
		for(var i=0;i<_apptReminderTypeArray.Length;i++) {
			var useDefaultForApptType=GetUseDefaultForApptType(_apptReminderTypeArray[i],_clinic.Id);
			if(useDefaultForApptType) {
				clinicNum=0;
			}
			else {
				clinicNum=_clinic.Id;
			}
		...
		for(var i=0;i<listApptReminderRules.Count();i++)  {
			var listSendOrders=listApptReminderRules[i].SendOrder.Split(',').Select(x => Enum.Parse(typeof(CommType),x).ToString()).ToList();
			var sendOrder=string.Join(", ",listSendOrders);
			if(!listApptReminderRules[i].Language.IsNullOrEmpty()) {
				continue;//only show default langauges in this grid.
			}
			row=new GridRow();
			var useDefaultForApptType=GetUseDefaultForApptType(listApptReminderRules[i].TypeCur,_clinic.Id);
			if(_clinic.Id>0 && useDefaultForApptType) {
			...
			var strTSPrior=DateTools.ToStringDH(new TimeSpan(ticksTSPrior));
		...
		checkUseDefaultsArrival.Checked=GetIsClinicPrefEnabled(PrefName.ApptArrivalUseDefaults,_clinic.Id);
		checkUseDefaultsConfirmation.Checked=GetIsClinicPrefEnabled(PrefName.ApptConfirmUseDefaults,_clinic.Id);
		checkUseDefaultsInviteBefore.Checked=GetIsClinicPrefEnabled(PrefName.PatientPortalInviteUseDefaults,_clinic.Id);
		checkUseDefaultsInvite.Checked=GetIsClinicPrefEnabled(PrefName.PatientPortalInviteUseDefaults,_clinic.Id);
		checkUseDefaultsReminder.Checked=GetIsClinicPrefEnabled(PrefName.ApptReminderUseDefaults,_clinic.Id);
		checkUseDefaultThanks.Checked=GetIsClinicPrefEnabled(PrefName.ApptThankYouUseDefaults,_clinic.Id);
		checkUseDefaultNewPatTY.Checked=GetIsClinicPrefEnabled(PrefName.ApptNewPatientThankYouUseDefaults,_clinic.Id);
		checkUseDefaultsGeneralMessage.Checked=GetIsClinicPrefEnabled(PrefName.ApptGeneralMessageUseDefaults,_clinic.Id);
	...
	private void menuItemAdvancedSettings_Click(object sender,EventArgs e) {
		using var formEServicesAutoMsgingAdvanced=new FormEServicesAutoMsgingAdvanced();
	...
	private void GridRemindersMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		...
		var apptReminderRule=(ApptReminderRule)gridRemindersMain.ListGridRows[e.Row].Tag;
		var useDefaultForApptType=GetUseDefaultForApptType(apptReminderRule.TypeCur,_clinic.Id);
		if(_clinic.Id>0 && useDefaultForApptType && !SwitchFromDefaults(apptReminderRule.TypeCur)) {
			return;
		}
		var listApptReminderRules=_listApptReminderRules.FindAll(x => x.ClinicNum==apptReminderRule.ClinicNum && x.TypeCur==apptReminderRule.TypeCur);
		using var formApptReminderRuleEdit = new FormApptReminderRuleEdit(apptReminderRule,listApptReminderRules);
		...
		if(formApptReminderRuleEdit.ApptReminderRuleCur==null) {//Delete
			...
			var idxRule=_listApptReminderRules.IndexOf(apptReminderRule);
			...
			var listApptReminderRulesLanguage=formApptReminderRuleEdit.GetListLanguageRules().FindAll(x => x.ApptReminderRuleNum!=0);
			...
			for(var i = 0;i<listApptReminderRulesLanguage.Count;i++) {
				var index=_listApptReminderRules.IndexOf(listApptReminderRulesLanguage[i]);
	...
	private void AddRule(ApptReminderType apptReminderType,bool isBeforeAppointment=true) {
		var useDefaultForApptType=GetUseDefaultForApptType(apptReminderType,_clinic.Id);
		if(_clinic.Id>0 && useDefaultForApptType) {
		...
		var apptReminderRule=ApptReminderRules.CreateDefaultReminderRule(apptReminderType,_clinic.Id,isBeforeAppointment);
		var listApptReminderRulesClinic=_listApptReminderRules.FindAll(x => x.ClinicNum==apptReminderRule.ClinicNum);
		using var formApptReminderRuleEdit=new FormApptReminderRuleEdit(apptReminderRule,listApptReminderRulesClinic);
		formApptReminderRuleEdit.ShowDialog();
		if(formApptReminderRuleEdit.DialogResult!=DialogResult.OK) {
			return;
		}
	...
	private void AddRemoveLanguageRules(FormApptReminderRuleEdit formApptReminderRuleEdit) {
		if(formApptReminderRuleEdit.ApptReminderRuleCur!=null) {
			for(var i=0; i<formApptReminderRuleEdit.ListApptReminderRulesNonDefaultAdded.Count; i++) {
				_listApptReminderRules.Add(formApptReminderRuleEdit.ListApptReminderRulesNonDefaultAdded[i]);
			}
		}
		for(var i=0;i<formApptReminderRuleEdit.ListApptReminderRulesNonDefaultRemoving.Count;i++) {
	...
	private bool SwitchFromDefaults(ApptReminderType apptReminderType) {
		...
		var listApptReminderRulesDefault=_listApptReminderRules.FindAll(x => x.ClinicNum==0 && x.TypeCur==apptReminderType).Select(x => x.Copy()).ToList();
		_listApptReminderRules.RemoveAll(x => x.ClinicNum==_clinic.Id && x.TypeCur==apptReminderType);
		for(var i=0;i<listApptReminderRulesDefault.Count;i++) {
			listApptReminderRulesDefault[i].ClinicNum=_clinic.Id;
		}
		_listApptReminderRules.AddRange(listApptReminderRulesDefault);
		var prefName=GetPrefNameForApptReminderType(apptReminderType);
		var clinicPref=_listClinicPrefs.FirstOrDefault(x => x.ClinicNum==_clinic.Id && x.PrefName==prefName);
		if(clinicPref==null) {
	...
	private bool SwitchToDefaults(ApptReminderType apptReminderType) {
		if(_listApptReminderRules.Any(x => x.ClinicNum==_clinic.Id && x.TypeCur==apptReminderType)) {
			var prompt=Lans.g("Delete custom ")+apptReminderType.GetDescription()+Lans.g(" rules for this clinic and switch to using defaults? This cannot be undone.");
			if(ODMessageBox.Show(this,prompt,"",MessageBoxButtons.OKCancel)!=DialogResult.OK) {
				return false;
			}
		}
		_listApptReminderRules.RemoveAll(x => x.TypeCur==apptReminderType && x.ClinicNum==_clinic.Id);
		return true;
	}
	...
	private bool ActivateEService(PrefName prefNameEnabled,ApptReminderType apptReminderType,eServiceCode? eServiceCodeValidate=null) {		
		var isAutoEnabled=PrefC.GetBool(prefNameEnabled);	
		if(!isAutoEnabled && eServiceCodeValidate!=null) { //Not yet activated with HQ.
			var isEserviceActive=WebServiceMainHQProxy.IsEServiceActive(_signupOut,(eServiceCode)eServiceCodeValidate);
			if(!isEserviceActive) {
	...
	private void AddDefaults(ApptReminderType apptReminderType,params TimeSpan?[] arrayTimeSpans) {
		if(_listApptReminderRules.FindAll(x => x.ClinicNum == 0).Count(x => x.TypeCur==apptReminderType)==0) {
			...
			for(var i=0;i<arrayTimeSpans.Length;i++) {
				var apptReminderRule=ApptReminderRules.CreateDefaultReminderRule(apptReminderType,0);
	...
	private bool GetIsClinicPrefEnabled(PrefName prefName,long clinicNum) {
		var clinicPref=_listClinicPrefs.FirstOrDefault(x => x.PrefName==prefName && x.ClinicNum==clinicNum);
	...
	private void ButSave_Click(object sender,EventArgs e) {
		...
		var listClinicNums=_listApptReminderRules.Select(x => x.ClinicNum).Distinct().ToList();
		for(var i=0;i<listClinicNums.Count;i++) {
			var listApptReminderRulesClinicDefaults=_listApptReminderRules			//List of all active default rules for the clinic
				.FindAll(x => x.ClinicNum==listClinicNums[i]
				              && x.Language==string.Empty
				              && x.IsEnabled);
			for(var k=0;k<listApptReminderRulesClinicDefaults.Count;k++) {
				...
				{
					ODMessageBox.Show(this,Lans.g("Duplicate rules are not allowed for the same type and days."));
					return;
				}
			}
		}
