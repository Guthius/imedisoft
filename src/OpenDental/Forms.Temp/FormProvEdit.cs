...
public partial class FormProvEdit : FormODBase {
	...
	private List<ProviderClinicLink> _listProviderClinicLinks= [];
	///<summary>The clinics this user has access to.</summary>
	private List<ClinicDto> _listClinicsForUser= [];
	...
	private void FormProvEdit_Load(object sender, System.EventArgs e) {
		...
		_listProviderClinicsOld=ProviderClinics.GetListForProvider(ProviderCur.ProvNum,
			_listClinicsForUser.Select(x => x.Id)
		...
		if(true) {
			...
			var doSelectAll=(_listProviderClinicLinks.Count==0 || _listClinicsForUser.All(x => _listProviderClinicLinks.Any(y => y.ClinicNum==x.Id)));
			var listClinicsForUserUnhidden=_listClinicsForUser.FindAll(x => !x.IsHidden);
			listBoxClinics.Items.AddList(listClinicsForUserUnhidden,x => x.Abbr);
			var listClinicNumsForClinicLinks=_listProviderClinicLinks.Select(x => x.ClinicNum).ToList();;
			for(var i=0;i<listClinicsForUserUnhidden.Count;i++) {
				if(!doSelectAll && listClinicNumsForClinicLinks.Contains(listClinicsForUserUnhidden[i].Id)) {
	...
	private void FillImage() {
		...
		var fullImagePath=Path.Combine(ImageStore.GetProviderImagesFolder(),ProviderCur.WebSchedImageLocation);
		try {
			pictureWebSched.Image=FileAtoZ.GetImage(fullImagePath);
		}
		catch(Exception ex) {
			Shown+=new EventHandler((_,_) => FriendlyException.Show(Lans.g("Unable to display image."),ex));
		}
	}
	...
	private void FillGridProvIdent() {
		...
		var col=new GridColumn(Lan.g("TableQueue","Payor ID"),90,HorizontalAlignment.Center);
		...
		GridRow row;
		for(var i=0;i<ProviderIdentArray.Length;i++)	{
			row=new GridRow();
	...
	private void gridProvIdent_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		var frmProviderIdentEdit=new FrmProviderIdentEdit();
	...
	private void butDelete_Click(object sender, System.EventArgs e) {
		var rowSelected=gridProvIdent.GetSelectedIndex();
		if(rowSelected==-1){
	...
	private void butPickPict_Click(object sender,EventArgs e) {
		...
		var atoZFileName=Path.Combine(ImageStore.GetProviderImagesFolder(),Path.GetFileName(localFileName));
		if(File.Exists(atoZFileName)) {
			var attempts=1;
			var newAtoZFileName=FileAtoZ.AppendSuffix(atoZFileName,"_"+attempts);
			while(File.Exists(newAtoZFileName)) {
				if(attempts++ > 1000) {
		...
		try {
			File.Copy(localFileName,atoZFileName);
		}
		catch(Exception ex) {
			FriendlyException.Show(Lans.g("Unable to upload image."),ex);
			return;
		}
		...
		try {
			...
			FriendlyException.Show(Lans.g("Unable to display image."),ex);
		}
	}
	...
	private void butClinicOverrides_Click(object sender,EventArgs e) {
		...
		using var FormProvAdditional=new FormProvAdditional(_listProviderClinicsNew,ProviderCur);
	...
	private void butSave_Click(object sender,System.EventArgs e) {
		...
		if(checkIsHidden.Checked) {
			...
			var listApptViewItems=ApptViewItems.GetForProvider(ProviderCur.ProvNum);
			if(!listApptViewItems.IsNullOrEmpty()) {	
				...
				if(!listApptView.IsNullOrEmpty()) {
					...
					var msg=Lans.g("Not allowed to hide a Provider associated to an Appointment View.");
					msg+=("\r\n"+Lans.g("To continue remove them from the following Appointment View(s):"));
		...
		if(provNumClaimBillingOverride!=0) {
			var providerClaimBillingOverride=comboProv.GetSelected<Provider>();
		...
		if(IsNew) {
			var provNum=Providers.Insert(ProviderCur);
			...
			if(ProviderCur.DateTerm.Year > 1880 && ProviderCur.DateTerm < DateTime.Now) {
				var listClaimPaySplits=Claims.GetOutstandingClaimsByProvider(ProviderCur.ProvNum,ProviderCur.DateTerm);
		...
		if(true) {
			var listClinicNumsSelectedLinks=listBoxClinics.GetListSelected<ClinicDto>().Select(x => x.Id).ToList();
			var listClinicsAll=Clinics.GetDeepCopy();
			var listClinicNumsForUser=_listClinicsForUser.Select(x => x.Id).ToList();
			var canUserAccessAllClinics=(_listClinicsForUser.Count==listClinicsAll.Count);
			if(checkAllClinics.Checked) {
				if(canUserAccessAllClinics) {
					...
					listClinicNumsSelectedLinks=_listClinicsForUser.Select(x => x.Id).ToList();
				}
			}
				   ...
				   && !_listProviderClinicLinks.Any(x => x.ClinicNum > -1 && !listClinicNumsForUser.Contains(x.ClinicNum))) 
				{
					...
					listClinicNumsSelectedLinks.AddRange(listClinicsAll.Where(x => !listClinicNumsForUser.Contains(x.Id)).Select(x => x.Id));
				}
			}
			var listProviderClinicLinksNew=_listProviderClinicLinks.Select(x => x.Copy()).ToList();
