...
public partial class FormIncomeTransferManage:FormODBase {
	...
	private void FillTransfers() {
		var translationName=gridTransfers.TranslationName;
		...
		var listPaymentTransfers=Payments.GetTransfers(_family.GetPatNums()).OrderBy(x => x.PayDate).ToList();
		for(var i = 0;i<listPaymentTransfers.Count;i++) {
			var row=new GridRow();
	...
	private void FillGridCharges(bool doRefreshData=true) {
		...
		if(_constructResults==null || doRefreshData) {
			_constructResults=PaymentEdit.ConstructAndLinkChargeCredits(_family.GetPatNums(),_patient.PatNum,
				[],new Payment(), [],isIncomeTxfr:true,dateAsOf:datePickerAsOf.Value);
		}
		//Get information for all patients including those not currently present within the family.
		var listPatNums=_constructResults.ListAccountEntries.Select(x => x.PatNum).Distinct().ToList();
		var listPatients=Patients.GetLimForPats(listPatNums);
		//Group up account entries by Pat/Prov/Clinic
		var listListsAccountEntries=_constructResults.ListAccountEntries
	...
	private List<GridRow> GetRowsForGroup(List<AccountEntry> listAccountEntries,List<Patient> listPatients) {
		...
		var listPositiveEntries=listAccountEntries.FindAll(x => CompareDecimal.IsGreaterThanZero(x.AmountEnd));
		row.Cells.Add(listPositiveEntries.Sum(x => x.AmountEnd).ToString("c"));
		var listNegativeEntries=listAccountEntries.FindAll(x => CompareDecimal.IsLessThanZero(x.AmountEnd));
	...
	private void CreatePaymentAndRefresh(List<PaySplit> listPaySplits,bool isRigorous) {
		...
		var logText=Payments.GetSecuritylogEntryText(payment,payment,isNew:true)+", "+Lans.g($"from Income Transfer Manager using {strLogic}.");
		SecurityLogs.MakeLogEntry(EnumPermType.PaymentCreate,payment.PatNum,logText);
		var strErrorMsg=Ledgers.ComputeAgingForPaysplitsAllocatedToDiffPats(_patient.PatNum,listPaySplits);
	...
	private void butTransfer_Click(object sender,EventArgs e) {
		...
		var listAccountEntries=_constructResults.ListAccountEntries.Select(x => x.Copy()).ToList();
		if(!PaymentEdit.TryCreateIncomeTransfer(listAccountEntries,datePickerAsOf.Value,out var results)) {
