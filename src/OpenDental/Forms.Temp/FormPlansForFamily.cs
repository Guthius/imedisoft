...
public partial class FormPlansForFamily : FormODBase {
	...
	private void FillGrid(){
		...
		for(var i=0;i<_listInsSubs.Count;i++){
			...
			var countPatPlans=PatPlans.GetCountBySubNum(_listInsSubs[i].InsSubNum);
	...
	private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		var insPlan=InsPlans.GetPlan(_listInsSubs[e.Row].PlanNum,_listInsPlans);
		using var formInsPlan=new FormInsPlan(insPlan,null,_listInsSubs[e.Row]);
