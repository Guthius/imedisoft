...
public partial class FormClaimOverpay:FormODBase {
	...
	private void FillGrid(){
		...
		var claimProcSelected=_listClaimProcs.Find(x => x.ClaimProcNum==_selectedClaimProcNum);//Retrieve the claimproc that should be pre-selected in the grid.
		...
		GridRow row;
		var selectedIndex=0;
		for(var i=0;i<_listPaymentClaimProcs.Count;i++) {//Loop through the non-under/overpayment claimprocs to insert them into the grid.
			row=new GridRow();
			...
			var procedure=Procedures.GetProcFromList(_listProcedures,_listPaymentClaimProcs[i].ProcNum);//will return a new procedure if none found.
			...
			var insPayAmt=_listClaimProcs.FindAll(x => x.ProcNum==_listPaymentClaimProcs[i].ProcNum && !x.IsOverpay).Sum(x => x.InsPayAmt);
			row.Cells.Add(insPayAmt.ToString("F"));
			var claimProcOverpay=_listClaimProcs//Should be at-most one overpayment claimproc per regular claimproc
	...
	private bool IsValid() {
		...
		for(var i=0;i<gridMain.ListGridRows.Count;i++){
			var insOverpaidAmount=SIn.Double(gridMain.ListGridRows[i].Cells[_colInsOverpaidIndex].Text);
			var insUnderpaidAmount=SIn.Double(gridMain.ListGridRows[i].Cells[_colInsUnderpaidIndex].Text);
	...
	private void Save() {
		var listDeleteClaimProcs=new List<ClaimProc>();
		var listUpdateClaimProcs=new List<ClaimProc>();
		var listInsertClaimProcs=new List<ClaimProc>();
		var listRequiredCanadaParentProcNums=new List<long>();
		for(var i=0;i<gridMain.ListGridRows.Count;i++){
			var claimProc=(ClaimProc)gridMain.ListGridRows[i].Tag;
			var claimProcOverpay=_listClaimProcs.Find(x => x.ClaimNum==claimProc.ClaimNum && x.ProcNum==claimProc.ProcNum && x.IsOverpay);
			var overpaidAmt=SIn.Double(gridMain.ListGridRows[i].Cells[_colInsOverpaidIndex].Text);
			var underpaidAmt=SIn.Double(gridMain.ListGridRows[i].Cells[_colInsUnderpaidIndex].Text);
			if(overpaidAmt==0 && underpaidAmt==0) {
				...
				var isOverpaid=(overpaidAmt!=0);//overpaidAmt cannot be negative, because of validation before saving.
				var insEstTotalOverride=isOverpaid?(-overpaidAmt):underpaidAmt;
				...
				var procOverpay=_listProcedures.FirstOrDefault(x => x.ProcNum==claimProcOverpay.ProcNum);
		...
		for(var i=0;i<listRequiredCanadaParentProcNums.Count;i++) {
			var claimProc=_listPaymentClaimProcs.FirstOrDefault(x => x.ProcNum==listRequiredCanadaParentProcNums[i]);
			var claimProcOverpay=_listClaimProcs.Find(x => x.ClaimNum==claimProc.ClaimNum && x.ProcNum==claimProc.ProcNum && x.IsOverpay);
	...
	private void butToSupplemental_Click(object sender,EventArgs e) {
		...
		Save();
		_listClaimProcs=ClaimProcs.RefreshForClaims([_claimNum]).ToList();
		_listClaimProcs=ClaimProcs.GetForClaimOverpay(_listClaimProcs,_claimNum);
		var listClaimProcsOverpaid=_listClaimProcs.FindAll(x => x.IsOverpay);
		if(listClaimProcsOverpaid.Count==0) {
			MsgBox.Show(this,"There are no pending supplemental insurance payments for this claim.");
			return;
		}
		for(var i=0;i<listClaimProcsOverpaid.Count;i++) {
			var claimProcOld=listClaimProcsOverpaid[i].Copy();
