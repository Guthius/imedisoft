...
public partial class FormMedicationEdit : FormODBase {
	...
	private List<string> _listPatNameMeds;
	private string[] _stringArrayPatNameAllergies;
		
	private List<string> _listBrands;
	...
	private void FillForm() {
		...
		if(MedicationCur.MedicationNum==MedicationCur.GenericNum) {
			...
			_listBrands=Medications.GetBrands(MedicationCur.MedicationNum);
			comboBrands.Items.Clear();
			comboBrands.Items.AddList(_listBrands);
			if(_listBrands.Count>0) {
		...
		_listPatNameMeds=Medications.GetPatNamesForMed(MedicationCur.MedicationNum);
		comboPatients.Items.Clear();
		comboPatients.Items.AddList(_listPatNameMeds);
		if(_listPatNameMeds.Count>0) {
			comboPatients.SelectedIndex=0;
		}
		var allergyDef=AllergyDefs.GetAllergyDefFromMedication(MedicationCur.MedicationNum);
		if(allergyDef!=null) {
	...
	private void butSave_Click(object sender, System.EventArgs e) {
		...
		if(CultureInfo.CurrentCulture.Name.EndsWith("US")) {//United States
			                                                                      ...
			                                                                      +"Click OK to continue without an RxNorm, or click Cancel to stay in this window."))
			{
				...
				var listMedicationsExistingMeds=Medications.GetAllMedsByRxCui(MedicationCur.RxCui);
