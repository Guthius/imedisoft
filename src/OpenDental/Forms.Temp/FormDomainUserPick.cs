...
public partial class FormDomainUserPick:FormODBase {
	...
	private void FillGrid() {
		var searchResultCollection=GetUsers();
		...
		GridColumn col;
		for(var i=0;i<_stringArrayFields.Length;i++) {
		...
		for(var i=0;i<searchResultCollection.Count;i++){
			row=new GridRow();
			row.Tag="";
			for(var j=0;j<_stringArrayFields.Length;j++) {
	...
	private SearchResultCollection GetUsers() {
		using var directoryEntry=new DirectoryEntry(PrefC.GetString(PrefName.DomainLoginPath));
