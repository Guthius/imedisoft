...
public partial class FormClaimAttachPasteDXC:FormODBase {
	...
	private List<AttachmentItem> _listAttachmentItems= [];
	...
	private void FormClaimAttachPasteDXC_Load(object sender,EventArgs e) {
		...
		var bitmap=_listAttachmentItems[0].Bitmap;
		if(bitmap==null) {
			return;
		}
	...
	private void FillGrid() {
		...
		GridRow gridRow;
		for(var i=0;i<_listAttachmentItems.Count;i++) {
			gridRow=new GridRow();
	...
	private bool GetImagesFromClipboard() {
		...
		var bitmap=ODClipboard.GetImage();//Null if no bitmap on clipboard
		...
		for(var i=0;i<listFilePaths.Count;i++) {
	...
	private void BatchAddAttachments(List<ClaimConnect.ImageAttachment> listImageAttachments) {
		for(var i=0;i<listImageAttachments.Count;i++) {
			var listImageAttachmentsOnlyOne=new List<ClaimConnect.ImageAttachment>();
	...
	private bool SendAttachmentsToDXCAndSaveLocally() {
		...
		for(var i=0;i<listImageAttachments.Count;i++) {
			if(PrefC.GetBool(PrefName.SaveDXCAttachments)) {
				...
				var documentCur=ImageStore.Import(listImageAttachments[i].ImageFileAsBase64,defNumImageType,ImageType.Attachment,PatientCur);
	...
	private void gridMain_CellClick(object sender,ODGridClickEventArgs e) {
		var bitmap=_listAttachmentItems[e.Row].Bitmap;
		if(bitmap==null) {
	...
	private void contextMenuImageGrid_Popup(object sender,EventArgs e) {
		var idxAttachSelected=gridMain.GetSelectedIndex();
		if(idxAttachSelected==-1) {
			return;
		}
		var bitmap=_listAttachmentItems[idxAttachSelected].Bitmap;
		if(bitmap==null) {
			MsgBox.Show("Cannot load the image for this file");
			return;
		}
