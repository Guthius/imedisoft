...
public partial class FormWebSchedAppts:FormODBase {
	...
	private void FormWebSchedAppts_Load(object sender,System.EventArgs e) {
		...
		var defaultStatus=PrefC.GetLong(PrefName.WebSchedNewPatConfirmStatus);
	...
	private void FillGrid() {
		...
		for(var i=0;i<table.Rows.Count;i++) { 
			...
			var dateTimeCreated=SIn.DateTime(table.Rows[i]["DateTimeCreated"].ToString());
			var aptDateTime=SIn.DateTime(table.Rows[i]["AptDateTime"].ToString());
			var birthdate=SIn.Date(table.Rows[i]["Birthdate"].ToString());
	...
	private DataTable GetAppointments() {
		var listProvNums=Providers.GetDeepCopy(isShort:true).Select(x => x.ProvNum).ToList();
		var listStatus=comboConfStatus.GetListSelected<long>();
		var table=RpAppointments.GetAppointmentTable(
	...
	private void mainGridMenuItemPatChart_Click(object sender,EventArgs e) {
		var listRowsSelected=gridMain.SelectedGridRows;
		if(listRowsSelected.Count==1) {
			var aptNum=SIn.Long(listRowsSelected[0].Tag.ToString());
	...
	private void OpenEditAppointmentWindow(GridRow row) {
		var aptNum=SIn.Long(row.Tag.ToString());
