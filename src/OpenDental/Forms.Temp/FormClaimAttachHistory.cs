...
public partial class FormClaimAttachHistory:FormODBase {
	...
	private void ClearAttachmentID() {
		...
		var oldAttachmentID=ClaimCur.AttachmentID;
		ClaimCur.AttachmentID="";
		ClaimCur.Narrative="";//Clear narrative for new attachment
		ClaimCur.Attachments= [];//Empty ClaimAttach list
		var secDateTEdit=ClaimCur.SecDateTEdit;//Preserve the date prior to any claim updates affecting it.
	...
	private bool ValidateXConnect() {
		...
		//Usually super fast, but with a web call, they need a way to cancel if they lock up
		...
		for(var i=0;i<xConnectWebResponse.response.claimItems.Count();i++) {
			...
			if(!string.IsNullOrEmpty(xConnectClaimItemResponse.itemStatus.message)) {
				textClaimStatus.Text=xConnectClaimItemResponse.itemStatus.message;//This message contains the attachment requirements
				return false;
			}
		}
		textClaimStatus.Text="The claim is valid";
	...
	private void FillGrid() {
		...
		GridRow gridRow;
		for(var i=0;i<ClaimCur.Attachments.Count;i++) {
			gridRow=new GridRow();
	...
	private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		...
		var patFolder=ImageStore.GetPatientFolder(PatientCur,ImageStore.GetDataFolder());
		//Local storage
		var pathAndFileName=ODFileUtils.CombinePaths(patFolder,claimAttach.ActualFileName);
		try {
	...
	private void buttonDeleteImageAttachments_Click(object sender,EventArgs e) {
		...
		var counterOldClaimAttach=0;//Track the number of Claim Attaches with a bad ImageReferenceId
