...
public partial class FormPaySplitEdit : FormODBase {
	...
	///<summary>A list of all account entries when this window was launched from the Payment window.</summary>
	...
	///<summary>A list of all payment plan related account entries when this window was launched from the Payment window.</summary>
	private List<FauxAccountEntry> _listFauxAccountEntries= [];
	#endregion
	...
	private void FormPaySplitEdit_Load(object sender, System.EventArgs e) {
		var listPatientLinks=PatientLinks.GetLinks(_family.ListPats.Select(x => x.PatNum).ToList(),PatientLinkType.Merge);
		var listPatientsNonMerged=_family.ListPats.Where(x => !PatientLinks.WasPatientMerged(x.PatNum,listPatientLinks)).ToList();
		...
		if(listPatientsNonMerged.Count>0) {
			...
			var listPatientsOnlyGuarantor=new List<Patient>();
		...
		_listFauxAccountEntries=ListAccountEntries.FindAll(x => x.GetType()==typeof(FauxAccountEntry))
			.Select(x => x as FauxAccountEntry)
			.ToList();
		SetEnabledProc();
		SetCheckPayPlanEnabled();
		FillPatient();
		FillProcedure();
		FillAdjustment();
	}
	...
	///<summary>Sets the Enabled property on the 'Attached to Payment Plan' check box accordingly.</summary>
	private void SetCheckPayPlanEnabled() {
		checkPayPlan.Enabled=false;
		//Enable the box when it is already checked so that it can be unchecked if desired.
		if(checkPayPlan.Checked) {
			checkPayPlan.Enabled=true;
			return;
		}
		//Enable the box when the user knows better than us.
		if(_isEditAnyway) {
			checkPayPlan.Enabled=true;
			return;
		}
		//Enable the box if there is at least one patient payment plan detected.
		if(_listFauxAccountEntries.Any(x => !x.IsDynamic)) {
			checkPayPlan.Enabled=true;
			return;
		}
		//Enable the box when there is an outstanding charge.
		for(var i=0;i<_listFauxAccountEntries.Count;i++) {
			if(_listFauxAccountEntries[i].AmountEnd<=0) {
				continue;
			}
			if(_procedure!=null && _listFauxAccountEntries[i].ProcNum!=_procedure.ProcNum) {
				continue;
			}
			if(_adjustment!=null && _listFauxAccountEntries[i].AdjNum!=_adjustment.AdjNum) {
				continue;
			}
			checkPayPlan.Enabled=true;
			return;
		}
		//Enable the box when there is a dynamic payment plan with no charges at all.
		var listFauxAccountEntriesDynamic=_listFauxAccountEntries.FindAll(x => x.IsDynamic);
		//Look for any dynamic payment plan that only has credits (no debits).
		var hasNoDebits=listFauxAccountEntriesDynamic.GroupBy(x => x.PayPlanNum)
			.Any(x => x.All(x => x.ChargeType==PayPlanChargeType.Credit));
		if(hasNoDebits) {
			checkPayPlan.Enabled=true;
		}
	}
	...
	private void UpdateUnearnedType() {
	...
	private void FillPatient(){
		...
		if(PaySplitCur.PatNum==0){
			...
			for(var i=0;i<_family.ListPats.Length;i++) {//This is the same order added to listPatient.
	...
	private void FillProcedure(){
		...
		var listClaimProcs=ClaimProcs.Refresh(_procedure.PatNum);
		var adjustmentArray=Adjustments.Refresh(_procedure.PatNum);
		var listPaySplitsForProc=PaySplits.Refresh(_procedure.PatNum).Where(x => x.ProcNum==_procedure.ProcNum).ToList();
		var listPaySplitsForProcPaymentWindow=ListPaySplits.Where(x => x.ProcNum==_procedure.ProcNum && x.PayNum==PaySplitCur.PayNum).ToList();
		...
		var procWriteoff=-ClaimProcs.ProcWriteoff(listClaimProcs,_procedure.ProcNum);
		var procInsPaid=-ClaimProcs.ProcInsPay(listClaimProcs,_procedure.ProcNum);
		var procInsEst=-ClaimProcs.ProcEstNotReceived(listClaimProcs,_procedure.ProcNum);
		var procAdj=Adjustments.GetTotForProc(_procedure.ProcNum,adjustmentArray);
		//next line will still work even if IsNew
		int countSplitsAttached;
		var procPrevPaid=-PaySplits.GetTotForProc(_procedure.ProcNum,listPaySplitsForProc.ToArray(),PaySplitCur,out countSplitsAttached);
	...
	private void FillAdjustment() {
		...
		var listPaySplits=ListPaySplits.FindAll(x => x.AdjNum==_adjustment.AdjNum);
	...
	private void butAttachProc_Click(object sender, System.EventArgs e) {
		using var formProcSelect=new FormProcSelect(PaySplitCur.PatNum);
	...
	private void butAttachAdjust_Click(object sender,EventArgs e) {
		var listAdjustmentsPat=Adjustments.GetAdjustForPats([PaySplitCur.PatNum]);
		var listPaySplitsAdj=PaySplits.GetForAdjustments(listAdjustmentsPat.Select(x => x.AdjNum).ToList());
		using var formAdjustSelect=new FormAdjustSelect(SIn.Double(textAmount.Text),PaySplitCur,ListPaySplits,listAdjustmentsPat,listPaySplitsAdj);
		if(formAdjustSelect.ShowDialog()!=DialogResult.OK) {
			return;
		}
		_adjustment=formAdjustSelect.SelectedAdjustment;
	...
	private void checkPayPlan_Click(object sender, System.EventArgs e) {
		...
		var listPayPlans=PayPlans.GetForPatNum(_family.ListPats[listPatient.SelectedIndex].PatNum).FindAll(x => x.PlanNum==0);
	...
	///<summary>Returns true if the current payment split had the payment plan fields set to the oldest outstanding payment plan charge.
	///Returns false if the current payment split was completely left alone (no payment plan fields were manipulated).</summary>
	private void AttachPayPlan(PayPlan payPlan) {
		//Production can only be associated with one payment plan at a time.
		//If the user has selected a piece of production (procedure or adjustment) then automatically find a payment plan charge for said production.
		//Prefer the oldest charge with an amount due or use the oldest charge associated with the production.
		FauxAccountEntry fauxAccountEntryOutstanding=null;
		var listFauxAccountEntries=_listFauxAccountEntries.FindAll(x => x.PayPlanNum==payPlan.PayPlanNum);
		//Check to see if this dynamic payment plan has only credits (no debits).
		if(listFauxAccountEntries.IsNullOrEmpty() || listFauxAccountEntries.All(x => x.ChargeType==PayPlanChargeType.Credit)) {
			PaySplitCur.PayPlanNum=payPlan.PayPlanNum;
			PaySplitCur.PayPlanChargeNum=0;
			_procedure=null;
			_adjustment=null;
			//Synchronize the UI with the selected production object.
			FillProcedure();
			FillAdjustment();
			if(payPlan.IsDynamic) {
				//Create a split as prepayment and do not attach proc.
				PaySplitCur.UnearnedType=PrefC.GetLong(PrefName.DynamicPayPlanPrepaymentUnearnedType);
				//Definition could be hidden but this combo will dynamically show the hidden definition.
				comboUnearnedTypes.SetSelectedDefNum(PaySplitCur.UnearnedType);
				UpdateUnearnedType();
				MsgBox.Show("Selected Payment Plan does not have any outstanding charges. Payment Split will be created as a Payment Plan Prepayment.");
			}
			return;
		}
		var listFauxAccountEntriesForProduction=new List<FauxAccountEntry>();
		for(var i=0;i<listFauxAccountEntries.Count;i++) {
			if(_procedure!=null && listFauxAccountEntries[i].ProcNum!=_procedure.ProcNum) {
				continue;
			}
			if(_adjustment!=null && listFauxAccountEntries[i].AdjNum!=_adjustment.AdjNum) {
				continue;
			}
			listFauxAccountEntriesForProduction.Add(listFauxAccountEntries[i]);
			if(listFauxAccountEntries[i].AmountEnd<=0) {
				continue;
			}
			fauxAccountEntryOutstanding=listFauxAccountEntries[i];
			break;
		}
		FauxAccountEntry fauxAccountEntryOldest=null;
		if(fauxAccountEntryOutstanding==null && listFauxAccountEntriesForProduction.Count > 0) {
			fauxAccountEntryOldest=listFauxAccountEntriesForProduction.First();
		}
		if(fauxAccountEntryOldest==null) {
			//Look for the oldest oustanding account entry first.
			fauxAccountEntryOldest=listFauxAccountEntries.Find(x => CompareDecimal.IsGreaterThanZero(x.AmountEnd));
		}
		if(fauxAccountEntryOldest==null) {
			fauxAccountEntryOldest=listFauxAccountEntries.First();
		}
		PaySplitCur.PayPlanNum=fauxAccountEntryOldest.PayPlanNum;
		PaySplitCur.PayPlanChargeNum=fauxAccountEntryOldest.PayPlanChargeNum;
		//Automatically synchronize the production objects with the production that the payment plan charge is associated with.
		if(fauxAccountEntryOldest.ProcNum > 0) {
			var accountEntry=ListAccountEntries.Find(x => x.GetType()==typeof(Procedure) && x.ProcNum==fauxAccountEntryOldest.ProcNum);
			if(accountEntry!=null) {
				_procedure=(Procedure)accountEntry.Tag;
			}
		}
		else if(fauxAccountEntryOldest.AdjNum > 0) {
			var accountEntry=ListAccountEntries.Find(x => x.GetType()==typeof(Adjustment) && x.AdjNum==fauxAccountEntryOldest.AdjNum);
			if(accountEntry!=null) {
				_adjustment=(Adjustment)accountEntry.Tag;
			}
		}
		//Synchronize the UI with the selected production object.
		FillProcedure();
	...
	private void butSave_Click(object sender, System.EventArgs e) {
		...
		if(IsNew) {
			...
			var providerAbbr=Providers.GetAbbr(PaySplitCur.ProvNum);
