...
public partial class FormReportsMore:FormODBase {
	...
	private void FillLists() {
		...
		for(var i=0;i<_listDisplayReportsProdInc.Count;i++) {
			if(GroupPermissions.HasPermission(Security.CurUser,EnumPermType.Reports,_listDisplayReportsProdInc[i].DisplayReportNum,groupPermissions:_listGroupPermissions)) {
		...
		if(_listDisplayReportsProdInc.Count==0) {
			...
			listProdInc.Height=Math.Min((_listDisplayReportsProdInc.Count+1) * 15,listProdInc.Height);
		}
		//Daily----------------------------------------------------------------------------------------------------------------------------------------
		for(var i=0;i<_listDisplayReportsDaily.Count;i++) {
			if(GroupPermissions.HasPermission(Security.CurUser,EnumPermType.Reports,_listDisplayReportsDaily[i].DisplayReportNum,groupPermissions:_listGroupPermissions)) {
		...
		if(_listDisplayReportsDaily.Count==0) {
			...
			listDaily.Height=Math.Min((_listDisplayReportsDaily.Count+1) * 15,listDaily.Height);
		}
		//Monthly--------------------------------------------------------------------------------------------------------------------------------------
		for(var i=0;i<_listDisplayReportsMonthly.Count;i++) {
			if(GroupPermissions.HasPermission(Security.CurUser,EnumPermType.Reports,_listDisplayReportsMonthly[i].DisplayReportNum,groupPermissions:_listGroupPermissions)) {
		...
		if(_listDisplayReportsMonthly.Count==0) {
			...
			listMonthly.Height=Math.Min((_listDisplayReportsMonthly.Count+1) * 15,listMonthly.Height);
		}
		//Lists----------------------------------------------------------------------------------------------------------------------------------------
		for(var i=0;i<_listDisplayReportsList.Count;i++) {
			if(GroupPermissions.HasPermission(Security.CurUser,EnumPermType.Reports,_listDisplayReportsList[i].DisplayReportNum,groupPermissions:_listGroupPermissions)) {
		...
		if(_listDisplayReportsList.Count==0) {
			...
			listLists.Height=Math.Min((_listDisplayReportsList.Count+1) * 15,listLists.Height);
		}
		//Public Health--------------------------------------------------------------------------------------------------------------------------------
		for(var i=0;i<_listDisplayReportsPublicHealth.Count;i++) {
			if(GroupPermissions.HasPermission(Security.CurUser,EnumPermType.Reports,_listDisplayReportsPublicHealth[i].DisplayReportNum,groupPermissions:_listGroupPermissions)) {
		...
		if(_listDisplayReportsPublicHealth.Count==0) {
			...
			listPublicHealth.Height=Math.Min((_listDisplayReportsPublicHealth.Count+1) * 15,listPublicHealth.Height);
		}
		...
		for(var i=0;i<_listDisplayReportsArizonaPrimary.Count;i++) {
			if(GroupPermissions.HasPermission(Security.CurUser,EnumPermType.Reports,_listDisplayReportsArizonaPrimary[i].DisplayReportNum,groupPermissions:_listGroupPermissions)) {
		...
		if(_listDisplayReportsArizonaPrimary.Count==0 || !UsingArizonaPrimaryCare()) {
			...
			listArizonaPrimaryCare.Height=Math.Min((_listDisplayReportsArizonaPrimary.Count+1) * 15,49);
		}
	}
	...
	public static bool UsingArizonaPrimaryCare() {
		...
		var patientFieldNames=new[] {
			"SPID#",
			"Eligibility Status",
			"Household Gross Income",
			"Household % of Poverty",
		};
		var fieldCounts=new int[patientFieldNames.Length];
		var listPatFieldDefs=PatFieldDefs.GetDeepCopy(true);
		for(var i=0;i<listPatFieldDefs.Count;i++) {
			for(var j=0;j<patientFieldNames.Length;j++) {
	...
	private void butUserQuery_Click(object sender,EventArgs e) {
		...
		if(Security.IsAuthorized(EnumPermType.UserQueryAdmin,true)) {
			using var formUserQuery=new FormUserQuery(null);
			formUserQuery.ShowDialog();
			SecurityLogs.MakeLogEntry(EnumPermType.UserQuery,0,"");
		}
		else {
			...
			if(formQueryFavorites.DialogResult == DialogResult.OK) {
				using var formUserQuery=new FormUserQuery(formQueryFavorites.UserQueryCur.QueryText,true);
	...
	private void butPW_Click(object sender,EventArgs e) {
		try {
			if(!Programs.IsEnabledByHq(ProgramName.PracticeWebReports,out var err)) {
				MsgBox.Show(err);
				return;
			}
		...
		SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Practice Web");
	}

	private void listProdInc_MouseDown(object sender,MouseEventArgs e) {
		var selected=listProdInc.IndexFromPoint(e.Location);
		if(selected==-1) {
			return;
		}
		if(!GroupPermissions.HasPermission(Security.CurUser,EnumPermType.Reports,_listDisplayReportsProdInc[selected].DisplayReportNum,groupPermissions:_listGroupPermissions)) {
	...
	private void listDaily_MouseDown(object sender,MouseEventArgs e) {
		var selected=listDaily.IndexFromPoint(e.Location);
		if(selected==-1) {
			return;
		}
	...
	private void listMonthly_MouseDown(object sender,MouseEventArgs e) {
		var selected=listMonthly.IndexFromPoint(e.Location);
		if(selected==-1) {
			return;
		}
	...
	private void listLists_MouseDown(object sender,MouseEventArgs e) {
		var selected=listLists.IndexFromPoint(e.Location);
		if(selected==-1) {
			return;
		}
	...
	private void listPublicHealth_MouseDown(object sender,MouseEventArgs e) {
		var selected=listPublicHealth.IndexFromPoint(e.Location);
		if(selected==-1) {
			return;
		}
	...
	private void listArizonaPrimaryCare_MouseDown(object sender,MouseEventArgs e) {
		var selected=this.listArizonaPrimaryCare.IndexFromPoint(e.Location);
		if(selected==-1) {
			return;
		}
	...
	private void OpenReportLocalHelper(DisplayReport displayReport,List<GroupPermission> listReportPermissions,bool doValidatePerm=true)
	{
		...
		switch(displayReport.InternalName) {
			...
			case DisplayReports.ReportNames.ProceduresIndividual:
				Close();
				break;
		}
	}
		...
		,List<GroupPermission> listReportPermissions=null,bool doValidatePerm=true)
	{
		if(doValidatePerm) {
			...
			if(!GroupPermissions.HasPermission(Security.CurUser,EnumPermType.Reports,displayReport.DisplayReportNum,groupPermissions:listReportPermissions)) {
		...
		switch(displayReport.InternalName) {
			case "ODToday"://Today
				using(var formRpProdInc=new FormRpProdInc()) {
				...
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Production and Income report run for today.");
				break;
			case "ODYesterday"://Yesterday
				...
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Production and Income report run for yesterday.");
				break;
			case "ODThisMonth"://This Month
				...
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Production and Income report run for this month.");
				break;
			case "ODLastMonth"://Last Month
				...
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Production and Income report run for last month.");
				break;
			case "ODThisYear"://This Year
				...
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Production and Income report run for this year.");
				break;
			case "ODMoreOptions"://More Options
				using(var formRpProdInc = new FormRpProdInc()) {
					formRpProdInc.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Production and Income report 'more options' accessed.");
				break;
			case "ODProviderPayrollSummary":
				using(var formRpProviderPayroll=new FormRpProviderPayroll()) {
					formRpProviderPayroll.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Provider Payroll Summary report run.");
				break;
			case "ODProviderPayrollDetailed":
				using(var formRpProviderPayroll=new FormRpProviderPayroll(true)) {
					formRpProviderPayroll.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Provider Payroll Detailed report run.");
				break;
			case "ODAdjustments"://Adjustments
				if(Security.CurUser.ProvNum==0 && !Security.IsAuthorized(EnumPermType.ReportDailyAllProviders,true)) {
				...
				using(var formRpAdjSheet=new FormRpAdjSheet()) {
					formRpAdjSheet.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Adjustments report run.");
				break;
			case "ODPayments"://Payments
				...
				using(var formRpPaySheet=new FormRpPaySheet()) {
					formRpPaySheet.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Daily Payments report run.");
				break;
			case DisplayReports.ReportNames.ProceduresIndividual:
				if(Security.CurUser.ProvNum==0 && !Security.IsAuthorized(EnumPermType.ReportDailyAllProviders,true)) {
					MsgBox.Show("FormReportsMore","The current user needs to be a provider or have the 'All Providers' permission for Daily reports");
				...
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Daily Procedures report run.");
				return ReportNonModalSelection.ProceduresIndividual;
			case DisplayReports.ReportNames.ProductionByProcedure:
				if(Security.CurUser.ProvNum==0 && !Security.IsAuthorized(EnumPermType.ReportDailyAllProviders,true)) {
					MsgBox.Show("FormReportsMore","The current user needs to be a provider or have the 'All Providers' permission for Daily reports");
					break;
				}
				using(var formRpProdByProc=new FormRpProdByProc()) {
					formRpProdByProc.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Production by Procedure report run.");
				break;
			case DisplayReports.ReportNames.ODProcOverpaid:
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Procedures overpaid report run.");
			...
			case "ODWriteoffs"://Writeoffs
				...
				using(var formRpWriteoffSheet=new FormRpWriteoffSheet()) {
					formRpWriteoffSheet.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Daily Writeoffs report run.");
				break;
			case DisplayReports.ReportNames.IncompleteProcNotes://Incomplete Procedure Notes
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Daily Procedure Notes report run.");
			...
			case "ODRoutingSlips"://Routing Slips
				...
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Routing Slips report run.");
				break;
			case "ODNetProdDetailDaily":
				using(var formRpNetProdDetail=new FormRpNetProdDetail(true)) {
					formRpNetProdDetail.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Daily Net Prod report run.");
				break;
			case DisplayReports.ReportNames.UnfinalizedInsPay:
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Unfinalized Insurance Payment report run.");
				return ReportNonModalSelection.UnfinalizedInsPay;
			case DisplayReports.ReportNames.PatPortionUncollected:
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Patient Portion Uncollected report run.");
				return ReportNonModalSelection.PatPortionUncollected;
			case "ODAgingAR"://Aging of Accounts Receivable Report
				using(var formRpAging=new FormRpAging()) {
					formRpAging.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Aging of A/R report run.");
				break;
			case DisplayReports.ReportNames.ClaimsNotSent://Claims Not Sent
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Claims Not Sent report run.");
				return ReportNonModalSelection.UnsentClaim;
			case "ODCapitation"://Capitation Utilization
				using(var formRpCapitation=new FormRpCapitation()) {
					formRpCapitation.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Capitation report run.");
				break;
			case "ODFinanceCharge"://Finance Charge Report
				using(var formRpFinanceCharge=new FormRpFinanceCharge()) {
					formRpFinanceCharge.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Finance Charges report run.");
				break;
			case DisplayReports.ReportNames.OutstandingInsClaims://Outstanding Insurance Claims
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Outstanding Insurance Claims report run.");
			...
			case DisplayReports.ReportNames.ProcNotBilledIns://Procedures Not Billed to Insurance
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Procedures not billed to insurance report run.");
				return ReportNonModalSelection.ProcNotBilledIns;
			case "ODPPOWriteoffs"://PPO Writeoffs
				using(var formRpPPOwriteoffs=new FormRpPPOwriteoffs()) {
					formRpPPOwriteoffs.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"PPO Writeoffs report run.");
				break;
			case "ODPaymentPlans"://Payment Plans
				using(var formRpPayPlans=new FormRpPayPlans()) {
					formRpPayPlans.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Payment Plans report run.");
				break;
			case "ODReceivablesBreakdown"://Receivable Breakdown
				using(var formRpReceivablesBreakdown = new FormRpReceivablesBreakdown()) {
					formRpReceivablesBreakdown.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Receivable Breakdown report run.");
				break;
			case "ODUnearnedIncome"://Unearned Income
				using(var formRpUnearnedIncome=new FormRpUnearnedIncome()) {
			...
			case "ODInsuranceOverpaid"://Insurance Overpaid
				using(var formRpInsOverpaid=new FormRpInsOverpaid()) {
					formRpInsOverpaid.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Insurance Overpaid report run.");
				break;
			case "ODPresentedTreatmentProd"://Treatment Planned Presenter
				using(var formRpPresentedTreatmentProduction=new FormRpPresentedTreatmentProduction()) {
					formRpPresentedTreatmentProduction.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Treatment Plan Presenter report run.");
				break;
			case "ODTreatmentPresentationStats"://Treatment Planned Presenter
				using(var formRpTreatPlanPresentationStatistics = new FormRpTreatPlanPresentationStatistics()) {
					formRpTreatPlanPresentationStatistics.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Treatment Plan Presented Procedures report run.");
				break;
			case "ODInsurancePayPlansPastDue"://Insurance Payment Plans
				using(var formRpInsPayPlansPastDue = new FormRpInsPayPlansPastDue()) {
					formRpInsPayPlansPastDue.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Insurance Payment Plan report run.");
				break;
			case DisplayReports.ReportNames.InsAging://Insurance Aging Report
				using(var formRpInsAging=new FormRpInsAging()) {
					formRpInsAging.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Insurance Aging report run");
				break;
			case DisplayReports.ReportNames.CustomAging://Insurance Aging Report
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Custom Aging report run");
				return ReportNonModalSelection.CustomAging;
			case "ODActivePatients"://Active Patients
				using(var formRpActivePatients=new FormRpActivePatients()) {
					formRpActivePatients.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Active Patients report run.");
				break;
			case "ODAppointments"://Appointments
				using(var formRpAppointments=new FormRpAppointments()) {
					formRpAppointments.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Appointments report run.");
				break;
			case "ODBirthdays"://Birthdays
				using(var formRpBirthday=new FormRpBirthday()) {
					formRpBirthday.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Birthdays report run.");
				break;
			case "ODBrokenAppointments"://Broken Appointments
				using(var formRpBrokenAppointments=new FormRpBrokenAppointments()) {
					formRpBrokenAppointments.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Broken Appointments report run.");
				break;
			case "ODInsurancePlans"://Insurance Plans
				using(var formRpInsCo=new FormRpInsCo()) {
					formRpInsCo.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Insurance Plans report run.");
				break;
			case "ODNewPatients"://New Patients
				using(var formRpNewPatients=new FormRpNewPatients()) {
					formRpNewPatients.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"New Patients report run.");
				break;
			case "ODPatientsRaw"://Patients - Raw
				if(!Security.IsAuthorized(EnumPermType.UserQuery)) {
					break;
				}
				using(var formRpPatients=new FormRpPatients()) {
					formRpPatients.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Patients - Raw report run.");
				break;
			case "ODPatientNotes"://Patient Notes
				if(!Security.IsAuthorized(EnumPermType.UserQuery)) {
					break;
				}
				using(var formSearchPatNotes=new FormSearchPatNotes()) {
					formSearchPatNotes.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Patient Notes report run.");
				break;
			case "ODPrescriptions"://Prescriptions
				using(var formRpPrescriptions=new FormRpPrescriptions()) {
					formRpPrescriptions.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Rx report run.");
				break;
			case "ODProcedureCodes"://Procedure Codes - Fee Schedules
				using(var formRpProcCodes=new FormRpProcCodes()) {
					formRpProcCodes.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Procedure Codes - Fee Schedules report run.");
				break;
			case "ODReferralsRaw"://Referrals - Raw
				if(!Security.IsAuthorized(EnumPermType.UserQuery)) {
					break;
				}
				using(var formRpReferrals=new FormRpReferrals()) {
					formRpReferrals.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Referrals - Raw report run.");
				break;
			case "ODReferralAnalysis"://Referral Analysis
				using(var formRpReferralAnalysis=new FormRpReferralAnalysis()) {
					formRpReferralAnalysis.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Referral Analysis report run.");
				break;
			case DisplayReports.ReportNames.ReferredProcTracking://Referred Proc Tracking
				using(var formReferralProcTrack=new FormReferralProcTrack()) {
					formReferralProcTrack.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"ReferredProcTracking report run.");
				break;
			case DisplayReports.ReportNames.TreatmentFinder://Treatment Finder
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Treatment Finder report run.");
				return ReportNonModalSelection.TreatmentFinder;
			case DisplayReports.ReportNames.WebSchedAppointments://Web Sched Appts
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Web Sched Appointments report run.");
				return ReportNonModalSelection.WebSchedAppointments;
			case "ODRawScreeningData"://Raw Screening Data
				if(!Security.IsAuthorized(EnumPermType.UserQuery)) {
					break;
				}
				using(var formRpPHRawScreen=new FormRpPHRawScreen()) {
					formRpPHRawScreen.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"PH Raw Screening");
				break;
			case "ODRawPopulationData"://Raw Population Data
				if(!Security.IsAuthorized(EnumPermType.UserQuery)) {
					break;
				}
				using(var formRpPHRawPop=new FormRpPHRawPop()) {
					formRpPHRawPop.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"PH Raw population");
				break;
			case "ODDentalSealantMeasure"://FQHC Dental Sealant Measure
				using(var formRpDentalSealantMeasure=new FormRpDentalSealantMeasure()) {
					formRpDentalSealantMeasure.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"FQHC Dental Sealant Measure report run.");
				break;
			case "ODEligibilityFile"://Eligibility File
				using(var formRpArizonaPrimaryCareEligibility=new FormRpArizonaPrimaryCareEligibility()) {
					formRpArizonaPrimaryCareEligibility.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Arizona Primary Care Eligibility");
				break;
			case "ODEncounterFile"://Encounter File
				using(var formRpArizonaPrimaryCareEncounter=new FormRpArizonaPrimaryCareEncounter()) {
					formRpArizonaPrimaryCareEncounter.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Arizona Primary Care Encounter");
				break;
			case "ODDiscountPlan"://Discount Plans
				using(var formRpDiscountPlan=new FormRpDiscountPlan()) {
					formRpDiscountPlan.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Discount Plans report run.");
				break;
			case "ODMonthlyProductionGoal"://Monthly Production Goal
				using(var formRpProdGoal=new FormRpProdGoal()) {
					formRpProdGoal.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Monthly Production Goal report run.");
				break;
			case "ODHiddenPaySplits":
				using(var formRpHiddenPaySplits=new FormRpHiddenPaySplits()) {
					formRpHiddenPaySplits.ShowDialog();
				}
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Hidden PaySplits report run.");
				break;
			case "ODDynamicPayPlanOvercharged":
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"Payment Plans Overcharged report run.");
				return ReportNonModalSelection.DPPOvercharged;
			case DisplayReports.ReportNames.EraAutoProcessed:
				SecurityLogs.MakeLogEntry(EnumPermType.Reports,0,"ERAs Automatically Processed report run.");
	...
	private void setupToolStripMenuItem_Click(object sender,EventArgs e) {
		...
		using var formReportSetup = new FormReportSetup(0,false); //no need to pass in a usergroupnum.
