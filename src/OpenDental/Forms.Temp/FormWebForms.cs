...
public partial class FormWebForms : FormODBase
{
    ...
    private void FormWebForms_Load(object sender, EventArgs e)
    {
        ...
        {
            if (string.IsNullOrEmpty(_webForms_Preference.CultureName))
            {
                //Just in case.
    ...
    private void FillGrid()
    {
        ...
        var table = Sheets.GetWebFormSheetsTable(dateFrom, dateTo, comboClinics.ListClinicNumsSelected);
        for (var i = 0; i < table.Rows.Count; i++)
        {
            var patNum = SIn.Long(table.Rows[i]["PatNum"].ToString());
            var sheetNum = SIn.Long(table.Rows[i]["SheetNum"].ToString());
    ...
    private bool CertCallBack(object sender, X509Certificate x509Certificate, X509Chain x509Chain, SslPolicyErrors sslPolicyErrors)
    {
        ...
        for (var i = 0; i < x509Chain.ChainStatus.Length; i++)
        {
    ...
    private void butRetrieve_Click(object sender, System.EventArgs e)
    {
        ...
        var strMessage = Lan.g(this, "Done");
        var result = RetrieveWebForms();
        if (result.IsFailure())
        {
        ...
        Cursor = Cursors.Default;
        var msgBoxCopyPaste = new MsgBoxCopyPaste(strMessage);
    ...
    {
        var resultRet = new Result();
        var result = WebFormL.TryRetrievePatientTransfersCemt();
    ...
    {
        var sheetNum = (long) gridMain.ListGridRows[e.Row].Tag;
        var sheet = Sheets.GetSheet(sheetNum);
        GlobalFormOpenDental.GoToModule(EnumModuleType.Family, patNum: sheet.PatNum);
    }
    ...
    {
        var sheetNum = (long) gridMain.ListGridRows[gridMain.SelectedIndices[0]].Tag;
        var sheet = Sheets.GetSheet(sheetNum);
