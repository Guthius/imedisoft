...
public partial class FormTaskListBlocks:FormODBase {
	...
	private List<BlockedTaskPref> _listBlockedTaskPrefs= [];
	...
	private void InitializeTree() {
		...
		var listTaskListsSubs=TaskLists.RefreshUserTrunk(Security.CurUser.UserNum);
	...
	private void BuildTaskListTree(List<TaskList> listTaskListsSubs) {
		...
		var inboxNum=Security.CurUser.TaskListInBox;
		if(inboxNum!=0) {
			var taskList=listTaskListsSubs.Find(x => x.TaskListNum==inboxNum);
			if(taskList!=null) {
				listTaskListsSubs.Add(taskList);
			}
		}
		//Create a list of all leaves downstream from user's subscriptions.
		var listTaskListsLeafSubs=new List<TaskList>();
		for(var i=0;i<listTaskListsSubs.Count;i++) {
		...
		for(var i=0;i<treeSubscriptions.Nodes.Count;i++) {
	...
	private List<TaskList> GetLeafSubsFromTask(TaskList taskListNode,List<TaskList> listTaskListsSubs) {
		var listTaskListsChildren=TaskLists.RefreshChildren(taskListNode.TaskListNum,Security.CurUser.UserNum,Security.CurUser.TaskListInBox,TaskType.All);
		if(listTaskListsChildren.Count==0) {//base case: is a leaf
			return [taskListNode];
		}
		var listTaskListsLeaves=new List<TaskList>();
	...
	private List<TreeNode> TreeBuilder(List<TaskList> _listTaskListsSubs) {
		...
		var dictLookup=new Dictionary<long, TreeNode>();
		var listTreeNodesRoot=new List<TreeNode>();
		//Remove duplicate lists.
		_listTaskListsSubs=_listTaskListsSubs.GroupBy(x => x.TaskListNum).Select(y => y.First()).ToList();
		var dictTaskListSubs=_listTaskListsSubs.ToDictionary(x => x.TaskListNum);
		//Completly fill out subscribed list with parent TaskLists
		for(var i=0;i<_listTaskListsSubs.Count;i++) {
		...
		var listTaskLists=dictTaskListSubs.Values.ToList();
	...
	private Dictionary<long,TaskList> GetAllTaskListsRecursive(Dictionary<long, TaskList> dictCurTaskLists, long taskListNum) {
		...
		var taskList = _listTaskListsAll.Find(x => x.TaskListNum==taskListNum);
		if(taskList==null) {
			return dictCurTaskLists;
		}
	...
	private void treeSubscriptions_AfterCheck(object sender,TreeViewEventArgs e) {
		...
		var isNodeChecked=e.Node.Checked;
	...
	private void SetBlockedPrefsRecursive(TreeNode treeNode) {
		...
		var blockedTaskPref = new BlockedTaskPref();
	...
	private void butSave_Click(object sender,EventArgs e) {
		...
		for(var i = 0;i<_listBlockedTaskPrefs.Count;i++) {
			var userOdPref = _listUserOdPrefsBlocks.Find(x => x.Fkey==_listBlockedTaskPrefs[i].UserOdPref.Fkey);
			if(userOdPref==null) {
				continue;
			}
