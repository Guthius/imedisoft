...
public partial class FormMarkupTableEdit:FormODBase {
	...
	private void ParseMarkup(){
		...
		var stringArrayLines=Markup.Split(["{|\r\n","{|\n","\r\n|-\r\n","\n|-\n","\r\n|}","\n|}"],StringSplitOptions.RemoveEmptyEntries);
		for(var i=0;i<stringArrayLines.Length;i++) {
			if(stringArrayLines[i].StartsWith("!")) {//header
				...
				stringArrayCells=stringArrayLines[i].Split(["!!"],StringSplitOptions.None);
				for(var c=0;c<stringArrayCells.Length;c++) {
					var colName="";
					...
					var width=stringArrayCells[c].Substring(7);//90"|Column Heading 1
			...
			if(stringArrayCells.Length!=_listColNames.Count || stringArrayCells.Length!=_listColWidths.Count) {
				ODMessageBox.Show("Table is corrupt.  There are "+_listColNames.Count+" columns, but row "+((i-1)/2)
				                  +" has "+stringArrayCells.Length+" cells.  Please manually edit the markup in the following window.");
				ManuallyEdit();
				return;
			}
	...
	private void ManuallyEdit() {
		using var msgBoxCopyPaste=new MsgBoxCopyPaste(Markup);
	...
	private void FillGrid() {
		...
		GridColumn col;
		for(var c=0;c<_listColNames.Count;c++){
	...
	private string GenerateMarkup() {
		...
		for(var c=0;c<_listColWidths.Count;c++) {
			if(c>0) {
				stringBuilder.Append("!!");
			}
			if(_listColWidths[c]>0) {//otherwise, no width specified for this column.  Dynamic.
				stringBuilder.Append("Width=\""+_listColWidths[c]+"\"|");
			}
		...
		for(var i=0;i<_table.Rows.Count;i++) {
			...
			for(var c=0;c<_listColWidths.Count;c++) {
				...
				stringBuilder.Append(_table.Rows[i][c]);
			}
			stringBuilder.AppendLine();
		}
		stringBuilder.Append("|}");
	...
	private void butColumnLeft_Click(object sender,EventArgs e) {
		...
		var point=new Point(gridMain.SelectedCell.X-1,gridMain.SelectedCell.Y);
	...
	private void butColumnAdd_Click(object sender,EventArgs e) {
		...
		for(var i=0;i<_table.Rows.Count;i++) {
			for(var j=gridMain.Columns.Count-1;j>index;j--) {
				_table.Rows[i][j+1]=_table.Rows[i][j];
			}
			_table.Rows[i][index+1]="";
		}
		var point=new Point(index,gridMain.SelectedCell.Y);
		if(gridMain.SelectedCell.X==gridMain.Columns.Count-1) {//only if this is the last column
			point=new Point(index+1,gridMain.SelectedCell.Y);//shift the selected column to the right
		}
		FillGrid();//gridMain.SelectedCell gets cleared.
		if(point.Y>-1) {
			gridMain.SetSelected(point);
	...
	private void butColumnDelete_Click(object sender,EventArgs e) {
		...
		var point=new Point(Math.Max(gridMain.SelectedCell.X-1,0),gridMain.SelectedCell.Y);
		FillGrid();//gridMain.SelectedCell gets cleared.
		gridMain.SetSelected(point);
	}
	...
	private void butRowUp_Click(object sender,EventArgs e) {
		...
		var point=new Point(gridMain.SelectedCell.X,gridMain.SelectedCell.Y-1);
		FillGrid();//gridMain.SelectedCell gets cleared.
		gridMain.SetSelected(point);
	}
	...
	private void butRowAdd_Click(object sender,EventArgs e) {
		...
		var point=new Point(pointSelectedCell.X,pointSelectedCell.Y+1);
		FillGrid();//gridMain.SelectedCell gets cleared.
		gridMain.SetSelected(point);
	}
	...
	private void butRowDelete_Click(object sender,EventArgs e) {
		...
		var point=new Point(gridMain.SelectedCell.X,Math.Max(gridMain.SelectedCell.Y-1,0));
		FillGrid();//gridMain.SelectedCell gets cleared.
	...
	private void butPaste_Click(object sender,EventArgs e) {
		var pointStarting=gridMain.SelectedCell;
		...
		var listTblBuilder = new List<List<string>>();//tableBuilder[Y][X] to access cell.
		...
		var stringArrayTableRows = clipBoardText.Split(["\r\n"],StringSplitOptions.None);
		rowsNeeded=stringArrayTableRows.Length;
		List<string> listCurrentRows;
		for(var i=0;i<stringArrayTableRows.Length;i++) {
			listCurrentRows = [];//currentRow[X] to access cell data
			var stringArrayRowCells = stringArrayTableRows[i].Split('\t');
			for(var c = 0;c<stringArrayRowCells.Length;c++) {
				listCurrentRows.Add(stringArrayRowCells[c]);
			}
		...
		for(var x=0;x+pointStarting.X<_table.Columns.Count;x++) {
			for(var y=0;y+pointStarting.Y<_table.Rows.Count;y++) {
		...
		if(_table.Rows.Count < pointStarting.Y + rowsNeeded) {
			var newRowsNeededCount=(pointStarting.Y+rowsNeeded)-_table.Rows.Count;
			for(var i=0;i<newRowsNeededCount;i++) {
				_table.Rows.Add(_table.NewRow());
			}
		}
		//Paste new data into data Table---------------------------------------------------------------------------------------------------
		for(var i=0;i<listTblBuilder.Count;i++) {
			for(var j=0;j<listTblBuilder[i].Count;j++) {
		...
		//Redraw Grid
		var point=new Point(gridMain.SelectedCell.X,gridMain.SelectedCell.Y);
		FillGrid();//gridMain.SelectedCell gets cleared.
		gridMain.SetSelected(point);
	}
	...
	private void butSave_Click(object sender,EventArgs e) {
		...
		for(var h=0;h<gridMain.Columns.Count;h++) {//loops through every header in the main grid
			var s=gridMain.Columns[h].Heading;
			s=s.Replace("&","&amp;");
		...
		for(var i=0;i<_table.Rows.Count;i++) {//loops through each row in the table
			for(var j=0;j<_table.Columns.Count;j++) {//loops through each column in the row
				var xmlDocument=new XmlDocument();
				var s=_table.Rows[i][j].ToString();
				s=s.Replace("&","&amp;");
				s=s.Replace("&amp;<","&lt;");//because "&" was changed to "&amp;" in the line above.
				s=s.Replace("&amp;>","&gt;");//because "&" was changed to "&amp;" in the line above.
				s="<body>"+s+"</body>";
