...
public partial class FormMassEmailTemplate:FormODBase {
	...
	private void FormMassEmailTemplate_Load(object sender,EventArgs e) {
		var isImportOrCopy=_emailHostingTemplate.IsNew && !string.IsNullOrEmpty(_emailHostingTemplate.BodyHTML);
	...
	private void butDeleteTemplate_Click(object sender,EventArgs e) {
		...
		if(_emailHostingTemplate.TemplateId!=0) {
			...
			var iAccountApi=EmailHostingTemplates.GetAccountApi(_emailHostingTemplate.ClinicNum);
	...
	private bool Save() {
		...
		if(_emailType==EmailType.Html && !string.IsNullOrEmpty(_htmlText)) {
			...
			try {
				xhtml=MarkupEdit.TranslateToXhtml(_htmlText,true);
			}
			catch(Exception e) {
		...
		var listBodyReplacementsHTML=EmailHostingTemplates.GetListReplacements(xhtml).Distinct().ToList();
		var listBodyReplacementsPlainText=EmailHostingTemplates.GetListReplacements(textboxPlainText.Text).Distinct().ToList();
		var doBodyReplacementsMatch=listBodyReplacementsHTML.Count==listBodyReplacementsPlainText.Count && listBodyReplacementsHTML.All(x => listBodyReplacementsPlainText.Contains(x));
		...
		if(_emailHostingTemplate.IsNew || _emailHostingTemplate.TemplateId==0) {//templates inserted during convert didn't have their Id's set, we need to create them.
			...
			try {
				var createTemplateResponse=iAccountApi.CreateTemplate(createTemplateRequest);
