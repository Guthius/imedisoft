...
public partial class FormReferenceEdit:FormODBase {
	...
	private void FillMain() {
		...
		var col=new GridColumn("PatNum",65);
		...
		for(var i=0;i<_listCustRefEntries.Count;i++) {
			var row=new GridRow();
			var patient=Patients.GetLim(_listCustRefEntries[i].PatNumCust);
	...
	private void gridMain_CellDoubleClick(object sender,UI.ODGridClickEventArgs e) {
		using var formReferenceEntryEdit=new FormReferenceEntryEdit(_listCustRefEntries[e.Row]);
