...
public partial class FormPatientPortal:FormODBase {
	...
	private void FormPatientPortal_Load(object sender,EventArgs e) {
		...
		if(_userWeb==null) {
			...
			_userWeb.UserName=UserWebs.CreateUserNameFromPat(_patient,UserWebFKeyType.PatientPortal, []);
	...
	private void butGiveAccess_Click(object sender,EventArgs e) {
		if(butGiveAccess.Text=="Provide Online Access") {//When form open opens with a blank password
			...
			var error=UserWebs.ValidatePatientAccess(_patient);
			...
			//1. Fill password.
			var passwordGenerated=UserWebs.GenerateRandomPassword(8);
	...
	private void butPrint_Click(object sender,EventArgs e) {
		...
		var error=Patients.IsPortalPasswordValid(textOnlinePassword.Text);
	...
	private void butSave_Click(object sender,EventArgs e) {
		...
		if(shouldPrint) {
			var dialogResult=ODMessageBox.Show(Lan.g(this,"Online Username or Password changed but was not printed, would you like to print?")
