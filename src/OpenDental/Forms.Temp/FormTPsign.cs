...
public partial class FormTPsign : FormODBase {
	...
	private void FormTPsign_Load(object sender, System.EventArgs e) {
		...
		SetSize();
		ToolBarMain.Buttons["PageNum"].Text=(previewContr.StartPage+1)
		                                    +" / "+TotalPages;
		...
		var keyData= TreatPlans.GetKeyDataForSignatureHash(TreatPlanCur,_listProcTPs);
	...
	private void SetSize(){
		if(!ToolBarMain.Buttons["FullPage"].IsTogglePushed) {//100%
			previewContr.Zoom=1;
			return;
		}
	...
	public void LayoutToolBar(){
		...
		var toolBarButton=new ODToolBarButton("",-1,"","PageNum");
	...
	private void OnFullPage_Click(){
		ToolBarMain.Buttons["100"].IsTogglePushed=!ToolBarMain.Buttons["FullPage"].IsTogglePushed;
		ToolBarMain.Invalidate();
		SetSize();
	}

	private void On100_Click(){
		ToolBarMain.Buttons["FullPage"].IsTogglePushed=!ToolBarMain.Buttons["100"].IsTogglePushed;
		ToolBarMain.Invalidate();
		SetSize();
	}
	...
	private void SaveSignature() {
		...
		SheetField sheetField;
		if(_sigChanged) {
			var keyData = TreatPlans.GetKeyDataForSignatureSaving(TreatPlanCur,_listProcTPs);
		...
		var keyData2 = TreatPlans.GetKeyDataForSignatureSaving(TreatPlanCur,_listProcTPs);
	...
	private void butSave_Click(object sender,EventArgs e) {
		...
		if(TreatPlanCur.Signature.Length>0 && TreatPlanCur.DocNum==0 && PrefC.GetBool(PrefName.TreatPlanSaveSignedToPdf)) {
			...
			var doResave=MsgBox.Show(this,MsgBoxButtons.YesNo,"Cannot find saved copy of signed PDF, would you like to resave the document?");
		...
		{
			var docs=SaveFileAsDocDelegate_(true,SheetTP);
			if(docs.Count>0) {
