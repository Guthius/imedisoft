...
public partial class FormFeeScheds:FormODBase {
	...
	private void FormFeeSchedules_Load(object sender, System.EventArgs e) {
		...
		CheckItemOrders();
		var arrayFeeSchedTypes=Enum.GetValues(typeof(FeeScheduleType));
		var pref=(AllowedFeeSchedsAutomate)PrefC.GetInt(PrefName.AllowedFeeSchedsAutomate);
		for(var i=0;i<arrayFeeSchedTypes.Length;i++) {
			var feeScheduleType=((FeeScheduleType)arrayFeeSchedTypes.GetValue(i));
	...
	private void FillGrid(){
		_listFeeSchedsForType= [];
		for(var i=0;i<listType.SelectedIndices.Count;i++) {
			var listFeeSchedsToAdd=FeeScheds.GetListForType((FeeScheduleType)listType.SelectedIndices[i],true,_listFeeScheds);
			_listFeeSchedsForType.AddRange(listFeeSchedsToAdd);
		}
		_listFeeSchedsForType.Sort(CompareItemOrder);
		gridMain.BeginUpdate();
		gridMain.Columns.Clear();
		var col=new GridColumn(Lan.g("TableFeeScheds","Description"),200);
	...
	private void butAdd_Click(object sender, System.EventArgs e) {
		var frmFeeSchedEdit=new FrmFeeSchedEdit();
		frmFeeSchedEdit.FeeSchedCur=new FeeSched();
		frmFeeSchedEdit.FeeSchedCur.IsNew=true;
		frmFeeSchedEdit.FeeSchedCur.ItemOrder=_listFeeScheds.Count;
		if(listType.SelectedIndices.Count==1){
			frmFeeSchedEdit.FeeSchedCur.FeeSchedType=(FeeScheduleType)(listType.SelectedIndices[0]);
		}
		frmFeeSchedEdit.ShowDialog();
		if(!frmFeeSchedEdit.IsDialogOK) {
			return;
		}
		FeeScheds.RefreshCache();
		...
		FillGrid();
		for(var i=0;i<_listFeeSchedsForType.Count;i++){
			if(frmFeeSchedEdit.FeeSchedCur.FeeSchedNum==_listFeeSchedsForType[i].FeeSchedNum){
	...
	private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		...
		var frmFeeSchedEdit=new FrmFeeSchedEdit();
		frmFeeSchedEdit.FeeSchedCur=(FeeSched)gridMain.ListGridRows[e.Row].Tag;
		frmFeeSchedEdit.ShowDialog();
		if(!frmFeeSchedEdit.IsDialogOK) {
			return;
		}
	...
	private void butUp_Click(object sender,EventArgs e) {
		...
		_hasChanged=true;
		var oldItemOrder=((FeeSched)gridMain.ListGridRows[idx].Tag).ItemOrder;
	...
	private void butIns_Click(object sender,EventArgs e) {
		using var FormF=new FormFeesForIns();
		FormF.ShowDialog();
		//DialogResult=DialogResult.OK;
	}
	...
	private void butCleanUp_Click(object sender,EventArgs e) {
		...
		var changed=FeeScheds.CleanupAllowedScheds();
		ODMessageBox.Show(changed+" "+Lan.g(this,"unused fee schedules deleted."));
		if(changed==0) {
			return;
		}
		_hasChanged=true;
	...
	private void butHideUnused_Click(object sender,EventArgs e) {
		...
		var actionProgress=ODProgress.Show(startingMessage:Lans.g("Backing up database..."));
		ODEvent.Fire(ODEventType.ProgressBar,Lans.g("Hiding unused fee schedules..."));
		var countChanged=FeeScheds.HideUnusedScheds();
		actionProgress?.Invoke();
		ODMessageBox.Show(countChanged+" "+Lans.g("unused fee schedules hidden."));
		if(countChanged==0) {
			return;
		}
		_hasChanged=true;
