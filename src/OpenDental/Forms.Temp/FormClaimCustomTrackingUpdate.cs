...
public partial class FormClaimCustomTrackingUpdate:FormODBase {
	...
	public FormClaimCustomTrackingUpdate(Claim claimCur,ClaimTracking claimTracking) {
		...
		_listClaims= [claimCur];
		ListClaimTrackingsNew= [claimTracking];
	}
	...
	private void FormClaimCustomTrackingUpdate_Load(object sender,EventArgs e) {
		...
		var listDefsClaimCustomTracking=Defs.GetDefsForCategory(DefCat.ClaimCustomTracking,true);
		comboCustomTracking.Items.AddDefs(listDefsClaimCustomTracking);
		var claimTracking=ListClaimTrackingsNew.FirstOrDefault();
	...
	private void FillComboErrorCode() {
		var defArrayErrorCodes = Defs.GetDefsForCategory(DefCat.ClaimErrorCode,true).ToArray();
		...
		//Fill comboErrorCode.
		var claimTracking=ListClaimTrackingsNew.FirstOrDefault();
		for(var i=0;i<defArrayErrorCodes.Length;i++) {
	...
	private void SetTextErrorDesc() {
		...
		if(comboErrorCode.Enabled && comboErrorCode.GetSelected<Def>()!=null) {
			textErrorDesc.Text=comboErrorCode.GetSelected<Def>().ItemValue;
		}
	}
	...
	private void butSave_Click(object sender,EventArgs e) {
		...
		var defErrorCode=comboErrorCode.GetSelected<Def>();
		for(var i=0;i<_listClaims.Count;i++) { //when not called from FormRpOutstandingIns, this should only have one claim.
			var claimOld=_listClaims[i].Copy();
			_listClaims[i].CustomTracking=comboCustomTracking.GetSelectedDefNum();
			Claims.Update(_listClaims[i],claimOld);
			var claimTrackingCur=ListClaimTrackingsNew[i];
