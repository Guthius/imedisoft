...
public partial class FormUpdate : FormODBase {
	...
	private void FormUpdate_Load(object sender, System.EventArgs e) {
		...
		if(PrefC.GetBool(PrefName.UpdateWindowShowsClassicView)) {
			...
			butShowUpdateHistory.Location=new Point(76+labelVersion.Width,30);
			panelClassic.Visible=true;
			panelClassic.Location=new Point(67,57);
	...
	private void SetButtonVisibility() {
		...
		var showMsi=PrefC.GetBool(PrefName.UpdateShowMsiButtons);
		var showDownloadAndInstall=_dateTimeUpdate < DateTime.Now;
	...
	private void butCheckForUpdates_Click(object sender,EventArgs e) {
		...
		{
			ODMessageBox.Show(Lan.g(this,"Updates are only allowed from the web server")+": "+PrefC.GetString(PrefName.WebServiceServerName));
			return;
		}
		...
		try {
			...
			var friendlyMessage=Lan.g(this,"Error checking for updates.");
		...
		if(!string.IsNullOrEmpty(_betaAvailableDisplay)) {
			...
			var canUpdate=_dateTimeUpdate < DateTime.Now;
	...
	private void butShowUpdateHistory_Click(object sender,EventArgs e) {
		using var formUpdateHistory=new FormUpdateHistory();
	...
	private void DownloadInstallPatchForVersion(string version,string updateCode,bool showFormUpdateInstallMsg) {
		if(showFormUpdateInstallMsg) {
			using var FormUIM=new FormUpdateInstallMsg();
			FormUIM.ShowDialog();
			if(FormUIM.DialogResult!=DialogResult.OK) {
				return;
			}
		}
		var patchName="Setup.exe";
		var fileNameWithVers=version;//6.9.23F
		...
		var destDir=ImageStore.GetDataFolder();
		...
		if(destDir==null) {//Not using A to Z folders?
			...
			if(true && !Directory.Exists(destPath2)) {
	...
	private void DownloadAndStartMSI(string updateCode) {
		var fileName=PrefC.GetString(PrefName.UpdateWebsitePath)+updateCode+"/OpenDental.msi";
		try {
