...
public partial class FormBilling : FormODBase {
	...
	public List<long> ClinicNumsSelectedInitial= [];
	...
	protected override void ProcessSignalODs(List<Signalod> signals) {
		if(signals.Any(x=>x.IType==InvalidType.BillingList)){
	...
	private DataTable GetBillingDataTable() {
		...
		if(true && comboClinic.ListClinicNumsSelected.Count>0) {
	...
	private void FillGrid() {
		var scrollPos=gridBill.ScrollValue;
		var selectedKeys=gridBill.SelectedIndices.OfType<int>().Select(x => SIn.Long(((DataRow)gridBill.ListGridRows[x].Tag)["StatementNum"].ToString())).ToList();
		var table=GetBillingDataTable();
		...
		if(_isInitial){
			...
			for(var i=0;i<gridBill.ListGridRows.Count;i++) {
		...
		labelTotal.Text=Lan.g(this,"Total=")+table.Rows.Count;
		labelSelected.Text=Lan.g(this,"Selected=")+gridBill.SelectedIndices.Length;
	}

	private void FillComboEmail() {
		var curUserNum=Security.CurUser.UserNum;
		var listEmailAddresses=EmailAddresses.GetEmailAddressesForComboBoxes(curUserNum);
	...
	private void gridBill_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		var statement=Statements.GetStatement(SIn.Long(((DataRow)gridBill.ListGridRows[e.Row].Tag)["StatementNum"].ToString()));
	...
	private void menuItemGoTo_Click(object sender,EventArgs e) {
		...
		var patNum=SIn.Long(((DataRow)gridBill.ListGridRows[gridBill.GetSelectedIndex()].Tag)["PatNum"].ToString());
	...
	private void butEdit_Click(object sender,EventArgs e) {
		...
		using var FormStatementOptions=new FormStatementOptions(true);
		FormStatementOptions.ListStatementModesForSms=ListStatementModesForSms;
		var listStatementNums=new List<long>();
		foreach(var index in gridBill.SelectedIndices) {
			listStatementNums.Add(SIn.Long(((DataRow)gridBill.ListGridRows[index].Tag)["StatementNum"].ToString()));
		}
		var listStatementsSelected=Statements.GetStatements(listStatementNums);
		if(listStatementsSelected.Count==1) {
			ShowStatementOptions(listStatementsSelected[0]);
			return;
		}
		//At least 2 statements selected
		var countSkipped=listStatementsSelected.RemoveAll(x => x.LimitedCustomFamily!=EnumLimitedCustomFamily.None);
		var stringWarning=Lan.g(this,"Limited (Custom) Statements must be edited individually.");
	...
	private void butSend_Click(object sender,System.EventArgs e) {
		...
		sendStatementsIO.ActionPrompt=(prompt,useCopyPasteDialog) => {
			if(useCopyPasteDialog) {
				var msgBoxPatErrors=new MsgBoxCopyPaste(prompt);
		...
		sendStatementsIO.FuncChooseSaveFile=(initialSaveDirectory) => {
			...
			var chooseSaveFile=new ChooseSaveFile();
		...
		sendStatementsIO.ActionProgressEvent=(odEventArgs) => {
			...
			labelPrinted.Text=Lan.g(this,"Printed=")+sendStatementsIO.CountStatementsPrinted;
			labelEmailed.Text=Lan.g(this,"E-mailed=")+sendStatementsIO.CountStatementsEmailed;
			labelSentElect.Text=Lan.g(this,"SentElect=")+sendStatementsIO.CountStatementsSentElectronic;
			labelTexted.Text=Lan.g(this,"Texted=")+sendStatementsIO.CountStatmentsSentPayPortalText;
		...
		sendStatementsIO.FuncSendEhgStatement=(xmlData,_) => {
			OpenDentBusiness.Bridges.EHG_statements.Send(xmlData,sendStatementsIO.CurStatementBatch.ClinicNum,out var alertMsg);
			return alertMsg;
		};
		...
		if(sendStatementsIO.PdfMasterDocument!=null) {
			var tempFileOutputDocument = PrefC.GetRandomTempFile(".pdf");
	...
	private void FormBilling_FormClosing(object sender,FormClosingEventArgs e) {
		...
		var result=ODMessageBox.Show(Lan.g(this,"You may leave this window open while you work.  If you do close it, do you want to delete all unsent bills?"),
		...
		var progressOD=new ProgressWin();
		progressOD.ActionMain=() => { 
			foreach(var clinicNum in dictionaryClinicStatmentsToDelete.Keys) {
