...
public partial class FormMassEmailBirthdays:FormODBase {
	...
	private void FormMassEmailBirthdays_Load(object sender,EventArgs e) {
		...
		var listEmailHostingTemplateNums=_listApptReminderRules.Select(x => x.EmailHostingTemplateNum).ToList();
		_listEmailHostingTemplates=EmailHostingTemplates.GetMany(listEmailHostingTemplateNums);
		#endregion
		#region Fill UI
		var stringArrayLanguages=PrefC.GetString(PrefName.LanguagesUsedByPatients).Split(',');///From pref that gets languages used by patients. Abbreviated form.
	...
	private bool IsClinicsUsingDefaults(){
		var clinicPref=_listClinicPrefsDefaultNew.Find(x => x.ClinicNum==comboClinic.ClinicNumSelected);
		return clinicPref==null || clinicPref.ValueString=="1";
	}
	
	private void SetClinicsUsingDefaults(bool isUsingDefaults){
		if(!true || comboClinic.ClinicNumSelected==0) {
	...
	private void comboClinic_SelectionChangeCommitted(object sender,EventArgs e) {
		var clinicNumPrevious=((ApptReminderRule)tabControlLanguages.SelectedTab.Tag).ClinicNum;
		if(!_isLoading) {
			var errors=ValidateCurrentSelection();
	...
	private string ValidateCurrentSelection() {
		...
		var listApptReminderRules = _listApptReminderRules.FindAll(x => x.ClinicNum==ClinicNumEffective());
		for(var i=0;i<listApptReminderRules.Count;i++){
			var emailHostingTemplate=_listEmailHostingTemplates.FirstOrDefault(x => listApptReminderRules[i].EmailHostingTemplateNum==x.EmailHostingTemplateNum);
	...
	private void FillForClinic() {
		...
		if(!true || comboClinic.ClinicNumSelected==0) {
			...
			var clinicPref=_listClinicPrefsEnabledNew.Find(x => x.ClinicNum==comboClinic.ClinicNumSelected);
		...
		var listApptReminderRules=_listApptReminderRules
		...
		for(var i=0;i<listApptReminderRules.Count;i++){
			var emailHostingTemplate=_listEmailHostingTemplates.FirstOrDefault(x => x.EmailHostingTemplateNum==listApptReminderRules[i].EmailHostingTemplateNum);
			var userControlEmailTemplate=new UserControlEmailTemplate();
			...
			var tabPage=new UI.TabPage();
			var language="";
			if(string.IsNullOrEmpty(listApptReminderRules[i].Language)) {
				language=Lan.g(this,"Default");
			}
			else {
				var cultureInfo=MiscUtils.GetCultureFromThreeLetter(listApptReminderRules[i].Language);
				if(cultureInfo==null) {
			...
			LayoutManagerForms.Add(tabPage,tabControlLanguages);		
		}
	...
	private void butLangaugeAdd_Click(object sender,EventArgs e) {
		...
		var listLanguageRulesCurrent=new List<string>();
		...
		for(var i=0;i<stringArrayLanguages.Length;i++) {
			...
			var cultureInfo=MiscUtils.GetCultureFromThreeLetter(stringArrayLanguages[i]);
			if(cultureInfo==null) {
		...
		var apptReminderRule=_listApptReminderRules
			.FindAll(x => x.ClinicNum==ClinicNumEffective())
			.First(x => x.Language=="");
		...
		var apptReminderRuleCopy=CopyRule(apptReminderRule,apptReminderRule.ClinicNum);
	...
	private void butLanguageRemove_Click(object sender,EventArgs e) {
		...
		var apptReminderRule=(ApptReminderRule)tabControlLanguages.SelectedTab.Tag;
		RemoveRules(apptReminderRule);
		tabControlLanguages.TabPages.Remove(tabControlLanguages.SelectedTab);
	}
	...
	private void butEditTemplate_Click(object sender,EventArgs e) {
		...
		var emailHostingTemplate=_listEmailHostingTemplates.First(x => apptReminderRule.EmailHostingTemplateNum==x.EmailHostingTemplateNum);
		...
		using var formMassEmailTemplate=new FormMassEmailTemplate(emailHostingTemplate,canDeleteTemplate:false);
	...
	private bool SwitchFromDefaultsBirthday() {
		...
		var listApptReminderRules=_listApptReminderRules.FindAll(x => x.ClinicNum==0);
		for(var i=0;i<listApptReminderRules.Count;i++){
	...
	private ApptReminderRule CopyRule(ApptReminderRule apptReminderRule,long clinicNum) {
		var emailHostingTemplateDefaultRule=_listEmailHostingTemplates.FirstOrDefault(x => x.EmailHostingTemplateNum==apptReminderRule.EmailHostingTemplateNum);
		var emailHostingTemplateClinicRule=GenericTools.DeepCopy<EmailHostingTemplate,EmailHostingTemplate>(emailHostingTemplateDefaultRule);
		...
		var apptReminderRuleCopy=GenericTools.DeepCopy<ApptReminderRule,ApptReminderRule>(apptReminderRule);
	...
	private void RemoveRules(params ApptReminderRule[] apptReminderRuleArray) {
		for(var i=0;i<apptReminderRuleArray.Length;i++) {
	...
	private void MapUiToRules(ApptReminderRule apptReminderRule) {
		...
		var listApptReminderRules=_listApptReminderRules.FindAll(x => x.ClinicNum==apptReminderRule.ClinicNum && x.Language!="").ToList();
		for(var i=0;i<listApptReminderRules.Count;i++){
	...
	private void SaveAll() {
		...
		if(true) {
			...
			for(var i=0;i< comboClinic.ListClinics.Count;i++){
				ApptReminderRules.SyncByClinicAndTypes(_listApptReminderRules.FindAll(x => x.ClinicNum==comboClinic.ListClinics[i].Id),comboClinic.ListClinics[i].Id,ApptReminderType.Birthday);
			}
		}
		else {
