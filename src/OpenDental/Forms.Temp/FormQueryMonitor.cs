...
public partial class FormQueryMonitor:FormODBase {
	...
	private List<string> _listFilterTextsToLower= [];
	...
	List<DbQueryObj> _listDbQueryObjs= [];
	...
	private void FormQueryMonitor_Load(object sender,EventArgs e) {
		...
		var contextMenu=gridFeed.ContextMenu;
		var menuItemStackTrace=new System.Windows.Forms.MenuItem();
	...
	private void MenuItemShowStackTrace_Click(object sender,EventArgs e) {
		var index=gridFeed.GetSelectedIndex();
		if(index!=-1) {
			var dbQueryObj=gridFeed.ListGridRows[index].Tag as DbQueryObj;
			var msgBoxCopyPaste=new MsgBoxCopyPaste(dbQueryObj.StackTrace);
			msgBoxCopyPaste.Show();
		}
	}

	private void MenuItemPopupShowStackTrace(object sender, EventArgs e) {
		var menuItem=gridFeed.ContextMenu.MenuItems
			.OfType<System.Windows.Forms.MenuItem>().FirstOrDefault(x => x.Name=="showStackTrace");
		if(menuItem==null) { return; }//Should not happen
		var menuItemSeperator=gridFeed.ContextMenu.MenuItems
			.OfType<System.Windows.Forms.MenuItem>().FirstOrDefault(x => x.Text=="-");
		if(menuItemSeperator==null) { return; }//Should not happen
		var indexRowClick=gridFeed.GetSelectedIndex();
		if(indexRowClick!=-1) {
			var dbQueryObj=gridFeed.ListGridRows[indexRowClick].Tag as DbQueryObj;
	...
	private void ToggleUserStackTracePref() {
		var userOdPref=UserOdPrefs.GetFirstOrNewByUserAndFkeyType(Security.CurUser.UserNum, UserOdFkeyType.QueryMonitorHasStackTraces);
		var userOdPrefOld=userOdPref.Clone();
	...
	private void TimerProcessQueue_Tick(object sender,EventArgs e) {
		...
		var listDbQueryObjs=new List<DbQueryObj>();
		while(_concurrentQueueDbQueryObjs.Count!=0) {
			if(!_concurrentQueueDbQueryObjs.TryDequeue(out var query)) {
				break;
			}
	...
	private void GridFeed_CellClick(object sender,ODGridClickEventArgs e) {
		...
		textElapsed.Clear();
		textCommand.Clear();
		var dbQueryObj=gridFeed.ListGridRows[e.Row].Tag as DbQueryObj;
		if(dbQueryObj==null) {
			return;
		}
	...
	private void AddQueriesToGrid(params DbQueryObj[] dbQueryObjArray) {
		...
		for(var i=0;i<dbQueryObjArray.Length;i++) {
			if(!_dictionaryDbQueryObjs.ContainsKey(dbQueryObjArray[i].Guid)) {
				...
				var doAdd=true;
				var commandToLower=dbQueryObjArray[i].CommandText.ToLower();
				for(var j=0;j<_listFilterTextsToLower.Count;j++){
	...
	private void RefreshFilteredQueryList() {
		...
		var listDbQueryObj=_dictionaryDbQueryObjs.Values.ToList();
		for(var i=0;i<listDbQueryObj.Count;i++) {
			//Check to see if the query meets the current filters when this is the first time we've seen this query.
			var doAdd=true;
			var commandToLower=listDbQueryObj[i].CommandText.ToLower();
			for(var j=0;j<_listFilterTextsToLower.Count;j++){
				//If the command has a query that we want to ignore
				if(commandToLower.Contains(_listFilterTextsToLower[j])){
					doAdd=false;
	...
	private void FillGrid() {
		...
		var listDbQueryObjs=UIHelper.TakeLast(_listDbQueryObjs,500).ToList();
		gridFeed.BeginUpdate();
		gridFeed.ListGridRows.Clear();
		for(var i=0; i<listDbQueryObjs.Count;i++) {
			string command;
			string server;
	...
	private void ButLog_Click(object sender,EventArgs e) {
		...
		var logFolderPath="QueryMonitorLogs";
		var logFileName="";
		...
		try{
			...
			while(File.Exists(Path.Combine(OpenDentBusiness.FileIO.FileAtoZ.GetPreferredAtoZpath(), logFolderPath, logFileName))) {
				logFileName=$"QueryMonitorLog_{DateTime.Now.ToString("yyyyMMdd_HHmmss")}.txt";
				Thread.Sleep(100);
			}
		}
	...
	private void butFilter_Click(object sender,EventArgs e) {
		var frmQueryMonitorFilter=new FrmQueryMonitorFilters();
