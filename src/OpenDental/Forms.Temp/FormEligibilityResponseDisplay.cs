...
namespace OpenDental {
	...
	public partial class FormEligibilityResponseDisplay:FormODBase {
		...
		private void prepareDataSetAndTable() {
			...
			// Define DataColumns
			var dataColumnBenefitInfo = new DataColumn("Benefit Info",System.Type.GetType("System.String"));
			var dataColumnCoverageLevel = new DataColumn("Coverage Level",System.Type.GetType("System.String"));
			var dataColumnPlanCoverage = new DataColumn("Plan Coverage",System.Type.GetType("System.String"));
			var dataColumnTimeQualifier = new DataColumn("Time Qualifier",System.Type.GetType("System.String"));
			var dataColumnBenefitAmt = new DataColumn("Benefit Amount",System.Type.GetType("System.String"));
			var dataColumnMsg = new DataColumn("Message",System.Type.GetType("System.String"));
			var dataColumnPercent = new DataColumn("Percent",System.Type.GetType("System.String"));
			var dataColumnProcCode = new DataColumn("ProcCode",System.Type.GetType("System.String"));
			var dataColumnDeliveryPattern = new DataColumn("Delivery Pattern",System.Type.GetType("System.String"));
		...
		private void populateDataTable() {
			...
			for(var i=0;i<xmlNodeListBenefits.Count;i++) {
				...
				for(var c=0;c<xmlNodeListChildNodes.Count;c++) {
				...
				var dataRowResponse = _table.NewRow();
		...
		private bool SetupThePrinting() {//TODO: Implement ODprintout pattern
			...
			using var font = new Font("Tahoma",18,FontStyle.Bold,GraphicsUnit.Point);
		...
		private void MyPrintDocument_PrintPage(object sender,PrintPageEventArgs e) {
			var hasMore = _dataGridViewPrinter.DrawDataGridView(e.Graphics);
			if(hasMore == true){
				e.HasMorePages = true;
			}
		}
...
class DataGridViewPrinter {
	...
	public DataGridViewPrinter(DataGridView dataGridView,PrintDocument printDocument,bool isCentered,bool hasTitle,string title,Font fontTitle,Color colorTitle,bool doesUsePaging) {
		...
		_listHeightRows = [];
		_listWidthCols = [];
		_listIntArraysColumnPoints = [];
		_listWidthColumnPoints = [];
	...
	private void Calculate(Graphics g) {
		...
		for(var i = 0;i < _dataGridView.Columns.Count;i++) {
			...
			for(var j = 0;j < _dataGridView.Rows.Count;j++) {
		...
		var endPoint = _dataGridView.Columns.Count;
		...
		var widthTemp = _widthDataGridView;
		var printArea = (float)_widthPage - (float)_leftMargin - (float)_rightMargin;
			...
			for(idx = 0;idx < _dataGridView.Columns.Count;idx++) {
				if(_dataGridView.Columns[idx].Visible) {
					...
					if(widthTemp > printArea) {
						widthTemp -= _listWidthCols[idx];
						_listIntArraysColumnPoints.Add([startPoint,endPoint]);
	...
	private void DrawHeader(Graphics g) {
		...
		if(_doesUsePaging) {
			_pageNumber++;
			var pageNumber = "Page " + _pageNumber;
			...
			using var font = new Font("Tahoma",8,FontStyle.Regular,GraphicsUnit.Point);
			var rectangle = new RectangleF((float)_leftMargin,_yCoord,(float)_widthPage - (float)_rightMargin - (float)_leftMargin,g.MeasureString(pageNumber,font).Height);
		...
		if(_hasTitle) {
			...
			var rectangle = new RectangleF((float)_leftMargin,_yCoord,(float)_widthPage - (float)_rightMargin - (float)_leftMargin,g.MeasureString(_title,_fontTitle).Height);
		...
		var solidBrushFore = new SolidBrush(colorForeground);
		// Setting the HeaderBack style
		var colorBackground = _dataGridView.ColumnHeadersDefaultCellStyle.BackColor;
		...
		var solidBrushBackground = new SolidBrush(colorBackground);
		// Setting the LinePen that will be used to draw lines and rectangles (derived from the GridColor property of the DataGridView control)
		using var pen = new Pen(_dataGridView.GridColor,1);
		// Setting the HeaderFont style
		var fontHeader = _dataGridView.ColumnHeadersDefaultCellStyle.Font;
		...
		var rectangleF = new RectangleF(xCoord,_yCoord,_listWidthColumnPoints[_columnPoint],_heightRowHeader);
		g.FillRectangle(solidBrushBackground,rectangleF);
		// Setting the format that will be used to print each cell of the header row
		var stringFormatCell = new StringFormat();
		...
		float width;
		for(var i = (int)_listIntArraysColumnPoints[_columnPoint].GetValue(0);i < (int)_listIntArraysColumnPoints[_columnPoint].GetValue(1);i++) {
			if(!_dataGridView.Columns[i].Visible) {
	...
	private bool DrawRows(Graphics g) {
		...
		while(_currentRow < _dataGridView.Rows.Count) {
			if(_dataGridView.Rows[_currentRow].Visible) { // Print the cells of the CurrentRow only if that row is visible
				...
				for(var c = (int)_listIntArraysColumnPoints[_columnPoint].GetValue(0);c < (int)_listIntArraysColumnPoints[_columnPoint].GetValue(1);c++) {
					...
					var rectangleFCell = new RectangleF(xCoord,_yCoord,width,_listHeightRows[_currentRow]);
					// Printing the cell text
