...
public partial class FormEServicesTexting:FormODBase {
	...
	private ClinicDto GetSmsClinicSelected() {
		if(gridSmsSummary.GetSelectedIndex()<0) {
			return new ClinicDto();
		}
		if((ClinicDto)gridSmsSummary.ListGridRows[gridSmsSummary.GetSelectedIndex()].Tag==null){
			return new ClinicDto();
		}
		return (ClinicDto)gridSmsSummary.ListGridRows[gridSmsSummary.GetSelectedIndex()].Tag;
	}
	...
	private void FormEServicesTexting_Load(object sender,EventArgs e) {
		...
		butDefaultClinic.Visible=true;
		butDefaultClinicClear.Visible=true;
		FillGridSmsUsage();
		FillOptInSettings();
	}
	...
	private void butSaveShortCodes_Click(object sender,EventArgs e) {
		if(string.IsNullOrWhiteSpace(textShortCodeOptInClinicTitle.Text)) {
			var err=Lan.g(this,"Not allowed to set ")+labelShortCodeOptInClinicTitle.Text+Lan.g(this," to an empty value.");
			ODMessageBox.Show(err);
			return;
		}
		var doSetInvalidClinicPrefs=false;
		var doSetInvalidPrefs=false;
	...
	private void butDefaultClinic_Click(object sender,EventArgs e) {
		if(GetSmsClinicSelected().Id==0) {
			MsgBox.Show(this,"Select clinic to make default.");
			return;
		}
		Prefs.UpdateLong(PrefName.TextingDefaultClinicNum,(GetSmsClinicSelected().Id));
	...
	private void FillGridSmsUsage() {
		...
		if(!true) { //No clinics so just get the practice as a clinic.
			listClinics.Clear();
			listClinics.Add(Clinics.GetPracticeAsClinicZero());
		}
		var tablePhones=SmsPhones.GetSmsUsageLocal(listClinics.Select(x => x.Id).ToList(),dateTimePickerSms.Value,
			WebServiceMainHQProxy.EServiceSetup.SignupOut.SignupOutPhone.ToSmsPhones(SignupOut.Phones));
		var listEServicesSmsPhones=tablePhones.Rows.Cast<DataRow>().Select(x => new EServicesSmsPhone {
		...
		var doShowDiscount=listEServicesSmsPhones.Any(x => CompareDouble.IsGreaterThan(x.SentDiscount,0));
		gridSmsSummary.BeginUpdate();
		gridSmsSummary.Columns.Clear();
		if(true) {
			gridSmsSummary.Columns.Add(new GridColumn(Lan.g(this,"Default"),80) { TextAlign=HorizontalAlignment.Center });
		}
		gridSmsSummary.Columns.Add(new GridColumn(Lan.g(this,"Location"),170,HorizontalAlignment.Left));
		gridSmsSummary.Columns.Add(new GridColumn(Lan.g(this,"Subscribed"),80,HorizontalAlignment.Center));
		gridSmsSummary.Columns.Add(new GridColumn(Lan.g(this,"Primary\r\nPhone Number"),105,HorizontalAlignment.Center));
		gridSmsSummary.Columns.Add(new GridColumn(Lan.g(this,"Country\r\nCode"),60,HorizontalAlignment.Center));
		gridSmsSummary.Columns.Add(new GridColumn(Lan.g(this,"Limit"),80,HorizontalAlignment.Right));
		gridSmsSummary.Columns.Add(new GridColumn(Lan.g(this,"Sent\r\nFor Month"),70,HorizontalAlignment.Right));
		if(doShowDiscount) {
			gridSmsSummary.Columns.Add(new GridColumn(Lan.g(this,"Sent\r\nPre-Discount"),80,HorizontalAlignment.Right));
			gridSmsSummary.Columns.Add(new GridColumn(Lan.g(this,"Sent\r\nDiscount"),70,HorizontalAlignment.Right));
		}
		gridSmsSummary.Columns.Add(new GridColumn(Lan.g(this,"Sent\r\nCharges"),70,HorizontalAlignment.Right));
		gridSmsSummary.Columns.Add(new GridColumn(Lan.g(this,"Received\r\nFor Month"),70,HorizontalAlignment.Right));
		gridSmsSummary.Columns.Add(new GridColumn(Lan.g(this,"Received\r\nCharges"),70,HorizontalAlignment.Right));
		gridSmsSummary.ListGridRows.Clear();
		for(var i=0;i<listClinics.Count;i++) {
			var row=new GridRow();
			if(true) { //Default texting clinic?
				row.Cells.Add(listClinics[i].Id==PrefC.GetLong(PrefName.TextingDefaultClinicNum) ? "X" : "");
			}
			row.Cells.Add(listClinics[i].Abbr); //Location.
			var eServicesSmsPhone=listEServicesSmsPhones.FirstOrDefault(x => x.ClinicNum==listClinics[i].Id);
			if(eServicesSmsPhone==null) {
				...
				row.Cells.Add(listClinics[i].SmsContractSignedOn.HasValue ? Lan.g(this,"Yes") : Lan.g(this,"No"));
		...
		if(listClinics.Count>1) {//Total row if there is more than one clinic (Will not display for practice because practice will have no clinics.
			...
			row.Cells.Add("");
			row.Cells.Add("");
			row.Cells.Add(Lans.g("Total"));
			row.Cells.Add(listClinics.Where(x => listEServicesSmsPhones.Any(y => y.ClinicNum==x.Id)).Sum(x => x.SmsMonthlyLimit).ToString("c",new CultureInfo("en-US")));
