...
public partial class FormEClipboardImageCaptureDefEdit : FormODBase {
	...
	private void FormEClipboardImageCaptureDefEdit_Load(object sender,EventArgs e) {
		...
		var interval=new Interval(EClipboardImageCaptureDefCur.ResubmitInterval);
	...
	private void listDefinition_SelectionChangeCommitted(object sender,EventArgs e) {
		var defNum=listDefinition.GetSelectedKey<Def>(x=>x.DefNum);
		if(defNum<=0){
	...
	private void butDefinitions_Click(object sender,EventArgs e) {
		var defNumSelected=listDefinition.GetSelectedKey<Def>(x=>x.DefNum);
		using var formEClipboardDefs=new FormEClipboardDefs();
	...
	private void butSave_Click(object sender,EventArgs e) {
		...
		var enumOcrCaptureTypeSelected=listOCRCaptureType.GetSelected<EnumOcrCaptureType>();
		if(enumOcrCaptureTypeSelected!=EnumOcrCaptureType.Miscellaneous){//EnumOcrCaptureType validation - If not a misc image, do this check.
			var listEClipboardImageCaptureDefsForClinic=ListEClipboardImageCaptureDefs.FindAll(x=>
