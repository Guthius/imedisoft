...
namespace OpenDental {
	public partial class FormPreferences:FormODBase {
		...
		private List<PrefInf> _listPrefInfs= [];
		...
		private List<TreeNodeAndControls> _listTreeNodeAndControls= [];
		...
		private void FormModulePrefs_Load(object sender,EventArgs e) {
			...
			panelInfo.Size=new Size(16,16);
			panelInfo.Visible=false;
			var graphicsPath=new GraphicsPath();
			graphicsPath.AddEllipse(0,0,14.5f,14.5f);
			...
			var xmlSerializer=new XmlSerializer(_listPrefInfs.GetType());
			...
			var listTreeNodes=GetListNodesFlat();
			for(var i=0;i<listTreeNodes.Count;i++) {
				treeNodeAndControls=new TreeNodeAndControls();
				treeNodeAndControls.Node=listTreeNodes[i];
				var listControls=GetListControlsFlat((UserControl)treeNodeAndControls.Node.Tag);//Get all controls for said node
				var listControlAndPrefInfos=new List<ControlAndPrefInfo>();
				ControlAndPrefInfo controlAndPref;
				for(var c=0;c<listControls.Count;c++){
		...
		private void FormPreferences_MouseMove(object sender,MouseEventArgs e){
			...
			var prefInf=GetPrefInf(control);
			...
			panelInfo.BringToFront();
			var pointScreen=control.PointToScreen(e.Location);
			var pointWindow=PointToClient(pointScreen);
			var pointShow=new Point(pointWindow.X+10,pointWindow.Y-10);
			panelInfo.Location=pointShow;
		}
		...
		private void FormPreferences_MouseDown(object sender,MouseEventArgs e){
			...
			var html="<!DOCTYPE html><html><head><meta charset=\"UTF-8\"></head>"
			         +"<body style='font-family:sans-serif; font-size:"+(int)11+"px'>"
			...
			var pointWindowR=panelMain.PointToScreen(new Point(476,0));//we only care about x (# of pixels moved to the right from left of panelMain)
			var pointControlUR=control.PointToScreen(new Point(0,panelMain.Top-50));//we only care about y (# of pixels moved upwards from top of control)
			var pointShow=new Point(pointWindowR.X,pointControlUR.Y);
		...
		private PrefInf GetPrefInf(Control control){
			...
			var category=parent.Name.Substring(11);
			var prefInf=_listPrefInfs.Find(x=>x.ControlName==control.Name && x.Category==category);
			if(prefInf==null){
				//Then match by text
		...
		private void PanelInfo_Paint(object sender,PaintEventArgs e) {
			...
			g.FillEllipse(Brushes.Blue,1.5f,1.5f,12,12);
			using var font=new Font("Times New Roman",10f,FontStyle.Bold);
			...
			var rectangleFBounds=graphicsPath.GetBounds();
			g.TranslateTransform(1.5f+6,1.5f+6);//to center of ellipse
		...
		private void textSearch_TextChanged(object sender,EventArgs e) {
			...
			for(var i=0;i<_listTreeNodeAndControls.Count;i++) {
				var isCategoryYellow=false;
				if(textSearch.Text!="" && _listTreeNodeAndControls[i].Node.Text.ToLower().Contains(textSearch.Text.ToLower())){
					isCategoryYellow=true;
				}
				//labels
				for(var c=0;c<_listTreeNodeAndControls[i].ListControlAndPrefInfos.Count;c++){
					var controlAndPref=_listTreeNodeAndControls[i].ListControlAndPrefInfos[c];
					if(controlAndPref.Control_ is Label label){
						...
						var prefInf=controlAndPref.PrefInf_;
		...
		private List<TreeNode> GetListNodesFlat(){
			var listTreeNodes=new List<TreeNode>();
			for(var i=0;i<treeMain.Nodes.Count;i++){
		...
		private void treeMain_AfterSelect(object sender,TreeViewEventArgs e) {
			var userControlSelected=treeMain.SelectedNode.Tag as UserControl;
		...
		private void FillUserControls() {
			var listUserControls=new List<UserControl>();
			...
			for(var i=0;i<listUserControls.Count;i++) {
				listUserControls[i].Dock=DockStyle.Fill;
				listUserControls[i].Controls.Add(panelMain);
			}
			FillTree();
		}
		...
		private bool SaveSyncedPrefs(){
			var prefValSync=_listPrefValSyncs.Find(x=>x.PrefName_==PrefName.ProcessSigsIntervalInSecs);
