...
public partial class FormBillingOptions : FormODBase {
	...
	private void FormBillingOptions_Load(object sender, System.EventArgs e) {
		...
		comboInsFilingCodes.Items.Add(Lan.g(this,"None"));
		if(_listInsFilingCodes.Count > 0) {
			comboInsFilingCodes.Items.Add(Lan.g(this,"(all)"));
			comboInsFilingCodes.Items.AddList(_listInsFilingCodes, x => x.Descript);
		}
	...
	private void RefreshClinicPrefs() {
		var listPrefNames=new List<PrefName> {
			...
			PrefName.BillingDaysExcludeInsPending,
			...
			PrefName.BillingSelectInsFilingCodes
		};
		...
		foreach(var clinicNum in _dictionaryClinicPrefsNew.Keys) {
			var listExistingClinicPrefs=_dictionaryClinicPrefsNew[clinicNum].Select(x => x.PrefName).ToList();
			var listMissingClincPrefs=listPrefNames.FindAll(x => !listExistingClinicPrefs.Contains(x));
			foreach(var prefName in listMissingClincPrefs) {
				switch(EnumTools.GetAttributeOrDefault<PrefNameAttribute>(prefName).ValueType) {
					case PrefValueType.BOOL:
						var defaultBool=PrefC.GetBool(prefName);
					...
					case PrefValueType.STRING:
						var defaultStr=PrefC.GetString(prefName);
						_dictionaryClinicPrefsNew[clinicNum].Add(new ClinicPref(clinicNum,prefName,defaultStr));
						break;
					case PrefValueType.INT:
						var defaultInt=PrefC.GetInt(prefName);
						_dictionaryClinicPrefsNew[clinicNum].Add(new ClinicPref(clinicNum,prefName,SOut.Int(defaultInt)));
						break;
				}
			}
		}
	...
	private void SetFiltersForClinicNums(List<long> listClinicNums,bool isTextNoteExcluded=false) {
		   ...
		   || !_dictionaryClinicPrefsNew.ContainsKey(listClinicNums[0]))//They have not saved their default filter options for the selected clinic. Use default prefs.
		{
			...
			var stringArrayselectedBillTypes=PrefC.GetString(PrefName.BillingSelectBillingTypes).Split([','],StringSplitOptions.RemoveEmptyEntries);
			for(var i = 0;i<stringArrayselectedBillTypes.Length;++i) { 
				try{
					var order=Defs.GetOrder(DefCat.BillingTypes,Convert.ToInt64(stringArrayselectedBillTypes[i]));
					if(order==-1) {
						continue;
					}
			...
			#endregion
			#region Ins Filing Codes
			comboInsFilingCodes.SetAll(false);
			var stringArraySelectedInsFilingCodes=PrefC.GetString(PrefName.BillingSelectInsFilingCodes).Split(",",StringSplitOptions.RemoveEmptyEntries);
			if(stringArraySelectedInsFilingCodes.Length==0) {//Set to None.
				comboInsFilingCodes.SelectedIndex=0;
			}
			else if(stringArraySelectedInsFilingCodes.Contains("-1")) {//(all)
				comboInsFilingCodes.SelectedIndex=1;
			}
			else {
				for(var i=0;i < stringArraySelectedInsFilingCodes.Length;++i) {
					try {
						var order=_listInsFilingCodes.FindIndex(x => x.InsFilingCodeNum==Convert.ToInt64(stringArraySelectedInsFilingCodes[i]));
						if(order==-1) {
							continue;
						}
			...
			var listClinicPrefs=_dictionaryClinicPrefsNew[listClinicNums[0]];//By definition of how ClinicPrefs are created, First will always return a result.
			checkIncludeChanged.Checked=SIn.Bool(listClinicPrefs.First(x => x.PrefName==PrefName.BillingIncludeChanged).ValueString);
			#region BillTypes
			listBillType.ClearSelected();
			var stringArraySelectedBillTypes=listClinicPrefs.First(x => x.PrefName==PrefName.BillingSelectBillingTypes)
				.ValueString.Split([','],StringSplitOptions.RemoveEmptyEntries);
			for(var i = 0;i<stringArraySelectedBillTypes.Length;++i) {
				try {
					var order=Defs.GetOrder(DefCat.BillingTypes,Convert.ToInt64(stringArraySelectedBillTypes[i]));
					if(order==-1) {
			...
			#region Ins Filing Codes
			comboInsFilingCodes.SetAll(false);
			var stringArraySelectedInsFilingCodes=listClinicPrefs.First(x => x.PrefName==PrefName.BillingSelectInsFilingCodes)
				.ValueString.Split(",",StringSplitOptions.RemoveEmptyEntries);
			if(stringArraySelectedInsFilingCodes.Length==0) {//Set to None.
				comboInsFilingCodes.SelectedIndex=0;
			}
			...
			#endregion
			checkBadAddress.Checked=SIn.Bool(listClinicPrefs.First(x => x.PrefName==PrefName.BillingExcludeBadAddresses).ValueString);
			checkExcludeInactive.Checked=SIn.Bool(listClinicPrefs.First(x => x.PrefName==PrefName.BillingExcludeInactive).ValueString);
			checkShowNegative.Checked=!SIn.Bool(listClinicPrefs.First(x => x.PrefName==PrefName.BillingExcludeNegative).ValueString);
			checkExcludeInsPending.Checked=SIn.Bool(listClinicPrefs.First(x => x.PrefName==PrefName.BillingExcludeInsPending).ValueString);
			checkExcludeIfProcs.Checked=SIn.Bool(listClinicPrefs.First(x => x.PrefName==PrefName.BillingExcludeIfUnsentProcs).ValueString);
			textExcludeLessThan.Text=listClinicPrefs.First(x => x.PrefName==PrefName.BillingExcludeLessThan).ValueString;
			checkIgnoreInPerson.Checked=SIn.Bool(listClinicPrefs.First(x => x.PrefName==PrefName.BillingIgnoreInPerson).ValueString);
			checkBoxBillShowTransSinceZero.Checked=SIn.Bool(listClinicPrefs.First(x => x.PrefName==PrefName.BillingShowTransSinceBalZero).ValueString);
			if(!isTextNoteExcluded) {
				textNote.Text=listClinicPrefs.First(x => x.PrefName==PrefName.BillingDefaultsNote).ValueString;
			}
			if(checkExcludeInsPending.Checked) {
				textDaysInsPendingExclude.Value=SIn.Int(listClinicPrefs.First(x => x.PrefName==PrefName.BillingDaysExcludeInsPending).ValueString);
				textDaysInsPendingExclude.Enabled=true;
			}
			else {
				textDaysInsPendingExclude.Value=0;
	...
	private bool areFilingCodeSelectionsValid() {
		if(comboInsFilingCodes.SelectedIndices.Count < 2) {
			return true;
		}
		var isNoneAndAllSelected=comboInsFilingCodes.SelectedIndices.Contains(0) && comboInsFilingCodes.SelectedIndices.Contains(1);//Cannot select both None and all.
		var isNoneAllAndFilingCodesSelected=(comboInsFilingCodes.SelectedIndices.Contains(0) || comboInsFilingCodes.SelectedIndices.Contains(1))
		                                    && (comboInsFilingCodes.SelectedIndices.Any(x => x > 1));//Cannot select None and/or All and individual filing codes.
		if(isNoneAndAllSelected || isNoneAllAndFilingCodesSelected) {
			return false;
		}
		return true;
	}
	...
	private void butSaveDefault_Click(object sender,System.EventArgs e) {
		...
		var selectedInsFilingCodes="";//indicates None.
		if(comboInsFilingCodes.SelectedIndices.Count > 0 && !comboInsFilingCodes.SelectedIndices.Contains(0)) {
			selectedInsFilingCodes="-1";//(all)
			if(!comboInsFilingCodes.SelectedIndices.Contains(1)) {
				selectedInsFilingCodes=string.Join(",",comboInsFilingCodes.SelectedIndices.OfType<int>().Select(x => _listInsFilingCodes[x-2].InsFilingCodeNum));
			}
		}
		var ageOfAccount="";
		...
		var listClinicNums = comboClinic.ListClinicNumsSelected;
		if(listClinicNums.Count != 1 || listClinicNums.Contains(0))//Clinics not enabled (count 0) or multiple selected(>1) or 'Unassigned' selected.
		{
			if(Prefs.UpdateBool(PrefName.BillingIncludeChanged,checkIncludeChanged.Checked)
			   ...
			   |Prefs.UpdateString(PrefName.BillingSelectInsFilingCodes,selectedInsFilingCodes))
			{
			...
			_dictionaryClinicPrefsNew[listClinicNums[0]].First(x => x.PrefName==PrefName.BillingIncludeChanged).ValueString=SOut.Bool(checkIncludeChanged.Checked);
			...
			_dictionaryClinicPrefsNew[listClinicNums[0]].First(x => x.PrefName==PrefName.BillingExcludeBadAddresses).ValueString=SOut.Bool(checkBadAddress.Checked);
			_dictionaryClinicPrefsNew[listClinicNums[0]].First(x => x.PrefName==PrefName.BillingExcludeInactive).ValueString=SOut.Bool(checkExcludeInactive.Checked);
			_dictionaryClinicPrefsNew[listClinicNums[0]].First(x => x.PrefName==PrefName.BillingExcludeNegative).ValueString=SOut.Bool(!checkShowNegative.Checked);
			_dictionaryClinicPrefsNew[listClinicNums[0]].First(x => x.PrefName==PrefName.BillingExcludeInsPending).ValueString=SOut.Bool(checkExcludeInsPending.Checked);
			_dictionaryClinicPrefsNew[listClinicNums[0]].First(x => x.PrefName==PrefName.BillingDaysExcludeInsPending).ValueString=SOut.Int(textDaysInsPendingExclude.Value);
			_dictionaryClinicPrefsNew[listClinicNums[0]].First(x => x.PrefName==PrefName.BillingExcludeIfUnsentProcs).ValueString=SOut.Bool(checkExcludeIfProcs.Checked);
			...
			_dictionaryClinicPrefsNew[listClinicNums[0]].First(x => x.PrefName==PrefName.BillingIgnoreInPerson).ValueString=SOut.Bool(checkIgnoreInPerson.Checked);
			_dictionaryClinicPrefsNew[listClinicNums[0]].First(x => x.PrefName==PrefName.BillingShowTransSinceBalZero).ValueString=SOut.Bool(checkBoxBillShowTransSinceZero.Checked);
			_dictionaryClinicPrefsNew[listClinicNums[0]].First(x => x.PrefName==PrefName.BillingDefaultsNote).ValueString=textNote.Text;
			_dictionaryClinicPrefsNew[listClinicNums[0]].First(x => x.PrefName==PrefName.BillingSelectInsFilingCodes).ValueString=selectedInsFilingCodes;
		}
		else {//No existing ClinicPrefs for the currently selected clinic in comboClinics.
			_dictionaryClinicPrefsNew.Add(listClinicNums[0], []);
			...
			_dictionaryClinicPrefsNew[listClinicNums[0]].Add(new ClinicPref(listClinicNums[0],PrefName.BillingDaysExcludeInsPending,SOut.Int(textDaysInsPendingExclude.Value)));
			...
			_dictionaryClinicPrefsNew[listClinicNums[0]].Add(new ClinicPref(listClinicNums[0],PrefName.BillingSelectInsFilingCodes,selectedInsFilingCodes));
		}
	...
	private void SetClinicFilters() {
		var listSelectedClinicNums=comboClinic.ListClinicNumsSelected;
	...
	private void FillDunning(){
		...
		if(comboClinic.IsAllSelected) {
			listClinicNums= [];//Empty list to allow query to run for all clinics.
		}
		else {
		...
		var column=new GridColumn("Billing Type",80);
		gridDunning.Columns.Add(column);
		...
		GridRow row;
		for(var i = 0;i<_listDunnings.Count;++i) { 
			row=new GridRow();
			...
			if(_listDunnings[i].AgeAccount==0){
				...
				row.Cells.Add(Lan.g(this,"Over ")+_listDunnings[i].AgeAccount);
			}
	...
	private void gridDun_CellDoubleClick(object sender, OpenDental.UI.ODGridClickEventArgs e) {
		using var formDunningEdit=new FormDunningEdit(_listDunnings[e.Row]);
	...
	private void butDunningSetup_Click(object sender, System.EventArgs e) {
		using var formDunningSetup=new FormDunningSetup();
		formDunningSetup.ShowDialog();
		FillDunning();
	}

	private void butDefaults_Click(object sender,EventArgs e) {
		using var formBillingDefaults=new FormBillingDefaults();
	...
	private void SetDefaults(){
		...
		if(SmsPhones.IsIntegratedTextingEnabled()) {
			...
			var stringArrayBillingDefaultsModesToText=PrefC.GetString(PrefName.BillingDefaultsModesToText)
				.Split([","],StringSplitOptions.RemoveEmptyEntries);
			for(var i = 0;i<stringArrayBillingDefaultsModesToText.Length;++i) {
				listModeToText.SetSelected(SIn.Int(stringArrayBillingDefaultsModesToText[i]),true);
			}
		}
	}
	...
	private void butCreate_Click(object sender, System.EventArgs e) {
		...
		var sendStatementsIO=new SendStatementsIO();
		sendStatementsIO.Source="FormBillingOptions";
		sendStatementsIO.ActionPrompt=(prompt,_) => { ODMessageBox.Show(prompt); };
		...
		if(!BillingL.RunAgingEnterprise(sendStatementsIO)) {
			return;
		}
		int countCreated;
	...
	private void comboInsFilingCodes_SelectionChangeCommitted(object sender,EventArgs e) {
		if(comboInsFilingCodes.SelectedIndices.Count < 1) {//No selections made, default to None.
			comboInsFilingCodes.SelectedIndices= [0];
		}
	}
	...
	private int CreateManyHelper(List<long> listClinicNums) {
		...
		var dictionaryClinicPatAgingData=AgingData.GetAgingData(checkSinglePatient.Checked,checkIncludeChanged.Checked,
				checkExcludeInsPending.Checked,checkExcludeIfProcs.Checked,checkSuperFam.Checked,listClinicNums,textDaysInsPendingExclude.Value)
			.GroupBy(x => x.Value.ClinicNum)
			.ToDictionary(x => x.Key,x => x.ToDictionary(y => y.Key,y => y.Value));
		var countAllClinics=0;
		for(var i=0;i<listClinicNums.Count;i++) {
			...
			if(!dictionaryClinicPatAgingData.TryGetValue(listClinicNums[i],out var dictionaryPatAgingData)) {
				dictionaryPatAgingData=new Dictionary<long,PatAgingData>();
			}
			var countThisClinic=CreateHelper(listClinicNums[i],checkUseClinicDefaults.Checked,true,dictionaryPatAgingData);
		...
		using var msgBoxCopyPaste=new MsgBoxCopyPaste(_popUpMessage);
	...
	private int CreateHelper(long clinicNum,bool useClinicPrefs=false,bool suppressPopup=false,Dictionary<long,PatAgingData> dictionaryPatAgingData=null) {
		if(useClinicPrefs) {//Clincs must be enabled.
			...
			SetFiltersForClinicNums([clinicNum],textNote.Visible);
		}
		else {
		...
		var lastStatement=SIn.Date(textLastStatement.Text);
		...
		var billingNums=new List<long>();//[listBillType.SelectedIndices.Count];
		for(var i=0;i<listBillType.SelectedIndices.Count;i++){
		...
		if(comboInsFilingCodes.SelectedIndices[0]==0) {//None
			...
			for(var i=0;i < comboInsFilingCodes.SelectedIndices.Count;i++) {
		...
		var listPatAgings=new List<PatAging>();
		if(dictionaryPatAgingData==null) {//If not passed in, we must generate the data here.
			dictionaryPatAgingData=AgingData.GetAgingData(checkSinglePatient.Checked,checkIncludeChanged.Checked,checkExcludeInsPending.Checked,
				checkExcludeIfProcs.Checked,checkSuperFam.Checked,listClinicNums,textDaysInsPendingExclude.Value);
		}
		...
		var listPendingInsPatNums=new List<long>();
		var listUnsentPatNums=new List<long>();
		var dictionaryPatAgingTransactions=new Dictionary<long,List<PatAgingTransaction>>();
		if(checkExcludeInsPending.Checked || checkExcludeIfProcs.Checked || checkIncludeChanged.Checked) {
			foreach(var kvp in dictionaryPatAgingData) {
				if(true && !listClinicNums.IsNullOrEmpty() && !listClinicNums.Contains(kvp.Value.ClinicNum)) {
					continue;
				}
		...
		var progressOD=new ProgressWin();
		progressOD.ActionMain=() => {
			var listPatNumsToExclude=PatPlans.GetPatNumsByInsFilingCodes(listInsFilingCodeNums);
			listPatAgings=Patients.GetAgingList(getAge,lastStatement,billingNums,checkBadAddress.Checked,!checkShowNegative.Checked,
				SIn.Double(textExcludeLessThan.Text),checkExcludeInactive.Checked,checkIgnoreInPerson.Checked,listClinicNums,checkSuperFam.Checked,
		...
		try {
			...
			var text=Lan.g(this,"Error getting list:")+" "+ex.Message+"\r\n\n\n"+ex.StackTrace;
		...
		var listPatientsSuperHeads=new List<Patient>();
		...
		if(checkSuperFam.Checked) {
			var listPatAgingsSuperFamilies=new List<PatAging>();
			for(var i=listPatAgings.Count-1;i>=0;i--) {//Go through each PatAging in the retrieved list
				...
				var patientSuperFamilyHead=listPatientsSuperHeads.FirstOrDefault(x => x.PatNum==listPatAgings[i].SuperFamily);
				...
				if(listPatAgings[i].HasSuperBilling && patientSuperFamilyHead.HasSuperBilling) {
					var patAging=listPatAgingsSuperFamilies.FirstOrDefault(x => x.PatNum==patientSuperFamilyHead.PatNum);//Attempt to find an existing PatAging for the superhead.
					if(patAging==null) {
		...
		if(listPatAgings.Count==0){
			...
			_popUpMessage+=Lan.g(this,"Statements created")+": "+SOut.Int(listPatAgings.Count)+"\r\n";
		}
		#endregion
		IsHistoryStartMinDate=string.IsNullOrWhiteSpace(textDateStart.Text);
		var dateRangeFrom=SIn.Date(textDateStart.Text);
		var dateRangeTo=DateTime.Today;//Needed for payplan accuracy.//new DateTime(2200,1,1);
		if(textDateEnd.Text!=""){
			dateRangeTo=SIn.Date(textDateEnd.Text);
		}
		Statement statement;
		...
		for(var i = 0;i<listPatAgings.Count;++i) {
			...
			var doSendSms=Statements.DoSendSms(listPatAgings[i],dictionaryPatAgingData,listModeToText.GetListSelected<StatementMode>());
			...
			var shortGuidUrl=listShortGuidUrls.FirstOrDefault(x => x.IsForSms==doSendSms);
			...
			var dateBalZeroCur=patAgingData?.DateBalZero??DateTime.MinValue;//dateBalZeroCur will be DateTime.MinValue if PatNum isn't in dict
			...
			var listDunningsForPat=_listDunnings.FindAll(x => (x.BillingType==0 || x.BillingType==listPatAgings[i].BillingType) //same billing type
			                                                  && x.ClinicNum==listPatAgings[i].ClinicNum | x.ClinicNum==-2 //matches a specific clinic or unassigned clinic, xor "All" Clinics
			...
			var dunning=listDunningsForPat.LastOrDefault(x => !x.IsSuperFamily);
