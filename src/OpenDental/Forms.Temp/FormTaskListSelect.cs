...
public partial class FormTaskListSelect : FormODBase {
	...
	public List<long> ListSelectedLists= [];
	...
	private List<TaskList> GetUserInboxTaskLists() {
		var listTaskListsUserInbox=TaskLists.GetMany(Userods.GetDeepCopy(true).Select(x => x.TaskListInBox).ToList());
	...
	private List<TaskList> GetListUser() {
		...
		var listTaskListsRetVal=GetFilteredList();
		if(_taskListUserInbox!=null) {
		...
		var taskListTriage=listTaskListsRetVal.Find(x => x.Descript==" Triage");
	...
	private List<TaskList> GetFilteredList() {
		...
		var listTaskListsFiltered=new List<TaskList>();
		var dictAllTaskLists=_listTaskListsAll.ToDictionary(x => x.TaskListNum);//Convert to dictionary for faster lookups
		var listTaskLists=checkIncludeAll.Checked?_listTaskListsAll:_listTaskListsUnfiltered;
		StringBuilder stringBuilderbSubListPath;//String builder because we want speeeed
		for(var i=0;i<listTaskLists.Count;i++) {
			var taskList=listTaskLists[i].Copy(); //Copy so we don't modify the original in case it's another list's parent.
			stringBuilderbSubListPath=new StringBuilder();
			var parentNumList=taskList.Parent;
			var isSubList=false;
			while(dictAllTaskLists.TryGetValue(parentNumList,out var taskListParent)) {
	...
	private void SetLabelText() {
		...
		var strUserSetup=(_taskObjectType==TaskObjectType.None ? Lan.g(this,"If a user is not in the list, then their inbox has not been setup yet. ") : "");
		var objTypeStr=GetOTypeDescription();
		if(checkMulti.Checked) {
	...
	private void checkMulti_CheckedChanged(object sender,EventArgs e) {
		if(checkMulti.Checked && false) {//If user is in checkMulti state, deselect the list
	...
	protected override bool ProcessCmdKey(ref Message message,Keys keysData) {
		...
		var wasProcessed=base.ProcessCmdKey(ref message,keysData);
		return wasProcessed;
	}
	...
	private void butOK_Click(object sender, System.EventArgs e) {
		if(gridMain.GetSelectedIndex()==-1){
			var msg=Lan.g(this,"Please select a ")+GetOTypeDescription()+Lan.g(this," first.");
