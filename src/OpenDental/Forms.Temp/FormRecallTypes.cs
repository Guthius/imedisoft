...
public partial class FormRecallTypes:FormODBase {
	...
	private void FillGrid(){
		...
		var col=new GridColumn(Lan.g("TableRecallTypes","Description"),110);
	...
	private string GetStringForType(long recallTypeNum) {
		...
		var listRecallTriggers=RecallTriggers.GetForType(recallTypeNum);
		var retVal="";
		for(var i=0;i<listRecallTriggers.Count;i++){
			if(i>0){
				retVal+=",";
			}
			retVal+=ProcedureCodes.GetStringProcCode(listRecallTriggers[i].CodeNum);
		}
		return retVal;
	}

	private void butAdd_Click(object sender, System.EventArgs e) {
		using var formRecallTypeEdit=new FormRecallTypeEdit();
		formRecallTypeEdit.RecallTypeCur=new RecallType();
		formRecallTypeEdit.RecallTypeCur.IsNew=true;
		formRecallTypeEdit.ShowDialog();
		FillGrid();
		_hasChanged=true;
	}
	...
	private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		...
		using var formRecallTypeEdit=new FormRecallTypeEdit();
		formRecallTypeEdit.RecallTypeCur=_listRecallTypes[e.Row].Copy();
		formRecallTypeEdit.ShowDialog();
		FillGrid();
		_hasChanged=true;
		//}*/
	...
	private void butSynch_Click(object sender,EventArgs e) {
		...
		var actionCloseRecallSyncProgress=ODProgress.Show(Lan.g(this,"Running Prep Queries")+"...",false,true);
		var isSyncCompleted=Recalls.SynchAllPatients();
	...
	private void FormRecallTypes_FormClosing(object sender,FormClosingEventArgs e) {
		   ...
		   ==DialogResult.Yes)
		{
			Cursor=Cursors.WaitCursor;
			GC.Collect();//free up resources since this could take a lot of memory
			var actionCloseRecallSyncProgress=ODProgress.Show(Lan.g(this,"Running Prep Queries")+"...");
			var isSyncSuccessful=Recalls.SynchAllPatients();
			actionCloseRecallSyncProgress?.Invoke();
			GC.Collect();//clean up resources, force the garbage collector to collect since resources may remain tied-up
			Cursor=Cursors.Default;
			if(!isSyncSuccessful) {
