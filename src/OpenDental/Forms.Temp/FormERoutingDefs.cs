...
public partial class FormERoutingDefs:FormODBase {
	...
	private void FillGrid() {
		...
		var listERouting = ERoutingDefs.GetByClinic(_clinicNum);
	...
	private void butAdd_Click(object sender,EventArgs e) {
		using var formERoutingEdit = new FormERoutingDefEdit(_clinicNum);
	...
	private void butDuplicate_ClicK(object sender,EventArgs e) {
		...
		var eRoutingDefOrig=gridERouting.SelectedTag<ERoutingDef>();
		var eRoutingDefCopy=eRoutingDefOrig.Copy();
		var eRoutingDefNum=ERoutingDefs.Insert(eRoutingDefCopy);
		//get a copy of the actiondefs from the original item, inserting new copies pointing to our new patientflowdef.
		var listERoutingActions=ERoutingActionDefs.GetAllByERoutingDef(eRoutingDefOrig.ERoutingDefNum);
		for(var i=0; i<listERoutingActions.Count; ++i) {
			var newAction=new ERoutingActionDef();
		...
		var listERoutingDefLinks=ERoutingDefLinks.GetListERoutingTypesForERoutingDefNum(eRoutingDefOrig.ERoutingDefNum);
		for(var i=0; i<listERoutingDefLinks.Count; ++i) {
	...
	private void GridCell_DoubleClick(object sender, ODGridClickEventArgs e) {
		...
		using var formERoutingDefEdit = new FormERoutingDefEdit(gridERouting.SelectedTag<ERoutingDef>());
	...
	private void ChangeClinic(long clinicNum) {
		...
		var isEditingEnabled=_clinicNum==0 || !_clinicPrefHelper.GetBoolVal(PrefName.ERoutingUseHQDefaults, _clinicNum);
