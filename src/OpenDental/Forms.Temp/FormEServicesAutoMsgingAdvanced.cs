...
public partial class FormEServicesAutoMsgingAdvanced:FormODBase {
	...
	private void FormAutomatedConfirmationStatuses_Load(object sender,EventArgs e) {
		...
		var apptEConfirmStatusSentNum=PrefC.GetLong(PrefName.ApptEConfirmStatusSent);
		var apptEConfirmStatusAcceptedNum=PrefC.GetLong(PrefName.ApptEConfirmStatusAccepted);
		var apptEConfirmStatusDeclinedNum=PrefC.GetLong(PrefName.ApptEConfirmStatusDeclined);
		var apptEConfirmStatusSendFailedNum=PrefC.GetLong(PrefName.ApptEConfirmStatusSendFailed);
		//SENT
		...
		for(var i=0;i<_listDefNumsUneditable.Count;i++) {
	...
	private void FillLists() {
		...
		_listDefNumsUneditable=
		[
			_defNumTimeArrived, PrefC.GetLong(PrefName.AppointmentTimeDismissedTrigger),
	...
	private void SaveToDb() {
		var isPrefRefreshRequired=false;
		#region Confirmation Settings
		...
		var isClinicPrefRefreshRequired=false;
	...
	private void ParseThankYouTitle() {
		var clinicPref=GetClinicPrefFromList(PrefName.ApptThankYouCalendarTitle,_clinicNumCur);
	...
	private ClinicPref GetClinicPrefFromList(PrefName prefName, long clinicNum) {
		var clinicPref=_listClinicPrefs.FirstOrDefault(x => x.ClinicNum==clinicNum && x.PrefName==prefName);
		if(clinicPref==null) {
			if(prefName==PrefName.ApptThankYouCalendarTitle) {
				var thankYouTitle=PrefC.GetString(PrefName.ApptThankYouCalendarTitle);
				clinicPref=new ClinicPref(clinicNum,prefName,thankYouTitle);
			}
			else if (prefName==PrefName.ThankYouTitleUseDefault) {
				var thankYouUseDefault=PrefC.GetBool(PrefName.ThankYouTitleUseDefault);
	...
	private void FillThankYouTitleAndUseDefault() {
		...
		var clinicPrefThankYouTitle=GetClinicPrefFromList(PrefName.ApptThankYouCalendarTitle,_clinicNumCur);
		if(_clinicNumCur==0) {
			checkUseDefault.Visible=false;
		...
		var clinicPrefThankYouUseDefault=GetClinicPrefFromList(PrefName.ThankYouTitleUseDefault,_clinicNumCur);
		checkUseDefault.Visible=true;
		var useDefault=SIn.Bool(clinicPrefThankYouUseDefault.ValueString);
		checkUseDefault.Checked=useDefault;
		if(useDefault) {
			textCalendarIcsTitle.Enabled=false;
			var clinicPrefThankYouTitleHeadquarters=GetClinicPrefFromList(PrefName.ApptThankYouCalendarTitle,0);
	...
	private void checkUseDefault_Click(object sender,EventArgs e) {
		...
		var clinicPref=GetClinicPrefFromList(PrefName.ThankYouTitleUseDefault,_clinicNumCur);
	...
	private void gridMain_CellClick(object sender,ODGridClickEventArgs e) {	
		...
		var row=gridMain.ListGridRows[e.Row];
		var cell=row.Cells[e.Col];
		...
		if(e.Col==1) { //Color column
			...
			var listDefNums=(List<long>)gridMain.Columns[e.Col].Tag;
	...
	private void butSave_Click(object sender,EventArgs e) {
		...
		var listSelectedEConfirmationStatuses=new List<int>();
		...
		var countDistinct=listSelectedEConfirmationStatuses.Distinct().Count();
		var countAll=listSelectedEConfirmationStatuses.Count();
	...
	private void butExclusionDays_Click(object sender,EventArgs e) {
		using var formeConfirmationExclusionDays=new FormeConfimationExclusionDays(_clinicNumCur);
		formeConfirmationExclusionDays.ShowDialog();
	}
}