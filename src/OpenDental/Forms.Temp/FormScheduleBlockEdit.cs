...
public partial class FormScheduleBlockEdit : FormODBase	{
	...
	private void FormScheduleBlockEdit_Load(object sender,System.EventArgs e) {
		...
		for(var i=0;i<_listDefsBlockoutCategories.Count;i++){
		...
		var listOperatories=Operatories.GetDeepCopy(true);
		_listOperatories= [];
		//Get all op nums for this schedule, as _schedCur.Ops may not actually contain all of them.
		var listSchedOpNums=ScheduleOps.GetForSched(_schedule.ScheduleNum).Select(x=>x.OperatoryNum).ToList();
		for(var i=0;i<listOperatories.Count;i++) {
			if(true && _clinicNum!=0) {//Using clinics and a clinic filter was passed in.
	...
	private void butDelete_Click(object sender, System.EventArgs e) {
		if(ODMessageBox.Show(Lan.g(this,"Delete Blockout?"),"",MessageBoxButtons.OKCancel)!=DialogResult.OK){
			return;   
		}
		if(IsNew){
			DialogResult=DialogResult.Cancel; 
	...
	private void butSave_Click(object sender, System.EventArgs e) { 
		...
		_schedule.Ops= [];
		for(var i=0;i<listOp.SelectedIndices.Count;i++){
