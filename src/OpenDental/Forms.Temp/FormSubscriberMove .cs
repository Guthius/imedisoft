...
public partial class FormSubscriberMove:FormODBase {
	...
	private void butViewInsPlanInto_Click(object sender,EventArgs e) {
		if(_insPlanInto==null || InsPlans.GetPlan(_insPlanInto.PlanNum, [])==null) {
			MsgBox.Show(this,"Valid insurance plan not selected.\r\nPlease select a valid insurance plan using the picker button.");
			return;
		}
		using var formInsPlan=new FormInsPlan(_insPlanInto,null,null);
		formInsPlan.ShowDialog();
	}

	private void butViewInsPlanFrom_Click(object sender,EventArgs e) {
		if(_insPlanFrom==null || InsPlans.GetPlan(_insPlanFrom.PlanNum, [])==null) {
			MsgBox.Show(this,"Valid insurance plan not selected.\r\nPlease select a valid insurance plan using the picker button.");
			return;
		}
		using var formInsPlan=new FormInsPlan(_insPlanFrom,null,null);
		formInsPlan.ShowDialog();
	}
	...
	private void butSave_Click(object sender,EventArgs e) {
		...
		try {
			...
			using var msgBox=new MsgBoxCopyPaste(ex.Message);//No translaion here, because translation was done in the business layer.
			msgBox.ShowDialog();
