...
using CreditCard = Imedisoft.Core.Entities.CreditCard;
using Def = Imedisoft.Core.Entities.Def;
using Patient = Imedisoft.Core.Entities.Patient;
using PayConnectResponseWeb = Imedisoft.Core.Entities.PayConnectResponseWeb;
using Payment = Imedisoft.Core.Entities.Payment;
using PaySplit = Imedisoft.Core.Entities.PaySplit;
...
public partial class FormPayment:FormODBase {
	...
	private List<PaySplit> _listPaySplitsForSecLog= [];
	...
	private List<long> GetListProcCodesFiltered() {
		var listProcCodesFiltered=new List<long>();
		//Proc codes
		var listCodes=textFilterProcCodes.Text.Split(',').ToList();
		for(var i = 0;i<listCodes.Count;i++) {
			var retrievedCode=ProcedureCodes.GetCodeNum(listCodes[i].Trim());  //returns 0 if code not found
			if(retrievedCode!=0) {
	...
	private void FormPayment_Load(object sender,System.EventArgs e) {
		...
		//}
		gridCharges.Bounds=new Rectangle(x: 1,y: 22, width: tabControlCharges.ClientSize.Width-2,height: tabControlCharges.ClientSize.Height-23);
		gridTreatPlan.Bounds=new Rectangle(x: 1,y: 22, width: tabControlCharges.ClientSize.Width-2,height: tabControlCharges.ClientSize.Height-23);
		if(IsNew) {
			...
			var isAccountingTransaction=AccountingAutoPays.GetDeepCopy().Exists(x => x.PayType==_payment.PayType);
		...
		for(var i = 0;i<_listDefsPaymentType.Count;i++) {
		...
		if(IsNew) {
			...
			var transaction=_loadData.Transaction;
			if(transaction==null) {
				textDepositAccount.Visible=false;
			}
			else {
				//add only the description based on PaymentCur attached to transaction
				var listJournalEntries=JournalEntries.GetForTrans(transaction.TransactionNum);
				for(var i = 0;i<listJournalEntries.Count;i++) {
					var account=Accounts.GetAccount(listJournalEntries[i].AccountNum);
		...
		if(_patient.SuperFamily<=0) {
			...
			var listPatientsSuperFamExclusive=_loadData.SuperFam.ListPats.Where(x => !_family.IsInFamily(x.PatNum)).ToList();
		...
		comboClinicOutstandingFilter.IncludeAll=true;
		comboPatientOutstandingFilter.IncludeAll=true;
		comboTypeOutstandingFilter.IncludeAll=true;
		comboClinicsPaySplitsFilter.IncludeAll=true;
		comboPatientPaySplitsFilter.IncludeAll=true;
		var doAutoSplit=false;
		if(IsNew) {
		...
		if(PaySplitNumInitial!=0) {
			...
			var paySplitInit=_listPaySplits.FirstOrDefault(x => x.SplitNum==PaySplitNumInitial);
	...
	private void butAddManualSplit_Click(object sender,EventArgs e) {
		...
		paySplit.IsNew=true;
		using var formPaySplitEdit=new FormPaySplitEdit(GetFamilyOrSuperFamily());
		formPaySplitEdit.ListAccountEntries=_listAccountEntriesCharges;
		...
		if(formPaySplitEdit.ShowDialog()==DialogResult.OK) {
			if(!_dictPatients.ContainsKey(paySplit.PatNum)) {
				...
				var patient=Patients.GetLim(paySplit.PatNum);
				if(patient!=null) {
	...
	private void butEmailReceipt_Click(object sender,EventArgs e) {
		...
		var tempFile=ODFileUtils.CombinePaths(PrefC.GetTempFolderPath(),
			DateTime.Now.ToString("yyyyMMdd")+"_"+DateTime.Now.TimeOfDay.Ticks+random.Next(1000)+".pdf");
		...
		var emailAddress=EmailAddresses.GetByClinic(_patient.ClinicNum);
		...
		var emailAttachRcpt=new EmailAttach {
			DisplayedFileName="Receipt.pdf",
			ActualFileName=Path.GetFileName(tempFile)
		};
		emailMessage.Attachments= [emailAttachRcpt];
	...
	private void butPay_Click(object sender,EventArgs e) {
		if(checkPayTypeNone.Checked) {
			...
			var constructResults=PaymentEdit.ConstructAndLinkChargeCredits(_family.GetPatNums(),_patient.PatNum,_listPaySplits,
				_payment,ListAccountEntriesPayFirst,isIncomeTxfr:true);
			if(PaymentEdit.TryCreateIncomeTransfer(constructResults.ListAccountEntries,DateTime.Now,out var incomeTransferData)) {
				...
				var msgBoxCopyPaste=new MsgBoxCopyPaste(incomeTransferData.StringBuilderErrors.ToString().TrimEnd());
				msgBoxCopyPaste.Show();
			}
			//Display any warning messages to the user.
			if(incomeTransferData.StringBuilderWarnings.Length > 0) {
				var msgBoxCopyPaste=new MsgBoxCopyPaste("The following warnings happened during the income transfer process."
				                                        +"\r\n"+incomeTransferData.StringBuilderWarnings);
				msgBoxCopyPaste.Show();
			}
			return;
		}
		var payAmtOrig=SIn.Double(textAmount.Text);
		...
		if(!CompareDouble.IsZero(payAmtOrig)) {
			var payAmtRemaining=(payAmtOrig-_listPaySplits.Sum(x => x.SplitAmt));
		...
		var payResultsCreatedSplits=PaymentEdit.MakePayment(listListsAccountEntriesGrid,_payment,SIn.Decimal(textAmount.Text),
	...
	private void butPayConnect_Click(object sender,EventArgs e) {
		...
		if(comboCreditCards.SelectedIndex < _listCreditCards.Count) {
			var creditCard=_listCreditCards[comboCreditCards.SelectedIndex];
	...
	private void gridSplits_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		var paySplitOld=(PaySplit)gridSplits.ListGridRows[e.Row].Tag;
		var paySplit=paySplitOld.Copy();
	...
	private void checkPayTypeNone_CheckedChanged(object sender,EventArgs e) {
		...
		if(checkPayTypeNone.Checked) {
			...
			FillFiltersChargesTreatPlan(doPreserveValues: false);
			groupBoxFilteringOutstanding.Enabled=false;
		...
		groupBoxFilteringOutstanding.Enabled=true;
	...
	private void comboGroupBy_SelectionChangeCommitted(object sender,EventArgs e) {
		...
		if(comboGroupBy.SelectedIndex==1) { //Group By Providers
			comboTypeOutstandingFilter.IsAllSelected=true;
			comboClinicOutstandingFilter.IsAllSelected=true;
			comboTypeOutstandingFilter.Enabled=false;
			comboClinicOutstandingFilter.Enabled=false;
			Reinitialize();
			return;
		}
		if(comboGroupBy.SelectedIndex==2) { //Group by providers and clinics
			comboTypeOutstandingFilter.Enabled=false;
			comboClinicOutstandingFilter.Enabled=true;
			Reinitialize();
			return;
		}
		//Not grouping by anything
		comboTypeOutstandingFilter.Enabled=true;
		comboClinicOutstandingFilter.Enabled=true;
		Reinitialize();
	}
	#endregion
	...
	private bool ShowOverridePrompt() {
		if(_payment.IsCcCompleted) {
			var prompt = "Warning: This payment already contains data from a previously successful card transaction.  "
	...
	private void AddIndexToDictForPaySplit(int index,AccountEntry accountEntry,Dictionary<PaySplitHelper,List<int>> dictPaySplitIndices) {
		//Loop through all splits in the collection and make sure that each split is associated to this account entry index.
		var listPaySplitsCollection=accountEntry.SplitCollection.ToList();
		for(var i = 0;i<listPaySplitsCollection.Count;i++) {
			var paySplitHelper=new PaySplitHelper(listPaySplitsCollection[i]);
			if(dictPaySplitIndices.TryGetValue(paySplitHelper,out var listIndices)) {
				if(!listIndices.Contains(index)) {
					dictPaySplitIndices[paySplitHelper].Add(index);
				}
				continue;
			}
			dictPaySplitIndices[paySplitHelper]= [index];
		}
	}
	...
	private bool AddOneSplit(bool doPromptForPayPlan = false) {
		...
		if(doPromptForPayPlan && _loadData.ListValidPayPlans.Count > 0) {
			using var formPayPlanSelect=new FormPayPlanSelect(_loadData.ListValidPayPlans,true);
	...
	private List<PaySplit> GetSuggestedAutoSplits() {
		...
		var listPatNums=GetFamilyOrSuperFamily().ListPats.Select(x => x.PatNum).ToList();
		var constructResults=PaymentEdit.ConstructAndLinkChargeCredits(listPatNums,_patient.PatNum,_listPaySplits,_payment
	...
	private bool CanAddNewCreditCard(Program program,string progPropertyDescription) {
		if(!Programs.IsEnabledByHq(program,out var err)) {
			MsgBox.Show(err);
			return false;
		}
		...
		var hasPreventCcAdd=SIn.Bool(ProgramProperties.GetPropVal(program.ProgramNum,progPropertyDescription,_payment.ClinicNum));
		var creditCardSelected=comboCreditCards.GetSelected<CreditCard>();
	...
	private void CellClickHelper(GridOD grid) {
		gridSplits.SetAll(false);
		var listSelectedCharges=new List<AccountEntry>();
		if(comboGroupBy.SelectedIndex!=0) { //Grouping is active
			var listListsAccountEntries=grid.SelectedTags<List<AccountEntry>>();
			for(var i = 0;i<listListsAccountEntries.Count;i++) {
		...
		//No grouping
		var listAccountEntries=grid.SelectedTags<AccountEntry>();
		for(var i = 0;i<listAccountEntries.Count;i++) {
	...
	private void CheckUIState() {
		...
		var programEdgeExpress=Programs.GetCur(ProgramName.EdgeExpress);
		var programPayConnect=Programs.GetCur(ProgramName.PayConnect);
		var programPaySimple=Programs.GetCur(ProgramName.PaySimple);
		if(_programX==null || programPayConnect==null || programPaySimple==null || programEdgeExpress==null) {//Should not happen.
		...
		if(programPayConnect.Enabled) {
			...
			if(butPayConnect.Visible==true) {
				...
				if(programVersion=="2") {
					...
					var integrationType=ProgramProperties.GetPropVal(programPayConnect.ProgramNum,"PayConnect2.0 Integration Type: 0 for normal, 1 for surcharge",_payment.ClinicNum);
	...
	private void CreateSplit(AccountEntry accountEntryCharge,decimal payAmt,bool isManual = false) {
		var payResultsCreatedSplit=PaymentEdit.CreatePaySplit(accountEntryCharge,payAmt,_payment,SIn.Decimal(textAmount.Text),_listAccountEntriesCharges,
			isManual);
	...
	private void CreatePartialSplitClickHelper(GridOD grid) {
		if(comboGroupBy.Visible && comboGroupBy.SelectedIndex > 0) {
			var listListsAccountEntriesSelected=grid.SelectedTags<List<AccountEntry>>();
			for(var i = 0;i<listListsAccountEntriesSelected.Count;i++) {
				CreatPartialSplitForAccountEntries(listListsAccountEntriesSelected[i].ToArray());
			}
			return;
		}
		var listAccountEntriesSelected=grid.SelectedTags<AccountEntry>();
		for(var i = 0;i<listAccountEntriesSelected.Count;i++) {
	...
	private void CreatPartialSplitForAccountEntries(params AccountEntry[] accountEntryArray) {
		...
		var listAccountEntries=accountEntryArray.ToList();
		for(var i = 0;i<listAccountEntries.Count;i++) {
			var splitCountOld=_listPaySplits.Count;
	...
	private MigraDoc.DocumentObjectModel.Document CreatePDFDoc(string receiptStr) {
		var stringArrayReceiptLines=receiptStr.Split("\r\n",StringSplitOptions.None);
		...
		var fontBodyX=MigraDocHelper.CreateFont(8,false);
	...
	private void DeleteSelected(bool doCreateSecLog = true) {
		...
		var listPaySplits=gridSplits.SelectedTags<PaySplit>();
	...
	private bool DoShowAccountEntry(AccountEntry accountEntryCharge) {
		...
		var listPatNumsFiltered=comboPatientOutstandingFilter.GetListSelected<Patient>().Select(x => x.PatNum).ToList();
		if(!listPatNumsFiltered.Contains(accountEntryCharge.PatNum)) {
			return false;
		}
		var listProvNums=comboProviderOutstandingFilter.GetSelectedProvNums();
		if(!listProvNums.Contains(accountEntryCharge.ProvNum)) {
			return false;
		}
		var listClinicNumsFiltered=comboClinicOutstandingFilter.GetListSelected<ClinicDto>().Select(x => x.Id).ToList();
		if(true && comboGroupBy.SelectedIndex!=1 && !listClinicNumsFiltered.Contains(accountEntryCharge.ClinicNum)) {
			return false;
		}
		//proc code filter
		...
		if(amtMaxEndOutstanding.Value!=0 && accountEntryCharge.AmountEnd > amtMaxEndOutstanding.Value) {
			return false;
		}
		//Charge Amount Filter
		if(amtMinEndOutstanding.Value!=0 && accountEntryCharge.AmountEnd < amtMinEndOutstanding.Value) {
			return false;
		}
		//daterange filter
		...
		//Type Filter
		var listTypeFiltered=comboTypeOutstandingFilter.GetListSelected<string>();
	...
	private bool DoHighlightPaySplit(PaySplit paySplit) {
		if(comboPatientPaySplitsFilter.IsAllSelected==true
		   ...
		   && (!true || comboClinicsPaySplitsFilter.IsAllSelected==true)
		   && amtMinPaySplits.Value==0
		   && amtMaxPaySplits.Value==0)
		{
			return false;
		}
		var listPatNumsFiltered=comboPatientPaySplitsFilter.GetListSelected<Patient>().Select(x => x.PatNum).ToList();
		if(comboPatientPaySplitsFilter.IsAllSelected==false && !listPatNumsFiltered.Contains(paySplit.PatNum)) {
			return false;
		}
		var listProvNums=comboProviderPaySplitsFilter.GetSelectedProvNums();
		if(comboProviderPaySplitsFilter.IsAllSelected==false && !listProvNums.Contains(paySplit.ProvNum)) {
			return false;
		}
		var listClinicNumsFiltered=comboClinicsPaySplitsFilter.GetListSelected<ClinicDto>().Select(x => x.Id).ToList();
		if(comboClinicsPaySplitsFilter.IsAllSelected==false && true && !listClinicNumsFiltered.Contains(paySplit.ClinicNum)) {
			return false;
		}
		//Charge Amount Filter
	...
	private GridRow FillChargesHelper(List<AccountEntry> listAccountEntriesForRow,bool doIncludeClinic) {
		var accountEntryCharge=listAccountEntriesForRow.First();
		var row=new GridRow();
		row.Tag=listAccountEntriesForRow;
		row.Cells.Add(Providers.GetAbbr(accountEntryCharge.ProvNum));//Provider
		if(checkPayTypeNone.Checked) {
			if(!_dictPatients.TryGetValue(accountEntryCharge.PatNum,out var pat)) {
		...
		var procCodeLimit=(doIncludeClinic ? 9 : 10);//this column is shorter when filtering by prov + clinic
	...
	private void FillCreditCards() {
		...
		var isXWebCardPresent=_listCreditCards.Any(x => x.IsXWeb());
		var isPayConnectPortalCardPresent=_listCreditCards.Any(x => x.IsPayConnectPortal());
		_xWebResponse=_loadData.XWebResponse;
		_payConnectResponseWeb=_loadData.PayConnectResponseWeb;
		var _payConnectTransType="";
	...
	private void FillFiltersChargesTreatPlan(bool doPreserveValues) {
		if(_listAccountEntriesCharges==null) {
			return;
		}
		FillFilterFromToDates(doPreserveValues);
		FillFilterMinMaxAmtsChargesTreatPlan(doPreserveValues);
		FillFilterPatientsChargesTreatPlan(doPreserveValues);
		FillFilterProvidersChargesTreatPlan(doPreserveValues);
		FillFilterClinicsChargestreatPlan(doPreserveValues);
		FillFilterTypes(doPreserveValues);
	}

	private void FillFiltersPaySplits(bool doPreserveValues) {
		if(_listPaySplits==null) {
			return;
		}
		FillFilterMinMaxAmtsPaySplits(doPreserveValues);
		FillFilterPatientsPaySplits(doPreserveValues);
		FillFilterProvidersPaySplits(doPreserveValues);
		FillFilterClinicsPaySplits(doPreserveValues);
	}
	...
	private void FillFilterFromToDates(bool doPreserveValues) {
		...
		var listDatesAccountEntry=_listAccountEntriesCharges
	...
	private void FillFilterMinMaxAmtsChargesTreatPlan(bool doPreserveValues) {
		if(doPreserveValues) {
			return;//It's fine to have values set in these amount boxes that fall outside of the range of the current account entries.
		}
		amtMinEndOutstanding.Value=0;
		amtMaxEndOutstanding.Value=0;
	}

	private void FillFilterMinMaxAmtsPaySplits(bool doPreserveValues) {
		if(doPreserveValues) {
			return;//It's fine to have values set in these amount boxes that fall outside of the range of the current account entries.
		}
		amtMinPaySplits.Value=0;
	...
	private void FillFilterPatientsChargesTreatPlan(bool doPreserveValues) {
		var wasAllSelected=comboPatientOutstandingFilter.IsAllSelected;
		var listPatNumsSelected=comboPatientOutstandingFilter.GetListSelected<Patient>().Select(x => x.PatNum).ToList();
		comboPatientOutstandingFilter.Items.Clear();
		//Fill the patient outstanding and treat plan filter combo box with the known patients relating to the list of account charges.
		var listPatNums=_listAccountEntriesCharges.Select(x => x.PatNum).Distinct().ToList();
		for(var i = 0;i<listPatNums.Count;i++) {
			if(_dictPatients.TryGetValue(listPatNums[i],out var pat)) {
				comboPatientOutstandingFilter.Items.Add(pat.GetNameFirstOrPreferred(),pat);
				if(doPreserveValues && listPatNumsSelected.Contains(pat.PatNum)) {
					comboPatientOutstandingFilter.SetSelected(comboPatientOutstandingFilter.Items.Count-1);
				}
			}
		}
		if(wasAllSelected || !doPreserveValues) {
			comboPatientOutstandingFilter.IsAllSelected=true;
		}
	}

	private void FillFilterPatientsPaySplits(bool doPreserveValues) {
		var wasAllSelected=comboPatientPaySplitsFilter.IsAllSelected;
		var listPatNumsSelected=comboPatientPaySplitsFilter.GetListSelected<Patient>().Select(x => x.PatNum).ToList();
		comboPatientPaySplitsFilter.Items.Clear();
		//Fill the patient pay splits filter combo box with the known patients relating to the list of account charges.
		var listPatNums=_listPaySplits.Select(x => x.PatNum).Distinct().ToList();
		for(var i=0;i<listPatNums.Count;i++) {
			if(_dictPatients.TryGetValue(listPatNums[i],out var pat)) {
				comboPatientPaySplitsFilter.Items.Add(pat.GetNameFirstOrPreferred(),pat);
				if(doPreserveValues && listPatNumsSelected.Contains(pat.PatNum)) {
					comboPatientPaySplitsFilter.SetSelected(comboPatientPaySplitsFilter.Items.Count-1);
				}
			}
		}
		if(wasAllSelected || !doPreserveValues) {
			comboPatientPaySplitsFilter.IsAllSelected=true;
		}
	}

	private void FillFilterProvidersChargesTreatPlan(bool doPreserveValues) {
		var wasAllSelected=comboPatientOutstandingFilter.IsAllSelected;
		var listProvNumsSelected=comboProviderOutstandingFilter.GetSelectedProvNums();
		...
		if(wasAllSelected || !doPreserveValues) {
			comboProviderOutstandingFilter.IsAllSelected=true;
		}
	}

	private void FillFilterProvidersPaySplits(bool doPreserveValues) {
		var wasAllSelected=comboPatientPaySplitsFilter.IsAllSelected;
		var listProvNumSelected=comboProviderPaySplitsFilter.GetSelectedProvNums();
	...
	private void FillFilterClinicsChargestreatPlan(bool doPreserveValues) {
		...
		if(!_listClinics.IsNullOrEmpty() && !_listClinics.Any(x => x.Abbr==Lan.g(this,"Unassigned"))) {
			_listClinics.Add(new ClinicDto { Abbr=Lan.g(this,"Unassigned") });
		}
		comboClinicOutstandingFilter.Items.Clear();
		var listClinicNums=_listAccountEntriesCharges.Select(x => x.ClinicNum).Distinct().ToList();
		var listClinics=_listClinics.FindAll(x => listClinicNums.Contains(x.Id));
		for(var i = 0;i<listClinics.Count;i++) {
			comboClinicOutstandingFilter.Items.Add(listClinics[i].Abbr,listClinics[i]);
			if(doPreserveValues && listClinicNumsSelected.Contains(listClinics[i].Id)) {
				comboClinicOutstandingFilter.SetSelected(comboClinicOutstandingFilter.Items.Count-1);
			}
		}
		if(wasAllSelected || !doPreserveValues) {
			comboClinicOutstandingFilter.IsAllSelected=true;
	...
	private void FillFilterClinicsPaySplits(bool doPreserveValues) {
		...
		comboClinicsPaySplitsFilter.Items.Clear();
		var listClinicNums=_listPaySplits.Select(x => x.ClinicNum).Distinct().ToList();
		var listClinics=_listClinics.FindAll(x => listClinicNums.Contains(x.Id));
		for(var i=0;i<listClinics.Count;i++) {
			comboClinicsPaySplitsFilter.Items.Add(listClinics[i].Abbr,listClinics[i]);
			if(doPreserveValues && listClinicNumsSelected.Contains(listClinics[i].Id)) {
				comboClinicsPaySplitsFilter.SetSelected(comboClinicsPaySplitsFilter.Items.Count-1);
			}
		}
		if(wasAllSelected || !doPreserveValues) {
	...
	private void FillFilterTypes(bool doPreserveValues) {
		var wasAllSelected=comboTypeOutstandingFilter.IsAllSelected;
		var listTypesSelected=comboTypeOutstandingFilter.GetListSelected<string>();
		comboTypeOutstandingFilter.Items.Clear();
		comboTypeOutstandingFilter.Items.AddList(_listAccountEntriesCharges.Select(x => x.GetType().Name).Distinct().ToList(),x => GetDisplayStringForType(x));
		if(!wasAllSelected && doPreserveValues) {
			//Reselect providers that were selected before refilling the combo box.
			for(var i = 0;i<=comboTypeOutstandingFilter.Items.Count;i++) {
				if(comboTypeOutstandingFilter.Items.GetObjectAt(i) is string type && listTypesSelected.Contains(type)) {
					comboTypeOutstandingFilter.SetSelected(i);
				}
			}
		}
		if(wasAllSelected || !doPreserveValues) {
	...
	private void FillGridCharges() {
		...
		var listAccountEntriesOutstandingCharges=_listAccountEntriesCharges.FindAll(x => DoShowAccountEntry(x)
		...
		if(comboGroupBy.SelectedIndex==1) {//Group by 'Provider'
			...
			var listAccountEntriesFiltered=listAccountEntriesOutstandingCharges.FindAll(x => (checkShowAll.Checked || !CompareDecimal.IsZero(x.AmountEnd)));
			var listProvPatClinics=new List<ProvPatClinic>();
			for(var i = 0;i<listAccountEntriesFiltered.Count;i++) {
				var provPatClinic=new ProvPatClinic();
			...
			for(var i = 0;i<listProvPatClinics.Count;i++) {
				var listAccountEntriesProvPat=listAccountEntriesFiltered.FindAll(x => x.PatNum==listProvPatClinics[i].PatNum && x.ProvNum==listProvPatClinics[i].ProvNum);
				for(var j = 0;j<listAccountEntriesProvPat.Count;j++) {
		...
		if(comboGroupBy.SelectedIndex==2) {//Group by 'Clinic and Provider'
			...
			for(var i = 0;i<listProvPatClinics.Count;i++) {
				var listAccountEntriesPPC=listAccountEntriesFiltered.FindAll(x => x.PatNum==listProvPatClinics[i].PatNum && x.ProvNum==listProvPatClinics[i].ProvNum && x.ClinicNum==listProvPatClinics[i].ClinicNum);
				for(var j = 0;j<listAccountEntriesPPC.Count;j++) {
		...
		gridCharges.ListGridRows.Clear();
		GridRow row;
		for(var i = 0;i<listAccountEntriesOutstandingCharges.Count;i++) {
			...
			if(!checkShowAll.Checked && CompareDecimal.IsZero(listAccountEntriesOutstandingCharges[i].AmountEnd)) {
				var doShowZeroCharge=false;
				var listPaySplits=gridSplits.GetTags<PaySplit>();
				for(var j = 0;j<listPaySplits.Count;j++) {
			...
			if(!_dictPatients.TryGetValue(listAccountEntriesOutstandingCharges[i].PatNum,out var patCur)) {
				patCur=Patients.GetLim(listAccountEntriesOutstandingCharges[i].PatNum);
				_dictPatients[patCur.PatNum]=patCur;
			}
			var patName=patCur.GetNameLFnoPref();
			if(listAccountEntriesOutstandingCharges[i].Tag.GetType()==typeof(FauxAccountEntry)
			   ...
			   && ((FauxAccountEntry)listAccountEntriesOutstandingCharges[i]).Guarantor!=patCur.PatNum) {
				if(!_dictPatients.TryGetValue(((FauxAccountEntry)listAccountEntriesOutstandingCharges[i]).Guarantor,out var guarantor)) {
	...
	private void FillGridSplits() {
		...
		var listProcsNumsMissing=_listPaySplits.Where(x => x.ProcNum!=0 && !_loadData.ListProcsForSplits.Any(y => y.ProcNum==x.ProcNum))
		...
		var listPaySplits=_listPaySplits.ToList();
		var listPaySplitsFiltered=new List<PaySplit>();
		for(var i=0;i<listPaySplits.Count;i++) {
			splitTotal+=(decimal)listPaySplits[i].SplitAmt;
			if(!DoHighlightPaySplit(listPaySplits[i])) {
				continue;
			}
		...
		for(var i=0;i<listPaySplitsFiltered.Count;i++) {
			row=new GridRow();
	...
	private GridRow FillGridsSplitsHelper(PaySplit paySplit) {
		var paySplitHelper=new PaySplitHelper(paySplit);
		var index=gridSplits.ListGridRows.Count;
		var row=new GridRow();
		row.Tag=paySplit;
		_dictGridSplitsPaySplitIndices[paySplitHelper]= [index];
		row.Cells.Add(Providers.GetAbbr(paySplit.ProvNum));//Prov
		if(true) {//Clinics
			if(paySplit.ClinicNum!=0) {
				row.Cells.Add(Clinics.GetAbbr(paySplit.ClinicNum));//Clinic
			}
			else {
		...
		Patient patient;
		if(!_dictPatients.TryGetValue(paySplit.PatNum,out patient)) {
			patient=Patients.GetLim(paySplit.PatNum);
			_dictPatients[patient.PatNum]=patient;
		}
		var patName=patient.LName + ", " + patient.FName;
		row.Cells.Add(patName);//Patient
		var procedure=new Procedure();
		if(paySplit.ProcNum!=0) {
			procedure=_loadData.ListProcsForSplits.FirstOrDefault(x => x.ProcNum==paySplit.ProcNum)??new Procedure();
		}
		row.Cells.Add(ProcedureCodes.GetStringProcCode(procedure.CodeNum));//ProcCode
		var isUnallocated=false;
		var listTypeStrs=new List<string>();
		if(paySplit.PayPlanNum > 0) {
			listTypeStrs.Add("PayPlanCharge");
		}
		if(paySplit.ProcNum > 0) {//Procedure
			listTypeStrs.Add("Proc: "+Procedures.GetDescription(procedure));
		}
		else if(paySplit.PayPlanChargeNum > 0) {//Might be payment plan interest when not a procedure.
			...
			var isLegacyInterest=(paySplit.PayPlanDebitType==PayPlanDebitTypes.Unknown
			                      && _listAccountEntriesCharges.Where(x => x.GetType()==typeof(FauxAccountEntry) && x.PayPlanChargeNum==paySplit.PayPlanChargeNum)
				                      .Cast<FauxAccountEntry>()
				                      .Any(x => !CompareDecimal.IsZero(x.Interest) && CompareDecimal.IsEqual(x.AmountAvailable,(decimal)paySplit.SplitAmt)));
			if(paySplit.PayPlanDebitType==PayPlanDebitTypes.Interest || isLegacyInterest) {
				listTypeStrs.Add("(interest)");
			}
		}
		if(paySplit.AdjNum > 0) {
			listTypeStrs.Add("Adjustment");
		}
		if(paySplit.UnearnedType > 0) {
			listTypeStrs.Add(Defs.GetName(DefCat.PaySplitUnearnedType,paySplit.UnearnedType));
		}
		...
		if(!_family.GetPatNums().Contains(paySplit.PatNum)) {
			listTypeStrs.Add("(split to another family)");
		}
		row.Cells.Add(paySplit.SplitAmt.ToString("f"));//Amount
		return row;
	}
	...
	private void FillGridTreatPlan() {
		...
		GridRow row;
		for(var i = 0;i<_listAccountEntriesCharges.Count;i++) {
			...
			if(!_dictPatients.TryGetValue(_listAccountEntriesCharges[i].PatNum,out var patCur)) {
			...
			if(_listAccountEntriesCharges[i].Tag.GetType()==typeof(FauxAccountEntry)) {
				if(!_dictPatients.TryGetValue(((FauxAccountEntry)_listAccountEntriesCharges[i]).Guarantor,out var guarantor)) {
	...
	private void FilterChangeCommittedChargesTreatPlan(object sender,EventArgs e) {
		if(_isInit) {
			return;
		}
		FillGridCharges();
	...
	private void FilterChangeCommittedPaySplits(object sender, EventArgs e) {
		if(_isInit) {
			return;
		}
		FillGridSplits();
	...
	private List<List<AccountEntry>> GetAccountEntriesFromGrid(GridOD grid) {
		var listListsAccountEntriesSelectedCharges=new List<List<AccountEntry>>();
		var hasSelectedIndices=(grid.SelectedIndices.Length > 0);
		...
		if(comboGroupBy.SelectedIndex > 0 && tabControlCharges.SelectedTab==tabPageOutstanding) {
			...
			var listAccountEntries= grid.SelectedTags<AccountEntry>();
			for(var i = 0;i<listAccountEntries.Count;i++) {
				listListsAccountEntriesSelectedCharges.Add([listAccountEntries[i]]);
			}
		}
		if(!hasSelectedIndices) {
	...
	private string GetCodesDescriptForEntries(int procCodeLimit,params AccountEntry[] accountEntryArray) {
		...
		var listAccountEntries=accountEntryArray.Where(x => x.GetType()==typeof(Procedure)).ToList();
		for(var i = 0;i<listAccountEntries.Count;i++) {
	...
	private string GetXChargeTransactionTypeCommands(int tranType,bool hasXToken,bool isNotRecurring,CreditCard creditCard,string cashBack) {
		...
		switch(tranType) {
			...
			case 6:
				...
				var adjustTransactionID="";
				var stringArrayNoteSplit=Regex.Split(textNote.Text,"\r\n");
				var listNoteSplits=stringArrayNoteSplit.ToList();
				for(var i = 0;i<listNoteSplits.Count;i++) {
	...
	private void HandleVoidPayment(string payNote,double approvedAmt,string receipt,CreditCardSource creditCardSource,bool isVoidingRefund=false) {
		...
		if(!IsNew || _wasCreditCardSuccessful) {//Create a new negative payment if the void is being run from an existing payment
			...
			var strErrorMsg=Ledgers.ComputeAgingForPaysplitsAllocatedToDiffPats(_patient.PatNum,_listPaySplits);
	...
	private void HandleVoidPaymentForFormClosing(string payNote,string receipt,bool showApprovedAmtNotice,double approvedAmt,
		...
		var paymentVoid=Payments.InsertVoidPayment(_payment,_listPaySplits,receipt,payNote,creditCardSource);
	...
	private bool HasPayConnect() {
		...
		if(program.Enabled) {
			...
			var payconnectVersion=ProgramProperties.GetPropVal(program.ProgramNum,"Program Version",_payment.ClinicNum);
			if(payconnectVersion=="1") {
				...
				var password=CDT.Class1.TryDecrypt(ProgramProperties.GetPropVal(program.ProgramNum,"Password",_payment.ClinicNum));
				...
				var apiSecret=ProgramProperties.GetPropVal(program.ProgramNum,"API Secret",_payment.ClinicNum);
				if(apiSecret.IsNullOrEmpty()) {
					isSetupRequired=true;
				}
			}
			var paymentType=ProgramProperties.GetPropVal(program.ProgramNum,"PaymentType",_payment.ClinicNum);
	...
	private bool HasPaySimple() {
		_listDefsPaymentType=_listDefsPaymentType??Defs.GetDefsForCategory(DefCat.PaymentTypes,true);
		var program=Programs.GetCur(ProgramName.PaySimple);
		var isSetupRequired=false;
		if(program.Enabled) {
			...
			var paymentType=ProgramProperties.GetPropValForClinicOrDefault(program.ProgramNum,PaySimple.PropertyDescs.PaySimplePayTypeCC,_payment.ClinicNum);
	...
	private bool HasEdgeExpress() {
		_listDefsPaymentType=_listDefsPaymentType??Defs.GetDefsForCategory(DefCat.PaymentTypes,true);
		var program=Programs.GetCur(ProgramName.EdgeExpress);
		if(!program.Enabled) {
			ODException.SwallowAnyException(() =>
				Process.Start("http://www.opendental.com/resources/redirects/redirectopenedge.html"));
			return false;
	...
	private void HighlightChargesForSplits() {
		...
		var listPaySplits=gridSplits.SelectedTags<PaySplit>();
		for(var i = 0;i<listPaySplits.Count;i++) {
			var paySplitHelper=new PaySplitHelper(listPaySplits[i]);
			if(gridCharges.AllowSelection && _dictGridChargesPaySplitIndices.TryGetValue(paySplitHelper,out var listChargeIndices)) {
				for(var j = 0;j<listChargeIndices.Count;j++) {
					gridCharges.SetSelected(listChargeIndices[j],true);
				}
			}
			if(_dictGridTreatPlanPaySplitIndices.TryGetValue(paySplitHelper,out var listTreatPlanIndices)) {
				for(var j = 0;j<listTreatPlanIndices.Count;j++) {
	...
	private void Init(bool doAutoSplit = false,bool doSelectAllSplits = false,bool doPayFirstAcctEntries = false,bool doPreserveValues = false) {
		_isInit=true;
		var listAccountEntriesPayFirst=new List<AccountEntry>();
		if(doPayFirstAcctEntries && ListAccountEntriesPayFirst!=null) {
			listAccountEntriesPayFirst=ListAccountEntriesPayFirst;
		}
		var doShowExplicitCreditsOnly=checkIncludeExplicitCreditsOnly.Checked && checkIncludeExplicitCreditsOnly.Enabled;
		var initData=PaymentEdit.Init(_loadData,listAccountEntriesPayFirst,_dictPatients,checkPayTypeNone.Checked,_doPreferCurrentPat,
		...
		FillFiltersChargesTreatPlan(doPreserveValues);
		FillFiltersPaySplits(doPreserveValues);
		FillGridSplits();
		FillGridCharges();
		FillGridTreatPlan();
	...
	private string MakeEdgeExpressTransactionRCM(EdgeExpressTransType edgeExpressTransType,double amt,bool doCreateToken,
		...
		try {
			...
			FriendlyException.Show(Lans.g("A payment was initiated but no response was received. The payment may "
		...
		var approvedAmt=SIn.Double(rcmResponse.APPROVEDAMOUNT);
		...
		var receipt=rcmResponse.RECEIPTTEXT;
		...
		if(edgeExpressTransType==EdgeExpressTransType.CreditReturn && CompareDouble.IsGreaterThan(approvedAmt,0)) {
			...
			var isVoidingRefund=SIn.Double(textAmount.Text)>0;
		...
		if(creditCard!=null && !string.IsNullOrEmpty(creditCard.XChargeToken) && creditCard.CCExpiration!=null) {
			...
			var listCreditCards=CreditCards.Refresh(_patient.PatNum);
	...
	private string MakeEdgeExpressTransactionCNP(EdgeExpressTransType edgeExpressTransType,double amt,bool doCreateToken,
		...
		switch(edgeExpressTransType) {
			case EdgeExpressTransType.CreditSale:
				...
				using(var formWebBrowser = new FormWebBrowser(xWebResponse.HpfUrl)) {//Braces required within switch statements.
	...
	private void PayConnectReturn() {
		var programPayConnect=Programs.GetCur(ProgramName.PayConnect);
		var payconnectVersion=ProgramProperties.GetPropVal(programPayConnect.ProgramNum,"Program Version",_payment.ClinicNum);
		var refNum=_payConnectResponseWeb.RefNumber;
	...
	private void PayConnectVoid() {
		var refNum=_payConnectResponseWeb.RefNumber;
		var amountCharged=SIn.Double(textAmount.Text);
		if(amountCharged>0) {
			amountCharged*=-1;
		}
		if(refNum.IsNullOrEmpty()) {
	...
	private void Reinitialize(bool doRefreshConstructData = false,bool doSelectAllSplits = false) {
		var listPatNumsFamily=_family.GetPatNums();
	...
	private bool CheckDynamicPaymentPlanRebalance() {
		var listPayPlanNums=_listPaySplits.Where(x => x.PayPlanNum>0).Select(x => x.PayPlanNum).ToList();
		if(listPayPlanNums.IsNullOrEmpty()) {
			return true;
		}
		var listPayPlanNumsDynamic=PayPlans.GetMany(listPayPlanNums.ToArray()).Where(x => x.IsDynamic).Select(x => x.PayPlanNum).ToList();
		var listPaySplitsPayPlanDynamic=_listPaySplits.Where(x => x.PayPlanNum.In(listPayPlanNumsDynamic.ToArray())).ToList();
		if(!PayPlanEdit.AreAnyPayPlansOverpaid(listPaySplitsPayPlanDynamic)) {
			return true;
		}
		var dialogResult=ODMessageBox.Show(Lan.g(this,"One or more Current Payment Splits are overpaying interest or principal for payment plan charges."
		...
		var isPayOnPrincipal=(dialogResult==DialogResult.Yes);
		var listPayPlanRecalculationDatas=PayPlanEdit.GetRecalculationDataForDynamicPaymentPlans(_patient,listPaySplitsPayPlanDynamic);
	...
	private bool SavePaymentToDb() {
		...
		if(checkPayTypeNone.Checked) {
			...
			var amt=SIn.Double(textAmount.Text);
		...
		var unearnedCur=_listPaySplits.FindAll(x => x.UnearnedType>0 && !listHiddenUnearnedTypes.Contains(x.UnearnedType)).Sum(x => x.SplitAmt);
		var unearnedTotal=(double)PaySplits.GetTotalAmountOfUnearnedForPats(_family.GetPatNums(),payNumExcluded:_payment.PayNum);
		...
		if((SIn.Double(textAmount.Text)==0 && listPayType.SelectedIndex==-1) || checkPayTypeNone.Checked) {
			...
			paymentTypeParam="";
			var dictProcNegSplits=_listPaySplits.FindAll(x => x.ProcNum > 0)
				.GroupBy(x => x.ProcNum)
				.ToDictionary(x => x.Key,x => x.ToList());
			var listWarnForProcNums=new List<long>();
			foreach(var procNum in dictProcNegSplits.Keys) {
				...
				var accountEntryProc=_listAccountEntriesCharges.FirstOrDefault(x => x.GetType()==typeof(Procedure) && x.ProcNum==procNum);
		...
		var isAccountingSynchRequired=false;
		var accountingOldAmt=_paymentOld.PayAmt;
		...
		try {
			isAccountingSynchRequired=Payments.ValidateLinkedEntries(accountingOldAmt,SIn.Double(textAmount.Text),
		...
		if(IsNew && checkRecurring.Checked && !RecurringCharges.HasRecurringChargesForPayment(_payment.PayNum)) {
			//This payment has been applied to a recurring charge, so create a recurringcharge entry if needed. Every payment must have a corresponding recurringcharge
			//since the filtering logic in CreditCards.GetRecurringChargeList expects every recurring payment to be associated to a creditcard via recurringcharge.
			var familyBalance=_family.Guarantor.BalTotal - _family.Guarantor.InsEst;
			var creditCard=comboCreditCards.GetSelected<CreditCard>();
			double payPlanBalance=0;
			if(creditCard.PayPlanNum > 0) {
				payPlanBalance=PayPlans.GetBalance(creditCard.PayPlanNum);
			}
			var recurringCharge=new RecurringCharge();
			recurringCharge.PatNum=_payment.PatNum;
			recurringCharge.ClinicNum=_payment.ClinicNum;
			recurringCharge.DateTimeCharge=_payment.RecurringChargeDate;
			recurringCharge.ChargeStatus=RecurringChargeStatus.ChargeSuccessful;
			recurringCharge.FamBal=familyBalance;
			recurringCharge.PayPlanDue=payPlanBalance;
			recurringCharge.TotalDue=familyBalance+payPlanBalance;
			recurringCharge.RepeatAmt=creditCard.ChargeAmt;
			recurringCharge.ChargeAmt=_payment.PayAmt;
			recurringCharge.UserNum=Security.CurUser.UserNum;
			recurringCharge.PayNum=_payment.PayNum;
			recurringCharge.CreditCardNum=creditCard.CreditCardNum;
			recurringCharge.ErrorMsg="This recurring charge was created manually from the Payment window.";
			RecurringCharges.Insert(recurringCharge);
		}
		try {
		...
		var hasChanged=PaySplits.Sync(_listPaySplits,_payment.PayNum);
		for(var i = 0;i<_listPaySplitsForSecLog.Count;i++) {
		...
		if(isAccountingSynchRequired && !checkPayTypeNone.Checked) {
			Payments.AlterLinkedEntries(accountingOldAmt,_payment.PayAmt,_payment.PayNum,accountingNewAcct,_payment.PayDate,
		...
		if(hasChanged) {
			var strErrorMsg=Ledgers.ComputeAgingForPaysplitsAllocatedToDiffPats(_patient.PatNum,_listPaySplits.Union(_listPaySplitsOld).ToList());
			if(!string.IsNullOrEmpty(strErrorMsg)) {
				ODMessageBox.Show(strErrorMsg);
			}
		}
	...
	private void SelectPaySplit(PaySplit paySplit) {
		...
		var paySplitHelper=new PaySplitHelper(paySplit);
		if(_dictGridSplitsPaySplitIndices.TryGetValue(paySplitHelper,out var listSplitIndices)) {
			for(var i = 0;i<listSplitIndices.Count;i++) {
	...
	private void SelectPaySplitsForAccountEntries(List<AccountEntry> listAccountEntries) {
		...
		var listPayPlanChargeNums=listAccountEntries.Where(x => x.PayPlanChargeNum > 0).Select(x => x.PayPlanChargeNum).ToList();
		if(listPayPlanChargeNums.Count > 0) {
			var listPayPlanChargeEntries=_listAccountEntriesCharges.FindAll(x => listPayPlanChargeNums.Contains(x.PayPlanChargeNum));
			listAccountEntries.AddRange(listPayPlanChargeEntries.Except(listAccountEntries));
		}
		for(var i = 0;i<listAccountEntries.Count;i++) {
			var listPaySplitsCollections=listAccountEntries[i].SplitCollection.ToList();
			for(var j = 0;j<listPaySplitsCollections.Count;j++) {
	...
	private void SetComboDepositAccounts() {
		...
		var accountingAutoPay=AccountingAutoPays.GetForPayType(
	...
	private void ToggleShowHideSplits() {
		if(panelSplits.Visible) {
			...
			Height =251+100;//Plus 100 to give room for the buttons
	...
	private void UpdateChargeTotalWithSelectedEntries() {
		...
		var listGridRowsSelected=grid.SelectedGridRows;
	...
	private void VoidEdgeExpressTransaction(string transactionId) {
		...
		if(EdgeExpress.RCM.IsRCMRunning) {
			...
			if(rcmResponse is null) {
				...
				var approvedAmt=SIn.Double(rcmResponse.APPROVEDAMOUNT);
				var showApprovedAmtNotice=false;
		...
		if(!hasVoided) {
			...
			try {
				...
				MsgBox.Show(Lans.g("There was a problem voiding the transaction.  Please try again or attempt a return instead."));
				return;
			}
	...
	private void VoidPayConnectTransaction(string refNum,string amount) {
		...
		if(_creditCardRequestPayConnect==null || payconnectVersion=="2") {//The payment was made through the terminal or by PayConnect v2.
			...
			progressOD.ActionMain=() => {
				if(payconnectVersion=="1") {
					var posRequest=PosRequest.CreateVoidByReference(refNum);
					var posResponse=DpsPos.ProcessCreditCard(posRequest);
					payConnectResponse=PayConnectTerminal.ToPayConnectResponse(posResponse);				
				}
				else if(payconnectVersion=="2") {
					var voidReferenceIDRequest=new VoidReferenceIDRequest();
			...
			var transResponse=PayConnect.ProcessCreditCard(_creditCardRequestPayConnect,_payment.ClinicNum,x => ODMessageBox.Show(x));
		...
		var payNote=Lan.g(this,"Transaction Type")+": "+Enum.GetName(typeof(PayConnectService.transType),PayConnectService.transType.VOID)
		...
		var paymentVoid=Payments.InsertVoidPayment(_payment,_listPaySplits,receiptStr,payNote,CreditCardSource.PayConnect,payAmt:amountCharged);
	...
	private void VoidPaySimpleTransaction(string refNum,string originalReceipt) {
		...
		var stringArrayReceiptFields=originalReceipt.Replace("\r\n","\n").Replace("\r","\n").Split("\n",StringSplitOptions.RemoveEmptyEntries);
		var ccNum="";
		var expDateStr="";
		var nameOnCard="";
		var listReceiptFields=stringArrayReceiptFields.ToList();
		for(var i = 0;i<listReceiptFields.Count;i++) {
		...
		Cursor=Cursors.Default;
		var paymentVoid=Payments.InsertVoidPayment(_payment,_listPaySplits,receiptStr,apiResponse.ToNoteString(),CreditCardSource.PaySimple);
		SecurityLogs.MakeLogEntry(EnumPermType.PaymentCreate,paymentVoid.PatNum,
			Patients.GetLim(paymentVoid.PatNum).GetNameLF()+", "+paymentVoid.PayAmt.ToString("c"));
	}
	...
	private void VoidXChargeTransaction(string transID,string amount,bool isDebit) {
		var processStartInfo=new ProcessStartInfo(_programX.Path);
		...
		processStartInfo.Arguments+="/RECEIPT:Pat"+_payment.PatNum+" ";//aka invoice#
	...
	private void XWebVoid() {
		...
		var amount=_xWebResponse.Amount;
	...
	public string MakePayConnectTransaction(double prepaidAmt = 0) {
		...
		var amount=Math.Abs(SIn.Decimal(textAmount.Text));//PayConnect always wants a positive number even for voids and returns.
		if(prepaidAmt==0) {
		...
		var cardNumber="";
		var receipt="";
		var amountChargedString="";
		var wasPaymentAttempted=false;
		PayConnectResponse payConnectResponse=null;
		var transType=PayConnectService.transType.SALE;
		if(payconnectVersion=="1") {
			using var formPayConnect=new FormPayConnect(_payment.ClinicNum,_patient,amount,creditCard);
			...
			using var formPayConnect2=new FormPayConnect2(_payment.ClinicNum,_patient,creditCard,amount);
			...
			var statusResponse=PayConnect2.GetTransactionStatus(_payment.ClinicNum,payConnectResponse.RefNumber);
			...
			var cardPaymentMethod=statusResponse.GetStatusResponse.PaymentMethod.CardPaymentMethod;
			...
			var integrationType=ProgramProperties.GetPropVal(program.ProgramNum,"PayConnect2.0 Integration Type: 0 for normal, 1 for surcharge",_payment.ClinicNum);
		...
		if(prepaidAmt==0 && payConnectResponse!=null) {//Regular credit cards (not prepaid cards).
			...
			var paytype=ProgramProperties.GetPropVal(program.ProgramNum,"PaymentType",_payment.ClinicNum);//paytype could be an empty string
		...
		var amountCharged=(double)amount;
		...
		var surchargeFee=_payment.MerchantFee;
		...
		if(payConnectResponse!=null) {
			...
			if(payconnectVersion=="2") {
				var integrationType=PayConnect2.GetIntegrationType(_payment.ClinicNum);
	...
	public string MakePaySimpleTransaction(double prepaidAmt = 0,string carrierName = "") {
		...
		CreditCard creditCard=null;
		List<CreditCard> listCreditCards=null;
		var amount=Math.Abs(SIn.Decimal(textAmount.Text));//PaySimple always wants a positive number even for voids and returns.
		if(prepaidAmt==0) {
			listCreditCards=CreditCards.Refresh(_patient.PatNum);
			if(comboCreditCards.SelectedIndex < listCreditCards.Count) {
				creditCard=listCreditCards[comboCreditCards.SelectedIndex];
		...
		using var formPaySimple=new FormPaySimple(_payment.ClinicNum,_patient,amount,creditCard,carrierName:carrierName);
		...
		if(prepaidAmt==0) {//Regular credit cards (not prepaid cards).
			...
			listCreditCards=CreditCards.Refresh(_patient.PatNum);
			var paySimpleToken=creditCard==null ? "" : creditCard.PaySimpleToken;
			...
			var paytype=ProgramProperties.GetPropValForClinicOrDefault(program.ProgramNum,PaySimple.PropertyDescs.PaySimplePayTypeCC,_payment.ClinicNum);
			if(!PrefC.GetBool(PrefName.PaymentsPromptForPayType)) {
				listPayType.SelectedIndex=Defs.GetOrder(DefCat.PaymentTypes,SIn.Long(paytype));
			}
			SetComboDepositAccounts();
		...
		if(formPaySimple.ApiResponseOut!=null) { //The transaction succeeded.
			...
			if(formPaySimple.ApiResponseOut.CCSource==CreditCardSource.PaySimpleACH) {
				var paytype=ProgramProperties.GetPropValForClinicOrDefault(program.ProgramNum,PaySimple.PropertyDescs.PaySimplePayTypeACH,
					_payment.ClinicNum);
				_payment.PaymentStatus=PaymentStatus.PaySimpleAchPosted;
				_payment.ExternalId=formPaySimple.ApiResponseOut.RefNumber;
				var defOrder=Defs.GetOrder(DefCat.PaymentTypes,SIn.Long(paytype));
	...
	public string MakeXChargeTransaction(double prepaidAmt = 0) {
		...
		var doNeedToken=false;
		var isNewCard=false;
		var hasXToken=false;
		var isNotRecurring=false;
		if(prepaidAmt==0) {
			//These UI changes only need to happen for regular credit cards when the payment window is displayed.
			var xPayTypeNum=ProgramProperties.GetPropVal(_programX.ProgramNum,"PaymentType",_payment.ClinicNum);
			//still need to add functionality for accountingAutoPay
			if(!PrefC.GetBool(PrefName.PaymentsPromptForPayType)) {
		...
		var processStartInfo=new ProcessStartInfo(_xPath);
		...
		using var formXchargeTrans=new FormXchargeTrans();
		var tranType=0;//Default to 0 "Purchase" for prepaid cards.
		...
		if(prepaidAmt==0) {//All regular cards (not prepaid)
			...
			_payment.ProcessStatus=ProcessStat.OfficeProcessed;
			tranType=formXchargeTrans.IdxTransactionType;
			var cashAmt=formXchargeTrans.CashBackAmount;
			cashBack=cashAmt.ToString("F2");
			_doPromptSignature=formXchargeTrans.IsPromptSignatureChecked;
			_doPrintReceipt=formXchargeTrans.IsPrintReceiptChecked;
		}
		...
		var doUpdateCard=false;
		var newAccount="";
		var dateNewExpiration=new DateTime();
		...
		*/
		while(line!=null) {
			...
			if(line.StartsWith("EXPIRATION=")) {
				var expStr=line.Substring("EXPIRATION=".Length);//Expiration should be MMYY
		...
		if(doNeedToken && !string.IsNullOrEmpty(xChargeToken) && prepaidAmt==0) {//never save token for prepaid cards
			...
			var dateExp=new DateTime(SIn.Int("20"+StringTools.TruncateBeginning(expiration,2)),SIn.Int(StringTools.Truncate(expiration,2)),1);
		...
		if(isNewCard && prepaidAmt==0) {//never save card information to the patient account for prepaid cards
			if(!string.IsNullOrEmpty(xChargeToken) && formXchargeTrans.IsSaveTokenChecked) {
	...
	public string MakeEdgeExpressTransaction(double prepaidAmt = 0,long clinicNum = -1) {
		...
		if(!ValidateForCreditCardPayment(prepaidAmt!=0,out var cc)) {
			return null;
		}
		using var formEdgeExpressTrans=new FormEdgeExpressTrans();
		...
		if(prepaidAmt==0) {
			var program=Programs.GetCur(ProgramName.EdgeExpress);
			var payType=ProgramProperties.GetPropVal(program.ProgramNum,ProgramProperties.PropertyDescs.EdgeExpress.PaymentType,_payment.ClinicNum);//payType could be an empty string
			if(!PrefC.GetBool(PrefName.PaymentsPromptForPayType)) {
				listPayType.SelectedIndex=Defs.GetOrder(DefCat.PaymentTypes,SIn.Long(payType));
			}
		}
		_payment.ProcessStatus=ProcessStat.OfficeProcessed;
		var edgeExpressTransType=formEdgeExpressTrans.EdgeExpressTransTypeCur;
		_doPromptSignature=formEdgeExpressTrans.DoPromptSignature;
		_doPrintReceipt=formEdgeExpressTrans.DoPrintReceipt;
		var aliasToken=cc?.XChargeToken;
		var doCreateToken=formEdgeExpressTrans.DoSaveToken && aliasToken.IsNullOrEmpty() && prepaidAmt==0;
		var amt=SIn.Double(textAmount.Text);
		if(prepaidAmt!=0) {
			amt=prepaidAmt;
		}
		var transactionId=formEdgeExpressTrans.TransactionId;
		var cashBackAmt=formEdgeExpressTrans.AmtCashBack;
		var edgeExpressApiTypeSelection=formEdgeExpressTrans.EdgeExpressApiTypeCur;
		...
		if(edgeExpressApiTypeSelection==EdgeExpressApiType.Web) {
			try {
				...
				var frmFriendlyException=new FrmFriendlyException("Error processing EdgeExpress request",ex,false);
	...
	private void butDeletePayment_Click(object sender,System.EventArgs e) {
		...
		var transaction=Transactions.GetAttachedToPayment(_payment.PayNum);
		if(transaction != null) {
	...
	private void FormPayment_FormClosing(object sender,FormClosingEventArgs e) {
		...
		for(var i = 0;i<_listPaySplits.Count;i++) {
			var paySplit=_listPaySplitsOld.FirstOrDefault(x => x.SplitNum==_listPaySplits[i].SplitNum);
			var secLogText="Payment changes canceled:";
			var changesMade="";
		...
		var datePay=SIn.Date(textDate.Text);
		...
		var amount="";
		var transactionID="";
		var paySimplePaymentId="";
		var isDebit=false;
		var isEdgeExpress=false;
		var stringArrayNoteFields=textNote.Text.Replace("\r\n","\n").Replace("\r","\n").Split("\n",StringSplitOptions.RemoveEmptyEntries);
		var listNoteFields=stringArrayNoteFields.ToList();
		for(var i = 0;i<listNoteFields.Count;i++) {
		...
		if(refNum!="") {//Void the PayConnect transaction if there is one
			...
			var originalReceipt=_payment.Receipt;
