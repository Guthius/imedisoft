...
public partial class FormSheetFieldExam:FormODBase {
	...
	private void FormSheetFieldDefEdit_Load(object sender,EventArgs e) {
		var listSheetDefsAvail=SheetDefs.GetCustomForType(SheetTypeEnum.ExamSheet);
	...
	private void FillFieldList() {
		var listSortedAvailFields=new List<string>(); //will alphabetize, since we are adding either FieldName,ReportableName, or RadioButtonGroup depending on field type
		//Add exam sheet fields to the list
		var listSheetFieldDefsAvail=SheetFieldDefs.GetForExamSheet(listBoxExamSheets.GetSelected<SheetDef>().SheetDefNum);
		for(var i=0;i<listSheetFieldDefsAvail.Count;i++) {
			if(listSheetFieldDefsAvail[i].FieldType==SheetFieldType.ComboBox) {
				if(!string.IsNullOrEmpty(listSheetFieldDefsAvail[i].ReportableName)) {
	...
	private void listAvailFields_DoubleClick(object sender,EventArgs e) {
		...
		ExamFieldSelected=SheetTypeEnum.ExamSheet+":"+listBoxExamSheets.GetSelected<SheetDef>().Description+";"
		                  +listBoxAvailFields.SelectedItem;//either RadioButtonGroup or ReportableName or internally defined field name
