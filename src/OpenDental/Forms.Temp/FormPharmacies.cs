...
public partial class FormPharmacies:FormODBase {
	...
	private void FormPharmacies_Load(object sender, System.EventArgs e) {
		...
		for(var i=0;i<_listPharmacies.Count;i++){
	...
	private void FillGrid(){
		...
		for(var i=0;i<_listPharmacies.Count;i++) {
			...
			if(true) {
				var listClinicNums=listPharmClinics.FindAll(x => x.PharmacyNum==_listPharmacies[i].PharmacyNum).Select(y => y.ClinicNum).ToList();
				var listClinics=Clinics.GetClinics(listClinicNums);
	...
	private void butAdd_Click(object sender, System.EventArgs e) {
		using var formPharmacyEdit=new FormPharmacyEdit();
