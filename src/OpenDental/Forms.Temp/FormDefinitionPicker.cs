...
public partial class FormDefinitionPicker:FormODBase {
	...
	private List<Def> _listDefsInitialSelected= [];
	...
	private void FormDefinitionPicker_Load(object sender,EventArgs e) {
		...
		FillGrid();
		var listDefNumsSelected=_listDefsInitialSelected.Select(x => x.DefNum).ToList();
	...
	private void FillListDefs(DefCat defCat,long defNumExclude) {
		...
		var defExclude=_listDefsShowing.FirstOrDefault(x => x.DefNum==defNumExclude);
		if(defExclude==null) {//either defNum is 0 or it is an invalid DefNum, either way nothing to exclude
			return;
		}
		var listInvalidNums=new List<long> { defExclude.DefNum };
		for(var i=0;i<listInvalidNums.Count;i++) {
			var strDefNum=listInvalidNums[i].ToString();
			var listDefNums=_listDefsShowing.FindAll(x => x.ItemValue==strDefNum).Select(x => x.DefNum).ToList();//children
	...
	private void FillGrid() {
		var listDefNumsSelected=new List<long>();
		...
		GridRow row;
		for(var i=0;i<_listDefsShowing.Count;i++) {
			...
			if(_listDefsShowing[i].Category==DefCat.AutoNoteCats && !string.IsNullOrWhiteSpace(_listDefsShowing[i].ItemValue)) {
				var defParent=_listDefsShowing.FirstOrDefault(x => x.DefNum.ToString()==_listDefsShowing[i].ItemValue);
				if(defParent==null) {
