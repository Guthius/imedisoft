...
public partial class FormClaimPrint : FormODBase {
	...
	public bool PrintImmediate(string auditDescription,PrintSituation printSituation,long auditPatNum,bool doUseLastPrinterSettingsIfAvailable=false) {
		...
		var printerSettings=(PrinterSettings)ODprintout.CurPrintout?.PrintDoc?.PrinterSettings?.Clone();
		var pageSettings=(PageSettings)ODprintout.CurPrintout?.PrintDoc?.DefaultPageSettings?.Clone();
		var isOriginAtMargins=ODprintout.CurPrintout?.PrintDoc?.OriginAtMargins;
		...
		if(doUseLastPrinterSettingsIfAvailable && printerSettings!=null && pageSettings!=null) {
			...
			var printer=Printers.GetFirstOrDefault(x => x.PrintSit==printSituation);
			...
			if(printer!=null && printer.IsVirtualPrinter) {
				...
				var printFileName=PrinterL.GetFilePrinterPath(printer);
	...
	private void pd2_PrintPage(object sender, PrintPageEventArgs ev){//raised for each page to be printed.
		...
		var procLimit=ProcLimitForFormat();
		...
		if(!DoPrintBlank){
			...
			for(var i=0;i<_listInsPlans.Count;i++){
		...
		var g=ev.Graphics;
		float xPosText;
		for(var i=0;i<ClaimFormCur.Items.Count;i++){
			if(ClaimFormCur.Items[i].ImageFileName==""){//field
				...
				switch(ClaimFormCur.Items[i].ImageFileName) {
					...
					default:
						...
						var fileName=Path.Combine(ImageStore.GetDataFolder(),ClaimFormCur.Items[i].ImageFileName);
		...
		if(_totalPages==_pagesPrinted){
			ev.HasMorePages=false;
			labelTotPages.Text="1 / "+_totalPages;
		}
		else{
	...
	private string[][] FillRenaissance() { 
		//IsRenaissance=true;
		var procLimit=10;
		var displayStringsFilled=TryFillDisplayStrings(true);//claimprocs is filled in FillDisplayStrings
		...
		_totalPages=(int)Math.Ceiling((double)_listClaimProcs.Count/(double)procLimit);
		var stringDoubleArrayRetVals=new string[_totalPages][];
	...
	private bool TryFillDisplayStrings(bool isRenaissance){
		...
		//	return false;
		//}
		var listClaims=Claims.Refresh(patient.PatNum);
		...
		var insPlan2=InsPlans.GetPlan(_claim.PlanNum2,_listInsPlans);
		var insSub2=InsSubs.GetSub(_claim.InsSubNum2,_listInsSubs);
		if(insPlan2==null){
		...
		var clinic=Clinics.GetClinic(_claim.ClinicNum);//null if clinics are not enabled.
		...
		//	return false;
		//}
		var patientOtherSubscriber=new Patient();
		...
		var listToothInitials=ToothInitials.GetPatientData(patient.PatNum);
		...
		bool doIncludeThis;
		Procedure procedure;
		for(var i=0;i<_listClaimProcsForClaim.Count;i++){//fill the arraylist
			...
			doIncludeThis=true;
			for(var j=0;j<_listClaimProcs.Count;j++){//loop through existing claimprocs
		...
		var dateSignature=DateTime.Today;
		...
		var listStringsMissingTeeth=ToothInitials.GetMissingOrHiddenTeeth(listToothInitials);
		ProcedureCode procedureCode;
		for(var j=listStringsMissingTeeth.Count-1;j>=0;j--) {//loop backwards to keep index accurate as items are removed
			//if the missing tooth is missing because of an extraction being billed here, then exclude it
			for(var p=0;p<_listClaimProcs.Count;p++) {
		...
		for(var i=0;i<ClaimFormCur.Items.Count;i++){
			...
			switch(ClaimFormCur.Items[i].FieldName){
				...
				case "IsGroupHealthPlan":
					var eclaimCode=InsFilingCodes.GetEclaimCode(_insPlan.FilingCode);
					if(patient.MedicaidID=="" 
				...
				case "SubscrLastFirst":
					...
					break;
				case "SubscrLastName":
					_stringArrayDisplay[i]=patientSubscriber.LName;
					break;
				case "SubscrFirstName":
					_stringArrayDisplay[i]=patientSubscriber.FName;
					break;
				case "SubscrAddress":
				...
				case "SubscrPhone"://needs work.  Only used for 1500
					if(isRenaissance) {
						...
						var phone=patientSubscriber.HmPhone.Replace("(","");
						phone=phone.Replace(")","    ");
						phone=phone.Replace("-","  ");
				...
				case "SubscrID":
					...
					var strPatID=PatPlans.GetPatID(_insSub.InsSubNum,listPatPlans);
					if(strPatID=="") {
				...
				case "PatientPhone"://needs work.  Only used for 1500
					if(isRenaissance) {
						...
						var patientPhone=patient.HmPhone.Replace("(","");
				...
				case "BillingDentistPh456":
					if(clinic.UseBillingAddressOnClaims && clinic.PhoneNumber.Length==10) {
						_stringArrayDisplay[i]=clinic.PhoneNumber.Substring(3,3);
					}
				...
				case "BillingDentistPh78910":
					if(clinic.UseBillingAddressOnClaims && clinic.PhoneNumber.Length==10) {
						_stringArrayDisplay[i]=clinic.PhoneNumber.Substring(6);
					}
					else if(clinic.PhoneNumber.Length==10) {
						_stringArrayDisplay[i]=clinic.PhoneNumber.Substring(6);
					}
				...
				case "BillingDentistPhoneFormatted":
					if(clinic.UseBillingAddressOnClaims && clinic.PhoneNumber.Length==10) {
						_stringArrayDisplay[i]=TelephoneNumbers.ReFormat(clinic.PhoneNumber);
					}
					else if(clinic.PhoneNumber.Length==10) {
						_stringArrayDisplay[i]=TelephoneNumbers.ReFormat(clinic.PhoneNumber);
					}
				...
				case "BillingDentistPhoneRaw"://Behaves just like the old BillingDentistAddress field, but is overridden by the Pay-To address if the Pay-To address has been specified.
					if(clinic.UseBillingAddressOnClaims && clinic.PhoneNumber.Length==10) {
						_stringArrayDisplay[i]=clinic.PhoneNumber;
					}
					else if(clinic.PhoneNumber.Length==10) {
						_stringArrayDisplay[i]=clinic.PhoneNumber;
					}
				...
				case "PayToDentistAddress": //Behaves just like the old BillingDentistAddress field, but is overridden by the Pay-To address if the Pay-To address has been specified.
					if(clinic.PayToAddressLine1!="") {
						_stringArrayDisplay[i]=clinic.PayToAddressLine1;
					}
					else if(clinic.UseBillingAddressOnClaims && clinic.BillingAddressLine1!="") {
						_stringArrayDisplay[i]=clinic.BillingAddressLine1;
					}
					else if(clinic.AddressLine1!="") {
						_stringArrayDisplay[i]=clinic.AddressLine1;
					}
					break;
				case "PayToDentistAddress2": //Behaves just like the old BillingDentistAddress2 field, but is overridden by the Pay-To address if the Pay-To address has been specified.
					if(clinic.PayToAddressLine1!="") {
						_stringArrayDisplay[i]=clinic.PayToAddressLine2;
					}
					else if(clinic.UseBillingAddressOnClaims && clinic.BillingAddressLine1!="") {
						_stringArrayDisplay[i]=clinic.BillingAddressLine2;
					}
					else if(clinic.AddressLine1!="") {
						_stringArrayDisplay[i]=clinic.AddressLine2;
					}
					break;
				...
				case "ICDindicator"://For the 1500_02_12 claim form.
					byte icdVersion=0;
					var listBytesDxVersions=new List<byte>();
			...
			switch(ClaimFormCur.Items[i].FieldName){
				...
				case "P1UnitQty":
					if(insPlan.ShowBaseUnits) {
						short bunit;
						short.TryParse(GetProcInfo("BaseUnits",1+startProc),out bunit);
						short uqty;
						short.TryParse(GetProcInfo("UnitQty",1+startProc),out uqty);
						qty=bunit+uqty;
					}
					else if(GetProcInfo("UnitQty",1+startProc)!="") {
						qty=short.Parse(GetProcInfo("UnitQty",1+startProc));
					}
					else {
						qty=0;
					}
				...
				case "P2UnitQty":
					if(insPlan.ShowBaseUnits) {
						short baseUnit;
						short.TryParse(GetProcInfo("BaseUnits",2+startProc),out baseUnit);
						short unitQty;
						short.TryParse(GetProcInfo("UnitQty",2+startProc),out unitQty);
						qty=baseUnit+unitQty;
					}
					else if(GetProcInfo("UnitQty",2+startProc)!="") {
						qty=short.Parse(GetProcInfo("UnitQty",2+startProc));
					}
					else {
						qty=0;
					}
				...
				case "P3UnitQty":
					if(insPlan.ShowBaseUnits) {
						short baseUnit;
						short.TryParse(GetProcInfo("BaseUnits",3+startProc),out baseUnit);
						short unitQty;
						short.TryParse(GetProcInfo("UnitQty",3+startProc),out unitQty);
						qty=baseUnit+unitQty;
					}
					else if(GetProcInfo("UnitQty",3+startProc)!="") {
						qty=short.Parse(GetProcInfo("UnitQty",3+startProc));
					}
					else {
						qty=0;
					}
				...
				case "P4UnitQty":
					if(insPlan.ShowBaseUnits) {
						short baseUnit;
						short.TryParse(GetProcInfo("BaseUnits",4+startProc),out baseUnit);
						short unitQty;
						short.TryParse(GetProcInfo("UnitQty",4+startProc),out unitQty);
						qty=baseUnit+unitQty;
					}
					else if(GetProcInfo("UnitQty",4+startProc)!="") {
						qty=short.Parse(GetProcInfo("UnitQty",4+startProc));
					}
					else {
						qty=0;
					}
				...
				case "P5UnitQty":
					if(insPlan.ShowBaseUnits) {
						short baseUnit;
						short.TryParse(GetProcInfo("BaseUnits",5+startProc),out baseUnit);
						short unitQty;
						short.TryParse(GetProcInfo("UnitQty",5+startProc),out unitQty);
						qty=baseUnit+unitQty;
					}
					else if(GetProcInfo("UnitQty",5+startProc)!="") {
						qty=short.Parse(GetProcInfo("UnitQty",5+startProc));
					}
					else {
						qty=0;
					}
				...
				case "P6UnitQty":
					if(insPlan.ShowBaseUnits) {
						short baseUnit;
						short.TryParse(GetProcInfo("BaseUnits",6+startProc),out baseUnit);
						short unitQty;
						short.TryParse(GetProcInfo("UnitQty",6+startProc),out unitQty);
						qty=baseUnit+unitQty;
					}
					else if(GetProcInfo("UnitQty",6+startProc)!="") {
						qty=short.Parse(GetProcInfo("UnitQty",6+startProc));
					}
					else {
						qty=0;
					}
				...
				case "P7UnitQty":
					if(insPlan.ShowBaseUnits) {
						short baseUnit;
						short.TryParse(GetProcInfo("BaseUnits",7 + startProc),out baseUnit);
						short unitQty;
						short.TryParse(GetProcInfo("UnitQty",7 + startProc),out unitQty);
						qty=baseUnit+unitQty;
					}
					else if(GetProcInfo("UnitQty",7+startProc)!="") {
						qty=short.Parse(GetProcInfo("UnitQty",7+startProc));
					}
					else {
						qty=0;
					}
				...
				case "P8UnitQty":
					if(insPlan.ShowBaseUnits) {
						short baseUnit;
						short.TryParse(GetProcInfo("BaseUnits",8 + startProc),out baseUnit);
						short unitQty;
						short.TryParse(GetProcInfo("UnitQty",8 + startProc),out unitQty);
						qty=baseUnit+unitQty;
					}
					else if(GetProcInfo("UnitQty",8+startProc)!="") {
						qty=short.Parse(GetProcInfo("UnitQty",8+startProc));
					}
					else {
						qty=0;
					}
				...
				case "P9UnitQty":
					if(insPlan.ShowBaseUnits) {
						short baseUnit;
						short.TryParse(GetProcInfo("BaseUnits",9 + startProc),out baseUnit);
						short unitQty;
						short.TryParse(GetProcInfo("UnitQty",9 + startProc),out unitQty);
						qty=baseUnit+unitQty;
					}
					else if(GetProcInfo("UnitQty",9+startProc)!="") {
						qty=short.Parse(GetProcInfo("UnitQty",9+startProc));
					}
					else {
						qty=0;
					}
				...
				case "P10UnitQty":
					if(insPlan.ShowBaseUnits) {
						short baseUnit;
						short.TryParse(GetProcInfo("BaseUnits",10 + startProc),out baseUnit);
						short unitQty;
						short.TryParse(GetProcInfo("UnitQty",10 + startProc),out unitQty);
						qty=baseUnit+unitQty;
					}
					else if(GetProcInfo("UnitQty",10+startProc)!="") {
						qty=short.Parse(GetProcInfo("UnitQty",10+startProc));
					}
					else {
						qty=0;
					}
				...
				case "P11UnitQty":
					if(insPlan.ShowBaseUnits) {
						short baseUnit;
						short.TryParse(GetProcInfo("BaseUnits",11 + startProc),out baseUnit);
						short unitQty;
						short.TryParse(GetProcInfo("UnitQty",11 + startProc),out unitQty);
						qty=baseUnit+unitQty;
					}
					else if(GetProcInfo("UnitQty",11+startProc)!="") {
						qty=short.Parse(GetProcInfo("UnitQty",11+startProc));
					}
					else {
						qty=0;
					}
				...
				case "P12UnitQty":
					if(insPlan.ShowBaseUnits) {
						short baseUnit;
						short.TryParse(GetProcInfo("BaseUnits",12 + startProc),out baseUnit);
						short unitQty;
						short.TryParse(GetProcInfo("UnitQty",12 + startProc),out unitQty);
						qty=baseUnit+unitQty;
					}
					else if(GetProcInfo("UnitQty",12+startProc)!="") {
						qty=short.Parse(GetProcInfo("UnitQty",12+startProc));
					}
					else {
						qty=0;
					}
				...
				case "P13UnitQty":
					if(insPlan.ShowBaseUnits) {
						short baseUnit;
						short.TryParse(GetProcInfo("BaseUnits",13 + startProc),out baseUnit);
						short unitQty;
						short.TryParse(GetProcInfo("UnitQty",13 + startProc),out unitQty);
						qty = baseUnit + unitQty;
					}
					else if(GetProcInfo("UnitQty",13+startProc)!="") {
						qty=short.Parse(GetProcInfo("UnitQty",13+startProc));
					}
					else {
						qty=0;
					}
				...
				case "P14UnitQty":
					if(insPlan.ShowBaseUnits) {
						short baseUnit;
						short.TryParse(GetProcInfo("BaseUnits",14 + startProc),out baseUnit);
						short unitQty;
						short.TryParse(GetProcInfo("UnitQty",14 + startProc),out unitQty);
						qty=baseUnit+unitQty;
					}
					else if(GetProcInfo("UnitQty",14+startProc)!="") {
						qty=short.Parse(GetProcInfo("UnitQty",14+startProc));
					}
					else {
						qty=0;
					}
				...
				case "P15UnitQty":
					if(insPlan.ShowBaseUnits) {
						short baseUnit;
						short.TryParse(GetProcInfo("BaseUnits",15 + startProc),out baseUnit);
						short unitQty;
						short.TryParse(GetProcInfo("UnitQty",15 + startProc),out unitQty);
						qty=baseUnit+unitQty;
					}
					else if(GetProcInfo("UnitQty",15+startProc)!="") {
						qty=short.Parse(GetProcInfo("UnitQty",15+startProc));
					}
					else {
						qty=0;
					}
				...
				case "TotalFee":
					...
					for(var f=startProc;f<startProc+totProcs;f++) {//eg f=0;f<10;f++
						if(f < _listClaimProcs.Count) {
							fee+=(decimal)((ClaimProc)_listClaimProcs[f]).FeeBilled;
							if(CultureInfo.CurrentCulture.Name.EndsWith("CA")) {//Canadian. en-CA or fr-CA
								var listProceduresLab=Procedures.GetCanadianLabFees(_listClaimProcs[f].ProcNum,_listProcedures);
	...
	private void FillMedValueCodes(){
		...
		if(_listClaimValCodeLog.Count>0){
			...
			for(var i=0;i<_listClaimValCodeLog.Count;i++){
				claimValCodeLogArray[i]=(ClaimValCodeLog)_listClaimValCodeLog[i];
			}
			for(var i=_listClaimValCodeLog.Count;i<12;i++){
	...
	private void FillMedCondCodes(){
		var claimCondCodeLog=ClaimCondCodeLogs.GetByClaimNum(_claim.ClaimNum);
	...
	private void FillMedInsStrings(){
		...
		var strInsFilingCode=InsFilingCodes.GetEclaimCode(_insPlan.FilingCode);
		...
		for(var i=0;i<_listClaimProcsForClaim.Count;i++) {
			for(var j=0;j<_listClaimProcsForPat.Count;j++) {
				...
				if(strInsFilingCode=="MC") { //Medicaid
					var strInsFilingCodeClaimProc="";
					for(var k=0;k<_listInsPlans.Count;k++) {
					...
					for(var k=0;k<_listPatPlans.Count;k++) {
		...
		for(var i=0;i<ClaimFormCur.Items.Count;i++) {
			var stringFormat=ClaimFormCur.Items[i].FormatString;
			if(ClaimFormCur.Items[i].FieldName=="MedInsCrossoverIndicator") {
				...
				var totalValAmt=(decimal)ClaimValCodeLogs.GetValAmountTotal(_claim.ClaimNum,"23");
				var amtDue=((decimal)_claim.ClaimFee-priorPaymentsA-priorPaymentsB-totalValAmt);
				if(stringFormat=="") {
	...
	private int ProcLimitForFormat(){
		...
		for(var i=0;i<=15;i++){
			for(var j=0;j<ClaimFormCur.Items.Count;j++){
				if(ClaimFormCur.Items[j].FieldName=="P"+i+"Fee"){
					retVal=i;
				}
			}//for j
		}
		if(retVal==0){//if claimform doesn't use fees, use procedurecode
			for(var i=0;i<=15;i++){
				for(var j=0;j<ClaimFormCur.Items.Count;j++){
					if(ClaimFormCur.Items[j].FieldName=="P"+i+"Code"){
						retVal=i;
					}
				}//for j
			}
	...
	private string GetProcInfo(string field,int procIndex, string stringFormat) {
		...
		var procedure=Procedures.GetProcFromList(_listProcedures,_listClaimProcs[procIndex].ProcNum);
		...
		switch(field) {
			...
			case "Fee":
				var totalProcFees=(decimal)_listClaimProcs[procIndex].FeeBilled;
				if(CultureInfo.CurrentCulture.Name.EndsWith("CA")) {//Canadian. en-CA or fr-CA
					var listProceduresLab=Procedures.GetCanadianLabFees(_listClaimProcs[procIndex].ProcNum,_listProcedures);
					for(var i=0;i<listProceduresLab.Count;i++) {
			...
			case "Desc":
				if(procedureCode.DrugNDC!="") {
					...
					var strDrugUnit="UN";//Unit
					var drugQty=procedure.DrugQty;
					...
					var procedureCodeSent=ProcedureCodes.GetProcCode(_listClaimProcs[procIndex].CodeSent);
					var descript=Procedures.GetClaimDescript(_listClaimProcs[procIndex],procedureCodeSent,procedureCode,_insPlan);
			...
			case "Diagnosis":
				...
				for(var d=0;d<_stringArrayDiagnoses.Length;d++){
					if(_stringArrayDiagnoses[d]==procedure.DiagnosticCode){
						return (d+1).ToString();
					}
				}
			...
			case "DiagnosisPoint":
				...
				var strPointer="";//Output will be in alphabetical order.
				for(var d=0;d<_stringArrayAllDiagnoses.Length;d++) {
			...
			case "FeeMinusLab":
				if(CultureInfo.CurrentCulture.Name.EndsWith("CA")) {//Canadian. en-CA or fr-CA
					if(stringFormat=="") {
						...
						var amt = _listClaimProcs[procIndex].FeeBilled * 100;
						return amt.ToString();
					}
					else {
	...
	private int GetToothRangeCount(string toothNums) {
		var listStringsToothNums=RemoveToothRangeFormat(toothNums);
	...
	private string CalculateUnitQtyField(int index,int startProc,InsPlan insPlan) {
		...
		if(insPlan.ShowBaseUnits) {
			short bUnit;
			short.TryParse(GetProcInfo("BaseUnits",index+startProc),out bUnit);
			short uQty;
			short.TryParse(GetProcInfo("UnitQty",index+startProc),out uQty);
			qty=bUnit+uQty;
		}
		else if(GetProcInfo("UnitQty",index+startProc)!="") {
			qty=short.Parse(GetProcInfo("UnitQty",index+startProc));
		}
		else {
			qty=0;
		}
	...
	private string GenerateSystemAndTeethField(int index,int startProc) {
		var strToothNum=GetProcInfo("ToothNum",index+startProc);
		if(string.IsNullOrEmpty(strToothNum)) {
			var area=GetProcInfo("Area",index+startProc);
		...
		var listStringsToothNums=RemoveToothRangeFormat(strToothNum);
	...
	private string GenerateSupplementalInfoField(int index,int startProc) {
		var listSuppInfo=new List<string>();
		var systemAndTeeth=GenerateSystemAndTeethField(index,startProc);
		if(systemAndTeeth=="") { 
			var strDrugNDC=GetProcInfo("DrugNDC",index+startProc);
		...
		var eClaimNote=GetProcInfo("eClaimNote",index+startProc);
	...
	private List<string> RemoveToothRangeFormat(string toothRange) {
		...
		var stringArrayToothRanges=toothRange.Split(',');
		//List of toothNums
		var listStringsRetVals=new List<string>();
		foreach(var toothRangeVal in stringArrayToothRanges) {
			if(toothRangeVal.Contains("-")) {
				var arrayRange=toothRangeVal.Split('-');
				var start=SIn.Int(arrayRange[0]);
				var end=SIn.Int(arrayRange[1]);
	...
	private void butBack_Click(object sender, System.EventArgs e){
		...
		Preview2.StartPage--;
		labelTotPages.Text=(Preview2.StartPage+1) + " / " + _totalPages;
	}
