...
public partial class FormWebSchedCarrierRule:FormODBase {
	private string _defaultCarrierOther="Other (Default)";
	private string _defaultCarrierSelfPay="Self Pay/No Insurance (Default)";
	...
	private void FormWebSchedCarrierRule_Load(object sender,EventArgs e) {
		...
		if(true && comboClinics.ClinicNumSelected==-1) { 
	...
	private void FillGridWebSchedCarrierRules(bool getRulesFromDb=false) {		
		...
		var col=new GridColumn(Lan.g("GridWebSchedCarrierRules","Insurance Carrier Name"),200);
		...
		GridRow row;
		for(var i=0;i<_listWebSchedCarrierRules.Count;i++) {
			row=new GridRow();
	...
	private void FillListBoxCarrierNames() {
		...
		var listCarrierNamesFiltered=new List<string>();
		if(!_listWebSchedCarrierRules.Any(x => x.CarrierName==_defaultCarrierOther)) {
			listCarrierNamesFiltered.Add(_defaultCarrierOther);
		}
		if(!_listWebSchedCarrierRules.Any(x => x.CarrierName==_defaultCarrierSelfPay)) {
			listCarrierNamesFiltered.Add(_defaultCarrierSelfPay);
		}
	...
	private void gridWebSchedCarrierRules_DoubleClick(object sender,EventArgs e) {
		...
		var webSchedCarrierRule=gridWebSchedCarrierRules.SelectedTag<WebSchedCarrierRule>();
		if(webSchedCarrierRule==null) {
			MsgBox.Show(this,"Please select a rule to be edited.");
			return;
		}
		using var formWebSchedCarrierRuleEdit=new FormWebSchedCarrierRuleEdit();
		formWebSchedCarrierRuleEdit.WebSchedCarrierRule=webSchedCarrierRule;
		if(webSchedCarrierRule.CarrierName==_defaultCarrierOther) {
	...
	private void butRight_Click(object sender,EventArgs e) {
		var listSelectedWebSchedCarrierRuleNums=gridWebSchedCarrierRules.SelectedTags<WebSchedCarrierRule>().Select(x => x.WebSchedCarrierRuleNum).ToList();
		if(listSelectedWebSchedCarrierRuleNums.Count==0) {
			MsgBox.Show(this,"Please select at least one Carrier.");
			return;
		}
		var deleteSelectedMsg=Lans.g("This will delete all of the Carrier Rules selected. Continue?");
	...
	private void butLeft_Click(object sender,EventArgs e) {
		var listSelectedCarriers=listBoxCarriers.GetListSelected<string>();
		if(listSelectedCarriers.Count==0) {
			MsgBox.Show(this,"Please select at least one Carrier.");
			return;
		}
		var listWebSchedCarrierRulesAdded=new List<WebSchedCarrierRule>();
		for(var i=0;i<listSelectedCarriers.Count;i++) {
			var webSchedCarrierRule=new WebSchedCarrierRule();
			...
			if(listSelectedCarriers[i]==_defaultCarrierOther) {
				webSchedCarrierRule.DisplayName="Other";
				webSchedCarrierRule.Rule=RuleType.AllowWithInput;
			}
			else if(listSelectedCarriers[i]==_defaultCarrierSelfPay) {
	...
	private void butCopyRules_Click(object sender,EventArgs e) {
		using var formWebSchedCarrierRuleCopyToClinics=new FormWebSchedCarrierRuleCopyToClinics();
		formWebSchedCarrierRuleCopyToClinics.ShowDialog();
		if(formWebSchedCarrierRuleCopyToClinics.DialogResult==DialogResult.OK && _hasClinicSelected) {
			FillGridWebSchedCarrierRules(true);
			FillListBoxCarrierNames();
		}
	}
	...
	private void butSuggest_Click(object sender,EventArgs e) {
		...
		var numDays=60;
		var listSuggestedCarrierNames=Claims.GetTopVolumeCarrierNamesForClinicAndPeriod(_clinicNum,numDays,10);
		if(listSuggestedCarrierNames.Count==0) {
			MsgBox.Show(Lans.g("No suggestions can be made since there has been no claim activity for the last")+" "+numDays+" "+Lans.g("days."));
			return;
		}
		var listCarrierNamesToCompare=listBoxCarriers.Items.GetAll<string>();
		var listCarrierNameRulesToAdd=new List<string>();
		var suggestedCarriers="";
		//Check to see which carriers currently do not already have rules, if not add to the list of rules to add
		for(var i=0;i<listSuggestedCarrierNames.Count;i++) {
		...
		var suggestedCarrierMsg=Lans.g("Based on the highest number of claims sent, we recommend the following Carriers be added to your rule list:")
		                        +"\r\n\r\n"
		                        +suggestedCarriers
		                        +"\r\nContinue?";
		...
		var listWebSchedCarrierRulesAdded=new List<WebSchedCarrierRule>();
		for(var i=0;i<listCarrierNameRulesToAdd.Count;i++) { 
			var webSchedCarrierRule=new WebSchedCarrierRule();
			webSchedCarrierRule.ClinicNum=_clinicNum;
