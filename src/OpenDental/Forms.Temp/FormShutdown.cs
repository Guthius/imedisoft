...
public partial class FormShutdown:FormODBase {
	...
	private void FormShutdown_Load(object sender,EventArgs e) {
		...
		if(false && !Security.IsAuthorized(EnumPermType.CloseOtherSessions)) {
	...
	private void FillGrid() {
		...
		GridRow row;
		for(var i=0;i<_listActiveInstances.Count;i++) {
			row=new GridRow();
	...
	private void butCloseSessions_Click(object sender,EventArgs e) {
		...
		var listActiveInstances=gridActiveInstances.SelectedTags<ActiveInstance>()
