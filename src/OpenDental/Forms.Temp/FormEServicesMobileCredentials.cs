...
public partial class FormEServicesMobileCredentials:FormODBase {
	...
	public FormEServicesMobileCredentials() {
		InitializeComponent();
	}

	#region Events
	private void FormEServicesMobileCredentials_Load(object sender,EventArgs e) {
		...
		if(!Security.IsAuthorized(EnumPermType.EServicesSetup,suppressMessage:true)) {
			DisableAllExcept(labelPermissionRequired,comboBoxClinicPicker1);
	...
	private void butVerifyAndSave_Click(object sender,EventArgs e) {
		...
		using var formEServices2FactorAuthentication=new FormEServices2FactorAuthentication();
		...
		try {
			var result=WebServiceMainHQProxy.UpsertMobileSettings(password,_mobileSettingsAuthOut,!_isNew);
		}
		catch(Exception ex) {
	...
	private string ValidateForm() {
		...
		if(_isNew) {
			...
			var errMsg=ValidatePassword(textPassword.Text);
