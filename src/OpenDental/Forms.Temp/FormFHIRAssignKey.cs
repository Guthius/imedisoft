...
public partial class FormFHIRAssignKey:FormODBase {
	...
	private void ButOK_Click(object sender,EventArgs e) {
		...
		var frmLicenseAccept=new FrmLicenseAccept(Properties.Resources.OpenDentalApiEndUserLicense, "Open Dental API End User License");
		if(!frmLicenseAccept.ShowDialog()) {
			MsgBox.Show(this, "You must accept the terms to add an API key.");
			return;
		}
		var officeData=PayloadHelper.CreatePayload(PayloadHelper.CreatePayloadContent(textKey.Text,"APIKey"),eServiceCode.FHIR);
		string result;
		...
		var message=WebSerializer.DeserializeTag<string>(result,"Response");
