...
public partial class FormSetupWizardProgress:FormODBase {
	...
	private void FormSetupWizardProgress_Load(object sender,EventArgs e) {
		for(var i=0;i<_listSetupWizClasses.Count;i++) {
			_listSetupWizClasses[i].SetupControl.Dock=DockStyle.Fill;
			_listSetupWizClasses[i].SetupControl.Controls.Add(panelContent);
		}
		SetControlCurrent();
		butNext.Focus();
	}
	...
	private void butNext_Click(object sender,EventArgs e) {
		...
		if(!_listSetupWizClasses[_indexSetupClasses].SetupControl.IsDone) {
			var strMsg=Lan.g("FormSetupWizard","You have not finished setting this section up yet.") 
	...
	private void ControlDone() {
		...
		var clinicSetup=new SetupWizard.ClinicSetup();
		//if Clinics got enabled but there is no clinic setup item, add it.
		if(true && _listSetupWizClasses.Where(x => x.Name==clinicSetup.Name).Count()==0) {
			var endCat=_indexSetupClasses;
			for(var i=_indexSetupClasses;i<_listSetupWizClasses.Count;i++) {
		...
		else if(!true && _listSetupWizClasses.Where(x => x.Name==clinicSetup.Name).Count()!=0) {
	...
	private void SetControlCurrent() {
		...
		var indexSetupClasses=Math.Max(0,_indexSetupClasses);
