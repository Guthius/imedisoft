...
public partial class FormStatementOptions : FormODBase {
	...
	private void FormStatementOptions_Load(object sender, System.EventArgs e) {
		if(ListStatements==null){//this whole section is for single statements only
			...
			if(StatementCur.StatementType!=StmtType.LimitedStatement && PrefC.GetBool(PrefName.ShowFeatureSuperfamilies)) {
				var patientGuarantor=Patients.GetFamily(StatementCur.PatNum).ListPats[0];
			...
			if(!_isFromBilling) {
				checkSendSms.Text=Lans.g("Sent text");//The user cannot send a text from this window, so we want to make it clear that they can't.
		...
		#region Bulk Edit
			...
			allSame=true;
			for(var i=1;i<ListStatements.Count;i++) {
	...
	private void checkIsSent_Click(object sender,EventArgs e) {
		if(_wasInitiallySent && !checkIsSent.Checked){//user unchecks the Sent box in order to edit
			...
			if(StatementCur.DocNum!=0){
				...
				var filePathPatImg=ImageStore.GetPatientFolder(patient,ImageStore.GetDataFolder());
	...
	private void butPrint_Click(object sender,EventArgs e) {
		...
		if(StatementCur.DocNum!=0 && checkIsSent.Checked) {
			var patient=Patients.GetPat(StatementCur.PatNum);
			var filePathPatImg=ImageStore.GetPatientFolder(patient,ImageStore.GetDataFolder());
			if(!File.Exists(ImageStore.GetFilePath(Documents.GetByNum(StatementCur.DocNum),filePathPatImg))) { 
	...
	private void butPrintSheets() {
		...
		if(patient!=null) {
			patientGuar = Patients.GetPat(patient.Guarantor);
		}
		DataSet dataSet=null;
		var superFamily=Patients.GetPat(StatementCur.PatNum).SuperFamily;
		var isLimitedCustomWithSuperFamily=IsLimitedCustomStatement() && superFamily>0;
		if(checkSuperStatement.Checked || isLimitedCustomWithSuperFamily) {
			StatementCur.SuperFamily=superFamily;
			StatementCur.PatNum=StatementCur.SuperFamily;
		...
		Statements.CalcBalTotalInsEst(StatementCur,dataSet,patient,patientGuar);
		if(!SaveToDb()) {
	...
	private bool SaveAsDocument(bool doPrintSheet=false,string pdfFileName="",Sheet sheet=null,DataSet dataSet=null) {
		...
		var isLimitedCustom=checkSuperStatement.Checked || IsLimitedCustomStatement();
		var sheetDef=SheetUtil.GetStatementSheetDef(StatementCur);
		try {
	...
	private bool CreatePdfForSheet() {
		...
		if(checkSuperStatement.Checked || isLimitedCustomWithSuperFamily) {
			...
			var patNum=Statements.GetPatNumForGetAccount(StatementCur);
		...
		var sheetDef=SheetUtil.GetStatementSheetDef(StatementCur);
		var sheet=SheetUtil.CreateSheet(sheetDef,StatementCur.PatNum,StatementCur.HidePayment);
		sheet.Parameters.Add(new SheetParameter(true,"Statement") { ParamValue=StatementCur });
		SheetFiller.FillFields(sheet,dataSet,StatementCur);
		SheetUtil.CalculateHeights(sheet,dataSet,StatementCur);
		var tempPath=ODFileUtils.CombinePaths(PrefC.GetTempFolderPath(),StatementCur.PatNum+".pdf");
		SheetPrinting.CreatePdf(sheet,tempPath,StatementCur,dataSet:dataSet);
		long category=0;
		for(var i=0;i<_listDefsImageCat.Count;i++) {
	...
	private void butPreviewSheets() {
		...
		if(StatementCur.DocNum!=0 && checkIsSent.Checked) {//initiallySent && checkIsSent.Checked){
			...
			if(StatementCur.Mode_==StatementMode.Electronic && electronicBillingType.In(BillingUseElectronicEnum.EHG,BillingUseElectronicEnum.ClaimX) && !PrefC.GetBool(PrefName.BillingElectCreatePDF)) {
	...
	private void LaunchArchivedPdf(Patient patient) {
		var filePathPatFolder=ImageStore.GetPatientFolder(patient,ImageStore.GetDataFolder());
		var document=Documents.GetByNum(StatementCur.DocNum);
		var fileName=ImageStore.GetFilePath(document,filePathPatFolder);
	...
	private void textDate_KeyPress(object sender,KeyPressEventArgs e) {
		...
		var caret=textDate.SelectionStart;
		if(e.KeyChar=='+') {
	...
	private void textDateStart_KeyPress(object sender,KeyPressEventArgs e) {
		...
		var caret=textDateStart.SelectionStart;
		if(e.KeyChar=='+') {
			dateDisplayed=dateDisplayed.AddDays(1);
		}
		if(e.KeyChar=='-') {
	...
	private void textDateEnd_KeyPress(object sender,KeyPressEventArgs e) {
		...
		var caret=textDateEnd.SelectionStart;
		if(e.KeyChar=='+') {
			dateDisplayed=dateDisplayed.AddDays(1);
		}
		if(e.KeyChar=='-') {
	...
	private void checkSinglePatient_Click(object sender,EventArgs e) {
		if(checkSinglePatient.Checked) {
			...
			checkIntermingled.Checked=false;
			StatementCur.SinglePatient=true;
			StatementCur.Intermingled=false;
		}
		else if(StatementCur.IsInvoice) {
			checkSinglePatient.Checked=true;
		}
		else {
			StatementCur.SinglePatient=false;
		}
	}
	...
	private void checkIntermingled_Click(object sender,EventArgs e) {
		if(checkIntermingled.Checked) {
			...
			checkIntermingled.Checked=true;
			StatementCur.SinglePatient=false;
			StatementCur.Intermingled=true;
		}
	}
	...
	private void butDelete_Click(object sender,EventArgs e) {
		...
		try { 
			if(ListStatements==null){
				Statements.DeleteStatements([StatementCur]);
			}
			else{//bulk edit
	...
	private void butSave_Click(object sender, System.EventArgs e) {
		...
		if(ListStatements==null) {//if single statement
			DataSet dataSet=null;
			var superFamily=Patients.GetPat(StatementCur.PatNum).SuperFamily;
			var isLimitedCustomWithSuperFamily=IsLimitedCustomStatement() && superFamily>0;
			if(checkSuperStatement.Checked || isLimitedCustomWithSuperFamily) {
			...
			//We don't want to calculate here for bulk statements. Already done.
			Statements.CalcBalTotalInsEst(StatementCur,dataSet);
		}
		if(!SaveToDb()){
			return;
		}
	...
	private bool SaveToDb(){
		...
		if(ListStatements==null){
			...
			if(checkBoxBillShowTransSinceZero.Checked) {
				var patient=Patients.GetPat(StatementCur.PatNum);
				var listPatAgings=Patients.GetAgingListSimple([], [patient.Guarantor],true);
				var tableBals=Ledgers.GetDateBalanceBegan(listPatAgings,checkSuperStatement.Checked);
				if(tableBals.Rows.Count > 0) {
					var dateFrom=SIn.Date(tableBals.Rows[0]["DateZeroBal"].ToString());
		...
		var tablePatsDates=new DataTable();
		if(checkBoxBillShowTransSinceZero.Checked) {//populate lookup table of patients and dates
			var listPatients=Patients.GetMultPats(ListStatements.Select(x=>x.PatNum).ToList()).ToList();
			var listPatAgings=Patients.GetAgingListSimple(listPatients.Select(x=>x.BillingType).Distinct().ToList(), []);
		...
		for(var i=0;i<ListStatements.Count;i++){
			...
			if(checkBoxBillShowTransSinceZero.Checked) {
				...
				var dataRowArray=tablePatsDates.Select("PatNum='"+ListStatements[i].PatNum+"'");
	...
	private void FormStatementOptions_FormClosing(object sender,FormClosingEventArgs e) {
		...
		if(ListStatements==null && _isStatementNew && StatementCur.IsInvoice) {
			try {
				...
				Statements.DeleteStatements([StatementCur],forceImageDelete:true);
			}
			catch(Exception ex) {
