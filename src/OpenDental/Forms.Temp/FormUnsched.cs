...
public partial class FormUnsched:FormODBase {
	...
	private void FormUnsched_Load(object sender, System.EventArgs e) {
		...
		if(PrefC.GetBool(PrefName.EasyHidePublicHealth)){
			...
			for(var i=0;i<_listSites.Count;i++) {
	...
	private void setupToolStripMenuItem_Click(object sender,EventArgs e) {
		using var formUnschedListSetup=new FormUnschedListSetup();
	...
	private void InitDateRange() {
		var dayCount=PrefC.GetInt(PrefName.UnschedDaysPast);
		if(dayCount==-1) {
	...
	private void grid_MouseUp(object sender,MouseEventArgs e) {
		if(e.Button==MouseButtons.Right && grid.SelectedIndices.Length>0) {
			...
			var patient=Patients.GetLim(_listAppointmentsUnsched[grid.SelectedIndices[grid.SelectedIndices.Length-1]].PatNum);
	...
	private void SelectPatient_Click() {
		...
		var patNum=_listAppointmentsUnsched[grid.SelectedIndices[grid.SelectedIndices.Length-1]].PatNum;
	...
	private void SeeChart_Click() {
		...
		var patient=Patients.GetPat(_listAppointmentsUnsched[grid.SelectedIndices[grid.SelectedIndices.Length-1]].PatNum);//If multiple selected, just take the last one to remain consistent with SendPinboard_Click.
	...
	private void Delete_Click() {
		...
		var listAppointmentsWithNote=new List<Appointment>();
		...
		if(listAppointmentsWithNote.Count>0) {//There were notes in the appointment(s) we are about to delete and we must ask if they want to save them in a commlog.
			...
			var dialogResult=ODMessageBox.Show(commlogMsg,"Question...",MessageBoxButtons.YesNoCancel);
			if(dialogResult==DialogResult.Cancel) {
				return;
			}
			else if(dialogResult==DialogResult.Yes) {
				for(var i = 0;i<listAppointmentsWithNote.Count;i++) {
	...
	private void FillGrid() {
		...
		var clinicNum=true ? comboClinic.ClinicNumSelected : -1;
		...
		var scrollVal=grid.ScrollValue;
		grid.BeginUpdate();
		grid.Columns.Clear();
		var col=new GridColumn(Lan.g("TableUnsched","Patient"),140);
		...
		GridRow row;
		for(var i = 0;i<_listAppointmentsUnsched.Count;i++) {
			row=new GridRow();
			var patName=Lan.g(this,"UNKNOWN");
	...
	private void grid_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		var currentSelection=e.Row;//tbApts.SelectedRow;
		var currentScroll=grid.ScrollValue;//tbApts.ScrollValue;
		var patient=Patients.GetPat(_listAppointmentsUnsched[e.Row].PatNum);//If multiple selected, just take the one that was clicked on.
		GlobalFormOpenDental.PatientSelected(patient,true);
		using var formApptEdit=new FormApptEdit(_listAppointmentsUnsched[e.Row].AptNum);
