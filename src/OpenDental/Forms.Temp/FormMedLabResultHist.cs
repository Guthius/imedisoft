...
public partial class FormMedLabResultHist:FormODBase {
	...
	private void FillGridResultHist() {
		var medLab=MedLabs.GetOne(MedLabResultCur.MedLabNum);
		...
		var listMedLabResults=MedLabResults.GetResultHist(MedLabResultCur,patNum,medLab.SpecimenID,medLab.SpecimenIDFiller);
		for(var i=0;i<listMedLabResults.Count;i++) {
			row=new GridRow();
			var obsVal=listMedLabResults[i].ObsText;
