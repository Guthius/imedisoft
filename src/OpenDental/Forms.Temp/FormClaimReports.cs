...
public partial class FormClaimReports : FormODBase {
	...
	private void FormClaimReports_Shown(object sender,EventArgs e) {
		...
		var clearinghouseHq=_listClearinghousesHq[comboClearhouse.SelectedIndex];
	...
	private void butRetrieve_Click(object sender,EventArgs e) {
		...
		var clearhouseHq=_listClearinghousesHq[comboClearhouse.SelectedIndex];
		...
		var progressExtended=new ODProgressExtended(this
			,new ProgressBarHelper((Lans.g("Clearinghouse Progress")),progressBarEventType:ProgBarEventType.Header),lanThis: this.Name);
		//For Tesia, user wouldn't normally manually retrieve.
		if(clearhouseHq.ISA08=="113504607") {
			if(PrefC.GetLong(PrefName.ClearinghouseDefaultDent)!=clearhouseHq.ClearinghouseNum) //default
			{
				//But they might need to in these situations.
				var errorMessage=Clearinghouses.RetrieveAndImport(clearinghouseClin,false,progressExtended);
		...
		labelRetrieving.Visible=true;
		var errorMesssage=Clearinghouses.RetrieveAndImport(clearinghouseClin,false,progressExtended);
		progressExtended.UpdateProgressDetailed("",tagString:"reports",percentVal:"100%",barVal:100);
