...
public partial class FormSheetImport:FormODBase {
	...
	private void FillRows() {
		...
		var listFieldNamesPersonal=new List<string> {
		...
		var listFieldNamesAddr=new List<string> {
		...
		var listFieldNamesIns1=new List<string> {
		...
		var listFieldNamesIns2=new List<string> {
		   ...
		   || (SheetCur!=null && SheetCur.SheetType==SheetTypeEnum.PatientForm)) 
		{
			...
			if(fieldVal!=null) {//field exists on form
				...
				if(fieldVal=="") {//no box was checked
					...
					try {
						var patientGender=(PatientGender)Enum.Parse(typeof(PatientGender),fieldVal);
			...
			if(fieldVal!=null) {//field exists on form
				...
				if(fieldVal=="") {//no box was checked
					...
					try {
						var patientPosition=(PatientPosition)Enum.Parse(typeof(PatientPosition),fieldVal);
			...
			if(fieldVal!=null) {
				...
				if(fieldVal=="") {
					...
					try {
						var contactMethod=(ContactMethod)Enum.Parse(typeof(ContactMethod),fieldVal);
			...
			if(fieldVal!=null) {
				...
				var referral=Referrals.GetReferralForPat(_patient.PatNum);
			...
			if(fieldVal!=null) {
				...
				if(fieldVal=="") {
					...
					try {
						var relat=(Relat)Enum.Parse(typeof(Relat),fieldVal);
		...
		if(SheetCur!=null && SheetCur.SheetType==SheetTypeEnum.MedicalHistory){
			...
			if(SheetCur!=null){
				var listAllergiesPat=Allergies.GetAll(_patient.PatNum,showInactive:true);
				//Get list of all the allergy check boxes
				var listSheetFieldsAllergy=new List<SheetField>();
				listSheetFieldsAllergy=GetSheetFieldsStartWith("allergy:");
				for(var i=0;i<listSheetFieldsAllergy.Count;i++) {
					fieldVal="";
					importRow=new ImportRow();
					...
					for(var j=0;j<listAllergiesPat.Count;j++) {
					...
					var sheetFieldOppositeBox=GetOppositeSheetFieldCheckBox(listSheetFieldsAllergy,listSheetFieldsAllergy[i]);
			...
			var listSheetFieldsCurrentMed=new List<SheetField>();
			var listSheetFieldsNewMed=new List<SheetField>();
			var listSheetFieldsInputMed=GetSheetFieldsStartWith("inputMed");
			var listSheetFieldsCheckMed=GetSheetFieldsStartWith("checkMed");
			for(var i=0;i<listSheetFieldsInputMed.Count;i++) {
			...
			var listMedicationPatsFull=MedicationPats.Refresh(_patient.PatNum,false);
			for(var i=0;i<listSheetFieldsCurrentMed.Count;i++) {
				...
				importRow.OldValDisplay="N";
				importRow.OldValObj=null;
				for(var j=0;j<listMedicationPatsFull.Count;j++) {
					var strMedName=listMedicationPatsFull[j].MedDescript;//for meds that came back from NewCrop
				...
				var listSheetFieldsRelatedChkBoxes=GetRelatedMedicalCheckBoxes(listSheetFieldsCheckMed,listSheetFieldsCurrentMed[i]);
				for(var j=0;j<listSheetFieldsRelatedChkBoxes.Count;j++) {//Figure out which corresponding checkbox is checked.
			...
			for(var i=0;i<listSheetFieldsNewMed.Count;i++) {
			...
			var listSheetFieldsProblems=GetSheetFieldsStartWith("problem:");
			for(var i=0;i<listSheetFieldsProblems.Count;i++) {
				fieldVal="";
				if(i<1) {
				...
				importRow.OldValDisplay="";
				importRow.OldValObj=null;
				for(var j=0;j<listDiseases.Count;j++) {
				...
				var sheetFieldOppositeBox=GetOppositeSheetFieldCheckBox(listSheetFieldsProblems,listSheetFieldsProblems[i]);
		...
		if(EFormCur!=null && _hasSectionAllergies){
			_listImportRows.Add(CreateSeparator("Allergies"));
			var listAllergyDefs=AllergyDefs.GetAll(includeHidden:false);//from db. AllergyDefs are oddly not cached.
			var listAllergiesPat=Allergies.GetAll(_patient.PatNum,showInactive:false);//just active allergies
			var listStringsPat=new List<string>();//this is for later
			//First, we add a list of all existing allergies
			for(var i=0;i<listAllergiesPat.Count;i++){
				var allergyDef=listAllergyDefs.Find(x=>x.AllergyDefNum==listAllergiesPat[i].AllergyDefNum);
				var descript=allergyDef.Description;
			...
			var isCheckedAllergiesNone=IsCheckedEForm("allergiesNone");
			if(isCheckedAllergiesNone!=null) {
				...
				if(isCheckedAllergiesNone==true) {
					for(var a=0;a<listAllergiesPat.Count;a++) {
						var importRowAllerg=_listImportRows.Find(x=>x.OldValObj==listAllergiesPat[a]);
			...
			if(fieldVal!=null) {
				...
				var listStrsInput=fieldVal.Split([','],StringSplitOptions.RemoveEmptyEntries).Select(x=>x.Trim()).ToList();
				//get list of allergies already covered by checkboxes or radiobuttons
				var listStrAllergiesChecks=EFormCur.ListEFormFields.FindAll(x=>x.DbLink.StartsWith("allergy:")).Select(x=>x.DbLink.Substring(8)).ToList();
				//First loop is to consider which ones to add based on user input
				for(var a=0;a<listStrsInput.Count;a++){
					...
					var allergyDef=listAllergyDefs.Find(x=>x.Description.ToLower()==listStrsInput[a].ToLower());
					if(allergyDef==null){
						...
						var allergyDefOther=listAllergyDefs.Find(x=>x.Description=="Other");
			...
			var listEFormFieldsAllergyCheck=EFormCur.ListEFormFields.FindAll(x=>x.DbLink.StartsWith("allergy:"));
			//Allergy checkboxes and radiobuttons:
			for(var i=0;i<listEFormFieldsAllergyCheck.Count;i++){
				var strAllergyCheck=listEFormFieldsAllergyCheck[i].DbLink.Substring(8);
				var allergyDef=listAllergyDefs.Find(x=>x.Description.ToLower()==strAllergyCheck.ToLower());
				if(allergyDef==null){
				...
				var allergyPat=listAllergiesPat.Find(x=>x.AllergyDefNum==allergyDef.AllergyDefNum);
				...
				if(allergyPat==null){
					...
					{
						//remove
						var importRowAllerg=_listImportRows.Find(x=>
		...
		if(EFormCur!=null && _hasSectionMeds){
			_listImportRows.Add(CreateSeparator("Medications"));
			var eFormFieldMedList=EFormCur.ListEFormFields.Find(x=>x.FieldType==EnumEFormFieldType.MedicationList);
			//No need to check for null because we already did that when setting _hasSectionMeds
			var eFormMedListLayout=JsonConvert.DeserializeObject<EFormMedListLayout>(eFormFieldMedList.ValueLabel);
			var listMedicationPats=MedicationPats.Refresh(_patient.PatNum,includeDiscontinued:false);
			//First, we add a list of all existing meds
			for(var i=0;i<listMedicationPats.Count;i++){
				var medication=Medications.GetMedication(listMedicationPats[i].MedicationNum);
				var strMed=listMedicationPats[i].MedDescript;//used if this medicationPat came from an eRx or eForm import and medication is null
				if(medication!=null){
			...
			for(var m=0;m<listEFormMeds.Count;m++){
				var strMedNewVal=listEFormMeds[m].MedName;
				if(listEFormMeds[m].StrengthFreq!=""){
					strMedNewVal+=" ("+listEFormMeds[m].StrengthFreq+")";
				}
				MedicationPat medicationPat=null;
				for(var a=0;a<listMedicationPats.Count;a++){
					var medication=Medications.GetMedication(listMedicationPats[a].MedicationNum);
					var strMed=listMedicationPats[a].MedDescript;//used if this medicationPat came from an eRx or eForm import and medication is null
					if(medication!=null){
						strMed=medication.MedName;
					}
				...
				if(medicationPat!=null){
					...
					var importRowMed=_listImportRows.Find(x=>x.OldValObj==medicationPat);
		...
		if(EFormCur!=null && _hasSectionProblems){
			...
			for(var i=0;i<listDiseasesPat.Count;i++){
				var diseaseDef=DiseaseDefs.GetFirstOrDefault(x=>x.DiseaseDefNum==listDiseasesPat[i].DiseaseDefNum);
				var descript=diseaseDef.DiseaseName;
			...
			var isCheckedProblemsNone=IsCheckedEForm("problemsNone");
			if(isCheckedProblemsNone!=null) {
				...
				if(isCheckedProblemsNone==true) {
					for(var a=0;a<listDiseasesPat.Count;a++) {
						var importRowDisease=_listImportRows.Find(x=>x.OldValObj==listDiseasesPat[a]);
			...
			if(fieldVal!=null) {
				...
				var listStrProblemChecks=EFormCur.ListEFormFields.FindAll(x=>x.DbLink.StartsWith("problem:")).Select(x=>x.DbLink.Substring(8)).ToList();
				//First loop is to consider which ones to add based on user input
				for(var a=0;a<listStrsInput.Count;a++){
					if(listStringsPat.Contains(listStrsInput[a])){
						continue;
					...
					var diseaseDef=DiseaseDefs.GetFirstOrDefault(x=>x.DiseaseName.ToLower()==listStrsInput[a].ToLower());
					if(diseaseDef==null){
						...
						var diseaseDefOther=DiseaseDefs.GetFirstOrDefault(x=>x.DiseaseName=="Other");
				...
				for(var a=0;a<listDiseasesPat.Count;a++){
					var diseaseDef=DiseaseDefs.GetFirstOrDefault(x=>x.DiseaseDefNum==listDiseasesPat[a].DiseaseDefNum);
					...
					var importRowProb=_listImportRows.Find(x=>
			...
			var listEFormFieldsProblemCheck=EFormCur.ListEFormFields.FindAll(x=>x.DbLink.StartsWith("problem:"));
			//Problem checkboxes and radiobuttons:
			for(var i=0;i<listEFormFieldsProblemCheck.Count;i++){
				var strProblemCheck=listEFormFieldsProblemCheck[i].DbLink.Substring(8);
				var diseaseDef=DiseaseDefs.GetFirstOrDefault(x=>x.DiseaseName.ToLower()==strProblemCheck.ToLower());
				if(diseaseDef==null){
				...
				var diseasePat=listDiseasesPat.Find(x=>x.DiseaseDefNum==diseaseDef.DiseaseDefNum);
				...
				if(diseasePat==null){
					...
					{
						//remove
						var importRowProblem=_listImportRows.Find(x=>
							x.TypeObj==typeof(Disease)
							&& x.OldValObj!=null
	...
	private string GetInputValue(string fieldName) {
		string result=null;
		if(SheetCur!=null){
			var sheetField=SheetCur.SheetFields.Find(
		...
		if(EFormCur!=null){
			var eFormField=EFormCur.ListEFormFields.Find(
	...
	private string GetRadioValue(string fieldName) {
		if(SheetCur!=null){
			var listSheetFields=SheetCur?.SheetFields?.FindAll(x => x.FieldType==SheetFieldType.CheckBox && x.FieldName==fieldName);
			if(listSheetFields==null || listSheetFields.Count==0) {
				return null;
			}
			var sheetField=listSheetFields.FirstOrDefault(x => x.FieldValue=="X");
			return sheetField==null ? "" : sheetField.RadioButtonValue;
		}
		if(EFormCur!=null){
			var eFormField=EFormCur.ListEFormFields.Find(x=>x.DbLink==fieldName);
			if(eFormField is null){
				return null;
			}
	...
	private List<SheetField> GetSheetFieldsStartWith(string fieldName) {
		var listSheetFieldsReturnVal=new List<SheetField>();
		if(SheetCur==null) {
	...
	private SheetField GetOppositeSheetFieldCheckBox(List<SheetField> sheetFieldList,SheetField sheetFieldCur) {
		for(var i=0;i<sheetFieldList.Count;i++) {
	...
	private List<SheetField> GetRelatedMedicalCheckBoxes(List<SheetField> checkMedList,SheetField inputMed) {
		var listSheetFieldsCheckBoxes=new List<SheetField>();
		for(var i=0;i<checkMedList.Count;i++) {
	...
	private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		...
		if(_listImportRows[e.Row].FieldName=="referredFrom") {
			...
			var referralSelected=frmReferralSelect.ReferralSelected;
		...
		#region Enum
			...
			using var formSheetImportEnumPicker=new FormSheetImportEnumPicker(_listImportRows[e.Row].FieldName);
			for(var i=0;i<Enum.GetNames(_listImportRows[e.Row].TypeObj).Length;i++) {
			...
			if(formSheetImportEnumPicker.DialogResult==DialogResult.OK) {
				var selectedI=formSheetImportEnumPicker.listResult.SelectedIndex;
		...
		#region DateTime
			...
			var inputBox=new InputBox(inputBoxParam);//Display the date format that the current computer will use when parsing the date.
			inputBox.ShowDialog();
			if(inputBox.IsDialogCancel) {
				return;
			}
			var dateTimeEntered=inputBox.DateResult;
		        ...
		        || _listImportRows[e.Row].TypeObj==typeof(Disease)) 
		{
			   ...
			   && _listImportRows[e.Row].ImpValObj.GetType()==typeof(long)) 
			{
				...
				var descript=Medications.GetDescription(formMedications.SelectedMedicationNum);
		...
		#region Subscriber
			...
			var patientSubscriber=new Patient();
			using var formSubscriberSelect=new FormSubscriberSelect(_family);
			...
			var patName=Patients.GetNameFirst(patientSubscriber.FName,patientSubscriber.Preferred);
		...
		#region Carrier
			...
			var sheetImportRowCarrierName=GetImportRowByFieldName(insStr+"CarrierName");
			var sheetImportRowCarrierPhone=GetImportRowByFieldName(insStr+"CarrierPhone");
	...
	private bool CheckSheetForInsFields(bool isPrimary) {
		...
		var sheetImportRowRelation=GetImportRowByFieldName(insStr+"Relat");
		var sheetImportRowSubscriber=GetImportRowByFieldName(insStr+"Subscriber");
		var sheetImportRowSubscriberId=GetImportRowByFieldName(insStr+"SubscriberID");
		var sheetImportRowCarrierName=GetImportRowByFieldName(insStr+"CarrierName");
		var sheetImportRowCarrierPhone=GetImportRowByFieldName(insStr+"CarrierPhone");
	...
	private void SetDoImportToFalseForIns(bool isPrimary) {
		...
		var sheetImportRowRelation=GetImportRowByFieldName(insStr+"Relat");
		var sheetImportRowSubscriberId=GetImportRowByFieldName(insStr+"SubscriberID");
		var sheetImportRowEmployerName=GetImportRowByFieldName(insStr+"EmployerName");
		var sheetImportRowGroupName=GetImportRowByFieldName(insStr+"GroupName");
		var sheetImportRowGroupNum=GetImportRowByFieldName(insStr+"GroupNum");
		if(sheetImportRowRelation!=null) {
	...
	private bool ValidateAndImportInsurance(bool isPrimary) {
		...
		var patientSubscriber=(Patient)sheetImportRowSubscriber.ImpValObj;
		...
		if(insPlan.PlanNum==0) {
			...
			for(var i=0;i<listInsSubs.Count;i++) {
		...
		if(isPlanNew) {
			...
			insSubNum=InsSubs.Insert(insSub);
			Benefit benefit;
			var listCovCats=CovCats.GetDeepCopy(true);
	...
	private DialogResult InsuranceImportQuestion(string importValue,bool isPrimary) {
		...
		var msgBoxButton=MessageBoxButtons.OKCancel;
		var createNewPlanMsg="";
	...
	private void butImport_Click(object sender,EventArgs e) {
		...
		if(EFormCur!=null
		   || (SheetCur!=null && SheetCur.SheetType==SheetTypeEnum.PatientForm)) 
		{
			var isImportingPriIns=false;
			var isImportingSecIns=false;
			var listRefAttaches=new List<RefAttach>();
			for(var i=0;i<_listImportRows.Count;i++) {
				if(!_listImportRows[i].DoImport) {
					continue;
				}
			...
			if(_isAddressSameForFam) {
				...
				if(!isAuthArchivedEdit && _family.HasArchivedMember()) {
					ODMessageBox.Show(Lans.g("Security","Not authorized for")+"\r\n"+GroupPermissions.GetDesc(EnumPermType.ArchivedPatientEdit)+"\r\n"
					                  +Lans.g("Archived patients in the family will not be updated.  All other family members will be updated as usual."));
				}
			...
			for(var i=0;i<_listImportRows.Count;i++) {
				...
				if(_listImportRows[i].TypeObj==typeof(Allergy)) {
					...
					if(_listImportRows[i].OldValObj!=null) {
						var allergyOld=(Allergy)_listImportRows[i].OldValObj;
						if(hasValue==YN.Yes) {
					...
					var listAllergyDefs=AllergyDefs.GetAll(false);
					var sheetFieldAllergy=(SheetField)_listImportRows[i].NewValObj;
					//Find what allergy user wants to import.
					for(var j=0;j<listAllergyDefs.Count;j++) {
						if(listAllergyDefs[j].Description==sheetFieldAllergy.FieldName.Remove(0,8)) {
							var allergyNew=new Allergy();
				...
				#region Medications (sheets)
					...
					if(_listImportRows[i].OldValObj!=null) {
						...
						var medicationPatOld=(MedicationPat)_listImportRows[i].OldValObj;
						if(hasValue==YN.Yes) {
					...
					var listMedications=Medications.GetList("");
					var sheetFieldMed=(SheetField)_listImportRows[i].NewValObj;
				...
				#region Diseases (sheets)
					...
					if(_listImportRows[i].OldValObj!=null) {
						var diseaseOld=(Disease)_listImportRows[i].OldValObj;
						if(hasValue==YN.Yes) {
					...
					var sheetFieldDisease=(SheetField)_listImportRows[i].NewValObj;
					var listDiseaseDefs=DiseaseDefs.GetDeepCopy(true);
					//Find what allergy user wants to import.
					for(var j=0;j<listDiseaseDefs.Count;j++) {
						if(listDiseaseDefs[j].DiseaseName==sheetFieldDisease.FieldName.Remove(0,8)) {
							var diseaseNew=new Disease();
		...
		if(EFormCur!=null && _hasSectionAllergies){
			...
			for(var i=0;i<_listImportRows.Count;i++) {
				...
				if(_listImportRows[i].DoRemove){
					var allergy=_listImportRows[i].OldValObj as Allergy;
				...
				if(_listImportRows[i].DoAdd){
					...
					var allergyDef=listAllergyDefs.Find(x=>x.Description.ToLower()==_listImportRows[i].NewValDisplay.ToLower());
					if(allergyDef==null){
						//This allergy that they typed in does not have any matching allergyDef, so we will use "Other".
						...
						var allergyOther=listAllergiesPat.Find(x=>x.AllergyDefNum==allergyDefOther.AllergyDefNum && x.Reaction==_listImportRows[i].NewValDisplay);
		...
		if(EFormCur!=null && _hasSectionMeds){
			...
			for(var i=0;i<_listImportRows.Count;i++) {
				...
				if(_listImportRows[i].MedDoImportCol2 && !_listImportRows[i].DoAdd){
					var medicationPat=_listImportRows[i].OldValObj as MedicationPat;
				...
				if(_listImportRows[i].DoAdd){
					var medicationPat=new MedicationPat();
					medicationPat.PatNum=_patient.PatNum;
					var medication=Medications.GetFirstOrDefault(x=>x.MedName.ToLower()==_listImportRows[i].MedName.ToLower());
		...
		if(EFormCur!=null && _hasSectionProblems){
			...
			for(var i=0;i<_listImportRows.Count;i++) {
				...
				if(_listImportRows[i].DoRemove){
					var disease=_listImportRows[i].OldValObj as Disease;
				...
				if(_listImportRows[i].DoAdd){
					...
					var diseaseDef=DiseaseDefs.GetFirstOrDefault(x=>x.DiseaseName.ToLower()==_listImportRows[i].NewValDisplay.ToLower());
					if(diseaseDef==null){
						...
						var diseaseOther=listDiseasesPat.Find(x=>x.DiseaseDefNum==diseaseDefOther.DiseaseDefNum && x.PatNote==_listImportRows[i].NewValDisplay);
		...
		#endregion Problems (eForms)
		#region ICE
		var listSheetImportRowsFieldsIce = _listImportRows.FindAll(x => x.FieldName.StartsWith("ICE") && x.DoImport);
		if(listSheetImportRowsFieldsIce.Count>0) {
			for(var i=0;i<listSheetImportRowsFieldsIce.Count;i++){
	...
	private string SecurityLogHelper() {
		...
		var indexDoImport=gridMain.Columns.GetIndex("Do Import");//Denotes an imported field with 'X'.
		var indexFieldName=gridMain.Columns.GetIndex("FieldName");
		var indexCurrentValue=gridMain.Columns.GetIndex("Current Value");
		var indexImportValue=gridMain.Columns.GetIndex("Import Value");
		//Grabbing all of the GridRows marked for import, denoted with an 'X'.
		var listGridRowsEdited=gridMain.ListGridRows.FindAll(x=>x.Cells[indexDoImport].Text=="X");
		...
		for(var i = 0;i<listGridRowsEdited.Count;i++) {
