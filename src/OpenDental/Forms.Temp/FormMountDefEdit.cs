...
public partial class FormMountDefEdit : FormODBase{
	...
	private void FormMountDefEdit_Load(object sender, System.EventArgs e) {
		...
		using var formMountDefGenerate=new FormMountDefGenerate();
	...
	private void butDown_Click(object sender, EventArgs e){
		var selectedIdx=_selectedIndex;
		if(selectedIdx==-1) {
		...
		var mountItemDef=_listMountItemDefs[selectedIdx];
		mountItemDef.ItemOrder++;
		MountItemDefs.Update(mountItemDef);
		var mountItemDefBelow=_listMountItemDefs[selectedIdx+1];
	...
	private void butReorder_Click(object sender,EventArgs e) {
		...
		_selectedIndex=-1;
		for(var i=0;i<_listMountItemDefs.Count;i++){
	...
	private void butUp_Click(object sender, EventArgs e){
		...
		mountItemDef.ItemOrder--;
		MountItemDefs.Update(mountItemDef);
		var mountItemDefAbove=_listMountItemDefs[selectedIdx-1];
	...
	private void controlDrawing_Paint(object sender, PaintEventArgs e){
		...
		for(var i=0;i<_listMountItemDefs.Count;i++){
			...
			if(_listMountItemDefs[i].TextShowing.IsNullOrEmpty()){
				using var font2=new Font(Font.FontFamily,(8.25f));
				s="#"+_listMountItemDefs[i].ItemOrder
				     +"\r\nPos: "+_listMountItemDefs[i].Xpos+", "+_listMountItemDefs[i].Ypos
				     +"\r\nSize: "+_listMountItemDefs[i].Width+" x "+_listMountItemDefs[i].Height
				     +"\r\nRot: ";
				s+=_listMountItemDefs[i].RotateOnAcquire
				...
				using var font=new Font(Font.FontFamily,_listMountItemDefs[i].FontSize*_ratio);
		...
		if(_listMountItemDefs[_selectedIndex].TextShowing.IsNullOrEmpty()){
			s="#"+_listMountItemDefs[_selectedIndex].ItemOrder
			     +"\r\nPos: "+_listMountItemDefs[_selectedIndex].Xpos+", "+_listMountItemDefs[_selectedIndex].Ypos
			     +"\r\nSize: "+_listMountItemDefs[_selectedIndex].Width+" x "+_listMountItemDefs[_selectedIndex].Height
			     +"\r\nRot: ";
			s+=_listMountItemDefs[_selectedIndex].RotateOnAcquire
			...
			using var font=new Font(Font.FontFamily,_listMountItemDefs[_selectedIndex].FontSize*_ratio);
			g.DrawString(s,font,Brushes.DarkRed,point);
		}
	}
	...
	private void controlDrawing_MouseDown(object sender, MouseEventArgs e){
		if(_isReordering){
			var idxClicked=HitTest(e.Location);
			if(idxClicked==-1){
				return;
			}
			...
			_reorderIndex++;
			controlDrawing.Invalidate();
			var countNumbered=_listMountItemDefs.Count(x=>x.TextShowing=="");
	...
	private void controlDrawing_MouseMove(object sender, MouseEventArgs e){
		...
		//we are dragging
		var x=(int)((e.X-_pointMouseDownOrig.X+_pointItemOrig.X-_rectangleMount.X)/_ratio);
		if(x<0){
			x=0;
		}
		...
		var y=(int)((e.Y-_pointMouseDownOrig.Y+_pointItemOrig.Y-_rectangleMount.Y)/_ratio);
		if(y<0){
			y=0;
		}
	...
	private void controlDrawing_MouseDoubleClick(object sender, MouseEventArgs e){
		...
		using var formMountItemDefEdit=new FormMountItemDefEdit();
	...
	private void butSave_Click(object sender, System.EventArgs e) {
		...
		var def=Defs.GetDef(DefCat.ImageCats, MountDefCur.DefaultCat);
	...
	private void CalcRatio(){
		...
		var ratioWidth=(float)_rectangleBack.Width/MountDefCur.Width;
		var ratioHeight=(float)_rectangleBack.Height/MountDefCur.Height;
		_ratio=ratioWidth;
		_isWider=false;
