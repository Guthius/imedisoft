...
public partial class FormWebFormNextForms:FormODBase {
	...
	private void FillGrids() {
		...
		var col=new GridColumn(Lan.g("FormWebFormNextForms","Description"),0);
		gridWebFormsAvailable.Columns.Add(col);
		gridWebFormsAvailable.ListGridRows.Clear();
		GridRow row;
		var listWebForms_SheetDefsAvailable=_listWebForms_SheetDefsAll.FindAll(x => !_listWebForms_SheetDefsSelected.Contains(x));
		for(var i=0;i<listWebForms_SheetDefsAvailable.Count;i++) {
			row=new GridRow();
		...
		for(var i=0;i<_listWebForms_SheetDefsSelected.Count;i++) {
			row=new GridRow();
	...
	private void butLeft_Click(object sender,EventArgs e) {
		var listWebForms_SheetDefsToRemove=gridWebFormsSelected.SelectedTags<WebForms_SheetDef>();
		...
		for(var i=0;i<listWebForms_SheetDefsToRemove.Count;i++) {
	...
	private void butRight_Click(object sender,EventArgs e) {
		var listWebForms_SheetDefsToAdd=gridWebFormsAvailable.SelectedTags<WebForms_SheetDef>();
	...
	private void butUp_Click(object sender,EventArgs e) {
		...
		for(var i=0;i<listIndicesSelected.Count;i++) {//Swap places with the previous row for every selected sheet def.
			var webForms_SheetDefPrev=_listWebForms_SheetDefsSelected[listIndicesSelected[i]-1];
	...
	private void butDown_Click(object sender,EventArgs e) {
		...
		for(var i=0;i<listIndicesSelected.Count;i++) {//Swap places with the next row for every selected sheet def.
			var webForm_SheetDefPrev=_listWebForms_SheetDefsSelected[listIndicesSelected[i]+1];
			_listWebForms_SheetDefsSelected[listIndicesSelected[i]+1]=_listWebForms_SheetDefsSelected[listIndicesSelected[i]];
