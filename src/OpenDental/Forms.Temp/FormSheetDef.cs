...
public partial class FormSheetDef:FormODBase {
	...
	private void FormSheetDef_Load(object sender,EventArgs e) {
		...
		var funcItemToString=new Func<SheetTypeEnum,string>((sheetType) => { return Lan.g("enumSheetTypeEnum",sheetType.GetDescription()); });
		//not allowed to change sheettype once created.
		var listSheetTypeEnums =Enum.GetValues(typeof(SheetTypeEnum)).Cast<SheetTypeEnum>().ToList();
		...
		if(!IsInitial) {
			for(var i=0; i<listSheetType.Items.Count;i++) {
		...
		for(var i=0;i<SheetDefCur.SheetFieldDefs.Count;i++){
			var sheetFieldDef=SheetDefCur.SheetFieldDefs[i].Copy();
	...
	private void listSheetType_SelectedIndexChanged(object sender,EventArgs e) {
		...
		var sheetTypeEnumSelected=listSheetType.GetSelected<SheetTypeEnum>();
	...
	private void butReduceFontSize_Click(object sender,EventArgs e) {
		...
		var listSheetFieldDefs=SheetDefCur.SheetFieldDefs;
