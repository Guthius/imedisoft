...
public partial class FormProcCodeEdit : FormODBase {
	...
	private void FormProcCodeEdit_Load(object sender, System.EventArgs e) {
		var listProcedureCodes=ProcedureCodes.GetADAcodes();
		...
		menuItem.Text="Fee Schedule Notes";
		var contextMenu=new ContextMenu();
		contextMenu.MenuItems.Add(menuItem);
		gridFees.ContextMenu=contextMenu;
	...
	private void FillFees(){
		...
		var col=new GridColumn(Lan.g("TableProcFee","Sched"),120);
	...
	private void gridFees_CellDoubleClick(object sender,OpenDental.UI.ODGridClickEventArgs e) {
		var fee=Fees.GetFee(_procedureCode.CodeNum,_listFeeScheds[e.Row].FeeSchedNum,0,0,_listFees);
	...
	private void MenuItem_Click(object sender,EventArgs e) {
		var index=gridFees.GetSelectedIndex();
		if(index<0){
			MsgBox.Show("Please select a fee schedule first");
			return;
		}
		var frmFeeSchedNotes=new FrmFeeSchedNotes();
		frmFeeSchedNotes.FeeSchedCur=_listFeeScheds[index];
		frmFeeSchedNotes.ClinicNum=0;
	...
	private void FillNotes(){
		...
		#region gridNotes
		var col=new GridColumn(Lan.g("TableProcedureNotes","Prov"),80);
		...
		#endregion
		#region gridTpNotes
		var column=new GridColumn(Lan.g("TableTpProcedureNotes","Prov"),80);
		...
		GridRow row;
		for(var i=0;i<_listProcCodeNotes.Count;i++){
			row=new GridRow();
			row.Cells.Add(Providers.GetAbbr(_listProcCodeNotes[i].ProvNum));
			row.Cells.Add(_listProcCodeNotes[i].ProcTime);
			row.Cells.Add(_listProcCodeNotes[i].Note);
			if(_listProcCodeNotes[i].ProcStatus==ProcStat.TP) {
	...
	private void AddNote(bool isTp) {
		using var formProcCodeNoteEdit=new FormProcCodeNoteEdit(isTp);
	...
	private void AddAutoNote(ODtextBox textBox) {
		...
		var cursorIdx=textBox.SelectionStart;
		...
		if(formAutoNotes.ShowDialog()==DialogResult.OK) {
			...
			textBox.Text=textBox.Text.Insert(cursorIdx,"[["+formAutoNotes.SelectedAutoNote.AutoNoteName+"]]");
		}
	}
	...
	private bool ValidateDiagnosticCodes() {
		...
		var listCodes=textDiagnosticCodes.Text.Split(",",StringSplitOptions.RemoveEmptyEntries).ToList();
		if(listCodes.Count>4) {
			MsgBox.Show(this,"Cannot have more than 4 ICD-10 codes.");
			return false;
		}
		var listCodesNotInDb=new List<string>();
		...
		if(listCodesNotInDb.Count>0) {
			var message=Lan.g(this,"Warning: ICD-10 code(s)")+" "+string.Join(", ", listCodesNotInDb)+" "+Lan.g(this,"not recognized in database.");
	...
	private void butNoBillInsByInsPlan_Click(object sender,EventArgs e) {
		using var formInsPlanOverrides=new FormInsPlansOverrides(_procedureCodeOld.CodeNum);
	...
	private void gridNotes_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		using var formProcCodeNoteEdit=new FormProcCodeNoteEdit(false);
	...
	private void gridTpNotes_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		using var formProcCodeNoteEdit=new FormProcCodeNoteEdit(true);
		formProcCodeNoteEdit.ProcCodeNoteCur=_listProcCodeNotes[e.Row].Copy();
		formProcCodeNoteEdit.ShowDialog();
		FillNotes();
	}
	...
	private void butMore_Click(object sender,EventArgs e) {
		using var formProcCodeEditMore=new FormProcCodeEditMore(_procedureCode);
	...
	private void butAuditTrail_Click(object sender,EventArgs e) {
		...
		var frmAuditOneType=new FrmAuditOneType(0,listPermissions,Lan.g(this,"All changes for")+" "+_procedureCode.AbbrDesc+" - "+_procedureCode.ProcCode,_procedureCode.CodeNum);
	...
	private void listTreatArea_SelectedIndexChanged(object sender,EventArgs e) {
		var treatmentArea=listTreatArea.GetSelected<TreatmentArea>();
	...
	private void butSave_Click(object sender, System.EventArgs e) {
		...
		_procedureCode.BaseUnits=SIn.Int(textBaseUnits.Text);
		...
		if(ProcedureCodes.Update(_procedureCode,_procedureCodeOld)) {//whether new or not.
			...
			secLog+=SecurityLogEntryHelper(_procedureCodeOld.DiagnosticCodes,_procedureCode.DiagnosticCodes,"diagnostic codes");
