...
public partial class FormHL7Defs:FormODBase {
	...
	private void FillGrid1() {
		...
		grid1.BeginUpdate();
		grid1.Columns.Clear();
		var col=new GridColumn(Lan.g(this,"Description"),100);
		...
		for(var i=0;i<_listHL7DefsInternal.Count;i++) {
			var row=new GridRow();
	...
	private void FillGrid2() {
		...
		for(var i=0;i<_listHL7DefsCustom.Count;i++) {
			var row=new GridRow();
	...
	private void butDuplicate_Click(object sender,System.EventArgs e) {
		...
		var hL7def=_listHL7DefsCustom[grid2.GetSelectedIndex()].Clone();
		hL7def.IsEnabled=false;
		var hL7DefNum=HL7Defs.Insert(hL7def);
		for(var m=0;m<hL7def.hl7DefMessages.Count;m++) {
			hL7def.hl7DefMessages[m].HL7DefNum=hL7DefNum;
			var hL7DefMessageNum=HL7DefMessages.Insert(hL7def.hl7DefMessages[m]);
			for(var s=0;s<hL7def.hl7DefMessages[m].ListHL7DefSegments.Count;s++) {
				hL7def.hl7DefMessages[m].ListHL7DefSegments[s].HL7DefMessageNum=hL7DefMessageNum;
				var hL7DefSegmentNum=HL7DefSegments.Insert(hL7def.hl7DefMessages[m].ListHL7DefSegments[s]);
				for(var f=0;f<hL7def.hl7DefMessages[m].ListHL7DefSegments[s].hl7DefFields.Count;f++) {
					hL7def.hl7DefMessages[m].ListHL7DefSegments[s].hl7DefFields[f].HL7DefSegmentNum=hL7DefSegmentNum;
					HL7DefFields.Insert(hL7def.hl7DefMessages[m].ListHL7DefSegments[s].hl7DefFields[f]);
				}
			}
		}
	...
	private void butCopy_Click(object sender,EventArgs e) {
		...
		var hl7def=_listHL7DefsInternal[grid1.GetSelectedIndex()].Clone();
	...
	private void butHistory_Click(object sender,EventArgs e) {
		using var formHL7Msgs=new FormHL7Msgs();
	...
	private void grid1_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		using var formHL7DefEdit=new FormHL7DefEdit();
