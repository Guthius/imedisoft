...
public partial class FormImagePickerDXC:FormODBase {
	...
	public FormImagePickerDXC() {
		...
		_imageSelector.LayoutTransform=new System.Windows.Media.ScaleTransform(1,1);
	...
	public void FillTree() {
		...
		var listDefNumsToExpand=new List<long>();
		var listDefNums=listDefsImageCats.FindAll(x=>
	...
	private void imageSelector_SelectionChangeCommitted(object sender,EventArgs e) {
		var nodeType=_imageSelector.GetSelectedType();
		var priKey=_imageSelector.GetSelectedKey();
	...
	private bool SendSelectedToDXCAndSaveLocally() {
		...
		if(_docNumSelected>0) {
			var document=Documents.GetByNum(_docNumSelected);
		...
		var defClaimAttachCat=Defs.GetCatList((int)DefCat.ImageCats).ToList().FindAll(x => x.ItemValue.Contains("C") && !x.IsHidden).FirstOrDefault();
		...
		//Saves mount bitmap to A-Z folder.
			...
			var documentCur=ImageStore.Import(_claimConnectImageAttachment.ImageFileAsBase64,defNumImageType,ImageType.Attachment,PatientCur);
	...
	private void imageSelector_ItemDoubleClick(object sender,EventArgs e) {
		var attachmentSentAndSaved=SendAttachment();
