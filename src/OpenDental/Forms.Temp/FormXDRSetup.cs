...
public partial class FormXDRSetup:FormODBase {
	...
	private void FillForm() {
		...
		var listToolButItems=ToolButItems.GetForProgram(_program.ProgramNum);
		...
		var programProperty=_listProgramProperties.Find(x=>x.ClinicNum==_clinicNum && x.PropertyDesc==XDR.PropertyDescs.LocationID);
	...
	private void SaveLocationIdToList() {
		...
		if(_clinicNum==0) {
			...
			var programProperty=_listProgramProperties.Find(x=>x.ClinicNum==0 && x.PropertyDesc==XDR.PropertyDescs.LocationID);
			if(programProperty==null) {
		...
		var programPropertyLocationID=_listProgramProperties.Find(x=>x.ClinicNum==_clinicNum && x.PropertyDesc==XDR.PropertyDescs.LocationID);
		if(programPropertyLocationID==null) {//this clinic does not yet have a pp for LocationID.
			var programPropertyLocationIDNew=new ProgramProperty();
	...
	private void SaveProgram() {
		...
		for(var i=0;i<listToolBars.SelectedIndices.Count;i++) {
	...
	private void UpsertLocationIdsForClinics() {
		var listProgramPropertiesDb=ProgramProperties.GetForProgram(_program.ProgramNum).FindAll(x => x.PropertyDesc==XDR.PropertyDescs.LocationID);
		for(var i=0;i<_listProgramProperties.Count;i++) {
			if(_listProgramProperties[i].PropertyDesc!=XDR.PropertyDescs.LocationID) {
				continue;
			}
			var programProperty=listProgramPropertiesDb.Find(x => x.ProgramPropertyNum == _listProgramProperties[i].ProgramPropertyNum);
			if(programProperty!=null) {
	...
	private void butSave_Click(object sender,EventArgs e) {
		if(checkEnabled.Checked && !Programs.IsEnabledByHq(ProgramName.XDR,out var err)) {
