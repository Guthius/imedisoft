...
//todo: checkbox
		...
		GridRow row;
		DateTime date;
		for(var i=0;i<_tableReferrals.Rows.Count;i++) {
			row=new GridRow();
			row.Cells.Add(Patients.GetPat(SIn.Long(_tableReferrals.Rows[i]["PatNum"].ToString())).GetNameLF());
			row.Cells.Add(_tableReferrals.Rows[i]["LName"]+", "+_tableReferrals.Rows[i]["FName"]+" "+_tableReferrals.Rows[i]["MName"]);
			...
			var referalToStatus=(ReferralToStatus)SIn.Int(_tableReferrals.Rows[i]["RefToStatus"].ToString());
	...
	private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		var frmRefAttachEdit=new FrmRefAttachEdit();
		var refAttach=_listRefAttaches[e.Row].Copy();
		frmRefAttachEdit.RefAttachCur=refAttach;
		frmRefAttachEdit.ShowDialog();
		FillGrid();
		//reselect
		for(var i=0;i<_listRefAttaches.Count;i++){
