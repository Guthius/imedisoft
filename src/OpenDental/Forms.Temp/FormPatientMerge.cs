...
public partial class FormPatientMerge:FormODBase {
	...
	private void butChangePatientInto_Click(object sender,EventArgs e) {
		...
		if(frmPatientSelect.IsDialogOK){
			var patNumSelected=frmPatientSelect.PatNumSelected;//to prevent warning about marshal-by-reference
	...
	private void butMerge_Click(object sender,EventArgs e) {
		...
		var patFromHasDiscountPlan=DiscountPlanSubs.HasDiscountPlan(_patientFrom.PatNum);
		var patToHasDiscountPlan=DiscountPlanSubs.HasDiscountPlan(_patientTo.PatNum);
		...
		if(_patientFrom.PatNum==_patientFrom.Guarantor) {
			var family=Patients.GetFamily(_patientFrom.PatNum);
		...
		var listTasksPatient=Tasks.RefreshPatientTickets(_patientFrom.PatNum);//Get this before the merge, because the merge updates Task.KeyNum.
		var isSuccessfulMerge=false;
		try {
		...
		if(isSuccessfulMerge) {
			...
			for(var i=0;i<listTasksPatient.Count;i++) {
				var signal=new Signalod();
			...
			FormOpenDental.S_HandleRefreshedTasks(listSignals,listTasksPatient.Select(x => x.TaskNum).ToList(),listTasksPatient, []
				, []);
			#endregion
			...
			if(true) {
				...
				var atozFrom=ImageStore.GetPatientFolder(_patientFrom,atozPath);
				var atozTo=ImageStore.GetPatientFolder(_patientTo,atozPath);
				var stringArrayFromFiles=Directory.GetFiles(atozFrom);
				if(atozFrom==atozTo) {
					...
					for(var i=0;i<stringArrayFromFiles.Length;i++) {
						var fileName=Path.GetFileName(stringArrayFromFiles[i]);
						var destFileName=fileName;
						var destFilePath=ODFileUtils.CombinePaths(atozTo,fileName);
						if(File.Exists(destFilePath)) {
							...
							//the change.
							destFileName=_patientFrom.PatNum+"_"+fileName;
							destFilePath=ODFileUtils.CombinePaths(atozTo,destFileName);
							while(File.Exists(destFilePath)) {
								destFileName=_patientFrom.PatNum+"_"+fileName+"_"+DateTime.Now.ToString("yyyyMMddhhmmss");
								destFilePath=ODFileUtils.CombinePaths(atozTo,destFileName);
							}
						}
						try {
