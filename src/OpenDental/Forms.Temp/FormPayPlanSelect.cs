...
public partial class FormPayPlanSelect : FormODBase {
	...
	private void FillGrid() {
		...
		var col=new GridColumn("Date",70);
		...
		gridMain.Columns.Add(col);
		var listPayPlanNums=_listPayPlansValid.Select(x => x.PayPlanNum).ToList();
		var listPaySplits=PaySplits.GetForPayPlans(listPayPlanNums);
		var listPatients=Patients.GetLimForPats(_listPayPlansValid.Select(x=>x.PatNum).ToList());
		gridMain.ListGridRows.Clear();
		for(var i=0;i < _listPayPlansValid.Count;i++) {
			//no db calls are made in this loop because we have all the necessary information already.
			var planPlan=_listPayPlansValid[i];
			if(checkShowActiveOnly.Checked && planPlan.IsClosed){//User doesn't want to show closed payment plans.
				continue;
			}
			var patient=listPatients.Where(x => x.PatNum == planPlan.PatNum).FirstOrDefault();
			var row=new GridRow();
