...
public partial class FormFeeSchedGroupEdit:FormODBase {
	...
	private List<ClinicDto> _listClinicsInGroup;
	...
	private List<FeeSchedGroup> _listFeeSchedGroupsOther= [];
	...
	private void FillGridAvailable() {
		...
		GridRow row;
		var listClinicsAvailible=GetAvailableClinics();
		for(var i=0;i<listClinicsAvailible.Count();i++) {
			row=new GridRow();
	...
	private void FillGridGroup() {
		...
		GridRow row;
		for(var i=0;i<_listClinicsInGroup.Count();i++) {
			row=new GridRow();
	...
	private List<ClinicDto> GetAvailableClinics() {
		var listClinicsAvailable = Clinics.GetForUserod(Security.CurUser);
		//Get list of Clinics in the current group.  Use in memory list to account for uncommitted changes.
		var listClinicNumsInUse=_listClinicsInGroup.Select(x => x.Id).ToList();
		if(_listFeeSchedGroupsOther.Count>0) {
			for(var i=0;i<_listFeeSchedGroupsOther.Count();i++) {
		...
		listClinicsAvailable.RemoveAll(x => listClinicNumsInUse.Contains(x.Id));
	...
	private bool ValidateUserInput() {
		...
		for(var i=0;i<_listClinicsInGroup.Count();i++) {
			...
			if(_listFeeSchedGroupsOther.Any(x => x.ListClinicNumsAll.Contains(_listClinicsInGroup[i].Id))) { 
	...
	private void butPickFeeSched_Click(object sender,EventArgs e) {
		...
		for(var i=0;i<_listFeeScheds.Count();i++) {
			var row=new GridRow(_listFeeScheds[i].Description,_listFeeScheds[i].IsHidden?"X":"");
			row.Tag=_listFeeScheds[i];
			listGridRowValues.Add(row);
		}
		var formTitle=Lan.g(this,"Fee Schedule Picker");
		var gridTitle=Lan.g(this,"Fee Schedules");
	...
	private void butRight_Click(object sender,EventArgs e) {
		...
		for(var i=0;i<gridAvailable.SelectedGridRows.Count();i++) {
			_listClinicsInGroup.Add((ClinicDto)gridAvailable.SelectedGridRows[i].Tag);
		}
		RefreshAvailableClinics();
	}
	...
	private void butLeft_Click(object sender,EventArgs e) {
		...
		for(var i=0;i<gridGroup.SelectedGridRows.Count();i++)  {
			_listClinicsInGroup.Remove((ClinicDto)gridGroup.SelectedGridRows[i].Tag);
		}
		RefreshAvailableClinics();
	}
	...
	private void butSave_Click(object sender,EventArgs e) {
		...
		var feeSched=comboFeeSched.GetSelected<FeeSched>();
		var listClinicNumsNew=_listClinicsInGroup.Select(x => x.Id).ToList();
		...
		if(_feeSchedGroup.IsNew || _feeSchedGroup.ListClinicNumsAll.Count()<1 || _feeSchedGroup.FeeSchedNum!=feeSched.FeeSchedNum) {
			                                        ...
			                                        +"  Answering no will result in the default fees for the fee schedule being used."))
			{
				...
				var listGridRowValues=new List<GridRow>();
				for(var i=0;i<_listClinicsInGroup.Count();i++) {
					var row=new GridRow(_listClinicsInGroup[i].Abbr,_listClinicsInGroup[i].Description);
					row.Tag=_listClinicsInGroup[i];
					listGridRowValues.Add(row);
				}
				var formTitle=Lan.g(this,"Clinic Picker");
				var gridTitle=Lan.g(this,"Clinics");
				using var formGridSelection=new FormGridSelection(listGridColumnHeaders,listGridRowValues,formTitle,gridTitle);
				if(formGridSelection.ShowDialog()!=DialogResult.OK) {
					MsgBox.Show(this,"A default clinic was not selected.");
					return;
				}
				var clinicNumMaster=((ClinicDto)formGridSelection.ListSelectedTags[0]).Id;//DialogResult.OK means a selection was made.
				...
				progressOD.StartingMessage=Lans.g("Creating Group, Please Wait...");
		...
		//and the fees should have already been synched previously.
			...
			if(listClinicNumsNew.Except(_feeSchedGroup.ListClinicNumsAll).Count()!=0 || _feeSchedGroup.ListClinicNumsAll.Except(listClinicNumsNew).Count()!=0) {
				...
				var listClinicNumsToSync=listClinicNumsNew.Where(x => !_feeSchedGroup.ListClinicNumsAll.Contains(x)).ToList();
