...
public partial class FormTransworldSetup:FormODBase {
	...
	private void FormTransworldSetup_Load(object sender,EventArgs e) {
		...
		if(!true) {//clinics are not enabled, use ClinicNum 0 to indicate 'Headquarters' or practice level program properties
			...
			for(var i = 0;i<listClinics.Count;i++) {
				...
				if(Clinics.ClinicNum==listClinics[i].Id) {//set selected index to the currently selected clinic in FormOpenDental
					_selectedClinicNum=listClinics[i].Id;
		...
		var dateTimeSend=PrefC.GetDateT(PrefName.TransworldServiceTimeDue);
		...
		var stringArraySendFreqs=PrefC.GetString(PrefName.TransworldServiceSendFrequency).Split([' '],StringSplitOptions.RemoveEmptyEntries);
		if(stringArraySendFreqs.Length==2) {
			var sendFreq=SIn.Int(stringArraySendFreqs[0],false);
	...
	private void FillFields() {
		...
		for(var i = 0;i<listProgramPropertiesClinic.Count;i++) {
	...
	private void SetAdvertising() {
		var programProperty=_dictionaryClinicListProgProps[0].FirstOrDefault(x => x.PropertyDesc=="Disable Advertising");//dict guaranteed to contain key 0
	...
	private void SyncWithHQ() {
		var listProgramPropertiesHq=_dictionaryClinicListProgProps[0];//dict guaranteed to contain ClinicNum 0
		...
		var stringArraySyncedProps=new[] { "SftpServerAddress","SftpServerPort","SftpUsername","SftpPassword","ClientIdAccelerator","ClientIdCollection",
			"IsThankYouLetterEnabled","SelectedServices","SyncExcludePosAdjType","SyncExcludeNegAdjType" };
		for(var i = 0;i<listProgramProperties.Count;i++) {//update the currently selected props with the current form values
			switch(listProgramProperties[i].PropertyDesc) {
				...
				case "Disable Advertising HQ":
					...
					var isAdvertDisabledHQ=(listProgramPropertiesHq.FirstOrDefault(x => x.PropertyDesc=="Disable Advertising HQ")?.PropertyValue=="1");
					...
					continue;
				case "SelectedServices":
					var listSelectedServices=new List<int>();
	...
	private void MakeLogEntry(ProgramProperty programProperty) {
		...
		if(true) {
			if(programProperty.ClinicNum==0) {
				...
				var clinicDesc=Clinics.GetDesc(programProperty.ClinicNum);
	...
	private void butSave_Click(object sender,EventArgs e) {
		if(checkEnabled.Checked && !Programs.IsEnabledByHq(ProgramName.Transworld,out var err)) {
		...
		var sendFreq=(int)numericSendFrequency.Value;
		var accountUpdatesRuntime=DateTime.MinValue;
		...
		var billTypePaidInFullDefNum=comboPaidInFullBillType.GetSelectedDefNum();
		...
		var updateFreq=numericSendFrequency.Value+" "+(FrequencyUnit)comboSendFrequencyUnits.SelectedIndex;
		...
		for(var i=0;i<_listProgramProperties.Count;i++){//Iterates through the original properties for each clinic as found when we load this form
			if(_listProgramProperties[i].IsHighSecurity) { 
				List<ProgramProperty> listProgramPropertiesClinic;
		...
		var listClinicNums=_listProgramProperties.Select(x => x.ClinicNum).Where(y => y!=0).Distinct().ToList();//Grab non-HQ clinics
		...
		var listProgramPropertiesNew=_dictionaryClinicListProgProps.Where(x => x.Key!=0).SelectMany(x=> x.Value).ToList();
		_dictionaryClinicListProgProps.TryGetValue(0,out var listProgramPropertiesHQ); //Get the HQ clinic set of properties.
		for(var i=0;i<listProgramPropertiesNew.Count;i++) {
