...
public partial class FormEtrans270Edit:FormODBase {
	...
	public FormEtrans270Edit(long patPlanNum,long planNum,long subNum,bool isDependent,long subPatNum,bool isCoinsuranceInverted) {
		...
		textCurrentGroupNum.Text=InsPlans.GetPlan(planNum, []).GroupNum;
	}
	...
	private void FormEtrans270Edit_Load(object sender,EventArgs e) {
		...
		var patNum=(EtransCur.PatNum==0?_patNum:EtransCur.PatNum);//Older 270/217s were always for the subscriber and have etrans.PatNum of 0.
		this.Text+=": "+Patients.GetNameLF(patNum);
		var clearinghouse=Clearinghouses.GetFirstOrDefault(x => x.ClearinghouseNum==EtransCur.ClearingHouseNum);
		var isClaimConnect=clearinghouse?.CommBridge==EclaimsCommBridge.ClaimConnect;
		var isEds=clearinghouse?.CommBridge==EclaimsCommBridge.EDS;
	...
	private void SelectForImport() {
		for(var i=0;i<_listEB271s.Count;i++) {
	...
	private void RemoveDuplicateBenefits(List<EB271> listEB271sImported) {
		var listEB271sImportedDistinct=RemoveDuplicateBenefitsFrom271(listEB271sImported);
		...
		var listBenefitDescriptions=ListBenefits.Select(x => x.ToString()).ToList();
		for(var i=0;i<listEB271sImportedDistinct.Count;i++) {
	...
	private List<EB271> RemoveDuplicateBenefitsFrom271(List<EB271> listEB271s) {
		var listEB21sDistinct=new List<EB271>();
		var listEBStrings=new List<string>();
	...
	private void FillGridDates() {
		...
		var col=new GridColumn(Lan.g(this,"Date"),150);
	...
	private void FillGrid(){
		...
		var col=new GridColumn(Lan.g(this,"Response"),360);
	...
	private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		var isAuthorized=Security.IsAuthorized(EnumPermType.InsPlanEdit,suppressMessage:true);
		if(e.Col<2) {//raw benefit
			using var formEtrans270EBraw=new FormEtrans270EBraw();
			...
			if(_listEB271s[e.Row].Benefitt==null) {//create new benefit
				...
				using var formBenefitEdit=new FormBenefitEdit(0,_planNum);
	...
	private void gridBen_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		var idxSelectedBenefit=ListBenefits.IndexOf(ListBenefits[e.Row]);
		using var formBenefitEdit=new FormBenefitEdit(0,_planNum);
	...
	private void butImport_Click(object sender,EventArgs e) {
		...
		ListEB271sImported= [];
		for(var i=0;i<gridMain.SelectedIndices.Length;i++) {
			var EB271Selected=_listEB271s[gridMain.SelectedIndices[i]];
	...
	private void butShowRequest_Click(object sender,EventArgs e) {
		using var msgBoxCopyPaste=new MsgBoxCopyPaste(_msgText);
		msgBoxCopyPaste.ShowDialog();
	}

	private void butShowResponse_Click(object sender,EventArgs e) {
		using var msgBoxCopyPaste=new MsgBoxCopyPaste(_msgTextAck);
		msgBoxCopyPaste.ShowDialog();
	}
	...
	private void butPrint_Click(object sender,EventArgs e) {
		...
		if(isClaimConnect && string.IsNullOrEmpty(_htmlResponse)) {
			var x12message=EtransMessageTexts.GetMessageText(EtransCur.EtransMessageTextNum);
			...
			try {//Similar to x270Controller.RequestBenefits().
				...
				var frmFriendlyException=new FrmFriendlyException(Lans.g("FormInsPlan","Connection Error:")+"\r\n"+ex.GetType().Name,ex,isUnhandledException:false);
		...
		if(!string.IsNullOrEmpty(_htmlResponse)) {
			var formWebBrowser=new FormWebBrowser(htmlContent:_htmlResponse);
			formWebBrowser.Show();
			return;
		}
	...
	private void pd_PrintPage(object sender,System.Drawing.Printing.PrintPageEventArgs e) {
		...
		var g=e.Graphics;
		string text;
		using var fontHeading=new Font("Arial",12,FontStyle.Bold);
		...
		if(!_isHeadingPrinted) {
			...
			var insSub=InsSubs.GetSub(this._subNum, []);
			var insPlan=InsPlans.GetPlan(this._planNum, []);
			var patient=Patients.GetPat(insSub.Subscriber);
