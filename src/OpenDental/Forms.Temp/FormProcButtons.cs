...
public partial class FormProcButtons : FormODBase {
	...
	private void SetVisibility() {
		for(var i=0;i<Controls.Count;i++) {//make all controls visible. Then hide below.
	...
	private void FillButtons(){
		...
		for(var i=0;i<_procButtonArray.Length;i++) {
			...
			listViewItem=new ListViewItem([_procButtonArray[i].Description],_procButtonArray[i].ProcButtonNum.ToString());
	...
	private void listViewButtons_DoubleClick(object sender,EventArgs e) {
		...
		var procButton=_procButtonArray[listViewButtons.SelectedIndices[0]].Copy();
	...
	private void panelQuickButtons_RowDoubleClick(object sender,ODButtonPanelEventArgs e) {
		...
		for(var i=0;e.Item!=null && i<e.Item.Tags.Count;i++) {
		...
		if(formProcButtonQuickEdit.ProcButtonQuickCur==null) {//clicked on either a blank row or to the right of existing buttons on a row.
			...
			for(var i=0;i<_listProcButtonQuicks.Count;i++){ //Set ItemOrder
