...
public partial class FormPaySimpleSetup:FormODBase {
	...
	private List<ProgramProperty> _listOnlinePaymentProgramProperties= [];
	...
	private void FillFields() {
		...
		var payTypeDefNumCC=ProgramProperties.GetPropValFromList(_listProgramProperties,PaySimple.PropertyDescs.PaySimplePayTypeCC,clinicNum);
		var payTypeDefNumACH=ProgramProperties.GetPropValFromList(_listProgramProperties,PaySimple.PropertyDescs.PaySimplePayTypeACH,clinicNum);
	...
	private void SynchWithHQ() {
		...
		var hqUsername=ProgramProperties.GetPropValFromList(_listProgramProperties,PaySimple.PropertyDescs.PaySimpleApiUserName,0);//HQ Username before updating to value in textbox
		var hqKey=ProgramProperties.GetPropValFromList(_listProgramProperties,PaySimple.PropertyDescs.PaySimpleApiKey,0);//HQ Key before updating to value in textbox
		var hqPayTypeCC=ProgramProperties.GetPropValFromList(_listProgramProperties,PaySimple.PropertyDescs.PaySimplePayTypeCC,0);//HQ PaymentType before updating to combo box selection
		var payTypeCC="";
		if(comboPaymentTypeCC.SelectedIndex>-1) {
			payTypeCC=comboPaymentTypeCC.GetSelected<Def>().DefNum.ToString();
		}
		var hqPayTypeACH=ProgramProperties.GetPropValFromList(_listProgramProperties,PaySimple.PropertyDescs.PaySimplePayTypeACH,0);//HQ PaymentType before updating to combo box selection
		var payTypeACH="";
		...
		var listClinicNumsDistinct=_listProgramProperties.Select(x => x.ClinicNum).Where(x => x>0).Distinct().ToList();
	...
	private void linkLabel1_LinkClicked(object sender,LinkLabelLinkClickedEventArgs e) {
		var url="https://paysimple.com/partner/open-dental";
		Process.Start(url);
	}
	...
	private string WebhookHelper(string username,string key,long clinicNum) {
		...
		var listApiWebhookResponses=new List<PaySimple.ApiWebhookResponse>();
		if(username=="" || key=="") {
			return errorMessage;
		}
		try {
		...
		Cursor=Cursors.Default;
		var listHooksNeeded=new List<string>();
		...
		if(listHooksNeeded.Count > 0) {			
			var url=WebServiceMainHQProxy.GetWebServiceMainHQInstance().GetPaySimpleWebHookUrl();		
			try {
	...
	private void checkAllowOnlinePayments_Click(object sender,EventArgs e) {
		...
		if(!checkAllowOnlinePayments.Checked) {
			var programPropToRemove=_listOnlinePaymentProgramProperties.Find(x => x.ClinicNum==clinicNum);
			if(programPropToRemove!=null) {
				_listOnlinePaymentProgramProperties.Remove(programPropToRemove);
			}
			return;
		}
		var programProperty=ProgramProperties.GetOnlinePaymentsEnabledForClinic(clinicNum,ProgramName.PaySimple);
		if(programProperty!=null) {
			var msg=Lan.g(this,"Online payments is already enabled for another processor and must be disabled in order to use PaySimple online payments. "
	...
	private void butSave_Click(object sender,System.EventArgs e) {
		...
		if(checkEnabled.Checked && !Programs.IsEnabledByHq(ProgramName.PaySimple,out var err)) {
			MsgBox.Show(err);
			return;
		}
		...
		for(var i = 0;i<_listOnlinePaymentProgramProperties.Count;i++) {
