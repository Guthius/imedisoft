...
public partial class FormWebFormSetup:FormODBase {
	...
	*/
	...
	private List<long> _listNextFormIdsSelected= [];
	...
	private void FormWebFormSetup_Load(object sender,EventArgs e) {
		if(IsPickerForWebSched) {
			...
			for(var i = 0;i<Controls.Count;i++) {
				if(Controls[i]==gridMain || Controls[i]==groupConstructURL || Controls[i]==butSave) {
		...
		if(IsPickerForWebSched) {
			...
			if(!string.IsNullOrEmpty(UrlWebForm)) {
				...
				var decodeURL=HttpUtility.UrlDecode(UrlWebForm);
				var uRIUnparsed=new Uri(decodeURL);
				var nameValueCollection=HttpUtility.ParseQueryString(uRIUnparsed.Query);
		...
		var DateTimeExample=new DateTime(2024,6,18);
		var listCultureDateFormats=new List<CultureDateFormat>();
	...
	private void FormWebFormSetup_Shown(object sender,EventArgs e) {
		...
		var webHostSynchServerURL=PrefC.GetString(PrefName.WebHostSynchServerURL);
	...
	private void FillGrid() {
		if(!WebForms_SheetDefs.TryDownloadSheetDefs(out _listWebForms_SheetDefs)) {
			...
			_listWebForms_SheetDefs= [];
		}
		gridMain.Columns.Clear();
		var gridColumn=new GridColumn(Lan.g(this,"Description"),200);
		gridMain.Columns.Add(gridColumn);
		gridMain.ListGridRows.Clear();
		for(var i=0;i<_listWebForms_SheetDefs.Count;i++) {
	...
	private void OpenBrowser() {
		...
		var stringArrayLines=textURLs.Text.Split(
			["\r\n","\r","\n"],
	...
	private void butSavePrefs_Click(object sender,EventArgs e) {
		...
		_webForms_PreferenceOld=_webForms_Preference.Copy();
		var freqDownloadAlert=3600000;//Default, which is every 1 hour
	...
	private void butNextForms_Click(object sender,EventArgs e) {
		...
		var listSheetDefDescriptionsSelected=textNextForms.Text.Split(", ",StringSplitOptions.RemoveEmptyEntries).ToList();
		var listWebForms_SheetDefsSelected=new List<WebForms_SheetDef>();
		for(var i=0;i < listSheetDefDescriptionsSelected.Count;i++) {//Loop through each selected description to preserve order.
			listWebForms_SheetDefsSelected.Add(_listWebForms_SheetDefs.Find(x => x.Description==listSheetDefDescriptionsSelected[i]));
		}
		using var formWebFormNextForms=new FormWebFormNextForms(_listWebForms_SheetDefs,listWebForms_SheetDefsSelected);
	...
	private void comboDateFormat_SelectionChangeCommitted(object sender,EventArgs e) {
		var cultureDateFormat=comboDateFormat.GetSelected<CultureDateFormat>();
	...
	private void ConstructURLs() {
		...
		for(var i=0;i<gridMain.SelectedTags<WebForms_SheetDef>().Count();i++) {
			var url=SheetDefBaseURL(gridMain.SelectedTags<WebForms_SheetDef>()[i]);
			...
			if(comboClinic.ClinicNumSelected > 0) {//'None' is not selected
				url+="&CID="+comboClinic.ClinicNumSelected; 
			}
	...
	private void butAdd_Click(object sender,EventArgs e) {
		...
		var listSheetDefs=frmSheetPicker.ListSheetDefsSelected;
	...
	private void butUpdate_Click(object sender,EventArgs e) {
		...
		var webForms_sheetDef=gridMain.SelectedTag<WebForms_SheetDef>();
		var sheetDef=SheetDefs.GetFirstOrDefault(x => x.SheetDefNum==webForms_sheetDef.SheetDefNum);
		...
		WebFormL.TryAddOrUpdateSheetDef(this,sheetDef,isNew:false, [webForms_sheetDef]);
		FillGrid();
		Cursor=Cursors.Default;
	}
	...
	private void butDelete_Click(object sender,EventArgs e) {
		...
		var failures=0;
		var listWebForms_SheetDefs=gridMain.SelectedTags<WebForms_SheetDef>().ToList();//Sheets to be deleted
		var listWebForms_SheetDefsNextForms=listWebForms_SheetDefs.FindAll(x => _listNextFormIdsSelected.Contains(x.WebSheetDefID));//Sheets currently in the 'Next Forms' field.
		if(listWebForms_SheetDefsNextForms.Count>0) {//If a sheet that is to be deleted is currently in the 'Next Forms' field.
			var sheetDescriptions=string.Join(",\n",listWebForms_SheetDefsNextForms.Select(x => x.Description));
		...
		var unlinkWebForm=Lan.g(this,"Click 'Preferences' in the eServices Automated Messaging window to change web form selections.");
		//check if any web form to be deleted is linked to ApptNewPatientThankYouWebSheetDefID ClinicPref by WebSheetDefID
		if(true) {
			var listClinicPrefs=ClinicPrefs.GetPrefAllClinics(PrefName.ApptNewPatientThankYouWebSheetDefID,includeDefault:false);
			...
			var listClinicPrefMatches=listClinicPrefs
			...
			if(listClinicPrefMatches.Count!=0) {
				...
				for(var i = 0;i<listWebForms_SheetDefs.Count;i++) {
				...
				for(var i = 0;i<listClinicPrefMatches.Count;i++) {
					//get the clinics associated with each clinic pref
					listClinics.Add(Clinics.GetFirstOrDefault(x => x.Id==listClinicPrefMatches[i].ClinicNum));
				}
				var listClinicPrefDefaults=ClinicPrefs.GetPrefAllClinics(PrefName.AutoMsgingUseDefaultPref,includeDefault:false);
				...
				var listClinicDefaults=listClinics.FindAll(x => listClinicPrefDefaults.Any(y => y.ClinicNum==x.Id));
		...
		var listApptNewPatientThankYouWebSheetDefIDs=PrefC.GetString(PrefName.ApptNewPatientThankYouWebSheetDefID)
			.Split(",",StringSplitOptions.RemoveEmptyEntries)
			.Select(x => SIn.Long(x))
			.ToList();
		//Find the WebSheetDefIDs that are not okay to delete.
		var listWebSheetDefIDsInUse=listWebForms_SheetDefs
			.Select(x => x.WebSheetDefID)
			.Intersect(listApptNewPatientThankYouWebSheetDefIDs)
			.ToList();
		if(listWebSheetDefIDsInUse.Count>0) {
			var listWebFormDescriptions=listWebForms_SheetDefs
		...
		var listNextFormsDescriptions=new List<string>();
		for(var i=0;i<_listNextFormIdsSelected.Count;i++) {
			var webForms_SheetDef=_listWebForms_SheetDefs.Find(x => x.WebSheetDefID==_listNextFormIdsSelected[i]);
	...
	private void butCopyToClipboard_Click(object sender,EventArgs e) {
		...
		var webSheetDef=gridMain.SelectedTag<WebForms_SheetDef>();
		try {
	...
	private void butSave_Click(object sender,EventArgs e) {
		...
		if(!string.IsNullOrWhiteSpace(textRedirectURL.Text) && !Uri.IsWellFormedUriString(textRedirectURL.Text,UriKind.Absolute)) {
			var error=label3.Text+Lan.g(this," should use the following format: http://www.opendental.com or https://www.opendental.com");
