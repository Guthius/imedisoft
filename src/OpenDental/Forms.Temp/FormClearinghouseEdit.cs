...
public partial class FormClearinghouseEdit : FormODBase {
	...
	public bool IsNew;
	private Clearinghouse _clearinghouseClin;
	...
	public List<Clearinghouse> ListClearinghousesClin;
	...
	private void FormClearinghouseEdit_Load(object sender,System.EventArgs e) {
		...
		for(var i=0;i<Enum.GetNames(typeof(EclaimsCommBridge)).Length;i++) {
			var translatedCommBridgeName=Lan.g("enumEclaimsCommBridge",Enum.GetNames(typeof(EclaimsCommBridge))[i]);
	...
	private void FillFields() {
		_clearinghouseClin=null;//Default to null so that if no clinic is found below we will use the defualt HQ options.
		for(var i=0;i<ListClearinghousesClin.Count;i++) {
			if(ListClearinghousesClin[i].ClinicNum==ClinicNum)	{
				_clearinghouseClin=ListClearinghousesClin[i];
			}
		}
		ClearinghouseCur=Clearinghouses.OverrideFields(ClearinghouseHq,_clearinghouseClin);
	...
	private void FillListBoxEraBehavior() {
		var isCanadaEraClearinghouse= comboCommBridge.GetSelected<EclaimsCommBridge>()
			.In(EclaimsCommBridge.Claimstream,EclaimsCommBridge.ITRANS,EclaimsCommBridge.ITRANS2);
		if(!isCanadaEraClearinghouse) {//If US
			listBoxEraBehavior.Height=30;
		}
		else {
			listBoxEraBehavior.Height=43;
		}
		listBoxEraBehavior.Items.Clear();
		var listEraBehaviors=new List<EraBehaviors>((EraBehaviors[])Enum.GetValues(typeof(EraBehaviors)));
		if(!isCanadaEraClearinghouse) {//If US
			listEraBehaviors.RemoveAt((int)EraBehaviors.DownloadDoNotReceive);//Do not include DownloadDoNotReceive in non-Canada.
		}
		for(var i=0;i<listEraBehaviors.Count;i++) {
			var eraBehaviors=listEraBehaviors[i];
			var description=eraBehaviors.GetDescription();
			...
			else if(eraBehaviors==EraBehaviors.DownloadAndReceive) {//This is the only download option available in US.
			...
			listBoxEraBehavior.Items.Add(description,eraBehaviors);
			if(eraBehaviors==ClearinghouseCur.IsEraDownloadAllowed) {
	...
	private void RevealClearinghousePass(Clearinghouse clearinghouse) {
		var revealedClearinghousePass=Clearinghouses.GetRevealPassword(clearinghouse.Password);
	...
	private bool SaveToCache() {
		...
		if(ClinicNum==0) {
			...
			if(_clearinghouseClin==null) {
				_clearinghouseClin=new Clearinghouse();
			}
			//Save Clin Values to ClearinghouseClin, then update ListClinicClearinghousesCur.
			_clearinghouseClin.HqClearinghouseNum=ClearinghouseHq.ClearinghouseNum;
			_clearinghouseClin.ClinicNum=ClinicNum;
			...
			if(textExportPath.Text==ClearinghouseHq.ExportPath) {
				...
				_clearinghouseClin.ExportPath=textExportPath.Text;
			}
			if(textSenderTIN.Text==ClearinghouseHq.SenderTIN) {
				_clearinghouseClin.SenderTIN="";//The value is the same as the default.  Save blank so that default can be updated dynamically.
			}
			else {
				_clearinghouseClin.SenderTIN=textSenderTIN.Text;
			}
			if(textPassword.Text==ClearinghouseHq.Password) {
				_clearinghouseClin.Password="";//The value is the same as the default.  Save blank so that default can be updated dynamically.
			}
			else {
				_clearinghouseClin.Password=textPassword.Text;
			}
			if(textResponsePath.Text==ClearinghouseHq.ResponsePath) {
				_clearinghouseClin.ResponsePath="";//The value is the same as the default.  Save blank so that default can be updated dynamically.
			}
			else {
				_clearinghouseClin.ResponsePath=textResponsePath.Text;
			}
			if(textClientProgram.Text==ClearinghouseHq.ClientProgram) {
				_clearinghouseClin.ClientProgram="";//The value is the same as the default.  Save blank so that default can be updated dynamically.
			}
			else {
				_clearinghouseClin.ClientProgram=textClientProgram.Text;
			}
			if(textLoginID.Text==ClearinghouseHq.LoginID) {
				_clearinghouseClin.LoginID="";//The value is the same as the default.  Save blank so that default can be updated dynamically.
			}
			else {
			...
			if(textSenderName.Text==ClearinghouseHq.SenderName) {
				...
				_clearinghouseClin.SenderName=textSenderName.Text;
			}
			if(textSenderTelephone.Text==ClearinghouseHq.SenderTelephone) {
				_clearinghouseClin.SenderTelephone="";//The value is the same as the default.  Save blank so that default can be updated dynamically.
			}
			else {
				_clearinghouseClin.SenderTelephone=textSenderTelephone.Text;
			}
			if(ListClearinghousesClin.Exists(x => x.ClinicNum==ClinicNum)) {
				...
				Clearinghouses.SyncOverridesForClinic(ref ListClearinghousesClin,_clearinghouseClin);
			}
			else {//Add the clinic specific clearinghouse if not found in current list of clearinghouses.
				ListClearinghousesClin.Add(_clearinghouseClin);
			}
			#endregion
		}
		if(ClearinghouseCur.CommBridge==EclaimsCommBridge.ClaimConnect && (ClearinghouseCur.LoginID!=textLoginID.Text || ClearinghouseCur.Password!=textPassword.Text)){
			var progamPayConnect=Programs.GetCur(ProgramName.PayConnect);
			if(progamPayConnect.Enabled || PrefC.GetEnum<BillingUseElectronicEnum>(PrefName.BillingUseElectronic)==BillingUseElectronicEnum.EHG) {
	...
	private bool ValidateFields() {
		...
		if(checkIsClaimExportAllowed.Checked && textExportPath.Text!="") {
			var hasResult=CheckOrCreateDirectory(textExportPath.Text,pathName:"Export");
			if(!hasResult) {
				return false;
			}
		}
		if(textResponsePath.Text!="") {//Clinic
			var hasResult=CheckOrCreateDirectory(textResponsePath.Text,pathName:"Report");
			if(!hasResult) {
				return false;
			}
		}
	...
	private bool CheckOrCreateDirectory(string directoryName,string pathName) {
		var isUsingODCloudClient=x837Controller.DoSendBatchToCloudClient(ClearinghouseCur);
		var didDirectoryExist=false;
		...
		try
		{
			...
			var message = "Not able to create folder "+directoryName+".  Continue anyway?";
	...
	private void comboCommBridge_SelectionChangeCommitted(object sender,EventArgs e) {
		...
		var hasEclaimsEnabled= comboCommBridge.GetSelected<EclaimsCommBridge>().In(
	...
	private void butSave_Click(object sender,System.EventArgs e) {
		...
		ListClearinghousesClin.ForEach(x => {
		...
		if(IsNew) {
			var clearinghouseNumNew=Clearinghouses.Insert(ClearinghouseHq);
			for(var i=0;i<ListClearinghousesClin.Count;i++) {
				ListClearinghousesClin[i].HqClearinghouseNum=clearinghouseNumNew;
			}
		}
		else {
			Clearinghouses.Update(ClearinghouseHq,ClearinghouseHqOld);
		}
		Clearinghouses.Sync(ListClearinghousesClin,ListClearinghousesClinOld);
		//After saving, reveal all passwords.
		RevealClearinghousePass(ClearinghouseHq);
		for(var i=0;i<ListClearinghousesClin.Count;i++) {
			RevealClearinghousePass(ListClearinghousesClin[i]);
		}
		//Reveal the "olds" just in case someone uses them outside this form.
		RevealClearinghousePass(ClearinghouseHqOld);
		for(var i=0;i<ListClearinghousesClinOld.Count;i++) {
		...
		var hasPrefChanged=Prefs.UpdateBool(PrefName.SaveDXCSOAPAsXML,checkSaveDXCSoap.Checked);//Can't do this in the if check or we risk not running the right side of the OR.
		if(ClearinghouseCur.CommBridge==EclaimsCommBridge.ClaimConnect) {
