...
public partial class FormPayPlanCredits:FormODBase {
	...
	private void FillGrid() {
		...
		for(var i=0;i<_listPayPlanEntries.Count;i++) { //for all account charges
			...
			var cell=new GridCell(_listPayPlanEntries[i].DateStr);
	...
	private List<PayPlanEdit.PayPlanEntry> CreatePayPlanEntries(bool showImplicitlyPaidOffProcs) {
		...
		var listPayPlanEntries=new List<PayPlanEdit.PayPlanEntry>();
		...
		if(ListPayPlanChargesCredit.Exists(x => x.ProcNum==0)) {//only add "Unattached" if there is a credit without a procedure. 
			listPayPlanEntries.Add(PayPlanEdit.CreatePayPlanEntryForUnattachedProcs(_patient.FName));
		}
	...
	private void SetTextBoxes() {
		var listPayPlanEntriesSelected=gridMain.SelectedTags<PayPlanEdit.PayPlanEntry>();
		var isUpdateButton=false;//keep track of the state of the button, if it is add or update. 
		if(listPayPlanEntriesSelected.Count==0) { //if there are no entries selected
			...
			var payPlanEntrySelected=listPayPlanEntriesSelected[0]; //all textboxes should be editable
			textAmt.ReadOnly=false;
			textDate.ReadOnly=false;
			textNote.ReadOnly=false;
			if(payPlanEntrySelected.IsChargeOrd) { //if it's a PayPlanCharge
				...
				if(payPlanEntrySelected.Proc==null) {
					...
					var listPayPlanEntriesForProc=_listPayPlanEntries
	...
	private void butAddOrUpdate_Click(object sender,EventArgs e) {
		...
		var listProcNumsDynamicPayPlan=_constructResults.ListAccountEntries.FindAll(x => x.GetType()==typeof(FauxAccountEntry)
		...
		if(listPayPlanEntriesSelected.Count==0 ) {
			...
			var payPlanChargeAdd=PayPlanEdit.CreateUnattachedCredit(textDate.Text,_patient.PatNum,textNote.Text,_payPlan.PayPlanNum,
			...
			var payPlanEntrySelected=listPayPlanEntriesSelected[0];
			...
			var payPlanChargeSelected=new PayPlanCharge();
			...
			var listPayPlanEntriesProcSelected=listPayPlanEntriesSelected.FindAll(x => !x.IsChargeOrd && x.Proc!=null);
			...
			var countProcsSkipped=listPayPlanEntriesProcSelected.RemoveAll(x => listProcNumsDynamicPayPlan.Contains(x.Proc.ProcNum));
	...
	private void butDelete_Click(object sender,EventArgs e) {
		var listPayPlanEntriesSelected=new List<PayPlanEdit.PayPlanEntry>();
		...
		var listPayPlanChargesSelected=listPayPlanEntriesSelected.Where(x => x.Charge != null).Select(x => x.Charge).ToList();
		...
		for(var i=0;i<listPayPlanChargesSelected.Count;i++) {
