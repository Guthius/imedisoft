...
public partial class FormLimitedStatementSelect : FormODBase
{
    ...
    private List<LimitedRow> _listLimitedRows = [];
    ...
    public List<long> ListPayClaimNums = [];
    public List<long> ListAdjNums = [];
    public List<long> ListPayNums = [];
    public List<long> ListProcNums = [];
    public List<long> ListPatNums = [];
    public Patient PatCur = null;
    public List<long> ListPatNumsFamily = [];
    public List<long> ListPatNumsSuperFamily = [];
    ...
    {
        var listLimitedCustomFamily = new List<EnumLimitedCustomFamily>
        {
            EnumLimitedCustomFamily.Patient,
            EnumLimitedCustomFamily.Family,
            EnumLimitedCustomFamily.SuperFamily,
        };
        for (var i = 0; i < listLimitedCustomFamily.Count; i++)
        {
        ...
        odDatePickerFrom.SetDateTime(_listLimitedRows
            ...
            .Select(x => x.DateTime)
            .Concat([DateTime.Today.AddMonths(-1)]).Min());
    ...
    private void ConstructListFromTable()
    {
        for (var i = 0; i < TableAccount.Rows.Count; i++)
        {
            ...
            limitedRow.ListProcsOnObject = TableAccount.Rows[i]["procsOnObj"].ToString().Split([','], StringSplitOptions.RemoveEmptyEntries).Select(x => SIn.Long(x)).ToList();
            limitedRow.ListAdjustsOnObj = TableAccount.Rows[i]["adjustsOnObj"].ToString().Split([','], StringSplitOptions.RemoveEmptyEntries).Select(x => SIn.Long(x)).ToList();
            limitedRow.ListPaymentsOnObj = TableAccount.Rows[i]["paymentsOnObj"].ToString().Split([','], StringSplitOptions.RemoveEmptyEntries).Select(x => SIn.Long(x)).ToList();
    ...
    private void FillGrid()
    {
        ...
        GridRow row;
        for (var i = 0; i < _listLimitedRows.Count; i++)
        {
            var limitedRow = _listLimitedRows[i];
            ...
            var date = limitedRow.DateTime.Date;
        ...
        {
            var limitedRow = gridMain.ListGridRows[i].Tag as LimitedRow;
    ...
    {
        var listLimitedRowsSelected = gridMain.SelectedTags<LimitedRow>();
        var listSelectedPayClaimNums = listLimitedRowsSelected.FindAll(x => x.AccountEntryType_ == AccountEntryType.ClaimPayment).Select(x => x.PrimaryKey).Distinct().ToList();
        var listSelectedProcNums = listLimitedRowsSelected.FindAll(x => x.AccountEntryType_ == AccountEntryType.Procedure).Select(x => x.PrimaryKey).Distinct().ToList();
        var listSelectedPayNums = listLimitedRowsSelected.FindAll(x => x.AccountEntryType_ == AccountEntryType.Payment).Select(x => x.PrimaryKey).Distinct().ToList();
        var listSelectedAdjNums = listLimitedRowsSelected.FindAll(x => x.AccountEntryType_ == AccountEntryType.Adjustment).Select(x => x.PrimaryKey).Distinct().ToList();
        var listSelectedIndices = gridMain.SelectedIndices.ToList();
        for (var i = 0; i < listSelectedIndices.Count; i++)
        {
            var limitedRowSelected = gridMain.ListGridRows[listSelectedIndices[i]].Tag as LimitedRow;
            if (limitedRowSelected.AccountEntryType_ == AccountEntryType.ClaimPayment)
            {
                ...
                var listLimitedRowsClaim = _listLimitedRows.FindAll(x => x.AccountEntryType_ == AccountEntryType.Claim && listSelectedPayClaimNums.Contains(x.PrimaryKey));
                ...
                {
                    var limitedRow = gridMain.ListGridRows[j].Tag as LimitedRow;
    ...
    private void butNone_Click(object sender, EventArgs e)
    {
        ...
        var listLimitedRows = new List<LimitedRow>(); //clear the lookup
    ...
    private class LimitedRow
    {
        ...
        public List<long> ListProcsOnObject = [];
        public List<long> ListPaymentsOnObj = [];
        public List<long> ListAdjustsOnObj = [];
