...
public partial class FormVaccineObsEdit:FormODBase {
	...
	private void FormVaccineObsEdit_Load(object sender,EventArgs e) {
		...
		var stringArrayQuestionNames=Enum.GetNames(typeof(VaccineObsIdentifier));
		for(var i=0;i<stringArrayQuestionNames.Length;i++) {
			comboObservationQuestion.Items.Add(stringArrayQuestionNames[i]);
			var vaccineObsIdentifier=(VaccineObsIdentifier)i;
		...
		var listUcumCodes=Ucums.GetAllCodes();
		for(var i=0;i<listUcumCodes.Count;i++) {
	...
	private void SetFlags() {
		var vaccineObsType=listValueType.GetSelected<VaccineObsType>();
		...
		var vaccineObsIdentifier=(VaccineObsIdentifier)comboObservationQuestion.SelectedIndex;
