...
public partial class FormClaimCreate : FormODBase {
	...
	private void FillPlanData(){
		...
		var col=new GridColumn(Lan.g("TableInsPlans","Plan"),50);
		...
		GridRow row;
		var listGridRows=new List<GridRow>(); //create a list of gridrows so that we can order them by Ordinal after creating them.
		for(var i=0;i<_listInsSubs.Count;i++) {
			row=new GridRow();
			   ...
			   !_listPatPlans.Exists(x => x.InsSubNum==_listInsSubs[i].InsSubNum)) 
			{
				...
				var patPlan=_listPatPlans.FirstOrDefault(x => x.InsSubNum==_listInsSubs[i].InsSubNum);
				if(patPlan==null) {
					continue;
				}
			...
			var insPlan=InsPlans.GetPlan(_listInsSubs[i].PlanNum,_listInsPlans);
			row.Tag=_listInsSubs[i];
			row.Cells.Add(_family.GetNameInFamLF(_listInsSubs[i].Subscriber));
			var carrierName=Carriers.GetName(insPlan.CarrierNum);
		...
		var stringArrayEnumRelats=Enum.GetNames(typeof(Relat));
		for(var i=0;i<stringArrayEnumRelats.Length;i++){
