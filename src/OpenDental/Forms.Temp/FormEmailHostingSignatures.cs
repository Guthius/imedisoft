...
public partial class FormEmailHostingSignatures:FormODBase {
	...
	private ClinicDto _clinicPrev;
	private bool _isAuthorized;

	private ClinicDto GetClinicSelected() {
	...
	private void FillSignature() {
		var clinic=GetClinicSelected();
		//We only need EmailHosting (either Mass or Secure) to be able to make the web calls to update signatures.
		var isClinicActivated=Clinics.IsMassEmailSignedUp(clinic.Id) || Clinics.IsSecureEmailSignedUp(clinic.Id);
		...
		var plainTextSignature=EmailHostingTemplates.GetSignature(clinic.Id,isHtml:false);
		var htmlSignature=EmailHostingTemplates.GetSignature(clinic.Id,isHtml:true);
	...
	private bool TrySaveEmailSignatures(long clinicNum) {
		...
		var signaturePlainText=textboxPlainText.Text;
		var htmlSignature=webBrowserSignature.Document.Body.InnerHtml;
	...
	private void comboClinicMassEmail_SelectionChangeCommitted(object sender,EventArgs e) {
		if(GetClinicSelected().Id==_clinicPrev.Id) {//didn't change the selected clinic
			return;
		}
		//Don't let them continue if enabled and signatures are not set correctly.
		if(!TrySaveEmailSignatures(_clinicPrev.Id)) {
			//Switch back to previous clinic.
			comboClinicMassEmail.ClinicNumSelected=_clinicPrev.Id;
			return;
		}
		_clinicPrev=GetClinicSelected();
		FillSignature();
