...
public partial class FormInsCatsSetup:FormODBase {
	...
	private void FillSpans() {
		...
		var col=new GridColumn(Lan.g(this,"Category"),90);
		...
		for(var i=0;i<_listCovCats.Count;i++) {
			row=new GridRow();
			...
			for(var j=0;j<listCovSpans.Count;j++){
				row=new GridRow();
	...
	private void gridMain_CellDoubleClick(object sender,OpenDental.UI.ODGridClickEventArgs e) {
		...
		if(gridMain.ListGridRows[e.Row].Tag.GetType()==typeof(CovCat)){
			...
			using var formInsCatEdit=new FormInsCatEdit((CovCat)gridMain.ListGridRows[e.Row].Tag);
			formInsCatEdit.ShowDialog();
			if(formInsCatEdit.DialogResult!=DialogResult.OK) {
				return;
			}	
		}
		else{//covSpan
			selectedKey=((CovSpan)gridMain.ListGridRows[e.Row].Tag).CovSpanNum;
			using var formInsSpanEdit=new FormInsSpanEdit((CovSpan)gridMain.ListGridRows[e.Row].Tag);
			formInsSpanEdit.ShowDialog();
			if(formInsSpanEdit.DialogResult!=DialogResult.OK){
				return;
			}
		}
		changed=true;
	...
	private void butAddSpan_Click(object sender, System.EventArgs e){
		...
		using var formInsSpanEdit=new FormInsSpanEdit(covSpan);
		formInsSpanEdit.IsNew=true;
		formInsSpanEdit.ShowDialog();
		if(formInsSpanEdit.DialogResult!=DialogResult.OK){
			return;
		}
	...
	private void butUp_Click(object sender, System.EventArgs e) {
		...
		var catNum=((CovCat)gridMain.ListGridRows[gridMain.SelectedIndices[0]].Tag).CovCatNum;
	...
	private void butAddCat_Click(object sender, System.EventArgs e) {
		...
		using var formInsCatEdit=new FormInsCatEdit(covcat);
		formInsCatEdit.IsNew=true;
		formInsCatEdit.ShowDialog();
		if(formInsCatEdit.DialogResult==DialogResult.OK){
			changed=true;
			FillSpans();
	...
	private string CheckDefaults(){
		...
		var retVal="";
		int count;
		for(var i=1;i<15;i++) {//starts with 1 because we don't care about None category
			count=CovCats.CountForEbenCat((EbenefitCategory)i);
			if(count>1){
				retVal+="Duplicate category: "+((EbenefitCategory)i)+"\r\n";
			}
			if(count==0){
				retVal+="Missing category: "+((EbenefitCategory)i)+"\r\n";
			}
		}
		if(retVal != "") {
