...
public partial class FormHL7DefMessageEdit:FormODBase {
	...
	private void FormHL7DefMessageEdit_Load(object sender,EventArgs e) {
		FillGrid();
		for(var i=1;i<Enum.GetNames(typeof(MessageTypeHL7)).Length;i++) {//Start at enum 1, 0 is NotDefined and is not displayed for user to select.  Used for unsupported message types
			comboMsgType.Items.Add(Lan.g("enumMessageTypeHL7",Enum.GetName(typeof(MessageTypeHL7), i)));
		}
		for(var i=1;i<Enum.GetNames(typeof(MessageStructureHL7)).Length;i++) {//start at enum 1, 0 is NotDefined and is not displayed for user to select.  Used for unsupported message structures
			comboMessageStructure.Items.Add(Lan.g("enumMessageStructureHL7",Enum.GetName(typeof(MessageStructureHL7),i)));
		}
	...
	private void FillGrid() {
		if(!IsHL7DefInternal && !HL7DefMessageCur.IsNew) {
			HL7DefMessageCur.ListHL7DefSegments=HL7DefSegments.GetDeepFromDb(HL7DefMessageCur.HL7DefMessageNum);
		}
		gridMain.BeginUpdate();
		gridMain.Columns.Clear();
		var col=new GridColumn(Lan.g(this,"Seg"),35);
		...
		if(HL7DefMessageCur!=null && HL7DefMessageCur.ListHL7DefSegments!=null) {
			for(var i=0;i<HL7DefMessageCur.ListHL7DefSegments.Count;i++) {
				var row=new GridRow();
				row.Cells.Add(Lan.g("enumSegmentNameHL7",HL7DefMessageCur.ListHL7DefSegments[i].SegmentName.ToString()));
				row.Cells.Add(HL7DefMessageCur.ListHL7DefSegments[i].ItemOrder.ToString());
				row.Cells.Add(HL7DefMessageCur.ListHL7DefSegments[i].CanRepeat?"X":"");
				row.Cells.Add(HL7DefMessageCur.ListHL7DefSegments[i].IsOptional?"X":"");
				row.Cells.Add(HL7DefMessageCur.ListHL7DefSegments[i].Note);
	...
	private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		using var formHL7DefSegmentEdit=new FormHL7DefSegmentEdit();
		formHL7DefSegmentEdit.HL7DefSegmentCur=HL7DefMessageCur.ListHL7DefSegments[e.Row];
	...
	private void butDelete_Click(object sender,EventArgs e) {
		...
		for(var s=0;s<HL7DefMessageCur.ListHL7DefSegments.Count;s++) {
			for(var f=0;f<HL7DefMessageCur.ListHL7DefSegments[s].hl7DefFields.Count;f++) {
				HL7DefFields.Delete(HL7DefMessageCur.ListHL7DefSegments[s].hl7DefFields[f].HL7DefFieldNum);
			}
			HL7DefSegments.Delete(HL7DefMessageCur.ListHL7DefSegments[s].HL7DefSegmentNum);
		}
	...
	private void butSave_Click(object sender,EventArgs e) {
		...
		if(HL7DefMessageCur.ListHL7DefSegments[0].SegmentName!=SegmentNameHL7.MSH) {
