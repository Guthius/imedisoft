...
public partial class FormEmailMessageEdit : FormODBase {
		...
		,EmailMessage emailMessageReplyingTo=null,params List<EmailMessage>[] listEmailMessagesAll)
	{
		...
		if(emailAddress==null) {
			...
			var formAddressMatchResult=emailPreview.TryGetFromEmailAddress(out emailAddress,_emailMessage.FromAddress);
		...
		var listListsEmailMessagesHistory=listEmailMessagesAll.ToList().FindAll(x => x!=null); //Get every non-null email list (Inbox, Sent, etc)
	...
	private void ConfigureSendButtons() {
		...
		if(emailPreview.IsComposing) {
			...
			var menuItemSendDefault=contextMenu.MenuItems[0];
		...
		//Add reply button if this is a received email
			...
			contextMenu=new ContextMenu([menuItemReply,menuItemCancel]);
	...
	private void ToolBarSend_Paint(object sender,PaintEventArgs e) {
		//ODToolBarButton does not have dimensions until after ToolBarOD's first OnPaint()...
		var width=toolBarSend.Buttons[0]?.Bounds.Width??0;
		if(width!=0 && width!=toolBarSend.Width) {
			//Resize and relocate toolBarSend so it displays properly in the form.
			toolBarSend.Width=width;
			toolBarSend.Location=new Point(emailPreview.Location.X+emailPreview.Width-toolBarSend.Width,toolBarSend.Location.Y);
		}
	...
	private ContextMenu GetSendMenu() {
		...
		for(var i=0;i<listMenuItems.Count;i++) {
	...
	private void FillTemplates() {
		...
		for(var i=0;i<_listEmailTemplates.Count;i++) {
	...
	private void butDeleteTemplate_Click(object sender, System.EventArgs e) {
		...
		if(ODMessageBox.Show(Lan.g(this,"Delete e-mail template?"),"",MessageBoxButtons.OKCancel)
		   !=DialogResult.OK){
			return;
		}
	...
	private void butInsertTemplate_Click(object sender, System.EventArgs e) {
		...
		if(emailPreview.BodyText!="" || emailPreview.Subject!="" || emailPreview.HasAttachments){
			if(ODMessageBox.Show(Lan.g(this,"Replace existing e-mail text with text from the template?  Existing attachments will not be deleted.")
				   ,"",MessageBoxButtons.OKCancel)!=DialogResult.OK){
				return;
			}
		}
		ChangeViewToPlainText();//reset the view to plaintext, to show that body text was replaced.
		var listEmailTemplates=_listEmailTemplates;
		var listEmailAttaches=EmailAttaches.GetForTemplate(listEmailTemplates[listTemplates.SelectedIndex].EmailTemplateNum);
		listEmailAttaches.ForEach(x => x.EmailTemplateNum=0); //Unattach the emailattachments from the email template.
		var emailTemplateSelected=listEmailTemplates[listTemplates.SelectedIndex];
		if(EmailMessages.IsHtmlEmail(emailTemplateSelected.TemplateType)) {
			var isRawHtml=emailTemplateSelected.TemplateType==EmailType.RawHtml;
	...
	private void listTemplates_MouseMove(object sender,MouseEventArgs e) {
		var idxItem=listTemplates.IndexFromPoint(e.Location);
		if(idxItem==-1) {
			toolTipMessage.Hide(listTemplates);
			return;
		}
		var description=listTemplates.Items[idxItem].ToString();
	...
	private EmailAutograph FindDefaultEmailAutograph() {
		if(emailPreview.TryGetFromEmailAddress(out var emailAddressOutgoing)==FromAddressMatchResult.Failed) {
			return null;
		}
	...
	private void SetDefaultAutograph() {
		...
		var emailAutograph=FindDefaultEmailAutograph();
	...
	private void listAutographs_DoubleClick(object sender,EventArgs e) { //edit an autograph
		...
		using var formEmailAutographEdit=new FormEmailAutographEdit(listAutographs.GetSelected<EmailAutograph>());
	...
	private void butInsertAutograph_Click(object sender,EventArgs e) {
		if(listAutographs.SelectedIndex==-1) {
			ODMessageBox.Show(Lan.g(this,"Please select an autograph before inserting."));
			return;
		}
	...
	private void butDeleteAutograph_Click(object sender,EventArgs e) {
		...
		if(ODMessageBox.Show(Lan.g(this,"Delete autograph?"),"",MessageBoxButtons.OKCancel) != DialogResult.OK) {
			return;
		}
	...
	private void butDecrypt_Click(object sender,EventArgs e) {
		if(EmailMessages.GetReceiverUntrustedCount(_emailMessage.FromAddress) >= 0) {//Not trusted yet.
			var msgTrust=Lan.g(this,"The sender address must be added to your trusted addresses before you can decrypt the email")
			             +". "+Lan.g(this,"Add")+" "+_emailMessage.FromAddress+" "+Lan.g(this,"to trusted addresses")+"?";
			if(ODMessageBox.Show(msgTrust,"",MessageBoxButtons.OKCancel)==DialogResult.OK) {
				Cursor=Cursors.WaitCursor;
		...
		Cursor=Cursors.WaitCursor;
		var emailAddress=emailPreview.EmailAddressPreview;
		try {
			...
			ODMessageBox.Show(Lan.g(this,"Decryption failed.")+"\r\n"+ex.Message);
	...
	private void butRawMessage_Click(object sender,EventArgs e) {
		using var msgBoxCopyPaste=new MsgBoxCopyPaste(_emailMessage.RawEmailIn);
	...
	private void butRefresh_Click(object sender,EventArgs e) {
		...
		Cursor=Cursors.WaitCursor;
		var emailAddressReceived=EmailAddresses.GetFirstOrDefault(x => x.EmailUsername==_emailMessage.RecipientAddress);
		try {
			...
			ODMessageBox.Show(Lan.g(this,"Refreshing failed.")+"\r\n"+ex.Message);
			Cursor=Cursors.Default;
			return;
		}
		Cursor=Cursors.Default;
	...
	private EmailAddress GetOutgoingEmailForSending() {
		...
		var result=emailPreview.TryGetFromEmailAddress(out emailAddress);
		switch(result) {
			case FromAddressMatchResult.Failed:
				ODMessageBox.Show(Lan.g(this,"No email account found in Email Setup for")+": "+emailPreview.FromAddress);
				break;
			case FromAddressMatchResult.Success:
				//emailAddress set succesfully
				break;
			case FromAddressMatchResult.Multi:
				if(ODMessageBox.Show(Lan.g(this,"Multiple email accounts matching")+" "+emailPreview.FromAddress+"\r\n"
	...
	private void butDirectMessage_Click(object sender,EventArgs e) {
		...
		if(emailPreview.FromAddress=="" || emailPreview.ToAddress=="") {
			ODMessageBox.Show("Addresses not allowed to be blank.");
			return;
		}
		var emailAddressFrom=GetOutgoingEmailForSending();
		...
		if(emailPreview.FromAddress!=emailAddressFrom.EmailUsername) {
			...
			ODMessageBox.Show(Lan.g(this,"From address must match email address username in email setup.")+"\r\n"+Lan.g(this,"From address must be exactly")+" "+emailAddressFrom.EmailUsername);
			return;
		}
		...
		SaveMsg();
		var emailMessageOld=_emailMessage.SentOrReceived;
	...
	private bool HasValidInlineAttachments(string bodyText,bool isRawHtml) {
		...
		var imageOpeningTag="[[img:";
		var imageClosingTag="]]";
		var bodyTextRemaining=bodyText;
		while(bodyTextRemaining.Contains(imageOpeningTag)) {
			var index=bodyTextRemaining.IndexOf(imageOpeningTag);
			if(index==-1) {
				break;//No more inline images
			}
			if(!bodyTextRemaining.Contains(imageClosingTag)) {
				ODMessageBox.Show(Lan.g(this,"One or more image tags do not close within this template."));
				return false;
			}
			index+=imageOpeningTag.Length;
			bodyTextRemaining=bodyTextRemaining.Substring(index,bodyTextRemaining.Length-index);
			var imgName=bodyTextRemaining.Substring(0,bodyTextRemaining.IndexOf(imageClosingTag));
			string fullPath;
			try {
				fullPath=ImageStore.GetEmailImagePath();
			}
			catch(Exception ex) {
				ODMessageBox.Show(Lan.g(this,ex.Message));
				return false;
			}
			fullPath=Path.Combine(fullPath,SOut.String(imgName));
			if(!File.Exists(fullPath)) {
				ODMessageBox.Show(Lan.g(this,$"{imgName} could not be found."));
				return false;
			}
		}
		return true;
	...
	private bool ValidateFieldsForSend(bool isSecureEmail=false) {
		...
		if(!string.IsNullOrWhiteSpace(errorMsg)) {
			ODMessageBox.Show(this,Lan.g(this,"The following error(s) need to be addressed before you can send your email")+$":\n{errorMsg}");
			return false;
		}
		return true;
	}
	...
	private void butSend_Click(object sender, System.EventArgs e) {
		...
		if(EmailMessages.IsReceived(_emailMessage.SentOrReceived)) {
			...
			using var formEmailMessageEdit=new FormEmailMessageEdit(EmailMessages.CreateReply(_emailMessage,emailPreview.EmailAddressPreview)
		...
		if(emailAutograph!=null && MarkupEdit.ContainsOdHtmlTags(emailAutograph.AutographText) && _emailMessage.HtmlType==EmailType.Regular) {
			var hasValidHTML=false;
			try {
				var markupText=emailPreview.BodyText+"\r\n\r\n"+emailAutograph.AutographText;
	...
	private void MoveCCandBCCIntoToField() {
		var listAddressesToMove=emailPreview.CcAddress.Split(",",StringSplitOptions.RemoveEmptyEntries).ToList();
		listAddressesToMove.AddRange(emailPreview.BccAddress.Split(",",StringSplitOptions.RemoveEmptyEntries).ToList());
		for(var i=0;i<listAddressesToMove.Count;i++) {
