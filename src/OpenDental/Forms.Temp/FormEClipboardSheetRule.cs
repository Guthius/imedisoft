...
public partial class FormEClipboardSheetRule:FormODBase {
	...
	private void FormEClipboardSheetRule_Load(object sender,EventArgs e) {
		...
		if(EClipboardSheetDefCur.SheetDefNum!=0){
			var listSheetDefNumsIgnored=EClipboardSheetDefs.GetListIgnoreSheetDefNums(EClipboardSheetDefCur);
			...
			var eFormDef=EFormDefs.GetFirstOrDefault(x=>x.EFormDefNum==EClipboardSheetDefCur.EFormDefNum);
			if(eFormDef!=null){
		...
		var interval=new Interval(EClipboardSheetDefCur.ResubmitInterval);
	...
	private void butSelectSheetsToIgnore_Click(object sender,EventArgs e) {
		...
		var sheetDef=SheetDefs.GetFirstOrDefault(x=>x.SheetDefNum==EClipboardSheetDefCur.SheetDefNum);
...
/*		private void butChangeForm_Click(object sender,EventArgs e) {
	...
	private void butSave_Click(object sender,EventArgs e) {
		if(radioFreqTimeSpan.Checked){
			...
			var numYears=textVIntYears.Value;
			var numMonths=textVIntMonths.Value;
			var numDays=textVIntDays.Value;
		...
		var isMinAgeBlank=textMinAge.Text.IsNullOrEmpty();
		var minAge=-1;
		...
		var isMaxAgeBlank=textMaxAge.Text.IsNullOrEmpty();
		var maxAge=-1;
