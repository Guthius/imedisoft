...
public partial class FormRxNorms:FormODBase {
	...
	private void FormRxNorms_Shown(object sender,EventArgs e) {
		                                           ...
		                                           +"The RxNorm codes will add about 10 MB to your database size.  Download RxNorm codes?"))
		{
			...
			formCodeSystemsImport.listEnumCodeSystemNames= [EnumCodeSystemName.RXNORM];
	...
	private void FillGrid(bool isExact) {
		...
		var listMedicationRxCuis=Medications.GetWhere(x => x.RxCui!=0).Select(x => x.RxCui.ToString()).Distinct().ToList();
		var listMedPatRxCuis=MedicationPats.GetForRxCuis(_listRxNorms.Select(x => x.RxCui).ToList()).Select(x => x.RxCui.ToString()).ToList();
		gridMain.BeginUpdate();
		gridMain.Columns.Clear();
		var col=new GridColumn(Lan.g("FormRxNorms","Code"),80);
		...
		GridRow row;
		for(var i=0;i<_listRxNorms.Count;i++) { 
			row=new GridRow();
