...
public partial class FormSheetFieldStatic:FormODBase {
	...
	private void FormSheetFieldStatic_Load(object sender,EventArgs e) {
		...
		for(var i=0;i<installedFontCollection.Families.Length;i++){
		...
		for(var i=0;i<Enum.GetNames(typeof(System.Windows.Forms.HorizontalAlignment)).Length;i++) {
	...
	private void FillFields() {
		var listStaticTextFields=Enum.GetValues(typeof(EnumStaticTextField))
		...
		for(var i=0;i<listStaticTextFields.Count;i++) {
	...
	private void ListFields_MouseDown(object sender,MouseEventArgs e) {
		...
		var fieldStr=((UI.ListBox)sender).SelectedItem.ToString();
	...
	private void UpdateTextSizeLabels(object sender,EventArgs e) {
		...
		using var font=new Font(comboFontName.GetSelected<string>(),fontSize,fontStyle);
		using var g=this.CreateGraphics();
		g.TextRenderingHint=TextRenderingHint.ClearTypeGridFit;
		var sizeF=g.MeasureString(textFieldValue.Text,font);//for some reason, this is not off by the MS amount
		labelTextW.Text=Lan.g(this,"TextW:")+" "+((int)sizeF.Width+1);
		labelTextH.Text=Lan.g(this,"TextH:")+" "+((int)sizeF.Height);
	}

	private void butExamSheet_Click(object sender,EventArgs e) {
		using var formSheetFieldExam=new FormSheetFieldExam();
	...
	private void butCalcWidth_Click(object sender,EventArgs e) {
		...
		using var font=new Font(comboFontName.GetSelected<string>(),float.Parse(textFontSize.Text),fontStyle);
		using var stringFormat=new StringFormat(StringFormatFlags.NoClip) {Trimming=StringTrimming.None};
		var heightEntered=SIn.Int(textHeight.Text);
		var initialTextWidth=(int)Math.Ceiling(g.MeasureString(textFieldValue.Text,font).Width);
		...
		var newWidth=initialTextWidth/(int)Math.Ceiling(heightEntered/font.GetHeight());
		// Increase width until all text fits inside the proposed height or we hit the upper bound
		while (newWidth<textWidth.MaxVal) {
			var sizeFNew=g.MeasureString(textFieldValue.Text,font,newWidth,stringFormat);
	...
	private void butCalcHeight_Click(object sender,EventArgs e) {
		...
		using var font=new Font(comboFontName.GetSelected<string>(),float.Parse(textFontSize.Text),fontStyle);
		using var stringFormat=new StringFormat(StringFormatFlags.NoClip) {Trimming=StringTrimming.None};
		var sizeFNew=g.MeasureString(textFieldValue.Text,font,SIn.Int(textWidth.Text),stringFormat);
	...
	private void butSave_Click(object sender,EventArgs e) {
		...
		try {// check if font is compatible with PDFSharp by running it through XFont, if it suceeds, add to the list, otherwise throw error.
			var _=new XFont(comboFontName.GetSelected<string>(),fontSize,XFontStyle.Regular);
		}
		catch {
