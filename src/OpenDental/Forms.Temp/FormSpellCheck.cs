...
public partial class FormSpellCheck:FormODBase {
	...
	private void FillGrid() {
		...
		var col=new GridColumn("",200,false);
		gridMain.Columns.Add(col);
		gridMain.ListGridRows.Clear();
		GridRow row;
		for(var i=0;i<_listDictCustoms.Count;i++) {
			row=new GridRow();
	...
	private void butAdd_Click(object sender,EventArgs e) {
		...
		var newWord=Regex.Replace(textCustom.Text,"[\\s]|[\\p{P}\\p{S}-['-]]","");//don't allow words with spaces or punctuation except ' and - in them
		for(var i=0;i<_listDictCustoms.Count;i++) {//Make sure it's not already in the custom list
		...
		var dictCustomWord=new DictCustom();
	...
	private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		...
		var dictCustomOrigWord=_listDictCustoms[e.Row];
		inputBoxParam.Text=dictCustomOrigWord.WordText;
		var inputBoxEditWord=new InputBox(inputBoxParam);
		...
		var newWord=Regex.Replace(inputBoxEditWord.StringResult,"[\\s]|[\\p{P}\\p{S}-['-]]","");//don't allow words with spaces or punctuation except ' and - in them
		for(var i=0;i<_listDictCustoms.Count;i++) {//Make sure it's not already in the custom list
