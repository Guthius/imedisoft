...
public partial class FormDiscountPlanEdit:FormODBase {
	...
	private void FormDiscountPlanEdit_Load(object sender,EventArgs e) {
		...
		for(var i=0;i<_listDefsAdjTypes.Count;i++) {
		...
		if(_countPats>10000) {//10,000 per Nathan. copied from FormInsPlan.cs
			...
			comboPatient.Items.AddList<string>(_listPatNames.ToArray());
	...
	private void checkHidden_Click(object sender,EventArgs e) {
		...
		var msgText=Lan.g(this,"Specified Discount Plan will be hidden.  It will no longer be available for assigning, but existing patients on plan will remain.");
	...
	private void butListPatients_Click(object sender,EventArgs e) {
		using var formPatientListDiscount=new FormPatientListDiscount();
		formPatientListDiscount.DiscountPlanCur=DiscountPlanCur;
		formPatientListDiscount.ListPatNames=_listPatNames;
		formPatientListDiscount.ShowDialog();
	}
	...
	private string GetSecurityLogMessage() {
		...
		var feeSchedDescriptionCur=FeeScheds.GetFirstOrDefault(x => x.FeeSchedNum==DiscountPlanCur.FeeSchedNum,true)?.Description??"";
		var adjustmentTypeCur=_listDefsAdjTypes.FirstOrDefault(x => x.DefNum==DiscountPlanCur.DefNum)?.ItemName??"";
		var feeSchedDescriptionOld=FeeScheds.GetFirstOrDefault(x => x.FeeSchedNum==_discountPlanOld.FeeSchedNum,true)?.Description??"";
		var adjustmentTypeOld=_listDefsAdjTypes.FirstOrDefault(x => x.DefNum==_discountPlanOld.DefNum)?.ItemName??"";
		var isNew=DiscountPlanCur.IsNew;
		if(isNew) {
	...
	private void butSave_Click(object sender,EventArgs e) {
		...
		var logMessage=GetSecurityLogMessage();
