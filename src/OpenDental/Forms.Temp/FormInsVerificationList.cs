...
public partial class FormInsVerificationList:FormODBase {
	...
	private List<long> _listClinicNumsVerifyClinicsFilter= [];
	///<summary>-1 and 0 represent "All".</summary>
	private List<long> _listDefNumsVerifyRegionsFilter= [-1];
	...
	private List<Userod> _listUserodsInRegionWithAssignedIns= [];
	/// <summary>This will contain all (including hidden) users with the InsPlanVerify permission</summary>
	private List<Userod> _listUserodsInRegion= [];
	private List<Def> _listDefsVerifyStatuses= [];
	...
	private void FillDisplayInfo(InsVerifyGridObject insVerifyGridObject) {
		...
		var patPlanVerify=PatPlans.GetByPatPlanNum(insVerifyGridObject.GetPatPlanNum());
		if(patPlanVerify==null) {//Should never happen, but if it does, return because it is just for display purposes.
			return;
		}
		var insSubVerify=InsSubs.GetOne(patPlanVerify.InsSubNum);
		textSubscriberID.Text=insSubVerify.SubscriberID;
		var patientSubscriberVerify=Patients.GetPat(insSubVerify.Subscriber);
	...
	private void FillInsuranceDisplay(InsPlan insPlanVerify) {
		...
		var employer=Employers.GetEmployer(insPlanVerify.EmployerNum);
		if(employer!=null) {
			textInsPlanEmployer.Text=employer.EmpName;
		}
		var carrierVerify=Carriers.GetCarrier(insPlanVerify.CarrierNum);
	...
	private void FillComboBoxes() {
		...
		for(var i=0;i<_listDefsVerifyStatuses.Count;i++) {
		...
		if(_listDefsRegion.Count!=0) {
			_listDefsRegion.RemoveAll(x => !listClinicsForUser.Any(y => y.RegionId==x.DefNum));
			listBoxVerifyRegions.Items.Add(Lan.g(this,"All"));
			for(var i = 0;i<_listDefsRegion.Count;i++) {
		...
		_listDefNumsVerifyRegionsFilter= [-1];
		FillClinicListBox(listClinicsForUser);
	}

	///<summary>Fills the listbox for clinics. Pass in a list of clinics for the current user.</summary>
	private void FillClinicListBox(List<ClinicDto> listClinicsForUser) {
		listBoxVerifyClinics.Items.Clear();
		var index=0;
		var isAllClinicsEnabled=true && !Security.CurUser.ClinicIsRestricted && !PrefC.GetBool(PrefName.EnterpriseApptList);
		if(isAllClinicsEnabled) {
			...
			listBoxVerifyClinics.Items.Add(Lan.g(this,"All"),new ClinicDto { Id=-1 });
			index++;
		}
		var isRegionSelected=!_listDefNumsVerifyRegionsFilter.Contains(0) && !_listDefNumsVerifyRegionsFilter.Contains(-1);
		for(var i=0;i<listClinicsForUser.Count;i++) {
			if(isRegionSelected && !_listDefNumsVerifyRegionsFilter.Contains(listClinicsForUser[i].RegionId??0)) {
			...
			if(_listClinicNumsVerifyClinicsFilter.Contains(listClinicsForUser[i].Id)){
		...
		if(!Security.CurUser.ClinicIsRestricted && !isRegionSelected) {//Show "Unassigned" if user is not restricted and no region is selected.
			...
			listBoxVerifyClinics.Items.Add(Lan.g(this,"Unassigned"),new ClinicDto { Id=0 });
			if(_listClinicNumsVerifyClinicsFilter.Contains(0)) {
				listBoxVerifyClinics.SetSelected(index);
			}
			index++;
		}
		if(listBoxVerifyClinics.GetListSelected<ClinicDto>().Count==0 && listBoxVerifyClinics.Items.Count>0) {//Select the first one if none are selected.
	...
	private void FillUsers() {
		...
		for(var i=0;i<_listUserodsInRegionWithAssignedIns.Count;i++) {
		...
		for(var i=0;i<_listUserodsInRegion.Count;i++) {
	...
	private void PickUser(bool isAssigning) {
		var frmUserPick=new FrmUserPick();
		frmUserPick.IsSelectionMode=true;
		//The list has hidden and visible users. Don't allow the user to pick a hidden user.
		frmUserPick.ListUserodsFiltered=_listUserodsInRegion.FindAll(x => x.IsHidden==false); 
		if(!isAssigning) {
			frmUserPick.IsPickAllAllowed=true;
		}
		...
		if(isAssigning) {//Setting the user
			_userNumAssign=frmUserPick.UserNumSelected;
			FillControls();
			return;
		}
		//Filter by user
		_userNumVerify=frmUserPick.UserNumSelected;
		FillControls();
	}

	private List<InsVerifyGridRow> GetRowsForGrid(GridOD grid) {
		var listInsVerifyGridRows=new List<InsVerifyGridRow>();
		...
		var excludePatClones=(PrefC.GetBool(PrefName.ShowFeaturePatientClone)==true) && PrefC.GetBool(PrefName.InsVerifyExcludePatientClones);
		var dateStartStandard=DateTime.Today;
		var dateEndStandard=DateTime.Today.AddDays(SIn.Int(textAppointmentScheduledDaysStandard.Text));//Don't need to add 1 because we will be getting only the date portion of this datetime.
		var dateStartMedicaid=DateTime.Today;
		var dateEndMedicaid=DateTime.Today.AddDays(SIn.Int(textAppointmentScheduledDaysMedicaid.Text));
		if(grid==gridPastDue) {
		...
		var dateLastPatEligibilityStandard=DateTime.Today.AddDays(-SIn.Int(textPatientEnrollmentDaysStandard.Text));
		var dateLastPlanBenefitsStandard=DateTime.Today.AddDays(-SIn.Int(textInsBenefitEligibilityDaysStandard.Text));
		var dateLastPatEligibilityMedicaid=DateTime.Today.AddDays(-SIn.Int(textPatientEnrollmentDaysMedicaid.Text));
		var dateLastPlanBenefitsMedicaid=DateTime.Today.AddDays(-SIn.Int(textInsBenefitEligibilityDaysMedicaid.Text));
		var insVerifyListType=new InsVerifyListType();
		...
		progressOD.ActionMain=() => {
			var listInsVerifyGridObjects=InsVerifies.GetVerifyGridList(dateStartStandard,dateEndStandard,dateLastPatEligibilityStandard,dateLastPlanBenefitsStandard,
	...
	private void FillGrid() {
		...
		var indexSelectedRow=grid.GetSelectedIndex();
		var isAssignGrid=grid.In(gridAssignStandard,gridAssignMedicaid);
		grid.BeginUpdate();
		grid.Columns.Clear();
		GridColumn col;
		col=new GridColumn(Lans.g("Type"),45);
		grid.Columns.Add(col);
		if(true) {
			col=new GridColumn(Lans.g("Clinic"),90);
			grid.Columns.Add(col);
		}
		col=new GridColumn(Lans.g("Patient"),120);
		grid.Columns.Add(col);
		col=new GridColumn(Lans.g("Appt Date Time"),130,HorizontalAlignment.Center,GridSortingStrategy.DateParse);
		grid.Columns.Add(col);
		col=new GridColumn(Lans.g("Carrier"),160);
		grid.Columns.Add(col);
		col=new GridColumn(Lans.g("Last Verified"),90,HorizontalAlignment.Center,GridSortingStrategy.DateParse);
		grid.Columns.Add(col);
		col=new GridColumn(Lans.g("Status"),110);
		grid.Columns.Add(col);
		if(!isAssignGrid) {
			col=new GridColumn(Lans.g("Status Date"),80,HorizontalAlignment.Center);
			grid.Columns.Add(col);
		}
		col=new GridColumn(Lans.g("Assigned to"),120){ IsWidthDynamic=true };
		grid.Columns.Add(col);
		grid.ListGridRows.Clear();
		for(var i=0;i<listInsVerifyGridRows.Count;i++) {
	...
	private void OnVerify(PlanToVerify planToVerifyEnum) {
		...
		var insVerifyGridObject=((InsVerifyGridObject)grid.ListGridRows[grid.GetSelectedIndex()].Tag);
		...
		var appointmentScheduledDays=textAppointmentScheduledDaysStandard.Text;
		var patientEnrollmentDays=textPatientEnrollmentDaysStandard.Text;
		var insBenefitEligibilityDays=textInsBenefitEligibilityDaysStandard.Text;
	...
	private void gridContextMenuStrip_Popup(object sender,EventArgs e) {
		...
		var verifyDescription=Lan.g(this,"Go to Insurance Plan");
		...
		var menuItemAssignUserTool=new MenuItem(Lan.g(this,"Assign to User"));
		for(var i=0;i<_listUserodsInRegion.Count;i++) {
			if(_listUserodsInRegion[i].IsHidden) {
				continue;
			}
			var menuItemAssignUserDropDown=new MenuItem(_listUserodsInRegion[i].UserName);
		...
		var menuItemVerifyStatusTool=new MenuItem(Lan.g(this,"Set Verify Status to"));
		for(var i=0;i<_listDefsVerifyStatuses.Count;i++) {
			var menuItemVerifyStatusDropDown=new MenuItem(_listDefsVerifyStatuses[i].ItemName);
	...
	private void OnOpenInsPlan() {
		...
		var patPlanNum=_insVerifyGridObjectRowSelected.GetPatPlanNum();
		if(patPlanNum==0) {				
			return;
		}
		...
		if(_insVerifyGridObjectRowSelected.InsVerifyPat!=null) {				
			...
			insPlan=InsPlans.GetPlan(_insVerifyGridObjectRowSelected.InsVerifyPlan.FKey, []);				
		}
		if(insPlan==null) {
	...
	private void gridMainRight_click(object sender,System.EventArgs e) {
		switch(_contextMenuRightClick.MenuItems.IndexOf((MenuItem)sender)) {
			case 0:
				GlobalFormOpenDental.GoToModule(EnumModuleType.Family, patNum:_insVerifyGridObjectRowSelected.GetPatNum());
				break;
			case 1:
				OnOpenInsPlan();
				break;
	...
	private void butAssignUser_Click(object sender,EventArgs e) {
		...
		var listInsVerifyGridObjects=GetSelectedInsVerifyList();
		for(var i=0;i<listInsVerifyGridObjects.Count;i++) {
	...
	private void listBoxVerifyRegions_MouseUp(object sender,MouseEventArgs e) {
		_listDefNumsVerifyRegionsFilter= [];
		for(var i=0;i<listBoxVerifyRegions.SelectedIndices.Count;i++) {
	...
	private void UpdateSelectedClinicNums() {
		_listClinicNumsVerifyClinicsFilter=listBoxVerifyClinics.GetListSelected<ClinicDto>().Select(x => x.Id).ToList();
	...
	private void comboVerifyUser_SelectionChangeCommitted(object sender,EventArgs e) {
		...
		var userod=_listUserodsInRegionWithAssignedIns.FirstOrDefault(x => x.UserName==comboVerifyUser.GetSelected<string>());
	...
	private void menuItemVerifyStatusDropDown_Click(object sender, EventArgs e) {
		var defStatus=(Def)((MenuItem)sender).Tag;
	...
	private void menuItemAssignUserDropDown_Click(object sender, EventArgs e) {
		var userod=(Userod)((MenuItem)sender).Tag;
		if(tabControl1.SelectedTab==tabVerify) {
			if(_insVerifyGridObjectRowSelected.InsVerifyPat!=null) {
	...
	private class InsVerifyGridRow {
		...
		public InsVerifyGridRow(InsVerifyGridObject insVerifyGridObject,Dictionary<long,Def> dictionaryStatusDefs,List<Userod> listUserods) {
			...
			if(insVerifyGridObject.IsPatAndInsRow()) {//If showing a consolidated row, use the PatInsVerify information, since they are same anyways.
				...
				var isPatLastAssignedNewer=insVerifyGridObject.InsVerifyPat.DateLastAssigned>=insVerifyGridObject.InsVerifyPlan.DateLastAssigned;
