...
public partial class FormEtrans277Edit:FormODBase {
	...
	private void FillGrid() {
		...
		var listClaimTrackingNumbers=_x277.GetClaimTrackingNumbers();
		//bool showInstBillType=false;
		var showServiceDateRange=false;
		for(var i=0;i<listClaimTrackingNumbers.Count;i++) {
			var stringArrayClaimInfo=_x277.GetClaimInfo(listClaimTrackingNumbers[i]);
		...
		GridColumn col;
		var width=this.Width-2*gridMain.Left-10;
		...
		for(var i=0;i<listClaimTrackingNumbers.Count;i++) {
			...
			var claimStatus="";
			var claimAmount=SIn.Decimal(stringArrayClaimInfo[9]);
