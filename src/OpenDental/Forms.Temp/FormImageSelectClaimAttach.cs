...
public partial class FormImageSelectClaimAttach:FormODBase {
	...
	private void FillGrid(){
		...
		var col=new GridColumn(Lan.g(this,"Date"),100);
		...
		for(var i=0;i<_documentArray.Length;i++){
			row=new GridRow();
	...
	private void ImportDocument(string path) {
		var defNumCategory=Defs.GetImageCat(ImageCategorySpecial.C);
		if(defNumCategory==0) {
	...
	private void SaveAttachment(Document document){
		...
		if(!ImageHelper.HasImageExtension(document.FileName)){
			if(CanAttachTxt && Path.GetExtension(document.FileName).ToLower()==".txt") {
				...
				var listAllowedFormats=new List<string> { "images" };
				if(CanAttachTxt) {
		...
		var oldPath=ODFileUtils.CombinePaths(patientfolder,document.FileName);
		if(!File.Exists(oldPath)) {
			MsgBox.Show(this,"File not found: "+oldPath);
			return;
		}
		var random=new Random();
		var newName=DateTime.Now.ToString("yyyyMMdd")+"_"+DateTime.Now.TimeOfDay.Ticks+random.Next(1000)
	...
	private void timerMonitorClipboard_Tick(object sender,EventArgs e) {
		...
		var bitmapClipboard=GetImageFromClipboard(isSilent:true,doShowProgressBar:false);
	...
	private void butImport_Click(object sender,EventArgs e) {
		...
		var selectedFilePath=importFilePath;
		try {
			var bitmap=(Bitmap)Image.FromFile(selectedFilePath);
			bitmap.Dispose();
	...
	private void butPasteImage_Click(object sender,EventArgs e) {
		var bitmapPasted=GetImageFromClipboard();
