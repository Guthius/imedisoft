...
public partial class FormCreditCardManage : FormODBase
{
    ...
    private void FillGrid()
    {
        ...
        if (PrefC.HasOnlinePaymentEnabled(out var programNameForPayments))
        {
        ...
        {
            var creditCard = _listCreditCards[i];
            ...
            row = new GridRow();
            var strCardNum = creditCard.CCNumberMasked;
            if (Regex.IsMatch(strCardNum, "^\\d{12}(\\d{0,7})"))
            {
                //Credit cards can have a minimum of 12 digits, maximum of 19
                var idxLast4Digits = strCardNum.Length - 4;
            ...
            if (true)
            {
                var abbrCardClinic = Clinics.GetAbbr(creditCard.ClinicNum);
    ...
    {
        using var formCreditCardEdit = new FormCreditCardEdit(_patient);
        formCreditCardEdit.CreditCardCur = (CreditCard) gridMain.ListGridRows[e.Row].Tag;
        formCreditCardEdit.ShowDialog();
        FillGrid();
        if (gridMain.ListGridRows.Count > 0)
        {
            //could have deleted the only CC, make sure there's at least one row
            var idxCreditCard = gridMain.ListGridRows.OfType<GridRow>().ToList().FindIndex(x => ((CreditCard) x.Tag).CreditCardNum == formCreditCardEdit.CreditCardCur.CreditCardNum);
            gridMain.SetSelected(Math.Max(0, idxCreditCard));
        }
    }
    ...
    private void butReuseCard_Click(object sender, EventArgs e)
    {
        ...
        if (selectedIndex == -1)
        {
            //If None Selected call butAddCard
            MsgBox.Show("Please select a credit card to reuse.");
            return;
        }

        var creditCard = (CreditCard) gridMain.ListGridRows[selectedIndex].Tag;
        var isRecurring = CreditCards.IsRecurring(creditCard);
        FormCreditCardEdit formCreditCardEdit;
        if (isRecurring)
        {
            //If selected HAS recurring charge, assign to FormCreditCardEdit.CreditCardCur, and create new card
        ...
        {
            //If selected DOES NOT have recurring charge, act like the cell was double clicked
    ...
    private void butAdd_Click(object sender, EventArgs e)
    {
        ...
        var hasEdgeExpress = false;
        var hasXCharge = false;
        var hasPayConnect = false;
        var hasPaySimple = false;
        var dictionaryEnabledProcessors = new Dictionary<string, int>();
        var idx = 0;
        ...
        {
            var hasPreventCreditCardAdd = SIn.Bool(ProgramProperties.GetPropVal(Programs.GetCur(programName).ProgramNum, preventPropName, Clinics.ClinicNum));
            var errMsg = "";
            var isPayProgEnabled = Programs.IsEnabled(programName) && !hasPreventCreditCardAdd && Programs.IsEnabledByHq(programName, out errMsg);
        ...
        {
            var listCreditCardProcessors = dictionaryEnabledProcessors.Select(x => x.Key).ToList();
        ...
        else if (dictionaryEnabledProcessors.Count == 0)
        {
            //not storing CC numbers and both PayConnect and X-Charge are disabled
            var errMsg = stringBuilder.ToString();
        ...
        if (hasEdgeExpress)
        {
            var programNum = Programs.GetProgramNum(ProgramName.EdgeExpress);
            ...
            var hasCredentials = ProgramProperties.GetPropVal(programNum, ProgramProperties.PropertyDescs.EdgeExpress.XWebID, Clinics.ClinicNum) != ""
            ...
            var doUseRCM = MsgBox.Show("EdgeExpress", MsgBoxButtons.YesNo, "Would you like to add this card via the terminal?\r\n"
            ...
            try
            {
                if (doUseRCM)
                {
                    var rcmResponse = EdgeExpress.RCM.CreateAlias(_patient, Clinics.ClinicNum, isWebPayment: false);
                ...
                {
                    var xWebResponse = EdgeExpress.CNP.GetUrlForCreditCardAlias(_patient.PatNum, CreditCardSource.EdgeExpressCNP, isWebPayment: false, doUseCurrentClinicNum: true);
                    Cursor = Cursors.Default;
                    var formWebBrowser = new FormWebBrowser(xWebResponse.HpfUrl);
                    formWebBrowser.ShowDialog();
                    //Process without creating a new payment.
                    var xWebResponseProcessed = EdgeExpress.CNP.ProcessTransaction(xWebResponse, doCreatePayment: false);
            ...
            {
                Cursor = Cursors.Default;
                FriendlyException.Show(Lans.g("There was a problem adding the credit card.  Please try again."), ex);
            }
        }
        ...
        if (hasXCharge)
        {
            ...
            var xPassword = ProgramProperties.GetPropVal(program.ProgramNum, "Password", Clinics.ClinicNum).Trim();
            ...
            while (!File.Exists(path) || string.IsNullOrEmpty(xPassword) || string.IsNullOrEmpty(xUsername))
            {
                ...
                using var formXchargeSetup = new FormXchargeSetup(); //refreshes program and program property caches on OK click
            ...
            xPassword = MiscUtils.Decrypt(xPassword);
            ...
            {
                ShowError(
                    "Could not delete XResult.txt file. " +
            ...
            try
            {
                ...
                if (insertCard && xChargeToken != "")
                {
                    creditCard = CreditCards.CreateNewOpenEdgeCard(_patient.PatNum, Clinics.ClinicNum, xChargeToken, exp.Substring(0, 2), exp.Substring(2, 2), accountMasked, CreditCardSource.XServer);
            ...
            {
                const string errorMessage = "There was a problem adding the credit card. Please try again.";
        ...
        if (hasPayConnect)
        {
            ...
            {
                using var formPayConnect = new FormPayConnect(Clinics.ClinicNum, _patient, (decimal) 0.01, creditCard, isAddingCard: true);
                
                formPayConnect.ShowDialog();
            }
            else if (payconnectVersion == "2")
            {
                using var formPayConnect2IFrame = new FormPayConnect2iFrame(Clinics.ClinicNum, isAddingCard: true);
                
                formPayConnect2IFrame.ShowDialog();
                
                var response = formPayConnect2IFrame.GetResponse();
                if (response is null)
                {
                    return;
                }

                if (response.ResponseType == PayConnect2.ResponseType.IFrame && !response.iFrameResponse.Response.CardToken.IsNullOrEmpty())
                {
                    var cardData = response.iFrameResponse.Response;
                    var expiryYear = int.Parse(cardData.ExpiryYear);
                    var expiryMonth = int.Parse(cardData.ExpiryMonth);
                    

                    var newCreditCard = new CreditCard();
                    newCreditCard.IsNew = true;
                    newCreditCard.PatNum = _patient.PatNum;
                    var listCreditCardsItemOrderCount = CreditCards.RefreshAll(_patient.PatNum);
        ...
        if (hasPaySimple)
        {
            using var formPaySimple = new FormPaySimple(Clinics.ClinicNum, _patient, (decimal) 0.01, creditCard, isAddingCard: true);
        ...
        {
            gridMain.SetSelected(gridMain.ListGridRows.Count - 1);
        }
    }
    ...
    private void ButtonMoveTo_Click(object sender, EventArgs e)
    {
        ...
        {
            ShowError("Cannot move a credit card with active recurring charges. Please clear recurring charges before moving the card to another patient.");
            return;
        }

        if (!ConfirmOk("Move this credit card information to a different patient account?"))
        {
            return;
        }
        ...
        var selectedIndex = gridMain.GetSelectedIndex();
        var creditCard = _listCreditCards[selectedIndex];
        var patNum = creditCard.PatNum;
        
        creditCard.PatNum = frmPatientSelect.PatNumSelected;
        creditCard.ItemOrder = CreditCards.GetMaxItemOrderForPat(frmPatientSelect.PatNumSelected) + 1;
        CreditCards.Update(creditCard);
        ...
        SecurityLogs.MakeLogEntry(EnumPermType.CreditCardMove, patNum, $"Credit card moved to PatNum: {frmPatientSelect.PatNumSelected}");
        SecurityLogs.MakeLogEntry(EnumPermType.CreditCardMove, frmPatientSelect.PatNumSelected, $"Credit card moved from PatNum: {patNum}");
    }
    ...
    private void butUp_Click(object sender, EventArgs e)
    {
        ...
        var idxOld = _listCreditCards[selectedIndex].ItemOrder;
        for (var i = 0; i < _listCreditCards.Count; i++)
        {
            if (_listCreditCards[i].ItemOrder == idxOld)
            {
                var creditCardOld = _listCreditCards[i];
                var creditCardNew = _listCreditCards[i - 1];
