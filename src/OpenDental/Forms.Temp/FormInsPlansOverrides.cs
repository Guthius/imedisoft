...
public partial class FormInsPlansOverrides:FormODBase {
	...
	private void FillGrid(bool isGetAll=false){
		...
		var listInsPlanPreferencesNoBillIns=InsPlanPreferences.GetManyForPlanNums(_codeNum,InsPlanPrefFKeyType.ProcCodeNoBillIns,listPlanNums);
		for(var i=0;i<_table.Rows.Count;i++) {
			row=new GridRow();
			...
			var noBillInsOverride="";
			var insPlanPreference=listInsPlanPreferencesNoBillIns.Find(x => x.PlanNum==SIn.Long(_table.Rows[i]["PlanNum"].ToString()));
	...
	private void butDelete_Click(object sender,EventArgs e) {
		var listPlanNums=gridMain.SelectedTags<long>();//Get list of all the selected insurance PlanNums
		var numSelectedPlans=listPlanNums.Count;
		if(numSelectedPlans==0) { 
			MsgBox.Show(this,"Please select insurance plan(s) first.");
			return;
		}
		var message=$"{Lan.g(this,"You are about to delete the 'Do not usually bill to Ins' override for")} {numSelectedPlans} {Lan.g(this,"insurance plan(s). Are you sure?")}";
	...
	private void UpsertNoBillInsOverrides(NoBillInsOverride noBillInsOverride) { 
		...
		var message=Lan.g(this,$"You are about to set the 'Do not usually bill to Ins' override to '{noBillInsOverride.GetDescription()}' for")+
