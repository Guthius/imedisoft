...
public partial class FormEtrans835ClaimEdit:FormODBase {
	...
	private void FillClaimAdjustments() {
		...
		var widthCol=gridClaimAdjustments.Width-10-colWidthReason-colWidthAdjAmt;
		...
		_sumClaimAdjAmt=0;
		for(var i=0;i<_hx835_Claim.ListClaimAdjustments.Count;i++) {
			var hx835_Adj=_hx835_Claim.ListClaimAdjustments[i];
			var row=new GridRow();
			row.Tag=hx835_Adj;
	...
	private void FillProcedureBreakdown() {
		...
		var col=new GridColumn("ProcDescript",80,HorizontalAlignment.Left);
		...
		for(var i=0;i<_hx835_Claim.ListProcs.Count;i++) {
			...
			var hx835_Proc=_hx835_Claim.ListProcs[i];
			var row=new GridRow();
			row.Tag=hx835_Proc;
			...
			if(ProcedureCodes.IsValidCode(hx835_Proc.ProcCodeAdjudicated)) {
				var procedureCode=ProcedureCodes.GetProcCode(hx835_Proc.ProcCodeAdjudicated);
	...
	private void FillAdjudicationInfo() {
		...
		for(var i=0;i<_hx835_Claim.ListAdjudicationInfo.Count;i++) {
			var hx835_Info=_hx835_Claim.ListAdjudicationInfo[i];
			var row=new GridRow();
			row.Tag=hx835_Info;
	...
	private void FillSupplementalInfo() {
		...
		var widthCol=gridSupplementalInfo.Width-10-colWidthAmt;
		...
		for(var i=0;i<_hx835_Claim.ListSupplementalInfo.Count;i++) {
			var hx835_info=_hx835_Claim.ListSupplementalInfo[i];
			var row=new GridRow();
			row.Tag=hx835_info;
	...
	private void gridAdjudicationInfo_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		var hx835_Info=(Hx835_Info)gridAdjudicationInfo.ListGridRows[e.Row].Tag;
		var msgBoxCopyPaste=new MsgBoxCopyPaste(hx835_Info.FieldName+"\r\n"+hx835_Info.FieldValue);
		msgBoxCopyPaste.Show(this);//This window is just used to display information.
	}

	private void gridSupplementalInfo_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		var hx835_Info=(Hx835_Info)gridSupplementalInfo.ListGridRows[e.Row].Tag;
		var msgBoxCopyPaste=new MsgBoxCopyPaste(hx835_Info.FieldName+"\r\n"+hx835_Info.FieldValue);
		msgBoxCopyPaste.Show(this);//This window is just used to display information.
	}
	...
	private void butPrint_Click(object sender,EventArgs e) {
		...
		x835.ListClaimsPaid=
		[
			_hx835_Claim //Only print the current claim.
		];
