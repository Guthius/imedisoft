...
public partial class FormProdSelect:FormODBase {
	...
	public FormProdSelect(DynamicPaymentPlanModuleData dynamicPaymentPlanModuleData) {
		InitializeComponent();

		_patient=dynamicPaymentPlanModuleData.Patient;
		_listAccountEntries=PayPlanEdit.GetAccountEntriesUnattachedToDynamicPaymentPlan(dynamicPaymentPlanModuleData);
	}
	...
	private void FillGrid() {
		var tran=gridProduction.TranslationName;
		...
		var listProdTypes=new List<Type> { typeof(Adjustment),typeof(Procedure) };
		for(var i=0;i<_listAccountEntries.Count;i++) {
			...
			if(_listAccountEntries[i].GetType()==typeof(Procedure)) {
				var procedure=(Procedure)_listAccountEntries[i].Tag;
				...
				var adjustment=(Adjustment)_listAccountEntries[i].Tag;
