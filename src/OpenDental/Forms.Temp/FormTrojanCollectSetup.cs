...
public partial class FormTrojanCollectSetup : FormODBase {
	...
	private void ButBrowse_Click(object sender,EventArgs e) {
		var selectedPath=textExportFolder.Text;
		if(string.IsNullOrEmpty(selectedPath) || !Directory.Exists(selectedPath)) {
			selectedPath="C:\\";
		}
		using(var folderBrowserDialog=new FolderBrowserDialog { SelectedPath=selectedPath,Description=Lan.g(this,"Select Export Folder Location") }) {
			try {
	...
	private bool Validation() {
		...
		else if(!Programs.IsEnabledByHq(ProgramName.TrojanExpressCollect,out var err)) {
	...
	private void butSave_Click(object sender,EventArgs e) {
		...
		var password=CDT.Class1.TryEncrypt(textPassword.Text);
