...
public partial class FormClaimPayBatch:FormODBase {
	...
	private List<ClaimPaySplit> _listDetachedClaims= [];
	...
	private void FillGrids(bool doRefreshOutstandingClaims=true){
		...
		var listClaimNumsToUpdate=new List<long>();
		var listPaymentRows=new List<int>();
		for(var i=0;i<_listClaimsAttached.Count;i++) {
		...
		var scrollValue=gridOut.ScrollValue;
		var selectedIdx=gridOut.GetSelectedIndex();
		if(doRefreshOutstandingClaims || _listClaimsOutstanding==null) {
			...
			for(var i=0;i<_listDetachedClaims.Count;i++) {
		...
		for(var i=0;i<_listClaimsOutstanding.Count;i++) {
	...
	private void RunAgingForClaims() {
		var listGuarNums = Patients.GetGuarantorsForPatNums(_listClaimsAttached.Select(x => x.PatNum).ToList());
		...
		for(var i=0;i<listGuarNums.Count;i++) {
	...
	private void butClaimPayEdit_Click(object sender,EventArgs e) {
		using var formClaimPayEdit=new FormClaimPayEdit(_claimPayment);
	...
	private void butAttach_Click(object sender,EventArgs e) {
		...
		var hasClaimNoValidReceivedPayments=false;
		var paymentRow=gridAttached.ListGridRows.Count;//1-indexed
		var listClaimPaySplits=gridOut.SelectedTags<ClaimPaySplit>();
		...
		//We do not refresh the lower grid here because we had complaints it's annoyingly slow for very large offices.
		//The code above does verify that the claimprocs are not already attached to another payment,
		//But the query does explicitly skip any that are attached to another payment.
		//In that case, the Attach button wouldn't work, and user could click Refresh.
		//In a thought experiment, we came up with an edge case that we will ignore:
		//A single claim might have both received and unreceived claimprocs.
		//The attach button would make the UI not match the db. Refresh would fix.
	}
	...
	private void butDetach_Click(object sender,EventArgs e) {
		...
		var listClaimPaySplits=gridAttached.SelectedTags<ClaimPaySplit>();
		...
		for(var i=0;i<gridAttached.ListGridRows.Count;i++) {
			var claimPaySplitAttached=(ClaimPaySplit)gridAttached.ListGridRows[i].Tag;
	...
	private void gridAttached_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		...
		var claimPaySplit=(ClaimPaySplit)gridAttached.ListGridRows[e.Row].Tag;
		var claim=Claims.GetClaim(claimPaySplit.ClaimNum);
		if(claim==null) {
			MsgBox.Show(this,"The claim has been deleted.");
			FillGrids();
			return;
		}
		using var formClaimEdit=new FormClaimEdit(claim,Patients.GetPat(claim.PatNum),Patients.GetFamily(claim.PatNum));
	...
	private void butUp_Click(object sender,EventArgs e) {
		...
		var selectedIndices=new int[gridAttached.SelectedIndices.Length];//remember the selected rows so that we can reselect them
		for(var i=0;i<gridAttached.SelectedIndices.Length;i++) {
		...
		for(var i=0;i<selectedIndices.Length;i++) {
			var claimPaySplitAbove=(ClaimPaySplit)gridAttached.ListGridRows[selectedIndices[i]-1].Tag;
			var claimPaySplitSelected=(ClaimPaySplit)gridAttached.ListGridRows[selectedIndices[i]].Tag;
	...
	private void butDown_Click(object sender,EventArgs e) {
		...
		var selectedIndices=new int[gridAttached.SelectedIndices.Length];
		for(var i=0;i<gridAttached.SelectedIndices.Length;i++) {
			selectedIndices[i]=gridAttached.SelectedIndices[i];
		}
		...
		for(var i=selectedIndices.Length-1;i>=0;i--) {//go backwards
			var claimPaySplitBelow=(ClaimPaySplit)gridAttached.ListGridRows[selectedIndices[i]+1].Tag;
			var claimPaySplitSelected=(ClaimPaySplit)gridAttached.ListGridRows[selectedIndices[i]].Tag;
	...
	private void gridOut_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		...
		var claimPaySplit=(ClaimPaySplit)gridOut.ListGridRows[e.Row].Tag;
		var claim=Claims.GetClaim(claimPaySplit.ClaimNum);
		if(claim==null) {
			MsgBox.Show(this,"The claim has been deleted.");
			FillGrids();
	...
	private void menuItemGotoAccount_Click(object sender,EventArgs e) {
		...
		var claimPaySplitSelected=gridAttached.SelectedTag<ClaimPaySplit>();
		GotoPatNum=claimPaySplitSelected.PatNum;
		GotoClaimNum=claimPaySplitSelected.ClaimNum;
		var pat=Patients.GetPat(GotoPatNum);
		GlobalFormOpenDental.PatientSelected(pat,isRefreshCurModule:false);
		GlobalFormOpenDental.GoToModule(EnumModuleType.Account, claimNum:GotoClaimNum);
	}
	...
	private void menuItemGotoOut_Click(object sender,EventArgs e) {
		...
		var claimPaySplitSelected=gridOut.SelectedTag<ClaimPaySplit>();
		GotoPatNum=claimPaySplitSelected.PatNum;
		GotoClaimNum=claimPaySplitSelected.ClaimNum;
		var pat=Patients.GetPat(GotoPatNum);
		GlobalFormOpenDental.PatientSelected(pat,isRefreshCurModule:false);
	...
	private void ShowSecondaryClaims() {
		var listClaimPaySplitsAttached=(List<ClaimPaySplit>)gridAttached.Tag;
		var tableSecondaryClaims=Claims.GetSecondaryClaims(listClaimPaySplitsAttached);
		if(tableSecondaryClaims.Rows.Count==0) {
			return;
		}
		var message="Some of the payments have secondary claims: \r\n"
		            +"Date of Service | PatNum | Patient Name";
		for(var i=0;i<tableSecondaryClaims.Rows.Count;i++) {
			//claimProc=secondaryClaims[i];
			message+="\r\n"+SIn.Date(tableSecondaryClaims.Rows[i]["ProcDate"].ToString()).ToShortDateString()
			               +" | "+tableSecondaryClaims.Rows[i]["PatNum"]
		...
		using var msgBox=new MsgBoxCopyPaste(message);
		msgBox.ShowDialog();
	}
	...
	private bool IsAmountAndTotalEqual(bool isSilent=false) {
		...
		var claimPayment=ClaimPayments.GetOne(_claimPayment.ClaimPaymentNum);
		double amountRaw;
		...
		var totalRaw=listClaimPaySplitsAttached.Sum(x => x.InsPayAmt);//textTotal is filled like this every time FillGrid is invoked.
		...
		var digits=NumberFormatInfo.CurrentInfo.NumberDecimalDigits;
		var amountRounded=Math.Round(amountRaw,digits,MidpointRounding.AwayFromZero);
		var totalRounded=Math.Round(totalRaw,digits,MidpointRounding.AwayFromZero);
	...
	private void butViewEra_Click(object sender,EventArgs e) {//Only clickable when IsFromClaim is true
		var listClaimNums=_listClaimsAttached.Select(x => x.ClaimNum).Distinct().ToList();
