...
public partial class FormHieSetup:FormODBase {
	private long _clinicNumPreviouslySelected;
	...
	private void AddNeededHieClinic(long clinicNum) {
		if(!_listHieClinics.Any(x => x.ClinicNum==clinicNum)) {
			var hieClinicHQ=_listHieClinics.FirstOrDefault(x => x.ClinicNum==0);
			if(hieClinicHQ==null) {
	...
	private void FillUIFromFields() {
		var hieClinic=_listHieClinics.FirstOrDefault(x => x.ClinicNum==comboClinic.ClinicNumSelected);
		...
		dateTimeOfExportCCD.Value=DateTime.Today+hieClinic.TimeOfDayExportCCD;
	}
	...
	private void FillFieldsFromUI(long clinicNum) {
		...
		var hieCarrierFlagSelected=HieCarrierFlags.AllCarriers;
