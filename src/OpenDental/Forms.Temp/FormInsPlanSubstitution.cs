...
public partial class FormInsPlanSubstitution:FormODBase {
	...
	private List<InsPlanSubstitution> _listInsPlanSubstitution= [];
	...
	private void FormInsPlanSubstitution_Load(object sender,EventArgs e) {
		...
		var listSubstitutionLinks=SubstitutionLinks.GetAllForPlans(_insPlan.PlanNum);
		...
		_listSubConditions= [];
		for(var i=0;i<Enum.GetNames(typeof(SubstitutionCondition)).Length;i++) {
	...
	private List<InsPlanSubstitution> FillInsPlanSubList(List<ProcedureCode> listProcedureCodes,List<SubstitutionLink> listSubstitutionLinks) {
		var listInsPlanSubstitution=new List<InsPlanSubstitution>();
		var listProcedureCodesSubstitution=listProcedureCodes.FindAll(x => !string.IsNullOrEmpty(x.SubstitutionCode));
		for(var i=0;i<listProcedureCodesSubstitution.Count;i++) {
		...
		for(var i=0;i<listSubstitutionLinks.Count;i++) {
			var procedureCode=listProcedureCodes.FirstOrDefault(x => x.CodeNum==listSubstitutionLinks[i].CodeNum);
	...
	private void FillGridMain() {
		var insPlanSubSelected=gridMain.SelectedTag<InsPlanSubstitution>();
		...
		var listInsPlanSubstitution=_listInsPlanSubstitution.OrderBy(x => x.ProcCode.ProcCode).ToList();
		for(var i=0;i<listInsPlanSubstitution.Count;i++) {
		...
		if(insPlanSubSelected!=null) {
			var index=gridMain.ListGridRows.FindIndex(x => (x.Tag as InsPlanSubstitution).ProcCode.CodeNum==insPlanSubSelected.ProcCode.CodeNum);
			if(index > -1) {
	...
	private void AddRow(GridOD grid,InsPlanSubstitution insPlanSubstitution) {
		...
		var enumSubstCondition=insPlanSubstitution.SubCondition.ToString();
		var subCode=insPlanSubstitution.ProcCode.SubstitutionCode;
		var procedureCodeSubstitution=_listProcedureCodes.FirstOrDefault(x => x.ProcCode==insPlanSubstitution.ProcCode.SubstitutionCode);
		var subCodeDescript="";
		...
		if(insPlanSubstitution.SubLink!=null) {
			...
			if(!string.IsNullOrEmpty(insPlanSubstitution.SubLink.SubstitutionCode)) {
				var procedureCodeSubstitution2=_listProcedureCodes.FirstOrDefault(x=>x.ProcCode==insPlanSubstitution.SubLink.SubstitutionCode);
		...
		var cell=new GridCell(Lan.g("enumSubstitutionCondition",enumSubstCondition));
	...
	private void ButAdd_Click(object sender, EventArgs e){
		...
		var procSelected=_listProcedureCodes.FirstOrDefault(x=>x.CodeNum==formProcCodes.CodeNumSelected);
		...
		var insPlanSubstitution=new InsPlanSubstitution(procSelected,substitutionLink);
		_listInsPlanSubstitution.Add(insPlanSubstitution);
		FillGridMain();
		//Set the substitution link we just added as selected.
		var index=gridMain.ListGridRows.ToList().FindIndex(x => (x.Tag as InsPlanSubstitution)==insPlanSubstitution);
		if(index>-1) {
	...
	private void gridMain_CellSelectionCommitted(object sender,ODGridClickEventArgs e) {
		...
		//Get the grid tag
		var insPlanSubstitutionOld=gridMain.SelectedTag<InsPlanSubstitution>();
		if(insPlanSubstitutionOld==null) {
			return;
		}
		//Get the selected substitution condition.
		var substitutionConditionSelected=(SubstitutionCondition)_listSubConditions.IndexOf(gridMain.ListGridRows[e.Row].Cells[e.Col].Text);
		...
		if(insPlanSubstitutionOld.SubLink==null) {
			...
			var insPlanSubstitutionNew=new InsPlanSubstitution(insPlanSubstitutionOld.ProcCode,substitutionLink);
	...
	private void butDelete_Click(object sender,EventArgs e) {
		...
		var insPlanSubstitution=gridMain.SelectedTag<InsPlanSubstitution>();
		...
		var msgText=(Lans.g("Delete the selected insurance specific substitution code for procedure ")+"\""+insPlanSubstitution.ProcCode.ProcCode+"\"?");
		if(_listInsPlanSubstitution.Where(x=>x.ProcCode.CodeNum==insPlanSubstitution.ProcCode.CodeNum && x.SubLink!=null).Count()<2 && insPlanSubstitution.ProcCode.SubstitutionCode!="") {
			msgText+="\r\n"+(Lans.g("Deleting the insurance specific substitution code will default to the global substitution code for this procedure")+".");
		}
	...
	private void butSave_Click(object sender,EventArgs e) {
		var msgText=Lan.g(this,"You have chosen to exclude all substitution codes.  "
		...
		var areAllGlobalSubsOverriddenWithNever=true;
		var listCodeNums=_listInsPlanSubstitution.DistinctBy(x=>x.ProcCode.CodeNum).Select(x=>x.ProcCode.CodeNum).ToList();
		for(var i=0;i<listCodeNums.Count;i++){
			var listInsPlanSubstitution=_listInsPlanSubstitution.FindAll(x=>x.ProcCode.CodeNum==listCodeNums[i]);
		...
		if(InsPlanSubstitution.HasDuplicates(_listInsPlanSubstitution)) {
			MsgBox.Show(Lans.g("You have made a duplicate substitutution code and need to fix this before continuing."));
			return;
		}
		var listSubstitutionLinks=_listInsPlanSubstitution.Where(x=>x.SubLink!=null).Select(x=>x.SubLink).ToList();
