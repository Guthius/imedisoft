...
{
    private OpenGLWinFormsControl.PixelFormatValue[] _pixelFormatValueArray = [];
    private ToothChartDirectX.DirectXDeviceFormat[] _directXDeviceFormatArray = [];
    private int _formatNumSelected;
    ...
    private void FormGraphics_Load(object sender, EventArgs e)
    {
        ...
        {
            //Remote computer has not updated yet. 
    ...
    private void FillGrid()
    {
        ...
        if (this.radioDirectXChart.Checked)
        {
            ...
            var col = new GridColumn(Lan.g(this, "FormatNum"), 80);
            ...
            for (var i = 0; i < _directXDeviceFormatArray.Length; i++)
            {
                var row = new GridRow();
        ...
        else if (this.radioOpenGLChart.Checked)
        {
            ...
            gridFormats.Columns.Add(col);
            gridFormats.EndUpdate();
            for (var i = 0; i < _pixelFormatValueArray.Length; i++)
            {
                var row = new GridRow();
    ...
    private void RefreshFormats()
    {
        ...
        {
            using var openGLWinFormsControl = new OpenGLWinFormsControl();
            //Get raw formats.
            var rawformats = OpenGLWinFormsControl.GetPixelFormats(openGLWinFormsControl.GetHdc());
    ...
    {
        var formatNum = Convert.ToInt32(gridFormats.ListGridRows[e.Row].Cells[0].Text);
    ...
    {
        var directXDeviceFormatArray = ToothChartDirectX.GetStandardDeviceFormats();
        for (var i = 0; i < directXDeviceFormatArray.Length; i++)
        {
    ...
    {
        using var toothChartWrapperTest = new ToothChartWrapper();
        ...
        try
        {
            using var bitmap = toothChartWrapperTest.GetBitmap();
        }
        catch
        {
            callingForm.Controls.Remove(toothChartWrapperTest);
    ...
    {
        var isChanged = Prefs.UpdateBool(PrefName.DirectX11ToothChartUseIfAvail, radioDirectX11Use.Checked);
        ...
        else
        {
            ...
            try
            {
                ...
                {
                    throw new Exception(Lan.g(this, "Could not initialize pixel format ") + _formatNumSelected + ".");
                }
            }
            catch (Exception ex)
            {
