...
public partial class FormSecureEmailSetup:FormODBase {
	...
	private List<EmailPlatform> _listEmailPlatforms= [];
	...
	private void FormMassEmailSetup_Load(object sender,EventArgs e) {
		...
		//Disable all controls except web browser if user does not have EServicesSetup permission.
		if(!Security.IsAuthorized(EnumPermType.EServicesSetup,suppressMessage:true)) {
			DisableAllExcept(webBrowser);
		}
		var secureEmailUrl=_signupOut?.EServices.FirstOrDefault(x => x.EService==eServiceCode.EmailSecureAccess)?.HostedUrl;
	...
	private void FillNoClinics() {
		var isSignedUp=Clinics.IsSecureEmailSignedUp(0);
		var isEnabled=Clinics.IsSecureEmailEnabled(0);
		var emailPlatformDefault=SIn.Enum<EmailPlatform>(ClinicPrefs.GetPrefValue(PrefName.EmailDefaultSendPlatform,0),true);
		var emailStatementsDefault=SIn.Enum<EmailPlatform>(ClinicPrefs.GetPrefValue(PrefName.EmailStatementsSecure,0),true);
	...
	private void checkEnabled_Click(object sender,EventArgs e) {
		var status=PrefC.GetEnum<HostedEmailStatus>(PrefName.EmailSecureStatus);
	...
	private void comboPlatform_SelectionChangeCommitted(object sender,EventArgs e) {
		var emailPlatform=(EmailPlatform)comboPlatform.SelectedItem;
	...
	private void comboStatements_SelectionChangeCommitted(object sender,EventArgs e) {
		var emailPlatformStatements=(EmailPlatform)comboStatements.SelectedItem;
		Prefs.UpdateString(PrefName.EmailStatementsSecure,emailPlatformStatements.ToString());
		DataValid.SetInvalid(InvalidType.Prefs);
	}

	private void FillClinicsGrid() {
		var listClinics=Clinics.GetForUserod(Security.CurUser);//Intentionally does not include clinic0
		...
		var clinicNumDefault=PrefC.GetLong(PrefName.EmailSecureDefaultClinic);
		for(var i=0;i<listClinics.Count();i++) {
			var row=new GridRow();
			var clinic=listClinics[i];
			var hasCredentials=Clinics.HasEmailHostingCredentials(clinic.Id);
			var isSecureEmailActivated=Clinics.IsSecureEmailSignedUp(clinic.Id);
			var isSecureEmailEnabled=Clinics.IsSecureEmailEnabled(clinic.Id);
			var emailPlatformDefault=SIn.Enum<EmailPlatform>(ClinicPrefs.GetPrefValue(PrefName.EmailDefaultSendPlatform,clinic.Id),true);
			var emailPlatformStatements=SIn.Enum<EmailPlatform>(ClinicPrefs.GetPrefValue(PrefName.EmailStatementsSecure,clinic.Id),true); //throws error invalid pref name
			row.Cells.Add(Clinics.GetAbbr(clinic.Id,listClinics));
			AddSignupCell(row,hasCredentials,isSecureEmailActivated,isSecureEmailEnabled,new EventHandler((_,_) => SignupSecureEmail(clinic.Id)));
	...
	private void UpdateControlVisibility() {
		...
		var allowEdit=Security.IsAuthorized(EnumPermType.EServicesSetup,true);
		var isSignedUpClinic0=Clinics.IsSecureEmailSignedUp(0);
		var areAnyClinicsSignedUp=GetClinicNums().Any(x => Clinics.IsSecureEmailSignedUp(x));
	...
	private List<long> GetClinicNums() {
		if(true) {
			return gridClinics.GetTags<ClinicDto>().Select(x => x.Id).ToList();
		}
		else {
			return [0];
		}
	...
	private void gridClinics_CellClick(object sender,ODGridClickEventArgs e) {
		var clinic=gridClinics.SelectedTag<ClinicDto>();
		var col=gridClinics.Columns[e.Col];
	...
	private void gridClinics_CellSelectionCommitted(object sender,ODGridClickEventArgs e) {
		...
		var index=gridClinics.ListGridRows[e.Row].Cells[e.Col].ComboSelectedIndex;
		if(!index.Between(0,listEmailPlaforms.Count-1)) {
			return;
		}
		var emailPlatform=listEmailPlaforms[index];
		long clinicNum=0;
		if (gridClinics.SelectedTag<ClinicDto>()!=null) {
			clinicNum = gridClinics.SelectedTag<ClinicDto>().Id;
		}
		if(clinicNum==0) {
			Prefs.UpdateString(PrefName.EmailDefaultSendPlatform,emailPlatform.ToString());
			DataValid.SetInvalid(InvalidType.Prefs);
	...
	private void ToggleClinicEnabled(long clinicNum) {
		var status=(HostedEmailStatus)ClinicPrefs.GetInt(PrefName.EmailSecureStatus,clinicNum);
		status=EnumTools.ToggleFlag(status,HostedEmailStatus.Enabled);
	...
	private void InitDefaultClinic(long clinicNum=-1) {
		...
		if(clinicNum==-1) {
			clinicNum=0;
			if (Clinics.GetFirstOrDefault(x => Clinics.IsSecureEmailSignedUp(x.Id))!=null) {
				clinicNum = Clinics.GetFirstOrDefault(x => Clinics.IsSecureEmailSignedUp(x.Id)).Id;
			}
		}
	...
	private void SignupSecureEmail(long clinicNumClicked) {
		...
		var listClinicPrefsOld=ClinicPrefs.GetPrefAllClinics(PrefName.EmailSecureStatus,true);
		if(WebServiceMainHQProxy.GetWebServiceMainHQInstance() is WebServiceMainHQMockDemo webService) {
			...
			using var formSignup=new FormEServicesSignup(_signupOut);
	...
	private void SignUpClinicZero(List<ClinicPref> listClinicPrefsOld) {
		var signUpOutStatus=_signupOut?.EServices?.Find(x=>x.ClinicNum==0 && x.EServiceCodeInt==(int)eServiceCode.EmailSecureAccess);
		if(signUpOutStatus==null) {
			return;
		}
		var oldPref=(listClinicPrefsOld.Find(x => x.ClinicNum==0)?.ValueString)?.Trim()??"";
		var oldStatusWasDisabled = oldPref=="0" || oldPref=="";
	...
	private void SignUpAllClinics(List<ClinicPref> listClinicPrefsOld) {
		var listSignups=_signupOut?.EServices?.FindAll(x=>x.EServiceCodeInt==(int)eServiceCode.EmailSecureAccess);
		if((listSignups?.Count??0)==0) {
			return;
		}
		for(var i=0;i<listSignups.Count;i++) {
			if(listSignups[i].ClinicNum==0) {
				continue; // If ClinicNum is 0 skip;
			}
			var oldPref=(listClinicPrefsOld.Find(x => x.ClinicNum==listSignups[i].ClinicNum)?.ValueString)?.Trim()??"";
			var oldStatusWasDisabled = oldPref=="0" || oldPref=="";
			if (listSignups[i].IsEnabled && oldStatusWasDisabled) {
				var status=(HostedEmailStatus)ClinicPrefs.GetInt(PrefName.EmailSecureStatus,listSignups[i].ClinicNum);
				status=EnumTools.AddFlag(status,HostedEmailStatus.Enabled);
	...
	private void butAddresses_Click(object sender,EventArgs e) {
		using var formVerification=new FormEmailHostingAddressVerification();
		formVerification.ShowDialog();
	}

	private void butSignatures_Click(object sender,EventArgs e) {
		using var formSignatures=new FormEmailHostingSignatures();
