...
public partial class FormPayPlanChargeSelection:FormODBase {
	...
	public FormPayPlanChargeSelection(List<PayPlanCharge> listPayPlanCharges,DynamicPaymentPlanModuleData dynamicPaymentPlanModuleData) {
		InitializeComponent();

		_listPayPlanCharges=listPayPlanCharges;
		_payPlan=dynamicPaymentPlanModuleData.PayPlan;
	...
	private void FillListBox() {
		...
		var listProcNums=_listPayPlanCharges.Where(x => x.LinkType==PayPlanLinkType.Procedure).Select(x => x.FKey).ToList();
		...
		for(var i=0;i<_listPayPlanCharges.Count;i++) {
			...
			if(_listPayPlanCharges[i].LinkType==PayPlanLinkType.Procedure) {
				var curProc=listProcedures.FirstOrDefault(x => x.ProcNum==_listPayPlanCharges[i].FKey);
				if(curProc!=null) {
					if(_listPayPlanCharges[i].ChargeDate==DateTime.MaxValue && (curProc.ProcStatus==ProcStat.TP || curProc.ProcStatus==ProcStat.TPi)) {
						descript="";
					}
					var curProcCode=listProcedureCodes.FirstOrDefault(x=> x.CodeNum==curProc.CodeNum);
					if(curProcCode!=null) {
	...
	private void listBoxPayPlanCharges_DoubleClick(object sender,EventArgs e) {
		if(listBoxPayPlanCharges.SelectedIndex!=-1) {
			var payPlanChargeOld=_listPayPlanCharges[listBoxPayPlanCharges.SelectedIndex].Copy();
			using var formPayPlanChargeEdit=new FormPayPlanChargeEdit(_listPayPlanCharges[listBoxPayPlanCharges.SelectedIndex],_payPlan,
				_dynamicPaymentPlanModuleData);
			...
			if(formPayPlanChargeEdit.PayPlanChargeCur==null) {
				...
				if(_listPayPlanCharges[listBoxPayPlanCharges.SelectedIndex].PayPlanChargeNum==0){ //Pay plan charge is new
					_listPayPlanCharges[listBoxPayPlanCharges.SelectedIndex].PayPlanChargeNum=PayPlanCharges.Insert(_listPayPlanCharges[listBoxPayPlanCharges.SelectedIndex]);
				}
				else { //Pay plan charge existed
	...
	private void butZeroOutCharges_Click(object sender,EventArgs e) {
		...
		for(var i=0;i<_listPayPlanCharges.Count;i++) {
			var payPlanChargeOld=_listPayPlanCharges[i].Copy();
