...
public partial class FormEServicesWebSchedNotify:FormODBase {
	...
	private const int _clinicNumDefault=0;
	...
	private List<ClinicPref> _listClinicPrefsWebSchedVerify= [];
	...
	private List<PrefName> _listPrefNamesWebSchedVerify= [];
	///<summary>The list of preferences to use for ASAP.</summary>
	private List<PrefName> _listPrefNamesWebSchedVerifyAsap=
	[
	...
	private List<PrefName> _listPrefNamesWebSchedVerifyExistingPat=
	[
	...
	private List<PrefName> _listPrefNamesWebSchedVerifyNewPat=
	[
	...
	private List<PrefName> _listPrefNamesWebSchedVerifyRecall=
	[
	...
	public FormEServicesWebSchedNotify(WebSchedNotifyType webSchedNotifyType) {
		InitializeComponent();

		_webSchedType=webSchedNotifyType;
	}
	...
	private void FormEServicesWebSchedNotify_Load(object sender,EventArgs e) {
		...
		for(var i=0;i<_listPrefNamesWebSchedVerify.Count;i++) {
			_listClinicPrefsWebSchedVerify.AddRange(ClinicPrefs.GetPrefAllClinics(_listPrefNamesWebSchedVerify[i]));
			var pref=Prefs.GetPref(_listPrefNamesWebSchedVerify[i].ToString());
			_listClinicPrefsWebSchedVerify.Add(new ClinicPref { ClinicNum=_clinicNumDefault,PrefName=_listPrefNamesWebSchedVerify[i],ValueString=pref.ValueString });
		}
		...
		if(true && comboClinicVerify.ClinicNumSelected!=0) {
	...
	private void RefreshEmail(WebBrowser webBrowserEmailBody,string emailText,bool isRawHtml) {
		if(isRawHtml) {
			webBrowserEmailBody.DocumentText=emailText;
			return;//text is already in HTML, it does not need to be translated. 
		}
		ODException.SwallowAnyException(() => {
			var text=MarkupEdit.TranslateToXhtml(emailText,isEmail:true);
			webBrowserEmailBody.DocumentText=text;
		});
	}
	...
	private void butEditEmail_Click(object sender,EventArgs e) {
		...
		formEmailEdit.MarkupText=GetTemplateVal(_prefNameEmailBody);
		formEmailEdit.DoCheckForDisclaimer=true;
		formEmailEdit.IsRawAllowed=true;
		formEmailEdit.IsRaw=(EmailType)GetInt(_prefNameEmailType)==EmailType.RawHtml;
		...
		var emailType=EmailType.Html;
		if(formEmailEdit.IsRaw) {
			emailType=EmailType.RawHtml;
		}
		UpdateClinicPref(_prefNameEmailType,((int)emailType).ToString());
	...
	private void butRestore_Click(object sender,EventArgs e) {
		var isAccepted=MsgBox.Show(this,MsgBoxButtons.YesNo,"Undo all changes to the template in this clinic?");
		if(!isAccepted) {
			return;
		}
		for(var i=0;i<_listPrefNamesWebSchedVerify.Count;i++) {
	...
	private void toolStripMenuItemCopy_Click(object sender,EventArgs e) {
		...
		((TextBox)contextMenuStrip.SourceControl).Copy();
	}
	...
	private void toolStripMenuItemCut_Click(object sender,EventArgs e) {
		var toolStripItem=(ToolStripItem)sender;
		var contextMenuStrip=(ContextMenuStrip)toolStripItem.Owner;
		((TextBox)contextMenuStrip.SourceControl).Cut();
	}
	...
	private void toolStripMenuItemPaste_Click(object sender,EventArgs e) {
		var toolStripItem=(ToolStripItem)sender;
		var contextMenuStrip=(ContextMenuStrip)toolStripItem.Owner;
		((TextBox)contextMenuStrip.SourceControl).Paste();
	}
	...
	private void toolStripMenuItemInsertReplacements_Click(object sender,EventArgs e) {
		var toolStripItem=(ToolStripItem)sender;
		var contextMenuStrip=(ContextMenuStrip)toolStripItem.Owner;
		var textBox=((TextBox)contextMenuStrip.SourceControl);
		//PHI is not supposed to be communicated via text message.
		var allowPHI=(!textBox.Name.In(textMessageTemplate.Name));
		...
		var frmMessageReplacements=new FrmMessageReplacements(listMessageReplaceTypes,allowPHI);
	...
	private void toolStripMenuItemSelectAll_Click(object sender,EventArgs e) {
		var toolStripItem=(ToolStripItem)sender;
		var contextMenuStrip=(ContextMenuStrip)toolStripItem.Owner;
		((TextBox)contextMenuStrip.SourceControl).SelectAll();
	}
	...
	private void toolStripMenuItemUndo_Click(object sender,EventArgs e) {
		var toolStripItem=(ToolStripItem)sender;
		var contextMenuStrip=(ContextMenuStrip)toolStripItem.Owner;
		((TextBox)contextMenuStrip.SourceControl).Undo();
	}
	...
	private void WebSchedVerify_RadioButtonCheckChanged(object sender,EventArgs e) {
		var radioChanged=(RadioButton)sender;
		if(!radioChanged.Checked) {
			return;
		}
		var webSchedVerifyType=(WebSchedVerifyType)radioChanged.Tag;
		if(!_listClinicPrefsWebSchedVerify.Any(x => x.ClinicNum==comboClinicVerify.ClinicNumSelected)) {
			return;
		}
	...
	private string GetTemplateVal(PrefName prefName) {
		var clinicPref=_listClinicPrefsWebSchedVerify.FirstOrDefault(x => x.ClinicNum==comboClinicVerify.ClinicNumSelected && x.PrefName==prefName);
		var clinicPrefDefault=_listClinicPrefsWebSchedVerify.FirstOrDefault(x => x.ClinicNum==_clinicNumDefault && x.PrefName==prefName);
		//ClinicPref won't be available if it has not been created previously.
		var valueStringClinicPref=clinicPrefDefault.ValueString;
		if(clinicPref!=null) {
			valueStringClinicPref=clinicPref.ValueString;
		}
		return valueStringClinicPref;
	}
	...
	private void SetRadioButtonVal(PrefName prefName) {
		var webSchedVerifyType=(WebSchedVerifyType)SIn.Int(GetTemplateVal(prefName));
		var radioMatch=groupBoxRadio.Controls.OfType<RadioButton>().FirstOrDefault(x => (WebSchedVerifyType)x.Tag==webSchedVerifyType);
	...
	private void TryRestoreClinicPrefOld(PrefName prefName) {
		var clinicPref=_listClinicPrefsWebSchedVerify_Old.FindAll(x => x.ClinicNum==comboClinicVerify.ClinicNumSelected && x.PrefName==prefName).FirstOrDefault();
		if(clinicPref==null) {
	...
	private void SaveWebSchedVerify() {
		var listClinicNums=Clinics.GetForUserod(Security.CurUser).Select(x => x.Id).ToList();
		for(var i = 0;i<_listPrefNamesWebSchedVerify.Count;i++) {
			for(var j = 0;j<listClinicNums.Count;j++) {
				var clinicPrefNew=_listClinicPrefsWebSchedVerify.FirstOrDefault(x => x.PrefName==_listPrefNamesWebSchedVerify[i] && x.ClinicNum==listClinicNums[j]);
				var clinicPrefOld=_listClinicPrefsWebSchedVerify_Old.FirstOrDefault(x => x.PrefName==_listPrefNamesWebSchedVerify[i] && x.ClinicNum==listClinicNums[j]);
			...
			var clinicPrefDefault=_listClinicPrefsWebSchedVerify.FirstOrDefault(x => x.PrefName==_listPrefNamesWebSchedVerify[i] && x.ClinicNum==_clinicNumDefault);
