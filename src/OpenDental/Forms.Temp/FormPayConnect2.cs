...
public partial class FormPayConnect2:FormODBase {
	...
	private void FormPayConnect2_Load(object sender,EventArgs e) {
		...
		for(var i = 0;i<listPayTerminals.Count;i++) {
			var name = listPayTerminals[i].Name;
		...
		if(!SIn.Bool(ProgramProperties.GetPropVal(_program.ProgramNum,"TerminalProcessingEnabled",_clinicNum))){
			...
			var procMethod=ProgramProperties.GetPropValForClinicOrDefault(_program.ProgramNum,
	...
	private bool ProcessTransaction() {
		...
		var amountInCents=GetAmountFieldAsCents();
		var magData="";
		if(radioTerminal.Checked) {
			...
			var terminalId=((PayTerminal)comboTerminal.SelectedItem).TerminalID;
			var amount=(decimal)textAmount.Value;//textAmount will be a value between 0 and 100,000,000
			if(amount>0) {
				var request=new CreateTerminalTransactionRequest();
			...
			if(radioSale.Checked) {
				...
				if(_creditCard==null || _creditCard.PayConnectToken.IsNullOrEmpty()) {
					var getStatusResponse=_payConnect2Response.GetStatusResponse;
					if(getStatusResponse==null) {
						return false;
					}
					var pc2GatewayResponse=getStatusResponse.GatewayResponse.ToString();
				...
				var refundReferenceIDRequest=new RefundReferenceIDRequest();
		...
		var surchargeAmount=_response.AmountSurcharged;
	...
	private PayConnect2Response SendSignature() {
		...
		var addSignatureRequest=new AddSignatureReferenceIDRequest();
		addSignatureRequest.ReferenceId=_response.RefNumber;
		using(var bitmapSigImage=sigBoxWrapper.GetSigImage())
		using(var memoryStream=new MemoryStream()) {
			bitmapSigImage.Save(memoryStream,ImageFormat.Jpeg);
			var byteArrayImageBytes=memoryStream.ToArray();
	...
	private void SaveCreditCard() {
		...
		var statusResponse=PayConnect2.GetTransactionStatus(_clinicNum,_response.RefNumber);
		...
		DateTime expiryDate;
		var didParse=DateTime.TryParseExact(cardPaymentMethod.Expiry, ["MMyy","yyyyMM"],CultureInfo.CurrentCulture.DateTimeFormat,DateTimeStyles.None,out expiryDate);
		if(didParse) {
