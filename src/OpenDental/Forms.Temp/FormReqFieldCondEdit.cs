...
public partial class FormReqFieldCondEdit:FormODBase {
	...
	private void FormReqFieldCondEdit_Load(object sender,EventArgs e) {
		...
		if(PrefC.GetString(PrefName.LanguagesUsedByPatients)!="") {
			_listLanguages=PrefC.GetString(PrefName.LanguagesUsedByPatients).Split([','],StringSplitOptions.RemoveEmptyEntries).ToList();
		}
	...
	private void listConditionType_SelectedIndexChanged(object sender,EventArgs e) {
		...
		var requiredFieldNameSelected=_listRequiredFieldNames[listConditionType.SelectedIndex];
		switch(requiredFieldNameSelected) {
			...
			case RequiredFieldName.Gender:
				...
				var arrayPatientGenders=(PatientGender[])Enum.GetValues(typeof(PatientGender));
				for(var i=0;i<arrayPatientGenders.Length;i++) {
			...
			case RequiredFieldName.Position:
				SetFieldVisibleHelper(true);
				var arrayPatientPositions=(PatientPosition[])Enum.GetValues(typeof(PatientPosition));
				for(var i=0;i<arrayPatientPositions.Length;i++) {
			...
			case RequiredFieldName.PatientStatus:
				SetFieldVisibleHelper(true);
				var arrayPatientStatuses=(PatientStatus[])Enum.GetValues(typeof(PatientStatus));
				for(var i=0;i<arrayPatientStatuses.Length;i++) {
			...
			case RequiredFieldName.BillingType:
				SetFieldVisibleHelper(true);
				for(var i=0;i<_listDefsBillingTypes.Count;i++) {
	...
	private void ListValuesSetIndices() {
		...
		for(var i=0;i<listConditionValues.Items.Count;i++) {
			...
			if(requiredFieldNameSelected==RequiredFieldName.Clinic
			   && i>0
			   && ListRequiredFieldConditions.Exists(x => x.ConditionValue==_listClinics[i-1].Id.ToString()))//subtract 1 for 'Unassigned'
			{
				listConditionValues.SelectedIndices.Add(i);
				continue;
			}
	...
	private void butSave_Click(object sender,EventArgs e) {
		...
		switch(requiredFieldNameSelected) {
			...
			case RequiredFieldName.DateTimeDeceased:
				...
				var requiredFieldCondition1=new RequiredFieldCondition();
				...
				if(requiredFieldNameSelected==RequiredFieldName.Birthdate) {
					try {
						agePlaceholder=int.Parse(textConditionValue1.Text);
					} 
					catch {
				...
				var requiredFieldCondition2=new RequiredFieldCondition();
				...
				if(requiredFieldNameSelected==RequiredFieldName.Birthdate) {
					try {
						agePlaceholder=int.Parse(textConditionValue2.Text);
					}
					catch {
						MsgBox.Show(this,"Please enter a valid integer.");
						return;
			...
			case RequiredFieldName.PrimaryProvider:
				...
				for(var i=0;i<listFkNums.Count;i++) {
