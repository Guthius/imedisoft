...
public partial class FormPayPeriodManager:FormODBase {
	...
	private void FormPayPeriodManager_Load(object sender, System.EventArgs e) {
		var listDayOptions=new List<string>();
		listDayOptions.Add("None");
		listDayOptions.Add("Sunday");
		listDayOptions.Add("Monday");
		listDayOptions.Add("Tuesday");
		listDayOptions.Add("Wednesday");
		listDayOptions.Add("Thursday");
		listDayOptions.Add("Friday");
		listDayOptions.Add("Saturday");
		comboDay.Items.AddList(listDayOptions);
		var payPeriod=PayPeriods.GetMostRecent();
		if(payPeriod==null) {
		...
		var payPeriodInterval=(PayPeriodInterval)PrefC.GetInt(PrefName.PayPeriodIntervalSetting);
		...
		var dayOfWeek=PrefC.GetInt(PrefName.PayPeriodPayDay);
	...
	private void FillGrid() {
		...
		var col=new GridColumn("Start Date",80);
		...
		GridRow row;
		for(var i=0;i<_listPayPeriods.Count;i++) {
			row=new GridRow();
	...
	private void GenerateSemiMonthlyPayPeriods() {
		var numPeriods=SIn.Int(textPayPeriods.Text);
		var numPeriodsGenerated=0;
		var daysAfterPayPeriod=textDaysAfterPayPeriod.Value;
		var hasChosenEndDates=radioEndDate.Checked;
		var doExcludeWeekends=checkExcludeWeekends.Checked;
		var doPayBeforeWeekends=radioPayBefore.Checked;
		var dateTimeStartPayPeriod=dateTimeStart.Value;
		...
		var period1Day=SIn.Int(textDay1.Text);
		var period2Day=SIn.Int(textDay2.Text);
		...
		if(checkLast.Checked) {
			...
			var periodLength=Math.Abs(period2Day-period1Day);
		...
		if(dateTimeStartPayPeriod.Day<=textDay1.Value) {
			...
			var dateTimePrevMonth=dateTimeStartPayPeriod.AddMonths(-1);
			if(checkLast.Checked) {
	...
	private void gridMain_CellDoubleClick(object sender, ODGridClickEventArgs e) {
		...
		using var formPayPeriodEdit=new FormPayPeriodEdit(_listPayPeriods[e.Row],_listPayPeriods);
	...
	private void butGenerate_Click(object sender, EventArgs e) {
		...
		var startDate=dateTimeStart.Value;//Original value
	...
	private DateTime GetDateOfDay(DateTime dateStart,DayOfWeek dayOfWeek) {
		var dateTimeResult=dateStart.AddDays(1);//PayDate cannot be the same as the last day of the pay period.
		while(dateTimeResult.DayOfWeek!=dayOfWeek) {
	...
	private void butSave_Click(object sender,EventArgs e) {
		...
		//Save payperiods
		for(var i=0;i<_listPayPeriods.Count;i++) {//PayPeriods are always new in this form.
			PayPeriods.Insert(_listPayPeriods[i]);
		}
		//Save Preferences
