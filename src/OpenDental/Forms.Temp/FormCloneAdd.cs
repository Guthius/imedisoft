...
public partial class FormCloneAdd:FormODBase {
	...
	private void FillClinicComboBoxes() {
		_listSpecialtyClinics= [];
		//Fill the list of clinics for this user.
		var listClinicsForUsers=Clinics.GetForUserod(Security.CurUser);
		...
		var listClinicsNoSpecialties=listClinicsForUsers.Select(x => x).ToList();
		//Fill the list of defLinks used by clones of this patient.
		var listClonePatNums=PatientLinks.GetPatNumsLinkedFrom(_patientMaster.PatNum,PatientLinkType.Clone);
		var listDefLinksPatsCur=DefLinks.GetListByFKeys(listClonePatNums,DefLinkType.Patient);
		//Fill the list of clinics defLink
		var listDefLinksClinics=DefLinks.GetDefLinksByType(DefLinkType.ClinicSpecialty);
		...
		var listDefsSpecialties=Defs.GetDefsForCategory(DefCat.ClinicSpecialty,true);
		...
		if(listDefsSpecialties.Count > 0) {
			listClinicsNoSpecialties.RemoveAll(x => listDefLinksClinics.Select(y => y.FKey).ToList().Contains(x.Id));
		}
		//Remove all clinics that do not have any specialties from the original list of clinics for the user.
		listClinicsForUsers.RemoveAll(x => !listDefLinksClinics.Select(y => y.FKey).ToList().Contains(x.Id));
		...
		for(var i=0;i<listDefsSpecialties.Count;i++) {
			comboSpecialty.Items.Add(listDefsSpecialties[i].ItemName,listDefsSpecialties[i]);
			//Get a list of all deflinks for the def
			var listDefLinks=listDefLinksClinics.FindAll(x => x.DefNum==listDefsSpecialties[i].DefNum).ToList();
			_listSpecialtyClinics.Add(new SpecialtyClinics { 
				DefNumClinicSpecialty=listDefsSpecialties[i].DefNum,
				ListClinics=listClinicsForUsers.FindAll(x => listDefLinks.Select(y => y.FKey).ToList().Contains(x.Id))
			});
		}
	...
	private void FillComboClinic() {
		...
		{
			comboClinic.Items.Add(Lan.g(this,"Unassigned"),new ClinicDto());
		}
		var listClinics=_listSpecialtyClinics.First(x => x.DefNumClinicSpecialty==comboSpecialty.GetSelected<Def>().DefNum).ListClinics;
	...
	private void butClone_Click(object sender,EventArgs e) {
		...
		long defNum=0;
		if(true) {
			clinicNum=comboClinic.GetSelected<ClinicDto>().Id;
		}
		defNum=comboSpecialty.GetSelected<Def>().DefNum;
		var patientClone=Patients.CreateCloneAndSynch(_patientMaster,_family,_listInsPlans,_listInsSubs,_listBenefits,_provNumSelected,clinicNum);
