...
public partial class FormProcCodeNew : FormODBase {
	...
	private void FormProcCodeNew_Load(object sender,EventArgs e) {
		...
		_listDefsProcCodeCat=Defs.GetDefsForCategory(DefCat.ProcCodeCats,true);
		for(var i=0;i<_listDefsProcCodeCat.Count;i++) {
			comboCategory.Items.Add(_listDefsProcCodeCat[i].ItemName);
		}
	...
	private int GetCategoryIndex(string name){
		for(var i=0;i<_listDefsProcCodeCat.Count;i++) {
			if(_listDefsProcCodeCat[i].ItemName==name){
				return i;
			}
		}
		return 0;
	...
	private bool AddProc(){
		...
		procedureCode.ProcCode=textNewCode.Text;
		//code.ProcTime="/X/";//moved to contructor.
		//code.GraphicColor=Color.FromArgb(0);//moved to contructor.
		procedureCode.Descript=textDescription.Text;
		procedureCode.AbbrDesc=textAbbreviation.Text;
		procedureCode.NoBillIns=checkNoBillIns.Checked;
		procedureCode.IsHygiene=checkIsHygiene.Checked;
		procedureCode.IsProsth=checkIsProsth.Checked;
		procedureCode.PaintType=(ToothPaintingType)comboPaintType.SelectedIndex;
		procedureCode.TreatArea=(TreatmentArea)comboTreatArea.SelectedIndex;
	...
	private void butDefault_Click(object sender,EventArgs e) {
		IsChanged=true;//because of add definition for new proc category
