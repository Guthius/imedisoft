...
public partial class FormTimeCardRuleEdit:FormODBase {
	...
	private void butSave_Click(object sender,EventArgs e) {
		...
		var timeSpanOverHoursPerDay=TimeSpan.Zero;
		if(textOverHoursPerDay.Text!="") {
			try {
		...
		var timeSpanAfterTimeOfDay=TimeSpan.Zero;
		if(textAfterTimeOfDay.Text!="") {
			try {
		...
		var timeSpanBeforeTimeOfDay=TimeSpan.Zero;
		if(textBeforeTimeOfDay.Text!="") {
			try {
		...
		var timeSpanMinClockIn=TimeSpan.Zero;
		...
		if(_isInsertMode) {//Insert new entry for every list selection.
			var listTimeCardRulesNew=new List<TimeCardRule>();
