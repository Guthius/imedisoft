...
public partial class FormRequiredFields:FormODBase {
	...
	private void FormRequiredFields_Load(object sender,EventArgs e) {
		...
		FillMain();
		var arrayRequiredFieldTypes=(RequiredFieldType[])Enum.GetValues(typeof(RequiredFieldType));
		for(var i=0;i<arrayRequiredFieldTypes.Length;i++) {
	...
	private void FillAvailable() {
		...
		var arrayRequiredFieldNames=(RequiredFieldName[])Enum.GetValues(typeof(RequiredFieldName));
		for(var i=0;i<arrayRequiredFieldNames.Length;i++) {
		...
		var listRequiredFieldNames=_listRequiredFields.Select(y => y.FieldName).Distinct().ToList();
	...
	private void FillMain() {
		...
		for(var i=0;i<_listRequiredFields.Count;i++) {
			...
			var conditionsCount=_listRequiredFields[i].ListRequiredFieldConditions.Count;
	...
	private void FillConditions() {
		...
		var col=new GridColumn(Lan.g(this,"Type"),120);
		...
		var allCondValues="";
		for(var i=0;i<_requiredField.ListRequiredFieldConditions.Count;i++) {
			row=new GridRow();
			...
			if(requiredFieldNameType==RequiredFieldName.Birthdate || requiredFieldNameType==RequiredFieldName.AdmitDate 
			                                                      || requiredFieldNameType==RequiredFieldName.DateTimeDeceased || requiredFieldNameType==RequiredFieldName.DischargeDate) {
				//Continuous values
			...
			var condVal=_requiredField.ListRequiredFieldConditions[i].ConditionValue;
	...
	private void gridConditions_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		using var formReqFieldCondEdit=new FormReqFieldCondEdit();
		...
		if(formReqFieldCondEdit.DialogResult==DialogResult.OK) {
			...
			var mainSelectedIndex=gridMain.SelectedIndices[0];
			FillMain();
			FillConditions();
	...
	private void butRight_Click(object sender,EventArgs e) {
		...
		for(var i=0;i<listAvailableFields.SelectedIndices.Count;i++) {
	...
	private void butAdd_Click(object sender,EventArgs e) {
		...
		var mainSelected=gridMain.SelectedIndices[0];
		using var formReqFieldCondEdit=new FormReqFieldCondEdit();
		formReqFieldCondEdit.IsNew=true;
		formReqFieldCondEdit.RequiredField_=_requiredField;
		formReqFieldCondEdit.ShowDialog();
		if(formReqFieldCondEdit.DialogResult==DialogResult.OK) {
			var conditionsBeforeCount=_requiredField.ListRequiredFieldConditions.Count;
			_requiredField.RefreshConditions();
			FillConditions();
	...
	private void butDelete_Click(object sender,EventArgs e) {
		...
		for(var i=0;i<gridConditions.SelectedIndices.Length;i++) {
