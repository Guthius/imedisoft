...
public partial class FormEtrans835Edit:FormODBase {
	...
	private void FormEtrans835Edit_Load(object sender,EventArgs e) {
		...
		var listEtrans835Attaches=Etrans835Attaches.GetForEtrans(EtransCur.EtransNum);
		_x835=new X835(EtransCur,MessageText835,TranSetId835,listEtrans835Attaches);
		RefreshFromDb();
		FillAll();
		var menuItemCollection=new Menu.MenuItemCollection(_contextMenuClaimDetails);
	...
	private void FillClaimDetails() {
		...
		var col=new GridColumn(Lan.g(this,"Patient"),70,HorizontalAlignment.Left);
		...
		_sumClaimInsPaid=0;
		var listMatchedRowIndices=new List<int>();
		var listHx835_ClaimsPaid=_x835.ListClaimsPaid;
		for(var i=0;i<listHx835_ClaimsPaid.Count;i++) {
			var hx835_Claim=listHx835_ClaimsPaid[i];
			var row=new GridRow();
			var listRows=new List<GridRow>();
			listRows.Add(row);
	...
	private void SetClaimDetailRows(List<GridRow> listRows,List<Hx835_Claim> listHx835_ClaimsPaid,bool isRefreshNeeded=false) {
		...
		for(var i=0;i<listRows.Count;i++) {
			...
			var strDateService=hx835_Claim.DateServiceStart.ToShortDateString();
	...
	private void RunAgingForClaims() {
		var listPatNumsOnClaims = Claims.GetClaimsFromClaimNums(_listEtrans835Attaches.Select(x => x.ClaimNum).ToList()).Select(x => x.PatNum).ToList();
		var listGuarantors = Patients.GetGuarantorsForPatNums(listPatNumsOnClaims);
		...
		for(var i=0;i<listGuarantors.Count;i++) {
	...
	private void FillProviderAdjustmentDetails() {
		...
		var widthCol=gridProviderAdjustments.Width-colWidthNPI-colWidthFiscalPeriod-colWidthReasonCode-colWidthRefIdent-colWidthAmount;
		...
		_sumProvAdjAmt=0;
		for(var i=0;i<_x835.ListProvAdjustments.Count;i++) {
			var hx835_ProvAdj=_x835.ListProvAdjustments[i];
			var row=new GridRow();
	...
	private void butRawMessage_Click(object sender,EventArgs e) {
		var msgBoxCopyPaste=new MsgBoxCopyPaste(MessageText835);
	...
	private void gridProviderAdjustments_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		var hx835_ProvAdj=(Hx835_ProvAdj)gridProviderAdjustments.ListGridRows[e.Row].Tag;
	...
	private void gridClaimDetails_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		var hx835_Claim=(Hx835_Claim)gridClaimDetails.ListGridRows[e.Row].Tag;
		var claim=hx835_Claim.GetClaimFromDb();
		var isAttachNeeded=(!hx835_Claim.IsAttachedToClaim);
		...
		var doesPatientNameMatch=hx835_Claim.DoesPatientNameMatch(patient);
		...
		if(hx835_Claim.IsSplitClaim) {
			...
			var listHx835_ClaimsOtherNotDetachedSplit=hx835_Claim.GetOtherNotDetachedSplitClaims();
			for(var i=0;i<listHx835_ClaimsOtherNotDetachedSplit.Count;i++) {
		...
		if(hx835_Claim.IsProcessed(_listHx835_ShortClaimProcs,_listEtrans835Attaches)) {
			...
			var listClaimProcsForClaim=ClaimProcs.RefreshForClaim(claim.ClaimNum);
		...
		if(isReadOnly) {
			var formEtrans835ClaimEdit=new FormEtrans835ClaimEdit(_x835,hx835_Claim);
		...
		if(!gridClaimDetails.IsDisposed) {//Not sure how the grid is sometimes disposed, perhaps because of it being non-modal.
			...
			if(hx835_Claim.IsSplitClaim) {//Need to update multiple rows.
				var listHx835_ClaimsSplit=hx835_Claim.GetOtherNotDetachedSplitClaims();
				var listRowsAdditional=gridClaimDetails.ListGridRows
					.Where(x =>
	...
	private Claim ClaimSelectionHelper(Hx835_Claim hx835_ClaimPaid) {
		var ptTableSearchParams=new PtTableSearchParams(doLimit:false,hx835_ClaimPaid.PatientName.Lname,hx835_ClaimPaid.PatientName.Fname,"","",hideInactive:false,
			"","","","","",0,guarOnly:false,showArchived:false,DateTime.MinValue,0,"","","","","","","");
		var tablePts=Patients.GetPtDataTable(ptTableSearchParams);//Mimics FormPatientSelect.cs
		long selectedPatNum=0;
		if(tablePts.Rows.Count==1) {
			selectedPatNum=SIn.Long(tablePts.Rows[0]["PatNum"].ToString());
		}
		using var formEtrans835ClaimSelect=new FormEtrans835ClaimSelect(selectedPatNum,hx835_ClaimPaid);
		formEtrans835ClaimSelect.ShowDialog();
		if(formEtrans835ClaimSelect.DialogResult!=DialogResult.OK) {
			return null;
		}
		var claim=formEtrans835ClaimSelect.ClaimSelected; //Set claim so below we can act if a claim was already linked.
	...
	private void gridClaimDetails_RightClickHelper(object sender,EventArgs e) {
		var index=gridClaimDetails.GetSelectedIndex();
		if(index==-1) {//Should not happen, menu item is only enabled when exactly 1 row is selected.
			return;
		}
		var hx835_Claim=(Hx835_Claim)gridClaimDetails.ListGridRows[index].Tag;
		var idxMenuItem=((MenuItem)sender).Index;//sender is the selected menuItem
	...
	private void butDetachClaim_Click(object sender,EventArgs e) {
		...
		var listHx835_ClaimsSelected=gridClaimDetails.SelectedTags<Hx835_Claim>();//See SetClaimDetailRows(...)
		...
		var listClaimsAttached=_x835.RefreshClaims();
		var listHx835_ClaimsReceived=listHx835_ClaimsSelected.FindAll(x => listClaimsAttached.Any(y => y.ClaimNum==x.ClaimNum && y.ClaimStatus=="R"));
		var stringBuilderWarnings=new StringBuilder();
	...
	private void DetachClaimHelper(List<int> listGridIndices) {
		gridClaimDetails.BeginUpdate();
		for(var i=0;i<listGridIndices.Count;i++) {
			var row=gridClaimDetails.ListGridRows[listGridIndices[i]];
			var hx835_Claim=(Hx835_Claim)row.Tag;
	...
	private void buttonFindClaimMatch_Click(object sender,EventArgs e) {
		var listHx835_ClaimsDetached=gridClaimDetails
			.SelectedIndices
		...
		var listX12ClaimMatches=_x835.GetClaimMatches(listHx835_ClaimsDetached);
		var listClaimNums=Claims.GetClaimFromX12(listX12ClaimMatches);//Can return null.
		if(listClaimNums!=null) {
			...
			for(var i=0;i<listHx835_ClaimsDetached.Count;i++) {
			...
			var listRows=gridClaimDetails.ListGridRows
				.Where(x =>
	...
	private void butClaimDetails_Click(object sender,EventArgs e) {
		...
		var hx835_Claim=(Hx835_Claim)gridClaimDetails.ListGridRows[gridClaimDetails.SelectedIndices[0]].Tag;
		var formEtrans835ClaimEdit=new FormEtrans835ClaimEdit(_x835,hx835_Claim);
		formEtrans835ClaimEdit.Show(this);//This window is just used to display information.
	}
	...
	private void butAutoProcess_Click(object sender,EventArgs e) {
		...
		var listHx835_ShortClaims=_x835.RefreshClaims().Select(x => new Hx835_ShortClaim(x)).ToList();
		var x835Status=_x835.GetStatus(_x835.GetClaimDataList(listHx835_ShortClaims),_listHx835_ShortClaimProcs,_listEtrans835Attaches);
		...
		var automationResultMessage=EraAutomationResult.CreateMessage(listEraAutomationResults,isForSingleEra:true);
		...
		var etrans835=Etrans835s.GetByEtransNums(EtransCur.EtransNum).FirstOrDefault();
		if(etrans835==null) {
		...
		if(eraAutomationResult.Status==X835Status.Finalized) {
			...
			Etrans835s.Upsert(etrans835,_x835,x835AutoProcessed:X835AutoProcessed.SemiAutoIncomplete);
			var etransOld=EtransCur.Copy();
			Etranss.Update(EtransCur,etransOld);//Update so that note gets saved even if cancel is clicked.
			var msgBoxCopyPaste=new MsgBoxCopyPaste(automationResultMessage);
	...
	private void FormEtrans835Edit_FormClosing(object sender,FormClosingEventArgs e) {
		...
		var isReceived=true;
		for(var i=0;i<gridClaimDetails.ListGridRows.Count;i++) {
