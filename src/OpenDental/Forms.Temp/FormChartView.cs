...
public partial class FormChartView : FormODBase {
	...
	private void FormChartView_Load(object sender,EventArgs e) {
		var stringArrayChartViewDateTypes=Enum.GetNames(typeof(ChartViewDates));
		var arrayChartViewDateValues=Enum.GetValues(typeof(ChartViewDates));
		for(var i=0;i<stringArrayChartViewDateTypes.Length;i++) {
			comboDatesShowing.Items.Add(stringArrayChartViewDateTypes[i]);
			if(ChartViewCur.DatesShowing==(ChartViewDates)arrayChartViewDateValues.GetValue(i)){
				comboDatesShowing.SelectedIndex=i;
			}
		}
		if(ChartViewCur.IsNew) {
			textBoxViewDesc.Text="";
		}
		else {
	...
	private void FillGrids() {
		...
		var col=new GridColumn(Lan.g("FormChartView","FieldName"),110);
		...
		GridRow row;
		for(var i=0;i<_listDisplayFieldsShowing.Count;i++) {
			row=new GridRow();
		...
		gridMain.EndUpdate();
		var listDisplayFieldsAvail=DisplayFields.GetAllAvailableList(DisplayFieldCategory.None);
		for(var i=0;i<_listDisplayFieldsShowing.Count;i++) {
			for(var j=0;j<listDisplayFieldsAvail.Count;j++) {
		...
		for(var i=0;i<listDisplayFieldsAvail.Count;i++) {
	...
	private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		using var formDisplayFieldEdit=new FormDisplayFieldEdit();
		formDisplayFieldEdit.AllowZeroWidth=true;
	...
	private void butLeft_Click(object sender,EventArgs e) {
		...
		var listDisplayFieldsSelected=listAvailable.GetListSelected<DisplayField>();
		for(var i=0;i<listDisplayFieldsSelected.Count;i++) {
	...
	private void butUp_Click(object sender,EventArgs e) {
		...
		var intArraySelected=new int[gridMain.SelectedIndices.Length];
		...
		for(var i=0;i<intArraySelected.Length;i++) {
	...
	private void butDown_Click(object sender,EventArgs e) {
		...
		for(var i=intArraySelected.Length-1;i>=0;i--) {//go backwards
