...
public partial class FormEtrans834Import:FormODBase {
	...
	private void FillGridInsPlanFiles() {
		...
		if(gridInsPlanFiles.Columns.Count==0) {
			...
			var col=new UI.GridColumn("Errors",80,UI.GridSortingStrategy.StringCompare);
		...
		for(var i=0;i<_stringArrayFilePaths.Length;i++) {
			var row=new UI.GridRow();
			gridInsPlanFiles.ListGridRows.Add(row);
			var filePath=_stringArrayFilePaths[i];
			row.Tag=filePath;				
	...
	private void Load834_Safe() {
		try {
			this.BeginInvoke(new Update834Delegate(Load834_Unsafe), []);
		}
	...
	private void Load834_Unsafe() {
		...
		var selectedIndex=-1;
		_x12object=null;
		for(var i=0;i<gridInsPlanFiles.ListGridRows.Count;i++) {
			var row=gridInsPlanFiles.ListGridRows[i];
			if(i < previewLimitCount) {
				gridInsPlanFiles.BeginUpdate();
			}
			var filePath=(string)row.Tag;
			...
			var memberCount=x12object.GetSegmentCountById("INS");
			var planCount=x12object.GetSegmentCountById("HD");
