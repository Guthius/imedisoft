...
public partial class FormFamilyBalancer:FormODBase {
	...
	private void timerProgress_Tick(object sender,EventArgs e) {
		...
		var progressBarValue=(_familyBalancer.GuarantorTotal-_familyBalancer.ConcurrentQueueGuarantors.Count);
	...
	private void timerOutput_Tick(object sender,EventArgs e) {
		...
		while(_familyBalancer.ConcurrentQueueOutputMessages.TryDequeue(out var outputMessage)) {
		...
		var lineCount=textOutput.Lines.Length;
		var lineLimit=1000;
		if(lineCount > lineLimit) {
			var stringArrayNewLines=new string[lineLimit];
	...
	private void InstantiateRigorousBalancerThread() {
		...
		_threadRigorousBalancer.AddExceptionHandler(_ => { });
	}
	...
	private void InstantiateFifoBalancerThread() {
		...
		_threadFifoBalancer.AddExceptionHandler(_ => { });
	}
	...
	private void InstantiateRecreateThread() {
		...
		_threadRecreate.AddExceptionHandler(_ => { });
	}
	...
	private void InstantiateOverpaymentThread() {
		...
		_threadInsuranceOverpayment.AddExceptionHandler(_ => { });
	}
	...
	private void butConfigureAuto_Click(object sender,EventArgs e) {
		var frmFamilyBalancerAutoEdit=new FrmFamilyBalancerSetup();
