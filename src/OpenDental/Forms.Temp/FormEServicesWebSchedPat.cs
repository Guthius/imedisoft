...
public partial class FormEServicesWebSchedPat:FormODBase {
	...
	private void FormEServicesWebSchedPat_Load(object sender,EventArgs e) {
		...
		SetPreferences();
		var searchAfterDays=PrefC.GetInt(_prefNameSearchAfterDays);
		...
		var confirmStatus=PrefC.GetLong(_prefNameConfirmStatus);
	...
	private void FillComboClinics() {
		...
		var listSignupOutEServices=WebServiceMainHQProxy.GetSignups<WebServiceMainHQProxy.EServiceSetup.SignupOut.SignupOutEService>(SignupOut,eServiceCode.WebSchedNewPatAppt);
		var listEServiceClinicNames=new List<EServiceClinicName>();
		ClinicDto clinic;
		for(var i=0;i<listSignupOutEServices.Count;i++) {
			if(true){
				if(!listClinicsAll.Exists(x => x.Id==listSignupOutEServices[i].ClinicNum)) {
					continue;
				}
				if(listClinicsAll.Find(x => x.Id==listSignupOutEServices[i].ClinicNum).IsHidden) {
					continue;
				}
			}
			else {//clinics off
			...
			var eServiceClinicName=new EServiceClinicName();
			clinic=listClinicsAll.FirstOrDefault(x => x.Id==listSignupOutEServices[i].ClinicNum);
			if(clinic==null) {
		...
		for(var i=0;i<listEServiceClinicNames.Count();i++) {
	...
	private void FillGridApptOps() {
		var widthColOpName=150;
		var widthColClinic=150;
		if(!true) {
		...
		var col=new GridColumn(Lan.g("TableOperatories","ApptTypes"),40);
		...
		var listOperatories=Operatories.GetOpsForWebSchedNewOrExistingPatAppts(_isNewPat);
		...
		var listDefs=Defs.GetDefs(_defCatApptTypes,listDefNums);
		GridRow row;
	...
	private void FillGridApptTimeSlots() {
		...
		var signupOutEService=comboClinics.GetSelected<WebServiceMainHQProxy.EServiceSetup.SignupOut.SignupOutEService>();
		if(!signupOutEService.IsEnabled) {
			return;//Do nothing, this clinic is excluded.
		}
		var dateStart=SIn.DateTime(textApptsDateStart.Text);
	...
	private void FillBlockoutTypes() {
		...
		var listDefsForApptTypes=Defs.GetDefsForCategory(_defCatApptTypes,isShort:true);
		//Get every blockout type deflink for the reason defs.
		var listDefLinksRestrictedTo=DefLinks.GetDefLinksByTypeAndDefs(DefLinkType.BlockoutType,listDefsForApptTypes.Select(x => x.DefNum).ToList());
		var listDefNumsRestrictedTo=listDefLinksRestrictedTo.Select(x => x.FKey).Distinct().ToList();
		var listDefsRestrictedToBlockouts=Defs.GetDefs(DefCat.BlockoutTypes,listDefNumsRestrictedTo);
		var listAllowedBlockoutTypes=GetAllowedBlockoutTypes();
		var listDefsAllowedBlockouts=Defs.GetDefs(DefCat.BlockoutTypes,listAllowedBlockoutTypes.FindAll(x => !listDefNumsRestrictedTo.Contains(x)));
		//Fill the list box for Restricted to Reasons that will display the blockout name followed by associated reason names.
		var listItemNamesBlockoutReasonAssociation=new List<string>();
		var listDefNumsAssociatedReason=new List<long>();
		var listItemNamesAssociatedReason=new List<string>();
	...
	private void FillGridReasons() {
		var listDefs=Defs.GetDefsForCategory(_defCatApptTypes,true);
		var listDefLinks=DefLinks.GetDefLinksByType(DefLinkType.AppointmentType);
		var listAppointmentTypes=AppointmentTypes.GetWhere(x => listDefLinks.Any(y => y.FKey==x.AppointmentTypeNum),true);
		...
		var col=new GridColumn(Lan.g(this,"Pattern"),80);
		col.IsWidthDynamic=true;
		...
		for(var i=0;i<listDefs.Count;i++) {
			...
			var defLink=listDefLinks.FirstOrDefault(x => x.DefNum==listDefs[i].DefNum);
			if(defLink==null) {
				continue;//Corruption?
			}
	...
	private void butBlockouts_Click(object sender,EventArgs e) {
		var listAllowedBlockoutTypes=GetAllowedBlockoutTypes();
		var listDefsAllowedBlockoutTypes=Defs.GetDefs(DefCat.BlockoutTypes,listAllowedBlockoutTypes);
		using var formDefinitionPicker=new FormDefinitionPicker(DefCat.BlockoutTypes,listDefsAllowedBlockoutTypes);
		...
		var strListIgnoreBlockoutTypes=string.Join(",",formDefinitionPicker.ListDefsSelected.Select(x => x.DefNum));
	...
	private void butInsRules_Click(object sender,EventArgs e) {
		using var formWebSchedCarrierRule=new FormWebSchedCarrierRule();
		formWebSchedCarrierRule.ShowDialog();
	}

	private void butNotify_Click(object sender,EventArgs e) {
		var webSchedNotifyType=WebSchedNotifyType.ExistingPat;
		if(_isNewPat) {
			webSchedNotifyType=WebSchedNotifyType.NewPat;
		}
		using var formEServicesWebSchedNotify=new FormEServicesWebSchedNotify(webSchedNotifyType);
	...
	private void butRestrictedToReasonsEdit_Click(object sender,EventArgs e) {
		...
		using var formDefinitions=new FormDefinitions(_defCatApptTypes);
	...
	private void textApptSearchDays_Validated(object sender,EventArgs e) {
		...
		var appointmentDays=SIn.Int(textApptSearchDays.Text);
