...
public partial class FormPerioGraphical:FormODBase {
	...
	private void LayoutTeeth(){
		...
		var listDefs=Defs.GetDefsForCategory(DefCat.MiscColors,true);
		...
		var listPerioMeasures=PerioMeasures.GetAllForExam(_perioExam.PerioExamNum);
		#region CAL old
		if (!ToothChartRelay.IsSparks3DPresent){
			...
			for(var t=1;t<=32;t++) {
				...
				perioMeasureGM=null;
				for(var i=0;i<listPerioMeasures.Count;i++) {
		...
		for (var i=0;i<listPerioMeasures.Count;i++) {
			if(listPerioMeasures[i].SequenceType==PerioSequenceType.SkipTooth) {
				...
				var mob=listPerioMeasures[i].ToothValue;
		...
		//if(ToothChartRelay.IsSparks3DPresent){
		var listProcedures=Procedures.Refresh(FormOpenDental.PatNumCur);
		for(var t=1;t<=32;t++){
			var listProceduresForTooth=listProcedures.FindAll(x => x.ToothNum==t.ToString() && x.ProcStatus.In(ProcStat.C,ProcStat.EC,ProcStat.EO));
			var isImplant=false;
			var isCrown=false;
			for(var p=0;p<listProceduresForTooth.Count;p++) {
				var procedureCode=ProcedureCodes.GetProcCode(listProceduresForTooth[p].CodeNum);
	...
	public void RenderPerioPrintout(Graphics g,Patient patient,Rectangle rectangleMarginBounds) {
		...
		var titleStr=Lan.g(this,"Periodontal Examination");
		...
		y+=sizeF.Height;
		//PatientName
		var patNameStr=_patient.GetNameFLFormal();
		...
		var examDateStr=_perioExam.ExamDate.ToShortDateString();//Locale specific exam date.
		...
		y+=sizeF.Height;
		var sizeFAvailable=new SizeF(rectangleMarginBounds.Width,rectangleMarginBounds.Height-y);
		using var bitmapToothChart=_toothChartRelay.GetBitmap();
		var ratioBitmapToOutput=(float)bitmapToothChart.Height/(float)sizeFAvailable.Height;
		...
		var sizeFBitmapToOut=new SizeF((float)bitmapToothChart.Width/ratioBitmapToOutput,(float)bitmapToothChart.Height/ratioBitmapToOutput);
	...
	private void butSave_Click(object sender,EventArgs e) {
		...
		using var bitmap=new Bitmap(750,1000);
