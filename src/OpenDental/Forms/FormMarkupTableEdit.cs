...
namespace OpenDental {
	public partial class FormMarkupTableEdit:FormODBase {
		...
		private void butColumnLeft_Click(object sender,EventArgs e) {
			...
			Point point=new Point(gridMain.SelectedCell.X-1,gridMain.SelectedCell.Y);
		...
		private void butHeaders_Click(object sender,EventArgs e) {
			FrmWikiTableHeaders frmWikiTableHeaders=new FrmWikiTableHeaders();
			frmWikiTableHeaders.ListColNames=_listColNames;//Just passes the reference to the list in memory, so no need to "collect" the changes afterwords.
			frmWikiTableHeaders.ListColWidths=_listColWidths;//Just passes the reference to the list in memory, so no need to "collect" the changes afterwords.
			frmWikiTableHeaders.ShowDialog();
			FillGrid();
		}
		...
		private void butColumnAdd_Click(object sender,EventArgs e) {
			...
			Point point=new Point(index,gridMain.SelectedCell.Y);
		...
		private void butColumnDelete_Click(object sender,EventArgs e) {
			...
			Point point=new Point(Math.Max(gridMain.SelectedCell.X-1,0),gridMain.SelectedCell.Y);
			FillGrid();//gridMain.SelectedCell gets cleared.
			gridMain.SetSelected(point);
		}
		...
		private void butRowUp_Click(object sender,EventArgs e) {
			...
			Point point=new Point(gridMain.SelectedCell.X,gridMain.SelectedCell.Y-1);
			FillGrid();//gridMain.SelectedCell gets cleared.
			gridMain.SetSelected(point);
		}
		...
		private void butRowAdd_Click(object sender,EventArgs e) {
			...
			Point point=new Point(pointSelectedCell.X,pointSelectedCell.Y+1);
			FillGrid();//gridMain.SelectedCell gets cleared.
			gridMain.SetSelected(point);
		}
		...
		private void butRowDelete_Click(object sender,EventArgs e) {
			...
			Point point=new Point(gridMain.SelectedCell.X,Math.Max(gridMain.SelectedCell.Y-1,0));
			FillGrid();//gridMain.SelectedCell gets cleared.
		...
		private void butPaste_Click(object sender,EventArgs e) {
			...
			for(int i=0;i<stringArrayTableRows.Length;i++) {
				...
				for(int c = 0;c<stringArrayRowCells.Length;c++) {
			...
			//Redraw Grid
			Point point=new Point(gridMain.SelectedCell.X,gridMain.SelectedCell.Y);
			FillGrid();//gridMain.SelectedCell gets cleared.
			gridMain.SetSelected(point);
		}
