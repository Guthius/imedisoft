...
namespace OpenDental{
	...
	public partial class FormApptViews : FormODBase {
		...
		private void butCopyView_Click(object sender,EventArgs e) {
			int idxSelected=listViews.SelectedIndex;
			if(idxSelected==-1) {
				MsgBox.Show("Select a view to copy first.");
				return;
			}
			if(!MsgBox.Show(this,MsgBoxButtons.OKCancel,"This will make a copy of the selected view. Click OK to continue, otherwise click Cancel.")) {
				return;
			}
			ApptView apptViewOriginal=_listApptViews[idxSelected];
			ApptView apptViewCopy=apptViewOriginal.Copy();
			apptViewCopy.ItemOrder=_listApptViews.Count;//Make last in item order
			//Increment view description. This is not necessary as descriptions can be duplicates without issue
			int uniqueId=1;
			string viewDescription="";
			while(true) {
				viewDescription=apptViewCopy.Description+"_"+uniqueId;
				if(!_listApptViews.Any(x => x.Description==viewDescription)) {
					break;
				}
				uniqueId++;
			}
			apptViewCopy.Description=viewDescription;
			long apptViewNum=ApptViews.Insert(apptViewCopy);
			//Create copies of the view items as well
			List<ApptViewItem> listApptViewItems=ApptViewItems.GetWhere(x => x.ApptViewNum==apptViewOriginal.ApptViewNum);
			List<ApptViewItem> listNewApptViewItems=new List<ApptViewItem>();
			for(int i=0;i<listApptViewItems.Count;i++) {
				ApptViewItem apptViewItem=listApptViewItems[i].Copy();
				apptViewItem.ApptViewNum=apptViewNum;
				listNewApptViewItems.Add(apptViewItem);
			}
			ApptViewItems.InsertMany(listNewApptViewItems);
			_changed=true;
			FillViewList();
			listViews.SetSelected(listViews.Items.Count-1);//Set new view selected
		}
		...
		private void butUp_Click(object sender, System.EventArgs e) {
			if(listViews.SelectedIndex==-1){
				ODMessageBox.Show(Lan.g(this,"Please select a category first."));
				return;
			}
