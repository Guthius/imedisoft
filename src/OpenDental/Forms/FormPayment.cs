...
namespace OpenDental {
	...
	public partial class FormPayment:FormODBase {
		...
		private List<AccountEntry> _listAccountEntriesCharges=new List<AccountEntry>();
		...
		private bool DoShowAccountEntry(AccountEntry accountEntryCharge) {
			...
			List<long> listPatNumsFiltered=comboPatientOutstandingFilter.GetListSelected<Patient>().Select(x => x.PatNum).ToList();
			if(!listPatNumsFiltered.Contains(accountEntryCharge.PatNum)) {
				return false;
			}
			List<long> listProvNums=comboProviderOutstandingFilter.GetSelectedProvNums();
			if(!listProvNums.Contains(accountEntryCharge.ProvNum)) {
				return false;
			}
			List<long> listClinicNumsFiltered=comboClinicOutstandingFilter.GetListSelected<ClinicDto>().Select(x => x.Id).ToList();
			...
			//Type Filter
			List<string> listTypeFiltered=comboTypeOutstandingFilter.GetListSelected<string>();
		...
		private bool DoHighlightPaySplit(PaySplit paySplit) {
			...
			List<long> listPatNumsFiltered=comboPatientPaySplitsFilter.GetListSelected<Patient>().Select(x => x.PatNum).ToList();
			if(comboPatientPaySplitsFilter.IsAllSelected==false && !listPatNumsFiltered.Contains(paySplit.PatNum)) {
				return false;
			}
			List<long> listProvNums=comboProviderPaySplitsFilter.GetSelectedProvNums();
			if(comboProviderPaySplitsFilter.IsAllSelected==false && !listProvNums.Contains(paySplit.ProvNum)) {
				return false;
			}
			List<long> listClinicNumsFiltered=comboClinicsPaySplitsFilter.GetListSelected<ClinicDto>().Select(x => x.Id).ToList();
		...
		private void FillFilterPatientsChargesTreatPlan(bool doPreserveValues) {
			bool wasAllSelected=comboPatientOutstandingFilter.IsAllSelected;
			List<long> listPatNumsSelected=comboPatientOutstandingFilter.GetListSelected<Patient>().Select(x => x.PatNum).ToList();
		...
		private void FillFilterPatientsPaySplits(bool doPreserveValues) {
			bool wasAllSelected=comboPatientPaySplitsFilter.IsAllSelected;
			List<long> listPatNumsSelected=comboPatientPaySplitsFilter.GetListSelected<Patient>().Select(x => x.PatNum).ToList();
			...
			List<long> listPatNums=_listPaySplits.Select(x => x.PatNum).Distinct().ToList();
			for(int i=0;i<listPatNums.Count;i++) {
				if(_dictPatients.TryGetValue(listPatNums[i],out Patient pat)) {
		...
		private void FillFilterProvidersChargesTreatPlan(bool doPreserveValues) {
			bool wasAllSelected=comboPatientOutstandingFilter.IsAllSelected;
			List<long> listProvNumsSelected=comboProviderOutstandingFilter.GetSelectedProvNums();
			...
			if(!wasAllSelected && doPreserveValues) {
				...
				for(int i = 0;i<=comboProviderOutstandingFilter.Items.Count;i++) {
		...
		private void FillFilterProvidersPaySplits(bool doPreserveValues) {
			bool wasAllSelected=comboPatientPaySplitsFilter.IsAllSelected;
			List<long> listProvNumSelected=comboProviderPaySplitsFilter.GetSelectedProvNums();
			...
			if(!wasAllSelected && doPreserveValues) {
				//Reselect providers that were selected before refilling the combo box.
				for(int i=0;i<=comboProviderPaySplitsFilter.Items.Count;i++) {
		...
		private void FillFilterClinicsChargestreatPlan(bool doPreserveValues) {
			...
			bool wasAllSelected=comboClinicOutstandingFilter.IsAllSelected;
			List<long> listClinicNumsSelected=comboClinicOutstandingFilter.GetListSelected<ClinicDto>().Select(x => x.Id).ToList();
		...
		private void FillFilterClinicsPaySplits(bool doPreserveValues) {
			...
			bool wasAllSelected=comboClinicsPaySplitsFilter.IsAllSelected;
			List<long> listClinicNumsSelected=comboClinicsPaySplitsFilter.GetListSelected<ClinicDto>().Select(x => x.Id).ToList();
			if(!_listClinics.IsNullOrEmpty() && !_listClinics.Any(x => x.Abbr==Lan.g(this,"Unassigned"))) {
				_listClinics.Add(new ClinicDto() { Abbr=Lan.g(this,"Unassigned") });
			}
			comboClinicsPaySplitsFilter.Items.Clear();
			List<long> listClinicNums=_listPaySplits.Select(x => x.ClinicNum).Distinct().ToList();
			var listClinics=_listClinics.FindAll(x => listClinicNums.Contains(x.Id));
			for(int i=0;i<listClinics.Count;i++) {
		...
		private void FillFilterTypes(bool doPreserveValues) {
			bool wasAllSelected=comboTypeOutstandingFilter.IsAllSelected;
			List<string> listTypesSelected=comboTypeOutstandingFilter.GetListSelected<string>();
			...
			if(!wasAllSelected && doPreserveValues) {
				//Reselect providers that were selected before refilling the combo box.
				for(int i = 0;i<=comboTypeOutstandingFilter.Items.Count;i++) {
		...
		private void FillGridSplits() {
			...
			List<PaySplit> listPaySplits=_listPaySplits.ToList();
			List<PaySplit> listPaySplitsFiltered=new List<PaySplit>();
			for(int i=0;i<listPaySplits.Count;i++) {
		...
		private GridRow FillGridsSplitsHelper(PaySplit paySplit) {
			...
			if(paySplit.ProcNum > 0) {//Procedure
				...
				bool isLegacyInterest=(paySplit.PayPlanDebitType==PayPlanDebitTypes.Unknown
		...
		private bool SavePaymentToDb() {
			...
			if(IsNew && checkRecurring.Checked && !RecurringCharges.HasRecurringChargesForPayment(_payment.PayNum)) {
				...
				double familyBalance=_family.Guarantor.BalTotal - _family.Guarantor.InsEst;
				CreditCard creditCard=comboCreditCards.GetSelected<CreditCard>();
