...
namespace OpenDental {
	...
	public partial class FormApptsOther:FormODBase {
		...
		private List<Appointment> _listPlannedAppts=new List<Appointment>();
		private List<Appointment> _listPlannedApptsIncomplete=new List<Appointment>();
						...
						if(IsShowCompletePlanned) {
							for(int p=0;p<_listPlannedAppts.Count;p++) {
								if(_listPlannedAppts[p].AptNum==ListApptOthers[i].AptNum) {
									strText+="#"+_listPlannedAppts[p].ItemOrderPlanned.ToString();
								}
							}
						}
						else {
		...
		public void RefreshData() {
			...
			if(IsForTask && !PrefC.GetBool(PrefName.TasksForApptAllowMultiple)) {
				List<long> listAttached=Appointments.GetApptNumsAttachedToTask(ListApptOthers.Select(x=>x.AptNum).ToList());
