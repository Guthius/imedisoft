...
namespace OpenDental {
	public partial class FormEhrPatientSmoking:FormODBase {
		...
		private void comboTobaccoStatus_SelectionChangeCommitted(object sender,EventArgs e) {
			...
			if(!_listTobaccoStatuses.Any(x => x.CodeValue==FormS.SnomedSelected.SnomedCode)) {
				_listCustomTobaccoCodes.Add(new EhrCode() { CodeValue=FormS.SnomedSelected.SnomedCode,Description=FormS.SnomedSelected.Description });
				_listCustomTobaccoCodes=_listCustomTobaccoCodes.OrderBy(x => x.Description).ToList();
			...
			comboTobaccoStatus.SelectedIndex=_listTobaccoStatuses.FindIndex(x => x.CodeValue==FormS.SnomedSelected.SnomedCode);//add 1 for ...choose from
		}
