...
namespace OpenDental {
	...
	public partial class FormPaySplitEdit : FormODBase {
		...
		private List<FauxAccountEntry> _listFauxAccountEntries=new List<FauxAccountEntry>();
		#endregion
		...
		private void SetCheckPayPlanEnabled() {
			...
			for(int i=0;i<_listFauxAccountEntries.Count;i++) {
			...
			List<FauxAccountEntry> listFauxAccountEntriesDynamic=_listFauxAccountEntries.FindAll(x => x.IsDynamic);
			//Look for any dynamic payment plan that only has credits (no debits).
			bool hasNoDebits=listFauxAccountEntriesDynamic.GroupBy(x => x.PayPlanNum)
		...
		private void butAttachProc_Click(object sender, System.EventArgs e) {
			using FormProcSelect formProcSelect=new FormProcSelect(PaySplitCur.PatNum);
		...
		private void checkPayPlan_Click(object sender, System.EventArgs e) {
			...
			PayPlan payPlan=listPayPlans.Find(x => x.PayPlanNum==formPayPlanSelect.PayPlanNumSelected);
		...
		private void AttachPayPlan(PayPlan payPlan) {
			...
			List<FauxAccountEntry> listFauxAccountEntries=_listFauxAccountEntries.FindAll(x => x.PayPlanNum==payPlan.PayPlanNum);
			...
			List<FauxAccountEntry> listFauxAccountEntriesForProduction=new List<FauxAccountEntry>();
			for(int i=0;i<listFauxAccountEntries.Count;i++) {
			...
			if(fauxAccountEntryOldest.ProcNum > 0) {
				AccountEntry accountEntry=ListAccountEntries.Find(x => x.GetType()==typeof(Procedure) && x.ProcNum==fauxAccountEntryOldest.ProcNum);
				if(accountEntry!=null) {
					_procedure=(Procedure)accountEntry.Tag;
				}
			}
			else if(fauxAccountEntryOldest.AdjNum > 0) {
				AccountEntry accountEntry=ListAccountEntries.Find(x => x.GetType()==typeof(Adjustment) && x.AdjNum==fauxAccountEntryOldest.AdjNum);
				if(accountEntry!=null) {
