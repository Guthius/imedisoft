...
namespace OpenDental {
	public partial class FormProvStudentBulkEdit:FormODBase {
		...
		private void FillGrid() {
			...
			for(int i=0;i<table.Rows.Count;i++) {
				if(table.Rows[i]["ProvNum"].ToString()==provNumSelected.ToString()) {
		...
		private void butBulkEdit_Click(object sender,EventArgs e) {
			for(int i=0;i<gridStudents.SelectedIndices.Length;i++) {
				Provider providerStudSelected=Providers.GetProv(PIn.Long(gridStudents.ListGridRows[i].Cells[0].Text));
				providerStudSelected.ProvColor=butColor.BackColor;
				providerStudSelected.OutlineColor=butOutlineColor.BackColor;
				Providers.Update(providerStudSelected);
			}
