...
namespace OpenDental{
	...
	public partial class FormClaimsSend:FormODBase {
		...
		public void FillClinicsList(long claimCustomTrackingNum) {//Not using ComboBoxClinicPicker due to adding number of unsent clinics to label of each item
			...
			List<ClinicDto> listClinicsPrevious=new List<ClinicDto>();
			...
			if(!Security.CurUser.ClinicIsRestricted) {
				ClinicDto clinicUnassigned=new ClinicDto();
				clinicUnassigned.Abbr=Lan.g(this,"Unassigned/Default");
				comboClinic.Items.Add(clinicUnassigned.Abbr,clinicUnassigned);
				comboClinic.SetSelectedKey<ClinicDto>(clinicUnassigned.Id,x=>x.Id,x=>clinicUnassigned.Abbr);
			}
			for(int i=0;i<_listClinics.Count;i++) {
				...
				if(_listClinics[i].Id==Clinics.ClinicNum) {
					comboClinic.SetSelectedKey<ClinicDto>(Clinics.ClinicNum,x=>x.Id,x=>strClinicWithCount);
				}
			}
			List<int> listSelectedIndices=new List<int>();
			List<long> listClinicNumOptions=comboClinic.Items.GetAll<ClinicDto>().Select(x=>x.Id).ToList();
			for(int i=0;i<listClinicsPrevious.Count;i++) {
				int selectedClinicIndex=listClinicNumOptions.IndexOf(listClinicsPrevious[i].Id);
