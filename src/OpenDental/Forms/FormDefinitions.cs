...
namespace OpenDental{
	...
	public partial class FormDefinitions : FormODBase {
		...
		private void FormDefinitions_Load(object sender, System.EventArgs e) {
			...
			List<DefCat> listDefCats=((DefCat[])Enum.GetValues(typeof(DefCat))).ToList();
		...
		private void FormDefinitions_Closing(object sender,System.ComponentModel.CancelEventArgs e) {
			...
			List<KeyValuePair<DefCat,List<Def>>> listKeyValuePairs=Defs.GetDeepCopy().ToList();
			for(int i=0;i<listKeyValuePairs.Count;i++) {
				for(int j=0;j<listKeyValuePairs[i].Value.Count;j++) {
