...
namespace OpenDental{
	...
	public partial class FormApptEdit:FormODBase {
		...
		private void FormApptEdit_Load(object sender,System.EventArgs e) {
			...
			//AppointmentType
			List<AppointmentType> listAppointmentTypes=AppointmentTypes.GetWhere(x => !x.IsHidden || x.AppointmentTypeNum==_appointment.AppointmentTypeNum);
		...
		private void comboStatus_SelectionChangeCommitted(object sender,EventArgs e) {
			...
			if(PrefC.GetBool(PrefName.BrokenApptRequiredOnMove)) {
				if(_appointmentOld.AptStatus==ApptStatus.Scheduled && GetApptStatusSelected()==ApptStatus.UnschedList) {
					FrmApptBreakRequired frmApptBreakRequired=new FrmApptBreakRequired();
		...
		private void butPickHyg_Click(object sender,EventArgs e) {
			FrmProviderPick frmProviderPick=new FrmProviderPick(comboProvHyg.Items.GetAll<Provider>());//none option will show.
		...
		private void butInsPlan1_Click(object sender,EventArgs e) {
			using FormInsPlanSelectFam formInsPlanSelect=new FormInsPlanSelectFam(_appointment.PatNum);
		...
		private void butDeleteProc_Click(object sender,EventArgs e) {
			...
			if(comboApptType.SelectedIndex>0) {
				AppointmentType appointmentType=comboApptType.GetSelected<AppointmentType>();
				if(appointmentType.RequiredProcCodesNeeded!=EnumRequiredProcCodesNeeded.None) {
					...
					string message=Appointments.CheckRequiredProcForApptType(listProceduresForReqCheck);
					if(message!=""){
						MsgBox.Show(message);
						return;
					}
			...
			for(int i = gridProc.SelectedIndices.Length-1;i>=0;i--) {
				...
				if(procedure.ProcStatus.In(ProcStat.C,ProcStat.EO,ProcStat.EC)) {
					EnumPermType perm=EnumPermType.ProcCompleteStatusEdit;
		...
		private void butAdd_Click(object sender,EventArgs e) {
			...
			Procedure procedure=Procedures.ConstructProcedureForAppt(formProcCodes.CodeNumSelected,_appointment,_patient,_listPatPlans,_listInsPlans,_listInsSubs,listFees);
			...
			if(apptField==null) {
				...
				if(apptFieldDef==null) {//This could happen if the field def was deleted while the appointment window was open.
					...
					if(apptFieldDef.FieldType==ApptFieldType.Text) {
						FrmApptFieldEdit frmApptFieldEdit=new FrmApptFieldEdit(apptField);
						frmApptFieldEdit.ShowDialog();
					}
					else if(apptFieldDef.FieldType==ApptFieldType.PickList) {
						FrmApptFieldPickEdit FrmApptFieldPickEdit=new FrmApptFieldPickEdit(apptField);
		...
		private void butAudit_Click(object sender,EventArgs e) {
			...
			FrmAuditOneType frmAuditOneType=new FrmAuditOneType(_patient.PatNum,listPermissions,Lan.g(this,"Audit Trail for Appointment"),_appointment.AptNum);
		...
		private void butTask_Click(object sender,EventArgs e) {
			...
			if(_appointment.AptNum!=0 && !PrefC.GetBool(PrefName.TasksForApptAllowMultiple)){
				List<Task> listTasksToOpen=Tasks.GetMany(_appointment.AptNum);
				if(listTasksToOpen.Count==1){
					//Exactly one attached. Show it.
					using FormTaskEdit formTaskEditExisting=new FormTaskEdit(listTasksToOpen.FirstOrDefault());
					formTaskEditExisting.ShowDialog();
					return;
				}
				else if(listTasksToOpen.Count>1){
					//Many attached. Show FormTasksForAppt.
					FormTasksForAppt formTasksForAppt=new FormTasksForAppt(_appointment.AptNum);
		...
		private void CalculatePatternFromProcs(bool ignoreTimeLocked = false) {
			...
			bool timeLocked=checkTimeLocked.Checked;
			if(!IsNew && !OpenDentBusiness.Security.IsAuthorized(OpenDentBusiness.EnumPermType.AppointmentResize,suppressMessage:true)) {
				timeLocked=true;
		...
		private void FillProcedures() {
			...
			bool isMedical=Clinics.IsMedicalPracticeOrClinic(comboClinic.ClinicNumSelected);
		...
		private string GetLabCaseDescript() {
			...
			for(int i=0;i<_listLabCases.Count;i++) {
				...
				if(_listLabCases[i].DateTimeChecked.Year>1880) {//Logic from Appointments.cs lines 1098 to 1117
					...
					return strLabCaseDescript.ToString();
				}
		...
		private bool UpdateListAndDB(bool isClosing = true,bool doCreateSecLog = false,bool doInsertHL7 = false) {
			...
			{
				TimeSpan timeSpan=TimeSpan.FromMinutes(_appointment.Pattern.Length*5);
				string message=Lans.g("Security","Not authorized for")+" "+GroupPermissions.GetDesc(EnumPermType.AppointmentResize)
...
//todo? Consider changing PatternSecondary length to match Pattern length.  But there are many places in the program where this would need to be done.  Probably easier to assume they can be out of synch.
			...
			if(_appointmentOld.AptNum > 0) {
				...
				Appointment appointment=Appointments.GetOneApt(_appointmentOld.AptNum);
				if(appointment!=null) {
