	...
	public partial class FormBillingOptions : FormODBase {
				...
				foreach(PrefName prefName in listMissingClincPrefs) {
					switch(EnumTools.GetAttributeOrDefault<PrefNameAttribute>(prefName).ValueType) {
						...
						case PrefValueType.INT:
							int defaultInt=PrefC.GetInt(prefName);
		...
		private void SetFiltersForClinicNums(List<long> listClinicNums,bool isTextNoteExcluded=false) {
				...
				|| !_dictionaryClinicPrefsNew.ContainsKey(listClinicNums[0]))//They have not saved their default filter options for the selected clinic. Use default prefs.
			{
				...
				string[] stringArraySelectedInsFilingCodes=PrefC.GetString(PrefName.BillingSelectInsFilingCodes).Split(",",StringSplitOptions.RemoveEmptyEntries);
				if(stringArraySelectedInsFilingCodes.Length==0) {//Set to None.
					...
					for(int i=0;i < stringArraySelectedInsFilingCodes.Length;++i) {
						try {
							int order=_listInsFilingCodes.FindIndex(x => x.InsFilingCodeNum==Convert.ToInt64(stringArraySelectedInsFilingCodes[i]));
							if(order==-1) {
								continue;
							}
				...
				#region BillTypes
				listBillType.ClearSelected();
				string[] stringArraySelectedBillTypes=listClinicPrefs.First(x => x.PrefName==PrefName.BillingSelectBillingTypes)
				...
				#region Ins Filing Codes
				comboInsFilingCodes.SetAll(false);
				string[] stringArraySelectedInsFilingCodes=listClinicPrefs.First(x => x.PrefName==PrefName.BillingSelectInsFilingCodes)
		...
		private bool areFilingCodeSelectionsValid() {
			...
			bool isNoneAndAllSelected=comboInsFilingCodes.SelectedIndices.Contains(0) && comboInsFilingCodes.SelectedIndices.Contains(1);//Cannot select both None and all.
			bool isNoneAllAndFilingCodesSelected=(comboInsFilingCodes.SelectedIndices.Contains(0) || comboInsFilingCodes.SelectedIndices.Contains(1))
		...
		private void butSaveDefault_Click(object sender,System.EventArgs e) {
			...
			string selectedInsFilingCodes="";//indicates None.
		...
		private void butCreate_Click(object sender, System.EventArgs e) {
			...
			sendStatementsIO.ActionPrompt=(prompt,useCopyPasteDialog) => { ODMessageBox.Show(prompt); };
		...
		private void comboInsFilingCodes_SelectionChangeCommitted(object sender,EventArgs e) {
			if(comboInsFilingCodes.SelectedIndices.Count < 1) {//No selections made, default to None.
				comboInsFilingCodes.SelectedIndices=new List<int>() { 0 };
			}
		}
		...
		private int CreateHelper(long clinicNum,bool useClinicPrefs=false,bool suppressPopup=false,Dictionary<long,PatAgingData> dictionaryPatAgingData=null) {
			...
			DateTime lastStatement=SIn.Date(textLastStatement.Text);
			...
			List<long> listInsFilingCodeNums=new List<long>();
			bool filterByInsFilingCodes=true;
			...
			if(comboInsFilingCodes.SelectedIndices[0]==0) {//None
				...
				for(int i=0;i < comboInsFilingCodes.SelectedIndices.Count;i++) {
			...
			Dictionary<long,List<PatAgingTransaction>> dictionaryPatAgingTransactions=new Dictionary<long,List<PatAgingTransaction>>();
			...
			DateTime dateRangeFrom=SIn.Date(textDateStart.Text);
							...
							&& (x.FieldValue.ToLower().Contains(MsgToPayTagReplacer.STATEMENT_URL_TAG.ToLower()) || x.FieldValue.ToLower().Contains(MsgToPayTagReplacer.STATEMENT_SHORT_TAG.ToLower())))))
			{
				...
				try {
					int countForSms=listPatAgings.Count(x => Statements.DoSendSms(x,dictionaryPatAgingData,listModeToText.GetListSelected<StatementMode>()));
			...
			for(int i = 0;i<listPatAgings.Count;++i) {
				...
				bool doSendSms=Statements.DoSendSms(listPatAgings[i],dictionaryPatAgingData,listModeToText.GetListSelected<StatementMode>());
