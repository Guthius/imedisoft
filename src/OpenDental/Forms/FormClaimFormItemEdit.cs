...
namespace OpenDental {
	...
	public partial class FormClaimFormItemEdit : FormODBase,IFormClaimFormItemEdit {
		...
		public ClaimFormItem ClaimFormItemCur;
		...
		private void FillForm(){
			textImageFileName.Text=ClaimFormItemCur.ImageFileName;
			textFormatString.Text=ClaimFormItemCur.FormatString;
			listFieldName.Items.Clear();
			for(int i=0;i<FieldNames.Length;i++){
				listFieldName.Items.Add(FieldNames[i]);
				if(FieldNames[i]==ClaimFormItemCur.FieldName){
		...
		private void SaveAndClose(){
			ClaimFormItemCur.ImageFileName=textImageFileName.Text;
			ClaimFormItemCur.FormatString=textFormatString.Text;
			if(listFieldName.SelectedIndex==-1){
				ClaimFormItemCur.FieldName="";
			}
			else{
				ClaimFormItemCur.FieldName=FieldNames[listFieldName.SelectedIndex];
			}
			if(IsNew)
				ClaimFormItems.Insert(ClaimFormItemCur);
			else
				ClaimFormItems.Update(ClaimFormItemCur);
			DialogResult=DialogResult.OK;
		}
