...
namespace OpenDental {
	...
	public partial class FormProcCodes:FormODBase {
		...
		private void FormProcCodes_Load(object sender,System.EventArgs e) {
			...
			MenuItem menuItemFeeSchedNotes=new MenuItem();
		...
		private void FillComboBoxes() {
			...
			string defaultClinicText=(true ? "Default" : "None");
			...
			if(!true) {//No clinics
					...
					&& (checkGroups1.Checked || checkGroups2.Checked || checkGroups3.Checked))
				{
					...
					List<long> listClinicNums=_listClinics.Select(x => x.Id).ToList();
		...
		private void GridCellUpdateAndSecurityCheck(ODGridClickEventArgs e) {
			...
			if(PrefC.GetBool(PrefName.ShowFeeSchedGroups) && provNum==0 && !isEditingGroup) {//Ignore provider fees and don't block from editing a group.
				...
				if(feeSchedGroupForClinic!=null) {
					...
					MsgBox.Show(Lans.g(this,"Fee Schedule")+": "+feeSched.Description+" "+Lans.g(this,"for Clinic")+": "+(_listClinics.FirstOrDefault(x => x.Id==clinicNum)?.Abbr??"")+" "
		...
		private void MenuItem_Click(object sender,EventArgs e) {
			...
			int col=gridMain.PointToCol(_pointLastClicked.X);
			if(col!=4 && col!=5 && col!=6){
				MsgBox.Show("Please select a fee column");
				return;
			}
			//Now we are guaranteed to be within the fee schedule columns
			FrmFeeSchedNotes frmFeeSchedNotes =new FrmFeeSchedNotes();
			if(col==4){//Fee col 1
			...
			if(col==5){//Fee col 2
				...
				int selectedIndex=comboFeeSched2.SelectedIndex-1;
				if(selectedIndex==-1){
			...
			if(col==6){//Fee col 3
				//ComboFeeSched2 and comboFeeSched3 have a dummy "None" option that we need to worry about
				int selectedIndex=comboFeeSched3.SelectedIndex-1;
				if(selectedIndex==-1){
					MsgBox.Show("Please select a fee schedule for this column first");
					return;
				}
		...
		private void butPickClinic_Click(object sender,EventArgs e){
			List<ClinicDto> listClinicsToShow=new List<ClinicDto>();
