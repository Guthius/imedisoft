	...
	public partial class FormErxSetup:FormODBase {
		...
		private void FormErxSetup_Load(object sender,EventArgs e) {
			try {
				...
				if(true) {
					foreach(var clinicCur in Clinics.GetAllForUserod(Security.CurUser)) {
						if(!listProgramPropertiesClinicIDs.Exists(x => x.ClinicNum==clinicCur.Id)) {//Only add a program property if it doesn't already exist.
							ProgramProperty programProperty=new ProgramProperty();
							programProperty.ProgramNum=_program.ProgramNum;
							programProperty.ClinicNum=clinicCur.Id;
							programProperty.PropertyDesc=Erx.PropertyDescs.ClinicID;
							programProperty.PropertyValue="";
							_listProgramProperties.Add(programProperty);
						}
						if(!listProgramPropertiesClinicKeys.Exists(x => x.ClinicNum==clinicCur.Id)) {//Only add a program property if it doesn't already exist.
							ProgramProperty programProperty=new ProgramProperty();
							programProperty.ProgramNum=_program.ProgramNum;
							programProperty.ClinicNum=clinicCur.Id;
							programProperty.PropertyDesc=Erx.PropertyDescs.ClinicKey;
		...
		private void FillGridDoseSpot() {
			...
			for(int i = 0;i<listProgramPropertiesHQClinicIDs.Count();i++) {
				...
				doseSpotGridRowModel.ClinicCur=new ClinicDto();
			...
			if(true) {
				foreach(var clinicCur in Clinics.GetAllForUserod(Security.CurUser)) {
					...
					List<ProgramProperty> listProgramPropertiesClinicIDs=GetPropertyForClinic(clinicCur.Id,Erx.PropertyDescs.ClinicID);
					List<ProgramProperty> listProgramPropertiesClinicKeys=GetPropertyForClinic(clinicCur.Id,Erx.PropertyDescs.ClinicKey);
					...
					for(int i = 0;i<listProgramPropertiesClinicIDs.Count();i++) {
						DoseSpotGridRowModel doseSpotGridRowModel=new DoseSpotGridRowModel();
						try {
							// TODO: doseSpotGridRowModel.ClinicCur=clinicCur.Copy();
