		...
		private void butProcess_Click(object sender,EventArgs e) {
			...
			for(int i=0;i<_listNewCropCharges.Count;i++) {
				if(_listNewCropCharges[i].repeatCharge==null) {//No such repeating charge exists yet for the given npi.
					...
					OrthoCaseProcedureLinker orthoCaseProcedureLinker=OrthoCaseProcedureLinker.CreateOneForPatient(_listNewCropCharges[i].repeatCharge.PatNum);
					try {
						procedurePrevMonth=RepeatCharges.AddProcForRepeatCharge(_listNewCropCharges[i].repeatCharge,dateNow,dateNow,orthoCaseProcedureLinker,isNewCropInitial:true);
						if(isFutureDated) {//Since we are future dating the repeating charge we need a second proc for the current month
							procedureCurrentMonth=RepeatCharges.AddProcForRepeatCharge(_listNewCropCharges[i].repeatCharge,dateNow,dateNow,orthoCaseProcedureLinker,isNewCropInitial:false,isNewCropFutureDated:true);
						}
					}
