...
namespace OpenDental {
	...
	public partial class FormInsPlan : FormODBase {
		...
		private void FillPayor() {
			...
			List<string> listPayorNames=ElectIDs.GetDescripts(textElectID.Text);
			if(listPayorNames.Count>1) {
				comboElectIDdescript.Items.Add("multiple payors use this ID");
			}
			for(int i=0;i<listPayorNames.Count;i++) {
		...
		private void textElectID_Validating(object sender,System.ComponentModel.CancelEventArgs e) {
			...
			List<string> listElectIDs=ElectIDs.GetDescripts(textElectID.Text);
		...
		private void butChange_Click(object sender,EventArgs e) {
			...
			try {
				...
				if(PrefC.GetBool(PrefName.SubscriberAllowChangeAlways)) {
					DialogResult dialogResult=ODMessageBox.Show(Lan.g(this,"Warning!  Do not change unless fixing database corruption.  ")+"\r\n"+ex.Message);
			...
			FrmFamilyMemberSelect frmFamilyMemberSelect=new FrmFamilyMemberSelect(family,true);
			...
			Patient patientSubscriber=Patients.GetLim(frmFamilyMemberSelect.SelectedPatNum);
		...
		private void butImportTrojan_Click(object sender,System.EventArgs e) {
			...
			if(trojanObject.PlanNote!="") {
				if(textPlanNote.Text=="") {
					...
					using FormNotePick formNotePick=new FormNotePick();
			...
			List<Benefit> listBenefitsToKeep=_listBenefits.FindAll(x => Benefits.IsFrequencyLimitation(x) || Benefits.IsAgeLimit(x));
			_listBenefits=new List<Benefit>();
		...
		private void EligibilityCheckDentalXchange() {
			Cursor = Cursors.WaitCursor;
			OpenDentBusiness.com.dentalxchange.webservices.WebServiceService DCIService 
				= new OpenDentBusiness.com.dentalxchange.webservices.WebServiceService();
			OpenDentBusiness.com.dentalxchange.webservices.Credentials DCICredential 
				= new OpenDentBusiness.com.dentalxchange.webservices.Credentials();
			OpenDentBusiness.com.dentalxchange.webservices.Request DCIRequest = new OpenDentBusiness.com.dentalxchange.webservices.Request();
			OpenDentBusiness.com.dentalxchange.webservices.Response DCIResponse = new OpenDentBusiness.com.dentalxchange.webservices.Response();
			string loginID;
			string passWord;
			// Get Login / Password
		...
		private bool FillPlanCurFromForm() {
			...
			if(comboCopay.SelectedIndex==0) {
				...
				if(IsFixedBenefitMismatch(FeeScheds.GetFirstOrDefault(x => x.FeeSchedNum==comboCopay.GetSelectedKey<FeeSched>(x=>x.FeeSchedNum)))) {
					ODMessageBox.Show(Lans.g(this,"PPO Fixed Benefits Fee Schedules can only be assigned to PPO Fixed Benefits plan types. "
						+"Please make a valid selection from ")+labelCopayFeeSched.Text);
					return false;	
				}
				if(ODMessageBox.Show(this,Lans.g(this,"The selected ")+labelCopayFeeSched.Text
		...
		private void butPatOrtho_Click(object sender,EventArgs e) {
			...
			double defaultFee=SIn.Double(textOrthoAutoFee.Text);
			string carrierName=SIn.String(textCarrier.Text);
			string subID=SIn.String(textSubscriberID.Text);
			if(defaultFee==0) {
