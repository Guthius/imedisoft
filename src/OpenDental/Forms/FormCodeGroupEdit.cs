	...
	public partial class FormCodeGroupEdit:FormODBase {
		...
		private void FormCodeGroupEdit_Load(object sender,EventArgs e) {
			if(!CultureInfo.CurrentCulture.Name.EndsWith("US")) {//United States
				labelCodeListDesc.Text=Lans.g(this,"List of procedure codes with commas and dashes. No spaces.");
			}
		...
		private void butProcCodesAdd_Click(object sender,EventArgs e) {
			...
			List<string> listProcCodes=GetProcCodesFromUI();//this list can included hyphenated entries.
		...
		private void butSave_Click(object sender,EventArgs e) {
			...
			string errorMessage="";
			List<string> listCodesByComma=GetProcCodesFromUI();
			for(int i=0;i<listCodesByComma.Count;i++) {
				List<string> listCodesByDash=listCodesByComma[i].Split('-').ToList();
				if(listCodesByDash.Count==1) {
					continue;
				}
				if(listCodesByDash.Count>2) {
					errorMessage+="Ranges of codegroups must be formatted with a dash between two codes.\r\n";
					break;
				}
			}
			...
			CodeGroup.ProcCodes=string.Join(",",listCodesByComma);
