...
namespace OpenDental {
	public partial class FormMassEmailAnalytics:FormODBase {
		...
		private void FillGridAnalytics() {
			...
			long clinicNum=true ? comboClinicAnalytics.ClinicNumSelected : -1;
		...
		private string GetCountString(PromotionAnalytic promotionAnalytic,int total,params PromotionLogStatus[] paramsPromotionLogStatuses) {
			int count=0;
			for(int i=0;i<paramsPromotionLogStatuses.Length;i++) {
				...
				promotionAnalytic.DictionaryCounts.TryGetValue(paramsPromotionLogStatuses[i],out int countStatus);
				count+=countStatus;
			}
