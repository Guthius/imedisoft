	...
	public partial class FormASAP:FormODBase {
		...
		private void MakeAppointment_Click() {
			...
			for(int i=0;i<listRecallsSelected.Count;i++) {
				...
				GlobalFormOpenDental.GoToModule(EnumModuleType.Appointments, listPinApptNums:new List<long>() { appointment.AptNum }, patNum:patient.PatNum, dateSelected:DateTime.Today);
			}
			FillGridRecalls();
		}
		...
		private void butText_Click(object sender,EventArgs e) {
			...
			using FormTxtMsgMany formTxtMsgMany=new FormTxtMsgMany(listPatCommsToSend,textTemplate,clinic.Id,SmsMessageSource.AsapManual);
		...
		private string GetTextMessageText(ClinicDto curClinic) {
			...
			if(inputBox.IsDialogOK) {
				...
				if(textTemplate.Contains("[Time]") && inputBox.TimeSpanResult!=TimeSpan.Zero){//A time was entered.
					...
					DateTime dateTime=DateTime.MinValue+inputBox.TimeSpanResult;
		...
		private void CheckClinicsSignedUpForWebSched() {
			...
			_threadWebSchedSignups=new ODThread((ODThread o) => {
			...
			_threadWebSchedSignups.AddExceptionHandler((Exception ex) => { });
			_threadWebSchedSignups.AddExitHandler((ODThread o) => {
				try {
		...
		private void SendWebSched() {
			...
			bool isSignedUp=(_listClinicNumsWebSched.Contains(clinicNum) || (!true && _listClinicNumsWebSched.Count > 0));
			if(!isSignedUp) {
