...
namespace OpenDental {
	public partial class FormArManager:FormODBase {
		...
		private void LayoutMenu() {
			...
			menuMain.EndUpdate();
			if(!true) {//clinics disabled the menu will be hidden, but will still contain ClinicNum 0
		...
		private void SetTotalsLocAndVisible(DisplayFieldCategory displayCat,int hScrollValue) {
			...
			Dictionary<string,System.Windows.Forms.TextBox> dictColTextBox = new Dictionary<string,System.Windows.Forms.TextBox>() {
		...
		private List<int> GetPatAgingIndexUnsentFiltered() {
			...
			double minBalance=Math.Round(SIn.Double(textUnsentMinBal.Text),3);
			DateTime dtLastPay=DateTime.Today.AddDays(-SIn.Int(textUnsentDaysLastPay.Text));
		...
		private void butSend_Click(object sender,EventArgs e) {
			...
			List<long> listPatNumsToReselect=listPatAging.FindAll(x => !dictClinicSelectedServices.ContainsKey(true?x.ClinicNum:0)
			...
			foreach(PatAging pAgingCur in listPatAging) {
				long clinicNum=true?pAgingCur.ClinicNum:0;
		...
		private List<int> GetPatAgingIndexSentFiltered() {
			...
			#endregion Validate Inputs
			#region Get Filter Data
			double minBalance=Math.Round(SIn.Double(textSentMinBal.Text),3);
			DateTime dtLastPay=DateTime.Today.AddDays(-SIn.Int(textSentDaysLastPay.Text));
		...
		private List<int> GetPatAgingIndexExcludedFiltered() {
			...
			#endregion Validate Inputs
			#region Get Filter Data
			double minBalance=Math.Round(SIn.Double(textExcludedMinBal.Text),3);
			DateTime dtLastPay=DateTime.Today.AddDays(-SIn.Int(textExcludedDaysLastPay.Text));
