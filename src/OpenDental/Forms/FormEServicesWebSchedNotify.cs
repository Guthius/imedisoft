...
namespace OpenDental {
	public partial class FormEServicesWebSchedNotify:FormODBase {
		...
		private void toolStripMenuItemInsertReplacements_Click(object sender,EventArgs e) {
			ToolStripItem toolStripItem=(ToolStripItem)sender;
			ContextMenuStrip contextMenuStrip=(ContextMenuStrip)toolStripItem.Owner;
			TextBox textBox=((TextBox)contextMenuStrip.SourceControl);
			//PHI is not supposed to be communicated via text message.
			bool allowPHI=(!textBox.Name.In(textMessageTemplate.Name));
		...
		private void WebSchedVerify_RadioButtonCheckChanged(object sender,EventArgs e) {
			RadioButton radioChanged=(RadioButton)sender;
		...
		private void SaveWebSchedVerify() {
			List<long> listClinicNums=Clinics.GetForUserod(Security.CurUser).Select(x => x.Id).ToList();
			for(int i = 0;i<_listPrefNamesWebSchedVerify.Count;i++) {
				for(int j = 0;j<listClinicNums.Count;j++) {
					ClinicPref clinicPrefNew=_listClinicPrefsWebSchedVerify.FirstOrDefault(x => x.PrefName==_listPrefNamesWebSchedVerify[i] && x.ClinicNum==listClinicNums[j]);
					ClinicPref clinicPrefOld=_listClinicPrefsWebSchedVerify_Old.FirstOrDefault(x => x.PrefName==_listPrefNamesWebSchedVerify[i] && x.ClinicNum==listClinicNums[j]);
				...
				ClinicPref clinicPrefDefault=_listClinicPrefsWebSchedVerify.FirstOrDefault(x => x.PrefName==_listPrefNamesWebSchedVerify[i] && x.ClinicNum==_clinicNumDefault);
