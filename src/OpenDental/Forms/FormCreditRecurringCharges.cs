...
namespace OpenDental {
	public partial class FormCreditRecurringCharges:FormODBase {
		...
		private void FillGrid(bool isFromDb=false) {
			...
			List<long> listSelectedClinicNums=listClinics.SelectedIndices.OfType<int>().Select(x => _listClinics[x].Id).ToList();
			...
			for(int i=0;i<listRecurringChargeDatas.Count;i++){
				...
				double rptChargeAmt=listRecurringChargeDatas[i].RecurringCharge.RepeatAmt;//includes repeat charge (from procs) and attached payplan -
