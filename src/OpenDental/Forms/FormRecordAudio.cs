...
namespace OpenDental {
	public partial class FormRecordAudio:FormODBase {
		public string Sound;
		private DateTime _dateRecordStart;
		...
		private void timerRecord_Tick(object sender,EventArgs e) {
			labelTimer.Text=(DateTime.Now-_dateRecordStart).ToStringHmmss();
		}

		private void butStart_Click(object sender,EventArgs e) {
			if(butStart.Text==Lan.g(this,"Record")) {
				timerRecord.Start();
				_dateRecordStart=DateTime.Now;
		...
		private void butPlay_Click(object sender,EventArgs e) {
			byte[] byteArray=File.ReadAllBytes(_tempPath);
			SoundHelper.PlaySound(byteArray);
		}
		...
		private void butSave_Click(object sender,EventArgs e) {
			...
			File.Copy(_tempPath,saveFileDialog.FileName,true);
		}
