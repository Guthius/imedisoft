...
namespace OpenDental {
	public partial class FormProcCodeEditMore:FormODBase {
		...
		private void FillAndSortListFees() {
			List<long> listClinicNums=Clinics.GetForUserod(Security.CurUser,true).Select(x => x.Id).ToList();
		...
		private void FillGrid() {
			...
			for(int i=0;i<_listFees.Count;i++) {
				...
				string dateEffective=_listFees[i].DateEffective.ToShortDateString();
