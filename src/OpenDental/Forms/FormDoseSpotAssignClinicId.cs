	...
	public partial class FormDoseSpotAssignClinicId:FormODBase {
		...
		private void FormDoseSpotAssignUserId_Load(object sender,EventArgs e) {
			...
			_listClinics.RemoveAll(x =>//Remove all clinics that already have a DoseSpot Clinic ID OR Clinic Key entered
				_listProgramPropertiesClinicIDs.FindAll(y => !string.IsNullOrWhiteSpace(y.PropertyValue)).Select(y => y.ClinicNum).Contains(x.Id)
				|| _listProgramPropertiesClinicKeys.FindAll(y => !string.IsNullOrWhiteSpace(y.PropertyValue)).Select(y => y.ClinicNum).Contains(x.Id)
			);
			FillComboBox();
		...
		private void FillComboBox(long selectedClinicNum=-1) {
			...
			for(int i=0;i<_listClinics.Count;i++){
				...
				if(_listClinics[i].Id==selectedClinicNum) {
		...
		private void butSave_Click(object sender,EventArgs e) {
			...
			_clinicErx.ClinicNum=comboClinics.GetSelected<ClinicDto>().Id;
		...
		private void butClinicPick_Click(object sender,EventArgs e) {
			...
			formClinics.Clinics=_listClinics;
