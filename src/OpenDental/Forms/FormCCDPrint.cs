...
namespace OpenDental {
	public partial class FormCCDPrint:FormODBase {
		...
		private void Init(){
			...
			CCDField fieldA02=_ccdReceived.GetFieldById("A02");
			if(fieldA02?.valuestr!=null) {
				Etrans etransSent=Etranss.GetForSequenceNumberCanada(fieldA02.valuestr);
				if(etransSent!=null && etransSent.EtransMessageTextNum!=0) {
					try {
						string messageSent=EtransMessageTexts.GetMessageText(etransSent.EtransMessageTextNum);
			...
			CCDField ccdFieldLanguageOfInsured=_ccdReceived.GetFieldById("G27");
			...
			if(_formatVersionNumber=="04") {//FormId field does not exist in version 02 in any of the message texts.
				CCDField ccdFieldFormId=_ccdReceived.GetFieldById("G42");//Usually exists in version 04 response messages.
				...
				if(_transactionCode=="10") {//Eligibility Response.
					...
					_formId="01";//EOB Form
					CCDField ccdFieldG02=_ccdReceived.GetFieldById("G02");
			...
			CCDField ccdFieldStatus=_ccdReceived.GetFieldById("G05");
			...
			if(_copiesToPrint<=0) { //Show the form on screen if there are no copies to print.
				ShowDisplayMessages();
				CCDField ccdFieldPayTo=_ccdReceived.GetFieldById("F01");
			...
			CCDField ccdFieldEmbeddedTransaction=_ccdReceived.GetFieldById("G40");
		...
		private void butOverride_Click(object sender,EventArgs e) {
			...
			List<Procedure> listProcedures=Procedures.Refresh(_etrans.PatNum);
			List<ClaimProc> listClaimProcs=ClaimProcs.Refresh(_etrans.PatNum);
		...
		public void ShowDisplayMessages(){
			...
			CCDField[] ccdFieldArrayDisplayMessage=_ccdReceived.GetFieldsById("G32");
			...
			CCDField[] ccdFieldArrayNoteOutputFlags=_ccdReceived.GetFieldsById("G41");
			CCDField[] ccdFieldArrayNoteNumbers=_ccdReceived.GetFieldsById("G45");
			CCDField[] ccdFieldArrayNoteTexts=_ccdReceived.GetFieldsById("G26");
		...
		private void pd_PrintPage(object sender,System.Drawing.Printing.PrintPageEventArgs e){
			...
			if(_claimResponseDefNum!=0) {//Add current page as an image to the PDF.
				using(Bitmap pageImage=new Bitmap(e.PageBounds.Size.Width,e.PageBounds.Size.Height))
				using(Graphics g=Graphics.FromImage(pageImage)) {
					...
					MigraDoc.DocumentObjectModel.Section section=_pdfDocument.AddSection();
					TextFrame textFrame=MigraDocHelper.CreateContainer(section);
			...
			if(_pagesPrinted<_totalPages){					
				...
				if(_claimResponseDefNum!=0) {//Import the PDF to the patient images in the Claim Response folder.
					...
					using(MemoryStream ms=new MemoryStream()) {
						...
						ms.Position=0;
						byte[] arrayImageBytes=new byte[ms.Length];//Cannot use ms.GetBuffer() because the buffer can be longer than ms.Length.
						ms.Read(arrayImageBytes,0,(int)ms.Length);
						Document doc=ImageStore.Import(arrayImageBytes,_claimResponseDefNum,ImageType.File,_patient,fileExtension:".pdf");
		...
		private void PrintEmail_24(Graphics g){
			...
			CCDField ccdField=_ccdReceived.GetFieldById("G54");
			...
			_x=_documentGenerator.StartElement(_verticalLine);
			CCDField[] ccdFieldArrayNoteLines=_ccdReceived.GetFieldsById("G53");//BODY OF MESSAGE
		...
		private void PrintSummaryReconciliation_15(Graphics g){
			...
			_documentGenerator.StartElement();
			CCDField ccdField=_ccdReceived.GetFieldById("G34");//Payment reference
			if(ccdField!=null) {
			...
			_documentGenerator.HorizontalLine(g,_penBreakLine,_documentGenerator.bounds.Left,_documentGenerator.bounds.Right,0);
			_documentGenerator.StartElement();
			CCDField[] ccdFieldArrayCdaProviderNumbers=_ccdReceived.GetFieldsById("B01");
			CCDField[] ccdFieldArrayCarrierIdentificationNumbers=_ccdReceived.GetFieldsById("A05");
			CCDField[] ccdFieldArrayOfficeSequenceNumbers=_ccdReceived.GetFieldsById("A02");
			CCDField[] ccdFieldArrayTransactionReferenceNumbers=_ccdReceived.GetFieldsById("G01");
			CCDField[] ccdFieldArrayTransactionPayments=_ccdReceived.GetFieldsById("G38");
		...
		private void PrintPaymentReconciliation_16(Graphics g){
			...
			PrintOfficeSequenceNumber(g,_x,0);
			_documentGenerator.StartElement();
			CCDField ccdField=_ccdReceived.GetFieldById("B04");
			if(ccdField!=null){
				_documentGenerator.DrawField(g,ccdField.GetFieldName(_isFrench),ccdField.valuestr,true,_x,0);
			}
			_documentGenerator.StartElement();
			...
			_documentGenerator.StartElement();
			CCDField[] ccdFieldArraycdaProviderNumbers=_ccdReceived.GetFieldsById("B01");
			CCDField[] ccdFieldArrayProviderOfficeNumbers=_ccdReceived.GetFieldsById("B02");
			CCDField[] ccdFieldArrayBillingProviderNumbers=_ccdReceived.GetFieldsById("B03");
			CCDField[] ccdFieldArrayCarrierIdentificationNumbers=_ccdReceived.GetFieldsById("A05");
			CCDField[] ccdFieldArrayOfficeSequenceNumbers=_ccdReceived.GetFieldsById("A02");
			CCDField[] ccdFieldArrayTransactionReferenceNumbers=_ccdReceived.GetFieldsById("G01");
			CCDField[] ccdFieldArrayTransactionPayments=_ccdReceived.GetFieldsById("G38");
		...
		private void DefaultPrint(Graphics g) {
			...
			_documentGenerator.DrawString(g,_text,_center-g.MeasureString(_text,_fontHeading).Width/2,0,_fontHeading);
			_x=_documentGenerator.StartElement(_verticalLine);
			CCDField[] ccdFieldArrayLoadedFields=_ccdReceived.GetLoadedFields();
		...
		private void PrintDentaide(Graphics g){
			...
			_text="";
			CCDField ccdFieldG01=_ccdReceived.GetFieldById("G01");
			if(ccdFieldG01!=null) {
			...
			if(_transactionCode=="13") {//predetermination ACK
				...
				List<Etrans> listEtrans=Etranss.GetAllForOneClaim(_claim.ClaimNum);//Claim not null here.
			...
			string relatCode=GetPatientRelationshipCode("C03",_claim?.PatRelat);
			...
			_x=_documentGenerator.StartElement();
			string initialPlacementUpperStr=GetDbDataStr("F15",_claim?.CanadianIsInitialUpper);
			string initialPlacementLowerStr=GetDbDataStr("F18",_claim?.CanadianIsInitialLower);			
			...
			CCDField ccdFieldOrthodonticRecordFlag=_ccdReceived.GetFieldById("F25");
			...
			CCDField[] ccdFieldArrayNoteNumbers=_ccdReceived.GetFieldsById("G45");//Used to looking up note reference numbers.
			if(_listClaimProcs!=null) {
				...
				//of each procedure.
				CCDField[] ccdFieldsArrayProcedureLineNumbers=_ccdReceived.GetFieldsById("F07");
				CCDField[] ccdFieldsArrayEligibleAmounts=_ccdReceived.GetFieldsById("G12");
				CCDField[] ccdFieldsArrayDeductibleAmounts=_ccdReceived.GetFieldsById("G13");
				CCDField[] ccdFieldsArrayEligiblePercentages=_ccdReceived.GetFieldsById("G14");
				CCDField[] ccdFieldsArrayDentaidePayAmounts=_ccdReceived.GetFieldsById("G15");
				CCDField[] ccdFieldsArrayExplainationNoteNumbers1=_ccdReceived.GetFieldsById("G16");
				CCDField[] ccdFieldsArrayExplainationNoteNumbers2=_ccdReceived.GetFieldsById("G17");
				CCDField[] ccdFieldsArrayEligibleLabAmounts1=_ccdReceived.GetFieldsById("G43");
				CCDField[] ccdFieldsArrayDeductibleLabAmounts1=_ccdReceived.GetFieldsById("G56");
				CCDField[] ccdFieldsArrayEligibleLabPercentage1=_ccdReceived.GetFieldsById("G57");
				CCDField[] ccdFieldsArrayBenefitLabAmount1=_ccdReceived.GetFieldsById("G58");
				CCDField[] ccdFieldsArrayEligibleLabAmounts2=_ccdReceived.GetFieldsById("G02");
				CCDField[] ccdFieldsArrayDeductibleLabAmounts2=_ccdReceived.GetFieldsById("G59");
				CCDField[] ccdFieldsArrayEligibleLabPercentage2=_ccdReceived.GetFieldsById("G60");
				CCDField[] ccdFieldsArrayBenefitLabAmount2=_ccdReceived.GetFieldsById("G61");
				float noteColumn=_x;
				...
				if(isEOB) {
					...
					CCDField[] ccdFieldsArrayProcs=_ccdReceived.GetFieldsById("G19");
					CCDField[] ccdFieldsArrayCarrierEligibleAmts=_ccdReceived.GetFieldsById("G20");
					CCDField[] ccdFieldsArrayCarrierEligibleLabAmts=_ccdReceived.GetFieldsById("G44");
					CCDField[] ccdFieldsArrayCarrierDeductAmts=_ccdReceived.GetFieldsById("G21");
					CCDField[] ccdFieldsArrayCarrierAts=_ccdReceived.GetFieldsById("G22");
					CCDField[] ccdFieldsArrayCarrierBenefitAmts=_ccdReceived.GetFieldsById("G23");
					CCDField[] ccdFieldsArrayCarrierNotes1=_ccdReceived.GetFieldsById("G24");
					CCDField[] ccdFieldsArrayCarrierNotes2=_ccdReceived.GetFieldsById("G25");
		...
		private void PrintProcedureListEOB(Graphics g) {
			...
			CCDField[] ccdFieldsArrayEligibleAmounts=_ccdReceived.GetFieldsById("G12");
			CCDField[] ccdFieldsArrayDeductibleAmounts=_ccdReceived.GetFieldsById("G13");
			CCDField[] ccdFieldsArrayEligiblePercentage=_ccdReceived.GetFieldsById("G14");
			CCDField[] ccdFieldsArrayBenefitAmountForTheProcedures=_ccdReceived.GetFieldsById("G15");
			CCDField[] ccdFieldsArrayExplainationNotes1=_ccdReceived.GetFieldsById("G16");
			CCDField[] ccdFieldsArrayExplainationNotes2=_ccdReceived.GetFieldsById("G17");
			CCDField[] ccdFieldsArrayEligibleLabAmounts1=_ccdReceived.GetFieldsById("G43");
			CCDField[] ccdFieldsArrayDeductibleLabAmounts1=_ccdReceived.GetFieldsById("G56");
			CCDField[] ccdFieldsArrayEligibleLabPercentage1=_ccdReceived.GetFieldsById("G57");
			CCDField[] ccdFieldsArrayBenefitLabAmount1=_ccdReceived.GetFieldsById("G58");
			...
			//List the carrier inserted procedures into the procedure list.
			CCDField[] ccdFieldsArrayCarrierProcs=_ccdReceived.GetFieldsById("G19");
			CCDField[] ccdFieldsArrayCarrierEligibleAmts=_ccdReceived.GetFieldsById("G20");
			CCDField[] ccdFieldsArrayCarrierEligibleLabAmts=_ccdReceived.GetFieldsById("G44");
			CCDField[] ccdFieldsArrayCarrierDeductAmts=_ccdReceived.GetFieldsById("G21");
			CCDField[] ccdFieldsArrayCarrierAts=_ccdReceived.GetFieldsById("G22");
			...
			string unallocatedDeductible=_ccdReceived.GetFieldById("G29").valuestr;
		...
		private SizeF PrintVertificationNo(Graphics g,float X,float Y){
			CCDField ccdFieldVertificationNo=_ccdReceived.GetFieldById("G30");//Present in EOBs.
		...
		private SizeF PrintTransactionReferenceNumber(Graphics g,float X,float Y){
			CCDField[] ccdFieldCarrierClaimNos=_ccdReceived.GetFieldsById("G01");
		...
		private SizeF PrintDisposition(Graphics g,float X,float Y) {
			CCDField ccdFieldDisposition=_ccdReceived.GetFieldById("G07");
		...
		private SizeF PrintComment(Graphics g,float X,float Y) {
			string comment="";
			CCDField ccdFieldComment=_ccdReceived.GetFieldById("G07");
		...
		private SizeF PrintTreatmentProviderID(Graphics g,float X,float Y) {
			CCDField ccdFieldTreatmentProviderID=_ccdReceived.GetFieldById("B01");
		...
		private SizeF PrintTreatmentProviderOfficeNumber(Graphics g,float X,float Y) {
			CCDField ccdFieldCdaOfficeNumber=_ccdReceived.GetFieldById("B02");
		...
		private SizeF PrintOfficeSequenceNumber(Graphics g,float X,float Y) {
			CCDField[] ccdFieldOfficeSequenceNumbers=_ccdReceived.GetFieldsById("A02");
		...
		private SizeF PrintCertificateNo(Graphics g,float X,float Y,bool isPrimary) {
			...
			CCDField ccdFieldC12=_ccdReceived.GetFieldById("C12");//Plan flag. 'N' for NIHB, 'A' for Newfoundland MCP Plan - Provincial Medical Plan, 'V' for Veteran's Affairs Plan. There may be other values.
			CCDField ccdFieldC13=_ccdReceived.GetFieldById("C13");//Band number for NIHB plans.
			CCDField ccdFieldC14=_ccdReceived.GetFieldById("C14");//Family number for NIHB plans.
		...
		private void PrintAccidentBullet(Graphics g,string questionStr){
			...
			DateTime dateAccident=GetDbDataDateTime("F02",_claim?.AccidentDate);
			bool isAccident=IsValidDate(dateAccident);
		...
		private void PrintInitialPlacementBullet(Graphics g){
			string initialPlacementUpper=GetDbDataStr("F15",_claim?.CanadianIsInitialUpper);
			string initialPlacementLower=GetDbDataStr("F18",_claim?.CanadianIsInitialLower);
			_x+=_documentGenerator.DrawString(g,_bullet.ToString()+". ",_x,0).Width;
			_bullet++;
			_documentGenerator.PushX(_x);//Begin indentation.
		...
		private void PrintPaymentSummary(Graphics g) {
			...
			CCDField ccdFieldG03=_ccdReceived.GetFieldById("G03");
			...
			CCDField ccdFieldF01=_ccdReceived.GetFieldById("F01");
			...
			if((initialPlacementUpper=="Y" || initialPlacementUpper=="O") &&
				...
				for(int i=0;i<_listProcedureExtracted.Count;i++) {//Count specified by field F22
					...
					if(IsValidDate(_listProcedureExtracted[i].ProcDate)) {//Tooth is considered unextracted if it doesn't have a date.
						float thWidth=_documentGenerator.DrawString(g,Tooth.Display(_listProcedureExtracted[i].ToothNum).PadLeft(2,' ')+" ",_x,0).Width;//Field F23
		...
		private void PrintNoteList(Graphics g) {
			CCDField[] ccdFieldsArrayNoteOutputFlags=_ccdReceived.GetFieldsById("G41");
			CCDField[] ccdFieldsArrayNoteNumbers=_ccdReceived.GetFieldsById("G45");
			CCDField[] ccdFieldsArrayNoteTexts=_ccdReceived.GetFieldsById("G26");
		...
		private int PrintErrorList(Graphics g) {
			CCDField[] ccdFieldArrayErrors=_ccdReceived.GetFieldsById("G08");
		...
		private bool ThisIsPrimary(){
			string strCarrierIdentificationNumber=_ccdReceived.GetFieldById("A05").valuestr;//Exists in all formats but 24-Email, and 16-Payment Reconciliation Response
		...
		private string GetAllProcedureTypeDescriptions(){
			string text="";
			CCDField ccdFieldProcedureTypeCodes=_ccdReceived.GetFieldById("F16");
		...
		private string GetDbDataStr(string fieldId,string strDefault) {
			string result=_ccdReceived?.GetFieldById(fieldId)?.valuestr;
			if(result==null) {
		...
		private DateTime GetDbDataDateTime(string fieldId,DateTime? dateDefault) {
			string dateAsStr=GetDbDataStr(fieldId,"");
			if(dateAsStr=="") {
		...
		private int GetDbDataInt(string fieldId,int? intDefault) {
			string intAsStr=GetDbDataStr(fieldId,"");
			if(intAsStr=="") {
		...
		private bool GetDbDataBool(string fieldId,bool? boolDefault) {
			string boolAsStr=GetDbDataStr(fieldId,"");
			if(boolAsStr=="") {
