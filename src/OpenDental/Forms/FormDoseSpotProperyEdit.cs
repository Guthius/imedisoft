...
namespace OpenDental {
	public partial class FormDoseSpotPropertyEdit:FormODBase {
		...
		public FormDoseSpotPropertyEdit(ClinicDto clinic,string strClinicIdPropertyVal,string strClinicKeyVal,List<ProgramProperty> listProgramProperties) {
			InitializeComponent();
		...
		private void FormDoseSpotPropertyEdit_Load(object sender,EventArgs e) {
			...
			if(_clinic.Id==0) {//Clinics disabled or is HQ.
			...
			ProgramProperty programPropertyClinicID=ListProgramProperties
					.FirstOrDefault(x => x.ClinicNum!=_clinic.Id && x.PropertyDesc==Erx.PropertyDescs.ClinicID && x.PropertyValue!="");
		...
		private void menuItemSetup_Click(object sender,EventArgs e) {
			...
			using FormClinicEdit formClinicEdit=new FormClinicEdit(_clinic);
			formClinicEdit.ShowDialog();
			if(formClinicEdit.DialogResult==DialogResult.OK) {
				Clinics.Update(formClinicEdit.ClinicCur);
		...
		private void butRegisterClinic_Click(object sender,EventArgs e) {
			...
			try {
				userID=DoseSpot.GetUserID(Security.CurUser,_clinic.Id);
				if(programPropertyDoseSpotApiVersion.PropertyValue=="1") {
					DoseSpot.RegisterClinic(_clinic.Id,programPropertyClinicID.PropertyValue,programPropertyClinicKey.PropertyValue,userID,out clinicID,out clinicKey);
				} 
				else if(programPropertyDoseSpotApiVersion.PropertyValue=="2"){
					DoseSpotV2.RegisterClinic(_clinic.Id,programPropertyClinicID.PropertyValue,programPropertyClinicKey.PropertyValue,userID,out clinicID,out clinicKey);
				}
			}
			catch(Exception ex) {
