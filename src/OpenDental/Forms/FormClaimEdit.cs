...
namespace OpenDental{
	...
	public partial class FormClaimEdit : FormODBase {
		...
		private void gridPay_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			...
			ClaimPayment claimPayment=ClaimPayments.GetOne(SIn.Long(_dataTablePayments.Rows[e.Row]["ClaimPaymentNum"].ToString()));
		...
		private bool PayAsTotalToClaimProcsExplicitly() { 
			...
			double result=SIn.Double(inputBox.StringResult);
...
//fix: need to debug the recalculation feature to take this status into account.
		...
		private bool TrySelectClaimPayment(List<ClaimPayment> listClaimPayments,out ClaimPayment claimPayment) {
			...
			listClaimPayments.ForEach(x => {
				GridRow row=new GridRow(x.CarrierName,x.CheckDate.ToShortDateString(),SOut.Double(x.CheckAmt));
				row.Tag=x;
		...
		private void butAttachAdd_Click(object sender,EventArgs e) {
			...
			if(textRadiographs.IsValid()) {
				int radiographs=SIn.Int(textRadiographs.Text);
				radiographs++;
		...
		private void menuItemOpen_Click(object sender,EventArgs e) {
			...
			FileAtoZ.OpenFile(FileAtoZ.CombinePaths(EmailAttaches.GetAttachPath(),_claim.Attachments[selectedIndex].ActualFileName),
		...
		private void listCanadianAttachments_DoubleClick(object sender,EventArgs e) {
			...
			FileAtoZ.OpenFile(FileAtoZ.CombinePaths(EmailAttaches.GetAttachPath(),_claim.Attachments[listCanadianAttachments.SelectedIndex].ActualFileName),
		...
		private void listCanadianAttachments_MouseDown(object sender,MouseEventArgs e) {
			//A right click also needs to select an items so that the context menu will work properly.
			if(e.Button==MouseButtons.Right) {
				int clickedIndex=listCanadianAttachments.IndexFromPoint(e.Location);
				if(clickedIndex!=-1) {
		...
		private bool CheckProviderTerm() {
			List<long> listLongInvalidProvNums=Providers.GetInvalidProvsByTermDate(new List<long> 
				{ comboProvBill.GetSelectedProvNum(),_provNumOrdering,comboProvTreat.GetSelectedProvNum() },SIn.DateTime(textDateService.Text));
		...
		private bool CanadianWarnings(){
			...
			if(warning!=""){
				DialogResult dialogResult=ODMessageBox.Show("Warnings:\r\n"+warning+"\r\nDo you wish to continue anyway?","",
		...
		private void SaveCleanup(){
			...
			if(comboClaimStatus.SelectedIndex==_listClaimStatuses.IndexOf(ClaimStatus.Received) && _isPaymentEntered) {
				...
				if(PrefC.GetBool(PrefName.ClaimPrimaryReceivedRecalcSecondary) && !receivedAsNoPay && _claim.ClaimType=="P") {
					List<Claim> listClaims=Claims.GetPrimaryOrSecondaryClaimsNotReceived(_listClaimProcsForClaim);
