...
namespace OpenDental{
	...
	public partial class FormRecallTypes:FormODBase {
		...
		private string GetStringForType(long recallTypeNum) {
			...
			List<RecallTrigger> listRecallTriggers=RecallTriggers.GetForType(recallTypeNum);
			string retVal="";
			for(int i=0;i<listRecallTriggers.Count;i++){
				if(i>0){
					retVal+=",";
				}
		...
		private void butAdd_Click(object sender, System.EventArgs e) {
			using FormRecallTypeEdit formRecallTypeEdit=new FormRecallTypeEdit();
