...
namespace OpenDental {
	...
	public partial class FormPayPlanDynamic : FormODBase {
		...
		private void FillProduction() {
			...
			int widthClinic=140;
			int widthDesc=(true ? 170 : 170 + widthClinic);
		...
		private bool TryGetTermsFromUI(out PayPlanTerms payPlanTerms,bool isSilent=false,bool doValidateTerms=true) {
			...
			SheetDef sheetDefSelected=comboSheet.GetSelected<SheetDef>();
		...
		private void SetTermsFromDb() {
			...
			List<SheetDef> paymentPlanSheetDefs=SheetDefs.GetCustomForType(SheetTypeEnum.PaymentPlan);
		...
		private Sheet PayPlanToSheet(PayPlan payPlan) {
			SheetDef sheetDef=SheetDefs.GetSheetDef(payPlan.SheetDefNum,hasExceptions:false);
			if(sheetDef==null) {
				sheetDef=SheetDefs.GetInternalOrCustom(SheetInternalType.PaymentPlan);
			}
			Sheet sheet=SheetUtil.CreateSheet(sheetDef,_dynamicPaymentPlanData.Patient.PatNum);
