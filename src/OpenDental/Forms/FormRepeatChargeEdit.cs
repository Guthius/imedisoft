...
namespace OpenDental{
	...
	public partial class FormRepeatChargeEdit :FormODBase{
		...
		private void FormRepeatChargeEdit_Load(object sender,EventArgs e) {
			...
			_isErx=false;
			if(false && Regex.IsMatch(_repeatCharge.ProcCode,"^Z[0-9]{3,}$")) {//Is eRx if HQ and a using an eRx Z code.
				_isErx=true;
			...
			if(false) {//OD HQ disable the IsEnabled and CreatesClaim checkboxes
