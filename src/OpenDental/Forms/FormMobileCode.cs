...
namespace OpenDental {
	public partial class FormMobileCode:FormODBase {
		...
		private void SetFormLocked(string unlockCode){
			_isFormLocked=true;
			BarcodeWriter barcodeWriterQr=new BarcodeWriter();
			...
			using Bitmap bitmapQr=new Bitmap(barcodeWriterQr.Write(unlockCode)); 
			...
			char[] charArrayRawCode=unlockCode.ToCharArray();
			int i=1;
			for(int idx=0;idx<charArrayRawCode.Length;idx++) {
				TextBox textBox;
				switch(i) {
					case 1:
						textBox=textBox1;
		...
		private void SetFormUnlocked(){
			...
			picBoxMain.Image=null;
			List<TextBox> listTextBoxes=Controls.OfType<TextBox>().ToList();
