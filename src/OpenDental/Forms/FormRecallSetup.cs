...
namespace OpenDental{
	...
	public partial class FormRecallSetup : FormODBase {
		...
		private void FillManualRecall() {
			CultureInfo cultureInfoDefault=PrefC.GetLanguageAndRegion();//Default combobox selection to practice's language.
			...
			List<string> listLanguagesUsedByPatients=PrefC.GetString(PrefName.LanguagesUsedByPatients).Split(new char[] {','},StringSplitOptions.RemoveEmptyEntries).ToList();
			//Add patient's language options to combobox if available.
			for(int i=0;i<listLanguagesUsedByPatients.Count;i++) {
				string curLanguage = listLanguagesUsedByPatients[i];
				...
				CultureInfo cultureInfo=MiscUtils.GetCultureFromThreeLetter(curLanguage);
				if(cultureInfo==null) {
			...
			List<Def> listDefsUnschedStatus=Defs.GetDefsForCategory(DefCat.RecallUnschedStatus,true);
			...
			List<long> listRecallTypes=new List<long>();
			...
			for(int i=0;i<listRecallTypesCache.Count;i++) {
		...
		private void SetLanguageSelected() {
			CultureInfo cultureSelected=comboLanguage.GetSelected<CultureInfo>();
		...
		private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			...
			string newPrefValue;
			bool isHtmlTemplate= prefName.In(PrefName.WebSchedMessage,PrefName.WebSchedMessage2,PrefName.WebSchedMessage3,
