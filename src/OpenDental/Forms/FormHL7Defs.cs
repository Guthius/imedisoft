	...
	public partial class FormHL7Defs:FormODBase {
		...
		private void butDuplicate_Click(object sender,System.EventArgs e) {
			...
			for(int m=0;m<hL7def.hl7DefMessages.Count;m++) {
				...
				for(int s=0;s<hL7def.hl7DefMessages[m].ListHL7DefSegments.Count;s++) {
					hL7def.hl7DefMessages[m].ListHL7DefSegments[s].HL7DefMessageNum=hL7DefMessageNum;
					long hL7DefSegmentNum=HL7DefSegments.Insert(hL7def.hl7DefMessages[m].ListHL7DefSegments[s]);
					for(int f=0;f<hL7def.hl7DefMessages[m].ListHL7DefSegments[s].hl7DefFields.Count;f++) {
