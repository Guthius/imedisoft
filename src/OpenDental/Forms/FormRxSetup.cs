...
namespace OpenDental{
	...
	public partial class FormRxSetup : FormODBase {
		...
		private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			using FormRxDefEdit formRxDefEdit=new FormRxDefEdit(gridMain.SelectedTag<RxDef>());
		...
		private void butAdd_Click(object sender, System.EventArgs e) {
			...
			RxDefs.Insert(rxDef);//It gets deleted if user clicks cancel
			using FormRxDefEdit formRxDefEdit=new FormRxDefEdit(rxDef);
			formRxDefEdit.IsNew=true;
			formRxDefEdit.ShowDialog();
			FillGrid();
		}
		...
		private void butAdd2_Click(object sender, System.EventArgs e) {
			...
			RxDef rxDef=gridMain.SelectedTag<RxDef>().Copy();
			RxDefs.Insert(rxDef);//Now it has a new id.  It gets deleted if user clicks cancel. Alerts not copied.
			using FormRxDefEdit formRxDefEdit=new FormRxDefEdit(rxDef);
			formRxDefEdit.IsNew=true;
			formRxDefEdit.ShowDialog();
			FillGrid();
		...
		private void butCombine_Click(object sender,EventArgs e) {
			...
			RxDef rxDef=formRxCombine.RxDefCur;
			if(!VerifyRxCombineData(rxDef,listRxDefs)) {
				return;
			}
			//Capture the names of the combined prescriptions before merge for logEntries
			List<string> listRxDefNames=new List<string>();
			string rxDefTo=rxDef.Drug;
			for(int i=0;i<listRxDefs.Count;i++) {
				if(listRxDefs[i].RxDefNum==rxDef.RxDefNum) {
					continue;
				}
				listRxDefNames.Add(listRxDefs[i].Drug);
			}
			try {
