...
namespace OpenDental {
	public partial class FormMountDefGenerate:FormODBase {
		...
		private void butOK_Click(object sender,EventArgs e) {
			...
			if(listType.SelectedIndex==0){//FMX horiz BW
				...
				if(MountDefCur.DefaultCat==0){
					long defNum=Defs.GetByExactName(DefCat.ImageCats,"FMXs");
					MountDefCur.DefaultCat=defNum;//could be 0
				}
			...
			if(listType.SelectedIndex==2){//4BW horiz
				...
				if(MountDefCur.DefaultCat==0){
					long defNum=Defs.GetByExactName(DefCat.ImageCats,"BWs");
					MountDefCur.DefaultCat=defNum;//could be 0
				}
		...
		private void AddText(int x,int y,int w,int h){
			...
			mountItemDef.Width=w;
			mountItemDef.Height=h;
			mountItemDef.TextShowing=@"Patient: [NameLF]
PatNum: [PatNum]
Date: [MountDate]
Provider: [MountProv]
";
			mountItemDef.FontSize=80;
			MountItemDefs.Insert(mountItemDef);
		}
