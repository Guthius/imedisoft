...
namespace OpenDental{
	...
	public partial class FormProvEdit : FormODBase {
		...
		private List<ClinicDto> _listClinicsForUser=new List<ClinicDto>();
		...
		private void FormProvEdit_Load(object sender, System.EventArgs e) {
			...
			if(true) {
				...
				bool doSelectAll=(_listProviderClinicLinks.Count==0 || _listClinicsForUser.All(x => _listProviderClinicLinks.Any(y => y.ClinicNum==x.Id)));
		...
		private void butSave_Click(object sender,System.EventArgs e) {
			...
			if(IsNew) {
				...
				if(ProviderCur.IsInstructor) {
					...
					userod.SetPassword(Authentication.GenerateLoginDetailsSHA512(textPassword.Text));
				...
				try {
					if(_userodExisting!=null && (ProviderCur.IsInstructor || ProviderCur.SchoolClassNum!=0)) {
						...
						_userodExisting.SetPassword(Authentication.GenerateLoginDetailsSHA512(textPassword.Text));
			...
			if(true) {
				List<long> listClinicNumsSelectedLinks=listBoxClinics.GetListSelected<ClinicDto>().Select(x => x.Id).ToList();
