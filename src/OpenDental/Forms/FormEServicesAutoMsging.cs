...
namespace OpenDental {
	...
	public partial class FormEServicesAutoMsging:FormODBase {
		...
		private void SaveToDb() {
			List<ApptReminderRule> listApptReminderRules=_listApptReminderRules.FindAll(x => x.ClinicNum==_clinic.Id);
		...
		private void SetListClinicsAndRulesHelper() {
			if(true) {//CLINICS
				...
				_listClinics=new List<ClinicDto>();
		...
		private void FillRemindConfirmData() {
			...
			for(int i=0;i<_apptReminderTypeArray.Length;i++) {
				bool useDefaultForApptType=GetUseDefaultForApptType(_apptReminderTypeArray[i],_clinic.Id);
			...
			for(int i=0;i<listApptReminderRules.Count();i++)  {
				...
				row=new GridRow();
				bool useDefaultForApptType=GetUseDefaultForApptType(listApptReminderRules[i].TypeCur,_clinic.Id);
		...
		private void GridRemindersMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			...
			bool useDefaultForApptType=GetUseDefaultForApptType(apptReminderRule.TypeCur,_clinic.Id);
		...
		private void AddRule(ApptReminderType apptReminderType,bool isBeforeAppointment=true) {
			bool useDefaultForApptType=GetUseDefaultForApptType(apptReminderType,_clinic.Id);
			if(_clinic.Id>0 && useDefaultForApptType) {
			...
			ApptReminderRule apptReminderRule=ApptReminderRules.CreateDefaultReminderRule(apptReminderType,_clinic.Id,isBeforeAppointment);
		...
		private bool SwitchFromDefaults(ApptReminderType apptReminderType) {
			...
			ClinicPref clinicPref=_listClinicPrefs.FirstOrDefault(x => x.ClinicNum==_clinic.Id && x.PrefName==prefName);
			if(clinicPref==null) {
