...
public partial class BrokenApptGraphOptionsCtrl : BaseGraphOptionsCtrl
{
    ...
    private void FillComboAdj()
    {
        ...
        comboAdjType.Items.Add(Lans.g("Adj types not setup"));
    ...
    private void FillComboBrokenProc()
    {
        ...
        switch (brokenApptCodeDb)
        {
            case BrokenApptProcedure.None:
            case BrokenApptProcedure.Missed:
                index = comboBrokenProcType.Items.Add(Lans.g(brokenApptCodeDb.ToString()) + ": (D9986)");
                break;

            case BrokenApptProcedure.Cancelled:
                index = comboBrokenProcType.Items.Add(Lans.g(brokenApptCodeDb.ToString()) + ": (D9987)");
                break;

            case BrokenApptProcedure.Both:
                comboBrokenProcType.Items.Add(Lans.g(BrokenApptProcedure.Missed.ToString()) + ": (D9986)");
                comboBrokenProcType.Items.Add(Lans.g(BrokenApptProcedure.Cancelled.ToString()) + ": (D9987)");
                index = comboBrokenProcType.Items.Add(Lans.g(brokenApptCodeDb.ToString()));
                break;
        }
