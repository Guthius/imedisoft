...
namespace OpenDental {
	class SheetPrintingJob {
		...
		public void DrawSheetFirstPage(Graphics g,Sheet sheet) {
			//pd_DrawFieldsHelper() is normally called once for each page of a sheet in pd_PrintPage().
			//Here, _yPosPrint and _pagesPrinted will always be zero, so only the first page is drawn.
			pd_DrawFieldsHelper(sheet,g,null);
			//Draw a footer to prevent fields near the top of the second page from showing on the first page.
