...
namespace OpenDental {
	...
	///<summary>Creates an internal deep copy of the SheetDef and it's SheetFieldDefs. The original object will not be modified.</summary>
		...
		public bool MergeMobileSheetFieldDefs(SheetDef sheetDef,bool requireValidation,Action<string> err=null) {
			try {
				...
				if(sheetDef.HasMobileLayout) {
					List<SheetFieldDef> listErrorMobileSheetField=new List<SheetFieldDef>();
					//Check all sheet fields.
					for(int i=0;i<listMobileSheetDefs.Count;i++) {
						SheetFieldDef mobileSheetDef=listMobileSheetDefs[i];
		...
		public void SetErrorFieldsColors(List<SheetFieldDef> listErrorMobileSheetFields) {
			...
			List<SheetEditMobilePanelandControl> listSheetEditMobilePanels=panelPreview.Controls.Cast<SheetEditMobilePanel>()
				.SelectMany(x => x.Controls.Cast<Control>(),(x,y) => new SheetEditMobilePanelandControl(x,y)).ToList();
			for(int i = 0;i<listErrorMobileSheetFields.Count;i++) {
				SheetEditMobilePanelandControl control=listSheetEditMobilePanels.FirstOrDefault(x =>  x.CtrlTag!=null && x.CtrlTag.SheetFieldDefNum==listErrorMobileSheetFields[i].SheetFieldDefNum);
		...
		bool MobileSheetFieldDefHasErrors(SheetFieldDef mobileSheetDef,List<SheetEditMobilePanelandControl> listCheckBoxes) {
			...
			//Validate checkboxes
				...
				SheetEditMobilePanelandControl checkBox=listCheckBoxes.FirstOrDefault(x => x.CtrlTag.SheetFieldDefNum==mobileSheetDef.SheetFieldDefNum);
	...
	public class SheetEditMobileRadioButton:System.Windows.Forms.CheckBox, IHasDragAttributes {
		...
		protected override void OnPaint(PaintEventArgs e) {
			...
			using(SolidBrush brush=new SolidBrush(this.ForeColor)) {
				int textWidth=(int)Math.Ceiling(g.MeasureString(this.Text,this.Font).Width);
