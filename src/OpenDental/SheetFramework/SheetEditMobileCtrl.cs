...
public partial class SheetEditMobileCtrl : UserControl
{
    ...
    public void SetSheetDef(SheetDef sheetDef)
    {
        ...
        var sheetDefNumOld = _sheetDef == null ? -1 : _sheetDef.SheetDefNum;
        if (sheetDef == null)
        {
        ...
        var updateExisting = sheetDefNumOld == sheetDef.SheetDefNum;
        if (updateExisting)
        {
            //This is just an update so save changes before setting new fields.
            MergeMobileSheetFieldDefs(sheetDef, false);
        }

        //Make a deep copy of the SheetDef.
        _sheetDef = sheetDef.Copy();
        _sheetDef.SheetFieldDefs = sheetDef.SheetFieldDefs == null ? [] : sheetDef.SheetFieldDefs.Select(x => x.Copy()).ToList();
        _sheetDef.Parameters = sheetDef.Parameters == null ? [] : sheetDef.Parameters.Select(x => x.Copy()).ToList();
        var newPanels = new List<SheetEditMobilePanel>();
        var isReadOnly = new Func<bool>(() => { return IsReadOnly; });
        ...
        var alreadyGrouped = radioFields1.SelectMany(x => x.Items)
        ...
        var removed = mobileFields.RemoveAll(x => removeSheetDefNums.Any(y => y == x.SheetFieldDefNum));
        ...
        foreach (var sheetFields in finalFields)
        {
            #region addSheetDef Action

            //The first SFD in the list will represent all items in the group.
            var sheetField = sheetFields[0];
            var panel = new SheetEditMobilePanel(isReadOnly)
            {
                Width = panelPreview.Width - 22,
                Header = string.IsNullOrEmpty(sheetField.UiLabelMobile) ? (sheetFields.Count > 1 ? "RadioGroup" : sheetField.FieldType.ToString()) + " - " + sheetField.FieldName : sheetField.UiLabelMobile,
            ...
            if (sheetField.SheetFieldDefNum == ScrollToSheetFieldDefNum)
            {
                //We will scroll to this panel after we are done adding all panels.
                panelScrollTo = panel;
            }

            var mouseEnter = new EventHandler((_, _) => SetControlAndChildrenBackColor(panel, Color.LightGray));
            var mouseLeave = new EventHandler((_, _) =>
            {
                if (panel.NeedsMove)
                {
            ...
            {
                var controlDraggingOver = (Control) sender;
                var controlDragging = (Control) e.Data.GetData(e.Data.GetFormats()[0]);
                if (controlDragging is SheetEditMobileRadioButton)
                {
                    //Dragging over a Radio Item.
                ...
                {
                    //Dragging over the main panel.
                    SetControlAndChildrenBackColor(panel, Color.LightYellow);
                }
            });
            var dragLeave = new EventHandler((_, _) =>
            {
                if (panel.NeedsMove)
                {
                    SetControlAndChildrenBackColor(panel, _colorFieldsNeedMove);
            ...
            var addCheckBox = new Func<SheetFieldDef, SheetEditMobileRadioButton>((sheetFieldRadioItem) =>
            {
                ...
                {
                    radioItem.TextClick += new EventHandler((_, _) =>
                    {
                        SheetFieldDefEdit.Invoke(this, new SheetFieldDefEditArgs([sheetFieldRadioItem.SheetFieldDefNum], true));
                        SetHighlightedFieldDefs([sheetFieldRadioItem.SheetFieldDefNum]);
                    });
                }

                radioItem.Selected += new EventHandler((_, _) => { SheetFieldDefSelected?.Invoke(this, sheetFieldRadioItem.SheetFieldDefNum); });
                radioItem.MouseEnter += new EventHandler((_, _) => { SetControlAndChildrenBackColor(radioItem, Color.LightGray); });
                radioItem.MouseLeave += new EventHandler((_, _) =>
                {
                    if (panel.NeedsMove)
                    {
                ...
                && (sheetFields.GroupBy(x => x.RadioButtonGroup).Count() != 1 || sheetFields.All(x => x.RadioButtonGroup.IsNullOrEmpty())))
            {
                ...
                var checkItems = new List<CheckBox>();
                var addCheckOption = new Action<SheetFieldDef>((sheetFieldCheckItem) =>
                {
                    var checkboxItem = addCheckBox(sheetFieldCheckItem);
                    checkboxItem.Text = sheetFieldCheckItem.UiLabelMobileRadioButton;
                    checkboxItem.MouseMove += new MouseEventHandler((_, e) => TryStartDragging(checkboxItem, e.Location));
            ...
            else if (sheetFields.Count > 1 || !string.IsNullOrEmpty(sheetFields[0].RadioButtonValue))
            {
                ...
                var radioItems = new List<SheetEditMobileRadioButton>();
                var addRadioOption = new Action<SheetFieldDef>((sheetFieldRadioItem) =>
                {
                    #region addRadioOption

                    var radioItem = addCheckBox(sheetFieldRadioItem);
                    ...
                    if (SheetFieldDefEdit == null)
                    {
                        radioItem.TextClick += new EventHandler((_, _) =>
                        {
                            var input = new InputBox(GetTranslation("Edit label for radio item"), sheetFieldRadioItem.UiLabelMobileRadioButton);
                            input.ShowDialog();
                    ...
                    radioItem.CheckedChanged += new EventHandler((_, _) =>
                    {
                    ...
                    radioItem.MouseMove += new MouseEventHandler((_, e) => TryStartDragging(radioItem, e.Location));
            ...
            else if (sheetField.FieldType == SheetFieldType.ComboBox)
            {
                ...
                var split = sheetField.FieldValue;
                var i = split.IndexOf(';');
                if (i == 0)
                {
            ...
            else if (sheetField.FieldType == SheetFieldType.CheckBox)
            {
                ...
                var checkBox = addCheckBox(sheetField);
            ...
            {
                throw new Exception("Unsupported mobile FieldType: " + sheetField.FieldType);
            }

            panel.DoubleClick += new EventHandler((_, _) =>
            {
                ...
                var input = new InputBox(GetTranslation("Edit label for") + " " +
                ...
                var newLabel = input.StringResult;
            ...
            panel.MouseDown += new MouseEventHandler((_, e) =>
            {
                #region MouseDown
            ...
            panel.MouseUp += new MouseEventHandler((_, _) =>
            {
                #region MouseUp

                if (panel.DragAtt.CanMouseUp)
                {
                    //We have not moved the mouse far enough to be considered a drag so it must be a click.							
            ...
            panel.MouseMove += new MouseEventHandler((_, e) =>
            {
                #region MouseMove
            ...
            panel.MouseLeave += new EventHandler((_, _) => { this.Cursor = Cursors.Default; });
            ...
            var panelHeight = 15 + panel.Padding.Top;
        ...
        var listPanelsNeedColor = panelPreview.Controls.Cast<SheetEditMobilePanel>().Where(x => x.NeedsMove == true).ToList();
        var listSfdNum = new List<long>();
        //Old panels with color
        foreach (var panel in listPanelsNeedColor)
        {
            var tag = (SheetFieldDef) panel.Tag;
            if (tag == null)
            {
                foreach (Control ctrl in panel.Controls)
                {
                    var currentSFD = (SheetFieldDef) ctrl.Tag;
                    if (currentSFD != null)
                    {
    ...
    public bool MergeMobileSheetFieldDefs(SheetDef sheetDef, bool requireValidation, Action<string> err = null)
    {
        try
        {
            ...
            if (!SheetDefs.IsMobileAllowed(sheetDef.SheetType))
            {
                //Return silently. No need to merge.
                return true;
            }
            ...
            var listMobileSheetDefs = SheetFieldDefs; //List of SheetFieldDefs pulled from panel UI
            var languageCheck = listMobileSheetDefs.FirstOrDefault()?.Language ?? ""; //Blank string is used for 'Default' translation.
            ...
            if (_tabOrderMobileToMimicTabOrder)
            {
                ...
                var listUnOrderable = new List<SheetFieldDef>();
                var listOrderableFields = new List<SheetFieldDef>();
            ...
            var tabOrderMobile = 1;
            var listMobileSheetDefsAlreadyOrdered = new List<SheetFieldDef>();
            foreach (var sheetField in listMobileSheetDefs.Where(x => string.IsNullOrWhiteSpace(x.Language)))
            {
                ...
                if (sheetField.FieldType == SheetFieldType.CheckBox)
                {
                    var listSheetFieldDefNew = OpenDentBusiness.SheetFieldDefs.GetRadioGroupForSheetFieldDef(sheetField, sheetDef.SheetFieldDefs.FindAll(x => string.IsNullOrWhiteSpace(x.Language)));
                    ...
                    listSheetFieldDefNew = listSheetFieldDefNew.OrderByDescending(x => listMobileSheetDefs.Any(y => x.SheetFieldDefNum == y.SheetFieldDefNum))
                        ...
                        .ToList();
                    foreach (var newSFDCheckboxInGroup in listSheetFieldDefNew)
                    {
            ...
            tabOrderMobile = 1;
            if (SyncSheetFieldsWithDefualt)
            {
                foreach (var sheetFieldNonDefLang in sheetDef.SheetFieldDefs.Where(x => !string.IsNullOrWhiteSpace(x.Language)))
                {
                    //Since we cannot rely on sheetfielddefnum to be set, we need to mix in all the other fields to find the appropriate SFD.
                    var sheetFieldDef = listMobileSheetDefs.FirstOrDefault(x => OpenDentBusiness.SheetFieldDefs.CompareSheetFieldDefsByValueForMobileLayout(x, sheetFieldNonDefLang, true) && string.IsNullOrEmpty(x.Language));
            ...
            else
            {
                ...
                var listSheetFieldDefsByLanguage = listMobileSheetDefs.Where(x => !string.IsNullOrWhiteSpace(x.Language)).GroupBy(x => x.Language).Select(x => x.ToList()).ToList();
                foreach (var sheetFieldDefLanguageGrouping in listSheetFieldDefsByLanguage)
                {
                    listMobileSheetDefsAlreadyOrdered = [];
                    foreach (var sheetFieldDef in sheetFieldDefLanguageGrouping)
                    {
                        ...
                        {
                            var listSheetFieldDefNew = OpenDentBusiness.SheetFieldDefs.GetRadioGroupForSheetFieldDef(sheetFieldDef, sheetDef.SheetFieldDefs.FindAll(x => sheetFieldDef.Language == x.Language));
            ...
            var listNonMobileSheetDefs = _sheetDef.SheetFieldDefs.Where(x => x.Language == languageCheck && !sheetDefNumsMobile.Contains(x.SheetFieldDefNum)).ToList();
            ...
            if (sheetDef.HasMobileLayout)
            {
                var listErrorMobileSheetField = new List<SheetFieldDef>();
                //Check all sheet fields.
                for (var i = 0; i < listMobileSheetDefs.Count; i++)
                {
                    var mobileSheetDef = listMobileSheetDefs[i];
                    if (MobileSheetFieldDefHasErrors(mobileSheetDef, checkBoxes))
                    {
                        ...
                        listErrorMobileSheetField.Add(mobileSheetDef);
                    }
                }

                if (listErrorMobileSheetField.Count > 0)
                {
                    SetErrorFieldsColors(listErrorMobileSheetField);
                    //Only show message box if requiring validation.
            ...
            var listAllSheetDefs = listMobileSheetDefs.Union(listNonMobileSheetDefs).ToList();
    ...
    {
        var listHighlightedSheetFields = new List<long>();
    ...
    public void SetErrorFieldsColors(List<SheetFieldDef> listErrorMobileSheetFields)
    {
        if (listErrorMobileSheetFields.IsNullOrEmpty())
        {
            return;
        }

        var listSheetEditMobilePanels = panelPreview.Controls.Cast<SheetEditMobilePanel>()
            .SelectMany(x => x.Controls.Cast<Control>(), (x, y) => new SheetEditMobilePanelandControl(x, y)).ToList();
        for (var i = 0; i < listErrorMobileSheetFields.Count; i++)
        {
            var control = listSheetEditMobilePanels.FirstOrDefault(x => x.CtrlTag != null && x.CtrlTag.SheetFieldDefNum == listErrorMobileSheetFields[i].SheetFieldDefNum);
            if (control != null && control.Ctrl != null && control.Pnl != null)
            {
                MarkErroneous(control.Pnl);
            }
        }
    }
    ...
    {
        var finalTitle = GetTranslation("Mobile Preview") + " - " + title;
        if (_formFloat != null)
        {
        ...
        _formFloat.FormClosing += new FormClosingEventHandler((_, _) =>
        {
    ...
    {
        if (_formFloat is null)
        {
            return false;
        }

        if (_formFloat.IsDisposed)
        {
            return false;
        }

        if (_formFloat.Visible)
        {
            return true;
        }
    ...
    private void TryStartDragging(IHasDragAttributes dragAttr, Point mouseLoc)
    {
        ...
        if (!dragAttr.DragAtt.CanStartDragging)
        {
            //The mouse isn't down so we can't be dragging.
            return;
        }

        if (!dragAttr.DragAtt.HasStartedDragging(mouseLoc))
        {
            //We have not moved the mouse far enough to be considered a drag so don't start yet.
            return;
        }
        ...
        var ctrlDrag = dragAttr.DragAtt.Ctrl;
    ...
    private void scrollTimer_Tick(object sender, EventArgs e)
    {
        ...
        {
            var p = panelPreview.AutoScrollPosition;
    ...
    {
        var p = panelPreview.PointToClient(new Point(e.X, e.Y));
        if (p.Y < 50)
        {
        ...
        else if (p.Y > panelPreview.ClientSize.Height - 50)
        {
    ...
    private void panel_DragEnter(object sender, DragEventArgs e)
    {
        ...
        var control = (Control) e.Data.GetData(e.Data.GetFormats()[0]);
        if (control == null)
        {
            return;
        }
    ...
    private void panel_DragDrop(object sender, DragEventArgs e)
    {
        ...
        var controlDroppedOnto = (Control) sender;
        var controlDragging = (Control) e.Data.GetData(e.Data.GetFormats()[0]);
        ...
        if (controlDragging is SheetEditMobileRadioButton)
        {
            ...
            var panelDropColor = panelDrop.NeedsMove ? _colorFieldsNeedMove : _colorPanelBG;
            SetControlAndChildrenBackColor(panelDrop, panelDropColor);
            if (isPanelDragging)
            {
                var panelDraggingColor = panelDragging.NeedsMove ? _colorFieldsNeedMove : _colorPanelBG;
                SetControlAndChildrenBackColor(panelDragging, panelDraggingColor);
            }
        }
        else
        {
            if (controlDroppedOnto != panelPreview)
            {
                //Dropped onto a sub-control so find its position in the panel.
                if (controlDroppedOnto is SheetEditMobilePanel)
                {
                    //Dropped onto whitespace on a panel.
                    panelDrop = (SheetEditMobilePanel) controlDroppedOnto;
                }
                else
                {
                    //Dropped onto a control on a panel (button, label, etc)
                    panelDrop = (SheetEditMobilePanel) controlDroppedOnto.Parent;
                }
            ...
            {
                //Position at the exact index it was dropped.
    ...
    private void butOrderFields_Click(object sender, EventArgs e)
    {
        ...
        var numUnorderableMobileFields = SheetFieldDefs
    ...
    bool MobileSheetFieldDefHasErrors(SheetFieldDef mobileSheetDef, List<SheetEditMobilePanelandControl> listCheckBoxes)
    {
        //Validate StaticText
        if (mobileSheetDef.FieldType == SheetFieldType.StaticText)
        {
            //StaticText fields must have a valid FieldValue. UiLabelMobile is limited to 255 characters so we will use FieldValue where we might need more text.
            return IsErroneousStaticText(mobileSheetDef);
        }
        //Validate checkboxes
        else if (mobileSheetDef.FieldType == SheetFieldType.CheckBox)
        {
            var checkBox = listCheckBoxes.FirstOrDefault(x => x.CtrlTag.SheetFieldDefNum == mobileSheetDef.SheetFieldDefNum);
            return IsErroneousCheckbox(mobileSheetDef, checkBox?.Ctrl);
        }

        //All other field types must have a valid UiLabelMobile.
        return string.IsNullOrEmpty(mobileSheetDef.UiLabelMobile);
    }

    ///<summary>Returns true if the Text field value is empty.</summary>
    private bool IsErroneousStaticText(SheetFieldDef sheetFieldCur)
    {
        return string.IsNullOrEmpty(sheetFieldCur.FieldValue);
    }
    ...
    private bool IsErroneousCheckbox(SheetFieldDef sheetFieldCur, Control checkBox = null)
    {
        //All checkboxes that aren't checkMeds should have valid Text (UiLabelMobileRadioButton).
...
#region Class -SheetEditMobileRadioButton
...
public class SheetEditMobileRadioButton : CheckBox, IHasDragAttributes
{
    ...
    public SheetEditMobileRadioButton(Func<bool> isReadOnly)
    {
        ...
        this.MouseMove += new MouseEventHandler((_, e) =>
        {
        ...
        this.MouseLeave += new EventHandler((_, _) =>
        {
            if (_isReadOnly != null && _isReadOnly())
            {
                return;
        ...
        this.MouseDown += new MouseEventHandler((_, e) =>
        {
            if (e.Button != MouseButtons.Left)
            {
                return;
        ...
        this.MouseUp += new MouseEventHandler((_, e) =>
        {
            if (DragAtt.CanMouseUp)
            {
                //We have not moved the mouse far enough to be considered a drag so it must be a click.
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        var g = e.Graphics;
        g.Clear(BackColor);
        var rBounds = new Rectangle(0, 0, this.Width - 2, this.Height - 2);
        var rDotExterior = new Rectangle(rBounds.Right - rBounds.Height, rBounds.Top, rBounds.Height, rBounds.Height);
        //Text.
        using (var brush = new SolidBrush(this.ForeColor))
        {
            var textWidth = (int) Math.Ceiling(g.MeasureString(this.Text, this.Font).Width);
            //Add 4px padding and assure min width.
            var width = Math.Max(textWidth + 4, 20);
            //Add 3px margin.
            _rText = new Rectangle(rDotExterior.Left - width - 3, rBounds.Top, width, rBounds.Height);
            ControlPaint.DrawButton(g, _rText, _isOverText ? ButtonState.Checked : ButtonState.Normal);
            if (string.IsNullOrEmpty(Text))
            {
                //Missing required field. Fill with red.
                g.FillRectangle(Brushes.Red, new Rectangle(_rText.Left + 2, _rText.Top + 2, _rText.Width - 4, _rText.Height - 4));
            }
            else
            {
                g.DrawString(this.Text, this.Font, brush, _rText, new StringFormat {Alignment = StringAlignment.Center, LineAlignment = StringAlignment.Center});
            }
        }
        ...
        if (_isHighlighted)
        {
            ...
            {
                p.DashPattern = [2, 2];
                g.DrawRectangle(p, rBounds);
            }
        }

        //Dot exterior.
        g.DrawEllipse(Pens.Black, rDotExterior);
        //Set draggable region for cursor.
        _rDraggable = new Rectangle(0, 0, _rText.Left, this.Height);
    }
}

#endregion Class -SheetEditMobileRadioButton

#region Class - SheetEditMobilePanel
...
public class SheetEditMobilePanel : FlowLayoutPanel, IHasDragAttributes
{
    ...
    {
        get { return this.Width - (PEN_WIDTH * 4); }
    }
    ...
    public SheetEditMobilePanel(Func<bool> isReadOnly)
    {
        ...
        this.MouseMove += new MouseEventHandler((_, _) =>
        {
            if (_isReadOnly != null && _isReadOnly())
            {
                return;
        ...
        this.Resize += new EventHandler((_, _) => { Controls[0].Width = ChildControlWidth; });
    }
    ...
    protected override void OnPaintBackground(PaintEventArgs e)
    {
        ...
        using (Brush b = new SolidBrush(this.ForeColor))
        {
            ...
            {
                footerHeight = (int) Math.Ceiling(e.Graphics.MeasureString(FieldType, font).Height) + 3;
                e.Graphics.DrawString(FieldType, font, b, 2, this.Height - footerHeight);
                //Draw 'Double-click to edit' label.
                if (_isOverBounds)
                {
                    var width = (int) Math.Ceiling(e.Graphics.MeasureString(ButtonLabel, this.Font).Width) + 6;
...
#endregion Class - SheetEditMobilePanel

#region Classes

public class SheetEditMobilePanelandControl
{
    public SheetEditMobilePanel Pnl;
    public SheetFieldDef PnlTag;
    public Control Ctrl;
    public SheetFieldDef CtrlTag;

    public SheetEditMobilePanelandControl(SheetEditMobilePanel panel, Control control)
    {
        Pnl = panel;
        Ctrl = control;
        CtrlTag = (SheetFieldDef) ((Ctrl.Tag is SheetFieldDef) ? Ctrl.Tag : null);
        PnlTag = (SheetFieldDef) ((panel.Tag is SheetFieldDef) ? panel.Tag : null);
    }
}
...
{
    public Control Ctrl { private set; get; }
