	...
	public partial class ControlManage : UserControl{
		...
		private void butManageAR_Click(object sender,EventArgs e) {
			...
			if(_formArManager==null || _formArManager.IsDisposed) {
				while(!ValidateConnectionDetails()) {//only validate connection details if the ArManager form does not exist yet
					string msgText="An SFTP connection could not be made using the connection details "+(true ? "for any clinic " : "")
		...
		private void gridEmp_CellClick(object sender,ODGridClickEventArgs e) {
			if(gridEmp.SelectedIndices.Length!=1) {
				EnableTimeControlsForEmpI(-1);//More than one employee selected or no employee selected, disable time clock controls
				return;
			}
			bool isPrefTimeCardSecurityEnabled=PrefC.GetBool(PrefName.TimecardSecurityEnabled);
			if(!isPrefTimeCardSecurityEnabled) {
				EnableTimeControlsForEmpI(e.Row);//No security, enable time clock controls for anyone
				return;
			}
			if(Security.CurUser.EmployeeNum==((Employee)gridEmp.ListGridRows[e.Row].Tag).EmployeeNum) {
				EnableTimeControlsForEmpI(e.Row);//User is the selected employee, enable time clock controls
				return;
			}
			if(Security.IsAuthorized(EnumPermType.TimecardsEditAll,true)){
				EnableTimeControlsForEmpI(e.Row);//User has permission to edit all time cards, enable time clock controls
				return;
			}
			//Disable controls for all other cases
		...
		///<summary>Enables time clock controls for the employee at gridEmp[index].  -1 is also valid and disables all time clock controls.</summary>
		...
		private bool ValidateConnectionDetails() {
			...
			if(true) {
				listClinicNums=Clinics.GetAllForUserod(Security.CurUser).Select(x => x.Id).ToList();
