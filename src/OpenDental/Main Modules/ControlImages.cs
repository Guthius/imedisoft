...
public partial class ControlImages : UserControl{
	...
	private FrmBetterDiagLayers _frmBetterDiagLayers;
	private FormLauncher _formLauncherVideo;
	private FrmPearlLayers _frmPearlLayers;
	...
	private List<FormImageFloat> _listFormImageFloats= [];
	...
	///<summary>List of Pearl types that are toggled to display. Set by FrmPearlLayers. Maintains same state for the entire session, even as user changes to different images.</summary>
	private List<OpenDentBusiness.Pearl.EnumCategoryOD> _listPearlEnumCategoryODsShown=null;
	///<summary>List of BetterDiagnostics types that are toggled to display. Set by FrmBetterDiagLayers. Maintains same state for the entire session, even as user changes to different images.</summary>
	private List<EnumCategoryBetterDiag> _listEnumCategoryBetterDiagsShown=null;
	...
	private bool _showDrawingsAI=true;
	...
	public ControlImages(){
		...
		OpenDentBusiness.Bridges.Pearl.EventRefreshDisplay+=pearl_EventRefreshDisplay;
		OpenDentBusiness.Bridges.BetterDiag.EventRefreshDisplay+=betterDiag_EventRefreshDisplay;
	}
	...
	public void FillImageSelector(bool keepSelection) {
		...
		var scrollVal=imageSelector.ScrollValue;
		var listDefsImageCats=Defs.GetDefsForCategory(DefCat.ImageCats,true);
		imageSelector.SetCategories(listDefsImageCats);
		var dataSet=Documents.RefreshForPatient(_patient.PatNum,PrefC.GetBool(PrefName.ImagingOrderDescending));
	...
	public void ControlImages_KeyDown(Keys keys){
		...
		var controlImageDisplay=GetControlImageDisplaySelected();
	...
	public void LayoutToolBars() {
		toolBarMain.Clear();
		var contextMenuPrint=new WpfControls.UI.ContextMenu();
		contextMenuPrint.Add(new WpfControls.UI.MenuItem(Lan.g(this,"Print All in Category"),ToolBarPrintCategory));
		toolBarMain.Add(Lan.g(this,"Print"),ToolBarPrint_Click,WpfControls.UI.EnumIcons.Print,tag:TB.Print.ToString(),toolBarButtonStyle:WpfControls.UI.ToolBarButtonStyle.DropDownButton,contextMenuDropDown:contextMenuPrint);
		...
		var programNameAI=Programs.GetActiveImagingAIProgram();
		if(programNameAI==ProgramName.Pearl){
			groupAILayers.Text="Pearl AI";
			checkToothParts.Text="Tooth Parts";
			if(_listPearlEnumCategoryODsShown is null){//happens once on startup
				_listPearlEnumCategoryODsShown= [];
				var programNum=Programs.GetProgramNum(ProgramName.Pearl);
				var programProperty=ProgramProperties.GetFirstOrDefault(x => x.ProgramNum==programNum && x.PropertyDesc=="Show Pearl annotations by default");
				if(programProperty!=null && programProperty.PropertyValue.ToLower()=="true") {
					//Add all categories to list except tooth parts.
					_listPearlEnumCategoryODsShown=OpenDentBusiness.Bridges.Pearl.GetAllCategoryODsExceptToothParts();
				}
			}
		}
		else if(programNameAI==ProgramName.BetterDiagnostics) {
			groupAILayers.Text="BetterDiagnostics AI";
			checkToothParts.Text="Teeth";
			if(_listEnumCategoryBetterDiagsShown is null) {
				_listEnumCategoryBetterDiagsShown=Enum.GetValues(typeof(EnumCategoryBetterDiag)).Cast<EnumCategoryBetterDiag>().ToList();
				_listEnumCategoryBetterDiagsShown=_listEnumCategoryBetterDiagsShown.Except(OpenDentBusiness.Bridges.BetterDiag.GetBetterDiagToothPartsCategories()).ToList();//Hide tooth parts on startup
			}
		}
		var controlImageDisplay=GetControlImageDisplaySelected();
		if(!(controlImageDisplay is null)){
			controlImageDisplay.SetShowDrawings(_showDrawingsOD,_showDrawingsAI,_listPearlEnumCategoryODsShown,_listEnumCategoryBetterDiagsShown);
		}
		//ToolbarPaint-------------------------------------------------------------------------------------
		#region toolbarPaint
	...
	public void LayoutToolBarDraw() {
		...
		controlImageDisplay.SetShowDrawings(_showDrawingsOD,_showDrawingsAI,_listPearlEnumCategoryODsShown,_listEnumCategoryBetterDiagsShown);
		var bitmapColor=new Bitmap(22,22);//no using. Pass bitmap to toolbar, which will handle disposing the old one.
		...
		using var solidBrush=new SolidBrush(_colorFore);
		var rectangleF=new RectangleF(0,0,22,22);
	...
	public void CloseFloaters(){
		for(var i=_listFormImageFloats.Count-1;i>=0;i--){//go backwards
	...
	private void LaunchFloater(ControlImageDisplay controlImageDisplay,int x,int y,int w,int h,bool isMin=false,bool isMax=false){
		...
		var formImageFloat=new FormImageFloat();
		...
		var pointMousePosDesktop=Cursor.Position;//variable is for debugging
	...
	public void LaunchFloaterFromChart(long patNum,long docNum,long mountNum){
		var controlImageDisplay=CreateControlImageDisplay();
		_familyCur=Patients.GetFamily(patNum);
		_patient=_familyCur.GetPatient(patNum);
		_patFolder=ImageStore.GetPatientFolder(_patient,ImageStore.GetDataFolder());
		...
		formImageFloat.EventWinPicked+=DockOrFloat_EventWinClicked;
		_listFormImageFloats.Add(formImageFloat);
		var screenArray=System.Windows.Forms.Screen.AllScreens;
		var size=new Size();
		...
		formImageFloat.ControlImageDisplay.SetZoomSliderToFit();
	}
	...
	public void SelectTreeNode1(NodeTypeAndKey nodeTypeAndKey,string localPathImportedCloud="",bool isChartModuleFloater=false) {
		...
		if(controlImageDisplay==null){
			formImageFloat=_listFormImageFloats.FirstOrDefault(x=>x.ControlImageDisplay.GetNodeTypeAndKey().IsMatching(nodeTypeAndKey));
			if(formImageFloat!=null){
				//The one we want is in a floater
				controlImageDisplay=formImageFloat.ControlImageDisplay;
		...
		{
			controlImageDock.SetControlImageDisplay(null);//This disables all toolbutton items since ControlImageDisplay is null.
			DisableAllToolBarButtons(enablePrint:true);
		}
		else{
			...
			for(var i=0;i<_listFormImageFloats.Count;i++){
	...
	public void SetDocumentOrMount(NodeTypeAndKey nodeTypeAndKey,ControlImageDisplay controlImageDisplay,FormImageFloat formImageFloat=null){
		if(nodeTypeAndKey!=null && nodeTypeAndKey.NodeType==EnumImageNodeType.Document){
			var document=Documents.GetByNum(nodeTypeAndKey.PriKey,doReturnNullIfNotFound:true);
			if(document==null) {
		...
		if(nodeTypeAndKey!=null && nodeTypeAndKey.NodeType==EnumImageNodeType.Mount){
			var mount=Mounts.GetByNum(nodeTypeAndKey.PriKey);
	...
	public void SetTitle(ControlImageDisplay controlImageDisplay,FormImageFloat formImageFloat=null){
		var nodeTypeAndKey=controlImageDisplay.GetNodeTypeAndKey();
		var document=controlImageDisplay.GetDocumentShowing(0);
		var mount=controlImageDisplay.GetMountShowing();
		var text="";
	...
	private void butAILayers_Click(object sender, EventArgs e) {
		var programName=Programs.GetActiveImagingAIProgram();
		if(programName==ProgramName.Pearl) {
			if(_frmPearlLayers is null) {
				_frmPearlLayers=new FrmPearlLayers();
			}
			_frmPearlLayers.ListEnumCategoryODsShown=_listPearlEnumCategoryODsShown;
			_frmPearlLayers.ShowPearlLayers=_showDrawingsAI;
			//Tell frmPearlLayers how to refresh the display
			_frmPearlLayers.EventRefreshControlImages+=(s,e) => {
				_listPearlEnumCategoryODsShown=_frmPearlLayers.ListEnumCategoryODsShown;
				_showDrawingsAI=_frmPearlLayers.ShowPearlLayers;
				pearl_EventRefreshDisplay(s,e);
			};
			_frmPearlLayers.EventClickDeleteAnnotations+=pearl_EventClickDeleteAnnotations;
			_frmPearlLayers.FormClosing+=(_,_) => {
			...
			if(_frmBetterDiagLayers is null) {
				_frmBetterDiagLayers=new FrmBetterDiagLayers();
			}
			_frmBetterDiagLayers.ListEnumCategoryBetterDiagsShown=_listEnumCategoryBetterDiagsShown;
			_frmBetterDiagLayers.ShowBetterDiagLayers=_showDrawingsAI;
			//Tell frmBetterDiagLayers how to refresh the display
			_frmBetterDiagLayers.EventRefreshControlImages=(s,e) => {
				_listEnumCategoryBetterDiagsShown=_frmBetterDiagLayers.ListEnumCategoryBetterDiagsShown;
				_showDrawingsAI=_frmBetterDiagLayers.ShowBetterDiagLayers;
				betterDiag_EventRefreshDisplay(s,e);
			};
			_frmBetterDiagLayers.FormClosing+=(_,_) => {
				_frmBetterDiagLayers=null;
			};
			_frmBetterDiagLayers.Location=this.PointToScreen(Point.Empty);
			_frmBetterDiagLayers.Location.Y+=50;//Place below toolbars
			_frmBetterDiagLayers.Show();
			_frmBetterDiagLayers.Activate();
		}
	}
	...
	private void checkShow_Click(object sender, EventArgs e) {
		_showDrawingsAI=checkShow.Checked;
		var programName=Programs.GetActiveImagingAIProgram();
		if(programName==ProgramName.Pearl) {
			pearl_EventRefreshDisplay(sender,e);
		}
		else if(programName==ProgramName.BetterDiagnostics) {
			betterDiag_EventRefreshDisplay(sender,e);
		}
	}

	private void checkToothParts_Click(object sender, EventArgs e) {
		var programName=Programs.GetActiveImagingAIProgram();
		if(programName==ProgramName.Pearl) { 
			var listEnumCategoryODs=OpenDentBusiness.Bridges.Pearl.GetToothPartsCategoryODs();
			if(checkToothParts.Checked) {
				_listPearlEnumCategoryODsShown=_listPearlEnumCategoryODsShown.Union(listEnumCategoryODs).ToList();
			}
			else {
				_listPearlEnumCategoryODsShown=_listPearlEnumCategoryODsShown.Except(listEnumCategoryODs).ToList();
			}
			pearl_EventRefreshDisplay(sender,e);
		}
		else if(programName==ProgramName.BetterDiagnostics) {
			var listEnumCategoryBetterDiags=OpenDentBusiness.Bridges.BetterDiag.GetBetterDiagToothPartsCategories();
			if(checkToothParts.Checked) {
				_listEnumCategoryBetterDiagsShown=_listEnumCategoryBetterDiagsShown.Union(listEnumCategoryBetterDiags).ToList();
			}
			else {
				_listEnumCategoryBetterDiagsShown=_listEnumCategoryBetterDiagsShown.Except(listEnumCategoryBetterDiags).ToList();
			}
			betterDiag_EventRefreshDisplay(sender,e);
		}
	}
	...
	private void DockOrFloat_EventButClicked(object sender,WpfControls.UI.EnumImageFloatWinButton enumImageFloatWinButton) {
		...
		var drawing_PointScreen=new Point();
		...
		var screenThis=System.Windows.Forms.Screen.FromPoint(drawing_PointScreen);
		   ...
		   || enumImageFloatWinButton==WpfControls.UI.EnumImageFloatWinButton.Maximize)
		{
			...
			//But to do that, I would need to load the bitmap, rotate as needed, and then do the calc.  Maybe later.
			//So make it square, about 50% of height. Push it to the right.  They can resize.
			var width=screenThis.WorkingArea.Height/2;
	...
	private void Docker_EventPopFloater(object sender,EventArgs e) {
		...
		var pointUL=PointToScreen(controlImageDock.Location);
		var pointBR=PointToScreen(new Point(controlImageDock.Right,controlImageDock.Bottom));
		var rectangle=new Rectangle(
			pointUL.X,
			pointUL.Y,
			pointBR.X-pointUL.X,
			pointBR.Y-pointUL.Y);
		var controlImageDisplay=controlImageDock.ControlImageDisplay_;
		LaunchFloater(controlImageDisplay,rectangle.X,rectangle.Y,rectangle.Width,rectangle.Height);
	}
	...
	private void imageSelector_DragDropImport(object sender,DragDropImportEventArgs e) {
		...
		for(var i=0;i<e.ListFileNames.Count;i++){
			...
			if(OpenDentBusiness.Bridges.Pearl.DoAutoUploadForImageCategory(document.DocCategory)) {
				var pearl=OpenDentBusiness.Bridges.Pearl.SetupPearlForSendingSingle(_patient,document,filePath:e.ListFileNames[i]);
				pearl?.SendOnThread();
			}
			else if(OpenDentBusiness.Bridges.BetterDiag.DoAutoUploadForImageCategory(document.DocCategory)) {
				OpenDentBusiness.Bridges.BetterDiag.SendSingleOnThread(_patient,document,filePath:e.ListFileNames[i]);
			}
		}
		FillImageSelector(false);
		var nodeTypeAndKey=new NodeTypeAndKey(EnumImageNodeType.Document,document.DocNum);
		SelectTreeNode1(nodeTypeAndKey);
	}
	...
	private void imageSelector_DraggedToCategory(object sender,DragEventArgsOD e) {
		if(e.DocNum!=0){
			...
			var docOldCat=Defs.GetDef(DefCat.ImageCats,document.DocCategory).ItemName;
			var docNewCat=Defs.GetDef(DefCat.ImageCats,e.DefNumNew).ItemName;
			var logText=Lan.g(this,"Document moved")+": "+document.FileName;
			if(document.Description!="") {
				var docDescript=document.Description;
			...
			var docOld=document.Copy();
			document.DocCategory=e.DefNumNew;
			Documents.Update(document,docOld);
		}
		else if(e.MountNum!=0){
			var mountShowing=GetMountShowing();
			var mountOriginalCat=Defs.GetDef(DefCat.ImageCats,mountShowing.DocCategory).ItemName;
			var mountNewCat=Defs.GetDef(DefCat.ImageCats,e.DefNumNew).ItemName;
	...
	private void imageSelector_ItemDoubleClick(object sender,EventArgs e) {
		...
		var ext=ImageStore.GetExtension(document);
		var listImageExtensions=new List<string> {".bmp",".dcm",".dicom",".gif",".jpg",".jpeg",".png",".tif",".tiff"};
		if(listImageExtensions.Contains(ext)) {
			var frmDocInfo=new FrmDocInfo(_patient,GetDocumentShowing(0));
		...
		if(true)
		{
			var tempFile=ImageStore.GetFilePath(document,_patFolder);
			try {
				var filePath=ImageStore.GetFilePath(document,_patFolder);
	...
	private void FormImageFloat_Activated(object sender, EventArgs e){
		...
		var formImageFloat=(FormImageFloat)sender;
		...
		formImageFloat.IsImageFloatSelected=true;
		var controlImageDisplay=formImageFloat.ControlImageDisplay;
		controlImageDisplay.EnableToolBarButtons();
		var imageNodeType=controlImageDisplay.GetSelectedType();
		if(/* ODBuild.IsDebug() */ false && controlImageDisplay.Parent!=null) {
			var parent=controlImageDisplay.Parent.GetType().ToString();
			//Debug.WriteLine("ControlImages.FormImageFloat_Activated, Parent:"+parent);
		}
		var priKey=controlImageDisplay.GetSelectedPriKey();
		imageSelector.SetSelected(imageNodeType,priKey);
		var zoomSliderState=controlImageDisplay.ZoomSliderStateInitial;
	...
	private void ControlImageDock_ImageClosed(object sender, EventArgs e){
		...
		var isCategorySelected=imageSelector.GetSelectedType()==EnumImageNodeType.Category;
	...
	private void imageSelector_ItemReselected(object sender,EventArgs e) {
		...
		controlImageDisplay.ClearObjects();
		var nodeTypeAndKey=controlImageDisplay.GetNodeTypeAndKey();
		var formImageFloat=_listFormImageFloats
			.FirstOrDefault(x=>x.ControlImageDisplay==controlImageDisplay);//will be null if docked
		SetDocumentOrMount(nodeTypeAndKey,controlImageDisplay,formImageFloat);
		controlImageDisplay.SelectTreeNode2(nodeTypeAndKey);
		SetTitle(controlImageDisplay,formImageFloat);
		if(imageSelector.GetSelectedType()==EnumImageNodeType.Mount) {
	...
	private void imageSelector_SelectionChangeCommitted(object sender,EventArgs e) {
		...
		var nodeTypeAndKey=new NodeTypeAndKey(nodeType,priKey);
		SelectTreeNode1(nodeTypeAndKey);
		Application.DoEvents();
		var formImageFloat=_listFormImageFloats.FirstOrDefault(x=>x.ControlImageDisplay.GetNodeTypeAndKey().IsMatching(nodeTypeAndKey));
		if(formImageFloat!=null){
			//The one we want is in a floater
	...
	private void _fileSystemWatcher_FileCreated(object sender,FileSystemEventArgs e) {
		...
		if(!IsMountShowing()){//single
			try {
				if(OpenDentBusiness.Bridges.Pearl.DoAutoUploadForImageCategory(document.DocCategory)) {
					var pearl=OpenDentBusiness.Bridges.Pearl.SetupPearlForSendingSingle(_patient,document,filePath:e.FullPath);
					pearl?.SendOnThread();
				}
				else if(OpenDentBusiness.Bridges.BetterDiag.DoAutoUploadForImageCategory(document.DocCategory)) {
					OpenDentBusiness.Bridges.BetterDiag.SendSingleOnThread(_patient,document,filePath:e.FullPath);
				}
		...
		var bitmap=(Bitmap)Image.FromFile(e.FullPath);
		var controlImageDisplay=GetControlImageDisplaySelected();//will always succeed because we verified floater idxSelected in IsMountShowing()
		if(IsMountItemSelected()){
			//They shouldn't have selected an item, but we will try to find an unoccupied spot.
			var listAvail2=GetAvailSlots(1);
			if(listAvail2 is null){//no more available slots, so start saving in unmounted area
				SetIdxSelectedInMount(-1);
			}
			else{
				var mountItem2=controlImageDisplay.GetListMountItems().FirstOrDefault(x=>x.MountItemNum==listAvail2[0].MountItemNum);
		...
		var idxSelectedInMount=GetIdxSelectedInMount();
		...
		if(OpenDentBusiness.Bridges.Pearl.DoAutoUploadForImageCategory(document.DocCategory)) {
			var pearl=OpenDentBusiness.Bridges.Pearl.SetupPearlForSendingSingle(_patient,document,bitmap,mountItem:mountItem);
			pearl?.SendOnThread();
		}
		else if(OpenDentBusiness.Bridges.BetterDiag.DoAutoUploadForImageCategory(document.DocCategory)) {
			OpenDentBusiness.Bridges.BetterDiag.SendSingleOnThread(_patient,document,bitmap,mountItem:mountItem);
		}
		bitmap?.Dispose();
		try {
			File.Delete(e.FullPath);
	...
	private void menuForms_Click(object sender,System.EventArgs e) {
		var formName=((WpfControls.UI.MenuItem)sender).Text;
		Document doc;
		try {
		...
		var controlImageDisplay=GetControlImageDisplaySelected();
		var frmDocInfo=new FrmDocInfo(_patient,doc,isDocCreate:true);
	...
	private void menuTree_Click2(object sender,System.EventArgs e) {
		...
		switch(((WpfControls.UI.MenuItem)sender).Name) {
			...
			case "Info":
				...
				break;
			case "Tasks":
				controlImageDisplay.ToolBarTasks_Click(this,new EventArgs());
				break;
		}
	}
	...
	private void panelSplitter_MouseDown(object sender,MouseEventArgs e) {
		...
		var yCenter=panelSplitter.Height/2;
		var buttonHeight=25;
	...
	private void panelSplitter_MouseMove(object sender,MouseEventArgs e) {
		...
		var pointDelta =new Point(Control.MousePosition.X-_pointMouseDown.X,Control.MousePosition.Y-_pointMouseDown.Y);
		float xDelta96=(pointDelta.X);
		var widthNew=_widthTree96StartDrag+xDelta96;
		if(widthNew<0){
			widthNew=0;
		}
		//if(widthNew>500){
	...
	private void panelSplitter_MouseUp(object sender,MouseEventArgs e) {
		...
		var userOdPref=UserOdPrefs.GetFirstOrDefault(x=>x.FkeyType==UserOdFkeyType.ImageSelectorWidth && x.UserNum==Security.CurUser.UserNum);
	...
	private void panelSplitter_Paint(object sender,PaintEventArgs e) {
		...
		float lengthSide=15;//(15f);
		var xCenter=panelSplitter.Width/2f;
		var yCenter=panelSplitter.Height/2f;
		if(_isTreeDockerCollapsed){
			xCenter+=0.5f;
		...
		using var brush=new SolidBrush(ColorOD.Gray(70));
		e.Graphics.FillPolygon(brush,points);
	}

	///<summary>Occurs when a Pearl image result is received. Attempts to refresh the currently shown image.</summary>
	private void pearl_EventRefreshDisplay(object sender,EventArgs eventArgs) {
		if(IsDisposed) {
			return;
		}
		if(InvokeRequired) {
			Invoke((Action)(() => { pearl_EventRefreshDisplay(sender,eventArgs); }));
			return;
		}
		try {
			var controlImageDisplay=this.GetControlImageDisplaySelected();
			controlImageDisplay?.SetShowDrawings(_showDrawingsOD,_showDrawingsAI,_listPearlEnumCategoryODsShown,_listEnumCategoryBetterDiagsShown);
			labelAIProcessing.Visible=false;
			checkShow.Visible=true;
			checkToothParts.Visible=true;
			butAILayers.Visible=true;
			//FrmPearlLayers could have changed what is showing, so update docked checkboxes to match.
			checkShow.Checked=_showDrawingsAI;
			var listEnumCategoryODsToothParts=OpenDentBusiness.Bridges.Pearl.GetToothPartsCategoryODs();
			checkToothParts.Checked=true;
			for(var i=0;i<listEnumCategoryODsToothParts.Count;i++) {
				if(!_listPearlEnumCategoryODsShown.Contains(listEnumCategoryODsToothParts[i])) {
					checkToothParts.Checked=false;
					break;
				}
			}
	...
	private void pearl_EventClickDeleteAnnotations(object sender,EventArgs e) {
		...
		long docNum=-1;
		var isMountShowing=false;
		if(controlImageDisplay.IsMountItemSelected()) {
			isMountShowing=true;
			var idx=GetIdxSelectedInMount();
		...
		var dialogResult=ODMessageBox.Show("Delete annotations for the currently selected "+(isMountShowing ? "mount item" : "document")+"?","Pearl AI",MessageBoxButtons.YesNo);
	...
	private void betterDiag_EventRefreshDisplay(object sender,EventArgs eventArgs) {
		if(IsDisposed) {
			return;
		}
		if(InvokeRequired) {
			Invoke((Action)(() => { betterDiag_EventRefreshDisplay(sender,eventArgs); }));
			return;
		}
		try {
			var controlImageDisplay=this.GetControlImageDisplaySelected();
			controlImageDisplay?.SetShowDrawings(_showDrawingsOD,_showDrawingsAI,_listPearlEnumCategoryODsShown,_listEnumCategoryBetterDiagsShown);
			//FrmBetterDiagLayers could have changed what is showing, so update docked checkboxes to match.
			checkShow.Checked=_showDrawingsAI;
			var listEnumCategoryBetterDiagsToothParts=OpenDentBusiness.Bridges.BetterDiag.GetBetterDiagToothPartsCategories();
			checkToothParts.Checked=true;
			for(var i=0;i<listEnumCategoryBetterDiagsToothParts.Count;i++) {
				if(!_listEnumCategoryBetterDiagsShown.Contains(listEnumCategoryBetterDiagsToothParts[i])) {
					checkToothParts.Checked=false;
					break;
				}
			}
			//Docked checkboxes could have changed what is showing, so update FrmBetterDiagLayers to match.
			if(_frmBetterDiagLayers!=null) {
				_frmBetterDiagLayers.ListEnumCategoryBetterDiagsShown=_listEnumCategoryBetterDiagsShown;
				_frmBetterDiagLayers.ShowBetterDiagLayers=_showDrawingsAI;
				_frmBetterDiagLayers.RefreshUI();
			}
			LayoutControls();
		}
		catch {
	...
	private void unmountedBar_Remount(object sender, WpfControls.UI.UnmountedObj e){
		...
		var idx=controlImageDisplay.GetIdxSelectedInMount();
		if(idx==-1){
			var listAvail=GetAvailSlots(1);
		...
		var mountItemOld=e.MountItem_;
		var mountItemNew=controlImageDisplay.GetListMountItems()[idx];
		var listImageDraws=ImageDraws.RefreshForDoc(e.Document_.DocNum);
		if(listImageDraws.Count>0 && (mountItemOld.Xpos!=mountItemNew.Xpos || mountItemOld.Ypos!=mountItemNew.Ypos)) {
			var bitmapWidth=e.BitmapImage_.PixelWidth;
			var bitmapHeight=e.BitmapImage_.PixelHeight;
			//Calculate scale of bitmap within old mount item
			var scaleOld=ImageDraws.CalcBitmapScaleToFitMountItem(bitmapWidth,bitmapHeight,mountItemOld.Width,mountItemOld.Height);
			//Calculate scale needed to make bitmap fit in new mount item.
			var scale=ImageDraws.CalcBitmapScaleToFitMountItem((int)(bitmapWidth*scaleOld),(int)(bitmapHeight*scaleOld),mountItemNew.Width,mountItemNew.Height);
			//Calculate padding used to make drawings centered in old mount item.
			var pointPaddingOld=ImageDraws.CalcBitmapPaddingToFitMountItem(bitmapWidth,bitmapHeight,mountItemOld.Width,mountItemOld.Height,scaleOld);
			//Calculate padding needed to make drawings centered in new mount item.
			var pointPadding=ImageDraws.CalcBitmapPaddingToFitMountItem((int)(bitmapWidth*scaleOld),(int)(bitmapHeight*scaleOld),mountItemNew.Width,mountItemNew.Height,scale);
			var pointFTranslateToOrigin=new PointF();
			...
			for(var i=0;i<listImageDraws.Count;i++) {
				...
				if(listImageDraws[i].DrawType==ImageDrawType.Text) {
					listPointFs= [(PointF) listImageDraws[i].GetTextPoint()];
				}
		...
		var document=controlImageDisplay.GetDocumentShowing(idx);
		if(document!=null){
			//unmount the old document
			var mountItemCopy=controlImageDisplay.GetListMountItems()[idx].Copy();
	...
	private void unmountedBar_Retake(object sender, EventArgs e){
		...
		var listMountItems=controlImageDisplay.GetListMountItems();
		var mountItem=listMountItems[idx];
		...
		var mountItemUnmounted=controlImageDisplay.GetListMountItems()[idx].Copy();
	...
	private void windowingSlider_Scroll(object sender,EventArgs e) {
		...
		if(IsMountItemSelected()) {
			var idx=GetIdxSelectedInMount();
			var document=GetDocumentShowing(idx);
	...
	private void ToolBarImport_Click(object sender, EventArgs e){
		var defNumCategory=imageSelector.GetSelectedCategory();
		var enumImageNodeType=imageSelector.GetSelectedType();
		var controlImageDisplay=GetOrMakeControlImageDisplay();
		...
		if(enumImageNodeType==EnumImageNodeType.Category) {
			var nodeTypeAndKeyCategory=new NodeTypeAndKey(enumImageNodeType,defNumCategory);
			controlImageDisplay.ClearObjects();
		...
		var nodeTypeAndKey=controlImageDock.ControlImageDisplay_.GetNodeTypeAndKey();
		if(nodeTypeAndKey is null){
	...
	private void ToolBarMoveToPatient(object sender, EventArgs e){
		...
		if(IsMountShowing()) {
			...
			if(!SecurityL.IsAuthorizedToDeleteImage(GetDocumentShowing(0))) {
				return;
			}
		}
		...
		var patFolderOld=ImageStore.GetPatientFolder(_patient,ImageStore.GetDataFolder());
		Document document=null;
		Mount mount=null;
		...
		if(mount!=null){
			var listMountItems=MountItems.GetItemsForMount(mount.MountNum);
			var documentArray=Documents.GetDocumentsForMountItems(listMountItems);
			try {
			...
			Mounts.Delete(mount);
			var defDocCategory = Defs.GetDef(DefCat.ImageCats,mount.DocCategory);
			var logText = "Mount Deleted: "+mount.Description+" with category "
	...
	private void ToolBarPaste_Click(object sender, EventArgs e){
		...
		//The above line clears out the category information, so we might need to reset that.
		if(enumImageNodeType==EnumImageNodeType.Category) {
			var nodeTypeAndKey=new NodeTypeAndKey(enumImageNodeType,defNumCategory);
			controlImageDisplay.ClearObjects();
			controlImageDisplay.SelectTreeNode2(nodeTypeAndKey);
		}
	...
	private void ToolBarPrint_Click(object sender,EventArgs e){
		...
		if(IsDocumentShowing()){
			if(Path.GetExtension(GetDocumentShowing(0).FileName).ToLower()==".pdf") {//Selected document is PDF, we handle differently than documents that aren't pdf.
				var controlImageDisplay=GetControlImageDisplaySelected();//Will always work because we used IsDocumentShowing
	...
	private void ToolBarPrintCategory(object sender, EventArgs e){
		...
		var listDocumentsCategory=listDocuments.FindAll(x => x.DocCategory==defNumCategory
		                                                     && x.MountItemNum==0//Mounts are not currently supported
		                                                     && (ImageHelper.HasImageExtension(x.FileName) || x.FileName.ToLower().EndsWith(".pdf")));
		if(listDocumentsCategory.IsNullOrEmpty())
		{
			MsgBox.Show("No supported files found in category. Supported file types are images and PDFs. Mounts are not supported at this time.");
			return;
		}
		using var controlImageDisplayPrint=new ControlImageDisplay();
		if(listDocumentsCategory.Count!=listDocuments.Count(x => x.DocCategory==defNumCategory)) {
			controlImageDisplayPrint.ShowError=true;//Some of the images in the category will not be printed. Let the user know.
		}
		controlImageDisplayPrint.PatientCur=_patient;
		controlImageDisplayPrint.SetShowDrawings(_showDrawingsOD,_showDrawingsAI,_listPearlEnumCategoryODsShown,_listEnumCategoryBetterDiagsShown);
		//Create one big pdf of all the documents being printed.
		var path="";
		try {
			path=controlImageDisplayPrint.CreatePDF(listDocumentsCategory);
		}
		catch (Exception ex) {
			FriendlyException.Show("Error occurred while printing",ex);
			return;
		}
		if(File.Exists(path)) {
			controlImageDisplayPrint.PrintPDF(path);
			var progressWin=new ProgressWin();
			progressWin.StartingMessage=Lans.g("Printing...");
			progressWin.ActionMain=() =>
			{
				while(true) {
					if(!controlImageDisplayPrint.IsPrinting || progressWin.IsCancelled) {
						break;
					}
					Thread.Sleep(1000);//Block the UI while printing is completed async. This protects against threading issues
				}
			};
			progressWin.ShowDialog();
		}
		if(controlImageDisplayPrint.ShowError) {
			MsgBox.Show(this,"One or more documents failed to print.\r\nSupported document types are images and PDFs.");
			controlImageDisplayPrint.ShowError=false;
		}
	}

	private void ToolBarProgram_Click(object sender,EventArgs e){
		var frameworkElementSender=(System.Windows.FrameworkElement)sender;
		var program=(Program)frameworkElementSender.Tag;//Could be ToolBarButton or MenuItem
		if(!program.ProgName.In(ProgramName.Pearl.ToString(),ProgramName.BetterDiagnostics.ToString())){
			WpfControls.ProgramL.Execute(program.ProgramNum,_patient);
			return;
		}
		//All code below this line in this method handles the Pearl or BD program link buttons.
		//Those buttons can only work from the Imaging module, so we handle it here.
		...
		var listBitmaps=controlImageDisplay.GetListBitmaps();
		if(listBitmaps.IsNullOrEmpty()) {
			return;
		}
		if(program.ProgName==ProgramName.BetterDiagnostics.ToString()) {
			document=GetDocumentShowing(0);
			if(IsMountShowing()){
				var listMountItems=controlImageDisplay.GetListMountItems();
				for(var i=0;i<listBitmaps.Count;i++) {
					document=controlImageDisplay.GetDocumentShowing(i);
					if(listBitmaps[i]==null || document==null) {
						continue;
					}
					var listImageDraws=controlImageDisplay.GetImageDraws();
					var imageDrawExisting=listImageDraws.FirstOrDefault(x => x.ImageAnnotVendor==EnumImageAnnotVendor.BetterDiagnostics && x.DocNum==document.DocNum);
					if(imageDrawExisting!=null) {
						continue;//If we already have BetterDiagnostics results for this document, don't send again.
					}
					OpenDentBusiness.Bridges.BetterDiag.SendSingleOnThread(_patient,document,listBitmaps[i],mountItem:listMountItems[i]);
				}
			}
			else{//one doc
				document=GetDocumentShowing(0);
				...
				var imageDrawExisting=listImageDraws.FirstOrDefault(x => x.ImageAnnotVendor==EnumImageAnnotVendor.BetterDiagnostics && x.DocNum==document.DocNum);
				if(imageDrawExisting!=null) {
					return;//If we already have BetterDiagnostics results for this document, don't send again.
				}
				OpenDentBusiness.Bridges.BetterDiag.SendSingleOnThread(_patient,document,listBitmaps[0]);
			}
			return;
		}
		//All code in this method below this line is for Pearl.
		if(IsMountShowing()){
			var listMountItems=controlImageDisplay.GetListMountItems();
			for(var i=0;i<listBitmaps.Count;i++) {
				document=controlImageDisplay.GetDocumentShowing(i);
				...
				var pearl=OpenDentBusiness.Bridges.Pearl.SetupPearlForSendingSingle(_patient,document,listBitmaps[i],mountItem:listMountItems[i]);
				pearl?.SendOnThread();
			}
		}
		else{//one doc
			...
			var pearlRequest=PearlRequests.GetOneByDocNum(document.DocNum);
			if(pearlRequest!=null && pearlRequest.RequestStatus!=EnumPearlStatus.TimedOut) {
				MsgBox.Show("This image was already sent to Pearl.");
				return;
			}
			var pearl=OpenDentBusiness.Bridges.Pearl.SetupPearlForSendingSingle(_patient,document,listBitmaps[0]);
			pearl?.SendOnThread();
		}
		if(_listPearlEnumCategoryODsShown.IsNullOrEmpty()) {//if no Pearl annotations are set to show right now
			//turn them all on so that user can see result
			_listPearlEnumCategoryODsShown=Enum.GetValues(typeof(OpenDentBusiness.Pearl.EnumCategoryOD))
	...
	private void ToolBarScan_Click(ImageType imgType){
		...
		var errorCode=EZTwain.LastErrorCode();
		if(errorCode!=0) {
		...
		var xdpi=EZTwain.DIB_XResolution(handleDIB);
		var ydpi=EZTwain.DIB_XResolution(handleDIB);
		var handleBitmap=EZTwain.DIB_ToDibSection(handleDIB);
		try {
		...
		Document doc = null;
		try {//Create corresponding image file.
			var doPrintHeading=false;
			if(imgType==ImageType.Radiograph) {
				doPrintHeading=true;
			}
			doc=ImageStore.Import(bitmapScanned,GetCurrentCategory(),imgType,_patient,printHeading:doPrintHeading);
		}
		catch(Exception ex) {
			saved=false;
			Cursor=Cursors.Default;
		...
		if(saved) {
			...
			SelectTreeNode1(new NodeTypeAndKey(EnumImageNodeType.Document,doc.DocNum));
			var controlImageDisplay=GetControlImageDisplaySelected();
			var frmDocInfo=new FrmDocInfo(_patient,GetDocumentShowing(0),isDocCreate:true);
			frmDocInfo.ShowDialog();
			if(frmDocInfo.IsDialogCancel) {
				DeleteDocument(false,false,doc);
			}
	...
	private void ToolBarScanMulti_Click(object sender,EventArgs e) {
		...
		var duplexEnabled=EZTwain.EnableDuplex(ComputerPrefs.LocalComputer.ScanDocDuplex);//This line seems to cause problems.
		if(ComputerPrefs.LocalComputer.ScanDocGrayscale) {
	...
	private void ToolBarSign_Click(object sender,EventArgs e){
		...
		var frmDocSign=new FrmDocSign(GetDocumentShowing(0),_patient);//Updates our local document and saves changes to db also.
		var pointLocal=new Point(panelSplitter.Left,Height);
		var pointScreen=PointToScreen(pointLocal);
	...
	private ControlImageDisplay CreateControlImageDisplay(){
		var controlImageDisplay=new ControlImageDisplay();
		controlImageDisplay.ImageSelector_=imageSelector;
		controlImageDisplay.EventEnableToolBarButtons+=(_,toolBarButtonState)=>EnableToolBarButtons(toolBarButtonState);
		controlImageDisplay.EventFillTree+=(_,keepSelection)=>FillImageSelector(keepSelection);
		controlImageDisplay.EventSelectTreeNode+=(_,nodeTypeAndKey)=>{
			if(nodeTypeAndKey is null){
				nodeTypeAndKey=controlImageDisplay.GetNodeTypeAndKey();
			}
			SelectTreeNode1(nodeTypeAndKey);
		};
		controlImageDisplay.EventSetCropPanEditAdj+=(_,cropPanAdj)=>SetCropPanAdj(cropPanAdj);
		controlImageDisplay.EventSetDrawMode+=(_,drawMode)=>SetDrawMode(drawMode);
		controlImageDisplay.EventSetWindowingSlider+=FormImageFloat_SetWindowingSlider;
		controlImageDisplay.EventThumbnailNeedsRefresh+=(_,_)=>ThumbnailRefresh();
		...
		controlImageDisplay.EventZoomSliderSetValueAndMax+=(_,newVal)=>zoomSlider.SetValueAndMax(newVal);
	...
	private void EnableToolBarsPatient(bool enable) {
		...
		//Acquire button?
		for(var i=0;i<toolBarPaint.Buttons.Count;i++) {
	...
	private void FillSignature() {
		...
		if(GetDocumentShowing(0).SigIsTopaz) {
			if(GetDocumentShowing(0).Signature!=null && GetDocumentShowing(0).Signature!="") {
				...
				var keystring=ImageStore.GetHashString(GetDocumentShowing(0),_patFolder);
	...
	private ControlImageDisplay GetControlImageDisplaySelected(){
		...
		var formImageFloat=_listFormImageFloats.FirstOrDefault(x=>x.IsImageFloatSelected);
		if(formImageFloat is null){
			return null;
		}
		return formImageFloat.ControlImageDisplay;
	}
	...
	public void LayoutControls(){
		elementHostToolBarMain.Bounds=new Rectangle(0,0,Width,25);
		elementHostWindowingSlider.Bounds=new Rectangle(4,elementHostToolBarMain.Bottom+2,154,20);
		elementHostZoomSlider.Bounds=new Rectangle(elementHostWindowingSlider.Right+4,elementHostToolBarMain.Bottom,231,25);
		toolBarPaint.Bounds=new Rectangle(elementHostZoomSlider.Right+1,elementHostToolBarMain.Bottom,Width-(elementHostZoomSlider.Right+1),25);
		if(_isTreeDockerCollapsed){
			imageSelector.Visible=false;
			panelAILayers.Visible=false;
			panelSplitter.Bounds=new Rectangle(0,toolBarPaint.Bottom,10,Height-toolBarPaint.Bottom);
			panelSplitter.Invalidate();
		}
		else{
			imageSelector.Visible=true;
			panelAILayers.Visible=ShouldLayersPanelShow();
			if(_widthTree96>Width-150){//this is the min set in FormODBase for FormImageFloat
				_widthTree96=Width-150;
			}
			var heightPanelPearl=62;
			if(!panelAILayers.Visible) {
				heightPanelPearl=0;
			}
			panelAILayers.Bounds=new Rectangle(0,Height-heightPanelPearl,(int)_widthTree96,heightPanelPearl);
			elementHostImageSelector.Bounds=new Rectangle(0,toolBarPaint.Bottom,(int)_widthTree96,Height-heightPanelPearl-toolBarPaint.Bottom);
			panelSplitter.Bounds=new Rectangle(elementHostImageSelector.Width+1,toolBarPaint.Bottom,10,Height-toolBarPaint.Bottom);
			panelSplitter.Invalidate();
		}
		var heightPanelNote=Math.Min(114,Height-toolBarPaint.Bottom);
		if(!panelNote.Visible){
			heightPanelNote=0;
		}
		panelNote.Bounds=new Rectangle(
			panelSplitter.Right+1,
			Height-heightPanelNote-1,
			Width-panelSplitter.Right-1,
			heightPanelNote);
		var heightUnmountedBar=Math.Min(200,Height-toolBarPaint.Bottom-heightPanelNote);
		if(!elementHostUnmountedBar.Visible){
			heightUnmountedBar=0;
		}
		elementHostUnmountedBar.Bounds=new Rectangle(
			panelSplitter.Right+1,
			Height-heightPanelNote-heightUnmountedBar-1,
			Width-panelSplitter.Right-1,
			heightUnmountedBar);
		var panelMainTop=toolBarPaint.Bottom;
		if(panelImportAuto.Visible){
			panelImportAuto.Bounds=new Rectangle(
				panelSplitter.Right+1,
				toolBarPaint.Bottom,
				Width-panelSplitter.Right-1,
				44);	
		...
		if(panelDraw.Visible){
			panelDraw.Bounds=new Rectangle(
				panelSplitter.Right+1,
				toolBarPaint.Bottom,
				Width-panelSplitter.Right-1,
				25);	
		...
		controlImageDock.Bounds=new Rectangle(
			panelSplitter.Right+1,
	...
	private void RefreshModuleData(long patNum) {
		...
		_familyCur=Patients.GetFamily(patNum);
		_patient=_familyCur.GetPatient(patNum);
		_patFolder=ImageStore.GetPatientFolder(_patient,ImageStore.GetDataFolder());//This is where the pat folder gets created if it does not yet exist.
	...
	private void SetUnmountedBarVisibility(){
		...
		var listUnmountedObjs=controlImageDock.ControlImageDisplay_.GetUmountedObjs();
	...
	private void ShowFilterWindow(){
		...
		var frmImageFilter=new FrmImageFilter();
	...
	private void ThumbnailRefresh(){
		if(IsDocumentShowing()){
			var bitmapThumb=Documents.GetThumbnail(GetDocumentShowing(0),_patFolder);
			imageSelector.SetThumbnail(bitmapThumb);
		}
		if(IsMountShowing()){
			var bitmapThumb=Mounts.GetThumbnail(GetMountShowing().MountNum,_patFolder);
			imageSelector.SetThumbnail(bitmapThumb);
		}
	}
	...
	private void ToolBarColor_Click(){
		...
		using var formImageDrawColor=new FormImageDrawColor();
	...
	private void ToolBarMountAcquire_Click(object sender, EventArgs e){
		...
		var frmMountAndAcquire=new FrmMountAndAcquire();
		frmMountAndAcquire.ShowDialog();
		if(frmMountAndAcquire.IsDialogCancel){
			return;
		}
		var mountDef=frmMountAndAcquire.MountDefSelected;
		if(mountDef!=null){
			long defNumCategory;
			...
			var mount=Mounts.CreateMountFromDef(mountDef,_patient.PatNum,defNumCategory);
			var defDocCategory=Defs.GetDef(DefCat.ImageCats,mount.DocCategory);
			var logText="Mount Created: "+mount.Description+" with category "
			            +defDocCategory.ItemName;
		...
		var imagingDevice=frmMountAndAcquire.ImagingDeviceSelected;
	...
	private void AcquireMulti(string deviceDescription=""){
		...
		var listBitMapsDexis=new List<Bitmap>();
		while(true) {
			var hdib=EZTwain.Acquire(ParentForm.Handle);
			if(hdib==IntPtr.Zero) {
				break;
			}
			var bitmap=(Bitmap)EZTwain.DIB_ToImage(hdib);
			if(isDexis) {
		...
		for(var i=listBitMapsDexis.Count-1;i >= 0;i--) {//Iterate through the list backwords to correctly place the mounts
	...
	private bool PlaceAcquiredBitmapInUI(Bitmap bitmap){
		...
		try {
			document=ImageStore.Import(bitmap,GetCurrentCategory(),ImageType.Radiograph,_patient,mimeType:"image/tiff",printHeading:true);
		}
		catch(Exception ex) {
			bitmap?.Dispose();
			ODMessageBox.Show(Lan.g(this,"Unable to save")+": "+ex.Message);
		...
		if(!IsMountShowing()){//single
			...
			SelectTreeNode1(new NodeTypeAndKey(EnumImageNodeType.Document,document.DocNum));
			if(OpenDentBusiness.Bridges.Pearl.DoAutoUploadForImageCategory(document.DocCategory)) {
				var pearl=OpenDentBusiness.Bridges.Pearl.SetupPearlForSendingSingle(_patient,document,bitmap);
				pearl?.SendOnThread();
			}
			else if(OpenDentBusiness.Bridges.BetterDiag.DoAutoUploadForImageCategory(document.DocCategory)) {
				OpenDentBusiness.Bridges.BetterDiag.SendSingleOnThread(_patient,document,bitmap);
			}
			bitmap?.Dispose();
			return true;
		}
		//From here down is mount======================================================================================
		var controlImageDisplay=GetControlImageDisplaySelected();//will always succeed because we verified in IsMountShowing
		   ...
		   || GetIdxSelectedInMount()==-1)
		{
			...
			var listMountItemsAvail2=GetAvailSlots(1);
			if(listMountItemsAvail2 is null){//no more available slots, so start saving in unmounted area
				SetIdxSelectedInMount(-1);
			}
			else{
				var mountItem2=controlImageDisplay.GetListMountItems().FirstOrDefault(x=>x.MountItemNum==listMountItemsAvail2[0].MountItemNum);
				if(mountItem2==null){//should not be possible
					return false;
				}
				SetIdxSelectedInMount(controlImageDisplay.GetListMountItems().IndexOf(mountItem2));
	...
	private void UpdateToolbarButtons() {
		if(this.Enabled && _patient!=null) {
			...
			DisableAllToolBarButtons(enablePrint:true);//Disable item specific tools until item chosen.
		}
		else {
	...
	///<summary>Returns whether the docked AI Layers panel should be shown or hidden. Only true when an AI Imaging program enabled and AI ImageDraws exist for any currently shown image.</summary>
	private bool ShouldLayersPanelShow() {
		var programName=Programs.GetActiveImagingAIProgram();
		...
		if(IsDocumentShowing()) {
			...
			var listMountItems=controlImageDisplay.GetListMountItems();
			for(var i=0;i<listMountItems.Count;i++) {
				var document=GetDocumentShowing(i);
				if(document!=null) {
					listDocuments.Add(document);
				}
			}
		}
		//Check for any AI annotations on the currently shown image(s).
		var listImageDraws=controlImageDisplay.GetImageDraws();
		var imageDraw=listImageDraws.FirstOrDefault(x => x.ImageAnnotVendor!=EnumImageAnnotVendor.OpenDental);
		if(imageDraw!=null) {
			return true;
		}
		...
		if(programName==ProgramName.Pearl) {
			if(IsDocumentShowing()) {
				var pearlRequest=PearlRequests.GetOneByDocNum(listDocuments[0].DocNum);
				if(pearlRequest!=null && pearlRequest.RequestStatus.In(EnumPearlStatus.Polling,EnumPearlStatus.Uploading)) {
					isProcessing=true;
					labelAIProcessing.Text=pearlRequest.RequestStatus+"...";
				}
			}
			else if(IsMountShowing()) {
				for(var i=0;i<listDocuments.Count;i++) {
					var pearlRequest=PearlRequests.GetOneByDocNum(listDocuments[i].DocNum);
					if(pearlRequest!=null && pearlRequest.RequestStatus.In(EnumPearlStatus.Polling,EnumPearlStatus.Uploading)) {
						isProcessing=true;
						labelAIProcessing.Text=pearlRequest.RequestStatus+"...";
						break;
			...
			if(isProcessing) {
				...
				checkToothParts.Visible=false;
				butAILayers.Visible=false;
				return true;
			}
		}
		return false;
