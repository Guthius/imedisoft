...
namespace OpenDental{
	...
	public partial class ControlImages : UserControl{
		...
		public void LayoutToolBars() {
			toolBarMain.Clear();
			WpfControls.UI.ContextMenu contextMenuPrint=new WpfControls.UI.ContextMenu();
			...
			ProgramName programNameAI=Programs.GetActiveImagingAIProgram();
			if(programNameAI==ProgramName.Pearl){
				...
				if(_listPearlEnumCategoryODsShown is null){//happens once on startup
					_listPearlEnumCategoryODsShown=new List<OpenDentBusiness.Pearl.EnumCategoryOD>();
		...
		private void butAILayers_Click(object sender, EventArgs e) {
			...
			if(programName==ProgramName.Pearl) {
				...
				_frmBetterDiagLayers.FormClosing+=(s,e) => {
		...
		private void checkToothParts_Click(object sender, EventArgs e) {
			ProgramName programName=Programs.GetActiveImagingAIProgram();
			if(programName==ProgramName.Pearl) { 
				List<OpenDentBusiness.Pearl.EnumCategoryOD> listEnumCategoryODs=OpenDentBusiness.Bridges.Pearl.GetToothPartsCategoryODs();
				...
				List<EnumCategoryBetterDiag> listEnumCategoryBetterDiags=OpenDentBusiness.Bridges.BetterDiag.GetBetterDiagToothPartsCategories();
				if(checkToothParts.Checked) {
		...
		private void pearl_EventRefreshDisplay(object sender,EventArgs eventArgs) {
			...
			try {
				...
				for(int i=0;i<listEnumCategoryODsToothParts.Count;i++) {
		...
		private void betterDiag_EventRefreshDisplay(object sender,EventArgs eventArgs) {
			...
			try {
				...
				checkShow.Checked=_showDrawingsAI;
				List<EnumCategoryBetterDiag> listEnumCategoryBetterDiagsToothParts=OpenDentBusiness.Bridges.BetterDiag.GetBetterDiagToothPartsCategories();
				checkToothParts.Checked=true;
				for(int i=0;i<listEnumCategoryBetterDiagsToothParts.Count;i++) {
		...
		private void ToolBarPrintCategory(object sender, EventArgs e){
			...
			List<Document> listDocuments=Documents.GetPatientData(_patient.PatNum);
			List<Document> listDocumentsCategory=listDocuments.FindAll(x => x.DocCategory==defNumCategory
			...
			using ControlImageDisplay controlImageDisplayPrint=new ControlImageDisplay();
			...
			if(File.Exists(path)) {
				...
				progressWin.StartingMessage=Lans.g(this,"Printing...");
		...
		private void ToolBarProgram_Click(object sender,EventArgs e){
			System.Windows.FrameworkElement frameworkElementSender=(System.Windows.FrameworkElement)sender;
			Program program=(Program)frameworkElementSender.Tag;//Could be ToolBarButton or MenuItem
			...
			if(program.ProgName==ProgramName.BetterDiagnostics.ToString()) {
				...
				if(IsMountShowing()){
					...
					for(int i=0;i<listBitmaps.Count;i++) {
						...
						List<ImageDraw> listImageDraws=controlImageDisplay.GetImageDraws();
						ImageDraw imageDrawExisting=listImageDraws.FirstOrDefault(x => x.ImageAnnotVendor==EnumImageAnnotVendor.BetterDiagnostics && x.DocNum==document.DocNum);
		...
		public void LayoutControls(){
			...
			if(_isTreeDockerCollapsed){
				...
				int heightPanelPearl=LayoutManager.Scale(62);
				if(!panelAILayers.Visible) {
					heightPanelPearl=0;
				}
				LayoutManager.Move(panelAILayers,new Rectangle(0,Height-heightPanelPearl,LayoutManager.Scale(_widthTree96),heightPanelPearl));
				LayoutManager.Move(elementHostImageSelector,new Rectangle(0,toolBarPaint.Bottom,LayoutManager.Scale(_widthTree96),Height-heightPanelPearl-toolBarPaint.Bottom));
		...
		private bool ShouldLayersPanelShow() {
			...
			if(IsDocumentShowing()) {
				...
				for(int i=0;i<listMountItems.Count;i++) {
					Document document=GetDocumentShowing(i);
					if(document!=null) {
						listDocuments.Add(document);
					}
				}
			...
			List<ImageDraw> listImageDraws=controlImageDisplay.GetImageDraws();
			ImageDraw imageDraw=listImageDraws.FirstOrDefault(x => x.ImageAnnotVendor!=EnumImageAnnotVendor.OpenDental);
			if(imageDraw!=null) {
				return true;
			}
