...
namespace OpenDental {
	...
	public partial class ControlAccount:UserControl {
		...
		public ControlAccount() {
			...
			LayoutManager.Add(gridInsEstOpenClaims,this);
		...
		private void menuItemInvoice_Click(object sender,EventArgs e) {
			...
			Patient patientGuar=Patients.GetPat(_patient.Guarantor);
		...
		private void menuItemLimitedCustom_Click(object sender,EventArgs e) {
			...
			DataSet dataSetSuperFam=AccountModules.GetSuperFamAccount(statement, isComputeAging:false, doIncludePatLName:true, listPatients:listPatients);
		...
		private void menuItemQuickProcs_Click(object sender,EventArgs e) {
			...
			int indexClicked=contextMenuQuickProcs.MenuItems.IndexOf((MenuItem)sender);
			if(!PrefC.GetString(PrefName.SalesTaxProcCode).IsNullOrEmpty()) {
				indexClicked-=1;
			}
			Def defQuickCharge=_arrayDefsAcctProcQuickAdd[indexClicked];
		...
		private void menuItemSalesTaxProc_Click(object sender,EventArgs e) {
			...
			List<Procedure> listProcs=Procedures.GetManyProc(listProcNumsSelected,includeNote:false);
		...
		private void menuItemSendPaymentToDevice_Click(object sender,EventArgs e) {
			...
			MobileAppDevice mobileAppDevice=MobileAppDevices.ShouldCreateMobileNotification(_patient.PatNum);
		...
		private void menuItemSendMessageToPay_Click(object sender,EventArgs e) {
			...
			FormMessageToPayEdit formMessageToPayEdit=new FormMessageToPayEdit(_patient);
		...
		public void LayoutToolBar() {
			...
			if(!PrefC.GetBool(PrefName.EasyHideRepeatCharges)) {
				...
				if(false) {//contextMenuRepeat items only get initialized when at HQ.
					toolBarButton.Style=ODToolBarButtonStyle.DropDownButton;
		...
		private void toolBarButPay_Click(double payAmt,bool preferCurrentPat=false,bool isPrePay=false,bool isIncomeTransfer=false,bool isTsiPayment=false) {
			...
			if(isPrePay && CompareDecimal.IsGreaterThanZero(unearnedAmt)) {
				if(listAccountEntries.Count<1) {
					using FormProcSelect formProcSelect=new FormProcSelect(_patient.PatNum,true,doShowAdjustments:true,doShowTreatmentPlanProcs:false);
		...
		private void RefreshOrthoCasesGridRows() {
			...
			List<OrthoProcLink> listOrthoProcLinksForPat=OrthoProcLinks.GetManyByOrthoCases(_listOrthoCases.ConvertAll(x => x.OrthoCaseNum));
			List<Procedure> listProceduresLinkedForPat=Procedures.GetManyProc(listOrthoProcLinksForPat.ConvertAll(x => x.ProcNum),false);
...
;
		...
		private void FillInsEstBreakdown() {
			List<long> listPatNumsForFamily=_family.ListPats.Select(x=>x.PatNum).ToList();
			DataTable table=ClaimProcs.GetClaimProcEstimatesForPatients(listPatNumsForFamily);
		...
		private void FillAutoOrtho(bool doCalculateFirstDate=true) {
			...
			int txTimeInMonths=OrthoSchedules.CalculateAutoOrthoTimeInMonths(dateFirstOrthoProc,dateSpan,txMonthsTotal);
			string strDateDiff="";
		...
		private void FillOrthoCasesGrid() {
			...
			GridColumn col=new GridColumn(Lan.g("TableOrthoCases","Is Active"),70,HorizontalAlignment.Center);
		...
		private void FillGridAcctPat(bool isSelectingFamily) {
			...
			gridAcctPat.EndUpdate();
			int index=gridAcctPat.ListGridRows.FindIndex(x=>((Patient)x.Tag).PatNum==_patient.PatNum);
			if(isSelectingFamily) {
		...
		private void FillGridAcctPatAsSuperFam() {
			...
			gridAcctPat.EndUpdate();
			for(int i=0;i<gridAcctPat.ListGridRows.Count;i++) {
		...
		private void FillTpUnearned() {
			...
			List<Procedure> listProceduresForHiddenSplits=Procedures.GetManyProc(_listPaySplitsHidden.ConvertAll(x => x.ProcNum),includeNote:false);
			...
			for(int i=0;i<_listPaySplitsHidden.Count;i++) {
				...
				long codeNum=listProceduresForHiddenSplits.Find(x => x.ProcNum==_listPaySplitsHidden[i].ProcNum)?.CodeNum??0;
		...
		private void PrintStatement(Statement statement) {
			...
			MobileAppDevice mobileAppDevice=listMobileAppDevices.Find(x => x.PatNum==_patient.PatNum);
		...
		private void PushPaymentToEclipboard(MobileAppDevice mobileAppDevice) {
			string errMsg=MobileNotifications.CI_SendPayment(mobileAppDevice.MobileAppDeviceNum,_patient.PatNum);
