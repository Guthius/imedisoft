...
namespace OpenDental{
	...
	public partial class FormOpenDental:FormODBase {
		...
		public void FormOpenDentalShown() {
			...
			if(true && Security.CurUser!=null) {//If block must be run before StartCacheFillForFees() so correct clinic filtration occurs.
				if(clinicNumCLA!=""){
		...
		private void menuClick_OpenPickList(object sender,EventArgs e) {
			...
			if(formClinics.SelectedClinicId==0) {//'Headquarters' was selected.
	...
	private void TaskGoTo(TaskObjectType taskObjectType,long keyNum){
		...
		private void menuItemSecurity_Click(object sender,EventArgs e) {
			...
			if(!true) {//clinics not enabled, refresh current module and return
				RefreshCurrentModule();
				return;
			}
			//clinics is enabled
		...
		private void RefreshMenuReports() {
			...
			if(listToolButItems.Count==0) {//if there is one or more items, the add button further down will handle the layout.
