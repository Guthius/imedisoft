...
namespace OpenDental
{
    public partial class FormOpenDental : FormODBase
    {
        ...
        public FormOpenDental(string[] cla)
        {
            ...
            if (LayoutManagerForms.IsTestingMode())
            {
                //For rapid testing, we need it to start center screen, not max.
                /*
        ...
        public static Dictionary<long, Dictionary<long, DateTime>> DicBlockedAutomations
        {
            get
            {
                ...
                for (int i = 0; i < listAutoNums.Count; i++)
                {
                    ...
                    for (int j = 0; j < listPatNums.Count; j++)
                    {
                        ...
                        if (_dictionaryBlockedAutomations[listAutoNums[i]][listPatNums[j]] > DateTime.Now)
                        {
                    ...
                    if (_dictionaryBlockedAutomations[listAutoNums[i]].Count() == 0)
                    {
        ...
        {
            get { return _formOpenDentalSingleton.controlChart.GetListTabProcPageTitles(); }
        }
        ...
        public void FormOpenDentalShown()
        {
            ...
            while (true)
            {
                ...
                if (CommandLineArgs_.ArrayCommandLineArgs.Length == 0 && !Web.IsWeb)
                {
                    formSplash.Show(this);
                }
                
                if (!PrefsStartup(isSilentUpdate))
                {
            ...
            _ = false; //calling this as soon as the pref cache is refreshed so false will be set right away
            ...
            _menuItemPayloadMonitor.Available = false;
            ...
            if (!hasConnected)
            {
                //It does not successfully connect to Dropbox or Sftp, showing an error message.
            ...
            if (_strCmdLineShow == "searchpatient")
            {
                ...
                {
                    PatNumCur = frmPatientSelect.PatNumSelected;
            ...
            if (!PrefC.GetBool(PrefName.LicenseAgreementAccepted))
            {
                ...
                {
                    //License Agreement already signed in local database, just need to send to HQ
        ...
        private void FormOpenDental_KeyDown(object sender, System.Windows.Forms.KeyEventArgs e)
        {
            ...
            {
                //ensure field is present and not null before triggering key down
                if (controlImages.Visible)
                {
                    controlImages.ControlImages_KeyDown(e.KeyCode); //for video capture trigger
                }
            }
        ...
        private void toolButPatient_Click()
        {
            ...
            if (Regex.IsMatch(textClip, @"^patnum:\d+$"))
            {
                ...
                {
                    // if not same then we are doing a fresh search and should just look for the patnum
                    if (TrySetPatient(patNum))
                    {
                        //don't show the patient select form and just load the patient we found
                        return;
                    }
                }
            }

            // if the patnum was the same as last time then we have already tried this search once, so show FrmPatientSelect
            FrmPatientSelect frmPatientSelect = new FrmPatientSelect();
            frmPatientSelect.CanAddPatients = true;
            frmPatientSelect.ShowDialog(); //if there is no match, open the form as it normally would
            if (frmPatientSelect.IsDialogOK)
            {
        ...
        private void ProcessCommandLine()
        {
            ...
            if (!Programs.UsingEcwTightOrFullMode() && CommandLineArgs_.ArrayCommandLineArgs.Length == 0)
            {
                ...
                || userName != "" && (Security.CurUser == null || Security.CurUser.UserName != userName))
            {
                ...
                UnselectActive();
                AllNeutral();
                Userod userod = Userods.GetUserByName(userName);
                if (userod == null)
                {
                ...
                if (passHash != userod.PasswordHash || !Programs.UsingEcwTightOrFullMode())
                {
        ...
        private bool PrefsStartup(bool isSilentUpdate)
        {
            ...
            if (updateComputerName != "" && !updateComputerName.ToUpper().In(clientNameUpper, machineNameUpper))
            {
                ...
                string compNameToDisplay = updateComputerName;
                ...
                switch (section)
                {
                    case "ChartModule":
                        if (_dictionaryChartPrefsCache.Count == 0
                            || false != (bool) _dictionaryChartPrefsCache["DistributorKey"]
        ...
        private void UpdateToolbarButtons()
        {
            if (PatNumCur == 0)
            {
                //Only on startup or log out / log in.
                if (!Programs.UsingEcwTightMode())
                {
                    //eCW tight only gets Patient Select and Popups toolbar buttons
        ...
        private void LayoutControls()
        {
            ...
            if (userControlTasks1.Visible)
            {
                if (menuItemDockBottom.Checked)
                {
                    if (panelSplitter.Height > LayoutManager.Scale(9))
                    {
                ...
                {
                    //docked Right
                    if (panelSplitter.Width > LayoutManager.Scale(9))
                    {
        ...
        private void FillPatientButton(Patient patient)
        {
            ...
            for (int i = _listPopupEvents.Count - 1; i >= 0; i--)
            {
                //go backwards
        ...
        {
            bool isChangingClinic = Clinics.ClinicNum != clinic.Id;
        ...
        private void PreprocessForTasks()
        {
            ...
            else if (_listTasksRemindersOverLimit.Count > 0)
            {
                ...
                List<UserOdPref> listUserOdPrefsBlockedTasks = UserOdPrefs.GetByUserAndFkeyType(Security.CurUser.UserNum, UserOdFkeyType.TaskListBlock);
                for (int i = _listTasksRemindersOverLimit.Count - 1; i >= 0; i--)
                {
                    //TaskPopupHelper
        ...
        public static void GetAndProcessSignals(Action<bool> onShutdown, Action<List<FormODBase>, List<Signalod>> onProcess, Action onDone, bool isAllInvalidTypes)
        {
            ...
            ODThread threadRefreshSignals = new ODThread((o) =>
            {
                ...
                onProcess(FormODBase.ListODFormsSubscribed, listSignals);
            });
        ...
        {
            ModuleEventArgs e = new ModuleEventArgs(dateSelected ?? DateTime.MinValue, listPinApptNums ??= new List<long>(), selectedAptNum, moduleType, claimNum, patNum, docNum, doShowSearch);
        ...
        private bool IsPatInRestrictedClinic(long patNum)
        {
            ...
            List<long> listUserClinicNums = Clinics.GetForUserod(Security.CurUser, !Security.CurUser.ClinicIsRestricted).Select(x => x.Id).ToList();
        ...
        private bool AreYouSurePrompt(long userNum, string message)
        {
            ...
            if (userOdPrefLogOffMessage == null)
            {
                ...
                {
                    //Don't save the checkbox if inputBox times out
                    return true;
                }
        ...
        private void menuItemProviders_Click(object sender, System.EventArgs e)
        {
                ...
                && !Security.IsAuthorized(EnumPermType.ProviderAlphabetize, suppressMessage: true))
            {
                ...
                {
                    //If none of the provider related permissions are true and Dental Schools is turned off, display a single message.
                    MsgBox.Show(Lans.g("Security", "Not authorized.") + "\r\n"
                                                                      + Lans.g("Security", "A user with the SecurityAdmin permission must grant you access for") + ":\r\n"
        ...
        private void NonModalReportSelectionHelper(ReportNonModalSelection reportNonModalSelection)
        {
            switch (reportNonModalSelection)
            {
                ...
                case ReportNonModalSelection.ProceduresIndividual:
                    FormRpProcSheet formRpProcSheet = new FormRpProcSheet();
        ...
        private bool TryLaunchPatientDashboard(SheetDef sheetDefWidget)
        {
            ...
            {
                UserOdPref userOdPrefDashboard = new UserOdPref()
                {
        ...
        {
            get { return !_formOpenDentalSingleton.splitContainer.Panel2Collapsed && _formOpenDentalSingleton.userControlDashboard.IsInitialized; }
        }
            ...
            if (Security.CurUser == null)
            {
                ...
                else if (PrefC.GetBool(PrefName.DomainLoginEnabled) && !string.IsNullOrWhiteSpace(PrefC.GetString(PrefName.DomainLoginPath)))
                {
                    ...
                    try
                    {
                        ...
                        Dictionary<long, string> dictDomainUserNumsAndNames = Userods.GetUsersByDomainUserNameNoCache(domainUser);
        ...
        private List<Form> GetOpenForms()
        {
            ...
            for (int f = Application.OpenForms.Count - 1; f >= 0; f--)
            {
                ...
                if (listFormsOpen.Contains(formOpen))
                {
                    //MS list has duplicates
                    continue;
                }
        ...
        public bool CloseOpenForms(bool isForceClose, int forceCloseTimeoutMS = 15000, bool isSnip = false)
        {
            ...
            if (controlImages != null)
            {
                //controlImagesJ never initialized when using the old imaging module 
        ...
        private void FinishLogOff(bool isForced)
        {
            ...
            if (userod.UserNum == Security.CurUser.UserNum || !true)
            {
