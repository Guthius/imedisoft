...
public partial class ControlImagesOld:System.Windows.Forms.UserControl {
	...
	public ControlImagesOld() {
		...
		var platformUnix=Environment.OSVersion.Platform==PlatformID.Unix;
	...
	private void TreeDocuments_AfterCollapse(object sender,TreeViewEventArgs e) {
		var nodeIdTag=(NodeIdTag)e.Node.Tag;
	...
	private void treeDocuments_DragDrop(object sender,DragEventArgs e) {
		var treeNodeOver=treeMain.GetNodeAt(treeMain.PointToClient(Cursor.Position));
		if(treeNodeOver==null) {
			return;
		}
		var nodeIdTagOver=(NodeIdTag)treeNodeOver.Tag;
		...
		var nodeIdTag=new NodeIdTag();
		var arrayFiles = (string[])e.Data.GetData(DataFormats.FileDrop);
		var errorMessage="";
		for(var i=0;i<arrayFiles.Length;i++) {
			var draggedFilePath=arrayFiles[i];
			var fileName=draggedFilePath.Substring(draggedFilePath.LastIndexOf("\\")+1);
			...
			var frmDocInfo=new FrmDocInfo(_patient,docSave,isDocCreate:true);
	...
	private void TreeDocuments_MouseDoubleClick(object sender,MouseEventArgs e) {
		var clickedNode=treeMain.GetNodeAt(e.Location);
		if(clickedNode==null) {
			return;
		}
		var nodeId=(NodeIdTag)clickedNode.Tag;
		...
		if(nodeId.NodeType==EnumNodeType.Mount) {
			...
			var nodeDoc=new Document();
			var fullFilePath="";//Complete path of the file
			var fileName="";//File's name only
			var extension="";//File's extension
			if(nodeId.NodeType==EnumNodeType.Doc) {
				...
				var eobAttach=EobAttaches.GetOne(nodeId.PriKey);
	...
	private void TreeDocuments_MouseDown(object sender,MouseEventArgs e) {
		...
		var treeNodeOver=treeMain.GetNodeAt(e.Location);
		if(treeNodeOver==null) {
			return;
		}
		var nodeIdTagDown=(NodeIdTag)treeNodeOver.Tag;
	...
	private void TreeDocuments_MouseUp(object sender,System.Windows.Forms.MouseEventArgs e) {
		...
		var nodeOriginal=GetTreeNode(_nodeIdTagDown);
		...
		if(_nodeIdTagDown.NodeType==EnumNodeType.Mount) {
			var mount=Mounts.GetByNum(_nodeIdTagDown.PriKey);
			var mountSourceCat=Defs.GetDef(DefCat.ImageCats,mount.DocCategory).ItemName;
			var mountDestCat=Defs.GetDef(DefCat.ImageCats,nodeOverCategoryDefNum).ItemName;
			mount.DocCategory=nodeOverCategoryDefNum;
			SecurityLogs.MakeLogEntry(EnumPermType.ImageEdit,mount.PatNum,Lan.g(this,"Mount moved from")+" "+mountSourceCat+" "
			                                                              +Lan.g(this,"to")+" "+mountDestCat);
			Mounts.Update(mount);
		}
		else if(_nodeIdTagDown.NodeType==EnumNodeType.Doc) {
			var doc=Documents.GetByNum(_nodeIdTagDown.PriKey);
			var docSourceCat=Defs.GetDef(DefCat.ImageCats,doc.DocCategory).ItemName;
			var docDestCat=Defs.GetDef(DefCat.ImageCats,nodeOverCategoryDefNum).ItemName;
			doc.DocCategory=nodeOverCategoryDefNum;
			var logText=Lan.g(this,"Document moved")+": "+doc.FileName;
			if(doc.Description!="") {
				var docDescript=doc.Description;
			...
			SecurityLogs.MakeLogEntry(EnumPermType.ImageEdit,doc.PatNum,logText,doc.DocNum,doc.DateTStamp);
	...
	private void CaptureComplete(object sender,EventArgs e) {
		if(this.InvokeRequired) {
			var c=new CaptureCallback(CaptureComplete);
			Invoke(c, sender, e);
			return;
		}
		...
		var capturedImage=_suniDeviceControl.capturedImage;
		var doc=ImageStore.ImportImageToMount(capturedImage,rotationAngle,_listMountItems[_idxSelectedInMount].MountItemNum,GetCurrentCategory(),_patient);
	...
	private void CaptureFinalize(object sender,EventArgs e) {
		if(this.InvokeRequired) {
			var c=new CaptureCallback(CaptureFinalize);
			Invoke(c, sender, e);
			return;
		}
	...
	private void menuForms_Click(object sender,System.EventArgs e) {
		var formName = ((MenuItem)sender).Text;
		Document doc = null;
		try {
	...
	private void menuMounts_Click(object sender,System.EventArgs e) {
		var idx = ((MenuItem)sender).Index;
		var listMountDefs=MountDefs.GetDeepCopy();
		//MsgBox.Show(listMountDefs[idx].Description);
		var mount=Mounts.CreateMountFromDef(listMountDefs[idx],_patient.PatNum,GetCurrentCategory());
		FillTree(false);
	...
	private void pictureBoxMain_MouseMove(object sender,System.Windows.Forms.MouseEventArgs e) {
		   ...
		   || ToolBarPaint.Buttons["Hand"].IsTogglePushed)//Hand mode.
		{
			...
			var intersect=ODMathLib.IntersectRectangles(Math.Min(e.Location.X,_pointMouseDown.X),
	...
	private void pictureBoxMain_MouseUp(object sender,System.Windows.Forms.MouseEventArgs e) {
		...
		if(ToolBarPaint.Buttons["Hand"]==null//if button is not visible, it's always hand mode.
		   || ToolBarPaint.Buttons["Hand"].IsTogglePushed) {
			...
			var zoomCrop=_zoomImage*_zoomOverall;
			var pointTrans=_pointFTranslation;
			var point1=ControlPointToRawImagePoint(_rectangleCrop.Location,_documentShowing,
				_intArrayWidthsImagesCur[_idxSelectedInMount],_intArrayHeightsImagesCur[_idxSelectedInMount],zoomCrop,pointTrans);
			var point2=ControlPointToRawImagePoint(new Point(_rectangleCrop.Location.X+_rectangleCrop.Width,
			...
			var oldCropRect=DocCropRect(_documentShowing,_intArrayWidthsImagesCur[_idxSelectedInMount],_intArrayHeightsImagesCur[_idxSelectedInMount]);
			var finalCropRect=ODMathLib.IntersectRectangles(rawCropRect.X,rawCropRect.Y,rawCropRect.Width,
			...
			var prevCropRect=DocCropRect(_documentShowing,_intArrayWidthsImagesCur[_idxSelectedInMount],_intArrayHeightsImagesCur[_idxSelectedInMount]);
			...
			if(nodeIdTag.NodeType==EnumNodeType.Doc) {
				DeleteThumbnailImage(_documentShowing);
				var newCropRect=DocCropRect(_documentShowing,_intArrayWidthsImagesCur[_idxSelectedInMount],_intArrayHeightsImagesCur[_idxSelectedInMount]);
				//Update the location of the image so that the cropped portion of the image does not move in screen space.
				var pointPrevCropCenter=new PointF(prevCropRect.X+prevCropRect.Width/2.0f,prevCropRect.Y+prevCropRect.Height/2.0f);
				var pointNewCropCenter=new PointF(newCropRect.X+newCropRect.Width/2.0f,newCropRect.Y+newCropRect.Height/2.0f);
				var arrayPoints=new[] {
					pointPrevCropCenter,
					pointNewCropCenter
				};
				var matrix=GetDocumentFlippedRotatedMatrix(_documentShowing);
	...
	private void printDocument_PrintPage(object sender,System.Drawing.Printing.PrintPageEventArgs e) {
		...
		var bitmapCloned=(Bitmap)_bitmapShowing.Clone();
		...
		var ratio=Math.Min(rectf.Width/(float)bitmapPrint.Width,rectf.Height/(float)bitmapPrint.Height);
		var g=e.Graphics;
	...
	private void MountMenuSwap_Click(object sender,EventArgs e) {
		var mountItemNum=_documentArrayInMount[_idxSelectedInMount].MountItemNum;
		...
		var idxDocsToUpdate=new bool[_documentArrayInMount.Length];
	...
	public void FillTree(bool keepSelection) {
		var nodeIdTagSelection=new NodeIdTag();
		...
		if(_claimPaymentNum!=0) {
			var listEobs=EobAttaches.Refresh(_claimPaymentNum);
			for(var i=0;i<listEobs.Count;i++) {
				var node=new TreeNode(listEobs[i].FileName);
		...
		for(var i=0;i<listDefsImageCats.Count;i++) {
		...
		var dataSet=Documents.RefreshForPatient(_patient.PatNum);
		var rows=dataSet.Tables["DocumentList"].Rows;
		for(var i=0;i<rows.Count;i++) {
			var treeNode=new TreeNode(SIn.Date(rows[i]["DateCreated"].ToString()).ToString("d")+": "+rows[i]["description"]);
			var idxParentFolder=SIn.Int(rows[i]["idxCategory"].ToString());
		...
		var treeNodeSelected=treeMain.SelectedNode;//Save the selection so we can reselect after collapsing.
		...
		for(var i=0;i<_listDefNumsExpandedCats.Count;i++) {
			for(var j=0;j<treeMain.Nodes.Count;j++) {//Enumerate the image categories.
	...
	*/
	...
	public static Matrix GetScreenMatrix(Document doc,int docOriginalImageWidth,int docOriginalImageHeight,float imageScale,PointF imageTranslation) {
		var matrixDoc=GetDocumentFlippedRotatedMatrix(doc);
		matrixDoc.Scale(imageScale,imageScale);
		var rectCrop=DocCropRect(doc,docOriginalImageWidth,docOriginalImageHeight);
		//The screen matrix of a GDI image is always relative to the upper left hand corner of the image.
		var pointPreOrigin=new PointF(-rectCrop.Width/2.0f,-rectCrop.Height/2.0f);
		var pointsMatrixScreen=new[]{
			pointPreOrigin,
	...
	public void LayoutToolBar() {
		...
		if(_claimPaymentNum==0) {
			...
			var listMountDefs=MountDefs.GetDeepCopy();
			for(var i=0;i<listMountDefs.Count;i++){
			...
			ToolBarMain.Buttons.Add(button);
			//Program links:
			ProgramL.LoadToolBar(ToolBarMain,EnumToolBar.ImagingModule);
		}
		else {//claimpayment
			ToolBarMain.Buttons.Add(new ODToolBarButton(ODToolBarButtonStyle.Separator));
			ToolBarMain.Buttons.Add(new ODToolBarButton(Lan.g(this,"Close"),-1,Lan.g(this,"Close window"),"Close"));
	...
	public void SelectTreeNode(TreeNode treeNode,string localPathCloud="") {
		...
		var isNodeOldDoc=_nodeIdTagOld.NodeType==EnumNodeType.Doc;
		var docNumOld=_nodeIdTagOld.PriKey;
		...
		if(nodeIdTag.NodeType==EnumNodeType.Category) {
			...
			_bitmapArrayRaw=ImageStore.OpenImages(listDocs.ToArray(),_patFolder);
			...
			if(_bitmapArrayRaw[0]==null) {
				if(ImageHelper.HasImageExtension(_documentShowing.FileName)) {
					var srcFileName = ODFileUtils.CombinePaths(_patFolder,_documentShowing.FileName);
			...
			SetWindowingSlider();
			var doShowPrint=pictureBoxMain.Visible;
	...
	private void ToolBarCapture_Click() {
		...
		if(ToolBarMain.Buttons["Capture"].IsTogglePushed) {
			...
			if(nodeIdTag.NodeType!=EnumNodeType.Mount) {//No mount is currently selected.
				...
				var border=Math.Max(_suniDeviceControl.SensorSize.Width,_suniDeviceControl.SensorSize.Height)/24;
				...
				if(copied) {
					...
					if(frmDocInfo.IsDialogCancel) {
						...
						if(doc.ImgType==ImageType.Photo) {
							ODEvent.Fire(ODEventType.Patient,_patient);//Possibly updated the patient picture.
						}
			...
			if(Path.GetExtension(fileName).ToLower()==".pdf") {//Selected document is PDF, we handle differently than documents that aren't pdf.
				...
				if(dlg.ShowDialog()==DialogResult.OK) {
					...
					if(((NodeIdTag)treeMain.SelectedNode.Tag).NodeType==EnumNodeType.Eob) { //This happens when printing an EOB from the Batch Ins Claim
						SecurityLogs.MakeLogEntry(EnumPermType.Printing,0,"EOB printed");
					}
					else if(description=="") {
						SecurityLogs.MakeLogEntry(EnumPermType.Printing,_patient.PatNum,"Patient image "+fileName+" printed");
					}
					else {
						SecurityLogs.MakeLogEntry(EnumPermType.Printing,_patient.PatNum,"Patient image "+description+" printed");
					}
				}
			}
		}
	...
	private void ToolBarScan_Click(string scanType) {
		...
		if(errorCode!=0) {
			...
			if(errorCode==(int)EZTwainErrorCode.EZTEC_USER_CANCEL) {//19
				...
				message=errorCode+" "+((EZTwainErrorCode)errorCode);
			}
		...
		var xdpi=EZTwain.DIB_XResolution(hdib);
		var ydpi=EZTwain.DIB_XResolution(hdib);
		var hbitmap=EZTwain.DIB_ToDibSection(hdib);
		try {
		...
		if(_claimPaymentNum!=0) {//eob
			...
			if(saved) {
				...
				SelectTreeNode(GetTreeNode(MakeIdDoc(doc.DocNum)));
				var frmDocInfo=new FrmDocInfo(_patient,_documentShowing,isDocCreate:true);
				frmDocInfo.ShowDialog();
				if(frmDocInfo.IsDialogCancel) {
					DeleteSelection(false,false,doc);
				}
	...
	private void ToolBarSign_Click() {
		...
		var frmDocSign=new FrmDocSign(_documentShowing,_patient);//Updates our local document and saves changes to db also.
		var signLeft=treeMain.Left;
		var pointLocal=new Point(panelNote.Left-7,Height);//-7 to cover panel note completely
	...
	private void ToolBarZoomIn_Click() {
		_zoomLevel++;
		var c=new PointF(pictureBoxMain.ClientRectangle.Width/2.0f,pictureBoxMain.ClientRectangle.Height/2.0f);
		var p=new PointF(c.X-_pointFTranslation.X,c.Y-_pointFTranslation.Y);
	...
	private static Bitmap ApplyDocumentSettingsToImage(Document document, Bitmap bitmapReference, ImageSettingFlags imageSettingFlags) {
		   ...
		   document.CropW > 0 && document.CropH > 0)//No clip area yet defined, so no clipping is performed.
		{
			...
			var cropDims = ODMathLib.IntersectRectangles(0, 0, bitmapReference.Width, bitmapReference.Height,//Intersect image rectangle with
		...
		var bitmapCropped = new Bitmap(rectangleCrop.Width, rectangleCrop.Height, PixelFormat.Format32bppArgb);
		var g = Graphics.FromImage(bitmapCropped);
		var rectangleCroppedDims = new Rectangle(0, 0, bitmapCropped.Width, bitmapCropped.Height);
	...
	private void LayoutAll() {
		ToolBarMain.Bounds=new Rectangle(0,0,Width,25);
		treeMain.Bounds=new Rectangle(0,ToolBarMain.Bottom+4,228,Height-(ToolBarMain.Height+6));
		if(_claimPaymentNum!=0) {//eob or amendment
			ToolBarPaint.Bounds=new Rectangle(treeMain.Right+5,ToolBarMain.Bottom,Width-(treeMain.Width+5),25);
		}
		else {//ordinary images module
			ToolBarPaint.Bounds=new Rectangle(windowingSlider.Location.X+windowingSlider.Width+4,ToolBarMain.Bottom,
				Width-windowingSlider.Width-(treeMain.Width+5),25);
		}
		var panelNoteHeight=panelNote.Visible?panelNote.Height:0;
		pictureBoxMain.Bounds=new Rectangle(treeMain.Right+5,ToolBarPaint.Bottom+4,Width-(treeMain.Width+8), Height-panelNoteHeight-(ToolBarPaint.Bottom+4));
		panelNote.Bounds=new Rectangle(pictureBoxMain.Left,Height-panelNoteHeight-1,pictureBoxMain.Width, (int)Math.Min(114,Height-pictureBoxMain.Location.Y));
		if(_odWebView2!=null && _odWebView2.Visible) {
			_odWebView2.Bounds=pictureBoxMain.Bounds;
		}
		panelUnderline.Bounds=new Rectangle(treeMain.Right+6,ToolBarPaint.Bottom-1,Width-(treeMain.Width+6),2);
		panelVertLine.Bounds=new Rectangle(treeMain.Right+5,ToolBarMain.Bottom,2,25);
	}
	...
	private void EnableToolBarsPatient(bool enable) {
		for(var i=0;i<ToolBarMain.Buttons.Count;i++) {
	...
	private void DeleteTempPdf(long docNum) {
		var doc=Documents.GetByNum(docNum,doReturnNullIfNotFound:true);//Get old document.
		if(doc!=null && Path.GetExtension(doc.FileName).ToLower()==".pdf") {//Adobe acrobat file.
			var pdfFilePath=ODFileUtils.CombinePaths(PrefC.GetTempFolderPath(),doc.DocNum+(_patient!=null ? _patient.PatNum.ToString() : "")+".pdf");
	...
	//	progressWin.ActionMain=() => bitmapApiImage=XVWeb.GetBitmap(apteryxImage);
	//	try{
	...
	private long GetCurrentCategory() {
		...
		if(treeMain.SelectedNode!=null && ((NodeIdTag)treeMain.SelectedNode.Tag).NodeType==EnumNodeType.Doc) {
			var treeNode=treeMain.SelectedNode;
			while(treeNode.Parent!=null) {//Find the corresponding root level node.
				treeNode=treeNode.Parent;
			}
	...
	private TreeNode GetTreeNode(NodeIdTag nodeIdTag,TreeNodeCollection treeNodeCollection) {
		...
		foreach(TreeNode treeNode in treeNodeCollection) {
			...
			var treeNodeChild=GetTreeNode(nodeIdTag,treeNode.Nodes);
	...
	private void FillSignature() {
		...
		if(_documentShowing.SigIsTopaz) {
			if(_documentShowing.Signature!=null && _documentShowing.Signature!="") {
				...
				var keystring=GetHashString(_documentShowing);
	...
	private void InvalidateSettings(ImageSettingFlags settings,bool reloadZoomTransCrop) {
		var mountIdxsToUpdate=new bool[this._bitmapArrayRaw.Length];
	...
	private void InvalidateSettings(ImageSettingFlags settings,bool resetZoomTrans,bool[] mountIdxsToUpdate) {
		if(this.InvokeRequired) {
			var c=new InvalidateSettingsCallback(InvalidateSettings);
			Invoke(c, settings, resetZoomTrans);
			return;
		}
	...
	private void RenderCurrentImage(Document docCopy,int originalWidth,int originalHeight,float zoom,PointF translation) {
		...
		if(pictureBoxMain.InvokeRequired) {
			var c=new RenderImageCallback(RenderCurrentImage);
			Invoke(c, docCopy, originalWidth, originalHeight, zoom, translation);
			return;
		}
		var width=pictureBoxMain.Bounds.Width;
		var height=pictureBoxMain.Bounds.Height;
		if(width<=0 || height<=0) {
			return;
		}
	...
	private void GetNextUnusedMountItem() {
		...
		var hotStart=_idxSelectedInMount;
		var d=_idxSelectedInMount;
		do {
			if(_documentArrayInMount[_idxSelectedInMount]==null) {
	...
	*/
	...
	private void DeleteSelection(bool verbose,bool securityCheck,Document docToDelete=null) {
		...
		if(nodeIdTag.NodeType==EnumNodeType.Doc) {
			...
			if(securityCheck) {
				if(!SecurityL.IsAuthorizedToDeleteImage(doc)) {
					return;
				}
			}
			var taskAttachment=TaskAttachments.GetOneByDocNum(doc.DocNum);
		...
		if(nodeIdTag.NodeType==EnumNodeType.Mount) {
			...
			if(_idxSelectedInMount>=0 && _documentArrayInMount[_idxSelectedInMount]!=null) {
				...
				_documentShowing=new Document();
				docs= [_documentArrayInMount[_idxSelectedInMount]];
				...
				for(var i=0;i<mountItems.Count;i++) {
	...
	private int GetIdxAtMountLocation(Point location) {
		var relativeLocation=new PointF(
		...
		for(var i=0;i<_listMountItems.Count;i++) {
			var itemLocation=new RectangleF(_listMountItems[i].Xpos,_listMountItems[i].Ypos,
	...
	private void Worker() {
		while(true) {
			try {
					...
					if((imageSettingFlags & ImageSettingFlags.COLORFUNCTION)!=ImageSettingFlags.NONE || 
					   (imageSettingFlags & ImageSettingFlags.CROP)!=ImageSettingFlags.NONE) {
						...
						if(_bitmapShowing!=null) {
							...
							var bitmapShowingOld=_bitmapShowing;
							_bitmapShowing=null;
				...
				else if(_enumNodeTypeForSettings==EnumNodeType.Eob) {
					if((imageSettingFlags & ImageSettingFlags.COLORFUNCTION)!=ImageSettingFlags.NONE || 
					   (imageSettingFlags & ImageSettingFlags.CROP)!=ImageSettingFlags.NONE) {
						if(_bitmapShowing!=null) {
							var oldRenderImage=_bitmapShowing;
							_bitmapShowing=null;
	...
	private static void ResetZoomTrans(int docImageWidth,int docImageHeight,Document doc,Rectangle viewport,
		...
		var imageRect=CalcImageDims(docImageWidth,docImageHeight,doc);
	...
	private static RectangleF CalcImageDims(int imageWidth,int imageHeight,Document doc) {
		var orientation=GetScreenMatrix(doc,imageWidth,imageHeight,1,new PointF(0,0));
		var corners=new[] {
	...
	private static PointF ControlPointToRawImagePoint(PointF pointScreen,Document doc,
		...
		var matrix=GetDocumentFlippedRotatedMatrix(doc);
		matrix.Scale(scaleImage,scaleImage);
		//Now we have a matrix representing the image in its current state-space.
		var docMatAxes=matrix.Elements;
		var px=pointScreen.X-pointTranslation.X;
		var py=pointScreen.Y-pointTranslation.Y;
		//The origin of our internal image axis is always relative to the center of the crop rectangle.
		var rectangleCrop=DocCropRect(doc,widthOriginalImage,widthOriginalHeight);
		var cropRectCenter=new PointF(rectangleCrop.X+rectangleCrop.Width/2.0f,
