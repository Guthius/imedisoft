...
namespace OpenDental
{
    ...
    public partial class FormOpenDental : FormODBase
    {
        ...
        private void BeginCanadianItransCarrierThread()
        {
            ...
            ODThread odThread = new ODThread((int) TimeSpan.FromHours(1).TotalMilliseconds, (o) => { ItransNCpl.TryCarrierUpdate(); });
        ...
        private void BeginComputerHeartbeatThread()
        {
            ...
            threadCompHeartbeat.AddExceptionHandler(_ => { });
        ...
        private void BeginEhrCodeListThread()
        {
            ...
            ODThread odThread = new ODThread(o =>
            {
                ...
                ODException.SwallowAnyException(() => { EhrCodes.UpdateList(); });
            });
            odThread.AddExceptionHandler(_ => { });
        ...
        private void LogOffWorker()
        {
            ...
            ODThread thread = new ODThread((o) => { ODException.SwallowAnyException(() => { LogOffNow(true); }); });
            thread.Start();
        }
        ...
        private void BeginODServiceMonitorThread()
        {
            ...
            threadOpenDentalServiceCheck.AddExceptionHandler(_ => { });
        ...
        private void BeginPodiumThread()
        {
            ...
            odThread.AddExceptionHandler((ex) => { Logger.WriteException(ex, Podium.LOG_DIRECTORY_PODIUM); });
        ...
        private void BeginRegKeyThread()
        {
            ...
            odThread.AddExceptionHandler(_ => { }); //silently fail.
        ...
        private void BeginTasksThread(List<Signalod> listSignalTasks, List<long> listEditedTaskNums)
        {
            ...
            threadTasks.AddExceptionHandler(_ => { });
        ...
        private void TimeSyncWorker(ODThread o)
        {
            ...
            if (nistOffset != double.MaxValue)
            {
                ...
                ODException.SwallowAnyException(() => { WindowsTime.SetTime(DateTime.Now.AddMilliseconds(nistOffset)); });
            }
        }
        ...
        private void BeginUpdateFormTextThread()
        {
            ...
            odThreadUpdateFormText.AddExceptionHandler(_ => { });
        ...
        private void BeginRegistrationKeyIsDisabledThread()
        {
            ...
            threadRegistrationKeyIsDisabled.AddExceptionHandler(_ => { });
