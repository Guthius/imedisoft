	...
	public partial class ControlTreat : System.Windows.Forms.UserControl{
		...
		private void menuConsent_Click(object sender,EventArgs e) {
			...
			if(listGridRows.Count()>0) {//loop through selected procedures
				List<EnumStaticTextField> listEnumStaticTextFields=SheetFiller.GetAllStaticTextFieldsForSheet(sheet);
				StaticTextFieldDependency staticTextFieldDependencies=StaticTextData.GetStaticTextDependencies(listEnumStaticTextFields);
		...
		private void TrySendTreatPlan(){
			...
			MobileAppDevice mobileAppDevice=MobileAppDevices.ShouldCreateMobileNotification(PatientCur.PatNum);
