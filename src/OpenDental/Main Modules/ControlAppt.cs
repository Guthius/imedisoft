...
namespace OpenDental {
	...
	public partial class ControlAppt:UserControl {
		...
		private void contrApptPanel_ApptMainAreaDoubleClicked(object sender,UI.ApptMainClickEventArgs e) {
			...
			if(_patient==null || frmPatientSelect.PatNumSelected!=_patient.PatNum) {//if the patient was changed
		...
		private void ContrApptPanel_ApptMainAreaRightClicked(object sender, UI.ApptMainClickEventArgs e){
			...
			if(true) {//No clear for day if clinics enabled
		...
		private void ContrApptPanel_ApptRightClicked(object sender, UI.ApptRightClickEventArgs e){
			DataRow dataRowAppointment=contrApptPanel.TableAppointments.Select().FirstOrDefault(x => SIn.Long(x["AptNum"].ToString())==contrApptPanel.SelectedAptNum);
			...
			long clinicNum=(dataRowAppointment is null) ? 0 : SIn.Long(dataRowAppointment["ClinicNum"].ToString()); 
			if(OpenDentBusiness.AutoComm.Byod.IsSetup(clinicNum,out string err)) {//Check-In Links feature is enabled.  (handles true)
				...
				menuApt.Items.Add(menuItem);
				long confirmed=(dataRowAppointment is null) ? 0 : SIn.Long(dataRowAppointment["Confirmed"].ToString()); 
			...
			menuApt.Items.Add(menuItem);
			if(!SmsPhones.IsIntegratedTextingEnabled()) { //Does not require a completed appt, only requires texting.
				menuItem.Enabled=false;
			}
			//CareCredit
		...
		private void toolBarPrint_Click() {
			if(contrApptPanel.ListOpsVisible.Count==0 || contrApptPanel.ListDayOfWeeks.IsNullOrEmpty()) {//no ops visible.
		...
		private void butClearPin_Click(object sender,EventArgs e) {
			...
			DateTime dateTime=SIn.DateTime(dataRow["AptDateTime"].ToString());
		...
		private void pinBoard_ApptMovedFromPinboard(object sender,UI.ApptFromPinboardEventArgs e) {
			...
			ApptStatus apptStatus=(ApptStatus)SIn.Int(e.DataRowAppt["AptStatus"].ToString());
			long patNum=SIn.Long(e.DataRowAppt["PatNum"].ToString());
			...
			Appointment appointment=Appointments.GetOneApt(SIn.Long(e.DataRowAppt["AptNum"].ToString()));
			if(appointment==null) {
			...
			if(appointment.AptStatus!=ApptStatus.PtNote&&appointment.AptStatus!=ApptStatus.PtNoteCompleted) {
				...
				if((assignedDent!=0&&assignedDent!=appointment.ProvNum)||(assignedHyg!=0&&assignedHyg!=appointment.ProvHyg)) {
					...
					if(frmApptProvPrompt.IsDialogOK) {//True if changing provider
		...
		private void pinBoard_PreparingToDragFromPinboard(object sender,UI.ApptDataRowEventArgs e) {
			string pattern=SIn.String(e.DataRowAppt["Pattern"].ToString());
		...
		private void menuApt_Click(object sender,System.EventArgs e) {
			...
			switch(((ToolStripMenuItem)sender).Name) {
				...
				case MenuItemNames.SendMessageToPay:
					...
					FormMessageToPayEdit formMessageToPayEdit=new FormMessageToPayEdit(patient);
		...
		private void menuTextApptsForDayView_Click(object sender,EventArgs e) {
			...
			List<long> listPatNums=new List<long>();
			for(int i=0;i<contrApptPanel.TableAppointments.Rows.Count;i++){
				long opNum=SIn.Long(contrApptPanel.TableAppointments.Rows[i]["Op"].ToString());
		...
		private void ButAdvSearch_Click(object sender, EventArgs e){
			...
			formApptSearchAdvanced.SetSearchArgs(aptNum,listProvNumsInBox,textBefore.Text,textAfter.Text,SIn.Date(dateSearch.Text));
		...
		public void InitializeOnStartup(){
			...
			if(!true) {//Clear All Blockouts for Day is too aggressive when Clinics are enabled.
		...
		public void ModuleSelectedGoToAppt(long aptNum,DateTime dateSelected){
			...
			DataRow dataRow=contrApptPanel.TableAppointments.Select().FirstOrDefault(x => SIn.Long(x["AptNum"].ToString())==aptNum);
			if(dataRow!=null) {
		...
		public void FillViews() {
			...
			List<ApptView> listApptViewsClinic=ApptViews.GetWhere(x => !(true && Clinics.ClinicNum!=x.ClinicNum));
			int offset=1;
		...
		private void RefreshPinboardImages(){
			...
			for(int i=0;i<pinBoard.ListPinBoardItems.Count;i++){
				...
				DataRow dataRow=contrApptPanel.TableAppointments.Rows.OfType<DataRow>().FirstOrDefault(x=>SIn.Long(x["AptNum"].ToString())==pinBoard.ListPinBoardItems[i].AptNum);
				if(dataRow==null){
					continue;
				}
		...
		private ApptView GetApptViewForUser() {
			...
			if(apptView==null) { //if no entry in the ComputerPref table (or "none" view in ComputerPref table)
				if(comboView.Items.GetAll<ApptView>().Any(x => x.ApptViewNum!=ApptViews.ApptViewNumNone)) { //An appointment view other than "none" to select
		...
		private void MoveAppointment(Appointment appointment,Appointment appointmentOld,List<Schedule> listSchedeulesForOp=null,bool isOpUpdate=false){
			...
			if(appointment.AptStatus!=ApptStatus.PtNote && appointment.AptStatus!=ApptStatus.PtNoteCompleted) {
				...
				if((assignedDent!=0 && assignedDent!=appointment.ProvNum) || (assignedHyg!=0 && assignedHyg!=appointment.ProvHyg)) {
					...
					if(isOpUpdate || frmApptProvPrompt.IsDialogOK) {//Short circuit logic.  If we're updating op through right click, never ask.
						if(assignedDent!=0) {//the dentist will only be changed if the spot has a dentist.
							appointment.ProvNum=assignedDent;
							provChanged=true;
						}
...
//todo:
		...
		private void SetWeeklyView(bool isWeeklyView,bool skipModuleSelection=false) {
			...
			long apptViewNum=contrApptPanel.ApptViewCur?.ApptViewNum??ApptViews.ApptViewNumNone;
			if(isWeeklyView) {
				if(_patient==null) {
					ModuleSelected(0,listOpNums:ApptViewItems.GetOpsForView(apptViewNum),listProvNums:ApptViewItems.GetProvsForView(apptViewNum));
					return;
