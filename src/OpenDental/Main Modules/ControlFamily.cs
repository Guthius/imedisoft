...
public partial class ControlFamily : UserControl {
	...
	private void gridPat_CellClick(object sender,ODGridClickEventArgs e) {
		var gridCell=gridPat.ListGridRows[e.Row].Cells[e.Col];
	...
	private void gridPat_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		...
		//Check tags and perform corresponding action for said tag type.
			...
			for(var i=0;i<formReference.ListCustReferencesSelected.Count;i++) {
			...
			using var formReferenceEntryEdit=new FormReferenceEntryEdit((CustRefEntry)gridPat.ListGridRows[e.Row].Tag);
			...
			var patFieldDef=(PatFieldDef)gridPat.ListGridRows[e.Row].Tag;
			var patField=PatFields.GetByName(patFieldDef.FieldName,_arrayPatFields);
			...
			var patField=(PatField)gridPat.ListGridRows[e.Row].Tag;
	...
	private void MenuItemPopupUnmaskDOB(object sender,EventArgs e) {
		var menuItemDOB=gridPat.ContextMenu.MenuItems.OfType<MenuItem>().FirstOrDefault(x => x.Name=="ViewDOB");
		if(menuItemDOB==null) { 
			return;//Should not happen
		}
		var menuItemSeperator=gridPat.ContextMenu.MenuItems.OfType<MenuItem>().FirstOrDefault(x => x.Text=="-");
		if(menuItemSeperator==null) { 
			return;//Should not happen
		}
		var idxRowClick=gridPat.PointToRow(_pointLastClicked.Y);
		var idxColClick=gridPat.PointToCol(_pointLastClicked.X);//Make sure the user clicked within the bounds of the grid.
	...
	private void MenuItemPopupUnmaskSSN(object sender,EventArgs e) {
		var menuItemSSN=gridPat.ContextMenu.MenuItems.OfType<MenuItem>().FirstOrDefault(x => x.Name=="ViewSS#");
		if(menuItemSSN==null) { 
			return;//Should not happen
		}
		var menuItemSeperator=gridPat.ContextMenu.MenuItems.OfType<MenuItem>().FirstOrDefault(x => x.Text=="-");
	...
	private void MenuItemUnmaskDOB_Click(object sender,EventArgs e) {
		...
		var rowClick=gridPat.PointToRow(_pointLastClicked.Y);
		gridPat.BeginUpdate();
		var row=gridPat.ListGridRows[rowClick];
		row.Cells[row.Cells.Count-1].Text=Patients.DOBFormatHelper(_patient.Birthdate,false);
		gridPat.EndUpdate();
		var logtext="Date of birth unmasked in Family Module";
	...
	private void gridRecall_DoubleClick(object sender,EventArgs e) {
		...
		using var formRecallsPat=new FormRecallsPat();
	...
	private void gridIns_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		...
		var patPlan=PatPlans.GetByPatPlanNum(_listPatPlans[e.Col-1].PatPlanNum);
		if(patPlan==null) {
		...
		var insSub=_listInsSubs.Find(x => x.InsSubNum==patPlan.InsSubNum);
		var insPlan=InsPlans.GetPlan(insSub.PlanNum,_listInsPlans);
		var insHistPref=(string)((GridOD)sender).ListGridRows[e.Row].Tag;
		if(string.IsNullOrEmpty(insHistPref)) {
			Cursor=Cursors.Default;
			using var formInsPlan=new FormInsPlan(insPlan,patPlan,insSub);
			...
			Cursor=Cursors.Default;
			using var formInsHistSetup=new FormInsHistSetup(patPlan.PatNum,insSub);
	...
	private void menuItemRemoveDiscount_Click(object sender,EventArgs e) {
		...
		DiscountPlanSubs.UpdateAssociatedDiscountPlanAmts([_discountPlanSub],true);
		...
		var logText="The discount plan "+DiscountPlans.GetPlan(_discountPlanSub.DiscountPlanNum).Description+" was dropped.";
	...
	private void menuPlansForFam_Click(object sender,EventArgs e) {
		using var formPlansForFamily=new FormPlansForFamily();
	...
	private void gridPatientClone_CellClick(object sender,ODGridClickEventArgs e) {
		...
		var patient=(Patient)gridPatientClones.ListGridRows[e.Row].Tag;
		GlobalFormOpenDental.PatientSelected(patient,false);
	...
	private void FillPatientData() {
		...
		if(PrefC.GetBool(PrefName.PatientSSNMasked)) {
			...
			var contextMenu=gridPat.ContextMenu;
		...
		var listDefsMiscColor=Defs.GetDefsForCategory(DefCat.MiscColors,true);
		for(var f=0;f<listDisplayFields.Count;f++) {
			...
			switch(displayField.InternalName) {
				...
				#region Billing Type
				case "Billing Type":
					var billingtype=Defs.GetName(DefCat.BillingTypes,_patient.BillingType);
				...
				#region Guardians
				case "Guardians":
					var listGuardians=_loadData.ListGuardians??Guardians.Refresh(_patient.PatNum);
					var str="";
					for(var g=0;g<listGuardians.Count;g++) {
				...
				case "Pat Restrictions":
					var listPatRestricts=_loadData.ListPatRestricts??PatRestrictions.GetAllForPat(_patient.PatNum);
					if(listPatRestricts.Count==0) {
						row.Cells.Add(Lan.g("TablePatient","None"));//row added outside of switch statement
					}
					for(var i=0;i<listPatRestricts.Count;i++) {
						row=new GridRow();
				...
				#region References
				case "References":
					var custREList=_loadData.ListCustRefEntries??CustRefEntries.GetEntryListForCustomer(_patient.PatNum);
					...
					for(var i=0;i<custREList.Count;i++) {
						row=new GridRow();
				...
				#region Referrals
				case "Referrals":
					var listRefs=_loadData.ListRefAttaches??RefAttaches.Refresh(_patient.PatNum);
					var listRefsFiltered= new List<RefAttach>();
					...
					//}
					for(var i=0;i<listRefs.Count;i++) {
						row=new GridRow();
						...
						var referral=Referrals.GetFromList(listRefs[i].ReferralNum);
						var refInfo=Referrals.GetNameLF(listRefs[i].ReferralNum);
						var phoneInfo=Referrals.GetPhone(listRefs[i].ReferralNum);
				...
				case "Super Head":
					...
					if(_patient.SuperFamily!=0) {
						var supHead=_loadData.SuperFamilyGuarantors.FirstOrDefault(x => x.PatNum==_patient.SuperFamily)??Patients.GetPat(_patient.SuperFamily);
	...
	private void FillFamilyData() {
		...
		var col=new GridColumn(Lan.g("TablePatient","Name"),140);
		...
		for(var i=0;i<_family.ListPats.Length;i++) {
			...
			for(var j=0;j<_listRecalls.Count;j++) {
			...
			var idx=gridFamily.ListGridRows.Count-1;
	...
	private void ToolButAdd_Click() {
		...
		var listPatientsTemp=_family.ListPats.ToList();
		listPatientsTemp.Add(patientTemp);
		_family.ListPats=listPatientsTemp.ToArray();
		using var formPatientEdit=new FormPatientEdit();
		formPatientEdit.Patient=patientTemp;
		formPatientEdit.Family=_family;
		...
		if(formPatientEdit.DialogResult==DialogResult.OK) {
			...
			var patientOld=patientTemp.Copy();
	...
	private void ToolButDelete_Click() {
		...
		var listAppointments=Appointments.GetPatientData(_patient.PatNum);
		var listClaims=Claims.Refresh(_patient.PatNum);
		var adjustmentArray=Adjustments.Refresh(_patient.PatNum);
		var paySplitArray=PaySplits.Refresh(_patient.PatNum);//
		var listClaimProcs=ClaimProcs.Refresh(_patient.PatNum);
		var listCommlogs=Commlogs.Refresh(_patient.PatNum);
		var countPayPlans=PayPlans.GetDependencyCount(_patient.PatNum);
		var listInsSubs=InsSubs.RefreshForFam(_family);
		var listInsPlans=InsPlans.RefreshForSubList(listInsSubs);
		var listMedicationPats=MedicationPats.Refresh(_patient.PatNum,false);
		_listPatPlans=PatPlans.Refresh(_patient.PatNum);
		//CovPats.Refresh(planList,PatPlanList);
		var listRefAttaches=RefAttaches.Refresh(_patient.PatNum);
		var listSheets=Sheets.GetForPatient(_patient.PatNum);
		var repeatChargeArray=RepeatCharges.Refresh(_patient.PatNum);
		var listCreditCards=CreditCards.Refresh(_patient.PatNum);
		var listPatNumClones=Patients.GetClonePatNumsAll(_patient.PatNum);
		var discountPlanNum=DiscountPlanSubs.GetDiscountPlanNumForPat(_patient.PatNum);
		var hasProcs=listProcedures.Count>0;
		var hasAppt=listAppointments.Count>0;
		var hasClaims=listClaims.Count>0;
		var hasAdj=adjustmentArray.Length>0;
		var hasPay=paySplitArray.Length>0;
		var hasClaimProcs=listClaimProcs.Count>0;
		var hasComm=listCommlogs.Count>0;
		var hasPayPlans=countPayPlans>0;
		//Has patplan or is a subscriber to an insplan.
		var hasInsPlans=_listPatPlans.Count>0 || listInsSubs.Any(x => x.Subscriber==_patient.PatNum);
		var hasMeds=listMedicationPats.Count>0;
		var isSuperFamilyHead=_patient.PatNum==_patient.SuperFamily;
		var hasRef=listRefAttaches.Count>0;
		var hasSheets=listSheets.Count>0;
		var hasRepeat=repeatChargeArray.Length>0;
		var hasCC=listCreditCards.Count>0;
		var hasPerio=PerioExams.HasPerio(_patient.PatNum);
		var hasClones=listPatNumClones.Count>1;//The list of "clones for all" will always include the current patient.
		var hasDiscount=discountPlanNum>0;
		var hasAllergies=Allergies.GetAll(_patient.PatNum,true).Any();
		var hasProblems=Diseases.GetPatientDiseases(_patient.PatNum,includeInactive:true).Any();
		if(hasProcs || hasAppt || hasClaims || hasAdj || hasPay || hasClaimProcs || hasComm || hasPayPlans || hasInsPlans
		   || hasRef || hasMeds || isSuperFamilyHead || hasSheets || hasRepeat || hasCC || hasPerio || hasClones || hasDiscount || hasAllergies || hasProblems) 
		{
			var message=Lan.g(this,"You cannot delete this patient without first deleting the following data:")+"\r";
			if(hasProcs) {
		...
		if(_patient.PatNum==_patient.Guarantor) {//if selecting guarantor
			if(_family.ListPats.Length==1) {
				...
				for(var i=0;i<_listRecalls.Count;i++) {
	...
	private void ToolButMove_Click() {
		...
		if(_patient.PatNum==_patient.Guarantor) {//if guarantor selected
			...
			if(_family.ListPats.Length==1) {//and no other family members
				...
				for(var i=0;i<_family.ListPats.Count();i++) {
					if(_family.ListPats[i].PatNum==_patient.PatNum) {
						continue;
					}
					var listPatientLinks=PatientLinks.GetLinks(_family.ListPats[i].PatNum,PatientLinkType.Merge);//If there is another family member, make sure it is merged.  
		...
		switch(ODMessageBox.Show(Lan.g(this,"Create new family instead of moving to an existing family?"),"",MessageBoxButtons.YesNoCancel)) {
			...
			case DialogResult.No://move to an existing family
				...
				var patientInNewFam=Patients.GetPat(frmPatientSelect.PatNumSelected);
	...
	private void FillGridRecall() {
		...
		var listDisplayFieldsRecall=DisplayFields.GetForCategory(DisplayFieldCategory.FamilyRecallGrid);
		/*int width=0;
		...
		var widthAvail=Width-gridRecall.Left;
		gridRecall.Width=widthAvail;
		gridRecall.Columns.Clear();
		GridColumn col;
		for(var i=0;i<listDisplayFieldsRecall.Count;i++) {
		...
		var listRecallsPat=new List<Recall>();
		for(var i=0;i<_listRecalls.Count;i++) {
			if(_listRecalls[i].PatNum==_patient.PatNum) {
		...
		GridRow row;
		GridCell cell;
		for(var i=0;i<listRecallsPat.Count;i++) {
			row=new GridRow();
			for(var j=0;j<listDisplayFieldsRecall.Count;j++) {
				switch (listDisplayFieldsRecall[j].InternalName) {
					case "Type":
						var cellStr=RecallTypes.GetDescription(listRecallsPat[i].RecallTypeNum);
						...
						if(listRecallsPat[i].IsDisabled) {
							...
							if(listRecallsPat[i].DatePrevious.Year>1800) {
								...
								if(listRecallsPat[i].RecallInterval!=new Interval(0,0,0,0)) {
									var dateDue=listRecallsPat[i].DatePrevious+listRecallsPat[i].RecallInterval;
	...
	private void AddSuperGuarPriInsToFam(long guarNum) {
		var patientSuperFamGuar=Patients.GetPat(_patient.SuperFamily);
		if(patientSuperFamGuar==null) {//should never happen, but just in case
			return;
		}
		var listInsSubsSuper=InsSubs.GetListForSubscriber(patientSuperFamGuar.PatNum);
		if(listInsSubsSuper.Count==0) {//super family guar is not the subscriber for any insplans
			return;
		}
		var listPatPlansSuper=PatPlans.Refresh(patientSuperFamGuar.PatNum);
		if(listPatPlansSuper.Count==0) {//super family guar doesn't have an active insplan
			return;
		}
		var listInsPlansSuper=InsPlans.RefreshForSubList(listInsSubsSuper);
		var insSub=InsSubs.GetSub(
		...
		var family=Patients.GetFamily(guarNum);
		var listInsSubsForFam=InsSubs.RefreshForFam(family);
		var listInsPlansForFam=InsPlans.RefreshForSubList(listInsSubsForFam);
		var hasPatPlanAdded=false;
		for(var i=0;i<family.ListPats.Count();i++) {//possibly filter by PatStatus, i.e. .Where(x => x.PatStatus==PatientStatus.Patient)
			...
			if(string.IsNullOrWhiteSpace(insSub1.SubscriberID)) {
				...
				using var formInsPlan=new FormInsPlan(InsPlans.GetPlan(insSub1.PlanNum,listInsPlansForFam),patPlanNew,insSub1);
				...
				var listClaimProcs=ClaimProcs.Refresh(family.ListPats[i].PatNum);
				var listProcedures=Procedures.Refresh(family.ListPats[i].PatNum);
				var listBenefits=Benefits.Refresh(listPatPlansForPat,listInsSubsForFam);
			...
			if(family.ListPats[i].HasIns!="I") {
				var patientOld=family.ListPats[i].Copy();
	...
	private void FillGridSuperFam() {
		...
		for(var i=0;i<listDisplayFields.Count;i++) {
			var displayName=listDisplayFields[i].DescriptionOverride;
			...
			var gridColumn=new GridColumn(Lan.g("gridSuperFam",displayName),listDisplayFields[i].ColumnWidth);
		...
		gridSuperFam.Width=Math.Max(splitContainerSuperClones.Width,listDisplayFields.Sum(x=>x.ColumnWidth)+SystemInformation.VerticalScrollBarWidth);
			...
			for(var j=0;j<listDisplayFields.Count;j++) {
				switch(listDisplayFields[j].InternalName) {
					...
					case "Stmt":
						...
						break;
					case "": //Patfields
						var celltext=GetCellTextSuperFamPatField(patientGuar,listDisplayFields[j].Description);
		...
		gridSuperFam.EndUpdate();
		for(var i=0;i<gridSuperFam.ListGridRows.Count;i++) {
	...
	private string GetCellTextSuperFamPatField(Patient patientGuar, string description) {
		var patField=_listPatFieldsForSuperFam.Find(x=>x.PatNum==patientGuar.PatNum && x.FieldName==description);//Will frequently be null because many patients will have no PatField yet
		var celltext=PatFields.GetAbbrOrValue(patField,description);//GetAbbrOrValue returns "" if patField is null.
		if(patField==null) {//If patField is null here we aren't going to get any more info, so return.
			return celltext;
		}
		var patFieldDef=PatFieldDefs.GetFieldDefByFieldName(patField.FieldName);
		switch(patFieldDef.FieldType) {
			case PatFieldType.Checkbox:
				celltext=patField.FieldValue=="1" ? "X" : "";
				break;
			case PatFieldType.Currency:
				if(decimal.TryParse(celltext,out var result)) {
	...
	private void ToolButAddSuper_Click() {
		...
		var patientSelected=Patients.GetPat(frmPatientSelect.PatNumSelected);
		var superFamilyGuarantorNumPrevious=patientSelected.SuperFamily;
		if(patientSelected.SuperFamily==_patient.SuperFamily) {
			MsgBox.Show(this,"That patient is already part of this superfamily.");
			return;
		}
		var listPatientsSuperFam=new List<Patient>();
		...
		var diagResult=DialogResult.None;
		if(listPatientsSuperFam.Any(x => x.Guarantor!=x.SuperFamily)) {//super family consists of more than one family
			...
			var msgTxt=Lans.g("You are about to move the head of another super family.  Would you like to move all members of that super family "
			                  +"to this super family?")+"\r\n\r\n"
			                                           +Lans.g("Yes - All members of the selected super family will be moved to this super family.")+"\r\n\r\n"
			                                           +Lans.g("No - The selected patient's current super family will be disbanded and only the selected patient's family will be added to "
			                                                   +"this super family.")+"\r\n\r\n"
			                                           +Lans.g("Cancel - Do nothing.");
		...
		if(diagResult==DialogResult.No) {
			var listPatientNumsInSuperFam=Patients.GetAllFamilyPatNumsForSuperFam([superFamilyGuarantorNumPrevious]);
			for(var i=0;i<listPatientNumsInSuperFam.Count;i++) { //Create SecurityLog for each member of the superfamily being disbanded.
	...
	private void ToolButDisbandSuper_Click() {
		...
		var listPatientNumsInSuperFam=Patients.GetAllFamilyPatNumsForSuperFam([_patient.SuperFamily]);
		for(var i=0;i<listPatientNumsInSuperFam.Count;i++) {
			SecurityLogs.MakeLogEntry(
				EnumPermType.PatientEdit,
				listPatientNumsInSuperFam[i],
	...
	private void ToolButRemoveSuper_Click() {
		var superFamilyGuarantorNumPrevious=_patient.SuperFamily;
		...
		for(var i=0;i<_family.ListPats.Length;i++) {//remove whole family
			var patientTemp=_family.ListPats[i].Copy();
	...
	private void FillInsData() {
		if(_patient!=null && _discountPlanSub!=null) {
			...
			var defAdjType=Defs.GetDef(DefCat.AdjTypes,discountPlan.DefNum);
			var rowDiscount=new GridRow();
			...
			var discountPlanAnnualMax=discountPlan.AnnualMax.ToString();
		...
		var listInsSubs=new List<InsSub>();//prevents repeated calls to db.
		...
		for(var i=0;i<listInsPlans.Count;i++) {
			switch(listInsPlans[i].PlanType){
				...
				case "":
					...
					break;
				case "p":
					var feeSchedCopay=FeeScheds.GetFirstOrDefault(x => x.FeeSchedNum==listInsPlans[i].CopayFeeSched);
		...
		for(var i=0;i<listInsPlans.Count;i++) {
			...
			var dateService=new DateTime(2000,listInsPlans[i].MonthRenew,1);
			row.Cells.Add(Lan.g("TableCoverage","Service year begins:")+" "+dateService.ToString("MMMM"));
		}
		gridIns.ListGridRows.Add(row);
		//Benefits-----------------------------------------------------------------------------------------------------
		var benefitMatrix=Benefits.GetDisplayMatrix(_loadData.ListBenefits,_listPatPlans,_listInsSubs);
		string strDesc;
		string strVal;
		ProcedureCode procedureCode=null;
		for(var y=0;y<benefitMatrix.GetLength(1);y++) {//rows
			var isAgeLimitation=false;
			row=new GridRow();
			strDesc="";
			//some of the columns might be null, but at least one will not be.  Find it.
			for(var x=0;x<benefitMatrix.GetLength(0);x++) {//columns
				...
				if(benefitMatrix[x,y].BenefitType==InsBenefitType.CoInsurance && benefitMatrix[x,y].Percent!=-1) {
					if(benefitMatrix[x,y].CodeNum>0) {
						...
						strDesc+=CodeGroups.GetGroupName(benefitMatrix[x,y].CodeGroupNum)+" % ";
					}
					else {
				        ...
				        && benefitMatrix[x,y].TimePeriod==BenefitTimePeriod.Lifetime)
				{
					...
					var desc = CodeGroups.GetGroupName(benefitMatrix[x,y].CodeGroupNum) + " age limit";
					strDesc+=Lan.g(this,desc)+" ";
					isAgeLimitation=true;
				}
				else if(Benefits.IsFrequencyLimitation(benefitMatrix[x,y])) {
			...
			for(var x=0;x<benefitMatrix.GetLength(0);x++) {//columns
				...
				if(benefitMatrix[x,y].Percent != -1) {
					strVal+=benefitMatrix[x,y].Percent+"% ";
				}
				...
				if(benefitMatrix[x,y].QuantityQualifier==BenefitQuantity.NumberOfServices) {//eg 2 times per CalendarYear
					if(benefitMatrix[x,y].TimePeriod==BenefitTimePeriod.NumberInLast12Months) {
						strVal+=benefitMatrix[x,y].Quantity+" "+Lan.g(this,"times in the last 12 months")+" ";
					}
					else {
						strVal+=benefitMatrix[x,y].Quantity+" "+Lan.g(this,"times per")+" "
					...
					if(benefitMatrix[x,y].Quantity!=0) {
						strVal+=benefitMatrix[x,y].Quantity+" ";
					}
					if(isAgeLimitation) {
				...
				{
					strVal=CovCats.GetDesc(benefitMatrix[x,y].CovCatNum)+" "+Lan.g(this,"Wait ")+benefitMatrix[x,y].Quantity;
		...
		gridIns.ListGridRows.Add(row);
		//InsHist
		var listProceduresEoAndC=Procedures.GetProcsByStatusForPat(_patient.PatNum, ProcStat.EO, ProcStat.C); //all for the patient, all plans
		var listProcNums=listProceduresEoAndC.Select(x=>x.ProcNum).ToList();
		var listClaimProcsForEoAndCProcs=ClaimProcs.GetForProcs(listProcNums);//all for the patient, all plans
		var listPrefNames=Prefs.GetInsHistPrefNames();
		for(var i=0;i<listPrefNames.Count();i++) {
			row=new GridRow();
			row.Cells.Add(Lan.g("TableCoverage",listPrefNames[i].GetDescription()));
			for(var j=0;j<_listPatPlans.Count();j++) {
				var listClaimProcsForPlan=listClaimProcsForEoAndCProcs.FindAll(x=>x.InsSubNum==_listPatPlans[j].InsSubNum && x.Status.In(ClaimProcStatus.InsHist,ClaimProcStatus.Received));
				var listProceduresForPlan=listProceduresEoAndC.FindAll(x=>listClaimProcsForPlan.Any(y=>y.ProcNum==x.ProcNum));
				var procedure=Procedures.GetMostRecentInsHistProc(listProceduresForPlan,ProcedureCodes.GetCodeNumsForInsHistPref(listPrefNames[i]),listPrefNames[i]);
				if(procedure==null) {
	...
	private void ToolButIns_Click() {
		...
		var result=ODMessageBox.Show(Lan.g(this,"Is this patient the subscriber?"),"",MessageBoxButtons.YesNoCancel);
		...
		if(result==DialogResult.Yes) {//current patient is subscriber
			...
			using var formSubsciberSelect=new FormSubscriberSelect(_family);
		...
		InsSub insSub=null;
		var isNewPlan=false;
		var listInsSubs=InsSubs.GetListForSubscriber(patientSubscriber.PatNum);
		if(listInsSubs.Count==0) {
			isNewPlan=true;
		}
		else {
			var frmInsSelectSubscr=new FrmInsSelectSubscr(patientSubscriber.PatNum,_patient.PatNum);
			...
			if(frmInsSelectSubscr.SelectedInsSubNum==0) {//'New' option selected.
				...
				insPlan=InsPlans.GetPlan(insSub.PlanNum, []);
			}
		}
		...
		if(isNewPlan) {
			...
			Benefit benefit;
			var listCovCats=CovCats.GetWhere(x => x.DefaultPercent!=-1,true);
	...
	private void FillGridPatientClones() {
		...
		gridPatientClones.Width=gridPatientClones.Columns.Sum(x=>x.ColWidth)+SystemInformation.VerticalScrollBarWidth;
		...
		var selectedIndex=-1;
		GridRow row;
		var listDefs=Defs.GetDefsForCategory(DefCat.ClinicSpecialty);
		for(var i=0;i<_loadData.ListPatientsClones.Count;i++) {
			...
			var defLink=_loadData.ListDefLinksSpecialty.Find(x=>x.FKey==_loadData.ListPatientsClones[i].PatNum);
			//Check for null because an office could have just turned on clinics and a specialty would not have been required prior.
			if(defLink==null) {
				row.Cells.Add("");
			}
			else {
				var defSpecialty=Defs.GetDef(DefCat.ClinicSpecialty,defLink.DefNum,listDefs);//DefNum will be valid
	...
	private void ToolButAddClone_Click() {
		...
		if(PatientLinks.IsPatientAClone(_patient.PatNum)) {
			var patNumMaster=PatientLinks.GetOriginalPatNumFromClone(_patient.PatNum);
			var patientMaster=Patients.GetPat(patNumMaster);
	...
	private void ToolButSynchClone_Click() {
		...
		var strDataUpdated=Patients.SynchClonesWithPatient(_patient,_family,_listInsPlans,_listInsSubs,_listBenefits,_listPatPlans);
		ModuleSelected(_patient.PatNum);
	...
	private void FillPatientPicture() {
		...
		if(_patient==null || 
		   false) {//Do not use patient image when A to Z folders are disabled.
			return;
		}
		pictureBoxPat.Image?.Dispose();
		var patFolder="";
		try{
			patFolder=ImageStore.GetPatientFolder(_patient,ImageStore.GetDataFolder());
		}
		catch{ }
		if(patFolder==""){
			return;
		...
		if(_loadData.HasPatPict==YN.Unknown) {
			...
			pictureBoxPat.Image=Documents.GetPatPict(patFolder,_loadData.PatPict);
		}
	}
	...
	private bool MovePats(Patient patientOld,Family family=null) {
		...
		if(family!=null) {//Move all family members marked as merged silently (The family should only contain guarantor and any merged pats at this point)
			for(var i=0;i<family.ListPats.Count();i++) {
				...
				var patientOldFam=family.ListPats[i].Copy();
	...
	private void RefreshModuleData(long patNum) {
		...
		if(_patient!=null && _discountPlanSub!=null && _loadData.ListPatPlans.Count>0) {
			...
			var logText=Lan.g(this,"The discount plan")+" "+DiscountPlans.GetPlan(_discountPlanSub.DiscountPlanNum).Description+" "+Lan.g(this,"was automatically dropped due to patient having an insuarance plan.");
			SecurityLogs.MakeLogEntry(EnumPermType.DiscountPlanAddDrop,_patient.PatNum,logText);
			var messageText="Discount plan removed due to patient having both an insurance plan and a discount plan. "
	...
	private void UpdateToolbarButtons() {
		if(_patient!=null) {//if there is a patient
			...
			var showSuperfamilyGrid=PrefC.GetBool(PrefName.ShowFeatureSuperfamilies) && _patient.SuperFamily!=0;
			var showPatientCloneGrid=PrefC.GetBool(PrefName.ShowFeaturePatientClone) && _loadData.ListPatientsClones!=null && _loadData.ListPatientsClones.Count > 1;
			splitContainerSuperClones.Visible=showSuperfamilyGrid || showPatientCloneGrid;
			if(splitContainerSuperClones.Visible) {
				var widthSuperClonePanel=GetWidthSuperClonesPanel(showSuperfamilyGrid,showPatientCloneGrid);
				splitContainerSuperClones.Width=widthSuperClonePanel;
				gridIns.Location=new Point(splitContainerSuperClones.Right+2,gridIns.Top);
			}
			else {
				gridIns.Location=splitContainerSuperClones.Location;
			}
			gridIns.Width=Width-gridIns.Left;
	...
	private int GetWidthSuperClonesPanel(bool showPanelForSuperfamilies,bool showPanelForPatientClone) {
		...
		if(showPanelForSuperfamilies) {
			var widthDisplayFields=DisplayFields.GetForCategory(DisplayFieldCategory.SuperFamilyGridCols).Sum(x => x.ColumnWidth);
			widthSuperFam=widthDisplayFields+SystemInformation.VerticalScrollBarWidth;
		}
		if(showPanelForPatientClone) {
			var widthColumns=gridPatientClones.Columns.Sum(x => x.ColWidth);
			widthClones=widthColumns+SystemInformation.VerticalScrollBarWidth;
		}
	...
	private void UpdateConflictingOrdinals(PatPlan patPlan) {
		...
		var listPatPlansConflictingOrdinal=_listPatPlans.Where(x => x.Ordinal==patPlan.Ordinal && x.PatPlanNum!=patPlan.PatPlanNum).ToList();
		for(var i=0;i<listPatPlansConflictingOrdinal.Count();i++) {
