...
public partial class FormRpDPPOvercharged: FormODBase {
	...
	private void FillGrid() {
		#region Column Sizes
		var colWidthStartDate=65;
		var colWidthNames=125;
		var colWidthPovAndClin=90;
		var colAmtsAndDescript=70;
		var colWidthWideAmts=80;
		var colWidthOverriden=64;
		#endregion
		...
		if(gridMain.Columns.Count==0) {
			...
			if(true) {//Only show when clinics are on
		...
		for(var i=0;i<_reportCur?.ReportObjects.Count;i++) {
			if(_reportCur.ReportObjects[i].ObjectType!=ReportObjectType.QueryObject) {
				continue;
			}
			var queryObj=(QueryObject)_reportCur.ReportObjects[i];
	...
	private void RefreshReport() {
		var hasValidationPassed=ValidateDateFields();
		var tableOverchargedDPP=new DataTable();
		...
		var subtitleClinics="";
		if(true){//Do not show if clinics are not on
		...
		_reportCur=new ReportComplex(true,true);
		...
		#region Col Sizes
		var colWidthStartDate=71;
		var colWidthNames=115;
		var colWidthProvClinDescript=70;
		var colWidthOverriden=41;
		var colWidthAmt=70;
		var colWidthWideAmt=78;
		var fontBold=new Font("Tahoma",9,FontStyle.Bold);
		#endregion
		var query=_reportCur.AddQuery(tableOverchargedDPP,DateTime.Today.ToShortDateString());
	...
	private void butFix_Click(object sender,EventArgs e) {
		...
		var listSelectedGridRows=gridMain.SelectedGridRows;
		//If no rows are selected, select them all
		if(listSelectedGridRows.Count==0) {
			listSelectedGridRows=gridMain.ListGridRows;
		}
		var listChargesToInsert=new List<PayPlanCharge>();
		var listPatNums=listSelectedGridRows.Select(x => (DataRow)x.Tag)
			.Select(x => SIn.Long(x["PatNum"].ToString()))
			.ToList();
		var longArrPayPlanNums=listSelectedGridRows.Select(x => (DataRow)x.Tag)
			.Select(y => SIn.Long(y["PayPlanNum"].ToString()))
			.ToArray();
		var listFamilies=Patients.GetFamilies(listPatNums);
		var listPayPlans=PayPlans.GetMany(longArrPayPlanNums);
		for(var i = 0;i<listSelectedGridRows.Count;i++) {
			var dataRow=(DataRow)listSelectedGridRows[i].Tag;
			var payPlan=listPayPlans.ToList().Find(x => x.PayPlanNum==SIn.Long(dataRow["PayPlanNum"].ToString()));
			var family=listFamilies.Find(x => x.ListPats.Select(x => x.PatNum).Contains(SIn.Long(dataRow["PatNum"].ToString())));
			if(family==null || payPlan==null) {
				continue;
			}
			//Create Negative PayPlan Charge that counters the debit
			var payPlanChargeOffset=PayPlanEdit.CreateDebitChargeDynamic(payPlan,
				family,
	...
	private void butPrint_Click(object sender,EventArgs e) {
		using var FormR=new FormReportComplex(_reportCur);
		FormR.ShowDialog();
	}
