...
public partial class FormRpBirthday : FormODBase {
	...
	private void butLeft_Click(object sender, System.EventArgs e) {
		...
		var dateFrom=SIn.Date(validDateFrom.Text);
		if(dateFrom.Year < 1880) {
			MsgBox.Show(this,"Please fix the From date first.");
			return;
		}
		var dateTo=SIn.Date(validDateTo.Text);
		var toLastDay=false;
	...
	private void butPostcards_Click(object sender,EventArgs e) {
		...
		var totalPages=(int)Math.Ceiling((double)BirthdayTable.Rows.Count/(double)PrefC.GetLong(PrefName.RecallPostcardsPerSheet));
	...
	private void pdCards_PrintPage(object sender,PrintPageEventArgs ev) {
		...
		while(yPos<ev.PageBounds.Height-100 && patientsPrinted<BirthdayTable.Rows.Count) {
			...
			if(PrefC.GetBool(PrefName.RecallCardsShowReturnAdd)) {
				...
				var phone=PrefC.GetString(PrefName.PracticePhone);
				if(phone.Length==10) {
			...
			//Patient's Address-----------------------------------------------------------------------
			str=BirthdayTable.Rows[patientsPrinted]["FName"]+" "
			                                                //+BirthdayTable.Rows[patientsPrinted]["MiddleI"].ToString()+" "
			                                                +BirthdayTable.Rows[patientsPrinted]["LName"]+"\r\n"
			                                                +BirthdayTable.Rows[patientsPrinted]["Address"]+"\r\n";
			if(BirthdayTable.Rows[patientsPrinted]["Address2"].ToString()!="") {
				str+=BirthdayTable.Rows[patientsPrinted]["Address2"]+"\r\n";
			}
			str+=BirthdayTable.Rows[patientsPrinted]["City"]+", "
			                                                +BirthdayTable.Rows[patientsPrinted]["State"]+"   "
			                                                +BirthdayTable.Rows[patientsPrinted]["Zip"]+"\r\n";
	...
	private void butReport_Click(object sender, System.EventArgs e){
		...
		var query=report.AddQuery(RpBirthday.GetBirthdayTable(dateFrom,dateTo),"","",SplitByKind.None,1,true);
