...
namespace OpenDental{
	...
	public partial class FormRpPaySheet : FormODBase{
		...
		private void AddMerchantFeeColumn(DataTable table) {
			...
			DataColumn col=table.Columns.Add("Merchant Fee");
		...
		private void butOK_Click(object sender,System.EventArgs e) {
			...
			if(tablePat.Rows.Count!=0) {
				...
				if(checkShowMerchantFees.Checked) {
					queryObject.AddGroupSummaryField("Total Merchant Fees:","Amount","Merchant Fee",SummaryOperation.Sum,new List<int>(summaryGroupsPatientPayments),Color.Black,fontBold,0,4);
			...
			if(checkShowOnlinePatientPaymentsSeparately.Checked && tableOnlinePat.Rows.Count!=0) {
				...
				if(checkShowMerchantFees.Checked) {
					queryObject.AddGroupSummaryField("Total Merchant Fees:","Amount","Merchant Fee",SummaryOperation.Sum,new List<int>(summaryGroupsOnlinePatientPayments),Color.Black,fontBold,0,4);
			...
			if(queryObject!=null) {	//If we have any query objects to report, summarize the totals of each of their amt columns combined to include at the end of the report.
				...
				if(checkShowMerchantFees.Checked) {
					queryObject.AddGroupSummaryField("Total All Merchant Fees:","Amount","Merchant Fee",SummaryOperation.Sum,new List<int>(summaryGroupsAllPatientPayments),Color.Black,fontBold,0,4);
