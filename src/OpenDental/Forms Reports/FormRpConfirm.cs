...
public partial class FormRpConfirm : FormODBase{
	...
	private void butOK_Click(object sender, System.EventArgs e) {
		var fieldsSelected=new List<string>(); 
		...
		for(var i = 0;i<listSelect2.SelectedIndices.Count;i++) {
			fieldsSelected.Add(listSelect2.Items.GetTextShowingAt(listSelect2.SelectedIndices[i]));
		}
		var command="SELECT ";
		for(var i=0;i<fieldsSelected.Count;i++){
			if(i>0){
				command+=",";
			}
		...
		for(var i=0;i<AptNums.Length;i++){
			if(i>0){
				command+=" OR";
			}
		...
		using var FormQ=new FormQuery(report);
