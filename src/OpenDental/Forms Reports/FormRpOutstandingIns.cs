...
public partial class FormRpOutstandingIns:FormODBase {
	...
	private List<Userod> _listClaimSentEditUsers= [];
	private List<ClaimTracking> _listNewClaimTrackings= [];
	private List<ClaimTracking> _listOldClaimTrackings= [];
	...
	private void FormRpOutIns_Load(object sender,EventArgs e) {
		...
		if(PrefC.GetInt(PrefName.OutstandingInsReportDateFilterTab)==(int)RpOutstandingIns.DateFilterTab.DaysOld) {
			tabControlDate.SelectedTab=tabDaysOld;
		}
		else {
			tabControlDate.SelectedTab=tabDateRange;
		}
		FillProvs();
		FillDateFilterBy();
		...
		var listSubUserMenu=new List<MenuItem>();
		...
		var menuItemCollection=new Menu.MenuItemCollection(rightClickMenu);
		menuItemCollection.AddRange(listMenuItems.ToArray());
		rightClickMenu.Popup+=new EventHandler((_,_) => {
			...
			if(true) {
				var clinicNum=gridMain.SelectedTags<RpOutstandingIns.OutstandingInsClaim>()[0].ClinicNum;
				//If the user is clinic restricted, only enable 'Got to Account' when the clinicNum matches
				rightClickMenu.MenuItems[0].Enabled= Clinics.GetForUserod(Security.CurUser, true, comboClinics.HqDescription).Select(x => x.Id).Contains(clinicNum);
			}
		});
		FillCustomTrack();
		FillPreauth();
	...
	private void FillUsers() {
		...
		comboUserAssigned.Items.Add(Lans.g("Unassigned"),new Userod {UserNum=0});
	...
	private void FillGrid(bool isOnLoad = false) {
		...
		var listDisplayFields = DisplayFields.GetForCategory(DisplayFieldCategory.OutstandingInsReport);
		...
		var dateTimeFrom=GetDateFrom();
		var dateTimeTo=GetDateTo();
		var carrier=textCarrier.Text;
		var comboFilterBy=comboDateFilterBy.GetSelected<RpOutstandingIns.DateFilterBy>();
		var isIgnoreCustomChecked=checkIgnoreCustom.Checked;
		var listClinicNums=comboClinics.ListClinicNumsSelected;//Includes all clinics in combobox if 'All' is selected.
		var listSelectedUserNums=new List<long>();
		...
		,bool checkIgnoreCustomChecked)
	{
		...
		string type;
		total=0;
		var listErrorDefs=Defs.GetDefsForCategory(DefCat.ClaimErrorCode,true);
		foreach(var claimCur in listOustandingInsClaims) {
			...
			foreach(var fieldCur in listDisplayFields) {
				switch(fieldCur.InternalName) {
					...
					case "User":
						var username="";
						var userod=Userods.GetUser(claimCur.UserNum);
						if(userod!=null) {
						...
						break;
					case "PatName":
						var patName=claimCur.PatLName+", "+claimCur.PatFName+" "+claimCur.PatMiddleI;
	...
	private void gridMain_RightClickHelper(object sender,EventArgs e) {
		...
		var menuCode=(int)((MenuItem)sender).Tag;
		switch(menuCode) {
			case 0://Go to Account
				GlobalFormOpenDental.GoToModule(EnumModuleType.Account,patNum:((RpOutstandingIns.OutstandingInsClaim)gridMain.ListGridRows[index].Tag).PatNum);
				break;
			case 1://Assign to Me
	...
	private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		...
		var claim=Claims.GetClaim(((RpOutstandingIns.OutstandingInsClaim)gridMain.ListGridRows[e.Row].Tag).ClaimNum);
		if(claim==null) {
		...
		using var FormCE=new FormClaimEdit(claim,pat,fam);
		FormCE.IsNew=false;
		FormCE.ShowDialog();
	}

	private void buttonUpdateCustomTrack_Click(object sender,EventArgs e) {			
		var listClaimNum=new List<long>();
		...
		var listClaims=Claims.GetClaimsFromClaimNums(listClaimNum);
		using var FormCT=new FormClaimCustomTrackingUpdate(listClaims);
	...
	private void AssignUserHelper(long assignUserNum) {
		...
		var listTrackingNumsAndClaimNums=new List<Tuple<long,long>>();
		foreach(var index in gridMain.SelectedIndices) {
			var outstandingInsClaim = (RpOutstandingIns.OutstandingInsClaim)gridMain.ListGridRows[index].Tag;
			var claimTrackingNum=_listNewClaimTrackings.FirstOrDefault(x => x.ClaimNum==outstandingInsClaim.ClaimNum)?.ClaimTrackingNum??0;
			var claimNum = outstandingInsClaim?.ClaimNum??0;
	...
	private long PickUser(bool isAssigning) {
		...
		frmUserPick.ListUserodsFiltered=_listClaimSentEditUsers;
		if(!isAssigning) {
		...
		return frmUserPick.UserNumSelected;
	}

	/// <summary>Sets date controls in both date tabs</summary>
	private void SetDates(DateTime dateFrom,DateTime dateTo) {
		var daysOldMin=SOut.Int((int)Math.Round((DateTime.Today-dateTo.Date).TotalDays,0));//Calculate min days old from dateTo.
		var daysOldMax="";
		if(dateFrom>dateTo) {
	...
	private DateTime GetDateTo(UI.TabPage tabPage) {
		var dateMin=DateTime.Today;
		if(tabPage==tabDaysOld) {
			var daysOldMin=0;
	...
	///<summary>Gets date control for report from a specific tab in tabControlDate</summary>
	private DateTime GetDateFrom(UI.TabPage tabPage) {
	...
	private void tabControlDate_SelectedIndexChanged(object sender,EventArgs e) {
		DateTime dateFrom;
		DateTime dateTo;
		if(tabControlDate.SelectedTab==tabDaysOld){
			//gets date from previous tab that we are leaving
			dateFrom=GetDateFrom(tabDateRange);
			dateTo=GetDateTo(tabDateRange);
		}
		else{//tabDateRange
			dateFrom=GetDateFrom(tabDaysOld);
			dateTo=GetDateTo(tabDaysOld);
		}
	...
	private void RefreshSelectedInfo() {
		var listSelected=gridMain.SelectedGridRows.Select(x => x.Tag)
		...
		if(listSelected.Count == 0) {
			...
			var insClaim = listSelected.First();
			//fill textboxes
			...
			//fill textboxes
			var multiSelectedStr = "<"+Lans.g("Multiple Selected")+">";
	...
	private void butExport_Click(object sender,System.EventArgs e) {			
		var fileName=Lan.g(this,"Outstanding Insurance Claims");
	...
	private void butZeroClaims_Click(object sender,EventArgs e) {
		...
		var listClaimNums=gridMain.SelectedTags<RpOutstandingIns.OutstandingInsClaim>().Select(x => x.ClaimNum).ToList();
		var listClaims=Claims.GetClaimsFromClaimNums(listClaimNums);
