...
public partial class FormRpUnearnedIncome:FormODBase {
	...
	private void FillProviders() {
		...
		foreach(var provCur in listProvs) {
	...
	private void FillUnearnedTypes() {
		var listDefs = Defs.GetDefsForCategory(DefCat.PaySplitUnearnedType,true);
		foreach(var defCur in listDefs) {
	...
	private void FillDates() {
		var dateThisFirst = new DateTime(DateTime.Today.Year,DateTime.Today.Month,1);
	...
	private void butUnearnedAllocationOK_Click(object sender,EventArgs e) {
		...
		var listClinicNums = listUnearnedAllocationClins.GetListSelected<ClinicDto>().Select(x => x.Id).ToList();
		...
		var query=report.AddQuery(table,"Unearned Allocation Report");
		...
		if(checkUnearnedAllocationAllTypes.Checked) {
			...
			var unearnedTypes = string.Join(", ",listUnearnedAllocationTypes.GetListSelected<Def>().Select(x => x.ItemName));
		...
		if(true) {//show sub titles if clinics are enabled. 
			if(checkUnearnedAllocationAllClins.Checked) {
				...
				var clinNames = string.Join(", ",listUnearnedAllocationClins.GetListSelected<ClinicDto>().Select(x => x.Abbr));
		...
		//Display report
		using var FormRC = new FormReportComplex(report);
		FormRC.ShowDialog();
	...
	private void butNetUnearnedOK_Click(object sender,EventArgs e) {
		...
		var listClinicNums = listNetUnearnedClins.GetListSelected<ClinicDto>().Select(x => x.Id).ToList();
		...
		if(checkNetUnearnedAllTypes.Checked) {
			...
			var unearnedTypes = string.Join(", ",listNetUnearnedTypes.GetListSelected<Def>().Select(x => x.ItemName).ToList());
			report.AddSubTitle("UnearnedTypes",unearnedTypes);
		}
		if(checkNetUnearnedAllProvs.Checked) {
			report.AddSubTitle("Provs",Lan.g(this,"All Providers"));
		...
		if(true) {//show sub titles if clinics are enabled. 
			if(checkNetUnearnedAllClins.Checked) {
				...
				var clinNames = string.Join(", ",listNetUnearnedClins.GetListSelected<ClinicDto>().Select(x => x.Abbr));
				report.AddSubTitle("Clinics",clinNames);
			}
		}
		report.AddPageNum();
	...
	private void butLineItemOK_Click(object sender,EventArgs e) {
		...
		var listClinicNums = new List<long>(); //stores clinicNums of the selected indices
		listClinicNums.AddRange(listLineItemClins.GetListSelected<ClinicDto>().Select(x => x.Id).ToList());//empty if "All" is checked.
		if(checkLineItemAllClins.Checked) {//All Clinics selected; add all visible or hidden unrestricted clinics to the list
			AddHiddenClinicNums(ref listClinicNums);
		}
		var report = new ReportComplex(true,false);
		var table = RpUnearnedIncome.GetLineItemUnearnedData(listClinicNums,dateLineItemFrom.SelectionStart,dateLineItemTo.SelectionStart,
		...
		report.AddSubTitle("Practice Title",PrefC.GetString(PrefName.PracticeTitle));
		var dateRange = dateLineItemFrom.SelectionStart.ToShortDateString()+" - "+dateLineItemTo.SelectionStart.ToShortDateString();
		...
		if(true) {//show sub titles if clinics are enabled. 
			if(checkLineItemAllClins.Checked) {
				...
				var clinNames = string.Join(",",listLineItemClins.GetListSelected<ClinicDto>().Select(x => x.Abbr));
				report.AddSubTitle("Clinics",clinNames);
			}
		}
		QueryObject query;
	...
	private void butUnearnedAcctOk_Click(object sender,EventArgs e) {
		...
		var listClinicNums = new List<long>(); //stores clinicNums of the selected indices
		listClinicNums.AddRange(listUnearnedAcctClins.GetListSelected<ClinicDto>().Select(x => x.Id).ToList());
		if(checkUnearnedAcctAllClins.Checked) {//All Clinics selected; add all visible or hidden unrestricted clinics to the list
			AddHiddenClinicNums(ref listClinicNums);
		}
		var report = new ReportComplex(true,false);
		var table = RpUnearnedIncome.GetUnearnedAccountData(listClinicNums);
		...
		if(true) {//show sub titles if clinics are enabled. 
			if(checkUnearnedAcctAllClins.Checked) {
				...
				var clinNames = string.Join(",",listUnearnedAcctClins.GetListSelected<ClinicDto>().Select(x => x.Abbr));
				report.AddSubTitle("Clinics",clinNames);
			}
		}
		QueryObject query;
	...
	private void AddHiddenClinicNums(ref List<long> listClinicNums) {
		listClinicNums=listClinicNums.Union(Clinics.GetAllForUserod(Security.CurUser).Select(x => x.Id).Append(0)).ToList();
	}

	///<summary>tabControl event handler for selected tab.</summary>
	private void tabControlMain_SelectedIndexChanged(object sender,EventArgs e) {
		if(tabControlMain.SelectedIndex==1) {//Unearned Allocation
			try {
				var versionMySQL=new Version(MiscData.GetMySqlVersion());
