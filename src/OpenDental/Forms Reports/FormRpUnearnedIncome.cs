...
namespace OpenDental {
	public partial class FormRpUnearnedIncome:FormODBase {
		...
		private void butUnearnedAllocationOK_Click(object sender,EventArgs e) {
			...
			List<long> listClinicNums = listUnearnedAllocationClins.GetListSelected<ClinicDto>().Select(x => x.Id).ToList();
			...
			if(true) {//show sub titles if clinics are enabled. 
				if(checkUnearnedAllocationAllClins.Checked) {
					...
					string clinNames = string.Join(", ",listUnearnedAllocationClins.GetListSelected<ClinicDto>().Select(x => x.Abbr));
		...
		private void butNetUnearnedOK_Click(object sender,EventArgs e) {
			...
			List<long> listClinicNums = listNetUnearnedClins.GetListSelected<ClinicDto>().Select(x => x.Id).ToList();
			...
			if(true) {//show sub titles if clinics are enabled. 
				if(checkNetUnearnedAllClins.Checked) {
					...
					string clinNames = string.Join(", ",listNetUnearnedClins.GetListSelected<ClinicDto>().Select(x => x.Abbr));
					report.AddSubTitle("Clinics",clinNames);
				}
			}
			report.AddPageNum();
		...
		private void butLineItemOK_Click(object sender,EventArgs e) {
			...
			if(true) {//show sub titles if clinics are enabled. 
				if(checkLineItemAllClins.Checked) {
					...
					string clinNames = string.Join(",",listLineItemClins.GetListSelected<ClinicDto>().Select(x => x.Abbr));
					report.AddSubTitle("Clinics",clinNames);
				}
			}
			QueryObject query;
		...
		private void butUnearnedAcctOk_Click(object sender,EventArgs e) {
			...
			if(true) {//show sub titles if clinics are enabled. 
				if(checkUnearnedAcctAllClins.Checked) {
					...
					string clinNames = string.Join(",",listUnearnedAcctClins.GetListSelected<ClinicDto>().Select(x => x.Abbr));
					report.AddSubTitle("Clinics",clinNames);
				}
			}
			QueryObject query;
