...
public partial class FormRpBrokenAppointments:FormODBase {
		...
		if(radioProcs.Checked) {
			...
			switch(brokenApptCodeDB) {
				...
				case BrokenApptProcedure.Missed:
					...
					listOptions.Items.Add(Lans.g(brokenApptCodeDB.ToString())+": (D9986)");
				...
				case BrokenApptProcedure.Cancelled:
					...
					listOptions.Items.Add(Lans.g(brokenApptCodeDB.ToString())+": (D9987)");
				...
				case BrokenApptProcedure.Both:
					...
					listOptions.Items.Add(Lans.g(BrokenApptProcedure.Missed.ToString())+": (D9986)");
					listOptions.Items.Add(Lans.g(BrokenApptProcedure.Cancelled.ToString())+": (D9987)");
					listOptions.Items.Add(Lans.g(brokenApptCodeDB.ToString()));
	...
	private void radioAdj_CheckedChanged(object sender,EventArgs e) {
		if(radioAdj.Checked) {
			...
			var brokenApptAdjDefNum=PrefC.GetLong(PrefName.BrokenAppointmentAdjustmentType);
			for(var i=0; i<_listPosAdjTypes.Count;i++) {
	...
	private void butOK_Click(object sender,EventArgs e) {
		...
		for(var i=0;i<listClinics.SelectedIndices.Count;i++) {
			if(Security.CurUser.ClinicIsRestricted) {
		...
		var brokenApptSelection=BrokenApptProcedure.None;
		if(radioProcs.Checked) {
		...
		var fontSubTitle=new Font("Tahoma",11,FontStyle.Bold);
