...
public partial class FormRpClaimNotSent : FormODBase {
	...
	private void FillGrid() {
		...
		var listClinicNums=comboClinicMulti.ListClinicNumsSelected;
		var table=RpClaimNotSent.GetClaimsNotSent(_startDate,_endDate,listClinicNums,false
		...
		for(var i = 0;i<table.Rows.Count;i++) {
			...
			var dateService=SIn.Date(table.Rows[i]["DateService"].ToString());
			row.Cells.Add(dateService.ToShortDateString());
			var type=table.Rows[i]["ClaimType"].ToString();
			switch(type) {
				case "P":
					type="Pri";
					break;
			...
			var unsentClaim=new UnsentInsClaim();
			unsentClaim.ClaimNum=SIn.Long(table.Rows[i]["ClaimNum"].ToString());
			unsentClaim.PatNum=SIn.Long(table.Rows[i]["PatNum"].ToString());
			var claimTrackingCur=_listNewClaimTrackings.FirstOrDefault(x => x.ClaimNum==unsentClaim.ClaimNum);
	...
	private bool ValidateFilters() {
		...
		if(true && comboClinicMulti.ListClinicNumsSelected.Count==0) {
	...
	private void butRunReport_Click(object sender,EventArgs e) {
		...
		var listClinicNums=comboClinicMulti.ListClinicNumsSelected;
		var table=RpClaimNotSent.GetClaimsNotSent(_startDate,_endDate,listClinicNums,true,(ClaimNotSentStatuses)comboBoxInsFilter.SelectedItem);
		var subtitleClinics="";
		...
		var query=report.AddQuery(table,"Date: " + DateTime.Today.ToShortDateString());
		if(true) {
	...
	private void gridMain_RightClickHelper(object sender,EventArgs e) {
		...
		switch(menuCode) {
			case 0://Go to Account
				GlobalFormOpenDental.GoToModule(EnumModuleType.Account,patNum:((UnsentInsClaim)gridMain.ListGridRows[index].Tag).PatNum);
				break;
		}
	}
	...
	private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
		...
		var claim=Claims.GetClaim(((UnsentInsClaim)gridMain.ListGridRows[e.Row].Tag).ClaimNum);
		if(claim==null) {
