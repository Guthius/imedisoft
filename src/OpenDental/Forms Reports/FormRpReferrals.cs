...
public partial class FormRpReferrals : FormODBase	{
	...
	private void FillDropListFilter(){
		for(var i=0;i<ALrefFilter.Count;i++){
			DropListFilter.Items.Add((string)ALrefFilter[i]);
		}
	}
	...
	private void createSQLfrom(){
		var listfieldsSelected=new List<string>(); 
		if(listSelect.SelectedIndices.Count>0){
			SQLselect="SELECT ";
			for(var i=0;i<listSelect.SelectedIndices.Count;i++){
				if(i>0){
					SQLselect+=", ";
				}
	...
	private void DropListFilter_SelectedIndexChanged(object sender, System.EventArgs e) {
		...
		switch(DropListFilter.SelectedItem.ToString()){
			...
			case "Specialty":
				...
				var specDefs=Defs.GetDefsForCategory(DefCat.ProviderSpecialties,true).ToArray();
				for(var i=0;i<specDefs.Length;i++) {
	...
	private void butAddFilter_Click(object sender, System.EventArgs e) {
		if(isText){
			if(listConditions.SelectedIndex==0){
				listPrerequisites.Items.Add(DropListFilter.SelectedItem+" LIKE '%"+textBox.Text+"%'");
			}
			else{
				listPrerequisites.Items.Add(DropListFilter.SelectedItem+" "+listConditions.SelectedItem+" '"+textBox.Text+"'");
			}
		}//end if(isText)
		else if(isDropDown){
			if(DropListFilter.SelectedItem.ToString()=="Specialty"){
				sItem="";
				var specDefs=Defs.GetDefsForCategory(DefCat.ProviderSpecialties,true).ToArray();
				for(var i=0;i<listOptions.SelectedIndices.Count;i++){
					if(i==0){ 
						sItem="(";
					}
					else{ 
					...
					sItem+="Specialty "+listConditions.SelectedItem+" '"+
					       specDefs[listOptions.SelectedIndices[i]].DefNum+"'"; 
				...
				for(var i=0;i<listOptions.SelectedIndices.Count;i++){
					if(listConditions.SelectedIndex==0){ 
						listPrerequisites.Items.Add(DropListFilter.SelectedItem+" LIKE '%"+listOptions.SelectedIndices[i]+"%'");  
					}
					else{
						listPrerequisites.Items.Add(DropListFilter.SelectedItem+" "
						                                                       +listConditions.SelectedItem+" '"
						                                                       +listOptions.SelectedIndices[i]+"'");   
					}
				}
			}
			listOptions.SelectedIndex=-1;
	...
	private void butDeleteFilter_Click(object sender, System.EventArgs e) {
		for(var i=listPrerequisites.SelectedIndices.Count;i>0;i--){
