...
public partial class FormQuery : FormODBase {
	...
	private void butAdd_Click(object sender, System.EventArgs e) {
		using var FormQE=new FormQueryEdit();
		FormQE.UserQueryCur=new UserQuery();
		FormQE.UserQueryCur.QueryText=textQuery.Text;
		FormQE.UserQueryCur.IsNew=true;
		FormQE.UserQueryCur.IsPromptSetup=true;
		FormQE.UserQueryCur.DefaultFormatRaw=PrefC.GetBool(PrefName.UserQueryDefaultRaw);
		FormQE.ShowDialog();
	...
	private void butBack_Click(object sender, System.EventArgs e){
		...
		labelTotPages.Text=(printPreviewControl2.StartPage+1)
		                   +" / "+_totalPages;
	}
...
//this test case worked, so now it is just a matter of finishing this off, and Excel export will be done.
	...
	private void butExport_Click(object sender, System.EventArgs e){
		...
		var fileName=_reportSimpleGrid.Title;
		...
		try{
			...
			{
				var line="";
				for(var i=0;i<_reportSimpleGrid.ColCaption.Length;i++) {
					var columnCaption=_reportSimpleGrid.ColCaption[i];
				...
				sw.WriteLine(line);
				string cell;
				for(var i=0;i<_reportSimpleGrid.TableQ.Rows.Count;i++){
					line="";
					for(var j=0;j<_reportSimpleGrid.TableQ.Columns.Count;j++){
	...
	private void butFavorites_Click(object sender, System.EventArgs e) {
		using var FormQF=new FormQueryFavorites();
	...
	private void FormQuery_Load(object sender, System.EventArgs e) {
		printPreviewControl2.Bounds=new Rectangle(0,0,ClientSize.Width,ClientSize.Height-39);
		grid2.Font=bodyFont;
		...
		if(IsReport){
			...
			panelZoom.Visible=true;
			PrintReport(true);
			labelTotPages.Text="/ "+_totalPages;
	...
	public static DataTable MakeReadable(DataTable tableIn,ReportSimpleGrid reportIn,bool doConvertPatNum=true){
		...
		var tableOut=tableIn.Clone();//copies just the structure
		for(var j=0;j<tableOut.Columns.Count;j++){
		...
		//string str;
		//Type t;
		for(var i=0;i<tableIn.Rows.Count;i++){
			thisRow=tableOut.NewRow();//new row with new schema
			for(var j=0;j<tableIn.Columns.Count;j++){
		...
		DateTime date;
		var colTotals=new decimal[tableOut.Columns.Count];
		for(var j=0;j<tableOut.Columns.Count;j++){
			for(var i=0;i<tableOut.Rows.Count;i++){
				try{
					...
					else switch(tableOut.Columns[j].Caption.ToLower())
					{
						...
						case "inswoest":
							...
							break;
						case "toothnum":
							tableOut.Rows[i][j]=Tooth.Display(tableOut.Rows[i][j].ToString());
							break;
						//definitions:
						case "adjtype":
							tableOut.Rows[i][j]
	...
	public bool PrintReport(bool justPreview){
		...
		var margins=new Margins(25,50,50,60);
		...
		_pagesPrinted=0;
		_linesPrinted=0;
		var printout=PrinterL.CreateODprintout(pd2_PrintPage,Lan.g(this,"Query printed"),margins:margins,printoutOrientation:orient);
		if(printout!=null && printout.PrintDoc!=null) {
			...
			printout.PrintDoc.EndPrint+=(_,_) => {
	...
	private void AutoSizeColumns(DataTable table) {
		...
		//int colWidth;
		int tempWidth;
		for(var i = 0;i<_reportSimpleGrid.ColWidth.Length;i++) {
			_reportSimpleGrid.ColWidth[i]
				=(int)grfx.MeasureString(table.Columns[i].Caption,grid2.Font).Width;
			for(var j = 0;j<table.Rows.Count;j++) {
				tempWidth=(int)TextRenderer.MeasureText(table.Rows[j][i].ToString(),grid2.Font).Width;
	...
	private void FillForm() {
		...
		for(var iCol = 0;iCol<_reportSimpleGrid.TableQ.Columns.Count;iCol++) {
	...
	private void LayoutHelperForState(QueryExecuteState executeState) {
		switch (executeState) {
			...
			case QueryExecuteState.Executing:
				...
				this.Text=Lans.g("User Query - Executing Query...");
			...
			case QueryExecuteState.Formatting:
				...
				this.Text=Lans.g("User Query - Formatting Query...");
	...
	private bool UserHasStackTracePref() {
		var pref = UserOdPrefs.GetFirstOrNewByUserAndFkeyType(Security.CurUser.UserNum, UserOdFkeyType.QueryMonitorHasStackTraces);
		if(pref.IsNew) {
			pref.ValueString="0";
	...
	private void pagePrinter(PrintPageEventArgs ev) {
		...
		if(!_headerPrinted){
			...
			for(var i=0;i<_reportSimpleGrid.SubTitle.Count;i++){
				if(isPrintablePage) {
		...
		if(isPrintablePage) {
			...
			ev.Graphics.DrawString(Lan.g(this,"Page:")+" "+(_currentPage)
				,bodyFont,Brushes.Black
				,bounds.Right
				 -ev.Graphics.MeasureString(Lan.g(this,"Page:")+" "+(_currentPage)
					 ,bodyFont).Width,yPos);
		}
		...
		var fontHeight=bodyFont.GetHeight(ev.Graphics);
		var yPosTableTop=yPos;
		...
		{
			var isColWrap=PrefC.GetBool(PrefName.ReportsWrapColumns);
			if(isColWrap && yPos > yPosTableTop) {//First row always prints.  Otherwise the row might be pushed to next page if too tall.
				...
				var isRowTooTall=false;//Bool to indicate if a row we are about to print is too tall for the avaible space on page.
				for(var iCol2=0;iCol2<_reportSimpleGrid.TableQ.Columns.Count;iCol2++){
					...
					var cellText=grid2[_linesPrinted,iCol2].ToString();
					var rectHeight=ev.Graphics.MeasureString(cellText,bodyFont,cellWidth).Height;
			...
			var rowHeight=fontHeight;//When wrapping, we get the hight of the tallest cell in the row and increase yPos by it.
			for(var iCol=0;iCol<_reportSimpleGrid.TableQ.Columns.Count;iCol++){//For each cell in the row, print the cell contents.
				var cellHeight=rowHeight;
				if(isColWrap) {
					cellHeight=0;//Infinate height.
				}
				var cellWidth=0;
				var cellText=grid2[_linesPrinted,iCol].ToString();
			...
			if(yPos<bounds.Bottom){
				...
				for(var iCol=0;iCol<_reportSimpleGrid.TableQ.Columns.Count;iCol++){
					if(_reportSimpleGrid.ColAlign[iCol]==HorizontalAlignment.Right){
						...
						var textWidth=(float)(ev.Graphics.MeasureString
		...
		if(_totalsPrinted){
			if(yPos+_reportSimpleGrid.Summary.Count*subtitleFont.GetHeight(ev.Graphics)< bounds.Top+bounds.Height){
				...
				for(var i=0;i<_reportSimpleGrid.Summary.Count;i++){
					if(_reportSimpleGrid.SummaryFont!=null && _reportSimpleGrid.SummaryFont!=""){
						var acctnumFont=new Font(_reportSimpleGrid.SummaryFont,12);
						if(isPrintablePage) {
	...
	private void pd2_PrintPage(object sender, PrintPageEventArgs ev){
		...
		{ 
			...
			labelTotPages.Text=pageCur+" / "+_totalPages;
			_pagesPrinted=0;
			_linesPrinted=0;
