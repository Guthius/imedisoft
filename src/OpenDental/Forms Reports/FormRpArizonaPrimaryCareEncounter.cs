...
public partial class FormRpArizonaPrimaryCareEncounter:FormODBase {
	...
	private void butRun_Click(object sender,EventArgs e) {
		...
		this.textLog.Text="";
		var outFile=ODFileUtils.CombinePaths(this.textEncounterFolder.Text,this.textEncounterFile.Text);
		...
		var payDefNumTab=DataCore.GetTable(command);
		...
		var payDefNum=SIn.Long(payDefNumTab.Rows[0][0].ToString());
		var outputText="";
		...
		var billableProcedures="'D0120','D0140','D0150','D0160','D1110','D1120','D1201','D1203','D1204','D1205','D1208',"+
		...
		for(var i=0;i<primaryCarePatients.Rows.Count;i++) {
			...
			var appointmentList=DataCore.GetTable(command);
			for(var j=0;j<appointmentList.Rows.Count;j++){
				var aptNum=SOut.Long(SIn.Long(appointmentList.Rows[j][0].ToString()));
				var datesql="CURDATE()";
				command="SELECT "+
				        ...
				        "(CASE WHEN p.EmployerNum=0 THEN (CASE WHEN (ADDDATE(p.BirthDate, INTERVAL 18 YEAR)>"+datesql+") THEN 3 ELSE 2 END) ELSE 1 END) EmploymentStatus,"+
				...
				var rowWarnings="";
				//Patient's ID Number
				var pcin=primaryCareReportRow.Rows[0]["PCIN"].ToString();
				if(pcin.Length<9) {
				...
				//Patient's address
				var householdAddress=SOut.String(SIn.String(primaryCareReportRow.Rows[0]["Address"].ToString()));
				...
				//Patient's city
				var householdCity=SOut.String(SIn.String(primaryCareReportRow.Rows[0]["City"].ToString()));
				...
				//Patient's State
				var householdState=SOut.String(SIn.String(primaryCareReportRow.Rows[0]["State"].ToString()));
				...
				//Patient's zip code
				var householdZip=SOut.String(SIn.String(primaryCareReportRow.Rows[0]["Zip"].ToString()));
				...
				var insuranceRelationship=SOut.String(SIn.String(primaryCareReportRow.Rows[0]["InsRelat"].ToString()));
				...
				var siteId=SIn.String(primaryCareReportRow.Rows[0]["ClinicDescription"].ToString());
				if(siteId=="null"){
					siteId="";
				}
				...
				outputRow+=siteId;
				//Physician ID
				var physicianId=SIn.String(primaryCareReportRow.Rows[0]["PhysicianID"].ToString());
				if(physicianId.Length>12){
					var newPhysicianId=physicianId.Substring(0,12);
				...
				var physicianFirstAndMiddle=SIn.String(primaryCareReportRow.Rows[0]["PhysicianFAndMNames"].ToString());
				if(physicianFirstAndMiddle.Length>12){
					var newPhysicianFirstAndMiddle=physicianFirstAndMiddle.Substring(0,12);
				...
				var physicianLastName=SIn.String(primaryCareReportRow.Rows[0]["PhysicianLName"].ToString());
				if(physicianLastName.Length>20){
					var newPhysicianLastName=physicianLastName.Substring(0,20);
