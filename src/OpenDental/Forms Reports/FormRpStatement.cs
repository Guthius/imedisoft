...
public partial class FormRpStatement : FormODBase {
	...
	private void FormRpStatement_Load(object sender, System.EventArgs e) {
		...
		labelTotPages.Text="1 / "+totalPages;
	...
	public static string CreateStatementPdfSheets(Statement stmt,Patient pat,Family fam,DataSet dataSet,bool isSilent=false) {
		var tempPdfFile="";
		try {
			tempPdfFile=Statements.CreateStatementPdfSheets(stmt,pat,fam,dataSet);
		}
		catch {
			if(!isSilent) {
	...
	public void PrintStatement(Statement stmt,bool previewOnly,DataSet dataSet,Family fam,Patient pat) {
		...
		var margins=new Margins(40,40,40,60);
		...
		PrinterL.CreateODprintout(
			auditDescription:Lan.g(this,"Statement from")+" "+stmt.DateTStamp.ToShortDateString()+" "+Lans.g("printed"),
		...
		var doc=CreateDocument(ODprintout.CurPrintout.PrintDoc,fam,pat,dataSet);
		var printdoc=new MigraDoc.Rendering.Printing.MigraDocPrintDocument();
	...
	private MigraDoc.DocumentObjectModel.Document CreateDocument(PrintDocument pd,Family fam,Patient pat,DataSet dataSet){
		...
		var section=doc.AddSection();//so that Swiss will have different footer for each patient.
		string text;
		MigraDoc.DocumentObjectModel.Font font;
		//GetPatGuar(PatNums[famIndex][0]);
		//Family fam=Patients.GetFamily(Stmt.PatNum);
		var PatGuar=fam.ListPats[0];//.Clone();
		...
		var parformat=new ParagraphFormat();
		...
		if(Stmt.IsInvoice) {
			if(CultureInfo.CurrentCulture.Name=="en-NZ" || CultureInfo.CurrentCulture.Name=="en-AU") {//New Zealand and Australia
				...
				text+=" #"+Stmt.StatementNum;//Some larger customers of OD need this to show in order to properly pay.
			}
		}
		else if(Stmt.IsReceipt) {
			text=Lan.g(this,"RECEIPT");
			if(CultureInfo.CurrentCulture.Name.EndsWith("SG")) {//SG=Singapore
				text+=" #"+Stmt.StatementNum;
			}
		}
		else {
		...
		if(PrefC.GetBool(PrefName.StatementShowReturnAddress)) {
			...
			{
				var clinic=Clinics.GetClinic(PatGuar.ClinicNum);
				par=frame.AddParagraph();
				par.Format.Font=font;
		...
		if(!Stmt.HidePayment) {
			...
			float rowHeight=26;
			var wfont=new System.Drawing.Font("Arial",7,FontStyle.Bold);
			var g=this.CreateGraphics();//just to measure strings
		...
		if(CultureInfo.CurrentCulture.Name.EndsWith("CH")) {//CH is for switzerland. eg de-CH
			...
			par.AddText(((PatGuar.City+", "+PatGuar.State).Trim(',', ' ')+" "+PatGuar.Zip).Trim());
		}
		...
		*/
		...
		var fontBold=MigraDocHelper.CreateFont(10, true);
		if(Stmt.IsInvoice) {
		...
		if(Stmt.IsInvoice) {
			...
			if(Stmt.SinglePatient) {
				...
				var patBal=pat.EstBalance-patInsEst;
				text = pat.EstBalance.ToString("c");
				par.AddFormattedText(text,font);
				par.AddLineBreak();
		...
		gridPP.Columns.Add(gcol);
		gridPP.Width=gridPP.Columns.WidthAll()+20;
		gridPP.EndUpdate();
		...
		var tablePP=dataSet.Tables["payplan"];
		GridCell gcell;
		if(tablePP.Rows.Count>0){
			...
			for(var p=0;p<tablePP.Rows.Count;p++){
				grow=new GridRow();
		...
		gridPat.Width=gridPat.Columns.WidthAll()+20;
		gridPat.EndUpdate();
		...
		for(var i=0;i<dataSet.Tables.Count;i++){
			...
			foreach(DataRow rowCur in tableAccount.Rows) {
				...
				if(CultureInfo.CurrentCulture.Name=="en-AU") {//English (Australia)
					if(rowCur["prov"].ToString().Trim()!="") {
						grow.Cells.Add(rowCur["prov"]+" - "+rowCur["description"]);
					}
			...
			var totalPos=((float)(doc.DefaultPageSetup.PageWidth.Inch//-doc.DefaultPageSetup.LeftMargin.Inch
					//-doc.DefaultPageSetup.RightMargin.Inch)
				)*100f)/2f+(float)gridPat.Columns.WidthAll()/2f+7;
			var rectF=new RectangleF(0,0,totalPos,16);
			if(patnum!=0){
		...
		if(CultureInfo.CurrentCulture.Name.EndsWith("CH")){//CH is for switzerland. eg de-CH
			...
			var swfont=MigraDocHelper.CreateFont(10);
			...
			//Bank account number--------------------------------------------------
			var origBankNum=PrefC.GetString(PrefName.PracticeBankNumber);//must be exactly 9 digits. 2+6+1.
			...
			if(origBankNum.Length==9) {
				dashedBankNum=origBankNum.Substring(0,2)+"-"
				                                        +origBankNum.Substring(2,6).TrimStart('0')+"-"
			...
			//Amount------------------------------------------------------------
			var amountdue=PatGuar.BalTotal-PatGuar.InsEst;
			...
			//Patient Address-----------------------------------------------------
			var patAddress=PatGuar.FName+" "+PatGuar.LName+"\r\n"
			               +PatGuar.Address+"\r\n";
			if(PatGuar.Address2!="") {
			...
			var referenceNum=PrefC.GetString(PrefName.BankRouting);//6 digits
			...
			var spacedRefNum=referenceNum.Substring(0,2)+" "+referenceNum.Substring(2,5)+" "+referenceNum.Substring(7,5)
	...
	private int Modulo10(string strNumber){
		...
		var intTransfer=0;
		for(var intIndex=0;intIndex<strNumber.Length;intIndex++){
			if(!char.IsDigit(strNumber[intIndex])) {
				continue;
			}
			var digit=Convert.ToInt32(strNumber.Substring(intIndex,1));
			var modulus=(intTransfer+digit) % 10;
