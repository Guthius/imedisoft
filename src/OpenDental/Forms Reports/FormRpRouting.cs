...
public partial class FormRpRouting:FormODBase {
	...
	private void PrintRoutingSlips(List<long> aptNums,long sheetDefNum) {
		...
		var sheetBatch=SheetUtil.TryCreateBatch(sheetDef,aptNums);
	...
	private void butOK_Click(object sender, System.EventArgs e){
		...
		if(true && listClin.SelectedIndices.Count==0 && !checkClinAll.Checked){
		...
		if(true) {
			if(checkClinAll.Checked) {
				...
				listClinicNums=listClin.SelectedIndices.Select(x => _listClinics[x].Id).ToList();
			}
		}
		var aptNums=RpRouting.GetRouting(date,listProvNums,listClinicNums);
		PrintRoutingSlipsForAppts(aptNums);
		DialogResult=DialogResult.OK;
	}
