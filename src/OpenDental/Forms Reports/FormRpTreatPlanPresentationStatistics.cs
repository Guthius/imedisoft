...
public partial class FormRpTreatPlanPresentationStatistics:FormODBase {
	...
	private void RunReport() {
		...
		var listSelectedUsers = new List<Userod>();
		...
		var listSelectedClinics = new List<ClinicDto>();
		if(true) {
			if(checkAllClinics.Checked) {
				report.AddSubTitle("Clinics",Lan.g(this,"All Clinics"));
				listSelectedClinics.Add(new ClinicDto {
				...
				listSelectedClinics.AddRange(Clinics.GetAllForUserod(Security.CurUser)
					.Where(x => !listSelectedClinics.Select(y => y.Id).Contains(x.Id)));
			}
			else {
		...
		var clinicNums = listSelectedClinics.Select(y => y.Id).ToList();
		var userNums = listSelectedUsers.Select(y => y.UserNum).ToList();
		var table=RpTreatPlanPresentationStatistics.GetTreatPlanPresentationStatistics(date1.SelectionStart,date2.SelectionStart,radioFirstPresented.Checked
