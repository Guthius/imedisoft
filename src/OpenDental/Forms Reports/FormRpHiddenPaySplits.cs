...
public partial class FormRpHiddenPaySplits : FormODBase {
	...
	private bool IsValid() {
		...
		if(true && !checkAllClinics.Checked && listBoxClinic.SelectedIndices.Count==0) {
	...
	private void ButOK_Click(object sender,EventArgs e) {
		...
		var listUnearnedTypeDefNums=new List<long>();
		var subtitleProvs="";
		var subtitleClinics="";
		...
		if(true && checkAllClinics.Checked) {
			subtitleClinics="All Clinics (includes hidden)";
			listClinicNums=listBoxClinic.Items.GetAll<ClinicDto>().Select(x => x.Id).ToList();
		}
		else if(true && !checkAllClinics.Checked) {
			subtitleClinics=string.Join(", ",listBoxClinic.GetListSelected<ClinicDto>().Select(x => x.Abbr).ToList());
			listClinicNums=listBoxClinic.GetListSelected<ClinicDto>().Select(x => x.Id).ToList();
		}
		var table=new DataTable();
		table=RpHiddenPaySplits.GetReportData(listProvNums,listUnearnedTypeDefNums,listClinicNums,true
		...
		var query=report.AddQuery(table,"Hidden PaySplits");
		...
		using var formComplex=new FormReportComplex(report);
