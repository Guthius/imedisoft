	...
	public partial class FormRpAccountingBalanceSheet:FormODBase {
		...
		private void FormRpAccountingBalanceSheet_Load(object sender, System.EventArgs e) {
			if(DateTime.Today.Month>6){//default to this year
				monthCalendar.SetDateSelected(new DateTime(DateTime.Today.Year,12,31));
			}
			else{//default to last year
				monthCalendar.SetDateSelected(new DateTime(DateTime.Today.Year-1,12,31));
			}
		}
		...
		private void butOK_Click(object sender, System.EventArgs e) {
			...
			DataTable tableAssets=Accounts.GetAccountTotalByType(monthCalendar.GetDateSelected(),AccountType.Asset);
			DataTable tableLiabilities=Accounts.GetAccountTotalByType(monthCalendar.GetDateSelected(),AccountType.Liability);
			DataTable tableEquity=Accounts.GetAccountTotalByType(monthCalendar.GetDateSelected(),AccountType.Equity);
			//Add two new rows to the equity data table to show Retained Earnings (Auto) and NetIncomeThisYear
			//tableEquity.LoadDataRow(new object[] { "Retained Earnings (Auto)",Accounts.GetRetainedEarningsAuto(monthCalendar.GetDateSelected()) }, LoadOption.OverwriteChanges);
			tableEquity.LoadDataRow(new object[] { "NetIncomeThisYear",Accounts.NetIncomeThisYear(monthCalendar.GetDateSelected()) }, LoadOption.OverwriteChanges);
			...
			reportComplex.AddSubTitle("Date",monthCalendar.GetDateSelected().ToShortDateString(),fontSubTitle);
			//setup query
