...
public partial class FormRpPPOwriteoffs : FormODBase {
	...
	private void ExecuteIndividual(){
		...
		if(writeoffType==PPOWriteoffDateCalc.ClaimPayDate) {
			query.AddColumn("Write-off Estimate",80,FieldValueType.Number,font);
			query.AddColumn("Write-off Adjustment",80,FieldValueType.Number,font);
		}
		else {
			query.AddColumn("Write-off",80,FieldValueType.Number,font);
		}
		...
		if(writeoffType==PPOWriteoffDateCalc.ClaimPayDate) {
			query.AddGroupSummaryField("PPO Fee (Stand Fee + Write-off Estimate + Write-off Adjustment)","Stand Fee","PPO Fee",SummaryOperation.Sum, [1],Color.Black,new Font("Tahoma",9,FontStyle.Bold),0,50);
		}
		else {
			query.AddGroupSummaryField("PPO Fee (Stand Fee + Write-off)","Stand Fee","PPO Fee",SummaryOperation.Sum, [1],Color.Black,new Font("Tahoma",9,FontStyle.Bold),0,50);
		}
	...
	private void ExecuteGroup() {
		...
		var writeoffType=GetWriteoffType();
		var table=RpPPOwriteoff.GetWriteoffTable(date1.SelectionStart,date2.SelectionStart,radioIndividual.Checked,textCarrier.Text,writeoffType);
		var query=report.AddQuery(table,Lan.g(this,"Date")+": "+DateTime.Today.ToString("d"),"",SplitByKind.None,1,true);
		...
		if(writeoffType==PPOWriteoffDateCalc.ClaimPayDate) {
			query.AddGroupSummaryField("PPO Fee (Stand Fee + Write-off Estimate + Write-off Adjustment)","PPO Fee","PPO Fee",SummaryOperation.Sum, [1],Color.Black,new Font("Tahoma",9,FontStyle.Bold),0,50);
		}
		else {
			query.AddGroupSummaryField("PPO Fee (Stand Fee + Write-off)","PPO Fee","PPO Fee",SummaryOperation.Sum, [1],Color.Black,new Font("Tahoma",9,FontStyle.Bold),0,50);
		}
