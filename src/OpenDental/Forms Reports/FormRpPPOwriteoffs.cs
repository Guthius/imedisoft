...
namespace OpenDental{
	...
	public partial class FormRpPPOwriteoffs : FormODBase {
		...
		private void ExecuteIndividual(){
			...
			if(writeoffType==PPOWriteoffDateCalc.ClaimPayDate) {
				query.AddGroupSummaryField("PPO Fee (Stand Fee + Write-off Estimate + Write-off Adjustment)","Stand Fee","PPO Fee",SummaryOperation.Sum,new List<int>(){1},Color.Black,new Font("Tahoma",9,FontStyle.Bold),0,50);
			}
			else {
				query.AddGroupSummaryField("PPO Fee (Stand Fee + Write-off)","Stand Fee","PPO Fee",SummaryOperation.Sum,new List<int>(){1},Color.Black,new Font("Tahoma",9,FontStyle.Bold),0,50);
			}
		...
		private void ExecuteGroup() {
			...
			if(writeoffType==PPOWriteoffDateCalc.ClaimPayDate) {
				query.AddGroupSummaryField("PPO Fee (Stand Fee + Write-off Estimate + Write-off Adjustment)","PPO Fee","PPO Fee",SummaryOperation.Sum,new List<int>(){1},Color.Black,new Font("Tahoma",9,FontStyle.Bold),0,50);
			}
			else {
				query.AddGroupSummaryField("PPO Fee (Stand Fee + Write-off)","PPO Fee","PPO Fee",SummaryOperation.Sum,new List<int>(){1},Color.Black,new Font("Tahoma",9,FontStyle.Bold),0,50);
			}
