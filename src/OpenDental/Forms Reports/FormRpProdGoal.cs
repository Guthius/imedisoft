...
public partial class FormRpProdGoal : FormODBase {
	...
	private void RunProdGoal() {
		...
		var listSelectedClinicNums=new List<long>();
		if(true) {
			if(listClin.SelectedIndices.Count>0) {
				var offset=Security.CurUser.ClinicIsRestricted?0:1;
				listClinics.AddRange(listClin.SelectedIndices.Select(x => offset==1 && x==0?new ClinicDto { Abbr=Lan.g(this,"Unassigned") }:_listClinics[x-offset]));
			}
			if(checkAllClin.Checked) {
			...
			if(listConflicts.Count>0) {
				...
				using var msgBox=new MsgBoxCopyPaste(errorMsg);
				msgBox.ShowDialog();
				return;
			}
		}
		...
		using(var ds=RpProdGoal.GetData(_dateFrom,_dateTo,listProvs,listClinics,checkAllProv.Checked,hasAllClinics,GetWriteoffType()))
		using(var dt=ds.Tables["Total"])
		using(var dtClinic=true?ds.Tables["Clinic"]:new DataTable())
		using(var font=new Font("Tahoma",8,FontStyle.Regular)) {
			...
			if(true) {
				report.AddSubTitle("Clinics",hasAllClinics?Lan.g(this,"All Clinics (includes hidden)"):listClinics.Count==0?"":string.Join(", ",listClinics.Select(x => x.Abbr)));
			}
			//setup query
			QueryObject query;
			...
			var weekdayWidth=65;
			var prodWidth=90;
			var prodGoalWidth=90;
			var schedWidth=85;
			var adjWidth=85;
			var writeoffWidth=95;
			...
			for(var i=0;i<groups;i++) {//groups will be 1 or 2 if there are clinic breakdowns
			...
			var colNameAlign="Write-off"+(GetWriteoffType()==PPOWriteoffDateCalc.ClaimPayDate?" Est":"");//Column used to align the summary fields.
			var summaryText="Total Production (Production + Scheduled + Adjustments - Write-off"
			                +(GetWriteoffType()==PPOWriteoffDateCalc.ClaimPayDate?" Ests - Write-off Adjs":"s")+"): ";
			query.AddGroupSummaryField(summaryText,colNameAlign,"Tot Prod",SummaryOperation.Sum, [groups],Color.Black,
