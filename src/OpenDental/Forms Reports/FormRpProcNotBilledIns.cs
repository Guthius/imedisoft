...
namespace OpenDental{
	...
	public partial class FormRpProcNotBilledIns : FormODBase {
		...
		private void FormProcNotAttach_Load(object sender, System.EventArgs e) {
			...
			EnumShowProcsBeforeIns[] arrayShowProcsBeforeIns=(EnumShowProcsBeforeIns[])Enum.GetValues(typeof(EnumShowProcsBeforeIns));
			for(int i=0;i<arrayShowProcsBeforeIns.Length;i++) {
					...
					if(claimProcCur.ClaimNum > 0 && claimProcCur.Status!=ClaimProcStatus.Preauth && claimProcCur.Status!=ClaimProcStatus.Estimate) {
						...
						switch(claimCur.ClaimType) {
							...
							case "S":
								...
								break;
							case "Other":
								int tertiaryOrdinal=PatPlans.GetOrdinal(PriSecMed.Tertiary,listPatPlans,listInsPlans,listInsSubs);
