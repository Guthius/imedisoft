...
public partial class FormRpProcNotBilledIns : FormODBase {
	...
	private void FormProcNotAttach_Load(object sender, System.EventArgs e) {
		...
		listBoxShowProcsBeforeIns.Items.Clear();
		var arrayShowProcsBeforeIns=(EnumShowProcsBeforeIns[])Enum.GetValues(typeof(EnumShowProcsBeforeIns));
		for(var i=0;i<arrayShowProcsBeforeIns.Length;i++) {
			listBoxShowProcsBeforeIns.Items.Add(Lan.g(this,arrayShowProcsBeforeIns[i].GetDescription()),arrayShowProcsBeforeIns[i]);
			if(arrayShowProcsBeforeIns[i]==EnumShowProcsBeforeIns.None) {//Default to None
				listBoxShowProcsBeforeIns.SelectedIndex=listBoxShowProcsBeforeIns.Items.Count-1;
			}
		}
		FillGrid();
	}
	...
	public void FillGrid() {
		...
		for(var i=0;i<_myReport.ReportObjects.Count;i++) {
			...
			for(var j=0;j<queryObj.ReportTable.Rows.Count;j++) {
				...
				if(true) {
					var clinicNum=SIn.Long(queryObj.ReportTable.Rows[j][6].ToString());
					if(clinicNum==0) {
	...
	private void RefreshReport() {
		var hasValidationPassed=ValidateFields();
		//Above line also sets "All" clinics option if no items are selected.
		var tableNotBilled=new DataTable();
		if(hasValidationPassed) {
			...
			tableNotBilled=RpProcNotBilledIns.GetProcsNotBilled(comboClinics.ListClinicNumsSelected,checkMedical.Checked,_myReportDateFrom,_myReportDateTo,
				listBoxShowProcsBeforeIns.GetSelected<EnumShowProcsBeforeIns>(),checkShowProcsInProcess.Checked);
		}
		var subtitleClinics="";
		if(true) {
		...
		var listProcedureCodes=textProcedureCodes.Text.Split(",",StringSplitOptions.RemoveEmptyEntries).Select(x=>x.Trim()).ToList();
		var listDataRows=tableNotBilled.Select().ToList();
		for(var i=0;i<listDataRows.Count;i++) { //Filters listDataRows for specific ProcCodes
			var dataRow=listDataRows[i];	
			var procedureCode=ProcedureCodes.GetFirstOrDefault(x=>x.CodeNum==SIn.Long(dataRow["CodeNum"]
				.ToString()));
		...
		var query=_myReport.AddQuery(tableNotBilled,DateTime.Today.ToShortDateString());
	...
	private void butPrint_Click(object sender,EventArgs e) {
		using var FormR=new FormReportComplex(_myReport);
		FormR.ShowDialog();
	}
	...
	private void butNewClaims_Click(object sender,EventArgs e) {
		...
		var listNotBilledProcs=new List<ProcNotBilled>();
		var listPatNums=new List<long>();
		var patOld=new Patient();
		var listPatClaims=new List<Claim>();
		var listPatClaimProcs=new List<ClaimProc>();
		var listCurClaimProcs=new List<ClaimProc>();
		...
		var listProcNumsPastLockDate=new List<long>();
		for(var i=0;i<gridMain.ListGridRows.Count;i++) {//Loop through every row in gridMain to construct datatable and listNotBilledProcs.
			//Table is passed to toolBarButIns_Click(...) and must contain data for every row in the grid.
			var rowCur=(DataRow)gridMain.ListGridRows[i].Tag;
			var procNumCur=SIn.Long(rowCur["ProcNum"].ToString());
			var procCur=Procedures.GetOneProc(procNumCur,false);
			if(procCur.ProcDate <= dateRestricted) {//current procedure is past or on the lock date. 
				listProcNumsPastLockDate.Add(procNumCur);
			}
			var patNumCur=procCur.PatNum;
			if(patOld.PatNum!=patNumCur) {//Procedures in gridMain are ordered by patient, so when the patient changes, we know previous patient is complete.
				...
				listInsSubs=InsSubs.RefreshForFam(Patients.GetFamily(patNumCur));
				listInsPlans=InsPlans.RefreshForSubList(listInsSubs);
			}
			listCurClaimProcs=ClaimProcs.GetForProc(listPatClaimProcs,procNumCur);
			var hasPriClaim=false;
			var hasSecClaim=false;
			var hasTertiaryClaim=false;
			for(var j=0;j<listCurClaimProcs.Count;j++) {
				var claimProcCur=listCurClaimProcs[j];
				if(claimProcCur.ClaimNum > 0 && claimProcCur.Status!=ClaimProcStatus.Preauth && claimProcCur.Status!=ClaimProcStatus.Estimate) {
					var claimCur=Claims.GetFromList(listPatClaims,claimProcCur.ClaimNum);
					switch(claimCur.ClaimType) {
						...
						case "S":
							...
							break;
						case "Other":
							var tertiaryOrdinal=PatPlans.GetOrdinal(PriSecMed.Tertiary,listPatPlans,listInsPlans,listInsSubs);
							if(listPatPlans.Exists(x => x.Ordinal==tertiaryOrdinal && x.InsSubNum==claimCur.InsSubNum)) {
								hasTertiaryClaim=true;
							}
							break;
					}
				}
			}
			var isSelected=gridMain.SelectedIndices.Contains(i);
			listNotBilledProcs.Add(new ProcNotBilled(patOld,procNumCur,i,isSelected,hasPriClaim,hasSecClaim,hasTertiaryClaim,procCur.ClinicNum,procCur.PlaceService));
			var row=table.NewRow();
			row["ProcNum"]=procNumCur;
			#region Calculate chargesDouble
			var writeOffCapSum=listPatClaimProcs.Where(x => x.Status==ClaimProcStatus.CapComplete).Sum(y => y.WriteOff);
		...
		var listGroupedProcs=new List<List<ProcNotBilled>>();
		Patient patCur=null;
		List<Procedure> listPatientProcs=null;
		var procNotBilled=new ProcNotBilled();//When automatically grouping,  this is used as the procedure to group by.
		long patNumOld=0;
		var claimCreatedCount=0;
		var patIndex=0;
		...
		while(patIndex < listPatNums.Count) {
			var listProcs=listNotBilledProcs.Where(x => x.Patient.PatNum==listPatNums[patIndex] && x.IsRowSelected && !x.IsAttached 
			...
			if(listProcs.Any(x => listProcs[0].PlaceService!=x.PlaceService) || listProcs.Any(x => listProcs[0].ClinicNum!=x.ClinicNum)) {
				   ...
				   && PatPlans.GetOrdinal(PriSecMed.Secondary,listPatPlans,listInsPlans,listInsSubs)==0) //Does not have secondary dental ins.
				{
					...
					if(!listProcs[0].HasTertiaryClaim&&PatPlans.GetOrdinal(PriSecMed.Tertiary,listPatPlans,listInsPlans,listInsSubs)>0) {//Tertiary claim.
						claimCreatedCount++;
					}
				}
			}
		...
		claimCreatedCount=0;
		var claimErrors="";
		foreach(var listProcs in listGroupedProcs) { 
			...
			for(var j=0;j<listProcs.Count;j++) {
			...
			var arraySelectedIndices=(int[])gridMain.SelectedIndices.Clone();
			var createClaimDataWrapper=ClaimL.GetCreateClaimDataWrapper(patCur,Patients.GetFamily(patCur.PatNum)
				,ClaimL.GetCreateClaimItems(table,arraySelectedIndices),false);
			createClaimDataWrapper=ClaimL.CreateClaimFromWrapper(false,createClaimDataWrapper,procNotBilled.HasPriClaim,procNotBilled.HasSecClaim,procNotBilled.HasTertiaryClaim);
			var errorTitle=patCur.PatNum+" "+patCur.GetNameLFnoPref()+" - ";
		...
		if(!string.IsNullOrEmpty(claimErrors)) {
			using var form=new MsgBoxCopyPaste(claimErrors);
			form.ShowDialog();
		}
		...
		List<InsSub> listInsSubs,List<InsPlan> listInsPlans)
	{
		...
		while(!isMedical && Procedures.GetUniqueDiagnosticCodes(listProcedures,false).Count > 4) {//dental
			var index=listProcedures.Count-1;
	...
	private void menuItemGridGoToAccount_Click(object sender,EventArgs e) {
		...
		var row=(DataRow)gridMain.ListGridRows[gridMain.GetSelectedIndex()].Tag;
...
internal class ProcNotBilled {
	...
	public ProcNotBilled() {
		HasPriClaim=false;
		HasSecClaim=false;
		HasTertiaryClaim=false;
	}

	public ProcNotBilled(Patient pat,long procNum,int rowIndex,bool isRowSelected,
		bool hasPriClaim,bool hasSecClaim,bool hasTertirayClaim,long clinicNum,PlaceOfService placeService)
	{
		Patient=pat;
		ProcNum=procNum;
		RowIndex=rowIndex;
		...
		HasTertiaryClaim=hasTertirayClaim;
		ClinicNum=clinicNum;
