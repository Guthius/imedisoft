...
public partial class FormRpPatients:FormODBase {
	...
	private void FillFilterDropList() {
		for(var i = 0;i<_listPatFilter.Count;i++) {
	...
	private void CreateSQLselect() {
		...
		if(listPatientSelect.SelectedIndices.Count>0) {
			...
			for(var i = 0;i<listPatientSelect.SelectedIndices.Count;i++) {
				...
				if (field.Contains("st Complete Proc")) { // check for both firST and laST versions
					var strMinOrMax=field.StartsWith("F")? "MIN" : "MAX";
		...
		if(listReferredToSelect.SelectedIndices.Count>0) {
			...
			for(var i = 0;i<listReferredToSelect.SelectedIndices.Count;i++) {
	...
	private void CreateSQLwhereComparison() {
		...
		string field;
		for(var i = 0;i<listPrerequisites.Items.Count;i++) {
	...
	private void DropListFilter_SelectedIndexChanged(object sender,System.EventArgs e) {
		...
		switch(DropListFilter.SelectedItem.ToString()) {
			...
			case "BillingType":
				SetListBoxConditions();
				listBoxColumns.Items.Clear();
				for(var i = 0;i<_listBillingTypeDefs.Count;i++) {
					sItem=_listBillingTypeDefs[i].ItemName;
			...
			case "RecallStatus":
				SetListBoxConditions();
				listBoxColumns.Items.Clear();
				for(var i = 0;i<_listRecallUnschedStatusDefs.Count;i++) {
					sItem=_listRecallUnschedStatusDefs[i].ItemName;
	...
	private void butAddFilter_Click(object sender,System.EventArgs e) {
		...
		if(IsText) {
			if(DropListFilter.SelectedItem.ToString()=="Primary Carrier") {
				if(listConditions.SelectedIndex==0) {
					...
					listPrerequisites.Items.Add("insplan.Carrier "+listConditions.SelectedItem+" '"+TextBox.Text+"'");
				}
				...
				if(listConditions.SelectedIndex==0) {
					...
					listPrerequisites.Items.Add("insplan.Carrier "
					                            +listConditions.SelectedItem+" '"+TextBox.Text+"'");
				}
				UsingInsPlans[UsingInsPlans.Count-1]=true;
				...
				if(listConditions.SelectedIndex==0) {
					...
					listPrerequisites.Items.Add("referral.patnum=0 AND referral.LName "+listConditions.SelectedItem
					                                                                   +" '"+TextBox.Text+"'");
				}
				...
				if(listConditions.SelectedIndex==0) {
					listPrerequisites.Items.Add("patient."+DropListFilter.SelectedItem+" LIKE '%"+TextBox.Text+"%'");
				}
				else {
					listPrerequisites.Items.Add("patient."+DropListFilter.SelectedItem+" "
					                            +listConditions.SelectedItem+" '"+TextBox.Text+"'");
				}
			}
		}
		...
		#region IsDate
			...
			if(DropListFilter.SelectedItem.ToString()=="First Visit Date") {
				if(listConditions.SelectedIndex==0) {
					...
					listPrerequisites.Items.Add("*HAVING MIN(procdate) "+listConditions.SelectedItem
					                                                    +" "+SOut.Date(DateTime.Parse(TextDate.Text)));
					ProcLogFirstDate="MIN(procdate) "+listConditions.SelectedItem
					                                 +" "+SOut.Date(DateTime.Parse(TextDate.Text));
				}
				...
				if(listConditions.SelectedIndex==0) {
					...
					listPrerequisites.Items.Add("*HAVING MAX(procdate) "+listConditions.SelectedItem
					                                                    +" "+SOut.Date(DateTime.Parse(TextDate.Text)));
					ProcLogLastDate="MAX(procdate) "+listConditions.SelectedItem
					                                +" "+SOut.Date(DateTime.Parse(TextDate.Text));
				}
				...
				if(listConditions.SelectedIndex==0) {
					listPrerequisites.Items.Add("MONTH(Birthdate) "
					                            +"= '"
					                            +DateTime.Parse(TextDate.Text).Month+"'");
				}
				else {
					listPrerequisites.Items.Add("SUBSTRING(Birthdate,6,5) "
					                            +listConditions.SelectedItem+" '"
				...
				var isMin=DropListFilter.SelectedItem.ToString().StartsWith("F");
				var body=$"(SELECT {(isMin?"MIN":"MAX")}(ProcDate) FROM procedurelog WHERE procedurelog.PatNum = patient.PatNum AND procedurelog.ProcStatus = 2)";
				var operation = listConditions.SelectedItem.ToString().Trim().ToUpper();
				var date=SOut.Date(DateTime.Parse(TextDate.Text),false);
				...
				if(listConditions.SelectedIndex==0) {
					listPrerequisites.Items.Add(DropListFilter.SelectedItem
					                            +" Like '%"+SOut.Date(DateTime.Parse(TextDate.Text),false)+"%'");
				}
				else {
					listPrerequisites.Items.Add(DropListFilter.SelectedItem+" "
					                                                       +listConditions.SelectedItem+" "+SOut.Date(DateTime.Parse(TextDate.Text)));
				}
			}
		}
		#endregion IsDate
		#region IsDropDown
			...
			if(DropListFilter.SelectedItem.ToString()=="FeeSched") {
				sItem="";
				for(var i = 0;i<listBoxColumns.SelectedIndices.Count;i++) {
					if(i>0) {
						UsingInsPlans.Add(false);
						UsingRefDent.Add(false);
						UsingRefPat.Add(false);
					...
					sItem+="patient.FeeSched "+listConditions.SelectedItem+" '"
					       +_listFeeScheds[listBoxColumns.SelectedIndices[i]].FeeSchedNum+"'";
				...
				for(var i = 0;i<listBoxColumns.SelectedIndices.Count;i++) {
					...
					sItem+="patient.BillingType "+listConditions.SelectedItem+" '"
					       +_listBillingTypeDefs[listBoxColumns.SelectedIndices[i]].DefNum+"'";
					if(i==listBoxColumns.SelectedIndices.Count-1) {
						sItem+=")";
					}
					listPrerequisites.Items.Add(sItem);
				...
				for(var i = 0;i<listBoxColumns.SelectedIndices.Count;i++) {
					...
					sItem+="recall.RecallStatus "+listConditions.SelectedItem+" '"
				...
				for(var i = 0;i<listBoxColumns.SelectedIndices.Count;i++) {
					...
					sItem+="patient.PatStatus "+listConditions.SelectedItem+" '"
					       +listBoxColumns.SelectedIndices[i]+"'";
					if(i==listBoxColumns.SelectedIndices.Count-1) {
						sItem+=")";
					}
					listPrerequisites.Items.Add(sItem);
				...
				for(var i = 0;i<listBoxColumns.SelectedIndices.Count;i++) {
					...
					sItem+="patient.Gender "+listConditions.SelectedItem+" '"
					       +listBoxColumns.SelectedIndices[i]+"'";
					if(i==listBoxColumns.SelectedIndices.Count-1) {
						sItem+=")";
					}
				...
				for(var i = 0;i<listBoxColumns.SelectedIndices.Count;i++) {
					...
					sItem+="patient.Position "+listConditions.SelectedItem+" '"
					       +listBoxColumns.SelectedIndices[i]+"'";
					if(i==listBoxColumns.SelectedIndices.Count-1) {
						sItem+=")";
					}
				...
				for(var i = 0;i<listBoxColumns.SelectedIndices.Count;i++) {
					...
					if(listConditions.SelectedIndex==0) {
						listPrerequisites.Items.Add(DropListFilter.SelectedItem+" LIKE '%"
						                                                       +listBoxColumns.SelectedIndices[i]+"%'");
					}
					else {
						listPrerequisites.Items.Add(DropListFilter.SelectedItem+" "
	...
	private void RemoveListPrerequisitesItem(int index) {
		var item=(string)listPrerequisites.Items.GetObjectAt(index);
		...
		//Check if item being removed has a ) to move to previous item.
			...
			var temp=((string)listPrerequisites.Items.GetObjectAt(index-1))+")";
