...
public partial class FormRpAging : FormODBase {
	...
	private void FormRpAging_Load(object sender, System.EventArgs e) {
		_listProviders=Providers.GetListReports();
		var lastAgingDate=PrefC.GetDate(PrefName.DateLastAging);
	...
	private RpAgingParamObject GetParamsFromForm() {
		var rpo=new RpAgingParamObject();
		...
		if(true) {
			...
			if(checkAllClin.Checked && !Security.CurUser.ClinicIsRestricted){
				...
				rpo.ListClinicNums.AddRange(Clinics.GetDeepCopy().Select(x => x.Id));
			}
			else {
				rpo.ListClinicNums=listClin.GetListSelected<ClinicDto>().Select(x => x.Id).ToList();
			}
		}
	...
	private void butGenerateQuery_Click(object sender,EventArgs e) {
		...
		var queryStr=RpAging.GetQueryString(GetParamsFromForm());
		var msgBox=new MsgBoxCopyPaste(queryStr);
		msgBox.Show(this);
	}
	...
	private void butOK_Click(object sender, System.EventArgs e) {
		...
		if(checkAllClin.Checked) {
			...
			report.AddSubTitle("Clinics",string.Join(", ",listClin.GetListSelected<ClinicDto>().Select(x => x.Abbr)));
		}
		//Patient Account Aging Query-----------------------------------------------
		var isWoEstIncluded=true;
		...
		var query=report.AddQuery(tableAging,"Date "+DateTime.Today.ToShortDateString());
