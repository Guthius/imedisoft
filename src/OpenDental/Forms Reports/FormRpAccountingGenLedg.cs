...
public partial class FormRpAccountingGenLedg:FormODBase {
	...
	private void butOK_Click(object sender, System.EventArgs e) {
		...
		var balanceRE=Accounts.GetRE_PreviousYears(monthCalendarEnd.GetDateSelected());
		var accountRE=Accounts.GetFirstOrDefault(x=>x.IsRetainedEarnings);
		//guaranteed to always be exactly 1, so we don't check for null
		var accountNumRE=accountRE.AccountNum;
		//create the report
		var report=new ReportComplex(true,false);
		var data=Accounts.GetGeneralLedger(monthCalendarStart.GetDateSelected(),monthCalendarEnd.GetDateSelected());
		for(var i=0;i<data.Rows.Count;i++) {
			var accountNum=SIn.Long(data.Rows[i]["AccountNum"].ToString());
			var accountType=SIn.Enum<AccountType>(data.Rows[i]["AcctType"].ToString());
			var balance=CalcRunningSum(accountNum,
		...
		var fontTitle=new Font("Tahoma",9);
		var fontSubTitle=new Font("Tahoma",8);
