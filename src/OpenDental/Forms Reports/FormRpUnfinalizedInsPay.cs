...
public partial class FormRpUnfinalizedInsPay:FormODBase {
	...
	private void FillGrid() {
		...
		var listClinicNums=comboClinics.ListClinicNumsSelected;//Only clinics this person has access to.
		//fill the grid
		gridMain.BeginUpdate();
		//columns
		gridMain.Columns.Clear();
		var col=new GridColumn(Lan.g("TableUnfinalizedInsPay","Type"),120,GridSortingStrategy.StringCompare);
		...
		GridRow row;
		foreach(var unfinalCur in _listUnfinalizedInsPay) {
			...
			if(true && (!listClinicNums.Contains(unfinalCur.ClinicCur.Id))) {
				continue;
			}
			row=new GridRow();
			var patName=unfinalCur.PatientCur.GetNameLFnoPref();
			if(unfinalCur.CountPats > 1) {
				patName+=" "+Lan.g(this,"(multiple)");
			}
			var carrierName=unfinalCur.CarrierCur.CarrierName;
			row.Cells.Add(Lan.g(this,unfinalCur.Type.GetDescription()));
			row.Cells.Add(patName);
			row.Cells.Add(carrierName);
			if(true) {
				var clinicAbbr=unfinalCur.ClinicCur.Abbr;
				if(unfinalCur.ClinicCur.Id==0) {//Unassigned, see RpUnfinalizedInsPay.UnfinalizedInsPay(...)
					clinicAbbr=Lan.g(this,clinicAbbr);
				}
				else if(unfinalCur.ClinicCur.IsHidden) {
					clinicAbbr+=$" ({Lan.g(this,"Hidden")})";
				}
	...
	private void gridMain_MouseUp(object sender,MouseEventArgs e) {
		...
		if(e.Button==MouseButtons.Right) {
			var unfinalPay=(RpUnfinalizedInsPay.UnfinalizedInsPay)gridMain.ListGridRows[gridMain.SelectedIndices[0]].Tag;
	...
	private void createCheckToolStripMenuItem_Click(object sender,EventArgs e) {
		...
		var listClaimProcForClaim=ClaimProcs.RefreshForClaim(unfinalPay.ClaimCur.ClaimNum);
		...
		var famCur=Patients.GetFamily(unfinalPay.PatientCur.PatNum);
		var listInsSub=InsSubs.RefreshForFam(famCur);
		var listInsPlan=InsPlans.RefreshForSubList(listInsSub);
		...
		var listClaimProcs=ClaimProcs.AttachAllOutstandingToPayment(claimPayment.ClaimPaymentNum,PrefC.DateClaimReceivedAfter);
	...
	private void openClaimToolStripMenuItem_Click(object sender,EventArgs e) {
		...
		if(unfinalPay.ClaimCur==null) {
			...
			var famCur=Patients.GetFamily(unfinalPay.PatientCur.PatNum);
			using var FormCE=new FormClaimEdit(unfinalPay.ClaimCur,unfinalPay.PatientCur,famCur);
			FormCE.ShowDialog();
		}
		LoadData();
		FillGrid();
	...
	private void goToAccountToolStripMenuItem_Click(object sender,EventArgs e) {
		var unfinalPay=(RpUnfinalizedInsPay.UnfinalizedInsPay)gridMain.ListGridRows[gridMain.SelectedIndices[0]].Tag;
		GlobalFormOpenDental.GoToModule(EnumModuleType.Account,patNum:unfinalPay.PatientCur.PatNum);
	}
	...
	private void openEOBToolStripMenuItem_Click(object sender,EventArgs e) {
		...
		using var FormCPB=new FormClaimPayBatch(unfinalPay.ClaimPaymentCur);
		FormCPB.ShowDialog();
		LoadData();
		FillGrid();
	}
	...
	private void pd_PrintPage(object sender,PrintPageEventArgs e) {
		...
		if(!_headingPrinted) {
			...
			if(comboBoxMultiType.IsAllSelected) {
				...
				text="For Types: ";
				for(var i=0;i<comboBoxMultiType.SelectedIndices.Count;i++) {
					if(i!=0) {
						text+=", ";
					}
			...
			if(true) {
				...
				if(comboClinics.IsAllSelected) {
					text+="All Clinics (Includes hidden)";
				} else {
					text+=comboClinics.GetStringSelectedClinics();
				}
				g.DrawString(text,subHeadingFont,Brushes.Black,center-g.MeasureString(text,subHeadingFont).Width/2,yPos);
				yPos+=20;
			}
	...
	private void butExport_Click(object sender,System.EventArgs e) {
		var fileName=Lan.g(this,"Unfinalized Insurance Payments");
