...
public partial class FormRpCustomAging:FormODBase {
	...
	private void FillBillType() {
		...
		var listBillTypes = Defs.GetDefsForCategory(DefCat.BillingTypes,true);
		for(var i = 0;i < listBillTypes.Count;i++) {
	...
	private void FillGrid(List<AgingPat> listAgingPats) {
		...
		var col = new GridColumn("Patient",200,GridSortingStrategy.StringCompare);
		...
		foreach(var agingPatCur in listAgingPats) {
			var row = new GridRow();
			var patName = agingPatCur.Pat.LName + ", " + agingPatCur.Pat.FName;
	...
	private AgingOptions.AgingInclude GetAgingIncludes() {
		var agingInclude = AgingOptions.AgingInclude.None;
	...
	private void goToAccountToolStripMenuItem_Click(object sender,EventArgs e) {
		...
		GlobalFormOpenDental.GoToModule(EnumModuleType.Account,patNum:((AgingPat)gridMain.SelectedGridRows[0].Tag).Pat.PatNum);
	}
	...
	private void butPrint_Click(object sender,EventArgs e) {
		...
		var tableReportCur=new DataTable();
		...
		foreach(var agingPatCur in gridMain.ListGridRows.Select(x => (AgingPat)x.Tag)) {
			var row = tableReportCur.NewRow();
		...
		var listAgingInc=new List<string>();
		...
		var query=report.AddQuery(tableReportCur,"Date "+DateTime.Today.ToShortDateString());
