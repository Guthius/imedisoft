...
namespace OpenDental
{
    public class AppointmentL
    {
        ...
        {
            GlobalFormOpenDental.GoToModule(EnumModuleType.Appointments, listPinApptNums: new List<long>() {appointment.AptNum}, patNum: appointment.PatNum, dateSelected: DateTime.Today);
        }
        ...
        public static PlannedApptStatus CreatePlannedAppt(Patient patient, int itemOrder, List<long> listPreSelectedProcNums = null)
        {
            ...
            Patient patientMerged = GetPatientMergePrompt(patient.PatNum);
        ...
        public static void SendByodLink(Appointment appointment)
        {
            ...
            progressOD.ActionMain = () =>
            {
                ...
                if (Byod.IsEnabledForConfirmed(appointment.Confirmed, appointment.ClinicNum, out string err))
                {
                    ...
                    message = Byod.GetCheckInMsg(new List<Appointment> {appointment}, listPatComms);
                }
                else
                {
                    MessageBox.Show(err);
