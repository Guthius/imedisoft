...
public class AppointmentL
{
    public static Appointment CreateRecallApt(Patient patient, long recallNum, DateTime dateAppt = default)
    {
        ...
        {
            foreach (var t in recalls)
            {
                if (t.RecallTypeNum != RecallTypes.PerioType && t.RecallTypeNum != RecallTypes.ProphyType)
                {
                    continue;
                }
    ...
    public static void BreakApptHelper(Appointment appointment, Patient patient, ProcedureCode procedureCode)
    {
        ...
        {
            suppressHistory = procedureCode.ProcCode is "D9986" or "D9987";
        }

        Appointments.SetAptStatus(appointment, ApptStatus.Broken, suppressHistory);
        if (appointment.AptStatus != ApptStatus.Complete)
        {
            SecurityLogs.MakeLogEntry(EnumPermType.AppointmentEdit, patient.PatNum, appointment.ProcDescript + ", " + appointment.AptDateTime + ", Broken from the Appts module.", appointment.AptNum, datePrevious);
        }
        else
        {
            SecurityLogs.MakeLogEntry(EnumPermType.AppointmentCompleteEdit, patient.PatNum, appointment.ProcDescript + ", " + appointment.AptDateTime + ", Broken from the Appts module.", appointment.AptNum, datePrevious);
        }
        ...
        if (HL7Defs.IsExistingHL7Enabled())
        {
            ...
            var messageHL7 = MessageConstructor.GenerateSIU(patient, Patients.GetPat(patient.Guarantor), EventTypeHL7.S15, appointment);
        ...
        var listPaySplitsForApptProcs = new List<PaySplit>();
        ...
        if (procedureCode != null)
        {
            ...
            procedureBroken.Note += appointment.AptDateTime.ToString("ddd") + " " + appointment.AptDateTime;
            procedureBroken.PlaceService = Clinics.GetPlaceService(procedureBroken.ClinicNum);
            var listInsSubs = InsSubs.RefreshForFam(Patients.GetFamily(patient.PatNum));
            ...
            double procFee;
            long feeSchedNumPat;
            var noBillIns = InsPlanPreferences.NoBillIns(procedureCode, insPlanPrimary);
            ...
            var listClaimProcs = ClaimProcs.RefreshForProc(procedureBroken.ProcNum);
            ...
            using var formProcBroken = new FormProcBroken(procedureBroken, isNonRefundable);
        ...
        if (PrefC.GetBool(PrefName.BrokenApptAdjustment))
        {
            ...
            using var formAdjust = new FormAdjust(patient, adjustment);
        ...
        if (PrefC.GetBool(PrefName.BrokenApptCommLog))
        {
            ...
            commLog.Note += appointment.AptDateTime.ToString("ddd") + " " + appointment.AptDateTime;
        ...
        if (isNonRefundable && !wasBrokenProcDeleted && listPaySplitsForApptProcs.Count > 0)
        {
            ...
            for (var p = 0; p < listProcedures.Count; p++)
            {
                ...
                var listPaySplits = listPaySplitsForApptProcs.FindAll(x => x.ProcNum == listProcedures[p].ProcNum);
                for (var s = 0; s < listPaySplits.Count; s++)
                {
                    if (amtBrokenProc == 0)
                    {
                        break;
                    ...
                    var splitAmt = Math.Min(amtBrokenProc, listPaySplits[s].SplitAmt);
                    Payments.CreateTransferForTpProcs(listProcedures[p], [listPaySplits[s]], procedureBroken, splitAmt);
                    var amtPaidOnApt = listPaySplitsForApptProcs.Sum(x => x.SplitAmt);
                    if (amtPaidOnApt > splitAmt)
                    {
                        ...
                        var remainingAmt = amtPaidOnApt - splitAmt;
                        ...
                        var incomeTransferData = PaymentEdit.IncomeTransferData.CreateTransfer(listPaySplits[s], true, remainingAmt);
    ...
    public static DateRange GetAsapRange(long operatoryNum, DateTime dateTimeSelected, long aptNum, List<Schedule> listSchedules)
    {
        ...
        var dateTimeSlotEnd = dateTimeSelected.Date.AddDays(1); //Midnight tomorrow
        //Get appoinments for the period of the entire day of dateTimeSelected
        var listAppointments = Appointments.GetAppointmentsForOpsByPeriod([operatoryNum], dateTimeSelected, dateTimeSelected);
        for (var i = 0; i < listAppointments.Count; i++)
        {
            ...
            var dateEndApt = appointment.AptDateTime.AddMinutes(appointment.Pattern.Length * 5);
        ...
        var listDefsBlockoutTypes = Defs.GetDefsForCategory(DefCat.BlockoutTypes);
        var listSchedulesBlockouts = Schedules.GetListForType(listSchedules, ScheduleType.Blockout, 0)
            .Where(x => x.SchedDate == dateTimeSelected.Date && x.Ops.Contains(operatoryNum)).ToList();
        for (var i = 0; i < listSchedulesBlockouts.Count; i++)
        {
            ...
            var dateTimeBlockoutStop = listSchedulesBlockouts[i].SchedDate.Add(listSchedulesBlockouts[i].StopTime);
            ...
            var dateTimeBlockoutStart = listSchedulesBlockouts[i].SchedDate.Add(listSchedulesBlockouts[i].StartTime);
    ...
    public static Appointment MakeNewAppointment(Patient patient, bool useApptDrawingSettings, DateTime? dateTNew = null, long? opNumNew = null)
    {
        ...
        var apptDateTime = DateTime.MinValue;
        long opNum = 0;
        if (dateTNew != null)
        {
        ...
        var appointment = Appointments.MakeNewAppointment(patient, apptDateTime, opNum, useApptDrawingSettings);
    ...
    public static Patient GetPatientMergePrompt(long patNum)
    {
        ...
        var listPatients = Patients.GetMultPats(listPatientLinks.Where(x => x.PatNumTo != patNum).Select(x => x.PatNumTo).ToList()).ToList();
        ...
        for (var i = 0; i < listPatients.Count; i++)
        {
            ...
            if (MsgBox.Show(MsgBoxButtons.YesNo,
                    ...
                    + " " + listPatients[i].GetNameLF() + " #" + listPatients[i].PatNum + "?"))
            {
    ...
    public static PlannedApptStatus CreatePlannedAppt(Patient patient, int itemOrder, List<long> listPreSelectedProcNums = null)
    {
        ...
        var patientMerged = GetPatientMergePrompt(patient.PatNum);
        if (patientMerged != null)
        {
            patient = patientMerged;
            GlobalFormOpenDental.PatientSelected(patient, isRefreshCurModule: true, isApptRefreshDataPat: false);
        }
        ...
        var listProcs = Procedures.GetManyProc(listPreSelectedProcNums, false); //Returns empty list if null.
        ...
        appointment.ItemOrderPlanned = itemOrder;
        Appointments.Insert(appointment);
        using var formApptEdit = new FormApptEdit(appointment.AptNum, listProcNumsPreSelected: listPreSelectedProcNums);
        ...
        {
            var listProcedures = Procedures.GetProcsForSingle(appointment.AptNum, true);
            var isHygiene = listProcedures.Count > 0 && listProcedures.Select(x => ProcedureCodes.GetProcCode(x.CodeNum)).ToList().All(x => x.IsHygiene);
        ...
        GlobalFormOpenDental.RefreshCurrentModule(isClinicRefresh: false); //if procs were added in appt, then this will display them
    ...
    public static bool OKtoSendToPinboard(ApptOther apptOther, List<ApptOther> listApptOthers, Control control)
    {
        if (apptOther.AptStatus == ApptStatus.Planned)
        {
            ...
            var PlannedIsSched = false;
            for (var i = 0; i < listApptOthers.Count; i++)
            {
                if (listApptOthers[i].NextAptNum == apptOther.AptNum)
                {
