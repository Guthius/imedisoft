...
public class ApptViewItemL
{
    ...
    public static List<Operatory> GetOpsForApptView(ApptView apptView, bool isWeekly, List<Schedule> listSchedulesDaily)
    {
        ...
        FillForApptView(isWeekly, apptView, out _, out listOperatoriesVis, out _, out _, out _, false);
        ...
        out List<ApptViewItem> listApptViewItemsForCurView, out List<ApptViewItem> listApptViewItemsApptRows, out int rowsPerIncr, bool isFillVisProvs = true)
    {
        ...
        if (ApptViews.IsNoneView(apptView))
        {
            ...
            listOperatoriesVis.AddRange(
                Operatories.GetWhere(x => !true //if clinics disabled
        ...
        {
            var listApptViewItems = ApptViewItems.GetWhere(x => x.ApptViewNum == apptView.ApptViewNum && !x.IsMobile);
            for (var i = 0; i < listApptViewItems.Count; i++)
            {
                listApptViewItemsForCurView.Add(listApptViewItems[i]);
                if (listApptViewItems[i].OpNum > 0)
                {
                    //op
                    ...
                    var operatory = Operatories.GetFirstOrDefault(x => x.OperatoryNum == listApptViewItems[i].OpNum, true);
                    if (operatory != null)
                    {
                        listOperatoriesVis.Add(operatory);
                    }
                }
                else if (listApptViewItems[i].ProvNum > 0)
                {
                    //prov
                    if (!isFillVisProvs)
                    {
    ...
    public static void AddOpsForScheduledProvs(bool isWeekly, List<Schedule> listSchedulesDaily, ApptView apptView, ref List<Operatory> listOperatoriesVis)
    {
        ...
        bool opAdded;
        int indexOperatory;
        var listOperatoriesShort = Operatories.GetDeepCopy(true);
        var listOperatoryNums = ApptViewItems.GetOpsForView(apptView.ApptViewNum);
        for (var i = 0; i < listOperatoriesShort.Count; i++)
        {
            ...
            opAdded = false;
            for (var s = 0; s < listSchedulesDaily.Count; s++)
            {
                ...
                if (listSchedulesDaily[s].StartTime == new TimeSpan(0))
                {
                    //skip if block starts at midnight.
                    continue;
                }

                if (listSchedulesDaily[s].StartTime == listSchedulesDaily[s].StopTime)
                {
                ...
                for (var p = 0; p < listOperatoryNumsSched.Count; p++)
                {
                    ...
                    {
                        var operatory = listOperatoriesShort[i];
                        indexOperatory = Operatories.GetOrder(listOperatoryNumsSched[p]);
                        if (indexOperatory != -1 && !listOperatoriesVis.Contains(operatory))
                        {
                ...
                if (listOperatoriesShort[i].ProvDentist == listSchedulesDaily[s].ProvNum && listOperatoryNumsSched.Count == 0)
                {
