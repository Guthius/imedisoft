...
namespace OpenDental {
	...
	public class PayPlanL {
		...
		public static int CompareMergedPayPlanRows(GridRow x,GridRow y) {
			DateTime dateTimeX=DateTime.Parse(x.Cells[0].Text);
			DateTime dateTimeY=DateTime.Parse(y.Cells[0].Text);
			DynamicPayPlanRowData dynamicPayPlanRowDataX=(DynamicPayPlanRowData)x.Tag;
			DynamicPayPlanRowData dynamicPayPlanRowDataY=(DynamicPayPlanRowData)y.Tag;
			if(dateTimeX<dateTimeY) {
				return -1;
			}
			else if(dateTimeX>dateTimeY) {
			...
			//Show charges before Payment on the same date.
			if(dynamicPayPlanRowDataX.IsChargeRow()) {//x is charge (Type.Equals doesn't seem to work in sorters for some reason)
				if(dynamicPayPlanRowDataY.IsPaymentRow()) {//y is credit, x goes first
					return -1;
				}
			}
			else {//x is credit
				if(dynamicPayPlanRowDataY.IsChargeRow()) {//y is charge
					return 1;
				}
				//x and y are both Payments
			}
		...
		public static GridRow CreateRowForPayPlanCharge(PayPlanCharge payPlanCharge,int payPlanChargeOrdinal,bool isDynamic=false) {
			...
			if(isDynamic && payPlanCharge.LinkType==PayPlanLinkType.Procedure) {
				Procedure procedure=Procedures.GetOneProc(payPlanCharge.FKey,false);
				if(procedure!=null) {
					if(payPlanCharge.ChargeDate==DateTime.MaxValue && (procedure.ProcStatus==ProcStat.TP || procedure.ProcStatus==ProcStat.TPi)) {
						descript="";
					}
					ProcedureCode procCode=ProcedureCodes.GetProcCodeFromDb(procedure.CodeNum);
					if(procCode!=null) {
		...
		public static void MakeSecLogEntries(PayPlan payPlan,PayPlan payPlanOld,bool hasSignatureChanged,bool isSigOldValid,bool isSigBlank,
			bool isSigValid, bool isPrinting=false) 
		{
		...
		public static List<GridRow> CreateRowsForDynamicPayPlanCharges(List<PayPlanCharge> listPayPlanCharges,DataTable tableBundledPayments,bool ungrouped) {
			...
			if(ungrouped) {//Charge rows ungrouped.
				...
				for(int i=0;i<listPayPlanCharges.Count;i++) {
					...
					if(listPayPlanCharges[i].LinkType==PayPlanLinkType.Procedure) {
						Procedure procedure=listProcedures.FirstOrDefault(x => x.ProcNum==listPayPlanCharges[i].FKey);
						if(procedure!=null) {
							if(listPayPlanCharges[i].ChargeDate==DateTime.MaxValue && (procedure.ProcStatus==ProcStat.TP || procedure.ProcStatus==ProcStat.TPi)) {
								descript="";
							}
							ProcedureCode procedureCode=listProcedureCodes.FirstOrDefault(x=> x.CodeNum==procedure.CodeNum);
			...
			List<DateTime> listDateTimesCharges=listPayPlanCharges.Select(x => x.ChargeDate).OrderBy(x=>x).Distinct().ToList();
			for(int i=0;i<listDateTimesCharges.Count;i++) {
				List<PayPlanCharge> listPayPlanChargesForDate=listPayPlanCharges.FindAll(x=>x.ChargeDate==listDateTimesCharges[i]);
	...
	public class DynamicPayPlanRowData {
		public List<long> ListPayPlanChargeNums { get => ListPayPlanCharges.Select(x=>x.PayPlanChargeNum).ToList(); }
		public long PayNum=0;
		public bool IsDownPayment=false;
