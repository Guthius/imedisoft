...
class AutoNoteL
{
    ...
    {
        var listDefNumsExpanded = new List<long>();
        ...
        {
            //either not fill on load or no user pref, store the expanded node state to restore after filling tree
        ...
        var treeNodeSelected = treeViewNotes.SelectedNode;
        TreeNode treeNodeTop = null;
        var topNodePath = treeViewNotes.TopNode?.FullPath;
        treeViewNotes.BeginUpdate();
        treeViewNotes.Nodes.Clear(); //clear current tree contents
        var listDefs = Defs.GetDefsForCategory(DefCat.AutoNoteCats, true);
        ...
        for (var i = 0; i < listDefs.Count; i++)
        {
            ...
            var treeNodeCat = CreateNodeAndChildren(listDefs[i].DefNum, listDefs, listAutoNotes, treeViewNotes);
            treeViewNotes.Nodes.Add(treeNodeCat);
        }

        //List<TreeNode> listTreeNodesOutsideCat = new List<TreeNode>();
        for (var i = 0; i < listAutoNotes.Count; i++)
        {
            if (listAutoNotes[i].Category != 0)
            {
                //because we only want the ones with no categories
                continue;
            }

            //add node to root
            var treeNodeAutoNote = new TreeNode(listAutoNotes[i].AutoNoteName, 1, 1);
        ...
        for (var i = 0; i < treeViewNotes.Nodes.Count; i++)
        {
            listTreeNodes.Add(treeViewNotes.Nodes[i]);
        }

        var listTreeNodesFlat = listTreeNodes.SelectMany(x => GetNodeAndChildren(x)).ToList(); //get flat list of all nodes, copy entire tree
        for (var i = 0; i < listTreeNodesFlat.Count; i++)
        {
    ...
    private static TreeNode CreateNodeAndChildren(long defNumCat, List<Def> listDefs, List<AutoNote> listAutoNotes, TreeView treeViewNotes)
    {
        ...
        var def = listDefs.Find(x => x.DefNum == defNumCat);
        var treeNodeCat = new TreeNode(def.ItemName, 0, 0);
        treeNodeCat.Tag = def;
        //Make a list of the child cats
        var listDefsChildren = listDefs.FindAll(x => x.ItemValue == defNumCat.ToString());
        //Add child categories recursively:
        for (var i = 0; i < listDefsChildren.Count; i++)
        {
            var treeNode = CreateNodeAndChildren(listDefsChildren[i].DefNum, listDefs, listAutoNotes, treeViewNotes);
            treeNodeCat.Nodes.Add(treeNode);
        }

        //Add AutoNotes for current cat
        var listAutoNotesForCat = listAutoNotes.FindAll(x => x.Category == defNumCat);
        for (var i = 0; i < listAutoNotesForCat.Count; i++)
        {
            var treeNodeAutoNote = new TreeNode(listAutoNotesForCat[i].AutoNoteName, 1, 1);
    ...
    {
        var listTreeNodesRetVal = new List<TreeNode>();
        ...
        for (var i = 0; i < listTreeNodesRetVal.Count; i++)
        {
    ...
    {
        var treeNodeTop = treeView.TopNode;
        var treeNodeSelected = treeView.SelectedNode;
        treeView.BeginUpdate();
        if (doSetChecked)
        {
            while (treeNodeTop.Parent != null)
            {
                //store the topNode's root to set the topNode after collapsing all nodes
                treeNodeTop = treeNodeTop.Parent;
            }

            while (treeNodeSelected != null && treeNodeSelected.Parent != null)
            {
