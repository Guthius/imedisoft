...
public class FeeL
{
    ...
    public static void ImportFees(string fileName, long feeSchedNum, long clinicNum, long provNum, DateTime dateEffective = new DateTime())
    {
        var listFees = Fees.GetListExact(feeSchedNum, clinicNum, provNum, dateEffective);
        var listFeesAdded = new List<Fee>();
        string[] fields;
        var counter = 0;
        var lineCount = File.ReadAllLines(fileName).Length; //quick and dirty
        ...
        while (line != null)
        {
            ...
            var codeNum = ProcedureCodes.GetCodeNum(fields[0]);
            if (codeNum == 0)
            {
                line = streamReader.ReadLine();
                continue;
            ...
            var feeFound = listFeesAdded.Find(x => x.CodeNum == codeNum);
            if (feeFound != null)
            {
                //if we have added this fee from the file before we will skip it
                line = streamReader.ReadLine();
                continue;
            }

            var fee = Fees.GetFee(codeNum, feeSchedNum, clinicNum, provNum, listFees, dateEffective);
            var feeOldStr = "";
            ...
            var percent = (double) counter * 100d / (double) lineCount;
            counter++;
    ...
    public static void SaveAllowedFeesFromClaimPayment(List<ClaimProc> listClaimProcs, List<InsPlan> listInsPlans)
    {
        ...
        var insPlan = InsPlans.GetPlan(listClaimProcs[0].PlanNum, listInsPlans);
        var allowedFeeSchedNum = insPlan.AllowedFeeSched;
        if (allowedFeeSchedNum == 0)
        {
        ...
        var allowedFeesEntered = listClaimProcs.Any(x => x.AllowedOverride != -1);
        ...
        for (var i = 0; i < listClaimProcs.Count; i++)
        {
            ...
            var procedure = Procedures.GetProcFromList(listProcedures, listClaimProcs[i].ProcNum);
            ...
            var fee = Fees.GetFee(codeNum, allowedFeeSchedNum, procedure.ClinicNum, procedure.ProvNum);
            var datePrevious = DateTime.MinValue;
            if (fee == null)
            {
                fee = new Fee();
