...
public class PatFieldL
{
    ...
    public static void AddPatFieldsToGrid(GridOD grid, List<PatField> listPatFields, FieldLocations fieldLocation)
    {
        ...
        for (var i = 0; i < listPatFieldDefs.Count; i++)
        {
            ...
            var row = new GridRow();
            var patField = listPatFields.FirstOrDefault(x => x.FieldName == listPatFieldDefs[i].FieldName);
        ...
        var listPatFieldsFiltered = listPatFields.Where(x => !listPatFieldDefs.Exists(y => y.FieldName == x.FieldName)).ToList();
        for (var i = 0; i < listPatFieldsFiltered.Count; i++)
        {
            var row = new GridRow();
    ...
    public static void OpenPatField(PatField patField, PatFieldDef patFieldDef, long patNum, bool isForOrtho = false)
    {
        ...
        if (patField == null)
        {
            ...
            {
                patField.IsNew = true;
                using var formPatFieldPickEdit = new FormPatFieldPickEdit(patField);
                formPatFieldPickEdit.ShowDialog();
            }

            if (patFieldDef.FieldType == PatFieldType.Date)
            {
                using var formPatFieldDateEdit = new FormPatFieldDateEdit(patField);
            ...
            {
                using var formPatFieldCheckEdit = new FormPatFieldCheckEdit(patField);
            ...
            {
                using var formPatFieldCurrencyEdit = new FormPatFieldCurrencyEdit(patField);
