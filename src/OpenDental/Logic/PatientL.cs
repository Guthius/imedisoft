...
public class PatientL
{
    ...
    private static List<Patient> _listPatientsLastFive = [];
    ...
    public static void RemoveFromMenu(long patNum)
    {
        lock (_lock)
        {
            //Lock the list before removal to prevent concurrency issues.
    ...
    public static void RemoveAllFromMenu(ContextMenu contextMenu)
    {
        ...
        lock (_lock)
        {
            //Lock the list before clear to prevent concurrency issues.
    ...
    public static void AddFamilyToMenu(ContextMenu contextMenu, EventHandler eventHandlerOnClick, long patNum, Family family)
    {
        ...
        for (var i = 0; i < _listPatientsLastFive.Count; i++)
        {
            var name = _listPatientsLastFive[i].GetNameLF();
    ...
    public static bool AddPatsToMenu(Patient patient)
    {
        //add current patient
        if (patient.PatNum == 0)
        {
        ...
        if (_listPatientsLastFive.Count > 0 && patient.PatNum == _listPatientsLastFive[0].PatNum)
        {
        ...
        //Patient has changed
        var idx = _listPatientsLastFive.FindIndex(x => x.PatNum == patient.PatNum);
        lock (_lock)
        {
            //Lock the list before removal and/or insertion to prevent concurrency issues.
            if (idx > -1)
            {
    ...
    {
        var index = contextMenu.MenuItems.IndexOf((MenuItem) sender);
    ...
    {
        var retVal = PrefC.GetString(PrefName.MainWindowTitle);
    ...
    public static string GetMainTitleSamePat()
    {
        ...
        if (PrefC.GetLong(PrefName.ShowIDinTitleBar) == 1)
        {
            retVal += " - " + _patientSelected.PatNum;
        }
        else if (PrefC.GetLong(PrefName.ShowIDinTitleBar) == 2)
        {
    ...
    {
        string retVal;
        var timeSpanLeft = PrefC.GetDateT(PrefName.UpdateDateTime) - DateTime.Now;
        var strTimeLeft = "";
