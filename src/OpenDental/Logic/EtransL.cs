...
public class EtransL
{
    ...
    {
        var sheet = SheetUtil.CreateSheet(SheetDefs.GetInternalOrCustom(SheetInternalType.ERA));
        var x835Copy = x835.Copy();
    ...
    {
        var listEtrans835Attaches = Etrans835Attaches.GetForClaimNums(listClaimNums.ToArray());
        var listEtranss = Etranss.GetMany(listEtrans835Attaches.Select(x => x.EtransNum).ToArray());
    ...
    {
        var listAttaches = Etrans835Attaches.GetForClaimNums(claim.ClaimNum);
    ...
    private static void ViewEra(List<Etrans> listEtrans, List<Etrans835Attach> listEtrans835Attaches, long claimNumSpecific = 0)
    {
        ...
        if (listEtrans.Count == 1)
        {
            //This also takes care of older 835s such that there are multiple 835s in 1 X12 msg but only 1 etrans row.
        ...
        var formEtrans835PickEra = new FormEtrans835PickEra(listEtrans, claimNumSpecific);
    ...
    {
        var messageText835 = EtransMessageTexts.GetMessageText(etrans.EtransMessageTextNum);
        var x12Object835 = new X12object(messageText835);
        var listTranSetIds = x12Object835.GetTranSetIds();
        ...
        var formEtrans835Edit = new FormEtrans835Edit(specificClaimNum);
        ...
        bool areAllClinicsSelected, List<X835AutoProcessed> listX835AutoProcessedsStatuses = null, bool includeAcknowledged = true)
    {
        ...
        if (showStatusAndClinics && true && !areAllClinicsSelected)
        {
            //Filter by selected clinics if applicable.
            ...
            var listEtransNums = eraData.ListAttached.Select(x => x.EtransNum).Distinct().ToList();
    ...
    {
        var listCarriersForAllEtrans = Carriers.GetCarriers(listEtrans835Attaches.Select(x => x.CarrierNum).Distinct().ToList());
        for (var i = listEtranss.Count - 1; i >= 0; i--)
        {
            var listCarrierNumsForEtrans = listEtrans835Attaches.FindAll(x => x.EtransNum == listEtranss[i].EtransNum).Select(x => x.CarrierNum).ToList();
            var listCarriersForEtrans = listCarriersForAllEtrans.FindAll(x => listCarrierNumsForEtrans.Contains(x.CarrierNum));
            var etrans835 = listEtrans835sFiltered.FirstOrDefault(x => x.EtransNum == listEtranss[i].EtransNum);
            var isEtransAutomatable = Etranss.IsEtransAutomatable(listCarriersForEtrans, etrans835.PayerName, isFullyAutomatic: false);
    ...
    {
        var listEtransNumsMissingEtrans835s = Etranss.GetErasMissingEtrans835(dateFrom, dateTo);
        var listEtransMissingEtrans835s = Etranss.GetMany(listEtransNumsMissingEtrans835s.ToArray());
        ...
        for (var i = 0; i < listEtransMissingEtrans835s.Count; i++)
        {
            ...
            var messageText835 = EtransMessageTexts.GetMessageText(listEtransMissingEtrans835s[i].EtransMessageTextNum);
            var listEtrans835Attaches = Etrans835Attaches.GetForEtrans(listEtransMissingEtrans835s[i].EtransNum);
            var x835 = new X835(listEtransMissingEtrans835s[i], messageText835, listEtransMissingEtrans835s[i].TranSetId835, listEtrans835Attaches);
    ...
    public static void GoToAccountForHx835_Claim(Hx835_Claim hx835_ClaimPaid)
    {
        ...
        {
            var claim = Claims.GetClaim(hx835_ClaimPaid.ClaimNum);
            if (claim != null)
            {
                GlobalFormOpenDental.GoToModule(EnumModuleType.Account, patNum: claim.PatNum);
                return;
            }
        }

        var ptTableSearchParams = new PtTableSearchParams(false, hx835_ClaimPaid.PatientName.Lname, hx835_ClaimPaid.PatientName.Fname, "", "", false,
            "", "", "", "", "", 0, false, false, DateTime.MinValue, 0, "", "", "", "", "", "", "");
        var tablePt = Patients.GetPtDataTable(ptTableSearchParams); //Mimics FormPatientSelect.cs
    ...
    {
        var x835Status = x835.GetStatus(x835.GetClaimDataList(listHx835_ShortClaims), listHx835_ShortClaimProcs, listEtrans835Attaches);
        switch (x835Status)
        {
    ...
    {
        var isSupplementalPay = claim.ClaimStatus == "R" || listClaimProcsForClaim.All(x => x.Status.In(
        ...
        if (claim.ClaimType != "PreAuth" && claim.ClaimType != "Cap")
        {
            //By definition, capitation insurance pays in one lump-sum, not over an extended period of time.
            ...
            var listPayPlans = PayPlans.GetValidInsPayPlans(claim.PatNum, claim.PlanNum, claim.InsSubNum, claim.ClaimNum);
        ...
        var listClaimProcsForClaimModified = listClaimProcsForClaim.Select(x => x.Copy()).ToList();
        Etranss.TryImportEraClaimData(hx835_ClaimPaid, claim, patient, isAutomatic: false, listClaimProcsForClaimModified, insPayPlanNum);
        ...
        using var formEtrans835ClaimPay = new FormEtrans835ClaimPay(x835, hx835_ClaimPaid, claim, patient, family, listInsPlans, listPatPlans, listInsSubs, isSupplementalPay);
        formEtrans835ClaimPay.ListClaimProcs = listClaimProcsForClaimModified;
        if (formEtrans835ClaimPay.ShowDialog() != DialogResult.OK)
        {
            //Modal because this window can edit information
            ...
            var listClaimProcNumsForClaim = listClaimProcsForClaim.Select(x => x.ClaimProcNum).ToList();
            //Any claimprocs in the modified list that were not in the original list, were inserted by TryImportEraClaimData() above.
            var listClaimProcsToDelete = listClaimProcsForClaimModified.FindAll(x => !listClaimProcNumsForClaim.Contains(x.ClaimProcNum));
    ...
    {
        var listHx835_ClaimsSkippedPreauths = x835.ListClaimsPaid.FindAll(x => x.IsPreauth && !x.IsAttachedToClaim);
        ...
        var listClaims = x835.GetClaimsForFinalization();
        ...
        if (listClaims.Exists(x => x.ClaimNum == 0 || x.ClaimStatus != "R"))
        {
            ...
            var patNumColumnLength = 6; //Minimum of 6 because that is the width of the column name "PatNum"
            if (listClaims.Exists(x => x.ClaimNum != 0))
            {
                //There are claims that were found in DB, need to consider claim.PatNum.ToString() lengths.
            ...
            var listPatNums = listClaims.Select(x => x.PatNum).Distinct().ToList();
            var listPatientsLims = Patients.GetLimForPats(listPatNums);
            var maxLen = 0;
            //To determine what the longest patient name is, loop through the list and set maxLen to the longest name.
            for (var i = 0; i < listPatientsLims.Count; i++)
            {
                var strName = Patients.GetNameLF(listPatientsLims[i]);
            ...
            var maxNamesLength = Math.Max(7, maxLen); //Minimum of 7 to account for column title width "Patient".
            var maxX835NameLength = 0;
            if (listClaims.Exists(x => x.ClaimNum == 0))
            {
                //There is a claim that could not be found in the DB. Must consider Hx835_Claims.PatientName lengths.
                maxX835NameLength = listClaims
                    .Where(x => x.ClaimNum == 0)
                    .Max(x => ((Hx835_Claim) x.TagOD).PatientName.ToString().Length);
            }

            var maxColumnLength = Math.Max(maxNamesLength, maxX835NameLength);

            #endregion

            #region Construct msg

            var msg = "One or more claims are not received.\r\n"
            ...
            {
                if (listClaims[i].ClaimNum == 0)
                {
                    //Current claim was not found in DB and was not detached, so we will use the Hx835_Claim object to get name.
                    var hx835_Claim = (Hx835_Claim) listClaims[i].TagOD;
                ...
                var claim = listClaims[i];
                if (claim.ClaimStatus == "R")
                {
                    continue;
                }

                var name = listPatientsLims.Find(x => x.PatNum == claim.PatNum).GetNameLF().PadRight(maxColumnLength); //why the padRight?
        ...
        var listClaimProcsAll = ClaimProcs.RefreshForClaims(listClaims.Where(x => x.ClaimNum != 0).Select(x => x.ClaimNum).ToList());
        if (listClaimProcsAll.Exists(x => !x.Status.In(ClaimProcStatus.Received, ClaimProcStatus.Supplemental, ClaimProcStatus.CapClaim)))
        {
            var patNumColumnLength = Math.Max(6, listClaimProcsAll.Max(x => x.PatNum.ToString().Length)); //PatNum column length
            var listPatNums = listClaims.Select(x => x.PatNum).Distinct().ToList();
            var listPatientsLims = Patients.GetLimForPats(listPatNums);
            var maxNamesLength = 0;
            //To determine what the longest patient name is, loop through the list and set maxLen to the longest name.
            for (var i = 0; i < listPatientsLims.Count; i++)
            {
                var strName = Patients.GetNameLF(listPatientsLims[i]);
            ...
            var msg = "One or more claim procedures are set to the wrong status and are not ready to be finalized.\r\n"
            ...
            for (var i = 0; i < listClaims.Count; i++)
            {
                var listClaimProcs = listClaimProcsAll.FindAll(x => x.ClaimNum == listClaims[i].ClaimNum);
                if (listClaimProcs.All(x => x.Status.In(ClaimProcStatus.Received, ClaimProcStatus.Supplemental, ClaimProcStatus.CapClaim)))
                {
                    continue;
                }

                var name = listPatientsLims.Find(x => x.PatNum == listClaims[i].PatNum).GetNameLF().PadRight(maxNamesLength);
        ...
        var patient = Patients.GetPat(listClaims[0].PatNum);
    ...
    {
        var importInsurancePlansReturnData = new ImportInsurancePlansReturnData();
        var rowIndex = 1;
        importInsurancePlansReturnData.stringBuilderErrorMessages = new StringBuilder();
        var listImportedSegments = new List<int>(); //Used to reconstruct the 834 with imported patients removed.
        for (var i = 0; i < x834.ListTransactions.Count; i++)
        {
            var hx834_Tran = x834.ListTransactions[i];
            for (var j = 0; j < hx834_Tran.ListMembers.Count; j++)
            {
                var hx834_Member = hx834_Tran.ListMembers[j];
                actionShowStatus?.Invoke(rowIndex, hx834_Member.Pat);
                Employer employerLocal = null;
                if (doCreateEmployer)
                {
                    var hx834_EmployerImport = hx834_Member.ListMemberEmployers.FirstOrDefault();
                    if (hx834_EmployerImport != null && hx834_EmployerImport.MemberEmployer != null && !hx834_EmployerImport.MemberEmployer.GetNameLF().IsNullOrEmpty())
                    {
                        ...
                        var employerName = hx834_EmployerImport.MemberEmployer.GetNameLF();
                        var listEmployersSimilar = Employers.GetAllByName(employerName);
                ...
                if (hx834_Member.Pat.PatNum == 0)
                {
                    ...
                    var listPatientFor834ImportsMatches = PatientFor834Import.GetPatientLimitedsByNameAndBirthday(hx834_Member.Pat, listPatientsFor834ImportsLimited);
                ...
                else if (hx834_Member.Pat.PatNum == 0 && doCreatePatient)
                {
                    ...
                    Patients.Insert(hx834_Member.Pat);
                    var patientOldMember = hx834_Member.Pat.Copy();
                    ...
                    var patientFor834ImportLimited = new PatientFor834Import(hx834_Member.Pat);
                    var patIdx = listPatientsFor834ImportsLimited.BinarySearch(patientFor834ImportLimited); //Preserve sort order by locating the index in which to insert the newly added patient.
                    var insertIdx = ~patIdx; //According to MSDN, the index returned by BinarySearch() is a "bitwise compliment", since not currently in list.
                ...
                {
                    //member.Pat.PatNum!=0
                    var patientFor834ImportDb = listPatientsFor834ImportsLimited.FirstOrDefault(x => x.PatNum == hx834_Member.Pat.PatNum); //Locate by PatNum, in case user selected manually.
                    ...
                    if (patientFor834ImportDb != null)
                    {
                        var patientDB = Patients.GetPat(patientFor834ImportDb.PatNum);
                        ...
                        var patIdx = listPatientsFor834ImportsLimited.BinarySearch(patientFor834ImportDb); //Preserve sort order by locating the index in which to insert the newly added patient.
                        ...
                        var insertIdx = patIdx >= 0 ? patIdx : ~patIdx;
                ...
                if (isMemberImported)
                {
                    ...
                    var listPatPlansNumsToRemove = doDropExistingInsurance ? PatPlans.Refresh(hx834_Member.Pat.PatNum) : [];
                    //Import insurance changes for patient.
                    for (var k = 0; k < hx834_Member.ListHealthCoverage.Count; k++)
                    {
                        var hx834_HealthCoverage = hx834_Member.ListHealthCoverage[k];
                        if (k > 0)
                        {
                            rowIndex++;
                        }

                        var listCarriers = Carriers.GetByNameAndTin(hx834_Tran.Payer.Name, hx834_Tran.Payer.IdentificationCode);
                        ...
                        //ins plan.
                        var is834ExplicitlyDropping = hx834_HealthCoverage.HealthCoverage.MaintenanceTypeCode == "002";
                        //The insPlanNew will only be inserted if necessary.  Created temporarily in order to determine if insert is needed.
                        var insPlanNew = InsertOrUpdateInsPlan(null, hx834_Member, listCarriers[0], employerLocal, false);
                        ...
                        var listInsPlans = InsPlans.GetAllByCarrierNum(listCarriers[0].CarrierNum);
                        InsPlan insPlanMatch = null;
                        for (var p = 0; p < listInsPlans.Count; p++)
                        {
                        ...
                        var family = Patients.GetFamily(hx834_Member.Pat.PatNum);
                        var listInsSubs = InsSubs.RefreshForFam(family);
                        var listPatPlans = PatPlans.Refresh(hx834_Member.Pat.PatNum);
                        var listInsPlansUpdated = new List<InsPlan>();
                        var listInsSubsUpdated = new List<InsSub>();
                        var listPatPlansUpdated = new List<PatPlan>();
                        ...
                        var listInsPlansForSubs = InsPlans.GetByInsSubs(listInsSubs.Select(x => x.InsSubNum).ToList());
                        for (var p = 0; p < listInsSubs.Count; p++)
                        {
                            var insSub = listInsSubs[p];
                            var carrier = Carriers.GetCarrier(listInsPlansForSubs.First(x => x.PlanNum == insSub.PlanNum).CarrierNum);
                        ...
                        {
                            //Not dropping plan per 834. Update or Insert the plan.
                            var isInsPlanUpdate = insPlanMatch != null; //InsPlan actually exists, so the plan itself is just updating.
                            ...
                            var isInsSubUpdate = insSubMatch != null; //InsSub actually exists, so the ins sub itself is just updating.
                            ...
                            var isPatPlanUpdate = patPlanMatch != null; //PatPlan actually exists, so the pat plan itself is just updating.
                    ...
                    {
                        var listInsSubsForLog = InsSubs.GetMany(listPatPlansNumsToRemove.Select(x => x.InsSubNum).ToList());
                        var listInsPlansForLog = InsPlans.GetPlans(listInsSubsForLog.Select(x => x.PlanNum).ToList());
                        for (var p = 0; p < listPatPlansNumsToRemove.Count; p++)
                        {
                            var insSub = listInsSubsForLog.FirstOrDefault(x => x.InsSubNum == listPatPlansNumsToRemove[p].InsSubNum);
                            var insPlan = listInsPlansForLog.FirstOrDefault(x => x.PlanNum == insSub.PlanNum);
                    ...
                    {
                        var x12Segment = x834.GetNextSegmentById(hx834_Member.MemberLevelDetail.SegmentIndex + 1, "SE"); //SE segment is required.
                        endSegIndex = x12Segment.SegmentIndex - 1;
                    }

                    for (var s = hx834_Member.MemberLevelDetail.SegmentIndex; s <= endSegIndex; s++)
                    {
        ...
        if (listImportedSegments.Count > 0 && importInsurancePlansReturnData.skippedPatsCount > 0)
        {
            //Some patients imported, while others did not.
        ...
        else if (listImportedSegments.Count > 0)
        {
            //All patinets imported.
            MoveFileToArchiveFolder(x834);
        }
        else if (importInsurancePlansReturnData.skippedPatsCount > 0)
        {
            //No patients imported.  All patients were skipped.
    ...
    {
        var dir = Path.GetDirectoryName(x834.FilePath);
        var dirArchive = ODFileUtils.CombinePaths(dir, "Archive");
        ...
        var destPathBasic = ODFileUtils.CombinePaths(dirArchive, Path.GetFileName(x834.FilePath));
        var destPathExt = Path.GetExtension(destPathBasic);
        var destPathBasicRoot = destPathBasic.Substring(0, destPathBasic.Length - destPathExt.Length);
        var destPath = destPathBasic;
        var attemptCount = 1;
    ...
    private static InsPlan InsertOrUpdateInsPlan(InsPlan insPlan, Hx834_Member hx834_Member, Carrier carrier, Employer employerLocal, bool isInsertAllowed = true, bool doCreateEmployer = false)
    {
        ...
        if (insPlan != null)
        {
            ...
            if (InsPlanCrud.UpdateComparison(insPlan, insPlanOld))
            {
    ...
    private static InsSub InsertOrUpdateInsSub(InsSub insSub, InsPlan insPlan, Hx834_Member hx834_Member, Hx834_HealthCoverage hx834_HealthCoverage, Carrier carrier)
    {
        ...
        var insSubOld = insSub.Copy();
        insSub.DateEffective = hx834_HealthCoverage.DateEffective;
        insSub.DateTerm = hx834_HealthCoverage.DateTerm;
        insSub.ReleaseInfo = hx834_Member.IsReleaseInfo;
        if (InsSubCrud.UpdateComparison(insSub, insSubOld))
        {
        ...
        Carrier carrier, List<PatPlan> listPatPlansOther)
    {
        if (patPlan != null)
        {
            var patPlanOld = patPlan.Copy();
            patPlan.Relationship = hx834_Member.PlanRelat;
            if (PatPlanCrud.UpdateComparison(patPlan, patPlanOld))
            {
        ...
        patPlan = new PatPlan();
        patPlan.Ordinal = 0;
        for (var p = 0; p < listPatPlansOther.Count; p++)
        {
