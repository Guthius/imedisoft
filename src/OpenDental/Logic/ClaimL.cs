...
public class ClaimL
{
    ...
    {
        var errorMessage = Clearinghouses.CheckClearinghouseDefaults();
    ...
    {
        var dataRows = dataTable.Select().ToList();

        var hasSelections = arraySelectedIndices.Length > 0;
        if (hasSelections)
        {
            dataRows = dataRows.Where((_, index) => index.In(arraySelectedIndices)).ToList();
        }
        
        return dataRows
            .Select(x => new CreateClaimItem
            {
    ...
    public static bool WarnUsersForDuplicateClaimProcs(CreateClaimDataWrapper createClaimDataWrapper, bool isVerbose = true)
    {
        ...
        var listClaimProcsAvailable = createClaimDataWrapper.CreateClaimData_.ListClaimProcs
        ...
        var listCreateClaimItemsSelected = createClaimDataWrapper.ListCreateClaimItems.FindAll(x => x.IsSelected);
        for (var i = 0; i < listCreateClaimItemsSelected.Count; i++)
        {
            var listClaimProcs = ClaimProcs.GetForProc(listClaimProcsAvailable, listCreateClaimItemsSelected[i].ProcNum);
        ...
        {
            var message = "Procedures with multiple claimprocs for the same insurance found: " + listProcNums.Count;
            for (var i = 0; i < listProcNums.Count; i++)
            {
                var procedure = Procedures.GetProcFromList(createClaimDataWrapper.CreateClaimData_.ListProcs, listProcNums[i]);
                message += "\r\n  " + Procedures.ConvertProcToString(procedure.CodeNum, procedure.Surf, procedure.ToothNum, true);
            }

            message += "\r\n\r\nThe above procedures need to have the duplicate claimprocs deleted before creating a claim.";
    ...
    public static CreateClaimDataWrapper GetCreateClaimDataWrapper(Patient patient, Family family, List<CreateClaimItem> listCreateClaimItems, bool isVerbose, bool isSelectionRequired = false)
    {
        ...
        if (listCreateClaimItems == null)
        {
            //An engineer incorrectly called this method.
        ...
        if (listCreateClaimItems.All(x => !x.IsSelected))
        {
            ...
            for (var i = 0; i < listCreateClaimItems.Count; i++)
            {
                ...
                var procedure = Procedures.GetProcFromList(createClaimDataWrapper.CreateClaimData_.ListProcs, listCreateClaimItems[i].ProcNum);
                var procedureCode = ProcedureCodes.GetFirstOrDefault(x => x.CodeNum == procedure.CodeNum) ?? new ProcedureCode();
                if (procedureCode.IsCanadianLab)
                {
                    continue;
                }

                var ordinal = PatPlans.GetOrdinal(PriSecMed.Primary, createClaimDataWrapper.CreateClaimData_.ListPatPlans
                    , createClaimDataWrapper.CreateClaimData_.ListInsPlans, createClaimDataWrapper.CreateClaimData_.ListInsSubs);
                var ordinalSec = PatPlans.GetOrdinal(PriSecMed.Secondary, createClaimDataWrapper.CreateClaimData_.ListPatPlans
                    , createClaimDataWrapper.CreateClaimData_.ListInsPlans, createClaimDataWrapper.CreateClaimData_.ListInsSubs);
                if (ordinal == 0)
                {
    ...
    public static CreateClaimDataWrapper CreateClaimFromWrapper(bool isVerbose, CreateClaimDataWrapper createClaimDataWrapper, bool hasPrimaryClaim = false, bool hasSecondaryClaim = false, bool hasTertiaryClaim = false)
    {
        createClaimDataWrapper.ErrorMessage = "";
        if (!Security.IsAuthorized(EnumPermType.ClaimView, true))
        {
        ...
        if (!hasPrimaryClaim)
        {
            ...
            claimLeadOrdinal = new Claim();
            claimLeadOrdinal.DateSent = DateTime.Today;
            claimLeadOrdinal.DateSentOrig = DateTime.MinValue;
            claimLeadOrdinal.ClaimStatus = "W";
            //Set claimPrimary to what CreateClaim returns because the reference to claimPrimary gets broken when inserting.
            claimLeadOrdinal = CreateClaim(claimLeadOrdinal, claimType, isVerbose, createClaimDataWrapper, "Primary Claim Error");
            if (claimLeadOrdinal.ClaimNum == 0)
            {
                createClaimDataWrapper.ShouldRefresh = true;
                return createClaimDataWrapper;
            }

            if (isVerbose && claimLeadOrdinal.ClaimNum != 0)
            {
                //Only provide the user with the option to cancel the claim if attempting to create a single claim manually.
                using var formClaimEdit = new FormClaimEdit(claimLeadOrdinal, createClaimDataWrapper.Patient_, createClaimDataWrapper.Family_);
                ...
                var unearnedAmount = (double) PaySplits.GetTotalAmountOfUnearnedForPats(createClaimDataWrapper.Family_.GetPatNums());
                AllocateUnearnedPayment(createClaimDataWrapper.Patient_, createClaimDataWrapper.Family_, unearnedAmount, claimLeadOrdinal);
            }
            else if (claimLeadOrdinal.ClaimNum != 0)
            {
                //isVerbose is false, still need to log.
                var patient = createClaimDataWrapper.Patient_;
                SecurityLogs.MakeLogEntry(EnumPermType.ClaimEdit, patient.PatNum, "New claim created for " + patient.LName + "," + patient.FName,
                    claimLeadOrdinal.ClaimNum, claimLeadOrdinal.SecDateTEdit);
            }
        }

        Claim claimNextOrdinal = null;
        var doCreateSecondaryClaim = !hasSecondaryClaim && PatPlans.GetOrdinal(PriSecMed.Secondary, createClaimDataWrapper.CreateClaimData_.ListPatPlans,
        ...
        if (doCreateSecondaryClaim)
        {
            ...
            claimNextOrdinal = new Claim();
            claimNextOrdinal.ClaimStatus = "H";
            //Set claimSecondary to CreateClaim because the reference to claimSecondary gets broken when inserting.
            claimNextOrdinal = CreateClaim(claimNextOrdinal, "S", isVerbose, createClaimDataWrapper, "Secondary Claim Error");
            if (claimNextOrdinal.ClaimNum == 0)
            {
                createClaimDataWrapper.ShouldRefresh = true;
                return createClaimDataWrapper;
            }
            ...
            var patient = createClaimDataWrapper.Patient_;
            SecurityLogs.MakeLogEntry(EnumPermType.ClaimEdit, patient.PatNum, "New claim created for " + patient.LName + "," + patient.FName,
                claimNextOrdinal.ClaimNum, claimNextOrdinal.SecDateTEdit);
        }

        var doCreateTertiaryClaim = !hasTertiaryClaim && PatPlans.GetOrdinal(PriSecMed.Tertiary, createClaimDataWrapper.CreateClaimData_.ListPatPlans,
                                                          createClaimDataWrapper.CreateClaimData_.ListInsPlans, createClaimDataWrapper.CreateClaimData_.ListInsSubs) > 0 //if there exists a tertiary plan
                                                      && !CultureInfo.CurrentCulture.Name.EndsWith("CA"); //And not Canada (don't create tertiary claim for Canada)
        if (doCreateTertiaryClaim)
        {
            //ClaimL.CalculateAndUpdate() could have added new claimprocs for additional insurance plans if they were added after the proc was completed
            createClaimDataWrapper.CreateClaimData_.ListClaimProcs = ClaimProcs.Refresh(createClaimDataWrapper.Patient_.PatNum);
            var claimTertiary = new Claim();
            claimTertiary.ClaimStatus = "U"; //Must be "U" (Unsent) to exactly mimic the value used when manually creating tertiary claims.
            //Set claimTertiary to CreateClaim because the reference to claimTertiary gets broken when inserting.
            claimTertiary = CreateClaim(claimTertiary, "Tertiary", isVerbose, createClaimDataWrapper, "Tertiary Claim Error");
            if (claimTertiary.ClaimNum == 0)
            {
                createClaimDataWrapper.ShouldRefresh = true;
                return createClaimDataWrapper;
            }
            ...
            var patient = createClaimDataWrapper.Patient_;
            SecurityLogs.MakeLogEntry(EnumPermType.ClaimEdit, patient.PatNum, "New tertiary claim created for " + patient.LName + "," + patient.FName,
                claimTertiary.ClaimNum, claimTertiary.SecDateTEdit);
        }

        if (claimLeadOrdinal != null)
        {
            claimLeadOrdinal = Claims.GetClaim(claimLeadOrdinal.ClaimNum);
            if (claimNextOrdinal != null
                && claimNextOrdinal.ClaimStatus.In("H", "U")
                && claimLeadOrdinal.ClaimStatus == "R"
                && Security.IsAuthorized(EnumPermType.ClaimSend, suppressMessage: true))
            {
                var listClaimProcs = createClaimDataWrapper.CreateClaimData_.ListClaimProcs.FindAll(x => x.ClaimNum == claimLeadOrdinal.ClaimNum);
                ...
                if (PrefC.GetBool(PrefName.ClaimMedReceivedPromptForPrimaryClaim) && claimLeadOrdinal.MedType.In(EnumClaimMedType.Medical, EnumClaimMedType.Institutional))
                {
                    PromptForPrimaryOrSecondaryClaim(listClaimProcs, false); //Check for unsent primary claimprocs.
                }
                else if (PrefC.GetBool(PrefName.PromptForSecondaryClaim))
                {
                    //Check for unsent secondary claimprocs.
                    PromptForPrimaryOrSecondaryClaim(listClaimProcs);
                }
            }
        }
    ...
    public static void AllocateUnearnedPayment(Patient patient, Family family, double unearnedAmt, Claim claim)
    {
        ...
        using var formProcSelect = new FormProcSelect(patient.PatNum, true, doShowAdjustments: true, doShowTreatmentPlanProcs: false);
    ...
    {
        var listCreateClaimItemsToolBarInsSelectedItems = createClaimDataWrapper.ListCreateClaimItems.FindAll(x => x.IsSelected);
        var procCount = listCreateClaimItemsToolBarInsSelectedItems.Count(x => x.ProcNumLab == 0); //Not a lab
        if (CultureInfo.CurrentCulture.Name.EndsWith("CA") && procCount > 7)
        {
            ...
            var procSelectedCount = 0;
            for (var i = 0; i < listCreateClaimItemsToolBarInsSelectedItems.Count; i++)
            {
                listCreateClaimItemsToolBarInsSelectedItems[i].IsSelected = procSelectedCount < 7;
                if (listCreateClaimItemsToolBarInsSelectedItems[i].ProcNumLab == 0)
                {
                    //Not a lab
                    procSelectedCount++;
                }
            ...
            var message = Lan.g("ContrAccount", "Only the first 7 procedures will be selected.  "
                                                + "You will need to create another claim for the remaining procedures.");
            LogClaimError(createClaimDataWrapper, message, isVerbose);
        }
    }
    ...
    public static Claim CreateClaim(Claim claim, string claimType, bool isVerbose, CreateClaimDataWrapper createClaimDataWrapper, string msgBoxHeader = "")
    {
        ...
        switch (claimType)
        {
            ...
            case "Tertiary":
                insSub = InsSubs.GetSub(PatPlans.GetInsSubNum(createClaimDataWrapper.CreateClaimData_.ListPatPlans
                    , PatPlans.GetOrdinal(PriSecMed.Tertiary, createClaimDataWrapper.CreateClaimData_.ListPatPlans, createClaimDataWrapper.CreateClaimData_.ListInsPlans
                        , createClaimDataWrapper.CreateClaimData_.ListInsSubs)), createClaimDataWrapper.CreateClaimData_.ListInsSubs);
                insPlan = InsPlans.GetPlan(insSub.PlanNum, createClaimDataWrapper.CreateClaimData_.ListInsPlans);
                claimTypeDesc = "tertiary";
                break;
            case "Med":
                ...
                break;
            case "Other":
                using (var formClaimCreate = new FormClaimCreate(patient.PatNum))
                {
        ...
        var listSelectedProcNums = createClaimDataWrapper.ListCreateClaimItems
            .Where(x => x.IsSelected)
            .Select(x => x.ProcNum).ToList();
        var listProceduresSelected = createClaimDataWrapper.CreateClaimData_.ListProcs.FindAll(x => listSelectedProcNums.Contains(x.ProcNum));
        var listProceduresBillIns = listProceduresSelected.FindAll(x => !Procedures.NoBillIns(x, createClaimDataWrapper.CreateClaimData_.ListClaimProcs, insPlan.PlanNum));
        var listProceduresNoBillIns = listProceduresSelected.FindAll(x => !listProceduresBillIns.Select(y => y.ProcNum).Contains(x.ProcNum));
        ...
        {
            var sbMsg = new StringBuilder($"The following procedures were marked as NoBillIns. They will be excluded from the {claimTypeDesc} claim.\r\n");
            for (var i = 0; i < listProceduresNoBillIns.Count; i++)
            {
                var procedureCode = ProcedureCodes.GetProcCode(listProceduresNoBillIns[i].CodeNum);
    ...
    /// </summary>
    public static void PromptForPrimaryOrSecondaryClaim(List<ClaimProc> listClaimProcsForClaim, bool isPromptForSecondaryClaim = true)
    {
        var listClaimsPromptingFor = Claims.GetPrimaryOrSecondaryClaimsNotReceived(listClaimProcsForClaim, isPromptForSecondaryClaim);
        if (listClaimsPromptingFor.Count == 0)
        {
            return; //No primary/secondary claims for the procedures attached to the medical/primary.
        }

        var claimType = "";
        PrefName prefName;
        if (isPromptForSecondaryClaim)
        {
            claimType = "secondary";
            prefName = PrefName.ClaimPrimaryRecievedForceSecondaryStatus;
        }
        else
        {
            //if not prompting for secondary, must be for primary
            claimType = "primary";
            prefName = PrefName.ClaimMedReceivedForcePrimaryStatus;
        }

        var msg = Lan.g("ContrAccount", "There is at least one unsent " + claimType + " claim for the received procedures.");
        msg += "\r\n";
        msg += Lan.g("ContrAccount", "Would you like to:");
        var strChangeStatus = Lan.g("ContrAccount", "Change the claim status to 'Waiting to send'");
        var strSendClaims = Lan.g("ContrAccount", "Send " + claimType + " claim(s) now");
        var strPrintClaims = Lan.g("ContrAccount", "Print " + claimType + " claim(s) now");
        var strDoNothing = Lan.g("ContrAccount", "Do nothing");
        ...
        if (inputBox.RadioButtonResult == strDoNothing)
        {
            return;
        }
        ...
        for (var i = 0; i < listClaimsPromptingFor.Count; i++)
        {
            listClaimsPromptingFor[i].ClaimStatus = "W";
            if (!isPromptForSecondaryClaim || !PrefC.GetBool(PrefName.ClaimPrimaryReceivedRecalcSecondary))
            {
                Claims.Update(listClaimsPromptingFor[i]);
            }
        }

        if (isPromptForSecondaryClaim && PrefC.GetBool(PrefName.ClaimPrimaryReceivedRecalcSecondary))
        {
            Claims.CalculateAndUpdateSecondaries(listClaimsPromptingFor);
        }

        if (inputBox.RadioButtonResult == strSendClaims)
        {
            //Most likely all of the procedures on the medical/primary claim will have all of the procedures on no more than 1 primary/secondary claim. Especially since most of time the 
            //claim will be created automatically (at least in the primary=>secondary scenario). The only time the secondary claims don't get created automatically is when the patient
            //doesn't have a primary/secondary claim at the time the primary claim gets created. Even if the user created the claim manually, the chances that the procedures on the
            //medical/primary have more than one claim are low.
            var listQueues = Claims.GetQueueList(listClaimsPromptingFor.Select(x => x.ClaimNum).ToList(), 0, 0);
            var listClaimSendQueueItemsSent = SendClaimSendQueueItems(listQueues.ToList(), 0); //Use clearinghouseNum of 0 to indicate automatic calculation of clearinghouse
            for (var i = 0; i < listClaimSendQueueItemsSent.Count; i++)
            {
        ...
        if (inputBox.RadioButtonResult != strPrintClaims)
        {
            return;
        }

        var doUsePrinterSettingsForAll = false;
        if (listClaimsPromptingFor.Count > 1)
        {
            var stringSameSettings = Lan.g("FormClaimPrint", "Use the same printer settings for all ");
            var stringClaims = Lan.g("FormClaimPrint", " claims?");
            var stringMessage = stringSameSettings + listClaimsPromptingFor.Count + stringClaims;
            doUsePrinterSettingsForAll = MsgBox.Show(MsgBoxButtons.YesNo, stringMessage);
        }

        for (var i = 0; i < listClaimsPromptingFor.Count; i++)
        {
            var claim = listClaimsPromptingFor[i];
            ...
            var stringAuditDescription = Lan.g("FormClaimPrint", "Claim from ") + claim.DateService.ToShortDateString();
            var doUseLastPrinterSettingsIfAvailable = doUsePrinterSettingsForAll && i > 0;
            ...
            //This call may receive the claim, but we don't want to prompt for primary/secondary claims because we are already in the process of doing that.
    ...
    {
        var listClaimSendQueueItemsRetVal = new List<ClaimSendQueueItem>(); //a list of queue items to send
        ...
        {
            //Clinics is in use
            var clinicNum0 = Claims.GetClaim(listClaimSendQueueItems[0].ClaimNum).ClinicNum;
            for (var i = 1; i < listClaimSendQueueItems.Count; i++)
            {
                var clinicNum = Claims.GetClaim(listClaimSendQueueItems[i].ClaimNum).ClinicNum;
        ...
        var clearinghouseNum0 = listClaimSendQueueItems[0].ClearinghouseNum;
        var claimMedType0 = Claims.GetClaim(listClaimSendQueueItems[0].ClaimNum).MedType;
        for (var i = 0; i < listClaimSendQueueItems.Count; i++)
        {
            //we start with 0 so that we can check medtype match on the first claim
            var clearinghouseNumI = listClaimSendQueueItems[i].ClearinghouseNum;
            ...
            var claimMedTypeI = Claims.GetClaim(listClaimSendQueueItems[i].ClaimNum).MedType;
            ...
            var clearinghouse = Clearinghouses.GetClearinghouse(clearinghouseNumI);
            ...
            {
                var msgText = Lan.g("ContrAccount", "There are ICD-9 codes attached to a procedure.  Would you like to send the claim without the ICD-9 codes? ");
        ...
        for (var i = 0; i < listClaimSendQueueItems.Count; i++)
        {
            var claimSendQueueitem = listClaimSendQueueItems[i].Copy();
        ...
        var claim = Claims.GetClaim(listClaimSendQueueItems[0].ClaimNum);
        ...
        var clearinghouseHq = ClearinghouseL.GetClearinghouseHq(listClaimSendQueueItemsRetVal[0].ClearinghouseNum);
        var clearinghouseClinic = Clearinghouses.OverrideFields(clearinghouseHq, claimClinicNum);
        var medType = claim.MedType;
        ...
        , long claimPlanNum, List<InsPlan> listInsPlans, string claimNote, string claimUniformBillType, ClaimCorrectionType claimCorrectionType)
    {
        ...
        {
            var insPlan = InsPlans.GetPlan(claimPlanNum, listInsPlans); //Does a query if listInsPlans is null or if claimPlanNum is not in list.
    ...
    {
        var claimProcCreditsGreaterThanProcFee = PrefC.GetEnum<ClaimProcCreditsGreaterThanProcFee>(PrefName.ClaimProcAllowCreditsGreaterThanProcFee);
        if (claimProcCreditsGreaterThanProcFee == ClaimProcCreditsGreaterThanProcFee.Allow)
        {
            return false;
        }

        var listProcDescripts = Claims.GetAllCreditsGreaterThanProcFees(listClaimProcsHypothetical);
        ...
        {
            using var msgBoxCopyPaste = new MsgBoxCopyPaste(Lan.g("FormClaimPayTotal", "Remaining amount is negative for the following procedures") + ":\r\n"
    ...
    public static bool AreWriteOffsGreaterThanProcFees(List<ClaimProc> listClaimProcsHypothetical)
    {
        ...
        var listProcDescripts = Claims.GetWriteOffsGreaterThanProcFees(listClaimProcsHypothetical);
        if (!listProcDescripts.IsNullOrEmpty())
        {
            using var msgBoxCopyPaste = new MsgBoxCopyPaste(Lan.g("FormClaimPayTotal", "Write-off amount is greater than the adjusted procedure fee for the following "
                                                                                       + "procedure(s)") + ":\r\n" + string.Join("\r\n", listProcDescripts) + "\r\n" + Lan.g("FormClaimPayTotal", "Not allowed to continue."));
    ...
    public static bool IsInitialPrimaryInsGreaterThanProcFees(List<ClaimProc> listClaimProcsHypothetical)
    {
        ...
        var listProcDescripts = Claims.GetInitialPrimaryInsGreaterThanProcFees(listClaimProcsHypothetical);
        if (!listProcDescripts.IsNullOrEmpty())
        {
            using var msgBoxCopyPaste = new MsgBoxCopyPaste(Lan.g("FormClaimPayTotal", "The sum of the initial primary insurance payment and write-off is greater "
