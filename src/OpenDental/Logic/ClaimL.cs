...
namespace OpenDental
{
    public class ClaimL
    {
        ...
        public static CreateClaimDataWrapper CreateClaimFromWrapper(bool isVerbose, CreateClaimDataWrapper createClaimDataWrapper, bool hasPrimaryClaim = false, bool hasSecondaryClaim = false, bool hasTertiaryClaim = false)
        {
            ...
            if (!hasPrimaryClaim)
            {
                ...
                if (isVerbose && claimLeadOrdinal.ClaimNum != 0)
                {
                    ...
                    using FormClaimEdit formClaimEdit = new FormClaimEdit(claimLeadOrdinal, createClaimDataWrapper.Patient_, createClaimDataWrapper.Family_);
            ...
            bool doCreateTertiaryClaim = !hasTertiaryClaim && PatPlans.GetOrdinal(PriSecMed.Tertiary, createClaimDataWrapper.CreateClaimData_.ListPatPlans,
            ...
            if (claimLeadOrdinal != null)
            {
                ...
                {
                    List<ClaimProc> listClaimProcs = createClaimDataWrapper.CreateClaimData_.ListClaimProcs.FindAll(x => x.ClaimNum == claimLeadOrdinal.ClaimNum);
        ...
        public static void AllocateUnearnedPayment(Patient patient, Family family, double unearnedAmt, Claim claim)
        {
            ...
            using FormProcSelect formProcSelect = new FormProcSelect(patient.PatNum, true, doShowAdjustments: true, doShowTreatmentPlanProcs: false);
        ...
        {
            List<Claim> listClaimsPromptingFor = Claims.GetPrimaryOrSecondaryClaimsNotReceived(listClaimProcsForClaim, isPromptForSecondaryClaim);
            ...
            string msg = Lan.g("ContrAccount", "There is at least one unsent " + claimType + " claim for the received procedures.");
            msg += "\r\n";
            ...
            string strSendClaims = Lan.g("ContrAccount", "Send " + claimType + " claim(s) now");
            string strPrintClaims = Lan.g("ContrAccount", "Print " + claimType + " claim(s) now");
            ...
            for (int i = 0; i < listClaimsPromptingFor.Count; i++)
            {
            ...
            if (inputBox.RadioButtonResult == strSendClaims)
            {
                ...
                ClaimSendQueueItem[] listQueues = Claims.GetQueueList(listClaimsPromptingFor.Select(x => x.ClaimNum).ToList(), 0, 0);
            ...
            if (listClaimsPromptingFor.Count > 1)
            {
                ...
                string stringMessage = stringSameSettings + listClaimsPromptingFor.Count.ToString() + stringClaims;
                doUsePrinterSettingsForAll = MsgBox.Show(MsgBoxButtons.YesNo, stringMessage);
            }

            for (int i = 0; i < listClaimsPromptingFor.Count; i++)
            {
                Claim claim = listClaimsPromptingFor[i];
