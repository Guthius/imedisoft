	...
	public class DentalTek {
		...
		public static void SendData(Program ProgramCur,Patient pat) {
			...
			if(dictPrefContactMethod.ContainsKey(pat.PreferContactMethod)) {
				...
				inputBoxParam.ListSelections=listPhoneNumbers;
				inputBoxParam.SelectedIndex=0;//This could be set to always display the inputbox form to always allow users to choose the number.
				...
				phoneNumber=new string(listPhoneNumbers[inputBox.SelectedIndex].Where(x => char.IsDigit(x)).ToArray());
