...
namespace OpenDental.Bridges{
	...
	public class TrophyEnhanced{
		...
		private static string AutomaticallyGetTrophyFolderNumbered(Patient pat,string trophyPath,bool doUseLastDigits) {
			...
			//In numbered mode, the folder numbering scheme is [trophyImageDir]\XX\PatNum.
			//If doUseDigits is true, the two digits are obtained by selecting the last 2 digits of the patients patnum, left padded with 0's to ensure the Patnum is long enough. Otherwise the two digits XX are obtained by retrieving the 5th and 6th (indexes 4 and 5) digits of the patient's PatNum, left padded with 0's to ensure the PatNum is at least 6 digits long.  Examples: PatNum=103, left pad to 000103, substring to 03, patient's folder location is [trophyDirectory]\03\103.  PatNum=1003457, no need to left pad, substring to 45, pat folder is [trophyDirectory]\45\1003457.
			string patnumFolderString=pat.PatNum.ToString().PadLeft(6,'0').Substring(4,2);
			if(doUseLastDigits) {
				string strPatNumWithZeros=pat.PatNum.ToString().PadLeft(2,'0');
				int startIndex=strPatNumWithZeros.Length-2;
				patnumFolderString=strPatNumWithZeros.Substring(startIndex,2);
			}
			string retVal=ODFileUtils.CombinePaths(patnumFolderString,pat.PatNum.ToString());//this is our default return value
		...
		private static string AutomaticallyGetTrophyFolder(Patient pat,string storagePath) {
			...
			if(retVal=="") {//perfect match not found
				if(listMatchesName.Count==1) {//exactly one name matched even though birthdays did not
					...
					FormLauncher formLauncher=new FormLauncher(EnumFormName.FormTrophyNamePick);
					formLauncher.SetField("ListMatches",listMatchesNot);
					formLauncher.ShowDialog();
					if(formLauncher.IsDialogCancel){
						return "";//triggers total exit
					}
					string pickedName=formLauncher.GetField<string>("PickedName");
