...
namespace OpenDental.UI
{
    public class ImageTools
    {
        ...
        public static Rectangle CalcExpandToFill(Size sizeMountItem, Size sizeImage, int degreesRotated)
        {
            ...
            var pointFUL = new PointF(-sizeImage.Width / 2f, -sizeImage.Height / 2f);
            var pointFLL = new PointF(-sizeImage.Width / 2f, sizeImage.Height / 2f);
            ...
            pointFUL = pointFs[0];
            pointFLL = pointFs[1];
            var widthResult = (float) Math.Round(Math.Min(Math.Abs(pointFUL.X) * 2, Math.Abs(pointFLL.X) * 2));
            var heightResult = (float) Math.Min(Math.Abs(pointFUL.Y) * 2, Math.Abs(pointFLL.Y) * 2);
            //example: widthResult=100,heightResult=50
            var ratioResultWH = widthResult / heightResult; //after the rotation//2
            var ratioMountItemWH = (float) sizeMountItem.Width / sizeMountItem.Height; //240/100=2.4
            if (ratioMountItemWH > ratioResultWH)
            {
                //mount item is proportionally wider than result
        ...
        {
            var sizeUnscaled = ImageHelper.CalcSizeFit(sizeImage.Width, sizeImage.Height, degreesRotated, sizeCanvas);
            //sizeUnscaled is correct proportions
            var scale = (float) sizeCanvas.Width / sizeUnscaled.Width;
            return scale;
        }
        ...
        public static double CalcZoomMount(Size sizeMount, Size sizeImage, Size sizeCrop, float degreesRotated)
        {
            ...
            var sizeFit = ImageHelper.CalcSizeFit(sizeImage.Width, sizeImage.Height, degreesRotated, sizeMount);
            var zoom = (float) sizeFit.Width / sizeCrop.Width * 100f;
            return zoom;
        }
        ...
        {
            var sizeFit = ImageHelper.CalcSizeFit(sizeImage.Width, sizeImage.Height, degreesRotated, sizeMount);
            var widthCrop = (float) (sizeFit.Width / zoom * 100f);
            var heightCrop = widthCrop / sizeMount.Width * sizeMount.Height;
