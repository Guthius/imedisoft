	...
	public partial class ComboClinic : UserControl{
		...
		public string HqDescription {
			...
			set {
				...
				List<ClinicDto> listClinicsSelected=comboBox.GetListSelected<ClinicDto>();
				FillClinics();//this wipes out the selected clinic(s), which can rarely be a problem.
				for(int i=0;i<comboBox.Items.Count;i++) {
					long clinicNum=((ClinicDto)comboBox.Items.GetObjectAt(i)).Id;
					if(listClinicsSelected.Exists(x=>x.Id==clinicNum)){
		...
		public long ClinicNumSelected {
			get {
				...
				if(IsMultiSelect){
					...
					return comboBox.GetListSelected<ClinicDto>()[0].Id;
				}
				//single select
				if(comboBox.SelectedIndex==-1) {
					long keyWhenMissing=comboBox.GetSelectedKey<ClinicDto>(x=>x.Id);
				...
				return comboBox.GetSelected<ClinicDto>().Id;//yes, this can return 0 for unassigned.
			}
			set {
				if(value==-1){
					throw new ApplicationException("Clinic num cannot be set to -1.  Instead, set IsNothingSelected.");
				}
				List<ClinicDto> listClinics=comboBox.Items.GetAll<ClinicDto>();
				int idx=listClinics.FindIndex(x=>x.Id==value);
				...
				comboBox.SetSelectedKey<ClinicDto>(value,x=>x.Id,funcOverrideText);
				SelectedIndexChanged?.Invoke(this,new EventArgs());
			}
		}
		...
		public List<long> ListClinicNumsSelected {
			get{
				...
				if(comboBox.SelectedIndices.Count==0) {
					long keyWhenMissing=comboBox.GetSelectedKey<ClinicDto>(x=>x.Id);//Guaranteed to return comboBox._keyWhenMissing
				...
				if(IncludeAll && IsAllSelected){
					if(IncludeHiddenInAll){
						...
						List<long> listClinicNums=listClinicsAll.Select(x=>x.Id).ToList();
						listClinicNums.Add(0);
						return listClinicNums;
					}
					//does not include All
					//does include the "unassigned/default/hq/none/all" clinicNum=0, if present
					return comboBox.Items.GetAll<ClinicDto>().Select(x=>x.Id).ToList();
				}
				return comboBox.GetListSelected<ClinicDto>().Select(x=>x.Id).ToList();
			}
			set{
				...
				for(int i=0;i<listClinics.Count;i++) {
					if(value.Contains(listClinics[i].Id)) {
						comboBox.SetSelected(i);
					}
				}
				SelectedIndexChanged?.Invoke(this,new EventArgs());
		...
		public List<ClinicDto> ListClinics {
			get{
				List<ClinicDto> listClinics=new List<ClinicDto>(comboBox.Items.GetAll<ClinicDto>());//copy
				return listClinics;
			}
		}
		...
		public string GetSelectedAbbr() {
			if(IsMultiSelect){//not the intended use, but we will support it
				...
				List<ClinicDto> listClinics=comboBox.GetListSelected<ClinicDto>();
				if(listClinics.Count==0){
					return "";
				}
				return listClinics[0].Abbr;
			}
			ClinicDto clinic=comboBox.GetSelected<ClinicDto>();
			if(clinic==null){
				return "";
			}
			return clinic.Abbr;
		...
		public ClinicDto GetSelectedClinic() {
			...
			return comboBox.GetSelected<ClinicDto>();
		}
		...
		private void FillClinics() {
			...
			List<ClinicDto> listClinicsForUser=null;
			if(IsTestModeNoDb){
				listClinicsForUser=new List<ClinicDto>();
				for(int i=1;i<40;i++){
			...
			Func<ClinicDto,string> funcItemToString=x=>{
				if(x.IsHidden){
