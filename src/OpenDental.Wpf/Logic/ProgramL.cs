...
namespace WpfControls {
	public class ProgramL {
		...
		public static void Execute(long programNum,Patient patient) {
			...
			else if(program.ProgName.In(ProgramName.EzDenti.ToString(),ProgramName.Ez3Di.ToString())) {
				EzDenti.SendData(program,patient);
				return;
			}
		...
		public static void LoadToolBar(ToolBar toolBar,EnumToolBar toolBarsAvail,EventHandler eventHandlerClick) {
			...
			for(int i=0;i<listToolButItems.Count;i++){
				...
				else if(program.ProgName==ProgramName.BetterDiagnostics.ToString()) {
					toolBarButton.SetToolTipText("Send currently displayed image(s) to BetterDiagnostics");
				}
