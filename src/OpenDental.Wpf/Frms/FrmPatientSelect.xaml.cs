...
///<summary>All this dialog does is set the patnum and it is up to the calling form to do an immediate refresh, or possibly just change the patnum back to what it was.</summary>
	public partial class FrmPatientSelect : FrmODBase{
		...
		///<summary>Allows prefilling of search fields.</summary>
		...
		///<summary>Should normally be false. Temporarily set this to true prior to directly calling RefreshFromDb() when you want to ignore the checkbox for "Refresh while typing". FillGrid() sets it back to false automatically.</summary>
		...
		public void FrmPatientSelect_Load(object sender,System.EventArgs e) {
			...
			butGetAll.Visible=PrefC.GetBool(PrefName.PatientSelectWindowShowGetAll);
		...
		///Currently only implements FName, LName, and HmPhone.</summary>
		public void PreFillSearchBoxes() {
			_isPreFillLoad=true; //Set to true to stop FillGrid from being called as a result of textChanged events
			if(PatientInitial.LName != "") {
				textLName.Text=PatientInitial.LName;
			}
			if(PatientInitial.FName != "") {
				textFName.Text=PatientInitial.FName;
			}
			if(PatientInitial.HmPhone != "") {
				textPhone.Text=PatientInitial.HmPhone;
			}
		}
		...
		private void checkRefresh_Click(object sender,EventArgs e) {
			if(checkRefresh.Checked==true) {
				...
				if(DoRefreshGrid()) {//only fill grid if PatientSelectSearchWithEmptyParams is true or there is something in at least one textbox
					...
					_filterControlsAndAction.PageNum=1;
					FillGrid(RefreshFromDb());
				}
			}
			else{
		...
		private object RefreshFromDb() {
					...
					if(checkShowArchived.Checked==true) {
						for(int i=0;i<comboClinic.ListClinics.Count;i++) {
							if(comboClinic.ListClinics[i].IsHidden) {
								Dispatcher.Invoke(()=>clinicNums+=","+comboClinic.ListClinics[i].Id.ToString());
							}
						}
					}
				}
			...
			PtTableSearchParams ptTableSearchParams=new PtTableSearchParams(_doLimitOnePage,LastName,FirstName,Phone,Address,
				...
				hasSpecialty,hasNextLastVisit,false,_filterControlsAndAction.PageNum);
		...
		private void FillGrid(object objectData) {
			...
			if(gridMain.ListGridRows.Count<40 || !_doLimitOnePage){
				butNext.IsEnabled=false;
			}
			butPrevious.IsEnabled=true;
			if(_filterControlsAndAction.PageNum==1){
				butPrevious.IsEnabled=false;
			}
			_doLimitOnePage=true;
		...
		private void gridMain_CellClick(object sender,GridClickEventArgs e) {
			...
			if(gridCell.ColorText==Color.FromRgb(0,0,255) //Blue color
				&& gridCell.Underline==true && Programs.GetCur(ProgramName.DentalTekSmartOfficePhone).Enabled) {
		...
		private void butAddPatient_Click(object sender,EventArgs e) {
			...
			if(textLName.Text=="" && textFName.Text=="" && textChartNumber.Text==""){
				MessageBox.Show(Lang.g(this,"Not allowed to add a new patient until you have done a search to see if that patient already exists. "
			...
			formLauncher.SetField("Patient", patient);
			formLauncher.SetField("Family", family);
			formLauncher.SetField("IsNew", true);
		...
		private void butAddMany_Click(object sender,EventArgs e) {
			...
			if(textLName.Text=="" && textFName.Text=="" && textChartNumber.Text==""){
				MessageBox.Show(Lang.g(this,"Not allowed to add a new patient until you have done a search to see if that patient already exists. Hint: just type a few letters into the Last Name box above.")); 
				return;
			}
			...
			IsNewPatientAdded=true;
			PatNumSelected=frmPatientAddAll.PatNumSelected;
			IsDialogOK=true;
		}

		private void butPrevious_Click(object sender,EventArgs e) {
			_filterControlsAndAction.PageNum--;
			_ignoreRefresh=true;
			FillGrid(RefreshFromDb());
		}
