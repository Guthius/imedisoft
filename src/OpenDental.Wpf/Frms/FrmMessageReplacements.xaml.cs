...
namespace OpenDental {
	...
	public partial class FrmMessageReplacements:FrmODBase {
		...
		///<summary>After the window closes, this is the selected text. Can be empty string if user didn't select anything.</summary>
		public string ReplacementTextSelected="";

		public FrmMessageReplacements(List<MessageReplaceType> listMessageReplaceTypes ,bool allowPHI=true) {
			InitializeComponent();
			_listMessageReplaceTypes=listMessageReplaceTypes;
			_allowPHI=allowPHI;
			gridMain.CellDoubleClick+=gridMain_CellDoubleClick;
			Load+=FrmMessageReplacements_Load;
			PreviewKeyDown+=FrmMessageReplacements_PreviewKeyDown;
		}

		private void FrmMessageReplacements_Load(object sender,EventArgs e) {
			Lang.F(this);
			if(!IsSelectionMode) {
				butOK.Visible=false;
			}
			FillGrid();
			bool hasGreyFields=gridMain.ListGridRows.Any(x=>x.ColorText==Colors.DarkSlateGray);
			if(!_allowPHI && hasGreyFields) {
				labelExplanation.Text+="  "+Lang.g(this,"Fields in grey are protected health information.");
			}
		}
		...
		private void FillGrid() {
			...
			GridColumn col=new GridColumn(Lang.g(this,"Type"),100);
			gridMain.Columns.Add(col);
			col=new GridColumn(Lang.g(this,"Replacement"),155);
			gridMain.Columns.Add(col);
			col=new GridColumn(Lang.g(this,"Description"),200){ IsWidthDynamic=true };
			gridMain.Columns.Add(col);
		...
		private void FrmMessageReplacements_PreviewKeyDown(object sender,KeyEventArgs e) {
		...
		private void PressOK(int index) {
			...
			ReplacementTextSelected=gridMain.ListGridRows[index].Cells[1].Text;
			IsDialogOK=true;
		}
