...
namespace OpenDental
{
    ...
    public class FormFrame : Form
    {
        ...
        public bool IsImageFloatDocked
        {
            ...
            get { return _isImageFloatDocked; }
            set
            {
                if (value)
                {
        ...
        public bool IsImageFloatSelected
        {
            //We use this instead of the built-in focus/active paradigm because we want the form to be selected even when it doesn't have focus.
            //Active means that it or one of its children has focus, so it's nearly identical to focus, but works better for forms where children have actual focus.
            get { return _isImageFloatSelected; }
            set
            {
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            Screen screen0 = Screen.FromControl(this); //automatically returns screen that contains largest portion of this form
...
//todo: close when this form closes.
        ...
        {
            Rectangle rectangleWorkingArea = Screen.FromHandle(this.Handle).WorkingArea;
        ...
        public void DisableAllExcept()
        {
            //params Control[] enabledControls) {
        ...
        private void PanelBorders_Paint(object sender, PaintEventArgs e)
        {
            ...
            if (ControlBox)
            {
                //there should always be a controlBox
                xPos -= wEdge;
            ...
            if (MaximizeBox)
            {
                ...
                if (WindowState == FormWindowState.Maximized)
                {
                    //show the double squares, each 8x8
            ...
            Screen screenPri = Screen.PrimaryScreen;
            float scaleRatio = 1;
            if (screen != screenPri)
            {
                ...
                if (scaleRatio != 1)
                {
                    //example, pri screen is 150%, so it makes the fonts on this screen too big
        ...
        private void PanelBorders_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            ...
            {
                //The up-down arrows are showing, in this case only maximize the height of the window
        ...
        private void PanelBorders_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            int widthBorder = 5;
            if (e.X < MaxInset() + widthBorder + 2 && e.Y < MaxInset() + widthBorder + 2)
            {
                //UL corner
                _enumMouseDownRegion = EnumMouseDownRegion.UL;
            }
            else if (e.X > Width - widthBorder - MaxInset() - 1 && e.Y < MaxInset() + widthBorder + 2)
            {
                //UR corner
                _enumMouseDownRegion = EnumMouseDownRegion.UR;
            }
            else if (e.X < MaxInset() + widthBorder + 2 && e.Y > Height - widthBorder - MaxInset() - 3)
            {
                //LL corner
                _enumMouseDownRegion = EnumMouseDownRegion.LL;
            }
            else if (e.X > Width - widthBorder - MaxInset() - 3 && e.Y > Height - widthBorder - MaxInset() - 3)
            {
                //LR corner
                _enumMouseDownRegion = EnumMouseDownRegion.LR;
            }
            else if (e.X < MaxInset() + widthBorder + 1)
            {
                //left
                _enumMouseDownRegion = EnumMouseDownRegion.Left;
            }
            else if (e.X > Width - widthBorder - MaxInset() - 1)
            {
                //right
                _enumMouseDownRegion = EnumMouseDownRegion.Right;
            }
            else if (e.Y > Height - widthBorder - MaxInset() - 1)
            {
                //bottom
        ...
        private void PanelBorders_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            MouseButtons mouseButtons = MouseButtons; //introducing variables for debugging
            ...
            if (MouseButtons == MouseButtons.Left)
            {
                ...
                if (_pointMouseScreenPrevious == MousePosition)
                {
            ...
            if (HasHelpButton && _regionButHelp != null)
            {
                ...
                {
                } //region could be disposed or other similar issues
            }
            ...
            _pointMouseScreenPrevious = MousePosition;
            Point pointDelta = new Point(MousePosition.X - _pointMouseDownScreen.X, MousePosition.Y - _pointMouseDownScreen.Y);
            if (IsImageFloatDocked)
            {
                if (Math.Abs(pointDelta.X) < 3 && Math.Abs(pointDelta.Y) < 3)
                {
                    //ignore small drags. This would be annoying if not docked.
                    return;
                }
        ...
        private void PanelBorders_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            Screen screen = Screen.FromHandle(this.Handle);
            Point pointMouse = MousePosition; //screen coordinates
            ...
            if (_isDraggingTitle)
            {
                ...
                else if (screen.Primary && MousePosition.X == screen.WorkingArea.Left)
                {
                    //snap to left
            ...
            if (_regionButHelp.IsVisible(e.Location) && HasHelpButton)
            {
                //IsVisible really means HitTest
                string formName = GetHelpOverride();
                //Name was already fixed to look like Form... instead of Frm...
                //This was done in in UIManager where it creates FormFrame.
                if (formName == "")
                {
                    //this is more common than the override
                    formName = Name;
                }
        ...
        protected override void WndProc(ref Message m)
        {
            ...
            if (m.Msg == WM_WINDOWPOSCHANGED)
            {
                ...
                FieldInfo restoredBoundsSpecified = typeof(Form).GetField("restoredWindowBoundsSpecified", BindingFlags.Instance | BindingFlags.NonPublic);
        ...
        {
            ComponentResourceManager resources = new ComponentResourceManager(typeof(FormFrame));
            this.SuspendLayout();
            // 
            // FormFrame
            // 
